-- @Author: wanghai
-- @Date:   2020-05-29 17:02:24
-- @Last Modified by:   wanghai
-- @Last Modified time: 2020-10-27 19:04:07


local QEArenaViewerNew = class("QEArenaViewerNew", function()
    return display.newScene("QEArenaViewerNew")
end)
...
function QEArenaViewerNew:ctor(options)
...
function QEArenaViewerNew:disableSlowMotion()
...
function QEArenaViewerNew:toggleSlowMotion()
...
function QEArenaViewerNew:cleanup()
    self:endBattle()
end

function QEArenaViewerNew:onReceiveData(message)
    if message == nil then
        return
    end
...
function QEArenaViewerNew:endBattle(isWin)
    if app.grid then
...
function QEArenaViewerNew:replayBattle()
...
function QEArenaViewerNew:onResetBattle(newData)
...
local function setHeroArtifact(member)
    ...
    if member.artLevel and member.artLevel ~= "" then
        ret.artifactLevel = tonumber(member.artLevel)
    end
...
function QEArenaViewerNew:getBaseActorInfo(member)
    local info = {}
    ...
    info.rankCode = "R0"
    info.godSkillGrade = member.godSkillGrade

    local database = QStaticDatabase:sharedDatabase()
    -- 有些英雄没有神技
    local godSkillConfig = database:getGodSkillByIdAndGrade(info.actorId, info.godSkillGrade)
    if godSkillConfig == nil then info.godSkillGrade = 0 end
    ...
    -- 体技
    if member.glyphs then
        self:getGlyphsInfo(info)
    end
    -- 外附魂骨
    info.spar = member.spar
    info.magicHerbs = member.magicHerbs
    ...
    if member.mid and member.mid ~= "" then
        ...
        if member.peijianId and member.peijianGrade
            and member.peijianQianghua then
            local peijianInfo = {}
            peijianInfo.zuoqiId = member.peijianId
            peijianInfo.grade = member.peijianGrade
            peijianInfo.enhanceLevel = member.peijianQianghua
            zuoqiInfo.wearZuoqiInfo = peijianInfo
        end
...
function QEArenaViewerNew:getGlyphsInfo(info)
    local glyphsList = {}
    local database = QStaticDatabase:sharedDatabase()
    local gradeConfig = database:getGradeByHeroId(info.actorId)
    for k, config in pairs(gradeConfig) do
        if config.glyph_id and config.grade_level <= info.grade then
            local glyphId = config.glyph_id
            local glyphLevel = 1
            local glyphConfigs = database:getGlyphSkillsBySkillID(glyphId)
            for k, value in pairs(glyphConfigs) do
                if value.glyph_level > glyphLevel then
                    glyphLevel = value.glyph_level
                end
            end
            table.insert(glyphsList, {glyphId = glyphId, level = glyphLevel})
        end
    end
    info.glyphs = glyphsList
end

function QEArenaViewerNew:onResetBattle2(newData)
    ...
    for _, member in ipairs(self._message.team1) do
        local info = self:getBaseActorInfo(member)
    ...
    config.userSoulSpirits = self._message.userSoulSpirits
    config.enemySoulSpirits = self._message.enemySoulSpirits
...
function QEArenaViewerNew:runConsole(config)
    config.isEditor = nil
    config.isReplay = nil
