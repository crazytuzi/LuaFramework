...
local QUIWidgetFightEndDetailClient = class("QUIWidgetFightEndDetailClient", QUIWidget)
local QUIWidgetHeroHead = import("..widgets.QUIWidgetHeroHead")

local NUMBERS = {"一","二","三"}
QUIWidgetFightEndDetailClient.EVENT_CLICK_HEAD = "EVENT_CLICK_HEAD"
QUIWidgetFightEndDetailClient.EVENT_CLICK_ONEREPLAY = "EVENT_CLICK_ONEREPLAY"


function QUIWidgetFightEndDetailClient:ctor(options)
	...
	QUIWidgetFightEndDetailClient.super.ctor(self, ccbFile, callBack, options)
...
function QUIWidgetFightEndDetailClient:ininGLLayer(glLayerIndex)
...
function QUIWidgetFightEndDetailClient:setInfo(info,reportIsShow)
    ...
    for i, fighter in pairs(heroGodarmList) do 
        ...
        local widgetHead = QUIWidgetHeroHead.new()
        widgetHead:setHero(tonumber(godarmInfo[1]))
        widgetHead:setTeam(i,false,false,true)
        widgetHead:setStar(tonumber(godarmInfo[2]))
        widgetHead:showSabc()
        widgetHead:setTeam(1)
        widgetHead:setScale(scale)
    ...
    local _force = 0
    if info.heroSilvesArenaForce and info.heroSilvesArenaForce ~= 0 then
        _force = info.heroSilvesArenaForce
    else
        _force = force
    end
    local num, unit = q.convertLargerNumber(_force or 0)
    self._ccbOwner.tf_force_1:setString(num..(unit or ""))
    ...
    local _force = 0
    if info.enemySilvesArenaForce and info.enemySilvesArenaForce ~= 0 then
        _force = info.enemySilvesArenaForce
    else
        _force = force
    end
    local num, unit = q.convertLargerNumber(_force or 0)
    self._ccbOwner.tf_force_2:setString(num..(unit or ""))
...
function QUIWidgetFightEndDetailClient:registerItemBoxPrompt( index, list )
...
function QUIWidgetFightEndDetailClient:showHeroInfo( x, y, heroHead )
    ...
    if unkonwType and unkonwType ~= 3 then
        ...
        self:dispatchEvent({name = QUIWidgetFightEndDetailClient.EVENT_CLICK_HEAD, actorId = actorId, isHero = true, index = self._info.index, isSoulSpirit = isSoulSpirit})
    end
end

function QUIWidgetFightEndDetailClient:showEnemyInfo( x, y, heroHead )
    local actorId = heroHead:getHeroActorID()
    local isSoulSpirit = heroHead:getIsSoulSpirit()
    local unkonwType = heroHead:getHeroType()
    if unkonwType and unkonwType ~= 3 then
        ...
        self:dispatchEvent({name = QUIWidgetFightEndDetailClient.EVENT_CLICK_HEAD, actorId = actorId, isHero = false, index = self._info.index, isSoulSpirit = isSoulSpirit})
    end
end

function QUIWidgetFightEndDetailClient:getContentSize()
	local size = self._ccbOwner.node_size:getContentSize()
	size.height = self._height+10
	return size
end

function QUIWidgetFightEndDetailClient:getInfo()
    return self._info
end

function QUIWidgetFightEndDetailClient:_onTriggerReplay(event)
    app.sound:playSound("common_cancel")
    self:dispatchEvent({name = QUIWidgetFightEndDetailClient.EVENT_CLICK_ONEREPLAY, info = self._info})
end

return QUIWidgetFightEndDetailClient
