...
local QBossHpViewMultilayer = class("QBossHpViewMultilayer", function()
...
local function createHpViewController(hp_per_layer, bar_create_func)
    ...
    function controller:_updateMainBar(index, to_percent)
        ...
        if main_bar == nil then
            main_bar = bar_create_func("main", index)
            _layerNode(index):addChild(main_bar, 0)
            main_bar_by_layer[index] = main_bar
        end
    ...
    function controller:update(dt, hp)
        ...
        local main_bar1 = main_bar_by_layer[1]
        if main_bar1 then
...
function QBossHpViewMultilayer:ctor()
...
function QBossHpViewMultilayer:setStar(star)
...
function QBossHpViewMultilayer:setName(name)
    self._owner.label_name:setString(name)
end

function QBossHpViewMultilayer:setLevel(level)
    self._owner.label_level:setString(tostring(level))
end

function QBossHpViewMultilayer:setBreakthrough(breakthrough)
...
function QBossHpViewMultilayer:setActor(actor)
...
function QBossHpViewMultilayer:setHPPerLayer(value)
...
function QBossHpViewMultilayer:getActor()
    return self._actor
end

function QBossHpViewMultilayer:_onFrame(dt)
    if self._actor then
...
function QBossHpViewMultilayer:_onHpChanged(event)
    self:updateHpBar()
    if self._actor then
...
function QBossHpViewMultilayer:updateHpBar(no_animation)
...
function QBossHpViewMultilayer:getMaxLayerCount()
    if self._maxLayerCount then
        return self._maxLayerCount
    end
end

function QBossHpViewMultilayer:getLayerCount()
    return self._barController:getLayerCount()
end

function QBossHpViewMultilayer:getHpPerLayer()
    return self._hpPerLayer
end

function QBossHpViewMultilayer:setIsEliteBoss(isEliteBoss)
