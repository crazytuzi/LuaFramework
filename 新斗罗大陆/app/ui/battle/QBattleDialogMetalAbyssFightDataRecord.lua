...
local QBattleDialogMetalAbyssFightDataRecord = class(".QBattleDialogMetalAbyssFightDataRecord", QBattleDialog)
...
QBattleDialogMetalAbyssFightDataRecord.TAB_DETAIL = "TAB_DETAIL"
QBattleDialogMetalAbyssFightDataRecord.TAB_DATA = "TAB_DATA"

function QBattleDialogMetalAbyssFightDataRecord:ctor(options)
	...
	QBattleDialogMetalAbyssFightDataRecord.super.ctor(self, ccbFile, {}, callBacks)
	options = options or {}

	self._closeCallback = options.callback
	self._ccbOwner.sp_vs:setVisible(false)
	self._selectTab = QBattleDialogMetalAbyssFightDataRecord.TAB_DETAIL
    self._detailInfo = {}
	self._dataRecord = {}
...
function QBattleDialogMetalAbyssFightDataRecord:initDetailData()
	...
	if #scoreList == 3 then
		local teamInfo3 = {index = 3, teamInfo = teams[3], log = logList[3]}
    ...
    if scoreList[3] ~= nil then
		...
		playerInfo.heroFighter = teams[3].hero.heroes
        playerInfo.heroSubFighter = teams[3].hero.supports
        playerInfo.heroSoulSpirit = teams[3].hero.soulSpirits
        playerInfo.enemyFighter = teams[3].enemy.heroes
        playerInfo.enemySubFighter =teams[1].enemy.supports
        playerInfo.enemySoulSpirit = teams[3].enemy.soulSpirits
        playerInfo.teamHeroSkillIndex = teams[3].hero.supportSkillHeroIndex or 0
    	playerInfo.teamHeroSkillIndex2 = teams[3].hero.supportSkillHeroIndex2 or 0
    	playerInfo.teamEnemySkillIndex = teams[3].enemy.supportSkillHeroIndex or 0
    	playerInfo.teamEnemySkillIndex2 = teams[3].enemy.supportSkillHeroIndex2 or 0
        playerInfo.heroGodarmList = teams[3].hero.godArmIdList
        playerInfo.enemyGodarmList = teams[3].enemy.godArmIdList  
        playerInfo.isMultiTeam =  (teams[3].hero.supportSkillHeroIndex2 and teams[3].hero.supportSkillHeroIndex2 ~= 0) 
        	or (teams[3].enemy.supportSkillHeroIndex2 and teams[3].enemy.supportSkillHeroIndex2 ~= 0)
        table.insert(self._detailInfo, playerInfo)
    end
end

-- 重置所有
function QBattleDialogMetalAbyssFightDataRecord:resetAll()
...
function QBattleDialogMetalAbyssFightDataRecord:selectTabs()
	self:resetAll()
	if self._selectTab == QBattleDialogMetalAbyssFightDataRecord.TAB_DETAIL then
		-- self._ccbOwner.btn_detail:setHighlighted(true)
		self._tabManager:selected(self._ccbOwner.btn_detail)
		self:showDetailRecord()
	elseif self._selectTab == QBattleDialogMetalAbyssFightDataRecord.TAB_DATA then
...
function QBattleDialogMetalAbyssFightDataRecord:showDetailRecord()
...
function QBattleDialogMetalAbyssFightDataRecord:showDataRecord()
...
function QBattleDialogMetalAbyssFightDataRecord:heroClickHandler(event)
...
function QBattleDialogMetalAbyssFightDataRecord:_backClickHandler()
	self:_onTriggerClose()
end

function QBattleDialogMetalAbyssFightDataRecord:_onTriggerData(event)
	self._ccbOwner.btn_data:setHighlighted(true)
	if self._selectTab == QBattleDialogMetalAbyssFightDataRecord.TAB_DATA then
		return
	end
	app.sound:playSound("common_cancel")

	self._selectTab = QBattleDialogMetalAbyssFightDataRecord.TAB_DATA
	self:selectTabs()
end

function QBattleDialogMetalAbyssFightDataRecord:_onTriggerDetail(event)
	self._ccbOwner.btn_detail:setHighlighted(true)
	if self._selectTab == QBattleDialogMetalAbyssFightDataRecord.TAB_DETAIL then
		return
	end
...
function QBattleDialogMetalAbyssFightDataRecord:_onTriggerClose(event)
...
return QBattleDialogMetalAbyssFightDataRecord