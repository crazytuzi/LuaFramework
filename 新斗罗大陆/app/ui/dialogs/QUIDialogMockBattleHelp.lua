...
local QUIDialogMockBattleHelp = class("QUIDialogMockBattleHelp", QUIDialogBaseHelp)
...
--初始化
function QUIDialogMockBattleHelp:ctor(options)
	QUIDialogMockBattleHelp.super.ctor(self,options)
    self:setShowRule(false)
end

function QUIDialogMockBattleHelp:initData()
	-- -- body
	local data = {}
	self._data = data
    if remote.mockbattle:getMockBattleSeasonTypeIsSingle() then
		table.insert(data, {oType = "describe", info = {helpType = "help_mock_battle"}})
	else
		table.insert(data, {oType = "describe", info = {helpType = "help_mock_battle2"}})
	end
...
function QUIDialogMockBattleHelp:initListView( ... )
	-- body
...
function QUIDialogMockBattleHelp:getRankNode()
...
function QUIDialogMockBattleHelp:showRule()
    app.sound:playSound("common_cancel")
end

return QUIDialogMockBattleHelp