...
local QUIDialogTavernBuyMore = class("QUIDialogTavernBuyMore", QUIDialog)
...
QUIDialogTavernBuyMore.ITEM_SELL_SCCESS = "ITEM_SELL_SCCESS"

function QUIDialogTavernBuyMore:ctor(options)
	...
	QUIDialogTavernBuyMore.super.ctor(self, ccbFile, callBacks, options)
	...
	if self._itemInfo.isHalf then
		self._ccbOwner.node_buy_num:removeAllChildren()
		local label = CCLabelTTF:create("首个半价在每日5点刷新", global.font_default, 20)
	    label:setColor(COLORS.k)
	    label:setPositionX(0)
	    label:setPositionY(-71)
		self._ccbOwner.node_buy_num:addChild(label)
		self._ccbOwner.node_buy_num:setVisible(true)
	end

	if self._itemInfo.resource_number_1 then
		self._ccbOwner.one_sell_money:setString(self._itemInfo.resource_number_1)
		self._ccbOwner.node_next_price:setVisible(true)
	else
...
function QUIDialogTavernBuyMore:viewDidAppear()
	QUIDialogTavernBuyMore.super.viewDidAppear(self)
	self.prompt = app:promptTips()
	self.prompt:addItemEventListener(self)
end

function QUIDialogTavernBuyMore:viewWillDisappear()
	QUIDialogTavernBuyMore.super.viewWillDisappear(self)
  	self.prompt:removeItemEventListener()
end

function QUIDialogTavernBuyMore:setDetailInfo()
...
function QUIDialogTavernBuyMore:setCurrencyInfo(index)
...
function QUIDialogTavernBuyMore:setItemInfo(index)
...
function QUIDialogTavernBuyMore:setNums()
	if self._itemInfo.isHalf then
		local price1 = self._currencyNum[1].value or 0
		self.needNum1 = price1 / 2 + price1 * (self.nums - 1)

		local price2 = self._currencyNum[2].value or 0
		self.needNum2 = price2 / 2 + price2 * (self.nums - 1)
	else
...
function  QUIDialogTavernBuyMore:_onSubTen(event)
...
function  QUIDialogTavernBuyMore:_onPlusTen(event)
	if tonumber(event) == CCControlEventTouchDown then
		self:_onDownHandler(10)
	else
		app.sound:playSound("common_increase")
...
function QUIDialogTavernBuyMore:_onUpHandler(num)
	self._isDown = false	
	self._isUp = true
	self:_subBuyNums(num)
end

function QUIDialogTavernBuyMore:_onDownHandler(num)
	self._isDown = true
	self._isUp = false
...
function QUIDialogTavernBuyMore:_subBuyNums(num)
...
function QUIDialogTavernBuyMore:viewAnimationOutHandler()
	self:removeSelfFromParent()
end

function QUIDialogTavernBuyMore:_onTriggerOK()
	app.sound:playSound("common_confirm")
	if self.nums == 0 then 
		app.tip:floatTip("购买数量不能为0")
		return
	end
	local bullCount = self.nums * self._itemInfo.itemCount
	local isTenBuy = self._itemInfo.itemCount == 10
	print("[QUIDialogTavernBuyMore:_onTriggerOK()] ", bullCount, remote.user.token)
	if bullCount > remote.user.token then
		QQuickWay:addQuickWay(QQuickWay.TOKEN_DROP_WAY, nil)
		return
	end
	app:getClient():luckyDrawBuyBull(bullCount, isTenBuy, function()
...
function QUIDialogTavernBuyMore:removeSelfFromParent()
	app:getNavigationManager():popViewController(app.middleLayer, QNavigationController.POP_TOP_CONTROLLER)
end

function QUIDialogTavernBuyMore:_backClickHandler()
	 self:_onTriggerClose()
end

function  QUIDialogTavernBuyMore:_onTriggerClose(e)
 	app.sound:playSound("common_close")
	self:playEffectOut()
end

return QUIDialogTavernBuyMore