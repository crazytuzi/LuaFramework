-- @Author: liaoxianbo
-- @Date:   2020-02-25 15:07:59
-- @Last Modified by:   xurui
-- @Last Modified time: 2020-04-09 11:18:28
local QUIDialog = import("..dialogs.QUIDialog")
local QUIDialogBuyGradePackageAwards = class("QUIDialogBuyGradePackageAwards", QUIDialog)
...
QUIDialogBuyGradePackageAwards.ITEM_SELL_SCCESS = "ITEM_SELL_SCCESS"

function QUIDialogBuyGradePackageAwards:ctor(options)
    ...
    QUIDialogBuyGradePackageAwards.super.ctor(self, ccbFile, callBacks, options)
	...
	self._ccbOwner.tf_content1:setVisible(false)
	self._ccbOwner.tf_content2:setVisible(false)
	self._ccbOwner.buy_count:setString("可购买"..(self._maxNum or 0).."次")
	self._ccbOwner.buy_count:setPositionX(0)
	self:setDetailInfo()
end

function QUIDialogBuyGradePackageAwards:viewDidAppear()
	QUIDialogBuyGradePackageAwards.super.viewDidAppear(self)
	self.prompt = app:promptTips()
	self.prompt:addItemEventListener(self)
end

function QUIDialogBuyGradePackageAwards:viewWillDisappear()
  	QUIDialogBuyGradePackageAwards.super.viewWillDisappear(self)

  	self.prompt:removeItemEventListener()
end

function QUIDialogBuyGradePackageAwards:setDetailInfo()
	...
	if self._itemInfo.itemType ~= "item" then
		...
		itemNum = remote.items:getItemsNumByID(self._itemId) or 0
	end
...
function QUIDialogBuyGradePackageAwards:setCurrencyInfo(index)
...
function QUIDialogBuyGradePackageAwards:setNums()
...
function QUIDialogBuyGradePackageAwards:_clickItemBox()
	if self._itemId == nil then return end
	app.tip:itemTip(ITEM_TYPE.GEMSTONE_PIECE, self._itemId)
end

function  QUIDialogBuyGradePackageAwards:_onSub(event)
...
function  QUIDialogBuyGradePackageAwards:_onSubTen(event)
	-- self._isMaxNum = false
...
function  QUIDialogBuyGradePackageAwards:_onPlus(event)
	app.sound:playSound("common_increase")
...
function  QUIDialogBuyGradePackageAwards:_onPlusTen(event)
	-- if self._isMaxNum then return end
	if tonumber(event) == CCControlEventTouchDown then
		self:_onDownHandler(10)
	else
...
function QUIDialogBuyGradePackageAwards:_onUpHandler(num)
	self._isDown = false	
	self._isUp = true
	self:_subBuyNums(num)
end

function QUIDialogBuyGradePackageAwards:_onDownHandler(num)
	self._isDown = true
	self._isUp = false
...
function QUIDialogBuyGradePackageAwards:_subBuyNums(num)
...
function QUIDialogBuyGradePackageAwards:_backClickHandler()
    self:_onTriggerClose()
end

function QUIDialogBuyGradePackageAwards:_onTriggerClose()
  	app.sound:playSound("common_close")
	self:playEffectOut()
end
function QUIDialogBuyGradePackageAwards:removeSelfFromParent()
	app:getNavigationManager():popViewController(app.middleLayer, QNavigationController.POP_TOP_CONTROLLER)
end

function  QUIDialogBuyGradePackageAwards:_onTriggerOK()
...
function QUIDialogBuyGradePackageAwards:viewAnimationOutHandler()
...
return QUIDialogBuyGradePackageAwards
