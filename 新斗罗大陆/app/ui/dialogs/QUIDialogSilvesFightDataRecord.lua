...
local QUIDialogSilvesFightDataRecord = class(".QUIDialogSilvesFightDataRecord", QUIDialog)
...
QUIDialogSilvesFightDataRecord.TAB_DETAIL = "TAB_DETAIL"
QUIDialogSilvesFightDataRecord.TAB_DATA = "TAB_DATA"

function QUIDialogSilvesFightDataRecord:ctor(options)
    ...
    QUIDialogSilvesFightDataRecord.super.ctor(self, ccbFile, callBacks, options)
	options = options or {}
	...
	self._selectTab = QUIDialogSilvesFightDataRecord.TAB_DETAIL
    self._detailInfo = {}
	self._dataRecord = {}
...
function QUIDialogSilvesFightDataRecord:initDetailData()
	...
	local logInfoList = self._statsDataList or {}
...
-- 重置所有
function QUIDialogSilvesFightDataRecord:resetAll()
...
function QUIDialogSilvesFightDataRecord:selectTabs()
	self:resetAll()
	if self._selectTab == QUIDialogSilvesFightDataRecord.TAB_DETAIL then
		-- self._ccbOwner.btn_detail:setHighlighted(true)
		self._tabManager:selected(self._ccbOwner.btn_detail)
		self:showDetailRecord()
	elseif self._selectTab == QUIDialogSilvesFightDataRecord.TAB_DATA then
...
function QUIDialogSilvesFightDataRecord:showDetailRecord()
...
function QUIDialogSilvesFightDataRecord:showDataRecord()
...
function QUIDialogSilvesFightDataRecord:heroClickHandler(event)
	if not event.info then
		return
	end
	local stat = event.info
	local isHero = event.isHero or false
    app:getNavigationManager():pushViewController(app.topLayer, {uiType = QUIViewController.TYPE_DIALOG, uiClass = "QUIDialogSkillData", 
    	options = {skillData = stat.skill, isHero = isHero}}, {isPopCurrentDialog = true})
end

function QUIDialogSilvesFightDataRecord:_backClickHandler()
	self:_onTriggerClose()
end

function QUIDialogSilvesFightDataRecord:_onTriggerData(event)
	self._ccbOwner.btn_data:setHighlighted(true)
	if self._selectTab == QUIDialogSilvesFightDataRecord.TAB_DATA then
		return
	end
	app.sound:playSound("common_small")

	self._selectTab = QUIDialogSilvesFightDataRecord.TAB_DATA
	self:selectTabs()
end

function QUIDialogSilvesFightDataRecord:_onTriggerDetail(event)
	self._ccbOwner.btn_detail:setHighlighted(true)
	if self._selectTab == QUIDialogSilvesFightDataRecord.TAB_DETAIL then
		return
	end
...
function QUIDialogSilvesFightDataRecord:_onTriggerClose(event)
...
return QUIDialogSilvesFightDataRecord