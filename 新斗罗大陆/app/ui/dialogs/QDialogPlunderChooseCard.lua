...
--
local QBattleDialog = import("..battle.QBattleDialog")
local QDialogPlunderChooseCard = class("QDialogPlunderChooseCard", QBattleDialog)

local QStaticDatabase = import("...controllers.QStaticDatabase")
local QUIWidgetItemsBox = import("..widgets.QUIWidgetItemsBox")

function QDialogPlunderChooseCard:ctor(options, owner)
	local ccbFile = "ccb/Dialog_plunder_achievementChard.ccbi"
	local callBacks = {
	...
	if owner == nil then
  	...
  	self._isDone = false
	QDialogPlunderChooseCard.super.ctor(self, ccbFile, owner, callBacks)
...
function QDialogPlunderChooseCard:setCardInfo()
...
function QDialogPlunderChooseCard:cardAnimation()
...
function QDialogPlunderChooseCard:_onTriggerClickCard1()
...
function QDialogPlunderChooseCard:_onTriggerClickCard2()
	if self._isDone == true then return end
    app.sound:playSound("common_menu")
...
function QDialogPlunderChooseCard:_onTriggerClickCard3()
	if self._isDone == true then return end
  	local b = self:clickCard(3)
  	if b then app.sound:playSound("common_menu") end
end

function QDialogPlunderChooseCard:clickCard(index)
	if self._isClicked then return end
	remote.plunder:plunderGetLootRandomAwardRequest(function()
...
function QDialogPlunderChooseCard:setItemInfo(index)
...
function QDialogPlunderChooseCard:setRestItemInfo(index)
	local rIndex = 2
	for i = 1, 3 do 
		if i ~= index then
...
function QDialogPlunderChooseCard:_onTriggerConfirm()
	if self._isDone == false then return end
    self:_onTriggerClose()
end

function QDialogPlunderChooseCard:_backClickHandler()
	if self._isDone == false then return end
    self:_onTriggerClose()
end

function QDialogPlunderChooseCard:_onTriggerClose()  
...
return QDialogPlunderChooseCard 