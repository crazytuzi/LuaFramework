...
local QUIDialogSoulSpiritBackpack = class("QUIDialogSoulSpiritBackpack", QUIDialog)
...
QUIDialogSoulSpiritBackpack.TAB_ALL = "TAB_ALL"
QUIDialogSoulSpiritBackpack.TAB_SOUL = "TAB_SOUL"
QUIDialogSoulSpiritBackpack.TAB_CONSUM = "TAB_CONSUM"

function QUIDialogSoulSpiritBackpack:ctor(options)
	local ccbFile = "ccb/Dialog_SoulSpirit_Packsack.ccbi"
	local callBacks = {
		{ccbCallbackName = "onTriggerTabAll", 				callback = handler(self, QUIDialogSoulSpiritBackpack._onTriggerTabAll)},
		{ccbCallbackName = "onTriggerTabSoul", 				callback = handler(self, QUIDialogSoulSpiritBackpack._onTriggerTabSoul)},
		{ccbCallbackName = "onTriggerTabConsum", 				callback = handler(self, QUIDialogSoulSpiritBackpack._onTriggerTabConsum)},
	}
	QUIDialogSoulSpiritBackpack.super.ctor(self,ccbFile,callBacks,options)
...
function QUIDialogSoulSpiritBackpack:startEnter()
	self:stopEnter()
    self._onFrameHandler = scheduler.scheduleGlobal(handler(self, self.onFrame), 0)
end

function QUIDialogSoulSpiritBackpack:stopEnter()
...
function QUIDialogSoulSpiritBackpack:viewDidAppear()
	QUIDialogSoulSpiritBackpack.super.viewDidAppear(self)
...
function QUIDialogSoulSpiritBackpack:viewWillDisappear()
  	QUIDialogSoulSpiritBackpack.super.viewWillDisappear(self)
...
function QUIDialogSoulSpiritBackpack:addArrowWidget(packs)
	local arr_node = QUIWidgetBagArrow.new({packs = packs})
	arr_node:setBagTag(3)
	self._ccbOwner.node_add_arr:addChild(arr_node)
end
-- 初始化中间的魂师选择框 swipe工能
function QUIDialogSoulSpiritBackpack:_initPageSwipe()
	...
	if options.tab ~= nil then
		...
		self:_selectTab(QUIDialogSoulSpiritBackpack.TAB_ALL)
	end
end

-- 处理各种touch event
function QUIDialogSoulSpiritBackpack:onEvent(event)
...
function QUIDialogSoulSpiritBackpack:onTouchEvent(event)
	if event == nil or event.name == nil then
        return
    end
...
function QUIDialogSoulSpiritBackpack:_removeAction()
	self:stopEnter()
...
function QUIDialogSoulSpiritBackpack:moveTo(posY, isAnimation, isCheck)
...
function QUIDialogSoulSpiritBackpack:_contentRunAction(posX,posY)
...
function QUIDialogSoulSpiritBackpack:_selectTab(tab, isRefresh)
	if tab ~= self.tab or isRefresh == true then
		...
		self:checkRedTips()
		options.tab = tab
		if tab == QUIDialogSoulSpiritBackpack.TAB_ALL then
			self._tabManager:selected(self._ccbOwner.node_tab_all)
			self._ccbOwner.tab_tips1:setVisible(false)
			self._items = remote.items:getItemsByCategory(ITEM_CONFIG_CATEGORY.SOULSPIRIT_PIECE, ITEM_CONFIG_CATEGORY.SOULSPIRIT_CONSUM, ITEM_CONFIG_CATEGORY.SOULSPIRIT_BOX)
		elseif tab == QUIDialogSoulSpiritBackpack.TAB_SOUL then
			self._tabManager:selected(self._ccbOwner.node_tab_soul)
			self._ccbOwner.tab_tips3:setVisible(false)
			self._items = remote.items:getItemsByCategory(ITEM_CONFIG_CATEGORY.SOULSPIRIT_PIECE)
		elseif tab == QUIDialogSoulSpiritBackpack.TAB_CONSUM then
			self._tabManager:selected(self._ccbOwner.node_tab_consum)
			self._ccbOwner.tab_tips2:setVisible(false)
			self._items = remote.items:getItemsByCategory(ITEM_CONFIG_CATEGORY.SOULSPIRIT_CONSUM, ITEM_CONFIG_CATEGORY.SOULSPIRIT_BOX)
		end
...
function QUIDialogSoulSpiritBackpack:sortItems(a, b)
	-- 过期
...
function QUIDialogSoulSpiritBackpack:rollToSelectItem(itemId)
...
function QUIDialogSoulSpiritBackpack:_initPage()
	--释放现有的BOX
...
function QUIDialogSoulSpiritBackpack:onFrame()
	local contentY = self._pageContent:getPositionY()
	for index,value in pairs(self._virtualBox) do
...
--[[获取BOX从滑动容器中]]
function QUIDialogSoulSpiritBackpack:getBox()
	local box = nil
...
--[[移除BOX从滑动容器中]]
function QUIDialogSoulSpiritBackpack:setBox(box)
	if box ~= nil then
...
--[[释放BOX到cache中]]
function QUIDialogSoulSpiritBackpack:releaseBox()
	if self._boxCache ~= nil then
		self._boxCache = {}
	end
...
function QUIDialogSoulSpiritBackpack:itemClickHandler(event)
...
function QUIDialogSoulSpiritBackpack:itemClicked(index, itemID)
...
function QUIDialogSoulSpiritBackpack:itemSelected(itemId)
	for index,value in pairs(self._virtualBox) do
...
function QUIDialogSoulSpiritBackpack:checkRedTips()
	self._ccbOwner.tab_tips1:setVisible(remote.items:checkItemRedTipsByCategory(ITEM_CONFIG_CATEGORY.SOULSPIRIT_PIECE, ITEM_CONFIG_CATEGORY.SOULSPIRIT_CONSUM))
	self._ccbOwner.tab_tips2:setVisible(remote.items:checkItemRedTipsByCategory(ITEM_CONFIG_CATEGORY.SOULSPIRIT_CONSUM))
	self._ccbOwner.tab_tips3:setVisible(remote.items:checkItemRedTipsByCategory(ITEM_CONFIG_CATEGORY.SOULSPIRIT_PIECE))
end

-- Tab 全部
function QUIDialogSoulSpiritBackpack:_onTriggerTabAll(tag, menuItem)
	if self.tab ~= QUIDialogSoulSpiritBackpack.TAB_ALL then
...
-- Tab 全部
function QUIDialogSoulSpiritBackpack:_onTriggerTabSoul(tag, menuItem)
	if self.tab ~= QUIDialogSoulSpiritBackpack.TAB_SOUL then
		app.sound:playSound("common_switch")
		self:hidePanel()
		self:_selectTab(QUIDialogSoulSpiritBackpack.TAB_SOUL)
	end
end
-- Tab 全部
function QUIDialogSoulSpiritBackpack:_onTriggerTabConsum(tag, menuItem)
	if self.tab ~= QUIDialogSoulSpiritBackpack.TAB_CONSUM then
		app.sound:playSound("common_switch")
		self:hidePanel()
		self:_selectTab(QUIDialogSoulSpiritBackpack.TAB_CONSUM)
	end
end

function QUIDialogSoulSpiritBackpack:hidePanel()
	self._selectPosition = 0
	if self._infoPanel ~= nil then
...
function QUIDialogSoulSpiritBackpack:onTriggerBackHandler(tag)
	self:_onTriggerBack()
end

function QUIDialogSoulSpiritBackpack:onTriggerHomeHandler(tag)
	self:_onTriggerHome()
end

-- 对话框退出
function QUIDialogSoulSpiritBackpack:_onTriggerBack(tag, menuItem)
	app:getNavigationManager():popViewController(app.mainUILayer, QNavigationController.POP_TOP_CONTROLLER)
end

-- 对话框退出
function QUIDialogSoulSpiritBackpack:_onTriggerHome(tag, menuItem)
	app:getNavigationManager():popViewController(app.mainUILayer, QNavigationController.POP_TO_CURRENT_PAGE)
end

return QUIDialogSoulSpiritBackpack