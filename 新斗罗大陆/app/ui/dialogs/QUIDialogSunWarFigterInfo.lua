...
local QUIDialogSunWarFigterInfo = class("QUIDialogSunWarFigterInfo", QUIDialog)
...
function QUIDialogSunWarFigterInfo:ctor(options)
 	local ccbFile = "ccb/Dialog_SunWarPrompt.ccbi"
    local callBacks = {}
    QUIDialogSunWarFigterInfo.super.ctor(self, ccbFile, callBacks, options)
    ...
    if #(self._info.heros or {}) + #(self._info.subheros or {}) + #(self._info.sub2heros or {}) + #(self._info.sub3heros or {}) <= 4 then
...
function QUIDialogSunWarFigterInfo:_getIcon( type, id, count )
    local node = nil
...
function QUIDialogSunWarFigterInfo:_showHpMp( heroHead, actorId, isSubhero, isAllDead, initMp )
...
function QUIDialogSunWarFigterInfo:_backClickHandler()
    self:_onTriggerClose()
end

-- 关闭对话框
function QUIDialogSunWarFigterInfo:_onTriggerClose()
    self:playEffectOut()
end

function QUIDialogSunWarFigterInfo:viewAnimationOutHandler()
    app:getNavigationManager():popViewController(app.middleLayer, QNavigationController.POP_TOP_CONTROLLER)
end

return QUIDialogSunWarFigterInfo