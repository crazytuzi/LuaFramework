...
local QTutorialPhase01SunWar = class("QTutorialPhase01SunWar", QTutorialPhase)
...
function QTutorialPhase01SunWar:start()
 ...
 --   	if app.tip.UNLOCK_TIP_ISTRUE == false then
	-- 	app.tip:showUnlockTips("UNLOCK_SUNWELL")
	-- else
	-- 	app.tip:addUnlockTips("UNLOCK_SUNWELL")
	-- end
...
--步骤管理
function QTutorialPhase01SunWar:stepManager()
...
--引导开始
function QTutorialPhase01SunWar:_guideStart()
	self:clearSchedule()
    self._tutorialInfo, self._sound = app.tutorial:splitTutorialWord("5401")
    self._distance = "left"
    self:createDialogue()
end

function QTutorialPhase01SunWar:_guideEnd()
    self:clearDialgue()
	self:finished()
end

--如果出错则直接跳掉引导过程
function QTutorialPhase01SunWar:_jumpToEnd()
	app.tutorial._runingStage:jumpFinished()
	self:finished()
end

--移动到指定位置
function QTutorialPhase01SunWar:_nodeRunAction(posX,posY)
	self._isMove = true
...
function QTutorialPhase01SunWar:createDialogue()
...
function QTutorialPhase01SunWar:_onTouch(event)
...
function QTutorialPhase01SunWar:clearSchedule()
...
function QTutorialPhase01SunWar:clearDialgue()
	if self._dialogueRight ~= nil then
		self._dialogueRight:removeFromParent()
		self._dialogueRight = nil
	end
