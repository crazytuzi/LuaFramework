...
local QTutorialPhase01EliteStarBox = class("QTutorialPhase01EliteStarBox", QTutorialPhase)
...
function QTutorialPhase01EliteStarBox:start()
...
--步骤管理
function QTutorialPhase01EliteStarBox:stepManager()
...
--引导开始
function QTutorialPhase01EliteStarBox:_guideStart()
	-- self:clearSchedule()
...
--引导玩家点击扩展标签
function QTutorialPhase01EliteStarBox:_guideClickScaling()
	--  self:clearSchedule()
    self:clearDialgue()
	local page = app:getNavigationManager():getController(app.mainUILayer):getTopPage()
...
function QTutorialPhase01EliteStarBox:_openScaling()
...
--引导玩家点击魂师总览按钮
function QTutorialPhase01EliteStarBox:_guideClickHero()
	self:clearSchedule()
	local page = app:getNavigationManager():getController(app.mainUILayer):getTopPage()
...
function QTutorialPhase01EliteStarBox:_openHero()
	self._handTouch:removeFromParent()
	self._dialog = app:getNavigationManager():getController(app.mainUILayer):getTopPage()
	self._dialog:_onInstance()

    -- 数据埋点
    -- app:triggerBuriedPoint(78)

	self:_guideClickCopy()
end

--引导玩家点击第一个副本
function QTutorialPhase01EliteStarBox:_guideClickCopy()
	self:clearSchedule()
...
--打开关卡页面
function QTutorialPhase01EliteStarBox:_openCopy()
	self._handTouch:removeFromParent()
	self._dialog:selectMap(1)

    -- 数据埋点
    -- app:triggerBuriedPoint(79)

	self:_guideClickHeroFrame()
end

--引导玩家点击魂师头像
function QTutorialPhase01EliteStarBox:_guideClickHeroFrame()
	--  self:clearSchedule()
	self._dialog = app:getNavigationManager():getController(app.mainUILayer):getTopDialog()

	self._CP = self._dialog._chest._ccbOwner.node_copper_open:convertToWorldSpaceAR(ccp(0,0))
	self._size = self._dialog._chest._ccbOwner.node_copper_open:getContentSize()
...
function QTutorialPhase01EliteStarBox:_openHeroInfo()
	self._handTouch:removeFromParent()
	app:triggerBuriedPoint(21110)
	app:sendGameEvent(GAME_EVENTS.GAME_EVENT_NEW_ROLE_GUIDE)  --新手引导结束
	self._dialog._chest:_onTriggerBoxCopper()
  	QNotificationCenter.sharedNotificationCenter():addEventListener(QTutorialEvent.EVENT_ELITE_STAR_BOX_SUCCESS, handler(self, self._guideClickOK))
end

--引导玩家点击魂师头像
function QTutorialPhase01EliteStarBox:_guideClickOK()
  	QNotificationCenter.sharedNotificationCenter():removeEventListener(QTutorialEvent.EVENT_ELITE_STAR_BOX_SUCCESS, handler(self, self._guideClickOK))
	local stage = app.tutorial:getStage()
	stage.eliteStar = 1
	stage.forced = QTutorialDirector.FORCED_GUIDE_STOP
...
function QTutorialPhase01EliteStarBox:_receiveReward()
	 self:clearSchedule()
...
--如果出错则直接跳掉引导过程
function QTutorialPhase01EliteStarBox:_jumpToEnd()
	app.tutorial._runingStage:jumpFinished()
	self:finished()
end

--移动到指定位置
function QTutorialPhase01EliteStarBox:_nodeRunAction(posX,posY)
	self._isMove = true
...
function QTutorialPhase01EliteStarBox:createDialogue()
	...
	if self._dialogueRight == nil then
		self._dialogueRight = QUIWidgetTutorialDialogue.new({avatarKey = self._avatarKey, isLeftSide = self._isLeft, name = name, heroId = heroInfo.id, text = self._word, isSay = true, sayFun = function()
			self._CP = {x = 0, y = 0}
			self._size = {width = display.width*2, height = display.height*2}
		end})
...
function QTutorialPhase01EliteStarBox:_onTouch(event)
...
function QTutorialPhase01EliteStarBox:clearSchedule()
...
function QTutorialPhase01EliteStarBox:clearDialgue()
	if self._dialogueRight ~= nil then
		self._dialogueRight:removeFromParent()
		self._dialogueRight = nil
	end
...
return QTutorialPhase01EliteStarBox
