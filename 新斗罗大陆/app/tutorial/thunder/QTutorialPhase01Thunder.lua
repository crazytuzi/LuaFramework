...
local QTutorialPhase01Thunder = class("QTutorialPhase01Thunder", QTutorialPhase)
...
function QTutorialPhase01Thunder:start()
	self._stage:enableTouch(handler(self, self._onTouch))
	self._step = 0
    self._tutorialInfo = {}

	local page = app:getNavigationManager():getController(app.mainUILayer):getTopPage()
	if page and page.cleanBuildLayer then
 ...
 --   	if app.tip.UNLOCK_TIP_ISTRUE == false then
	-- 	app.tip:showUnlockTips("UNLOCK_THUNDER")
	-- else
	-- 	app.tip:addUnlockTips("UNLOCK_THUNDER")
	-- end
...
--步骤管理
function QTutorialPhase01Thunder:stepManager()
...
--引导开始
function QTutorialPhase01Thunder:_guideStart()
	self:clearSchedule()
    self._tutorialInfo, self._sound = app.tutorial:splitTutorialWord("5201")
    self._distance = "left"
    self:createDialogue()
end

function QTutorialPhase01Thunder:_guideEnd()
    self:clearDialgue()
	self:finished()
end

--如果出错则直接跳掉引导过程
function QTutorialPhase01Thunder:_jumpToEnd()
	app.tutorial._runingStage:jumpFinished()
	self:finished()
end

--移动到指定位置
function QTutorialPhase01Thunder:_nodeRunAction(posX,posY)
	self._isMove = true
...
function QTutorialPhase01Thunder:createDialogue()
...
function QTutorialPhase01Thunder:_onTouch(event)
...
function QTutorialPhase01Thunder:clearSchedule()
...
function QTutorialPhase01Thunder:clearDialgue()
	if self._dialogueRight ~= nil then
		self._dialogueRight:removeFromParent()
		self._dialogueRight = nil
	end
...
return QTutorialPhase01Thunder
