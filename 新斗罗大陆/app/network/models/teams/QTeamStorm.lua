...
local QTeamStorm = class("QTeamStorm", QTeamNormal)
local QStaticDatabase = import("....controllers.QStaticDatabase")

function QTeamStorm:ctor(options)
	QTeamStorm.super.ctor(self, options)
end

--设置战队的数据
function QTeamStorm:setTeamDataWithBattleFormation(battleFormation)
...
--根据战队序号获取解锁的魂师最大位置
function QTeamStorm:getHerosMaxCountByIndex(index)
	...
	elseif index == 2 then
		if teamNum == 1 then
			if app.unlock:checkLock("UNLOCK_STORM_HELP_7") then
				count = 4
			elseif app.unlock:checkLock("UNLOCK_STORM_HELP_5") then
				count = 3
			elseif app.unlock:checkLock("UNLOCK_STORM_HELP_3") then
				count = 2
			elseif app.unlock:checkLock("UNLOCK_STORM_HELP_1") then
				count = 1
			end
		else
			if app.unlock:checkLock("UNLOCK_STORM_HELP_8") then
				count = 4
			elseif app.unlock:checkLock("UNLOCK_STORM_HELP_6") then
				count = 3
			elseif app.unlock:checkLock("UNLOCK_STORM_HELP_4") then
				count = 2
			elseif app.unlock:checkLock("UNLOCK_STORM_HELP_2") then
				count = 1
			end
		end
	elseif index == 5 then
...
--获取战队的最大队伍数量
function QTeamStorm:getTeamMaxIndex()
	return 2
end

function QTeamStorm:getDefaultTeam()
...
function QTeamStorm:getHelpUnlockLevel(index)
	...
	local unlockLevel = app.unlock:getConfigByKey("UNLOCK_STORM_HELP_"..unlockNum).team_level
	return unlockLevel
end

--检查战队中的魂师是否存在，不存在就删除掉
function QTeamStorm:checkTeamHero()
	local maxIndex = self:getTeamMaxIndex()
	for i=1,maxIndex do
		local maxCount = self:getHerosMaxCountByIndex(i)
