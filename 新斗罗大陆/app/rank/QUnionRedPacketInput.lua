...
local QUnionRedPacketInput = class("QUnionRedPacketInput", QBaseRank)
...
function QUnionRedPacketInput:ctor(options)
	QUnionRedPacketInput.super.ctor(self, options)
end

function QUnionRedPacketInput:update(success, fail)
	app:getClient():top50RankRequest("RECEIVE_REDPACKET_NUM_TOP_50", remote.user.userId, function (data)
...
function QUnionRedPacketInput:getRankItem()
...
function QUnionRedPacketInput:renderItem(item, index)
	...
	if style ~= nil and info ~= nil then
		...
		style:setTFByIndex(3, "领取钻石：")
		style:setTFByIndex(4, (info.weekReceiveRedpacketNum or ""))
...
function QUnionRedPacketInput:registerClick(listView, index)
	listView:registerClickHandler(index,"self",function ()
		return true
	end, nil, handler(self, self.clickHandler))
end

function QUnionRedPacketInput:clickHandler( x, y, touchNodeNode, list)
...
function QUnionRedPacketInput:getSelfItem()
	...
	style:setSpSpriteFrameByIndex(1, spf, 0.5)
	-- style:setSpByIndex(1, false)
	style:setTFByIndex(4, (myInfo.weekReceiveRedpacketNum or ""))
