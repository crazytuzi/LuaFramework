-- @Author: liaoxianbo
-- @Date:   2019-12-17 19:29:51
-- @Last Modified by:   liaoxianbo
-- @Last Modified time: 2019-12-26 16:13:11

local QBaseRank = import(".QBaseRank")
local QCollegetTrainRank = class("QCollegetTrainRank", QBaseRank)
...
function QCollegetTrainRank:ctor(options)
	QCollegetTrainRank.super.ctor(self, options)
end

function QCollegetTrainRank:_unpdateChapterInfo(selectInfo)
	self._btninfo = selectInfo
end

-- 本服
function QCollegetTrainRank:update(success, fail)
	--TODO: add response list
	app:getClient():top50RankCollegeTrainRequest("COLLEGE_TRAIN_ENV_TOP_50", remote.user.userId,self._btninfo.chapterId, function (data)
...
function QCollegetTrainRank:getRankItem()
...
function QCollegetTrainRank:renderItem(item, index)
...
function QCollegetTrainRank:registerClick(listView, index)
	listView:registerClickHandler(index,"self",function ()
		return true
	end, nil, handler(self, self.clickHandler))
end

function QCollegetTrainRank:clickHandler( x, y, touchNodeNode, list )
...
function QCollegetTrainRank:getSelfItem()
