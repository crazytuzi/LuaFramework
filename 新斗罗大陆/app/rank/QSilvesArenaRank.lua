...
local QSilvesArenaRank = class("QSilvesArenaRank", QBaseRank)
...
function QSilvesArenaRank:ctor(options)
	QSilvesArenaRank.super.ctor(self, options)
end

function QSilvesArenaRank:needsUpdate( ... )
	return true
end

function QSilvesArenaRank:update(success, fail)
	-- 需要修改数据接口
	-- SILVES_ARENA_SEASON_TOP_50 BLACK_ROCK_REALTIME_TOP_50
	app:getClient():top50RankRequest("SILVES_ARENA_SEASON_TOP_50", remote.user.userId, function (data)
...
function QSilvesArenaRank:getRankItem()
...
function QSilvesArenaRank:renderItem(item, index)
	...
	if style ~= nil and info ~= nil then
		...
		style:setTFByIndex(4, "平均战力：")
		style:setTFByIndex(5, fightNum or "")
		style:autoLayout()
	end
end

function QSilvesArenaRank:getSelfItem()
...
function QSilvesArenaRank:registerClick(listView, index)
	listView:registerClickHandler(index,"self",function ()
		return true
	end, nil, handler(self, self.clickHandler))
end

function QSilvesArenaRank:clickHandler( x, y, touchNodeNode, list)
