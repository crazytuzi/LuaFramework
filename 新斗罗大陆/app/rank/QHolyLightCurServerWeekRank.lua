-- @Author: xurui
-- @Date:   2017-09-17 17:08:57
-- @Last Modified by:   zhouxiaoshu
-- @Last Modified time: 2019-10-16 18:45:33

local QBaseRank = import(".QBaseRank")
local QHolyLightCurServerWeekRank = class("QHolyLightCurServerWeekRank", QBaseRank)
...
function QHolyLightCurServerWeekRank:ctor(options)
	QHolyLightCurServerWeekRank.super.ctor(self, options)
end

function QHolyLightCurServerWeekRank:needsUpdate( ... )
	return true
end

function QHolyLightCurServerWeekRank:update(success, fail)
	app:getClient():top50RankRequest("HOLY_RIGHT_ENV_REALTIME_TOP_50", remote.user.userId, function (data)
...
function QHolyLightCurServerWeekRank:getRefreshHour()
	-- local config = QStaticDatabase:sharedDatabase():getConfiguration()
	local refreshTime =  24
	return refreshTime
end

function QHolyLightCurServerWeekRank:getRankItem()
...
function QHolyLightCurServerWeekRank:renderItem(item, index)
...
function QHolyLightCurServerWeekRank:getSelfItem()
...
return QHolyLightCurServerWeekRank
