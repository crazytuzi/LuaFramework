...
function ProviderBase:callback_(event)
    ...
    printInfo("cc.wowAd.punchbox CALLBACK, event %s", event)
    local evt = {provider = "wowAd.PunchBox"}
...
function ProviderBase:doCommand(args)
    if args.command == "banner" or args.command == "interstitial" or args.command == "moregame" then
        ...
        printError("cc.wowAd.punchbox.ProviderBase:doCommand() - invaild command:" .. args.command)
    end
end
