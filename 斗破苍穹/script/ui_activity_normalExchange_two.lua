...
local function layoutScrollView(_listData, _initItemFunc)
    cleanScrollView()
    utils.updateScrollView(UIActivityNormalExchangeTwo, ui_scrollView, ui_svItem, _listData, _initItemFunc, { space = 10 })
end

local function setScrollViewItem(_item, _data)
    if _data == nil then
        return cclog(Lang.ui_activity_normalExchange_two1)
    end
    ...
    if itemData then
        ...
        if DictActivity and DictActivity.string["4"] ~= "" and DictActivity.string["5"] ~= "" then
            ...
            text_time:setString(string.format(Lang.ui_activity_normalExchange_two2,
            _startTime[2], _startTime[3], _startTime[5], _startTime[6], _endTime[2], _endTime[3], _endTime[5], _endTime[6]))
        else
            text_time:setString(Lang.ui_activity_normalExchange_two3)
        end
        local thingItems = { }
        for i = 1, 4 do
        ...
        text_hint:setString(Lang.ui_activity_normalExchange_two4 .. itemData.countLimit - itemData.exchangeCount)
            ...
            if eventType == ccui.TouchEventType.ended then
                ...
                if maxConvertNum > 0 then
                    if convertNum <= 0 then
                        UIManager.showToast(Lang.ui_activity_normalExchange_two5)
                        return
                    end
                    if maxConvertNum > 1 then
                        UISellProp.setData( { id = itemData.id, thing = itemData.getItem, convertNum = convertNum }, UIActivityNormalExchangeTwo)
                        UIManager.pushScene("ui_sell_prop")
                    else
                        UIManager.showLoading()
                        netSendPackage( { header = StaticMsgRule.overflowExchangeTwo, msgdata = { int = { id = itemData.id, count = 1 } } }, function(_msgData)
                            UIManager.showToast(Lang.ui_activity_normalExchange_two6)
                            UIManager.flushWidget(UIActivityNormalExchangeTwo)
                        end )
                    end
                else
                    UIManager.showToast(Lang.ui_activity_normalExchange_two7)
                end
            end
        end )
    end
...
function UIActivityNormalExchangeTwo.init()
    local image_basemap = UIActivityNormalExchangeTwo.Widget:getChildByName("image_basemap")
    ui_scrollView = image_basemap:getChildByName("view_info")
    ui_svItem = ui_scrollView:getChildByName("image_base_gift"):clone()
end

function UIActivityNormalExchangeTwo.onActivity(_params)
    DictActivity = _params
end

function UIActivityNormalExchangeTwo.setup()
    cleanScrollView()
    ccui.Helper:seekNodeByName( UIActivityNormalExchangeTwo.Widget ,  "text_name" ):setString(Lang.ui_activity_normalExchange_two8)
    UIManager.showLoading()
    netSendPackage( { header = StaticMsgRule.intoOverflowExchangeTwo, msgdata = { } }, function(_msgData)
        local msg = _msgData.msgdata.string["1"] --这个是完整的 所有的
        if msg then
...
function UIActivityNormalExchangeTwo.free()
    DictActivity = nil
    cleanScrollView()
end
