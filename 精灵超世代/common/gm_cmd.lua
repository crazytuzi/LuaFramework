...
function GmCmd:add_gm_button(layer)
    if not self.is_init then
        ...
        -- 打开关闭gm命令按钮
        self.open_button = self:addButton(layer, "GM",display.getLeft(self.root_wnd) + 200, display.getTop(self.root_wnd) - 10, 1, 99, function(sender, event_type)
        ...
        GlobalKeybordEvent:getInstance():add(function() self:f2() end, cc.Handler.EVENT_KEYBOARD_RELEASED, cc.KeyCode["KEY_F2"], 0)
        GlobalKeybordEvent:getInstance():add(function() self:f3() end, cc.Handler.EVENT_KEYBOARD_RELEASED, cc.KeyCode["KEY_F3"], 0)
        GlobalKeybordEvent:getInstance():add(function() self:f7() end, cc.Handler.EVENT_KEYBOARD_RELEASED, cc.KeyCode["KEY_F7"], 0)
...
function GmCmd:reloadluafile()
    ...
    local requireList = {"game.guildskill.view.guildskill_main_window"}
...
function GmCmd:hot_fix()
    for key, v in pairs(package.loaded) do 
        if string.find(key, "game") ~= nil or string.find(key, "common") ~= nil then
