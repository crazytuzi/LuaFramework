...
function PlayerHead:showLeader(status, x, y)
	if status then
		if self.leader_icon == nil then
			local x = x or 80 
			local y = y or 84
    		local icon_res = PathTool.getTargetRes("arenateam/txt_arenateam", "txt_arenateam_1",false)
			self.leader_icon = createSprite(icon_res, x, y,self,cc.p(0.5,0.5), LOADTEXT_TYPE) 
		else
