...
function DebugUIHelper:initUI()
    ...
    y = y - dy
    label = cc.Label:createWithTTF("ui资源名称:", DEFAULT_FONT, fontsize)
    label:setTextColor(cc.c4b(0,0,0,255))
    label:setAnchorPoint(1, 0)
    self.container:addChild(label)
    label:setPosition(x-2, y)
    self.resName = ccui.EditBox:create(cc.size(150, 30), ccui.Scale9Sprite:create())
    self.resName:setAnchorPoint(0, 0)
    self.container:addChild(self.resName)
    self.resName:setPosition(x + 5, y-5)
    self.resName:setFontColor(cc.c3b(0,0,255))
...
function DebugUIHelper:getObjRes()
    local obj = self.obj
    local res = nil
end
