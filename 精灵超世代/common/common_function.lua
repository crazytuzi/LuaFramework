require("common.common_function_bt")
...
spSkeletonAnimationCreate = function(js_path, atlas_path, pixelformal)
    ...
    if string.find(js_path, ".skel") ~= nil then
        spine__ = sp.SkeletonAnimation:createWithBinaryFile(js_path, atlas_path, 1, pixelformal)
    else
        spine__ = sp.SkeletonAnimation:createWithJsonFile(js_path, atlas_path, 1, pixelformal)-- setTimeScale
    end
    return spine__
end
...
--==============================--
--desc:创建模型,这里主要是模型类的.H打头的backpack_tips
...
function createSpineByName(spine_name, action_name, root, zorder, scale, pixelformal)
    ...
    local p,q = string.find(spine_name, "koudai_") 
    if p then
        --宝可梦创建
        local configedScale = Config.PartnerScaleData.data_scale_const[spine_name] or 1
        local spine = createSpineByNameV36(spine_name, root, zorder, configedScale)
        
        --通用位移限制 TODO：读取配置
        -- local noLimitP = string.find(spine_name, "koudai_lioulu/hero_lioulu")
        -- noLimitP = noLimitP or string.find(spine_name, "koudai_lukaliou/hero_lukaliou")
        -- noLimitP = noLimitP or string.find(spine_name, "koudai_guladuo/hero_guladuo")
        -- noLimitP = noLimitP or string.find(spine_name, "koudai_qingtengshe/hero_qingtengshe")

        -- if not noLimitP then

        --     local pp = string.find(spine_name, "koudai_liekongzuo/hero_liekongzuo")
        --     pp = pp or string.find(spine_name, "koudai_maoqiu/hero_maoqiu")
        --     if pp then
        --         local isLimitted = 1
        --         local offsetA = 200
        --         local offsetB = 200
        --         local maxDelta = 400
        --         spine:setBoneLimitted(isLimitted) --0不限制，1限制，默认为0
        --         spine:setBoneLimitOffsets(offsetA, offsetB, maxDelta)
        --     else
        --         local isLimitted = 1
        --         local offsetA = 50
        --         local offsetB = 150
        ...
        --训练师创建
        local p1,q1 = string.find(spine_name, "roles/")
        if p1 then
            local dirPath = "res/spine/"
            return createSpineByNameV36(spine_name, root, zorder, scale, dirPath)
        end
    end
...
local DefaultPathFix = "res/spine/heros/"
function createSpineByNameV36(spine_name, root, zorder, scale, dirPath)
    local PathFix = dirPath or DefaultPathFix
    local skelPath = PathFix..spine_name..".skel"
    local atlasPath = PathFix..spine_name..".atlas"
    local spObj = spSkeletonAnimationCreate(skelPath, atlasPath)
    if not tolua.isnull(root) then
         root:addChild(spObj, zorder or 0)
    end
    if scale and type(scale) == "number" then
...
--@return 
--==============================--
function createEffectSpine( effectName, pos, arPos, loop, action, call_back, pixelformal, not_play_action ,action_name)
    if loop == nil then loop = true end
    effectName = effectName  or "E88888"
    local skel_path, atlas_path, png_path, spine_path, return_name = PathTool.getSpineByName(effectName,action_name)
...
--@name:
--@srv_id:
--@word_break：是否需要换行("服号\n名字")
--@return 
--==============================--
function transformNameByServ(name, srv_id, word_break)
    local tmpName = name
    ...
    if not RoleController:getInstance():isTheSameSvr(srv_id) then
        ...
        if vo then
            ...
            if listOr[2] and listMe[1] and listOr[1]~=listMe[1] then
                if word_break then
                    tmpName = string.format(TI18N("[异域]\n%s"), tmpName)
                else
                    tmpName = string.format(TI18N("[异域]%s"), tmpName)
                end
            elseif #listOr > 1 then
                if word_break then
                    tmpName = string.format("[S%s]\n%s", listOr[#listOr], tmpName)
                else
...
function check_min_ver(Fun, is_force)
    if (not is_force) and UPDATE_SKIP then return end
...
--废弃
function TI18N_deprecated(msg)
    local lanCode = cc.Application:getInstance():getCurrentLanguageCode()
    local localizedData = nil
    local fixedCode = firstToUpper(lanCode)
    local ldConfig = Config[fixedCode.."LocalizedstringoutputSrc"] or {}
    localizedData = ldConfig.localizedStringOutput_src or {}

    local ret = localizedData[msg] or msg
    return ret
end

--add by chenbin on 2021-12-08;老版本和新版本资源目录不一样，老版本的main执行的不是新多语言
if not Config["LocalizedstringoutputSrc"] then
    local lanCode = cc.Application:getInstance():getCurrentLanguageCode()
...
function TI18N(msg)
    local ldConfig = Config["LocalizedstringoutputSrc"] or {}
    local localizedData = ldConfig.localizedStringOutput_src or {}

    local ret = localizedData[msg] or msg
    return ret
end

--csb多语言由c++ 实现，见class LocalizedString
--[[
function LocalizedStringForCsb(msg)
    local lanCode = cc.Application:getInstance():getCurrentLanguageCode()
    local localizedData = nil
    local fixedCode = firstToUpper(lanCode)
    local ldConfig = Config[fixedCode.."LocalizedstringoutputCsd"] or {}
    localizedData = ldConfig.localizedStringOutput_csd or {}
...
--废弃此方式
function LocalizedConfigRequire_deprecated(file)
    SDKConfigRequire(file)
    --require(file)
    local lanCode = cc.Application:getInstance():getCurrentLanguageCode()
    --local lanCode = GetLanguageCode()
    local localizedLuafile = string.gsub(file, "config.auto_config.", string.format("config.auto_config_%s.%s_", lanCode, lanCode))
    local status,err = pcall(function()
        local filename = string.gsub(localizedLuafile, "%.", "/") .. ".lua"
        local hasFile = cc.FileUtils:getInstance():isFileExist(filename)
        if hasFile then
            require(localizedLuafile)
        end
    end)
    if not status then
        print("Error in LocalizedConfigRequire:", err)
    end
end

function LocalizedConfigRequire(file)
    local lanCode = cc.Application:getInstance():getCurrentLanguageCode()
    local localizedFile = string.gsub(file, "config.auto_config.", string.format("config.auto_config_%s.", lanCode))
    require(localizedFile)
end

--更具SDK require 文件
function SDKConfigRequire(file)
    if CHARGE_CONFIG_TYPE == nil then require(file) return end

    local SDKLuafile = file .. "_" .. CHARGE_CONFIG_TYPE
    local filename = string.gsub(SDKLuafile, "%.", "/") .. ".lua"
    local hasFile = cc.FileUtils:getInstance():isFileExist(filename)
    if hasFile then
        local tabel = require(SDKLuafile)
        return
    end
    require(file)
    --PLATFORM_NAME
...
function commonShowEmptyIcon(parent, bool, setting)
    ...
    if bool then
        ...
        if not parent.empty_con then
            ...
            local label_color = setting.label_color or Config.ColorData.data_new_color4[6]
            local line_color = setting.line_color or nil
            ...
            bg:setScale(scale)
            parent.empty_label = createLabel(font_size, label_color, line_color, size.width / 2, offset_y, '', parent.empty_con, 2, cc.p(0.5, 0))
        else
...
function commonCountDownTime(label, less_time, setting)
    ...
    local end_title = setting.end_title or "%s" --chenbin：原本以..连接的结尾，现在改为format
    ...
    local _setTimeFormatString = function(time)
        ...
        if label_type and label_type == CommonAlert.type.rich and time_color then
            if time > 0 then
                local timeStr = string_format(end_title, TimeTool.GetTimeForFunction(time))
                label:setString(string_format("%s <div fontcolor=%s>%s</div>", time_title, time_color, timeStr))
            else
                doStopAllActions(label)
                local timeStr = string_format(end_title, "00:00:00")
                label:setString(string_format("%s <div fontcolor=%s>%s</div>", time_title, time_color, timeStr ))
            ...
            if time > 0 then
                local timeStr = string_format(end_title, TimeTool.GetTimeForFunction(time))
                label:setString(time_title .. timeStr)
            else
                doStopAllActions(label)
                local timeStr = string_format(end_title, "00:00:00")
                label:setString(time_title .. timeStr)
...
function addCountForCampIcon( node, num_data )
    ...
    if top_num and top_num > 0 then
        if not node.top_num_txt then
            ...
            node.top_num_txt = createLabel(20, Config.ColorData.data_new_color4[1], nil, node_size.width/2-2, node_size.height-15, top_num, node, nil, cc.p(0.5, 0.5))
        else
    ...
    if bottom_num and bottom_num > 0 then
        if not node.bottom_num_txt then
            ...
            node.bottom_num_txt = createLabel(20, Config.ColorData.data_new_color4[1], nil, node_size.width/2-2, 15, bottom_num, node, nil, cc.p(0.5, 0.5))
        else
    ...
    if bottom_num2 and bottom_num2 > 0 then
        if not node.bottom_num_txt2 then
            local node_size = node:getContentSize()
            node.bottom_sp2 = createSprite(PathTool.getResFrame("common","common_90089"), 54, 24, node, cc.p(0.5, 0.5))
            --node.bottom_sp2:setRotation(-45)
            node.bottom_num_txt2 = createLabel(20, cc.c4b(255,249,242,255), nil, 52, 24, bottom_num2, node, nil, cc.p(0.5, 0.5))
        else
        ...
        if node.bottom_num_txt then
            ...
            node.bottom_sp:setPosition(18, 24)
            node.bottom_num_txt:setPosition(16, 24)
        end
...
function checkUserProto(ios_show_user_proto)
    if PLATFORM_NAME == "xinxin" or PLATFORM_NAME == "demo" then
        return true
    end
    return false
...
-- label 自动缩放 根据文字内容和背景大小进行缩放
function setLabelAutoScale(label ,bg, offset)
    local offset = offset or 0
    local label_width = label:getContentSize().width+offset
    local bg_width = bg:getContentSize().width
    local scale = bg_width / label_width
    scale = math.min(1,scale)
    print("setLabelAutoScale",label_width,bg_width,scale)
    label:setScale(scale)
end

--==============================--
--desc:根据文字宽高设置背景大小
--time:2021-09-22 11:45:37
--@label:   
--@bg:      
--@off_x:   宽边距，负数表示不处理
--@off_y:   高边距，负数表示不处理
...
--return 
--==============================--
function setBgByLabel(label, bg, off_x, off_y)
    local off_x = off_x or 0
    local off_y = off_y or 0

    local label_size = label:getContentSize()
    local bg_size = bg:getContentSize()

    local bg_width, bg_height = bg_size.width, bg_size.height
    if (off_x >= 0) and (label_size.width + off_x > bg_size.width) then
        bg_width = label_size.width + off_x
    end
    if (off_y >= 0) and (label_size.height + off_y > bg_size.height) then
        bg_height = label_size.height + off_y
    end

    bg:setContentSize(cc.size(bg_width, bg_height))
    label:setPositionX(bg:getContentSize().width*0.5)
end

--==============================--
--desc:根据语言显示文本
--time:2021-10-22 16:12:23
--@label_zh: 中文文本
--@label_en: 英文文本
--return 
--==============================--
function showLabelByCode(label_zh, label_en)
    local code = cc.Application:getInstance():getCurrentLanguageCode()
    if code == "zh" then
        label_zh:setVisible(true)
        label_en:setVisible(false)
    else
        label_zh:setVisible(false)
        label_en:setVisible(true)
    end
end
