...
function MoneyTool.ZhGetMoneyString(value, is_symbol)
...
function MoneyTool.EnGetMoneyString(value, is_symbol)
  if value == nil then
    return 0
  end
  local value1 = 0  -- 整数部分
  ...
  elseif value < 1000000 then
    -- value = value * 0.001
    -- return string.format(TI18N("%.2fK"), value)
    value = math.modf(value * 0.01)
    value1, value2 = math.modf(value/10)
    unit = "K"
  elseif value < 1000000000 then
    -- value = value * 0.000001
    -- return string.format(TI18N("%.2fM"), value)
    value = math.modf(value * 0.00001)
    value1, value2 = math.modf(value/10)
    print("value, value1, value2", value, value1, value2)
    unit = "M"
  else
  ...
  if is_symbol then
    if value2 == 0 then
      return string.format(TI18N("%s%s"), MoneyTool.moneyFormat(value1), unit)
    else
      return string.format(TI18N("%s.%s%s"), MoneyTool.moneyFormat(value1), value2*10, unit)
    end
  else
    return string.format(TI18N("%s%s"), value/10, unit)
  end
...
function MoneyTool.GetMoneyString(value, is_symbol)
  ...
  if code == "zh" then
    return MoneyTool.ZhGetMoneyString(value, is_symbol)
  else
    return MoneyTool.EnGetMoneyString(value, is_symbol)
  end
