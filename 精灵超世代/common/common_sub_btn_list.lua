...
function CommonSubBtnList:ctor( parent, anchorpoint, pos, btn_size, call_back )
	...
	self.btn_size = btn_size or cc.size(168, 66)
end
...
function CommonSubBtnList:setData( data, select_index )
	...
	for i,btnInfo in ipairs(data) do
		...
		object.label:setString(string.format("<div fontcolor=#3d5078>%s</div>", object.title))
...
function CommonSubBtnList:getBtnResByIndex( index, is_select )
	...
	--end
	local res = PathTool.getResFrame("common","common_1124")
	if is_select then
...
function CommonSubBtnList:createSubButton( index )
    ...
    object.label = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(self.btn_size.width*0.5, self.btn_size.height*0.5))
...
function CommonSubBtnList:_onClickSubBtnByIndex( index )
	...
	for i,object in ipairs(self.btn_objects) do
		if object.index == index then
			...
			setLabelAutoScale(object.label,object.select_bg,40)
			if object.title then
				object.label:setString(string.format("<div fontcolor=#ffffff shadow=0,-2,2,#854000>%s</div>", object.title))
			end
		else
			object.normal_bg:setVisible(true)
			object.select_bg:setVisible(false)

			setLabelAutoScale(object.label,object.normal_bg,40)
			if object.title then
				object.label:setString(string.format("<div fontcolor=#3d5078>%s</div>", object.title))
