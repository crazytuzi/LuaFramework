...
function sdkOnSwitchAccount()
    print("sdkOnSwitchAccount", GAME_INITED, CALL_SDK_SWITCH_ACCOUNT, sdkCallFunc("isSwitchAccount"))
    --modified by chenbin:允许每次都能打开sdk界面
    ...
    elseif CALL_SDK_SWITCH_ACCOUNT == 2 then
        sdkCallFunc("switchAccount")
...
local DATA_TPYE_STR = {
    ...
    [5] = "exitServer";
    [6] = "fbshare";
    [7] = "initRewardedVideoAd";
    [8] = "loadRewardedVideoAd";
    [9] = "isReadyRewardedVideoAd";
    [10] = "showRewardedVideoAd";
    [11] = "inapp";
    [12] = "subs";
    [13] = "subs_privacy_policy";
    [14] = "subs_automatic_renewal_agreement";
}

-- SDK数据上报处理
-- dataType: 1.选择服务器, 2.创建角色的时候, 3.进入游戏时, 4.等级提升时, 5.退出游戏时, 6.创建充值订单
function sdkSubmitUserData(dataType, rdata)
    ...
    local account = LoginPlatForm:getInstance():getInfo().openid or ""
    ...
    local guildName = roleVo and roleVo.gname or ""
    local currentTimeStamp = GameNet:getInstance():getTime()
    local channel = device.getChannel()
    if channel == "66_1" then
        serverId = PLATFORM_NAME.."_"..serverId
    end

    local info = table.concat({dataType, serverId, serverName, roleId, roleName, roleCTime, roleLev, gold, vipLev, power, account, guildName, currentTimeStamp}, "#")
    if PLATFORM_NAME == "yingpai" then
        if dataType == 2 or dataType== 3 or dataType == 4 then
...
function sdkOnEnterServer()
    local role_vo = RoleController:getInstance():getRoleVo()
    local loginData = LoginController:getInstance():getModel():getLoginData()

    local info = table.concat({role_vo.rid, role_vo.lev, role_vo.name,
                               role_vo.srv_id, loginData.srv_id, role_vo.sex, role_vo.power,role_vo.vip_lev}, "#")
    sdkCallFunc("OnEnterServer", info)

end

function sdkOnCreateRole()
    local account = LoginPlatForm:getInstance():getInfo().openid
    local role_vo = RoleController:getInstance():getRoleVo()
    local info = table.concat({role_vo.rid, 1, role_vo.name,
                               role_vo.srv_id, loginData.srv_id, role_vo.sex, role_vo.power,power.vip_lev}, "#")
    sdkCallFunc("sdkOnCreateRole", info)
end

function sdkOnRoleUpgrade()
    local roleVo = RoleController:getInstance():getRoleVo()
    sdkCallFunc("sdkOnRoleUpgrade", roleVo.lev)
end

--事件埋点
function sdkUserEvent(params)
    sdkCallFunc("userEvent", params)

end

-- SDK充值处理
function sdkOnPay(money, buyNum, prodId, productName, productDesc)
    print("SDK充值处理: ",money, buyNum, prodId, productName, productDesc)
    ...
    if NEED_CHECK_VISITIOR_STATUS and DO_NOT_REALNAME_STATUS and (not ALLOW_SKIP_RECHARGE) then
        ...
        --打点
        if PLATFORM_NAME == "mojing_301" or PLATFORM_NAME == "mojing_302" then
            sdkSubmitUserData(6)
        end
        ...
        local productId = string.format("%s.%s", callFunc("package_name"), money) --现在商品类型统一使用价格区分
        if PAY_ID_FUNC then
        ...
        if config == nil then
            print("charge config can't found")
            return
        end
        ...
        local serverName = srvData.srv_id
        ...
        local ext = serverName .. "|" .. prodId
        local host = srvData.host
        local account_id = LoginPlatForm:getInstance():getUId()
        local cp_order_id = roleId .. "_" .. os.date("%Y-%m-%d %H:%M:%S")
        -- 冰鸟的英灵勇士充值的特殊处理
        ...
        local info = table.concat({productId, "goods", productDesc, price, buyNum, gold, ext,cp_order_id, serverName, roleName, roleLev,srvData.srv_id,roleId}, "#")
...
local function sdkOnPayByGM(money, buyNum, prodId, productName, productDesc)
    Send(10399, {msg="pay "..prodId})
end

if IS_GM_PAY then
    sdkOnPay = sdkOnPayByGM
end
...
-- sdk切换账号并且登录该账号
function sdkBackSwitchAccountWithLogin(code, msg)
    if not LoginPlatForm then return end
    if not restart then return end
    LoginPlatForm:getInstance():onSwitchAccount(msg)
end
...
function sdkBackLogin_bingniao_req(body, num, last_xhr)
    ...
    print("push_log==>", url, body)
    local xhr = cc.XMLHttpRequest:new()
    xhr.responseType = cc.XMLHTTPREQUEST_RESPONSE_STRING
    xhr:open("POST", url)
    xhr:registerScriptHandler(function()
        print("===log_result==>", xhr.readyState, xhr.response)
        if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
            local parseJson = function()
                local response = string.gsub(xhr.response, "\\", "")
                print("===log_result==>", xhr.readyState, response)
...
--获取当前货币类型
function getCurPriceType()
    if CUR_PRICE_TYPE then return CUR_PRICE_TYPE end
    return PRICE_TYPE.USD
end

function sdkSendErrorToBugly(msg)
    sdkCallFunc("SendErrorToBugly", msg)
end
