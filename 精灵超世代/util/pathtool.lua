...
function PathTool.getNormalSkillBg()
	return PathTool.getResFrame("common", "common_1005_1")
end
...
PathTool.WhiteAttrIcon = {
	["atk"] 		= "common_90021_white", --攻击
	["atk_per"]		= "common_90021_white", --攻击
	["hp"] 			= "common_90022_white", --血
	["hp_max"] 		= "common_90022_white", --血
	["hp_max_per"]  = "common_90022_white", --血
	["def"] 		= "common_90023_white", --防御
	["def_per"] 	= "common_90023_white", --防御
	["speed"] 		= "common_90038_white", --速度
	["crit_rate"] 	= "common_90043_white", --暴击率
	["crit_ratio"] 	= "common_90039_white", --暴伤害
	["hit_magic"] 	= "common_90040_white", --控制
	["dodge_magic"] = "common_90037_white", --抗控
	["tenacity"] 	= "common_90021_1_white", --抗暴
	["hit_rate"] 	= "common_90021_2_white", --命中
	["res"] 		= "common_90021_3_white", --免伤
	["dodge_rate"] 	= "common_90021_4_white", --闪避
	["cure"] 		= "common_90021_5_white", --治疗
	["be_cure"] 	= "common_90021_6_white", --被治疗
	["dam"] 		= "common_90021_7_white", --伤害加成

	["dam_p"]       = "common_90021_white", --物伤
	["dam_s"]       = "common_90021_white", --法伤
	["res_p"]       = "common_90021_3_white", --物免
	["res_s"]       = "common_90021_3_white", --法免

	--特殊符号
	["random"] 		= "common_90021_8_white", --问号 随机属性符号
	["skill"] 		= "common_90021_9_white", --技能专属符号
}
...
--获取属性图标
function PathTool.getWhiteAttrIconByStr(str)
	if PathTool.WhiteAttrIcon[str] then
		return  PathTool.WhiteAttrIcon[str]
	end
	return "common_90037_white"
end
-- 获取checkbox背景资源
...
-- 获取角色品质框背景
function PathTool.getHeroQualityBg(quality)
	if quality == nil then
		return PathTool.getResFrame("common","common_1005")
	end
	local quality = quality or 1
...
-- 获取精灵的资源
function PathTool.getElfinRes(res_id)
	if res_id == nil or res_id == 0 then
		res_id = 113001
	end
	return PathTool.checkRes(string_format("resource/elfin/elfin_res/%s.png", res_id))
end
...
-- 获取表情icon
function PathTool.getFaceIconRes( res_id )
	return PathTool.checkRes(string_format("resource/faceicon/%s.png", res_id))
end
...
function PathTool.getSpineByName(spine_name, action_name)
    ...
    if p then 
        return PathTool.getSpineByNameV36(spine_name, action_name)
    else
    	--训练师
        local p1,q1 = string.find(spine_name, "roles/")
        if p1 then
            local dirPath = "res/spine/"
            return PathTool.getSpineByNameV36(spine_name, action_name, dirPath)
        end

        --家用迷你形象等
        local p1,q1 = string.find(spine_name, "gameplay/")
        if p1 then
            local dirPath = "res/spine/"
            return PathTool.getSpineByNameV36(spine_name, action_name, dirPath)
        end
	...
	if not PathTool.isFileExist(atlas) then
		...
		spine_path = string_format("spine/%s/%s", spine_name, spine_name)
		local skel = string_format("%s.skel", spine_path)
		local atlas = string_format("%s.atlas", spine_path)
		local png = string_format("%s.png", spine_path)
...
function PathTool.getSpineByNameV36(spine_name, action_name, dirPath)
	local PathFix = dirPath or DefaultPathFix
	local spine_path = PathFix..spine_name
	local skelPath = PathFix..spine_name..".skel"
    local atlasPath = PathFix..spine_name..".atlas"
    local pngPath = PathFix..spine_name..".png"

	if not PathTool.isFileExist(atlasPath) then
		print(atlasPath .. "    不存在")
		print(debug.traceback())
		if not force then
			skelPath, atlasPath, pngPath, spine_path, spine_name = PathTool.defaultSpine(spine_name, action_name, true)
		end
	end
	return skelPath, atlasPath, pngPath, spine_path, spine_name
end
...
function PathTool.getPartnerBustRes(bust_id)
	...
	return string_format("resource/partner/%s.png", bust_id)
end
