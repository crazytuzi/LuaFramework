...
function HonorYuekaPanel:loadResources()
    self.res_list = {
		...
		{ path = PathTool.getPlistImgForDownLoad("bigbg/welfare","txt_cn_welfare_yueka_2"), type = ResourcesType.single }
    } 
...
function HonorYuekaPanel:configUI()
	...
	local res_txt = PathTool.getPlistImgForDownLoad("bigbg/welfare", "txt_cn_welfare_yueka_2")
	...
	local str = string_format(TI18N("<div outline=2,#3d5078>任意累计充值达到</div><div fontcolor=#0cff01 outline=2,#3d5078> %d元 </div><div outline=2,#3d5078>即可激活</div>"),card2_add_count)
    self.text_1:setString(str)

    local item_config = Config.ItemData.data_get_data(card_data.month_card2_return.val[1][1])
	local str = string_format(TI18N("充值就<div fontsize=32 fontcolor=#fff99e> 送%d天 </div>月卡"),add_get_day_2)
    self.text_2:setString(str)

    local item_config = Config.ItemData.data_get_data(item_bid_2)
	local str = string_format(TI18N("每日可领取<div fontsize=32 fontcolor=#fff99e> %d </div><img src=%s visible=true scale=0.30 />"),item_num_2,PathTool.getItemRes(item_config.icon))
