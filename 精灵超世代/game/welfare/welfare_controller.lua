...
function WelfareController:handle14100( data )
    ...
    --LimitTimeActionController:getInstance():sender28000()
end
...
function WelfareController:getWelfareSubList()
    ...
    if self.welfare_list ~= nil and next(self.welfare_list) ~= nil then
        for k,v in pairs(self.welfare_list) do
            ...
            --暂时屏蔽几个icon
            if v.bid == WelfareIcon.wechat or v.bid == WelfareIcon.bindphone or v.bid == WelfareIcon.invicode or v.bid == WelfareIcon.poste or v.bid == WelfareIcon.subscribe then
                need_add = false
            end
