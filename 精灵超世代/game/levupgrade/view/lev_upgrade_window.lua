...
function LevupgradeWindow:open_callback()
	...
	local sprite = self.title_container:getChildByName("sprite")
	loadSpriteTexture(sprite, PathTool.getPlistImgForDownLoad("common/txt_common", "txt_common_2"), LOADTEXT_TYPE )
...
function LevupgradeWindow:setAwardInfo( )
	...
	if self.award_data and next(self.award_data) ~= nil then
		...
		for i,v in ipairs(self.award_data) do
			...
			item:setPosition(cc.p(pos_x, panel_size.height/2-64))
		end
