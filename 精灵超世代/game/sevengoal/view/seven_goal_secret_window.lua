...
function SevenGoalSecretWindow:open_callback()
    ...
    self.charge_tesk = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(0,0.5), cc.p(19,124), nil, nil, 1000)
...
function SevenGoalSecretWindow:setChangeLev(lev,exp)
    ...
    print("period",period)
    dump(cycle_item)
    local item_config = Config.ItemData.data_get_data(cycle_item[period])
    if item_config then
        local res = PathTool.getItemRes(item_config.icon)
        loadSpriteTexture(self.icon_sprite,res,LOADTEXT_TYPE)
    end

    local count = self.role_vo:getActionAssetsNumByBid(cycle_item[period])
...
function SevenGoalSecretWindow:register_event()
    if not self.role_secret_event and self.role_vo then
        self.role_secret_event = self.role_vo:Bind(RoleEvent.UPDATE_ROLE_ACTION_ASSETS, function(id, value)
            local period = controller:getModel():getSevenGoalPeriod()
            local item_num = cycle_item[period] or 0
        ...
        for i=1,6 do
            delayRun(self.tesk_panel, i*2/60, function()
                ...
                if self.tesk_list[i] then
                    ...
                    local val = (i-1)%3
                    self.tesk_list[i]:setPosition(103+(210*val),pos_y-(260*math.floor(i/4)))
                end
            end)
        end
...
function SevenGoalSecretItem:configUI()
    ...
    self.reward_item:setPosition(cc.p(main_container:getContentSize().width/2, 121))
