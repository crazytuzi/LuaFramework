...
function SevenGoalAdventureWindow:open_callback()
    ...
    local cur_lev_bg = main_container:getChildByName("cur_lev_bg")
    cur_lev_bg:getChildByName("Text_6_0"):setString(TI18N("等级"))
    self.cur_lev_num = cur_lev_bg:getChildByName("cur_lev_num")
...
function SevenGoalAdventureWindow:setChargeLevData(exp,lev)
    ...
    --end
    self.cur_lev_num:setString(lev)
    local period = controller:getModel():getSevenGoalPeriod()
...
function SevenGoalAdventureItem:configUI()
    ...
    self:setContentSize(cc.size(607,159))
