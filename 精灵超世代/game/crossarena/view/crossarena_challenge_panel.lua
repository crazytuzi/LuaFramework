...
function CrossarenaChallengePanel:createRoorWnd()
    ...
    self.title_my_rank = bottom_panel:getChildByName("title_my_rank")
    self.title_my_rank:setString(TI18N("我的排名:"))
    self.txt_my_rank = bottom_panel:getChildByName("txt_my_rank")

    self.title_my_score = bottom_panel:getChildByName("title_my_score")
    self.title_my_score:setString(TI18N("我的积分:"))
    ...
    self.txt_my_rank:setPositionX(self.title_my_rank:getPositionX() + self.title_my_rank:getContentSize().width + 10)
    self.txt_my_score:setPositionX(self.title_my_score:getPositionX() + self.title_my_score:getContentSize().width + 10)
    self.txt_time:setPositionX(self.title_time:getPositionX() + self.title_time:getContentSize().width + 10)
end
...
function CrossarenaChallengePanel:_onClickShowAwardTips( node, data, pos )
    ...
    if self.tips_bg then
        local bg_size = cc.size(BackPackItem.Width*0.8*#data+80+(#data-1)*65, BackPackItem.Height*0.8+70)
    ...
    for i,v in pairs(data) do
        ...
        if item_config then
            if not award_item then
                ...
                award_item:setPosition(cc.p((BackPackItem.Width*0.8+65)*(i-1)+35, 95))
