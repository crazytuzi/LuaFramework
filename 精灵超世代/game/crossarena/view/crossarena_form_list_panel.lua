...
function CorssarenaFormListPanel:createRoorWnd(  )
    ...
    for i=1,3 do
    	...
    	if team_panel then
            ...
            object.hallow_item:setPosition(cc.p(570, 42))
            ...
            if self.fun_form_type == PartnerConst.Fun_Form.ArenapeakchampionDef then
                ...
                if i == 3 then
                    ...
                    object.three_label = createLabel(22,cc.c4b(0x68,0x45,0x2A,0xff),nil,600,110,str,team_panel,nil, cc.p(1, 0.5))
                end
            else
...
function CorssarenaFormListPanel:updateTeamInfo( index, is_all )
    ...
    if is_pvp and power_num > 0 and pvp_power > 0 then
        if team_object.pvp_arrow == nil  then
            team_object.pvp_arrow = createImage(team_object.team_panel, PathTool.getResFrame("common","common_1086"), 282, 109, cc.p(0.5,0.5), true)
         ...
         if team_object.show_pvp_tips == nil then
            team_object.show_pvp_tips = createLabel(16, cc.c3b(0x24,0x90,0x03), nil, 84, 109, TI18N("(公会pvp)"), team_object.team_panel, nil, cc.p(0, 0.5))
        else
    ...
    team_object.power_txt:setString(changeBtValueForPower(power_num))
    ...
    if is_all == true then
        ...
        if team_object.formation_type ~= 0 then
            local res = "res/resource/form/form_form_icon_"..team_object.formation_type..".png"
            loadSpriteTexture(team_object.form_icon, res, ccui.TextureResType.localType)
        end
