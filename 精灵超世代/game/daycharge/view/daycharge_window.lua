...
function DayChargeWindow:open_callback()
    ...
    self.charge_num:setPosition(310, 630)
...
function DayChargeWindow:openRootWnd()
    ...
    --self.first_effect[1] = createEffectSpine(PathTool.getEffectRes(650), cc.p(360, 693), cc.p(0.5, 0.5), true, PlayerAction.action)
    --self.main_container:addChild(self.first_effect[1])

    self.first_effect[1] = createEffectSpine(PathTool.getEffectRes(651), cc.p(360, 246), cc.p(0.5, 0.5), true, PlayerAction.action)
    self.main_container:addChild(self.first_effect[1])
    self.first_effect[1]:setVisible(false)
end
...
function DayChargeWindow:register_event()
	self:addGlobalEvent(DayChargetEvent.DAY_FIRST_CHARGE_EVENT, function(data)
		...
		if data.status == 0 then
			self.first_effect[1]:setVisible(true)
