...
function ElfinTipsWindow:setData( elfin_bid )
	...
	--self.base_panel:setBackGroundImage(background_res, LOADTEXT_TYPE_PLIST)
    ...
    if not self.elfin_item then
    	...
    	self.elfin_item:setPosition(cc.p(70, 64))
   		...
   		if skill_cfg then
		    ...
		    -- 技能描述
		   	...
		   	if not self.skill_desc then
		   		self.skill_desc = createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(22, skill_start_pos_y),4,nil,370)
		   	...
		   	new_height = new_height + skill_des_height + 20

		   	-- 冷却时间
		    if not self.extend_desc then
		    	self.extend_desc = createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(22,skill_start_pos_y-skill_des_height-10),nil,nil,370)
	        ...
	        if skill_cfg.cd == 0 then
	            ...
	            extend_str = _string_format("<div >%s</div><div fontcolor=#14ff32>%s</div><div >%s</div>", TI18N("冷却"),skill_cfg.cd, TI18N("回合") )
	        end
	        if skill_cfg.fire_cd ~= 0 then
	            extend_str = extend_str.._string_format("<div >%s</div><div >%s</div><div >%s</div>", TI18N("，第"), skill_cfg.fire_cd, TI18N("回合释放"))
	        end
		        ...
		        for i, v in ipairs(skill_cfg.buff_des[1]) do
		            ...
		            if config then
		                if buff_desc_str ~= "" then
		                    buff_desc_str = buff_desc_str.."<div >\n</div>"
		                end
		                local buff_desc = _string_format("<div >【%s】</div><div >\n%s</div>", config.name, config.desc)
		    ...
		    if buff_desc_str ~= "" then
		    	if self.buff_desc == nil then
		            self.buff_desc = createRichLabel(20,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(22,skill_start_pos_y-skill_des_height-extend_desc_height-20),4,nil,370)
		        ...
		        new_height = new_height + buff_desc_height + 20
