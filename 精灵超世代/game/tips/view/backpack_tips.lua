...
function BackpackTips:open_callback()
    ...
    self.goods_item:setPosition(cc.p(70+50,66))
    ...
    self.extend_desc = createRichLabel(20,Config.ColorData.data_new_color4[6],cc.p(0,0),cc.p(156,10),nil,nil,500)
    ...
    self.desc_label = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0, 1), cc.p(20, 100), 8, nil, 460)
...
function BackpackTips:clickBtn(index)
    ...
    elseif index == BackPackConst.tips_btn_type.drama_source then --跳转剧情副本界面 
        MainuiController:getInstance():changeMainUIStatus(MainuiConst.btn_index.drama_scene)
    elseif index == BackPackConst.tips_btn_type.hero_source then --跳转宝可梦信息界面 
        MainuiController:getInstance():changeMainUIStatus(MainuiConst.btn_index.partner)
    elseif index == BackPackConst.tips_btn_type.skill_source then --跳转宝可梦技能界面 
    ...
    elseif index == BackPackConst.tips_btn_type.fenjie then --宝可梦碎片分解
        local list = {}
    ...
    elseif index == BackPackConst.tips_btn_type.weekly_activity then
...
function BackpackTips:adjustContentSizeAndPos(  )
    ...
    target_height = target_height  + 14 --底部纹理
    ...
    self.scroll_view:setPositionY(self.desc_panel:getContentSize().height)
...
function BackpackTips:updateBtnList()
    ...
    elseif btn_sum == 3 then
        local object_1 = self.btn_list[1]
        local object_2 = self.btn_list[2]
        local object_3 = self.btn_list[3]
        if object_1.btn and object_2.btn and object_3.btn then
            --object_1.btn:setPositionX(104)
            --object_2.btn:setPositionX(431)
            object_1.btn:setScale(0.9)
            object_2.btn:setScale(0.9)
            object_3.btn:setScale(0.9)
        end
    end
	...
	for i, v in ipairs(tips_btn) do
        ...
        if object and object.btn then
            ...
            --object.label:enableOutline(CommonButton_Color[3],2)
            object.label:enableShadow(cc.c4b(15, 114, 179, 255), cc.size(0, -3),3)
            object.label:setPositionY(object.label:getPositionY())
...
function BackpackTips:checkRedPoint()
	...
	-- 		local hero_upgrade_star_fuse_windownum = config.compound_expend[1] [2]
...
dump(self.item_config)
    ...
    self.scroll_view:setContentSize(cc.size(self.scroll_size.width, math.min(max_height, 306))) -- 600 - 252 - 32
    ...
    if self.setting.is_market_place then
        if self.data.end_time and next( self.data.end_time) ~= nil then
            self.market_time_label = createRichLabel(20, cc.c4b(0xff,0x9b,0x1e,0xff), cc.p(0, 1), cc.p(17, -10000), 8, nil, 1000)
