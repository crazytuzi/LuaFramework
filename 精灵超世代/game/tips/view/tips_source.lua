...
function TipsSource:open_callback()
    ...
    self.own_label:setVisible(false)

    self.goods_item = BackPackItem.new()
    self.goods_item:setPosition(100,440)
    ...
    self.scrollView = createScrollView(self.scroll_size.width+4,self.scroll_size.height,0,10,self.scrollCon,ccui.ScrollViewDir.vertical)
	...
	if source_list and next(source_list)~=nil then
		...
		if final_list and next(final_list or {}) ~= nil then
			for i,v in ipairs(final_list) do
				...
				item:setData(v)
				item:setPosition(10,max_height-6-(SourceItem.HEIGHT+6)*(i-1))
			end
		end
	end
end
