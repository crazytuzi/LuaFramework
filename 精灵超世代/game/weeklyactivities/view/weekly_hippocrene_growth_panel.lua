...
-- -----------------------------------------------------hippocrene---------------
WeeklyHippocreneGrowthPanel = class("WeeklyHippocreneGrowthPanel", function()
...
function WeeklyHippocreneGrowthPanel:ctor()
	self.current_day = 0
	self.isPlay = false
	self.playNum = 1
	self.item_id = 17449
...
function WeeklyHippocreneGrowthPanel:loadResources()
    self.res_list = {
...
function WeeklyHippocreneGrowthPanel:loadResListCompleted()
	self:configUI()
	self:register_event()
end

function WeeklyHippocreneGrowthPanel:configUI()
	...
	self:setAnimationActi( task_data )
	--controll:updateTipsStatus()
end

function WeeklyHippocreneGrowthPanel:setLessTime(data)
...
function WeeklyHippocreneGrowthPanel:playFlowerAnim( )
...
function WeeklyHippocreneGrowthPanel:flowerAnimationComplete()
	if(self.flower)then
...
function WeeklyHippocreneGrowthPanel:setFlowerNum( num )
	self.playNum = num
end


function WeeklyHippocreneGrowthPanel:playWetOutAnimation(num)
...
function WeeklyHippocreneGrowthPanel:wetOutAnimationComplete()
	--self.isPlay = false
end

function WeeklyHippocreneGrowthPanel:setAnimationActi( data )
	local num = 0
	...
	local act_index =  num%4
	if num < 4 and act_index == 0 then 
		act_index = 1
	end
	if num > 3  and  act_index == 0 then 
		act_index = 4
	end
	self:setFlowerNum( act_index )
end

function WeeklyHippocreneGrowthPanel:register_event()
	...
	self.update_week_task_data = GlobalEvent:getInstance():Bind(WeeklyActivitiesEvent.UPDATE_WEEK_TASK_DATA, function(data)
		local task_data = model:getWeeklyTaskData()
		--task_data.info_list
		self:setAnimationActi( task_data )
...
function WeeklyHippocreneGrowthPanel:setVisibleStatus(bool)
	--bool = bool or false
	--self:setVisible(bool)
end

function WeeklyHippocreneGrowthPanel:DeleteMe()
    ...
    if self.update_week_task_data then 
    	GlobalEvent:getInstance():UnBind(self.update_week_task_data)
        self.update_week_task_data = nil
    end
end 