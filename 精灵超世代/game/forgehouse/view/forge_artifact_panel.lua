...
function ForgeArtifactPanel:layoutUI(  )
	...
	self.cost_txt = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0.5, 1), cc.p(temp_pos_x, temp_pos_y - 16))
	...
	self.progress_bar = cc.ProgressTimer:create(createSprite(PathTool.getResFrame("forgehouse","forgehouse_3"), 0, 0, nil, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST))
...
function ForgeArtifactPanel:updataZhufuInfo(  )
	local cur_lucky = 50 --HeroController:getInstance():getModel():getArtifactLucky()
	local max_lucky = 0
