...
function ForgeHouseWindow:open_callback()
	...
	local itembg_1 = self.equip_container:getChildByName("itembg_1")
	loadSpriteTexture(itembg_1,PathTool.getPlistImgForDownLoad("bigbg","bigbg_80",false), LOADTEXT_TYPE)

	local itembg_2 = self.equip_container:getChildByName("itembg_2")
	loadSpriteTexture(itembg_2,PathTool.getPlistImgForDownLoad("bigbg","bigbg_80",false), LOADTEXT_TYPE)
...
function ForgeHouseWindow:initEquipLayout(  )
	...
	for i=1,4 do
		...
		tab.title:setTextColor(Config.ColorData.data_new_color4[6])
    ...
    self.item_scrollview = CommonScrollViewLayout.new(self.scroll, cc.p(20, 0), ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)

    self.last_item = BackPackItem.new(nil,true,nil,0.8)
    self.equip_container:addChild(self.last_item)
    self.last_item:setPosition(cc.p(209, 928))
    self.last_item:setDefaultTip()

    self.cur_item = BackPackItem.new(nil,true,nil,0.8)
    self.equip_container:addChild(self.cur_item)
    self.cur_item:setPosition(cc.p(509, 928))
...
function ForgeHouseWindow:adaptationScreen()
	local bottom_y = display.getBottom(self.main_container)

	local bottom_panel_y = self.close_btn:getPositionY()
	self.close_btn:setPositionY(bottom_y + bottom_panel_y)
end
...
function ForgeHouseWindow:changeSelectedTab( index )
    ...
    if self.tab_object then
		...
		self.tab_object.lable:setTextColor(Config.ColorData.data_new_color4[6])
    ...
    if self.tab_object then
		...
		self.tab_object.lable:setTextColor(Config.ColorData.data_new_color4[1])
