...
PetardRedbagWindow = PetardRedbagWindow or BaseClass(BaseView)

function PetardRedbagWindow:__init()
	self.win_type = WinType.Mini
    self.view_tag = ViewMgrTag.DIALOGUE_TAG
	self.is_full_screen = false
	self.layout_name = "petard/petard_redbag_window"
...
function PetardRedbagWindow:open_callback(  )
	...
	main_container:getChildByName("win_title"):setString(TI18N("庆典红包"))
...
function PetardRedbagWindow:_onClickSubTabBtn( index )
	if self.cur_tab_index and self.cur_tab_index == index then return end
	self.cur_tab_index = index

	if index == PetardActionConst.Redbag_Tab.Redbag then
		if not self.my_redbag_scrollview then
			_controller:sender27002() -- 请求红包数据
			self:createMyRedbagScrollview()
		end
	elseif index == PetardActionConst.Redbag_Tab.Redmsg then
		if not self.redbag_msg_scrollview then
			_controller:sender27006() -- 请求红包传闻数据
			self:createRedbagMsgScrollview()
		end
	end

	if self.my_redbag_panel then
		self.my_redbag_panel:setVisible(index == PetardActionConst.Redbag_Tab.Redbag)
	end
	if self.redbag_msg_panel then
		self.redbag_msg_panel:setVisible(index == PetardActionConst.Redbag_Tab.Redmsg)
	end
end

function PetardRedbagWindow:createMyRedbagScrollview(  )
...
function PetardRedbagWindow:_createNewCell(  )
	local cell = PetardRedbagItem.new()
	cell:addClickCallBack(handler(self, self.onClickItemCallBack))
    return cell
end

function PetardRedbagWindow:_numberOfCells(  )
	if not self.my_redbag_data then return 0 end
    return #self.my_redbag_data
end

function PetardRedbagWindow:_updateCellByIndex( cell, index )
...
function PetardRedbagWindow:onClickItemCallBack( red_packet_id, red_status )
	...
	if red_status == 1 then -- 红包可领取，则先播放特效
		...
		_controller:sender27003(red_packet_id)
	end
end

function PetardRedbagWindow:handleRedEffect( status )
...
function PetardRedbagWindow:_onRedAniCallBack(  )
	if self.cur_red_packet_id then
		_controller:sender27003(self.cur_red_packet_id)
...
function PetardRedbagWindow:createRedbagMsgScrollview(  )
	...
	local setting = {
        item_class = PetardRedbagMsgItem,      -- 单元类
        start_x = 0,                  -- 第一个单元的X起点
        space_x = 0,                    -- x方向的间隔
        start_y = 0,                    -- 第一个单元的Y起点
        space_y = 5,                   -- y方向的间隔
...
function PetardRedbagWindow:register_event(  )
	...
	-- 红包数据
	self:addGlobalEvent(PetardActionEvent.Get_Redbag_Data_Event, function (  )
		self:updateMyRedbagList()
	end)

	-- 红包传闻
	self:addGlobalEvent(PetardActionEvent.Get_Redbag_Msg_Data_Event, function ( data )
		self:updateRedbagMsgList(data)
	end)
end

-- 更新我的红包列表
function PetardRedbagWindow:updateMyRedbagList(  )
	local redbag_data = _model:getPetardRedbagData()
...
-- 更新红包传闻列表
function PetardRedbagWindow:updateRedbagMsgList( data )
	data = data or {}
...
function PetardRedbagWindow:onClickChoseBtn(  )
	_controller:openRedbagWindow(false)
end

function PetardRedbagWindow:openRootWnd(  )
	
end

function PetardRedbagWindow:close_callback(  )
