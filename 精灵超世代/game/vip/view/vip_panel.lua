...
function VipPanel:layoutUI()
	...
	self.item_scroll = createScrollView(self.goods_scroll:getContentSize().width,self.goods_scroll:getContentSize().height,0,0,self.goods_scroll,ccui.ScrollViewDir.horizontal)
	...
	self.old_price = createRichLabel(20, Config.ColorData.data_new_color4[11], cc.p(0.5,0), cc.p(410,10))
	self.chargeCon:addChild(self.old_price)

	self.buy_price = createRichLabel(24, 175, cc.p(0.5,0.5), cc.p(self.buy_btn:getPositionX()-55,35))
	...
	self.desc_label = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(255,280),-5,nil,480)
...
function VipPanel:setData(index)
	...
	for k,v in pairs(items) do
		...
		item:setDefaultTip()
		item:setScale(scale)
		item:setPosition(5+(single_width+10)*(k-1),0)
		if spec_item and next(spec_item or {}) ~= nil then
	...
	for k,v in pairs(desc_list)  do
		...
		if self.desc_list[k] then
			...
			self.desc_list[k]:setPosition(5,max_height+10-((self.desc_list[k]:getContentSize().height+20)*k))
		else 
			local delta = k - desc_hasnum
			delayRun(self.info_scroll,0.05*delta,function (  )
				if not self.desc_list[k] then
					local label = createRichLabel(18, Config.ColorData.data_new_color4[6], cc.p(0,0), cc.p(0,0), 0, 0, self.info_scroll:getContentSize().width)
