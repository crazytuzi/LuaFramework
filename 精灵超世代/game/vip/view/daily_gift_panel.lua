...
function DailyGiftPanel:createItemScrollview( item_count )
	...
	local scroll_size = cc.size(con_size.width, con_size.height)
...
function DailyGiftItem:configUI(  )
    ...
    if self.buy_btn.label ~= nil then
		...
		self.buy_btn.label:enableShadow(Config.ColorData.data_new_color4[4],cc.size(0, -2),2)
    end 
...
function DailyGiftItem:setData( data )
	...
	if self.gift_config.limit_count <= buy_count then
		...
		self.buy_btn:setTitleText(string.format(TI18N("%0.2f元"), self.gift_config.val or 0))
	end
	...
	if not self.gift_desc_txt then
		self.gift_desc_txt = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 1), cc.p(180, 165), 0, nil, 350)
