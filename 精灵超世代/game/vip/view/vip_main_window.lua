...
function VipMainWindow:open_callback()
    ...
    for i=1, #self.tab_array do
        ...
        tab_btn:setPosition(cc.p(110+(i-1)*160, 110))
    ...
    self.now_vip = CommonNum.new(22, self.titleCon, 1, -5, cc.p(0.5, 0.5))
    self.now_vip:setPosition(50, 130)

    self.next_vip = CommonNum.new(22, self.titleCon, 1, -5, cc.p(0, 0.5))
    self.next_vip:setPosition(480, 188)

    self.charge_num = CommonNum.new(21, self.titleCon, 1, -5, cc.p(0, 0.5))
    self.charge_num:setPosition(125, 188)
    ...
    self.tab_scrollview = CommonScrollViewLayout.new(self.tabLayer, cc.p(0,0 ), ScrollViewDir.vertical, ScrollViewStartPos.top, tab_size, setting)
...
function VipMainWindow:updateBar()
    ...
    if not self.next_vip_desc then
        self.next_vip_desc = createSprite(nil, 120, 123, self.titleCon, cc.p(0, 0.5))
    end
...
function VipMainWindow:createSubPanel( index )
    ...
    if panel == nil then
    	if index == VIPTABCONST.VIP then --vip特权
            ...
            self.container:getChildByName("rightLayer"):addChild(panel)
                    ...
                    if item_list then
                        for k,item in pairs(item_list) do
                            if cell:getData().lev+1 == k then
                                ...
                                item:setTextColor(Config.ColorData.data_new_color4[1], Config.ColorData.data_new_color4[1])
                                item:setTextSelected(true)
                                panel:setData(cell:getData().lev)
                            else
                                item:setSelect(false)
                                item:setNormal(true)
                                item:setTextColor(Config.ColorData.data_new_color4[15], Config.ColorData.data_new_color4[11])
...
function VipPanelItem:setData(data)
    ...
    if self.role_vip.vip_lev == data.lev then
        ...
        self:setTextColor(Config.ColorData.data_new_color4[1], Config.ColorData.data_new_color4[1])
        self:setTextSelected(true)
    else
        self:setTextColor(Config.ColorData.data_new_color4[15], Config.ColorData.data_new_color4[11])
        self:setTextSelected(false)
    end
...
function VipPanelItem:setTextSelected(_bool)
    if _bool then
        self.text_vip:disableEffect(cc.LabelEffect.OUTLINE)
    else
        self.text_vip:enableOutline(Config.ColorData.data_new_color4[11], 2)
    end
end
