...
function PrivilegeItem:configUI(  )
    ...
    self:addChild(self.root_wnd)
    self:setCascadeOpacityEnabled(true)
    self:setContentSize(cc.size(690, 180))
    ...
    self.buy_btn_label = createRichLabel(22, Config.ColorData.data_new_color4[1], cc.p(0.5, 0.5), cc.p(btn_size.width/2, btn_size.height/2))
    self.buy_btn:addChild(self.buy_btn_label)

    self.desc_txt = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(200, 140), 5, nil, 400)
    ...
    if srv_data.status == TRUE then
        if self.privilege_cfg.limit_type == 0 then -- 永久限购
            ...
            if not self.left_day_txt then
                self.left_day_txt = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(570, 65))
            ...
            if day < 1 then
                -- 小于1天则显示为小时
                self.left_day_txt:setString(string.format(TI18N("<div fontsize=24>还有</div><div fontsize=24 fontcolor=#0e7709>%s</div>"), TimeTool.GetTimeFormatTwo(left_time)))
            else
                self.left_day_txt:setString(string.format(TI18N("还有<div fontcolor=#0e7709>%s</div>天"), tostring(day)))
            end
        ...
        if self.privilege_cfg.pay_type == 1 then
            self.buy_btn_label:setString(string.format(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,#920eb3>%s元</div>"), self.privilege_cfg.loss))
        elseif self.privilege_cfg.pay_type == 2 then
            self.buy_btn_label:setString(string.format("<img src='%s' scale=0.3 /><div fontcolor=#ffffff shadow=0,-2,2,#920eb3>%d</div>", PathTool.getItemRes(3), self.privilege_cfg.loss))
        end
    end
end
