...
function VipController:handle16700( data )
    ...
    for i,v in pairs(data.list) do
        if PLATFORM_NAME == "happyos" then

        end
    end
    dump(data,"handle16700")
...
function VipController:handle16712( data )
    --第一次充值
    if self.charge_sum == 0 and self.isFirstRechange == false and data.charge_sum > 0 then
        sdkUserEvent("first_purchase")
    end
    if self.isFirstRechange then
        self.isFirstRechange = false
    end
