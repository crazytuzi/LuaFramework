...
function VedioMainWindow:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getPlistImgForDownLoad("bigbg","bigbg_107"), type = ResourcesType.single },
    }
...
function VedioMainWindow:open_callback(  )
    ...
    local role_bg = self.title_bg:getChildByName("role_bg")
    loadSpriteTexture(role_bg,PathTool.getPlistImgForDownLoad("bigbg","bigbg_107"), LOADTEXT_TYPE)
    local title = self.title_bg:getChildByName("title")
    --loadSpriteTexture(title,"resource/vedio/txt_vedio/txt_vedio_title.png", LOADTEXT_TYPE)
    --print("title",role_bg,title,PathTool.getPlistImgForDownLoad("vedio/txt_vedio","txt_vedio_title"))
    loadSpriteTexture(title, PathTool.getPlistImgForDownLoad("vedio/txt_viedo","txt_vedio_title"), LOADTEXT_TYPE)
    ...
    local setting = {
        ...
        item_height = 500,              -- 单元的尺寸height
        row = 0,                        -- 行数，作用于水平滚动类型
        col = 1,                         -- 列数，作用于垂直滚动类型
        need_dynamic = true
    }
    self.vedio_scrollview = CommonScrollViewSingleLayout.new(scrollCon, cc.p(0,0) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function VedioMainWindow:createTabBtnList(  )
    ...
    self.tab_scrollview = CommonScrollViewLayout.new(tab_container, cc.p(0, 0) , ScrollViewDir.horizontal, ScrollViewStartPos.bottom, scroll_view_size, setting)
end
...
function VedioMainWindow:refreshTodayLikeNum(  )
    ...
    self.like_limit_num:setString(string.format(TI18N("%d/%d"), left_num, total_num))
end
...
function VedioMainWindow:_onClickFiltBtn(  )
    if self.cur_index then
        ...
        if not self.filt_layout then
            ...
            self.filt_bg = createImage(self.filt_layout, PathTool.getResFrame("vedio", "vedio_1023"), 0, 0, cc.p(1, 1), true, 1, true)
        ...
        local space_y = 4
        local distance = 10
        local btn_size = cc.size(180, 50)
        local bg_size = cc.size(190, distance*2 + #evt_list*(btn_size.height+space_y)-space_y)
        self.filt_bg:setContentSize(bg_size)
        for i,v in ipairs(evt_list) do
            local btn = self.filt_btn_list[i]
            if btn == nil then
                btn = createButton(self.filt_bg, "", 0, 0, btn_size, PathTool.getResFrame("vedio", "vedio_1025"), 20, Config.ColorData.data_new_color4[6])
