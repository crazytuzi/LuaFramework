...
function SuitShopMainWindow:open_callback()
    ...
    self.main_container:getChildByName("title_con"):getChildByName("title_label"):setString(TI18N("饰品商店"))
...
function SuitShopMainWindow:createTabList()
    if self.tab_view_list == nil  then
        ...
        --    posx = 39
        --else
            --tab_view:setContentSize(cc.size(SuitShopTab.Width*count+10,64))
        --end
        ...
        tab_view:setInnerContainerSize(cc.size(SuitShopTab.Width*count+10,64))

        for i=1,count do
            self.tab_view_list[i] = SuitShopTab.new()
            self.tab_view_list[i]:setPosition(((i-1)*(SuitShopTab.Width+5))+91,26)
...
function SuitShopMainWindow:onCellTouched(cell)
    ...
    if cell_data.id == SuitShopMainWindow.buy_item_id then
        ...
        local str = string.format(TI18N("是否消耗 <img src='%s' scale=0.3 />%s 购买商品?"), iconsrc, cell_data.price)
...
SuitShopTab.Width = 147
SuitShopTab.Height = 51
...
function SuitShopTab:setName(bool)
	if self.name then
        if bool then
            self.name:setTextColor(Config.ColorData.data_new_color4[1])
            self.name:enableShadow(Config.ColorData.data_new_color4[2],cc.size(0, -2),2)
        else
            self.name:setTextColor(Config.ColorData.data_new_color4[6])
            self.name:disableEffect(cc.LabelEffect.SHADOW)
        end
	end
end
...
function SuitShopTab:setData(index,count,data)
	...
	--if index == 1 then
	--	self.select:loadTexture(PathTool.getResFrame("common","common_2010"), LOADTEXT_TYPE_PLIST)
	--	self.select:setFlippedX(true)
	--	self.normal:loadTexture(PathTool.getResFrame("common","common_2009"), LOADTEXT_TYPE_PLIST)
...
function SuitShopItem:configUI()
    ...
    if not self.goods_item then
	    self.goods_item = BackPackItem.new(nil,true,nil,0.8)
...
function SuitShopItem:showLimitCount(status, has_buy, limit_num)
    if status then
        if self.limit_tips == nil then
            self.limit_tips = createRichLabel(18, 175, cc.p(0.5,0.5), cc.p(160,20))
