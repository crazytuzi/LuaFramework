...
function BackPackWindow:__init(sub_type)
    --print("BackPackWindow!!!33vv322!!eee!!!111!!quick_sell_btn!!")
...
function BackPackWindow:open_callback()
    ...
    self.title_img = self.mainContainer:getChildByName("title_img")
    if self.title_img ~= nil then
        self.title_img:loadTexture(PathTool.getPlistImgForDownLoad("bigbg","bigbg_100",false), LOADTEXT_TYPE)
    end
    ...
    local line = self.root_wnd:getChildByName("line")
    self.tableContainer = self.container:getChildByName("tab_container")
    local bgSize = self.tableContainer:getContentSize()
    line:setContentSize(cc.size(bgSize.width,50))

    local scroll_view_size = cc.size(bgSize.width-160, bgSize.height+10)
    local setting = {
        ...
        space_y = 0,                   -- y方向的间隔
        item_width = 154,               -- 单元的尺寸width
        item_height = 64,              -- 单元的尺寸height
        row = 1,                        -- 行数，作用于水平滚动类型
        col = 0,                         -- 列数，作用于垂直滚动类型

    }
    self.tab_scrollview = CommonScrollViewLayout.new(self.tableContainer, cc.p(140, 0) , ScrollViewDir.horizontal, ScrollViewStartPos.bottom, scroll_view_size, setting)
    local tab_setting = {}
    tab_setting.default_index = self.default_index or BackPackConst.item_tab_type.PROPS
    tab_setting.tab_size = cc.size(154, 47)
    tab_setting.select_color = cc.c4b(255,255,255,255)
    tab_setting.select_outline = cc.c4b(133,64,0,0)
    tab_setting.normal_color = cc.c4b(37,81,116,0xff)
    tab_setting.normal_outline = cc.c4b(255,255,255,0)
    tab_setting.select_res = PathTool.getResFrame("backpack", "common_1011")
    tab_setting.normal_res = PathTool.getResFrame("backpack", "common_1012")
    --tab_setting.img_rect = cc.rect(47, 40, 0, 0)
    tab_setting.tab_name = "tab_btn_"
    tab_setting.title_offset = cc.p(-6,-4)
    ...
    local scroll_view_size = cc.size(680,640)
    local setting = {
    ...
    self.item_scrollview = CommonScrollViewLayout.new(self.container, cc.p(24, 105), ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function BackPackWindow:register_event()
    ...
    registerButtonEventListener(self.quick_sell_btn, function()
        ...
        elseif self.cur_index == BackPackConst.item_tab_type.HERO then    --宝可梦碎片
...
function BackPackWindow:setPanelData()
    ...
    elseif self.cur_index == BackPackConst.item_tab_type.HERO then --宝可梦
    ...
    if self.cur_index == BackPackConst.item_tab_type.HERO then
        ...
        self.item_scrollview:setSpace(30)
    ...
    if self.cur_index == BackPackConst.item_tab_type.HERO then
        self.quick_sell_btn:getChildByName("label"):setString(TI18N("分解"))
    else
        self.quick_sell_btn:getChildByName("label"):setString(TI18N("一键出售"))
    end
...
function BackPackWindow:selectedItem(cell, force)
    if cell and cell:getData() then 
        ...
        elseif vo.sub_type == 3 then --宝可梦碎片
