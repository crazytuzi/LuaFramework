...
BackPackItem.Width = 114
BackPackItem.Height = 115
...
--[[	if self.need_num_bg == nil  then
	    ...
	    local size = size or cc.size(156,10)
	    self.need_num_bg:setContentSize(size)
	    --self.need_num_bg:setCapInsets(cc.rect(12, 10, 1, 1))

	end]]
	if self.need_num_label == nil then
		self.need_num_label = createLabel(22, Config.ColorData.data_new_color4[6], nil, x, y, "", self.root_wnd, nil, cc.p(0,0.5))
	end
	...
	if color and Config.ColorData.data_new_color4[color] then
		self.need_num_label:setTextColor(Config.ColorData.data_new_color4[color])
	end
...
function BackPackItem:IsGetStatus(bool,opacity,res)
	...
	if not self.is_get_select then
		local res = PathTool.getResFrame("common","common_2053")
		self.is_get_select = createImage(nil,res,self.size.width/2-2,self.size.height/2,cc.p(0.5,0.5),true,1,false)
...
function BackPackItem:showBiaoQian(bool,str)
	if not self.qian_icon then
		...
		self.qian_icon = createImage(self.main_container,res,33,88,cc.p(0.5,0.5),true,10,true)
		self.qian_label = createLabel(20,Config.ColorData.data_color4[1],Config.ColorData.data_color4[9],34,30,"",self.qian_icon,2, cc.p(0.5,0))
...
--增加一个名字，默认位置为底部
function BackPackItem:setGoodsName(name,pos,font_size,color,line_color,scale,anchorpoint,size)
	if not name then return end
	font_size = font_size or 24
	color = color or 6
	local scale = scale or 1
	local item_color = Config.ColorData.data_new_color4[6]
	if type(color) == "number" then 
		item_color = Config.ColorData.data_new_color4[color]
	else
		item_color = color
	end
	if not self.goods_name  then
		local ap = anchorpoint or cc.p(0.5,0)
		self.goods_name = createLabel(font_size,item_color,line_color,self.Width/2,-30,"",self.root_wnd,2, ap)
		self.goods_name:setScale(1 / scale)
		if size ~= nil then
			self.goods_name:setDimensions(size.width, size.height)
			self.goods_name:setAlignment(cc.TEXT_ALIGNMENT_CENTER)
		end
	end
	name = name or ""
...
function BackPackItem:setReceivedIcon1( status )
	if status == false then
		...
		if self.lay_received == nil then
			...
			local res = PathTool.getResFrame("common","common_2053")
			self.lay_received = createImage(nil,res,self.size.width/2,self.size.height/2,cc.p(0.5,0.5),true,1,false)
            res = PathTool.getResFrame("mainui","mainui_1037")
...
function BackPackItem:setGotIcon( status )
	if status == false then
		...
		if self.got_icon == nil then
			self.got_icon = createImage(self, PathTool.getResFrame("common","common_2053"),BackPackItem.Width/2,BackPackItem.Height/2, cc.p(0.5, 0.5), true, 20, true)
...
function BackPackItem:setExtendDesc(status, desc, color, outcolor, is_show_bg, font_size)
	if status == false then
		...
		color = color or item_color or 6
		print("heaven_colorid2:"..color)
		...
		if outcolor == nil then
			desc = string.format( "<div fontcolor=%s fontsize=%d >%s</div>", Config.ColorData.data_new_color_str[color], font_size, desc)
		else
			desc = string.format( "<div fontcolor=%s fontsize=%d outline=1,%s >%s</div>", Config.ColorData.data_new_color_str[color], font_size, Config.ColorData.data_new_color4[outcolor], desc )
		end
...
function BackPackItem:setSummonNumber(label)
	if not self.summon_text then
		self.summon_text = createLabel(22,Config.ColorData.data_new_color4[6],nil,62,-15,"",self.main_container,nil, cc.p(0.5,0.5))
	end
...
function BackPackItem:grayStatus(bool,opacity,str)
	...
	if not self.gray_bg then
		local res = PathTool.getResFrame("common","common_2053")
		self.gray_bg = createImage(nil,res,self.size.width/2,self.size.height/2,cc.p(0.5,0.5),true,1,false)
...
function BackPackItem:setBoxSelected( status )
    if status then
        if self.box_select == nil then
            local res = PathTool.getResFrame("common","common_2041")
            self.box_select = createImage(self, res, self.size.width/2, self.size.height/2, cc.p(0.5, 0.5), true, -1, true)
            self.box_select:setScale(1.8)
        else
