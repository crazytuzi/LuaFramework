...
function ItemExhibitionView:open_callback()
    ...
    self.title_sprite = main_container:getChildByName("Sprite")
        ...
        delayRun(self.root_wnd, i*0.1, function() 
            ...
            local function animationEventFunc(event)
                if event.eventData.name == "appear" then
                    ...
                    if (item.item.setExtendDesc) then
                        item.item:setExtendDesc(false)
                    end
...
function ItemExhibitionView:handleEffect(status)
    if status == false then
        loadSpriteTexture(self.title_sprite, PathTool.getResFrame("common", "common_1091"), LOADTEXT_TYPE_PLIST)
    else
        --local effect_id = 274
        --local action = PlayerAction.action_3
        ...
        --end
        print('self.title_sprite',self.title_sprite)

        loadSpriteTexture(self.title_sprite, PathTool.getPlistImgForDownLoad("common/txt_common", "txt_common_1"), LOADTEXT_TYPE )
