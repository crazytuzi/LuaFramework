...
function GuildController:requestExitGuild()
    ...
    if role_vo.position == GuildConst.post_type.leader then         -- 自己是帮主，则是解散公会
        ...
        end,TI18N("取消"),nil,nil,nil,{timer=3, timer_for=true, off_y = 43, title = TI18N("解散公会"), extend_str = extend_msg, extend_offy = -25, extend_aligment = cc.TEXT_ALIGNMENT_CENTER },nil,nil)
    else
        ...
        end, TI18N("取消"), nil, nil, nil, {timer = 3, timer_for = true, off_y = 43, title = TI18N("退出公会"), extend_str = extend_msg, extend_offy = -25, extend_aligment = cc.TEXT_ALIGNMENT_CENTER }, nil, nil) 
    end
end
