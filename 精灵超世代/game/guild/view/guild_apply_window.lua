...
function GuildApplyWindow:open_callback()
    ...
    local txt_desc = createRichLabel(18, cc.c3b(0x3d, 0x50, 0x78), cc.p(0.5, 0.5), cc.p(320, 15), nil, nil, 600)
    main_view:addChild(txt_desc);
    txt_desc:setString(TI18N("可通过点击列表中玩家的头像查看玩家信息哦"))
...
function GuildApplyWindow:updateApplyList()
    ...
    if next(list) == nil then
        ...
        if self.scroll_view == nil then
            ...
            self.scroll_view = CommonScrollViewLayout.new(self.item_gods, cc.p(0, 5), nil, nil, item_gods_size,
                list_setting)
        end
