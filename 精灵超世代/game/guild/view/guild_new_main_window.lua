...
function GuildNewMainWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getPlistImgForDownLoad("bigbg/guild","guild_bg_1"), type = ResourcesType.single },
	}
	self.icon_list = {} --图标
	self.effect_list = {} --特效
	self.npc_list = {} --特效
...
function GuildNewMainWindow:createAllSpines()
	local node = self.guild_bg or self.root_wnd:getChildByName("bg")

	local skel_path = "res/spine/dc_bkm_jiqiren/bkm_jiqiren.skel"
	local atlas_path = "res/spine/dc_bkm_jiqiren/bkm_jiqiren.atlas"
	...
	spine:setPosition(cc.p(335, 143))
	node:addChild(spine)

	local skel_path = "res/spine/dc_bck_qita/bkm_qita.skel"
	local atlas_path = "res/spine/dc_bck_qita/bkm_qita.atlas"
	local animName = "animation"
	local spine = spSkeletonAnimationCreate(skel_path, atlas_path, pixelformal)
	spine:setAnimation(0, animName, true)
	spine:setPosition(cc.p(540, 636))
	node:addChild(spine)
end
...
function GuildNewMainWindow:open_callback()
	...
	self.guild_sign_value:setTextAreaSize(cc.size(267, 130))
	...
	self.rank_btn:getChildByName("title"):setString(TI18N("排名"))
	...
	self.check_member_btn:getChildByName("title"):setString(TI18N("公会管理"))
	...
	self.notice_btn:getChildByName("title"):setString(TI18N("日志"))
...
function GuildNewMainWindow:openRootWnd()
	...
	self.guild_bg:loadTexture(PathTool.getPlistImgForDownLoad("bigbg/guild","guild_bg_1"), LOADTEXT_TYPE)
	...
	for i,v in ipairs(tempArr) do
		if self.map_layer2 then
			if self.icon_list[v.type] == nil then
				...
				self.icon_list[v.type]:setAnchorPoint(0,0)
				...
				local bg = createImage(self.icon_list[v.type], PathTool.getResFrame("guild", "guild_2024"),0,0,cc.p(0, 0),true,nil,true)
				bg:setCapInsets(cc.rect(45, 1, 1, 1))
				self.icon_list[v.type].bg = bg

				local label = createLabel(20, Config.ColorData.data_new_color4[6], nil, 83/2, 80/2, v.name, self.icon_list[v.type], 1, cc.p(0.5, 0.5))
				local size = label:getContentSize()

				bg:setContentSize(cc.size(size.width+93,bg:getContentSize().height))

				self.icon_list[v.type]:setContentSize(cc.size(size.width+93,bg:getContentSize().height))
				label:setPosition((size.width+93)/2, bg:getContentSize().height/2)
	...
	if self.icon_list[GuildConst.icon_type.guild_icon_type_4].is_unlock == false then
		...
		if config then
			...
			if is_unlock == false then
				setChildUnEnabled(true, self.icon_list[GuildConst.icon_type.guild_icon_type_4], Config.ColorData.data_new_color4[18])
			else
