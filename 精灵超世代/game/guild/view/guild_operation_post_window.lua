...
function GuildOperationPostWindow:open_callback()
    ...
    local ld_lbl = self.setleader_btn:getChildByName("label")
    ld_lbl:setString(TI18N("转让会长"))
    setLabelAutoScale(ld_lbl,self.setleader_btn,10)
    ...
    setLabelAutoScale(self.setmember_btn_label ,self.setmember_btn,10)

    self.center_y = self.setmember_btn:getPositionY()

    self.kickout_btn = container:getChildByName("kickout_btn")                  -- 踢人
    local kk_lbl = self.kickout_btn:getChildByName("label")
    kk_lbl:setString(TI18N("移除出公会")) 
    setLabelAutoScale(kk_lbl,self.kickout_btn,10)
