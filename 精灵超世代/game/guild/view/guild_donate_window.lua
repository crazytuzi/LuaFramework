...
function GuildDonateWindow:__init()
    ...
    self.res_list = {{
        path = PathTool.getPlistImgForDownLoad("guild", "guild"),
        type = ResourcesType.plist
    }}
    self.awards_list = {}
    self.layout_name = "guild/guild_donate_window"
...
function GuildDonateWindow:open_callback()
    ...
    local main_view = self.root_wnd:getChildByName("main_container"):getChildByName("main_panel")
    ...
    self.close_btn = main_view:getChildByName("close_btn")
    ...
    if config then
        for i, v in ipairs(Config.GuildData.data_donate_box) do
            ...
            awards:setPosition(self.start_x + self.total_width * percent, 80)
    ...
    self.desc = createRichLabel(18, Config.ColorData.data_new_color4[16], cc.p(0, 0.5), cc.p(157, 794), nil, nil, 400)
...
function GuildDonateWindow:register_event()
    registerButtonEventListener(self.close_btn, handler(self, self.onClickCloseBtn), true,
        REGISTER_BUTTON_SOUND_CLOSED_TYPY)
            ...
            if event_type == ccui.TouchEventType.ended then
                if object.config then
                    if object.is_show_tips == true then
                        CommonAlert.showItemApply(TI18N("当前捐献活跃度奖励"), object.config.rewards, nil,
                            TI18N("确定"), nil, nil, TI18N("奖励"), nil, nil, true)
                    else
    ...
    if self.update_donate_box_status_event == nil then
        self.update_donate_box_status_event = GlobalEvent:getInstance():Bind(GuildEvent.UpdateDonateBoxStatus,
            function(id)
                if id ~= nil then
                    ...
                    if object then
                        ...
                        if not tolua.isnull(object.box) then
                            self:SetBoxStatus(object)
                        end
...
function GuildDonateWindow:onClickCloseBtn()
...
function GuildDonateWindow:updateDonateBoxList()
    ...
    for i, object in ipairs(self.awards_list) do
        ...
        if not tolua.isnull(object.box_effect) then
            ...
            local box = createImage(object.container, PathTool.getResFrame("common", "common_1149"), 42, 0,
                cc.p(0.5, 0), true, 1)
            object.box = box
            local effect = createImage(object.container, PathTool.getResFrame("common", "common_1150"), 42, -20,
                cc.p(0.5, 0), true, 0)
...
function GuildDonateWindow:SetBoxStatus(obj)
    if obj.status == GuildConst.status.activity then
        setChildUnEnabled(false, obj.box)
        obj.effect:setVisible(false)
    elseif obj.status == GuildConst.status.un_activity then
        setChildUnEnabled(false, obj.box)
        obj.effect:setVisible(true)
    elseif obj.status == GuildConst.status.finish then
        setChildUnEnabled(true, obj.box)
        obj.effect:setVisible(false)
    end
end
...
function GuildDonateItem:ctor()
    ...
    self.title_desc = createRichLabel(18, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(420, 150), nil, nil,
        300)
    container:addChild(self.title_desc)
        ...
        if self.data.gain ~= nil then
            ...
            for i, v in ipairs(self.data.gain) do
                if self.awards_list[i] == nil then
                    ...
                    if item_config ~= nil then
                        _y = 96 - (i - 1) * 50
                        self.awards_list[i] = createRichLabel(22, Config.ColorData.data_new_color4[15], cc.p(0, 0.5),
                            cc.p(266, _y), nil, nil, 250)
                        self.root_wnd:addChild(self.awards_list[i])
                        self.awards_list[i]:setString(string.format(
                            "<img src=%s visible=true scale=0.3 /> <div outline=2,#3d5078>%s</div>",
                            PathTool.getItemRes(item_config.icon), v[2]))
                    end
                end
            end
        end

        local res_id = PathTool.getTargetRes("guild/txt_guild", string.format("txt_cn_guild_100%s", self.data.id),false)
        if self.res_id ~= res_id then
            self.res_id = res_id
            loadSpriteTexture(self.img, res_id,LOADTEXT_TYPE)
        end
    end
end
...
function GuildDonateItem:updateDonateStatus()
    ...
    if status == true then -- 已经没有捐献次数的情况
        ...
        -- self.donate_btn_label:enableOutline(Config.ColorData.data_new_color4[4], 2)
        self.donate_btn_label:enableShadow(Config.ColorData.data_new_color4[2], cc.size(0, -2), 2)
        self.donate_btn_label:setString(TI18N("捐献"))
        self.donate_btn:setVisible(true)
        self.pass_donate:setVisible(false)
    end
