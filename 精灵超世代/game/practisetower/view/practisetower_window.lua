...
function PractiseTowerWindow:open_callback()
    ...
    self.buy_tips = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(0.5,0.5), cc.p(93,25), nil, nil, 600)
    ...
    self.rank_desc_label = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(0,0.5), cc.p(35,17), nil, nil, 400)
    ...
    self.item_scrollview:setPositionX(83)

    self.black_bg:setPositionY(bottom_y)
    self.top_panel:setPositionY(top_y -165)
...
function PractiseTowerWindow:updateCount()
    ...
    local have_buycount = self.ctrl:getModel():getBuyCount() or 0
    local str = string.format(TI18N("今日可购买次数：<div fontcolor=#0cff01>   %s</div>"), have_buycount)
    ...
    if self.pt_data and self.pt_data.last_unixtime-GameNet:getInstance():getTime() <= 0 then
        ...
        self.rank_container:setPositionY(218)
    end
end
...
function PractiseTowerWindow:updateRank()
    ...
    self.rank_desc_label:setString(string.format(TI18N("当前排名：<div fontcolor=#0cff01>%s</div>"),my_rank ))--fontcolor=#fff5f0 href=xxx
end
