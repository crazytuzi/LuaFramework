...
function NewCoseList:createNewMessage()
    ...
    self.new_message = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(0.5,0.5), cc.p(msg_bg_size.width/2, msg_bg_size.height/2), nil, nil, 400)
    self.new_message:setString("")
...
function NewCoseList:updateMsg(force)
    ...
    if not force and not self.is_show_monkey and self.is_bottom == false then
        if self:isShowNewMsgChannel(self.channel) == true then
            ...
            local str = string.format(TI18N("<div click='xxx' outline=2,#000000>%s条未读信息  </div><img src='%s'/>"),self.new_message_num, PathTool.getResFrame("mainui","mainui_2007"))
            self.new_message:setString(str)
            return
        end
        --message("有信息来啦")
    end
...
function NewCoseList:showMonkey(bool, close_tips)
    ...
    if not self.no_people then
        ...
        self.no_people =  createLabel(26,Config.ColorData.data_new_color4[6],nil,self:getContentSize().width/2, self:getContentSize().height/2-90,close_tips or TI18N("暂时没有人说话"),self)
