...
function ArenateamMainWindow:open_callback(  )
    ...
    self.top_line = self.panel_bg:getChildByName("Image_1_0")
    ...
    self.open_tips_time = createRichLabel(22, cc.c4b(0x3d,0x50,0x78,0xff), cc.p(0.5, 0.5), cc.p(360, 347),nil,nil,1000)
...
function ArenateamMainWindow:setFightBtnIcon(status)
    ...
    local res
    if status == 1 then --组队
        res = PathTool.getResFrame("arenateam","arenateam_3")
...
function ArenateamMainItem:setData(data, index)
    ...
    self.power:setString(changeBtValueForPower(data.team_power))
