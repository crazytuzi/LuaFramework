...
function StarTowerMainWindow:open_callback()
    ...
    local fight_btn_size = self.fight_btn:getContentSize()
    self.fight_btn_label = createRichLabel(22, 0, cc.p(0.5, 0.5), cc.p(fight_btn_size.width/2, fight_btn_size.height/2))
    self.fight_btn:addChild(self.fight_btn_label)
    self.fight_btn_label:setString(string.format("<div fontColor=#ffffff fontsize=22 shadow=0,-3,3,%s>%s</div>", Config.ColorData.data_new_color_str[2], TI18N("挑战")))
    ...
    local clean_btn_size = self.clean_btn:getContentSize()
    self.clean_btn_label = createRichLabel(22, 0, cc.p(0.5, 0.5), cc.p(clean_btn_size.width/2, clean_btn_size.height/2))
    self.clean_btn:addChild(self.clean_btn_label)
    ...
    self.no_label =  createLabel(22,Config.ColorData.data_new_color4[6],nil,435,55,"",self.main_panel,nil,cc.p(0,0.5))
    self.no_label:setString(TI18N("挑战成功可进行扫荡"))
    self.no_label:setVisible(false)
    local no_label_width = 200
    local no_label_size = self.no_label:getContentSize()
    local rows = math.ceil(no_label_size.width / 200)
    local no_label_height = no_label_size.height * rows
    self.no_label:setDimensions(no_label_width, no_label_height)
...
function StarTowerMainWindow:changeClearTitleText()
    ...
    if count > 0 then
        self.clean_btn_label:setString(string.format(TI18N("<div shadow=0,-3,3,%s>扫荡</div>"),Config.ColorData.data_new_color_str[3]))
    else
        ...
        if buy_data then
            ...
            local str = string.format(TI18N("<img src=%s scale=0.3 visible=true /><div shadow=0,-3,3,%s>%s%s</div>"),PathTool.getItemRes(item_data.icon),Config.ColorData.data_new_color_str[3],buy_data.expend[1][2],TI18N("扫荡"))
            self.clean_btn_label:setString(str)
        else
            --容错的
            self.clean_btn_label:setString(string.format(TI18N("<div shadow=0,-3,3,%s>扫荡</div>"),Config.ColorData.data_new_color_str[3]))
        end
...
function StarTowerMainWindow:updateVideoData(data)
    ...
    if not self.fast_desc then 
        self.fast_desc = createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(20,-22),nil,nil,500)
        self.top_panel:addChild(self.fast_desc)
    end

    if not self.power_desc then 
        self.power_desc = createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(20,-62),nil,nil,500)
    ...
    for i,v in pairs(list) do
        if v and v.type == 1 then 
            local str = string.format(TI18N("最快通关：<div fontcolor=%s>%s</div>"),Config.ColorData.data_new_color_str[12],v.name)
            self.fast_desc:setString(str)
        else
            local str = string.format(TI18N("最低战力：<div fontcolor=%s>%s</div>"),Config.ColorData.data_new_color_str[12],v.name)
...
function StarTowerMainWindow:updateDesc()
    if not self.data then return end
    --boss描述
    self.boss_desc = createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(20,75),nil,nil,620)
    self.top_panel:addChild(self.boss_desc)
    --推荐战力
    self.boss_power =createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,0),cc.p(20,0),nil,nil,620)
    ...
    local str = string.format(TI18N("推荐战力：<div fontcolor=%s>%s</div>"),Config.ColorData.data_new_color_str[11],power)
