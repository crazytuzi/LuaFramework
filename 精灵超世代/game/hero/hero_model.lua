...
function HeroModel:config()
    ...
    --宝可梦信息界面 升星页签的参数  6星才限时页签(后面策划要求熔炼祭坛的也加入)
    ...
    --穿戴在宝可梦身上的神装信息结构 self.hero_holy_list[id] = good_vo
    self.hero_holy_list = {}

    --穿戴在宝可梦身上的神装ID信息结构 self.dic_itemid_to_partner_id[id] = partner_id
...
-------------------宝可梦信息--------------------------
--更新宝可梦信息列表 not_show_power:不显示战力变化提示
...
function HeroModel:updateHeroVo(info, not_show_power, is_detail_info)
    ...
    if not is_detail_info then
        --获取数据缺少的项 配置表信息
        --由于出现 camp_type 的类型不对问题..暂时把此赋值开放..查看导致宝可梦数据bug是否还出现
        local bid = info.bid
    ...
    if not is_add then
        ...
        if not is_detail_info then
            ...
            elseif info.break_lev and hero_vo.break_lev < info.break_lev then 
                if not self:isResonateCystalHero(info) then --不是共鸣水晶上面的宝可梦才计算进阶逻辑
                    open_type = 2
                    old_vo = clone(hero_vo)
                end
            end
...
--宝可梦上限
function  HeroModel:setHeroMaxCount(count)   --宝可梦上限
    if count then
        self.hero_max_count = count 
    end
end
--获取宝可梦上限
--return 宝可梦上限, 当前宝可梦数量
function  HeroModel:getHeroMaxCount()   --宝可梦上限
...
function HeroModel:getHeroPokedexByBid(key)
    ...
    if not data then 
        if PLATFORM_NAME == "demo" or PLATFORM_NAME == "release" then
            message(string.format("宝可梦图鉴没有[%s](bid_star)的数据", tostring(key)))
        end
        return nil  
    end
...
function HeroModel:isOpenHolyEquipMentOldByHerovo(hero_vo, is_show_tips)
    if not hero_vo then return false end
    --旧的数据记录一下
    -- ["open_star_condition"] = {val=6, desc="神装开启所需宝可梦星级"},
...
function HeroModel:createStar(num, star_con, star_setting, star_width, scale)
    local num = num or 0
    ...
    local _cStar = function(star_count, res, star_list, scale)
        local scl = scale or 1
        local width = star_width or (42 + 3)
    ...
    elseif num >= 10 then
        ...
        if new_num > 0 then
            if star_setting.star_label == nil then
                ...
                star_setting.star_label = createLabel(16,Config.ColorData.data_new_color4[1],Config.ColorData.data_new_color4[5],size.width * 0.5 - 2, size.height * 0.5,"10",star_setting.star10, 1, cc.p(0.5,0.5))
            else
...
function HeroModel:getHolyEquipSuitDes(equip_vo_list)
    ...
    for i,v in ipairs(suit_config) do
        ...
        if config then
            ...
            data.name = string_format(TI18N("%s(%s件套)"), v.name, v.num)
            data.id  = eqm_set 
...
--是否共鸣水晶上阵的宝可梦 如果 hero_vo 不确定是 hero_vo 类的 用此方法判断
