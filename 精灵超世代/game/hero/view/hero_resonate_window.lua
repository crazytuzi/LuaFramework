...
function HeroResonateWindow:open_callback()
    ...
    self.lev = createRichLabel(20, cc.c4b(0xff,0xff,0xff,0xff), cc.p(0.5, 0.5), cc.p(64,0),nil,nil,600)
    ...
    for i=1,3 do
        ...
        if tab_btn then
            ...
            object.label:setString(tab_name_list[object.index])
    ...
    registerButtonEventListener(self.look_btn, function(param,sender, event_type) 
        ...
        if self.select_index then
            if self.select_index == HeroConst.ResonateType.eResonate then
                ...
                if config then
                    TipsManager:getInstance():showCommonTips(config.desc, sender:getTouchBeganPosition(),20,nil,500)
                end
            ...
            elseif self.select_index == HeroConst.ResonateType.eStoneTablet then
...
function HeroResonateWindow:updateLevelInfo(scdata)
    if model:isResonateCystalMaxLev() then
        if scdata.is_break == 1 then
            self.lev:setString(string_format("<div fontcolor=#fff99e ><div fontcolor=#0cff01 >%s</div>/%s</div>", scdata.lev, scdata.max_cystal_lev))
        else
            self.lev:setString(string_format("<div fontcolor=#fff99e >%s</div>", scdata.lev))
        end
    else
        self.lev:setString(string_format("<div fontcolor=#fff99e >%s</div>", scdata.lev))
    end 
