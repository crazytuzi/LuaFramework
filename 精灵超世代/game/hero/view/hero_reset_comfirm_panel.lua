...
function HeroResetComfirmPanel:open_callback()
    ...
    self.top_val = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(337,335),5,nil,660)
    container:addChild(self.top_val)
    self.dec_val = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(337,120),5,nil,660)
    container:addChild(self.dec_val)
    self.count_label = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(1, 0.5), cc.p(660,50),5,nil,250)
    container:addChild(self.count_label)

    self.hero_item = HeroExhibitionItem.new(1, true)
    self.hero_item:setPosition(123, 218)
...
function HeroResetComfirmPanel:openRootWnd(hero_vo)
    ...
    self:upateShowList()
    local top_str = string_format(TI18N("<div fontcolor=#d63636>%s</div>重生为1级,<div fontcolor=#1db116>100%%</div>返还<div fontcolor=#1db116>升级, 进阶</div>耗材"), hero_vo.name)
    self.top_val:setString(top_str)
    self.dec_val:setString(TI18N("100级或以下的宝可梦才能重生"))
    ...
    self.count_label:setString(string_format(TI18N("今日剩余<div fontcolor=#1db116>%s</div>次"), self.count))
    self:updateBtnTime()
end
