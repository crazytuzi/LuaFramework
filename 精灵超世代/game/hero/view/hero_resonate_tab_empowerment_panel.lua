...
function HeroResonateTabEmpowermentPanel:layoutUI()
    ...
    self.hero_item_tips = createRichLabel(16, Config.ColorData.data_new_color4[6], cc.p(0.5,1),cc.p(0, -60),nil,nil,150)
    ...
    self.power = CommonNum.new(20, self.power_click, 99999, -2, cc.p(0.5, 0.5))
    self.power:setPosition(100,36)
    ...
    self.hero_star_tips = createRichLabel(18, Config.ColorData.data_new_color4[15], cc.p(0.5,0.5),cc.p(360, 370),nil,nil,500)
    self.bottom_panel:addChild(self.hero_star_tips)


    self.change_count_label = createRichLabel(18, cc.c4b(0xff,0xff,0xf,0xff), cc.p(1,0.5),cc.p(706, 220),nil,nil,1900)
...
function HeroResonateTabEmpowermentPanel:onClickSelectHeroBtn()
    ...
    setting.tips = string_format(TI18N("请选择下列<div fontcolor=#249003>%s星</div>至<div fontcolor=#249003>%s星</div>的宝可梦作为赋能创造的取样"), star1, star2)
...
function HeroResonateTabEmpowermentPanel:setPowerValue(value)
    ...
    self.power:setNum(changeBtValueForPower(value))
end
...
function HeroResonateTabEmpowermentPanel:setScdata(data)
    ...
    if self.change_count_label then
        local str = string_format(TI18N("<div fontcolor=#fff99e>剩余赋能次数:</div><div fontcolor=#0cff01 > %s<div>"), data.num)
    ...
    if self.scdata and self.scdata.star then
        ...
        -- if count >=5 then
            if star >= 30 then --ff,0xea,0xc6
                self.hero_star_tips:setString(string_format(TI18N("<div>赋能的目标星级为供奉宝可梦历史最高平均星级(当前<div fontcolor=#0cff01 >%s</div>星)</div>"), self.scdata.star))
                if self.select_hero_vo == nil then
                    self.hero_item_tips:setString(TI18N("<div>请放入赋能\n对象宝可梦<div>"))
                    self.hero_item_tips:setPositionX(0)
                    ...
                    self.resonate_btn_label:enableOutline(Config.ColorData.data_new_color4[2], 2)
                    self.resonate_btn:setTouchEnabled(true)
                    self.resonate_btn_label:setString(TI18N("赋能创造"))
                end

            else
                self.hero_item_tips:setString(string_format(TI18N("<div >需圣阵中宝可梦\n总星级达30(%s/30)</div>"), star))
        ...
        --     self.hero_item_tips:setString(string_format(TI18N("<div fontcolor=#f23b3b outline=2,#000000>需圣阵中宝可梦\n总星级达30(%s/30)</div>"), star))
        --     self.hero_item_tips:setPositionX(-60)
        --     self.hero_star_tips:setString("")
        --     self.resonate_btn_label:setString(TI18N("暂无赋能对象"))
        --     self.resonate_btn_label:disableEffect(cc.LabelEffect.OUTLINE)
