...
function ArtifactListWindow:open_callback()
    ...
    self.desc_label = createLabel(22,Config.ColorData.data_new_color4[6],nil,310,750,TI18N("穿戴宝石后宝可梦可获得宝石携带的被动技能"),self.container)
    self.desc_label:setAnchorPoint(cc.p(0.5,0.5))
    self.desc_label:setDimensions(600, 50)
    self.desc_label:setAlignment(cc.TEXT_ALIGNMENT_CENTER, cc.VERTICAL_TEXT_ALIGNMENT_CENTER)

    if self.item_scrollview == nil then
        local scroll_view_size = cc.size(620,620)
        local setting = {
            start_x = 5,                     -- 第一个单元的X起点
            space_x = 0,                     -- x方向的间隔
            start_y = 0,                     -- 第一个单元的Y起点
