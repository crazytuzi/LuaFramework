...
function ArtifactTipsWindow:openRootWnd(data, open_type, partner_id, pos, is_off_tips, setting)
    ...
    if self.open_type == PartnerConst.ArtifactTips.partner then
        ...
        self.tab_list[3]:getChildByName("text"):setString(TI18N("卸下"))
        self.tab_list[3].index = PartnerConst.Artifact_Type.Getoff
        --self.tab_list[4]:setTitleText(TI18N("替换"))
        self.tab_list[4]:getChildByName("text"):setString(TI18N("替换"))
    ...
    elseif self.open_type == PartnerConst.ArtifactTips.sell then
        ...
        self.tab_list[1]:getChildByName("text"):setString(TI18N("放入"))
    ...
    self:setSkillInfo()
    self.power_label:setNum(changeBtValueForPower(self.score_val))
    --限购信息
    ...
    for i,v in ipairs(self.data.attr) do
        ...
        if attr_name then
            if not self.base_list[index] then 
                self.base_list[index] = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(20, 28), nil, nil, 380)
            ...
            local attr_str = string_format("<img src='%s' scale=1 /> <div fontcolor=#3d5078> %s：</div><div fontcolor=#0e7709>%s</div>", PathTool.getResFrame("common", icon), attr_name, attr_val)
...
function ArtifactTipsWindow:createSkillItem(index, parent)
    ...
    local name = createLabel(24,Config.ColorData.data_new_color4[9],nil,140, 96,"",parent,1,cc.p(0,0))
    local desc = createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(140, 88),4,nil,260)
