...
function HeroTalentSkillLearnPanel:open_callback()
    ...
    self.skill_desc = createRichLabel(18, Config.ColorData.data_new_color4[6], cc.p(0, 1), cc.p(0,0),nil,nil,440)
...
function HeroTalentSkillLearnPanel:createTitleItem(name,x, y)
    ...
    item.bg = createScale9Sprite(res, x, y - self.title_height *0.5 , LOADTEXT_TYPE_PLIST, self.list_view.scroll_view)
    item.bg:setCapInsets(cc.rect(23, 0, 2, 42))
    item.bg:setContentSize(cc.size(400, 42))
    item.label = createLabel(26, Config.ColorData.data_new_color4[6], nil, x, y - self.title_height *0.5, name, self.list_view.scroll_view, nil, cc.p(0.5, 0.5))
end
...
function HeroTalentSkillLearnPanel:updateCellByIndex(cell, index)
    ...
    if skill_data and skill_data.config then
        ...
        if config then
            ...
            cell.skill_item:showName(true,config.name, nil,14,true)
