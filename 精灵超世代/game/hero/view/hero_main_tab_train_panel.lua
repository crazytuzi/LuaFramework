...
function HeroMainTabTrainPanel:layoutUI()
    ...
    self.level = createRichLabel(24, cc.c4b(46,70,118,255), cc.p(0, 0.5), cc.p(x,y),nil,nil,600)
    ...
    self.profession_icon:setPositionY(260)
    ...
    self.profession_name:setPositionY(260)
    self.introduce_label = createLabel(20, Config.ColorData.data_new_color4[6], nil, x, 235, "", self.tab_panel, 2, cc.p(0,0.5))

    self.look_btn = self.tab_panel:getChildByName("look_btn")
    self.look_btn:setPosition(680, 260)
    --技能scrollview
    ...
    for i,attr_str in ipairs(self.attr_list) do
        if self.attr_icon_list[i] then
            local res_id = PathTool.getWhiteAttrIconByStr(attr_str)
            local res = PathTool.getResFrame("common",res_id)
            loadSpriteTexture(self.attr_icon_list[i], res, LOADTEXT_TYPE_PLIST)   
        end
    end

    self.fuse_btn_label = createRichLabel(22,cc.c3b(0x24,0x90,0x03), cc.p(0.5,0.5),cc.p(500, 300), nil, nil, 400)
        ...
        for i,cost in ipairs(up_cost) do
            ...
            if self["level_up_cost"..i] then
                ...
                if dic_cost_list[cost[1]] and count < dic_cost_list[cost[1]] then
                    ...
                    self["level_up_cost"..i]:setTextColor(cc.c4b(214,54,54,255))
                else
                    self["level_up_cost"..i]:setTextColor(cc.c4b(14,119,9,255))
...
function HeroMainTabTrainPanel:showMaxLevelUI(status)
    if status then
        if self.max_level_img == nil then
            local res = PathTool.checkRes("resource/hero/txt_hero/txt_cn_hero_info_16.png")
            self.max_level_img = createImage(self.tab_panel, res, 172, 300,cc.p(0,0.5))
        else
