...
function HeroMainTabEquipPanel:layoutUI(power_click)
    ...
    for i=1,6 do
        ...
        local item = BackPackItem.new(false,true,nil,0.76,false)
        -- 引导需要
            ...
            if artifact_lock then
                if artifact_lock[1] == 'lev' then
                    if self.hero_vo.lev < artifact_lock[2] then
                        message(string_format( TI18N("宝可梦%s级解锁"),artifact_lock[2]))
                        return 
                    end
                elseif artifact_lock[1] == 'star' then
                    if self.hero_vo.star < artifact_lock[2] then
                        message(string_format( TI18N("宝可梦%s星解锁"),artifact_lock[2]))
                        return
                    end
                end
            end
    ...
    if self.lock_list then
        for i,v in ipairs(self.artifact_lock_list) do
            if not self.lock_list[i] then
                ...
                addRedPointToNodeByStatus(self.img_bg[i], is_redpoint, -1, -1)
                if self.img_bg[i].red_point then
                    self.img_bg[i].red_point:setScale(0.8 * 100/95)
                end
            else
                addRedPointToNodeByStatus(self.img_bg[i], false)
            end
        end
    end
end
    ...
    for i,v in ipairs(self.artifact_lock_list) do
        ...
        elseif v[1] == 'star' then
            ...
            if max_star and max_star < v[2] then
                ...
                self.img_bg[i]:setVisible(true)
                if self.img_bg[i].red_point then
                    self.img_bg[i].red_point:setVisible(false)
                end
                if item then
                    item:setVisible(true)
                end
