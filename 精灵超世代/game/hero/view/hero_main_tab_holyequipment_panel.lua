...
function HeroMainTabHolyequipmentPanel:layoutUI()
    ...
    for i=1,4 do
        ...
        local item = BackPackItem.new(false,true,nil,1,false)
        item:getBackground():setVisible(false)
        -- 引导需要
    ...
    self.tips_lab = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(1, 0.5), cc.p(self.look_btn:getContentSize().width, self.look_btn:getContentSize().height*0.5),10,nil,200)
...
function HeroMainTabHolyequipmentPanel:registerEvents()
    ...
    registerButtonEventListener(self.look_btn, function(param,sender, event_type) 
        ...
        if config then
            TipsManager:getInstance():showCommonTips(config.desc, sender:getTouchBeganPosition(), nil, nil, 530)
        end
    end ,true, 1)
    ...
    --     end)
    -- end
    -- --红点更新事件..宝可梦信息更新和 背包的信息更新 才能判断红点
...
function HeroMainTabHolyequipmentPanel:updateInfo(hero_vo)
    ...
    for i,_type in ipairs(self.equip_type_list) do
        ...
        if equip_vo then
            ...
            item:getBackground():setVisible(true)
            ...
            if config then
                item:setGoodsName(config.show_name, nil, 16, Config.ColorData.data_new_color4[1], Config.ColorData.data_new_color4[6])
            end
            ...
            item:setGoodsName("", nil, 16, Config.ColorData.data_new_color4[1], Config.ColorData.data_new_color4[6])
            item.equip_vo = nil
