...
-- @description:
--      宝可梦升星界面 点击 融合祭坛打开的界面  主要 4升5 5升6的
-- <br/> 2018年11月21日
--
-- --------------------------------------------------------------------
...
function HeroUpgradeStarFusePanel:ctor(hero_vo)
   ...
   -- 1 ~ 5星 星星列表
    ...
    --print("-------------test--->>",parnter_info[10101 + 100])
    --dump(parnter_info[10101 + 100],"所有宠物数据----------------------------》》")
end
...
function HeroUpgradeStarFusePanel:configUI()
    ...
    self.no_vedio_label:setString(TI18N("暂无此类型宝可梦"))
    --按钮
    ...
    --星星
    self.star_node = self.main_panel:getChildByName("star_node")
    self.star_node_cur = self.main_panel:getChildByName("star_node_cur")
    --模型
    self.mode_node = lay_hero:getChildByName("mode_node")

    --宝可梦信息进阶后形象
    local lay_hero_nex = self.main_panel:getChildByName("lay_hero_nex")
    self.mode_node_nex = lay_hero_nex:getChildByName("mode_node")

    --阵营
    self.camp_icon = self.main_panel:getChildByName("camp_icon")
    self.hero_name = self.main_panel:getChildByName("name")
    ...
    self.hero_comp_name = {}
    for i=1,4 do
        self.hero_comp_name[i] = createRichLabel(18, cc.c4b(0x92,0x0e,0xb3,0xff), cc.p(0.5,0.5), cc.p(0,0), nil, nil, 150)
...
function HeroUpgradeStarFusePanel:updateCampRedpoint()
 ...
 function HeroUpgradeStarFusePanel:updateBackGround()
     local bg_res = PathTool.getPlistImgForDownLoad("bigbg/hero","hero_upgrade_star_bg", true)
...
function HeroUpgradeStarFusePanel:updateHeadHeroInfo(fuse_data)
    ...
    --self:createStar(fuse_data.star,self.star_node)
    self:createStar(fuse_data.star,self.star_node,2)
    --绘画精灵模型
    if self.record_fuse_data == nil or self.record_fuse_data ~= fuse_data then
        local fuse_data_cur  =  deepCopy(fuse_data) or {}
        fuse_data_cur.star = fuse_data.star - 1
        fuse_data_cur.star_config.star = fuse_data.star_config.star - 1
        self:createStar(fuse_data_cur.star,self.star_node_cur,1)
        self:updateSpine(fuse_data_cur,self. mode_node_nex ,1)
        self:updateSpine(fuse_data,self.mode_node,2)
    end
    ...
    if self.show_hero_vo then
        ...
        for i,attr_str in ipairs(self.attr_list) do
            if self.attr_label_list[i] then
                ...
                self.attr_label_list[i]:setString(changeBtValueForHeroAttr(value, attr_str))
            end
        end
    end
    ...
    for i,item in ipairs(self.hero_item_list) do
        ...
        if hero_vo then
            ...
            if hero_vo.bid == 0 then
                ...
                str = string_format(TI18N("%d星宝可梦"),hero_vo.star)
            else
                str = string_format(TI18N("%s"),parnter_info[hero_vo.bid].name)
                item:setData(hero_vo)
            end
...
--更新星星显示
function HeroUpgradeStarFusePanel:createStar(num,_star_node,_index)
    local num = num or 0
    local width = 29 + 5 
    local star_setting = nil
    self.star_setting[_index] = model:createStar(num, _star_node , self.star_setting[_index], width)
end
 
--更新模型,也是初始化模型
function HeroUpgradeStarFusePanel:updateSpine(fuse_data,_model_node,_index)
    local fun = function()    
        if not self.model_tab[_index] then
            self.model_tab[_index] = BaseRole.new(BaseRole.type.partner, fuse_data, nil,{scale = 1})
            self.model_tab[_index]:setAnimation(0,PlayerAction.show,true) 
            self.model_tab[_index]:setCascade(true)
            self.model_tab[_index]:setPosition(cc.p(0,104))
            self.model_tab[_index]:setAnchorPoint(cc.p(0.5,0.5)) 
            _model_node:addChild(self.model_tab[_index])
            _model_node:setPositionY(66) 
            self.model_tab[_index]:setCascade(true)
            self.model_tab[_index]:setOpacity(0)
            self.model_tab[_index]:setScale(0.8)
            local action = cc.FadeIn:create(0.2)
            self.model_tab[_index]:runAction(action)
        end
    end
    if self.model_tab[_index]then
        doStopAllActions(self.model_tab[_index])
        self.model_tab[_index]:setCascade(true)
        local action = cc.FadeOut:create(0.2)
        self.model_tab[_index]:runAction(cc.Sequence:create(action, cc.CallFunc:create(function()
                doStopAllActions(self.model_tab[_index])
                self.model_tab[_index]:removeFromParent()
                self.model_tab[_index] = nil
                fun()
        end)))
    else
        fun()
    end
    --dump(self.model_tab,"这个多大---------》》")
end
...
function HeroUpgradeStarFusePanel:updateHeroList(select_camp, is_must_reset)
    ...
    if self.scroll_view == nil then
        local scroll_view_size = cc.size(640,186)
        local list_setting = {
            start_x = 0,
            space_x = 0,
            start_y = 0,
...
function HeroUpgradeStarFusePanel:DeleteMe()
    ...
    if next(self.model_tab) ~= nil then
        for i,v in pairs( self.model_tab ) do
             v:DeleteMe()
        end
        self.model_tab = {}
        self.spine = nil
