...
function HeroResonateComfirmLevPanel:openRootWnd(setting)
    ...
    local str = string_format("<img src='%s' scale=1 />%s",res,changeBtValueForPower(left_power))
...
function HeroResonateComfirmLevPanel:setPowerInfo(power)
    self.power = power
    local res = PathTool.getResFrame("common", "common_2016")
    local str = string_format("<img src='%s' scale=1 />%s",res,changeBtValueForPower(power))
