...
function HeroUpgradeStarSelectPanel:__init()
    ...
    self.res_list = {
        { path = PathTool.getPlistImgForDownLoad("hero/txt_hero","txt_cn_hero_temp_01"), type = ResourcesType.single },
...
function HeroUpgradeStarSelectPanel:open_callback()
    ...
    self.title:setString(TI18N("选择材料宝可梦"))
...
--@select_data 选择的数据
--@select_data.is_ignore_hero_hun 是否忽略宝可梦魂(圣物用)
--@ dic_other_selected 其他已选的宝可梦数据 dic_other_selected[partner_id] = hero_vo 模式
                ...
                for _,v in pairs(list) do
                    if v.config and v.config.camp_type == self.select_data.camp_type and v.config.eqm_jie == self.select_data.star then
                        for i=1, v.quantity do
                            local hero_vo = {}
                            hero_vo.id = -(i + v.config.camp_type * 10)  --因为宝可梦id是正序的..这里就负序
...
function HeroUpgradeStarSelectPanel:updateHeroList()
    if self.list_view == nil then
        ...
        local scroll_view_size = cc.size(600,200)
        local list_setting = {
            start_x = 30,
            space_x = 10,
            start_y = 5,
