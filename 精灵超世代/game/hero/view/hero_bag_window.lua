...
function HeroBagWindow:open_callback()
    ...
    self.img_girl = self.root_wnd:getChildByName("img_girl")
    self.img_girl:loadTexture(PathTool.getPlistImgForDownLoad("bigbg", "bigbg_101", false), LOADTEXT_TYPE)
    ...
    local tab_btn_type = {
        [1] = HeroConst.BagTab.eBagHero,     --宝可梦
    ...
    for i=1,4 do
        ...
        --self.select_btn.title:setFontSize(20)
        tab_btn.red_point = item:getChildByName("red_point")
        tab_btn.red_point:setVisible(false)

        --屏蔽精灵系统-----------------------
        if IS_HIDE_ELFIN then
            if i == HeroConst.BagTab.eElfin then
                item:setVisible(false)
            end
            if i == HeroConst.BagTab.eHalidom then
                item:setPositionX(440)
            end
        end
    ...
    self.embattle_btn:setPosition(609, 95)
...
function HeroBagWindow:onClickAddBtn()
    ...
    if config then 
        ...
        local str = string_format(TI18N("是否花费<img src='%s' scale=0.3 />%s扩充%s个宝可梦位置数量上限？"), iconsrc, count, config.add_num)
...
function HeroBagWindow:changeTabIndex(index)
    ...
    if self.select_btn then
        ...
        self.select_btn.title:disableEffect(cc.LabelEffect.SHADOW)
        --self.select_btn.title:setFontSize(20)
        self.select_btn.title:setTextColor(Config.ColorData.data_new_color4[6])
    end

    self.select_btn = self.tab_btn_list[index]
    if self.select_btn then 
        self.select_btn.select_bg:setVisible(true)
        --self.select_btn.title:setFontSize(22)
        self.select_btn.title:setTextColor(Config.ColorData.data_new_color4[1])
        self.select_btn.title:enableShadow(Config.ColorData.data_new_color4[2], cc.size(0, -2),2)
    end
    if self.library_btn then
        self.library_btn:setVisible(false)
    end

    if index == HeroConst.BagTab.eBagHero then --宝可梦页签
    ...
    self.embattle_btn:setVisible(index == HeroConst.BagTab.eBagHero)
...
function HeroBagWindow:openRootWnd(index, sub_type)
    ...
    if index == HeroConst.BagTab.eHalidom then -- 圣物需要判断是否已经开启，没开启则默认选中宝可梦分页
...
function HeroBagWindow:updateHeroList(select_camp)
    ...
    if not self.list_view then
        local scroll_view_size = cc.size(680,580)
        local setting = {
