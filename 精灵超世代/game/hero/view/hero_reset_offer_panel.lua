...
function HeroResetOfferPanel:open_callback()
    ...
    self.dec_val = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0, 1), cc.p(50,491),12,nil,580)
    ...
    self.cancel_btn_label:enableShadow(Config.ColorData.data_new_color4[2],cc.size(0, -2),2)
    ...
    self.confirm_btn_label:enableShadow(Config.ColorData.data_new_color4[3],cc.size(0, -2),2)
...
function HeroResetOfferPanel:onHeroComfirm()
    if self.is_show_tips then
        local str = TI18N("本次献祭含有5星或以上宝可梦，是否确认献祭？")
        CommonAlert.show( str, TI18N("确定"), function()
            if self.callback then
                self.callback()
            end    
        end, TI18N("取消"),nil,nil,nil,{timer=10, timer_for=true, off_y = 10, title = TI18N("宝可梦献祭"), extend_aligment = cc.TEXT_ALIGNMENT_CENTER })
    else
...
function HeroResetOfferPanel:onChipComfirm()
    if self.is_show_tips then
        local str = TI18N("本次献祭的碎片满足召唤宝可梦要求，献祭后可能会失去召唤的机会，是否继续？")
        ...
        --other_args.extend_str = TI18N("<div fontcolor=#249003 href=xxx>前往召唤宝可梦</div>")
...
--@reset_type --分解类型 1 宝可梦分解 2 碎片分解  3神装分解 4 10星置换 参考 HeroConst.ResetType
--dec 碎片分解的描述
function HeroResetOfferPanel:openRootWnd(hero_list, is_show_tips, callback, reset_type, dec)
    ...
    if reset_type == HeroConst.ResetType.eHeroReset then
        --宝可梦分解
        local  str = TI18N("献祭宝可梦可获得材料，若宝可梦有进行升级、进阶、升星培养，也会100%返还所消耗的进阶石、金币和宝可梦经验。本次献祭所得如下：")
        self.dec_val:setString(str)
        self.win_title:setString(TI18N("宝可梦献祭"))
        controller:sender11075(hero_list)   
    elseif reset_type == HeroConst.ResetType.eTenStarChang then
        --10星置换
        local str = TI18N("参与置换的5星宝可梦，若有进行升级培养，会100%返还培养材料（金币、宝可梦经验、进阶石）及身上装备与符文。本次置换返还所得如下：")
        self.dec_val:setString(str)
        self.win_title:setString(TI18N("宝可梦置换"))
    ...
    elseif reset_type == HeroConst.ResetType.eActionHeroReset then
        --活动宝可梦重生
        local str = TI18N("宝可梦重生可获得材料，若有进行升级培养，会100%返还培养材料（金币、宝可梦经验、进阶石）及身上装备与符文。本次置换返还所得如下：")
        local config =  Config.PartnerData.data_partner_const.reborn_desc3
        if config then
            str = config.desc
        end
        self.dec_val:setString(str)
        self.win_title:setString(TI18N("宝可梦重生"))
    ...
    elseif reset_type == HeroConst.ResetType.eHeroReturn then
        --宝可梦回退
        local str = TI18N("宝可梦回退可获得材料，若有进行升级培养，会100%返还培养材料（金币、宝可梦经验、进阶石）及身上装备与符文。本次置换返还所得如下：")
        ...
        self.win_title:setString(TI18N("宝可梦回退"))
    ...
    elseif reset_type == HeroConst.ResetType.eSpriteReturn then
        --精灵重生
        local str = TI18N("精灵重生会返还全部奥术之尘与等同于所消耗本体数量的宝可梦之魂，同时保留1个1级本体")
        self.dec_val:setString(str)
