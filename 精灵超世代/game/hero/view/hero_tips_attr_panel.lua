...
function HeroTipsAttrPanel:setData(hero_vo)
    for k,list in ipairs(self.attr_list) do
        for i,attr in ipairs(list) do
            ...
            attr.value:setString(changeBtValueForHeroAttr(value, attr_str))
        end
    end
    self:setBottomInfo()
end
...
function HeroTipsAttrPanel:setBottomInfo()
    ...
    self.guild_pvp_level_value:setString(string_format(TI18N("lv.%s(技能lv.%s)"), guild_pvp_attr_level, guild_pvp_skill_level))
