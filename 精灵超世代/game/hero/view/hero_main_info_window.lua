...
function HeroMainInfoWindow:__init()
   ...
   -- 1 ~ 5星 星星列表
    ...
    --显示模式 宝可梦模型 和 图鉴模式  定义参考 HeroConst.BagTab
...
function HeroMainInfoWindow:open_callback()
    ...
    self.power_label = self.power_click:getChildByName("power_label")
    --self.power_label:enableOutline(cc.c4b(255, 255, 255, 255), 2)
    self.power_label:enableShadow(cc.c4b(255, 255, 255, 255), cc.size(2, -2),2)
    self.fight_label = CommonNum.new(20, self.power_click, 99999, - 2, cc.p(0.5, 0.5))
    self.fight_label:setPosition(130, 42)
    self.fight_label:setRotation(-18);
    --影子来的
    ...
    for i=1,4 do
        ...
        tab_btn.title:setTextColor(cc.c4b(38, 82, 119, 255))
    ...
    self.hero_info_40 = self.main_panel:getChildByName("hero_info_30"):getChildByName("hero_info_40")
...
function HeroMainInfoWindow:changeTabType(index, check_repeat_click, sender)
    ...
    if self.cur_tab ~= nil then
        ...
        self.cur_tab.title:setTextColor(cc.c4b(38, 82, 119, 255))
    ...
    if _type == HeroConst.MainInfoTab.eMainUpgradeStar then
        self.hero_info_40:setContentSize(cc.size(720, 80))
    else
        self.hero_info_40:setContentSize(cc.size(720, 115))
    end

    if self.cur_tab ~= nil then
        --self.cur_tab.title:disableEffect(cc.LabelEffect.OUTLINE)
        --self.cur_tab.title:setTextColor(cc.c4b(0x60, 0x35, 0x1a, 0xff))
        self.cur_tab.title:setTextColor(cc.c4b(255, 255, 255, 255))
        self.cur_tab.title:enableShadow(cc.c4b(15, 115, 177, 255), cc.size(0, -2),2)
...
function HeroMainInfoWindow:showLevelUpAction()
    ...
    for i,v in ipairs(key_list) do
        ...
        if self.show_level_label_list[i] then
            local label = string_format("%s + %s", name, changeBtValueForHeroAttr(value, v))
...
function HeroMainInfoWindow:updateData()
    ...
    if self.show_model_type == HeroConst.BagTab.eBagHero then
        ...
        --因为修改 页签显示问题..兼容旧数据问题 改成打开宝可梦界面的时候申请 了
        ...
        for i,tab_btn in ipairs(self.tab_list) do
            if self.tab_type_list[i] then
                ...
                elseif tab_btn.type == HeroConst.MainInfoTab.eMainHolyequipment and self.is_holy_equp_lock then
                    ...
                    --tab_btn.title:setTextColor(cc.c4b(0xEE, 0xD1, 0xAF, 0xff))
                    --tab_btn.title:enableOutline(cc.c4b(0x53, 0x3D, 0x32, 0xff), 2)
                    tab_btn.title:disableEffect(cc.LabelEffect.SHADOW)
                    tab_btn.title:setTextColor(cc.c4b(38, 82, 119, 255))
                end
...
function HeroMainInfoWindow:updatePower()
    local power = changeBtValueForPower(self.select_hero_vo.power)
...
function HeroMainInfoWindow:createStar(num)
    local num = num or 0
    local width = 26
    self.star_setting = model:createStar(num, self.star_node, self.star_setting, width, 0.6)
end
