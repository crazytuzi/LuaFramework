...
function FormationSelectPanel:setData(formation_type)
    ...
    self.time_ticket = GlobalTimeTicket:getInstance():add(function()
        ...
        if self.config_list[index] ~= nil then
            ...
            self.item_list[index].con:setPosition( (index-1 + 0.5) * (item_width + 16), 140)
            index = index + 1
        else
            self:clearTimeTicket()
        end
...
function FormationSelectPanel:createItem(config, item_width, index, role_lev)
    ...
    --阵法icon
    local res = "res/resource/form/form_form_icon_"..config.type..".png"
    item.icon = createImage(item.con, res,  size.width/2,size.height/2, cc.p(0.5,0.5), false, 0, false)
    ...
    if is_select == true then
        ...
        local cur_name = createLabel(18,Config.ColorData.data_new_color4[1],nil, 20, 39, TI18N("当前"), item.cur_img, nil, cc.p(0.5,0.5),nil)
        cur_name:enableOutline(Config.ColorData.data_new_color4[11], 2)
    ...
    --阵法名字 --68452a
    item.name = createLabel(16,Config.ColorData.data_new_color4[6],nil, item_width * 0.5,-27 ,config.name, item.con,nil, cc.p(0.5,0.5),nil)
    ...
    if role_lev < config.need_lev and self.team_index ~= 999 then --999 新手训练营特殊类型
        ...
        createLabel(22, Config.ColorData.data_new_color4[1], Config.ColorData.data_new_color4[9], size.width/2,10,str, item.con, 2, cc.p(0.5,0))
