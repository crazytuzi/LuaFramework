...
function HeroBreakExhibitionWindow:updateAttrList()
    ...
    local show_list = {} 
    --战斗力
    local res = PathTool.getResFrame("common","common_90036")
    ...
    for i,v in ipairs(show_list) do
        if not self.attr_list[i] then 
            ...
            --    bg:setOpacity(160)
            --end
            local now_label =createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0,0), cc.p(150,300-i*50), 0, 0, 400)
            self.main_container:addChild(now_label) 
            local next_label = createRichLabel(22, Config.ColorData.data_new_color4[12], cc.p(0,0), cc.p(450,300-i*50), 0, 0, 400)
            self.main_container:addChild(next_label) 
            local res = PathTool.getResFrame("common","common_90017")
            local arrow =  createImage(self.main_container, res,350,240-(i-1)*50, cc.p(0,0), true, 0, false)
