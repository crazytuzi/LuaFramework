...
function HeroSacrificePanel:configUI()
    ...
    -- self.main_panel:getChildByName("win_title"):setString(TI18N("宝可梦献祭"))
    ...
    self.disband_btn_lable:setString(TI18N("放 生"))
...
function HeroSacrificePanel:register_event()
    ...
    if not self.del_hero_event  then
        ...
        self.lab_put_count:setString(string_format(TI18N("已放入宝可梦:%s/%s"),self.select_count, self.select_max_count))
        end)
    end
...
function HeroSacrificePanel:_onClickBtnPutIn()
    ...
    if self.select_count >= self.select_max_count then
        message(string_format(TI18N("每次最多可放生%s个宝可梦"), self.select_max_count))
        return
    end 
    local can_count = self:checkAutoPutCount()
    if can_count == 0 then
        message(TI18N("快速放入只能放入1~3星宝可梦"))
        return
    end
...
function HeroSacrificePanel:updateHeroList(select_camp, reset)
    ...
    if not self.list_view then
        local scroll_view_size = cc.size(640,248)
        self.hero_setting = {
        ...
        self.list_view = CommonScrollViewSingleLayout.new(self.disband_container, cc.p(x, y-110) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, self.hero_setting, cc.p(0.5,0))
    ...
    self.lab_have_count:setString(string_format(TI18N("%s"), #self.show_list))
