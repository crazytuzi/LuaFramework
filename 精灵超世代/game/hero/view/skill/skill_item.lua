...
function SkillItem:configUI()
    ...
    if self.show_lev then
        ...
        self.num_label = createLabel(20,cc.c4b(255,255,255,255),nil,8,107,"00",self,1, cc.p(0.5,0.5))
...
function SkillItem:setData(data)
    ...
    if self.data == nil then
        ...
        self:showName(false)
        self.background_res_id =  PathTool.getResFrame("common", "common_1005_1")--PathTool.getQualityBg(0)
        ...
        local background_res_id = PathTool.getResFrame("common", "common_1005_1")--PathTool.getQualityBg(0)
...
function SkillItem:showRecommondIcon(bool,qian_type)
    ...
    if not self.recommond_icon then 
        ...
        self.recommond_label = createLabel(16,Config.ColorData.data_color4[1],cc.c4b(0x95,0x0f,0x00,0xff),29,25,"",self.recommond_icon,2, cc.p(0.5,0))
    ...
    if partner_awakening_skill_config then
        if status then
            if self.skill_profession_icon == nil then
                self.skill_profession_icon = createImage(self, nil, 102, 36, cc.p(0.5,1), true)
            end
            local limit_career = partner_awakening_skill_config.limit_career or {}
            local resNum
            if limit_career and next(limit_career) ~= nil then
                resNum = limit_career[1][1] or 3
            else
                resNum = 3
            end
            self. skill_profession_icon:loadTexture(PathTool.getResFrame("common","txt_cn_common_career_"..resNum), LOADTEXT_TYPE_PLIST)
...
function SkillItem:showName(bool,name,pos,fontSize, is_bg,fontColor,res_img,res_size)
    ...
    if not self.name then 
        if is_bg and self.name_bg == nil then
            ...
            self.name_bg = createImage(self, nil, 60,-32, cc.p(0.5,0), true, 0, true)
            self.name_bg:setContentSize(resSize)
            self.name_bg:setCapInsets(cc.rect(10, 14, 1, 1))
        end
        fontSize = fontSize or 20
        fontColor = fontColor or Config.ColorData.data_new_color4[6]
...
function SkillItem:showNoneText( status, none_tips )
    if status == true then
        if not self.none_txt then
            self.none_txt = createLabel(28, Config.ColorData.data_new_color4[15], nil, self.size.width/2, self.size.height/2, TI18N("无"), self, nil, cc.p(0.5, 0.5))
        end
