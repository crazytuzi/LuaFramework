...
function HeroClothesLustratWindow:open_callback()
    ...
    self.main_container:getChildByName("title_con"):getChildByName("title_label"):setString(TI18N("饰品图鉴"))
...
function HeroClothesLustratWindow:createTabList()
    ...
    for i=1,count do
    	...
    	self.tab_view_list[i]:setPosition(((i-1)*HeroClothesLustratTab.Width+5)+91,24)
...
HeroClothesLustratTab.Width = 147
HeroClothesLustratTab.Height = 51
...
function HeroClothesLustratTab:setName(bool)
	if self.name then
		if bool then
...
function HeroClothesLustratItem:configUI()
    ...
    if not self.goods_item then
	    ...
	    self.goods_item:setPosition(cc.p(73, 85))
