...
function HolyequipmentChooseItem:layoutUI()
    ...
    self.plan_name_label = createRichLabel(22,175, cc.p(0, 0.5), cc.p(40, self.size.height/2))
    self.container:addChild(self.plan_name_label)

    self.choose_btn = createImage(self.container, PathTool.getResFrame("common","common_1125"), 325, self.size.height/2, cc.p(0.5,0.5), true, 1, true)
    ...
    local choose_label = createLabel(22, 1, nil, btn_size.width/2, btn_size.height/2, TI18N("选择"), self.choose_btn, nil, cc.p(0.5, 0.5))

    self.sp_line = createImage(self.container, PathTool.getResFrame("common","common_1016"), self.size.width/2, 0, cc.p(0.5,0), true, 1, true)
...
function HolyequipmentChooseItem:setData(data)
	...
	if data.is_open == 1 then
		...
		if not data.list or next(data.list) == nil then
			title_str = _string_format(TI18N("%d  %s<div fontcolor=#d63636 fontsize=20> (空)</div>"), data.id, data.name)
		end
