...
function HeroHolyEquipClothPanel:open_callback()
    ...
    self.main_panel:getChildByName("wnd_title"):setString(TI18N("饰品更换"))
...
function HeroHolyEquipClothPanel:initCurHeroHolyInfo()
    ...
    if self.hero_vo then
        ...
        self.cur_hero_head:showStrTips(true, TI18N("无宝可梦"),nil,22)
    end
    ...
    if not self.holy_tips_label then
        self.holy_tips_label = createRichLabel(22, Config.ColorData.data_new_color4[11], cc.p(0, 1), cc.p(40, 230), 10, nil, 600)
...
function EquipClothItem2:layoutUI()
    ...
    self.equip_item = BackPackItem.new(false,true,nil,0.9)
    self.equip_item:setPosition(cc.p(75,self.size.height/2))
    ...
    --装备名字
    self.equip_name = createLabel(22,Config.ColorData.data_new_color4[9],nil,146,80,"",self.main_panel,0, cc.p(0,0))

    self.plan_eqm_name = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(496, 98), nil, nil, 380)
...
function EquipClothItem2:setData(info)
    ...
    self.cloth_title:enableShadow(Config.ColorData.data_new_color4[2],cc.size(0, -2),2)
    self.cloth_btn:setTitleText(TI18N("穿戴"))
    if self.open_type == 2 then 
    	...
    	self.cloth_title:enableShadow(Config.ColorData.data_new_color4[3],cc.size(0, -2),2)
    end
...
function EquipClothItem2:updateHolyEquipInfo()
    self.equip_name:setPositionY(80)
    --基本属性的位置
    local label_x = 146
    local label_y = 60
    ...
    if self.holy_base_label == nil then
        self.holy_base_label = createLabel(20, Config.ColorData.data_new_color4[6], nil, label_x, label_y, "", self.main_panel, 0, cc.p(0,0.5))
    ...
    if main_attr1 and next(main_attr1) ~= nil then
        ...
        if res then
            if self.base_attr_label == nil then
                self.base_attr_label = createRichLabel(20, Config.ColorData.data_new_color4[10], cc.p(0, 0.5), cc.p(attr_x, label_y), nil, nil, 380)
                self.main_panel:addChild(self.base_attr_label)
            end
            -- <img src='%s' scale=1 />
            local attr_str = string_format(" %s <div fontcolor=#0e7709>%s </div>", attr_name, attr_val)
    ...
    if self.holy_random_label == nil then
        local x = label_x
        local y = label_y2
        self.holy_random_label = createLabel(20, Config.ColorData.data_new_color4[6], nil, x, y , "", self.main_panel, 0, cc.p(0,0.5))
    ...
    for i=1,2 do
        if self.random_holy_equip_attr[i] == nil then
            ...
            self.random_holy_equip_attr[i] = createRichLabel(20, Config.ColorData.data_new_color4[10], cc.p(0, 0.5), cc.p(x, y), nil, nil, 380)
        ...
        if random_eqm_attr then
            ...
            if res then
                ...
                local attr_str = string_format("<div fontcolor=#b57311> %s </div><div fontcolor=%s>%s</div>", attr_name, color, attr_val)
