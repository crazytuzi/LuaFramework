...
function HeroTipsPanel:open_callback()
    ...
    self.fight_label:setPosition(103, 34)
    ...
    self.advanced_key = self.top_panel:getChildByName("advancedKey")
    self.advanced_key:setString(TI18N("进阶:"))
    self.advanced_node = self.top_panel:getChildByName("advanced_node")
    self.advanced_node:setPositionX(self.advanced_key:getPositionX()+self.advanced_key:getContentSize().width+10)
    ...
    if self.btn_goto then
        self.btn_goto:getChildByName("label"):setString(TI18N("更换宝可梦"))
    end
end
...
-- @setting.is_show_form_btn 是否显示更换宝可梦按钮(目前只有一个地方传这个按钮..如果有多.需改成支持多个的方式 --by lwc)
-- 如果显示更换宝可梦按钮..必须is_hide_equip = false
function HeroTipsPanel:openRootWnd(hero_vo, setting)
    ...
    for i,attr_str in ipairs(self.attr_list) do
        ...
        if self.attr_label_list[i] then
            ...
            value = changeBtValueForHeroAttr(value, attr_str)
...
function HeroTipsPanel:updateHolyEquip(hero_vo)
    ...
    if self.holy_tips_label == nil then 
        self.holy_tips_label = createRichLabel(14, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(298.12, -284), nil, nil, 720)
