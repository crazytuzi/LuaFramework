...
function EsecsiceWindow:open_callback()
    ...
    --local title_img = self.main_container:getChildByName()
...
function EsecsiceItem:createRootWnd()
    ...
    self:setContentSize(cc.size(678, 180))
    ...
    self.name = self.mainContainer:getChildByName("name")
    self.limitMask = self.mainContainer:getChildByName("limitMask")
    self.name_img = self.mainContainer:getChildByName("name_img")
...
function EsecsiceItem:setData(data)
    ...
    local title_path = string.format("txt_activity_tab_title_%s",data.id)
    local title_res = PathTool.getPlistImgForDownLoad("activity/txt_activity", title_path)
    loadSpriteTexture(self.name_img, title_res,LOADTEXT_TYPE)

    --self.name:setString(data.name)
    if data.val[1] then
        for i,v in pairs(data.val[1]) do
            if not self.listReward[i] then
                local item = BackPackItem.new(nil,true,nil,0.65)
            ...
            if self.listReward[i] then
                self.listReward[i]:setPosition(cc.p(68*i+(30*(i-1)), 50))
