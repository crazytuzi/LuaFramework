...
function LadderBattleResultWindow:open_callback(  )
    ...
    get_title:setString(TI18N("获\n得\n奖\n励"))
    local get_title_en = container:getChildByName("get_title_en")
    get_title_en:setString("Reward")

    showLabelByCode(get_title, get_title_en)
    ...
    self.top_result = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(480, 377))
    ...
    self.bottom_result = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(480, 275))
    ...
    self.fight_text = createLabel(24, Config.ColorData.data_new_color4[6], nil, 360, 438, "",self.container, nil, cc.p(0.5,0.5))
...
function LadderBattleResultWindow:setBaseInfo(  )
    ...
    if self.data.atk_change_rank == 0 then -- 排名不变
        ...
        if self.data.atk_change_rank > 0 then -- 排名上升
        	top_str = string_format(TI18N("排名:<div fontcolor=#0e7709>%s</div><img src=%s scale=1 />"), my_rank, up_res)
        	if self.data.is_change_best_rank == 1 then
        		top_str = top_str .. string_format("    <img src=%s scale=1 />", PathTool.getTargetRes("ladder/txt_ladder", "txt_cn_ladder_highest", false, false))
        	end
        else
        	top_str = string_format(TI18N("排名:<div fontcolor=#d63636>%s</div><img src=%s scale=1 />"), my_rank, down_res)
        end
    end
    ...
    if self.data.def_change_rank == 0 then
        ...
        if self.data.def_change_rank > 1 then
            bottom_str = string_format(TI18N("排名:<div fontcolor=#0e7709>%s</div><img src=%s scale=1 />"), def_rank, up_res)
        else
            bottom_str = string_format(TI18N("排名:<div fontcolor=#d63636>%s</div><img src=%s scale=1 />"), def_rank, down_res)
        end
...
function LadderBattleResultWindow:setRewardsList(  )
    ...
    for i, v in ipairs(self.data.reward) do
        ...
        if item_conf then
            ...
            item:setExtendDesc(true, item_conf.name, 6)
            index = index + 1
        end
    end 
end
