...
-- @description    : 
		-- 天梯奖励
---------------------------------
LadderAwardWindow = LadderAwardWindow or BaseClass(BaseView)

local controller = LadderController:getInstance()
local model = controller:getModel()

function LadderAwardWindow:__init()
...
function LadderAwardWindow:open_callback(  )
	...
	tips_label:setString(TI18N("奖励将在周日23:00通过邮件发放"))
...
function LadderAwardWindow:openRootWnd(  )
	self:setData()
end

function LadderAwardWindow:setData(  )
	local award_config = Config.SkyLadderData.data_award
	if award_config then
		local award_data = {}
		for k,v in pairs(award_config) do
			local award = DeepCopy(v)
			table.insert(award_data, award)
		end
		...
		-- 我的排名
		local myBaseData = model:getLadderMyBaseInfo()
...
function LadderAwardWindow:register_event(  )
...
function LadderAwardWindow:_onClickBtnClose(  )
	controller:openLadderAwardWindow(false)
end

function LadderAwardWindow:close_callback(  )
...
-----------------------------------@ 奖励item
LadderAwardItem = class("LadderAwardItem", function()
    return ccui.Widget:create()
end)

function LadderAwardItem:ctor()
	self:configUI()
	self:register_event()
end

function LadderAwardItem:configUI(  )
...
function LadderAwardItem:register_event(  )
	
end

function LadderAwardItem:setData( data )
	data = data or {}
	...
	local start_x = 35
	self.item_scroll_view:removeAllChildren()
	for i,award in pairs(data.reward) do
		if type(award) == "table" then
			local bid = award[1]
            local num = award[2]
            local item_conf = Config.ItemData.data_get_data(bid)
	...
	self.item_scroll_view:setTouchEnabled(#data.reward > 3)
end

function LadderAwardItem:DeleteMe(  )
