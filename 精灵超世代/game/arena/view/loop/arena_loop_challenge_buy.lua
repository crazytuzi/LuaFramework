...
function ArenaLoopChallengeBuy:__init()
    ...
    self.title_str= ""
    self.layout_name = "arena/arena_loop_challenge_buy"
...
function ArenaLoopChallengeBuy:open_callback()
    ...
    self.title_txt = self.main_container:getChildByName("Text_1")
...
function ArenaLoopChallengeBuy:register_event()
    ...
    registerButtonEventListener(self.btn_redu, function()
        ...
        local str = string.format(TI18N("<div fontcolor=#3d5078>%s /</div><div fontcolor=#0e7709> %s</div>"),MoneyTool.GetMoneyString(have_num), MoneyTool.GetMoneyString(self.expend_num*self.change_num))
...
function ArenaLoopChallengeBuy:openRootWnd(setting)
    ...
    self.goods_item:setPosition(cc.p(self.main_container:getContentSize().width/2, 285))
    ...
    local item_name_size = self.item_name:getContentSize()
    local name_bg_size = self.name_bg:getContentSize()
    self.name_bg:setContentSize(cc.size(item_name_size.width + 40, name_bg_size.height))
    self.richlabel_num = createRichLabel(26, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(self.gold_change:getContentSize().width*0.5+10, self.gold_change:getContentSize().height*0.5), nil, nil, 250)
    ...
    local str = string.format(TI18N("<div fontcolor=#3d5078>%s /</div><div fontcolor=#0e7709> %s</div>"),MoneyTool.GetMoneyString(have_num), MoneyTool.GetMoneyString(self.expend_num))
    self.richlabel_num:setString(str)
