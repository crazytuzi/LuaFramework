...
function ArenaLoopRankPanel:ctor()
    ...
    self.power_name = my_container:getChildByName("power_name")
    ...
    self.my_score_info = createRichLabel(20, 175, cc.p(0.5, 0.5), cc.p(594, 18), nil, nil, 300)
    ...
    self.role_head:setPosition(194, 45)
    ...
    if data and role_vo then
        ...
        if data.rank and data.rank <= 3 then
            ...
            if data.rank == 0 then
                ...
                self.rank_img:setScale(0.8)
            end
        else
            if self.rank_num == nil then
                -- self.rank_num = CommonNum.new(1, self.my_container, 1, -2, cc.p(0.5, 0.5))
                -- self.rank_num:setPosition(96,46)
                self.rank_num = createLabel(48,Config.ColorData.data_new_color4[6],nil,96,46,"",self.my_container,0, cc.p(0.5,0.5))
            end
            self.rank_num:setVisible(true)
            -- self.rank_num:setNum(data.rank)
            self.rank_num:setString(data.rank or 0)
            self.rank_img:setVisible(false)
        end
...
function ArenaLoopRankItem:ctor()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("arena/arena_loop_rank_item"))
    self.size = self.root_wnd:getContentSize()
    ...
    self.power_name = container:getChildByName("power_name")
    self.power_name:setString(TI18N("战力："))
    self.role_power = container:getChildByName("role_power")
    -- self.power_bg = container:getChildByName("Image_1")
    self.score_info = createRichLabel(20, 175, cc.p(0.5, 0.5), cc.p(560, 18), nil, nil, 300)
    container:addChild(self.score_info)

    self.role_head = PlayerHead.new(PlayerHead.type.circle)
    self.role_head:setHeadLayerScale(0.8)
    self.role_head:setPosition(160, 55)
...
function ArenaLoopRankItem:setData(data)
    ...
    if data and role_vo then
        ...
        if data.worship_status == TRUE or role_vo:isSameRole(data.srv_id, data.rid) then
            ...
            setChildUnEnabled(false, self.wish_container, Config.ColorData.data_color4[1])
        end
        ...
        if data.rank <= 3 then
            ...
            if self.rank_num == nil then
                ...
                self.rank_num = createLabel(48,Config.ColorData.data_new_color4[6],nil,59,57,"",self.container,0, cc.p(0.5,0.5))
            end
            self.rank_num:setVisible(true)
            -- self.rank_num:setNum(data.rank)
            self.rank_num:setString(data.rank or 0)
