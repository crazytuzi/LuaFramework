...
function ArenaChampionMyMatchReadyPanel:ctor(view_type)
    ...
    self.img = container:getChildByName("img")
    ...
    self.extend_desc = createRichLabel(26, Config.ColorData.data_new_color4[1], cc.p(0, 1), cc.p(56, 228), 2, nil, 350)
...
function ArenaChampionMyMatchReadyPanel:countDownTimeTicket()
    ...
    local str = string_format(TI18N("开启倒计时:<div fontColor=%s >\n%s</div>\n竞技场排名前<div fontColor=%s >%s名</div>的玩家将自动参与"), Config.ColorData.data_new_color_str[17], TimeTool.GetTimeFormat(less_time), Config.ColorData.data_new_color_str[17], self.need_rank_index)
    if self.view_type == ArenaConst.champion_type.cross then
        str = string_format(TI18N("开启倒计时:<div fontColor=%s >\n%s</div>\n跨服天梯前<div fontColor=%s >%s名</div>的玩家将自动参与"), Config.ColorData.data_new_color_str[17], TimeTool.GetTimeFormat(less_time), Config.ColorData.data_new_color_str[17], self.need_rank_index)
    end    
    self.extend_desc:setString(str)
end
