...
function ArenaChampionMyLogItem:setExtendData(data)
	...
	if not tolua.isnull(node) and self.root_wnd == nil then
		...
		self.result_label = self.root_wnd:getChildByName("time") -- 获得积分
		self.match_step_label = self.root_wnd:getChildByName("fight_type") -- 该竞猜的战斗阶段
		...
		self.info_btn:getChildByName("label"):setString(TI18N("战报"))
