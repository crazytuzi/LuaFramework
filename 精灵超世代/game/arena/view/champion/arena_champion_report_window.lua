...
function ArenaChampionReportWindow:open_callback()
	...
	self.check_btn = main_panel:getChildByName("report_btn")
	...
	self.fail_img:setVisible(false)
	self.top_y = 754
	self.bottom_y = 418
end
...
function ArenaChampionReportItem:setData(data)
	if data then
		...
		-- end
		self.desc_1:setString(changeBtValueForBattle(data.hurt))
		self.desc_2:setString(changeBtValueForBattle(data.behurt))
		self.desc_3:setString(changeBtValueForBattle(data.curt))
