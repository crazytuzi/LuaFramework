...
function ArenaEnterChampionView:setBaseInfo()
	...
	elseif base_info.step == ArenaConst.champion_step.score then
		if base_info.step_status == ArenaConst.champion_step_status.unopened then
			self.match_step:setString(string_format("%s-%s", TI18N("选拔赛"), TI18N("暂未开启")))
		else
			self.match_step:setString(string_format("%s-%s", TI18N("选拔赛"), TI18N("正式进行")))
		end
	elseif base_info.step == ArenaConst.champion_step.match_32 then
		if base_info.step_status == ArenaConst.champion_step_status.unopened then
			self.match_step:setString(string_format("%s-%s", TI18N("32强赛"), TI18N("暂未开启")))
		else
			self.match_step:setString(string_format("%s-%s", TI18N("32强赛"), TI18N("正在进行")))
		end
	elseif base_info.step == ArenaConst.champion_step.match_4 then
		if base_info.step_status == ArenaConst.champion_step_status.over then
			self.match_step:setString(TI18N("冠军赛已结束"))
		else
			self.match_step:setString(string_format("%s-%s", TI18N("4强赛"), TI18N("正在进行")))
		end
	end
	...
	self.match_desc:setString(
		string_format(
			...
			TimeTool.getMD(base_info.start_time),
			TimeTool.getMD(base_info.end_time),
