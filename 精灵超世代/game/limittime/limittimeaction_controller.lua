...
function LimitTimeActionController:handle28000(data)
    if data ~= nil and next(data) ~= nil then
        ...
        if data.gifts ~= nil and next(data.gifts) ~= nil then
            for k,v in pairs(data.gifts) do
               local key =  role_vo.rid .."_"..v.award_id
               local isShow =  cc.UserDefault:getInstance():getIntegerForKey(key, 0)
               if(isShow == 0)then
                    self:openLimitTimeGiftWindow(true)
