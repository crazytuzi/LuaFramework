...
function LimitTimeGiftWindow:open_callback()
    ...
    self.gift_percent_num:setPosition(num_pos_x, 448)
    ...
    local function setRemainTimeString(time)
        if time > 0 then
            ...
            elseif time_model == 2 then
                ...
                node:setString(string.format(TI18N("%s后礼包消失"), TimeTool.GetTimeFormat(less_time)))
            end
        else
            doStopAllActions(node)
...
function LimitTimeGiftWindow:setPanelData(index)
    ...
    if self.data.gifts[index] == nil then return end
	...
	local tab_data = {}
		...
		if self.data.gifts[index].count >= limit_condition.limit_count then
            ...
            local price_type = getCurPriceType()
            local buy_label = string.format("%s%s %s", GetSymbolByType(price_type), limit_condition.price[price_type], TI18N("购买"))
            self.btn_buy_label:setString(buy_label)
			-- self.btn_buy_label:setString(limit_condition.price .. TI18N("元购买"))
		end
		self.gift_percent_num:setNum(limit_condition.art_words,true)
		if self.icon_per == nil then 
	  		self.icon_per = createSprite(nil, num_pos_x + self.gift_percent_num:getContentSize().width * 0.5 + 5, 420, self.main_container, cc.p(0, 0.5))
