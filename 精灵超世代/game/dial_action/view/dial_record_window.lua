...
DialRecordWindow = DialRecordWindow or BaseClass(BaseView)

function DialRecordWindow:__init()
...
function DialRecordWindow:open_callback(  )
...
function DialRecordWindow:register_event(  )
	registerButtonEventListener(self.close_btn, function (  )
		_controller:openDialRecordWindow(false)
	end, false, 2)
	
	-- 数据返回
	self:addGlobalEvent(DialActionEvent.Update_Dial_Record_Event, function (  )
		self:setData()
	end)
end

function DialRecordWindow:setData(  )
	-- 个人记录
	local my_record_data = _model:getMyselfDialRecordData()
	self.my_record_scrollview:setData(my_record_data, nil, nil, 1)

	-- 全服记录
	local all_record_data = _model:getAllDialRecordData()
	self.all_record_scrollview:setData(all_record_data, nil, nil, 2)
end

function DialRecordWindow:openRootWnd(  )
	_controller:sender16674(1)
end

function DialRecordWindow:close_callback(  )
...
---------------------------@ item
DialRecordItem = class("DialRecordItem", function()
    return ccui.Widget:create()
end)

function DialRecordItem:ctor()
	self:configUI()
	self:register_event()
...
function DialRecordItem:configUI(  )
	self.size = cc.size(542, 30)
	self:setTouchEnabled(false)
    self:setContentSize(self.size)
end

function DialRecordItem:register_event(  )
	
end

function DialRecordItem:setExtendData( r_type )
	self.r_type = r_type or 1
end

function DialRecordItem:setData( data )
...
function DialRecordItem:DeleteMe(  )
