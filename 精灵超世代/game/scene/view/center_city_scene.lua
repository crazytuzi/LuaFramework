...
function CenterCityScene:__init(config)
    ...
    self.speed_list             = {-0.2, 0, -0.3,-0.2, -0.1}
    self.step                   = 0
    ...
    self.layer_img_list         = {"1.png", "2.png", "3.png", "4.png","5.png"}
...
function CenterCityScene:createRootWnd()
    ...
    print("display.getMaxScale()~~~~~~~~~~",display.getMaxScale(),self.difference_width,self.size.width,self.size.height)
...
function CenterCityScene:renderMapPic()
    ...
    for i, name in ipairs(self.layer_img_list) do
        ...
        print("renderMapPic",res_path,i,name)
        if i == 1 then
            pos = cc.p(0, 20)
        elseif i == 2 then
            pos = cc.p(0, 0)
        ...
        dump(self.map_cache,"self.map_cache")
    end
end
...
function CenterCityScene:changeSceneEffect()
    ...
    for k, image_item in pairs(self.image_list) do
        if image_item.data and image_item.data.dun_id and image_item.data.dun_id ~= 0 then
            image_item:DeleteMe()
            self.image_list[k] = nil
        end
    end
    self:analysisEffect()
    self:analysisImages()
    ...
    if self.config ~= nil and self.config.building_list ~= nil then
        for i, v in ipairs(self.config.building_list) do
            if v.dun_id == 0 or self.cur_time_type == v.dun_id then
                if v.type == BuildItemType.effect or v.type == BuildItemType.npc then
...
function CenterCityScene:analysisImages()
    if self.config ~= nil and self.config.building_list ~= nil then
        for i, v in ipairs(self.config.building_list) do
            if v.dun_id == 0 or self.cur_time_type == v.dun_id then
                if v.type == BuildItemType.image then
                    if self.image_list[v.bid] == nil then
                        local data = {}
                        local build_item = BuildItem.New(v, v.type)
                        build_item:setParentWnd(self["map_layer" .. v.layer])
                        self.image_list[v.bid] = build_item
                    end
                end
            end
        end
...
function CenterCityScene:quequeCreateEffect()
    ...
    print("self.map_create_end ",self.map_create_end,self.effect_render_list )
    if self.map_create_end == true then
        ...
        if effect_config ~= nil then
            ...
            print("effect_config.res",effect_config.res,png_path)
...
function CenterCityScene:setFlowerEffect()
    if self.hua then
        self.hua:clearTracks()
        self.hua:removeFromParent()
        self.hua = nil
    end
    local res = "hua"
    if self.cur_time_type == 2 then
        res = "ye"
    end
    self.hua = createEffectSpine(res, cc.p(SCREEN_WIDTH*0.5, SCREEN_HEIGHT-265), cc.p(0.5, 0.5), true, PlayerAction.action)
    self.root:addChild(self.hua)
end