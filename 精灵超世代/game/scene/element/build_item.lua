...
function BuildItem:createRootwnd()
	...
	if self.is_build == true then
		...
		elseif self.build_type == BuildItemType.npc then
			self:createNpc()
		elseif self.build_type == BuildItemType.image then
			self:createImage()
		end
	end
end
...
function BuildItem:createBuild()
	...
	local temp_res = PathTool.getTargetRes("centerscene/centercity/"..cur_time_type.."/build", self.data.config.res)
	self.item_icon = createImage(self.node, temp_res,
		0,
		0,
		cc.p(0.5, 0.5),
		false,
	...
	--self.item_icon_bg:setCapInsets(cc.rect(43, 1, 1, 1))
	self.item_icon:ignoreContentAdaptWithSize(true)
	self.size = cc.size(self.item_icon:getContentSize().width, self.item_icon:getContentSize().height)
	self.node:setContentSize(self.size)
	self.item_icon:setPosition(self.size.width/2,self.size.height/2)
	temp_res = PathTool.getTargetRes("centerscene/centercity/"..cur_time_type.."/build", self.data.config.res .. "_sel")
	self.item_icon_sel = createImage(self.item_icon, temp_res,
			self.item_icon:getContentSize().width/2,
			self.item_icon:getContentSize().height/2,
			cc.p(0.5, 0.5),
			false,
			nil,false
	)
	self.item_icon_sel:setVisible(false)
	local config = Config.CityData.data_base[self.data.config.bid]
	--self.item_icon = createLabel(20,Config.ColorData.data_new_color4[6],nil,self.size.width * 0.5,self.size.height * 0.5,
	--		config.name,self.node,nil,cc.p(0.5, 0.5))
	-- 引导需要的,根据这个获取建筑点击对象
	self.node:setName("guidesign_build_"..self.data.config.bid)

	temp_res = PathTool.getTargetRes("centerscene/txt_centerscene","txt_centerscene_".. self.data.config.bid)
	local pos_x = name_x
	createImage(self.node, temp_res,
			self.data.config.name_x,
			self.data.config.name_y,
			cc.p(0.5, 0.5),
			false,
			nil,false
	)
...
function BuildItem:updateContentSize()
	local size = self.item_icon:getContentSize()
	self.size = cc.size(size.width,self.size.height)
	--self.item_icon_bg:setContentSize(self.size)

end

function BuildItem:createEffect()
	--print("createEffect",self.data.res)
		self.body =
		createEffectSpine(
				self.data.res,
...
function BuildItem:createImage()
	local hour = tonumber(os.date("%H"))
	local cur_time_type = 1
	if hour>= 6 and hour< 18 then
	    cur_time_type = 1
	...
	local temp_res = PathTool.getTargetRes("centerscene/centercity/"..cur_time_type.."/build", self.data.res)
	self.body = createImage(self.node, temp_res,
			0,
			0,
			cc.p(0.5, 0.5),
			false,
	...
	self.body:ignoreContentAdaptWithSize(true)
	self.size = cc.size(self.body:getContentSize().width, self.body:getContentSize().height)
	self.node:setContentSize(self.size)
end
...
function BuildItem:createNpc()
	...
	local function animationCompleteFunc(event) 
		if event.animation == PlayerAction.action_1 then
			self.body:setAnimation(0, PlayerAction.action_2, true)
			self.is_idle_ing = false
		end
	end
...
function BuildItem:clickNpcHandler()
	...
	self.body:setAnimation(0, PlayerAction.action_1, false)
end
...
function BuildItem:registEvent()
	...
	self.node:setSwallowTouches(false)
	self.node:setSwallowTouches(false)
	--self.item_icon:setAlphaCheck(true)
	self.node:addTouchEventListener(function(sender, event_type)
		--customClickAction(sender, event_type,1)
		if event_type == ccui.TouchEventType.ended then
			sender:runAction(cc.Sequence:create(cc.ScaleTo:create(0.1, 0.98), cc.ScaleTo:create(0.1, 1)))
			self.touch_end = sender:getTouchEndPosition()
			local is_click = true
			if self.touch_began ~= nil then
				is_click = math.abs( self.touch_end.x - self.touch_began.x ) <= 30 and math.abs( self.touch_end.y - self.touch_began.y ) <= 30
			end
			self:removeFinger()
			if is_click == true then
			...
			self.item_icon_sel:setVisible(false)
		elseif event_type == ccui.TouchEventType.began then
			sender:runAction(cc.ScaleTo:create(0.05, 1.02))
            self.touch_began = sender:getTouchBeganPosition()
			self.item_icon_sel:setVisible(true)
		elseif event_type == ccui.TouchEventType.moved then
			self.touch_moved = sender:getTouchEndPosition()
			if self.touch_began ~= nil then
				is_click = math.abs( self.touch_moved.x - self.touch_began.x ) <= 30 and math.abs( self.touch_moved.y - self.touch_began.y ) <= 30
			end
...
function BuildItem:setParentWnd(parent)
	...
	if self.data ~= nil then
		if self.is_build then
			...
			self.parent_wnd:addChild(self.node, self.data.orderZ and self.data.orderZ or 10)
		else
			self.node:setPosition(self.data.x, self.data.y)
			self.parent_wnd:addChild(self.node, self.data.orderZ and self.data.orderZ or 1)
		end
	end
end
...
function BuildItem:setRedPoint()
	...
	if status == false then
		...
		if self.tips_icon == nil then
			local pos_x = self.data.config.redPoint_x > 0 and self.data.config.redPoint_x or self.size.width
			local pos_y = self.data.config.redPoint_y > 0 and self.data.config.redPoint_y or self.size.height
			self.tips_icon = createSprite(PathTool.getResFrame("centerscene", "scene_0"), pos_x, pos_y, self.item_icon, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
		end
...
function BuildItem:setLockStatus()
	...
	if self.build_lock_status then
		if self.data.config.bid == CenterSceneBuild.crossshow then
			...
			setChildUnEnabled(true, self.node,cc.c4b(77,77,77,0xff))
		end
