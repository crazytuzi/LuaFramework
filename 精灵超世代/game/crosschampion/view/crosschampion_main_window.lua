...
function CrosschampionMainWindow:open_callback(  )
    ...
    top_panel:getChildByName("title"):setString(TI18N("周冠军赛标题"))
    ...
    -- 当前赛况
    self.match_step = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(210, 175), nil, nil, 560) 
    ...
    top_panel:setPositionY(top_off - 144)    
...
function CrosschampionMainWindow:setBaseInfo(  )
    ...
    if base_info.step == ArenaConst.champion_step.unopened then
        ...
        self.match_step:setString(TI18N("暂未开启"))
    elseif base_info.step == ArenaConst.champion_step.score then
        if base_info.step_status == ArenaConst.champion_step_status.unopened then
            ...
            self.match_step:setString(_string_format("%s-%s", TI18N("选拔赛"), TI18N("暂未开启")))
        else
            ...
            self.match_step:setString(_string_format("%s-%s", TI18N("选拔赛"), TI18N("正式进行")))
        end
    elseif base_info.step == ArenaConst.champion_step.match_64 then
        if base_info.step_status == ArenaConst.champion_step_status.unopened then
            ...
            self.match_step:setString(_string_format("%s-%s", TI18N("64强赛"), TI18N("暂未开启")))
        else
            ...
            self.match_step:setString(_string_format("%s-%s", TI18N("64强赛"), TI18N("正在进行")))
        end
    elseif base_info.step == ArenaConst.champion_step.match_8 then
        if base_info.step_status == ArenaConst.champion_step_status.over then
            ...
            self.match_step:setString(TI18N("周冠军赛已结束"))
        else
            ...
            self.match_step:setString(_string_format("%s-%s", TI18N("8强赛"), TI18N("正在进行")))
        end
    end
    ...
    if base_info.is_open == 1 then
        ...
        self.match_step:setString(TI18N("暂未开启"))
        self.match_desc:setString(TI18N("暂未开启"))
    elseif base_info.is_open == 2 then
        ...
        -- self.match_desc:setString(_string_format("<div fontcolor=#ff5353 outline=1,#000000>%s</div>", TI18N("暂未开启")))
        self.match_step:setString(_string_format(TI18N("参赛玩家不足%d名，本次周冠军赛无法开启"), limit_num))
        self.match_desc:setString(TI18N("暂未开启"))
    else
    ...
    if less_time > 0 then
        ...
        self.match_desc:setString(
            _string_format(
                ...
                TimeTool.getMD(start_time),
                TimeTool.getMD(end_time),
