...
function GuildBossRankRoleWindow:updateTitle()
    ...
    for i = 1, num do
        if i ~= num then
            ...
            local line = createImage(self.main_panel, res, line_offx, 696, cc.p(0, 0.5), true, 1, false)
            line:setScaleY(0.8)
        end
        local offx = pos_list[i] or 0
        local label = createLabel(20, Config.ColorData.data_color4[175], nil, offx, 696, '', self.main_panel, 0, cc.p(0, 0.5))
...
function GuildBossRankRoleWindow:updateRankList(data)
    ...
    if not self.list_view then
        ...
        self.list_view = CommonScrollViewSingleLayout.new(self.main_panel, cc.p(45, 215) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting, cc.p(0,0))
...
function GuildBossRankRoleWindow:updateMyData()
    ...
    self.four_label:setString(changeBtValueForBattle(tonumber(data.mydps)))
end
--
----更新前三名头像
...
--更新前三名头像
function GuildBossRankRoleWindow:updateRankData()
    --dump(self.data,"-----------------------------------------------------------7234658726385768")
    if not self.data then return end
    local rank_list
    if self.click_index == RankConstant.RankType.elite then
        rank_list = self.data.arena_elite_rank or {}
    else
        rank_list = self.rank_data.rank_list or {}
    end
    --dump(self.data.rank_list,"287462736------------------->>")
    local count = 0
    local size = self.main_panel:getContentSize()
