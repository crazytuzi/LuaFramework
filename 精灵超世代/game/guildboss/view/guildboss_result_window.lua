...
function GuildbossResultWindow:open_callback()
    ...
    self.fight_text = createLabel(24, Config.ColorData.data_new_color4[6], nil, 360, 400, "", container, nil, cc.p(0.5,0.5))
...
function GuildbossResultWindow:openRootWnd(data, fight_type)
	...
	if data ~= nil then
		self.data = data
		self.dps_value:setString(string.format(TI18N("总伤害：%s"), changeBtValueForBattle(data.all_dps)))
		local hero_vo
...
function GuildbossResultWindow:createRewardsList(award_list)
	...
	for i, v in ipairs(award_list) do
		...
		if item_config then
			_y = 251 - (index - 1) * 50
			rich_label = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(266, _y), nil, nil, 500)
...
function GuildbossResultWindow:createPlanesAwardList( award_list )
	...
	for i, v in ipairs(award_list) do
		if v.bid and v.num then
			...
			if item_config then
				item:setGoodsName(item_config.name,nil,nil)
			end
