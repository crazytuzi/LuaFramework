...
function VoyageOrderItem:configUI(  )
    ...
    self.order_name_label = createRichLabel(22, 0xFFFFFF, cc.p(0, 0.5), cc.p(16, 116), 0, 0, 500)
    self:addChild(self.order_name_label)

    self.get_btn = container:getChildByName("get_btn")
    local btn_size = self.get_btn:getContentSize()
    self.get_btn:setContentSize(cc.size(168, 62))
    btn_size = self.get_btn:getContentSize()
...
function VoyageOrderItem:refreshOrderInfo(  )
	...
	self.rarity_image:setCapInsets(cc.rect(11, 1, 264-251, 170))

	-- local order_name = config.name.."-"..VoyageConst.Order_Name[config.quality]

	local rarity_color = VoyageConst.Order_Rarity_Color[config.quality]
	local outline_color = VoyageConst.Order_Rarity_OutLine[config.quality]
	local order_name = TI18N(VoyageConst.Order_Name[config.quality])

	-- local order_name_label = "<div fontcolor=#ffffff outline=2,"..outline_color..">"..
	-- 	config.name.."-</div><div fontcolor="..rarity_color.." outline=2,"..outline_color..">"..
	-- 	order_name.."</div>"

	local order_name_label = string.format(TI18N("<div fontcolor=#ffffff outline=2,%s>%s-</div><div fontcolor=%s outline=2,%s>%s</div>"),
		outline_color, config.name, rarity_color, outline_color, order_name)

	self.order_name_label:setString(TI18N(order_name_label))
...
function VoyageOrderItem:refreshOrderBtnStatus(  )
	if self.data.status == VoyageConst.Order_Status.Unget then
		if not self.expend_label then
			self.expend_label = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(500, 88))
		...
		if expend then
			...
			self.expend_label:setString(string_format(TI18N("%s: <img src='%s' scale=0.5 /> %d"), TI18N("消耗"), res, num))
		end
		self.get_btn_label:setString(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,#854000>接取</div>"))
	...
	elseif self.data.status == VoyageConst.Order_Status.Underway then
		...
		self.get_btn_label:setString(string_format(TI18N("   <img src='%s' scale=0.3 /><div fontcolor=#ffffff shadow=0,-2,2,#854000>%d\n%s</div>"), PathTool.getItemRes(3), gold_num, TI18N("加速")))
	...
	elseif self.data.status == VoyageConst.Order_Status.Finish then
		self.get_btn_label:setString(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,#0E73B3>领取</div>"))
