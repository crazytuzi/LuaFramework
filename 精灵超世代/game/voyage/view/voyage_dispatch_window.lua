...
local icon_offx = 56.5
local icon_offy = 50.5
...
function VoyageDispatchWindow:__init()
	...
	self.need_hero_num = 0  -- 可选宝可梦最大数
	self.chose_heros = {}   -- 选中的宝可梦列表
	self.hero_boxs = {} 	-- 宝可梦头像框(最大宝可梦数量)
	self.hero_icons = {} 	-- 选中的宝可梦头像
...
function VoyageDispatchWindow:open_callback(  )
	...
	self.no_hero_image:getChildByName("label"):setString(TI18N("暂无该阵营宝可梦"))
...
function VoyageDispatchWindow:showBaseInfo( data )
	...
	if need_star > 0 then
		...
		registerButtonEventListener(self.star_icon, function (  )
			message(string.format(TI18N("需要一个%d星宝可梦"), star_num))
		end, false)
	...
	for i,camp_type in ipairs(need_camps) do
		...
		if camp_icon == nil then
			...
			registerButtonEventListener(camp_icon, function (  )
				message(string.format(TI18N("需要一个%s宝可梦"), HeroConst.CampAttrName[camp_type]))
			end, false)
...
function VoyageDispatchWindow:refreshConditionStatus(  )
	...
	for k,v in pairs(self.conditions or {}) do
		...
		if is_meet == false and self.con_status then
			if v.star_num then
				self.con_tips = string.format(TI18N("需要一个%d星宝可梦"), v.star_num)
			elseif v.camp_type then
				self.con_tips = string.format(TI18N("需要一个%s系宝可梦"), HeroConst.CampName[v.camp_type])
			end
	...
	if hero_vo.is_ui_select then
		for k,hero_icon in pairs(self.hero_icons or {}) do
			...
			if data.partner_id == hero_vo.partner_id then
				...
				local move_act = cc.MoveTo:create(0.07, cc.p(node_pos.x+icon_offx, node_pos.y+icon_offy))
...
function VoyageDispatchWindow:createChoseHeroIcon( hero_vo, item )
	...
	if hero_box then
		...
		if item then
			local world_pos = item:convertToWorldSpace(cc.p(0, 0))
			local node_pos = hero_box:convertToNodeSpace(world_pos)
			hero_icon:setPosition(cc.p(node_pos.x+icon_offx, node_pos.y+icon_offy))
			hero_icon:runAction(cc.MoveTo:create(0.07, cc.p(icon_offx, icon_offy)))
		else
			hero_icon:setPosition(cc.p(icon_offx, icon_offy))
		end
	end
end
...
function VoyageDispatchWindow:_onClickQuickChoseBtn(  )
	...
	local star_hero = {} 		-- 满足星级条件的宝可梦
	local all_camp_hero = {} 	-- 满足阵营条件的宝可梦
	local both_hero = {} 		-- 同时满足星级和阵营的宝可梦
	...
	local star_dif_num = star_need - #self.chose_heros -- 满足星级条件的宝可梦，还差的个数
