...
function BattlDramaHookRewardWindow:open_callback()
    ...
    self.head_icon:setPosition(183, 480)
    ...
    loadSpriteTexture(title, PathTool.getPlistImgForDownLoad("common/txt_common", "txt_common_7"), LOADTEXT_TYPE )
end
...
function BattlDramaHookRewardWindow:createItemList(item_list)
    ...
    if self.scroll_view == nil then
        ...
        local setting = {
            ...
            start_y = 20,
            space_y = 10,
            item_width = 120*item_scale,
            item_height = 145*item_scale,
            row = 5,
            col = 5,
            delay = 6,
            need_dynamic = true
...
function BattlDramaHookRewardItem:ctor()
    ...
    self:setContentSize(cc.size(120*item_scale,145*item_scale))
    self:setCascadeOpacityEnabled(true)

    self.item = BackPackItem.new(false, true, false, item_scale, false, true)
    self.item:setPosition(60*item_scale,85*item_scale)
