...
function EliteSummonPanel:ctor(bid)
	...
	self.res_list = {
        ...
        { path = PathTool.getPlistImgForDownLoad("elitesummon","elitesummon_bg"), type = ResourcesType.plist },
    } 
...
function EliteSummonPanel:configUI(  )
    ...
    local bg_res = PathTool.getPlistImgForDownLoad("elitesummon", "elitesummon_bg")
...
function EliteSummonPanel:setData(data)
    ...
    -- local str = string.format(TI18N("<div outline=2,#000000>剩余</div><div fontcolor=#5fde46 fontsize=20 outline=2,#000000>%d</div><div outline=2,#000000>次招募内必出UP宝可梦</div>"),data.must_count)
    local str = string.format(TI18N("剩余<div fontcolor=#5fde46 fontsize=20>%d</div>次招募内必出UP宝可梦"),self.data.must_count)
