...
function PresageSummonPanel:IsActionStatus(status)
	...
	elseif status == 1 then
		...
		local refresh_str = string_format(TI18N("<div outline=2,#37161a>剩余</div><div fontColor=#8bfa49 fontsize=22 outline=2,#37161a> %d </div><div outline=2,#37161a>次刷新必出两个五星宝可梦</div>"),must_num-data.count%must_num)
		if (must_num-data.count%must_num) == 1 then
			refresh_str = TI18N("本次刷新必出两个五星宝可梦")
		end
