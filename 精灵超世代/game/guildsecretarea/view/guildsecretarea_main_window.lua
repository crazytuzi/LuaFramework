...
function GuildsecretareaMainWindow:open_callback(  )
    ...
    self.background:setBackGroundImage(PathTool.getPlistImgForDownLoad("bigbg/guild","guild_bg_2",true), LOADTEXT_TYPE)
    ...
    self.rank_info_btn = createRichLabel(18, cc.c4b(0x83,0xe7,0x73,0xff), cc.p(0.5, 0.5), cc.p(84, 16))
    ...
    self.box_btn = createRichLabel(22,Config.ColorData.data_new_color4[7], cc.p(0.5, 0.5), cc.p(630, 360))
    ...
    self.less_time = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(134, 224),nil,nil,720)
    self.bottom_panel:addChild(self.less_time)

    self.cost_label = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(360, 308),nil,nil,720)
    ...
    local bg = createSprite(PathTool.getPlistImgForDownLoad("bigbg/guild", "guild_bg_3_top"), SCREEN_WIDTH*0.5, display.getTop(), self.container, cc.p(0.5,1), LOADTEXT_TYPE,-1)
...
function GuildsecretareaMainWindow:setBackgroundImg(index)
    ...
    local bg_res = PathTool.getPlistImgForDownLoad("bigbg/guildsecretarea", bg_name, false)
    ...
    if self.scdata.loss_list and next(self.scdata.loss_list) ~= nil then
        ...
        if is_matching then
            ...
            if self.scdata.dps > 0 and is_select then 
                if self.scdata.hp > 0 then
                    str = string_format(TI18N("挑战次数不足, 是否花费 <img src='%s' scale=0.3 /> %s购买一次挑战次数？并扫荡根据上次的伤害量%s进行结算"), iconsrc, count, self.scdata.dps)
                else
                    if self:isShowContinueTips() then
                        return
                    end
                    str = string_format(TI18N("挑战次数不足, 是否花费 <img src='%s' scale=0.3 /> %s购买一次挑战次数？并追击根据上次的伤害量%s进行结算"), iconsrc, count, self.scdata.dps)
                end
            else
                if self.scdata.hp <= 0 then
                    if self:isShowContinueTips() then
...
function GuildsecretareaMainWindow:onClickFightBtn(is_buy_back)
    ...
    if self.scdata.bid == 0 or self.is_start_fight then
        ...
        --可以弹出确认了
        local msg = string_format(TI18N("确定消耗<div fontcolor=#249003>%s</div>点公会活跃开始讨伐%s吗？"), self.boss_config.active, self.boss_config.name)
        local call_back = function()
...
function GuildsecretareaMainWindow:updateBuyCount()
    ...
    local str = string.format(TI18N("%s<div fontcolor=#3df424 outline=2,#000000>%s</div>"),TI18N("剩余购买次数:"), last_buy_time)
...
function GuildsecretareaMainWindow:updateRankInfo()
    ...
    for i=1,3 do
        ...
        if dps_data then
            table_insert(rank_list, {name = dps_data.name, all_dps = changeBtValueForBattle(tonumber(dps_data.dps))})
        else
    ...
    if rank_list and next(rank_list or {}) ~= nil then
        for i, v in ipairs(rank_list) do
            ...
            if item then
                ...
                if v.all_dps then
                    item.value:setString(TI18N("伤害") .. MoneyTool.GetMoneyString(changeBtValueForBattle(tonumber(v.all_dps)), false))
                    item.label:setPositionY(34)
                else
...
function GuildsecretareaMainWindow:createSingleRankItem(i,data)
    ...
    --sp:setScale(0.6)
    container.sp = sp
    local label = createLabel(18, Config.ColorData.data_new_color4[15], cc.c4b(0x22,0x01,0x01,0xff), 38, 20, "", container, 2, cc.p(0,0.5))
