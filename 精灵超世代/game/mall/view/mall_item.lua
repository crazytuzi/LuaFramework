...
function MallItem:configUI( ... )
	...
	self.goods_item = BackPackItem.new(true,true,false,0.9)
	...
	self.discount_label = createRichLabel(20,Config.ColorData.data_new_color4[6],cc.p(0,0),cc.p(150,10))
	self.discount_label:setString(TI18N(""))
...
function MallItem:setData( data )
	...
	if type(data.pay_type) == "number" then
		...
		if data.pay_type == self.special_item_id then
	...
	if is_show_limit_label == false then
		...
		elseif data["limit_day"] and data.limit_day>0 then
			...
			if data.shop_type==4 or data.shop_type == 2 then
				...
				-- 特殊物品消耗的商店
				if pay_type ~= self.special_item_id then
	...
	--折扣标签和折扣价格
	if data["label"] and data.label>0 and data.label<10 then --表里的
		self.discount:setVisible(true)
		-- self.discount_num:setString(data.label..TI18N("折"))
		local zhe_res = PathTool.getResFrame("common", MallConst.Variety_Zhe_Res[tonumber(data.label)])
		self.discount:loadTexture(zhe_res, LOADTEXT_TYPE_PLIST)
		self.discount:ignoreContentAdaptWithSize(true)
	elseif data["discount_type"] and data.discount_type>0 and data.discount_type<10 then --服务器的信息
		self.discount:setVisible(true)
    ...
    if self.data.type == MallConst.MallType.EliteShop and self.data.arena_elite_lev then
    	self:showEliteLevLimit(true, self.data.arena_elite_lev)
    else
    	self:showEliteLevLimit(false)
    end
...
function MallItem:showChipTag(status)
...
function MallItem:isShowLevLimit(status,lev)
...
function MallItem:initLimitGlevLayer()
	if not self.limit_glev_layer then
		...
		self.limit_glev_label = createRichLabel(20, 58, cc.p(0.5, 0), cc.p(211, 17))
		self.main_container:addChild(self.limit_glev_label, 9999)
	end
end

function MallItem:isShowGLevLimit(status, glev)
...
-- MallConst.MallType.EliteShop
function MallItem:showEliteLevLimit(status, elite_lev)
	self.limit_elite_status = nil
	
	if self.limit_glev_layer == nil and status == false then 
        return
    end
    if status then
	    local cur_lev = ElitematchController:getInstance():getModel():getEliteLev()
	    if cur_lev and cur_lev >= elite_lev then
	        status = false
	    end
	    if status then
		    self:initLimitGlevLayer()
		    ...
		    local limit_str 
	    	if config then
	        	limit_str = string.format(TI18N("<div fontcolor=#ffffff outline=2,#d95014>%s解锁</div>"), config.name) 
	        	self.limit_elite_status = config.name
	        else
	        	limit_str = TI18N("段位条件不满足")
	        	self.limit_elite_status = nil
	     	end
...
function MallItem:setSellAll( bool )
...
function MallItem:addCallBack( value )
	self.callback =  value
end

function MallItem:registerEvent()
		...
		if event_type == ccui.TouchEventType.ended then
				...
				if is_click == true then
                    ...
                    if self.limit_elite_status then 
                        message(string.format(TI18N("需段位达到%s解锁"), self.limit_elite_status))
                        return
                    end
        ...
        if self.role_update_event == nil then
            self.role_update_event = self.role_vo:Bind(RoleEvent.UPDATE_ROLE_ATTRIBUTE,function(key,value)
                ...
                if key == "gold" or key == "red_gold" or key =="hero_soul" or key == "arena_cent" or key == "friend_point" or key == "guild" then
					if self.pay_type and self.pay_type ~= self.special_item_id then
...
function MallItem:buyOneSuccess(data)
	local shop_num --商城类型
