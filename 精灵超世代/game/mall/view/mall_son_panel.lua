...
function MallSonPanel:configUI()
	...
	for i=1, 5 do
        ...
        tab_btn.label:setColor(Config.ColorData.data_new_color4[6])
    ...
    local scroll_view_size = cc.size(696,580)
    local setting = {
    ...
    self.item_scrollview = CommonScrollViewLayout.new(self.scrollCon, cc.p(0,0) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
end
...
function MallSonPanel:changeTabView( index )
    ...
    if self.cur_tab ~= nil then
        self.cur_tab.label:setColor(Config.ColorData.data_new_color4[6])
    ...
    if self.cur_tab ~= nil then
        self.cur_tab.label:setColor(Config.ColorData.data_new_color4[1])
...
function MallSonPanel:setTabBtnTouchStatus(status, index)
    ...
    if tab_btn then
        if status == true then
            ...
            tab_btn.label:setColor(Config.ColorData.data_new_color4[1])
