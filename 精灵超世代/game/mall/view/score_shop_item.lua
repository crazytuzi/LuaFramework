...
-- @Author: htp
-- @Editor: xhj
-- @date 2019/11/11 14:19:52
-- @description: 积分商城 item
---------------------------------

ScoreShopItem = class('ScoreShopItem',function()
	return ccui.Layout:create()
end)

function ScoreShopItem:ctor()
	self:configUI()
	self:registerEvent()
end
...
function ScoreShopItem:configUI( ... )
	self.root_wnd = createCSBNote(PathTool.getTargetCSB("mall/score_shop_item"))
	
    self:setAnchorPoint(cc.p(0, 1))
    self:addChild(self.root_wnd)
	self:setCascadeOpacityEnabled(true)
	self:setContentSize(cc.size(195,236))
	...
	self.name = self.main_container:getChildByName("name_txt")
	self.coin = self.main_container:getChildByName("coin")
	self.price = self.main_container:getChildByName("price")

	self.goods_item = BackPackItem.new(true,true,nil,0.9)
	self.goods_item:setPosition(self:getContentSize().width/2,self:getContentSize().height/2+25)
	self.main_container:addChild(self.goods_item)


	self.discount_label = createRichLabel(18,cc.c3b(24, 90, 03),cc.p(0.5,0),cc.p(self:getContentSize().width/2,65))
	...
	self.grey = self.main_container:getChildByName("got_shadow")
	self.grey:setLocalZOrder(20)
...
function ScoreShopItem:setData( data )
	...
	if limit_vip > 0 and is_show_limit_label then
		...
		self.discount_label:setString(string.format(TI18N("VIP %d 专属"),limit_vip))
		self:setSellAll( false )
		self.vip_limit_flag = limit_vip
	elseif limit_num>0 and is_show_limit_label then
		self.discount_label:setVisible(true)
		self.discount_label:setString(string.format(TI18N("%s %s/%s 个"),self.str,data.has_buy,limit_num))
	...
	elseif limit_rank>0 and is_show_limit_label then
		self.discount_label:setVisible(true)
		self.discount_label:setString(string.format(TI18N("需达到%d名"),limit_rank))
		self:setSellAll( false )
...
function ScoreShopItem:showChipTag(status)
...
function ScoreShopItem:isShowLevLimit(status,lev)
...
function ScoreShopItem:isShowGLevLimit(status, glev)
...
function ScoreShopItem:setSellAll( bool )
	self.grey:setVisible(bool)
	self:setTouchEnabled(not bool)
end

function ScoreShopItem:addCallBack( value )
	self.callback =  value
end

function ScoreShopItem:registerEvent()
	...
	if not self.buy_success_event then
		self.buy_success_event = GlobalEvent:getInstance():Bind(MallEvent.Buy_Success_Event,function ( data )
			...
			if self.data["id"] and data.eid == self.data.id and next(data.ext or {}) ~= nil then
				...
				self.discount_label:setString(string.format(TI18N("%s %s/%s 个"),self.str,self.data.has_buy,self.data.limit_num))
...
function ScoreShopItem:buyOneSuccess(data)
	local shop_num --商城类型
...
function ScoreShopItem:getData( )
	return self.data
end

function ScoreShopItem:createItemBgSprite()
	if not self.item_grid_bg then
		self.item_grid_bg = createSprite(PathTool.getPlistImgForDownLoad("bigbg/mall","skin_bg_3"), -30, 30, self, cc.p(0,1), LOADTEXT_TYPE, -1)
	end
	self.item_grid_bg:setVisible(true)
end

function ScoreShopItem:DeleteMe()
