...
function MallWindow2:open_callback()
    ...
    if self.background ~= nil then
        self.background:loadTexture(PathTool.getPlistImgForDownLoad("bigbg","bigbg_6",true), LOADTEXT_TYPE)
    ...
    self.btn_label = createRichLabel(24,Config.ColorData.data_new_color4[1],cc.p(0.5,0.5),cc.p(self.btn:getContentSize().width/2,self.btn:getContentSize().height/2))
    ...
    self.tips_bg = self.container:getChildByName('tips_bg')
    self.tips_bg:setVisible(false)
    self.tips_btn = self.container:getChildByName('tips_btn')
    self.tips_btn:setVisible(false)
    self.time = createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,0.5),cc.p(480,self.count:getPositionY() + 15))
    self.container:addChild(self.time)
    self.time:setVisible(false)
    self.time_down_text = createRichLabel(22,Config.ColorData.data_new_color4[6],cc.p(0,0.5),cc.p(480,self.count:getPositionY()-15))
...
function MallWindow2:setTimeFormatString(time)
    self.rest_time = time
    if time > 0 then
        self.time:setString(string.format(TI18N("免费刷新: <div fontcolor=%s>%s</div>"),Config.ColorData.data_new_color_str[12],TimeTool.GetTimeFormat(time)))
    else
        self.time:setString(string.format(TI18N("免费刷新: <div fontcolor=%s>00:00:00</div>"),Config.ColorData.data_new_color_str[12]))
    end
end
    ...
    if self.cur_shop_type == MallConst.MallType.Recovery then --神格
        if free_count <= 0 then
            ...
            if asset_cfg then
                ...
                btn_str = string.format(TI18N("<img src=%s scale=0.3 visible=true /><div shadow=0,-2,2,%s>%s重置</div>"), PathTool.getItemRes(Config.ItemData.data_get_data(bid).icon), Config.ColorData.data_new_color_str[3], num)
            end
        end
    elseif self.cur_shop_type == MallConst.MallType.SkillShop then --技能
        if free_count <= 0 then
            ...
            if config then
                ...
                btn_str = string.format(TI18N("<img src=%s scale=0.3 visible=true /><div shadow=0,-2,2,%s>%s刷新</div>"), PathTool.getItemRes(Config.ItemData.data_get_data(bid).icon), Config.ColorData.data_new_color_str[3], num)
            end
        else
            local asset_cfg = Config.ExchangeData.data_shop_exchage_cost["skill_refresh_free"] 
            if asset_cfg then
                btn_str = string.format("<div shadow=0,-2,2,%s>%s(%s/%s)</div>", Config.ColorData.data_new_color_str[3], TI18N("免费刷新"), free_count, asset_cfg.val)
            end
        end
