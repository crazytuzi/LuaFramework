...
function StoryTalk:refreshTopItem( data )
    ...
    if not self.topItem then
        ...
        self.topItem:setPosition(self.scroll_size.width*0.5, self.max_height+40)
        ...
        local name_txt = createLabel(24,Config.ColorData.data_new_color4[6],nil,nameBgSize.width/2,nameBgSize.height/2,data.name or "",nameBg,nil, cc.p(0.5, 0.5))

        -- 对话内容
        local item_msg = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0, 1), cc.p(20, bgSize.height-50), 10, nil, 628)
        item_msg:setString(WordCensor:getInstance():relapceFaceIconTag(data.msg or "")[2] or "")
        itemBg:addChild(item_msg)

        -- 箭头
        local arrow = createImage(itemBg, PathTool.getResFrame("common","common_2042"), bgSize.width-52, 27, cc.p(0.5, 0.5), true)
...
function StoryTalk:createItem(data)
    ...
    item:setPosition(self.scroll_size.width*0.5, self.max_height-16)
    ...
    -- 名称
    local name_txt = createLabel(22,Config.ColorData.data_new_color4[6],nil,20,100,data.name or "",itemBg,nil, cc.p(0, 0.5))

    -- 对话内容
    local item_msg = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 1), cc.p(20, bgSize.height-50), 10, nil, 606)
    item_msg:setString(WordCensor:getInstance():relapceFaceIconTag(data.msg or "")[2] or "")
    itemBg:addChild(item_msg)
