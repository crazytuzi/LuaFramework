...
function NewUntieRewardWindow1:open_callback()
    ...
    local label_lock = createRichLabel(22, cc.c4b(0x2b, 0x19, 0x5e, 0xff), cc.p(0.5, 0.5), cc.p(325, 336), nil, nil, 400)
    local str = string.format(TI18N("<div fontcolor=#fff99e >激活进阶赠礼，立即解锁</div><div fontcolor=#0cff01 >5阶</div><div fontcolor=#fff99e >奖励！</div>"))
...
function NewUntieRewardWindow1:setData()
    ...
    if card_list and card_list[period] and card_list[period] then
        ...
        if charge_id and charge_list[charge_id] then
            ...
            self.btn_change_label:setString(string.format("%s%s",GetSymbolByType(), charge_list[charge_id].val))
        end
    end
end
