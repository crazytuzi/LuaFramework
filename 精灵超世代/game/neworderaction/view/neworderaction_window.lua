...
function NewOrderactionWindow:open_callback()
    ...
    for i = 1, 3 do
        ...
        setLabelAutoScale(tab.name,tab.select,20)
        tab.index = i
        self.tab_view[i] = tab
    end
    ...
    self.btn_open_lock_label:setString(TI18N("解锁英灵殿"))
...
function NewOrderactionWindow:tabHeadTitle(index)
    if self.cur_herd_title ~= nil then
        ...
        self.cur_herd_title.name:setTextColor(Config.ColorData.data_new_color4[6])
        self.cur_herd_title.name:disableEffect(cc.LabelEffect.SHADOW)
    end
    self.cur_herd_title = self.tab_view[index]
    if self.cur_herd_title ~= nil then
        self.cur_herd_title.select:setVisible(true)
        self.cur_herd_title.name:setTextColor(Config.ColorData.data_new_color4[1])
        self.cur_herd_title.name:enableShadow(Config.ColorData.data_new_color4[2],cc.size(0, -2),2)
    end
end
...
function NewOrderActiodRewardItem:configUI()
    ...
    self.common_goods_item:setPosition(cc.p(170, 120 / 2))
end
