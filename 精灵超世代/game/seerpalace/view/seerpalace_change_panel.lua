...
function SeerpalaceChangePanel:ctor(  )
	...
	self.change_partner_id = 0 -- 有置换结果但未保存的宝可梦，0为没有
	self.change_new_partner_bid = 0 -- 有置换结果但未保存的新宝可梦，0为没有
	self.lock_partner_ids = {} -- 锁住不能置换的宝可梦
	self.cancel_partner_id = 0 -- 缓存取消保存的宝可梦id，取消之后要依然选中它
...
function SeerpalaceChangePanel:configUI(  )
    ...
    self.change_btn_label = createRichLabel(22, cc.c4b(255,255,255,255), cc.p(0.5, 0.5), cc.p(btn_size.width/2, btn_size.height/2))
    ...
    self.role_scrollview = CommonScrollViewLayout.new(self.role_layout, cc.p(40,88) , ScrollViewDir.horizontal, ScrollViewStartPos.top, scroll_view_size, setting)
...
function SeerpalaceChangePanel:refreshRoleList(  )
	...
	self.role_scrollview:addEndCallBack(function (  )
		if self.cur_role_vo and next(self.cur_role_vo) ~= nil and self.cur_camp_type then
			-- 记录了上一次选中宝可梦的数据，切换到全部或该宝可梦阵营时，该宝可梦继续为选中状态
		...
		elseif self.cancel_partner_id and self.cancel_partner_id ~= 0 then --选中取消置换的宝可梦
			local list = self.role_scrollview:getItemList()
			for k,v in pairs(list) do
				local data = v:getData()
...
function SeerpalaceChangePanel:_onClickPartner( item, vo )
	...
	if cost_config and cost_config.val then
		...
		if item_config then
			label_str = string.format("<img src=%s visible=true scale=0.5 /><div fontColor=#ffffff fontsize=%d outline=2,#6c2b00>%d %s</div>", PathTool.getItemRes(item_config.icon), 22, num, TI18N("转换"))
		end
	end
		...
		if left_role_vo and next(left_role_vo) ~= nil then
			...
			if not self.left_role_bid or not self.left_role_star or self.left_role_bid ~= left_role_vo.bid or self.left_role_star ~= left_role_vo.star then
			    ...
			    if not self.left_name_label then
			    	self.left_name_label = createRichLabel(24, 1, cc.p(0.5, 0.5), cc.p(100, 352))
		    ...
		    if not self.right_name_label then
		    	self.right_name_label = createRichLabel(24, 1, cc.p(0.5, 0.5), cc.p(100, 352))
			...
			local _cStar = function(star_count, res, star_list, parent_node)
		        ...
		        for i=1,star_count do
		            if not star_list[i] then 
		                local star = createImage(parent_node, res, 0, 318, cc.p(0.5, 0.5), true)
		                star_list[i] = star
		            end
		    ...
		    elseif role_star >= 10 then
		        ...
		        if self.left_star10 == nil then
		            self.left_star10 = createImage(self.left_panel, res, 100, 318,cc.p(0.5,0.5),true,0,false)
		            self.left_star10:setScale(1.2)
		        else
		            self.left_star10:setVisible(true)
		        end
		        if self.right_star10 == nil then
		            self.right_star10 = createImage(self.right_panel, res, 100, 318,cc.p(0.5,0.5),true,0,false)
