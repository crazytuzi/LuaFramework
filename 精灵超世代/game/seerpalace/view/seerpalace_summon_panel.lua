...
function SeerpalaceSummonPanel:configUI(  )
    ...
    self.btn_summon_label = createRichLabel(24, 1, cc.p(0.5, 0.5), cc.p(btn_size.width/2, btn_size.height/2))
...
function SeerpalaceSummonPanel:updateSummonBtnLabel(  )
    if self.select_card then
        ...
        if summon_cost then
            ...
            if item_config then
                self.btn_summon_label:setString(string.format(TI18N("<img src='%s' scale=0.3 /><div outline=2,%s>%d 召唤</div>"), PathTool.getItemRes(item_config.icon), Config.ColorData.data_new_color_str[16], num))
            end
        end
    else
        self.btn_summon_label:setString(string.format(TI18N("<div outline=2,%s>召唤</div>"), Config.ColorData.data_new_color_str[16]))
    end
end
...
function SeerpalaceSummonItem:configUI(  )
    ...
    self.ball = container:getChildByName("ball")
    self.brights = container:getChildByName("brights")
    self.check_bg = container:getChildByName("check_bg")

    self.brights:setVisible(false)
end
...
function SeerpalaceSummonItem:setIndex( index )
    ...
    local ball_res = PathTool.getResFrame("seerpalace", "seerpalace_ball_"..index)
    self.ball:loadTexture(ball_res, LOADTEXT_TYPE_PLIST)

    -- 引导需要
    -- self.summon_layer:setName("guide_card_" .. index)
    self.ball:setName("guide_card_" .. index)
end
...
function SeerpalaceSummonItem:register_event(  )
    ...
    registerButtonEventListener(self.check_bg, handler(self, self._onClickCheckLayer))
    registerButtonEventListener(self.ball, handler(self, self._onClickSummonLayer))
end
...
function SeerpalaceSummonItem:setSelectStatus( status )
    local time = 0.5
    local upward = 30
    local pos_x = self.ball:getPositionX()
    local pos_y = self.ball:getPositionY()
    if status == true then
        ...
        local ball_move_to = cc.MoveTo:create(time, cc.p(pos_x, pos_y+upward))
        local ease_out = cc.EaseSineOut:create(ball_move_to)
        self.ball:runAction(cc.Sequence:create(ease_out))

        self.brights:setVisible(true)
    else
