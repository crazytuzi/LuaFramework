...
function SeerpalaceMainWindow:open_callback(  )
    ...
    self.tips_label:setString(TI18N("随机捕捉4~5星宝可梦或其宝可梦碎片"))
    ...
    self.explain_btn = tab_container:getChildByName("explain_btn")
    ...
    tab_y = self.tips_label:getPositionY()
    self.tips_label:setPositionY(top_off - (container_size.height - tab_y))

    tab_y = self.btn_shop:getPositionY()
    self.btn_shop:setPositionY(top_off - (container_size.height - tab_y))
    tab_y = self.btn_gift:getPositionY()
	self.btn_gift:setPositionY(top_off - (container_size.height - tab_y))
	tab_y = self.btn_score_summons:getPositionY()
