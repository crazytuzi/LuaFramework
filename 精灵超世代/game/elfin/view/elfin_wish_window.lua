...
function ElfinWishWindow:open_callback(  )
    ...
    self.choose_tips_txt = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 1), cc.p(90, 660),nil,nil,550)
    container:addChild(self.choose_tips_txt)
    self.choose_tips_txt:setString(TI18N("在精灵召唤时，有<div fontcolor=#d63636>更高概率</div>获得许愿池中许愿的精灵（获得橙色精灵总概率不变）"))
end
...
function ElfinWishWindow:showCommonAlert( is_close )
	local str = TI18N('许愿池尚未加入精灵，现在召唤仍有概率获得橙色品质宝可梦，是否退出？')
	if is_close == true then
		...
		-- str = TI18N('许愿池尚未加入精灵，现在召唤仍有概率获得橙色品质宝可梦，是否卸下？')
		local function fun()
...
function ElfinWishWindow:updateIconShow(  )
	if self.cur_item_id_list and #self.cur_item_id_list>0 then
		...
		if self.item_node then
			...
			self.item_node:setPosition(cc.p(588*0.5, 176*0.5))
			self.elfin_info_panel:addChild(self.item_node)
			self.item_node:setData()
			self.item_node:showFlagByRes(true, PathTool.getResFrame("elfin", "elfin_1045"), 35, 90)
		end
	end
end
...
function ElfinWishItem:showItemQualityName( status )
	if self.item_node then
		if status == true then
