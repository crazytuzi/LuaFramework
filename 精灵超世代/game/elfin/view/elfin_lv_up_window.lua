...
function ElfinLvUpWindow:openRootWnd(id)
	...
	if id then
		...
		if hatch_cfg then
			...
			local res = ""
			local res_id = hatch_cfg.res_id
			if res_id == 1 then
				res = PathTool.getResFrame("elfin", "elfin_1047")
			else
				res = PathTool.getResFrame("elfin", "elfin_1046")
			end
			loadSpriteTexture(self.egg_home_sp, res, LOADTEXT_TYPE_PLIST)
		end
	end
