...
function ElfinAdjustWindow:open_callback(  )
	...
	self.tips_txt = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(338, 305), nil, nil, 500)
...
function ElfinAdjustWindow:setData(  )
    ...
    if #self.all_elfin_data > 0 then
        ...
        commonShowEmptyIcon(self.list_view, true, {text = TI18N("一个精灵都没有噢，快去孵化获取吧~"), font_size = 22})
    end
end
        ...
        if item_bid then
            ...
            if elfin_bid == 0 or not elfin_cfg then -- 已解锁，但未放置精灵
                ...
                if skill_cfg then
                    ...
                    skill_item:showName(true,skill_cfg.name,nil,20,true,Config.ColorData.data_new_color4[6],nil,cc.size(110,26))
