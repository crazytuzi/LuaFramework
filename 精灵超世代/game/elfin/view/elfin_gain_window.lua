...
function ElfinGainWindow:open_callback(  )
	...
	self.title_sp = self.container:getChildByName("success_bg"):getChildByName("Sprite_1")
	loadSpriteTexture(self.title_sp, "resource/elfin/txt_elfin/txt_elfin_2.png", LOADTEXT_TYPE)

	self.elfin_sp = self.container:getChildByName("elfin_sp")
    ...
    self.power_label:setPosition(cc.p(520, 225))
...
function ElfinGainWindow:setData( data )
	...
	if extra_awards and next(extra_awards) ~= nil then
		if not self.extra_txt then
			self.extra_txt = createRichLabel(24, cc.p(255,232,183,255), cc.p(0.5, 0.5), cc.p(490, 110))
			self.container:addChild(self.extra_txt)
		end
		local txt_str = TI18N("<div fontcolor=#3d5078>同时获得</div>")
		for i,item_info in ipairs(extra_awards) do
			local item_res = PathTool.getItemRes(item_info.item_cfg.icon)
			txt_str = txt_str .. _string_format("<div fontcolor=#0e7709>+</div> <img src='%s' scale=0.3 /> <div fontcolor=#0e7709>%d</div>", item_res, item_info.item_num)
		end
