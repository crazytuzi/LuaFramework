...
function HomeworldVisitWindow:open_callback(  )
	...
	self.image_3 = self.rank_panel:getChildByName("image_3")
	...
	if not self.sub_tab_scrollview then
		local panel_size = btn_panel:getContentSize()
		self.sub_tab_scrollview = CommonSubBtnList.new(btn_panel, cc.p(0.5, 0.5), cc.p(panel_size.width*0.5, panel_size.height*0.5), cc.size(168, 66), handler(self, self._onClickSubTabBtn))
	end
	self.sub_tab_scrollview:setData(self.sub_tab_array, 2)
	self.sub_tab_scrollview:setScale(0.8)
end
...
function HomeworldVisitWindow:updateWorshipNum(  )
	if not self.left_worship_num then
		self.left_worship_num = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(35, 720))
	...
	if max_num_cfg then
		self.left_worship_num:setString(_string_format(TI18N("今日可点赞次数:<div fontcolor=#0cff01>%d</div>/%d"), left_num, max_num_cfg.val))
	end
end
...
function HomeworldVisitWindow:updateRankList( data )
	...
	if not self.my_rank_txt then
		self.my_rank_txt = createLabel(26, Config.ColorData.data_new_color4[6], nil, 90, 136, nil, self.rank_panel, nil, cc.p(0.5, 0.5))
	end
	...
	if not self.my_name_txt then
		self.my_name_txt = createLabel(22, Config.ColorData.data_new_color4[6], nil, 238, 136, nil, self.rank_panel, nil, cc.p(0, 0.5))
	end
	self.my_name_txt:setString(role_vo.name)

	if not self.my_soft_txt then
		self.my_soft_txt = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(480, 136))
