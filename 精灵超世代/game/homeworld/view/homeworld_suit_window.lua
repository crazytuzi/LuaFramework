...
function HomeworldSuitWindow:updateSuitBaseInfo(  )
	...
	self.num_label:setString(_string_format(TI18N("%d"), self.suit_cfg.comfort))
	...
	if not self.desc_txt then
		self.desc_txt = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(185, 765), 5, nil, 380)
...
function HomeworldSuitItem:configUI(  )
	...
	self.btn_info = self.container:getChildByName("btn_info")
	self.btn_info:getChildByName("detail_txt"):setString(TI18N("详情"))
end

function HomeworldSuitItem:register_event(  )
	registerButtonEventListener(self.btn_info, function (  )
		if self.data then
...
function HomeworldSuitItem:setData( data )
	...
	if not self.confort_txt then
		self.confort_txt = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(self.size.width*0.5, 205))
...
function HomeworldSuitAward:configUI(  )
	...
	self.image_bg = createImage(self.root_wnd, PathTool.getResFrame("common", "common_1008"), self.size.width*0.5, self.size.height*0.5, cc.p(0.5, 0.5), true, nil, true)
	self.image_bg:setCapInsets(cc.rect(56, 56, 1, 1))
...
function HomeworldSuitAward:setData( data, is_get, cur_num )
	...
	if data.reward and data.reward[1] then
		...
		if item_config then
			if not self.award_icon then
				...
				self.award_icon = createSprite(res, self.size.width*0.5, self.size.height*0.5, self.root_wnd, cc.p(0.5, 0.5), LOADTEXT_TYPE)
				self.award_icon:setScale(0.4)
			end
			if not self.award_num then
				self.award_num = createLabel(14, Config.ColorData.data_new_color4[1], Config.ColorData.data_new_color4[6], self.size.width-8, 50, num, self.root_wnd, 2, cc.p(1, 0))
			end
		end
	end
	...
	if is_get == true then
		if not self.get_icon then
			self.get_icon = createSprite("resource/homeworld/txt_homeworld/txt_cn_homeworld_2.png", 30, 35, self.root_wnd, cc.p(0.5, 0.5), LOADTEXT_TYPE)
		end
