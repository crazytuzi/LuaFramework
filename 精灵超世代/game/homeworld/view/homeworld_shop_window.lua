...
function HomeworldShopWindow:initShopTypeBtn( index)
    ...
    local setting = {
        ...
        item_height = 55,              -- 单元的尺寸height
        row = 1,                        -- 行数，作用于水平滚动类型
        col = 0,                         -- 列数，作用于垂直滚动类型
    }
    self.tab_scrollview = CommonScrollViewLayout.new(self.shop_btn_panel, cc.p(0, 0) , ScrollViewDir.horizontal, ScrollViewStartPos.top, scroll_view_size, setting)
    self.tab_scrollview:setData(self.tab_array, handler(self, self._onClickTabBtn), nil, {default_index = index, tab_size = cc.size(128, 55),title_offset = cc.p(0,-5)})
...
function HomeworldShopWindow:updateSubTabBtn( index , default_index)
	...
	if index == 1 or index == 2 then
		...
		if not self.sub_tab_list then
			...
			self.sub_tab_list = CommonSubBtnList.new(self.shop_sub_panel, cc.p(0.5, 0.5), cc.p(panel_size.width*0.5, panel_size.height*0.5), cc.size(168, 66), handler(self, self._onClickSubTabBtn))
			self.sub_tab_list:setScale(0.8)
		end
