...
local MiniRoleAnimForAction = {
    [MiniRoleAction.idle] = "standby_loop",
    [MiniRoleAction.move] = "run_loop",
    [MiniRoleAction.caught_1] = "standby_loop",
    [MiniRoleAction.caught_2] = "standby_loop",
    [MiniRoleAction.caught_3] = "standby_loop",
    [MiniRoleAction.interaction] = "standby_loop",
}

local ROLE_SCALE_DEFAULT = 1
	...
	self.root_wnd:addTouchEventListener(function ( sender, event_type )
        ...
        elseif event_type == ccui.TouchEventType.ended or event_type == ccui.TouchEventType.canceled then
            ...
            if is_move == false then
                ...
                self:playRoleAction(MiniRoleAction.caught_3)
            end
...
function HomeworldRole:onClickRole(  )
    self:showRoleMoveAction(false)
	self:playRoleAction(MiniRoleAction.interaction)
end
...
function HomeworldRole:moveRoleByPos( pos )
    ...
    if not self.show_move_action then
        ...
        self:playRoleAction(MiniRoleAction.caught_1)
    end
end
...
function HomeworldRole:setData( data, is_my_home_role, is_owner )
    ...
    if self:getRandomGridPos() then
        ...
        local anim_name = MiniRoleAnimForAction[MiniRoleAction.idle]
        self.role_spine = createEffectSpine( effect_id, cc.p(60, 0), cc.p(0.5, 0), true, anim_name, handler(self, self._onRoleActionEndCallback) )
        self.role_spine:setScale(ROLE_SCALE_DEFAULT)
        self.root_wnd:addChild(self.role_spine)
        self.cur_role_action = MiniRoleAction.idle
...
function HomeworldRole:_onRoleActionEndCallback(  )
	-- 交互动作播放完毕则恢复闲转动作
	if self.cur_role_action == MiniRoleAction.interaction then
		self:playRoleAction(MiniRoleAction.idle)
        self:showRoleMoveAction(true)
    elseif self.cur_role_action == MiniRoleAction.caught_1 then
        self:playRoleAction(MiniRoleAction.caught_2)
    elseif self.cur_role_action == MiniRoleAction.caught_3 then
        self:playRoleAction(MiniRoleAction.idle)
        self:showRoleMoveAction(true)
	end
end
...
function HomeworldRole:playRoleAction( action )
    ...
    if action == MiniRoleAction.caught_1 then -- 提起来
        AudioManager:getInstance():playEffect(AudioManager.AUDIO_TYPE.COMMON, "c_home_character01")
    elseif action == MiniRoleAction.caught_3 then -- 放下去
        AudioManager:getInstance():playEffect(AudioManager.AUDIO_TYPE.COMMON, "c_home_character02")
    end

	local is_loop = true
	if action == MiniRoleAction.interaction or action == MiniRoleAction.caught_1 or action == MiniRoleAction.caught_3 then
		is_loop = false
	end
    self.role_spine:setToSetupPose()
    local anim_name = MiniRoleAnimForAction[action]
	self.role_spine:setAnimation(0, anim_name, is_loop)
    ...
    if act_cfg then
        ...
        elseif act_name == "move" then -- 移动动作
            ...
            if grids and next(grids) ~= nil then
                self:playRoleAction(MiniRoleAction.move)
                ...
                if grids[3] == Move_Dir.Left_Top or grids[3] == Move_Dir.Left_Down then
                    self.role_spine:setScaleX( 0 - ROLE_SCALE_DEFAULT)
                else
                    self.role_spine:setScaleX(ROLE_SCALE_DEFAULT)
                end
