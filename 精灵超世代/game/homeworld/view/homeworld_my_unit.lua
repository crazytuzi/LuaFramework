...
function HomeworldMyUnit:createRoorWnd(  )
    ...
    self.root_wnd:setPosition(cc.p(0, 225))
...
function HomeworldMyUnit:initTypeBtnList(  )
    ...
    local setting = {
        ...
        start_y = 0,                    -- 第一个单元的Y起点
        space_y = 0,                   -- y方向的间隔
        item_width = 147,               -- 单元的尺寸width
        item_height = 45,              -- 单元的尺寸height
        row = 1,                        -- 行数，作用于水平滚动类型
        col = 0,                         -- 列数，作用于垂直滚动类型
    }
    ...
    self.tab_scrollview:setData(self.tab_array, handler(self, self._onClickTabBtn), nil, {default_index = 1, tab_size = cc.size(147, 45), red_offset = cc.p(7, -10), red_scale = 0.7,title_offset = cc.p(0,-5)})
...
function HomeworldMyUnit:initUnitPanel(  )
    ...
    if not self.sub_tab_scrollview then
        ...
        self.sub_tab_scrollview = CommonSubBtnList.new(self.sub_btn_panel, cc.p(0.5, 0.5), cc.p(panel_size.width*0.5, panel_size.height*0.5), cc.size(168, 66), handler(self, self._onClickSubTabBtn))
    end
    self.sub_tab_scrollview:setData(self.sub_tab_array, 1)
    self.sub_tab_scrollview:setGuideName("my_unit_")
    self.sub_tab_scrollview:setScale(0.7)
end
