...
function PlanesBoardWindow:onClickComfirmBtn(  )
	...
	if self.board_id == PlanesConst.Recover_Id or self.board_id == PlanesConst.Revive_Id or self.board_id == PlanesConst.Switch_Id or self.board_id == PlanesConst.DesBarrier_Id then
		if self.board_id == PlanesConst.Recover_Id and not _model:checkIsHaveHpNotFullHero() then -- 回复泉水时，所有宝可梦都是满血
			CommonAlert.show(TI18N("当前所有非阵亡宝可梦均满血，使用回复泉水将不会有效，是否继续？"), TI18N("确认"), function (  )
				_controller:sender23104( self.grid_index, 1, {} )
			end, TI18N("取消"))
		elseif self.board_id == PlanesConst.Revive_Id and not _model:checkIsHaveDieHero() then -- 复活时，没有死亡的宝可梦
