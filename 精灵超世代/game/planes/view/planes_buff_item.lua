...
function PlanesBuffItem:configUI()
    ...
    self.pnl_scr = container:getChildByName("pnl_scr")
    self.scroll_desc = self.pnl_scr:getChildByName("scroll_desc")
    self.scroll_desc:setScrollBarEnabled(false)
    self.scroll_size = self.scroll_desc:getContentSize();

    self.desc_txt = createRichLabel(16, Config.ColorData.data_new_color4[1], cc.p(0, 1),
        cc.p(0, self.pnl_scr:getContentSize().height), nil, nil, self.size.width - 24)
    -- self.scroll_desc:setInnerContainerSize(cc.size(self.desc_txt.width, self.desc_txt.height))
    self.scroll_desc:addChild(self.desc_txt)
end
...
function PlanesBuffItem:setData(buff_cfg)
    ...
    -- 描述
    self.desc_txt:setString("<div outline=2,#3d5078>" .. self.buff_cfg.desc .. "</div>")
    local txt_size = self.desc_txt:getContentSize();
    local max = math.max(txt_size.height, self.scroll_size.height)
    self.scroll_desc:setContentSize(cc.size(self.scroll_size.width, max))
    self.scroll_desc:setInnerContainerSize(cc.size(self.scroll_size.width, math.min(max, self.scroll_size.height)))
    -- 图标
    ...
    if range_cfg and range_cfg[1] and next(range_cfg[1]) ~= nil then
        ...
        elseif effect_type == "partner" then -- 宝可梦
