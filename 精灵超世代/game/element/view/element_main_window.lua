...
function ElementMainWindow:__init(  )
	...
	self.res_list = {
        ...
        {path = PathTool.getPlistImgForDownLoad("bigbg/element","element_main_bg"), type = ResourcesType.single },
...
function ElementMainWindow:open_callback(  )
	...
	if self.background then
		self.background:loadTexture(PathTool.getPlistImgForDownLoad("bigbg/element","element_main_bg"), LOADTEXT_TYPE)
	...
	loadSpriteTexture(sp_land_1, PathTool.getPlistImgForDownLoad("bigbg/element","element_land_1"), LOADTEXT_TYPE)
	self.land_list[ElementConst.Ele_Type.Water] = sp_land_1
	local sp_land_2 = self.main_container:getChildByName("sp_land_2")
	loadSpriteTexture(sp_land_2, PathTool.getPlistImgForDownLoad("bigbg/element","element_land_2"), LOADTEXT_TYPE)
	self.land_list[ElementConst.Ele_Type.Fire] = sp_land_2
	...
	loadSpriteTexture(sp_land_4, PathTool.getPlistImgForDownLoad("bigbg/element","element_land_4"), LOADTEXT_TYPE)
	self.land_list[ElementConst.Ele_Type.Light] = sp_land_4
	local sp_land_5 = self.main_container:getChildByName("sp_land_5")
	loadSpriteTexture(sp_land_5, PathTool.getPlistImgForDownLoad("bigbg/element","element_land_5"), LOADTEXT_TYPE)
	self.land_list[ElementConst.Ele_Type.Dark] = sp_land_5
...
function ElementMainWindow:updateLeftBuyCount(  )
    if not self.left_buy_count then
        self.left_buy_count = createRichLabel(18, 1, cc.p(0.5, 0.5), cc.p(580, 140))
	...
	if not data.endtime or data.endtime <= 0 or data.endtime <= cur_time then -- 没开启
		...
		if monster_cfg then
			if not self.hero_head then
				...
				self.hero_head:setScale(0.65)
				self.hero_head:showBg()
				self.hero_head:setPosition(cc.p(10, 34))
