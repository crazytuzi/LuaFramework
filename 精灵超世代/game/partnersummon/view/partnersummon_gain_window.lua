...
function PartnerSummonGainWindow:__init(is_call)
    ...
    self.res_list = {
        ...
        { path = PathTool.getPlistImgForDownLoad("bigbg", "bigbg_13"), type = ResourcesType.single },
        { path = PathTool.getPlistImgForDownLoad("common/txt_common", "txt_common_1"), type = ResourcesType.single },
    }
...
function PartnerSummonGainWindow:createRootWnd()
    ...
    self.item_bg = createScale9Sprite(PathTool.getPlistImgForDownLoad("bigbg", "bigbg_13"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 55, LOADTEXT_TYPE, self.source_container)
    self.item_bg:setAnchorPoint(cc.p(0.5, 0.5))
    self.item_bg:setVisible(false)
    -- self.item_bg:setContentSize(cc.size(SCREEN_WIDTH + 100,640))

    self.title_bg = createSprite(PathTool.getPlistImgForDownLoad("common/txt_common", "txt_common_1"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 525, self.source_container, cc.p(0.5, 0.5), LOADTEXT_TYPE)
    ...
    self.comfirm_next_btn = createButton(self.btn_container, TI18N("确定"), 555, 120, cc.size(220, 77), PathTool.getResFrame("common", "common_1017"), 24)
    self.comfirm_next_btn:setRichText(string.format(TI18N("<div fontColor=#ffffff fontsize=34 shadow=0,-2,2,%s>确定</div>"), Config.ColorData.data_new_color_str[3]))
    ...
    if self.is_call == TRUE then
        self.again_btn = createButton(self.item_container,TI18N("再抽一次"),470,35,cc.size(168,66),PathTool.getResFrame("common","common_1018"))
        self.again_btn:setRichText(string.format(TI18N("<div fontColor=#ffffff fontsize=%d shadow=0,-2,2,%s>再抽一次</div>"), 22, Config.ColorData.data_new_color_str[2]))
        local again_btn_size = self.again_btn:getContentSize()
        self.item_label = createRichLabel(20, 0xfff99e, cc.p(0.5, 0.5), cc.p(again_btn_size.width/2,90),0,0,500)
        self.item_label:setString("")
        self.again_btn:addChild(self.item_label)
        self.comfirm_btn = createButton(self.item_container,TI18N("确定"),255,35,cc.size(168,66),PathTool.getResFrame("common","common_1017"))
        self.comfirm_btn:setRichText(string.format(TI18N("<div fontColor=#ffffff fontsize=%d shadow=0,-2,2,%s>确定</div>"), 22, Config.ColorData.data_new_color_str[3]))
...
function PartnerSummonGainWindow:openRootWnd(data,is_call)
    ...
    if self.group_id ~= PartnersummonConst.Summon_Type.Score and config then
        ...
        self.comfirm_btn:setPosition(cc.p(255, 35))
    else
        self.again_btn:setVisible(false)
        self.comfirm_btn:setPosition(cc.p(self.item_container:getContentSize().width/2, 35))
    end
...
function PartnerSummonGainWindow:updateEffectAction()
    ...
    local action_bg = "action"
    if self.config_data[self.group_id] then
        action = self.config_data[self.group_id].action_name
        action_bg = self.config_data[self.group_id].action_bg_name
    end
    action = "shibai2_effect"
    ...
    if self.type_flag == 0 then
        self.first_effect_spine_bg = self:playEffect(Config.EffectData.data_effect_info[1774],action_bg,SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 99, self.first_container)
        self.first_effect_spine_bg:setAnchorPoint(cc.p(0.5,0.5))
        self.first_effect_spine_bg:setScale(1.5)

        self.first_effect_spine = self:playEffect(Config.EffectData.data_effect_info[120],action,SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 99, self.first_container)
        ...
        local function animationEventFunc(event)
            --if event.eventData.name == "appear" then
                ...
                if self.first_effect_spine_bg then
                    self.first_effect_spine_bg:runAction(cc.RemoveSelf:create(true))
                    self.first_effect_spine_bg = nil
                end
                self.image_bg:setVisible(true)
                if not self.is_show_item then
                    if next(self.role_list or {}) == nil and self.is_call == TRUE then --表示这个没有宝可梦抽出
        ...
        self.first_effect_spine:registerSpineEventHandler(animationEventFunc, sp.EventType.ANIMATION_COMPLETE)
    else
...
function PartnerSummonGainWindow:handleFloorEffect( status )
    if status == true then
        if not tolua.isnull(self.first_container) and self.floor_effect == nil then
            local action = "fanhui_effect" -- self.floor_action or PlayerAction.action_1
            ...
            self.floor_effect = createEffectSpine(Config.EffectData.data_effect_info[120], cc.p(SCREEN_WIDTH/2, 370), cc.p(0.5, 0.5), false, action)
            ...
            --     -- end
            -- end
            -- self.floor_effect:registerSpineEventHandler(animationEventFunc, sp.EventType.ANIMATION_COMPLETE)
        end
    else
...
function PartnerSummonGainWindow:handleLightEffect( status )
    if status == true then
        if not tolua.isnull(self.first_container) and self.light_effect == nil then
            ...
            --     end
            -- end
            local action = "cjq_diuqiu_effect" -- self.light_action or PlayerAction.action_1
            ...
            self.light_effect = createEffectSpine(Config.EffectData.data_effect_info[120], cc.p(SCREEN_WIDTH/2, 400), cc.p(0.5, 0.5), false, action)
            self.first_container:addChild(self.light_effect, 102)

            local function animationEventFunc(event)
                -- if event.eventData.name == "appear" then
                    self:handleBookEffect(true)
                    self.light_effect:setVisible(false)
                -- end
            end
            self.light_effect:registerSpineEventHandler(animationEventFunc, sp.EventType.ANIMATION_COMPLETE)
        end
    else
...
function PartnerSummonGainWindow:handleBookEffect( status )
    if status == true then
        if not tolua.isnull(self.first_container) and self.book_effect == nil then
            ...
            -- self.book_effect = createEffectSpine(Config.EffectData.data_effect_info[120], cc.p(SCREEN_WIDTH/2, 400), cc.p(0.5, 0.5), false, "cjq_chenggong_effect")
            -- self.first_container:addChild(self.book_effect, 101)

            local function animationEndFunc(event)
                self.image_bg:setVisible(true)
                self.book_effect:setVisible(false)
                if self.time_summon_bg then
                    self.time_summon_bg:setVisible(false)
                end
                if not self.is_show_item then
            ...
            local action = "cjq_chenggong_effect"
            self.book_effect = createEffectSpine(Config.EffectData.data_effect_info[120], cc.p(SCREEN_WIDTH/2, 400), cc.p(0.5, 0.5), false, action, animationEndFunc)
            self.first_container:addChild(self.book_effect, 101)
        end
    else
...
function PartnerSummonGainWindow:showHeroDetailInfo(  )
    ...
    if hero_cfg then
        ...
        for _, cfg in pairs(self.role_list) do -- 可能同时召唤出两个一样的宝可梦
        ...
        local type_res_str = "txt_cn_common_career_" .. hero_cfg.type
        local type_res = PathTool.getResFrame("common", type_res_str)
        ...
        self.is_show_enter_ani = true -- 标识是否正在显示宝可梦详细信息的入场动画
        if hero_cfg.star >= 5 then
...
function PartnerSummonGainWindow:showDrawEffect1( status )
    if status == true then
        if not tolua.isnull(self.source_container) and self.draw_effect_1 == nil then
            self.draw_effect_1 = createEffectSpine(Config.EffectData.data_effect_info[1317], cc.p(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2), cc.p(0.5, 0.5), true, PlayerAction.action_2)
...
function PartnerSummonGainWindow:quickShowHeroInfo(  )
    ...
    local camp_pos_x = 360 - self.hero_name_txt:getContentSize().width*0.5 - 25
    self.hero_camp_sp:setPosition(cc.p(camp_pos_x, 1100))
    self.hero_name_txt:stopAllActions()
    self.hero_name_txt:setOpacity(255)
    self.hero_name_txt:setPosition(cc.p(360, 1100))
...
function PartnerSummonGainWindow:createHeroInfoNode(  )
    ...
    self.hero_share_btn:setVisible(false)
...
function PartnerSummonGainWindow:handleFlickerEffect(status, action_name)
    if status == true then
        ...
        if not tolua.isnull(self.draw_container) and self.flicker_effect == nil then
            ...
            self.flicker_effect:registerSpineEventHandler(animationEventFunc, sp.EventType.ANIMATION_COMPLETE)
        ...
        if not tolua.isnull(self.draw_container) and self.higher_flicker_effect == nil then
            ...
            local function animationEventFunc(event)
                -- if event.eventData.name == "appear" then
                    ...
                    self:showDrawEffect4(true)
                    self.higher_flicker_effect:setVisible(false)
                -- end
            end
            self.higher_flicker_effect:registerSpineEventHandler(animationEventFunc, sp.EventType.ANIMATION_COMPLETE)
...
function PartnerSummonGainWindow:showHeroInfoEnterAni(  )
    ...
    self.quick_show_voice = true -- 点击快速展示时，是否播放宝可梦的音效
    ...
    self.hero_name_txt:runAction(cc.Spawn:create(cc.FadeIn:create(0.25), cc.MoveTo:create(0.25, cc.p(360, 1100))))
    local camp_pos_x = 360 - self.hero_name_txt:getContentSize().width*0.5 - 25
    self.hero_camp_sp:runAction(cc.Spawn:create(cc.FadeIn:create(0.25), cc.MoveTo:create(0.25, cc.p(camp_pos_x, 1100))))
    ...
    self.hero_type_bg:runAction(cc.MoveTo:create(0.3, cc.p(81, 360)))
...
function PartnerSummonGainWindow:updateItemData(data)
    ...
    self.title_bg:setPosition(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 370)
    if sum >= col then
        sum = col
    end
    ...
    if self.is_call == TRUE then
        if self.tiems == 10 and self.item_label and self.five_icon_item then
            self.again_btn:setRichText(string.format(TI18N("<div fontColor=#ffffff fontsize=%d shadow=0,-2,2,%s>再抽十次</div>"), 22, Config.ColorData.data_new_color_str[2]))
...
function PartnerSummonGainWindow:updateTenSummon(  )
    if self.type_flag == 1 and self.item_label then
        ...
        if summon_have_num >= self.tiems then
            ...
            self.item_label:setString(string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#35ff14 outline=2,%s>%d</div><div fontColor=%s outline=2,%s>/%d</div>"), PathTool.getItemRes(item_icon), Config.ColorData.data_new_color_str[6], summon_have_num, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], self.tiems))
        else
            self._item_enough = false
            self.item_label:setString(string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#e14737 outline=2,%s>%d</div><div fontColor=%s outline=2,%s>/%d</div>"), PathTool.getItemRes(item_icon), Config.ColorData.data_new_color_str[6], summon_have_num, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], self.tiems))
        end
        return
    ...
    local str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#35ff14 outline=2,%s>%d</div><div fontColor=%s outline=2,%s>/%d</div>"), PathTool.getItemRes(item_icon), Config.ColorData.data_new_color_str[6], have_count, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], need_count)
    if have_count < need_count then
        str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#e14737 outline=2,%s>%d</div><div fontColor=%s outline=2,%s>/%d</div>"), PathTool.getItemRes(item_icon), Config.ColorData.data_new_color_str[6], have_count, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], need_count)
    end
...
function PartnerSummonGainWindow:updateSingleSummon()
    ...
    if free_count > 0 then --如果免费CD结束时间为0
        ...
        desc_str = string.format(TI18N("<div fontColor=%s outline=2,%s>本次免费</div>"), Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6])
    elseif config then
        local one_special_icon_item
        if config.ext_item_once and config.ext_item_once[1] and config.ext_item_once[1][1] then
            one_special_icon_item = config.ext_item_once[1][1]
        ...
        if self.one_icon_num < need_count and (one_icon_special_num < special_need_count or special_need_count == 0) then --特殊道具和道具数量不够
            ...
            desc_str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#e14737 outline=2,%s>%d</div><div fontColor=%s outline=2,%s>/%d</div>"), PathTool.getItemRes(item_icon), Config.ColorData.data_new_color_str[6], self.one_icon_num, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], need_count)          
        elseif one_icon_special_num >= special_need_count and special_need_count ~= 0 then
            ...
            desc_str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#35ff14 outline=2,%s>%d</div><div fontColor=%s outline=2,%s>/%d</div>"), PathTool.getItemRes(special_icon), Config.ColorData.data_new_color_str[6], self.one_icon_num, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], special_need_count)
        else
            self.single_status = PartnersummonConst.Status.Item
            desc_str = string.format(TI18N("<img src=%s visible=true scale=0.5 /><div fontColor=#35ff14 outline=2,%s>%d</div><div fontColor=%s outline=2,%s>/%d</div>"), PathTool.getItemRes(item_icon), Config.ColorData.data_new_color_str[6], self.one_icon_num, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], need_count)
        end
    end
    ...
    if self.temp_sp then
        ...
        if self.temp_sp and self.temp_sp.data then
            if not self.is_show_item then
                ...
                self.effect_spine_2:registerSpineEventHandler(animationEventFunc, sp.EventType.ANIMATION_COMPLETE)
