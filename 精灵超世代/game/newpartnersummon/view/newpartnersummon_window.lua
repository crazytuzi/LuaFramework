...
local item_upward = 8   -- 中间item往上移动坐标
...
function NewPartnerSummonWindow:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getPlistImgForDownLoad("newpartnersummon/txt_newpartnersummon", "newpartnersummon_1"), type = ResourcesType.single },
        { path = PathTool.getPlistImgForDownLoad("newpartnersummon/txt_newpartnersummon", "newpartnersummon_2"), type = ResourcesType.single },
        { path = PathTool.getPlistImgForDownLoad("newpartnersummon/txt_newpartnersummon", "newpartnersummon_3"), type = ResourcesType.single },
        { path = PathTool.getPlistImgForDownLoad("newpartnersummon/txt_newpartnersummon", "txt_cn_newpartnersummon_1"), type = ResourcesType.single },
        { path = PathTool.getPlistImgForDownLoad("newpartnersummon/txt_newpartnersummon", "txt_cn_newpartnersummon_2"), type = ResourcesType.single },
        { path = PathTool.getPlistImgForDownLoad("newpartnersummon/txt_newpartnersummon", "txt_cn_newpartnersummon_3"), type = ResourcesType.single },
    }
...
function NewPartnerSummonWindow:open_callback()
    ...
    self.txt_bg = main_container:getChildByName("txt_bg")
    ...
    self.rule_bg = main_container:getChildByName("rule_bg")
    self.btn_rule = self.rule_bg:getChildByName("btn_rule")
    ...
    for i=1,3 do
        ...
        -- 中间的需要调整y位置
        if i == 2 then
            tab.item:setPositionY(tab.item:getPositionY() + item_upward)
        end
        ...
        local res = PathTool.getPlistImgForDownLoad("newpartnersummon/txt_newpartnersummon", "newpartnersummon_"..i)
        tab.item:loadTexture(res, LOADTEXT_TYPE)
    ...
    -- self.desc_label = main_container:getChildByName("desc_label")
    self.desc_label = createRichLabel(30,58,cc.p(0.5,0.5),cc.p(self.txt_bg:getContentSize().width*0.5,self.txt_bg:getContentSize().height*0.5+24),0,0,500)
    self.txt_bg:addChild(self.desc_label)

    --推荐阵容
    self.btn_lineup = main_container:getChildByName("btn_lineup")
    self.btn_lineup:getChildByName("label"):setString(TI18N("推荐阵容"))
    --招募一次
    ...
    self.btn_summon_1:getChildByName("Text_1"):setString(TI18N("捕捉1次"))
    self.summon_1_diammand = createRichLabel(20,58,cc.p(0.5,0.5),cc.p(self.btn_summon_1:getContentSize().width*0.5,98))
    ...
    self.btn_summon_10:getChildByName("Text_1"):setString(TI18N("捕捉10次"))
    self.summon_10_diammand = createRichLabel(20,58,cc.p(0.5,0.5),cc.p(self.btn_summon_10:getContentSize().width*0.5,98))
        ...
        if self.role_assets_event == nil then
            self.role_assets_event = self.role_vo:Bind(RoleEvent.UPDATE_ROLE_ATTRIBUTE, function(key, value)
                if key == "recruit_hero" then
                    ...
                    self.progress_value:setString(string.format("%d/%d", cur_score, max_score))
...
function NewPartnerSummonWindow:showArenaPeakEffect(obj, bool)
    ...
    if bool == true then
        if obj.arenapeak_effect == nil then
            ...
            obj:getChildByName("Text_1"):setLocalZOrder(999)
        else
...
function NewPartnerSummonWindow:showScoreFullAction(status)
    ...
    self.box_effect = createEffectSpine(PathTool.getEffectRes(110), cc.p(self.score_btn:getContentSize().width*0.5, self.score_btn:getContentSize().height*0.5-25), cc.p(0.5, 0.5), true, action)
    self.box_effect:setScale(1.3)
    self.score_btn:addChild(self.box_effect)
end
...
function NewPartnerSummonWindow:isTouchRectLeft(x,y)
    ...
    local pos = self.btn_left:convertToWorldSpace(cc.p(0,0))
    local world_x = pos.x
    if x-world_x >= 0 and x-world_x <= width_ and y >= (y_ - bottom_y+start_y) and y <= (y_+start_y + height_ - bottom_y) then
        status = true
    end
    return status
end
...
function NewPartnerSummonWindow:isTouchRectRight(x,y)
    ...
    local bottom_y = 0--display.getBottom(self.main_container)
    local pos = self.btn_right:convertToWorldSpace(cc.p(0,0))
    local world_x = pos.x
    if x-world_x >= 0 and x-world_x <= width_ and y >= (y_ - bottom_y+start_y) and y <= (y_+start_y + height_ - bottom_y) then
        status = true
    end
    ...
    if dire >= 0 then
        ...
        for i=1,3 do
            if local_right_pos[self.cur_index][i] == 1 then
                ...
                local upward = 0
                -- 中间的需要调整y位置
                if local_right_pos[self.cur_index][i] == 2 then
                    upward = item_upward
                end
                local root_move_to = cc.MoveTo:create(time, cc.p(item_pos_x[local_right_pos[self.cur_index][i]], self.item_pos_y+upward))
                local ease_out = cc.EaseSineOut:create(root_move_to)
...
function NewPartnerSummonWindow:tabChangeIndex(index)
    ...
    local res_bg = PathTool.getPlistImgForDownLoad("bigbg/newpartnersummon","newpartnersummon_bg"..index,true)
    ...
    local res = PathTool.getPlistImgForDownLoad("bigbg/newpartnersummon","partnersummon_role"..index)
    ...
    local res = PathTool.getPlistImgForDownLoad("newpartnersummon/txt_newpartnersummon", "txt_cn_newpartnersummon_"..index)
    loadSpriteTexture(self.summon_type_txt, res, LOADTEXT_TYPE)
    ...
    self.txt_bg:loadTexture(PathTool.getResFrame("newpartnersummon", "txt_newpartnersummon_bg" .. index), LOADTEXT_TYPE_PLIST)
    self.desc_label:setString(TI18N(Config.RecruitData.data_partnersummon_txt[index].desc))
    -- local desc_str = string.format("<div fontcolor=#ffffff outline=2,#000000>  %d</div>", 111)
                    ...
                    if not self.summon_timer[index] then
                        self.summon_timer[index] = GlobalTimeTicket:getInstance():add(function()
                            if item_data.time_num and (item_data.time_num - GameNet:getInstance():getTime()) > 0 then
                                ...
                                self.item_list_time[index]:setString(string.format("<div fontColor=#ffffff outline=2,#000000>%s</div>", TI18N("免费捕捉")))
                                local str = string.format("<div fontcolor=%s outline=2,%s> %s</div>",Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],TI18N("免费捕捉"))
...
function NewPartnerSummonWindow:setOnceStatus(group_id)
    ...
    local str = string.format("<img src=%s visible=true scale=0.30 /><div fontcolor=%s outline=2,%s> %s</div>",res_1, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], consume_1)
    self.summon_1_diammand:setString(str)
end
--设置10连抽的按钮状态
function NewPartnerSummonWindow:setTenStatus(group_id)
    ...
    local str = string.format("<img src=%s visible=true scale=0.30 /><div fontcolor=%s outline=2,%s> %s</div>",res_10, Config.ColorData.data_new_color_str[15], Config.ColorData.data_new_color_str[6], consume_10)
...
function NewPartnerSummonWindow:heightSummonFiveStar()
    ...
    if num == 0 then
        ...
        -- end
        self.advanced_bg:setVisible(false)
        -- self.special_label:setString(TI18N("<div fontcolor=#ffffff fontsize=22 outline=2,#8f1c00>随机召唤1个或10个3~5星宝可梦</div>"))
    else
        self.advanced_bg:setVisible(true)
        local code = cc.Application:getInstance():getCurrentLanguageCode()
        local remain_x, remain_y, star_x, star_y = 28, 36, 212, 36
        if code == "zh" then
            remain_x = 28
            star_x = 212
        else
