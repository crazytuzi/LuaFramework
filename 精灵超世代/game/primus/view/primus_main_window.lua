...
function PrimusMainWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getPlistImgForDownLoad("bigbg", "bigbg_115",true), type = ResourcesType.single},
...
function PrimusMainWindow:open_callback()
    ...
    self.background:loadTexture(PathTool.getPlistImgForDownLoad("bigbg", "bigbg_115",true), LOADTEXT_TYPE)
    ...
    local title_lab = self.title_bg:getChildByName("title_lab")
    loadSpriteTexture(title_lab, "resource/primus/txt_primus/txt_primus_title.png", LOADTEXT_TYPE)
    ...
    self.explain_btn = self.title_bg:getChildByName("explain_btn")
...
function PrimusMainWindow:updateSpine(look_id, pos_index, is_self)
    ...
    local fun = function()
        if not station_item.spine then
            ...
            station_item.spine:setPosition(cc.p(0,50))
            station_item.spine:setAnchorPoint(cc.p(0.5,0))
            --station_item.spine:setScale(0.4)
