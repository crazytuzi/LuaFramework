                ...
                if event_type == ccui.TouchEventType.ended then
                    if sender.config ~= nil then
                        if sender.is_show_tips == true then
                            ...
                            local items = WeeklyActivitiesController:getInstance():getModel():getDailyAwards(awards.config.activity)
                            local rewards = clone(sender.config.rewards)
                            for i,v in ipairs(items) do
                                table.insert(rewards,v)
                            end
                            CommonAlert.showItemApply(TI18N("当前活跃度奖励"),rewards,nil,TI18N("确定"),nil,nil,TI18N("奖励"),nil,nil,true,nil, nil,{off_y=50})
                        else
