...
function TaskExpPanel:updateExpList(is_keep_pos)
    ...
    if not self.list_view then
        local scroll_view_size = cc.size(628,608-20)
        local setting = {
        ...
        local size = cc.size(628,560)
        self.list_view = CommonScrollViewSingleLayout.new(self.exp_container, cc.p(628 * 0.5, 560 * 0.5) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting, cc.p(0.5,0.5))
...
function taskExpItem:configUI(width, height)
    ...
    self.task_desc = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0, 1), cc.p(26, 85), nil, nil, 270)
...
function taskExpItem:updateUnFinish()
    ...
    if self.data.progress[1] then
        ...
        --BT适配
        if self.data.id == 52000 or self.data.id == 52010 or self.data.id == 52020 then
            target_val = changeBtValueForPower(target_val)
            value = changeBtValueForPower(value)
        end
    end
