...
function ServiceCenterWindow:createProblemContentBox() -- 问题描述
    ...
    if not self.problem_inputBox then
        self.problem_inputBox = createEditBox(self.quest_scroll, res, cc.size(520,140), 
        Config.ColorData.data_color3[274], 18, cc.c3b(0x8c,0x8c,0x8b), 18, 
        self.placeholder_list[1], cc.p(5,295), 150, LOADTEXT_TYPE_PLIST)
    end
    if self:getIsChangePlaceHolder() == true then
...
function ServiceCenterWindow:createPhoneNumBox()  --电话号码框
    ...
    if not self.pnonenum_inputBox then
        self.pnonenum_inputBox = createEditBox(self.tel_input, res,cc.size(250,45), Config.ColorData.data_color3[274],
         18,  cc.c3b(0x8c,0x8c,0x8b), 22, self.placeholder_list[2], cc.p(0,45), 11, 
         LOADTEXT_TYPE_PLIST,cc.EDITBOX_INPUT_MODE_PHONENUMBER, cc.KEYBOARD_RETURNTYPE_DONE)
    end
    if self:getIsChangePlaceHolder() == true then
...
function ServiceCenterWindow:createEmailBox( ... )
    ...
    if not self.email_inputBox then
        self.email_inputBox = createEditBox(self.email_input, res, cc.size(250,45), Config.ColorData.data_color3[274], 18, cc.c3b(0x8c,0x8c,0x8b), 22, self.placeholder_list[3], cc.p(0,45), nil, LOADTEXT_TYPE_PLIST, cc.EDITBOX_INPUT_MODE_EMAILADDR)
    end
    if self:getIsChangePlaceHolder() == true then
