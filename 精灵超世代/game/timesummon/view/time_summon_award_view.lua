...
function TimeSummonAwardView:setData(  )
	...
	if self.text_elite ~= TimesummonConst.ActonInfoType.EliteType2 then -- 不等于自选精英召唤
		if pro_config then
			...
			if not self.title_bg_1 then
				self.title_bg_1 = createImage(self.desc_scrollview, PathTool.getResFrame("common", "common_90010"), self.scroll_size.width*0.5, 0, cc.p(0.5, 1), true, nil, true)
				self.title_bg_1:setCapInsets(cc.rect(23, 0, 2, 42))
				self.title_bg_1:setContentSize(cc.size(610, 42))
				...
				local title_txt_1 = createLabel(24, Config.ColorData.data_new_color4[6], nil, self.title_bg_1:getContentSize().width*0.5, 22, tempLab, self.title_bg_1, nil, cc.p(0.5, 0.5))
			end
			...
			for i,v in ipairs(up_item_data) do
				...
				item:setPosition(cc.p(start_x + (i-1) * (BackPackItem.Width + offset_x), 561))--新宝可梦图标位置
			end
	
			if self.text_elite == TimesummonConst.ActonInfoType.ElfinType2 and num<=0 then
				local title_txt_4 = createLabel(24, Config.ColorData.data_new_color4[6], nil, self.title_bg_1:getContentSize().width * 0.5, -70, TI18N("请先将精灵加入许愿池"), self.title_bg_1, nil, cc.p(0.5, 0.5))
			end
	...
	if self.data then
		...
		if summon_cfg then
			if not self.title_bg_2 then
				self.title_bg_2 = createImage(self.desc_scrollview, PathTool.getResFrame("common", "common_90010"), self.scroll_size.width*0.5, 0, cc.p(0.5, 1), true, nil, true)
				self.title_bg_2:setCapInsets(cc.rect(23, 0, 2, 42))
				self.title_bg_2:setContentSize(cc.size(610, 44))
				local title_txt_2 = createLabel(24, Config.ColorData.data_new_color4[6], nil, self.title_bg_2:getContentSize().width*0.5, 22, TI18N("内容详情"), self.title_bg_2, nil, cc.p(0.5, 0.5))
			end
			if not self.award_desc then
				self.award_desc = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0.5, 1), cc.p(self.scroll_size.width*0.5, 430), 10, nil, 580)
	...
	if pro_config then
		if not self.title_bg_3 then
			self.title_bg_3 = createImage(self.desc_scrollview, PathTool.getResFrame("common", "common_90010"), self.scroll_size.width*0.5, 0, cc.p(0.5, 1), true, nil, true)
			self.title_bg_3:setCapInsets(cc.rect(23, 0, 2, 42))
			self.title_bg_3:setContentSize(cc.size(610, 44))
		end
		local title_txt_3 = createLabel(24, Config.ColorData.data_new_color4[6], nil, self.title_bg_3:getContentSize().width*0.5, 22, TI18N("概率公示"), self.title_bg_3, nil, cc.p(0.5, 0.5))
		...
		if self.text_elite ~= TimesummonConst.ActonInfoType.EliteType2 then
			...
			local check_label = createRichLabel(20,Config.ColorData.data_new_color4[6], cc.p(0,0.5),cc.p(35, 22))
...
function TimeSummonAwardItem:setData( data ,type)
	...
	if data.is_chip == 1 then
		...
		elseif type == TimesummonConst.ActonInfoType.EliteType2 then
			...
			self.type_text:setString(TI18N("宝可梦"))
		end
	end
end
