...
function PartnerCommentWindow:open_callback()
    ...
    --评论输入
    local size = cc.size(420,50)
    self.edit_box = createEditBox(self.main_panel, PathTool.getResFrame("common", "common_1021"),size, Config.ColorData.data_new_color4[1], 20, Config.ColorData.data_new_color4[1], 20, self.default_msg, nil, nil, LOADTEXT_TYPE_PLIST)
...
function PartnerCommentWindow:createHeroMessage()
    ...
    self.hero_item:setPosition(cc.p(85,720))
    self.main_panel:addChild(self.hero_item)
    self.hero_item:setData(self.hero_data)
    --名字
    self.hero_name = createLabel(26,Config.ColorData.data_new_color4[6],nil,160,735,"",self.main_panel,0, cc.p(0,0))
    self.hero_name:setString(self.hero_data.name)
    --喜欢人数
    self.like_num = createRichLabel(26,Config.ColorData.data_new_color4[11], cc.p(0,0), cc.p(160,680), 0, 0, 500)
    self.main_panel:addChild(self.like_num)
    --点评人数
    self.comment_num = createLabel(24,Config.ColorData.data_new_color4[6],nil,620,670,"",self.main_panel,0, cc.p(1,0))
end
...
function PokedexCommentItem:layoutUI()
    ...
    --评论者名字
    self.goods_name = createLabel(26,Config.ColorData.data_new_color4[6],nil,50,125,"",self.main_panel,0, cc.p(0,0.5))
    --评论内容
    self.comment_label =createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0,1), cc.p(50,90), 0, 0, 520)
