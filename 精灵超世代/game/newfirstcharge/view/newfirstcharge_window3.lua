...
function NewFirstChargeWindow3:__init()
	...
	self.res_list = {
		...
		{path = PathTool.getPlistImgForDownLoad("bigbg/action","txt_cn_action_bigbg_7"), type = ResourcesType.single},
		{path = PathTool.getPlistImgForDownLoad("bigbg/action","txt_cn_action_bigbg_8"), type = ResourcesType.single},
	}
...
function NewFirstChargeWindow3:open_callback()
	...
	local res_bg = PathTool.getPlistImgForDownLoad("bigbg/action","txt_cn_action_bigbg_7")
	if not self.bg_item_load then
		self.bg_item_load = loadSpriteTextureFromCDN(self.bg, res_bg, ResourcesType.single, self.bg_item_load)
	end
	...
	self.bg3 = container:getChildByName("bg3")
	local res_bg3 = PathTool.getPlistImgForDownLoad("bigbg/action","txt_cn_action_bigbg_8")
	if not self.bg3_item_load then
		self.bg3_item_load = loadSpriteTextureFromCDN(self.bg3, res_bg3, ResourcesType.single, self.bg3_item_load)
	end

	self.title_img_1 = container:getChildByName("title_img_1")
	self.title_img_2 = container:getChildByName("title_img_2")
	...
	--五星图片隐藏
	local img = container:getChildByName("five_star")
...
function NewFirstChargeWindow3:changeTabView(index)
	...
	if index == 1 then
		...
		self.title_img_1:setVisible(true)
		self.title_img_2:setVisible(false)
		self.name:setString(TI18N("快\n龙"))
	...
	elseif index == 2 then
		...
		self.title_img_1:setVisible(false)
		self.title_img_2:setVisible(true)
		self.name:setString(TI18N("超\n梦"))
	...
	for i=1, tableLen(list) do
		...
		for k=1, num do
			delayRun(self.main_container, create_index/15, function()
				local scroll_size = object.scroll:getContentSize()
				-- local _x = size * k - size * 0.5
				-- local _y = size * 0.5
				local _x = scroll_size.width/2 - (num-1) * size * 0.5
				if k > num/2 then
					_x = scroll_size.width/2 + (num-1) * size * 0.5
				end
				local _y = scroll_size.height/2
...
function NewFirstChargeWindow3:register_event()
	...
	self:addGlobalEvent(NewFirstChargeEvent.New_First_Charge_Event, function(data)
		...
		if code == "zh" then
			num = math.floor(role_vo.vip_exp*0.1)
		else
			num = role_vo.vip_exp
		end
		local totle_str = string_format(TI18N("已累充: %d"),num)
...
function NewFirstChargeWindow3:close_callback()
	...
	if self.bg3_item_load then
        self.bg3_item_load:DeleteMe()
    end
	self.bg3_item_load = nil
	controller:openNewFirstChargeView(false)
end 