...
--******** 文件说明 ********
-- @Author:      hyxing 
-- @description: 新版首充界面（耶梦加得的）
-- @DateTime:    2019-05-27 11:47:05
-- *******************************
NewFirstChargeWindow1 = NewFirstChargeWindow1 or BaseClass(BaseView)
...
function NewFirstChargeWindow1:__init()
	self.win_type = WinType.Big
	self.view_tag = ViewMgrTag.DIALOGUE_TAG
	self.layout_name = "newfirstcharge/newfirstcharge_window1"
	self.res_list = {
...
function NewFirstChargeWindow1:open_callback()
...
function NewFirstChargeWindow1:openRootWnd(index)
	index = index or 1
	self:changeTabView(index)
	controller:sender21000()
end

function NewFirstChargeWindow1:changeTabView(index)
	index = index or 1
...
--标题动作
function NewFirstChargeWindow1:titleBarnerAction(index)
	local time_in = 0.2
	self:stopAllActions()
	if index == 1 then
...
function NewFirstChargeWindow1:stopAllActions()
	for i,v in pairs(self.tab_view) do
		doStopAllActions(v.title_barner)
	end
end

function NewFirstChargeWindow1:fillItemList(list)
	self:hideItemEffect()
	local scale = 0.7
...
--隐藏物品特效
function NewFirstChargeWindow1:hideItemEffect()
	for i=1,2 do
		for j=1,3 do
			for m=1,2 do
...
function NewFirstChargeWindow1:register_event()
...
--红点
function NewFirstChargeWindow1:setRedPointTab()
...
function NewFirstChargeWindow1:updateData(data)
...
function NewFirstChargeWindow1:close_callback()
