...
function ArenapeakchampionGuessingTabGuessing:layoutUI()
    ...
    --我的代币
    self.item_count_label = createRichLabel(22, Config.ColorData.data_new_color4[1], cc.p(1, 0.5), cc.p(680,916),nil,nil,1000)
    ...
    self.left_fight_label:setPosition(123, 28)
    ...
    self.right_fight_label:setPosition(123, 28)
    ...
    self.match_value = createRichLabel(22, Config.ColorData.data_new_color4[1], cc.p(0.5, 0.5), cc.p(360,916),nil,nil,1000)
    ...
    if main_data and main_data.step ~= 0 then
        ...
        if main_data.round_status == 2 then --竞猜
            ...
            if main_data.round_status == 1 then
                local res = PathTool.getResFrame("common", "common_icon_25", false, "commonicon")
                self.fight_btn:loadTexture(res, LOADTEXT_TYPE_PLIST)
                self.fight_btn:getChildByName("label"):setString(TI18N("查看"))
            else
                local res = PathTool.getResFrame("common", "common_icon_15", false, "commonicon")
                self.fight_btn:loadTexture(res, LOADTEXT_TYPE_PLIST)
                self.fight_btn:getChildByName("label"):setString(TI18N("观战"))
            end
...
function ArenapeakchampionGuessingTabGuessing:updateCentrePanel( )
    ...
    if main_data then
        ...
        if str1 then
            match_str = string_format("<div outline=2,%s>%s%s</div>" ,Config.ColorData.data_new_color_str[6],match_str,str1)
        end
...
function ArenapeakchampionGuessingTabGuessing:unEnabledGuessBtn(status)
    if status then
        ...
        --self.left_guess_name:enableOutline(Config.ColorData.data_new_color4[6], 2)
        --self.right_guess_name:enableOutline(Config.ColorData.data_new_color4[6], 2)
...
function ArenapeakchampionGuessingTabGuessing:showEmptyInfo(is_show, main_data)
    ...
    if is_show then
        ...
        self.talk_img = self.empty_panel:getChildByName("talk_img")
        self.empty_label = createRichLabel(26,Config.ColorData.data_new_color4[1], cc.p(0.5, 0.5), cc.p(360,44),nil,nil,1000)
        -- self.empty_panel:addChild(self.empty_label)
        self.talk_img:addChild(self.empty_label)
        if main_data then
        ...
        self.match_value:setString(string_format("<div outline=2,%s>%s</div>" ,Config.ColorData.data_new_color_str[6], TI18N("未开赛")))
    else
...
function ArenapeakchampionGuessingTabGuessing:setTimeFormatString(time)
    if time > 0 then
        self.empty_label:setString(string_format(TI18N("<div outline=2,%s>距离比赛开启: <div fontcolor=#249003>%s</div></div>"), Config.ColorData.data_new_color_str[6],TimeTool.GetTimeFormatDay(time)))
    else
        self.empty_label:setString(string_format("<div outline=2,%s>%s</div>",Config.ColorData.data_new_color_str[6],TI18N("巅峰冠军赛即将开始")))
    end

    setBgByLabel(self.empty_label, self.talk_img, 30, -1)
end
