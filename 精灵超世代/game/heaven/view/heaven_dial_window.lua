...
function HeavenDialWindow:ctor(group_id)
    ...
    self.res_list = {{
        path = PathTool.getPlistImgForDownLoad("heavendial", "heavendial"),
        type = ResourcesType.plist
    }, {
        path = PathTool.getPlistImgForDownLoad("heaven", "heaven"),
        type = ResourcesType.plist
    }}
...
function HeavenDialWindow:configUI()
    ...
    self.tips_label = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(0.5, 0.5), cc.p(315, -40), nil, nil,
        300)
	bottom_panel:addChild(self.tips_label)
    ...
    self.score_title = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(0.5, 0.5), cc.p(85, 45), nil, nil,
        150)
    self.score_bg:addChild(self.score_title)
    self.score_title:setString(TI18N("最高总评分："))
    self.all_score = createLabel(20, Config.ColorData.data_new_color4[17], nil, 82, 33, "", self.score_bg, 2,
        cc.p(0, 0.5))
    self:handlerLinghtEffect(true)

    self.baodi_count_tips = createRichLabel(22, 1, cc.p(0.5, 0.5), cc.p(365, 230), nil, nil, 680)
    bottom_panel:addChild(self.baodi_count_tips)
    ...
    top_panel:setPositionY(top_off - 165)
...
function HeavenDialWindow:register_event()
    ...
    if not self.update_heaven_red_status then
        self.update_heaven_red_status = GlobalEvent:getInstance():Bind(HeavenEvent.Update_Heaven_Red_Status,
            function(bid)
    ...
    if not self.delete_goods_event then
        self.delete_goods_event = GlobalEvent:getInstance():Bind(BackpackEvent.DELETE_GOODS,
    ...
    if not self.modify_goods_event then
        self.modify_goods_event = GlobalEvent:getInstance():Bind(BackpackEvent.MODIFY_GOODS_NUM,
            function(bag_code, data_list)
                self:updateItemNum(bag_code, data_list)
            end)
    end
    ...
    if not self.update_all_score then
        self.update_all_score = GlobalEvent:getInstance():Bind(HeavenEvent.Update_All_Score, function()
            if self.all_score and _model then
                ...
                if Score then
                    Score = changeBtValueForPower(Score)
            ...
            if recruit_type == 3 then
                ...
                if self.cur_group_cfg and self.cur_group_cfg.gain_once and self.cur_group_cfg.gain_once[1] then
                    ...
                    if item_cfg and gain_item_bid and gain_item_num then
                        ...
                        local tips_str = string.format(TI18N(
                            ...
                            PathTool.getItemRes(3), self.cur_group_cfg.loss_gold_once, role_vo.gold)
                        tips_str = tips_str .. string.format(TI18N(
                            "祈祷后可获得<div fontColor=#289b14 fontsize= 26>%d</div><div fontColor=#d95014 fontsize= 26>%s</div><div fontColor=#764519>(同时附赠</div><div fontColor=#289b14 fontsize= 26>1</div>次随机奖励)</div>"),
                            gain_item_num, item_cfg.name)
                ...
                if self.cur_group_cfg and self.cur_group_cfg.gain_once and self.cur_group_cfg.gain_once[1] then
                    ...
                    if item_cfg and gain_item_bid and gain_item_num then
                        ...
                        local tips_str = string.format(TI18N(
                            ...
                            PathTool.getItemRes(3), self.cur_group_cfg.loss_gold_ten, role_vo.gold)
                        tips_str = tips_str .. string.format(TI18N(
                            "祈祷后可获得<div fontColor=#289b14 fontsize= 26>%d</div><div fontColor=#d95014 fontsize= 26>%s</div><div fontColor=#764519>(同时附赠</div><div fontColor=#289b14 fontsize= 26>10</div>次随机奖励)</div>"),
                            gain_item_num * 10, item_cfg.name)
                        CommonAlert.show(tips_str, TI18N("确定"), function()
                            self.dial_times = 10
                            self.dial_recruit_type = recruit_type
                            self:handleDialEffect(true)
...
function HeavenDialWindow:updateProgress()
    local dialy_data = _model:getHeavenDialById(self.cur_group_id)
    if not dialy_data then
        return
    end

    if self.cur_group_cfg then
        self.tips_label:setString(_string_format(TI18N("今日钻石召唤次数上限：%d/%d"), dialy_data.day_count,
            self.cur_group_cfg.day_limit_count))
    end
    ...
    if award_config then
        ...
        for i, v in ipairs(award_config) do
            ...
            if not self.txt_bg[i] then
                self.txt_bg[i] = createImage(self.progress_panel, PathTool.getResFrame("heaven", "heaven_1018"), pos_y,
                    -10, cc.p(0.5, 1), true, nil, true)
                self.txt_bg[i]:setCapInsets(cc.rect(25, 0, 2, 36))
                self.txt_bg[i]:setContentSize(cc.size(52, 25))
            end
            if not self.arriveLuckly_label[i] and self.award_item_list[i] then
                self.arriveLuckly_label[i] = createLabel(18, Config.ColorData.data_new_color4[1], nil, pos_y, -13, "",
                    self.progress_panel, 2, cc.p(0.5, 1))
            end
        ...
        for i, v in ipairs(award_config) do
            if i == 1 then
                ...
                if dialy_data.all_award_count <= v.times then
                    distance = distance + ((dialy_data.all_award_count - last_times) / (v.times - last_times)) *
                                   offset_y
                    break
                else
...
function HeavenDialWindow:showRewardItems(data, pos, touch_pos)
    ...
    if not self.tips_bg then
        self.tips_bg = createImage(self.tips_layer, PathTool.getResFrame("common", "common_1034"), size.width * 0.5,
            100, cc.p(0, 0), true, 10, true)
    ...
    for i, v in pairs(data) do
        if not list[i] then
            ...
            list[i]:setPosition(cc.p(BackPackItem.Width * (i - 1) + 35, 100))
            list[i]:setDefaultTip()
            self.backpackitem_list[i] = list[i]
            self.text_num = createLabel(22, Config.ColorData.data_new_color4[6], nil, 60, -25, "", list[i], nil,
                cc.p(0.5, 0.5))
...
function HeavenDialWindow:updateBaodiCount()
    ...
    self.baodi_count_tips:setString(_string_format(TI18N(
        "<div outline=2,#3d5078>再抽</div><div fontcolor=#0cff01 outline=2,#3d5078>%d</div><div outline=2,#3d5078>次必出</div><div fontcolor=#d63636 outline=2,#3d5078>良品</div><div outline=2,#3d5078>以上</div>"),
        count))
end
...
function HeavenDialWindow:getDefaultGroupId()
    ...
    for k, v in pairs(Config.HolyEqmLotteryData.data_group) do
        if (not v.open_cond or v.open_cond == 0 or _model:checkHeavenDialIsOpenByGId(v.group_id)) and v.group_id >
            default_id then
...
function HeavenDialWindow:setData()
    ...
    if not self.cur_group_cfg then
        return
    end
...
function HeavenDialWindow:updateDialCostInfo()
    ...
    if not item_cfg or not self.cur_group_cfg then
        return
    end
    ...
    if have_num >= 10 then
        ...
        if self.cur_group_cfg.discount_res and self.cur_group_cfg.discount_res ~= "" then
            ...
            loadSpriteTexture(self.sp_discount, PathTool.getResFrame("heavendial", self.cur_group_cfg.discount_res),
                LOADTEXT_TYPE_PLIST)
        else
            self.sp_discount:setVisible(false)
        end
...
function HeavenDialWindow:createStageItem()
    if self.stage_item_1 or not self.cur_group_cfg then
        return
    end

    self.stage_item_1 = HeavenDialItem.new()
    self.stage_item_1:setPosition(cc.p(350, 490))
...
function HeavenDialWindow:showItemRotateAni(dir)
    ...
    -- 判断是否达到左右边界
    if (dir == 2 and self.cur_group_id <= 1) or
        (dir == 1 and self.cur_group_id >= Config.HolyEqmLotteryData.data_group_length) then
        return
    end

    if self.rotate_ani_state then
        return
    end
...
function HeavenDialWindow:handlerLinghtEffect(status)
    if status == true then
        if not tolua.isnull(self.pos_light) and self.light_effect == nil then
            self.light_effect = createEffectSpine(Config.EffectData.data_effect_info[1090], cc.p(-6, -343),
                cc.p(0.5, 0.5), true, PlayerAction.action)
...
function HeavenDialWindow:handleDialEffect(status)
    if status == false then
        ...
        if not tolua.isnull(self.main_container) and self.dial_effect == nil then
            ...
            self.dial_effect = createEffectSpine(Config.EffectData.data_effect_info[1000],
                cc.p(main_size.width / 2, main_size.height / 2 - 200), cc.p(0.5, 0.5), false, PlayerAction.action,
                handler(self, self._onDialAniCallBack))
...
function HeavenDialItem:configUI()
    ...
    for i = 1, 4 do
        ...
        holy_item:addCallBack(function()
            self:selectHolyByIndex(i)
        end)
        holy_item:addLongTimeTouchCallback(function()
            self:onCellLongTimeTouched(i)
        end)
        holy_item:setPosition(cc.p(50.5, 53))
        holy_item:showAddIcon(true)
        holy_item:setBackgroundOpacity(0)
...
function HeavenDialItem:updateHolyItem()
    ...
    for k, v in pairs(self.heaven_list) do
        ...
        if equip_vo then
            ...
            holy_item:setGoodsName(equip_vo.name, cc.p(holy_item.Width / 2, -36), 20, cc.c4b(0xff, 0xea, 0xbc, 0xff))
            holy_item:showAddIcon(false)
            holy_item:setSelfBackground(equip_vo.quality)
        else
            holy_item:setData()
            holy_item.item_icon:setVisible(false)
            holy_item:setGoodsName(TI18N("心愿水晶"), cc.p(holy_item.Width / 2, -36), 20,
                cc.c4b(0xff, 0xea, 0xbc, 0xff))
            holy_item:showAddIcon(true)
...
function HeavenDialItem:handleStoneEffect(status, effect_id)
    if status == true then
        if not tolua.isnull(self.container) and self.stone_effect == nil and effect_id then
            self.stone_effect = createEffectSpine(Config.EffectData.data_effect_info[effect_id],
                cc.p(self.size.width / 2 + 20, 200), cc.p(0.5, 0), true, PlayerAction.action)
