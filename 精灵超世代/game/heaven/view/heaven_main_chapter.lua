...
function HeavenMainChapter:createChapterLine( index )
	...
	if l_info then
		...
		line_node:setContentSize(cc.size(line_width, 16))
...
function HeavenChapterItem:updateChapterItem(  )
	...
	if not self.award_icon then
		self.award_icon = createImage(self.container, PathTool.getResFrame("commonicon", "common_icon_16"), self.size.width-24, self.size.height-24, cc.p(0.5, 0.5), true)
	...
	if not self.title_bg then
		self.title_bg = createImage(self.container, PathTool.getResFrame("common", "common_30019"), self.size.width/2, 35, cc.p(0.5, 0.5), true, nil, true)
	    self.title_bg:setCapInsets(cc.rect(26, 0, 2, 42))
	    self.title_bg:setContentSize(cc.size(156, 42))
	end

	-- 章节
	if not self.title_txt then
		self.title_txt = createLabel(24,Config.ColorData.data_new_color4[15],Config.ColorData.data_new_color4[6],self.size.width/2,35,"",self.container,2,cc.p(0.5, 0.5))
	end
...
function HeavenChapterItem:adjustNodePos(  )
	...
	self.title_bg:setPosition(cc.p(self.size.width/2, 35))
	self.title_txt:setPosition(cc.p(self.size.width/2, 35))
...
function HeavenChapterItem:showPassIcon( status )
	if status == true then
		if not self.pass_icon then
			local res = PathTool.checkRes("resource/heaven/txt_heaven/txt_cn_heaven_pass.png")
			self.pass_icon = createImage(self.container, res, 25, self.size.height-60, cc.p(0.5, 0.5), false, 2)
		end
