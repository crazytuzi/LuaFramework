...
function HeavenChapterWindow:open_callback(  )
    ...
    self.award_panel:setPositionY(top_off - 280)
    bottom_panel:setPositionY(bottom_off+150)
...
function HeavenChapterWindow:updateCustomsInfo( cus_cfg_data )
    ...
    if customs_vo and customs_vo.state == 2 then
        ...
        loadSpriteTexture(self.first_sp, PathTool.checkRes("resource/heaven/txt_heaven/txt_cn_heaven_award.png"), LOADTEXT_TYPE)
    else
        show_award_data = cus_cfg_data.f_show_award
        loadSpriteTexture(self.first_sp, PathTool.checkRes("resource/heaven/txt_heaven/txt_cn_heaven_first.png"), LOADTEXT_TYPE)
    end
    ...
    self.power_txt:setString(_string_format(TI18N("推荐战力:%d"), changeBtValueForPower(cus_cfg_data.power)))
...
function HeavenChapterWindow:updateLeftBuyCount(  )
    if not self.left_buy_count then
        self.left_buy_count = createRichLabel(22, Config.ColorData.data_new_color4[1], cc.p(0.5, 0.5), cc.p(560, 20))
        self.bottom_panel:addChild(self.left_buy_count)
    end
    local left_count = _model:getTodayLeftBuyCount()
    self.left_buy_count:setString(string.format(TI18N("<div fontcolor=#ffffff>(剩余购买次数:</div><div fontcolor=#0cff01>%d</div><div fontcolor=#ffffff>)</div>"), left_count))
end
...
function HeavenChapterWindow:updateStarAwardInfo(  )
    ...
    for i,aData in ipairs(award_cfg) do
        ...
        local pos_x = (aData.limit_star/max_star)*344
        box_node:setPosition(cc.p(pos_x + 30, 20))
...
function HeavenChapterWindow:_onClickShowAwardTips( node, data, pos )
    ...
    if not self.tips_bg then
        self.tips_bg = createImage(self.tips_layer, PathTool.getResFrame("common","common_1034"), 0, 0, cc.p(0,0), true, 10, true)
        self.tips_bg:setTouchEnabled(true)
    end
    if self.tips_bg then
        local bg_size = cc.size((BackPackItem.Width+22)*#data+30, BackPackItem.Height+50)
    ...
    for i,v in pairs(data) do
        ...
        if item_config then
            if not award_item then
                ...
                award_item:setPosition(cc.p((BackPackItem.Width+22)*(i-1)+35, 80))
                award_item:setDefaultTip()
                --award_item:setExtendDesc(true, item_config.name)
            else
                award_item:setBaseData(v[1], v[2])
                --award_item:setExtendDesc(true, item_config.name)
                award_item:setPosition(cc.p((BackPackItem.Width+22)*(i-1)+35, 80))
...
function HeavenChapterWindow:showCustomsBossList(  )
    if self.cur_customs_cfg and self.cur_customs_cfg.type == 1 then
        ...
        if not self.boss_tips_bg then
            ...
            self.boss_tips_bg = createImage(self.boss_tips_mask, PathTool.getResFrame("common","common_1034"), node_pos.x, node_pos.y+75, cc.p(0.5, 0), true, nil, true)
            self.boss_tips_bg:setContentSize(cc.size(576, 362))
            self.boss_tips_bg:setTouchEnabled(true)
        end
        --if not self.boss_unit_bg_1 then
        --    self.boss_unit_bg_1 = createImage(self.boss_tips_bg, PathTool.getResFrame("common","common_90057"), 576/2, 250, cc.p(0.5, 0.5), true, nil, true)
        --    self.boss_unit_bg_1:setContentSize(cc.size(563, 120))
        --end
        if not self.boss_team_1 then
            self.boss_team_1 = createLabel(22, Config.ColorData.data_new_color4[6], nil, 20, 329, TI18N("队伍一:"), self.boss_tips_bg, nil, cc.p(0, 0.5))
        end
        --if not self.boss_unit_bg_2 then
        --    self.boss_unit_bg_2 = createImage(self.boss_tips_bg, PathTool.getResFrame("common","common_90057"), 576/2, 90, cc.p(0.5, 0.5), true, nil, true)
        --    self.boss_unit_bg_2:setContentSize(cc.size(563, 120))
        --end
        if not self.boss_team_2 then
            self.boss_team_2 = createLabel(22, Config.ColorData.data_new_color4[6], nil, 20, 168, TI18N("队伍二:"), self.boss_tips_bg, nil, cc.p(0, 0.5))
        end
...
function HeavenAwardBoxItem:setData( data, chapter_id )
    ...
    if not self.txt_bg then
        self.txt_bg = createImage(self.container, PathTool.getResFrame("heaven","heaven_1018"), self.size.width/2, 0, cc.p(0.5, 1), true, nil, true)
        self.txt_bg:setCapInsets(cc.rect(25, 0, 2, 36))
        self.txt_bg:setContentSize(cc.size(68, 25))
    end

    if not self.star_num_txt then
        self.star_num_txt = createLabel(20, Config.ColorData.data_new_color4[1], nil, self.size.width/2, 0, "", self.container, 2, cc.p(0.5, 1))
    end
