...
function HeavenMainWindow:open_callback(  )
	...
	self.btn_rule = tab_container:getChildByName("btn_rule")
	...
	tab_container:setPositionY(top_off - 165)
...
function HeavenMainWindow:register_event(  )
	...
	registerButtonEventListener(self.btn_rule, function ( param, sender, event_type )
		if self.cur_tab_index == HeavenConst.Tab_Index.Dungeon then
		...
		elseif self.cur_tab_index == HeavenConst.Tab_Index.DialRecord then
