...
function HeavenRankWindow:open_callback(  )
    ...
    self.no_rank = createLabel(24, Config.ColorData.data_new_color4[6], nil, data_pos[1]+offx, 45, TI18N('未上榜'), self.my_rank, 0, cc.p(0.5, 0.5))
    self.my_name = createLabel(24, Config.ColorData.data_new_color4[6], nil, name_x,name_y, '', self.my_rank, 0, cc.p(0.5, 0.5))
    self.star_label = createLabel(24, Config.ColorData.data_new_color4[6], nil, 465, 45, '', self.my_rank, 0, cc.p(0.5, 0.5))
...
function HeavenRankWindow:updateRankList( data )
    ...
    if not self.list_view then
        local scroll_view_size = cc.size(604, 510)
        local setting = {
        ...
        self.list_view = CommonScrollViewSingleLayout.new(self.rank_panel, cc.p(0, 6) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting, cc.p(0,0))
...
function HeavenRankWindow:updateMyData(  )
    ...
    if my_idx and my_idx >= 1 and my_idx <= 3 then
        ...
        self.my_rank_icon:loadTexture(PathTool.getResFrame('common', RankConstant.RankIconRes[my_idx]), LOADTEXT_TYPE_PLIST)
...
function HeavenRankWindow:updateRankData( )
    ...
    for i = 1, 3 do
	    ...
	    local font_size = 22
            ...
            self.first_list[i].name_txt = createLabel(font_size,Config.ColorData.data_new_color4[11], nil,pos_x, 752 - off_y,TI18N("虚位以待"),self.main_panel,2, cc.p(0.5,0.5))
            self.first_list[i].name_txt:setLocalZOrder(1)
        end
         -- 排名
        if not self.first_list[i].title_sp then
            self.first_list[i].title_sp = createSprite(PathTool.getResFrame("common",RankConstant.RankIconRes[i]), pos_x + 70, pos_y - 80, self.main_panel, cc.p(0.5,0.5))
    ...
    for i,v in ipairs(rank_list) do
        ...
        if idx > 0 and idx <=3 then 
            ...
            if face_id == 0 then face_id = 10401 end --新增的字段，后端可能发0过来
...
function HeavenRankWindow:showEmptyIcon(  )
    ...
    if not self.empty_con then
        ...
        self.empty_label = createLabel(26, Config.ColorData.data_new_color4[6], nil, size.width / 2, -10, '', self.empty_con, 0, cc.p(0.5, 0))
    end
