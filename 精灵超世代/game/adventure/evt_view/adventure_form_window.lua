...
function AdventureFormWindow:open_callback()
	...
	self.no_vedio_image:getChildByName("label"):setString(TI18N("暂无该类型宝可梦"))
	...
	for index=1,5 do
        ...
        item:setPosition(90 + (index-1)* 118, 258)
        bottom_panel:addChild(item)
		self.hero_item_list[index] = item
    end

	bottom_panel:getChildByName("pos_tips"):setString(TI18N("从列表中选择宝可梦"))
    ...
    local power_click_size = self.power_click:getContentSize()
    self.fight_label:setPosition(power_click_size.width/2, power_click_size.height/2+13)
end
...
function AdventureFormWindow:updateFightPower()
    ...
    self.fight_label:setNum(changeBtValueForPower(power))
end
...
function AdventureFormWindow:onClickSaveBtn()
	...
	if count == 0 then
		message(TI18N("请设置出战宝可梦"))
		return
	end
	local hero_array = hero_model:getAllHeroArray()
	local size = hero_array:GetSize() 
	if count < 5 and count < size then			-- 当前还有可上的宝可梦但是没设置上
		local msg = TI18N("当前上阵宝可梦不足5个，是否确认以此阵容进入冒险？")
