...
function LoginPlatForm:getSessid()
    if not self.is_logined then return "" end
    return string.format("%s", self.info.sessid or "")
end
...
function LoginPlatForm:onLoginInfo(str, isSwitch)
    ...
    local openid, username, token, channelId, timestamp, rid, srv_id, ip, port, host, srv_name,sessid = unpack(Split(str, "#"))

    self.info = {openid = openid, username = username, token = token, channelId = channelId, timestamp = timestamp, rid = rid, srv_id = srv_id, ip = ip, port = port, host = host, srv_name = srv_name,sessid=sessid}
    dump(self.info, "-------------------------->>")
...
--切换账号后的自动登录
function LoginPlatForm:autoLoginWhenSwitch(info)
    self:onLoginInfo(info)
end
...
--切换账号
function LoginPlatForm:onSwitchAccount(msg)
    print("LoginPlatForm:onSwitchAccount", msg)

    self:onLogout(msg)

    -- local function softRestart_callback()
    --     local lp = LoginPlatForm:getInstance()
    --     local openid, username, token, channelId, timestamp, rid, srv_id, ip, port, host, srv_name = unpack(Split(msg, "#"))
    --     lp.info = {openid = openid, username = username, token = token, channelId = channelId,
    --                  timestamp = timestamp, rid = rid, srv_id = srv_id, ip = ip, port = port, 
    --                  host = host, srv_name = srv_name}
    --     lp.is_logined = true
    --     lp.isInSwitchAccountLogin = true
    -- end

    -- self.is_logined = false
    -- self.info = {}
    -- if softRestart then softRestart(softRestart_callback) end
end
