...
function EnterPanel:layoutUI()
	...
	self.btn_user_center:setPosition(display.getLeft()+100, display.getTop()-50 )

	self.btn_notice = container:getChildByName("btn_notice")
	self.btn_notice:setPosition(display.getLeft()+100, self.btn_user_center:getPositionY() - self.btn_user_center:getContentSize().height-40)

	self.btn_discord = container:getChildByName("btn_discord")
	self.btn_discord:setPosition(display.getLeft()+100, self.btn_notice:getPositionY() - self.btn_discord:getContentSize().height-40)
	...
	self.info_panel = container:getChildByName("info_panel")
-- 	self.info_panel:setVisible(true)
    self.info_label = self.info_panel:getChildByName("info_label")
    self.info_label_0 = self.info_panel:getChildByName("info_label_0")
    self.info_label:setVisible(false)
	if PLATFORM_PROJECT_INFO then
		self.info_label:setVisible(true)
		self.info_label:setString(PLATFORM_PROJECT_INFO)
	end
	self.info_label_0:setVisible(CHARGE_CONFIG_TYPE and CHARGE_CONFIG_TYPE == "zh")

	self.year_tip_btn = container:getChildByName("year_tip_btn")

	self.year_tip_btn:setVisible(CHARGE_CONFIG_TYPE and CHARGE_CONFIG_TYPE == "zh")
    ...
    self.checkbox_lable:setString("<div fontcolor=#ffffff outline=2,#000000>我已详细阅读并同意<div href=onclick_1 fontcolor=#eab95b outline=2,#000000>游戏用户协议</div>和<div href=onclick_2 fontcolor=#eab95b outline=2,#000000>隐私保护指引</div></div>")
    self.checkbox_lable:addTouchLinkListener(function(type, value, sender, pos)
        if value == "onclick_1" then
            --用户协议
            local value = GAME_PROTO_URL
    ...
    self.is_agree = SysEnv:getInstance():getBool(SysEnv.keys.user_proto_agree, true)
	...
	if CHARGE_CONFIG_TYPE ~= nil then
		self.btn_discord:setVisible(false)
	end
end
...
function EnterPanel:registerEvent()
	...
	self.btn_discord:addTouchEventListener(function(sender, event_type)
		if event_type == ccui.TouchEventType.ended then
			sdkCallFunc("openUrl", "https://discord.gg/DDNJzWEFvj")
		end
	end)

	local str = "1、本游戏是一款放置挂机卡牌游戏，适用于年满12周岁及以上的用户，建议未成年人在家长监护下使用游戏产品。\n\n2、本游戏基于幻想的神奇宠物世界观，没有基于真实历史和现实事件改编的内容。玩家将作为一名训练师，抓捕获得宠物，带着宠物们一起在宠物世界中冒险，与其他训练师玩家进行宠物之间的比赛，与宠物一起在这幻想的世界中成长。\n\n3、游戏中有用户实名认证系统，认证为未成年人的用户将接受以下管理：\n未成年人在线时长限制（根据2021年8月通知调整）\n\n◆仅在周五、周六、周日和法定节假日每日20时至21时，向未成年人提供1小时游戏服务。其余时间未成年人将无法登录游戏。\n◆关闭游戏内游客账号登录功能，不再向未实名注册和登录的用户提供任何形式的游戏服务。\n\n未成年人付费服务限制（遵循2019年11月通知设置，无调整）\n为规范未成年人付费服务，对未成年人用户付费服务实行以下限制：\n◆8周岁以上未满16周岁的用户，单次充值金额不得超过50元人民币，每月充值金额累计不得超过200元人民币\n◆16周岁以上未满18周岁的用户，单次充值金额不得超过100元人民币，每月充值金额累计不得超过400元人民币。\n\n4、本游戏以策略卡牌对战为核心玩法，有助于锻炼玩家独立思考和分析解决问题的能力。游戏玩法简单易懂、关卡设计精妙，画面精美，致力于以出色的视听和游玩效果带给玩家积极愉悦的情绪体验，增强玩家的自信心。"

	self.year_tip_btn:addTouchEventListener(function(sender, event_type)
		if event_type == ccui.TouchEventType.ended then
			playButtonSound2()
			TipsManager:showCommonTips(str, sender:getTouchBeganPosition(), 16,nil, 500, true)
		end
	end)
