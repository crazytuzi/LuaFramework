...
function MailInfoWindow:open_callback()
    ...
    local height = self.info_container_size.height
    ...
    self.content = createRichLabel(24,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(20,height-50),5,0,525)
    self.scrollView:addChild(self.content)
    info_container:getChildByName("youxiao"):setVisible(false)
    self.youxiao = createLabel(20,Config.ColorData.data_new_color4[12],nil,width - 20 ,height - 25,TI18N("有效期"),self.scrollView,nil, cc.p(1, 0.5))
    ...
    local btn_size = self.btn:getContentSize()
    self.btn_label = createRichLabel(22,Config.ColorData.data_new_color4[1],cc.p(0.5, 0.5), cc.p(btn_size.width/2, btn_size.height/2))
    self.btn:addChild(self.btn_label)
    self.btn:getChildByName("Text_3"):setString(TI18N(""))
    -- self.btn:getChildByName("Text_3"):setString(TI18N("删除"))
...
function MailInfoWindow:setScrollViewSize(is_have_item)
    ...
    self.content:setPositionY(scroll_heigt - self.init_pos_y + 20)
...
function MailInfoWindow:changeButtonStatus( status )
	if status then
		...
		self.btn_label:setString(string.format(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,%s>领取</div>"), Config.ColorData.data_new_color_str[3]))
		...
		self.btn:loadTextures(res, res, "", LOADTEXT_TYPE_PLIST)
		self.btn_label:setString(string.format(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,%s>删除</div>"), Config.ColorData.data_new_color_str[2]))
