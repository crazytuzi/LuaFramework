...
function ActionLimitBuyItem:configUI()
    ...
    self.item_scrollview:setSwallowTouches(false)
    self.limit_label = createRichLabel(22, 175, cc.p(1,0.5), cc.p(665,139), nil, nil, 400)
    self.main_container:addChild(self.limit_label)

    self.old_price = createRichLabel(20, 175, cc.p(1,0.5), cc.p(660,104), nil, nil, nil)
    ...
    if lanCode ~= "zh" then
        self.price_line:setPositionX(100)
    end
end
...
function ActionLimitBuyItem:setData(data)
    ...
    if discount_list and new_price_list then
        local val = tostring(discount_list.aim_args_val or 0)
        local str = string.format("%s%s%s",TI18N("原价:"),GetSymbolByType(),val)
        --local str = string.format(TI18N("原价:￥ %s"), (discount_list.aim_args_val or 0))
        self.old_price:setString(str)
        local price_str = string.format("<div fontColor=#ffffff fontsize=24 outline=2,#2b610d>%s %s</div>", GetSymbolByType(), new_price_list.aim_args_val or 0)
    ...
    if data.sort_index == 2 then
        ...
        local price_str = string.format("<div fontColor=#ffffff fontsize=24>%s %s</div>", GetSymbolByType(), current_price or 0)
        self.btn:setRichText(price_str)
    else
        self.btn:setGrayAndUnClick(false, true)
        local price_str = string.format("<div fontColor=#ffffff fontsize=24 outline=2,#2b610d>%s %s</div>", GetSymbolByType(), current_price or 0)
        self.btn:setRichText(price_str)
    end
end
