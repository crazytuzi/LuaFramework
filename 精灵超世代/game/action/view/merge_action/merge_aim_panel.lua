...
function MergeAimPanel:configUI()
    ...
    self.process:setContentSize(cc.size(225,18))  -- Max = 512
...
function MergeAimPanel:setData(data)
    ...
    local length = 512 * data.score / data.max_score
    self.process:setContentSize(cc.size(length,18))
    self.Text_lev:setString(string.format(TI18N("%s级"),data.lev))
