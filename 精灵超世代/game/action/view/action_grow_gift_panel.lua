...
function ActionGrowGiftItem:configUI(  )
    ...
    self.time = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(1, 0.5), cc.p(670, 25))
...
function ActionGrowGiftItem:setData1( data )
    ...
    if data.limit_type == 4 then
        ...
        if data.original_price then
            self.old_price_txt:setString(TI18N("原价:") .. GetSymbolByType() .. data.original_price)
    ...
    -- 价格
    self.price_txt:setString(GetSymbolByType() .. data.price)
...
function ActionGrowGiftItem:setData( data )
    ...
    if left_num == 0 then -- 卖完
        ...
        if self.data.limit_type == 4 then -- 永久限购
            ...
            local cur_time = GameNet:getInstance():getTime()
            local less_time = data.end_time - cur_time
            commonCountDownTime(self.time, less_time, {time_title = TI18N("重置倒计时"), time_color = "#0e7709", label_type = CommonAlert.type.rich})
        end
    else
        ...
        if self.data.limit_type == 4 then -- 永久限购
            ...
            local cur_time = GameNet:getInstance():getTime()
            local less_time = data.end_time - cur_time
            commonCountDownTime(self.time, less_time, {time_title = TI18N("剩余"), time_color = "#0e7709", label_type = CommonAlert.type.rich})
        end
