...
function ActionSevenGoalWindow:open_callback()
    ...
    for i=1,4 do 
        ...
        btn.title:setTextColor(Config.ColorData.data_new_color4[6])
    ...
    for i=1, 7 do
        ...
        local str = string.format(TI18N("第%s天"),i)
        textDay:setString(str)
    ...
    self.item_scrollview_walfare = CommonScrollViewLayout.new(self.goods_walfare, cc.p(0.5,0) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size_walfara, setting)
    --self.item_scrollview_walfare:setPositionX(bgSize.width/2)
...
function ActionSevenGoalWindow:seventGoal(data)
    ...
    if not self.item_load then
        local res1 = PathTool.getPlistImgForDownLoad(str_res,"action_seven_goal_bg1",true)
...
function ActionSevenGoalWindow:selectByBtn( index )
    ...
    if self.cur_select ~= nil then
        ...
        self.cur_select.title:disableEffect(cc.LabelEffect.SHADOW)
        self.cur_select.title:setTextColor(Config.ColorData.data_new_color4[6])
    end
    self.cur_index = index
    self.cur_select = self.btn_list[self.cur_index]
    if self.cur_select ~= nil then
        self.cur_select.normal:setVisible(false)
        self.cur_select.select:setVisible(true)
        self.cur_select.title:setTextColor(Config.ColorData.data_new_color4[1])
        self.cur_select.title:enableShadow(Config.ColorData.data_new_color4[2],cc.size(0, -2),2)
    end
    ...
    setLabelAutoScale(self.btn_list[2].title,self.btn_list[2].normal,16)
    setLabelAutoScale(self.btn_list[3].title,self.btn_list[3].normal,16)
...
function ActionSevenGoalItem:configUI( )
    ...
    local btn_get_label = self.btn_get:getChildByName("text")
    if btn_get_label ~= nil then
        btn_get_label:enableShadow(Config.ColorData.data_new_color4[2],cc.size(0, -2),2)
    end
    self.btn_get:setVisible(false)

    self.btn_goto = self.main_container:getChildByName("btn_goto")
    --self.btn_goto:setTitleText(TI18N("前往"))
    self.btn_goto_label = self.btn_goto:getChildByName("text")
    if self.btn_goto_label ~= nil then
        self.btn_goto_label:enableShadow(Config.ColorData.data_new_color4[2],cc.size(0, -2),2)
    end
...
function ActionSevenGoalItem:setData( data )
    ...
    if self.type ~= 4 then
        ...
        if self.init_cur_day and self.cur_day > self.init_cur_day then
            ...
            self.btn_goto_label:disableEffect(cc.LabelEffect.SHADOW)
