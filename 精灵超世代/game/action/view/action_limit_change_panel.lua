...
function ActionLimitChangePanel:configUI()
    ...
    self.dec_val = createRichLabel(20, nil, cc.p(0, 1), cc.p(0,20),12,nil,540)
    ...
    local time_color = Config.ColorData.data_new_color4[1]
    if config_data then
        time_color = self:colorChangeData(config_data.time_color1)
    end
    self.time_val = createRichLabel(20, time_color, cc.p(0, 0.5), cc.p(0,0),nil,nil,540)
    ...
    if less_time > 0 then
        ...
        self.time_val:runAction(cc.RepeatForever:create(cc.Sequence:create(cc.DelayTime:create(1),
            cc.CallFunc:create(function()
                ...
                if less_time < 0 then
                    self.time_val:stopAllActions()
                    local str = string.format(TI18N("结束时间: <div fontcolor=#0cff01>活动已结束</div>"))
                    if self.holiday_bid == ActionChangeCommonType.limit_yuanzhen or self.holiday_bid == ActionChangeCommonType.limit_yuanzhen1 then
                        str = string.format(TI18N("结束时间: <div fontcolor=#0cff01>活动已结束</div>"))
                    end
