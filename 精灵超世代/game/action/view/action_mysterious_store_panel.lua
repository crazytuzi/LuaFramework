...
function ActionMysteriousStorePanel:configUI()
    ...
    self.btn_rule:setPositionX(660)
...
function ActionMysteriousStorePanel:updataItem(bag_code, data_list, is_update)
    ...
    if is_update then
        ...
        if self.item_icon == nil  then
            self.item_icon = createSprite(nil, 20, -515, self.title_con, cc.p(0.5, 0.5), nil)
            self.item_icon:setScale(0.4)
        end
        if self.item_num == nil  then
            self.item_num = createLabel(24, Config.ColorData.data_new_color4[6], nil, 50, -515, nil, self.title_con, nil, cc.p(0,0.5))
        end
...
function ActionStoreItem:setChangeData(data)
    ...
    self.txt_task:setString(string_format(TI18N("剩余:%d"), data.count))
