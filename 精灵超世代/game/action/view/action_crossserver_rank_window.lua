...
-- Create: 2018-06-28
-- --------------------------------------------------------------------
ActionCrossServerRankWindow = ActionCrossServerRankWindow or BaseClass(BaseView)
...
function ActionCrossServerRankWindow:__init()
    ...
    self.res_list = {
    	...
    	{ path = PathTool.getPlistImgForDownLoad("actioncrossserver","actioncrossserver"),type =ResourcesType.plist},
	}
...
function ActionCrossServerRankWindow:open_callback()
    ...
    if self.goto_btn.label ~= nil then
        self.goto_btn.label:enableOutline(Config.ColorData.data_color4[278], 2)
    end
    ...
    if self.detail_btn.label ~= nil then
        self.detail_btn.label:enableOutline(Config.ColorData.data_color4[277], 2)
    end

    self.scroll_con = self.container:getChildByName("scroll_con")
    local scroll_view_size = self.scroll_con:getContentSize()
    local setting = {
        item_class = ActionCrossServerRankItem,      -- 单元类
        start_x = 0,                  -- 第一个单元的X起点
        space_x = 2,                    -- x方向的间隔
        start_y = 0,                    -- 第一个单元的Y起点
        space_y = 0,                   -- y方向的间隔
        item_width = ActionCrossServerRankItem.Width,               -- 单元的尺寸width
        item_height = ActionCrossServerRankItem.Height,              -- 单元的尺寸height
        row = 0,                        -- 行数，作用于水平滚动类型
        col = 1                         -- 列数，作用于垂直滚动类型
    }
...
function ActionCrossServerRankWindow:openRootWnd( index )
	controller:sender22700(1)
	self.index = index or 1 
end

function ActionCrossServerRankWindow:createTabBtn( list  )
	self.tab_scroll:setInnerContainerSize(cc.size((ActionCrossServerRankTab.Width+10)*7,self.tab_scroll:getContentSize().height))
	local show_list = {}
	local temp = controller:getModel():getCrossServerRankListData(1)
	...
	for i,v in pairs(show_list) do
		if self.tab_list[v.id] == nil then 
			local item = ActionCrossServerRankTab.new()
			item:setPosition(10+(ActionCrossServerRankTab.Width+10)*(i-1),2)
...
function ActionCrossServerRankWindow:selectByTab( index,status)
...
function ActionCrossServerRankWindow:fileSevenRankInfo()
	if self.cur_index == nil then return end

	-- 这里显示从第4名开始,,,,可能需要前面插入每日任务
	local show_list = {}
	local quest_list = deepCopy(Config.DaysRankData.data_rank_quest[self.cur_index]) or {}
...
function ActionCrossServerRankWindow:register_event(  )
	if self.close_btn then
		self.close_btn:addTouchEventListener(function(sender, event_type)
			if event_type == ccui.TouchEventType.ended then
				playCloseSound()
				controller:openCorssserverRankWin(false)
			end
		end)
	end
	...
	self.detail_btn:addTouchEventListener(function ( sender,event_type )
		if event_type == ccui.TouchEventType.ended then
			...
			if self.rank_list[self.cur_index] and next(self.rank_list[self.cur_index])~=nil then
				RankController:getInstance():openRankView(true,Config.DaysRankData.data_rank_list[self.cur_index][1].jump,true)
			else
	...
	if not self.update_tab then
		self.update_tab = GlobalEvent:getInstance():Bind(ActionEvent.UPDATE_CROSSSERVER_RANK_LIST,function ( data )
	...
	if not self.update_single_rank then
		self.update_single_rank = GlobalEvent:getInstance():Bind(ActionEvent.UPDATE_CROSSSERVER_RANK_DATA,function ( data )
	...
	if not self.update_quest_event then
		self.update_quest_event = GlobalEvent:getInstance():Bind(ActionEvent.UPDATE_CROSSSERVER_QUEST, function() 
			self:fileSevenRankInfo()
		end)
	end
end
...
--- 设置前3名的状态
function ActionCrossServerRankWindow:updateTop3Info( rank_list )
...
--- 设置排行榜前3的基础奖励数据
function ActionCrossServerRankWindow:setTop3BaseInfo(  )
	for i,object in ipairs(self.top_3_list) do
		if object.head then
			object.head:setVisible(false)
		end
	...
	-- 取出第一名的称号	
	local face_id = nil
	if Config.DaysRankData.data_rank_data_const["rank_title"..self.cur_index] == nil then   -- 称号如果配置上去了就要不为空了
		face_id=Config.DaysRankData.data_rank_data_const["rank_title7"].val
	else
		face_id = Config.DaysRankData.data_rank_data_const["rank_title"..self.cur_index].val
	end
...
--设置倒计时
function ActionCrossServerRankWindow:setLessTime( less_time )
    if tolua.isnull(self.rank_time) then return end
    self.rank_time:stopAllActions()
    if less_time > 0 then
        self:setTimeFormatString(less_time)
        self.rank_time:runAction(cc.RepeatForever:create(cc.Sequence:create(
...
function ActionCrossServerRankWindow:setTimeFormatString(time)
    if time > 0 then
...
function ActionCrossServerRankWindow:close_callback()
...
-- Create: 2018-06-28
-- --------------------------------------------------------------------
ActionCrossServerRankTab = class("ActionCrossServerRankTab", function()
    return ccui.Widget:create()
end)

ActionCrossServerRankTab.Width = 123
ActionCrossServerRankTab.Height = 130

function ActionCrossServerRankTab:ctor()
	self:configUI()
	self:register_event()
end

function ActionCrossServerRankTab:configUI(  )
	self.root_wnd = createCSBNote(PathTool.getTargetCSB("action/action_seven_rank_tab"))
    self:addChild(self.root_wnd)
    self:setCascadeOpacityEnabled(true)
    self:setContentSize(cc.size(ActionCrossServerRankTab.Width,ActionCrossServerRankTab.Height))
...
function ActionCrossServerRankTab:register_event(  )
...
function ActionCrossServerRankTab:setData(data)
	self.index = data.id
	self.day:setString(TI18N("第")..self.index..TI18N("天"))
	...
	local res = PathTool.getResFrame("actioncrossserver","actioncrossserver_ion"..(self.index-7))
	loadSpriteTexture(self.icon,res,LOADTEXT_TYPE_PLIST)
end

function ActionCrossServerRankTab:setSelect( status )
	if status then 
...
function ActionCrossServerRankTab:getIndex(  )
	return self.index
end

function ActionCrossServerRankTab:setIsTouch( status )
	self.is_touch = status
end

function ActionCrossServerRankTab:addCallBack( value )
	self.callback =  value
end

function ActionCrossServerRankTab:DeleteMe()
	if self.item_load then 
        self.item_load:DeleteMe()
        self.item_load = nil
    end
...
-- Create: 2018-06-28
-- --------------------------------------------------------------------
ActionCrossServerRankItem = class("ActionCrossServerRankItem", function()
    return ccui.Widget:create()
end)

ActionCrossServerRankItem.Width = 686
ActionCrossServerRankItem.Height = 124

function ActionCrossServerRankItem:ctor()
	self:configUI()
	self:register_event()
end

function ActionCrossServerRankItem:configUI(  )
	self.root_wnd = createCSBNote(PathTool.getTargetCSB("action/action_seven_rank_item"))
    self:addChild(self.root_wnd)
    self:setCascadeOpacityEnabled(true)
    self:setContentSize(cc.size(ActionCrossServerRankItem.Width,ActionCrossServerRankItem.Height))
    self:setAnchorPoint(0,0)
...
function ActionCrossServerRankItem:register_event(  )
...
function ActionCrossServerRankItem:setData(data)
	if data then
		self.data = data
...
function ActionCrossServerRankItem:suspendAllActions()
end

function ActionCrossServerRankItem:DeleteMe()
	if self.item_scrollview then
        self.item_scrollview:DeleteMe()
        self.item_scrollview = nil
    end
