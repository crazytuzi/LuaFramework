...
ActionTimeShopPanel = class("ActionTimeShopPanel", function()
    return ccui.Widget:create()
end)

function ActionTimeShopPanel:ctor(bid)
	self.holiday_bid = bid
	self:configUI()
	self:register_event()
end

function ActionTimeShopPanel:configUI(  )
    ...
    local str_banner = "txt_cn_welfare_banner22"
    ...
    time_panel:setBackGroundColorOpacity(0)
...
function ActionTimeShopPanel:register_event(  )
...
function ActionTimeShopPanel:setLimitShopData( data )
    local shop_data = {}
    local len =  #data.aim_list
...
function ActionTimeShopPanel:setVisibleStatus(bool)
	bool = bool or false
    self:setVisible(bool) 
end

function ActionTimeShopPanel:DeleteMe()
...
---------------------------@ 子项
ActionTimeShopItem = class("ActionTimeShopItem", function()
    return ccui.Widget:create()
end)

function ActionTimeShopItem:ctor()
    self.touch_buy_limit_shop = true
	self:configUI()
	self:register_event()
end

function ActionTimeShopItem:configUI()
...
function ActionTimeShopItem:register_event()
...
function ActionTimeShopItem:setData( data )
    ...
    if data.status == 2 then -- 卖完
        ...
        -- 价格
        self.charge_price:setString(GetSymbolByType()..price)
    end
end

function ActionTimeShopItem:DeleteMe()
    if self.buy_limit_shop_ticket ~= nil then
        GlobalTimeTicket:getInstance():remove(self.buy_limit_shop_ticket)
        self.buy_limit_shop_ticket = nil
    end
