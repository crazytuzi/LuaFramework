...
function ActionPractiseTowerPanel:configUI()
    ...
    self.time_label = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(1, 0.5), cc.p(680, -30),nil,nil,500)
    ...
    if self.btn_tips then
        self.btn_tips:addTouchEventListener(function( sender,event_type )
            if event_type == ccui.TouchEventType.ended then
                playButtonSound2()
                TipsManager:getInstance():showCommonTips(Config.HolidayPractiseTowerData.data_const.rules.desc, sender:getTouchBeganPosition(), nil, nil, 600, true)
            end
        end)
    end
...
function ActionPractiseTowerPanel:setTimeValFormatString(time)
    if time > 0 then
        local str = string_format(TI18N("<div outline=2,#3d5078>剩余时间：</div><div fontcolor=#0cff01 outline=2,#3d5078> %s</div>"), TimeTool.GetTimeFormatDayIIIIIIII(time))
        self.time_label:setString(str)
    else
        self.time_label:setString(TI18N("<div outline=2,#3d5078>活动已结束</div>"))
    end
end
...
function ActionPractiseTowerPanel:createSingleRankItem(i)
	...
	local sp = createSprite(PathTool.getResFrame("common","common_"..(3007+i)),0,60/2,container)
	sp:setScale(0.5)
	container.sp = sp
	local label = createLabel(20,1,nil,30,45,"",container)
	label:setAnchorPoint(cc.p(0,0.5))
	label:setTextColor(Config.ColorData.data_new_color4[15])

	local value = createLabel(20,1,nil,30,20,"",container)
