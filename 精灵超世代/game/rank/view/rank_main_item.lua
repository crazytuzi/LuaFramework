...
function RankMainItem:layoutUI()
    ...
    self.rank_name = self.main_panel:getChildByName("rank_name")

    self.first_icon = self.root_wnd:getChildByName("first_icon")  
    self.name = self.main_panel:getChildByName("name")
 

    self.rank_label = createRichLabel(22, Config.ColorData.data_new_color4[1], cc.p(0,0), cc.p(151,28), 0, 0, 400)
...
function RankMainItem:setData(vo)
    ...
    local res_id = PathTool.getPlistImgForDownLoad("bigbg/rank", "rank_bg_" .. self.rank_type)
    ...
    local res_name_id = PathTool.getPlistImgForDownLoad("bigbg/rank", "txt_cn_rank_" .. self.rank_type)
    if self.res_name_id ~= res_name_id then
        self.res_name_id = res_name_id 
        self.res_name_load = loadSpriteTextureFromCDN(self.rank_name, res_name_id, ResourcesType.single, self.res_name_load)
    end
    ...
    if self.data.val1 ~= 0 then
        ...
        if self.rank_type ~= RankConstant.RankType.union then
            if not self.player_head then 
                ...
                self.player_head:setPosition(78, 78)
            ...
            if not self.union_icon then
                local res = PathTool.getResFrame("guild", "guild_1030")
                self.union_icon = createImage(self.main_panel, res, 78, 70, cc.p(0.5,0.5), true)
                ...
                --self.union_icon:loadTexture(res, LOADTEXT_TYPE_PLIST)
        ...
        if self.rank_type == RankConstant.RankType.arena then
            ...
            str = string.format(TI18N("<div outline=2,%s>竞技杯数：</div><div fontcolor=%s outline=2,%s>%s</div>"),Config.ColorData.data_new_color_str[6],Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],val)
        elseif self.rank_type == RankConstant.RankType.drama then
            ...
            if config then
                ...
                str = string.format(TI18N("<div outline=2,%s>剧情进度：</div><div fontcolor=%s outline=2,%s>%s</div>"),Config.ColorData.data_new_color_str[6],Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],config.name)
            end
        elseif self.rank_type == RankConstant.RankType.power or 
            ...
            str = string.format(TI18N("<div outline=2,%s>战力：</div><div fontcolor=%s outline=2,%s>%s</div>"),Config.ColorData.data_new_color_str[6],Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],val)
        ...
        elseif self.rank_type == RankConstant.RankType.tower then
            ...
            str = string.format(TI18N("<div outline=2,%s>通关层数：</div><div fontcolor=%s outline=2,%s>%s</div>"),Config.ColorData.data_new_color_str[6],Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],val)
        elseif self.rank_type == RankConstant.RankType.union then
            ...
            str = string.format(TI18N("<div outline=2,%s>总战力：</div><div fontcolor=%s outline=2,%s>%s</div>"),Config.ColorData.data_new_color_str[6],Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],val)
        elseif self.rank_type == RankConstant.RankType.star_power then
            local val = self.data.val1 or 0
            -- str = string.format(TI18N("<div outline=2,#1f1309>评分：%s</div>"),val)
            str = string.format(TI18N("<div outline=2,%s>评分：</div><div fontcolor=%s outline=2,%s>%s</div>"),Config.ColorData.data_new_color_str[6],Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],val)
        ...
        elseif self.rank_type == RankConstant.RankType.action_adventure then
            ...
            str = string.format(TI18N("<div outline=2,%s>神界探索度：</div><div fontcolor=%s outline=2,%s>%s</div>"),Config.ColorData.data_new_color_str[6],Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],val)
            self:showActionIcon(true)
        elseif self.rank_type == RankConstant.RankType.hallows_power then
            ...
            str = string.format(TI18N("<div outline=2,%s>圣器战力：</div><div fontcolor=%s outline=2,%s>%s</div>"),Config.ColorData.data_new_color_str[6],Config.ColorData.data_new_color_str[15],Config.ColorData.data_new_color_str[6],val)
        end
        self.name:setString(name)
...
function RankMainItem:DeleteMe()
    ...
    if self.res_name_load then
        self.res_name_load:DeleteMe()
    end
