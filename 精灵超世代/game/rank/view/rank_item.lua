...
function RankItem:configUI( ... )
	...
	self.background:setCapInsets(cc.rect(134,20,2,2))
	...
	self.rank_index =  createLabel(48,Config.ColorData.data_new_color4[6],nil,50,self.height/2,"",self,0, cc.p(0.5,0.5),"fonts/title.ttf")
...
function RankItem:setData( vo, index)
	...
	if self.index >= 1 and self.index <= 3 then
		...
		self.rank_icon:loadTexture(PathTool.getResFrame("common",RankConstant.RankIconRes[self.index]),LOADTEXT_TYPE_PLIST)
...
function RankItem:updateDataByRankType()
	...
	local data_pos = RankConstant.DataPos[self.rank_type]
	local head_x = data_pos[2] - 40
	local head_y = 70
	local name_x = data_pos[2]
	local name_y = 18

	if self.rank_type ~= RankConstant.RankType.union then 
		-- self.play_head = self:createPlayerHead(105,60)
		self.play_head = self:createPlayerHead(head_x,head_y)		
	...
	if self.rank_type == RankConstant.RankType.power or self.rank_type == RankConstant.RankType.action_power then 
		local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,name_x,name_y,"",self.other_panel,0, cc.p(0.5,0.5))
		label:setString(role_name)

		local label = createRichLabel(24, Config.ColorData.data_new_color4[6],cc.p(0.5,0.5), cc.p(data_pos[3],self.height/2), 0, 0, 500)
	...
	elseif self.rank_type == RankConstant.RankType.union_boss then
		...
		label:setString(role_name)
		--战力
		local label = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(data_pos[3], self.height / 2 + 34), 0, 0, 500)
		self.other_panel:addChild(label)
		local all_dps = self.data.all_dps or 0
		label:setString(changeBtValueForBattle(all_dps))
		--增加点赞功能
	...
	elseif self.rank_type == RankConstant.RankType.drama or self.rank_type == RankConstant.RankType.action_drama  then 
		...
		label:setString(role_name)
		--装备评分
		local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,data_pos[3],self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
	...
	elseif self.rank_type == RankConstant.RankType.union then 
		...
		--名字
		local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,name_x,80,"",self.other_panel,0, cc.p(0.5,0.5))
		label:setString(role_name)
		--宗主名字
		local label = createLabel(20,Config.ColorData.data_new_color4[6],nil,name_x,50,"",self.other_panel,0, cc.p(0.5,0.5))
		label:setString(TI18N("会长：")..(self.data.leader_name or ""))
		--等级
		local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,data_pos[3],self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
		label:setString(self.data.lev or 313)
		--人数
		local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,data_pos[4],self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
		local num1 = self.data.members_num or 0
		local num2 = self.data.members_max or 0
		label:setString(num1.."/"..num2)
		--战力
		local label = createRichLabel(24, Config.ColorData.data_new_color4[6],cc.p(0.5,0.5), cc.p(data_pos[5],self.height/2), 0, 0, 500)
		self.other_panel:addChild(label)
		local res = PathTool.getResFrame("common","common_90002")
	...
	elseif self.rank_type == RankConstant.RankType.tower or self.rank_type == RankConstant.RankType.action_tower then 
		...
		label:setString(role_name)
		--层数
		local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,data_pos[3],61,"",self.other_panel,0, cc.p(0.5,0.5))
		label:setString(self.data.val1 or 0)
		--通关时间
		local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,data_pos[4],self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
	...
	elseif self.rank_type == RankConstant.RankType.arena or self.rank_type == RankConstant.RankType.action_arena then 
		...
		label:setString(self.data.score or self.data.val1 or 0)
	...
	elseif self.rank_type == RankConstant.RankType.elite then
        --名字
		local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,198,self.height/2+20,"",self.other_panel,0, cc.p(0.5,0.5))
		...
		local power = createLabel(22,cc.c4b(0xff,0xee,0xac,0xff),nil,48,16,"",power_bg,0, cc.p(0.5,0.5))
        ...
        --积分
		local score = createLabel(24,Config.ColorData.data_new_color4[6],nil,509,self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
		score:setString(self.data.score)
	elseif self.rank_type == RankConstant.RankType.ladder then --天梯排行 --lwc
		-- local label = createLabel(24,Config.ColorData.data_new_color4[6],nil,name_x,name_y + 18,"",self.other_panel,0, cc.p(0.5,0.5))
		-- label:setString(role_name)

		-- local name = self.data.guild_name 
		-- local label1 = createLabel(24,Config.ColorData.data_new_color4[6],nil,data_pos[3],self.height/2,"",self.other_panel,0, cc.p(0.5,0.5))
		...
		-- local label = createRichLabel(24, Config.ColorData.data_new_color4[6],cc.p(0.5,0.5), cc.p(name_x,name_y - 18), 0, 0, 500)
		-- self.other_panel:addChild(label)
		-- -- local power = self.data.val1 or 0
		-- -- local res = PathTool.getResFrame('common', 'common_90002')
		-- -- label:setString(string.format("<img src='%s' /> %s", res, power))
...
function RankItem:addPraise(data)
	self._btnPraise = createButton(self.other_panel, nil, 504,43, cc.size(168,66),PathTool.getResFrame("common","common_1026"),26,Config.ColorData.data_color4[1])
	self._btnPraise:setScale(0.65)
	self._praise = createSprite(PathTool.getResFrame("common", "common_1045"), 40, 33, self._btnPraise, cc.p(0.5,0.5), LOADTEXT_TYPE_PLIST)
	...
	if self.index <= 3 then
		self._btnPraise:addTouchEventListener(function(sender, event_type)
			--customClickAction(sender,event_type,0.65)
            if event_type == ccui.TouchEventType.ended then
            	playButtonSound2()
    ...
    --点赞数字
	self._labelPraise = createLabel(24, Config.ColorData.data_color4[1], nil, 85, 33, '', self._btnPraise, 0, cc.p(0.5, 0.5))
