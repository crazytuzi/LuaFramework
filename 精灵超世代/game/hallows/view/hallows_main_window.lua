...
function HallowsMainWindow:open_callback()
	...
	title_bg:setPositionY(top_off - 165)
	self.explain_btn:setPositionY(top_off - 165)
end
...
function HallowsMainWindow:updateHallowsBaseInfo()
	...
	if self.hallows_model_id ~= hallows_effect_id then
		...
		loadSpriteTexture(self.trainer_sp, "resource/hallows/trainer_big_"..self.select_hallows.id..".png", LOADTEXT_TYPE)
	end
	...
	for i,attr in ipairs(step_config.attr) do
		...
		if attr_name then
			...
			if attr_text == nil then
				attr_text = createRichLabel(22, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(20, 28), nil, nil, 380)
				local attr_bg = self.attr_bgs[i]
				attr_text:setPosition(cc.p(11, 20))
            ...
            local attr_str = string_format("<img src='%s' scale=1 /> <div fontcolor=#3d5078>  %s：</div><div fontcolor=#3d5078>%s</div>", PathTool.getResFrame("common", icon), attr_name, tostring(attr_val))
            -- 加上幻化皮肤加成
            if model:checkIsHaveHallowsMagic() then
				...
				if add_value <= 0 then
					...
					attr_str = attr_str .. string_format("<div fontcolor=#0e7709> +%s</div>", tostring(math.ceil(add_value/1000*attr_val)))
				else
					if is_per then
	            		add_value = (add_value/10) .. "%"
	            	end
	            	attr_str = attr_str .. string_format("<div fontcolor=#0e7709> +%s</div>", tostring(add_value))
				end
            end
	...
	if hallows_skill and hallows_skill.skill_bid ~= 0 then
		...
		if not self.hallows_skill_name then
			self.hallows_skill_name = createLabel(24,Config.ColorData.data_new_color4[10],nil,135,267,"",self.skill_container,1,cc.p(0,0))
		end
		if model:getHallowsRefineIsOpen() then
			self.refine_is_open = true
			self.hallows_skill_name:setString(string_format(TI18N("%s+%d【精炼+%d级】"), config.name, vo.skill_lev, vo.refine_lev))
		...
		if not self.hallows_skill_desc then
			self.hallows_skill_desc = createRichLabel(20,Config.ColorData.data_new_color4[6],cc.p(0,1),cc.p(135, 250),2,nil,550)
		...
		self.hallows_skill_desc:setString(string_format(config.des, changeBtValueForPower(total_atk_val), changeBtValueForPower(refine_atk_val)))
...
function HallowsMainWindow:updateSkillList()
	...
	for i,config in ipairs(skill_attr_config) do
		...
		if attr_txt == nil then
			attr_txt = createLabel(20,cc.c3b(98,103,114),nil,0,0,"",self.skill_bg,nil,cc.p(0, 0.5))
			self.skill_attr_list[i] = attr_txt
		end
		attr_txt:setVisible(true)
		attr_txt:setPosition(cc.p(37, 92 - (i-1)*32))
		if vo.skill_lev >= config.lev_limit then
			attr_txt:setTextColor(Config.ColorData.data_new_color4[6])
		else
			attr_txt:setTextColor(cc.c3b(98,103,114))
		end
...
function HallowsMainWindow:updateMaxLvTips(  )
	if self.cur_index == HallowsConst.Tab_Index.uplv and self.is_max_hallows_lv then
		self.max_lv_tips:setString(TI18N("已满级"))
	...
	elseif self.cur_index == HallowsConst.Tab_Index.refine and self.is_max_refine_lv then
		self.max_lv_tips:setString(TI18N("精炼已等级"))
		self.maxlv_layout:setVisible(true)
	else
...
function HallowsMainWindow:updateRefineInfo(  )
	...
	if refine_cfg then
		self.atk_txt_1:setString(changeBtValueForPower(refine_cfg.add_dps))
		self.total_atk_txt_1:setString(changeBtValueForPower(refine_cfg.add_dps + skill_atk_val))
	else
		self.atk_txt_1:setString(0)
		self.total_atk_txt_1:setString(changeBtValueForPower(skill_atk_val))
	end
	...
	if next_refine_cfg then
		...
		self.atk_txt_2:setString(changeBtValueForPower(next_refine_cfg.add_dps))
		self.total_atk_txt_2:setString(changeBtValueForPower(next_refine_cfg.add_dps + skill_atk_val))
	end
end
...
function HallowsMainWindow:setCostDataToNode( item_icon, item_label, item_bid, item_num )
	...
	if item_config then
		...
		if count < item_num then
			item_label:setTextColor(Config.ColorData.data_new_color4[14])
			is_full = false
		else
			item_label:setTextColor(Config.ColorData.data_new_color4[15])
		end
	end
	return is_full
end
