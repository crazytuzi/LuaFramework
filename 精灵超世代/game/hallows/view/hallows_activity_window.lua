...
function HallowsActivityWindow:open_callback()
    ...
    self.desc:setString(TI18N("全体上阵宝可梦"))
...
function HallowsActivityWindow:openRootWnd(data, open_type)
	...
	if data then
		...
		elseif self.open_type == HallowsConst.Activity_Type.Magic then
			...
			self.desc:setString(TI18N("全神器对宝可梦属性提升"))
			...
			if attr_list and next(attr_list) then
				for i,v in ipairs(attr_list) do
					...
					if attr_name then
						...
						local str = string_format("%s<div fontcolor=#0cff01>    +%s</div>", attr_name, attr_val)
...
function HallowsActivityWindow:updateHallowsBaseInfo()
	...
	self.hallows_model = createEffectSpine(self.step_config.effect, cc.p(20, -120), cc.p(0.5, 0.5), true, PlayerAction.action_2)
	self.halidom_sp:addChild(self.hallows_model)
end 
