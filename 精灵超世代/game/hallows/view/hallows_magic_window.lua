...
function HallowsMagicWindow:open_callback(  )
	...
	win_title:setString(TI18N("变幻"))

	self.trainer_sp = main_container:getChildByName("trainer_sp")
	...
	self.magic_btn:getChildByName("label"):setString(TI18N("变 幻"))
	self.cancel_magic_btn = self.open_panel:getChildByName("cancel_magic_btn")
	self.cancel_magic_btn:getChildByName("label"):setString(TI18N("取消变幻"))
...
function HallowsMagicWindow:showHallowsMagicBaseInfo(  )
	...
	loadSpriteTexture(self.trainer_sp, "resource/hallows/trainer_magic_"..self.cur_magic_id..".png", LOADTEXT_TYPE)
	...
	if not self.magic_desc then
		self.magic_desc = createRichLabel(18, Config.ColorData.data_new_color4[15], cc.p(0, 0.5), cc.p(230, 445), 1, nil, 420)
...
function HallowsMagicWindow:showHallowsMagicItemInfo(  )
	...
	if not self.magic_item then
		...
		self.magic_item:setPosition(cc.p(self.main_size.width/2, 290))
...
function HallowsMagicWindow:showHallowsMagicOpenInfo(  )
	...
	for i,attr in ipairs(self.magic_config.attr) do
		...
		if attr_key and attr_val then
			...
			if attr_text == nil and attr_bg then
				attr_text = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(130, 19.5), nil, nil, 380)
	...
	if self.magic_data.eqm_hallows ~= 0 then
		...
		if hallows_cfg then
			self.magic_tips:setString(_string_format(TI18N("已变幻:%s"), hallows_cfg.name))
		...
		if config then
			...
			if item_list and item_list[1] then
				if not self.award_item then
					self.award_item = BackPackItem.new(false, true, false, 0.6, false, true)
					self.award_item:setPosition(cc.p(62, 50))
