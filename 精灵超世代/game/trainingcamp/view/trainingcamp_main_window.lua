...
function TrainingcampMainWindow:__init()
    ...
    self.res_list = {
        ...
        { path = PathTool.getPlistImgForDownLoad("bigbg","bigbg_114", true), type = ResourcesType.single },
	}
end

function TrainingcampMainWindow:open_callback()
    self.background = self.root_wnd:getChildByName("background")
    self.background:loadTexture(PathTool.getPlistImgForDownLoad("bigbg","bigbg_114",true), LOADTEXT_TYPE)
    ...
    local function _getTeamItem(panel,is_left)
        ...
        item.role_head:setScale(0.7)
        if is_left == true then
            item.role_head:setPosition(50, 335)
        else
            item.role_head:setPosition(20, 0)
        end
    ...
    self.txt_lab_1:setString(TI18N("当前无可选宝可梦"))
...
function TrainingcampMainWindow:setData()
    ...
    local res = PathTool.getResFrame("form", "form_icon_3")
