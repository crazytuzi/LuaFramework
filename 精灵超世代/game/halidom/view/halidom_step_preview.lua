...
function HalidomStepPreItem:setData( data )
	...
	for i,v in ipairs(attr_data) do
        ...
        if attr_name then
            ...
            local attr_str = _string_format(TI18N("<img src='%s' scale=1 /> <div fontcolor=#%s>  圣物%s属性+</div><div fontcolor=#%s>%s</div>"), PathTool.getResFrame("common", icon), txt_color, attr_name, txt_color, tostring(attr_val))
