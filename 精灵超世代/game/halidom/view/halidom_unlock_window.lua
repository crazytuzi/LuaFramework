...
function HalidomUnlockWindow:open_callback(  )
	...
	local res = PathTool.checkRes("resource/halidom/txt_halidom/halidom_unlock_title.png")
	self.title = createImage(self.title, res, nil, nil, nil, false)
	...
	self.halidom_sp = self.main_container:getChildByName("halidom_sp")
    ...
    self.attr_1 = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(160, 20))
    item_1:addChild(self.attr_1)
    self.attr_2 = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0, 0.5), cc.p(430, 20))
...
function HalidomUnlockWindow:setData(  )
	...
	self.desc:setString(string_format(TI18N("对所有%s系宝可梦属性提升"), camp_name))
	-- 属性值
	for i=1,2 do
	...
	if lv_cfg.attr and next(lv_cfg.attr) then
		for i,v in ipairs(lv_cfg.attr) do
			...
			if attr_name then
				...
				local str = string_format("%s<div fontcolor=#0e7709>    +%s</div>", attr_name, attr_val)
	...
	--	self.halidom_model = createEffectSpine(PathTool.getEffectRes(self.base_cfg.effect_id), cc.p(360, 420), cc.p(0.5, 0.5), true, PlayerAction.action_2)
	--	self.main_container:addChild(self.halidom_model)
	--end
	loadSpriteTexture(self.halidom_sp, "resource/halidom/halidom_"..self.base_cfg.id..".png", LOADTEXT_TYPE)
end
