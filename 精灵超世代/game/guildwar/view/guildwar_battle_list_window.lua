...
function GuildwarBattleListWindow:__init()
    ...
    self.res_list = {{
        ...
        path = PathTool.getPlistImgForDownLoad("bigbg", "bigbg_3"),
...
function GuildwarBattleListWindow:open_callback()
    ...
    self.tips_label = createRichLabel(20, Config.ColorData.data_color4[175], cc.p(0.5, 0.5), cc.p(300, 680), nil, nil, 500)
    ...
    self.item_scrollview = CommonScrollViewLayout.new(self.item_list, cc.p(0, 5), ScrollViewDir.vertical,
...
function GuildwarBattleListItem:configUI()
    ...
    --self.srvname_label_1:setVisible(false)
    --self.srvname_label_2:setVisible(false)
end
...
function GuildwarBattleListItem:setData(data)
   ...
   local srv_name_1 = string.format(TI18N("[S%s]"), tostring(srv_index_1))
    ...
    local srv_name_2 = string.format(TI18N("[S%s]"), tostring(srv_index_2))
