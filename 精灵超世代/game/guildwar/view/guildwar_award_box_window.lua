...
function GuildwarAwardBoxWindow:__init()
    ...
    self.res_list = {{
        path = PathTool.getPlistImgForDownLoad("guildwar", "guildwar"),
        type = ResourcesType.plist
    }, {
        path = PathTool.getPlistImgForDownLoad("bigbg/guildwar", "guildwar_3"),
        type = ResourcesType.single
    }, {
        path = PathTool.getPlistImgForDownLoad("bigbg/guildwar", "guildwar_4"),
        type = ResourcesType.single
    }}
end
...
function GuildwarAwardBoxWindow:open_callback()
    ...
    self.time_tips = cc.Label:createWithTTF("", DEFAULT_FONT, 18)
    self.time_tips:setTextColor(Config.ColorData.data_new_color4[12])
    self.time_tips:setAnchorPoint(0.5, 0.5)
    self.time_tips:setPosition(cc.p(360,95))
    self.time_tips:setMaxLineWidth(300)

    main_container:addChild(self.time_tips)

    -- self.tips_txt = main_container:getChildByName("tips_txt")
    self.tips_txt = cc.Label:createWithTTF("", DEFAULT_FONT, 18)
    self.tips_txt:setTextColor(Config.ColorData.data_new_color4[12])
    self.tips_txt:setAnchorPoint(0.5, 0.5)
    self.tips_txt:setPosition(cc.p(360,45))
    self.tips_txt:setMaxLineWidth(500)
	main_container:addChild(self.tips_txt)

    self.tips_txt:setString(TI18N(
        "宝箱在公会战结束后产生，成员每人可开启1次，试试你的手气吧！"))
    ...
    self.box_scrollview = CommonScrollViewLayout.new(box_list, cc.p(0, 0), ScrollViewDir.vertical,
        ...
        if not self.box_award_timer then
            self.box_award_timer = GlobalTimeTicket:getInstance():add(function()
                if self.data then
                    ...
                    self.time_tips:setString(string.format(TI18N("请在%s内领取宝箱"),
                        TimeTool.GetTimeFormat(left_time)))
                else
                    GlobalTimeTicket:getInstance():remove(self.box_award_timer)
                    self.box_award_timer = nil
                end
    ...
    if self.data.rid == 0 then
        ...
        if not self.load_image_flag then
            ...
            if self.data.status == GuildwarConst.box_type.gold then
                self.image_box:loadTexture(PathTool.getPlistImgForDownLoad("bigbg/guildwar", "guildwar_3"),
                    LOADTEXT_TYPE)
            elseif self.data.status == GuildwarConst.box_type.copper then
                self.image_box:loadTexture(PathTool.getPlistImgForDownLoad("bigbg/guildwar", "guildwar_4"),
                    LOADTEXT_TYPE)
            end
        end
    else
