...
function GuildwarBattleLogWindow:open_callback(  )
    ...
    for i=1,2 do
        ...
        if tab_btn then
            ...
            title:enableShadow(Config.ColorData.data_new_color4[8],cc.size(0, -2),2)
...
function GuildwarBattleLogWindow:changeSelectedTab( index )
	...
	if self.tab_object then
		self.tab_object.tab_btn:loadTextures(PathTool.getResFrame("common","common_1018"), "", "", LOADTEXT_TYPE_PLIST)
		self.tab_object.label:setTextColor(cc.c4b(245, 224, 185, 255)) 
		self.tab_object.label:enableShadow(Config.ColorData.data_new_color4[8],cc.size(0, -2),2)

		self.tab_object = nil
	end
	self.tab_object = self.tab_list[index]
	if self.tab_object then
		self.tab_object.tab_btn:loadTextures(PathTool.getResFrame("common","common_1017"), "", "", LOADTEXT_TYPE_PLIST)
		self.tab_object.label:setTextColor(cc.c4b(61, 80, 120, 255)) 
	end
