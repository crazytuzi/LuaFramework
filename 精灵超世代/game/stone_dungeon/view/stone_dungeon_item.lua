...
function StoneDungeonItem:configUI()
	...
	self:setContentSize(cc.size(676,112))
    ...
    self.textClear:setString(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,#2920eb3>扫荡</div>"))
    ...
    self.textChange:setString(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,#2920eb3>挑战</div>"))
...
function StoneDungeonItem:setChangeData(data)
    ...
    --self.diff_text:enableOutline(cc.c4b(0, 0, 0, 120), 2)
    self.textNumber:setString(string_format(TI18N("战力%d开启"), changeBtValueForPower(data.power) or 0))
    ...
    if clearance and clearance.status == 1 then --已经通关的
        if self.role_vo.max_power >= data.power then
            ...
            if self.free_count == 0 then
                self.textClear:setString(string_format(TI18N("<img src=%s visible=true scale=0.25 /><div fontcolor=#ffffff shadow=0,-2,2,#854000>%s 扫荡</div>"), PathTool.getItemRes(15), self.expend))
            else
                self.textClear:setString(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,#854000>扫荡</div>"))
            end
        else
            self.btnClear:setVisible(false)
        ...
        if self.role_vo.max_power >= data.power then
            ...
            if self.role_vo.lev < data.lev_limit then
                ...
                if self.free_count == 0 then
                    self.textChange:setString(string_format(TI18N("<img src=%s visible=true scale=0.25 /><div fontcolor=#ffffff shadow=0,-2,2,#920eb3>%s 挑战</div>"), PathTool.getItemRes(15), self.expend))
                else
                    self.textChange:setString(TI18N("<div fontcolor=#ffffff shadow=0,-2,2,#920eb3>挑战</div>"))
                end
            end
...
function StoneDungeonItem:firstAward(is_first)
	...
	if self.dungeonData.first_items[1] then
        ...
        if self.item_list[1] then
            ...
            self.item_list[1]:setPosition(cc.p(4,40))
...
function StoneDungeonItem:showAward(is_first)
	...
	for i,v in ipairs(self.dungeonData.show_items) do
		...
		if self.item_list[1+i] then
            if is_first then
		    	self.item_list[1+i]:setPosition(cc.p(4*(i+1) + 90*i,40))
		    else
		    	self.item_list[1+i]:setPosition(cc.p(4*i + 90*(i-1),40))
		    end
