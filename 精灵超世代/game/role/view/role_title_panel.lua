...
function TitleItem:configUI( ... )
    ...
    self.background:setContentSize(cc.size(self.width-14, self.height-14))
    self.background:setScale9Enabled(true)
    self.background:setCapInsets(cc.rect(130, 37, 1, 1))
    self.background:setContentSize(self.size)

    res = PathTool.getResFrame("common","common_2041")
    self.selected = createScale9Sprite(res, self.width/2,self.height/2, LOADTEXT_TYPE_PLIST, self.back)
    self.selected:setContentSize(cc.size(self.width, self.height))
    ...
    self.face_icon:setScale(0.7)
    self.attr_label = createRichLabel(22, 58, cc.p(0,0.5), cc.p(self.width/2-50,90),2,0,320)   
    self.attr_label:setString(TI18N("无属性加成"))
    self:addChild(self.attr_label)

    self.get_label = createRichLabel(16, 186, cc.p(0,0.5), cc.p(self.width/2-50,45),0,0,320) 
    ...
    self.use_icon = createImage(self, res, 35, 105, cc.p(0.5, 0.5), false, 1, false)
    self.use_icon:setVisible(false)
    res = PathTool.getResFrame("common", "txt_cn_common_30017")
...
function TitleItem:setSelected(bool)
    if bool then
        ...
        --loadScale9SpriteTexture(self.background, PathTool.getResFrame("common","common_2040"), LOADTEXT_TYPE_PLIST)
        self.selected:setVisible(false)
