...
function RolePersonalSpaceTabMessageBoardPanel:layoutUI()
    ...
    self.combobox_btn_label:setFontSize(16)
    ...
    self.input_edit = createEditBox(self.main_container, PathTool.getResFrame("common", "common_1021"), cc.size(300, 48), Config.ColorData.data_color3[81], 20, nil, 20, self.default_content_msg, nil, self.limit_sad_count, LOADTEXT_TYPE_PLIST)
    self.input_edit:setAnchorPoint(cc.p(0, 1))
        -- self.input_edit:setText(desc)
        -- self.input_edit:getText()
    self.input_edit:setPosition(62, 54)
...
function RolePersonalSpaceTabMessageBoardPanel:initScrollview()
    ...
    self.item_width     = 570             -- 单元的宽度
    self.item_height    = 190              -- 单元的高度
...
function RolePersonalSpaceTabMessageBoardPanel:registerEvents()
    ...
    registerButtonEventListener(self.face_btn, function(param, sender) self:onFaceBtn(sender)  end ,true, 1,nil,0.8)
...
function RolePersonalSpaceTabMessageBoardPanel:createNewComboxItem(height, callback)
    ...
    cell.label = createLabel(16, Config.ColorData.data_new_color4[6], nil, 12, height * 0.5 , "", cell.layout, nil, cc.p(0,0.5))
    local mark_res = PathTool.getResFrame("common", "common_1043")
    cell.mark_img = createSprite(mark_res, width - 16, height * 0.5 + 2, cell.layout, cc.p(1,0.5), LOADTEXT_TYPE_PLIST)
...
function RoleMessageBoardItem:configUI(width, height)
    ...
    self.desc = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0,1), cc.p(36, 80), 6, nil, 490)
    self.main_container:addChild(self.desc)

    self.head = PlayerHead.new(PlayerHead.type.circle)
    self.head:setAnchorPoint(cc.p(0, 0))
    self.head:setPosition(cc.p(16, 94))
    self.head:setScale(0.8)
...
function RoleMessageBoardItem:register_event( )
    registerButtonEventListener(self.chat_btn, function() self:onChatBtn()  end ,true, 1,nil,0.8)
    registerButtonEventListener(self.remove_btn, function() self:onRemoveBtn()  end ,true, 1,nil,0.8)
end
...
function RoleMessageBoardItem:setData(data, text_height, dic_role_info)
    ...
    self.box_bg_03:setContentSize(cc.size(self.box_bg_03_size.width, self.box_bg_03_size.height + text_height ))
    self.box_bg_04:setContentSize(cc.size(self.box_bg_04_size.width, self.box_bg_04_size.height + text_height ))
