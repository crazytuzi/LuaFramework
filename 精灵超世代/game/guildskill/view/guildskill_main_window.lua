...
function GuildskillMainWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getPlistImgForDownLoad("bigbg/guild", "guild_bg_3", true), type = ResourcesType.single},
    }
...
function GuildskillMainWindow:open_callback()
    ...
    self.background:loadTexture(PathTool.getPlistImgForDownLoad("bigbg/guild", "guild_bg_3", true), LOADTEXT_TYPE)
    ...
    self.title_name:setString(TI18N("联盟技能"))
    ...
    self.career_desc = createRichLabel(20, Config.ColorData.data_new_color4[1], cc.p(0.5, 0.5), cc.p(360, -42), nil, nil, 500)
    ...
    self.score_title:setString(TI18N("PVP战力：0"))
    ...
    self.reset_btn = createRichLabel(22,Config.ColorData.data_new_color4[12] , cc.p(0.5, 0.5), cc.p(520, 500))
    self.reset_btn:setString(string_format("<div #1db116 href=xxx>%s</div>", TI18N("重置本系PVP技能")))
    self.reset_btn:addTouchLinkListener(function(type, value, sender, pos)
        self:onResetBtn()
    end, { "click", "href" })
    self.bottom_panel:addChild(self.reset_btn)
    ...
    if config and config.val then
        ...
        for i,v in ipairs(config.val) do
            _x = 10 +((i-1) % 3) * (width + space_x)
            --if i == 3 or i == 6 then
            --    _x = _x - 16
            --end
            _y = max_height - (7 + height * 0.5 + (math.floor((i - 1) / 3)) * (height + space_y)) - 2
            self.attr_pos_x[i] = _x + 6
            self.attr_list[i] = createRichLabel(18,Config.ColorData.data_new_color4[6],cc.p(0,0.5),cc.p(self.attr_pos_x[i], _y))
            local img = createImage(self.attr_list[i], PathTool.getResFrame("common", "common_1145"), -10, 12, cc.p(0,0.5), true, -1, true)
            img:setContentSize(cc.size(width,height))
            img:setCapInsets(cc.rect(46, 1, 2, 2))
    ...
    local bg = createSprite(PathTool.getPlistImgForDownLoad("bigbg/guild", "guild_bg_3_top"), SCREEN_WIDTH*0.5, display.getTop(), self.main_container, cc.p(0.5,1), LOADTEXT_TYPE,-1)
...
function GuildskillMainWindow:onChangeBtn()
    ...
    if self.selected_tab and self.selected_tab.career then
        ...
        if #pvp_career_data.attr_formation == 0 then
            ...
            message(string_format(TI18N("本系技能达%s级时开启"), max_lv))
            return 
        end
...
function GuildskillMainWindow:updateChangeBtnImg()
    local res
    if self.is_pvp_show then
        res = PathTool.getTargetRes("guildskill/txt_guildskill","txt_cn_guildskill_906",false)
    else
        res = PathTool.getTargetRes("guildskill/txt_guildskill","txt_cn_guildskill_905",false)
    end
    if self.record_chang_btn_res ~= res then
        self.record_chang_btn_res = res
        self.change_btn:loadTexture(res, LOADTEXT_TYPE)
        -- loadSpriteTexture(self.change_btn,res,LOADTEXT_TYPE)
    end
end
...
function GuildskillMainWindow:changeSelectedTab(career, index)
    ...
    if self.selected_tab then
        self.selected_tab.label:setTextColor(Config.ColorData.data_new_color4[6])
        self.selected_tab.select_bg:setVisible(false)
        self.selected_tab = nil
    end
    self.selected_tab = self.tab_list[career]
    if self.selected_tab then
        self.selected_tab.label:setTextColor(Config.ColorData.data_new_color4[1])
...
function GuildskillMainWindow:skillItemRunAction(node, start_x, start_y, end_x, end_y)
    if node then
        ...
        local time = 0.2
        local act_move = cc.EaseBackOut:create(cc.MoveTo:create(time,cc.p(end_x, end_y)))
        local act_scale = cc.EaseBackOut:create(cc.ScaleTo:create(time,0.7))
        local fadeIn = cc.FadeIn:create(time)
        node:runAction(cc.Sequence:create(cc.DelayTime:create(0.29), cc.Spawn:create(act_move, act_scale, fadeIn)))
    end
    -- body
...
function GuildskillMainWindow:updateAttrItem(attr_item, data, attr_key, attr_str)
    ...
    local str_res = string.format("<img src='%s' scale=1.0 /><div fontcolor=#3d5078>  %s</div><div fontsize=18 fontcolor=#0e7709>%s</div>",PathTool.getResFrame("common", attr_res),attr_str,value_str)
...
function GuildskillMainWindow:updatePvPSkillList(career, is_chang_btn) 
    ...
    if self.pvp_skill_item == nil then
        self.pvp_skill_item = SkillItem.new(true,true,false,0.7, true)
    ...
    if is_chang_btn then
        self:skillItemRunAction(self.pvp_skill_item, 338, 0, 338, 114)
    end
    ...
    if pvp_skill_config then
        ...
        self.pvp_skill_item:showName(true,skill_config.name,nil,22,true,Config.ColorData.data_new_color4[1],PathTool.getResFrame("common","common_90003"),cc.size(168,31))
    end

    if self.pvp_tips_lable == nil then
        self.pvp_tips_lable = createRichLabel(20, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(315, 136), nil, nil, 500)
        local str = string_format(TI18N("(以上属性和技能仅在PVP战斗中生效)"))
    ...
    self.score_title:setString("PVP战力: "..power)
    self:updateSkillAttrNameAndVal()
    if total_lev > 0 then
        self.reset_btn:setVisible(true)
        self.reset_btn:setString(string_format("<div #1db116 href=xxx>%s</div>", TI18N("重置本系PVP技能")))
...
function GuildskillMainWindow:getPvpDescText(career, lev)
    if career == GuildskillConst.index.physics then
        return string_format(TI18N("物攻%s级<div fontcolor='#fff99e'>(%s/%s)</div>"),lev, lev, self.pvp_max_lv)
    elseif career == GuildskillConst.index.magic then
        return string_format(TI18N("特攻%s级<div fontcolor='#fff99e'>(%s/%s)</div>"),lev, lev, self.pvp_max_lv)
    elseif career == GuildskillConst.index.defence then
        return string_format(TI18N("防御%s级<div fontcolor='#fff99e'>(%s/%s)</div>"),lev, lev, self.pvp_max_lv)
    elseif career == GuildskillConst.index.assist then
        return string_format(TI18N("辅助%s级<div fontcolor='#fff99e'>(%s/%s)</div>"),lev, lev, self.pvp_max_lv)
    else
        return ""
    end
end
...
function GuildskillMainWindow:updateSkillList(career, is_upgrade, need_update)
    ...
    if object then
        ...
        if object.had_skill_up then
            ...
            self.reset_btn:setVisible(true)
            self.reset_btn:setString(string_format("<div #1db116 href=xxx>%s</div>", TI18N("重置本系常规技能")))
        else
            self.reset_btn:setVisible(false)
        end
        -- 下一块开启的描述显示
        ...
        if group_config ~= nil then
            ...
            self.career_desc:setString(string_format("%s<div fontcolor='#fff99e'>(%s/%s)</div>",group_config.group_name, group_config.group_seq, max_group))
        end
...
function GuildskillMainWindow:updateSingleAttr(attr_key, attr_value, attr_str)
    if self.attr_title and self.attr_value then
        ...
        if attr_name then
            ...
            -- 如果是百分比数值
            if PartnerCalculate.isShowPerByStr(attr_key) == true then
                self.attr_value:setString(attr_name .. "+"..(attr_value*0.1).."%")
            else
                self.attr_value:setString(attr_name .. "+"..attr_value)
            end
        end
    end
end
    ...
    if self.is_pvp_show then
        ...
        if max_lev and config.lev and config.lev >= max_lev  then
            ...
            if self.pvp_max_lev_label == nil then
                self.pvp_max_lev_label = createRichLabel(24, Config.ColorData.data_new_color4[6], cc.p(0.5, 0.5), cc.p(315, 44), nil, nil, 500)
            ...
            if cost_data then
                ...
                if item_config then
                    ...
                    if have_num >= num then
                        cost_txt:setTextColor(Config.ColorData.data_new_color4[15])
                    else
