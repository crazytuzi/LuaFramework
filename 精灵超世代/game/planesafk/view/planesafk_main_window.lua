...
function PlanesafkMainWindow:open_callback(  )
    ...
    self.map_container:setScale(display.getMaxScale())
    -- ui层
    ...
    self.reward_list = {}
    self.reward_list[1] = createRichLabel(24, Config.ColorData.data_new_color4[17], cc.p(0, 0.5), cc.p(10,70),nil,nil,600)
    self.reward_list[2] = createRichLabel(24, Config.ColorData.data_new_color4[17], cc.p(0, 0.5), cc.p(10,30),nil,nil,600)
...
function PlanesafkMainWindow:setData()
    ...
    --处理时间
    commonCountDownTime(self.time_val, time, {end_title=TI18N("%s后重置")})
...
function PlanesafkMainWindow:updateBoxInfo()
    ...
    if is_show_box_staus ~= 0 and is_show_box_staus ~= 3 and self.box_effect == nil then
        ...
        self.box_effect = createEffectSpine("baoxiang_pu", cc.p( PlanesafkConst.Grid_Width/2, PlanesafkConst.Grid_Height/2), cc.p(0.5, 0.5), true, PlayerAction.action_1, function()
...
function PlanesafkMainWindow:showRoleInfo( )
    ...
    local effect_id = "gameplay/crossmine/q_xiaozhi"
    if figure_cfg then
        effect_id = figure_cfg.look_id
    end
    self.map_role = createEffectSpine( effect_id, cc.p(0, 0), cc.p(0.5, 0), true, "standby_loop" )
...
function PlanesafkMainWindow:updateItemlist(line)
    ...
    if self.scrollview_list == nil then
        ...
        local scrollview_size = cc.size(720, 1080 + ext_height - 100 )
...
function PlanesafkMainWindow:close_callback(  )
    ...
    --由于查看地方宝可梦需要额外加成数据 这里做特殊显示.关闭的时候需要清空
