...
function PlanesafkEvtShopPanel:open_callback()
    ...
    local bg_res = PathTool.getPlistImgForDownLoad("planes/board_img", "board_img_3")
...
function PlanesafkEvtShopItem:configUI(width, height)
    ...
    self.discount_num:setString("")
end

function PlanesafkEvtShopItem:register_event( )
    registerButtonEventListener(self.comfirm_btn, handler(self, self.showAlert) ,true, 2,nil,0.67)
...
function PlanesafkEvtShopItem:setData(data, index)
    ...
    if shop_config.discount > 0 then
        ...
        local zhe_res = PathTool.getResFrame("common", MallConst.Variety_Zhe_Res[tonumber(shop_config.discount)])
    ...
    if self.parent and self.parent.data and self.parent.data.is_select == 1  then
        if self.data.is_flag == 1 then
            ...
            if item_config then 
                ...
                local price_str = string.format("<img src='%s' scale=0.3 /><div > %s</div>", res, MoneyTool.GetMoneyString(price))
