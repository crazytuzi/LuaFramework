...
function PlanesafkController:registerProtocals()
    ...
    self:RegisterProtocal(28609, "handle28609") -- 回血事件宝可梦变换
    ...
    self:RegisterProtocal(28613, "handle28613") -- 宝可梦背包
    ...
    self:RegisterProtocal(28622, "handle28622") -- 宝可梦租借
    self:RegisterProtocal(28623, "handle28623") -- 查看宝可梦信息
...
function PlanesafkController:onHandlePlanesEvtById( evt_type, data)
    ...
    elseif evt_type == PlanesafkConst.Evt_Type.Guard then -- 守卫
        self:openPlanesafkMasterWindow(true, data)
    elseif evt_type == PlanesafkConst.Evt_Type.Recover then -- 宝可梦恢复(回复泉水)
        if self.model:getAllPlanesHeroData() ~= nil then --宝可梦信息回来了才响应事件
            local cfg = {btn_str=TI18N("浸泡"), res_id="board_img_7", title=TI18N("回复泉水")}
            local config = Config.PlanesData.data_const.cure_desc
            if config then
                cfg.desc_1 = config.desc
            else
                cfg.desc_1 = TI18N("浸泡泉水可使所有存活的宝可梦恢复<div fontcolor=289b14>30%</div>的生命值，泉水仅可使用一次。")
            end
            self:openPlanesafkBoardWindow(true, PlanesafkConst.Recover_Id, data, {board_cfg = cfg})
        end
    elseif evt_type == PlanesafkConst.Evt_Type.Revive then -- 宝可梦恢复(复活祭坛)
        if self.model:getAllPlanesHeroData() ~= nil then --宝可梦信息回来了才响应事件
            local cfg = {btn_str=TI18N("复活"), res_id="board_img_4", title=TI18N("复活十字架")}
            local config = Config.PlanesData.data_const.reborn_desc
            if config then
                cfg.desc_1 = config.desc
            else
                cfg.desc_1 = TI18N("可以随机复活一位已阵亡的宝可梦并回复其<div fontcolor=289b14>70%</div>的生命值，若当前无阵亡宝可梦则回复生命值最低的一位宝可梦<div fontcolor=289b14>100%</div>的生命。")
            end
            self:openPlanesafkBoardWindow(true, PlanesafkConst.Revive_Id, data, {board_cfg = cfg})
        end
    elseif evt_type == PlanesafkConst.Evt_Type.LeaseHero then -- 租借宝可梦
