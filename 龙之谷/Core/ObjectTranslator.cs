    ...
    public class ObjectTranslator
    {
        ...
        private void pushNewObject(IntPtr luaState, object o, int index, string metatable)
        {
            ...
            if (metatable == "luaNet_metatable")
            {
                ...
                {
                    string meta = t.AssemblyQualifiedName;
                    Debugger.Log("<color=green>Create not wrap ulua type:" + meta+"</color>");
                    LuaDLL.lua_settop(luaState, -2);
        ...
        internal object[] popValues(IntPtr luaState, int oldTop)
        {
            ...
            else
            {
                ...
                if (count == 0)
                    return null;
                object[] returnValues = new object[count];
                for (int i = oldTop + 1, j = 0; i <= newTop; i++, ++j)
                {
                    returnValues[j] = getObject(luaState, i);
                }
        ...
        internal object[] popValues(IntPtr luaState, int oldTop, Type[] popTypes)
        {
            ...
            else
            {
                ...
                for (int i = oldTop + 1, j = 0; i <= newTop; i++, ++j)
                {
                    returnValues[j] = getAsType(luaState, i, popTypes[iTypes]);
                }
                LuaDLL.lua_settop(luaState, oldTop);
                return returnValues;
