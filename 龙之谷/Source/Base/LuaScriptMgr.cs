...
#if !LUA_ZIP
        string path = name;// Util.LuaPath(name);
        //if (File.Exists(path))
        //{
        //     str = File.ReadAllBytes(path);
        //}
        if (path.StartsWith("Hotfix") || name.EndsWith("_pb") || name.EndsWith("_pb.lua") || name.StartsWith("Lua"))
        {
            TextAsset data = XUtliPoolLib.XResourceLoaderMgr.singleton.ReadLuaBytes(path);
            if (data != null)
            {
                byte[] bytes = new byte[data.bytes.Length];
                Array.Copy(data.bytes, bytes, data.bytes.Length);
                str = bytes;
                XUtliPoolLib.XResourceLoaderMgr.singleton.ReleaseLuaBytes(path, data);
            }
        }
        else
        {
            TextAsset luaCode = Resources.Load<TextAsset>(Util.LuaResourcePath(name));
