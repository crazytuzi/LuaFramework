...
function PreloadManager:__init()
	...
	self.loadcfg = {
		...
		{"uis/views/miscpreload_prefab", "RoleTaskMode"},
		{"uis/views/miscpreload_prefab", "SceneLoadingView"}, 				--预加SceneLoadingView
		...
		{"uis/views/miscpreload_prefab", "SystemContentLeft"},				-- 预加载聊天框
		{"uis/views/miscpreload_prefab", "SystemContentLeft"},				-- 预加载聊天框
		{"uis/views/miscpreload_prefab", "ItemCell"},						-- 预加载格子
		{"uis/views/miscpreload_prefab", "PieceItem"},						-- 预加载格子
		{"uis/views/miscpreload_prefab", "MoneyBar"},						-- 预加载金钱bar
		{"uis/views/miscpreload_prefab", "ConsumablesBar"},					-- 预加载消耗品bar
		{"uis/views/miscpreload_prefab", "CampWarFollow"},					-- 预加载国家战事相关
...
function PreloadManager:Start()
	...
	if nil == self.timer_quest then
		self.timer_quest = GlobalTimerQuest:AddRunQuest(
			BindTool.Bind(self.OnTimer, self), 0)
	end
...
function PreloadManager:OnTimer()
