...
function SCWorldBossInfo:Decode()
	...
	self.fuli_killer_name = MsgAdapter.ReadStrN(32)
	self.fuli_killer_uid =  MsgAdapter.ReadInt()
	self.fuli_boss_status = MsgAdapter.ReadInt()
	self.cur_fuli_boss_id = MsgAdapter.ReadInt()
	self.boss_list = {}
...
-- 玩家请求进入世界BOSS地图
CSWorldBossOperaReq = CSWorldBossOperaReq or BaseClass(BaseProtocolStruct)
function CSWorldBossOperaReq:__init()
	self.msg_type = 10156
	self.opera_type = 0
	self.param_1 = 0
end

function CSWorldBossOperaReq:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
