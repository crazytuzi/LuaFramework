...
function SCGetPublicSaleItemListAck:Decode()
	...
	for i = 1, self.count do
		...
		sale_item.level = MsgAdapter.ReadShort()
		sale_item.prof = MsgAdapter.ReadShort()
		sale_item.color = MsgAdapter.ReadShort()
		sale_item.order = MsgAdapter.ReadShort()
...
function CSPublicSaleSearch:__init()
	...
	self.page_item_count = 3											-- 一页显示几个物品
	self.req_page = 0													-- 请求第几页
	self.total_page = 0													-- 总页数 客户端没有总页数的时候 要填成0 如果已经有总页数 填上总页数
	self.sort_type = 0 													-- 单价排序或总价排序
	self.rank_type = 0 													-- 排序方式
	self.need_notice = 0												-- 是否需要提示
...
function CSPublicSaleSearch:Encode()
	...
	MsgAdapter.WriteShort(self.sort_type)
	MsgAdapter.WriteShort(self.rank_type)
	MsgAdapter.WriteInt(self.need_notice)
