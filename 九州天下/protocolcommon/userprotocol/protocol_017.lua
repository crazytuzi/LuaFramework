...
function SCEquipmentGridInfo:Decode()
	-- print(ToColorStr("返回身上的装备的强化与神铸等级", TEXT_COLOR.GREEN))
	local count = MsgAdapter.ReadInt()
	-- print(ToColorStr(count, TEXT_COLOR.GREEN))
	self.equip_list = {}
	for i=0,count - 1 do
		local index = MsgAdapter.ReadInt()
		local data = {}
		...
		data.strengthen_level = ForgeData.Instance:GetGradeStrengthLevel(index, data.grid_strengthen_level) or 0
		self.equip_list[index] = data
		-- print(ToColorStr("ix"..index.." st"..data.grid_strengthen_level.." sz"..data.shenzhu_level, TEXT_COLOR.GREEN))
		--if data.strengthen_level ~= nil then
			-- print(ToColorStr("real_st "..data.strengthen_level, TEXT_COLOR.GREEN))
		--end
	end
end
...
function CSEquipCompound:__init()
	...
	self.equip_item_id = 0
	self.equi_index = 0
	self.index = 0
	self.compound = 0
	self.index_list_count = 0
	self.knapsack_equipment_index_list = {}
end

function CSEquipCompound:Encode()
	MsgAdapter.WriteBegin(self.msg_type)

	MsgAdapter.WriteInt(self.equip_item_id)
	MsgAdapter.WriteChar(self.equi_index)
	MsgAdapter.WriteChar(self.index)
	MsgAdapter.WriteChar(self.compound)
	MsgAdapter.WriteChar(self.index_list_count)
	for i = 1, 5 do
		MsgAdapter.WriteInt(self.knapsack_equipment_index_list[i])
	end
end

--装备合成回复
SCEquipCompoundRet = SCEquipCompoundRet or BaseClass(BaseProtocolStruct)
function SCEquipCompoundRet:__init()
	self.msg_type = 1789
	self.is_succ = 0
end

function SCEquipCompoundRet:Decode()
...
function SCMillionaireInfo:Decode()
	...
	self.gather_total_score = MsgAdapter.ReadShort()
end

-- 大富豪排行信息
SCMillionaireRankInfo = SCMillionaireRankInfo or BaseClass(BaseProtocolStruct)
function SCMillionaireRankInfo:__init()
	self.msg_type = 1723
end

function SCMillionaireRankInfo:Decode()
	self.rank_item_list = {}
	for i = 1, GameEnum.MILLIONARE_RANK_ITEM_MAX_COUNT do
		local vo = {}
		vo.role_uid = MsgAdapter.ReadInt()
		vo.rank_score = MsgAdapter.ReadInt()
		vo.player_name = MsgAdapter.ReadStrN(32)
		self.rank_item_list[i] = vo
	end
end

-- 申请大富豪排行信息
CSMillionaireRankInfo = CSMillionaireRankInfo or BaseClass(BaseProtocolStruct)
function CSMillionaireRankInfo:__init()
	self.msg_type = 1724
end

function CSMillionaireRankInfo:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
end