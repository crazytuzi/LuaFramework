...
function SCMountInfo:Decode()
	...
	-- self.active_special_image_flag_low = MsgAdapter.ReadUInt()
	-- self.active_special_image_flag_high = MsgAdapter.ReadUInt()
	self.active_special_image_flag_ex = {}
	for i = 1, 4 do
		...
		local len = i - 1
		for j = 1, 64 do
			self.active_special_image_flag_ex[j + (64 * len) - 1] = reward_flag[j]
		end
	end
	...
	for i = 0, GameEnum.MAX_MOUNT_SPECIAL_IMAGE_ID_NEW - 1 do
		self.special_img_grade_list[i] = MsgAdapter.ReadUChar()
	end

	self.used_huanhua_seq = MsgAdapter.ReadChar()
	MsgAdapter.ReadChar()
	MsgAdapter.ReadChar()
	MsgAdapter.ReadChar()

	self.huanhua_end_timestamp = {}
	for i = 0, GameEnum.HUANHUA_MAX_COUNT - 1 do
		self.huanhua_end_timestamp[i] = MsgAdapter.ReadUInt()
	end

	self.image_id_unactive_timestamp = {}
	for i = 0, GameEnum.MAX_IMAGE_ID - 1 do
		self.image_id_unactive_timestamp[i] = MsgAdapter.ReadUInt()
	end
...
function SCMonthCardInfo:Decode()
	self.active_timestamp = MsgAdapter.ReadUInt()   				-- 月卡激活时间戳
	self.is_active = MsgAdapter.ReadInt()							-- 是否激活
	self.reward_gold = MsgAdapter.ReadInt()							-- 能够拿取的元宝数
	self.buy_times = MsgAdapter.ReadInt()							-- 已经购买月卡的次数
	self.monthcard_first_reward_fetch_flag = MsgAdapter.ReadInt()	-- 月卡第一购买奖励拿取标记
end
...
function SCWingInfo:Decode()
	...
	for i = 1, 4 do
		...
		local reward_flag = bit:hl2b(active_flag_high,active_flag_low)
		for j = 1, 64 do
			self.active_special_image_flag_ex[j + (64 * (i - 1)) - 1] = reward_flag[j]
		end
	end
	self.clear_upgrade_time = MsgAdapter.ReadUInt()
	self.temp_img_id = MsgAdapter.ReadShort()
	self.temp_img_id_has_select = MsgAdapter.ReadShort()
	self.temporary_imageid_invalid_time = MsgAdapter.ReadUInt()
	...
	self.special_img_grade_list = {}
	for i = 0, GameEnum.MAX_WING_SPECIAL_IMAGE_ID - 1 do
		self.special_img_grade_list[i] = MsgAdapter.ReadUChar()
	end
...
function SCRedPaperDetailInfo:Decode()
	...
	self.currency_type = MsgAdapter.ReadInt()
	self.total_gold_num = MsgAdapter.ReadInt()
	self.fetch_gold_num = MsgAdapter.ReadInt()
	self.can_fetch_times = MsgAdapter.ReadInt()
	self.timeount_timestamp = MsgAdapter.ReadUInt()
	self.creater_uid = MsgAdapter.ReadInt()
	self.creater_name = MsgAdapter.ReadStrN(32)
...
function CSRedPaperCreateReq:Encode()
	...
	MsgAdapter.WriteInt(self.currency_type)
...
function SCShizhuangInfo:Decode()
	...
	for i = 0, SHIZHUANG_TYPE.MAX - 1 do
		...
		vo.active_flag = MsgAdapter.ReadInt(32)
		self.item_list[i] = vo
	end
	...
	self.valid_timestamp_list = {}
	for i = 0, SHIZHUANG_TYPE.MAX - 1 do
		local vo = {}
		vo.valid_timestamp_list = {}
		for j = 0, SHIZHUANG.SHIZHUANG_MAX_INDEX do
			vo.valid_timestamp_list[j] = MsgAdapter.ReadUInt()
		end
		self.valid_timestamp_list[i] = vo
	end
end
...
function SCQingyuanCoupleHaloInfo:Decode()
	...
	for i = 1, couple_halo_max_type_count do
...
function SCQingyuanLoveContractInfo:Decode()
	...
	self.lover_love_contract_timestamp = MsgAdapter.ReadUInt()				-- 是否给对方购买 0表示没有帮伴侣购买

	local self_notice_list_count = MsgAdapter.ReadInt()
	...
	for i = 0, GameEnum.MAX_NOTICE_COUNT - 1 do
		...
		if i < self_notice_list_count then
			self.self_notice_list[i] = {time = time, contract_notice = contract_notice}
		end
	end

	local lover_notice_list_count = MsgAdapter.ReadInt()
	self.lover_notice_list = {}
	for i = 0, GameEnum.MAX_NOTICE_COUNT - 1 do
		local time = MsgAdapter.ReadUInt()
		local contract_notice = MsgAdapter.ReadStrN(64)
		if i < lover_notice_list_count then
			self.lover_notice_list[i] = {time = time, contract_notice = contract_notice}
		end
	end
end
