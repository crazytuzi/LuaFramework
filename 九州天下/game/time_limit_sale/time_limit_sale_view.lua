...
function TimeLimitSaleView:__init()
	self.ui_config = {"uis/views/timelimitsale","TimeLimitSaleView"}
...
function TimeLimitSaleView:ClickTab(click_index)
	if click_index > self.open_phase then
		SysMsgCtrl.Instance:ErrorRemind(Language.TimeLimitSale.WaitNext)
	elseif click_index < self.open_phase then
		SysMsgCtrl.Instance:ErrorRemind(Language.TimeLimitSale.TimeOut)
	end
end
...
function TimeLimitSaleItem:ClickBuy()
	...
	left_act_times = math.ceil(left_act_times)
	if left_act_times <= 0 then
		TipsCtrl.Instance:ShowSystemMsg(Language.TimeLimitSale.WaitBtnText)
		return
	end
...
function TimeLimitSaleItem:OnFlush()
	...
	local role_des = string.format(Language.Common.CountDes, ToColorStr(can_buy_role_times, role_color), max_role_buy_times)
	...
	local server_des = string.format(Language.Common.CountDes, ToColorStr(can_buy_server_times, server_color), max_server_buy_times)
