...
function FindView:AllFind(find_type)
	...
	for k,v in pairs(self.scroller_data) do
		if v.total_type == 0 then
			...
			local cur_find_type = v.find_type or v.vo.find_type
			WelfareCtrl.Instance:SendGetActivityFindReward(cur_find_type, acitvity_find_type)
		end
	end
	self.select_type = -1
end
...
function FindView:AllFindClick(find_type)
	...
	if find_type == 1 then
		...
		des = string.format(Language.Welfare.FindBindGold, cost_count)
	else
...
function FindScrollerCell:__init()
	...
	self.show_item_cell = {}
	for i = 1, 4 do
		self.show_item_cell[i] = self:FindVariable("ItemCell_" .. i)
	end
...
function FindScrollerCell:OnFlush()
	...
	for k,v in pairs(CurrencyName) do
		if self.data[v] ~= nil and self.data[v] ~= 0 then
			...
			data.is_bind = true
			self.show_item_cell[self.count]:SetValue(true)
	...
	if self.count <= #self.item_cell_list then
		for i = self.count, #self.item_cell_list do
			self.show_item_cell[i]:SetValue(false)
		end
	end
	...
	local bundle, asset
	if self.data.total_type == 0 then
		bundle, asset = ResPath.GetImages("icon_gold_1001")
	else
		bundle, asset = ResPath.GetImages("icon_gold_1001")
	end
