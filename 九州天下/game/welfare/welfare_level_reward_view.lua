...
function LevelRewardView:__init(instance)
	self.cell_list = {}
end

function LevelRewardView:LoadCallBack()
...
function LevelRewardView:CellRefreshDel(cell, data_index)
	data_index = data_index + 1
	local reward_cell = self.cell_list[cell]
	if not reward_cell then
		reward_cell = LevelRewardItemCell.New(cell.gameObject)
		self.cell_list[cell] = reward_cell
	end
	reward_cell:SetData(self.scroller_data[data_index])
end

function LevelRewardView:SetCurTyoe(cur_type)
	self.cur_type = cur_type
end

function LevelRewardView:OnFlush()
	GlobalTimerQuest:AddDelayTimer(function()
		self.scroller_data = WelfareData.Instance:GetLevelRewardList()
		if self.scroller.scroller.isActiveAndEnabled then
			-- local function loadcomplete()
			-- 	self.scroller.scroller:JumpToIndex(0)
			-- end
