...
function MarryBaoBaoView:__init()
	self:SetMaskBg()
	self.ui_config = {"uis/views/baby", "BabyView"}
...
function MarryBaoBaoView:LoadCallBack()
	...
	self.image_view = BaoBaoComView.New(self:FindObj("ComContent"))
...
function MarryBaoBaoView:AsyncLoadView(index)
	...
	if index == TabIndex.marriage_baobao_att and self.baobao_arr_content ~= nil and self.attr_view == nil then
		self.baobao_arr_content.uiprefab_loader:Wait(function(obj)
			obj = U3DObject(obj)
	...
	elseif index == TabIndex.marriage_baobao_bless and self.baobao_bless_content ~= nil and self.bless_view == nil then
		...
		-- 		self.bless_view:Flush()
		-- 	end)
			self.baobao_bless_content.uiprefab_loader:Wait(function(obj)
				obj = U3DObject(obj)
				self.bless_view = BaoBaoBlessView.New(obj)
				self.bless_view:Flush()
			end)

	elseif index == TabIndex.marriage_baobao_guard and self.baobao_guard_content ~= nil and self.guard_view == nil then
		...
		-- 		self.guard_view:Flush()
		-- 	end)
			self.baobao_guard_content.uiprefab_loader:Wait(function(obj)
				obj = U3DObject(obj)
				self.guard_view = BaoBaoGuardView.New(obj)
				self.guard_view:Flush()
			end)

	elseif index == TabIndex.marriage_baobao_zizhi and self.baobao_zizhi_content ~= nil and self.aptitude_view == nil then
		...
		-- 		self.aptitude_view:Flush()
		-- 	end)
			self.baobao_zizhi_content.uiprefab_loader:Wait(function(obj)
				obj = U3DObject(obj)
				self.aptitude_view = BaoBaoAptitudeView.New(obj)
				self.aptitude_view:Flush()
			end)
...
function MarryBaoBaoView:OnFlush(param_t)
	for k,v in pairs(param_t) do
		...
		elseif "xilian_info" == k then
			if self.attr_view then
				self.attr_view:Flush("xilian_info")
			end
		end
	end
