...
function WeddingInviteView:__init()
	self.ui_config = {"uis/views/marriageview","WeddingInviteView"}
	self.scroller_data = {}
	self.cell_list = {}
	self.select_page = 0
end
...
function WeddingInviteView:OpenCallBack()
	...
	self.friend_callback = GlobalEventSystem:Bind(OtherEventType.FRIEND_INFO_CHANGE, BindTool.Bind(self.Flush, self, "friend"))
...
function WedingInviteScrollerCell:OnFlush()
	...
	AvatarManager.Instance:SetAvatarKey(id, self.data.avatar_key_big, self.data.avatar_key_small)
