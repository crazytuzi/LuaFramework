...
function QingYuanFuBenView:__init()
	self.ui_config = {"uis/views/marriageview","QingYuanFuBenView"}
	self.time_count = 0
...
function QingYuanFuBenView:LoadCallBack()
	...
	self.reward_cell = {}
	local gift_id = MarriageData.Instance:GetQingYuanFBPassReward()
	local reward_list = ItemData.Instance:GetGiftItemList(gift_id)
	for i = 1, 3 do
		self.reward_cell[i] = ItemCell.New()
		self.reward_cell[i]:SetInstanceParent(self:FindObj("RewardCell" .. i).gameObject)
		if reward_list[i] then
			self.reward_cell[i]:SetActive(true)
			self.reward_cell[i]:SetData(reward_list[i])
		else
			self.reward_cell[i]:SetActive(false)
		end
	end
...
function QingYuanFuBenView:Flush()
	...
	if self.refresh_tmp_time > 0 then
		self.left_time:SetValue(ToColorStr(math.floor(self.refresh_tmp_time), TEXT_COLOR.GREEN)..Language.Marriage.Flush_Monster)
	end
	...
	self.left_wave:SetValue(ToColorStr(self.data.curr_wave, COLOR.WHITE).."/"..self.data.max_wave_count)
	self:SetTime()
...
function QingYuanFuBenView:SetTime()
	...
	local text = Language.Marriage.Left_Time..ToColorStr(min..":"..sec, "#00ff00")
