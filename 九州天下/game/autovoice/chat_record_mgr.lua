...
function ChatRecordMgr:DownloadCallBack(path, paly_voice_call_back, end_call_back)
	AudioPlayer.Stop()		--停止播放语音
	AudioService.Instance:SetMasterVolume(0.0)
	if paly_voice_call_back then
		paly_voice_call_back(true)
	end
	AudioPlayer.Play(path, BindTool.Bind(self.StopCallBack, self, end_call_back))
end

--播放语音
function ChatRecordMgr:PlayVoice(file_path, paly_voice_call_back, end_call_back)
	if not file_path then
		return
	end
	local path = ChatRecordMgr.GetCacheSoundPath(file_path)
	local play_callback = BindTool.Bind(self.StopCallBack, self, end_call_back)
	if self:GetIsDownLoad(file_path) then
		...
		AudioPlayer.Play(path, play_callback)
	else
		self:DownloadVoice(file_path, BindTool.Bind(self.DownloadCallBack, self, path, paly_voice_call_back, end_call_back))
	end
end