...
function QuickDelView:__init()
	self.ui_config = {"uis/views/scoietyview", "QuickClearView"}
	self:SetMaskBg(true)
end
...
function QuickDelView:ClickClear()
	...
	if  not self.is_lev and not self.is_offline then
	...
	for k,v in ipairs(friend_info) do
		...
		if (self.is_lev and v.level < tonumber(lev)) or (self.is_offline and leave_day > tonumber(day)) then
	...
	SysMsgCtrl.Instance:ErrorRemind(Language.Society.ClearSucceed)
end
