...
function WriteMailView:__init()
	...
	-- for i = 1, 3 do
	-- 	local send_item = ItemCell.New(self:FindObj("Item" .. i))
...
function WriteMailView:BagRefreshCell(cell, data_index, cell_index)
	...
	for i = 1, BAG_ROW do
		...
		-- 获取数据信息
		local data = nil
		data = ItemData.Instance:GetGridData(index, GameEnum.TOGGLE_INFO.ALL_TOGGLE)
		group:SetInteractable(i, data ~= nil)
		data = data or {}
		data.locked = index >= ItemData.Instance.max_knapsack_valid_num
		data.index = index
...
function WriteMailView:OnInputValueChange()
	local length = StringUtil.GetCharacterCount(self.content.input_field.text)
	local max = self.content.input_field.characterLimit
	if length >= max then
