...
function TipsLockVipView:__init()
	self.ui_config = {"uis/views/tips/lockviptips", "LockVipTips"}
...
function TipsLockVipView:LoadCallBack()
	...
	self.des_image1 = self:FindVariable("DesImage1")
...
function TipsLockVipView:OnFlush()
	...
	local bundle, asset = ResPath.GetVipLoadImage(self.reason)
	if nil == str or str == "" then
		...
		self.des_image1:SetAsset("uis/views/tips/lockviptips/images_atlas", "Des_default")
	else
		self.des_image1:SetAsset(bundle, asset)
