...
function TipsShortCutEquipView:__init()
	self.ui_config = {"uis/views/tips/equiptips", "ShortcutEquip"}
...
function TipsShortCutEquipView:OnFlush()
	...
	--local power = EquipData.Instance:NewGetEquipCapacity(new_equip_data)
	local power = EquipData.Instance:GetCompareCap(new_equip_data, false, true, nil, true)
	if power <= 0 then
		self:Close()
		return
	end
	...
	self.timer_quest = GlobalTimerQuest:AddRunQuest(function()
		self.delay_time = self.delay_time - UnityEngine.Time.deltaTime
		if self.delay_time > 0 then
			local time = math.floor(self.delay_time)
			if time >= 0 then
