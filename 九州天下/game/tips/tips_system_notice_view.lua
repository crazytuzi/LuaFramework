...
function TipSystemNoticeView:__init()
	self.ui_config = {"uis/views/tips/noticeview", "SystemNoticeView"}
...
function TipSystemNoticeView:OnFlush()
	if self.current_index <= self.total_count then
		...
		GlobalTimerQuest:AddDelayTimer(BindTool.Bind(self.Calculate, self), 0.1)
	end
end

-- 计算滚动的时间的位置
function TipSystemNoticeView:Calculate()
	if nil == self.text_trans then return end
