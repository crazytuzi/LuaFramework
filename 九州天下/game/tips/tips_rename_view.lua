...
function TipsRenameView:__init()
	self.ui_config = {"uis/views/tips/renametip", "RenamePopupTip"}
		...
		if k == "all" then
			if self.show_need_pro ~= nil then
				...
				if v.item_id and v.item_id == 27582 then
					self.title:SetValue(Language.Common.ZhuiZhongLing)   -- 原来是中文，我只是把他改到Language
					...
					self.title:SetValue(Language.Common.ChongName)  	-- 原来是中文，我只是把他改到Language
				end
				if item_cfg then
					local bag_num = ItemData.Instance:GetItemNumInBagById(self.item_id)
					local str = item_cfg.name..":<color=#00931f> "..bag_num.."</color> / <color=#00931f>1</color>"
					if bag_num < 1 then
						str = item_cfg.name..":<color=#00931f> "..bag_num.."</color> / <color=#00931f>1</color>"
					end
...
function TipsRenameView:SetCharacterLimit(character_limit)
	self.character_limit = character_limit or 6
end
