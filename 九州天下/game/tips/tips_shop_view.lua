...
function TipShopView:OpenCallBack()
	...
	elseif self.consume_type == SHOP_BIND_TYPE.IS_LIMUIT then
		price = shop_item_cfg.vip_gold
	end
	if next(self.item_info) ~= nil then
		...
		self.level_color:SetValue(role_level < self.item_info.limit_level and "#ff0000" or "#00ff00")
	end
...
function TipShopView:FlushCoin()
	local count = 0
	--self.show_text:SetValue(self.consume_type ~= SHOP_BIND_TYPE.BIND)
	if self.consume_type == SHOP_BIND_TYPE.BIND then
		count = GameVoManager.Instance:GetMainRoleVo().bind_gold
	elseif self.consume_type == SHOP_BIND_TYPE.NO_BIND or self.consume_type == SHOP_BIND_TYPE.IS_LIMUIT then
		count = GameVoManager.Instance:GetMainRoleVo().gold
	end

	self.my_coin_text:SetValue(CommonDataManager.ConverMoney(count))
end
...
function TipShopView:OnPlusClick()
	...
	local limit_num = ShopData.Instance:GetLimitMaxNum(self.item_info.item_id)
	if can_buy_num > self.buy_num_value then
		...
		if self.consume_type == SHOP_BIND_TYPE.IS_LIMUIT then
			if self.buy_num_value > limit_num then
				self.buy_num_value = limit_num
			end
		else
...
function TipShopView:GetCanBuyNum()
	...
	if self.consume_type == 1 then
		money_can_buy = math.floor((main_vo.bind_gold + main_vo.gold) /ShopData.Instance:GetShopItemCfg(self.item_info.id).bind_gold)
	else
		money_can_buy = math.floor(main_vo.gold /ShopData.Instance:GetShopItemCfg(self.item_info.id).gold)
	end
...
function TipShopView:SetAllPrice()
	...
	elseif self.consume_type == 1 then
...
function TipShopView:OnBuyClick()
	...
	if self.buy_num_value > self:GetCanBuyNum() then
		if self.consume_type == 1 then
			TipsCtrl.Instance:ShowSystemMsg(Language.Common.NoEnoughGold)
		else
