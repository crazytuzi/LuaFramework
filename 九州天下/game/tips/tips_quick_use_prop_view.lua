...
function TipsQuickUsePropView:__init()
	self.ui_config = {"uis/views/tips/proptips", "PropQuickUseTip"}
	self:SetMaskBg(true)
...
function TipsQuickUsePropView:ClickUse(data)
	...
	--如果已激活时装就升级
	local types, index = FashionData.Instance:GetFashionTypeAndIndexById(data.item_id)
...
function TipsQuickUsePropView:OnClickUseAll()
	...
	for k, v in pairs(PackageData.Instance:GetQuickUseItem(self.data_list)) do
		--如果已激活时装就升级
		local types, index = FashionData.Instance:GetFashionTypeAndIndexById(v.item_id)
		if types and index then
			local level = FashionData.Instance:GetCurLevel(index, types)
			if level >= 1 then
				FashionCtrl.Instance:SendFashionUpgradeReq(types, index)
