...
function TipsCommonView:__init()
	self.ui_config = {"uis/views/tips/commontips", "CommonTips"}
	self.ok_func = nil
	...
	if self.ok_func ~= nil then
		if self.no_tip_toggle.toggle.isOn then
			if type(self.prefs_key) == "string" then
				UnityEngine.PlayerPrefs.SetInt(self.prefs_key, 1)
			end
		end
...
function TipsCommonView:SetData(data, cancle_data, is_show_no_tip, show_time, prefs_key, is_recycle, recycle_text, auto_text_des, hide_cancel, boss_id, no_auto_click_yes, no_btn_text, cal_time, auto_click_no)
...
function TipsCommonView:SetYexText(text)
	self.btn_text = text
end
