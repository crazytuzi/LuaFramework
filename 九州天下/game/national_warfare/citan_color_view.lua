-- 国家战事（刺探刷新颜色界面）
CiTanColorView = CiTanColorView or BaseClass(BaseView)

local Dis = 30	--主角和npc的距离

function CiTanColorView:__init()
	self.ui_config = {"uis/views/nationalwarfareview", "CiTanRefresh"}
...
function CiTanColorView:__delete()

end

function CiTanColorView:ReleaseCallBack()
...
function CiTanColorView:LoadCallBack()
	...
	self.btn_affirm_text:SetValue(Language.NationalWarfare.BtnTiJiaoQingBao)
end

function CiTanColorView:OnExplain()
	TipsCtrl.Instance:ShowHelpTipView(181)
end

function CiTanColorView:CloseCallBack()
	...
	if self.citan_list.get_qingbao_color ~= CAMP_TASK_BANZHUAN_COLOR.CAMP_TASK_BANZHUAN_COLOR_RED then
...
function CiTanColorView:OpenCallBack()
	for i = 1, 5 do
...
-- 关闭事件
function CiTanColorView:HandleClose()
	ViewManager.Instance:Close(ViewName.CiTanColorView)
end

-- 一键刷红颜色
function CiTanColorView:RefreshRed()
	local other_cfg = NationalWarfareData.Instance:GetCiTanOtherCfg()
	if other_cfg then
		local yes_func = function()
			self.is_refresh_red = true
			CampCtrl.Instance:SendCampTaskCommonOpera(CAMP_TASK_OPERA_TYPE.OPERA_TYPE_CITAN_REFRESH_COLOR, CAMP_TASK_TYPE.CAMP_TASK_TYPE_CITAN)
		end
...
-- 刷新颜色
function CiTanColorView:RefreshColorOpera()
	...
	if color == CAMP_TASK_BANZHUAN_COLOR.CAMP_TASK_BANZHUAN_COLOR_RED then
		...
		self.is_refresh_red = false
		CampCtrl.Instance:SendCampTaskCommonOpera(CAMP_TASK_OPERA_TYPE.OPERA_TYPE_CITAN_REFRESH_COLOR)
	end	
end

function CiTanColorView:ShareColor()
	if self.citan_list.has_share_color == 1 then
		SysMsgCtrl.Instance:ErrorRemind(Language.NationalWarfare.NoShareNum)
		return
	end
	CampCtrl.Instance:SendCampTaskCommonOpera(CAMP_TASK_OPERA_TYPE.OPERA_TYPE_CITAN_SHARE_COLOR)
end

function CiTanColorView:StartRefreshColor(is_refresh_red)
	if self.is_refresh_red or is_refresh_red then
		for i = 1, 5 do
			if self.animation_lists[i] then
		...
		self.has_share:SetValue(self.citan_list.has_share_color == 0)
		return
	end
	self.button_refresh.button.interactable = false
	...
	local next_time = self.citan_list.next_refresh_camp_info_timestmap - TimeCtrl.Instance:GetServerTime()
...
function CiTanColorView:RemoveRefreshCountDown()
...
-- 提交任务
function CiTanColorView:OnFinishTask()
...
-- 确认情报颜色
function CiTanColorView:OnAffirmTask()
	CampCtrl.Instance:SendCampTaskCommonOpera(CAMP_TASK_OPERA_TYPE.OPERA_TYPE_CITAN_CONFIRM_COLOR, CAMP_TASK_TYPE.CAMP_TASK_TYPE_CITAN)
...
function CiTanColorView:RemoveCountDown()
...
function CiTanColorView:Update()
...
function CiTanColorView:OnFlush(param_t)
	...
	local color_cfg = NationalWarfareData.Instance:GetRewardList(self.citan_list.get_qingbao_color, CAMP_TASK_TYPE.CAMP_TASK_TYPE_CITAN)
	self.citan_state:SetValue(Language.NationalWarfare.CiTanState)
	for k, v in pairs(param_t) do
		if k == "start" then
			if self.citan_list.cur_qingbao_color ~= CAMP_TASK_BANZHUAN_COLOR.CAMP_TASK_BANZHUAN_COLOR_INVALID 
				or self.citan_list.task_phase == CAMP_TASK_PHASE.CAMP_TASK_PHASE_COMPLETE then
				local color = self.citan_list.cur_qingbao_color ~= CAMP_TASK_BANZHUAN_COLOR.CAMP_TASK_BANZHUAN_COLOR_INVALID and 
							self.citan_list.cur_qingbao_color or self.citan_list.get_qingbao_color
				...
				if v.index == 1 then
					...
					if color == CAMP_TASK_BANZHUAN_COLOR.CAMP_TASK_BANZHUAN_COLOR_RED then
						...
						self.button_refresh.button.interactable = true
						self.btn_affirm_text:SetValue(Language.NationalWarfare.BtnTiJiao)
						self.show_share_btn:SetValue(false)
					end
				end
...
function CiTanColorView:OnMainRolePosChangeHandler(x, y)
