...
function LeiJiRewardView:LoadCallBack()
...
function LeiJiRewardView:OnFlush()
	self.reward_list = KaifuActivityData.Instance:GetLeiJiChargeRewardCfg()
	if self.list_view and self.reward_list then
...
function LeiJiChargeLevelCell:OnClickGet()
	...
	if is_active == 0 then
		SysMsgCtrl.Instance:ErrorRemind(Language.Activity.NoArriveLeiJiReward)
		return
	end
...
function LeiJiChargeLevelCell:SetCostData()
	self.text:SetValue(self.reward_data.charge_value or 0)
	self.text1:SetValue(self.reward_data.bind_gold_repayment or 0)
end
...
function LeiJiChargeLevelCell:OnFlush()
	...
	self.is_active:SetValue(is_fench ~= 1)

	self.can_get:SetValue(is_active == 1 and is_fench == 0)
	self.is_get:SetValue(is_active == 1 and is_fench == 1)
