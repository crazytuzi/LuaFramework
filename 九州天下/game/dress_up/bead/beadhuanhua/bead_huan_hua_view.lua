BeadHuanHuaView = BeadHuanHuaView or BaseClass(BaseView)

function BeadHuanHuaView:__init()
	self.ui_config = {"uis/views/advanceview","MountHuanHuaView"}
	self.play_audio = true
	self.item_id = 0
	self.index = 1
	self.grade = nil
	self.bead_special_image = nil
	self.res_id = nil
...
function BeadHuanHuaView:LoadCallBack()
	self.bead_display = self:FindObj("BeadDisplay")
	self.bead_display:SetActive(true)
	self.title_name = self:FindVariable("TitleName")
	self.title_name:SetValue(Language.HuanHua.Bead)
	...
	self.bead_name = self:FindVariable("ZuoQiName")
	...
	self.all_num, self.all_data = BeadData.Instance:GetShowSpecialInfo()
	...
	list_delegate.NumberOfCellsDel = BindTool.Bind(self.GetBeadNumberOfCells, self)
	list_delegate.CellRefreshDel = BindTool.Bind(self.RefreshBeadCell, self)
end

function BeadHuanHuaView:__delete()
	self.index = 1
	self.grade = nil
	self.item_id = nil
	self.res_id = nil
...
function BeadHuanHuaView:ReleaseCallBack()
...
function BeadHuanHuaView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	if item_id ~= nil and BeadData.Instance:CheckIsHuanHuaItem(item_id) then
		local now_num = BeadData.Instance:GetShowSpecialInfo()
		if self.all_num ~= now_num then
			self:Flush("beadhuanhua", {need_flush = true})
		end
	end
end

function BeadHuanHuaView:CloseCallBack()
	if self.item_data_event ~= nil then
		ItemData.Instance:UnNotifyDataChangeCallBack(self.item_data_event)
		self.item_data_event = nil
	end	
...
function BeadHuanHuaView:FlushData()
	--self.all_num, self.all_data = BeadData.Instance:GetShowSpecialInfo()
	self:Flush("beadhuanhua", {need_flush = true})
end

function BeadHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
	if attr_name == "level" and self:IsOpen() then
		self:FlushData()
	end
end

function BeadHuanHuaView:GetBeadNumberOfCells()
	--return BeadData.Instance:GetShowSpecialInfo()
	return self.all_num
end

function BeadHuanHuaView:RefreshBeadCell(cell, cell_index)
	--local bead_special_image = ConfigManager.Instance:GetAutoConfig("bead_auto").special_img
	--local _, bead_special_image = BeadData.Instance:GetShowSpecialInfo()
	local bead_cell = self.cell_list[cell]
	if bead_cell == nil then
		bead_cell = BeadHuanHuaCell.New(cell.gameObject)
		self.cell_list[cell] = bead_cell
	end
	bead_cell:SetToggleGroup(self.list_view.toggle_group)
	bead_cell:SetHighLight(self.index == cell_index + 1)
	local data = {}
	if self.all_data[cell_index+1] ~= nil then
		...
		data.is_show = BeadData.Instance:CanHuanhuaUpgrade()[image_id] ~= nil
	end

	bead_cell:SetData(data)
	bead_cell:ListenClick(BindTool.Bind(self.OnClickListCell, self, self.all_data[cell_index+1], cell_index+1, bead_cell))
end

function BeadHuanHuaView:OnClickChange(change_value)
	if change_value ~= nil then
		local mount_special_image = ConfigManager.Instance:GetAutoConfig("bead_auto").special_img
...
function BeadHuanHuaView:OnClickClose()
	self:Close()
	self.res_id = nil
	self.grade = nil
	self.used_imageid = nil
...
function BeadHuanHuaView:OpenCallBack()
	if self.item_data_event == nil then
		self.item_data_event = BindTool.Bind1(self.ItemDataChangeCallback, self)
		ItemData.Instance:NotifyDataChangeCallBack(self.item_data_event)
	end
	self:Flush("beadhuanhua")
	self.index = 1
end

--点击激活按钮
function BeadHuanHuaView:OnClickActivate()
...
--点击升级按钮
function BeadHuanHuaView:OnClickUpGrade()
	if self.all_data[self.index] == nil then
		return
	end

	local attr_cfg = BeadData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
	if attr_cfg ~= nil then
		if attr_cfg.grade >= BeadData.Instance:GetSpecialImageMaxUpLevelById(attr_cfg.special_img_id) then
			return
		end
	...
	BeadHuanHuaCtrl.Instance:BeadSpecialImaUpgrade(self.all_data[self.index].image_id)
end

--点击使用当前形象
function BeadHuanHuaView:OnClickUseIma()
	if self.all_data[self.index] == nil then
		return
	end
	BeadCtrl.Instance:SendUseBeadImage(self.all_data[self.index].image_id + GameEnum.MOUNT_SPECIAL_IMA_ID)
end

function BeadHuanHuaView:OnClickListCell(bead_special_data, index, bead_cell)
	self.bead_special_image = bead_special_data
	bead_cell:SetHighLight(true)
	if self.index == index then return end
	self.index = index or 1
	self.item_id = bead_special_data.item_id
	self:SetSpecialImageAttr(bead_special_data, bead_special_data.image_id)

	local mount_special_image = ConfigManager.Instance:GetAutoConfig("ugs_bead_auto").special_img
...
--获取激活坐骑符数量
function BeadHuanHuaView:GetHaveProNum(item_id, need_num)
...
function BeadHuanHuaView:SetSpecialImageAttr(bead_special_data, index)
	if bead_special_data == nil then
		return
	end
	local upgrade_cfg = BeadData.Instance:GetSpecialImageUpgradeInfo(bead_special_data.image_id)
	local image_cfg = BeadData.Instance:GetSpecialImageCfg(index)
	...
	local change_name_list = Split(bead_special_data.image_name, "-")
	local change_name = (#change_name_list >= 2) and (change_name_list[1] .. " - " .. change_name_list[2]) or bead_special_data.image_name
	--self.bead_name:SetValue("<color=" .. SOUL_NAME_COLOR[item_cfg and item_cfg.color or 5] .. ">" .. change_name .. "</color>")
	if self.name_obj ~= nil then
		local bundle, asset = ResPath.GetAdvanceEquipIcon("hh_bead_" .. (image_cfg.title_res or 1))
	...
	local attr_cfg = BeadData.Instance:GetSpecialImageUpgradeInfo(index)
	if attr_cfg ~= nil then
		...
		self:GetHaveProNum(bead_special_data.item_id, upgrade_cfg.stuff_num)
	end

	local attr2 = CommonStruct.Attribute()
	local next_cfg = BeadData.Instance:GetSpecialImageUpgradeInfo(index, nil, true)
	...
	local data = {item_id = bead_special_data.item_id, is_bind = 0}
...
--设置激活按钮显示和隐藏
function BeadHuanHuaView:IsShowActivate(image_id)
...
--设置升级按钮显示和隐藏
function BeadHuanHuaView:IsShowUpGrade(image_id)
	if image_id == nil then
		return
	end
	local special_img_up = BeadData.Instance:GetSpecialImageUpgradeCfg()
	local info_list = BeadData.Instance:GetBeadInfo()
	local active_flag = info_list.active_special_image_flag
	-- local bit_list = bit:d2b(active_flag)
...
--升级按钮是否置灰
function BeadHuanHuaView:IsGrayUpgradeButton(index)
	if index == nil or index < 0 then return end
	--local bead_special_image = ConfigManager.Instance:GetAutoConfig("bead_auto").special_img
	--local _, bead_special_image = BeadData.Instance:GetShowSpecialInfo()
	local upgrade_cfg = BeadData.Instance:GetSpecialImageUpgradeInfo(self.all_data[index].image_id)
	if upgrade_cfg.grade < BeadData.Instance:GetSpecialImageMaxUpLevelById(self.all_data[index].image_id) then --GameEnum.MAX_UPGRADE_LIMIT
...
function BeadHuanHuaView:OnFlush(param_list)
	for k, v in pairs(param_list) do
		if k == "beadhuanhua" then
			...
			self.title_icon:SetAsset("uis/views/advanceview_images", "icon_beadhuanhua")
			--local bead_special_image = ConfigManager.Instance:GetAutoConfig("bead_auto").special_img
			--local bead_special_image = BeadData.Instance:GetShowSpecialInfo()
			...
			local upgrade_cfg = BeadData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
			local info_list = BeadData.Instance:GetBeadInfo()
			local active_flag = info_list.active_special_image_flag
			-- local bit_list = bit:d2b(active_flag)
			...
			-- if self.show_btn_right ~= nil then
			-- 	self.show_btn_right:SetValue(self.index < #bead_special_image)
			-- end
		end
	end
end

BeadHuanHuaCell = BeadHuanHuaCell or BaseClass(BaseCell)

function BeadHuanHuaCell:__init()
...
function BeadHuanHuaCell:OnFlush()
	...
	local active_flag = info_list.active_special_image_flag
	-- local bit_list = bit:d2b(active_flag)
	local _, special_data = BeadData.Instance:GetShowSpecialInfo()
	local image_id = special_data[self.index].image_id
	self.is_possess_img:SetValue(active_flag[image_id] == 1)
end

function BeadHuanHuaCell:ListenClick(handler)
	self:ClearEvent("Click")
	self:ListenEvent("Click", handler)
end

function BeadHuanHuaCell:SetToggleGroup(toggle_group)
	self.root_node.toggle.group = toggle_group
end

function BeadHuanHuaCell:SetHighLight(enable)
