DressUpBeadView = DressUpBeadView or BaseClass(BaseRender)

local EFFECT_CD = 1.8
function DressUpBeadView:__init(instance)
	self.is_auto = false
...
function DressUpBeadView:__delete()
	if self.bead_model ~= nil then
		self.bead_model:DeleteMe()
		self.bead_model = nil
	end
...
function DressUpBeadView:LoadCallBack()
	...
	self.bead_name = self:FindVariable("Name")
	self.bead_rank = self:FindVariable("Rank")
	...
	self.bead_model = RoleModel.New()
	self.bead_display = self:FindObj("BeadDisplay")
	self.bead_model:SetDisplay(self.bead_display.ui3d_display)
...
function DressUpBeadView:OnClickSkill()
	AdvanceSkillCtrl.Instance:OpenView(ADVANCE_SKILL_TYPE.WING)
end

-- 开始进阶
function DressUpBeadView:OnStartAdvance(from_auto)
	local bead_info = BeadData.Instance:GetBeadInfo()
	if bead_info.show_grade == 0 then
		return
	end
	local grade_cfg = BeadData.Instance:GetBeadShowGradeCfg(bead_info.show_grade)

	local is_auto_buy_toggle = self.auto_buy_toggle.toggle.isOn

	if bead_info.show_grade >= BeadData.Instance:GetMaxGrade() and (bead_info.star_level % 10 == 0) then
		return
	end
	...
	BeadCtrl.Instance:SendUpGradeReq(is_auto_buy, self.is_auto)
	self.jinjie_next_time = Status.NowTime + (grade_cfg.next_time or 0.1)
end

function DressUpBeadView:AutoUpGradeOnce()
	...
	if self.cur_select_grade > 0 and self.cur_select_grade <= BeadData.Instance:GetMaxGrade() then
...
function DressUpBeadView:BeadUpgradeResult(result)
	self.is_can_auto = true
	if 0 == result then
		self.is_auto = false
		self:SetAutoButtonGray()
...
-- 自动进阶
function DressUpBeadView:OnAutomaticAdvance()
	local bead_info = BeadData.Instance:GetBeadInfo()
	...
	local function ok_callback()
		if TipsCommonAutoView.AUTO_VIEW_STR_T["auto_bead_up"] and TipsCommonAutoView.AUTO_VIEW_STR_T["auto_bead_up"].is_auto_buy then
			self.auto_buy_toggle.toggle.isOn = true
		end
	...
	if not self.auto_buy_toggle.toggle.isOn then
		TipsCtrl.Instance:ShowCommonAutoView("auto_bead_up", Language.Mount.AutoUpDes, ok_callback, cancel_callback, true, nil, nil, nil, true)
	else
		ok_callback()
	end
end
...
-- 顶级预览
function DressUpBeadView:OnClickTopLook()
	local bead_info = BeadData.Instance:GetBeadInfo()
	if not bead_info or not next(bead_info) then return end
	...
	local grade = self.is_on_look and BeadData.Instance:GetMaxGrade() or self.cur_select_grade

	self:SwitchGradeAndName(grade)
end

-- 点击进阶装备
function DressUpBeadView:OnClickEquipBtn()
	local is_active, activite_grade = BeadData.Instance:IsOpenEquip()
	...
	ViewManager.Instance:Open(ViewName.AdvanceEquipView, TabIndex.bead)
end

-- 使用当前羽翼
function DressUpBeadView:OnClickUse()
	if self.cur_select_grade == nil then
		return
	end
	local grade_cfg = BeadData.Instance:GetBeadShowGradeCfg(self.cur_select_grade)
	if not grade_cfg then return end
	BeadCtrl.Instance:SendUseBeadImage(grade_cfg.image_id)
...
--显示上一阶形象
function DressUpBeadView:OnClickLastButton()
	...
	-- if self.bead_display ~= nil then
		-- self.bead_display.ui3d_display:ResetRotation()
	-- end
end

--显示下一阶形象
function DressUpBeadView:OnClickNextButton()
	local bead_info = BeadData.Instance:GetBeadInfo()
	if not self.cur_select_grade or self.cur_select_grade > bead_info.show_grade or bead_info.show_grade == 0 then
		return
	end
	self.cur_select_grade = self.cur_select_grade + 1
	self:SetArrowState(self.cur_select_grade)
...
function DressUpBeadView:SwitchGradeAndName(index)
	if index == nil then return end
	local bead_grade_cfg = BeadData.Instance:GetBeadShowGradeCfg(index)
	if bead_grade_cfg == nil then return end
	local image_cfg = BeadData.Instance:GetBeadImageCfg(bead_grade_cfg.image_id)

	local bundle, asset = ResPath.GetDressUpEquipIcon("bead_name_" .. (image_cfg.title_res or 1))
	self.grade_name_img:SetAsset(bundle, asset)
	self.bead_rank:SetValue(bead_grade_cfg.gradename)

	if image_cfg and self.res_id ~= image_cfg.res_id then
		-- local new_attr = BeadData.Instance:GetBeadAttrSum()
		-- TipsCtrl.Instance:ShowAdvanceSucceView(image_cfg[bead_grade_cfg.image_id], new_attr, new_attr, "bead_view")
		...
		self.bead_name:SetValue(name_str)

		local bundle, asset = ResPath.GetLingZhuModel(image_cfg.res_id, true)	
		self.bead_model:SetMainAsset(bundle, asset, function ()
		end) 
		self.bead_model:SetDisplayPositionAndRotation("dress_up_bead")
		self.res_id = image_cfg.res_id
	end
end
 
-- 资质
function DressUpBeadView:OnClickZiZhi()
	ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "beadzizhi", {item_id = BeadDanId.ZiZhiDanId})
end

-- -- 成长
-- function DressUpBeadView:OnClickChengZhang()
-- 	ViewManager.Instance:Open(ViewName.TipChengZhang, nil,"beadchengzhang", {item_id = BeadDanId.ChengZhangDanId})
-- end

-- 幻化
function DressUpBeadView:OnClickHuanHua()
	ViewManager.Instance:Open(ViewName.BeadHuanHua)
	BeadHuanHuaCtrl.Instance:FlushView("beadhuanhua")
end

-- 点击技能
function DressUpBeadView:OnClickBeadSkill(index)
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "beadskill", {index = index - 1})
end

function DressUpBeadView:GetBeadSkill()
	for i = 1, 4 do
		local skill = self:FindObj("BeadSkill"..i)
		local icon = skill:FindObj("Image")
		local activite = skill:FindObj("ImgActivity")
		table.insert(self.bead_skill_list, {skill = skill, icon = icon, activite = activite})
	end
	for k, v in pairs(self.bead_skill_list) do
		local bundle, asset = ResPath.GetBeadSkillIcon(k)
		v.icon.image:LoadSprite(bundle, asset)
		v.skill.toggle:AddValueChangedListener(BindTool.Bind(self.OnClickBeadSkill, self, k))
	end
end

function DressUpBeadView:FlushSkillIcon()
	local skill_id_list = BeadData.Instance:GetBeadSkillId()
	local bead_info = BeadData.Instance:GetBeadInfo()
	...
	for k, v in pairs(self.bead_skill_list) do
		...
		-- end
		if skill_level <= 0 and bead_info.show_grade >= 1 then
			activity_info = BeadData.Instance:GetBeadSkillIsActvity(k - 1, bead_info.show_grade)
		end
		if v.activite then
...
-- 设置属性
function DressUpBeadView:SetBeadAtrr()
	local bead_info = BeadData.Instance:GetBeadInfo()
	--local image_cfg = BeadData.Instance:GetBeadImageCfg()
	if bead_info == nil or bead_info.show_grade == nil then
		self:SetAutoButtonGray()
		return
	end

	self.is_block = (bead_info.show_grade > 0 and bead_info.used_imageid == 0) and true or false
	self.is_masking:SetValue(self.is_block)

	if bead_info.bead_level == 0 or bead_info.show_grade == 0 then
		local bead_grade_cfg = BeadData.Instance:GetBeadGradeCfg(1)
		self:SetAutoButtonGray()
		self.remainder_num:SetValue(ItemData.Instance:GetItemNumInBagById(bead_grade_cfg.upgrade_stuff_id))
		-- self.no_clear_text:SetValue(false)
		-- self.clear_text:SetValue(false)
		return
	end
	local bead_grade_cfg = BeadData.Instance:GetBeadGradeCfg(bead_info.grade)

	if not bead_grade_cfg then return end

	if self.temp_grade < 0 then
		if bead_grade_cfg.show_grade == 0 then
			self.cur_select_grade = bead_info.show_grade
		elseif bead_grade_cfg.show_grade > 0 and bead_info.used_imageid == 0 then -- 屏蔽
			self.cur_select_grade = bead_info.show_grade
		else
			self.cur_select_grade = bead_info.used_imageid >= GameEnum.MOUNT_SPECIAL_IMA_ID and bead_info.show_grade
									or BeadData.Instance:GetBeadGradeByUseImageId(bead_info.used_imageid)
		end
		self:SetAutoButtonGray()
		self:SetArrowState(self.cur_select_grade)
		self:SwitchGradeAndName(self.cur_select_grade)
		self.temp_grade = bead_info.show_grade
	else
		if self.temp_grade < bead_info.show_grade then
			-- local new_attr = BeadData.Instance:GetBeadAttrSum()
			...
			-- TipsCtrl.Instance:ShowAdvanceSucceView(image_cfg[bead_grade_cfg.image_id], new_attr, self.old_attrs, "bead_view", new_capability, old_capability)
			...
			self:SetAutoButtonGray()
			self:SetArrowState(self.cur_select_grade)
			self:SwitchGradeAndName(bead_info.show_grade)
	...
	if self.old_grade_bless_val == nil then 
		self.old_grade_bless_val = bead_info.grade_bless_val --初始化
	end
	if self.old_star_level == nil then
		self.old_star_level = bead_info.star_level % 10
	end
	if bead_info.show_grade >= BeadData.Instance:GetMaxGrade() and (bead_info.star_level % 10 == 0) then
		self:SetAutoButtonGray()
		...
		self.cur_bless:SetValue(bead_info.grade_bless_val.."/"..bead_grade_cfg.bless_val_limit)
		self.exp_radio:SetValue(bead_info.grade_bless_val/bead_grade_cfg.bless_val_limit)
		--升星提示
		if self.old_grade_bless_val ~= bead_info.grade_bless_val then
			if(bead_info.grade_bless_val-self.old_grade_bless_val >= 100)  then
				TipsCtrl.Instance:ShowFloatingLabel(nil, 250, 30, false, true, ResPath.GetFloatTextRes("WordUpStarBaoji"))
			end
			self.old_grade_bless_val = bead_info.grade_bless_val
		end
		if self.old_star_level ~= bead_info.star_level % 10 then
			--升星提示
	...
	local baoju_plus = AdvanceData.Instance:GetMedalPlusNum("bead_attr_add")
	local baoju_grade = MedalData.Instance:GetMedalIsOneJie()
	if not baoju_grade then
		baoju_plus = 0
	end		
	for i = 0, 3 do
		if BeadData.Instance:GetBeadSkillCfgById(i) then
			skill_capability = skill_capability + BeadData.Instance:GetBeadSkillCfgById(i).capability
		end
	end
	self.skill_fight_power = skill_capability
	local attr = CommonDataManager.GetAttributteByClass(bead_grade_cfg)
	...
	self.bao_ji:SetValue(attr.ignore_fangyu + math.floor(attr.ignore_fangyu * baoju_plus / 100))
	self.jian_ren:SetValue(attr.jian_ren + math.floor(attr.jian_ren * baoju_plus / 100))
	self.baoju_plus_num:SetValue(baoju_plus)

	self.need_num:SetValue("  " .. bead_grade_cfg.upgrade_stuff_count)

	local bag_num = string.format(Language.Mount.ShowGreenStr, ItemData.Instance:GetItemNumInBagById(bead_grade_cfg.upgrade_stuff_id))
	if ItemData.Instance:GetItemNumInBagById(bead_grade_cfg.upgrade_stuff_id) < bead_grade_cfg.upgrade_stuff_count then
		bag_num = string.format(Language.Mount.ShowRedStr, ItemData.Instance:GetItemNumInBagById(bead_grade_cfg.upgrade_stuff_id))
	end
	self.remainder_num:SetValue(bag_num)
	self.prop_name:SetValue(ItemData.Instance:GetItemConfig(bead_grade_cfg.upgrade_stuff_id).name)
	-- local item_cfg = ItemData.Instance:GetItemConfig(bead_grade_cfg.upgrade_stuff_id)
	-- self.item_icon:SetAsset(ResPath.GetItemIcon(item_cfg.icon_id))
	self.item_cell:SetData({item_id=bead_grade_cfg.upgrade_stuff_id, num=0, is_bind=0})

	self.jia_shang:SetValue(attr.per_pofang)
	self.jian_shang:SetValue(attr.per_mianshang)
	self.show_zizhi_redpoint:SetValue(BeadData.Instance:IsShowZizhiRedPoint())
	-- self.show_chengzhang_redpoint:SetValue(BeadData.Instance:IsShowChengzhangRedPoint())
	self.show_huanhua_redpoint:SetValue(next(BeadData.Instance:CanHuanhuaUpgrade()) ~= nil)
	local can_uplevel_skill_list = BeadData.Instance:CanSkillUpLevelList()
	for i = 0, GameEnum.MOUNT_SKILL_COUNT - 1 do
...
function DressUpBeadView:SetArrowState(cur_select_grade)
	local bead_info = BeadData.Instance:GetBeadInfo()
	local max_grade = BeadData.Instance:GetMaxGrade()
	local grade_cfg = BeadData.Instance:GetBeadShowGradeCfg(cur_select_grade)
	if not bead_info or not bead_info.show_grade or not cur_select_grade or not max_grade or not grade_cfg then
		return
	end
	self.show_right_button:SetValue(cur_select_grade < bead_info.show_grade + 1 and cur_select_grade < max_grade)
	self.show_left_button:SetValue(grade_cfg.image_id > 1 or (bead_info.show_grade  == 1 and cur_select_grade > bead_info.show_grade))
	self:SetUseImageButtonState(cur_select_grade)
end

--设置使用形象按钮
function DressUpBeadView:SetUseImageButtonState(cur_select_grade)
	local bead_info = BeadData.Instance:GetBeadInfo()
	local max_grade = BeadData.Instance:GetMaxGrade()
	local grade_cfg = BeadData.Instance:GetBeadShowGradeCfg(cur_select_grade)
	...
	self.show_use_button:SetValue(cur_select_grade <= bead_info.show_grade and grade_cfg.image_id ~= bead_info.used_imageid)
	self.show_use_image:SetValue(grade_cfg.image_id == bead_info.used_imageid)
end

-- 点击自动进阶，服务器返回信息，设置按钮状态
function DressUpBeadView:SetAutoButtonGray()
	local bead_info = BeadData.Instance:GetBeadInfo()
	if bead_info.show_grade == nil then return end

	local max_grade = BeadData.Instance:GetMaxGrade()

	if not bead_info or not bead_info.show_grade or bead_info.show_grade <= 0
		or (bead_info.show_grade >= max_grade and (bead_info.star_level % 10 == 0)) then
...
-- 物品不足，购买成功后刷新物品数量
function DressUpBeadView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	local bead_info = BeadData.Instance:GetBeadInfo()
	local bead_grade_cfg = BeadData.Instance:GetBeadShowGradeCfg(bead_info.show_grade)
	if bead_grade_cfg == nil  or self.remainder_num == nil then
		return
	end
	local bag_num = string.format(Language.Mount.ShowGreenNum, ItemData.Instance:GetItemNumInBagById(bead_grade_cfg.upgrade_stuff_id))
	if ItemData.Instance:GetItemNumInBagById(bead_grade_cfg.upgrade_stuff_id) < bead_grade_cfg.upgrade_stuff_count then
		bag_num = string.format(Language.Mount.ShowRedNum, ItemData.Instance:GetItemNumInBagById(bead_grade_cfg.upgrade_stuff_id))
	end
	self.remainder_num:SetValue(bag_num)
end

function DressUpBeadView:SetNotifyDataChangeCallBack()
...
function DressUpBeadView:SetModle(is_show)
	if is_show then
		-- if not BeadData.Instance:IsActiviteBead() then
		-- 	print_error("Bead is not activite")
		-- 	return
		-- end
		local bead_info = BeadData.Instance:GetBeadInfo()
		local used_imageid = bead_info.used_imageid or 0
		local bead_grade_cfg = BeadData.Instance:GetBeadShowGradeCfg(bead_info.show_grade)
		if used_imageid >= GameEnum.MOUNT_SPECIAL_IMA_ID then
			used_imageid = BeadData.Instance:GetBeadShowGradeCfg(bead_info.show_grade).image_id
		end
		...
		if bead_grade_cfg and used_imageid > 0 and self.cur_select_grade < 0 then
			local cur_select_grade = bead_grade_cfg.show_grade == 0 and bead_info.show_grade or BeadData.Instance:GetBeadGradeByUseImageId(used_imageid)
		...
		-- 屏蔽形象时
		if bead_grade_cfg and used_imageid == 0 and self.cur_select_grade < 0 then
			cur_select_grade = bead_grade_cfg.show_grade 
			self:SwitchGradeAndName(cur_select_grade)
			self.cur_select_grade = self.cur_select_grade and cur_select_grade
		end
	else
...
function DressUpBeadView:ClearTempData()
	self.res_id = -1
	self.cur_select_grade = -1
	self.temp_grade = -1
	self.is_auto = false
...
function DressUpBeadView:RemoveNotifyDataChangeCallBack()
	if self.item_data_event ~= nil then
		ItemData.Instance:UnNotifyDataChangeCallBack(self.item_data_event)
		self.item_data_event = nil
	end
...
function DressUpBeadView:ResetModleRotation()
	-- if self.bead_display ~= nil then
		-- self.bead_display.ui3d_display:ResetRotation()
	-- end
end

function DressUpBeadView:FlushStars()
	local bead_info = BeadData.Instance:GetBeadInfo()
	if nil == bead_info or nil == bead_info.star_level then
		return
	end
	local index = bead_info.star_level % 10
...
function DressUpBeadView:OnAutoBuyToggleChange(isOn)
	if TipsCommonAutoView.AUTO_VIEW_STR_T["auto_bead_up"] then
		TipsCommonAutoView.AUTO_VIEW_STR_T["auto_bead_up"].is_auto_buy = isOn
	end
end

function DressUpBeadView:OpenCallBack()
	if self.show_effect then
		self.show_effect:SetValue(false)
	end
	if TipsCommonAutoView.AUTO_VIEW_STR_T["auto_bead_up"] then
		self.auto_buy_toggle.toggle.isOn = TipsCommonAutoView.AUTO_VIEW_STR_T["auto_bead_up"].is_auto_buy
	end
end

function DressUpBeadView:OnFlush(param_list)
	if not BeadData.Instance:IsActiviteBead() then
		return
	end
	...
	self.show_equip_redpoint:SetValue(DressUpData.Instance:IsEquipRedPointShow(ADVANCE_SKILL_TYPE.BEAD))
	self.upgrade_redpoint:SetValue(BeadData.Instance:CanJinjie())
	local bool = OpenFunData.Instance:CheckIsHide("advanceskill")
	self.skill_funopen:SetValue(bool)
end

function DressUpBeadView:OnClickMasking()
	local bead_info = BeadData.Instance:GetBeadInfo()
	if self.is_block then
		BeadCtrl.Instance:SendBeadReUseReq()
	else
		BeadCtrl.Instance:SendUnuseBeadImage(bead_info.used_imageid)
	end
end
