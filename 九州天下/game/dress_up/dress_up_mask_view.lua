DressUpMaskView = DressUpMaskView or BaseClass(BaseRender)

local EFFECT_CD = 1.8
function DressUpMaskView:__init(instance)
	self.is_auto = false
...
function DressUpMaskView:__delete()
	if self.mask_model ~= nil then
		self.mask_model:DeleteMe()
		self.mask_model = nil
	end
...
function DressUpMaskView:LoadCallBack()
	...
	self.mask_name = self:FindVariable("Name")
	self.mask_rank = self:FindVariable("Rank")
	...
	self.mask_model = RoleModel.New()
	self.mask_display = self:FindObj("MaskDisplay")
	self.mask_model:SetDisplay(self.mask_display.ui3d_display)
...
function DressUpMaskView:OnClickSkill()
	AdvanceSkillCtrl.Instance:OpenView(ADVANCE_SKILL_TYPE.WING)
end

-- 开始进阶
function DressUpMaskView:OnStartAdvance(from_auto)
	local mask_info = MaskData.Instance:GetMaskInfo()
	if mask_info.show_grade == 0 then
		return
	end
	local grade_cfg = MaskData.Instance:GetMaskShowGradeCfg(mask_info.show_grade)

	local is_auto_buy_toggle = self.auto_buy_toggle.toggle.isOn

	if mask_info.show_grade >= MaskData.Instance:GetMaxGrade() and (mask_info.star_level % 10 == 0) then
		return
	end
	...
	MaskCtrl.Instance:SendUpGradeReq(is_auto_buy, self.is_auto)
	self.jinjie_next_time = Status.NowTime + (grade_cfg.next_time or 0.1)
end

function DressUpMaskView:AutoUpGradeOnce()
	...
	if self.cur_select_grade > 0 and self.cur_select_grade <= MaskData.Instance:GetMaxGrade() then
...
function DressUpMaskView:MaskUpgradeResult(result)
	self.is_can_auto = true
	if 0 == result then
		self.is_auto = false
		self:SetAutoButtonGray()
...
-- 自动进阶
function DressUpMaskView:OnAutomaticAdvance()
	local mask_info = MaskData.Instance:GetMaskInfo()
	...
	local function ok_callback()
		if TipsCommonAutoView.AUTO_VIEW_STR_T["auto_mask_up"] and TipsCommonAutoView.AUTO_VIEW_STR_T["auto_mask_up"].is_auto_buy then
			self.auto_buy_toggle.toggle.isOn = true
		end
	...
	if not self.auto_buy_toggle.toggle.isOn then
		TipsCtrl.Instance:ShowCommonAutoView("auto_mask_up", Language.Mount.AutoUpDes, ok_callback, cancel_callback, true, nil, nil, nil, true)
	else
		ok_callback()
	end
end
...
-- 顶级预览
function DressUpMaskView:OnClickTopLook()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if not mask_info or not next(mask_info) then return end
	...
	local grade = self.is_on_look and MaskData.Instance:GetMaxGrade() or self.cur_select_grade

	self:SwitchGradeAndName(grade)
end

-- 点击进阶装备
function DressUpMaskView:OnClickEquipBtn()
	local is_active, activite_grade = MaskData.Instance:IsOpenEquip()
	...
	ViewManager.Instance:Open(ViewName.AdvanceEquipView, TabIndex.mask)
end

-- 使用当前羽翼
function DressUpMaskView:OnClickUse()
	if self.cur_select_grade == nil then
		return
	end
	local grade_cfg = MaskData.Instance:GetMaskShowGradeCfg(self.cur_select_grade)
	if not grade_cfg then return end
	MaskCtrl.Instance:SendUseMaskImage(grade_cfg.image_id)
	self.is_block = false
	self.is_masking:SetValue(self.is_block)
end

--显示上一阶形象
function DressUpMaskView:OnClickLastButton()
	...
	-- if self.mask_display ~= nil then
		-- self.mask_display.ui3d_display:ResetRotation()
	-- end
end

--显示下一阶形象
function DressUpMaskView:OnClickNextButton()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if not self.cur_select_grade or self.cur_select_grade > mask_info.show_grade or mask_info.show_grade == 0 then
		return
	end
	self.cur_select_grade = self.cur_select_grade + 1
	self:SetArrowState(self.cur_select_grade)
...
function DressUpMaskView:SwitchGradeAndName(index)
	if index == nil then return end
	local mask_grade_cfg = MaskData.Instance:GetMaskShowGradeCfg(index)
	if mask_grade_cfg == nil then return end
	local image_cfg = MaskData.Instance:GetMaskImageCfg(mask_grade_cfg.image_id)

	local bundle, asset = ResPath.GetDressUpEquipIcon("mask_name_" .. (image_cfg.title_res or 1))
	self.grade_name_img:SetAsset(bundle, asset)
	self.mask_rank:SetValue(mask_grade_cfg.gradename)

	if image_cfg and self.res_id ~= image_cfg.res_id then
		-- local new_attr = MaskData.Instance:GetMaskAttrSum()
		-- TipsCtrl.Instance:ShowAdvanceSucceView(image_cfg[mask_grade_cfg.image_id], new_attr, new_attr, "mask_view")
		...
		self.mask_name:SetValue(name_str)

		local main_role = Scene.Instance:GetMainRole()
		self.mask_model:SetRoleResid(main_role:GetRoleResId())
		self.mask_model:SetMaskResid(image_cfg.res_id)
		self.res_id = image_cfg.res_id
	end
end
 
-- 资质
function DressUpMaskView:OnClickZiZhi()
	ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "maskzizhi", {item_id = MaskDanId.ZiZhiDanId})
end

-- -- 成长
-- function DressUpMaskView:OnClickChengZhang()
-- 	ViewManager.Instance:Open(ViewName.TipChengZhang, nil,"maskchengzhang", {item_id = MaskDanId.ChengZhangDanId})
-- end

-- 幻化
function DressUpMaskView:OnClickHuanHua()
	ViewManager.Instance:Open(ViewName.MaskHuanHua)
	MaskHuanHuaCtrl.Instance:FlushView("maskhuanhua")
end

-- 点击技能
function DressUpMaskView:OnClickMaskSkill(index)
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "maskskill", {index = index - 1})
end

function DressUpMaskView:GetMaskSkill()
	for i = 1, 4 do
		local skill = self:FindObj("MaskSkill"..i)
		local icon = skill:FindObj("Image")
		local activite = skill:FindObj("ImgActivity")
		table.insert(self.mask_skill_list, {skill = skill, icon = icon, activite = activite})
	end
	for k, v in pairs(self.mask_skill_list) do
		local bundle, asset = ResPath.GetMaskSkillIcon(k)
		v.icon.image:LoadSprite(bundle, asset)
		v.skill.toggle:AddValueChangedListener(BindTool.Bind(self.OnClickMaskSkill, self, k))
	end
end

function DressUpMaskView:FlushSkillIcon()
	local skill_id_list = MaskData.Instance:GetMaskSkillId()
	local mask_info = MaskData.Instance:GetMaskInfo()
	...
	for k, v in pairs(self.mask_skill_list) do
		...
		-- end
		if skill_level <= 0 and mask_info.show_grade >= 1 then
			activity_info = MaskData.Instance:GetMaskSkillIsActvity(k - 1, mask_info.show_grade)
		end
		if v.activite then
...
-- 设置属性
function DressUpMaskView:SetMaskAtrr()
	local mask_info = MaskData.Instance:GetMaskInfo()
	--local image_cfg = MaskData.Instance:GetMaskImageCfg()
	if mask_info == nil or mask_info.show_grade == nil then
		self:SetAutoButtonGray()
		return
	end

	self.is_block = (mask_info.show_grade > 0 and mask_info.used_imageid == 0) and true or false
	self.is_masking:SetValue(self.is_block)

	if mask_info.mask_level == 0 or mask_info.show_grade == 0 then
		local mask_grade_cfg = MaskData.Instance:GetMaskGradeCfg(1)
		self:SetAutoButtonGray()
		self.remainder_num:SetValue(ItemData.Instance:GetItemNumInBagById(mask_grade_cfg.upgrade_stuff_id))
		-- self.no_clear_text:SetValue(false)
		-- self.clear_text:SetValue(false)
		return
	end
	local mask_grade_cfg = MaskData.Instance:GetMaskGradeCfg(mask_info.grade)

	if not mask_grade_cfg then return end

	if self.temp_grade < 0 then
		if mask_grade_cfg.show_grade == 0 then
			self.cur_select_grade = mask_info.show_grade
		elseif mask_grade_cfg.show_grade > 0 and mask_info.used_imageid == 0 then -- 屏蔽
			self.cur_select_grade = mask_info.show_grade
		else
			self.cur_select_grade = mask_info.used_imageid >= GameEnum.MOUNT_SPECIAL_IMA_ID and mask_info.show_grade
									or MaskData.Instance:GetMaskGradeByUseImageId(mask_info.used_imageid)
		end
		self:SetAutoButtonGray()
		self:SetArrowState(self.cur_select_grade)
		self:SwitchGradeAndName(self.cur_select_grade)
		self.temp_grade = mask_info.show_grade
	else
		if self.temp_grade < mask_info.show_grade then
			-- local new_attr = MaskData.Instance:GetMaskAttrSum()
			...
			-- TipsCtrl.Instance:ShowAdvanceSucceView(image_cfg[mask_grade_cfg.image_id], new_attr, self.old_attrs, "mask_view", new_capability, old_capability)
			...
			self:SetAutoButtonGray()
			self:SetArrowState(self.cur_select_grade)
			self:SwitchGradeAndName(mask_info.show_grade)
	...
	if self.old_grade_bless_val == nil then 
		self.old_grade_bless_val = mask_info.grade_bless_val --初始化
	end
	if self.old_star_level == nil then
		self.old_star_level = mask_info.star_level % 10
	end
	if mask_info.show_grade >= MaskData.Instance:GetMaxGrade() and (mask_info.star_level % 10 == 0) then
		self:SetAutoButtonGray()
		...
		self.cur_bless:SetValue(mask_info.grade_bless_val.."/"..mask_grade_cfg.bless_val_limit)
		self.exp_radio:SetValue(mask_info.grade_bless_val/mask_grade_cfg.bless_val_limit)
		--升星提示
		if self.old_grade_bless_val ~= mask_info.grade_bless_val then
			if(mask_info.grade_bless_val-self.old_grade_bless_val >= 100)  then
				TipsCtrl.Instance:ShowFloatingLabel(nil, 250, 30, false, true, ResPath.GetFloatTextRes("WordUpStarBaoji"))
			end
			self.old_grade_bless_val = mask_info.grade_bless_val
		end
		if self.old_star_level ~= mask_info.star_level % 10 then
			--升星提示
	...
	local baoju_plus = AdvanceData.Instance:GetMedalPlusNum("mask_attr_add")
	local baoju_grade = MedalData.Instance:GetMedalIsOneJie()
	if not baoju_grade then
		baoju_plus = 0
	end		
	for i = 0, 3 do
		if MaskData.Instance:GetMaskSkillCfgById(i) then
			skill_capability = skill_capability + MaskData.Instance:GetMaskSkillCfgById(i).capability
		end
	end
	self.skill_fight_power = skill_capability
	local attr = CommonDataManager.GetAttributteByClass(mask_grade_cfg)
	...
	self.bao_ji:SetValue(attr.ming_zhong + math.floor(attr.ming_zhong * baoju_plus / 100))
	self.jian_ren:SetValue(attr.jian_ren + math.floor(attr.jian_ren * baoju_plus / 100))
	self.baoju_plus_num:SetValue(baoju_plus)

	self.need_num:SetValue("  " .. mask_grade_cfg.upgrade_stuff_count)

	local bag_num = string.format(Language.Mount.ShowGreenStr, ItemData.Instance:GetItemNumInBagById(mask_grade_cfg.upgrade_stuff_id))
	if ItemData.Instance:GetItemNumInBagById(mask_grade_cfg.upgrade_stuff_id) < mask_grade_cfg.upgrade_stuff_count then
		bag_num = string.format(Language.Mount.ShowRedStr, ItemData.Instance:GetItemNumInBagById(mask_grade_cfg.upgrade_stuff_id))
	end
	self.remainder_num:SetValue(bag_num)
	self.prop_name:SetValue(ItemData.Instance:GetItemConfig(mask_grade_cfg.upgrade_stuff_id).name)
	-- local item_cfg = ItemData.Instance:GetItemConfig(mask_grade_cfg.upgrade_stuff_id)
	-- self.item_icon:SetAsset(ResPath.GetItemIcon(item_cfg.icon_id))
	self.item_cell:SetData({item_id=mask_grade_cfg.upgrade_stuff_id, num=0, is_bind=0})

	self.jia_shang:SetValue(attr.per_pofang)
	self.jian_shang:SetValue(attr.per_mianshang)
	self.show_zizhi_redpoint:SetValue(MaskData.Instance:IsShowZizhiRedPoint())
	-- self.show_chengzhang_redpoint:SetValue(MaskData.Instance:IsShowChengzhangRedPoint())
	self.show_huanhua_redpoint:SetValue(next(MaskData.Instance:CanHuanhuaUpgrade()) ~= nil)
	local can_uplevel_skill_list = MaskData.Instance:CanSkillUpLevelList()
	for i = 0, GameEnum.MOUNT_SKILL_COUNT - 1 do
...
function DressUpMaskView:SetArrowState(cur_select_grade)
	local mask_info = MaskData.Instance:GetMaskInfo()
	local max_grade = MaskData.Instance:GetMaxGrade()
	local grade_cfg = MaskData.Instance:GetMaskShowGradeCfg(cur_select_grade)
	if not mask_info or not mask_info.show_grade or not cur_select_grade or not max_grade or not grade_cfg then
		return
	end
	self.show_right_button:SetValue(cur_select_grade < mask_info.show_grade + 1 and cur_select_grade < max_grade)
	self.show_left_button:SetValue(grade_cfg.image_id > 1 or (mask_info.show_grade  == 1 and cur_select_grade > mask_info.show_grade))
	self:SetUseImageButtonState(cur_select_grade)
end

--设置使用形象按钮
function DressUpMaskView:SetUseImageButtonState(cur_select_grade)
	local mask_info = MaskData.Instance:GetMaskInfo()
	local max_grade = MaskData.Instance:GetMaxGrade()
	local grade_cfg = MaskData.Instance:GetMaskShowGradeCfg(cur_select_grade)
	...
	self.show_use_button:SetValue(cur_select_grade <= mask_info.show_grade and grade_cfg.image_id ~= mask_info.used_imageid)
	self.show_use_image:SetValue(grade_cfg.image_id == mask_info.used_imageid)
end

-- 点击自动进阶，服务器返回信息，设置按钮状态
function DressUpMaskView:SetAutoButtonGray()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if mask_info.show_grade == nil then return end

	local max_grade = MaskData.Instance:GetMaxGrade()

	if not mask_info or not mask_info.show_grade or mask_info.show_grade <= 0
		or (mask_info.show_grade >= max_grade and (mask_info.star_level % 10 == 0)) then
...
-- 物品不足，购买成功后刷新物品数量
function DressUpMaskView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	local mask_info = MaskData.Instance:GetMaskInfo()
	local mask_grade_cfg = MaskData.Instance:GetMaskShowGradeCfg(mask_info.show_grade)
	if mask_grade_cfg == nil  or self.remainder_num == nil then
		return
	end
	local bag_num = string.format(Language.Mount.ShowGreenNum, ItemData.Instance:GetItemNumInBagById(mask_grade_cfg.upgrade_stuff_id))
	if ItemData.Instance:GetItemNumInBagById(mask_grade_cfg.upgrade_stuff_id) < mask_grade_cfg.upgrade_stuff_count then
		bag_num = string.format(Language.Mount.ShowRedNum, ItemData.Instance:GetItemNumInBagById(mask_grade_cfg.upgrade_stuff_id))
	end
	self.remainder_num:SetValue(bag_num)
end

function DressUpMaskView:SetNotifyDataChangeCallBack()
...
function DressUpMaskView:SetModle(is_show)
	if is_show then
		-- if not MaskData.Instance:IsActiviteMask() then
		-- 	print_error("Mask is not activite")
		-- 	return
		-- end
		local mask_info = MaskData.Instance:GetMaskInfo()
		local used_imageid = mask_info.used_imageid or 0
		local mask_grade_cfg = MaskData.Instance:GetMaskShowGradeCfg(mask_info.show_grade)
		if used_imageid >= GameEnum.MOUNT_SPECIAL_IMA_ID then
			used_imageid = MaskData.Instance:GetMaskShowGradeCfg(mask_info.show_grade).image_id
		end
		...
		if mask_grade_cfg and used_imageid > 0 and self.cur_select_grade < 0 then
			local cur_select_grade = mask_grade_cfg.show_grade == 0 and mask_info.show_grade or MaskData.Instance:GetMaskGradeByUseImageId(used_imageid)
		...
		-- 屏蔽形象时
		if mask_grade_cfg and used_imageid == 0 and self.cur_select_grade < 0 then
			cur_select_grade = mask_grade_cfg.show_grade 
			self:SwitchGradeAndName(cur_select_grade)
			self.cur_select_grade = self.cur_select_grade and cur_select_grade
		end
	else
...
function DressUpMaskView:ClearTempData()
	self.res_id = -1
	self.cur_select_grade = -1
	self.temp_grade = -1
	self.is_auto = false
...
function DressUpMaskView:RemoveNotifyDataChangeCallBack()
	if self.item_data_event ~= nil then
		ItemData.Instance:UnNotifyDataChangeCallBack(self.item_data_event)
		self.item_data_event = nil
	end
...
function DressUpMaskView:ResetModleRotation()
	-- if self.mask_display ~= nil then
		-- self.mask_display.ui3d_display:ResetRotation()
	-- end
end

function DressUpMaskView:FlushStars()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if nil == mask_info or nil == mask_info.star_level then
		return
	end
	local index = mask_info.star_level % 10
...
function DressUpMaskView:OnAutoBuyToggleChange(isOn)
	if TipsCommonAutoView.AUTO_VIEW_STR_T["auto_mask_up"] then
		TipsCommonAutoView.AUTO_VIEW_STR_T["auto_mask_up"].is_auto_buy = isOn
	end
end

function DressUpMaskView:OpenCallBack()
	if self.show_effect then
		self.show_effect:SetValue(false)
	end
	if TipsCommonAutoView.AUTO_VIEW_STR_T["auto_mask_up"] then
		self.auto_buy_toggle.toggle.isOn = TipsCommonAutoView.AUTO_VIEW_STR_T["auto_mask_up"].is_auto_buy
	end
end

function DressUpMaskView:OnFlush(param_list)
	if not MaskData.Instance:IsActiviteMask() then
		return
	end
	...
	self.show_equip_redpoint:SetValue(DressUpData.Instance:IsEquipRedPointShow(ADVANCE_SKILL_TYPE.MASK))
	self.upgrade_redpoint:SetValue(MaskData.Instance:CanJinjie())
	local bool = OpenFunData.Instance:CheckIsHide("advanceskill")
	self.skill_funopen:SetValue(bool)
end

function DressUpMaskView:OnClickMasking()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if self.is_block then
		MaskCtrl.Instance:SendMaskReUseReq()
	else
		MaskCtrl.Instance:SendUnuseMaskImage(mask_info.used_imageid)
	end
end
