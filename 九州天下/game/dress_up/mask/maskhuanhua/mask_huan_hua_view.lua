...
function MaskHuanHuaView:LoadCallBack()
	...
	self.mask_display:SetActive(true)
	self.title_name = self:FindVariable("TitleName")
	self.title_name:SetValue(Language.HuanHua.Mask)
	...
	self.mask_name = self:FindVariable("ZuoQiName")
	...
	self.all_num, self.all_data = MaskData.Instance:GetShowSpecialInfo()
	...
	list_delegate.NumberOfCellsDel = BindTool.Bind(self.GetMaskNumberOfCells, self)
	list_delegate.CellRefreshDel = BindTool.Bind(self.RefreshMaskCell, self)
end
...
function MaskHuanHuaView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	if item_id ~= nil and MaskData.Instance:CheckIsHuanHuaItem(item_id) then
		local now_num = MaskData.Instance:GetShowSpecialInfo()
		if self.all_num ~= now_num then
			self:Flush("maskhuanhua", {need_flush = true})
		end
	end
end
...
function MaskHuanHuaView:FlushData()
	--self.all_num, self.all_data = MaskData.Instance:GetShowSpecialInfo()
	self:Flush("maskhuanhua", {need_flush = true})
end
...
function MaskHuanHuaView:GetMaskNumberOfCells()
	--return MaskData.Instance:GetShowSpecialInfo()
	return self.all_num
end

function MaskHuanHuaView:RefreshMaskCell(cell, cell_index)
	--local mask_special_image = ConfigManager.Instance:GetAutoConfig("mask_auto").special_img
	--local _, mask_special_image = MaskData.Instance:GetShowSpecialInfo()
	local mask_cell = self.cell_list[cell]
	if mask_cell == nil then
		mask_cell = MaskHuanHuaCell.New(cell.gameObject)
		self.cell_list[cell] = mask_cell
	end
	mask_cell:SetToggleGroup(self.list_view.toggle_group)
	mask_cell:SetHighLight(self.index == cell_index + 1)
	local data = {}
	if self.all_data[cell_index+1] ~= nil then
		...
		data.is_show = MaskData.Instance:CanHuanhuaUpgrade()[image_id] ~= nil
	end

	mask_cell:SetData(data)
	mask_cell:ListenClick(BindTool.Bind(self.OnClickListCell, self, self.all_data[cell_index+1], cell_index+1, mask_cell))
end

function MaskHuanHuaView:OnClickChange(change_value)
	if change_value ~= nil then
		local mount_special_image = ConfigManager.Instance:GetAutoConfig("mask_auto").special_img
...
function MaskHuanHuaView:OnClickClose()
	self:Close()
	self.res_id = nil
	self.grade = nil
	self.used_imageid = nil
...
function MaskHuanHuaView:OpenCallBack()
	...
	self:Flush("maskhuanhua")
	self.index = 1
end

--点击激活按钮
function MaskHuanHuaView:OnClickActivate()
...
--点击升级按钮
function MaskHuanHuaView:OnClickUpGrade()
	if self.all_data[self.index] == nil then
		return
	end

	local attr_cfg = MaskData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
	if attr_cfg ~= nil then
		if attr_cfg.grade >= MaskData.Instance:GetSpecialImageMaxUpLevelById(attr_cfg.special_img_id) then
			return
		end
	...
	MaskHuanHuaCtrl.Instance:MaskSpecialImaUpgrade(self.all_data[self.index].image_id)
end

--点击使用当前形象
function MaskHuanHuaView:OnClickUseIma()
	if self.all_data[self.index] == nil then
		return
	end
	MaskCtrl.Instance:SendUseMaskImage(self.all_data[self.index].image_id + GameEnum.MOUNT_SPECIAL_IMA_ID)
end

function MaskHuanHuaView:OnClickListCell(mask_special_data, index, mask_cell)
	self.mask_special_image = mask_special_data
	mask_cell:SetHighLight(true)
	if self.index == index then return end
	self.index = index or 1
	self.item_id = mask_special_data.item_id
	self:SetSpecialImageAttr(mask_special_data, mask_special_data.image_id)

	local mount_special_image = ConfigManager.Instance:GetAutoConfig("ugs_mask_auto").special_img
...
--获取激活坐骑符数量
function MaskHuanHuaView:GetHaveProNum(item_id, need_num)
...
function MaskHuanHuaView:SetSpecialImageAttr(mask_special_data, index)
	if mask_special_data == nil then
		return
	end
	local upgrade_cfg = MaskData.Instance:GetSpecialImageUpgradeInfo(mask_special_data.image_id)
	local image_cfg = MaskData.Instance:GetSpecialImageCfg(index)
	...
	local change_name_list = Split(mask_special_data.image_name, "-")
	local change_name = (#change_name_list >= 2) and (change_name_list[1] .. " - " .. change_name_list[2]) or mask_special_data.image_name
	--self.mask_name:SetValue("<color=" .. SOUL_NAME_COLOR[item_cfg and item_cfg.color or 5] .. ">" .. change_name .. "</color>")
	if self.name_obj ~= nil then
		local bundle, asset = ResPath.GetAdvanceEquipIcon("hh_mask_" .. (image_cfg.title_res or 1))
	...
	local attr_cfg = MaskData.Instance:GetSpecialImageUpgradeInfo(index)
	if attr_cfg ~= nil then
		...
		self:GetHaveProNum(mask_special_data.item_id, upgrade_cfg.stuff_num)
	end

	local attr2 = CommonStruct.Attribute()
	local next_cfg = MaskData.Instance:GetSpecialImageUpgradeInfo(index, nil, true)
	...
	local data = {item_id = mask_special_data.item_id, is_bind = 0}
...
--设置激活按钮显示和隐藏
function MaskHuanHuaView:IsShowActivate(image_id)
...
--设置升级按钮显示和隐藏
function MaskHuanHuaView:IsShowUpGrade(image_id)
	if image_id == nil then
		return
	end
	local special_img_up = MaskData.Instance:GetSpecialImageUpgradeCfg()
	local info_list = MaskData.Instance:GetMaskInfo()
	local active_flag = info_list.active_special_image_flag
	-- local bit_list = bit:d2b(active_flag)
...
--升级按钮是否置灰
function MaskHuanHuaView:IsGrayUpgradeButton(index)
	if index == nil or index < 0 then return end
	--local mask_special_image = ConfigManager.Instance:GetAutoConfig("mask_auto").special_img
	--local _, mask_special_image = MaskData.Instance:GetShowSpecialInfo()
	local upgrade_cfg = MaskData.Instance:GetSpecialImageUpgradeInfo(self.all_data[index].image_id)
	if upgrade_cfg.grade < MaskData.Instance:GetSpecialImageMaxUpLevelById(self.all_data[index].image_id) then --GameEnum.MAX_UPGRADE_LIMIT
...
function MaskHuanHuaView:OnFlush(param_list)
	for k, v in pairs(param_list) do
		if k == "maskhuanhua" then
			...
			self.title_icon:SetAsset("uis/views/advanceview_images", "icon_maskhuanhua")
			--local mask_special_image = ConfigManager.Instance:GetAutoConfig("mask_auto").special_img
			--local mask_special_image = MaskData.Instance:GetShowSpecialInfo()
			...
			local upgrade_cfg = MaskData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
			local info_list = MaskData.Instance:GetMaskInfo()
			local active_flag = info_list.active_special_image_flag
			-- local active_flag = bit:d2b(active_flag)
			...
			-- if self.show_btn_right ~= nil then
			-- 	self.show_btn_right:SetValue(self.index < #mask_special_image)
			-- end
		end
	end
end
...
function MaskHuanHuaCell:OnFlush()
	...
	local active_flag = info_list.active_special_image_flag
	-- local bit_list = bit:d2b(active_flag)
	local _, special_data = MaskData.Instance:GetShowSpecialInfo()
...
function MaskHuanHuaCell:SetToggleGroup(toggle_group)
	self.root_node.toggle.group = toggle_group
end

function MaskHuanHuaCell:SetHighLight(enable)
