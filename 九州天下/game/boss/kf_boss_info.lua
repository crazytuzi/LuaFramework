...
function KfBossInfoView:__init()
	self.ui_config = {"uis/views/bossview","CrossBossInfo"}
...
function KfBossInfoView:LoadCallBack()
	...
	--获取组件
	-- self.cross_boss_view = self:FindObj("cross_boss_view")
	...
	-- self.panel_animator = self.cross_boss_view:GetComponent(typeof(UnityEngine.Animator))
	-- self.panel_animator:ListenEvent("RankState", BindTool.Bind(self.RankState, self))
...
function KfBossInfoView:OpenCallBack()
	...
	if info then
		...
		GuajiCtrl.Instance:MoveToPos(info.scene_id, info.flush_pos_x, info.flush_pos_y, 10, 10, nil, nil, true)
	end
end
...
function KfBossInfoItem:ClickAttr()
	...
	MoveCache.end_type = MoveEndType.Auto
	GuajiCtrl.Instance:MoveToPos(scene_id, self.data.pos_x, self.data.pos_y, 10, 10, nil, nil, true)
...
function KfBossInfoItem:SetState()
	...
	local function timer_func(elapse_time, total_time)
		...
		self.time_color:SetValue("#ff0000ff")
