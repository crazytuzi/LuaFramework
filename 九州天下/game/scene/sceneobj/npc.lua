...
function Npc:InitInfo()
	...
	self.general_res = npc_config.general_res
	self.beauty_res = npc_config.beauty_res
	if not self.monster_id or self.monster_id == "" then
		self.monster_id = 0
	end
	if not self.general_res or self.general_res == "" then
		self.general_res = 0
	end
	if not self.beauty_res or self.beauty_res == "" then
		self.beauty_res = 0
	end
...
function Npc:InitShow()
	...
	if self.role_res <= 0 then
		...
		elseif self.general_res ~= 0 then
			self:InitModel(ResPath.GetMingJiangRes(self.general_res))
		elseif self.beauty_res ~= 0 then
			self:InitModel(ResPath.GetGoddessNotLModel(self.beauty_res))
		else
...
function Npc:InitModel(bundle, asset)
	if AssetManager.Manifest ~= nil and not AssetManager.IsVersionCached(bundle) then
		self:ChangeModel(SceneObjPart.Main, bundle, asset)		--ResPath.GetNpcModel(4026001)
...
function Npc:UpdateTitle()
	local task_cfg = TaskData.Instance:GetNpcOneExitsTask(self:GetNpcId())
	if task_cfg and TaskData.Instance:GetIsHoldBeautyTask(task_cfg.task_id) ~= 2 then
		local status = TaskData.Instance:GetTaskStatus(task_cfg.task_id)
		if status == TASK_STATUS.CAN_ACCEPT then
		...
		if status == TASK_STATUS.COMMIT or status == TASK_STATUS.ACCEPT_PROCESS then
			if task_cfg.task_type == TASK_TYPE.RI then
	...
	if scene_type == SceneType.HunYanFb and self.vo.npc_type and self.vo.npc_type == SceneObjType.FakeNpc then
		local user_info, _ = MarriageData.Instance:GetHunyanQuestionUserInfo()
		if self.vo.npc_idx and self.vo.npc_idx == user_info.cur_question_idx then
			self:ChangeSpecailTitle(5)
		else
			self:ChangeSpecailTitle(-1)
		end
	end
...
function Npc:UpdateArrow()
	local vo = GameVoManager.Instance:GetMainRoleVo()
	local xycity_cfg = LianFuDailyData.Instance:GetXYCityGroupCfg(vo.server_group)
	local is_show = false
	if xycity_cfg ~= nil and self.vo.npc_id == xycity_cfg.midao_npc then
		local midao_data = LianFuDailyData.Instance:GetMiDaoIsOpen()
		if midao_data ~= nil and midao_data[vo.server_group + 1] ~= nil then
			is_show = midao_data[vo.server_group + 1] == 1
		end
	end

	self:GetFollowUi():ShowDeliveryArrow(is_show)
end