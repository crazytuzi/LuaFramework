...
function FollowObj:WadnerForce()
	...
	if target then
		...
		if self.last_wander_time + self.wander_cd < Status.NowTime then
			...
			wander_pos = u3d.v3Add(target:GetLuaPosition(), random_pos)
			self.last_wander_pos = wander_pos
		end
		local target_to_wander = u3d.v3Sub(wander_pos, target:GetLuaPosition())
		if u3d.v3Length(target_to_wander, false) > self.sqrt_wander_distance then
			wander_pos = u3d.v3Add(target:GetLuaPosition(), u3d.v3Mul(u3d.v3Normalize(target_to_wander), self.wander_distance))
			self.last_wander_pos = wander_pos
		end
		local can_move = true
		...
		if AStarFindWay:IsBlock(logic_x, logic_y) then
			can_move = false
			local distance = u3d.v3Length(u3d.v3Sub(wander_pos, target:GetLuaPosition()))
			distance = distance - 0.5
			while(distance > 0) do
				wander_pos = u3d.v3Add(target:GetLuaPosition(), u3d.v3Mul(u3d.v3Normalize(target_to_wander), distance))
				distance = distance - 0.5
				local logic_x, logic_y = GameMapHelper.WorldToLogic(wander_pos.x, wander_pos.z)
