...
function HeadFrameContent:SetInfo()
	...
	self.frame_name:SetValue("LV:" .. self.cur_data.level .. self.cur_data.name)
...
HeadFrameAttrGroup = HeadFrameAttrGroup or BaseClass(BaseCell)
...
function HeadFrameAttrGroup:OnFlush()
	...
	for i=1,3 do
		self.attrs[i]:SetData(self.data)
	end
end

HeadFrameAttr = HeadFrameAttr or BaseClass(BaseCell)

function HeadFrameAttr:__init()
	self.attr_value = self:FindVariable("AttrValue")
	self.is_max_level = self:FindVariable("IsMaxLevel")
	self.rise_value = self:FindVariable("RiseValue")
end
...
function HeadFrameAttr:SetIndex(index)
	self.index = index
end

function HeadFrameAttr:OnFlush()
	if nil == self.data or nil == next(self.data) then
		return
	end
	local cur_data = HeadFrameData.Instance:GetChooseData(self.index)
	local is_max_level = self.data.level == cur_data.max_level

	self.attr_value:SetValue(self.data.attrs[self.index])
	self.is_max_level:SetValue(is_max_level)

	if not is_max_level then
		local next_attr = HeadFrameData.Instance:GetAttrData(self.data.level + 1, cur_data.seq)
		self.rise_value:SetValue(next_attr.attrs[self.index] - self.data.attrs[self.index])
	end
end
