...
function PersonalGoalsView:__init()
	self.ui_config = {"uis/views/serveractivity/goals", "PersonalGoalsView"}
...
function PersonalGoalsView:FistOpenCallBack()
	if not self.effect_obj then
		...
		if prefab then
			...
			transform:SetParent(self.first_effect.transform, false)
			...
			if nil == self.timer_quest then
				self.timer_quest = GlobalTimerQuest:AddDelayTimer(function()
						...
						self.is_fist_open = false
						self.show_content:SetValue(true)
...
function PersonalGoalsView:AfterRewardCallBack()
	if not self.effect_obj then
		...
		if prefab then
			...
			PrefabPool.Instance:Free(prefab)
			local transform = obj.transform
			transform:SetParent(self.word_effect.transform, false)
			self.effect_obj = obj.gameObject
			self.is_load_effect = false
			...
			if nil == self.timer_quest then
				self.timer_quest = GlobalTimerQuest:AddDelayTimer(function()
						...
						self.show_word_content:SetValue(true)
						GlobalTimerQuest:CancelQuest(self.timer_quest)
						self.timer_quest = nil
				end, 1.5)
			end
