...
function ManyFuBenView:FlushTeamRoom()
	...
	if info then
			...
			if teammate_info and self.teammates[i] then
				...
				if not ScoietyData.Instance:IsLeaderById(teammate_info.role_id) then
					...
					if AvatarManager.Instance:isDefaultImg(teammate_info.role_id) == 0 then
						self.teammates[i].portrait_raw.gameObject:SetActive(false)
						self.teammates[i].portrait.gameObject:SetActive(true)
						local bundle, asset = AvatarManager.GetDefAvatar(PlayerData.Instance:GetRoleBaseProf(teammate_info.prof), false, teammate_info.sex)
						self.teammates[i].head:SetAsset(bundle, asset)
					else
						local callback = function (path)
							if not self.teammates[i] then
								return
							end
							local portrait_raw = self.teammates[i].portrait_raw
							local portrait = self.teammates[i].portrait
							local avatar_path_small = path or AvatarManager.GetFilePath(teammate_info.role_id, false)
						...
						AvatarManager.Instance:GetAvatar(teammate_info.role_id, false, callback)
					end
		...
		if cap_info then
			...
			-- 设置头像
			if AvatarManager.Instance:isDefaultImg(cap_info.role_id) == 0 then
				self.cap_info.portrait_raw.gameObject:SetActive(false)
				self.cap_info.portrait.gameObject:SetActive(true)
				local bundle, asset = AvatarManager.GetDefAvatar(PlayerData.Instance:GetRoleBaseProf(cap_info.prof), false, cap_info.sex)
				self.cap_info.head:SetAsset(bundle, asset)
			else
				local callback = function (path)
					local avatar_path_small = path or AvatarManager.GetFilePath(cap_info.role_id, false)
					self.cap_info.portrait_raw.raw_image:LoadSprite(avatar_path_small, function()
						self.cap_info.portrait_raw.gameObject:SetActive(true)
						self.cap_info.portrait.gameObject:SetActive(false)
					end)
				end
				AvatarManager.Instance:GetAvatar(cap_info.role_id, false, callback)
			end
...
function KuaFuFuBenRoomScrollCell:OnFlush()
	if self.data then
		...
		if self.data.menber_num >= 3 then
			self.count:SetValue(ToColorStr(self.data.menber_num, TEXT_COLOR.RED) .. "/3")
		else
