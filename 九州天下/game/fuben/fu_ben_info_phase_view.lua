...
function FuBenInfoPhaseView:__init()
	self.ui_config = {"uis/views/fubenview", "PhaseFBInFoView"}
...
function FuBenInfoPhaseView:LoadCallBack()
	...
	self.condition_color1 = self:FindVariable("ConditionColor1")
	self.condition_color2 = self:FindVariable("ConditionColor2")
	...
	for i = 1, 3 do
		...
		self.rewards[i]:SetInstanceParent(self.items[i])
	end
...
function FuBenInfoPhaseView:ReleaseCallBack()
	...
	self.show_panel = nil
	self.condition_color1 = nil
	self.condition_color2 = nil
	self.item_data = {}
end
...
function FuBenInfoPhaseView:SetPhaseFBSceneData()
	...
	local tbl = Split(fuben_cfg.monster_id, "|")
	...
	local monster_id1 = tonumber(tbl[1])
	local monster_id2 = tonumber(tbl[2])
	if monster_cfg[monster_id1].type == 0 then
		self.monster_name:SetValue(monster_cfg[monster_id1].name)
		self.boss_name:SetValue(monster_cfg[monster_id2].name)
	else
		self.monster_name:SetValue(monster_cfg[monster_id2].name)
		self.boss_name:SetValue(monster_cfg[monster_id1].name)
	end
		...
		if self.is_first_open then
			...
			for k, v in pairs(self.rewards) do
				if reward[k - 1] then
					...
					self.items[k]:SetActive(false)
					if not is_set_exp and fuben_cfg.reward_exp > 0 then
	...
	if fb_scene_info and next(fb_scene_info) then
		...
		if fb_scene_info.kill_allmonster_num - fb_scene_info.kill_boss_num >= fb_scene_info.total_allmonster_num - fb_scene_info.total_boss_num
			and fb_scene_info.kill_allmonster_num - fb_scene_info.kill_boss_num > 0 then
			self.condition_color1:SetValue(TEXT_COLOR.GREEN_4)
		else
			self.condition_color1:SetValue(TEXT_COLOR.RED)
		end
		if fb_scene_info.kill_boss_num >= fb_scene_info.total_boss_num and fb_scene_info.kill_boss_num > 0 then
			self.condition_color2:SetValue(TEXT_COLOR.GREEN_4)
		else
			self.condition_color2:SetValue(TEXT_COLOR.RED)
		end
	end
end
