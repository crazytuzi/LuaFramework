...
function PushSpecialView:LoadCallBack()
...
function PushSpecialItem:OnFlush()
	...
	local str = 0
	if OneLevelIsPass then
		str = Language.PushFb.NeedPassLastLevel
	else
		str = string.format(Language.PushFb.NeedPassOneLevel, push_fb_cfg.need_pass_chapter + 1, push_fb_cfg.need_pass_level + 1)
	end
	self.pass_cond:SetValue(str)
	...
	self.is_open:SetValue((data.chapter == 0 and data.level == 0 and OneLevelIsPass) or
						  data.chapter < special_info.pass_chapter or
						 (OneLevelIsPass and special_info.pass_level >= data.level and data.chapter == special_info.pass_chapter and (data.chapter ~= 0 or data.level ~= 0)))
	...
	if self.old_res_id ~= res_id then
		...
		self.boss_model_view:SetModelTransformParameter(DISPLAY_MODEL_TYPE[DISPLAY_TYPE.MONSTER], res_id, DISPLAY_PANEL.PUSH_BOSS)
	end
