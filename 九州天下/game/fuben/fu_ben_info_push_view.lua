...
function FuBenInfoPushView:__init()
	self.ui_config = {"uis/views/fubenview", "PushFbInfoView"}
...
function FuBenInfoPushView:LoadCallBack()
	...
	self.reward_tip_str = self:FindVariable("RewardTipStr")
	self.star_res_list = {}
	for i = 1, 3 do
		self.star_res_list[i] = self:FindVariable("StarRes" .. i)
	end
...
function FuBenInfoPushView:ReleaseCallBack()
	...
	self.reward_tip_str = nil
	self.star_res_list = nil
end
...
function FuBenInfoPushView:AddTime()
	if self.total_use_time == nil or self.total_use_time < 0 or nil == next(self.star_info_cfg) then return end
	...
	for i = 1, cur_star_num do
		if self.star_res_list[i] ~= nil then
			local bundle, asset = ResPath.GetImages("star_1001")
			self.star_res_list[i]:SetAsset(bundle, asset)
		end
	end

	for i = cur_star_num + 1, 3 do
		if self.star_res_list[i] ~= nil then
			local bundle, asset = ResPath.GetImages("star_1000")
			self.star_res_list[i]:SetAsset(bundle, asset)
		end
	end
	self.next_star_num:SetValue(cur_star_num - 1)
	if cur_star_num > 0 then
		--self.lbl_count_down:SetValue(string.format(Language.ExpFuBen.GreenText, TimeUtil.FormatSecond(self.star_info_cfg[cur_star_num] - self.total_use_time, 2)))
		self.lbl_count_down:SetValue(TimeUtil.FormatSecond(self.star_info_cfg[cur_star_num] - self.total_use_time, 2))
	else
		--self.lbl_count_down:SetValue(string.format(Language.ExpFuBen.GreenText, "00:00"))
...
function FuBenInfoPushView:SetCountDown()
	...
	if next(fb_info) and (fb_scene_info.param3 + 1 <= max_level) and fb_scene_info.is_pass == 1 and fb_info.star >= 1 and fb_scene_info.is_finish == 1 then
		...
		if ViewManager.Instance:IsOpen(ViewName.FuBenPushInfoView) then
			...
			-- local history_data = FuBenData.Instance:GetPushFBLeveLInfo(fb_scene_info.param1, fb_scene_info.param2, fb_scene_info.param3)
			local reward_data = fb_info.reward_item_list
			-- if history_data ~= nil and history_data.pass_star ~= nil and history_data.pass_star <= 0 and fb_scene_info.param1 == PUSH_FB_TYPE.PUSH_FB_TYPE_NORMAL then
			-- 	local level_cfg = FuBenData.Instance:GetPushFBInfo(fb_scene_info.param1, fb_scene_info.param2, fb_scene_info.param3)
			-- 	if level_cfg ~= nil and level_cfg.first_pass_reward ~= nil then
			-- 		reward_data = level_cfg.first_pass_reward
			-- 	end
			-- end

			local first_falg = FuBenData.Instance:GetPushFirstReward()
			if first_falg then
				local level_cfg = FuBenData.Instance:GetPushFBInfo(fb_scene_info.param1, fb_scene_info.param2, fb_scene_info.param3)
				if level_cfg ~= nil and level_cfg.first_pass_reward ~= nil then
					reward_data = level_cfg.first_pass_reward
			...
			if not self.is_first_open and (have_enter_times or not next_is_pass) and (fb_scene_info.param1 == 0 and role_level >= fuben_next_cfg.enter_level_limit) or
				...
				self.upgrade_timer_quest = GlobalTimerQuest:AddDelayTimer(function()
					...
					ViewManager.Instance:Open(ViewName.FuBenFinishStarNextView, nil, "finish", {data = reward_data, pass_time = fb_scene_info.pass_time_s, star = fb_info.star})
				end, 2)
			elseif not self.is_first_open then
				self.upgrade_timer_quest = GlobalTimerQuest:AddDelayTimer(function()
					ViewManager.Instance:Open(ViewName.FBFinishStarView, nil, "finish", {data = reward_data, pass_time = fb_scene_info.pass_time_s, star = fb_info.star})
				end, 2)
			else
				if self.count_down ~= nil then
					CountDown.Instance:RemoveCountDown(self.count_down)
...
function FuBenInfoPushView:SetPhaseFBSceneData()
	...
	local fb_chapter = string.format(Language.PushFb.ChapterName, CommonDataManager.GetDaXie(fb_scene_info.param2 + 1, nil, true))
	local name_str = fb_chapter .. fuben_cfg.fb_name
	self.fb_name:SetValue(name_str)

	local capability = GameVoManager.Instance:GetMainRoleVo().capability
	local str_fight_power = string.format(Language.FB.ShowGreenNum, fuben_cfg.capability_show)
	if capability < fuben_cfg.capability_show then
		str_fight_power = string.format(Language.FB.ShowRedNum, fuben_cfg.capability_show)
	end
	...
	local first_falg = FuBenData.Instance:GetPushFirstReward()
	local read_falg = false
	if first_falg then
		read_falg = true
	else
		read_falg = history_star <= 0
	end	
	self.tongguan_des:SetValue(read_falg and Language.FB.FirstPassReward or Language.FB.DailyPassReward)
	if self.reward_tip_str ~= nil then
		self.reward_tip_str:SetValue(not read_falg and Language.PushFb.RewardDayTip or Language.PushFb.RewardFirstTip)
	end
	...
	local reward_cfg = read_falg and fuben_cfg.first_pass_reward or fuben_cfg.daily_reward
	self.item_data = {}
	for k, v in pairs(self.item_cells) do
		v:SetActive(false)
...
function FuBenInfoPushView:OnFlush(param_t)
	if Scene.Instance:GetSceneType() == SceneType.PushFuBen and self:IsOpen() then
