...
function FuBenVipView:RefreshMountCell(cell, data_index)
	...
	if fb_info and next(fb_info) then
		...
		vip_view:SetData(data,data_index)
		for i = 1, 3 do
			local item_data = {}
...
function VipListView:__init(instance)
	...
	self.num_count = self:FindVariable("NumCount")
	self.show_num = self:FindVariable("ShowNum")
end
...
function VipListView:SetData(data,index)
	...
	local num_count = FuBenData.Instance:GetVipFBInfo()[index + 1].today_times
	local cfg = ConfigManager.Instance:GetAutoConfig("vipfbconfig_auto").levelcfg
	self.num_count:SetValue(cfg[index +1].free_times - num_count)
	self.show_num:SetValue(cfg[index +1].free_times - num_count == 0)
end