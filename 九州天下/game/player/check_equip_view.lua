...
function CheckEquipView:__init()
	self.ui_config = {"uis/views/player","CheckEquipView"}
end
...
function CheckEquipView:OnEquipDetail(i)
	...
	TipsCtrl.Instance:OpenItem({item_id = show_item_id, show_star_num = star, speacal_from = is_show_star})
end
...
function CheckEquipView:OnFlush()
	...
	local item_cfg_1 = ItemData.Instance:GetItemConfig(self.equip_id1)
	local item_cfg_2 = ItemData.Instance:GetItemConfig(self.equip_id2)
	local item_cfg_3 = ItemData.Instance:GetItemConfig(self.equip_id3)
	self.icon_path_list[1]:SetAsset(ResPath.GetItemIcon(item_cfg_1.icon_id))
	self.icon_path_list[2]:SetAsset(ResPath.GetItemIcon(item_cfg_2.icon_id))
	self.icon_path_list[3]:SetAsset(ResPath.GetItemIcon(item_cfg_3.icon_id))
	for i = 1, 6 do
		local star = INDEXTOSTAR[i]
		local show_item_id = 0
		local is_show_star =  star > 0
		...
		local data = {item_id = show_item_id, show_star_num = star, speacal_from = is_show_star}
