...
function FunctionGuide:__init()
	...
	self.scene_enter_cfg = ListToMap(ConfigManager.Instance:GetAutoConfig("other_config_auto").scene_enter_record, "scene_id")
...
function FunctionGuide:ClearFbHangupGuide()
	self.is_hangup_guided_in_cur_fuben = false
	self.normal_guide_view:Close()
	self.girl_guide_view:Close()
end
...
function FunctionGuide:EndGuide()
	-- print("结束引导-------------->")
	if self.is_guiding and self.guide_view ~= ViewName.Main and not ViewManager.Instance:IsOpen(self.guide_view) then
		self.guide_view = ""
...
function FunctionGuide:ClickGuideAreaHandler()
	local step_cfg = self:GetGuideStepCfg(self.cur_guide_cfg, self.cur_guide_step)
	if nil == step_cfg or step_cfg.ui_name ~= GuideUIName.Tab then
		self:StartNextStep()
	end
end

function FunctionGuide:ClickIntroduceHandler()
	self:StartNextStep()
end

function FunctionGuide:OnSceneLoadComplete(old_scene_type, new_scene_type)
	-- if old_scene_type ~= SceneType.Common and new_scene_type == SceneType.Common then
	-- 	guide_cfg = self:GetGuideCfgByTrigger(GuideTriggerType.OutFb, old_scene_type)
	-- 	if guide_cfg ~= nil then
	-- 		if guide_cfg.with_param ~= "" then
	-- 			if TaskData.Instance:GetTaskIsAccepted(tonumber(guide_cfg.with_param)) then
	-- 				self:SetCurrentGuideCfg(guide_cfg)
	-- 			end
	-- 		else
	-- 			self:SetCurrentGuideCfg(guide_cfg)
	-- 		end
	-- 	end
	-- 	self:ClearFbHangupGuide() --清除,以下次进副本仍能引导
	-- end
end

function FunctionGuide:GetFirstEnterScene(scene_id)
	if scene_id and self.scene_enter_cfg[scene_id] then
		return self.scene_enter_cfg[scene_id]
	end
	return nil
end

function FunctionGuide:SetSceneEnterFirstGuide(scene_id)
	if scene_id then
		local guide_cfg = self:GetGuideCfgByTrigger(GuideTriggerType.FirstEnterFb, scene_id)
		if guide_cfg then
			self:SetCurrentGuideCfg(guide_cfg)
		end
	end
...
function FunctionGuide:ClickCallback()
	self.normal_guide_view:Close()
	self:StartNextStep()
end
...
function FunctionGuide:GetGuideCfgByTrigger(trigger_type, trigger_param)
	for k,v in pairs(self.guide_list) do
		if v.trigger_type == trigger_type then
			if v.trigger_type == GuideTriggerType.AcceptTask or v.trigger_type == GuideTriggerType.CommitTask then
...
function FunctionGuide:CheckConfigGuide()
	if self.is_audit_version == 1 then
		return
	end

	--配置的引导
	if self.cur_guide_cfg == nil or self.is_pausing then
		self.guide_view = ""
		return
	end

	local step_cfg = self:GetGuideStepCfg(self.cur_guide_cfg, self.cur_guide_step)
	if step_cfg and step_cfg.module_name ~= "" and step_cfg.module_name ~= "Main" and step_cfg.unuseful ~= 1 then
		self.guide_view = step_cfg.module_name
	end
