...
function YunbiaoData:GetTotalFreeHusonTimes()
	return ConfigManager.Instance:GetAutoConfig("husongcfg_auto").other[1].free_husong_times or 0
end
...
function YunbiaoData:GetRewardCfg()
	local list = {}
	local item_data = ConfigManager.Instance:GetAutoConfig("husongcfg_auto").task_reward_list
	for i=1,5 do
		local factor = ConfigManager.Instance:GetAutoConfig("husongcfg_auto").task_reward_factor_list[i] and ConfigManager.Instance:GetAutoConfig("husongcfg_auto").task_reward_factor_list[i].factor or 0
		local num = math.floor(item_data[1].reward_item[0].num * factor / 100)
		table.insert(list, num)
	end
	return list
end

function YunbiaoData:SetHuSongPreColor(husong_color)
	self.husong_color = husong_color
end

function YunbiaoData:GetHuSongPreColor()
	return self.husong_color
end

function YunbiaoData:SetHuSongGiveTimes(give_times)
	self.give_times = give_times
end

function YunbiaoData:GetHuSongGiveTimes()
	return self.give_times
end

function YunbiaoData:GetYunBiaoStuffCount()
	local complete_times = DayCounterData.Instance:GetDayCount(DAY_COUNT.DAYCOUNT_ID_ACCEPT_HUSONG_TASK_COUNT) 
	if complete_times > 0 then
		return 1
	end

	return math.ceil(self.give_times)
end