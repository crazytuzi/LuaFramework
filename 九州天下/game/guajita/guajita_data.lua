...
function GuaJiTaData:__init()
	...
	self.rune_auto_layer_cfg = ListToMap(self.rune_auto_cfg.layer, "fb_layer")
	self.patafb_level_cfg = ListToMap(ConfigManager.Instance:GetAutoConfig("rune_tower_cfg_auto").layer, "fb_layer")
...
function GuaJiTaData:GetRuneTowerLayerCfgByLayer(layer)
	...
	return self.rune_auto_layer_cfg[layer] or {}
end
...
function GuaJiTaData:GetSpecialRewardLevel(level)
	...
	for k, v in pairs(self:GetRuneTowerFBLevelCfg()) do
		if v.sp_show > 0 and level < v.fb_layer then
			return v
		end
	end
	return nil
