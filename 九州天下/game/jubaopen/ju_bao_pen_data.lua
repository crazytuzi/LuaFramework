...
function JuBaoPenData:GetNeedPutGold(index)
	local config_cornucopia_rate = self:GetCornucopiaRate()
	local need_put_gold = 0
	if config_cornucopia_rate then
		for k,v in pairs(config_cornucopia_rate) do
			if v.lun == index then
				need_put_gold = v.need_put_gold or 0
				break
			end
		end
	end
	return need_put_gold
end
...
function JuBaoPenData:GetMaxLun()
	local max_lun = 0
	local cornucopia = self:GetCornucopiaRate()
	if cornucopia then
		max_lun = #cornucopia
	end
	return max_lun
...
function JuBaoPenData:CheckIsShow()
	...
	if self.reward_lun <= max_lun and ActivityData.Instance:GetActivityIsOpen(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_CORNUCOPIA)
		and OpenFunData.Instance:CheckIsHide("jubaopen") and not is_first then
		return true
	end
	return false
end

function JuBaoPenData:SetFirstOpen(state)
	self.is_first_open = state
end

function JuBaoPenData:IsMaxTime()
	local max_lun = self:GetMaxLun()
	return self.reward_lun >= max_lun
end