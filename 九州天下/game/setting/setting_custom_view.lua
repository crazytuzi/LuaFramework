...
function SettingCustomView:__init(instance)
	...
	self.contact_way = self:FindObj("contact_way")
	self.audit_limit = self:FindVariable("audit_limit")
	...
	self.audit_limit:SetValue(is_show)
end
...
function SettingCustomView:OpenCustom()
	...
	self.contact_way.input_field.text = ""
end
...
function SettingCustomView:SendClick()
	...
	elseif self.content_input.input_field.text == "" then
		...
		list.issue_contact_way = self.contact_way.input_field.text
