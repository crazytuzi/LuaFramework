...
function AchieveData:__init()
	...
	self.title_cfg = ConfigManager.Instance:GetAutoConfig("chengjiu_auto").title
	self.reward_id_cfg = ListToMap(ConfigManager.Instance:GetAutoConfig("chengjiu_auto").reward, "id")
	...
	self.reward_info_list = ConfigManager.Instance:GetAutoConfig("chengjiu_auto").reward
...
function AchieveData:GetAchieveTitleDataByLevel(level)
	...
	for k,v in pairs(self.title_cfg) do
		...
		if nil ~= self.reward_list[v.id] then
			...
			if cfg ~= nil then
				if v.prog_type == 0 then
					...
					if self.reward_list[v.id].process >= cfg.param1 then
...
function AchieveData:GetThirdRewardDataList(client_type, second_index)
	...
	for i, v in ipairs(self.new_client_cfg[client_type][second_index]) do
		...
		local cfg = self.reward_id_cfg[v.id]
		if nil ~= self.reward_list[v.id] and cfg ~= nil then
			reward_data.icon_id =  cfg.icon_item_id
			reward_data.flag = self.reward_list[v.id].flag

			local param_1 = 1
			local param_2 = cfg.param2
			local process = 0
			...
			elseif (v.prog_type == 2 and self.reward_list[v.id].process == cfg.param1)			-- 进度类型2 激活女神或精灵 服务端需要道具id 所以用道具id判断
				or (v.prog_type ~= 2 and self.reward_list[v.id].process >= cfg.param1) then
				process = 1
				param_1 = 1
			end
