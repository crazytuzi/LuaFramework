...
function KuaFu1v1ViewLoser:__init(instance)
	self.ui_config = {"uis/views/kuafu1v1","KuaFu1v1Loser"}
...
function KuaFu1v1ViewLoser:OnFlush()
	...
	if info then
		...
		self:RemoveDelayTime()
		self.delay_time = GlobalTimerQuest:AddDelayTimer(function() self:SetNextInfo(score, add_score, current_config, next_config) end, 0.1)
	end
end

function KuaFu1v1ViewLoser:RemoveDelayTime()
...
function KuaFu1v1ViewLoser:SetNextInfo(score, add_score, current_config, next_config)
	...
	if current_config and current_config == new_current_config then
		if next_config then
			local temp = next_config.rank_score - current_config.rank_score
