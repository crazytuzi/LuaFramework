LuckyBoxData = LuckyBoxData or BaseClass()

function LuckyBoxData:__init()
	if LuckyBoxData.Instance ~= nil then
		ErrorLog("[LuckyBoxData] Attemp to create a singleton twice !")
	end
	LuckyBoxData.Instance = self
	self.is_can_play_ani = true
	self.money_tree_free_timestamp = 0
	self.is_shield = false
	RemindManager.Instance:Register(RemindName.LuckyBox, BindTool.Bind(self.GetZhuanCount, self))
end

function LuckyBoxData:__delete()
	RemindManager.Instance:UnRegister(RemindName.LuckyBox)
	LuckyBoxData.Instance = nil
end

function LuckyBoxData:GetIsShield()
	return self.is_shield
end

function LuckyBoxData:SetIsShield(is_shield)
	self.is_shield = is_shield
end

function LuckyBoxData:CanGetRewardBySeq(index)
...
function LuckyBoxData:GetRewardBySeq(seq)
	local seq_list = self:GetGridLotteryTreeAllRewardData()
...
function LuckyBoxData:GetOtherCfg()
	return ServerActivityData.Instance:GetCurrentRandActivityConfig().yaoqianshu_3
end

local zzl_cur_day = 0
function LuckyBoxData:GetZhuanZhuanLeOtherCfg()
...
function LuckyBoxData:SetZhuanZhuanLeInfo(protocol)
	self.money_tree_info = protocol
end

function LuckyBoxData:GetZhuanZhuanLeInfo()
...
function LuckyBoxData:SetMoneyTreeInfo(protocol)
...
function LuckyBoxData:GetZhuanZhuanLeKeyNum()
...
function LuckyBoxData:GetZhuanZhuanLFreeInterval()
	local cfg = self:GetZhuanZhuanLeOtherCfg()
...
function LuckyBoxData:GetZhuanZhuanLFreeTotalTimes()
	local cfg = self:GetZhuanZhuanLeOtherCfg()
	local free_time = 0
...
function LuckyBoxData:GetGridLotteryTreeAllRewardData()
	local act_cfg = self:GetOtherCfg()
	local open_day = TimeCtrl.Instance:GetCurOpenServerDay()
...
function LuckyBoxData:GetGridLotteryTreeRewardData()
	local act_cfg = self:GetOtherCfg()
...
-- 保持玩家领取累计奖励的seq
function LuckyBoxData:SetLinRewardSeq(seq_index)
	self.seq_index = seq_index
end

function LuckyBoxData:GetLinRewardSeq()
	return self.seq_index or 0
end

function LuckyBoxData:SetFreeTime(day_count)
	self.day_count = day_count
end

function LuckyBoxData:GetFreeTime()
	return self.day_count or 0
end

function LuckyBoxData:SetAniState(value)
	self.is_can_play_ani = not value
end

function LuckyBoxData:GetAniState(value)
	return self.is_can_play_ani
end

function LuckyBoxData:GetMianFeiTime()
	-- 下次免费时间
...
function LuckyBoxData:GetRewardInfo()
...
function LuckyBoxData:GetZhuanCount()
	...
	if ActivityData.Instance:GetActivityIsOpen(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MONEY_TREE_3) and nil == self.remind_timer then
...
function LuckyBoxData:ZhuanZhaunLePoindRemind()
	local remind_num = self:GetZhuanCount()
	ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MONEY_TREE_3,remind_num > 0)
end

--返回活动结束时间
function LuckyBoxData:GetActEndTime()
	local act_info = ActivityData.Instance:GetActivityStatuByType(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MONEY_TREE_3)
	if act_info then
...
function LuckyBoxData:UpdateRemind()
	RemindManager.Instance:Fire(RemindName.LuckyBox)
	ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MONEY_TREE_3,true)
end
