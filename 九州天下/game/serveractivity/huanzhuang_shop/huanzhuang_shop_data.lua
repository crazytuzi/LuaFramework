...
function HuanzhuangShopData:GetHuanZhuangShopCfg()
	...
	return ActivityData.Instance:GetRandActivityConfig(rand_act_cfg.magic_shop, ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_SINGLE_CHARGE_2)
end
...
function HuanzhuangShopData:GetRAMagicShopAllInfo(protocol)
	return self.activity_info
end

function HuanzhuangShopData:ShowHuanZhuangShopPoint()
	local is_show = MainUICtrl.Instance:GetHuanZhuangShopActivity()
	if not is_show then
		return 0
	end
...
function HuanzhuangShopData:MainuiOpenCreate()
	...
	MainUICtrl.Instance:ChangeHuanZhuangShopBtn(is_open and GameVoManager.Instance:GetMainRoleVo().level >= 130)
...
function HuanzhuangShopData:HuanZhuangShopOpen()
	self.is_first = false
end