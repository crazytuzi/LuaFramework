...
function JinYinTaData:GetHistoryRewardInfo(reward_index)
	local level_lottery_reward = self:GetRewardCfgByDay()
	if level_lottery_reward[reward_index + 1] then
		local reward_item = level_lottery_reward[reward_index + 1].reward_item
...
function JinYinTaData:GetRewardCfgByDay()
	...
	local temp_tab = {}
	if rand_config == nil then
		return temp_tab
	end

	local temp_index = 1
	for k,v in pairs(rand_config) do
		if v.opengame_day then
...
function JinYinTaData:GetLeijiJiangli()
	...
	rand_config = ActivityData.Instance:GetRandActivityConfig(rand_config)
	return rand_config or {}
end

--返回活动结束时间 
function JinYinTaData:GetActEndTime()
	local act_info = ActivityData.Instance:GetActivityStatuByType(ACTIVITY_TYPE.RAND_JINYINTA, ACTIVITY_TYPE.RAND_JINYINTA)
	if act_info then
...
function JinYinTaData:FlushHallRedPoindRemind()
 ...
 -- 	if free_time == 0 then 
 -- 		-- ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_JINYINTA,false)
 -- 		return
 -- 	end
