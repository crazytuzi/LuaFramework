...
function CollectiveGoalsData:__init()
	...
	self.goals_cfg = ListToMap(self:GetRoleGoalCfg().battlefield_goal, "act_sep")
...
function CollectiveGoalsData:GetRoleGoalCfg()
	if not self.role_goal_cfg then
		self.role_goal_cfg = ConfigManager.Instance:GetAutoConfig("rolegoalconfig_auto")
	end
	return self.role_goal_cfg
end
...
function CollectiveGoalsData:GetNextTime()
	local server_open_day = TimeCtrl.Instance:GetCurOpenServerDay()
	if server_open_day > 3 then return end
	local server_time = os.date('*t', TimeCtrl.Instance:GetServerTime())
	local activity_time_cfg = self:GetActTimeList()[server_open_day]
	if activity_time_cfg == nil then return end
	local activity_hour = activity_time_cfg[1] or 0 
	local activity_min = activity_time_cfg[2] or 0
	...
	if server_open_day == 3 and server_time > activity_time then
		time_str1 = ""
		time_str2 = ""
		self.flag = 0
		is_flush = true
