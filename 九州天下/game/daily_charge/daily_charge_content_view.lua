...
function DailyChargeContentView:SetModel(res)
	...
	self.model:SetMainAsset(ResPath.GetHunQiModel(res))
...
function DailyChargeContentView:RefreshCell(cell, cell_index)
	...
	the_cell:OnFlush(cell_index, DailyChargeData.Instance:GetTotalChongZhiJiu())
end
...
function DailyChargeContentView:OpenCallBack()
	...
	if list and list[32] == 1 and list[31] ~= 1 then
		...
		self.select_item_info = DailyChargeData.Instance:GetChongZhiReward(DailyChargeData.Instance:GetTotalChongZhiJiu()).select_reward_item[0]
	else
		self.charge_toggle_10.toggle.isOn = true
		self.chongzhi_state = DailyChargeData.Instance:GetTotalChongZhiYi()
		self.select_item_info = DailyChargeData.Instance:GetChongZhiReward(self.chongzhi_state).select_reward_item[0]
	end
	self:FlushBtnState()
	self:SetBtnText()
	self:FlushRedPoints()
...
function DailyChargeContentView:FlushRedPoints()
	...
	local recharge = recharge_cfg.today_recharge or 0
	local reward_flag_list = recharge_cfg.daily_chongzhi_fetch_reward_flag_list or {}

	if self.show_left_red_point then
		self.show_left_red_point:SetValue(recharge >= DailyChargeData.Instance:GetTotalChongZhiYi() and (next(reward_flag_list) and reward_flag_list[32 - 0] == 0))
	end

	if self.show_right_red_point then
		self.show_right_red_point:SetValue(recharge >= DailyChargeData.Instance:GetTotalChongZhiJiu() and (next(reward_flag_list) and reward_flag_list[32 - 1] == 0))
	end
end

function DailyChargeContentView:FlushBtnState()
	local recharge_cfg = DailyChargeData.Instance:GetChongZhiInfo()
	local recharge = recharge_cfg.today_recharge
	...
	if recharge < self.chongzhi_state then
		...
		local index = 0
		if self.chongzhi_state == DailyChargeData.Instance:GetTotalChongZhiYi() then
			index = 0
		elseif self.chongzhi_state == DailyChargeData.Instance:GetTotalChongZhiJiu() then
			index = 1
		end
