...
function KuaFuXiuLuoTowerFuBenInfoView:__init()
	self.ui_config = {"uis/views/kuafuxiuluotower","XiuLuoFuBenInfoView"}
...
function KuaFuXiuLuoTowerFuBenInfoView:ReleaseCallBack()
	...
	if self.layer_tips_show then
		GlobalTimerQuest:CancelQuest(self.layer_tips_show)
		self.layer_tips_show = nil
	end
end
...
function KuaFuXiuLuoTowerFuBenInfoView:OnLayerChange(data)
	...
	if data.is_drop_layer == 1 then
		...
		self.change_level:SetValue(CommonDataManager.GetDaXie(KuaFuXiuLuoTowerData.Instance:GetCurrentLayer() - 1))
	else
		--升层
	...
	if nil ~= self.layer_tips_show then
		GlobalTimerQuest:CancelQuest(self.layer_tips_show)
	end
	self.layer_tips_show = GlobalTimerQuest:AddDelayTimer(function()
...
function KuaFuXiuLuoTowerFuBenInfoView:OnFlush()
	...
	self.is_drop_level:SetValue(is_drop_level)
...
function XiuLuoTaskView:Flush()
	...
	if boss_num > 0 then
		...
		self.boss_state:SetValue(string.format(Language.XiuLuo.BossState[1], self.boss_name))
