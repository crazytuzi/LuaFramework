...
function KuaFuXiuLuoTowerData:__init()
	...
	self.layer_index_cfg = ListToMap(self.xiuluo_cfg.show_cfg, "show_id")
end
...
function KuaFuXiuLuoTowerData:GetLayerCfgByLayer(layer)
	return self.layer_index_cfg[layer] or nil
end
...
function KuaFuXiuLuoTowerData:GetBossReward()
	local config = ConfigManager.Instance:GetAutoConfig("kuafu_rongyudiantang_auto").other[1]
	local temp_list = Split(config.boss_diao, ",")
	local reward_list = {}
	for k,v in ipairs(temp_list) do
		local temp = Split(v, ":")
		reward_list[k] = {item_id = tonumber(temp[1]), num = tonumber(temp[2]), is_bind = tonumber(temp[3])}
	end
	return reward_list
end
