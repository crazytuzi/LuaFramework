...
function GuildBoxView:__init(instance)
	...
	self.show_high_light = {}
	for i = 0, 4 do
		self.show_high_light[i] = self:FindVariable("ShowHighLight" .. i)
	end
    ...
    RemindManager.Instance:Bind(self.remind_change, RemindName.GuildTabBoxWaBao)
    RemindManager.Instance:Bind(self.remind_change, RemindName.GuildTabBoxXieZu)
end
...
function GuildBoxView:OnItemDataChange(change_item_id, change_item_index, change_reason, put_reason, old_num, new_num)
	if put_reason == PUT_REASON_TYPE.PUT_REASON_GUILD_BOX_REWARD then
		TipsCtrl.Instance:OpenGuildRewardView({item_id = change_item_id, num = new_num})
	end
end
...
function GuildBoxView:OnFlush()
	...
	if self.box_info then
			...
			for i = 1, MAX_GUILD_BOX_COUNT do
				if self.box_info.info_list[i] then
					if self.box_info.info_list[i].is_reward == 0 and self.box_info.info_list[i].open_time == 0 then
						...
						self.show_high_light[0]:SetValue(self.box_info.info_list[i].box_level==0)
						self.show_high_light[1]:SetValue(self.box_info.info_list[i].box_level==1)
						self.show_high_light[2]:SetValue(self.box_info.info_list[i].box_level==2)
						self.show_high_light[3]:SetValue(self.box_info.info_list[i].box_level==3)
						self.show_high_light[4]:SetValue(self.box_info.info_list[i].box_level==4)
		...
		self.level_up_count:SetValue(math.max(self.other_config.box_free_up_count - self.box_info.uplevel_count, 0))
		self.select_color = 4
		self:OnClickColor(self.select_color)
	...
	-- for i = 0,4 do
	-- 		self.show_high_light[i]:SetValue(self.box_info.info_list[1].box_level==i)
	-- 	end
	self:FlushAssist()
	self:StartCountDown()
end
...
-- 红点回调
function GuildBoxView:RemindChangeCallBack(remind_name, num)
	local has_box_active = GuildData.Instance:GetRewardBoxIsActive()
    if RemindName.GuildTabBoxWaBao == remind_name then
        self.has_rest_count:SetValue(num > 0 and (not has_box_active))
    elseif RemindName.GuildTabBoxXieZu == remind_name then
        self.has_assist:SetValue(num > 0)
    end
end
...
function GuildBoxView:LevelUpFree()
	if self.rest_box_count == 0 then
		SysMsgCtrl.Instance:ErrorRemind(Language.Guild.MaxBoxLvUp)
		return
	end
	...
	self.level_up_count:SetValue(math.max(self.other_config.box_free_up_count - self.temp_level_up_count, 0))
...
function GuildBoxViewScrollCell:ListenAllEvent()
	self:ListenEvent("OnClick",
		function() GuildCtrl.Instance.view.box_view:OpenBox(self.data) end)
end
...
function GuildBoxView:FlushPreview()
	 local config = GuildData.Instance:GetBoxConfig()
	 if config then
	 	for i = 1, 5 do
	 		local item_id = config[i].item_reward.item_id
	 		local num = config[i].item_reward.num
	 		self.preview_cell[i].cell:SetData({item_id = item_id, num = num})
	 	end
	 end
end
