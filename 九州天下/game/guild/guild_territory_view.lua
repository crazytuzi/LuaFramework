...
function GuildTerritoryView:OnClickBox(index)
    ...
    if index == 5 then
        ...
        if post ~= GuildDataConst.GUILD_POST.TUANGZHANG then
            if #guild_president_extra_reward > 0 then
                TipsCtrl.Instance:ShowRewardView(guild_president_extra_reward)
            end
    ...
    if index == 5 then
        ...
        if gong_xian < self.gong_xian_config[index] then
...
function GuildTerritoryView:ShakeHuiZhangBox()
    if self.shake_huizhang_box then
        self.anim_list[5]:SetTrigger("Shake")
    end
end
