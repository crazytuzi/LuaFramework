...
function GuildAltarView:__init(instance)
	...
	self.cur_capability = self:FindVariable("CurCapability")
	self.next_capability = self:FindVariable("NextCapability")
	self.show_cur_capability = self:FindVariable("ShowCurCapability")
...
function GuildAltarView:FlushDetails(skill_index)
	...
	if config then
		...
		if gongxian >= config.uplevel_gongxian then
			self.contribution:SetValue(ToColorStr(gongxian_str, TEXT_COLOR.GREEN_4) .."/" .. config.uplevel_gongxian)
		else
...
function GuildAltarView:SetCurrentSkillInfo(info)
	self.current_point:SetValue(Language.Common.AttrNameNoUnderline[SkillEnum[self.select_index]].. ": " .. ToColorStr("+", TEXT_COLOR.GREEN_4) ..ToColorStr(info[SkillEnum[self.select_index]], TEXT_COLOR.GREEN_4))
	local value = {}
	...
	--self.show_cur_capability:SetValue(capability ~= 0)
	self.show_cur_capability:SetValue(true)
end

-- 设置下一等级技能的信息
function GuildAltarView:SetNextSkillInfo(info)
	self.max_level:SetValue(false)
	self.next_point:SetValue(Language.Common.AttrNameNoUnderline[SkillEnum[self.select_index]].. ": " .. ToColorStr("+", TEXT_COLOR.GREEN_4) ..ToColorStr(info[SkillEnum[self.select_index]], TEXT_COLOR.GREEN_4))
	local value = {}
	value[SkillEnum[self.select_index]] = info[SkillEnum[self.select_index]]
	local capability  = CommonDataManager.GetCapabilityCalculation(value)
	self.next_capability:SetValue(capability)
end
