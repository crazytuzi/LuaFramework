...
function ShenGePreviewView:__init()
    self.ui_config = {"uis/views/shengeview", "ShenGePreviewView"}
...
function ShenGePreviewView:FlushView()
	...
	for i = 1, 6 do
		for quality = 2, 5 do
			...
			local dataCo = TableCopy(data)
			if dataCo ~= nil then
				dataCo.item_id = item_id
				table.insert(self.list, dataCo)
			end
		end
	end
...
function ShenGePreviewGroupCell:SetImage(data_index)
	...
	self.image:SetAsset(ResPath.GetItemIcon(item_cfg.icon_id))
end
...
function ShenGeAnalyzeItemCell:__init()
	...
	self.item = ItemCell.New(self:FindObj("ItemCell"))
...
function ShenGeAnalyzeItemCell:OnFlush()
	...
	local level_str = string.format(Language.Rune.LevelDes, level_color, self.data.level)
	...
	local color_str = ToColorStr(cap_attr, TEXT_COLOR.GREEN)
	self.attr_text_1:SetValue(Language.ShenGe.ZhanLi .. color_str)
	...
	-- 	self.show_text_2:SetValue(true)
	-- else
	-- 	self.attr_text_2:SetValue("")
	-- 	self.show_text_2:SetValue(false)
	-- end
