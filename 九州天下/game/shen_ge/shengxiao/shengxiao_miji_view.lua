...
function ShengXiaoMijiView:__init()
	self.ui_config = {"uis/views/shengeview", "ShengXiaoMiji"}
...
function ShengXiaoMijiView:StarRoller()
	if self.anim_countdown == nil then
		...
		self.anim_countdown = GlobalTimerQuest:AddRunQuest(BindTool.Bind(self.ShowAnim, self), 0.1)
	end
end
...
function ShengXiaoMijiView:FlushStudyIcon()
	if self.study_data then
		...
		self.select_effect_path:SetAsset(ResPath.GetEffect(FEECTBYLEVEL[miji_cfg.level]))
	end
end
...
function MijiCell:SetData(data)
	...
	if data.value >= 0 then
		...
		self.effect_path:SetAsset(ResPath.GetEffect(FEECTBYLEVEL[miji_cfg.level]))
	end
