...
function ShengXiaoUpLevelView:FlushPointEffect()
	...
	for k,v in pairs(point_effect_pos_cfg) do
		local bundle, asset = "effects2/prefab/ui/ui_guangdian1_01_prefab", "UI_guangdian1_01"
...
function ShengXiaoUpLevelView:FlushRightInfo()
	...
	if self.cur_level > 0 then
		...
		self.btn_text:SetValue(Language.ShengXiao.Active)
	...
	if self.cur_level >= GameEnum.CHINESE_ZODIAC_LEVEL_MAX_LIMIT then
		self.lbl_cost_desc:SetValue(Language.ShengXiao.Max)
	else
...
function ShengXiaoUpLevelView:FlushLeftInfo()
	...
	self.show_enter_effect:SetValue(is_open and OpenFunData.Instance:CheckIsHide("shengxiao_uplevel"))
	self.enter_btn_detail:SetValue(is_open and Language.Boss.HasRefresh or Language.ShengXiao.FlushTime)
end
...
function ShengXiaoUpLevelView:ClickEnter()
	...
	TipsCtrl.Instance:ShowCommonExplainView(Language.ShengXiao.XingZuoYiJiExplain, ok_func, "enter_xingzuoyiji")
end
...
function ShengXiaoUpLevelView:FlushFlyAni(index)
	...
	self.center_display.rect:SetLocalPosition(local_pos_tbl.x, local_pos_tbl.y, 0)
...
function AttrItem:OnFlush()
	...
	self.attr_img:SetAsset("uis/images", Language.Forge.AttImageTab[self.data.attr_name])
end