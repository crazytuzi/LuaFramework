...
require("game/shen_ge/hun_qi/total_attr_view")
...
function HunQiCtrl:__init()
	...
	self.total_tips_view = TotalAttrTipView.New(ViewName.TotalAttrTipView)
...
function HunQiCtrl:__delete()
	...
	if self.total_tips_view then
		self.total_tips_view:DeleteMe()
		self.total_tips_view = nil
	end
...
function HunQiCtrl:OnShenzhouWeapondAllInfo(protocol)
	...
	if self.hunyin_resolve_view:IsOpen() then
		self.view.hunyin_content_view:Flush("resolve")
		self.hunyin_resolve_view:Flush("beibao")
	end
	RemindManager.Instance:Fire(RemindName.HunYin_LingShu)
	RemindManager.Instance:Fire(RemindName.HunYin_Inlay)
	RemindManager.Instance:Fire(RemindName.HunQiRemind)
	RemindManager.Instance:Fire(RemindName.HunYinRemind)
	RemindManager.Instance:Fire(RemindName.Gatheremind)
...
-- 打开总属性Tip
function HunQiCtrl:OpenTotalAttrTipView(param1, param2)
	self.total_tips_view:SetAttrData(param1, param2)
	self.total_tips_view:Open()
end
...
function HunQiCtrl:ChangeItemFlushView()
	if self.view:IsOpen() then
		self.view:Flush("hunqi")
		self.view:Flush("damo")
		self.view:Flush("gather_content")
