...
function CreelPanelView:OnFlush(param_list)
	...
	for i = 1, GameEnum.FISHING_FISH_TYPE_MAX_COUNT - 1 do
		if self.myfish_num[i] then
			self.myfish_num[i]:SetValue(FishingData.Instance:GetFishingUserInfo().fish_num_list[i + 1])
		end
	end

	-- 设置list数据
	local combination_cfg = TableCopy(FishingData.Instance:GetFishingCombinationCfg())
...
function FishingCreelPanelItemRender:__init()
	...
	self.fish = FishingData.Instance:GetFishingFishCfg()
...
function FishingCreelPanelItemRender:OnFlush()
	...
	for i = 1, GameEnum.FISHING_FISH_TYPE_MAX_COUNT - 1 do
		if self.myfishing_num[i] then
			local myfishing_num = FishingData.Instance:GetFishingUserInfo().fish_num_list[i + 1]
...
function  FishingCreelPanelItemRender:OnTimeHandler()
	FishingData.Instance:SetCreelViewtime(1)
end
...
function FishingCreelLogItemRender:OnFlush()
	...
	if self.lbl_log_text then
		local str = ""
		local fish_cfg = FishingData.Instance:GetFishingFishCfgByType(self.data.fish_type)
		if fish_cfg then
