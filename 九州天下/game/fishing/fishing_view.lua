...
function FishingView:__init()
	self.ui_config = {"uis/views/fishing", "FishingView"}
...
function FishingView:ReleaseCallBack()
...
function FishingView:LoadCallBack()
	--监听UI事件
...
function FishingView:OpenCallBack()
	-- 监听系统事件
...
function FishingView:OnCloseHandler()
...
--决定显示那个界面
function FishingView:ShowIndexCallBack(index)
	...
	-- if FishingData.Instance:GetFishingUserInfo().is_consumed_auto_fishing ~= 0 then
	-- 	self.is_fast_fishing_show:SetValue(false)
	-- end
	self:Flush()
end

function FishingView:OnFlush(param_list)
	for k, v in pairs(param_list) do
		...
		elseif k == "flush_fishing_area" then
			...
			-- 获取信息
			local fishing_user_info = FishingData.Instance:GetFishingUserInfo()
			local fishing_auto_go = FishingData.Instance:GetAutoGoFishing()
		...
		elseif k == "flush_fishing_paogan_btn" then				-- 刷新抛竿按钮
			...
			if FishingData.Instance:GetAutoFishing() == 0 then
				self.lbl_auto_fishing:SetValue(Language.Fishing.LabelAutoFishing[2])
				self.lbl_pull_rod:SetValue(Language.Fishing.LabelAutoFishing[0])
			end
		end
...
function FishingView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	self:FlushItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
end

function FishingView:GetMagicCount()
	local fishing_user_info = FishingData.Instance:GetFishingUserInfo() or nil	
...
function FishingView:OnOpenCreelHandler()
...
function FishingView:OnOpenFishBaitHandler()
...
function FishingView:OnOpenRaidersTipsHandler()													--游戏帮助
	-- 钓鱼Tips
	TipsCtrl.Instance:ShowHelpTipView(217)
end

-- 设置鱼篓关闭界面倒计时
function FishingView:SetCloseCreelTime()
	if self.creel_panel_count_down == nil then
		local count_down_time = FishingData.Instance:GetFishingCreelTimeCfg()
		function diff_time_func(elapse_time, total_time)
			local left_time = math.floor(count_down_time - elapse_time + 0.5)
			if FishingData.Instance:GetCreelViewtime() == 1 then
				CountDown.Instance:SetElapseTime(self.creel_panel_count_down, 0)
				FishingData.Instance:SetCreelViewtime(0)
			end
...
-- 活动倒计时
function FishingView:SetActTime(diff_time)
	if self.count_down == nil then
		function diff_time_func(elapse_time, total_time)
...
-- 拉杆倒计时处理函数
function FishingView:SetRodTime()
	local is_fast_fishing = FishingData.Instance:GetFishingUserInfo().is_consumed_auto_fishing
	local is_auto_fishing = FishingData.Instance:GetAutoFishing()
	...
	if self.pull_count_down == nil then
		local pull_count_down_time = FishingData.Instance:GetFishingOtherCfg().pull_count_down_s or 0
		...
		function diff_time_func(elapse_time, total_time)
			...
			if (bit:_and(1, bit:_rshift(fishing_user_info.special_status_flag, 2)) == 1 and length >= 465) or 
				...
				if self.pull_count_down ~= nil then
					...
					if FishingData.Instance:GetAutoFishing() == 1 then
						FishingCtrl.Instance:SendFishingOperaReq(FISHING_OPERA_REQ_TYPE.FISHING_OPERA_REQ_TYPE_PULL_RODS)  
					end
				end
				return
...
-- 快速拉杆
function FishingView:OnQiuckRodHandler()
	-- if FishingData.Instance:GetFishingUserInfo().is_consumed_auto_fishing ~= 0 then 
	-- 	return
	-- end
	local is_auto_fishing = FishingData.Instance:GetAutoFishing()
	...
	local other_cfg = FishingData.Instance:GetFishingOtherCfg()
	if other_cfg then
		local des = string.format(Language.Fishing.IsBuyAutoQiuckRod, other_cfg.auto_fishing_need_gold)
		local is_consumed_auto_fishing = FishingData.Instance:GetFishingUserInfo().is_consumed_auto_fishing
...
-- 自动钓鱼
function FishingView:OnAutoFishingHandler()
	local is_auto_fishing = FishingData.Instance:GetAutoFishing()
	if is_auto_fishing == 0 then
...
function FishingView:OnFishingHandler()
	local is_auto_fishing = FishingData.Instance:GetAutoFishing()
	...
	if is_auto_fishing == 0 then
		-- 设置自动钓鱼
		FishingData.Instance:SetAutoFishing(1)
		-- 使用0普通鱼饵
		if FishingData.Instance:GetFishingUserInfo().fishing_status == FISHING_STATUS.FISHING_STATUS_WAITING then
			 FishingCtrl.Instance:SendFishing(0)
		end
	else
		FishingData.Instance:SetAutoFishing(0)
...
--自动寻路
function FishingView:OnAutoGoFishingHandler()
	local fishing_location_cfg = FishingData.Instance:GetFishinglocationCfg()
	local rand_index = math.random(1, #fishing_location_cfg)
	local cfg = fishing_location_cfg[rand_index]
	if cfg then
		GuajiCtrl.Instance:MoveToPos(Scene.Instance:GetSceneId(), cfg.x, cfg.y)
		FishingData.Instance:SetAutoGoFishing(true)
	end
end
...
-- 钓鱼抛竿
function FishingView:OnBtnFishingHandler()
	local fish_bait = FishingData.Instance:GetBaitFishing(0)
	local fishing_user_info = FishingData.Instance:GetFishingUserInfo()
...
-- 法宝按钮
function FishingView:OnBtnGearHandler(gear_type)
...
-- 请求钓鱼状态
function FishingView:OnStartFishingHandler()
...
-- 角色移动处理函数
function FishingView:OnMainRolePosChangeHandler(x, y)
...
-- 角色移动结束处理函数
function FishingView:OnMainRoleMoveEndHandler()
	self.m_is_role_move = false
	FishingData.Instance:SetAutoGoFishing(false)
	self:Flush("flush_fishing_area")
end

function FishingView:RemoveFishingEffect(role_id)
...
function FishingView:ClearTimer()
...
function FishingView:Time()
	-- 剩余时间
	local server_time = TimeCtrl.Instance:GetServerTime()
    local Timetext = FishingData.Instance:GetFishingUserInfo().special_status_oil_end_timestamp
...
function FishingView:PortraitToggleChange(state)
