...
function KuafuGuildTaskDailyView:__init()
	...
	self.ui_config = {"uis/views/kuafuliujie","LiuJieSceneView2"}

	self.fight_state_button_handle = nil
	self.is_select_boss = false
...
function KuafuGuildTaskDailyView:OnMoveByClick()
...
function KuafuGuildTaskDailyView:OpenCallBack()
	...
	KuafuGuildBattleCtrl.Instance:SendCrossGuildBattleOperateReq(CROSS_GUILDBATTLE_OPERATE.CROSS_GUILDBATTLE_OPERATE_BOSS_INFO, 3150)
...
function KuafuGuildTaskDailyView:ChangeToggleState()
	...
	if scene_id ~= 3156 then
		...
		self.boss_toggle.toggle.isOn = self.is_select_boss
		self.task_toggle.toggle.isOn = not self.is_select_boss
		self.task_list.gameObject:SetActive(true)
	end

	if ViewManager.Instance:IsOpen(ViewName.KuafuTaskRecordView) then
		ViewManager.Instance:Close(ViewName.KuafuTaskRecordView)
	end
end

function KuafuGuildTaskDailyView:IsSelectBossToggle(value)
	self.is_select_boss = value
end
...
function KuafuGuildTaskDailyView:InitBoss()
	...
	delegate.CellRefreshDel = function(cell, data_index, cell_index)
		...
		self.boss_cell_list[cell]:SetItemIndex(data_index)
		local data = s_data[data_index]
...
function KuafuGuildTaskDailyView:GetSelectIndex()
...
function KuafuGuildTaskDailyView:FlushTaskHL()
	for k,v in pairs(self.cell_list) do
		v:FlushHL()
	end
end
...
function KuafuGuidTaskItem:__init()
	...
	self.double_text = self:FindVariable("DoubleText")
	self.show_task_hl = self:FindVariable("ShowHL")
...
function KuafuGuidTaskItem:OnClick()
	self.mother_view:SetSelectIndex(0)
...
function KuafuGuidTaskItem:OnFlush()
	...
	if self.data.record < self.data.cfg.task_count then
		...
		self.task_target:SetValue("<color=#ffffff>" .. str .. "</color>" .. color_str)
	else
	...
	if self.data.cfg.reward_item and self.data.cfg.reward_item.item_id then
		...
		self.item_text:SetValue("x" .. self.data.cfg.reward_item.num)
	end

	local scene_id = KuafuLiuJieSceneId[Scene.Instance:GetSceneId()]
	local is_double = KuafuGuildBattleData.Instance:GetIsDoubleRewardByIndex(scene_id + 1)
	if is_double then
		self.double_text:SetValue(Language.KuafuGuildBattle.DoubleText)
	else
		self.double_text:SetValue(Language.KuafuGuildBattle.NothingText)
	end

	self:FlushHL()
end

function KuafuGuidTaskItem:FlushHL()
	self.show_task_hl:SetValue(self.mother_view.auto_task_id == self.data.cfg.task_id)
end
