...
function KuafuGuildRecordView:__init()
	self:SetMaskBg()
	self.ui_config = {"uis/views/kuafuliujie","GuildRecord"}
...
function KuafuGuildRecordView:LoadCallBack()
	...
	local scene_name_cfg = KuafuGuildBattleData.Instance:GetSceneName()
	for i = 1, 5 do
		local scene_name = self:FindVariable("SceneName" .. i)
		self.scene_name[i] = scene_name
		self.scene_name[i]:SetValue(scene_name_cfg[i].scene_name)
	end
end
...
function KuafuGuildRecordView:MyGuildRank()
	...
	self.my_guild:SetValue(Language.Common.ScnenCampNameAbbr[main_role.camp])
	-- local camp_color = CAMP_COLOR[0]
	-- self.my_guild:SetValue(ToColorStr(myguild_name,camp_color))
	...
	for i, v in ipairs(data_list) do
		if i == cur_index then
			for x,z in ipairs(v) do
				if main_role.origin_merge_server_id == z.server_id and main_role.camp == NAME_TYPE_TO_CAMP[z.guild_name] then
					self.my_rank:SetValue(z.rank)
					self.my_sever:SetValue(server_id)
					self.my_score:SetValue(z.get_score)
					-- self.my_guild:SetValue(z.guild_name)
				end
			end
		end
	end
