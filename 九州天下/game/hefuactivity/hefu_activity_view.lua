...
function HefuActivityView:__init()
	self.ui_config = {"uis/views/hefuactivity", "HeFuView"}
...
function HefuActivityView:OpenChildPanel()
	...
	if nil == panel then
		AssetManager.LoadObjectSync(
			"uis/views/hefuactivity",
			"panel_" .. self.cur_sub_type,
			typeof(UnityEngine.GameObject),
			function(prefab)
				local obj = UnityEngine.Object.Instantiate(
					prefab, self.right_content.transform)
				obj = U3DObject(obj)
