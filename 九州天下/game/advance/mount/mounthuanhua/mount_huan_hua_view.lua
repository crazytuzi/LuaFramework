...
function MountHuanHuaView:LoadCallBack()
	...
	self.all_num, self.all_data = MountData.Instance:GetShowSpecialInfo()
...
function MountHuanHuaView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	if item_id ~= nil and MountData.Instance:CheckIsHuanHuaItem(item_id) then
		local now_num = MountData.Instance:GetShowSpecialInfo()
		if self.all_num ~= now_num then
			self:Flush("mounthuanhua", {need_flush = true})
		end
	end
end

function MountHuanHuaView:CloseCallBack()
	if self.item_data_event ~= nil then
		ItemData.Instance:UnNotifyDataChangeCallBack(self.item_data_event)
		self.item_data_event = nil
	end	
...
function MountHuanHuaView:FlushData()
	--self.all_num, self.all_data = MountData.Instance:GetShowSpecialInfo()
	self:Flush("mounthuanhua", {need_flush = true})
end

function MountHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function MountHuanHuaView:RefreshMountCell(cell, cell_index)
	...
	if self.all_data[cell_index+1] ~= nil then
		...
		data.is_show = MountData.Instance:CanHuanhuaUpgrade()[image_id] ~= nil
	end
...
function MountHuanHuaView:OnClickChange(change_value)
...
function MountHuanHuaView:OnClickUpGrade()
	...
	local attr_cfg = MountData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
	...
	MountHuanHuaCtrl.Instance:MountSpecialImaUpgrade(self.all_data[self.index].image_id)
end
...
function MountHuanHuaView:OnClickUseIma()
	...
	MountCtrl.Instance:SendUseMountImage(self.all_data[self.index].image_id + GameEnum.MOUNT_SPECIAL_IMA_ID)
end
...
function MountHuanHuaView:SetSpecialImageAttr(mount_special_data, index)
	...
	--if self.name_res ~= nil and image_cfg ~= nil and image_cfg.image_id then
		--local bundle, asset = ResPath.GetAdvanceEquipIcon("hh_mount_" .. image_cfg.image_id)
		--self.name_res:SetAsset(bundle, asset)

		if self.name_obj ~= nil then
			local bundle, asset = ResPath.GetAdvanceEquipIcon("hh_mount_" .. (image_cfg.title_res or 1))
	...
	if self.res_id ~= image_cfg.res_id then
		...
		self.mount_model:SetModelScale(Vector3(1, 1, 1))
		--模型预制体不能改，按照策划要求只能代码实现  普天同庆·骑
		if image_cfg.res_id == 7304001 then
			self.mount_model:SetModelScale(Vector3(0.5, 0.5, 0.5))
		end
	...
	local next_cfg = MountData.Instance:GetSpecialImageUpgradeInfo(index, nil, true)
...
function MountHuanHuaView:IsShowActivate(image_id)
	...
	self.show_activate_btn:SetValue(0 == bit_list[image_id])
...
function MountHuanHuaView:IsGrayUpgradeButton(index)
	...
	--local mount_special_image = MountData.Instance:GetSpecialImagesCfg()
	--local _, mount_special_image = MountData.Instance:GetShowSpecialInfo()
	local upgrade_cfg = MountData.Instance:GetSpecialImageUpgradeInfo(self.all_data[index].image_id)
	if upgrade_cfg.grade < MountData.Instance:GetSpecialImageMaxUpLevelById(self.all_data[index].image_id) then
...
MountHuanHuaCell = MountHuanHuaCell or BaseClass(BaseCell)
...
function MountHuanHuaCell:OnFlush()
