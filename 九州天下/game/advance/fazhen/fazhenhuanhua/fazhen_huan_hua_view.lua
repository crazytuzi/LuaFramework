FaZhenHuanHuaView = FaZhenHuanHuaView or BaseClass(BaseView)

function FaZhenHuanHuaView:__init()
...
function FaZhenHuanHuaView:LoadCallBack()
	...
	self.mount_model = RoleModel.New("fazhen_huanhua_panel",700)
	...
	self.all_num, self.all_data = FaZhenData.Instance:GetShowSpecialInfo()
...
function FaZhenHuanHuaView:__delete()
	self.index = 1
	self.grade = nil
	self.item_id = nil
	self.mount_special_image = nil
...
function FaZhenHuanHuaView:ReleaseCallBack()
...
function FaZhenHuanHuaView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	if item_id ~= nil and FaZhenData.Instance:CheckIsHuanHuaItem(item_id) then
		local now_num = FaZhenData.Instance:GetShowSpecialInfo()
		if self.all_num ~= now_num then
			self:Flush("fazhenhuanhua", {need_flush = true})
		end
	end
end

function FaZhenHuanHuaView:CloseCallBack()	
	if self.item_data_event ~= nil then
		ItemData.Instance:UnNotifyDataChangeCallBack(self.item_data_event)
		self.item_data_event = nil
	end	
...
function FaZhenHuanHuaView:FlushData()
	--self.all_num, self.all_data = FaZhenData.Instance:GetShowSpecialInfo()
	self:Flush("fazhenhuanhua", {need_flush = true})
end

function FaZhenHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
	if attr_name == "level" and self:IsOpen() then
		self:FlushData()
	end
end

function FaZhenHuanHuaView:GetMountNumberOfCells()
	--return FaZhenData.Instance:GetMaxSpecialImage()
	--return FaZhenData.Instance:GetShowSpecialInfo()
	return self.all_num
end

function FaZhenHuanHuaView:RefreshMountCell(cell, cell_index)
	--local mount_special_image = FaZhenData.Instance:GetSpecialImagesCfg()
	--local _, mount_special_image = FaZhenData.Instance:GetShowSpecialInfo()
	local mount_cell = self.cell_list[cell]
	if mount_cell == nil then
		mount_cell = FaZhenHuanHuaCell.New(cell.gameObject)
	...
	if self.all_data[cell_index+1] ~= nil then
		...
		data.is_show = FaZhenData.Instance:CanHuanhuaUpgrade()[image_id] ~= nil
	end
...
function FaZhenHuanHuaView:OnClickChange(change_value)
...
function FaZhenHuanHuaView:OnClickClose()
	self:Close()
	self.grade = nil
...
function FaZhenHuanHuaView:OpenCallBack()
	if self.item_data_event == nil then
		self.item_data_event = BindTool.Bind1(self.ItemDataChangeCallback, self)
		ItemData.Instance:NotifyDataChangeCallBack(self.item_data_event)
	end
	self:Flush("fazhenhuanhua")
	self.index = 1
	-- self:SetModleRestAni()
end

--点击激活按钮
function FaZhenHuanHuaView:OnClickActivate()
...
--点击升级按钮
function FaZhenHuanHuaView:OnClickUpGrade()
	if self.all_data[self.index] == nil then
		return
	end

	local attr_cfg = FaZhenData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
	if attr_cfg ~= nil then
		if attr_cfg.grade >= FaZhenData.Instance:GetSpecialImageMaxUpLevelById(attr_cfg.special_img_id) then
			return
		end
	...
	FaZhenCtrl.Instance:SendFaZhenOpera(FAZHEN_OPERA_REQ_TYPE.FAZHEN_OPERA_REQ_TYPE_UPGRADE_IMG, self.all_data[self.index].image_id)
end

--点击使用当前形象
function FaZhenHuanHuaView:OnClickUseIma()
	if self.all_data[self.index] == nil then
		return
	end

	FaZhenCtrl.Instance:SendUseFaZhenImage(self.all_data[self.index].image_id + GameEnum.MOUNT_SPECIAL_IMA_ID)
end

function FaZhenHuanHuaView:OnClickListCell(mount_special_data, index, mount_cell)
	...
	self:SetSpecialImageAttr(mount_special_data, mount_special_data.image_id)
...
--获取激活坐骑符数量
function FaZhenHuanHuaView:GetHaveProNum(item_id, need_num)
...
function FaZhenHuanHuaView:SetModleRestAni()
...
function FaZhenHuanHuaView:SetSpecialImageAttr(mount_special_data, index)
	if mount_special_data == nil then
		return
	end
	local upgrade_cfg = FaZhenData.Instance:GetSpecialImageUpgradeInfo(mount_special_data.image_id)
	local image_cfg = FaZhenData.Instance:GetSpecialImageCfg(index)
	...
	local change_name_list = Split(mount_special_data.image_name, "-")
	local change_name = (#change_name_list >= 2) and (change_name_list[1] .. " - " .. change_name_list[2]) or mount_special_data.image_name
	--self.mount_name:SetValue("<color=" .. SOUL_NAME_COLOR[item_cfg and item_cfg.color or 5] .. ">" .. change_name .. "</color>")
	if self.name_obj ~= nil then
		local bundle, asset = ResPath.GetAdvanceEquipIcon("hh_fazhen_" .. (image_cfg.title_res or 1))
	...
	if self.res_id ~= image_cfg.res_id then
		...
		self.mount_model:SetModelTransformParameter(DISPLAY_MODEL_TYPE[DISPLAY_TYPE.FAZHEN], image_cfg.res_id, DISPLAY_PANEL.HUAN_HUA)
	...
	local attr_cfg = FaZhenData.Instance:GetSpecialImageUpgradeInfo(index)
	...
	local next_cfg = FaZhenData.Instance:GetSpecialImageUpgradeInfo(index, nil, true)
...
--设置激活按钮显示和隐藏
function FaZhenHuanHuaView:IsShowActivate(image_id)
...
--设置升级按钮显示和隐藏
function FaZhenHuanHuaView:IsShowUpGrade(image_id)
	if image_id == nil then
		return
	end
	local special_img_up = FaZhenData.Instance:GetSpecialImageUpgradeCfg()
	local info_list = FaZhenData.Instance:GetFightMountInfo()
	local active_flag = info_list.active_special_image_flag
	local bit_list = bit:d2b(active_flag)
...
--升级按钮是否置灰
function FaZhenHuanHuaView:IsGrayUpgradeButton(index)
	if index == nil or index < 0 then return end
	--local _, mount_special_image = FaZhenData.Instance:GetShowSpecialInfo()
	local upgrade_cfg = FaZhenData.Instance:GetSpecialImageUpgradeInfo(self.all_data[index].image_id)
	if upgrade_cfg.grade < FaZhenData.Instance:GetSpecialImageMaxUpLevelById(self.all_data[index].image_id) then
...
function FaZhenHuanHuaView:OnFlush(param_list)
	for k, v in pairs(param_list) do
		if k == "fazhenhuanhua" then
			...
			local upgrade_cfg = FaZhenData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
			local info_list = FaZhenData.Instance:GetFightMountInfo()
			local active_flag = info_list.active_special_image_flag
			local bit_list = bit:d2b(active_flag)
...
FaZhenHuanHuaCell = FaZhenHuanHuaCell or BaseClass(BaseCell)

function FaZhenHuanHuaCell:__init()
...
function FaZhenHuanHuaCell:OnFlush()
	...
	local active_flag = info_list.active_special_image_flag
	local bit_list = bit:d2b(active_flag)
	local _, special_data = FaZhenData.Instance:GetShowSpecialInfo()
	local image_id = special_data[self.index].image_id
	self.is_possess_img:SetValue(bit_list[32 - image_id] == 1)
end

function FaZhenHuanHuaCell:ListenClick(handler)
	self:ClearEvent("Click")
	self:ListenEvent("Click", handler)
end

function FaZhenHuanHuaCell:SetToggleGroup(toggle_group)
	self.root_node.toggle.group = toggle_group
end

function FaZhenHuanHuaCell:SetHighLight(enable)
