...
function HaloHuanHuaView:LoadCallBack()
	...
	self.title_name:SetValue(Language.HuanHua.Halo)
	...
	self.halo_model = RoleModel.New("common_huanhua_panel",500)
	...
	self.all_num, self.all_data = HaloData.Instance:GetShowSpecialInfo()
...
function HaloHuanHuaView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	if item_id ~= nil and HaloData.Instance:CheckIsHuanHuaItem(item_id) then
		local now_num = HaloData.Instance:GetShowSpecialInfo()
...
function HaloHuanHuaView:CloseCallBack()
	if self.item_data_event ~= nil then
		ItemData.Instance:UnNotifyDataChangeCallBack(self.item_data_event)
		self.item_data_event = nil
	end	
...
function HaloHuanHuaView:FlushData()
	--self.all_num, self.all_data = HaloData.Instance:GetShowSpecialInfo()
	self:Flush("halohuanhua", {need_flush = true})
end

function HaloHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function HaloHuanHuaView:GetHaloNumberOfCells()
	--return HaloData.Instance:GetShowSpecialInfo()
	return self.all_num
end
...
function HaloHuanHuaView:RefreshHaloCell(cell, cell_index)
	...
	--local _, halo_special_image = HaloData.Instance:GetShowSpecialInfo()
	...
	if self.all_data[cell_index+1] ~= nil then
		...
		data.is_show = HaloData.Instance:CanHuanhuaUpgrade()[image_id] ~= nil
	end
...
function HaloHuanHuaView:OnClickChange(change_value)
	if change_value ~= nil then
		--local mount_special_image = ConfigManager.Instance:GetAutoConfig("halo_auto").special_img
...
function HaloHuanHuaView:OnClickUpGrade()
	...
	local attr_cfg = HaloData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
	...
	if self.all_data[self.index] ~= nil then
		HaloHuanHuaCtrl.Instance:HaloSpecialImaUpgrade(self.all_data[self.index].image_id)
	end
end
...
function HaloHuanHuaView:OnClickUseIma()
	if self.all_data[self.index] ~= nil then
		HaloCtrl.Instance:SendUseHaloImage(self.all_data[self.index].image_id + GameEnum.MOUNT_SPECIAL_IMA_ID)
	end
...
function HaloHuanHuaView:SetSpecialImageAttr(halo_special_data, index)
	...
	if self.name_obj ~= nil then
		local bundle, asset = ResPath.GetAdvanceEquipIcon("hh_halo_" .. (image_cfg.title_res or 1))
	...
	if self.res_id ~= image_cfg.res_id then
		...
		self.halo_model:SetModelTransformParameter(DISPLAY_MODEL_TYPE[DISPLAY_TYPE.HALO], image_cfg.res_id, DISPLAY_PANEL.HUAN_HUA)
	...
	local next_cfg = HaloData.Instance:GetSpecialImageUpgradeInfo(index, nil, true)
...
function HaloHuanHuaView:IsGrayUpgradeButton(index)
	...
	--local halo_special_image = ConfigManager.Instance:GetAutoConfig("halo_auto").special_img
	--local _, halo_special_image = HaloData.Instance:GetShowSpecialInfo()
	local upgrade_cfg = HaloData.Instance:GetSpecialImageUpgradeInfo(self.all_data[index].image_id)
	if upgrade_cfg.grade < HaloData.Instance:GetSpecialImageMaxUpLevelById(self.all_data[index].image_id) then
...
function HaloHuanHuaView:OnFlush(param_list)
	print("刷新HaloHuanHuaView:OnFlush()")
	for k, v in pairs(param_list) do
		if k == "halohuanhua" then
			...
			self.title_icon:SetAsset("uis/views/advanceview_images","icon_halohuanhua")
			--local halo_special_image = ConfigManager.Instance:GetAutoConfig("halo_auto").special_img
			--local _, halo_special_image = HaloData.Instance:GetShowSpecialInfo()
			...
			local upgrade_cfg = HaloData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
			local info_list = HaloData.Instance:GetHaloInfo()
			local active_flag = info_list.active_special_image_flag
			local bit_list = bit:d2b(active_flag)
...
HaloHuanHuaCell = HaloHuanHuaCell or BaseClass(BaseCell)
...
function HaloHuanHuaCell:OnFlush()
	...
	local active_flag = info_list.active_special_image_flag
	local bit_list = bit:d2b(active_flag)
	local _, special_data = HaloData.Instance:GetShowSpecialInfo()
