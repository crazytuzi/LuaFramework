...
function HaloData:__init()
	...
	self.halo_skill_cfg = nil
	self.image_list_cfg = nil
	self.halo_equip_cfg = nil
	...
	RemindManager.Instance:Register(RemindName.AdvanceHalo, BindTool.Bind(self.GetInAdvanceRedNum, self))
end
...
function HaloData:SetHaloInfo(protocol)
	self:SetLastInfo(self.halo_info.grade_bless_val, self.halo_info.grade)
	...
	self.halo_info.show_grade = math.ceil(protocol.grade / 10)
	self.halo_info.star_level = protocol.grade % 10

	self.halo_info.equip_info_list = protocol.equip_info_list
...
function HaloData:GetAllHaloCfg()
	return ConfigManager.Instance:GetAutoConfig("halo_auto") or {}
end

function HaloData:GetCurHaloCfg(grade)
...
--形象进阶+幻化的战力
function HaloData:GetHaloPower()
	local cfg = self:GetCurHaloCfg(self.halo_info.grade)
	local upgrade_power = CommonDataManager.GetCapabilityCalculation(cfg or {})
	local huanhua_power = 0
	for k,v in pairs(self.halo_info.active_special_image_list) do
		if v == 1 then
			local index = 32 - k
...
function HaloData:GetAllHaloLevelCfg()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.level_cfg then
		self.level_cfg = ListToMap(holo_cfg.level, "halo_level")
	end
...
function HaloData:GetHaloLevelCfg(halo_level)
	...
	--return halo_config.level[halo_level]
	return self:GetAllHaloLevelCfg()[halo_level]
end

function HaloData:GetMaxHaloLevelCfg()
	--return #ConfigManager.Instance:GetAutoConfig("halo_auto").level
	return #self:GetAllHaloLevelCfg()
end

function HaloData:GetHaloShowGradeCfg(halo_grade)
	--local halo_config = ConfigManager.Instance:GetAutoConfig("halo_auto")
	--return halo_config.grade[halo_grade * 10]
	if halo_grade ~= nil then
		return self:GetGradeCfg()[halo_grade * 10]
	end
...
function HaloData:GetHaloGradeCfg(halo_grade)
	-- local halo_config = ConfigManager.Instance:GetAutoConfig("halo_auto")
	-- return halo_config.grade[halo_grade]
	if halo_grade ~= nil then
		return self:GetGradeCfg()[halo_grade]
	end
...
function HaloData:GetSpecialImagesCfg()
	...
	if nil == self.special_img_cfg then
		self.special_img_cfg = holo_cfg.special_img
	end
	return self.special_img_cfg
end

function HaloData:GetSpecialImageCfg(image_id)
	-- local halo_config = ConfigManager.Instance:GetAutoConfig("halo_auto").special_img
...
function HaloData:GetMaxGrade()
	--return #ConfigManager.Instance:GetAutoConfig("halo_auto").grade / 10
...
function HaloData:GetGradeCfg()
	...
	if nil == self.grade_cfg then
		self.grade_cfg = ListToMap(holo_cfg.grade, "grade")
	end
	return self.grade_cfg
end

function HaloData:GetMaxSpecialImage()
	--return #ConfigManager.Instance:GetAutoConfig("halo_auto").special_img
	return #self:GetSpecialImagesCfg()
end

function HaloData:GetShowSpecialInfo()
	local num = 0
	local show_list = {}
	local role_level = GameVoManager.Instance:GetMainRoleVo().level
	local active_list = self.halo_info.active_special_image_list or {}
...
function HaloData:GetHoloIsActive(image_id)
	return self.halo_info.active_special_image_list[32 - image_id] ~= 0
end

function HaloData:CheckIsHuanHuaItem(item_id)
	local is_item = false
...
function HaloData:GetSpecialImageUpgradeCfg()
	return ConfigManager.Instance:GetAutoConfig("halo_auto").special_image_upgrade
end

function HaloData:GetSpecialImageUpgradeListCfg()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.special_image_upgrade_list then
		self.special_image_upgrade_list = ListToMapList(holo_cfg.special_image_upgrade, "special_img_id")
	end
	return self.special_image_upgrade_list
end

function HaloData:GetSpecialImageUpgradeList(special_img_id)
...
function HaloData:GetHaloSkillCfg()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.halo_skill_cfg then
		self.halo_skill_cfg = holo_cfg.halo_skill
	end
	return self.halo_skill_cfg
end

function HaloData:GetImageListCfg()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.image_list_cfg then
		self.image_list_cfg = ListToMap(holo_cfg.image_list, "image_id")
	end
	return self.image_list_cfg
end

function HaloData:GetHaloImageCfg(image_id)
	--return ConfigManager.Instance:GetAutoConfig("halo_auto").image_list
	if image_id ~= nil then
...
function HaloData:GetSingleHaloImageCfg(image_id)
	return ConfigManager.Instance:GetAutoConfig("halo_auto").image_list[image_id]
end

function HaloData:GetHaloEquipCfg()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.halo_equip_cfg then
		self.halo_equip_cfg = holo_cfg.halo_equip
	end
	return self.halo_equip_cfg
end

function HaloData:GetHaloEquipExpCfg()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.equip_exp_cfg then
		self.equip_exp_cfg = holo_cfg.equip_exp
	end
	return self.equip_exp_cfg
end

function HaloData:GetHaloEquipRandAttr()
	return ConfigManager.Instance:GetAutoConfig("halo_auto").equip_attr_range
end

function HaloData:GetAllHaloUpStarExpCfg()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.up_star_exp_cfg then
		self.up_star_exp_cfg = ListToMap(holo_cfg.up_star_exp, "star_level")
	end
	return self.up_star_exp_cfg
end

function HaloData:GetHaloUpStarExpCfg(star_level)
	--return ConfigManager.Instance:GetAutoConfig("halo_auto").up_star_exp
	if star_level ~= nil then
		return self:GetAllHaloUpStarExpCfg()[star_level]
	end
	return nil
end

function HaloData:GetHaloSkillId()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.skill_id_cfg then
		self.skill_id_cfg = holo_cfg.skill_id
	end
...
function HaloData:GetSpecialImageUpgradeInfo(index, grade, is_next)
	...
	local grade = grade
	if grade == nil and self.halo_info ~= nil and self.halo_info.special_img_grade_list ~= nil then
		grade = self.halo_info.special_img_grade_list[index] or 0
...
function HaloData:GetImageListInfo(index)
	...
	return self:GetHaloImageCfg(index)
end
...
function HaloData:GetHaloSkillCfgById(skill_idx, level, halo_info)
	local skill_id_list = self:GetHaloSkillId()
	local skill_id = 0
...
function HaloData:GetHaloSkillIsActvity(skill_idx, cur_grade)
	local can_activity = self:GetHaloSkillCfgById(skill_idx, 1)
...
function HaloData:GetHaloStarLevelCfg(star_level)
	...
	-- end
	if star_level ~= nil then
		return self:GetHaloUpStarExpCfg(star_level)
	end
...
function HaloData:CanHuanhuaUpgrade()
	...
	for i, j in pairs(self:GetSpecialImageUpgradeCfg()) do
		if j.stuff_num <= ItemData.Instance:GetItemNumInBagById(j.stuff_id)
			...
			self.halo_info.special_img_grade_list[j.special_img_id] < self:GetMaxSpecialImageCfgById(j.special_img_id)then
...
function HaloData:CanSkillUpLevelList()
	...
	for i, j in pairs(self:GetHaloSkillCfg()) do
		...
		if j.uplevel_stuff_num <= ItemData.Instance:GetItemNumInBagById(j.uplevel_stuff_id)
			...
			and j.grade <= self.halo_info.show_grade and j.skill_type ~= 0 then
...
function HaloData:GetSkillLevel(skill_index)
	local skill_id_list = self:GetHaloSkillId()
...
function HaloData:CanJinjie()
	...
	if cfg then
		if cfg.upgrade_stuff_count <= ItemData.Instance:GetItemNumInBagById(cfg.upgrade_stuff_id)
			and self.halo_info.grade < #self:GetGradeCfg() then
			return true
		end
	end
	return false
...
function HaloData:GetAllEquipInfoCfg()
	local holo_cfg = self:GetAllHaloCfg()
	if nil == self.equip_info_cfg then
		self.equip_info_cfg = ListToMap(holo_cfg.halo_equip_info, "equip_idx", "equip_level")
	end
...
function HaloData:GetHaloGradeByUseImageId(used_imageid)
	...
	local image_list = self:GetHaloImageCfg(used_imageid)
...
function HaloData:SetLastInfo(bless, grade)
	self.last_bless = bless or 0
	self.last_grade = grade or 0
end

function HaloData:GetLastGrade()
	return self.last_grade
end

function HaloData:ChangeShowInfo()
	self.show_bless = self.halo_info.grade_bless_val
end

function HaloData:GetShowBless()
	return self.show_bless or self.last_bless
end

function HaloData:GetInAdvanceRedNum()
	if OpenFunData.Instance:CheckIsHide("halo_jinjie") and (next(self:CanHuanhuaUpgrade()) ~= nil
		or self:IsShowZizhiRedPoint() or next(self:CanSkillUpLevelList()) ~= nil or self:CalEquipBtnRemind() or self:CanJinjie() or AdvanceSkillData.Instance:ShowSkillRedPoint(ADVANCE_SKILL_TYPE.HALO)) then
		return 1
	end
	return 0
end
...
-- 计算装备按钮的红点
function HaloData:CalEquipBtnRemind()
	local flag = false
	for i = 0, 3 do
...
function HaloData:GetOhterCfg()
	return self.GetAllHaloCfg().other[1]
end
