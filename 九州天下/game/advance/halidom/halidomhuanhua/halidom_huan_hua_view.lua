HalidomHuanHuaView = HalidomHuanHuaView or BaseClass(BaseView)

function HalidomHuanHuaView:__init()
...
function HalidomHuanHuaView:LoadCallBack()
	self.halo_display = self:FindObj("HalidomDisplay")
	self.halo_display:SetActive(true)
	self.title_name = self:FindVariable("TitleName")
	self.title_name:SetValue(Language.HuanHua.Halidon)
	...
	self.halo_model = RoleModel.New("halidom_huanhua_view",500)
	...
	self.all_num, self.all_data = HalidomData.Instance:GetShowSpecialInfo()
...
function HalidomHuanHuaView:__delete()
	self.index = 1
	self.grade = nil
	self.item_id = nil
	self.res_id = nil
...
function HalidomHuanHuaView:ReleaseCallBack()
...
function HalidomHuanHuaView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	if item_id ~= nil and HalidomData.Instance:CheckIsHuanHuaItem(item_id) then
		local now_num = HalidomData.Instance:GetShowSpecialInfo()
		if self.all_num ~= now_num then
			self:Flush("halidomhuanhua", {need_flush = true})
		end
	end
end

function HalidomHuanHuaView:CloseCallBack()
	if self.item_data_event ~= nil then
		ItemData.Instance:UnNotifyDataChangeCallBack(self.item_data_event)
		self.item_data_event = nil
	end	
...
function HalidomHuanHuaView:FlushData()
	--self.all_num, self.all_data = HalidomData.Instance:GetShowSpecialInfo()
	self:Flush("halidomhuanhua", {need_flush = true})
end

function HalidomHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
	if attr_name == "level" and self:IsOpen() then
		self:FlushData()
	end
end

function HalidomHuanHuaView:GetHaloNumberOfCells()
	--return HalidomData.Instance:GetMaxSpecialImage()
	--return HalidomData.Instance:GetShowSpecialInfo()
	return self.all_num
end

function HalidomHuanHuaView:RefreshHaloCell(cell, cell_index)
	--local halo_special_image = HalidomData.Instance:GetSpecialImagesCfg()
	--local _, halo_special_image = HalidomData.Instance:GetShowSpecialInfo()
	local halo_cell = self.cell_list[cell]
	if halo_cell == nil then
		halo_cell = HalidomHuanHuaCell.New(cell.gameObject)
	...
	if self.all_data[cell_index+1] ~= nil then
		...
		data.is_show = HalidomData.Instance:CanHuanhuaUpgrade()[image_id] ~= nil
	end
...
function HalidomHuanHuaView:OnClickChange(change_value)
	if change_value ~= nil then
		--local mount_special_image = HalidomData.Instance:GetSpecialImagesCfg()
...
function HalidomHuanHuaView:OnClickClose()
	self:Close()
	self.grade = nil
	self.res_id = nil
	self.used_imageid = nil
...
function HalidomHuanHuaView:OpenCallBack()
	if self.item_data_event == nil then
		self.item_data_event = BindTool.Bind1(self.ItemDataChangeCallback, self)
		ItemData.Instance:NotifyDataChangeCallBack(self.item_data_event)
	end
	self:Flush("halidomhuanhua")
	self.index = 1
end

--点击激活按钮
function HalidomHuanHuaView:OnClickActivate()
...
--点击升级按钮
function HalidomHuanHuaView:OnClickUpGrade()
	if self.all_data[self.index] == nil then
		return
	end

	local attr_cfg = HalidomData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
	if attr_cfg ~= nil then
		if attr_cfg.grade >= HalidomData.Instance:GetSpecialImageMaxUpLevelById(attr_cfg.special_img_id) then
			return
		end
	...
	HalidomHuanHuaCtrl.Instance:SendSpiritFazhenSpecialImgUpgrade(self.all_data[self.index].image_id)
end

--点击使用当前形象
function HalidomHuanHuaView:OnClickUseIma()
	if self.all_data[self.index] == nil then
		return
	end

	HalidomCtrl.Instance:SendSpiritFazhenUseImage(self.all_data[self.index].image_id + GameEnum.MOUNT_SPECIAL_IMA_ID)
end

function HalidomHuanHuaView:OnClickListCell(halo_special_data, index, halo_cell)
	...
	local _, mount_special_image = HalidomData.Instance:GetShowSpecialInfo()
...
--获取激活坐骑符数量
function HalidomHuanHuaView:GetHaveProNum(item_id, need_num)
...
function HalidomHuanHuaView:SetSpecialImageAttr(halo_special_data, index)
	if halo_special_data == nil then
		return
	end
	local upgrade_cfg = HalidomData.Instance:GetSpecialImageUpgradeInfo(halo_special_data.image_id)
	local image_cfg = HalidomData.Instance:GetSpecialImageCfg(index)
	...
	if self.name_obj ~= nil then
		local bundle, asset = ResPath.GetAdvanceEquipIcon("hh_halidom_" .. (image_cfg.title_res or 1))
	...
	if self.res_id ~= image_cfg.res_id then
		...
		self.halo_model:SetModelTransformParameter(DISPLAY_MODEL_TYPE[DISPLAY_TYPE.ZHIBAO], image_cfg.res_id, DISPLAY_PANEL.HUAN_HUA)
		self.halo_model:SetMainAsset(ResPath.GetBaoJuModel(image_cfg.res_id))
		-- self.halo_model:SetLoopAnimal("bj_rest")
		self.halo_model:SetLayer(1, 1.0)
		self.res_id = image_cfg.res_id
		-- local main_role = Scene.Instance:GetMainRole()
		-- self.halo_model:SetRoleResid(main_role:GetRoleResId())
		-- self.halo_model:SetZhiBaoResid(image_cfg.res_id)
	end

	self.used_imageid = info_list.used_imageid
	local attr = CommonStruct.Attribute()
	local attr_cfg = HalidomData.Instance:GetSpecialImageUpgradeInfo(index)
	if attr_cfg ~= nil then
	...
	local next_cfg = HalidomData.Instance:GetSpecialImageUpgradeInfo(index, nil, true)
...
--设置激活按钮显示和隐藏
function HalidomHuanHuaView:IsShowActivate(image_id)
...
--设置升级按钮显示和隐藏
function HalidomHuanHuaView:IsShowUpGrade(image_id)
	if image_id == nil then
		return
	end
	local special_img_up = HalidomData.Instance:GetSpecialImageUpgradeCfg()
	local info_list = HalidomData.Instance:GetHalidomInfo()
	local active_flag = info_list.active_special_image_flag
	local bit_list = bit:d2b(active_flag)
...
--升级按钮是否置灰
function HalidomHuanHuaView:IsGrayUpgradeButton(index)
	if index == nil or index < 0 then return end
	--local _, halo_special_image = HalidomData.Instance:GetShowSpecialInfo()
	local upgrade_cfg = HalidomData.Instance:GetSpecialImageUpgradeInfo(self.all_data[index].image_id)
	if upgrade_cfg.grade < HalidomData.Instance:GetSpecialImageMaxUpLevelById(self.all_data[index].image_id) then
...
function HalidomHuanHuaView:OnFlush(param_list)
	for k, v in pairs(param_list) do
		if k == "halidomhuanhua" then
			...
			self.title_icon:SetAsset("uis/views/advanceview_images","icon_Halidomhuanhua")
			--local _, halo_special_image = HalidomData.Instance:GetShowSpecialInfo()
			...
			local upgrade_cfg = HalidomData.Instance:GetSpecialImageUpgradeInfo(self.all_data[self.index].image_id)
			local info_list = HalidomData.Instance:GetHalidomInfo()
			local active_flag = info_list.active_special_image_flag
			local bit_list = bit:d2b(active_flag)
...
HalidomHuanHuaCell = HalidomHuanHuaCell or BaseClass(BaseCell)

function HalidomHuanHuaCell:__init()
...
function HalidomHuanHuaCell:OnFlush()
	...
	local active_flag = info_list.active_special_image_flag
	local bit_list = bit:d2b(active_flag)
	local _, special_data = HalidomData.Instance:GetShowSpecialInfo()
	local image_id = special_data[self.index].image_id
	self.is_possess_img:SetValue(bit_list[32 - image_id] == 1)
end

function HalidomHuanHuaCell:ListenClick(handler)
	self:ClearEvent("Click")
	self:ListenEvent("Click", handler)
end

function HalidomHuanHuaCell:SetToggleGroup(toggle_group)
	self.root_node.toggle.group = toggle_group
end

function HalidomHuanHuaCell:SetHighLight(enable)
