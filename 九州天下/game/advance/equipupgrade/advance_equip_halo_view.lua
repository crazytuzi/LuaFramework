AdvanceEquipHaloView = AdvanceEquipHaloView or BaseClass(BaseRender)

function AdvanceEquipHaloView:__init(instance)
	...
	self.view_change = GlobalEventSystem:Bind(AdvanceEquipupType.HALO_EQUIPUP_CHANGE, BindTool.Bind1(self.OnFlush, self))
end

function AdvanceEquipHaloView:__delete()
	self.cells = nil
	self.cur_index = nil
...
function AdvanceEquipHaloView:OnHandUpGrade(is_only_one_level)
	local halo_grade = HaloData.Instance:GetHaloInfo().grade
	if (halo_grade == 0) or nil then
		return
	end
	...
	local can_up_list = AdvanceData.Instance:GetHaloCanUplevel()
	if can_up_list[self.cur_index + 1] then
		local bind_list = AdvanceData.Instance:GetHaloCanUplevel(is_only_bind == 1, is_only_equip == 1)
	...
	AdvanceEquipUpCtrl.Instance:SendHaloEquipUpLevelReq(self.cur_index, is_only_one_level, is_only_bind, is_only_equip)
	print("点击提升等级按钮")
end

function AdvanceEquipHaloView:OnAutomatic()
	-- for i = 0, 3 do
	self:OnHandUpGrade(0)
	-- end
end

function AdvanceEquipHaloView:OnStorageExp()
	print("点击存储按钮")
end

function AdvanceEquipHaloView:OnClickItem(index)
	self.cur_index = index
	self:SetItemData(self.cur_index, true)
end

function AdvanceEquipHaloView:GetItemCells()
	local halo_equip_list = HaloData.Instance:GetHaloInfo().equip_info_list
	local item_cfg = nil
	local data = {}
	...
	for k, v in pairs(self.cells) do
		data.item_id = halo_equip_list[k - 1].equip_id
...
function AdvanceEquipHaloView:SetItemData(index, is_click)
	self.cells[index + 1]:SetHighLight(true)
	local halo_equip_list = HaloData.Instance:GetHaloInfo().equip_info_list
	local halo_equip_cfg = HaloData.Instance:GetHaloEquipCfg()
	local attr_list = halo_equip_list[index].attr_list

	local range_attr = HaloData.Instance:GetHaloEquipRandAttr()
	local item_cfg = ItemData.Instance:GetItemConfig(halo_equip_list[index].equip_id)
	local attr = CommonStruct.Attribute()
	local halo_grade = HaloData.Instance:GetHaloInfo().grade
	local equip_level_limit = HaloData.Instance:GetSpecialImageAttrSum().equip_limit
	if halo_grade > 0 then
		if halo_equip_list[index].level >= equip_level_limit then
			self.keep_exp:SetValue(halo_equip_list[index].exp)
		else
			self.keep_exp:SetValue(0)
		end
		self.had_limit:SetValue(halo_equip_list[index].level >= equip_level_limit)
		self.no_limit:SetValue(halo_equip_list[index].level < equip_level_limit)
	end
	self.info_level:SetValue(halo_equip_list[index].level)
	self.current_level:SetValue(halo_equip_list[index].level)
	self.next_level:SetValue(halo_equip_list[index].level + 1)

	for k, v in pairs(self.cells) do
		local data = {}
		local item_cfg1 = ItemData.Instance:GetItemConfig(halo_equip_list[k - 1].equip_id)
		if item_cfg1 then
			data.item_id = halo_equip_list[k - 1].equip_id
			v:SetIconGrayScale(false)
			v:ShowQuality(true)
		else
			data.item_id = HaloDataEquipId[k]
			v:SetIconGrayScale(true)
			v:ShowQuality(false)
		end
		if self.uplevel_list[k] then
			data.is_show_up_level = true
			if halo_equip_list[k - 1].level <= 0 then
	...
	for k, v in pairs(halo_equip_cfg) do
		if v.equip_idx == index  then
			if  index == 0 then
				self.show_speical_pro:SetValue(false)
				for i, j in pairs(self.show_base_pro_list) do
					self.show_random_pro_list[i]:SetValue(halo_equip_list[index].level > 0)
					j:SetValue(halo_equip_list[index].level > 0)
				end
				if item_cfg ~= nil then
					self.base_pro_list[1].base_pro:SetValue(v.gongji * halo_equip_list[index].level * item_cfg.color)
					self.base_pro_list[1].base_pro_name:SetValue(Language.Common.MountEquipAttr.gongji)
					self.base_pro_list[2].base_pro:SetValue(v.fangyu * halo_equip_list[index].level * item_cfg.color)
					self.base_pro_list[2].base_pro_name:SetValue(Language.Common.MountEquipAttr.fangyu)
					self.base_pro_list[3].base_pro:SetValue(v.maxhp * halo_equip_list[index].level * item_cfg.color)
					...
					attr.gong_ji = v.gongji * halo_equip_list[index].level * item_cfg.color + attr_list[0]
					attr.fang_yu = v.fangyu * halo_equip_list[index].level * item_cfg.color + attr_list[1]
					attr.max_hp = v.maxhp * halo_equip_list[index].level * item_cfg.color + attr_list[2]
				end
			else
				self.show_speical_pro:SetValue(halo_equip_list[index].level > 0)
				for i, j in pairs(self.show_base_pro_list) do
					if i == 1 then
						j:SetValue(halo_equip_list[index].level > 0)
						self.show_random_pro_list[i]:SetValue(halo_equip_list[index].level > 0)
				...
				if item_cfg ~= nil then
					if index == 1 then
						...
						attr.gong_ji = v.gongji * halo_equip_list[index].level * item_cfg.color + attr_list[0]
					elseif index == 2 then
						self.base_pro_list[1].base_pro:SetValue(v.fangyu * halo_equip_list[index].level * item_cfg.color)
						...
						attr.fang_yu = v.fangyu * halo_equip_list[index].level * item_cfg.color + attr_list[0]
					else
						self.base_pro_list[1].base_pro:SetValue(v.maxhp * halo_equip_list[index].level * item_cfg.color)
						...
						self.special_prof:SetValue(attr_list[1])
						self.special_prof_name:SetValue(Language.Common.MountEquipAttr.per_mianshang)
						attr.max_hp = v.maxhp * halo_equip_list[index].level * item_cfg.color + attr_list[0]
					end
				end
			end
			if halo_equip_list[index].level > 0 then
				local capability = CommonDataManager.GetCapability(attr)
				self.fight_power:SetValue(capability)
				local uplevel_exp = math.ceil(0.5 * (halo_equip_list[index].level) + 2 + (halo_equip_list[index].level)^3 * GameEnum.EQUIP_UPGRADE_PERCENT) * 10
				if is_click then
					self.exp_radio:InitValue(halo_equip_list[index].exp / uplevel_exp)
				else
					self.exp_radio:SetValue(halo_equip_list[index].exp / uplevel_exp)
				end
			else
...
function AdvanceEquipHaloView:OnFlush(param_t)
	if param_t ~= nil then
