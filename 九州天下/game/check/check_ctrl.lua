...
function CheckCtrl:OnGetRoleBaseInfoAck(protocol)
	self.data:SetRoleInfoChange(protocol)
	...
	if ActivityCtrl.Instance.detail_view:IsOpen() then
		ActivityCtrl.Instance:GuildBattleViewIsOpen()
	end
	-- 开服活动战场争霸人物信息
	...
	if nil ~= self.info_call_back_list[tonumber(protocol.role_id)] then
		self.info_call_back_list[protocol.role_id]()
		self.info_call_back_list[protocol.role_id] = nil
	end

	GuildCtrl.Instance:FlushName()
end
