...
function CheckMountView:OnFlush()
	if self.mount_attr then
		...
		if self.mount_attr.used_imageid == 0 then
			...
			local image_id = MountData.Instance:GetMountGradeCfg(grade).image_id
			local color = (grade / 3 + 1) >= 5 and 5 or math.floor(grade / 3 + 1)
			local mount_cfg = MountData.Instance:GetMountImageCfg(image_id)
			local str = mount_cfg ~= nil and mount_cfg.image_name or ""
...
function CheckMountView:SetModle()
	if self.mount_attr.client_grade + 1 ~= 0 then
		if self.mount_attr.used_imageid >= GameEnum.MOUNT_SPECIAL_IMA_ID then
			...
			local cfg = MountData.Instance:GetMountImageCfg(self.mount_attr.used_imageid)
			res_id = cfg ~= nil and cfg.res_id or 0
		...
		bundle, asset = ResPath.GetMountModel(mount_res_id)
