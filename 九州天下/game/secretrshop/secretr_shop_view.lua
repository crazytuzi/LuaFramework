...
local PAGE_ROW = 1					--行
local PAGE_COLUMN = 4				--列

function SecretrShopView:__init()
	self.ui_config = {"uis/views/secretrshopview","SecretrShopView"}
...
function SecretrShopView:LoadCallBack()
	...
	self.exchange_listview_data = SecretrShopData.Instance:GetRewardCfg()
...
function SecretrShopItem:OnFlush()
	...
	self.count_limit:SetValue(num_buy.."/"..self.data.count_limit)
end
