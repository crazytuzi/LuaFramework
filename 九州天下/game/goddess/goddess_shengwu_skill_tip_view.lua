...
function GoddessShengWuSkillView:__init()
	self:SetMaskBg(true)
	self.ui_config = {"uis/views/goddess","GoddessShengWuSkillTip"}
...
function GoddessShengWuSkillView:OnFlush()
	...
	if has_next_info then
		...
		next_skill_desc = string.gsub(next_data.skill_desc, "%b()%%", function (str)
			return  (tonumber(next_data[string.sub(str, 2, -3)]) / 1000)..""
		end)
		next_skill_desc = string.gsub(next_skill_desc, "%b[]%%", function (str)
			return (tonumber(next_data[string.sub(str, 2, -3)]) / 100) .. "%"
		end)
		next_skill_desc = string.gsub(next_skill_desc, "%[.-%]", function (str)
			return next_data[string.sub(str, 2, -2)]
		end)
		self.next_gongji:SetValue(next_skill_desc)
		self.next_level_text:SetValue(string.format(Language.Goddess.GoddessSkillTipText, next_data.shengwu_level))
	end
	...
	if has_next_info then
		self.now_level_text:SetValue(Language.Goddess.GoddessUpTextJiHuo)
	else
	...
	if is_zore then
		...
		cur_skill_desc = string.gsub(now_data.skill_desc, "%b()%%", function (str)
			return  (tonumber(now_data[string.sub(str, 2, -3)]) / 1000)..""
		end)
		cur_skill_desc = string.gsub(cur_skill_desc, "%b[]%%", function (str)
			return (tonumber(now_data[string.sub(str, 2, -3)]) / 100) .. "%"
		end)
		cur_skill_desc = string.gsub(cur_skill_desc, "%[.-%]", function (str)
			return now_data[string.sub(str, 2, -2)]
		end)
		self.cur_gongji:SetValue(cur_skill_desc)
	end
	--end
end