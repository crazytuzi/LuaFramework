...
function GoddessGongMingView:LoadCallBack(instance)
		for i=0,3 do
	...
	self:ListenEvent("OnClickCheck", BindTool.Bind(self.OnClickCheck, self))
...
function GoddessGongMingView:OnClickGongMingTip()
	...
	--GoddessCtrl.Instance:OpenGoddessAttrTipView(total_attr)
	TipsCtrl.Instance:OpenGeneralView(total_attr)
end

function GoddessGongMingView:OnClickGongMingIcon(index)
	if self.hook_state then
		...
		if next(next_data) then
			...
			elseif can_click == false then
				SysMsgCtrl.Instance:ErrorRemind(Language.Goddess.GoddessUpTextNoClick)
			else
				SysMsgCtrl.Instance:ErrorRemind(Language.Goddess.GoddessUpTextNo)
			end
		else
			SysMsgCtrl.Instance:ErrorRemind(Language.Goddess.GoddessUpTextManJi)
		end
		info_data = nil
	else
...
function GoddessGongMingView:OnClickTip()
	TipsCtrl.Instance:ShowHelpTipView(209)
end

function GoddessGongMingView:OnClickCheck()
