...
function GoddessSearchAuraView:__init()
	 self.ui_config = {"uis/views/aurasearchview","GoddessSearchAuraView"}
...
function GoddessSearchAuraView:InitScroller()
	...
	for i = 1, 6 do
		...
		if value_list ~= nil and value_list[i] ~= nil then
			self.scroller_list[i].cell:SetData({index = value_list[i], key = "start"})
		end
	end
...
function GoddessSearchAuraView:LoadCallBack()
	...
	self.is_double = self:FindVariable("IsDouble")
	self.search_btn = self:FindObj("SearchBtn")
	...
	self.is_double:SetValue(self.double_receive_toggle.toggle.isOn)
...
function GoddessSearchAuraView:ReleaseCallBack()
	...
	UnityEngine.PlayerPrefs.DeleteKey("aura_double_receive")
	UnityEngine.PlayerPrefs.DeleteKey("not_aura_double_receive")
	UnityEngine.PlayerPrefs.DeleteKey("search_consume")
end
...
function GoddessSearchAuraView:Roll()
	...
	if self.receive_nums <= 0 then
		...
		if current_free_time <= 0 then
			local search_consume = GoddessData.Instance:GetAuraSearchConsume()
			local str = string.format(Language.Goddess.SearchConsume, ToColorStr(search_consume, TEXT_COLOR.BLUE))
			local click_func = function ()
				GoddessCtrl.Instance:SentCSXiannvShengwuReqReq(GODDESS_REQ_TYPE.CHOU_LING, 0, 0, 0)
			end

			if UnityEngine.PlayerPrefs.GetInt("search_consume") == 1 then
				click_func()
			else
				TipsCtrl.Instance:ShowCommonTip(click_func, nil, str, nil, nil, true, false, "search_consume")
			end
			return
		end
	end
...
function GoddessSearchAuraView:ScrollerRoll(info,time)
	...
	--  animation_time=0.0001
		for i = 1, 6 do
			if info ~= nil and info[i] ~= nil and self.scroller_list[i] ~= nil then
				self.scroller_list[i].cell:SetData({index = info[i], key = "start"})
				if info[i] == 0 then
					self.ling_count = self.ling_count + 1
				end
			end
	...
	for i= 1, 6 do
		...
		if 6 == i then
			-- 取消按钮的灰度
			self.scroller_list[i].cell:SetOnCompleteCallBack(function()
				if self.ling_count < 6 then
					self.search_btn.button.interactable = true
				end
			end)
		end

		self.scroller_list[i].cell:StartScoller(animation_time, info_list[i], i)

		if self.search_btn.button.interactable then
			self.search_btn.button.interactable = false
		end

		if info[i]==1 then 
			self.ling_count=self.ling_count+1
			self.scroller_list[i].cell.flag=true
		end  
...
function GoddessSearchAuraView:FlushButtonState()
	if self.ling_count == 6 then 
		self.search_btn.button.interactable = false
	end
end
...
function GoddessSearchAuraView:ResetEff()
	...
	local data = GoddessData.Instance:GetAuraSearchListInfo()
	local check_data = TableCopy(data)
	function sortfun(a, b)
		return a < b
	end
	table.sort(check_data, sortfun)

	for i = 1, 6 do
		if self.scroller_list[i] ~= nil then
			self.scroller_list[i].cell:ShowEffect(data[i] == 0)
		end
	end
end
...
function GoddessSearchAuraView:Receive()
	...
	if GoddessData.Instance:GetAuraDoubleReceive() then 
		...
		if self.ling_count < 6 then
			if UnityEngine.PlayerPrefs.GetInt("aura_double_receive") == 1 then
				ok_fun()
			else
				TipsCtrl.Instance:ShowCommonTip(ok_fun, nil, Language.Goddess.AuraDoubleReceiveDesc, nil, nil, true, false, "aura_double_receive")
			end
		else
			ok_fun()
		end
   ...
   --  	TipsCtrl.Instance:ShowCommonTip(func,nil,Language.AuraSearch.Remind1,nil,nil,true,nil,"GoddessSearchAuraViewTips")
   		local double_receive_func = function ()
   			self.double_receive_toggle.toggle.isOn = true
			GoddessCtrl.Instance:SentCSXiannvShengwuReqReq(GODDESS_REQ_TYPE.FETCH_LING,1,0,0)
			self:ReceiveRest()
   		end
		...
		if self.ling_count == 6 then
			if UnityEngine.PlayerPrefs.GetInt("not_aura_double_receive") == 1 then
				ok_fun()
			else
				TipsCtrl.Instance:ShowCommonTip(double_receive_func, nil, Language.Goddess.NotAuraDoubleReceiveDesc, nil, ok_fun, true, false, "not_aura_double_receive")
			end
		else
			GoddessCtrl.Instance:SentCSXiannvShengwuReqReq(GODDESS_REQ_TYPE.FETCH_LING,0,0,0)
		end
...
function GoddessSearchAuraView:ReceiveRest()-- 恢复引灵按钮
	if self.ling_count == 6 then
		self.search_btn.button.interactable = true
	end
end
...
function GoddessSearchAuraView:FlushReceivedTime()
	...
	self.red_point:SetValue(self.receive_nums > 0)
end
...
function AuraScroller:StartScoller(time, target, index)
	if self.flag and target == 1 then 
		self.is_ling = true
		if self.effect then
	...
	if 6 == index then
		tween:OnComplete(self.on_complete)
	end
...
-- 设置tween完成后的回调函数
function AuraScroller:SetOnCompleteCallBack(call_back)
	self.on_complete = call_back
end
...
function AuraScroller:ShowEffect(flag)
	if self.effect == nil then
		PrefabPool.Instance:Load(AssetID("effects2/prefab/ui/ui_zhishengyijie_002_prefab", "UI_zhishengyijie_002"), function (prefab)
			...
			if not IsNil(transform) and self.root_node ~= nil and not IsNil(self.root_node.transform) then
