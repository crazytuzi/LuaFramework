...
function ClashTerritoryData:__init()
	...
	self.territory_tower_id = ListToMap(self.clash_territory_cfg.magic_tower, "magic_tower_id")
	self.territory_building_id = ListToMap(self.clash_territory_cfg.building, "building_id")
...
function ClashTerritoryData:IsTowerId(monster_id)
	local result_cfg = self.territory_tower_id[monster_id]
	if result_cfg then
		return true
	end
	return false
...
function ClashTerritoryData:GetTerritoryMonsterSide(monster_id)
	local res_cfg = self.territory_building_id[monster_id]
	if res_cfg then
		return res_cfg.side
	end
	return nil
