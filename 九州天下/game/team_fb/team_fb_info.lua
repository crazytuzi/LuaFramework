...
function TeamFuBenInfoView:__init()
	self.ui_config = {"uis/views/fubenview", "TeamTowerFBInFoView"}
...
function TeamFuBenInfoView:OnClickRefresh()
	...
	if team_info.curr_wave + 1 == team_info.clear_wave then
		FuBenCtrl.Instance:SendTowerDefendNextWave()
	else
		SysMsgCtrl.Instance:ErrorRemind(Language.FuBen.RefreshTip)
	end
end
