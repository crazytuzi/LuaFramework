...
function RuneExchangeView:LoadCallBack()
...
function RuneExchangeView:ItemCellClick(cell)
	...
	local level_name = RuneData.Instance:GetRuneNameByItemId(data.item_id)
...
function RuneExChangeItemCell:OnFlush()
	...
	local attr_type_1 = self.data.attr_type1
	local attr_type_2 = self.data.attr_type2
	if attr_type_1 then
		RuneData.Instance:CalcAttr(attr_info, attr_type_1, self.data.attr_value1)
	end
	if attr_type_2 then
		RuneData.Instance:CalcAttr(attr_info, attr_type_2, self.data.attr_value2)
	end
