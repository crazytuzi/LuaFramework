...
function RuneCtrl:RegisterAllProtocols()
	...
	self:RegisterProtocol(SCRuneSystemZhulingRandResultInfo, "OnRuneSystemZhulingRandResultInfo")	--符文注灵抽奖返回
...
function RuneCtrl:OnRuneSystemOtherInfo(protocol)
	...
	RemindManager.Instance:Fire(RemindName.RuneInlay)
	RemindManager.Instance:Fire(RemindName.RuneAnalyze)
	RemindManager.Instance:Fire(RemindName.RuneExchange)
	RemindManager.Instance:Fire(RemindName.RuneCompose)
...
function RuneCtrl:OnRuneSystemZhulingRandResultInfo(protocol)
	...
	RemindManager.Instance:Fire(RemindName.RuneJiLian)
end
...
function RuneCtrl:OpenRuneBagView(type)
	if self.bag_view then
		local view_type = type or 0
		self.bag_view:SetViewType(view_type)
		self.bag_view:Open()
	end
end