...
function LittlePetData:OnSCLittlePetAllInfo(protocol)
	...
	all_info_list.last_free_chou_timestamp = protocol.free_chou_timestamp		-- 免费抽奖时间戳
...
function LittlePetData:GetBagLittlePetDataList()
	...
	for _, v in pairs(bag_item_data_list) do
		...
		if nil ~= item_cfg
			...
			and item_cfg.use_type == GameEnum.USE_TYPE_LITTLE_PET then
...
function LittlePetData:GetAllFightPower()
	...
	for k,v in pairs(self.mine_pet_list) do
		local temp_power = 0
		local data = v
		local base_power = LittlePetData.Instance:CalPetBaseFightPower(true, data.id)
...
function LittlePetData:SinglePetToyRemind(pet_index, info_type)
	...
	if nil == pet_equip_info or nil == next(pet_equip_info) then 
...
function LittlePetData:SetChouJiangAniState(is_on)
	self.is_can_play_chou_jiang_ani = not is_on
end
