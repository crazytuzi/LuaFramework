...
function Story:SetTrigger(trigger, trigger_param)
	...
	if trigger == S_STEP_TRIGGER.CG_END then
		TaskCtrl.Instance:OpenChapterView()
	end
...
function Story:CheckMoveInArea()
	...
	local param_t = Split(step_cfg.trigger_param, "##")
	local x, y, w, h = tonumber(param_t[1]), tonumber(param_t[2]), tonumber(param_t[3]), tonumber(param_t[4])
...
function Story:IsPlay(param1, param2)
	if param1 == "role_level" then
		return tonumber(param2) ~= PlayerData.Instance:GetRoleVo().level
	elseif param1 == "task_id" then
		return not TaskData.Instance:GetTaskIsAccepted(tonumber(param2))
	end
	return false
end
...
function Story:CheckDelayDo(now_time)
	local del_list = {}
	for i, v in ipairs(self.do_delay_list) do
		if now_time >= v.do_time then
			self:DoOperate(v.operate, v.param_t)
			table.insert(del_list, i)
		end
	end

	for i = #del_list, 1, -1 do
		table.remove(self.do_delay_list, del_list[i])
	end
end
