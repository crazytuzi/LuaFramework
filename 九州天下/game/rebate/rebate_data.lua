...
function RebateData:GetRebateRemind()
	...
	return (self:GetBaiBeiGiftAwardFlag() and history_recharge >= DailyChargeData.Instance:GetMinRecharge()) and 1 or 0
end

function RebateData:SetFirstOpenFlag()
	self.is_first_open = false
end

function RebateData:GetBaiBeiGiftAwardFlag()
	if not RebateView.HasOpenRebate and nil ~= RebateCtrl.Instance.is_buy then
