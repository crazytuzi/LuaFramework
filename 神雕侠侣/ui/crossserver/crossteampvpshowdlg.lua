require "ui.dialog"
CrossTeampvpShowDlg = {}
setmetatable(CrossTeampvpShowDlg, Dialog)
CrossTeampvpShowDlg.__index = CrossTeampvpShowDlg

------------------- public: -----------------------------------
---- singleton /////////////////////////////////////////------
local _instance;
function CrossTeampvpShowDlg.getInstance()
	print("enter getinstance")
    if not _instance then
        _instance = CrossTeampvpShowDlg:new()
        _instance:OnCreate()
    end
...
function CrossTeampvpShowDlg.getInstanceAndShow()
...
function CrossTeampvpShowDlg.getInstanceNotCreate()
    return _instance
end

function CrossTeampvpShowDlg.DestroyDialog()
	if _instance then 
		_instance:OnClose()		
		_instance = nil
	end
...
function CrossTeampvpShowDlg.ToggleOpenClose()
	if not _instance then 
		_instance = CrossTeampvpShowDlg:new() 
		_instance:OnCreate()
	else
...
function CrossTeampvpShowDlg.GetLayoutFileName()
    return "teampvpshowdlg.layout"
end
function CrossTeampvpShowDlg:OnCreate()
    ...
    self.pane = CEGUI.Window.toScrollablePane(winMgr:getWindow("teampvpshowdlg/back/main"))
	  ...
	  self.refresh:subscribeEvent("Clicked",CrossTeampvpShowDlg.HandleClicked,self)
	  self.cells = {}
	  self.battleinfo = {}
end

------------------- private: -----------------------------------
function CrossTeampvpShowDlg:new()
    local self = {}
    self = Dialog:new()
    setmetatable(self, CrossTeampvpShowDlg)
    return self
end
function CrossTeampvpShowDlg:HandleClicked(args)
  require"manager.luaprotocolmanager":send(require "protocoldef.knight.gsp.cross.creqcrossbattleinfo":new())
end


function CrossTeampvpShowDlg:Process(battleinfo)
print("battleinfo",#battleinfo)
  local winMgr = CEGUI.WindowManager:getSingleton()
  ...
  if #battleinfo > 0 then
    for i = 1 , #battleinfo do
      ...
      teamname1:setText(battleinfo[i].servername1)
      teamname2:setText(battleinfo[i].servername2)
      
      local watchBtn = CEGUI.Window.toPushButton(winMgr:getWindow(i .. "teampvpshowcell/back/btn0"))
      watchBtn:subscribeEvent("Clicked",CrossTeampvpShowDlg.ItemClick,self)
      watchBtn:setID(i)
...
function CrossTeampvpShowDlg:CreateNewCell(id)
    local winMgr = CEGUI.WindowManager:getSingleton()
...
function CrossTeampvpShowDlg:ItemClick(args)
  local p = require "protocoldef.knight.gsp.cross.cwatchcrossbattle":new()
  p.watchid = self.battleinfo[CEGUI.toWindowEventArgs(args).window:getID()].watchid
  require "manager.luaprotocolmanager":send(p)
end
return CrossTeampvpShowDlg
