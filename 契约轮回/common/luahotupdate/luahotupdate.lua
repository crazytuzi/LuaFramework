...
function HU.InitFileMap(RootPath)
    for _, rootpath in pairs(RootPath) do
        ...
        for line in io.lines() do
            local index, _ = string.find(line, ".meta")
            if not index then
...
function HU.AddFileFromHUList()
    ...
    --local file = string.gsub(HU.UpdateListFile , "%." , "/") .. ".lua"
    --local FileList = dofile(FileList);--require(HU.UpdateListFile)
    ...
    for _, file in pairs(FileList) do
        ...
        elseif string.find(file, "%$") then
            ...
            for k, v in pairs(HU.FileMap) do
                for _, path in pairs(v) do
                    local file_parts = string.split(path.SysPath, "\\")
        ...
        elseif string.find(file, "#") then
            ...
            for k, v in pairs(HU.FileMap) do
                for _, path in pairs(v) do
                    local isFind = string.find(path.SysPath, search_str)
                    local isfindCtrl = string.find(path.SysPath, "Ctrl") or string.find(path.SysPath, "Controller") or string.find(path.SysPath, "Model");
                    if isFind and not isfindCtrl then
                        HU.HUMap[path.LuaPath] = path.SysPath
                    end
                end
            end
