PetEntrancePanel = PetEntrancePanel or class("PetEntrancePanel",BaseItem)
local PetEntrancePanel = PetEntrancePanel

function PetEntrancePanel:ctor(parent_node,layer)
    self.abName = "dungeon"
    self.assetName = "PetEntrancePanel"
    self.layer = layer

    self.model = DungeonModel:GetInstance()
    self.team_model = TeamModel.GetInstance()
    self.team_events = {}
    PetEntrancePanel.super.Load(self)
    self.dungeon_type = enum.SCENE_STYPE.SCENE_STYPE_DUNGE_PET;
    DungeonCtrl:GetInstance():RequestDungeonPanel(self.dungeon_type)
end

function PetEntrancePanel:dctor()
...
function PetEntrancePanel:LoadCallBack()
    self.nodes = {
        "left", "right","bg",
    }
...
function PetEntrancePanel:AddEvent()
    local function call_back()
        if self.left_team_list then
            self.left_team_list:SetVisible(false)
        end
        if self.left_team and self.left_team.gameObject and not self.left_team.gameObject.activeInHierarchy then
            return
        end
...
function PetEntrancePanel:SetData(data, bossid)
    self.data = data
...
function PetEntrancePanel:UpdateView()
    if self.data then
...
function PetEntrancePanel:CheckTeamTarget()
