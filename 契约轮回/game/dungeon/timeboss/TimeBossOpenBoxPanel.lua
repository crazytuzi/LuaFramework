TimeBossOpenBoxPanel = TimeBossOpenBoxPanel or class("TimeBossOpenBoxPanel",WindowPanel)
local TimeBossOpenBoxPanel = TimeBossOpenBoxPanel

function TimeBossOpenBoxPanel:ctor()
	self.abName = "dungeon"
	self.assetName = "TimeBossOpenBoxPanel"
	self.layer = "UI"
...
function TimeBossOpenBoxPanel:dctor()
end

function TimeBossOpenBoxPanel:Open(data)
	self.data = data
	TimeBossOpenBoxPanel.super.Open(self)
end

function TimeBossOpenBoxPanel:LoadCallBack()
	self.nodes = {
	...
	self:SetTileTextImage("dungeon_image", "timeboss_box_title")
end

function TimeBossOpenBoxPanel:AddEvent()

	local function call_back(data)
		self.data = data
		self:UpdateInfo(data, true)
	end
	self.events[#self.events+1] = self.model:AddListener(TimeBossEvent.UpdateBoxInfo, call_back)
	...
	self.events[#self.events+1] = self.model:AddListener(TimeBossEvent.RequstOpenBox, call_back)
...
function TimeBossOpenBoxPanel:OpenCallBack()
	self:UpdateView()
end

function TimeBossOpenBoxPanel:UpdateView()
	local boss_id = self.data.boss_id
	local timebosscfg = Config.db_timeboss[boss_id]
	if timebosscfg then
		local box_reward_show = String2Table(timebosscfg.box_reward_show)
...
function TimeBossOpenBoxPanel:UpdateInfo(data, update_icon)
	local boss_id = self.data.boss_id
	local timebosscfg = Config.db_timeboss[boss_id]
	if update_icon then
	...
	if timebosscfg then
		local box_time = timebosscfg.box_time
		...
		local rewardcfg1 = Config.db_timeboss_box_reward[key1]
		if rewardcfg1 then
		...
		local rewardcfg2 = Config.db_timeboss_box_reward[key2]
		if rewardcfg2 then
...
function TimeBossOpenBoxPanel:CloseCallBack(  )
	if self.items then
...
function TimeBossOpenBoxPanel:OpenNormal()
	local boss_id = self.data.boss_id
	local timebosscfg = Config.db_timeboss[boss_id]
	if not timebosscfg then
		return Notify.ShowText("The boss data doesn't exist")
	end
	local max_open_count = timebosscfg.box_time
	...
	if had_num > self.need_num1 then
		TimeBossController.GetInstance():RequestBoxOpen(1, self.open_count, self.data.boss_id)
	else
...
function TimeBossOpenBoxPanel:OpenVip()
	local timebosscfg = Config.db_timeboss[self.data.boss_id]
	if not timebosscfg then
		return Notify.ShowText("The boss data doesn't exist")
	end
	local max_open_count = timebosscfg.box_time
	...
	local need_vip = String2Table(timebosscfg.high_box_cond)[2]
	...
	if had_num > self.need_num2 then
		TimeBossController.GetInstance():RequestBoxOpen(2, self.open_count, self.data.boss_id)
	else
