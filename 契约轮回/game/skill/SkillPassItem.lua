-- @Author: lwj
-- @Date:   2019-10-12 17:32:54
-- @Last Modified time: 2019-10-12 17:35:57

SkillPassItem = SkillPassItem or class("SkillPassItem", BaseCloneItem)
local SkillPassItem = SkillPassItem

function SkillPassItem:ctor(parent_node, layer)
    SkillPassItem.super.Load(self)
end

function SkillPassItem:dctor()
...
function SkillPassItem:LoadCallBack()
...
function SkillPassItem:AddEvent()
...
function SkillPassItem:InitItem()
    --初始化物品所在界面
...
--设置物品在技能列表中的显示
function SkillPassItem:SetListShow()
    ...
    if not self.show_cf then
        logError("SkillPassItem,Line137, " .. self.data.id .. "this skill_id doesnt exist in db_skill_show")
        return
    end
    if self.show_cf.mode == 1 then
...
function SkillPassItem:SetData(data, requireLevel, index, iconName)
    self.data = data
...
function SkillPassItem:SetShowOrHide(data, is_at_list, is_change_skill)
...
--更新自动使用显示
function SkillPassItem:UpdateAutoUseShow()
