...
---
GodCeleRankLevelView = GodCeleRankLevelView or class("GodCeleRankLevelView", BasePanel)
local this = GodCeleRankLevelView

function GodCeleRankLevelView:ctor()
    self.abName = "sevenDayActive"
    self.assetName = "GodCeleRankLevelView"
    self.layer = "UI"
    ...
    --  self.actId = actId
    self.events = {}
    --    GodCeleRankLevelView.super.Load(self)
end

function GodCeleRankLevelView:Open(actId, sort)
    self.actId = actId
    self.sort = sort
...
function GodCeleRankLevelView:dctor()
...
function GodCeleRankLevelView:LoadCallBack()
    self.nodes = {
        "closeBtn", "rightObj/downTitleBG/downTitleTex", "rightObj/btn", "GodCeleRankLevelItem", "leftObj/levelDes", "rightObj/des",
...
function GodCeleRankLevelView:InitUI()
    ...
    for i = 1, #levelTab do
        self.items[i] = GodCeleRankLevelItem(self.GodCeleRankLevelItem.gameObject, self.Content, "UI")
...
function GodCeleRankLevelView:AddEvent()
    ...
    local function call_back()
        --if self.linkId == 900 then
        --end
        self:Close()
        UnpackLinkConfig(self.linkId .. "@" .. self.linkSubId)
    end
    AddClickEvent(self.btn.gameObject, call_back)
    self.events[#self.events + 1] = GlobalEvent:AddListener(GodCeleEvent.SevenDayRankClickLevelItem, handler(self, self.SevenDayRankClickLevelItem))
end

function GodCeleRankLevelView:SevenDayRankClickLevelItem(data, index)
    self:SetSelect(index)
    self:UpdateInfo(data)
end

function GodCeleRankLevelView:UpdateInfo(data)
...
function GodCeleRankLevelView:SetSelect(index)
