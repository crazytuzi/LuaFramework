...
---
GodCeleRankPanel = GodCeleRankPanel or class("GodCeleRankPanel", BaseItem)
local this = GodCeleRankPanel

function GodCeleRankPanel:ctor(parent_node, parent_panel, actID)

    self.abName = "sevenDayActive"
    self.assetName = "GodCeleRankPanel"
    self.layer = "UI"
    ...
    self.rewardItems = {}
    self.rankItems = {}
    GodCeleRankPanel.super.Load(self)
end

function GodCeleRankPanel:dctor()
...
function GodCeleRankPanel:LoadCallBack()
    ...
    self.nodes = {
        --"lastBtn",
        "rankObj/myRank/myRank", "GodCeleRankRewardItem", "GodCeleRankItem", "levelBtn",
...
function GodCeleRankPanel:InitUI()
    local rankId = OperateModel:GetInstance():GetConfig(self.actID).rank
...
function GodCeleRankPanel:InitTextPic()
...
function GodCeleRankPanel:TextureCallBack(sp)
    self.leftzi.sprite = sp
    LayerManager:GetInstance():AddOrderIndexByCls(self, self.leftzi.transform, nil, true, nil, nil, 4)
end

function GodCeleRankPanel:InitModel()
    ...
    if tab[1] == "model" then
        ...
        self.monster = UIModelCommonCamera(self.modelCon, nil, "model_soul_" .. id);--data.icon

        local config = {};
        config.scale = { x = 40, y = 40, z = 40 };
        config.pos = { x = -1990.4, y = -137.8, z = 414.4 };
...
function GodCeleRankPanel:SetMyInfo(main)
    ...
    elseif rankId == 150101 then
        self.myPower.text = string.format("My: %d", main.sort)
    else
        self.myPower.text = "My:" .. main.sort
    end
end

function GodCeleRankPanel:InitRewardItem()
    ...
    if rewardCfg then
        ...
        for i, v in pairs(rewardCfg) do
            self.rewardItems[v.id] = GodCeleRankRewardItem(self.GodCeleRankRewardItem.gameObject, self.rewardContent, "UI")
            self.rewardItems[v.id]:SetData(v, self.StencilId)
        end
    end
end
function GodCeleRankPanel:InitActTime()
    --  if self.openData then
...
function GodCeleRankPanel:AddEvent()
    ...
    local function call_back()
        ...
        lua_panelMgr:GetPanelOrCreate(GodCeleRankLevelView):Open(self.actID, self.showNum)
    end
...
function GodCeleRankPanel:GetActTime(time)
...
function GodCeleRankPanel:CountDown()
    local timeTab = nil;
    local timestr = "";
    local formatTime = "%d";
    ...
    if table.isempty(timeTab) then
        ...
        self.rTime.text = string.format("Event time left：<color=#%s>%s</color>%s", "ff0000", "Ended", "(Ranking rewards will be sent via mails at 00:00)")
        ...
        local color = "27C31F"
        self.rTime.text = string.format("Event time left：<color=#%s>%s</color>%s", color, timestr, "(Ranking rewards will be sent via mails at 00:00)")
        -- self.rTime.text = "活动剩余：" .. timestr;
    end

end

function GodCeleRankPanel:RankReturnList(data)
    local cfg = OperateModel:GetInstance():GetConfig(self.actID)
    ...
    if data.id == rankId then
        ...
        for i = 1, size do
            self.rankItems[i] = GodCeleRankItem(self.GodCeleRankItem.gameObject, self.rankContent, "UI")
            if i <= ranklen then
