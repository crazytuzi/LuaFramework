-- @Author: lwj
-- @Date:   2019-07-19 20:27:40 
-- @Last Modified time: 2019-07-19 20:27:42

OpenHighRewaItem = OpenHighRewaItem or class("OpenHighRewaItem", BaseCloneItem)
local OpenHighRewaItem = OpenHighRewaItem

function OpenHighRewaItem:ctor(parent_node, layer)
    OpenHighRewaItem.super.Load(self)
end

function OpenHighRewaItem:dctor()
...
function OpenHighRewaItem:LoadCallBack()
...
function OpenHighRewaItem:AddEvent()
...
function OpenHighRewaItem:SetData(data, stencil_id, stencil_type)
...
function OpenHighRewaItem:UpdateView()
    ...
    if state == enum.YY_TASK_STATE.YY_TASK_STATE_UNDONE then
        lua_resMgr:SetImageTexture(self, self.tag_img, "openHigh_image", "not_achi", true, nil, false)
...
function OpenHighRewaItem:LoadReward()
...
function OpenHighRewaItem:SetRedDot(isShow)
