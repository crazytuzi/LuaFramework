...
---
SevenDayRankLastPanel = SevenDayRankLastPanel or class("SevenDayRankLastPanel", WindowPanel)
local this = SevenDayRankLastPanel

function SevenDayRankLastPanel:ctor(parent_node, parent_panel)
    self.abName = "sevenDayActive"
    self.assetName = "SevenDayRankLastPanel"
    self.layer = "UI"
    self.events = {}
    self.rankItems = {}
...
function SevenDayRankLastPanel:dctor()
...
function SevenDayRankLastPanel:Open(rankID)
    self.rankID = rankID
    SevenDayRankLastPanel.super.Open(self)
end

function SevenDayRankLastPanel:LoadCallBack()
    self.nodes = {
...
function SevenDayRankLastPanel:InitUI()

end

function SevenDayRankLastPanel:AddEvent()
    self.events[#self.events+1] = GlobalEvent.AddEventListener(RankEvent.RankReturnList,handler(self,self.RankReturnList))
end

function SevenDayRankLastPanel:RankReturnList(data)
...
function SevenDayRankLastPanel:SetMyInfo(mine)
    local rankStr = ""
