...
--- DateTime: 2019/8/23 17:34
---
SevenDayPetRankLevelItem = SevenDayPetRankLevelItem or class("SevenDayPetRankLevelItem", BaseCloneItem)
local this = SevenDayPetRankLevelItem

function SevenDayPetRankLevelItem:ctor(obj, parent_node, parent_panel)
    SevenDayPetRankLevelItem.super.Load(self)
    self.events = {}
end

function SevenDayPetRankLevelItem:dctor()
    GlobalEvent:RemoveTabListener(self.events)
end

function SevenDayPetRankLevelItem:LoadCallBack()
    self.nodes = {
...
function SevenDayPetRankLevelItem:InitUI()

end

function SevenDayPetRankLevelItem:AddEvent()
    local function call_back()
        GlobalEvent:Brocast(SevenDayActiveEvent.SevenDayPetRankClickLevelItem, self.data,self.index)
    end
    AddClickEvent(self.bg.gameObject,call_back)
end

function SevenDayPetRankLevelItem:SetData(data,index)
    self.data = data
    self.index = index
...
function SevenDayPetRankLevelItem:SetSelect(show)
