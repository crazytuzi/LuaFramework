...
--- DateTime: 2019/8/23 15:34
---
SevenDayPetRechargePanel = SevenDayPetRechargePanel or class("SevenDayPetRechargePanel", BaseItem)
local this = SevenDayPetRechargePanel

function SevenDayPetRechargePanel:ctor(parent_node, parent_panel,actID)

    self.abName = "sevenDayActive"
    self.assetName = "SevenDayPetRechargePanel"
    self.layer = "UI"
...
function SevenDayPetRechargePanel:dctor()
...
function SevenDayPetRechargePanel:LoadCallBack()
    self.nodes = {
...
function SevenDayPetRechargePanel:InitUI()
   -- logError(self.actID)
...
function SevenDayPetRechargePanel:AddEvent()
...
function SevenDayPetRechargePanel:HandlerRewardInfo(data)
...
function SevenDayPetRechargePanel:DLIVER_YY_INFO(data)
...
function SevenDayPetRechargePanel:SetMask()
...
function SevenDayPetRechargePanel:UpdateRewards(tab)
    local rewards = tab
...
function SevenDayPetRechargePanel:InitActTime()
...
function SevenDayPetRechargePanel:PaySucc()
    local data = OperateModel:GetInstance():GetActInfo(self.actID)
    self:UpdateRewards(data.tasks)
end


function SevenDayPetRechargePanel:GetActTime(time)
...
function SevenDayPetRechargePanel:CountDown()
    local timeTab = nil;
    local timestr = "";
    local formatTime = "%d";
