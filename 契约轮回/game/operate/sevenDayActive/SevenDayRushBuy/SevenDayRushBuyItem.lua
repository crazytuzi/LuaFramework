...
--- DateTime: 2019/4/23 17:21
---
SevenDayRushBuyItem = SevenDayRushBuyItem or class("SevenDayRushBuyItem", BaseCloneItem)
local this = SevenDayRushBuyItem

function SevenDayRushBuyItem:ctor(obj, parent_node, parent_panel)
    SevenDayRushBuyItem.super.Load(self)
    self.events = {}
    self.itemicon = {}
end

function SevenDayRushBuyItem:dctor()
...
function SevenDayRushBuyItem:LoadCallBack()
    self.nodes = {
...
function SevenDayRushBuyItem:InitUI()

end

function SevenDayRushBuyItem:AddEvent()
...
function SevenDayRushBuyItem:SetData(data,actid,stencilId)
...
function SevenDayRushBuyItem:SetInfo()
  --  local cfg = Config.db_mall[self.data.id]
   -- if cfg then
    ...
    self.discountNum.text = (100 - 10 * self.data.discount) .. "%Off"
...
function SevenDayRushBuyItem:SetTitle(name)
    
end

function SevenDayRushBuyItem:SetPic(lastNum,curNum,lastId,curId)
...
function SevenDayRushBuyItem:CreateIcon()
    local index = 0
