...
---
SevenDayRushBuyPanel = SevenDayRushBuyPanel or class("SevenDayRushBuyPanel", BaseItem)
local this = SevenDayRushBuyPanel

function SevenDayRushBuyPanel:ctor(parent_node, parent_panel,actID)
    self.abName = "sevenDayActive"
    self.assetName = "SevenDayRushBuyPanel"
    self.layer = "UI"
...
function SevenDayRushBuyPanel:BeforeLoad()
    SevenDayRushBuyPanel.super.Load(self)
end

function SevenDayRushBuyPanel:dctor()
...
function SevenDayRushBuyPanel:LoadCallBack()
    ...
    self.nodes = {
        "ScrollView/Viewport/Content","rTime","SevenDayRushBuyItem","ScrollView/Viewport"
    }
...
function SevenDayRushBuyPanel:InitUI()
...
function SevenDayRushBuyPanel:CountDown()
    local timeTab = nil;
    local timestr = "";
    local formatTime = "%d";
...
function SevenDayRushBuyPanel:UpdateItems(items)
    local tab = items
    ...
    for i = 1, #tab do
        ...
        if not item then
            item  =   SevenDayRushBuyItem(self.SevenDayRushBuyItem.gameObject,self.Content,"UI")
...
function SevenDayRushBuyPanel:UpdateShopBoughtList()
...
function SevenDayRushBuyPanel:UpdateShopItemLimit()
  --  print2("fffffffffffffffffffff")
  --  dump(ShopModel:GetInstance().goodsBoughtList)
end

function SevenDayRushBuyPanel:SuccessToBuyGoodsInShop()
   -- print2("购买成功")
  --  Notify.ShowText("购买成功")
end

function SevenDayRushBuyPanel:HandleActItems(data)
  --  print2("活动商品")
   -- dump(data)
    local tab = {}
