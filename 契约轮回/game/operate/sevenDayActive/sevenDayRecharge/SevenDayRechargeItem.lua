...
---
SevenDayRechargeItem = SevenDayRechargeItem or class("SevenDayRechargeItem", BaseCloneItem)
local this = SevenDayRechargeItem

function SevenDayRechargeItem:ctor(obj, parent_node, parent_panel)
    SevenDayRechargeItem.super.Load(self)
    self.events = {}
    self.itemicon = {}
    self.model = SevenDayActiveModel:GetInstance()
end

function SevenDayRechargeItem:dctor()
...
function SevenDayRechargeItem:LoadCallBack()
    self.nodes = {
...
function SevenDayRechargeItem:InitUI()

end

function SevenDayRechargeItem:AddEvent()
...
function SevenDayRechargeItem:SetData(data, actId, type, stencilId)
...
function SevenDayRechargeItem:UpdateInfo(data)

    self:SetDes()
    self:SetState(data.state)
end

function SevenDayRechargeItem:SetDes()
    ...
    if self.type == 3 then
        ...
        elseif self.actId == 110305 then
            ...
            if self.actId == 150201 then
                local num = String2Table(self.cfgData.task)[3]
                local color = "0DB420"
                if self.data.count < num then
                    color = "FF0000"
                end
                ...
                SetLocalPositionX(self.numTex.transform, self.des.preferredWidth)
                --SetLocalPositionX(self.zhuanshiIcon.transform, self.numTex.preferredWidth + self.des.preferredWidth)
                SetLocalPositionX(self.lqTex, self.numTex.preferredWidth + self.des.preferredWidth + 5)
...
function SevenDayRechargeItem:SetState(state)
    if self.type == 3 then
...
function SevenDayRechargeItem:CreateRewards()
    for i, v in pairs(self.itemicon) do
        v:destroy()
    end
    self.itemicon = {}
