local CTerraWarRewardPage = class("CTerraWarRewardPage", CPageBase)

function CTerraWarRewardPage.ctor(self, cb)
	CPageBase.ctor(self, cb)
end

function CTerraWarRewardPage.OnInitPage(self)
...
function CTerraWarRewardPage.InitContent(self)
...
function CTerraWarRewardPage.InitTotalTable(self)
	self:InitWorldBox()
	self:InitDescLabel()
	self:InitOrgBox()
...
function CTerraWarRewardPage.InitWorldBox(self)
...
function CTerraWarRewardPage.InitDescLabel(self)
	local desclabel = self.m_DescLabelClone:Clone()
	desclabel:SetActive(true)
	desclabel:SetText("")
	self.m_TotalTable:AddChild(desclabel)
end

function CTerraWarRewardPage.InitOrgBox(self)
	local list = data.terrawardata.ORGREWARD

	local titlelabel = self.m_TitleLabelClone:Clone()
	titlelabel:SetActive(true)
	titlelabel:SetText("公会奖励")
	self.m_TotalTable:AddChild(titlelabel)
