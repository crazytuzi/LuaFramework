...
function CMainMenuChatBox.InitAudioBox(self)
...
function CMainMenuChatBox.ReloadMsg(self)
	self.m_MsgTable:Clear()
	self.m_CurAppendIdx = 0
	local len = #self.m_MsgList
	for i=len, len-self.m_AddCnt, -1 do
...
function CMainMenuChatBox.OnOpenMainView(self, iChannel)
	CChatMainView:ShowView(function(oView)
		oView:SwitchChannel(iChannel)
	end)
end

function CMainMenuChatBox.CheckBoxCollider(self)
...
function CMainMenuChatBox.OnExpand(self)
...
function CMainMenuChatBox.OnPull(self)
...
function CMainMenuChatBox.RefreshSize(self, nottask)
	local height = self.m_IsExpand and 240 or 90
	if self.m_LastHeight and self.m_LastHeight == height then
		return
	end
...
function CMainMenuChatBox.RefreshButtonPos(self, nottask)
	local isShowView = true
	local oView = CMainMenuView:GetView()
	if oView then
		isShowView = oView.m_IsShowView
		if oView.m_LB and oView.m_LB.m_ActivityBox then
			oView.m_LB.m_ActivityBox:SetActive(isShowView)
		end
	end
	if isShowView and g_AttrCtrl.grade >= data.globalcontroldata.GLOBAL_CONTROL.friend.open_grade then
		if self.m_IsExpand or nottask then
			self.m_AudioBox.m_AudioPart:SetLocalPos(Vector3.New(90, 0, 0))
			self.m_AudioBox.m_HousePart:SetLocalPos(Vector3.New(0, 0, 0))
		else
			self.m_AudioBox.m_AudioPart:SetLocalPos(Vector3.New(0, 0, 0))
			self.m_AudioBox.m_HousePart:SetLocalPos(Vector3.New(0, 90, 0))
		end
	else
		self.m_AudioBox.m_AudioPart:SetLocalPos(Vector3.New(0, 0, 0))
		self.m_AudioBox.m_HousePart:SetLocalPos(Vector3.New(0, 0, 0))
	end
	oView = CTeamPvpRankView:GetView()
	if oView then
		oView:SetActive(isShowView)
	end
end

function CMainMenuChatBox.CheckOpenGrade(self)
	self:RefreshButtonPos()
end

function CMainMenuChatBox.OnSpeech(self, iChannel, oBtn, bPress)
...
function CMainMenuChatBox.RecordTimeOut(self, iChannel)
...
function CMainMenuChatBox.OnClickAudio(self, iChannel)
...
function CMainMenuChatBox.OnExpandAudio(self)
	self.m_IsExpandAudio = not self.m_IsExpandAudio 
	self:OnResortAudio(not self.m_IsExpandAudio)
end

function CMainMenuChatBox.OnResortAudio(self, ishide)
...
function CMainMenuChatBox.IsShowChannelAudio(self, iChannel)
