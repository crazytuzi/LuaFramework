local CParSoulSelectBox = class("CParSoulSelectBox", CBox)

function CParSoulSelectBox.ctor(self, obj)
	CBox.ctor(self, obj)
	self.m_RowAmount = 3
...
CParSoulSelectBox.WearSel = true
function CParSoulSelectBox.InitItemPart(self)
	...
	self.m_BackTypeBtn:AddUIEvent("click", callback(self, "SwitchTypePart"))
	...
	for k, v in pairs(CParSoulSelectBox.g_SortData) do
	...
	self.m_WearBtn:SetSelected(CParSoulSelectBox.WearSel)
end

function CParSoulSelectBox.InitTypePart(self)
...
function CParSoulSelectBox.SetItemWrapCloneChild(self, oChild)
	oChild.m_IconList = {}
	for i = 1, 3 do
...
function CParSoulSelectBox.SetItemWrapRefreshFunc(self, oChild, dData)
	if dData then
		oChild:SetActive(true)
		for i = 1, 3 do
...
function CParSoulSelectBox.SetTypeWrapCloneChild(self, oBox)
	oBox:SetActive(true)
...
function CParSoulSelectBox.SetTypeWrapRefreshFunc(self, oChild, idx)
...
function CParSoulSelectBox.OnItemCtrlEvent(self, oCtrl)
...
function CParSoulSelectBox.SetPartnerID(self, iParID)
...
function CParSoulSelectBox.GetTypeEquipList(self)
...
function CParSoulSelectBox.SortTypeList(self, list, iSoulType, amountdict)
...
function CParSoulSelectBox.UpdateTypeBox(self, box, typedata)
...
function CParSoulSelectBox.RefreshTypePart(self)
	if not self.m_TypePart:GetActive() then
		self.m_TypePart.m_Update = true
	end
...
function CParSoulSelectBox.ShowTypePart(self)
	self.m_ItemPart:SetActive(false)
	self.m_TypePart:SetActive(true)
	self:RefreshTypePart()
end

function CParSoulSelectBox.SwitchTypePart(self)
	if self.m_TypePart.m_Update then
		self:ShowTypePart()
		self.m_TypePart.m_Update = false
	else
		self.m_ItemPart:SetActive(false)
		self.m_TypePart:SetActive(true)
	end
...
function CParSoulSelectBox.ShowItemPart(self)
	self.m_ItemPart:SetActive(true)
	self.m_TypePart:SetActive(false)
end

function CParSoulSelectBox.OnDealyUpdate(self)
...
function CParSoulSelectBox.RefreshItemPart(self)
...
function CParSoulSelectBox.GetParSoulList(self)
...
function CParSoulSelectBox.SortItem(self, list)
	local sortList = {}
...
function CParSoulSelectBox.GetDivideList(self, itemList)
	local newlist = {}
	local data = {}
...
function CParSoulSelectBox.OnSortChange(self, oBox)
	local idx = self.m_SortPopupBox:GetSelectedIndex()
	self.m_SortKey = CParSoulSelectBox.g_SortData[idx][1]
	self:RefreshItemPart()
end

function CParSoulSelectBox.OnChangeWear(self)
	CParSoulSelectBox.WearSel = self.m_WearBtn:GetSelected()
	self:RefreshItemPart()
end

function CParSoulSelectBox.OnClickType(self, typeid)
	self.m_CoreType = typeid
	self:ShowItemPart()
	self:RefreshItemPart()
end

function CParSoulSelectBox.OnShowGetHelp(self)
...
function CParSoulSelectBox.OnPress(self, typeid, box, press)
...
function CParSoulSelectBox.OnFastEquip(self, iCoreType)
	local itemList = g_ItemCtrl:GetParSoulList()
	local resultList = {}
	local iswearequip = self.m_WearBtn:GetSelected()
	...
	for _, oItem in ipairs(itemList) do
		if oItem:GetValue("soul_type") == iCoreType and oItem:GetValue("parid") == 0 then
	...
	local dPosDict = oPartner:GetParSoulList()
	local dSendPos = {}
	for i = 1, iAmount do
	...
	netpartner.C2GSUsePartnerSoulType(iCoreType, dSendList, oPartner.m_ID)
end

function CParSoulSelectBox.GetPosList(self, iAmount, iCoreType, resultList)
	local oPartner = g_PartnerCtrl:GetPartner(self.m_CurParID)
	local dPosDict = oPartner:GetParSoulList()
	local dPosList = {}
	for i = 1, iAmount do
...
function CParSoulSelectBox.SortAttrType(self, type2name, resultList)
	local typeSortList = {}
...
function CParSoulSelectBox.GetLockAmount(self)
...
function CParSoulSelectBox.OnClickItem(self, oItem)
	if oItem then
		local oPartner = g_PartnerCtrl:GetPartner(self.m_CurParID)
