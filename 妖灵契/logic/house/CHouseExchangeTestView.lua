local CHouseExchangeTestView = class("CHouseExchangeTestView", CViewBase)
--伙伴交流界面
function CHouseExchangeTestView.ctor(self, cb)
	CViewBase.ctor(self, "UI/House/HouseExchangeTestView.prefab", cb)

	self.m_GroupName = "House"
	
end

function CHouseExchangeTestView.OnCreateView(self)
	...
	self.m_ResetBtn = self:NewUI(21, CButton)

	-- self.m_SpeakTween = self.m_SpeakLabel:GetComponent(classtype.TweenScale)
	self.m_Mode = nil
	self.m_SoundPlayer = g_AudioCtrl:GetSoundPlayer()
	self.m_TouchCnt = 0
	self.m_ResetBtn:SetText(self.m_TouchCnt)
	self:CreatePartnerData()
	self:InitTouchPart()
	self:InitExpInfoBox()
	self:InitContent()
	self.m_IsTouchIn = false
	-- g_CameraCtrl:AutoActive()
	self.m_TouchPart:SetActive(false)
end

function CHouseExchangeTestView.InitContent(self)
...
function CHouseExchangeTestView.OnClickReset(self)
	self.m_TouchCnt = 0
	self.m_ResetBtn:SetText(self.m_TouchCnt)
	self:CreatePartnerData()
...
function CHouseExchangeTestView.CheckRedDot(self)
...
function CHouseExchangeTestView.OnClickHide(self)
...
function CHouseExchangeTestView.InitExpInfoBox(self)
	...
	function oExpInfoBox.Refresh(self, bPlay)
		...
		local dPartnerInfo = self.m_ParentView.m_PartnerInfo[oExpInfoBox.m_ParentView.m_CurType]
...
--宅邸引导
function CHouseExchangeTestView.OnGuide(self)
	...
	-- end
	self:RefreshExp(self.m_CurType, 1)
	g_GuideCtrl:ReqForwardTipsGuideFinish("house_touch_btn")
end

function CHouseExchangeTestView.OnQuit(self)
	g_HouseCtrl:LeaveHouse()
end

function CHouseExchangeTestView.OnClickPre(self)
...
function CHouseExchangeTestView.OnClickNext(self)
	if self.m_PartnerList == nil or #self.m_PartnerList < 2 then
		return
	end
	self.m_CurrentIndex = self.m_CurrentIndex + 1
...
function CHouseExchangeTestView.SetPartnerInfo(self, iType)
...
function CHouseExchangeTestView.RefeshPartnerInfo(self)
...
function CHouseExchangeTestView.InitTouchPart(self)
...
function CHouseExchangeTestView.SetMode(self, sMode)
	if sMode=="task" then
...
function CHouseExchangeTestView.OnTouchExchange(self, dData)
	...
	self:PlaySpeak(dData)
	self:RefreshExp(self.m_CurType, dData.part)
	-- if g_HouseCtrl.m_CurTouchCnt > 0 then
...
function CHouseExchangeTestView.RefreshExp(self, iType, iPart)
	local oInfo = self.m_PartnerInfo[iType]
	local iAdd = Utils.RandomInt(247, 413)

	-- for k,v in pairs(data.housedata.Love_Stage) do
	-- 	if oInfo.love_level >= v.min_level and oInfo.love_level <= v.max_level then
	-- 		iAdd = data.housedata.PartnerLove[iType][v.stage][iPart] or 400
	-- 	end
	-- end
	
	oInfo.love_ship = oInfo.love_ship + iAdd
	local oData = data.housedata.HouseLove[oInfo.love_level]
	while oInfo.love_ship >= oData.loveship do
		oInfo.love_ship = oInfo.love_ship - oData.loveship
		oInfo.love_level = oInfo.love_level + 1
		oData = data.housedata.HouseLove[oInfo.love_level]
	end
	self.m_TouchCnt = self.m_TouchCnt + 1
	self.m_ResetBtn:SetText(self.m_TouchCnt)
	self.m_ExpInfoBox:Refresh(true)
end

function CHouseExchangeTestView.CreatePartnerData(self)
	self.m_PartnerInfo = g_HouseCtrl:GetPartnerInfos()
	for k,v in pairs(self.m_PartnerInfo) do
		v.love_level = 0
		v.love_ship = 0
	end
end

function CHouseExchangeTestView.PlaySpeak(self, dData)
	if dData then
...
function CHouseExchangeTestView.HideTalk(self, oAudioPlayer)
...
function CHouseExchangeTestView.OnDragging(self, oDragObj)

end

function CHouseExchangeTestView.OnDragEnd(self, oDragObj)
...
function CHouseExchangeTestView.OnHouseEvent(self, oCtrl)
	self:CheckRedDot()
...
function CHouseExchangeTestView.OnCtrlGuideEvent(self, oCtrl)
...
function CHouseExchangeTestView.Destroy(self)
...
function CHouseExchangeTestView.OnClickClose(self)
...
function CHouseExchangeTestView.SetTouchIn(self, bValue)
	self.m_IsTouchIn = bValue
end

function CHouseExchangeTestView.OnTrain(self, iType)
