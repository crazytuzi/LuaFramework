local CPEWarResultView = class("CPEWarResultView", CViewBase)

function CPEWarResultView.ctor(self, cb)
	CViewBase.ctor(self, "UI/Activity/PartnerEquipFuben/PEWarResultView.prefab", cb)
	self.m_ExtendClose = "Black"
end

function CPEWarResultView.OnCreateView(self)
	...
	self.m_PassStarGrid = self:NewUI(21, CGrid)
...
function CPEWarResultView.OnShowView(self)
	self.m_WarID = nil
...
function CPEWarResultView.OnHideView(self)
...
function CPEWarResultView.DoHideCallback(self)
	
end

function CPEWarResultView.OrgFuBenWarEnd(self)
...
function CPEWarResultView.OnClickPowerPartner(self)
	g_WarCtrl:SetWarEndAfterCallback(function ()
...
function CPEWarResultView.OnClickPowerHero(self)
	g_WarCtrl:SetWarEndAfterCallback(function ()
		local level = data.globalcontroldata.GLOBAL_CONTROL.powerguide.open_grade
		if g_AttrCtrl.grade >= level then
...
function CPEWarResultView.OnWarEvnet(self, oCtrl)
...
function CPEWarResultView.SetWin(self, bWin)
...
function CPEWarResultView.RefreshResultTexture(self, sType)
...
function CPEWarResultView.ShowEndlessPVEResult(self)
...
function CPEWarResultView.SetWarID(self, id)
	self.m_WarID = id
	self:RefeshAll()
	self:AutoDoShiMen()
end

function CPEWarResultView.RefeshAll(self)
	...
	self.m_ApplyInfo = dResultInfo.apply
...
function CPEWarResultView.RefreshExpGrid(self)
	self.m_ExpGrid:Clear()
...
function CPEWarResultView.BoxExpAnim(self, oBox)
	...
	if oBox.m_CurExp >= oBox.m_MaxExp and oBox.m_CurGrade < oBox.m_LimitGrade  then
		...
		if not Utils.IsNil(oBox.m_LvLabel) then
			...
			CPEWarResultView:DoBoxExpEffect(oBox)
		end
		oBox.m_MaxExp = nil
...
function CPEWarResultView.DoBoxExpEffect(cls, oBox)
...
function CPEWarResultView.RefreshItemGrid(self)
	self.m_ItemGrid:Clear()
...
function CPEWarResultView.RefreshWinFailTip(self, bWin)
	local dResultInfo = g_WarCtrl.m_ResultInfo
...
function CPEWarResultView.IsPartnerEquip(self, shape)
	return 
end

function CPEWarResultView.RefreshEndlessRing(self)
	self.m_LittleTitleLabel:SetText(string.format("通关[F2B41AFF]%s[-]波", g_EndlessPVECtrl:GetRingInfo()))
end

function CPEWarResultView.SetFieldBossView(self)
...
function CPEWarResultView.CloseView(self)
	CViewBase.CloseView(self)
	g_WarCtrl:SetInResult(false)
end

function CPEWarResultView.OnNotify(self, oCtrl)
	if oCtrl.m_EventID == define.EndlessPVE.Event.OnWarEnd then
		self:RefreshEndlessRing()
	end
end

function CPEWarResultView.SetDelayCloseView(self)
...
function CPEWarResultView.Destroy(self)
	if self.m_DelayCloseTimer ~= nil then
		Utils.DelTimer(self.m_DelayCloseTimer)
		self.m_DelayCloseTimer = nil
	end
...
function CPEWarResultView.StopAutoDoingShiMenTimer(self)
...
function CPEWarResultView.AutoDoShiMen(self)
...
function CPEWarResultView.RefreshPassStarGrid(self)
	local d = self.m_ApplyInfo or {}
	local iStar = 0
	local iTurn = 0
	...
	local textList = {"战斗胜利", "己方无阵亡单位", string.format("%d回合通关", iTurn)}
	for i,oBox in ipairs(self.m_PassStarGrid:GetChildList()) do
		oBox.m_StarSprite:SetActive(iStar >= i)
		oBox.m_CondionLabel:SetText(textList[i])
	end
end


return CPEWarResultView