...
local function default_table(typename)
	...
	v = { __index = assert(decode_message(typename , "")),
		__newindex = function (t, k, v)
				rawset(t, k, v)
				printerror("protobuf can't change default table:", typename, k, v)
			end,}
...
-- function expandall(tbl)
-- 	if getmetatable(tbl) == decode_message_mt then
-- 		 expand(tbl)
-- 	end 
...
function registerProto(filename)
	local buffer = C_api.Utils.LoadProto(filename)
	c._env_register(P, buffer)
end
