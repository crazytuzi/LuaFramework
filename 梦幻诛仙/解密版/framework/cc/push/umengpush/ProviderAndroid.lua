...
function ProviderAndroid:addListener()
  luaj.callStaticMethod(SDK_CLASS_NAME, "addScriptListener", {
...
function ProviderAndroid:setTags(tags)
  local ok = luaj.callStaticMethod(SDK_CLASS_NAME, "setTags", {
    table.concat(tags, ",")
  })
  if not ok then
    printError("cc.push.ProviderAndroid:ctor() - call delAlias failed.")
...
function ProviderAndroid:delTags(tags)
  local ok = luaj.callStaticMethod(SDK_CLASS_NAME, "delTags", {
    table.concat(tags, ",")
  })
  if not ok then
    printError("cc.push.ProviderAndroid:ctor() - call delAlias failed.")
