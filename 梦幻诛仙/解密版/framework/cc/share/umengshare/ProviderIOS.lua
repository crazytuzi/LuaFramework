...
function ProviderIOS:addListener()
  luaoc.callStaticMethod(SDK_CLASS_NAME, "addScriptListener", {
    listener = handler(self, self.callback_)
  })
end
...
function ProviderIOS:shareImg(text, img)
  local ok = luaoc.callStaticMethod(SDK_CLASS_NAME, "share", {shareText = text, shareImage = img})
  if not ok then
