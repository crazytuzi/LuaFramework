local StudyEffectInfo = require("netio.protocol.mzm.gsp.children.StudyEffectInfo")
local SFinishCourseSuccess = class("SFinishCourseSuccess")
SFinishCourseSuccess.TYPEID = 12609302
function SFinishCourseSuccess:ctor(childid, study_effect_info)
  self.id = 12609302
  self.childid = childid or nil
  self.study_effect_info = study_effect_info or StudyEffectInfo.new()
end
function SFinishCourseSuccess:marshal(os)
  os:marshalInt64(self.childid)
  self.study_effect_info:marshal(os)
end
function SFinishCourseSuccess:unmarshal(os)
  self.childid = os:unmarshalInt64()
  self.study_effect_info = StudyEffectInfo.new()
  self.study_effect_info:unmarshal(os)
end
function SFinishCourseSuccess:sizepolicy(size)
  return size <= 65535
end
return SFinishCourseSuccess
