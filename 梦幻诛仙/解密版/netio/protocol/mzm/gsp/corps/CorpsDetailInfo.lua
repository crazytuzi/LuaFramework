...
local CorpsDetailInfo = class("CorpsDetailInfo")
function CorpsDetailInfo:ctor(corpsBriefInfo, members)
  self.corpsBriefInfo = corpsBriefInfo or CorpsBriefInfo.new()
  self.members = members or {}
end
function CorpsDetailInfo:marshal(os)
...
function CorpsDetailInfo:unmarshal(os)
  self.corpsBriefInfo = CorpsBriefInfo.new()
  self.corpsBriefInfo:unmarshal(os)
  for _size_ = os:unmarshalCompactUInt32(), 1, -1 do
    local k = os:unmarshalInt64()
    local BeanClazz = require("netio.protocol.mzm.gsp.corps.CorpsMemberDetailInfo")
    local v = BeanClazz.new()
    v:unmarshal(os)
    self.members[k] = v
  end
end
return CorpsDetailInfo
