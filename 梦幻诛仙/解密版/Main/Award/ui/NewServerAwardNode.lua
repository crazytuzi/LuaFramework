...
local NewServerAwardNode = Lplus.Extend(AwardPanelNodeBase, "NewServerAwardNode")
...
local GUIUtils = require("GUI.GUIUtils")
local def = NewServerAwardNode.define
...
def.override().OnShow = function(self)
  ...
  Event.RegisterEvent(ModuleId.ACHIEVEMENT, gmodule.notifyId.Achievement.ACHIEVEMENT_GOAL_INFO_CHANGE, NewServerAwardNode.OnAchievementGoaInfoChagnge)
  Event.RegisterEvent(ModuleId.ACHIEVEMENT, gmodule.notifyId.Achievement.ACHIEVEMENT_GET_SCORE_AWARD, NewServerAwardNode.OnAchievementScoreInfoChagnge)
end
def.override().OnHide = function(self)
  ...
  Event.UnregisterEvent(ModuleId.ACHIEVEMENT, gmodule.notifyId.Achievement.ACHIEVEMENT_GOAL_INFO_CHANGE, NewServerAwardNode.OnAchievementGoaInfoChagnge)
  Event.UnregisterEvent(ModuleId.ACHIEVEMENT, gmodule.notifyId.Achievement.ACHIEVEMENT_GET_SCORE_AWARD, NewServerAwardNode.OnAchievementScoreInfoChagnge)
end
def.static("table", "table").OnAchievementGoaInfoChagnge = function(p1, p2)
  local AwardPanel = require("Main.Award.ui.AwardPanel")
  local instance = AwardPanel.Instance().nodes[AwardPanel.NodeId.NewServerAward]
...
def.method("number").goOpertion = function(self, index)
  ...
  if isEnter then
    local AwardPanel = require("Main.Award.ui.AwardPanel")
    local instance = AwardPanel.Instance():DestroyPanel()
  end
end
...
def.method("number").setGoalListInfo = function(self, index)
  ...
  for i, v in ipairs(goalList) do
    ...
    warn("---------awardId:", v.id, v.fixAwardId)
    for k = 1, 3 do
...
return NewServerAwardNode.Commit()
