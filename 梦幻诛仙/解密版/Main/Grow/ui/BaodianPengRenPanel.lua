...
local BaodianPengRenPanel = Lplus.Extend(BaodianBasePanel, "BaodianPengRenPanel")
local def = BaodianPengRenPanel.define
...
local instance
def.static("=>", BaodianPengRenPanel).Instance = function()
  if instance == nil then
    instance = BaodianPengRenPanel()
  end
  return instance
end
def.override("userdata").ShowPanel = function(self, parentPanel)
  ...
  GameUtil.AddGlobalLateTimer(0, true, function()
    self:CreatePanel(RESPATH.PREFAB_BAODIAN_PENGREN, 2)
  end)
end
...
def.method().InitUI = function(self)
  self.mUIObjs = {}
  self.mUIObjs.ItemInfoView = self.m_panel:FindDirect("Group_BD_PengRen/Group_ItemInfo")
...
def.method().InitListView = function(self)
  ...
  for i = 1, listNum do
    ...
    local iconId = BaodianUtils.GetCfgById(self.mItemIds[i], "pengren").iconId
  ...
  local descLabel = self.m_panel:FindDirect("Group_BD_PengRen/Label_LongTip")
  local desc = BaodianUtils.GetBaodianDescByName("GROW_COOK_DESC")
  descLabel:GetComponent("UILabel").text = desc
end
def.method().UpdateRightView = function(self)
  local itemId = self.mCurItemId
  local itemcfg = BaodianUtils.GetCfgById(itemId, "pengren")
...
def.method().InitData = function(self)
  self.mItemCfg = BaodianUtils.GetPengRenCfg()
  self.mItemIds = {}
...
def.method("string").onClick = function(self, id)
  if string.find(id, "Texture_") then
    ...
    self:UpdateRightView()
  elseif id == "Btn_GetPengRen1" then
  ...
  elseif id == "Btn_GetPengRen2" then
    local hp = require("Main.Hero.HeroModule").Instance():GetHeroProp()
    local heroLevel = hp.level
...
BaodianPengRenPanel.Commit()
return BaodianPengRenPanel
