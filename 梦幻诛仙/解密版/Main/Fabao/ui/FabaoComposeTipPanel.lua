...
local FabaoComposeTipPanel = Lplus.Extend(ECPanelBase, "FabaoComposeTipPanel")
local def = FabaoComposeTipPanel.define
def.field("number").mTargetFabaoId = 0
...
local instance
def.static("=>", FabaoComposeTipPanel).Instance = function()
  if nil == instance then
    instance = FabaoComposeTipPanel()
  end
  return instance
end
def.method("string", "string", "number", "number", "number", "function", "table").ShowFabaoComposeTipPanel = function(self, title, desc, fabaoId, itemId, itemNum, cb, tag)
...
def.override().OnCreate = function(self)
  Event.RegisterEvent(ModuleId.ITEM, gmodule.notifyId.item.Item_OnBagInfoSynchronized, FabaoComposeTipPanel.OnBagInfoSyncronized)
  gmodule.network.registerProtocol("netio.protocol.mzm.gsp.item.SResItemYuanbaoPriceWithId", FabaoComposeTipPanel.OnAskItemPriceRes)
  self:InitUI()
  self:UpdateData()
  self:UpdateUI()
end
def.override().OnDestroy = function(self)
  Event.UnregisterEvent(ModuleId.ITEM, gmodule.notifyId.item.Item_OnBagInfoSynchronized, FabaoComposeTipPanel.OnBagInfoSyncronized)
  self.mTitle = ""
  self.mDesc = ""
  self.mTargetFabaoId = 0
  self.mNeedItemId = 0
  self.mNeedItemNum = 0
  self.mCallBack = nil
  self.mExtInfo = nil
...
def.static("table", "table").OnBagInfoSyncronized = function(p1, p2)
  if p1.bagId == ItemModule.FABAOBAG then
    local self = FabaoComposeTipPanel.Instance()
...
def.method().UpdateComfirmBtnState = function(self)
  ...
  local fabaoBase = ItemUtils.GetFabaoItem(self.mTargetFabaoId)
  if fabaoBase then
    local canUseYuanBao = fabaoBase.canUseYuanBao
    if not canUseYuanBao then
      uiToggle.value = false
      self.mIsWaitingYuanBaoPrice = false
      self.mNeedYuanBaoReplace = false
      self.mYuanBaoPriceMap = nil
      self.mNeedYuanBaoNum = 0
      self.m_panel:FindDirect("Img_Bg0/Group_UseYuanbao"):SetActive(false)
      self.m_panel:FindDirect("2D Sprite/Btn_Confirm/Label_UseGold"):SetActive(false)
      return
    end
  end
  self.m_panel:FindDirect("Img_Bg0/Group_UseYuanbao"):SetActive(true)
...
def.method().OnClickToggleBtn = function(self)
  ...
  if curValue then
    local fabaoBase = ItemUtils.GetFabaoItem(self.mTargetFabaoId)
    local fabaoItemBase = ItemUtils.GetItemBase(self.mTargetFabaoId)
    if fabaoBase then
      local canUseYuanBao = fabaoBase.canUseYuanBao
      if not canUseYuanBao then
        uiToggle.value = false
        Toast(string.format(textRes.Fabao[108], fabaoItemBase.name))
        self:UpdateComfirmBtnState()
        return
      end
    end
...
FabaoComposeTipPanel.Commit()
return FabaoComposeTipPanel
