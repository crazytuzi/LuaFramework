...
local OperationMountsChipUse = Lplus.Extend(OperationBase, "OperationMountsChipUse")
local def = OperationMountsChipUse.define
def.field("boolean").bind = false
def.override("number", "table", "table", "=>", "boolean").CanDispaly = function(self, source, item, itemBase)
  local ItemType = require("consts.mzm.gsp.item.confbean.ItemType")
  if source == ItemTipsMgr.Source.Bag and itemBase.itemType == ItemType.MOUNTS_CHIP_ITEM then
    return true
  else
    return false
  end
...
OperationMountsChipUse.Commit()
return OperationMountsChipUse
