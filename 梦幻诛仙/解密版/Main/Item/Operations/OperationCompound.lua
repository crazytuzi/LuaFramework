...
local OperationCompound = Lplus.Extend(OperationBase, "OperationCompound")
local def = OperationCompound.define
...
def.override("number", "number", "userdata", "table", "=>", "boolean").Operate = function(self, bagId, itemKey, m_panel, context)
  ...
  CollectSliderPanel.ShowCollectSliderPanel(textRes.Item[8305], 1.5, function()
    ...
    local compound = require("netio.protocol.mzm.gsp.item.CItemCompound").new(item.uuid[#item.uuid], itemNum)
...
OperationCompound.Commit()
return OperationCompound
