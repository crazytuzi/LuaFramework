...
local OperationSellAllToCommerce = Lplus.Extend(OperationBase, "OperationSellAllToCommerce")
local CommercePitchUtils = require("Main.CommerceAndPitch.CommercePitchUtils")
local ItemModule = require("Main.Item.ItemModule")
local ItemUtils = require("Main.Item.ItemUtils")
require("Main.module.ModuleId")
local def = OperationSellAllToCommerce.define
def.override("number", "table", "table", "=>", "boolean").CanDispaly = function(self, source, item, itemBase)
  if (source == ItemTipsMgr.Source.Bag or source == ItemTipsMgr.Source.Commerce or source == ItemTipsMgr.Source.FabaoBag) and itemBase.canSellAndThrow and itemBase.isProprietary == false and CommercePitchUtils.CanItemCommerceToSell(itemBase.itemid, item) and item.number > 1 then
    return true
  else
    return false
  end
...
def.override("number", "number", "userdata", "table", "=>", "boolean").Operate = function(self, bagId, itemKey, m_panel, context)
  ...
  local function doSell()
    ...
    Event.DispatchEvent(ModuleId.ITEM, gmodule.notifyId.item.Item_Sell_All_To_Commerce, tbl)
  end
  doSell()
  if item.number > 1 then
    return false
...
OperationSellAllToCommerce.Commit()
return OperationSellAllToCommerce
