...
local OperationMountsResetSkill = Lplus.Extend(OperationBase, "OperationMountsResetSkill")
local def = OperationMountsResetSkill.define
def.field("boolean").bind = false
def.override("number", "table", "table", "=>", "boolean").CanDispaly = function(self, source, item, itemBase)
  local ItemType = require("consts.mzm.gsp.item.confbean.ItemType")
  if source == ItemTipsMgr.Source.Bag and itemBase.itemType == ItemType.MOUNTS_REFRESH_ITEM then
    return true
  else
    return false
  end
...
OperationMountsResetSkill.Commit()
return OperationMountsResetSkill
