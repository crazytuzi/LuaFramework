...
local OperationFabaoExpItem = Lplus.Extend(OperationBase, "OperationFabaoExpItem")
local def = OperationFabaoExpItem.define
def.override("number", "table", "table", "=>", "boolean").CanDispaly = function(self, source, item, itemBase)
  if source == ItemTipsMgr.Source.Bag and itemBase.itemType == ItemType.FABAO_EXP_ITEM then
    return true
  else
    return false
  end
...
def.override("number", "number", "userdata", "table", "=>", "boolean").Operate = function(self, bagId, itemKey, m_panel, context)
  ...
  if false == hasFabaoTask and 0 == taskId then
    FabaoSocialPanel.Instance():ShowPanel(FabaoSocialPanel.NodeId.FabaoCZ)
    return true
  else
...
OperationFabaoExpItem.Commit()
return OperationFabaoExpItem
