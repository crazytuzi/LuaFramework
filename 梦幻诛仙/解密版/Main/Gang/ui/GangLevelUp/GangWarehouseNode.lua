...
local GangWarehouseNode = Lplus.Extend(TabNode, "GangWarehouseNode")
...
local def = GangWarehouseNode.define
...
def.method().FillDefault = function(self)
  ...
  Label_Lv:set_text(gangInfo.warehouseLevel)
  ...
  if gangInfo.warehouseLevel >= gangTbl.cangKuMaxLevel then
    ...
    manjiLabel:set_text(string.format(textRes.Gang[117], textRes.Gang.BuildType[1]))
  else
    ...
    local gangTblNext = GangUtility.GetWarehouseGangBasicCfg(gangInfo.warehouseLevel + 1)
    ...
    Label_CostNum1:set_text(warehouseTbl.levelUpNeedMoney)
    ...
    Label1:set_text(gangTblNext.fuLiNum)
    Label2:set_text(gangTblNext.gridSize)
    ...
    Label3:set_text(string.format(textRes.Gang[253], warehouseTbl.fuLiNum))
    Label4:set_text(string.format(textRes.Gang[253], warehouseTbl.gridSize))
  end
end
def.method().FillLevelUpInfo = function(self)
  ...
  if gangInfo.warehouseEndTime <= 0 then
    ...
    local warehouseTbl = GangUtility.GetWarehouseGangBasicCfg(gangInfo.warehouseLevel)
    self:FillOpen(gangInfo.warehouseEndTime, warehouseTbl.levelUpNeedTimeM * 60)
  end
...
def.method().OnOpenConsructClick = function(self)
  GangUtility.TryGangConstruct(GangBuildingEnum.CANGKU)
end
def.method().OnSendMessageClick = function(self)
  Toast(textRes.Gang[238])
  gmodule.network.sendProtocol(require("netio.protocol.mzm.gsp.gang.CCallBuildingLevelUpDonateReq").new(GangBuildingEnum.CANGKU))
end
def.method().OnDonateClick = function(self)
  local GangBuildingEnum = require("netio.protocol.mzm.gsp.gang.GangBuildingEnum")
  local GangBuildDonatePanel = require("Main.Gang.ui.GangBuildDonatePanel")
  GangBuildDonatePanel.ShowDonateBuildPanel(GangBuildingEnum.CANGKU)
end
...
GangWarehouseNode.Commit()
return GangWarehouseNode
