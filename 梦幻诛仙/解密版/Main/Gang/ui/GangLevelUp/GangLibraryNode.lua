...
local GangLibraryNode = Lplus.Extend(TabNode, "GangLibraryNode")
...
local def = GangLibraryNode.define
...
def.method().FillDefault = function(self)
  ...
  Label_Lv:set_text(gangInfo.bookLevel)
  ...
  if gangInfo.bookLevel >= gangTbl.shuYuanMaxLevel then
    ...
    manjiLabel:set_text(string.format(textRes.Gang[117], textRes.Gang.BuildType[5]))
  else
    ...
    local gangTblNext = GangUtility.GetBookGangBasicCfg(gangInfo.bookLevel + 1)
    ...
    Label_CostNum1:set_text(bookTbl.levelUpNeedMoney)
    Label_CostNum2:set_text(gangInfo.money)
    local Label1 = Group_NextLevel:FindDirect("Label_Content/Label1"):GetComponent("UILabel")
    Label1:set_text(gangTblNext.maxSkillLevel)
    local Label2 = Group_NextLevel:FindDirect("Label_Content/Label_1"):GetComponent("UILabel")
    Label2:set_text(string.format(textRes.Gang[253], bookTbl.maxSkillLevel))
  end
end
def.method().FillLevelUpInfo = function(self)
  ...
  if gangInfo.bookEndTime <= 0 then
    ...
    local bookTbl = GangUtility.GetBookGangBasicCfg(gangInfo.bookLevel)
    self:FillOpen(gangInfo.bookEndTime, bookTbl.levelUpNeedTimeM * 60)
  end
...
def.method().OnOpenConsructClick = function(self)
  GangUtility.TryGangConstruct(GangBuildingEnum.SHUYUAN)
end
def.method().OnSendMessageClick = function(self)
  Toast(textRes.Gang[238])
  gmodule.network.sendProtocol(require("netio.protocol.mzm.gsp.gang.CCallBuildingLevelUpDonateReq").new(GangBuildingEnum.SHUYUAN))
end
def.method().OnDonateClick = function(self)
  local GangBuildingEnum = require("netio.protocol.mzm.gsp.gang.GangBuildingEnum")
  local GangBuildDonatePanel = require("Main.Gang.ui.GangBuildDonatePanel")
  GangBuildDonatePanel.ShowDonateBuildPanel(GangBuildingEnum.SHUYUAN)
end
...
GangLibraryNode.Commit()
return GangLibraryNode
