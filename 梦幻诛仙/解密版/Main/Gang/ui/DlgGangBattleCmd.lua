...
local DlgGangBattleCmd = Lplus.Extend(ECPanelBase, "DlgGangBattleCmd")
local GangBattleMgr = require("Main.Gang.GangBattleMgr")
local GUIUtils = require("GUI.GUIUtils")
local GangData = require("Main.Gang.data.GangData")
local def = DlgGangBattleCmd.define
local dlg
def.field("table").roleInfo = nil
def.static("=>", DlgGangBattleCmd).Instance = function()
  if dlg == nil then
    dlg = DlgGangBattleCmd()
  end
  return dlg
end
...
def.method("string").onClick = function(self, id)
  ...
  if id == "Btn_PK" then
    ...
    gmodule.network.sendProtocol(require("netio.protocol.mzm.gsp.competition.CAttackReq").new(self.roleInfo.roleId))
    self:Hide()
...
def.method().ShowInfo = function(self)
  ...
  if myGangId == nil then
    warn("[DlgGangBattleCmd]MyGangId is nil when show target info")
  end
...
DlgGangBattleCmd.Commit()
return DlgGangBattleCmd
