...
local function argError(iArg, funcName, needs, gets, errLevel)
  error(("bad argument #%d to '%s' (%s expected, got %s)"):format(iArg, funcName, needs, gets), errLevel + 1)
end
local function argListError(iArg, memberName, needs, gets, errLevel)
  if gets == nil then
    error(("bad argument #%d to argument list of %s (%s)"):format(iArg, memberName, needs), errLevel + 1)
  else
    error(("bad argument #%d to argument list of %s (%s expected, got %s)"):format(iArg, memberName, needs, gets), errLevel + 1)
  end
end
local function initValueError(fieldName, needs, gets, errLevel)
  if gets == nil then
    error(("bad inital value to field '%s' (%s)"):format(fieldName, needs), errLevel + 1)
  else
    error(("bad inital value to field '%s' (%s expected, got %s)"):format(fieldName, needs, gets), errLevel + 1)
  end
end
local config = {
  reflection = false,
...
local function checkValidArgType(typeValue, iParam, memberName, errLevel)
  ...
  elseif getTypeTableMeta(typeValue) == nil then
...
local function checkValueCompatible(value, needType, format, who, errorLevel)
  if not isTypeCompatible(value, needType) then
    ...
    error(("bad %s (%s expected, got %s)"):format(what, formatTypeName(needType), formatObjectTypeName(value)), errorLevel + 1)
  end
end
...
local compareSignature = function(signatureLeft, signatureRight)
  local iParam = 1
  repeat
...
local function checkMethodOverrideSignature(signatureBase, signatureOverride, methodType, methodName, errLevel)
  ...
  if iDiff ~= nil then
    ...
    error(("bad %s type #%d to %s '%s' (%s expected, got %s)"):format(what, iWhat, methodType, methodName, formatTypeName(signatureBase[iDiff]), formatTypeName(signatureOverride[iDiff])), errLevel + 1)
  end
end
...
local function createEmptyType(typeName, forwardDeclare, errorLevel)
  ...
  if typeName ~= nil then
    forwardDeclaredType[typeName] = nil
  end
...
local function checkValuesCompatible(needTypes, errLevel, ...)
  ...
  if nGets < nNeeds then
    ...
    error(("too little %s (%d expected, got %d)"):format(what, nNeeds, nGets), errLevel + 1)
  end
  if not bVarlist and nGets > nNeeds then
    local what = needTypes.format:format(needTypes.who)
    error(("too many %s (%d expected, got %d)"):format(what, nNeeds, nGets), errLevel + 1)
  end
  ...
  if selfType ~= nil then
    ...
    if self == nil then
      ...
      error(("bad %s (%s expected, got %s)"):format(what, formatTypeName(selfType), formatObjectTypeName(self)), errLevel + 1)
    end
  end
  for i = 1, nNeeds do
    ...
    if not isTypeCompatible(getValue, needType) then
      ...
      error(("bad %s (%s expected, got %s)"):format(what, formatTypeName(needType), formatObjectTypeName(getValue)), errLevel + 1)
    end
  end
  return ...
end
local function createEmptyFunctionWrapper()
  local func, methodName, tableType, bCheckType, arguments, returns
  return function(...)
    local magic, op = ...
    if magic == lplusFunctionWrapperControlMagic then
      if op == "set" then
        ...
        local bInstanceMethod = methodInfo.style:find("s", 1, true) == nil and methodInfo.style:find("f", 1, true) == nil
...
local function createType(baseTypeTable, typeNameOrForwardDeclare, isInterface)
  local theForwardDeclare, typeName
  if type(typeNameOrForwardDeclare) == "table" then
    ...
    if forwardDeclareMeta == nil then
  ...
  if baseTypeTable ~= nil then
    ...
    if baseMeta == nil then
      if not bInitializingLplus then
  ...
  do
    ...
    function typeMeta.__tostring(_)
      return typeString
    end
    typeMeta.typeInfo = nil
    typeMeta.compatibleTypes = {
      [theType] = true
    }
    ...
    if feature_accessing_checking then
      do
        ...
        local function checkMemberInfo(memberName, bAccessInstanceMember, errorLevel)
          ...
          elseif bIsInstanceMember and memberInfo.memberType ~= "method" then
        ...
        function typeMeta.__call(_, ...)
          ...
          if select("#", ...) ~= 0 then
            error(("bad argument to construct of %s (no argument expected, got %d)"):format(tostring(theType), select("#", ...)), 2)
          end
          local objMeta = {typeTable = theType, magic = lplusObjectMagic}
          local memberInfoMap = typeMeta.memberInfoMap
          local objMembers = setmetatable({}, {
...
function Lplus.Unload(typeName)
  typeNameToTableMap[typeName] = nil
end
local l_tableidmap = {}
