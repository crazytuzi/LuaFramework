...
local function GetTypeChecker(cpp_type, field_type)
  ...
  elseif cpp_type == FieldDescriptor.CPPTYPE_INT64 or cpp_type == FieldDescriptor.CPPTYPE_UINT64 then
    return type_checkers.Int64StringChecker()
  end
...
local function _AddStaticMethods(descriptor, message_meta)
  ...
  function message_meta._member.GetDescriptor()
    return descriptor
  end
  function message_meta._member.GetFieldDescriptor(field_name)
...
local _AddGetMessageMethod = function(message_meta, message_class)
  function message_meta._member:GetMessage()
...
local function _AddHasFieldMethod(message_descriptor, message_meta)
  ...
  function message_meta._member:HasField(field_name)
    ...
    if field == nil then
      error("Protocol message has no singular \"" .. tostring(field_name) .. "\" field.")
    end
    if field.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE then
...
local function Message(descriptor)
  ...
  message_meta._field_descriptors = {}
