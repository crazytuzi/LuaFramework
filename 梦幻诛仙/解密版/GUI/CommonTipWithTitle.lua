...
local CommonTipWithTitle = Lplus.Extend(ECPanelBase, "CommonTipWithTitle")
local GUIMan = require("GUI.ECGUIMan")
local def = CommonTipWithTitle.define
...
local instance
def.static("=>", CommonTipWithTitle).Instance = function()
  if instance == nil then
    instance = CommonTipWithTitle()
  end
  return instance
end
def.method("string", "string", "number", "number", "number", "number").ShowTip = function(self, title, content, sourceX, sourceY, sourceW, sourceH)
  ...
  if self:IsShow() then
    ...
    self:CreatePanel(RESPATH.PREFAB_COMMON_TITLE_TIP, 2)
...
CommonTipWithTitle.Commit()
return CommonTipWithTitle
