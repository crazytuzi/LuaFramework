...
local CommonUseItem = Lplus.Extend(ECPanelBase, "CommonUseItem")
local ItemUtils = require("Main.Item.ItemUtils")
local ItemModule = require("Main.Item.ItemModule")
local GUIUtils = require("GUI.GUIUtils")
local ItemTipsMgr = require("Main.Item.ItemTipsMgr")
local def = CommonUseItem.define
local instance
def.static("=>", CommonUseItem).Instance = function()
  if instance == nil then
    instance = CommonUseItem()
  end
  return instance
end
...
def.method().UpdateItemData = function(self)
  if self.itemTypeList ~= nil then
    ...
    for k, _ in pairs(self.itemIdMap) do
      ...
      if itemBase then
        self.itemIdMap[k] = ItemModule.Instance():GetNumByItemType(ItemModule.BAG, itemBase.itemType)
      end
    end
  end
  self.itemList = {}
...
def.static("string", "table", "function", "function").ShowCommonUseByItemIdEx = function(title, itemIdList, callback, onDestroyCallback)
  local self = CommonUseItem.Instance()
  if itemIdList == nil or callback == nil then
    return
  end
...
def.method().UpdateItemDataEx = function(self)
  for k, _ in pairs(self.itemIdMap) do
    self.itemIdMap[k] = ItemModule.Instance():GetItemCountById(k)
  end
  self.itemList = {}
  for k, v in pairs(self.itemIdMap) do
    table.insert(self.itemList, {id = k, number = v})
...
def.override().OnCreate = function(self)
  Event.RegisterEventWithContext(ModuleId.ITEM, gmodule.notifyId.item.Item_OnBagInfoSynchronized, CommonUseItem.OnItemChange, self)
  self:UpdateTitle()
  self:UpdateItemList()
  self:UpdateSelectInfo()
end
...
def.override().OnDestroy = function(self)
  Event.UnregisterEvent(ModuleId.ITEM, gmodule.notifyId.item.Item_OnBagInfoSynchronized, CommonUseItem.OnItemChange)
    ...
    if self.callback then
      ...
      if self.lastUseItemId == itemData.id and 0 < itemData.number then
        if ret then
          ...
          if self.enableUseAll and self.repeatTimes == CommonUseItem.ALLUSETIME then
            local itemBase = ItemUtils.GetItemBase(itemId)
