...
function SLAXML:parse(xml, options)
  options = options or {stripWhitespace = false}
  ...
  while pos < #xml do
    if state == "text" then
      if not findPI() and not findComment() and not findCDATA() and not findElementClose() then
    ...
    elseif state == "attributes" and not findAttribute() and not closeElement() then
