--[[
******帮会商城信息处理*******

    -- by Chikui Peng
    -- 2016/4/13
]]

local FactionMallInfo = class("FactionMallInfo")

function FactionMallInfo:ctor()
end

function FactionMallInfo:initUI(ui,layer)
    self.logic = layer
    self.PageList = {}
    self.img_shensuo        = TFDirector:getChildByPath(ui, 'img_shensuo')
    self.panel_list = TFDirector:getChildByPath(ui, 'panel_list')
    self.btnNormalImg = {
        'ui_new/faction/btn_shop_putong1.png',
        'ui_new/faction/btn_shop_jingpin1.png',
        'ui_new/faction/btn_shop_chuanshuo1.png'
    }
    self.btnHoldImg = {
        'ui_new/faction/btn_shop_putong2.png',
        'ui_new/faction/btn_shop_jingpin2.png',
        'ui_new/faction/btn_shop_chuanshuo2.png'
    }
end

function FactionMallInfo:isOpen()
    local openLevel = FactionManager:getShopOpenLevel()
    if openLevel == 0 then
        return false
    elseif FactionManager.factionInfo == nil or FactionManager.factionInfo.level == nil or FactionManager.factionInfo.level < openLevel then
        return false
    end
    return true
end

function FactionMallInfo:show(index)
    self:registerEvents()
    self.panel_list:removeAllChildrenWithCleanup(true)
    self.logic.generalHead.buyButtonEventBound = nil
    self.logic.generalHead:setData(ModuleType.FactionMall,{HeadResType.FACTION_GX,HeadResType.COIN,HeadResType.SYCEE})
    for k,v in ipairs(self.logic.btnList) do
        if self.btnHoldImg[k] then
            v:setTextureNormal(self.btnNormalImg[k])
            v:setTexturePressed(self.btnHoldImg[k])
            v:setVisible(true)
            CommonManager:updateRedPoint(v,false)
        else
            v:setVisible(false)
        end
    end
    self.currentLayer = nil
    self.img_shensuo:setTexture('ui_new/shop/sc_shenzi.png')
    self.logic.btnList[1]:setPositionY(513)
    self:refreshButton()
    self:select(index)
end

function FactionMallInfo:close()
    self:removeEvents()
    self:dispose()
end

function FactionMallInfo:btnSelect(index)
    self:select(index)
end

function FactionMallInfo:brushBtnState()
    for k,v in ipairs(self.logic.btnList) do
        if self.btnHoldImg[k] then
            if self.selectedIndex == k then
                v:setTextureNormal(self.btnHoldImg[k])
            else
                v:setTextureNormal(self.btnNormalImg[k])
            end
            
            v:setTexturePressed(self.btnHoldImg[k])
        end
    end
end

function FactionMallInfo:refreshButton()
    local randomStore = MallManager:getrandomStoreTable()
    if randomStore[RandomStoreType.Gang_Normal]:isOpen() == false then
        self.logic.btnList[1]:setVisible(false)
    end
    if randomStore[RandomStoreType.Gang_2]:isOpen() == false then
        self.logic.btnList[2]:setVisible(false)
    end
    if randomStore[RandomStoreType.Gang_3]:isOpen() == false then
        self.logic.btnList[3]:setVisible(false)
    end
    local temp = 0
    local pos_y = self.logic.btnList[1]:getPositionY()
    for k,v in ipairs(self.logic.btnList) do
        if v:isVisible() then
            v:setPositionY( pos_y - (k-1)*110 )
            temp = temp + 1
        end
    end
    self.img_shensuo:setPositionY(pos_y + 22 - temp*110)
end

function FactionMallInfo:registerEvents()
    print("FactionMallInfo:registerEvents() ........")
end

function FactionMallInfo:removeEvents()
    print("FactionMallInfo:removeEvents() ........")
end

function FactionMallInfo:select(index)
    self.selectedIndex = index
    if index then
        self:selectNormalShop(index)
    else
        self.selectedIndex = 1
        self:selectNormalShop(1)
    end
    self:brushBtnState()
end

function FactionMallInfo:handleClose()
    AlertManager:close(AlertManager.TWEEN_1);
end

function FactionMallInfo:hideCurrentLayer()
    if self.currentLayer then
        self.currentLayer:setVisible(false)
    end
end

function FactionMallInfo:showCurrentLayer()
    if self.currentLayer then
        self.currentLayer:setVisible(true)
        self.currentLayer:show()
    end
end

function FactionMallInfo:selectNormalShop(index)
    self:hideCurrentLayer()
    local typeList = {RandomStoreType.Gang_Normal,RandomStoreType.Gang_2,RandomStoreType.Gang_3}
    if typeList[index] == nil then
        index = 1
    end
    
    if self.PageList[index] == nil then
        self.PageList[index] = require('lua.logic.factionMall.FactionShopPage'):new(typeList[index])
        self.panel_list:addChild(self.PageList[index])
    end
    
    self.currentLayer = self.PageList[index]
    self:showCurrentLayer()
end



-----断线重连支持方法
function FactionMallInfo:onShow()
    if self.currentLayer then
        self.currentLayer:onShow()
    end
end

function FactionMallInfo:dispose()
    for k,v in pairs(self.PageList) do
        v:dispose()
    end
    self.PageList = {}
    self.currentLayer = nil
end

return FactionMallInfo