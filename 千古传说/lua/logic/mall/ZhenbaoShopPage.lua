...
local ZhenbaoShopPage = class("ZhenbaoShopPage", BaseLayer)
...
function ZhenbaoShopPage:ctor(data)
    self.super.ctor(self,data)
    if data then
        self.type = data
    else
        self.type = RandomStoreType.Zhenbao
    end
    self.costType = EnumDropType.SYCEE
    self:init("lua.uiconfig_mango_new.shop.ZhenbaoShopPage")
end

function ZhenbaoShopPage:initUI(ui)
...
function ZhenbaoShopPage:removeUI()
...
--初始化TableView
function ZhenbaoShopPage:initTableView()
...
--刷新回调
function ZhenbaoShopPage:refreshCallback()
    --print("ZhenbaoShopPage:refreshCallback()")
    self:refreshUI()
end

--显示充值提示框
function ZhenbaoShopPage:showRechargeDialog()
...
--刷新按钮点击事件处理方法
function ZhenbaoShopPage.refreshButtonClickHandle(sender)
...
function ZhenbaoShopPage:getHasTip( widget )
...
function ZhenbaoShopPage:registerEvents()
    ...
    --table view 事件
    self.tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, ZhenbaoShopPage.cellSizeForTable)
    self.tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, ZhenbaoShopPage.tableCellAtIndex)
    self.tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, ZhenbaoShopPage.numberOfCellsInTableView)
...
function ZhenbaoShopPage:removeEvents()
    --TableView事件
...
function ZhenbaoShopPage:addTimer()
...
--销毁方法
function ZhenbaoShopPage:dispose()
    self:disposeAllPanels()
    self.super.dispose(self)
end

--销毁所有TableView的Cell中的Panel
function ZhenbaoShopPage:disposeAllPanels()
...
function ZhenbaoShopPage:updateTableSource()
...
function ZhenbaoShopPage:onShow()
    self.super.onShow(self)
    self:refreshUI()
end

function ZhenbaoShopPage:refreshUI()
    self:refreshTableView()
end

function ZhenbaoShopPage:refreshTableView()
    self:updateTableSource()
...
function ZhenbaoShopPage.cellSizeForTable(table,idx)
    return 180,537
end

function ZhenbaoShopPage.tableCellAtIndex(table, idx)
...
function ZhenbaoShopPage.numberOfCellsInTableView(table)
    local self = table.logic
...
function ZhenbaoShopPage:tableCellClick(cell)

end

function ZhenbaoShopPage:setType(type)
    self.type = type
    self:refreshTableView()
end

function ZhenbaoShopPage:setCostType(costType)
...
function ZhenbaoShopPage:setRefreshButtonEnabled(enabled)
...
function ZhenbaoShopPage:setRefreshButtonVisiable(visiable)
    self.btn_refresh:setVisible(visiable)
end


function ZhenbaoShopPage:refreshOpenState()
   -- self.store = MallManager:getRandomStoreByType(self.type)
    if self.store == nil then
        print("找不到随机商店信息 : ",self.type)
        return
...
function ZhenbaoShopPage:updateTime()
    local second = self.store.opentime + self.store.configure.showTime - GetGameTime()*1000
    if second <= 0 then
        if self.nEndTimerId then
            TFDirector:removeTimer(self.nEndTimerId)
...
function ZhenbaoShopPage:endTimeShow( second )
    if second < 0 then
        second = 0
    end
