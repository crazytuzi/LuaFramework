...
local BossFightArmyVSLayer = class("BossFightArmyVSLayer", BaseLayer)
-- local CardRole = require('lua.gamedata.base.CardRole')
CREATE_SCENE_FUN(BossFightArmyVSLayer)
CREATE_PANEL_FUN(BossFightArmyVSLayer)

BossFightArmyVSLayer.LIST_ITEM_WIDTH = 200 

function BossFightArmyVSLayer:ctor(data)
    self.super.ctor(self,data)
    self:init("lua.uiconfig_mango_new.demond.demondOtherArmyVSLayer")
end

function BossFightArmyVSLayer:initUI(ui)
...
function BossFightArmyVSLayer:setBossData(userData)
...
function BossFightArmyVSLayer:onShow()
...
function BossFightArmyVSLayer:refreshBaseUI()

end

function BossFightArmyVSLayer:refreshUI()
...
function BossFightArmyVSLayer:getRoleBtPos(pos)
...
function BossFightArmyVSLayer.cellClickHandle(sender)
...
function BossFightArmyVSLayer.onArmyClickHandle(sender)
    local self = sender.logic
...
function BossFightArmyVSLayer:getChangeBtn()
    return self.btn_challenge
end

--注册事件
function BossFightArmyVSLayer:registerEvents()
...
function BossFightArmyVSLayer:removeEvents()
...
function BossFightArmyVSLayer:drawLeftArea()
    self.button_self = {}
    self.button = {}
    for i=1,9 do
...
function BossFightArmyVSLayer:drawRightArea()
    self.button = {}
    for i=1,9 do
        local btnName = "panel_item" .. i
...
function BossFightArmyVSLayer.onBtnAttackClickHandle(sender)
    local self      = sender.logic
...
function BossFightArmyVSLayer:Attack()
    local bossId   = self.BossData.bossId

    print("开始挑战")

    if BossFightManager:requestAttackBoss(bossId) == true then
        AlertManager:close(AlertManager.TWEEN_NONE)
    end
end


function BossFightArmyVSLayer.cellTouchBeganHandle(cell)
...
function BossFightArmyVSLayer.cellTouchMovedHandle(cell)
    local self = cell.logic
    self.isMove = true
...
function BossFightArmyVSLayer.cellTouchEndedHandle(cell)
    local self = cell.logic
...
function BossFightArmyVSLayer:removeLongTouchTimer()
...
function BossFightArmyVSLayer:createSelectCussor(cell,pos)
    play_press()
...
function BossFightArmyVSLayer:moveSelectCussor(cell,pos)
...
function BossFightArmyVSLayer:releaseSelectCussor(cell,pos)
    print("BossFightArmyVSLayer:releaseSelectCussor")
    if cell.isClick == false  then
        print("111111111111111111111111BossFightArmyVSLayer:releaseSelectCussor = ", self.curIndex)
        if (self.curIndex == nil) then
            return
        end
        print("2222222222BossFightArmyVSLayer:releaseSelectCussor")
