...
local MiningMainArmyLayer = class("MiningMainArmyLayer", BaseLayer);

CREATE_SCENE_FUN(MiningMainArmyLayer);
CREATE_PANEL_FUN(MiningMainArmyLayer);

MiningMainArmyLayer.LIST_ITEM_HEIGHT = 200; 

function MiningMainArmyLayer:ctor(data)
    self.super.ctor(self,data);

    self.fightType = data
    -- self:init("lua.uiconfig_mango_new.role.ArmyLayer");
    self:init("lua.uiconfig_mango_new.bloodybattle.BloodyArmyLayer")
    
end

function MiningMainArmyLayer:onShow()
...
function MiningMainArmyLayer:refreshUI()
...
function MiningMainArmyLayer:updateStrategyBaseMsg()
...
function MiningMainArmyLayer:initUI(ui)
...
function MiningMainArmyLayer.onShowSortMenuClickHandle(sender)
...
function MiningMainArmyLayer.onSortSelectClickHandle(sender)
    local self = sender.logic;
...
function MiningMainArmyLayer.onSortCancelClickHandle(sender)
    local self = sender.logic;
    self.node_menu:setVisible(false);
    self.btn_sort:setVisible(true);
end
...
function MiningMainArmyLayer.scrollForTable(tableView)
    local self = tableView.logic;
    -- self:removeLongTouchTimer();
end

function MiningMainArmyLayer.cellSizeForTable(tableView,idx)
    return MiningMainArmyLayer.LIST_ITEM_HEIGHT,960
end

function MiningMainArmyLayer.tableCellAtIndex(tableView, idx)
    local self = tableView.logic;
    ...
    if nil == cell then
        ...
        for i=1,3 do
            ...
            -- m_node.img_xuetiao:setVisible(true)
...
function MiningMainArmyLayer.numberOfCellsInTableView(tableView)
    local self = tableView.logic;
    -- return math.max(math.ceil(MiningManager.cardRoleList:length()/3)  ,3);
    return math.max(math.ceil(CardRoleManager.cardRoleList:length()/3)  ,3);
end

--添加玩家节点
function MiningMainArmyLayer:loadItemNode(item_node,roleItem)
    ...
    item_node.bar_xuetiao:setVisible(true)
    local maxHp,curHp = self:getHp(roleItem)
...
function MiningMainArmyLayer:removeUI()
...
function MiningMainArmyLayer.cellClickHandle(sender)
    local self = sender.logic;
...
function MiningMainArmyLayer.cellTouchBeganHandle(cell)
...
function MiningMainArmyLayer.cellTouchMovedHandle(cell)
    local self = cell.logic;
    self.isMove = true;
...
function MiningMainArmyLayer.cellTouchEndedHandle(cell)
    local self = cell.logic;
...
function MiningMainArmyLayer:removeLongTouchTimer()
...
function MiningMainArmyLayer:createSelectCussor(cell,pos)
    play_press();
...
function MiningMainArmyLayer:moveSelectCussor(cell,pos)
...
function MiningMainArmyLayer:releaseSelectCussor(cell,pos)
    ...
    if cell:hitTest(pos) then     
        -- MiningMainArmyLayer.cellClickHandle(cell);
    end
end


function MiningMainArmyLayer:registerEvents()
...
function MiningMainArmyLayer:playAutoMatixComEffect()
...
function MiningMainArmyLayer:refreshTable()

    if self.table_select == nil then
        return
    end
    print("MiningMainArmyLayer:refreshTable")
    ZhengbaManager:reSortStrategy(self.fightType, CardRoleManager.cardRoleList)
...
function MiningMainArmyLayer:removeEvents()
...
function MiningMainArmyLayer:updateIcon( index )
    local role = ZhengbaManager:getRoleByIndex( self.fightType,index);
    if role then
        ...
        local maxHp,curHp = self:getHp(role)
        -- local maxHp = 100--role.blood_maxHp
        -- local curHp = 50--role.blood_curHp
        local bGray = false
        if curHp <= 0 then
            bGray = true
        end
...
-- 上阵
function MiningMainArmyLayer:OnBattle(gmid, posIndex)
   ZhengbaManager:OnBattle(self.fightType,gmid, posIndex)
end

-- 下阵
function MiningMainArmyLayer:OutBattle(gmid)
    ZhengbaManager:OutBattle(self.fightType,gmid)
end

-- 换位置
function MiningMainArmyLayer:ChangePos(oldPos, newPos)
    ZhengbaManager:ChangePos(self.fightType,oldPos, newPos)
end


function MiningMainArmyLayer:getHp(roleItem)
    local maxHp = roleItem.totalAttribute:getAttribute(1)
    local curHp = MiningManager:getOwnBloodByPos(roleItem.gmId)
    curHp = curHp or maxHp

    return curHp, maxHp
end

return MiningMainArmyLayer;
