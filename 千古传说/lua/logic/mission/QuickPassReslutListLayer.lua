local QuickPassReslutListLayer = class("QuickPassReslutListLayer", BaseLayer);

QuickPassReslutListLayer.LIST_ITEM_HEIGHT = 140; 

CREATE_SCENE_FUN(QuickPassReslutListLayer);
CREATE_PANEL_FUN(QuickPassReslutListLayer);
...
function QuickPassReslutListLayer:ctor(data)
    self.super.ctor(self,data);
    
    self:init("lua.uiconfig_mango_new.mission.QuickPassReslutListLayer");
end

function QuickPassReslutListLayer:initUI(ui)
    ...
    self.mubiaoNum      = TFDirector:getChildByPath(self.bg_mubiao, 'LabelBMFont_QuickPassReslutListLayer_1')
...
function QuickPassReslutListLayer:loadData(data)
...
function QuickPassReslutListLayer:onShow()
    self.super.onShow(self)
    self:refreshBaseUI();
end

function QuickPassReslutListLayer:refreshBaseUI()

end
function QuickPassReslutListLayer:refreshUI()
    ...
    local txt_tongbi	= TFDirector:getChildByPath(self, 'txt_tongbi');

    local txt_addcoin	= TFDirector:getChildByPath(self, 'txt_addcoin');
    if MainPlayer:bMaxLevel() then
        txt_tongbi:setVisible(true)
    	txt_addExp:setText("0");
    	local muilt = ConstantData:objectByID("Experience.Change.Money").value
    	txt_addcoin:setText(self.expSum * muilt )
    else
...
function QuickPassReslutListLayer:loadTotolList()
       ...
       if reslutItem.itemlist then
           for i,rewardItem in pairs(reslutItem.itemlist) do
               ...
               for item in rewardList:iterator() do
                   ...
                   if item.itemId == rewardItem.itemId and item.type == rewardItem.type then
...
function QuickPassReslutListLayer:loadRewardList()
    ...
    for index,reslutItem in pairs(self.itemlist) do
        ...
        reslut_node:setPosition(ccp(0,(#self.itemlist - index) * QuickPassReslutListLayer.LIST_ITEM_HEIGHT))
        self.list_reward:addChild(reslut_node)
        reslut_node:setVisible(false);
        reslut_nodeArr[index] = reslut_node;
    end

    self.list_reward:setInnerContainerSize(CCSizeMake(self.list_reward:getSize().width , #self.itemlist * QuickPassReslutListLayer.LIST_ITEM_HEIGHT + 20))
    self.list_reward:scrollToYTop(0);
    self.list_reward:setInnerContainerSizeForHeight(#self.itemlist * QuickPassReslutListLayer.LIST_ITEM_HEIGHT + 20)
    ...
    self.change = function()
        ...
        -- local newOffset = ccp(oldOffset.x,oldOffset.y + QuickPassReslutListLayer.LIST_ITEM_HEIGHT);
...
--添加玩家节点
function QuickPassReslutListLayer:loadRewardNode(reslut_node,reslutItem,index)
    ...
    if reslutItem.addCoin and reslutItem.addCoin > 0 then
...
function QuickPassReslutListLayer:removeUI()
    self.super.removeUI(self);
    TFDirector:removeTimer(self.reardTimeId);
end

function QuickPassReslutListLayer.onSkipClickHandle(sender)
...
function QuickPassReslutListLayer:onSumComplete()
...
function QuickPassReslutListLayer:registerEvents()
...
function QuickPassReslutListLayer:removeEvents()
    self.panel_tiaoguo:removeMEListener(TFWIDGET_CLICK)
    self.isSkip = false
    self.super.removeEvents(self)
end

return QuickPassReslutListLayer;
