...
function ActivityOnline:ctor(data)
    ...
    self.type = EnumActivitiesType.ONLINE_REWARD_NEW
...
function ActivityOnline:initUI(ui)
    ...
    for i=1,rewardCount do
    	...
    	local widget = require('lua.logic.activity.operation.RewardItemOnline'):new(self.type, rewardData.id, timeText, i)
...
function ActivityOnline:resortReward()
    ...
    for i=1,rewardCount do
        ...
        print("rewardData.type = ", self.type )
...
function ActivityOnline:onShow()
 ...
 --    local rewardCount = self.rewardWidgetArray:length()
    ...
    self.rewardList       = OperationActivitiesManager:getActivityRewardList(self.type)
...
function ActivityOnline.onclikReward(sender)
    ...
    -- print("x =", x)
    OperationActivitiesManager:sendMsgToGetActivityReward(EnumActivitiesType.ONLINE_REWARD_NEW, sender.index)
end
