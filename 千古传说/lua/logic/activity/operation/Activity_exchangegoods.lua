...
function Activity_exchangegoods:initUI(ui)
    ...
    local position = ccp(20,self.img_reward:getPosition().y-150)
    for i=1,rewardCount do
    	local rewardData = rewardList:objectAt(i)

    	local widget = require('lua.logic.activity.operation.exchangeCell'):new(self.type, rewardData.id)
    ...
    -- if EnumActivitiesType.ZHAOBUG == self.type or EnumActivitiesType.YAOHAOYOU == self.type then
    --     self.img_reward:setVisible(false)
    -- end
