...
local ZhenbashaiBattlefieldLayer = class("ZhenbashaiBattlefieldLayer", BaseLayer)

function ZhenbashaiBattlefieldLayer:ctor()
...
function ZhenbashaiBattlefieldLayer:initUI(ui)
...
function ZhenbashaiBattlefieldLayer:registerEvents(ui)
    self.super.registerEvents(self)
    ADD_ALERT_CLOSE_LISTENER(self, self.closeBtn);

end

function ZhenbashaiBattlefieldLayer:removeEvents()
    self.super.removeEvents(self)

end


function ZhenbashaiBattlefieldLayer:refreshReportLayer(index)
    ...
    if self.tableViewList[index] == nil then
        ...
        tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, ZhenbashaiBattlefieldLayer.cellSizeForTable)
        tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, ZhenbashaiBattlefieldLayer.tableCellAtIndex)
        tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, ZhenbashaiBattlefieldLayer.numberOfCellsInTableView)
...
function ZhenbashaiBattlefieldLayer.cellSizeForTable(table,idx)
    return 30,630
end

function ZhenbashaiBattlefieldLayer.tableCellAtIndex(table, idx)
    ...
    -- local reportList = ZhengbaManager:getReport( self.selectedIndex )
    cell.panel:setData(self.personGrandList:objectAt(idx + 1))
    return cell
end

function ZhenbashaiBattlefieldLayer.numberOfCellsInTableView(table)
    local self = table.logic
    -- local reportList = ZhengbaManager:getReport( self.selectedIndex )
    return self.personGrandList:length()
end
function ZhenbashaiBattlefieldLayer:selectSideButton(index)
    ...
    self.personGrandList:clear()
    local dataRecord = ZhengbaManager:getReport( self.selectedIndex )
...
--初始化buttongroup
function ZhenbashaiBattlefieldLayer:initButtonGroup(ui)
...
--获取当前选中的频道
function ZhenbashaiBattlefieldLayer:getSelectedChannelIndex()
    return self.selectedIndex
end

--选中某个分类，一般在其他地方调用。如：打开界面时默认选中某个界面的时候
function ZhenbashaiBattlefieldLayer:changeGroupChoice( index )
...
return ZhenbashaiBattlefieldLayer