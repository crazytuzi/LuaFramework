...
local YuanfenUsedLayer = class("YuanfenUsedLayer", BaseLayer)

CREATE_PANEL_FUN(YuanfenUsedLayer)


function YuanfenUsedLayer:ctor(id)
...
function YuanfenUsedLayer:initUI(ui)
...
function YuanfenUsedLayer:removeUI()
...
function YuanfenUsedLayer:onShow()
...
function YuanfenUsedLayer:initInfo( id )
...
function YuanfenUsedLayer:setNum( num )
...
function YuanfenUsedLayer:refreshUI()
...
function YuanfenUsedLayer.reduceButtonClickHandle(sender)
...
function YuanfenUsedLayer.addButtonClickHandle(sender)
	local self = sender.logic
	self.choiceNum = self.choiceNum + 1
	sender.logic:setNum(self.choiceNum)
	self:freshButtonState()
...
function YuanfenUsedLayer.useButtonClickHandle(sender)
	local self = sender.logic
	...
	if self.clickCallBack then
		TFFunction.call(self.clickCallBack , self.id , self.choiceNum)
	else
...
function YuanfenUsedLayer.sliderTouchBeginHandle(sender)
	local self = sender.logic
	self.bg_jindushuzhi:setVisible(true)
	self:freshSliderNum()
end

function YuanfenUsedLayer.sliderTouchMoveHandle(sender)
	local self = sender.logic
	self:freshSliderNum()
end

function YuanfenUsedLayer.sliderTouchEndHandle(sender)
	local self = sender.logic
...
function YuanfenUsedLayer:freshSliderNum()
...
--设置按钮状态
function YuanfenUsedLayer:freshButtonState()
...
--刷新回调
function YuanfenUsedLayer:refreshCallback()
    self:refreshUI()
end

function YuanfenUsedLayer:registerEvents()
...
function YuanfenUsedLayer:removeEvents()
...
function YuanfenUsedLayer:setClickFun(clickCallBack)
	self.clickCallBack = clickCallBack
end
return YuanfenUsedLayer;
