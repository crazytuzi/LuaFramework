...
function RewardItem:initUI(ui)
    ...
    if self.activityType == OperationActivitiesManager.Type_Continue_Recharge then
    	local desc1 = stringUtils.format(self.desc1, self.index)
...
function RewardItem:refreshUI()
	...
	if num >= 10000 then
        ...
        local floatValue = string.format("%.0f", math.floor(num/10000))
		desc = stringUtils.format(localizable.rewardItemcommmon_wan, floatValue)
	end
	...
	if self.activityType == OperationActivitiesManager.Type_Single_Recharge then
		...
		if reward then
			...
			if reward.gottime >= reward.maxtime then
				...
				self.txt_maxwarning:setText(stringUtils.format(localizable.common_tomax_times,reward.maxtime))
				
			else
				--self.txt_maxwarning:setText("最多累计"..reward.maxtime.."次")
				self.txt_maxwarning:setText(stringUtils.format(localizable.common_total_times,reward.maxtime))
			end
		end
    end
end
