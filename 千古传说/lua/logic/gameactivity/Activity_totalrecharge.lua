local Activity_totalrecharge = class("Activity_totalrecharge", BaseLayer)

local Score_ShowOneServerPoint = {-2,-30}
local Score_ShowMultiServerPoint = {-15,-43}


function Activity_totalrecharge:ctor(type)
    self.super.ctor(self)
    self.id   = type
    self.type =OperationActivitiesManager.Type_Exchange
    self:init("lua.uiconfig_mango_new.operatingactivities.chongzhi")
end

function Activity_totalrecharge:initUI(ui)
    ...
    -- local txt_title              = TFDirector:getChildByPath(self.bg_jifen, 'txt_title')
    -- txt_title:setText("当前充值")
...
function Activity_totalrecharge:removeUI()
    self.super.removeUI(self)
end

function Activity_totalrecharge:onShow()
    self.super.onShow(self)

    self:refreshUI()
end

function Activity_totalrecharge:dispose()
    self.super.dispose(self)
end

function Activity_totalrecharge:refreshUI()   
    self:drawRewardList()
...
function Activity_totalrecharge:setLogic(logic)
    self.logic = logic
end

function Activity_totalrecharge:registerEvents()
    print("Activity_totalrecharge:registerEvents()------------------")
...
function Activity_totalrecharge:removeEvents()
    print("Activity_totalrecharge:removeEvents()------------------")
    self.super.removeEvents(self)
    TFDirector:removeMEGlobalListener(OperationActivitiesManager.MSG_ACTIVITY_PROGRESS_UPDATE,self.updateRewardCallback)
end

function Activity_totalrecharge:getDateString(timestamp)
...
function Activity_totalrecharge:drawRewardList()
...
function Activity_totalrecharge.numberOfCellsInTableView_fate(table)
    local self = table.logic

    return self.rewardList:length()
end

function Activity_totalrecharge.cellSizeForTable_fate(table,idx)
    return 131, 553
end

function Activity_totalrecharge.tableCellAtIndex_fate(table, idx)
    local self = table.logic
...
function Activity_totalrecharge:drawCell(node)
    local itemList = {}
    for i=1,3 do
        itemList[i] = {}
    ...
    txt_desc:setText("当前排名段最低充值")


    local index = node.index

    local rankDesc = "活动充值第%d-%d名奖励"
    ...
    if minlevel ~= maxlevel then
        rankDesc = string.format(rankDesc, minlevel, maxlevel)
    else
        rankDesc = "活动充值第%d名奖励"
        rankDesc = string.format(rankDesc, minlevel)
    end
    ...
    local prizeUserName = string.format("第%d名:%s", rewardInfo.minlevel, rewardInfo.userName)
