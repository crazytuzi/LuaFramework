local CarbonQuickPassLayer = class("CarbonQuickPassLayer", BaseLayer);

CarbonQuickPassLayer.LIST_ITEM_HEIGHT = 140; 

CREATE_SCENE_FUN(CarbonQuickPassLayer);
CREATE_PANEL_FUN(CarbonQuickPassLayer);
...
function CarbonQuickPassLayer:ctor(data)
    self.super.ctor(self,data);
    
    self:init("lua.uiconfig_mango_new.mission.QuickPassReslutListLayer");
end

function CarbonQuickPassLayer:initUI(ui)
    ...
    self.mubiaoNum      = TFDirector:getChildByPath(self.bg_mubiao, 'LabelBMFont_CarbonQuickPassLayer_1')
...
function CarbonQuickPassLayer:loadData(data)
    self.itemlist = data
    print('CarbonQuickPassLayer = ',self.itemlist)
    
    self:refreshUI();
end


function CarbonQuickPassLayer:onShow()
    self.super.onShow(self)
    self:refreshBaseUI();
end

function CarbonQuickPassLayer:refreshBaseUI()

end
function CarbonQuickPassLayer:refreshUI()
...
function CarbonQuickPassLayer:loadTotolList()
...
function CarbonQuickPassLayer:loadRewardList()
    ...
    for index,reslutItem in pairs(self.itemlist) do
        ...
        reslut_node:setPosition(ccp(0,(#self.itemlist - index) * CarbonQuickPassLayer.LIST_ITEM_HEIGHT))
        self.list_reward:addChild(reslut_node)
        reslut_node:setVisible(false);
        reslut_nodeArr[index] = reslut_node;
    end

    self.list_reward:setInnerContainerSize(CCSizeMake(self.list_reward:getSize().width , #self.itemlist * CarbonQuickPassLayer.LIST_ITEM_HEIGHT + 20))
    self.list_reward:scrollToYTop(0);
    self.list_reward:setInnerContainerSizeForHeight(#self.itemlist * CarbonQuickPassLayer.LIST_ITEM_HEIGHT + 20)
    ...
    self.change = function()
        ...
        -- local newOffset = ccp(oldOffset.x,oldOffset.y + CarbonQuickPassLayer.LIST_ITEM_HEIGHT);
...
--添加玩家节点
function CarbonQuickPassLayer:loadRewardNode(reslut_node,reslutItem,index)
...
function CarbonQuickPassLayer:removeUI()
    self.super.removeUI(self);
    TFDirector:removeTimer(self.reardTimeId);
end

function CarbonQuickPassLayer.onSkipClickHandle(sender)
...
function CarbonQuickPassLayer:onSumComplete()
...
function CarbonQuickPassLayer:registerEvents()
...
function CarbonQuickPassLayer:removeEvents()
    self.panel_tiaoguo:removeMEListener(TFWIDGET_CLICK)
    self.isSkip = false
    self.super.removeEvents(self)
end

return CarbonQuickPassLayer;
