...
function MapLayer:ctor(data)
    ...
    -- effectType : 1全景 2前景 3背景
    local gEftID, fEftID, bEftID = self:getEffectIDs()
    self:addMapEffect(1, fightBGImg, gEftID)
    self:addMapEffect(2, fightBGImg, fEftID)
    self:addMapEffect(3, fightBGImg, bEftID)
    -- self:AddMapEffect(false, fightBGImg, mapID)
    -- self:AddMapEffect(true, fightBGImg, mapID)
    self:addSpellAnimationUI()
end
...
function MapLayer:addSpellAnimationUI()
	local spellUI = createUIByLuaNew("lua.uiconfig_mango_new.fight.SpellUiLayer")
	self.spellUI = spellUI
	self.spellUI:setVisible(false)
	self.spellUI:setZOrder(4)
	self:addChild(spellUI)

	self.labelName = TFDirector:getChildByPath(spellUI, 'jineng')
	self.imgCareer = TFDirector:getChildByPath(spellUI, 'zhiye')
end

function MapLayer:playSpellAnimation(skillName, profession)
	self.imgCareer:setTexture("ui_new/common/img_role_type" .. profession .. ".png")
	self.labelName:setText(skillName)
	self.spellUI:setVisible(true)
	self.spellUI:runAnimation("Action0", 1)
end

function MapLayer:addMapEffect(effectType, fightBGImg, effectID)
	if effectID == 0 then return end

	local effectName = self:getEffectName(effectType, effectID)

	if not ModelManager:existResourceFile(2, effectName) then return end
...
function MapLayer:getEffectName(effectType, effectID)
	local effectName = "fightbg_effect"
	if effectType == 1 then 
		return effectName .. effectID
	elseif effectType == 2 then
		return effectName .. effectID .. "_f"
	else
		return effectName .. effectID .. "_b"
	end
end

function MapLayer:getEffectIDs()
	...
	if fightType == 1 then
		if FightManager.fightBeginInfo.bGuideFight then
			...
			if missionInfo ~= nil then
				local effectIDs = string.split(missionInfo.fightbg_effect, ',')
				return effectIDs[1], effectIDs[2], effectIDs[3]
			end
		end
		return 0, 0, 0
	end
...
function MapLayer:GetMapID()
	...
	elseif fightType == 9 then --血战
		return 10
	elseif fightType == 9 then --10:世界boss
		return 4
	elseif fightType == 16 then --无量山
		return NorthClimbManager:getMapId()
	elseif fightType == 18 then --矿洞
		return 22
	elseif fightType == 12 then --矿洞
		return 30
	elseif fightType == 15 then --矿洞
		return 30
	else
		return 1
	end
...
function MapLayer.GetPosByIndex(posIndex)
	local function GetLeftPosByIndex(posIndex)
		...
		local nPosX = xStartPos + xOffset * nGirdOffsetX + nGirdOffsetY * 20
		-- local nPosY = 125 + 102*nGirdOffsetY + nGirdOffsetX
		local nPosY = yStartPos + yOffset * nGirdOffsetY
...
function MapLayer.GetScaleByYPos(yPos)
	local diffScale = maxScale - minScale
	local diffYPos = 2 * yOffset
	local factor = (yPos - yStartPos) / diffYPos
	return maxScale - (factor * diffScale)
end
