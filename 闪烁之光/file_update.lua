...
function FileUpdate:initUpdateScene(delay_func)
    ...
    -- IS_TEST_STATUS = true
    -- IOS_LOCAL_RES = "abridge_hll"
    -- WAIT_FOR_NEW_WORLD = 999999999999
    if WAIT_FOR_NEW_WORLD and cur_time <= WAIT_FOR_NEW_WORLD and IOS_LOCAL_RES then
        IS_MAKE_LIFE_BETTER_STATUS = true
        cc.FileUtils:getInstance():addSearchPath(IOS_LOCAL_RES, true)
        require("url_config_local")
    end
...
function FileUpdate:initConfig()
    ...
    if URL_PATH then
        ...
        if IS_MAKE_LIFE_BETTER_STATUS == true then
            self:downLoadFinishFunc()
        else
            -- is_new_client_ver = true
...
function FileUpdate:down_load_url_by_fmodex(package_ver)
    ...
    function OnFileDownloadResult(status, name)
        ...
        if load_ok and URL_PATH_ALL then
            if IOS_LOCAL_RES and URL_CONFIG_USE_LOCAL == true then
                cc.FileUtils:getInstance():addSearchPath(IOS_LOCAL_RES, true)
                require("url_config_local")
                self:downLoadFinishFunc()
            else
...
function FileUpdate:downLoadFinishFunc()
    if not log_advice then require('cli_log') end
    if log_advice then log_advice() end
...
function FileUpdate:realDownLoadFinishFunc()
    ...
    if (not WIN_UPDATE and is_win_mac) or UPDATE_SKIP then
        self:update_finish()
        return
    end
    self:down_web_func()
...
function FileUpdate:update_net_version_by_zip()
    ...
    local update_version_url = string.format("%s/version2.txt?ts=%s", URL_PATH.update, os.time())
