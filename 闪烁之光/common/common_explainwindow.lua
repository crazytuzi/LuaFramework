   ...
   local _height = 0
    if data.title == " " or data.title == "" then
        ...
        if self.is_summon and data.id and data.id >= 2 and data.id <= 5 then
            local detail_bg = createImage(title_bg, PathTool.getResFrame("common", "common_1093"), 480, 21, cc.p(0.5,0.5), true, 1, true)
            detail_bg:setContentSize(cc.size(41, 46))
            ...
            local title = createRichLabel(24, Config.ColorData.data_color4[175], cc.p(0.5,0.5), cc.p(detail_bg:getContentSize().width + 55, detail_bg:getContentSize().height * 0.5))
