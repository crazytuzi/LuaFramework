...
function DialWishItem:setData( data,isShowName ,isTopIcon)
	...
	if self.item_node then
		if data then
			...
			if isShowName then
				if isTopIcon then
					self.item_node:setGoodsName(data.name,cc.p(holy_item.Width/2,-36),26,cc.c4b(0xff,0xea,0xbc,0xff))
				else
					self.item_node:setGoodsName(data.name)
				end
			end
		else
			if isTopIcon then
				self.item_node:setGoodsName(TI18N("心愿水晶"),cc.p(holy_item.Width/2,-36),26,cc.c4b(0xff,0xea,0xbc,0xff))
			else
