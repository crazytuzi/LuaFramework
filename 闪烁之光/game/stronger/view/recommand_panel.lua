...
function RecommandPanel:createItemList(  )
	...
	self.max_height = math.max((166+offset_y)*#show_list,self.scroll:getContentSize().height)
	...
	for k,v in ipairs(show_list) do
		delayRun(self.scroll,0.05*k,function ()
			...
			item:setPosition(5,self.max_height-3-(166+offset_y)*(k-1))
			...
			item:setBtnCallBack(function ( cell )
				...
				if status then 
					...
					for k,v in pairs(self.item_list) do
						v:setPosition(5,height-3-(166+offset_y)*(k-1))
					end
					self.cur_select:setSelect(false)
					self.cur_select:showMessagePanel(false)
				else
	...
	if self.cur_select ~= nil then 
		...
		for k,v in pairs(self.item_list) do
			if k<=self.cur_index then 
				...
				v:setPosition(5,height-3-170-(166+offset_y)*(k-1))
			end
		end
	end
end
...
function RecommandItem:configUI(  )
    ...
    self:setCascadeOpacityEnabled(true)
	self:setContentSize(cc.size(606,166))
...
function RecommandItem:createMessagePanel( )
	if self.msg_panel == nil then 
		...
		self.msg_panel:setContentSize(cc.size(585,168))
	...
	local title = createRichLabel(22, 175, cc.p(0.5,0.5), cc.p(self.msg_panel:getContentSize().width/2,150))
	title:setString(TI18N("阵容分析"))
	self.msg_panel:addChild(title)

	self.desc = createRichLabel(20, 175, cc.p(0,1), cc.p(20,129), 5, 0, 550)
