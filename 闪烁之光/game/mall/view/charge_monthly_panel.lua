...
ChargeMonthlyPanel = ChargeMonthlyPanel or BaseClass()

function ChargeMonthlyPanel:__init(parent, offset_y)
    self.is_init = true
    self.parent = parent
...
function ChargeMonthlyPanel:loadResListCompleted( )
	self:createRootWnd()
    self:registerEvent()
end

function ChargeMonthlyPanel:createRootWnd( )
...
function ChargeMonthlyPanel:createNewCell( width, height )
    local cell = ChargeMonthlyItem.new(width, height)
    cell:addCallBack(function(id) self:onClickCallBack(id) end)
	return cell
end

function ChargeMonthlyPanel:numberOfCells(  )
    if not self.weekly_data then return 0 end
    return #self.weekly_data
end

function ChargeMonthlyPanel:updateCellByIndex( cell, index )
...
function ChargeMonthlyPanel:onClickCallBack( charge_id )
    if charge_id then
        self.cur_charge_id = charge_id
    end
end

function ChargeMonthlyPanel:registerEvent( )
...
function ChargeMonthlyPanel:updateItemData( data )
...
function ChargeMonthlyPanel:setData( data )
    if not data then return end
...
function ChargeMonthlyPanel:getRegisteDayReward(id,day)
    local reward_list = Config.MiscData.data_cycle_gift_reward
...
--排序
function ChargeMonthlyPanel:sortList(list)
...
--获取购买的数量
function ChargeMonthlyPanel:setBuyCount(data)
...
function ChargeMonthlyPanel:getBuyCount(id)
...
function ChargeMonthlyPanel:setLessTime(less_time)
...
function ChargeMonthlyPanel:setTimeFormatString(time)
    if time > 0 then
...
function ChargeMonthlyPanel:setVisibleStatus( status )
    ...
    if status == true and self.is_init == true then
    	self.is_init = false
        WelfareController:getInstance():sender21007(2)
...
function ChargeMonthlyPanel:addChild( node )
	if not tolua.isnull(self.root_wnd) and not tolua.isnull(node) then
        self.root_wnd:addChild(node)
    end
end

function ChargeMonthlyPanel:setPosition( pos )
	if not tolua.isnull(self.root_wnd) then
        self.root_wnd:setPosition(pos)
    end
end

function ChargeMonthlyPanel:__delete()
    doStopAllActions(self.time_txt)
...
-----------------------@ item
ChargeMonthlyItem = class("ChargeMonthlyItem", function()
	return ccui.Widget:create()
end)

function ChargeMonthlyItem:ctor(width, height)
...
function ChargeMonthlyItem:configUI( width, height )
...
function ChargeMonthlyItem:addCallBack( callback )
    self.callback = callback
end

function ChargeMonthlyItem:setData1( data )
    if not data then return end
    ...
    -- 价格
    self.price_txt:setString("￥" .. data.val)
...
function ChargeMonthlyItem:setData( data )
    if not data then return end
...
function ChargeMonthlyItem:getChargeId(  )
    if self.data then
        return self.data.charge_id
    end
end

function ChargeMonthlyItem:DeleteMe(  )
