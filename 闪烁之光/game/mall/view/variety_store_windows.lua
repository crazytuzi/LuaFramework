...
function VarietyStoreWindows:open_callback()
    ...
    self.time_label:setString("0:00:00")

    self.refresh_btn = main_container:getChildByName("refresh_btn")
    self.refresh_btn_label = createRichLabel(26, 1, cc.p(0.5, 0.5), cc.p(82, 30))
...
function VarietyStoreWindows:updateRefreshInfo(  )
	if self.data then
		...
		self.refresh_label:setString(string.format(TI18N("刷新次数:%d/%d"), self.data.count, max_num_cfg.val))
		
		-- 按钮上刷新文字
		local btn_str = ""
		if self.data.free_count > 0 then
			local free_num_cfg = Config.ExchangeData.data_shop_exchage_cost["max_free_times"]
			btn_str = string.format(TI18N("<div fontcolor=#ffffff outline=2,#294a15>刷新(%d/%d)</div>"), self.data.free_count, free_num_cfg.val)
			...
			if refresh_cost_cfg and refresh_cost_cfg.cost_list then
                ...
                if item_config then 
                    ...
                    btn_str = string.format(TI18N("<img src='%s' scale=0.3 /><div fontcolor=#ffffff outline=2,#294a15>%d 刷新</div>"),res,num)
                end
			end
		end
		...
		if self.data.refresh_time > 0 then
			...
			self.time_label:setString(TimeTool.GetTimeFormat(left_time))
		else
			self:openRefreshTimer(false)
			self.time_label:setString("0:00:00")
		end
...
function VarietyStoreItem:setData( data )
	...
	if not self.price_label then
		self.price_label = createRichLabel(26, cc.c3b(123,41,0), cc.p(0.5, 0.5), cc.p(self.size.width/2, 30))
