...
function ExchangeWindow:open_callback(  )
    ...
    for i=1,3 do
    	...
    	tab.price = createRichLabel(24, Config.ColorData.data_color4[1], cc.p(0.5,0.5), cc.p(tab.btn:getContentSize().width/2,tab.btn:getContentSize().height/2), nil, nil, nil)
    	tab.btn:addChild(tab.price)
    	tab.gold = createRichLabel(22, Config.ColorData.data_color4[1], cc.p(0.5,0.5), cc.p(pos[i],294), nil, nil, nil)
        main_container:addChild(tab.gold)
        local tips_bg = main_container:getChildByName("tips_bg_"..i)
        tab.tips = createRichLabel(20, cc.c4b(0x95,0x53,0x22,0xff), cc.p(0.5,0.5), cc.p(70,75), 5, nil, nil)
...
function ExchangeWindow:getUpdataRewardItem(data)
    ...
    for i,v in pairs(data.list) do
        ...
        if exchange_cfg and exchange_cfg[v.id] then
            ...
            self.object[i].tips:setString(string_format(TI18N("    <img src=%s visible=true scale=0.7 />挂机\n<div fontcolor=#249003>%.1f</div>小时收益"),common_2034_res,time_num))
        end
        if times <= 0 then
            ...
            local str = ""
            if v.price == 0 then
                str = string_format(TI18N("<div outline=2,#6c3018>免费获取</div>"))
            else
                local item_config = Config.ItemData.data_get_data(3)
                local res = PathTool.getItemRes(item_config.icon)
                str = string_format(TI18N("<img src=%s visible=true scale=0.30 />   <div outline=2,#6c3018>%d 获取</div>"), res, v.price)
            end
