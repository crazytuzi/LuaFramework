...
function ElitematchFightResultPanel:open_callback()
    ...
    self.fight_text = createLabel(24, cc.c4b(0xff,0xee,0xac,0xff), nil, 360, 555, "",self.container, nil, cc.p(0.5,0.5))
    ...
    if self.scdata and self.scdata.all_hurt_statistics then
        ...
        for i,v in ipairs(self.scdata.all_hurt_statistics) do
            if self.scdata.combat_type == BattleConst.Fight_Type.EliteKingMatchWar then
                v.atk_name  = string_format("%s(队伍%s)",atk_name, v.a_round)
                v.target_role_name  = string_format("%s(队伍%s)",target_role_name, v.b_round)
            else
                v.atk_name  = atk_name
...
function ElitematchFightResultPanel:setData(scdata)
    ...
    local list = {key = "精英积分:", left_value = left_count, right_value = self.scdata.add_score}
    table_insert(show_list, list)
    
    local list2 = {key = "段位经验:", left_value = self.scdata.end_score, right_value = self.scdata.add_score}
...
function ElitematchFightResultPanel:initShowList(show_list)
    ...
    for i=1,count do
        ...
        elseif data.score ~= nil then
            ...
            createLabel(24, cc.c4b(0xff,0xee,0xac,0xff), nil, 230, y, str, self.container, 2, cc.p(0, 0.5))
            local res , score
            ...
            if data.not_change then
                createLabel(24, cc.c4b(0xff,0xee,0xac,0xff), nil, 496, y, TI18N("不变"), self.container, 2, cc.p(0, 0.5))
            else
