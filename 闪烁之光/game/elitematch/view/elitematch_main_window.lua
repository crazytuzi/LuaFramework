...
function ElitematchMainWindow:open_callback(  )
    ...
    --比赛时间
    self.match_time = createRichLabel(22, cc.c4b(0xff,0xf8,0xbf,0xff), cc.p(0.5,0.5), cc.p(360,-21), nil, nil, 600)
    self.top_panel:addChild(self.match_time)
    --预计王者比赛时间
    self.king_match_time = createRichLabel(22, cc.c4b(0xff,0xf8,0xbf,0xff), cc.p(0.5,0.5), cc.p(360,-60), nil, nil, 600)
    ...
    self.my_rank_label = createRichLabel(22, cc.c4b(0xff,0xf8,0xbf,0xff), cc.p(0,0.5), cc.p(506,234), nil, nil, 600)
    ...
    self.cool_time = createRichLabel(20, cc.c4b(0xff,0xf8,0xbf,0xff), cc.p(0.5,0.5), cc.p(360,215), nil, nil, 600)
...
function ElitematchMainWindow:setData(scdata)
    ...
    if self.scdata.rank == 0 then
        rank_str = string.format(TI18N("<div outline=2,#000000>%s</div>"),TI18N("我的排名:暂未上榜")) 
    else
        rank_str = string.format(TI18N("<div outline=2,#000000>%s</div><div fontcolor=#3df424 outline=2,#000000>%s</div>"),TI18N("我的排名:"), self.scdata.rank) 
    end
...
function ElitematchMainWindow:updateTimeInfo()
    ...
    self.match_time:setString(string_format(TI18N("<div outline=2,#000000>S%s赛季%s时间: </div><div fontcolor=#3df424 outline=2,#000000>%s-%s</div>"), self.scdata.period, zone_name, start_time, end_time))
...
function ElitematchMainWindow:updateBuyCount()
    ...
    local str = string.format(TI18N("<div outline=2,#000000>%s</div><div fontcolor=#3df424 outline=2,#000000>%s</div>"),TI18N("剩余购买次数:"), count)
...
function ElitematchMainWindow:updateCommonInfo()
    ...
    if config then
        ...
        local bg_res = PathTool.getPlistImgForDownLoad("elitematch/elitematch_icon",config.name_ico, false)
    ...
    if config then
        ...
        self.reward_label:setString(string_format("<div outline=2,#000000>%s</div><div fontcolor=#ffb400 outline=2,#000000>\"%s\"</div><div outline=2,#000000>%s</div>", str1, config.name, str2))
        --奖励数据
        local setting = {}
        setting.scale = 0.8
        setting.max_count = 4
        ...
        if self.show_data.flag == 0 then
            --不可领取
            self.receive_btn_label:setString("领取")
            self.receive_btn_label:disableEffect(cc.LabelEffect.OUTLINE)
        ...
        elseif self.show_data.flag == 1 then
            ...
            self.receive_btn_label:setString("已领取")
            self.receive_btn_label:enableOutline(Config.ColorData.data_color4[264], 2) --橙色
            setChildUnEnabled(false, self.receive_btn)
            addRedPointToNodeByStatus(self.receive_btn, false, 5, 5)
        end
...
function ElitematchMainWindow:showProgressbar(status, percent, blue_percent, label)
    ...
    if status then
        ...
        if not self.comp_bar then
            ...
            local bg,comp_bar = createLoadingBar(res, res1, size, self.leve_up_node, cc.p(0.5,0.5), 0, 0, true, true)
            self.comp_bar_bg = bg
...
function ElitematchMainWindow:setTimeFormatString(time)
    ...
    local str = string.format(TI18N("<div outline=2,#000000>%s</div><div fontcolor=#3df424 outline=2,#000000>%s</div><div outline=2,#000000>%s</div>"), self.match_time_str1, timestr, self.match_time_str2)
