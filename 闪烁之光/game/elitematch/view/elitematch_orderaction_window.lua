...
-- @DateTime:    2019-11-21
-- *******************************
ElitematchOrderactionWindow = ElitematchOrderactionWindow or BaseClass(BaseView)
...
function ElitematchOrderactionWindow:__init()
    self.is_full_screen = true
    self.win_type = WinType.Full

    self.layout_name = "elitematch/elitematch_orderaction_window"
    self.res_list = {
...
function ElitematchOrderactionWindow:open_callback()
    self.background = self.root_wnd:getChildByName("background")
    self.background:setScale(display.getMaxScale())
    if self.background ~= nil then
    	self.background:loadTexture(PathTool.getPlistImgForDownLoad("bigbg/elitematch","elitematch_main_bg",true), LOADTEXT_TYPE)
    end
    ...
    self.btn_open_lock_label:setString(TI18N("解锁王者之证"))
    ...
    self.main_container:getChildByName("level_title"):setString(TI18N("当前胜场："))
    ...
    self.main_container:getChildByName("Text_33"):setString(TI18N("段位赛获得指定胜场解锁超值奖励"))
...
--设置适配屏幕
function ElitematchOrderactionWindow:adaptationScreen()
...
--进度条
function ElitematchOrderactionWindow:createProgress()
    if self.comp_bar == nil then
        local size = cc.size(315, 27)
        local res = PathTool.getResFrame("elitematch","elitematch_28")
        local res1 = PathTool.getResFrame("elitematch","elitematch_32")
...
function ElitematchOrderactionWindow:updatePercent(win_count)
...
function ElitematchOrderactionWindow:register_event()
    ...
    self:addGlobalEvent(ElitematchEvent.Elite_OrderAction_IsPopWarn_Event, function(data)
        if data then
            local totle_day = 30
            if (totle_day - data.cur_day) == 7 or (totle_day - data.cur_day) == 3 or (totle_day - data.cur_day) == 0 then
                if data.is_pop == 1 then
                    controller:openElitematchEndWarnView(true,data.cur_day)
                end
            end
        end
    end)
    ...
    registerButtonEventListener(self.btn_go, function()
        controller:openElitematchOrderactionWindow(false)
...
--奖励列表
function ElitematchOrderactionWindow:createTaskCell()
	local cell = ElitematchOrderActiodRewardItem.new()
    return cell
end
function ElitematchOrderactionWindow:numberOfTaskCells()
	if not self.reward_list then return 0 end
    return #self.reward_list
end
function ElitematchOrderactionWindow:updateTaskCellByIndex(cell, index)
...
--滑动的时候处理显示
function ElitematchOrderactionWindow:updateSlideShowByVertical()
...
--胜利数列表
function ElitematchOrderactionWindow:createTaskCell2()
	local cell = ElitematchOrderActiodRewardItem2.new()
    return cell
end
function ElitematchOrderactionWindow:numberOfTaskCells2()
	if not self.reward_list then return 0 end
    return #self.reward_list
end
function ElitematchOrderactionWindow:updateTaskCellByIndex2(cell, index)
	if not self.reward_list then return end
    local cell_data = self.reward_list[index]
    if not cell_data then return end
    cell:setData(cell_data)
...
function ElitematchOrderactionWindow:updateIconImg(status)
    if tolua.isnull(self.icon_img) or tolua.isnull(self.btn_open_lock) then
        return
    end
    if status == 1 then
        loadSpriteTexture(self.icon_img, PathTool.getResFrame("elitematch","elitematch_38"), LOADTEXT_TYPE_PLIST)        
    else
        loadSpriteTexture(self.icon_img, PathTool.getResFrame("elitematch","elitematch_39"), LOADTEXT_TYPE_PLIST)        
    end
...
function ElitematchOrderactionWindow:jumpToMoveByY(y)
...
--当等级变化的时候
function ElitematchOrderactionWindow:setChangeLevelStatus(cur_lev)
    local cur_period = model:getCurPeriod()
	if lev_reward_list and lev_reward_list[cur_period] then
    	self.reward_list = {}
...
--设置数据
function ElitematchOrderactionWindow:setBasicInitData(data)
...
function ElitematchOrderactionWindow:openRootWnd()
    model:setOrderactionRedStatus(0)
    model:setSCDataBackOrderaction()
    GlobalEvent:getInstance():Fire(ElitematchEvent.Elite_OrderAction_First_Red_Event)
    controller:sender24952()
    controller:sender24955()
end
function ElitematchOrderactionWindow:close_callback()
...
------------------------------------------
-- 奖励子项
ElitematchOrderActiodRewardItem = class("ElitematchOrderActiodRewardItem", function()
    return ccui.Widget:create()
end)

function ElitematchOrderActiodRewardItem:ctor()
    self:configUI()
    self:register_event()
end

function ElitematchOrderActiodRewardItem:configUI()
    self.root_wnd = cc.CSLoader:createNode(PathTool.getTargetCSB("elitematch/elitematch_orderaction_reward_item"))
...
function ElitematchOrderActiodRewardItem:register_event()
...
function ElitematchOrderActiodRewardItem:setData(data)
    if not data then return end
    self.data = data
    ...
    if self.common_goods_item and not tolua.isnull(self.common_goods_item) then
    	...
    	if data.status == 1 then
    		self.common_goods_item:IsGetStatus(true,nil,PathTool.getResFrame("elitematch","txt_cn_elitematch_17"))
    	else
    ...
    setting.get_status_res = PathTool.getResFrame("elitematch","txt_cn_elitematch_17")
...
function ElitematchOrderActiodRewardItem:DeleteMe()
...
------------------------------------------
-- 场次子项
ElitematchOrderActiodRewardItem2 = class("ElitematchOrderActiodRewardItem2", function()
    return ccui.Widget:create()
end)

function ElitematchOrderActiodRewardItem2:ctor()
    self.size = cc.size(100,139)
    self:configUI()
    self:register_event()
end

function ElitematchOrderActiodRewardItem2:configUI()
    self.root_wnd = ccui.Layout:create()
	self.root_wnd:setContentSize(self.size)
	self.root_wnd:setAnchorPoint(0.5,0.5)
    self:addChild(self.root_wnd)
    self.select_bg = createImage(self.root_wnd, PathTool.getResFrame("elitematch", "elitematch_26"),self.size.width/2,self.size.height/2+6,cc.p(0.5, 0.5),true)
    self.select_img = createImage(self.root_wnd, PathTool.getResFrame("elitematch", "elitematch_25"),self.size.width/2,self.size.height/2+6,cc.p(0.5, 0.5),true)
    self.select_img:setVisible(false)
    self.num_bg = createImage(self.root_wnd, PathTool.getResFrame("elitematch", "elitematch_35"),self.size.width/2,self.size.height/5+11,cc.p(0.5, 0.5),true,nil,true)
...
function ElitematchOrderActiodRewardItem2:register_event()
end

function ElitematchOrderActiodRewardItem2:setData(data)
    if not data then return end
    self.data = data
    local win_count = data.win_count or 0
...
function ElitematchOrderActiodRewardItem2:DeleteMe()
