...
function ChargePanel:createItemList( list )
	if list == nil then return end

	local item_height = 159
	...
	for k,v in ipairs(list) do
		delayRun(self.scrollView,0.05*k,function (  )
			if self.item_list[k] == nil then
				...
				item:setPosition(cc.p(2, height - (k-1) *item_height))
...
function ChargeItem:configUI(  )
    ...
    self:addChild(self.root_wnd)
	self:setCascadeOpacityEnabled(true)
	self:setContentSize(cc.size(674,159))
    ...
    self.extra_label = createRichLabel(30, 193, cc.p(0.5,0.5), cc.p(0,0))
...
function ChargeItem:setData( data )
	self.data = data
	self.charge_price:setString((data.need_rmb/100)..TI18N("元"))
	...
	if data.add_gold > 0 then
		...
		self.extra_label:setString(string.format(TI18N("<img src=%s scale=0.3 visible=true /><div outline=2,#975324>%s</div>"),PathTool.getItemRes(Config.ItemData.data_get_data(4).icon),data.add_gold))
	else
