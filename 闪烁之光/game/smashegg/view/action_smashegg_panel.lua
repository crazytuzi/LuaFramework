...
function ActionSmasheggPanel:onClickAddItemBtn(  )
    ...
    if item_cfg and give_exp_cfg and gold_once_cfg then
        ...
        tips_str = tips_str .. string.format(TI18N("<div fontColor=#764519>购买<div fontColor=#289b14 fontsize= 26>%d</div><div fontColor=#d95014 fontsize= 26>英雄经验</div><div fontColor=#764519>(同时附赠</div><div fontColor=#289b14 fontsize= 26>1个</div><div fontColor=#764519>%s)</div>"), give_exp_cfg.val, item_cfg.name)
...
function ActionSmasheggPanel:onClickSmashAllBtn(  )
	...
	if have_num >= left_egg_num then
        ...
        if item_cfg and give_exp_cfg and gold_once_cfg then
            ...
            tips_str = tips_str .. string.format(TI18N("<div fontColor=#764519>购买<div fontColor=#289b14 fontsize= 26>%d</div><div fontColor=#d95014 fontsize= 26>英雄经验</div><div fontColor=#764519>(同时附赠</div><div fontColor=#289b14 fontsize= 26>%d个</div><div fontColor=#764519>%s)</div>"), give_exp_cfg.val*left_egg_num, left_egg_num, item_cfg.name)
            CommonAlert.show(tips_str, TI18N("确定"), function (  )
                _controller:sender16683()
            end, TI18N("取消"), nil, CommonAlert.type.rich)
        end--]]
