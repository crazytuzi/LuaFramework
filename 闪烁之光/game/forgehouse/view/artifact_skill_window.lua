...
function ArtifactSkillWindow:changeSelectedTab( index )
	...
	if self.tab_object then
		local unselect_res = PathTool.getResFrame("common","common_2023")
		if self.tab_object.index == SKILL_TAB.MEDIUM then
			unselect_res = PathTool.getResFrame("common","common_2022")
		end
...
function ArtifactSkillWindow:createTitleItem(name,x, y)
    ...
    item.bg:setScale(5)
    item.label = createLabel(26, cc.c3b(0x64,0x32,0x23), nil, x, y - self.title_height *0.5, name, self.skill_scrollview.scroll_view, nil, cc.p(0.5, 0.5))
end
