...
ReturnActionRedbagInfoWindow = ReturnActionRedbagInfoWindow or BaseClass(BaseView)

function ReturnActionRedbagInfoWindow:__init()
...
function ReturnActionRedbagInfoWindow:open_callback(  )
	...
	local setting = {
        item_class = ReturnActionRedbagInfoItem,      -- 单元类
        start_x = 0,                  -- 第一个单元的X起点
        space_x = 0,                    -- x方向的间隔
        start_y = 0,                    -- 第一个单元的Y起点
        space_y = 0,                   -- y方向的间隔
...
function ReturnActionRedbagInfoWindow:register_event(  )
	registerButtonEventListener(self.close_btn, function (  )
		_controller:openReturnRedbagInfoWindow(false)
	end, true, 2)
end

function ReturnActionRedbagInfoWindow:openRootWnd( data )
	self:setData(data)
end

function ReturnActionRedbagInfoWindow:setData( data )
	...
	if data.send_face_id then
		self.role_head:setHeadRes(data.send_face_id)
	end
...
function ReturnActionRedbagInfoWindow:close_callback(  )
