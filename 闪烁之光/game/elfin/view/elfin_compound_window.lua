...
function ElfinCompoundWindow:open_callback(  )
	...
	for i=1,2 do
		...
		object.item_node:setPosition(cc.p(137+(i-1)*271, 189))
	...
	self.cur_skill_desc_scroll = createScrollView(350, 85, 282, 584, self.container, ccui.ScrollViewDir.vertical)
...
function ElfinCompoundWindow:setData(  )
	...
	if not self.cur_elfin_item then
		...
		self.cur_elfin_item:setPosition(cc.p(130, 698))
	...
	if not self.cur_elfin_skill then
		...
		self.cur_elfin_skill:setPosition(cc.p(320, 716))
	...
	for i,v in ipairs(com_cfg.expend) do
		...
		if item_cfg and object then
            ...
            if have_num < item_num then
            	is_can_com = false
                object.cost_num:setTextColor(cc.c3b(255, 93, 93))
            else
                object.cost_num:setTextColor(cc.c3b(255, 246, 228))
            end
		end
	end
