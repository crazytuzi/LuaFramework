...
function ElfinInfoWindow:open_callback(  )
    ...
    self.power_label:setPosition(cc.p(290, 402))
...
function ElfinInfoWindow:setData( data )
	...
	if elfin_item_cfg then
		...
		loadSpriteTexture(self.type_bg, res, LOADTEXT_TYPE_PLIST)
	...
	if elfin_cfg.skill then
		...
		if skill_cfg then
			...
			if not self.desc_txt then
				self.desc_txt = createRichLabel(22, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0, 1), cc.p(0, 0), 5, nil, 340)
