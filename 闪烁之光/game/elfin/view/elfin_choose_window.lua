...
function ElfinChooseWindow:open_callback(  )
	...
	self.skill_desc_scroll = createScrollView(350, 70, 238, 15, self.elfin_info_panel, ccui.ScrollViewDir.vertical)
    ...
    self.power_label:setPosition(cc.p(395, 115))
    ...
    self.choose_tips_txt = createRichLabel(18, 274, cc.p(0, 0), cc.p(33, 758))
    container:addChild(self.choose_tips_txt)
    self.choose_tips_txt:setString(_string_format(TI18N("重复选中可取消选择、<div fontcolor=#ef3a3a>卸下</div>精灵")))
end
...
function ElfinChooseWindow:updateElfinInfo(  )
	...
	if not self.elfin_item then
		self.elfin_item = BackPackItem.new(false, true, false)
		self.elfin_item:setPosition(cc.p(95, 126))
	...
	if elfin_cfg.skill then
		...
		if skill_cfg then
			...
			self.skill_name_txt:setString(_string_format("%s Lv.%d", skill_cfg.name, skill_cfg.level))
			if skill_cfg.type == "active_skill" then 
		        self.skill_type_txt:setString(TI18N("类型：主动技能"))
		    else 
		        self.skill_type_txt:setString(TI18N("类型：被动技能")) 
		    end
		    -- 描述
		    if not self.skill_desc_txt then
		    	self.skill_desc_txt = createRichLabel(18, cc.c4b(154,109,77,255), cc.p(0, 1), cc.p(0, 0), nil, nil, 350)
...
function ElfinChooseWindow:openRootWnd( setting )
	...
	if next(self.elfin_data_list) == nil then
		self.elfin_info_panel:setVisible(false)
		self.no_choose_tips:setVisible(true)
		commonShowEmptyIcon(self.elfin_list, true, {text = TI18N("一个精灵都没有噢，快去孵化获取吧~")})
	else
...
function ElfinChooseItem:showItemQualityName( status )
	if self.item_node then
		self.item_node:showItemQualityName(status)
	end
end
