...
function ElfinRousePanel:initView(  )
    ...
    self.elfin_book_txt = createRichLabel(24, cc.c4b(155,67,9,255), cc.p(0.5, 0.5), cc.p(621, 850))
    ...
    self.rouse_panel:getChildByName("rouse_tips"):setString(TI18N("精灵依附在费普斯身上，可借助其古树之力与英雄协同作战"))
    ...
    for i=1,2 do
        local object = {}
        object.cost_bg = self.rouse_cost_panel:getChildByName("cost_bg_" .. i)
...
function ElfinRousePanel:updateRouseInfo(  )
    ...
    self.fight_label:setNum(self.elfin_tree_data.power)
    ...
    for i=1,4 do
        ...
        if not skill_item then
            ...
            skill_item:setPosition(cc.p(250+(i-1)*112, 340))
        ...
        if not elfin_item then
            ...
            local pos_x = 180 + (1-i%2)*400
            local pos_y = 690 - (math.ceil(i/2)-1)*160
    ...
    for i,attr_key in ipairs(ElfinConst.Tree_Attrs) do
        ...
        if not attr_txt then
            attr_txt = createRichLabel(24, cc.c4b(0xc1,0xb7,0xab,0xff), cc.p(0, 0.5), nil, nil, nil, 380)
            self.rouse_panel:addChild(attr_txt)
            self.tree_attr_list[i] = attr_txt
        end
        attr_txt:setVisible(true)
        local pos_x = 92 + (1-i%2)*312
        local pos_y = 188 - (math.ceil(i/2)-1)*46
        attr_txt:setPosition(cc.p(pos_x, pos_y))
        local attr_icon = PathTool.getAttrIconByStr(attr_key)
        attr_txt:setString(_string_format("<img src='%s' scale=1 /><div fontcolor=#643223>    %s</div>", PathTool.getResFrame("common", attr_icon), attr_val))
    end
...
function ElfinRousePanel:showTreeCostInfo( status, index, item_bid, need_num )
    ...
    if status == true then
        object.cost_bg:setVisible(true)
        ...
        if item_cfg then
            ...
            if have_num < need_num then
                object.cost_txt:setTextColor(cc.c3b(255, 93, 93))
            else
                object.cost_txt:setTextColor(cc.c3b(255, 246, 228))
            end
        end
    else
        object.cost_bg:setVisible(false)
...
function ElfinRousePanel:onClickRouseRuleBtn( param, sender, event_type )
    ...
    if rule_cfg then
        TipsManager:getInstance():showCommonTips(rule_cfg.desc, sender:getTouchBeganPosition(), nil, nil, 480)
    end
end
...
function ElfinRousePanel:updateTreeLvupRedStatus(  )
    ...
    addRedPointToNodeByStatus(self.rouse_btn, red_status, 10, 10)
end
