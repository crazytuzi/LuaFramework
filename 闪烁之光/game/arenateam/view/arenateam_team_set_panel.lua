...
-- <br/> 2019年10月8日
-- --------------------------------------------------------------------
ArenateamTeamSetPanel = ArenateamTeamSetPanel or BaseClass(BaseView)
...
function ArenateamTeamSetPanel:__init()
    self.view_tag = ViewMgrTag.DIALOGUE_TAG
    self.win_type = WinType.Mini   
    self.is_full_screen = false
    self.layout_name = "arenateam/arenateam_team_set_panel"
...
function ArenateamTeamSetPanel:open_callback()
    ...
    self.title:setString(TI18N("申请条件"))
    ...
    self.checkbox_label:setString(TI18N("是否需审核"))
    self.checkbox:setSelected(false)

    self:onHideComboboxPanel()
end

function ArenateamTeamSetPanel:register_event()
...
--提交
function ArenateamTeamSetPanel:onClickBtnRight()
    ...
    GlobalEvent:getInstance():Fire(ArenateamEvent.ARENATEAM_TEAM_SET_CALLBACK, limit_lev, limit_power)
    controller:sender27225(limit_lev, limit_power, is_check)
end

--等级
function ArenateamTeamSetPanel:onClickBtnLevel()
    if not self.level_data_list then return end
    self.combobox_panel:setPositionY(310)
    self:updateComboboxList(self.level_data_list, 1)
end

--战力
function ArenateamTeamSetPanel:onClickBtnPower()
    if not self.power_data_list then return end
    self.combobox_panel:setPositionY(226)
    self:updateComboboxList(self.power_data_list, 2)
end

--关闭
function ArenateamTeamSetPanel:onClickBtnClose()
    controller:openArenateamTeamSetPanel(false)
end

function ArenateamTeamSetPanel:openRootWnd(setting)
    local setting = setting or {}
    self.team_limit_power = setting.team_limit_power or 0
    self.team_limit_lev = setting.team_limit_lev or 0
    self.team_is_check = setting.team_is_check or 0
    self:initData()
end

function ArenateamTeamSetPanel:initData()
    self.level_data_list = {}
    self.power_data_list = {}
    self.select_level_index = 1
    self.select_power_index = 1
    ...
    for i,v in ipairs(level_list) do
        ...
        if self.team_limit_lev and self.team_limit_lev == data.lev then
            self.select_level_index = i
        end
    ...
    for i,v in ipairs(power_list) do
        ...
        if self.team_limit_power and self.team_limit_power == data.power then
            self.select_power_index = i
        end
...
function ArenateamTeamSetPanel:updateComboboxList(data_list, _type)
...
function ArenateamTeamSetPanel:createNewCell(width, height)
...
--获取数据数量
function ArenateamTeamSetPanel:numberOfCells()
...
--index :数据的索引
function ArenateamTeamSetPanel:updateCellByIndex(cell, index)
    cell.index = index
...
function ArenateamTeamSetPanel:setCellTouched(cell)
...
--隐藏列表
function ArenateamTeamSetPanel:onHideComboboxPanel()
    -- body
...
function ArenateamTeamSetPanel:close_callback()
    controller:openArenateamTeamSetPanel(false)
end