...
function ArenateamFightTips:open_callback(  )
    ...
    -- end
    --F9D74E
    self.txt_name = createRichLabel(26, cc.c4b(0xf9,0xd7,0x4e,0xff), cc.p(1, 0.5), cc.p(669, 958),nil,nil,1000) 
    ...
    self.txt_my_name = createRichLabel(26, cc.c4b(0xf9,0xd7,0x4e,0xff), cc.p(1, 0.5), cc.p(669, 485),nil,nil,1000) 
    ...
    for i=1,3 do
        ...
        self.role_head_list[i]:setHeadLayerScale(0.8)
        ...
        self.my_head_list[i]:setHeadLayerScale(0.8)
...
function ArenateamFightTips:setData( data )
    ...
    self.data = data
    -- 名称
    local str =  string_format(TI18N("敌方队伍名:<div fontcolor=#a9bad2>%s</div>"), transformNameByServ(data.team_name, data.sid))
...
function ArenateamFightTips:updateMyselfInfo(scdata)
    ...
    local str =  string_format(TI18N("我方队伍名:<div fontcolor=#a9bad2>%s</div>"), self.scdata.team_name)
    ...
    for i=1,3 do
        delayRun(self.container, i*2 / display.DEFAULT_FPS, function (  )
            ...
            if member_data then
                ...
                self.my_hero_item_list[i] = self:updateHeroInfo(230, 352 - (i-1)*100, member_data.team_partner, self.my_hero_item_list[i])
            end
        end)
    end
end
