...
function LoginWindow:open_callback()
	...
	if IS_SY_GAME == true then
		if isVestPackage() == true then	-- android的一些特殊融合渠道包
			res_id = "E50094"
		else
			res_id = "E51008" 
		end
	else
		if IS_IOS_PLATFORM == true then	-- 只有ios才需要这样处理
			if FINAL_CHANNEL == "syios_smzhs" then	-- 神明召唤师
				res_id = "E50130"
			end
		end
	end
	if res_id ~= "" then
		self.spine = createEffectSpine(res_id, cc.p(0,0) , cc.p(0,0),true,PlayerAction.action,nil,cc.TEXTURE2_D_PIXEL_FORMAT_RGB_A8888)
		self.background:addChild(self.spine)
	end
