...
function RoleLoginCell:ctor(data, parent)
	...
	self.Bg = createImage(self, PathTool.getResFrame("login","login2_1013", false, "login2"), 0, self:getContentSize().height/2, cc.p(0, 0.5), true) 
    self:setAnchorPoint(0, 1)
	self.Bg:setContentSize(size)
	self.Bg:setScale9Enabled(true)

	self.select_bg = createImage(self, PathTool.getResFrame("login","login2_1014", false, "login2"), -3, self:getContentSize().height/2, cc.p(0, 0.5), true)
	...
	if self.isEmpty then
		...
		self.txt_name = createLabel(22, cc.c4b(255,248,202,255), nil, 90, 42, role_name, self, 0, cc.p(0, 0))
		self.txt_lev = createLabel(22, cc.c4b(255,248,202,255), nil, 90, 12, StringFormat("Lv.{0}", vo.lev), self, 0, cc.p(0, 0))
	end
end
