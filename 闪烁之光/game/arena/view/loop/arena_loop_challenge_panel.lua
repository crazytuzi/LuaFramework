...
function ArenaLoopChallengePanel:ctor()
    ...
    self.protect_btn:getChildByName("label"):setString("防守阵容")
    ...
    local change_bg = self.root_wnd:getChildByName("Sprite_6")
    change_bg:getChildByName("Text_5"):setString(TI18N("每周宝箱"))
    self.change_num = change_bg:getChildByName("change_num")
...
function ArenaLoopChallengePanel:showRewardItems(data, pos, touch_pos)
    ...
    if self.tips_bg then
        ...
        self.tips_bg:setPosition(self.box_list[touch_pos]:getPositionX(), 108)
    end
...
function ArenaLoopChallengeItem:ctor()
    self.root_wnd = createCSBNote(PathTool.getTargetCSB("arena/arean_loop_challenge_item"))
    self.size = self.root_wnd:getContentSize()
    ...
    self.challenge_btn_label = createRichLabel(24, cc.c4b(0xff,0xff,0xff,0xff), cc.p(0.5, 0.5), cc.p(self.challenge_btn:getContentSize().width*0.5, self.challenge_btn:getContentSize().height*0.5), nil, nil, 150)
...
function ArenaLoopChallengeItem:setData(data)
    ...
    if self.iten_free_num >= 1 then
        str = string_format(TI18N("<div outline=2,#294A15>免费挑战</div>"))
        ...
        if back_num >= 1 then
            str = string_format(TI18N("<div outline=2,#294A15>1 挑战</div>"))
        else
            str = string_format(TI18N("<div fontcolor=#ba3b3b fontsize=24> 1</div><div outline=2,#294A15> 挑战</div>"))
        end
    end
