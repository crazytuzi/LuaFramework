...
function ArenaChampionMatchWindow:open_callback()
    ...
    self.shop_btn = self.tab_container:getChildByName("shop_btn")
...
function ArenaChampionMatchWindow:setChampionStepInfo()
    ...
    if base_info.step == ArenaConst.champion_step.unopened then
        local desc = string_format(TI18N("下次冠军赛 %s"), TimeTool.getMD2(base_info.step_status_time))
        if self.view_type == ArenaConst.champion_type.cross then
            desc = string_format(TI18N("下次周冠军赛 %s"), TimeTool.getMD2(base_info.step_status_time))
        end
