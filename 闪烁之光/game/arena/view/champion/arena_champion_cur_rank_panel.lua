...
function ArenaChampionCurRankPanel:ctor(view_type)
    ...
    self.role_head:setPosition(190, 70)
...
function ArenaChampionCurRankPanel:updateMyInfo(data)
    ...
    if rank <= 3 then
        ...
        if rank == 0 then
            ...
            local res_id = PathTool.getResFrame("common", string.format("common_300%s", rank))
    ...
    if not self.my_score_label then
        self.my_score_label = createRichLabel(18, 175, cc.p(0.5, 0.5), cc.p(590, 34))
        self.my_container:addChild(self.my_score_label)
    end
    local my_score = data.score or 0

    self.my_score_label:setString(string_format("%s<div fontColor=#249003>%s</div>", TI18N("选拔赛分数:"), my_score))
...
function ArenaChampionCurRankItem:setExtendData(node)	
	if not tolua.isnull(node) and self.root_wnd == nil then
        ...
        self.score_label = createRichLabel(18, 175, cc.p(1, 0.5), cc.p(658, 30))
	...
	if data then
		...
		if data.rank <= 3 then
			...
			if data.rank == 0 then
				...
				local res_id = PathTool.getResFrame("common", string.format("common_300%s", data.rank))
				if self.rank_res_id ~= res_id then
					self.rank_res_id = res_id
        ...
        local desc = string_format("%s:<div fontColor=#249003>%s</div>", TI18N("选拔赛分数"), data.score)
