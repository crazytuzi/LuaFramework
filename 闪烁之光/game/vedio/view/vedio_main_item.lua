...
function VedioMainItem:configUI(  )
    ...
    for i=1,3 do
        for j=1,3 do
            local index = (i-1)*3 + j
            self.postion_list[index] = cc.p((width + space_x) * (j-1) + width * 0.5 , (height + space_y) * (3-i) + height * 0.5)
        end
    end
end
...
function VedioMainItem:register_event(  )
    ...
    registerButtonEventListener(self.collect_btn, function (  )
        if self.data then
            ...
            if self.data.is_collect == TRUE then
                ...
                self.collect_btn:loadTexture(PathTool.getResFrame("vedio", "txt_cn_vedio_collect"), LOADTEXT_TYPE_PLIST)
    ...
    registerButtonEventListener(self.like_btn, function (  )
        if self.data then
            ...
            self.like_btn:loadTexture(PathTool.getResFrame("vedio", "txt_cn_vedio_like_2"), LOADTEXT_TYPE_PLIST)
...
function VedioMainItem:setData( data )
    ...
    if data.is_collect == TRUE then
        self.collect_btn:loadTexture(PathTool.getResFrame("vedio", "txt_cn_vedio_un_collect"), LOADTEXT_TYPE_PLIST)
    else
        self.collect_btn:loadTexture(PathTool.getResFrame("vedio", "txt_cn_vedio_collect"), LOADTEXT_TYPE_PLIST)
    end
    ...
    if self.is_myself then
        self.like_btn:setTouchEnabled(false)
        setChildUnEnabled(true, self.like_btn)
        self.like_btn:loadTexture(PathTool.getResFrame("vedio", "txt_cn_vedio_like"), LOADTEXT_TYPE_PLIST)
    ...
    if data.ret == 1 then
        self.result_win:setPosition(cc.p(27, 431))
        self.result_loss:setPosition(cc.p(593, 431))
    elseif data.ret == 2 then
        self.result_win:setPosition(cc.p(593, 431))
        self.result_loss:setPosition(cc.p(27, 431))
    else
    ...
    self.atk_label_left:setString((data.a_power + left_pvp_power))
    -- 等级
    self.level_left:setString(data.a_lev .. TI18N("级"))
    ...
    self.atk_label_right:setString((data.b_power + right_pvp_power))
    -- 等级
    self.level_right:setString(data.b_lev .. TI18N("级"))
...
--创建一个新的item根据位置索引
function VedioMainItem:createHeroExhibitionItemByIndex(index, parent)
    local hero_item = HeroExhibitionItem.new(0.5, true, nil, nil ,true)
