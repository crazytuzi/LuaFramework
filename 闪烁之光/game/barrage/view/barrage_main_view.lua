        ...
        if event_type == ccui.TouchEventType.ended then
            ...
            if self.type ~= nil then
                ...
                if self.open_status == true then
                    self.close_btn:loadTexture(PathTool.getResFrame("barrage", "barrage_2"), LOADTEXT_TYPE_PLIST)
                    
                else
                    self.close_btn:loadTexture(PathTool.getResFrame("barrage", "barrage_1"), LOADTEXT_TYPE_PLIST)
...
function BarrageMainView:onClickInfoBtn( )
    ...
    if self.type ~= nil then
        ...
        self:setScheduleUpdate(self.open_status)
        if self.open_status == true then
            self.info_btn:loadTexture(PathTool.getResFrame("barrage", "barrage_5", false), LOADTEXT_TYPE_PLIST)
            self.info_btn_lable:setString(TI18N("开启弹幕"))
        else
            self.info_btn:loadTexture(PathTool.getResFrame("barrage", "barrage_6", false), LOADTEXT_TYPE_PLIST)
