...
function HomeworldShopItem:setData( data, shop_type )
	...
	if not self.desc_txt then
		self.desc_txt = createRichLabel(20, cc.c4b(134,79,53,255), cc.p(0.5, 0.5), cc.p(self.size.width*0.5, 218))
	...
	if not self.item_icon then
		self.item_icon = createSprite(item_res, self.size.width*0.5, 137, self.container, cc.p(0.5, 0.5), LOADTEXT_TYPE)
	...
	if not self.price_txt then
		self.price_txt = createRichLabel(20, cc.c4b(255,246,228,255), cc.p(0.5, 0.5), cc.p(self.size.width*0.5, 27))
		self.container:addChild(self.price_txt)
	end
    ...
    if pay_config then
    	...
    	local price_str = _string_format(TI18N("<img src='%s' scale=0.35 />  <div fontcolor=fff6e4 outline=2,#694a37>%d</div>"), pay_item_res, data.price)
