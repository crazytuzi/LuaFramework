...
function HomeworldFigureWindow:updateFigureInfo( item_data )
	...
	if item_data.status == HomeworldConst.Figure_State.Unlock then
		...
		self:showFigureModel(true, item_data.look_id)
	...
	elseif item_data.status == HomeworldConst.Figure_State.CanUnlock then
		...
		if item_data.loss[1] then
			...
			if item_config then
				self.btn_unlock_label:setString(_string_format(TI18N("<img src='%s' scale=0.35 /><div outline=2,#764519>%d解锁</div>"), PathTool.getItemRes(item_config.icon), num))
			end
		end
...
function HomeworldFigureWindow:showCucoloris( status )
	if status == true then
		if not self.image_cucoloris then
			self.image_cucoloris = createImage(self.main_container, nil, 175, 322, cc.p(0.5, 0.5), false)
...
-- 显示模型
function HomeworldFigureWindow:showFigureModel( status, effect_id )
	if status and effect_id then
		if not self.cur_effect_id or self.cur_effect_id ~= effect_id then
			...
			self.role_spine = createEffectSpine( effect_id, cc.p(175, 210), cc.p(0.5, 0), true, PlayerAction.idle )
