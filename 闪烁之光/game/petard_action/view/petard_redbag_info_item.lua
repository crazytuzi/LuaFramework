...
PetardRedbagInfoItem = class("PetardRedbagInfoItem", function()
    return ccui.Widget:create()
end)

function PetardRedbagInfoItem:ctor()
	self:configUI()
	self:register_event()
end

function PetardRedbagInfoItem:configUI(  )
	self.size = cc.size(453, 92)
	self:setTouchEnabled(false)
    self:setContentSize(self.size)

	local csbPath = PathTool.getTargetCSB("petard/petard_redbag_info_item")
...
function PetardRedbagInfoItem:register_event(  )
	
end

function PetardRedbagInfoItem:setData( data )
...
function PetardRedbagInfoItem:setIndex( index )
...
function PetardRedbagInfoItem:DeleteMe(  )
