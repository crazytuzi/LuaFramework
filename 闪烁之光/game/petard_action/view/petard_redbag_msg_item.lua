...
PetardRedbagMsgItem = class("PetardRedbagMsgItem", function()
    return ccui.Widget:create()
end)

function PetardRedbagMsgItem:ctor()
	self:configUI()
	self:register_event()
end

function PetardRedbagMsgItem:configUI(  )
	self.size = cc.size(596, 123)
	self:setTouchEnabled(false)
    self:setContentSize(self.size)

	local csbPath = PathTool.getTargetCSB("petard/petard_redbag_msg_item")
...
function PetardRedbagMsgItem:register_event(  )
	
end

function PetardRedbagMsgItem:setData( data )
...
function PetardRedbagMsgItem:DeleteMe(  )
