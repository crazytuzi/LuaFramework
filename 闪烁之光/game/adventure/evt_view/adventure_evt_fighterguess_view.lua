...
function AdventureEvtFighterGuessWindow:open_callback()
    ...
    self.role_bg = self.main_container:getChildByName("role_bg")
    loadSpriteTexture(self.role_bg, PathTool.getPlistImgForDownLoad("bigbg", "bigbg_22"), LOADTEXT_TYPE)
    ...
    self.swap_desc_label = createRichLabel(26, 175, cc.p(0.5,0.5), cc.p(227,835), nil, nil, 340)
...
function AdventureEvtFighterGuessWindow:updateGuessContainer()
    ...
    if not tolua.isnull(self.main_container) and self.left_effect == nil then 
        self.left_effect = createEffectSpine(PathTool.getEffectRes(134), cc.p(210, 640), cc.p(0.5, 0.5), true, PlayerAction.action_4)
        self.main_container:addChild(self.left_effect)
    end

    if not tolua.isnull(self.main_container) and self.right_effect == nil then 
        self.right_effect = createEffectSpine(PathTool.getEffectRes(134), cc.p(510, 640), cc.p(0.5, 0.5), true, PlayerAction.action_4)
...
function AdventureEvtFighterGuessWindow:selecItem(index)
    ...
    self.guide_container:setVisible(true)
    self.guide_container:runAction(cc.Sequence:create(cc.MoveTo:create(0.4,cc.p(123,425)),cc.CallFunc:create(function ()
...
function AdventureEvtFighterGuessWindow:showStr(str)
    ...
    for i, v in ipairs(list) do
        delayRun(self.root_wnd,0.1 * i,function ()
            ...
            self.swap_desc_label:setString(temp_str)
        end)
    end
end
