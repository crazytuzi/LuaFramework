...
function AdventureMainWindow:updateBoxTeskList(data)
    ...
    if self.cur_box_status ~= status_index then
        ...
        if not tolua.isnull(self.btn_box) and self.box_effect == nil then
            self.box_effect = createEffectSpine(PathTool.getEffectRes(602), cc.p(self.btn_box:getContentSize().width * 0.5, self.btn_box:getContentSize().height * 0.5-20), cc.p(0, 0), true, action)
