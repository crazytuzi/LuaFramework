...
function AdventureEvtShopView:open_callback()
    ...
    self.desc:setTextAreaSize(cc.size(330, 112))
    ...
    for i, v in ipairs(list) do
        ...
        if object == nil then
            ...
            if object.buy_config == nil then
                ...
                if buy_config then
                    object.label:setString(string.format("<img src=%s visible=true scale=0.3 />,<div fontColor=#ffffff fontsize=24 outline=1,#6c2b00>%s</div>", PathTool.getItemRes(buy_config.icon), v.pay_val))
...
function AdventureEvtShopView:createShopItem(index, start_x, cell_width, space, id)
    ...
    local item_name = createLabel(24, 175, nil, 50, 25, TI18N("我是物品名字"), node, nil, cc.p(0.5,0.5)) 

    local button = createImage(node, PathTool.getResFrame("common", "common_1017"), 50, -20, cc.p(0.5, 0.5), true, 1, true) 
    button:setContentSize(cc.size(140,61))
    ...
    local button_label = createRichLabel(20, 1, cc.p(0.5, 0.5), cc.p(67,30)) 
