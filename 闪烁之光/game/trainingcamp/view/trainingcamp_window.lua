...
function TrainingcampWindow:open_callback(  )
    ...
    for i=1, 2 do
        ...
        tab_btn.label:setTextColor(cc.c4b(0xc9,0xaf,0x72, 0xff))
        tab_btn.index = i
    ...
    local setting = {
        ...
        start_y = 0,                    -- 第一个单元的Y起点
        space_y = 5,                   -- y方向的间隔
        item_width = 633,               -- 单元的尺寸width
        item_height = 205,              -- 单元的尺寸height
        row = 0,                        -- 行数，作用于水平滚动类型
        col = 1,                         -- 列数，作用于垂直滚动类型
        need_dynamic = true
    }
...
function TrainingcampWindow:changeTabView(index)
    ...
    if self.cur_tab ~= nil then
        self.cur_tab.label:setTextColor(cc.c4b(0xc9,0xaf,0x72, 0xff))
...
function TrainingcampItem:configUI()
    ...
    self:setContentSize(cc.size(633, 205))
    ...
    self.tips_lab = createRichLabel(20, cc.c3b(64, 32, 23), cc.p(1, 0.5), cc.p(615,40),nil,nil,400)
...
function TrainingcampItem:setData(data)
    ...
    if isFinish == false then
        for i,v in ipairs(self.data.unlock) do
            if _model:IsFinishById(v) == false then --- 是否通关
                ...
                local conf = Config.TrainingCampData.data_info[v]
                if conf then
                    str = string.format(TI18N("通关<div fontcolor=#d95014>【%s】</div>解锁"),conf.name)    
                end
                break
            end
        end
    ...
    if data.type == 2 and is_open == false then
        str = TI18N("通关<div fontcolor=#d95014>【全部初阶关卡】</div>解锁")
    end
