...
function HalidomMainPanel:initView(  )
    ...
    for i=1,2 do
        ...
        if tab_btn then
            ...
            title:setTextColor(cc.c3b(255, 244, 228))
            if i == 1 then
    ...
    if self.cur_view_status == HalidomConst.View_Status.Lock then
        ...
        if lock_loss then
            ...
            if not self.lock_loss_item then
                ...
                self.lock_loss_item:setPosition(cc.p(175, 125))
        ...
        self.attr_tips:setString(_string_format(TI18N("%s系英雄属性加成:"), camp_name))
        -- 总属性
        ...
        if self.cur_halidom.vo then
            ...
            for i,attr in ipairs(self.cur_halidom.vo.all_attr or {}) do
                ...
                if attr_name then
                    ...
                    local attr_str = _string_format("<img src='%s' scale=1 /> <div fontcolor=#643223> %s：</div><div fontcolor=#289b14>%s</div>", PathTool.getResFrame("common", icon), attr_name, tostring(attr_val))
    ...
    if lvup_cfg.attr then
        for i,v in ipairs(lvup_cfg.attr) do
            ...
            if attr_name then
                ...
                attr_text:setPosition(cc.p(260 + (i-1) * 240, 344))
                attr_text:setVisible(true)
...
function HalidomMainPanel:updateStepInfo(  )
    ...
    for i=1,show_step_num do
        ...
        if step_object and step_object.step_icon_bg then
            ...
            step_object.step_icon_bg:setPosition(cc.p(start_x + (i-1)*distance_x, 344))
    ...
    if self.cur_halidom.vo.step >= max_step_num then
        ...
        if step_cfg.skill_icon then
            if not self.max_step_skill then
                ...
                self.max_step_skill:setPosition(cc.p(360, 250))
        ...
        if next_step_cfg then
            ...
            if self.cur_halidom.vo.step == 0 then
                ...
                if step_cfg.skill_icon then
                    if not self.cur_step_skill then
                        ...
                        self.cur_step_skill:setPosition(cc.p(250, 250))
                ...
                if next_step_cfg.skill_icon then
                    if not self.next_step_skill then
                        ...
                        self.next_step_skill:setPosition(cc.p(470, 250))
            ...
            if step_cost_cfg then
                ...
                if item_config then
                    ...
                    local have_num = BackpackController:getInstance():getModel():getItemNumByBid(bid)
                    if num > have_num then
                        self.step_cost_label:setTextColor(cc.c3b(255, 93, 93))
                    else
                        self.step_cost_label:setTextColor(cc.c3b(255, 246, 228))
                    end
            ...
            for i,hero_vo in ipairs(self.step_cost_hero_data) do
                ...
                hero_item:setPosition(cc.p(132 + (i-1) * (HeroExhibitionItem.Width * 0.8 + 10), 120))
...
function HalidomMainPanel:createHalidomSkillItem(  )
    ...
    local lv_txt = createLabel(20,cc.c4b(0x64,0x32,0x23,0xff),nil,16,16,"1",lv_bg,1,cc.p(0.5,0.5))
...
function HalidomMainPanel:createStepIcon(  )
    ...
    step_object.step_icon = createSprite(PathTool.getResFrame("halidom","halidom_1001"), 13.5, 15.5, step_object.step_icon_bg, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
    return step_object
end
...
function HalidomMainPanel:changeSelectedTab( index, force )
    ...
    if self.tab_object then
        ...
        self.tab_object.label:setTextColor(cc.c3b(255, 244, 228))
        self.tab_object = nil
    end
    ...
    if self.tab_object then
        ...
        self.tab_object.label:setTextColor(cc.c3b(100, 50, 35))
    end
