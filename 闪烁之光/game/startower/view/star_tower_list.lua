...
function StarTowerList:ctor(parent, pos, size)
    ...
    self.start_x        = 325             -- 第一个单元的起点X
    self.space_x        = 0             -- 横向间隔空间
    self.start_y        = 185             -- 第一个单元的起点Y
    self.space_y        = 0             -- 竖向间隔空间
    self.item_width     = 300        -- 单元的宽度
    self.item_height    = 120       -- 单元的高度
...
function StarTowerList:reloadData(select_index)
    ...
    delayRun(self,1 / display.DEFAULT_FPS,function ()
        if self.bottom_bg == nil then
            ...
            self.bottom_bg = createImage(self.container, res, self.start_x + self.item_width * 0.5 - 19, 15, cc.p(0.5,0), false, 3, false)
        end 
    end)
    ...
    delayRun(self,2 / display.DEFAULT_FPS,function ()
        if self.top_bg == nil then
            ...
            self.top_bg = createImage(self.container, res, self.start_x + self.item_width * 0.5 - 10, y, cc.p(0.5,0), false, 3, false)
        end
    end)
end
...
function StarTowerList:updateBgList()
    if self.scroll_view_bg == nil then
        ...
        local bg_height = 958 * scale --循环图的高度
