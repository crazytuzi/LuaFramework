...
function StarTowerWindow:open_callback()
    ...
    self.top_panel:getChildByName("title"):setString(TI18N("试练塔"))
...
function StarTowerWindow:updateCount()
    ...
    if config and config.buy_count then 
        ...
        local str = string.format(TI18N("<div outline=2,#000000>( %s</div><div fontcolor=#3df424 outline=2,#000000>%s</div><div outline=2,#000000> )</div>"),TI18N("剩余购买次数："), count)
...
function StarTowerWindow:updateTop3Info(rank_list)
    ...
    for i, v in ipairs(rank_list) do
        ...
        if item then
            ...
            item.value:setString((v.tower or 0)..TI18N("层"))
        end
    end
end
