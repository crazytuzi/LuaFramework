...
function StarTowerVideoWindow:showSharePanel(bool)
    ...
    if not self.share_panel then 
        ...
        local btn1 =  createButton(self.share_panel, TI18N("世界频道"), size.width/2, 120, cc.size(160,64), res, 26, Config.ColorData.data_color4[1])
...
function StarTowerVideoWindow:createDesc()
    ...
    self.my_head:setPosition(cc.p(90,62))
    ...
    self.my_name = createLabel(24,Config.ColorData.data_color4[175],nil,165,60,"",self.info_con,2, cc.p(0,0))
    self.my_name:setString(TI18N("我的通关录像"))
    self.my_time = createRichLabel(24,Config.ColorData.data_color4[175],cc.p(0,1),cc.p(165,55),nil,nil,300)
    self.info_con:addChild(self.my_time)
    local str = string.format(TI18N("时间：%s"),TimeTool.GetTimeFormat(self.data.my_time))
    ...
    for i,v in pairs(list) do
        if v and v.type == 1 then 
            local str = string.format(TI18N("通关时间：%s"),TimeTool.GetTimeFormat(v.time))
            ...
            local str = string.format(TI18N("最低战力：%s"),v.power)
