...
function StarTowerAwardWindow:updateAwardList()
    ...
    if not self.list_view then
        local scroll_view_size = cc.size(620,585)
        local setting = {
        ...
        self.list_view = CommonScrollViewLayout.new(self.main_panel, cc.p(13, 48) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
    end
...
function StarTowerAwardItem:setData(data)
    ...
    local str = string_format(TI18N("通过%d层(%d/%d)"),reward_data[item_data.id].tower,controller:getModel():getNowTowerId(),reward_data[item_data.id].tower)
