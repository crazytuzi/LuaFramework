...
function GuildRequestItem:setData(data)
	...
	if data ~= nil then
		...
		self.leader_value:setString(string.format(TI18N("会长：%s"), data.leader_name))
...
function GuildRequestItem:setApplyStatus()
	...
	if data.is_apply == TRUE then
		...
		if self.btn_res_id ~= PathTool.getResFrame("common","common_1027") then
			self.btn_res_id = PathTool.getResFrame("common","common_1027")
		...
		if self.btn_res_id ~= PathTool.getResFrame("common", "common_1098") then
			self.btn_res_id = PathTool.getResFrame("common", "common_1098")
			self.request_btn:loadTexture(self.btn_res_id, LOADTEXT_TYPE_PLIST)
		end 
...
function GuildRequestItem:updateLimitInfo( data )
    ...
    if data.apply_power == 0 then
        ...
        local power = math_floor(data.apply_power/10000)
