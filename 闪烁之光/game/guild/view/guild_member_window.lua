        ...
        if self.my_guild_info ~= nil and self.role_vo ~= nil then
            if self.role_vo.position == GuildConst.post_type.member then
                ...
                if config ~= nil then
                    ...
                    self.desc:setString(string.format("%s%s/%s   %s%s/%s", TI18N("人数："), self.my_guild_info.members_num, self.my_guild_info.members_max, TI18N("副会长："), num, config.num ))
                end 
            end
        end
    end
...
function GuildMemberWindow:updateMemberList(type)
    ...
    if self.show_menber_list ~= nil and next(self.show_menber_list) ~= nil then
        if self.scroll_view == nil then
            ...
            self.scroll_view = CommonScrollViewLayout.new(self.scroll_container, cc.p(0,7), nil, nil, list_size, list_setting) 
        end
        if type == 1 then
