...
set_title:setString(TI18N("验证设置：")) 
    ...
    self.edit_title = createEditBox(container, res, cc.size(403, 50), nil, 22, Config.ColorData.data_color4[151], 22, TI18N("请输入公会名字"), nil, nil, LOADTEXT_TYPE_PLIST)
    ...
    self.edit_title:setPosition(cc.p(185, 554))
    self.edit_title:setMaxLength(12)

    local res = PathTool.getResFrame("common", "common_99998")
    self.declaration_value = createEditBox(container, res, cc.size(403, 146), nil, 22, Config.ColorData.data_color4[151], 22, TI18N("请输入公会宣言内容"), nil, nil, LOADTEXT_TYPE_PLIST)
    self.declaration_value:setAnchorPoint(cc.p(0, 1))
    self.declaration_value:setPlaceholderFontColor(Config.ColorData.data_color4[151])
    self.declaration_value:setFontColor(Config.ColorData.data_color4[175])
    self.declaration_value:setPosition(cc.p(185, 471))
    ...
    self.create_btn_label = createRichLabel(26, 1, cc.p(0.5, 0.5), cc.p(size.width * 0.5, size.height * 0.5), nil, nil, size.width)
    ...
    if role_vo.vip_lev < lev.val then
        ...
        self.create_btn_label:setString(string.format("<img src=%s visible=true scale=0.3 />%s %s", PathTool.getItemRes(item_icon), gold_num, TI18N("创建公会")))
        setChildUnEnabled(true, self.create_btn)
    else
        self.create_btn_label:setString(string.format("<img src=%s visible=true scale=0.3 /><div outline=2,#764519>%s %s</div>", PathTool.getItemRes(item_icon), gold_num, TI18N("创建公会")))
    end
