...
function CrossgroundItem:configUI(  )
    ...
    self.open_desc:setPosition(30,145)
    ...
    self.join_desc = createRichLabel(22, cc.c4b(0xff,0xff,0xff,0xff), cc.p(0, 0.5), cc.p(30,30),nil,nil,1000)
    ...
    if self.container_1 then
        ...
        for i=1,2 do
            ...
            item.open_desc:setPosition(30,145)
            ...
            if i == 2 then
                item.join_desc = createRichLabel(22, cc.c4b(0xff,0xff,0xff,0xff), cc.p(0, 0.5), cc.p(0,30),nil,nil,1000)
            else
                item.join_desc = createRichLabel(22, cc.c4b(0xff,0xff,0xff,0xff), cc.p(0, 0.5), cc.p(30,30),nil,nil,1000)
            end
...
function CrossgroundItem:updateArenaPeakInfo(item, data)
    ...
    if data.id == CrossgroundConst.Ground_Type.peakChampion then
        ...
        if item.match_img == nil then
            ...
            item.match_img = createSprite(res, 343, 172, item.container, cc.p(1, 0.5))
            item.match_label = createLabel(20, cc.c3b(0xff,0xff,0xff), cc.c3b(0x59,0x18,0x18), 300, 173, text, item.container, 2, cc.p(0.5,0.5))
            self:showArenaPeakEffect(true, item.container)
        else
    ...
    elseif data.id == CrossgroundConst.Ground_Type.CrossArena then
        ...
        if status then
            if item.match_img == nil then
                local res = PathTool.getResFrame("crossground","crossground_1005")
                item.match_img = createSprite(res, 94 , 172, item.container, cc.p(0, 0.5))
                item.match_img:setScaleX(-1)
                item.match_label = createLabel(20, cc.c3b(0xff,0xff,0xff), cc.c3b(0x59,0x18,0x18), 44, 173, TI18N("巅峰赛季"), item.container, 2, cc.p(0.5,0.5))
            else
