...
function PlanesafkBoardWindow:onClickComfirmBtn(  )
    ...
    if self.board_id == PlanesafkConst.Recover_Id or self.board_id == PlanesafkConst.Revive_Id then
        if self.board_id == PlanesafkConst.Recover_Id and not model:checkIsHaveHpNotFullHero() then -- 回复泉水时，所有英雄都是满血
        ...
        elseif self.board_id == PlanesafkConst.Revive_Id and not model:checkIsHaveDieHero() then -- 复活时，没有死亡的英雄
