...
function PlanesafkItemUsePanel:open_callback()
    ...
    self.per = per
    self.bg_tips = createRichLabel(22, cc.c3b(0x64,0x32,0x23), cc.p(0, 1), cc.p(105, 555), 5, nil, 540)
    self.main_container:addChild(self.bg_tips)
    self.bg_tips:setString(string_format(TI18N("消耗一颗永恒晶石,恢复下列英雄全部生命和复活所有已经阵亡的英雄,且下一场战斗所有英雄攻击力增加%s%%"), per))

    self.bg_tips2 = createRichLabel(22, cc.c3b(0x24,0x90,0x03), cc.p(0.5, 1), cc.p(339, -10), 5, nil, 600)
...
function PlanesafkItemUsePanel:register_event()
    ...
    self:addGlobalEvent(BackpackEvent.BACKPACK_USE_ITEM_EVENT, function(data)
        ...
        -- message(TI18N("在永恒晶石的帮助下，英雄恢复了生命力"))
    end)
...
function PlanesafkItemUsePanel:onClickBtnRight()
    ...
    if item_config then 
        ...
        local str = string_format(TI18N("是否花费一个 <img src='%s' scale=0.4 /> 恢复所有英雄生命和复活所有已阵亡的英雄,且下一场战斗所有英雄攻击力增加%s%%"),res, per)
