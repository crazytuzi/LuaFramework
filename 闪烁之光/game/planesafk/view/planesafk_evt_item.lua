...
function PlanesafkEvtItem:updateNameTxt( offset_x, offset_y )
    ...
    if self.evt_cfg.type == PlanesafkConst.Evt_Type.Monster or self.evt_cfg.type == PlanesafkConst.Evt_Type.Guard then
        ...
        if self.fight_bg == nil then
            ...
            local res = PathTool.getResFrame("common","common_2015")
            local power_bg = createSprite(res, self.size.width*0.5, 46, self.fight_bg, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
            ...
            self.fight_txt = createLabel(22, cc.c4b(0xff,0xf5,0x8a,0xff), nil, 100, 46, "", self.fight_bg, 2, cc.p(0.5, 0.5))
        ...
        if self.data and self.data.combat_power then
            if self.data.combat_power == 0 then
                ...
                self.fight_txt:setString(self.data.combat_power)
            end
        end
        ...
        if not self.name_txt then
            self.name_txt = createLabel(20, cc.c4b(0xff,0xff,0xff,0xff), cc.c4b(0x50,0x2b,0x17,0xff), 0, 10, "", self.layer_bg, 2, cc.p(0.5, 1))
        else
