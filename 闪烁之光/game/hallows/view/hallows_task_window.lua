			...
			for i,attr in ipairs(hallows_base.attr) do
				...
				if attr_name then
					...
					if attr_text == nil then
						...
						attr_text:setPosition(cc.p(20, 20))
		            ...
		            local attr_str = string_format("<img src='%s' scale=1 /> <div fontcolor=#643223> %s%s：</div><div fontcolor=#643223>%s</div>", PathTool.getResFrame("common", icon), TI18N("全队"), attr_name, tostring(attr_val))
...
function HallowsTaskWindow:updateHallowsSkillInfo()
	...
	if hallows_skill and hallows_skill.skill_bid ~= 0 then
		...
		if not self.skill_name then
			self.skill_name = createLabel(24,cc.c4b(255,238,194,255),nil,135,92,"",self.skill_bg,1,cc.p(0,0))
		end
		self.skill_name:setString(config.name)

		if not self.skill_desc then
			self.skill_desc = createRichLabel(20,cc.c4b(255,238,194,255),cc.p(0,1),cc.p(135, 75),10,nil,550)
			self.skill_bg:addChild(self.skill_desc)
		end
		self.skill_desc:setString(string_format(config.des, config.hallows_atk, 0))
	end
end
...
function HallowsTaskItem:setExtendData(node)
	if not tolua.isnull(node) and self.root_wnd == nil then
		...
		self.task_desc = createRichLabel(22, 175, cc.p(0, 0.5), cc.p(15, 114), nil, nil, 300)
...
function HallowsTaskItem:setData(data)
	...
	if data then
		...
		if config then
			self.task_desc:setString(string_format("%s(%s/%s)", config.desc, data.value or 0, data.target_val or 0))
