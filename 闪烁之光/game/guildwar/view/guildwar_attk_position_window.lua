...
function GuildwarAttkPositionWindow:open_callback(  )
    ...
    self.check_def_label = createRichLabel(24, 181, cc.p(0, 0.5), cc.p(label_pos_x+5, label_pos_y))
...
function GuildwarAttkPositionWindow:setData( data )
	...
	if data.hp > 0 then
		...
		if pos_data then
			...
			self.com_label:setString(string.format("+%d战绩", math.ceil(com_value)))
			local diff_value = pos_data.warscore[3][2] or 0
			self.diff_label:setString(string.format("+%d战绩", math.ceil(diff_value)))
		end
	...
	if not self.enemy_battle_array_panel then
		...
		self.enemy_battle_array_panel:setPosition(cc.p(32, 545))
	end
