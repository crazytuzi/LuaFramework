...
function AdventureBuffTips:LoadLayoutFinish()
	...
	self.background = ccui.ImageView:create(PathTool.getResFrame("common", "common_1092"), LOADTEXT_TYPE_PLIST)
...
function AdventureBuffTips:showTips()
	...
	if self.desc_label == nil then
		self.desc_label = createLabel(20, cc.c4b(0xb4,0xa9,0x9a,0xff), nil, width*0.5, 16, TI18N("该属性仅本轮冒险生效"), self.root_wnd, nil, cc.p(0.5, 0))
	end
	if add_height > 0 then
		...
		if self.holiday_label == nil then
			self.holiday_label = createLabel(24, cc.c4b(0xe0,0xbf,0x99,0xff), nil, 30, height + add_height - 20, TI18N("活动加成:"), self.root_wnd, nil, cc.p(0, 1))
		end
	end
...
function AdventureBuffTips:createBuffList(width, height, add_height)
	local color = cc.c4b(0xe0,0xbf,0x99,0xff)
...
function AdventureBuffTips:recoutTextFieldSize(str_label, width, font_size)
	local label = createRichLabel(font_size, Config.ColorData.data_color4[161], cc.p(0, 1), nil, 6, 0, width)
