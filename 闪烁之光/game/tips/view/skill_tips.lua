...
function SkillTips:updateVo( skill_vo, is_lock, not_show_next, hide_flag, fire_cd, hallows_atk_val )
    ...
    if not hide_flag then
        ...
        if self.skill_vo.cd == 0 then
            ...
            extend_str = string_format("<div fontcolor=#ffeedd>%s</div><div fontcolor=#14ff32>%s</div><div fontcolor=#ffeedd>%s</div>", TI18N("冷却"),self.skill_vo.cd, TI18N("回合") )
        end
        ...
        if fire_cd ~= 0 then
            extend_str = extend_str..string_format("<div fontcolor=#ffeedd>%s</div><div fontcolor=#14ff32>%s</div><div fontcolor=#ffeedd>%s</div>", TI18N("，第"), fire_cd, TI18N("回合释放")) 
        end
    ...
    if buff_desc_str ~= "" then
        if self.buff_desc == nil then
            self.buff_desc = createRichLabel(20,cc.c4b(0xff,0xee,0xdd,0xff),cc.p(0,1),cc.p(22,10),4,nil,370)
    ...
    if self.skill_vo.open_desc ~= "" and not not_show_next and not hide_flag then
        ...
        elseif self.skill_vo.next_id == 0 then -- 已满级
            next_skill_des = string_format(TI18N("<div fontcolor=#ffeedd>技能已满级</div>"))
        else
            next_skill_des = self.skill_vo.skill_desc
        end
        if self.next_skill_txt == nil then
            self.next_skill_txt = createRichLabel(22,cc.c4b(0xff,0xee,0xdd,0xff),cc.p(0,1),cc.p(22,10),4,nil,370)
