...
function BackpackCompTips:showBtn(num)
    ...
    local btn = createButton(self.main_panel,TI18N("合成"),pos,60,cc.size(129,64),PathTool.getResFrame("common","common_1017"),24,Config.ColorData.data_color4[1])
    btn:setName("com_btn")
    btn:setRichText(TI18N('<div fontColor=#ffffff fontsize=24 outline=2,#6C2B00>合成</div>'))
    ...
    if num == 2 then
        local btn1 = createButton(self.main_panel,TI18N("详情"),self.main_panel:getContentSize().width/2 - 120,60,cc.size(129,64),PathTool.getResFrame("common","common_1017"),24,Config.ColorData.data_color4[1])
        btn1:setRichText(TI18N('<div fontColor=#ffffff fontsize=24 outline=2,#6C2B00>详情</div>'))
