...
function SeerpalacePreviewWindow:setData(  )
	...
	if award_config then
		...
		local scale = 0.9
		local desc_height = 40  --概率描述的高度
		local row = 4 -- 5列英雄
		local start_x = 22
		local space_x = 35
		local space_y = 30
		local offset_y = 20 -- 两种星级之间的间隔
		local content_h = 0
		...
		-- 5星
		local desc_txt_1 = createLabel(22,cc.c3b(234,181,80),nil,start_x,max_height-desc_height/2,five_star_config.desc,self.scroll_view,nil,cc.p(0, 0.5))
		for i,v in ipairs(five_star_config.items) do
			delayRun(self.list_panel, i/60, function ()
		        ...
		        local pos_y = max_height - desc_height - (col_index-1)*(BackPackItem.Height*scale+space_y) + 5
		...
		-- 4星
		local start_y = max_height - desc_height - (math.ceil(#five_star_config.items/row))*(BackPackItem.Height*scale+space_y) + space_y - offset_y
		local desc_txt_2 = createLabel(22,cc.c3b(234,181,80),nil,start_x,start_y-desc_height/2,four_star_config.desc,self.scroll_view,nil,cc.p(0, 0.5))
		for i,v in ipairs(four_star_config.items) do
			delayRun(self.list_panel, i/60, function (  )
		        ...
		        local col_index = math.ceil(i/row)
		        local pos_x = start_x + (row_index-1)*(BackPackItem.Width*scale+space_x)
		        local pos_y = start_y - desc_height - (col_index-1)*(BackPackItem.Height*scale+space_y)
		        item_node:setPosition(cc.p(pos_x, pos_y))
		        self.scroll_view:addChild(item_node)
			end)
		end
