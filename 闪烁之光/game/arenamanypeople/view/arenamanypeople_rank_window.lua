...
-- --------------------------------------------------------------------
ArenaManyPeopleRankWindow = ArenaManyPeopleRankWindow or BaseClass(BaseView)
...
function ArenaManyPeopleRankWindow:__init()
    self.win_type = WinType.Big
    self.view_tag = ViewMgrTag.DIALOGUE_TAG
    self.res_list = {
        
    }
    self.layout_name = "arenamanypeople/amp_rank_window"

    self.view_list = {}
end

function ArenaManyPeopleRankWindow:open_callback(  )
    ...
    self.tab_item_type = {
        [1] = ArenaManyPeopleConst.RankType.rank,
        [2] = ArenaManyPeopleConst.RankType.reward,
    }
    self.tab_list = {}
    for i=1,2 do
        ...
        if tab_btn then
            ...
            object.index = self.tab_item_type[i] or ArenaManyPeopleConst.RankType.rank
            self.tab_list[i] = object
        end
    end
end

function ArenaManyPeopleRankWindow:register_event(  )
...
--关闭
function ArenaManyPeopleRankWindow:onClosedBtn()
    controller:openArenaManyPeopleRankWindow(false)
end

--开启的tips倒计时
function ArenaManyPeopleRankWindow:setTimeValFormatString(time)
    if time > 0 then
        local str = string_format("赛季剩余时间：%s", TimeTool.GetTimeFormatDayIIIIIIII(time))
...
-- 切换标签页
function ArenaManyPeopleRankWindow:changeSelectedTab( index )
...
function ArenaManyPeopleRankWindow:createSubPanel(index)
    if not self.view_list then return end

    local panel = self.view_list[index]
    if panel == nil then
        if index == ArenaManyPeopleConst.RankType.rank then --排行榜
            panel = ArenaManyPeopleRankPanel.new() 
        elseif index == ArenaManyPeopleConst.RankType.reward then --排行奖励
            panel = ArenaManyPeopleAwardsPanel.new()
        end
...
function ArenaManyPeopleRankWindow:openRootWnd(setting)
    if not self.tab_item_type then return end
    local setting = setting or {}
    local index = setting.index or ArenaManyPeopleConst.RankType.rank
    self:changeSelectedTab(index)

    local scdata = model:getMyTeamInfo()
    if scdata then
        local time = scdata.end_time - GameNet:getInstance():getTime()
        if time < 0 then
            time = 0
        end
...
function ArenaManyPeopleRankWindow:close_callback()
