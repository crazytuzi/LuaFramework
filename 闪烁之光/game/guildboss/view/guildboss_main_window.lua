...
function GuildBossMainWindow:open_callback()
	...
	main_panel:getChildByName("win_title"):setString(TI18N("公会副本"))
	...
	self.buff_acitive_label = createRichLabel(20,1,cc.p(0,0.5),cc.p(215,22),nil,nil)
	self.buff_container:addChild(self.buff_acitive_label)

	self.comfirm_btn = createButton(main_panel,TI18N("激活"), self.buff_container_pos_x + 400, self.buff_container_pos_y + 22 , cc.size(88, 44), PathTool.getResFrame("common", "common_1027"), 24)
	...
	for i=1,1 do
		...
		object.boss_icon:setPosition(45,330)
	...
	if self.base_info then
		...
		if num > 0 then --如果存在道具
			if self.base_info.buff_lev == 0 then --表示没buff
				str = string.format(TI18N("是否使用<img src=%s visible=true scale=0.5 /><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>(拥有:</div><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>)激活全会伤害提升Buff\n</div>"),PathTool.getItemRes(item_icon),1,num)
    			str_ = str..string.format(TI18N("<div fontColor=#764519>(激活后效果为</div><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>)</div>"),desc)
			else
				str = string.format(TI18N("是否使用<img src=%s visible=true scale=0.5 /><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>(拥有:</div><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>)提升全会伤害提升Buff\n</div>"),PathTool.getItemRes(item_icon),1,num)
    			str_ = str..string.format(TI18N("<div fontColor=#764519>(激活后效果为</div><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>)</div>"),desc)
			end
		else
			if self.base_info.buff_lev == 0 then --表示没buff
				str = string.format(TI18N("是否消耗<img src=%s visible=true scale=0.5 /><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>激活全会伤害提升Buff\n</div>"),PathTool.getItemRes(Config.ItemData.data_assets_label2id.gold),cost,num)
				str_ = str..string.format(TI18N("<div fontColor=#764519>(激活后效果为</div><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>)</div>"),desc)
			else
				str = string.format(TI18N("是否消耗<img src=%s visible=true scale=0.5 /><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>提升全会伤害提升Buff\n</div>"),PathTool.getItemRes(Config.ItemData.data_assets_label2id.gold),cost,num)
				str_ = str..string.format(TI18N("<div fontColor=#764519>(激活后效果为</div><div fontColor=#289b14 fontsize= 24>%s</div><div fontColor=#764519>)</div>"),desc)
			end
		end
		local function fun()
		...
		self.update_rank_event = GlobalEvent:getInstance():Bind(GuildbossEvent.UpdateGuildDunRank,function ()
			...
			if rank_list and next(rank_list or {}) ~= nil then
				for i, v in ipairs(rank_list) do
					...
					if item then
						item:setPosition(-5,200 - (i-1) * item:getContentSize().height)
...
function GuildBossMainWindow:createSingleRankItem(i,data)
	...
	container:setContentSize(cc.size(200,55))
	local sp = createSprite(PathTool.getResFrame("common","common_300"..i),10,55,container)
	sp:setAnchorPoint(cc.p(0,1))
	sp:setScale(0.5)
	container.sp = sp
	local label = createLabel(20,1,nil,62,41,"",container)
	label:setAnchorPoint(cc.p(0,0.5))
	label:setTextColor(cc.c4b(0x89,0xed,0xff,0xff))
	
	local value = createLabel(20,1,nil,62,17,TI18N("[45万伤害]"),container)
	value:setAnchorPoint(cc.p(0,0.5))
	value:setTextColor(cc.c4b(0x2c,0xb2,0xff,0xff))
...
function GuildBossMainWindow:updateMonsterInfo(object)
	...
	if object.boss_id ~= config.boss_id then
		...
		object.spine:setPosition(90,83)
...
function GuildBossMainWindow:updateFillRewardsItems(dps_award, fixed_award, guild_award)
	...
	for i,v in ipairs(dps_award) do
		...
		_x = 100 + (index-1)*(BackPackItem.Width*scale+14) + BackPackItem.Width*scale*0.5
