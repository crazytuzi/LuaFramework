...
function ArenapeakchampionMainWindow:open_callback(  )
    ...
    self.match_value = createRichLabel(22, cc.c4b(0xff,0xd7,0x6b,0xff), cc.p(0, 0.5), cc.p(156,318),nil,nil,1000)
    self.time_value = createRichLabel(22, cc.c4b(0xff,0xf8,0xbf,0xff), cc.p(0, 0.5), cc.p(156,288),nil,nil,1000)
    self.condition_value = createRichLabel(22, cc.c4b(0xff,0xf8,0xbf,0xff), cc.p(0, 0.5), cc.p(156,255),nil,nil,1000)
    ...
    if login_data and login_data.isTry then
        ...
        if config then
            str = string_format(TI18N("先行服跨服竞技场前<div fontcolor=#ffd76b>%s</div>名,进入巅峰冠军赛的预选赛阶段"), config.val)
        end
    else
        local config = Config.ArenaPeakChampionData.data_const.battle_members
        if config then
            str = string_format(TI18N("跨服竞技场前<div fontcolor=#ffd76b>%s</div>名,进入巅峰冠军赛的预选赛阶段"), config.val)
        end
    end
...
function ArenapeakchampionMainWindow:initMatchText( )
    ...
    if self.scdata.step == 0 or self.scdata.step_status == 2 then
        ...
        local time_str = TimeTool.GetTimeFormatDay(time)
        self.time_value:setString(string_format(TI18N("%s-%s(距离赛季<div fontcolor=#ffd76b>结束</div>%s)"), start_str, end_str, time_str))
    end
end
...
function ArenapeakchampionMainWindow:showFlagSpine(bool)
    if bool == true then
        if self.play_effect == nil then
            self.play_effect = createEffectSpine("E27401", cc.p(0,-242), cc.p(0.5, 0.5), true, PlayerAction.action)
