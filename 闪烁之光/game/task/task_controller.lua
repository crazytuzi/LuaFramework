...
function TaskController:gotoTagertFun(progressConfig, extra, open_type)
    ...
    if progressConfig.cli_label == _progress.evt_recruit then --进行X次英雄召唤
        JumpController:getInstance():jumpViewByEvtData({1})
    elseif progressConfig.cli_label == _progress.evt_partner then --获得1个SS英雄2.获得XX个英雄3.集齐冰雪领域（图书馆XX类型收集）的所有英雄4.获得XX个SS英雄
    ...
    elseif progressConfig.cli_label == _progress.evt_partner_enchant_eqm or --精炼装备X次
        progressConfig.cli_label == _progress.evt_partner_levelup or --升级英雄X次
    ...
    elseif progressConfig.cli_label == _progress.evt_partner_stone_all_lv then -- 打开英雄主界面宝石
        
    elseif progressConfig.cli_label == _progress.evt_partner_star then -- 打开英雄主界面
        JumpController:getInstance():jumpViewByEvtData({19})
