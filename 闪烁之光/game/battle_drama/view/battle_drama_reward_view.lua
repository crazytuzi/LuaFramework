...
function BattleDramaRewardWindow:open_callback()
    ...
    container:getChildByName("win_title"):setString(TI18N("通关奖励"))
...
function BattlDramaRewardItem:setData(data)
    ...
    local str = string_format("%s%s关(%s/%s)", TI18N("通关"), drama_dungeon_info.floor, data.cur_dun, drama_dungeon_info.floor)
    ...
    if cur_dun >= drama_dungeon_info.floor then
        if is_received then
            ...
            self.comfirm_btn_label:setString(TI18N("领取"))
        ...
        self.comfirm_btn_label:setString(TI18N("前往"))
