...
function BttleTopDramaView:onlineCreate()
    if self.online_gift_node == nil and self:isCreateOnlineGift() == true then
        ...
        self.online_gift_node:setPosition(81, 162)
...
function BttleTopDramaView:updateCurMapNameAction()
    ...
    if not self.map_name_container then
        ...
        self.map_left_bg:setAnchorPoint(cc.p(0, 0.5))
        self.map_left_bg:setContentSize(cc.size(self.map_left_bg:getContentSize().width, self.map_name_container:getContentSize().height))
        ...
        self.map_name_label = createLabel(28, 207, nil, self.map_name_container:getContentSize().width / 2, self.map_name_container:getContentSize().height - 40, "", self.map_name_container)
        self.map_name_label:setAnchorPoint(cc.p(0.5, 0.5))
        self.before_rich_label = createRichLabel(22, 1, cc.p(0, 0.5), cc.p(15, 85), 5, nil, 1000)
        self.map_name_container:addChild(self.before_rich_label)
        self.desc_cur_label = createRichLabel(22, 1, cc.p(0, 0.5), cc.p(172, 92), 15, nil, 1000)
        self.map_name_container:addChild(self.desc_cur_label)
        self.after_rich_label = createRichLabel(22, 1, cc.p(0, 0.5), cc.p(238, 83), 15, nil, 1000)
        self.map_name_container:addChild(self.after_rich_label)
        self.desc_cur_label_2 = createRichLabel(22, 1, cc.p(0, 0.5), cc.p(278, 85), 13, nil, 1000)
    ...
    if _b_controller:getCtrlBattleScene() and self.drama_data then
        ...
        if _config and _config[self.drama_data.chapter_id] then
            ...
            if _b_controller:getCtrlBattleScene() then
                ...
                local m_bg_res = PathTool.getBattleSceneRes(_string_format("%s/m_bg", map_id), true)
...
function BttleTopDramaView:createSpecialIcon(res, content, load_type)
    ...
    content = content or ""
    local label = createLabel(18, cc.c4b(0xff,0xe7,0xbe,0xff), 2, 38, 2, content, container, 2, cc.p(0.5,0.5))
...
function BttleTopDramaView:updateWorldMapInfo()
    ...
    if self.world_map_node == nil then
        ...
        local name_label = createLabel(20,1,cc.c3b(72,44,27),node_size.width/2,0,"",self.world_map_node,1,cc.p(0.5, 1))
...
function BttleTopDramaView:createDramaButton(  )
    if not _tolua_isnull(self.btn_layout) then
        ...
        if not self.resour_collect then
            ...
            self.resour_collect:setPosition(- 326, 201)
...
function BttleTopDramaView:updateDramaDropInfo(  )
    if self.drama_reward_layout and not _tolua_isnull(self.drama_reward_layout) then
        ...
        for i, v in ipairs(drama_config.per_hook_items) do
            ...
            local config = Config.ItemData.data_get_data(v[1])
            local str = string.format(TI18N("<img src=%s visible=true scale=0.3 /><div fontcolor=#643223>%s/m</div>"), PathTool.getItemRes(config.icon), v[2])
            label:setString(str)
    ...
    if status == true then
        ...
        if self.btn_layout and not _tolua_isnull(self.btn_layout) then
            if _tolua_isnull(self.form_view) then
                ...
                self.form_view:setPosition(0, 208)
