...
function BattleDramaMapWindows:open_callback(  )
    ...
    -- 画网格线
    local space = 50
    local line_num_x = math.floor(display.width/space/2)
    local line_num_y = math.floor(display.height/space/2)
    for i=1,line_num_y do
        delayRun(self.background, 4 * i/display.DEFAULT_FPS,function()
            local drawnode = cc.DrawNode:create()
            local beginPos = cc.p(0, i*space)
            local endPos = cc.p(display.width, i*space)
            local c4f = { r = 54/255, g =50/255, b = 47/255, a = 1 }
            drawnode:drawSegment(beginPos, endPos, 2, c4f)
            drawnode:setAnchorPoint(cc.p(0, 0.5))
            drawnode:setPosition(cc.p(0, i*space))
            self.background:addChild(drawnode)
        end)
    end

    for i=1,line_num_x do
        delayRun(self.background, 4 * i/display.DEFAULT_FPS,function()
            local drawnode = cc.DrawNode:create()
            local beginPos = cc.p(i*space, 0)
            local endPos = cc.p(i*space, display.height)
            local c4f = { r = 54/255, g =50/255, b = 47/255, a = 1 }
            drawnode:drawSegment(beginPos, endPos, 2, c4f)
            drawnode:setAnchorPoint(cc.p(0.5, 0))
            drawnode:setPosition(cc.p(i*space, 0))
            self.background:addChild(drawnode)
        end)
    end
