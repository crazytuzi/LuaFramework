...
function LadderMainWindow:open_callback(  )
	...
	self.refresh_btn:setPosition(cc.p(left_off+75, bottom_off+217*scale_off))
    ...
    tips_label:setPosition(cc.p(right_off-96, bottom_off+265*scale_off))
...
function LadderMainWindow:refreshMyBaseInfo(  )
	...
	if not is_open then
		...
		if not self.join_text then
			self.join_text = createRichLabel(22, 1, cc.p(0.5, 0.5), cc.p(346, 38.5))
