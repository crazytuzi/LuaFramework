...
function VoyageMainWindow:open_callback(  )
	...
	self.refresh_btn_label = createRichLabel(26, 1, cc.p(0.5, 0.5), cc.p(refresh_btn_size.width/2, refresh_btn_size.height/2))
...
function VoyageMainWindow:refreshBtnStatus(  )
	...
	if free_times < free_count then
		self.refresh_btn_label:setString(TI18N("<div fontcolor=#ffffff outline=2,#764519>免费刷新</div>"))
	...
	if count >= self.cost_item_num then
		...
		self.refresh_btn_label:setString(string.format(TI18N("<img src='%s' scale=0.4 /><div fontcolor=#ffffff outline=2,#764519>%d 刷新</div>"), res, self.cost_item_num))
		setChildUnEnabled(false, self.refresh_btn)
		return
	end
	-- 钻石
	...
	if coin_times < coin_count then
		...
		local res = PathTool.getItemRes(item_config.icon)
		self.refresh_btn_label:setString(string.format(TI18N("<img src='%s' scale=0.3 /><div fontcolor=#ffffff outline=2,#764519>%d 刷新</div>"), res, num))
		setChildUnEnabled(false, self.refresh_btn)
		return
	end

	self.refresh_btn_label:setString(TI18N("<div fontcolor=#ffffff>刷新</div>"))
	...
	for i=1,2 do
		...
		if explain_config then
			...
			if status then
				...
				str = str .. TI18N("<div fontcolor=c92606>（未激活）                        </div>") .. string.format("<div href=xxx fontcolor=249003>%s</div>", TI18N("前往激活"))
			end
			if i == 1 then
				tips_str = str
			else
