    ...
    for i=1, 5 do
        if i == 1 then --默认1是等级上限的
            ...
            if attr then
                ...
                self.attr_labe_left_list[i]:setString(left_attr)
                self.attr_labe_right_list[i]:setString(right_attr)
            end
        end
    end
    --技能
    if next_break_config.skill_num > break_config.skill_num then
        ...
        if star_config then
            ...
            if skill_id ~= nil then
                ...
                self.skill_item = SkillItem.new(true,true,nil,0.8, true)
                local size = self.skill_panel:getContentSize()
                self.skill_item:setPosition(cc.p(size.width/2, size.height/2))
...
function HeroBreakPanel:ShowNoneSkillInfo( ... )
    ...
    self.skill_name:setString(TI18N("什么都没有"))
    local size = self.skill_panel:getContentSize()
