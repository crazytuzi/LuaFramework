...
function FormHallowsSelectItem:layoutUI()
    ...
    self.name_label =  createLabel(24, Config.ColorData.data_color4[1], nil, 154, 117, "", self.main_container, 2, cc.p(0,0.5))

    local word_width = 319
    self.dec_label = createRichLabel(22, cc.c3b(0x68,0x45,0x2a), cc.p(0,1), cc.p(152,90), 5, 0, word_width)
...
function FormHallowsSelectItem:setData(data)
    ...
    if skill_config then
        ...
        if StringUtil.SubStringGetTotalIndex(str) > 45 then
            str = StringUtil.SubStringUTF8(str, 1, 45)
            str = str.."..."
        end
    ...
    if data.hallows_vo == nil then
        ...
        self.lock_icon = createImage(self.main_container,res,80,80,cc.p(0.5,0.5),true,0,false)
        if self. non_label == nil then
            self.non_label =  createLabel(26, Config.ColorData.data_color4[206], nil, 514, 72, TI18N("未开启"), self.main_container, 2, cc.p(0,0.5))
        else
        ...
        if data.team_index ~= nil then
            ...
            if self.team_dec == nil then
                self.team_dec = createLabel(20, Config.ColorData.data_color4[206], nil, 611, 24, text, self.main_container, 2, cc.p(1,0.5))
            else
        ...
        if data.mine_text ~= nil then
            --矿脉的 说明被被人占领中
             if self.team_mine_dec == nil then
                self.team_mine_dec = createLabel(20, Config.ColorData.data_color4[206], nil, 611, 24, data.mine_text, self.main_container, 2, cc.p(1,0.5))
            else
