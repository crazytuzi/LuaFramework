        ...
        if self.item_config.sub_type == BackPackConst.item_tab_type.HOLYEQUIPMENT then --神装
            if self.holy_data and self.holy_data.list then --神装方案卸下装备
                ...
                if HeroController:getInstance():getModel():isOpenHolyEquipMentByHerovo(self.partner) == false then
                    local str = TI18N("因版本调整，当前该英雄尚未达到开启神装所需星级条件，卸下后将无法立即穿戴，是否确认卸下？")
                    local function fun()
...
function EquipTips:createattrItem(index, parent)
    ...
    object.attr = createRichLabel(22, cc.c4b(0xc1,0xb7,0xab,0xff), cc.p(0, 0.5), cc.p(4, 14), nil, nil, 380)
    object.node:addChild(object.attr)
    object.act_info = createRichLabel(22, cc.c4b(0xff,0xee,0xdd,0xff), cc.p(1, 0.5), cc.p(360, 14), nil, nil, 380)
