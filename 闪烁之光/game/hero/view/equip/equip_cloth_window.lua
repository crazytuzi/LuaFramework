...
function EquipClothItem:layoutUI()
    ...
    self.equip_item = BackPackItem.new(false,true,0.9)
    self.equip_item:setPosition(cc.p(75,80))
    ...
    --装备名字
    self.equip_name = createLabel(24,Config.ColorData.data_color4[156],nil,150,102,"",self.main_panel,0, cc.p(0,0))
...
function EquipClothItem:setData(data)
    ...
    if self.data.config.sub_type == BackPackConst.item_tab_type.HOLYEQUIPMENT then --神装
        ...
        if self.equip_lev == nil then
            self.equip_lev = createLabel(20,Config.ColorData.data_color4[156],nil,150,69,"",self.main_panel,0, cc.p(0,0))
        end
        --评分
        if self.equip_score == nil then
            self.equip_score = createLabel(24,cc.c4b(0xa9,0x5a,0x1d,0xff),nil,300,70,"",self.main_panel,0, cc.p(0,0))
        end
        ...
        self.equip_score:setString(TI18N("评分：")..score)
        self:updateAttrList()
