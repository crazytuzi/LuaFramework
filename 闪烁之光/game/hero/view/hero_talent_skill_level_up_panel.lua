...
function HeroTalentSkillLevelUpPanel:open_callback()
    ...
    self.skill_up_info.skill_desc = createRichLabel(22, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0, 1), cc.p(0,0),nil,nil,440)
    ...
    self.skill_down_info.skill_desc = createRichLabel(22, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0, 1), cc.p(0,0),nil,nil,440)
...
function HeroTalentSkillLevelUpPanel:showCostInfo(config)
    ...
    local start_x = - item_width * #config.expend/2 + 80
    for i,cost in ipairs(config.expend) do
        ...
        if item_config then
            ...
            self.item_list[i]:setGoodsName(item_config.name,cc.p(174,61),24,cc.c4b(0x64,0x32,0x23,0xff))
        end
    end
end
