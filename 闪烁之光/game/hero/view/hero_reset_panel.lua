...
function HeroResetPanel:configUI( )
    ...
    local desc_label = createRichLabel(20, cc.c4b(0xff,0xff,0xff,0xff), cc.p(0,0.5), cc.p(75, 400), nil, nil, 9000)
...
function HeroResetPanel:checkSender11066()
    ...
    local str = string_format(TI18N("是否消耗<img src='%s' scale=0.3 /><div fontcolor=#%s> %s x %s </div>对英雄进行回退？\n               操作无法撤回，请谨慎选择。"), iconsrc, color, item_config.name, num)
