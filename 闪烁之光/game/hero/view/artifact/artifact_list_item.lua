...
function ArtifactListItem:configUI()
    ...
    local bg = createImage(self, res, self.width/2, self.height/2, cc.p(0.5,0.5), true, 0, true)
    ...
    self.artifact_item:setPosition(cc.p(80,70))

    --self.name = createLabel(24,Config.ColorData.data_color4[156],nil,155,85,"name",self,2, cc.p(0,0))

    self.reset_btn = createImage(self, PathTool.getResFrame("hero","txt_cn_hero_info_37"), 444, 64, cc.p(0.5, 0.5), true)
    self.equip_btn = createImage(self, PathTool.getResFrame("hero","txt_cn_hero_info_38"), 542, 64, cc.p(0.5, 0.5), true)
    ...
    for i,v in ipairs(attr_list) do
        ...
        if attr_name then
            if not self.attr_list[i] then 
                self.attr_list[i] = createRichLabel(22, cc.c4b(0xc1,0xb7,0xab,0xff), cc.p(0, 0.5), cc.p(20, 28), nil, nil, 380)
                self:addChild(self.attr_list[i])
            end
            local label = self.attr_list[i]
            label:setPosition(cc.p(155+(i-1)*170,90))
            ...
            local attr_str = string.format("<img src='%s' scale=1 /> <div fontcolor=#68452a> %s:</div><div fontcolor=#68452a>%s</div>", PathTool.getResFrame("common", icon), attr_name, attr_val)
    ...
    for i,v in pairs(skill_list) do 
        if v and v.extra_k and (v.extra_k == 1 or v.extra_k == 2 or v.extra_k == 8) then
            if not self.skill_list[index] then 
                local label = createRichLabel(22, Config.ColorData.data_color4[156], cc.p(0,0), cc.p(110,35), 0, 0, 500)
                self:addChild(label)
            ...
            self.skill_list[index]:setPosition(cc.p(140+(index-1)*110,35))
