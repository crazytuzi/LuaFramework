...
function HolyequipmentPlanPanel:initCurHeroHolyInfo()
    ...
    if not self.cur_hero_head then
        ...
        self.cur_hero_head:setPosition(cc.p(575, 725))
    ...
    for i,holy_type in ipairs(HeroConst.HolyequipmentPosList) do
        ...
        holy_item:setPosition(cc.p(start_x + (i-1)*(BackPackItem.Width*0.8+15), 725))
    end

    --装备组合套装提示
    if not self.holy_tips_label then
        self.holy_tips_label = createRichLabel(20, cc.c3b(100,50,35), cc.p(0.5, 0.5), cc.p(340, 652), nil, nil, 600)
...
function HolyequipmentPlanItem:layoutUI()
    ...
    self.btn_open_label = createRichLabel(26, cc.c3b(37, 85, 5), cc.p(0.5, 0.5), cc.p(btn_size.width/2, btn_size.height/2))
...
function HolyequipmentPlanItem:setData(data)
    ...
    if data.is_open == 0 then
        ...
        if not self.use_hero_label then
            self.use_hero_label = createRichLabel(24, 274, cc.p(1, 0.5), cc.p(583, 180))
        ...
        if data.partner_id and data.partner_id ~= 0 then
            ...
            if hero_vo then
                self.use_hero_label:setString(_string_format(TI18N("使用者:<div fontcolor=249003 href=xxx>%s</div>"), hero_vo.name))
        ...
        if not self.holy_effect_label then
            self.holy_effect_label = createRichLabel(20, 274, cc.p(0.5, 0.5), cc.p(303, 28), nil, nil, 600)
