...
function HolyequipmentTotalAttrPanel:open_callback()
    ...
    self.container:getChildByName("base_attr"):setString(TI18N("神装总加成"))
...
function HolyequipmentTotalAttrPanel:createAttrItem(x, y, attr_key, attr_val)
    ...
    local attr_str = string.format("<img src='%s' scale=1 /><div fontcolor=#e0bf98> %s：</div>", res, attr_name)
    item.key_label:setString(attr_str)

    item.val_label = createLabel(22, cc.c4b(0xe0,0xbf,0x98,0xff),nil, x + size.width * 0.5 - 10, y, attr_val,self.attr_panel,nil, cc.p(1,0.5))
...
function HolyequipmentTotalAttrPanel:updateSkillID(skill_id, skill_desc)
    ...
    if config then
        ...
        if self.skill_desc == nil then
            self.skill_desc = createRichLabel(20,cc.c4b(0xbd,0xa7,0x8d,0xff),cc.p(0,1),cc.p(144, 96),4,nil,370)
