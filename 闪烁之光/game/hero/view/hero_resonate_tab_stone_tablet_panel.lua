...
function HeroResonateTabStoneTabletPanel:layoutUI()
    ...
    for i=1,3 do
        ...
        self.param_list[i] = {}
        self.param_list[i].key = createRichLabel(22, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0,0.5),cc.p(0, y),nil,nil,1900)
        self.param_list[i].left = createRichLabel(22, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0,0.5),cc.p(118, y),nil,nil,1900)
        self.param_list[i].right = createRichLabel(22, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0,0.5),cc.p(335, y),nil,nil,1900)
    ...
    self.star_tips = createRichLabel(22, cc.c4b(0x95,0x53,0x22,0xff), cc.p(0.5,0.5),cc.p(360, 338),nil,nil,1900)
...
function HeroResonateTabStoneTabletPanel:onClickHeroBtn(pos)
    ...
    setting.tips = TI18N("选择一个五星或以上星级的英雄配置到圣阵中")
...
function HeroResonateTabStoneTabletPanel:updateAttrInfo(attr, is_max_level)
    ...
    for i,v in ipairs(level_up_config.attr) do
        local total_val, add_val = self:countLevelAttr(v[2], rio, total_star)
        ...
        if param then
            ...
            local attr_val_str = string_format("%s<div fontcolor=#249003>(+%s)</div>", total_val, add_val)
            param.left:setString(attr_val_str)
            if next_level_up_config then --有下一级
                local base_val = next_level_up_config.attr[i][2] or 0
                local next_total_val, next_add_val = self:countLevelAttr(base_val, rio, total_star)
                local str = string_format("%s<div fontcolor=#249003>(+%s)</div>", next_total_val, next_add_val)
    ...
    local per = rio/10
    local str = string_format(TI18N("(历史供奉英雄最高总星数为:<div fontcolor=#249003>%s</div>, 星级加成:<div fontcolor=#249003>%s%%</div>)"), total_star, per)
    self.star_tips:setString(str)
end

--计算等级对应的属性
function HeroResonateTabStoneTabletPanel:countLevelAttr(base_val, base_add_val)
