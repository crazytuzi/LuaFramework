...
function HeroLookDrawWindow:register_event()
    ...
    registerButtonEventListener(self.cancel_btn, function()
        if self.share_type == HeroConst.ShareType.eHeroInfoShare then --默认英雄信息分享返回
            self:enterShardStatus(false)
        elseif self.share_type == HeroConst.ShareType.eLibraryInfoShare then --图书馆英雄分享返回
