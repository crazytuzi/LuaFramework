...
function ActionHeroConvertPanel:configUI( )
    ...
    if self.item_load == nil then
        local title_str = "txt_cn_hero_convert_bg"
        local res = PathTool.getPlistImgForDownLoad("bigbg/action", title_str, true)
    ...
    self.tips_label:setString(TI18N("可参与置换英雄:"))
...
function ActionHeroConvertPanel:checkSender16686()
    ...
    local str = string_format(TI18N("是否消耗<img src='%s' scale=0.3 /><div fontcolor=#%s> %s x %s </div>对英雄进行置换?操作无法撤回,请谨慎选择."), iconsrc, color, item_config.name, num)
...
function ActionHeroConvertPanel:onClickHeroItem()
    ...
    if config_list and next(config_list) ~= nil then
        ...
        if self.select_data.left_hero_vo == nil or self.select_data.left_hero_vo.partner_id ~= self.left_hero_vo.partner_id then
            ...
            self.select_data.count = config_data.expend --默认是5星英雄
        end
...
function ActionHeroConvertPanel:updateHeroList(select_camp, is_must_reset)
    ...
    if #self.show_list == 0 then
        commonShowEmptyIcon(self.lay_scrollview, true, {font_size = 22,scale = 0.5, offset_y = 36, text = TI18N("暂无该类型英雄")})
    else
