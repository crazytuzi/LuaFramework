...
function ActionFundTwoPanel:configUI(  )
    ...
    image_bg:loadTexture(PathTool.getPlistImgForDownLoad("bigbg/action","txt_cn_action_fund_bg_2",true), LOADTEXT_TYPE)
    ...
    self.buy_panel:getChildByName("title_time"):setTextColor(cc.c3b(218,182,254))
    self.buy_panel:getChildByName("tips_txt"):setString(TI18N("(每天登陆累计1天，最大30天)"))
    self.buy_panel:getChildByName("tips_txt"):setTextColor(cc.c3b(218,182,254))

    self.total_day_txt = self.buy_panel:getChildByName("total_day_txt")
    self.time_txt = self.buy_panel:getChildByName("txt_time")
    self.time_txt:setTextColor(cc.c3b(218,182,254))
