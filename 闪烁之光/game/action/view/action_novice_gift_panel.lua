...
ActionNoviceGiftItem.Width = 700
ActionNoviceGiftItem.Height = 152
...
function ActionNoviceGiftItem:configUI()
    ...
    self.btn = createButton(self.main_container, TI18N('购买'), 600, 57, cc.size(124, 54), PathTool.getResFrame('common', 'common_1027'), 24)
    self.btn:setRichText(TI18N("<div fontColor=#ffffff fontsize=22 outline=2,#764519>0元</div>"))
    -- self.btn:setPositionY(self.btn:getPositionY()-15)

    self:updateScrollviewList()
    self.limit_label = createRichLabel(22, 175, cc.p(0.5,0.5), cc.p(600,105), nil, nil, nil)
...
function ActionNoviceGiftItem:setData(data)
    ...
    if new_price_list then
        ...
        local price_str = string.format(TI18N("<div fontColor=#ffffff fontsize=24 outline=2,#764519>%d元</div>"), new_price_list.aim_args_val or 0)
    ...
    elseif _type == 3 then --活动周期限购
        if max_num and max_num ~= 0 and cur_num  then 
            str = string.format(TI18N("<div>限购:  (</div><div fontcolor=#249003>%s<div>/%s)"), cur_num,max_num)
        end
    end
    ...
    if data.sort_index == 2 then
        ...
        local price_str = string.format(TI18N("<div fontColor=#ffffff fontsize=24>%d元</div>"), current_price)
        self.btn:setRichText(price_str)
    else
        self.btn:setGrayAndUnClick(false, true)
        local price_str = string.format(TI18N("<div fontColor=#ffffff fontsize=24 outline=2,#764519>%d元</div>"), current_price)
        self.btn:setRichText(price_str)
    end
end
