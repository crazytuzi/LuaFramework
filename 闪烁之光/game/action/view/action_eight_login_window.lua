...
function ActionEightLoginWindow:open_callback()
	...
	self.btn:setTitleText(TI18N("查看详情"))
...
function ActionEightLoginWindow:selectByIndex( index, force )
	...
	local temp_y = 774
	local res = PathTool.getTargetRes("bigbg/action","action_eight_bg2",false,false)
	local res1 = PathTool.getTargetRes("bigbg/action","txt_cn_action_eight_login_title1",false,false)
	if index <= 2 then 
		res = PathTool.getTargetRes("bigbg/action","action_eight_bg2",false,false)
	...
	elseif index == 3 then
		...
		temp_y = 783
		res = PathTool.getTargetRes("bigbg/action","action_eight_bg3",false,false)
	...
	if day_conf then
		self.day_label:setString(day_conf.reward_desc)
	end
...
function EightLoginItem:configUI(  )
	...
	self.title_bg:setLocalZOrder(22)
	...
	self.effect = createEffectSpine("E31336",cc.p(self.main_container:getContentSize().width/2, self.main_container:getContentSize().height/2+8),cc.p(0.5, 0.5),true,"action")
...
function EightLoginItem:setTitleLab(status)
	...
	if data then
		...
		if status == 3 or status == 2 then
			...
			self.title_day:setString(TI18N("第")..self.day_index..TI18N("天"))
		elseif len+2 <= self.day_index then
			doStopAllActions(self.title_day)
			self.title_day:setString((self.day_index - len)..TI18N("天后"))
		else
