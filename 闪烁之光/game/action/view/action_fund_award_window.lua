...
function FuncAwardItem:configUI(  )
    ...
    image_bg:loadTexture(PathTool.getResFrame("actionfund","actionfund_1002"), LOADTEXT_TYPE_PLIST)
    self.txt_time = self.container:getChildByName("txt_time")
    self.txt_time:setTextColor(cc.c3b(100,50,35))
end
...
function FuncAwardItem:setData( data )
    ...
    if not self.item_node then
        self.item_node = BackPackItem.new(false, true, false, 0.9, false, true, false)
