...
function ActionHighValueGiftPanel:configUI()
    ...
    if send_data and next(send_data) ~= nil then
        ...
        local send_img = createImage(self.main_container, PathTool.getItemRes(bid), 229, 616, cc.p(0,0.5), false, 1, false)
        send_img:setScale(0.4)
        self.send_num = CommonNum.new(23, self.main_container, num, 1, cc.p(0, 0))
        self.send_num:setPosition(cc.p(229 + send_img:getContentSize().width * 0.4, 616))
    end
    ...
    if self.holiday_bid == ActionRankCommonType.over_value_gift_1 then
...
function ActionHighValueGiftPanel:setData(data)
    ...
    self.txt_limit_time:setString(TI18N(string_format("限购：%d/%d",current_count,totle_count)))
    --剩余购买次数
	...
	self.txt_price:setString(TI18N(string_format("原价：￥%d", price)))
	self.txt_discount_price:setString(TI18N(string_format("￥%d", discount_price)))
