...
function EmpolyPanel:open_callback()
    ...
    empty_label:setString(TI18N("暂无可雇佣英雄，快去叫好友派遣英雄吧~~~"))
...
function EmpolyPanel:meHelpHero(help_list)
	if next(self.hero_list) ~= nil then
		...
		if help_list and next(help_list) ~= nil and insert_status == false then --当英雄分解掉的时候
			help_list[1].sort = 1 --每次只能派出出一个英雄
