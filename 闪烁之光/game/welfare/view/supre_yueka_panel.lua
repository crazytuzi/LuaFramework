...
function SupreYuekaPanel:configUI()
    ...
    self.text_2 = createRichLabel(24, Config.ColorData.data_color4[1], cc.p(0.5,0.5), cc.p(524,240), nil, nil, 400)
    self.main_container:addChild(self.text_2)
    self.text_3 = createRichLabel(24, Config.ColorData.data_color4[1], cc.p(0.5,0.5), cc.p(524,194), nil, nil, 400)
    self.main_container:addChild(self.text_3)

	local str = string_format(TI18N("<div outline=2,#110800>任意累计充值达到</div><div fontcolor=#ffd862 outline=2,#000000> %d元 </div><div outline=2,#110800>即可激活</div>"),card1_add_count)
    self.text_1:setString(str)

    local item_config = Config.ItemData.data_get_data(card_data.month_card1_return.val[1][1])
	local str = string_format(TI18N("<div outline=2,#110800>充值就</div><div fontsize=34 fontcolor=#ffd862 outline=2,#000000> 送%d天 </div><div outline=2,#110800>月卡</div>"),add_get_day_1)
    self.text_2:setString(str)

    local item_config = Config.ItemData.data_get_data(item_bid_1)
	local str = string_format(TI18N("<div outline=2,#110800>每日可领取</div><div fontsize=34 fontcolor=#ffd862 outline=2,#000000> %d </div><img src=%s visible=true scale=0.30 />"),item_num_1,PathTool.getItemRes(item_config.icon))
