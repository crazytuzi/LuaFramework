...
local select_color = cc.c4b(0xe6,0xc7,0x96,0xff)
local normal_color = cc.c4b(0xff,0xf3,0xe0,0xff)
...
function ShopPanel:createRootWnd()
	...
	local top_path = PathTool.getPlistImgForDownLoad("bigbg/welfare", "welfare_shop_top")
	self.load_image_top = loadSpriteTextureFromCDN(image_top, top_path, ResourcesType.single)
	...
	for i=1,2 do
		...
		tab.normal:setVisible(false)
		tab.name = tab.btn:getChildByName("name")
		tab.name:setTextColor(select_color)
...
function ShopPanel:DeleteMe()
    ...
    if self.load_image_top then
        self.load_image_top:DeleteMe()
        self.load_image_top = nil
    end
...
function ShopItem:configUI()
    ...
    self.price_label = createRichLabel(26, cc.c3b(123,41,0), cc.p(0.5, 0.5), cc.p(ShopItem.width*0.5-9, 30))
