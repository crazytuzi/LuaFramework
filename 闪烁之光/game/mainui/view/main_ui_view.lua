    ...
    for k, v in pairs(FunctionIconVo.type) do
        ...
        if icon_container then
            ...
            if v == FunctionIconVo.type.right_top_1 then -- 右上横向
                _x = display.getRight() - 113
                _y = display.getTop() - self.top_container:getContentSize().height + 10
            elseif v == FunctionIconVo.type.right_top_2 then -- 右上纵向
                _x = display.getRight() - 11
                _y = display.getTop() - self.top_container:getContentSize().height - 28
            ...
            elseif v == FunctionIconVo.type.right_bottom_2 then
                _x = handle_x
                _y = handle_y + 55
            elseif v == FunctionIconVo.type.left_top then -- 左上纵向
                local handle_left_x = display.getLeft() + 9
                _x = handle_left_x
                _y = display.getTop() - self.top_container:getContentSize().height - 92
                --要记录位置
            end
    ...
    if _type == FunctionIconVo.type.right_top_2 then
        if next(self.layout_list[_type]) ~= nil then
            ...
            if not self.right_icon_bg then
                ...
                self.right_icon_bg = createImage(icon_container, PathTool.getResFrame("common", "mainui_1036"), con_size.width*0.5, 190, cc.p(0.5, 1), true, -1, true)
                self.right_icon_bg:setCapInsets(cc.rect(40, 120, 10, 1))
                -- 点击区域
                self.right_arrow_btn = ccui.Layout:create()
                self.right_arrow_btn:setTouchEnabled(true)
                self.right_arrow_btn:setContentSize(cc.size(100, 70))
                self.right_arrow_btn:setAnchorPoint(0.5,0)
                self.right_arrow_btn:setPosition(50, 0)
            ...
            if self.right_icon_unfold == true then
                self.right_icon_bg:setContentSize(cc.size(98, bg_hight))
            end
...
function MainUiView:showIconBgUnfoldAni( icon_bg, _type )
    ...
    self.icon_bg_timer_id = GlobalTimeTicket:getInstance():add(function (  )
        ...
        icon_bg:setContentSize(cc.size(98, height))
...
function MainUiView:adjustTopcontainerPos(  )
    ...
    if self.is_have_top_2 ~= is_have_top_2 then
        ...
        if is_have_top_2 == 1 then
            top_1:setPositionX(display.getRight() - 113)
        else
...
function MainUiView:updateRoleFc()
    ...
    self.fight_label:setString(self.cur_fc)
end
