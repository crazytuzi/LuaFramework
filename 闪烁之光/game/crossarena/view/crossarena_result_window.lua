...
function CrossArenaResultWindow:open_callback(  )
    ...
    self.top_result = createRichLabel(24, 275, cc.p(0, 0.5), cc.p(380, 475))
    ...
    self.bottom_result = createRichLabel(24, 275, cc.p(0, 0.5), cc.p(380, 355))
    self.container:addChild(self.bottom_result)

    self.fight_text = createLabel(24, cc.c4b(0xff,0xee,0xac,0xff), nil, 360, 543, "",self.container, nil, cc.p(0.5,0.5))
...
function CrossArenaResultWindow:setBaseInfo(  )
    ...
    if self.data.get_score == 0 then
        top_str = _string_format("积分:%s", self.data.score)
    else
        if self.data.result == 1 then
            top_str = _string_format("积分:<div fontcolor=#ffcc00>%s    </div><img src=%s scale=1 visible=true /><div fontcolor=#ffcc00>%s</div>", self.data.score, PathTool.getResFrame("common", "common_1086"), self.data.get_score)
        else
            top_str = _string_format("积分:<div fontcolor=#e14737>%s    </div><img src=%s scale=1 visible=true />><div fontcolor=#ff3a3a>%s</div>", self.data.score, PathTool.getResFrame("common", "common_1087"), self.data.get_score)
        end
    end
    ...
    if self.data.lose_score == 0 then
        bottom_str = _string_format("积分:%s", self.data.tar_score)
    else
        if self.data.result == 1 then
            bottom_str = _string_format("积分:<div fontcolor=#ffffff>%s    </div><img src=%s scale=1 visible=true /><div fontcolor=#ff3a3a>%s</div>", self.data.tar_score, PathTool.getResFrame("common", "common_1087"), math.abs(self.data.lose_score))
        else
            bottom_str = _string_format("积分:<div fontcolor=#ffffff>%s    </div><img src=%s scale=1 visible=true />><div fontcolor=#ffcc00>%s</div>", self.data.tar_score, PathTool.getResFrame("common", "common_1086"), math.abs(self.data.lose_score))
        end
...
function CrossArenaResultWindow:setRewardsList( data )
	...
	for i,v in ipairs(data) do
		...
		award_item:setPosition(cc.p(star_x + (i-1)*(distance_x+195), 165))
    ...
    local function call_back_2(  )
    	if is_auto then -- 自动翻转的需要显示购买按钮
            if not self.buy_btn then
                local btn_size = cc.size(161, 54)
                self.buy_btn = createButton(self.container, nil, self.size.width*0.5, -40, btn_size, PathTool.getResFrame("common", "common_1027"))
            ...
            if self.buy_btn_label and self.data.val and self.data.val[1] then
                ...
                local item_cfg = Config.ItemData.data_get_data(bid)
                if item_cfg then
                    self.buy_btn_label:setString(_string_format(TI18N("<img src=%s scale=0.3 /><div fontcolor=#712804>%d 获取</div>"), PathTool.getItemRes(item_cfg.icon), num))
                end
            end
        else
