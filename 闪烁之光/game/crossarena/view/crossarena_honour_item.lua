...
function CrossareanHonourItem:setData( data, index )
	...
	if data then
        ...
        -- 名称
        self.txt_role_name:setString(transformNameByServ(data.name, data.srv_id))
    ...
    if index then
        ...
        if index == 1 then
            loadSpriteTexture(self.image_top, PathTool.getResFrame("crossarena", "crossarena_13"), LOADTEXT_TYPE_PLIST)
        elseif index == 2 then
            loadSpriteTexture(self.image_top, PathTool.getResFrame("crossarena", "crossarena_12"), LOADTEXT_TYPE_PLIST)
            self.image_top:setPositionY(self.image_top:getPositionY()+12)
            self.image_title:setPositionY(self.image_title:getPositionY()+18)
            self.image_stage:setPositionY(self.image_stage:getPositionY()+12)
            self.txt_role_name:setPositionY(self.txt_role_name:getPositionY()+12)
        ...
        elseif index == 3 then
            loadSpriteTexture(self.image_top, PathTool.getResFrame("crossarena", "crossarena_14"), LOADTEXT_TYPE_PLIST)
        end
    end
end
