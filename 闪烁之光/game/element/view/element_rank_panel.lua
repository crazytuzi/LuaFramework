...
ElementRankPanel = class("ElementRankPanel",function()
...
function ElementRankPanel:ctor()
    self.is_init = true
...
function ElementRankPanel:registerEvent()
    if self.update_rank_event == nil then
        self.update_rank_event = GlobalEvent:getInstance():Bind(RankEvent.RankEvent_Get_Rank_data, function(data) 
            if data.type == RankConstant.RankType.element then
...
function ElementRankPanel:setNodeVisible(status)
    self:setVisible(status)
end

function ElementRankPanel:addToParent()
    -- 窗体打开只请求一次，不是标签显示
    if self.is_init == true then
        RankController:send_12900(RankConstant.RankType.element)
        self.is_init = false
    end
end

function ElementRankPanel:updateRankList(data)
...
function ElementRankPanel:DeleteMe()
    if self.update_rank_event ~= nil then
...
------------------------------@ item
ElementRankItem = class("ElementRankItem",function()
    return ccui.Layout:create()
end)

function ElementRankItem:ctor()
...
function ElementRankItem:registerEvent()
...
function ElementRankItem:addCallBack(call_back)
...
function ElementRankItem:setData(data)
    ...
    if data and role_vo then
        ...
        local msg = string.format(TI18N("<div>%s</div><div fontcolor=#249003 fontsize=22>%s</div>"), TI18N("本周通关数:"), data.val1 or 0)
        self.score_info:setString(msg)
    end
end

function ElementRankItem:DeleteMe()
    if self.rank_num ~= nil then
        self.rank_num:DeleteMe()
        self.rank_num = nil
    end
