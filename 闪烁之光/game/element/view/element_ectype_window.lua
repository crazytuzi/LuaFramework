...
function ElementEctypeWindow:open_callback(  )
	...
	self.sp_time = self.main_container:getChildByName("sp_time")
    ...
    self.item_scrollview = CommonScrollViewSingleLayout.new(item_list, cc.p(0,-self.offset_y) , ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function ElementEctypeWindow:setData( data )
    ...
    for i,skill_id in ipairs(self.monster_cfg.skill_list) do
    	...
    	item:setPosition(cc.p(start_x + (i-1)*(space_x+SkillItem.Width*scale), 877+self.offset_y))
