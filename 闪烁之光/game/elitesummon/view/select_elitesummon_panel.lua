...
function SelectEliteSummonPanel:configUI(  )
    ...
    self.btn_reward:getChildByName("Text_9"):setString(TI18N("点击选择up英雄"))
    ...
    --剩余次数
    self.remain_up = createRichLabel(20, cc.c4b(0xff,0xff,0xff,0xff), cc.p(1,0.5), cc.p(659,803))
    ...
    self.award_item:setPosition(cc.p(40, 728))
...
function SelectEliteSummonPanel:updateUpHeroInfo(  )
    ...
    local str = TI18N("<div outline=2,#000000>累计一定招募次数必出up英雄</div>")
    if self.up_hero_id and self.up_hero_id>0 then
        ...
        if self.data then
            str = string.format(TI18N("<div outline=2,#000000>剩余</div><div fontcolor=#5fde46 fontsize=20 outline=2,#000000>%d</div><div outline=2,#000000>次招募内必出UP英雄</div>"),self.data.must_count)
        end        
    else
