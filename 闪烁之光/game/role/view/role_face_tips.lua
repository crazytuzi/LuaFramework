...
function RoleFaceTips:createHeadInfo(  )
	if self.info_con == nil then
	    ...
	    self.time = createRichLabel(22,186,cc.p(0,1),cc.p(30,175))
    	...
    	self.time:setString(TI18N(string.format("使用期限：%s",val)))

    	self.condition = createRichLabel(22,1,cc.p(0,1),cc.p(30,140),0,0,190)
    	self.info_con:addChild(self.condition)
    	self.condition:setString(TI18N(string.format("激活条件：%s",self.data.desc)))
...
function RoleFaceTips:createTitleInfo(  )
    if self.title_info_con == nil then
        ...
        --属性
        local attr_title = createRichLabel(22,175,cc.p(0,1),cc.p(30,154))
        ...
        if self.data.attr and next(self.data.attr)~=nil then
            for k,v in pairs(self.data.attr) do
                local temp = string.format("<div fontcolor=249003>%s</div>",v[2])
                str = str..Config.AttrData.data_key_to_name[v[1]].."+"..temp.." "
            end
        else
            str = str..TI18N("无")
        end
        self.attr_label = createRichLabel(22,175,cc.p(0,1),cc.p(attr_title:getPositionX()+attr_title:getContentSize().width,154),0,0,200)
        self.title_info_con:addChild(self.attr_label)
        self.attr_label:setString(str)

        self.time = createRichLabel(22,186,cc.p(0,0),cc.p(30,162))
        ...
        self.condition:setString(TI18N(string.format("获得条件：%s",self.data.desc)))
