...
-- --------------------------------------------------------------------
RoleHeadPanel = class("RoleHeadPanel", function()
...
function RoleHeadPanel:layoutUI()
    ...
    -- 自定义头像打开设置的入口
    self.photo_btn = createImage(self.main_panel, PathTool.getResFrame("face","face_04"), self.size.width/2,580, cc.p(0.5,0.5), true)
    self.photo_btn:setTouchEnabled(true)
    self.photo_label = createLabel(22, cc.c3b(0x95,0x53,0x22), nil,self.photo_btn:getContentSize().width/2, 0, "", self.photo_btn, nil, cc.p(0.5, 1))
    ...
    local head_label = createLabel(22, cc.c3b(0x95,0x53,0x22), nil, 64, 0, "", self.custom_head, nil, cc.p(0.5, 1))
    ...
    local line = createImage(self.main_panel, PathTool.getResFrame("common","common_1016"), self.size.width/2,485, cc.p(0.5,0.5), true)
    line:setScale9Enabled(true)
    line:setContentSize(cc.size(590,3))

    local tips_label = createLabel(22, cc.c3b(0x95,0x53,0x22), nil,self.size.width/2, -75, "", self.main_panel, nil, cc.p(0.5, 0.5))
    tips_label:setString(TI18N("除了可选择精美的英雄头像，也可设置自定义个性头像哦！"))

    local res = PathTool.getResFrame("common","common_1017")
    self.use_btn = createButton(self.main_panel, TI18N("更 换"), self.size.width/2, -20, cc.size(161,62), res, 26, Config.ColorData.data_color4[1])
...
function RoleHeadPanel:setUserHeadStatus(is_had)
    if not CAN_USE_CAMERA then return end
    if self.check_is_had == is_had then return end
...
function RoleHeadPanel:registerEvents()
    registerButtonEventListener(self.photo_btn, function()
    ...
    if self.custom_head then
        self.custom_head:addCallBack(function(sender, event_type)
            self:setCustomHeadSelected(true)
        end)
    end
end
...
function RoleHeadPanel:setCustomHeadSelected(status)
    if self.is_select_custom == status then return end
...
function RoleHeadPanel:updateList(has_list)
    if self.list_view == nil then
        ...
        self.list_view = CommonScrollViewSingleLayout.new(self.main_panel, cc.p(6, 26), ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, list_setting, cc.p(0, 0)) 
    ...
    local x = 1
    -- 是否使用了自定义头像
    local is_use_custom = self.role_vo.face_file and self.role_vo.face_file ~= "" and self.role_vo.face_update_time and self.role_vo.face_update_time ~= 0
...
function RoleHeadPanel:onCellTouched(cell)
    ...
    local cell_data = self.show_list[index]
    if self.select_bid and self.select_bid == cell_data.bid then return end
    -- 自定义的取消选中
    if self.custom_head then
        self.custom_head:setSelected(false)
        self.is_select_custom = false
...
-- <br/>Create: 2017-xx-xx
-- --------------------------------------------------------------------
FaceHeadItem = class("FaceHeadItem", function()
	return ccui.Widget:create()
end)
...
function FaceHeadItem:setSelected(bool)
    if self.is_select == bool then return end
    self.is_select = bool
    if bool then
         --选择框
...
function FaceHeadItem:showUseIcon(bool)
    ...
    if not self.use_icon then 
        ...
        self.use_icon = createImage(self, res, 85, 30, cc.p(0.5,0.5), true, 1, false)
    end
