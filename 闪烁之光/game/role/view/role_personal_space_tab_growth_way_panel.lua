...
function RolePersonalSpaceTabGrowthWayPanel:initScrollview()
    ...
    self.item_width     = 620             -- 单元的宽度
    self.item_height    = 101              -- 单元的高度
            ...
            if config then
                ...
                for i=1,4 do
                    ...
                    if txt == nil then
                        ...
                        table_insert(params, string_format("<div fontcolor=#e25a00>%s</div>", txt.val))
                    end
                end
...
function RoleGrowtnWayItem:configUI(width, height)
    ...
    self.desc = createRichLabel(22, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0,1), cc.p(85, 48), 6, nil, 488)
...
function RoleGrowtnWayItem:setData(data, item_height)
    ...
    if data.is_last then
        ...
        local res = PathTool.getResFrame("rolegrowthway", "role_growth_way_06")
        ...
        self.desc:setPositionY(48)
        self.time:setString(TimeTool.getYMD3(data.time))
    end
