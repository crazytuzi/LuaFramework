...
function RoleBackgroundItem:setData(data)
    ...
    if self.data.config then
        res_id = PathTool.getPlistImgForDownLoad("rolepersonalspace/rolepersonalspace_head", self.data.config.icon)
    else
        --默认图片
...
function RoleBackgroundItem:showUseIcon(bool)
    ...
    if not self.use_icon then 
        ...
        self.use_icon = createImage(self, res, 557, 23, cc.p(0.5,0.5), true, 1, false)
    end
