...
function RoleFacePanel:layoutUI()
    ...
    self.scroll_view = createScrollView(self.size.width - 30,self.size.height - 12, 15, 6,self.main_panel,ccui.ScrollViewDir.vertical)
...
function RoleFaceItem:configUI( ... )
    ...
    self.icon_bg = createImage(self.back,res,self.width/2,69,cc.p(0.5,0.5),true,0,false)
    ...
    --选择框
    self.select = ccui.ImageView:create(PathTool.getResFrame("common","txt_cn_face_using"), LOADTEXT_TYPE_PLIST)
    ...
    -- 选择光效
    self.select_light = ccui.ImageView:create(PathTool.getResFrame("face","face_05"), LOADTEXT_TYPE_PLIST)
    ...
    self.face_name_bg:setContentSize(cc.size(109,33))
    self.face_name = createLabel(22,Config.ColorData.data_color4[1],nil,self.width/2-2,-23,"",self.back,0, cc.p(0.5,0))
...
function RoleFaceItem:showUseIcon(bool)
    ...
    if not self.use_icon then 
        ...
        self.use_icon = createImage(self, res, 79, 30, cc.p(0.5,0.5), true, 1, false)
    end
