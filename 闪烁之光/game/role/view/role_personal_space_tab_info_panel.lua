...
-- <br/> 2018年11月15日
-- --------------------------------------------------------------------
RolePersonalSpaceTabInfoPanel = class("RolePersonalSpaceTabInfoPanel", function()
...
function RolePersonalSpaceTabInfoPanel:layoutUI()
    ...
    self.combobox_max_size = cc.size(175, 190) --最大size 根据示意图得出来的
    ...
    self.role_head = PlayerHead.new(PlayerHead.type.other, nil, cc.size(117,117), res, mask_res)
    ...
    if self.parent.role_type == RoleConst.role_type.eOther then
        ...
        --自己
        self.role_head:addCallBack(function() controller:openRoleDecorateView(true) end)
...
function RolePersonalSpaceTabInfoPanel:registerEvents()
    ...
    registerButtonEventListener(self.city_btn, function() self:onCityBtn()  end ,false, 1)
    registerButtonEventListener(self.change_name_btn, function() self:onChangNameBtn()  end ,true, 1)
    registerButtonEventListener(self.title_btn, function() self:onTitleBtn()  end ,false, 1)
    --应策划要求.暂时没有点击事件
    ...
    for i,v in ipairs(self.set_btn_list) do
        registerButtonEventListener(v.btn, function() self:onSetBtn(i)  end ,true, 1)        
    end

    if self.All_Feedback_Event_Data == nil then
        self.All_Feedback_Event_Data = GlobalEvent:getInstance():Bind(NoticeEvent.All_Feedback_Event_Data, function()  --
    ...
    if self.parent.role_type == RoleConst.role_type.eMySelf then
        ...
        if self.use_title_event == nil then
            self.use_title_event = GlobalEvent:getInstance():Bind(RoleEvent.UseTitle,function ( base_id )
        ...
        if not self.update_fun_form then 
            self.update_fun_form =GlobalEvent:getInstance():Bind(HeroEvent.Update_Fun_Form,function(data)
        ...
        if not self.role_fans_rank_event then 
            self.role_fans_rank_event =GlobalEvent:getInstance():Bind(RoleEvent.ROLE_FANS_RANK_EVENT,function(data)
                if not data then return end
        ...
        if not self.role_city_event then 
            self.role_city_event =GlobalEvent:getInstance():Bind(RoleEvent.ROLE_CITY_EVENT,function(data)
                if not data then return end
        ...
        if not self.role_follow_event then 
            self.role_follow_event =GlobalEvent:getInstance():Bind(RoleEvent.ROLE_FOLLOW_EVENT,function(data)
                if not data then return end
...
function RolePersonalSpaceTabInfoPanel:updateRoleHead()
    ...
    self.role_head:setHeadRes(self.role_vo.face_id, false, LOADTEXT_TYPE, self.role_vo.face_file, self.role_vo.face_update_time)
end
...
function RolePersonalSpaceTabInfoPanel:onProvinceBtn()
    ...
    if self.province_config_list == nil then
        ...
        table.sort( self.province_config_list, function(a,b) return a.province_id < b.province_id end)
    end
    self.combobox_panel:setPositionX(232)
...
function RolePersonalSpaceTabInfoPanel:onChangNameBtn()
    ...
    if self.parent.role_type == RoleConst.role_type.eMySelf then
        ...
        if self.role_vo ~= nil and self.role_vo.is_first_rename == TRUE then
            ...
            msg = string.format(TI18N("<div fontcolor=#a95f0f>改名需消耗200 <img src=%s scale=0.3 visible=true /></div>"),PathTool.getItemRes(Config.ItemData.data_get_data(3).icon))
        end
        self.set_name_alert = CommonAlert.showInputApply(msg, TI18N("请输入名字(限制6字)"), TI18N("确 定"), 
            ...
            cc.size(270,80), 12, {off_y=-15},true)
...
function RolePersonalSpaceTabInfoPanel:onSetBtn(index)
    ...
    if index == 1 then
        if self.parent.role_type == RoleConst.role_type.eMySelf then
            ...
            --举报
            if self.parent.other_data then
                controller:openRoleReportedPanel(true, self.parent.other_data.rid, self.parent.other_data.srv_id, self.parent.other_data.name)
            end
        end
    elseif index == 2 then
        if self.parent.role_type == RoleConst.role_type.eMySelf then
        ...
        if self.parent.role_type == RoleConst.role_type.eMySelf then
            ...
            if self.parent.other_data then
                if self.parent.other_data.is_friend == 1 then
                    -- 私聊
                    ChatController:getInstance():openChatPanel(ChatConst.Channel.Friend, "friend", self.parent.other_data)
                    self.parent:onClosedBtn()
                else
                    -- 加好友
...
function RolePersonalSpaceTabInfoPanel:setBackgroundRes(backdrop_id)
    if not backdrop_id then return end

    if backdrop_id == 0 then
        --默认图片
        res_id = PathTool.getPlistImgForDownLoad("rolepersonalspace/rolepersonalspace_head", "100000")
    else
        local config = Config.ItemData.data_get_data(backdrop_id)
        if config then
            res_id = PathTool.getPlistImgForDownLoad("rolepersonalspace/rolepersonalspace_head", config.icon)
        else
            res_id = PathTool.getPlistImgForDownLoad("rolepersonalspace/rolepersonalspace_head", "100000")
        end
    end
    if self.record_title_img_res == nil or self.record_title_img_res ~= res_id then
        ...
        self.item_load_title_img_res = loadSpriteTextureFromCDN(self.title_img, res_id, ResourcesType.single, self.item_load_title_img_res) 
    end
end
...
function RolePersonalSpaceTabInfoPanel:setData()
    ...
    if self.parent.role_type == RoleConst.role_type.eMySelf then
        ...
        if self.parent.other_data then
            ...
            self.role_head:setHeadRes(self.parent.other_data.face_id, false, LOADTEXT_TYPE, self.parent.other_data.face_file, self.parent.other_data.face_update_time)
...
function RolePersonalSpaceTabInfoPanel:updateElitelev(elite_lev)
    if not elite_lev then return end
...
function RolePersonalSpaceTabInfoPanel:updateMySelfHeroInfo(data)
    if not self.hero_item_list then return end
    if not data then return end
    local dic_pos_id = {}
...
function RolePersonalSpaceTabInfoPanel:updateOtherHeroInfo()
    ...
    if not self.parent then return end
    if not self.parent.other_data then return end
...
function RolePersonalSpaceTabInfoPanel:updateHonorInfo(use_id)
    ...
    if vo and vo.res_id then 
        ...
        self.item_load_title = createResourcesLoad(res, ResourcesType.single, function()
                if not tolua.isnull(self.honor_img) then
                    loadSpriteTexture(self.honor_img, res, LOADTEXT_TYPE)
                end
        end,self.item_load_title)
    end
end
...
function RolePersonalSpaceTabInfoPanel:createNewCell(width, height)
    ...
    cell.label = createLabel(22, cc.c4b(0xff,0xf9,0xda,0xff), nil, 10, height * 0.5 , "", cell, nil, cc.p(0,0.5))
...
function RolePersonalSpaceTabInfoPanel:onShowTipsPanel(status)
    ...
    if status then 
        ...
        if self.tips_label1 == nil then
            self.tips_label1 = createRichLabel(20, cc.c4b(0xe0,0xbf,0x98,0xff), cc.p(0, 1), cc.p(25, 220), nil, nil, 240)
        ...
        if self.tips_label2 == nil then
            self.tips_label2 = createRichLabel(20, cc.c4b(0xe0,0xbf,0x98,0xff), cc.p(0, 1), cc.p(25, 86), nil, nil, 240)
        ...
        if self.my_rank == nil then
            self.my_rank = createRichLabel(20, cc.c4b(0xff,0x7d,0x27,0xff), cc.p(0, 1), cc.p(25,20), nil, nil, 600)
        ...
        if config then
            ...
            if not self.item_load_tips then
                self.item_load_tips = loadSpriteTextureFromCDN(self.tips_title_img, res, ResourcesType.single, self.item_load_tips)
            end
        ...
        if self.parent.role_type == RoleConst.role_type.eMySelf then
            if self.rank_btn == nil then
                self.rank_btn = createRichLabel(22,cc.c3b(36, 144, 3),  cc.p(0, 1),cc.p(286, 20))
                self.rank_btn:setString(string_format("<div href=xxx>%s</div>", TI18N("前往排行榜")))
                self.tips_panel:addChild(self.rank_btn)

                self.rank_btn:addTouchLinkListener(function(type, value, sender, pos)
