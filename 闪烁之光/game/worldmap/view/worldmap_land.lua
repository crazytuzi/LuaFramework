...
function WorldMapLand:createRootWnd()
    ...
    -- if self.land_id >= self.config.bid then     -- 当前挑战到的大陆版本比该板块大，就标识已经开启了，这个时候不需要创建静态云的,而直接创建当前大陆板块的名字
        self.name_container = createSprite(PathTool.getResFrame("worldmap", "worldmap_1007"), self.config.name_x, self.config.name_y, self, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 10)
        self.name_label = createLabel(20, 1, 9, 105, 40, self.config.name, self.name_container, nil, cc.p(0.5, 0.5))
