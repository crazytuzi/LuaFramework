...
function ChatWindow:initView()
    ...
    self.bg1:setCapInsets(cc.rect(62, 52, 2, 2))
    ...
    res = PathTool.getResFrame("common", "common_1002")
    ...
    self.bg2:setContentSize(cc.size(size.width-25,size.height-92))
    ...
    --频道背景
    local buttomBg = createScale9Sprite(PathTool.getResFrame("mainui","mainui_chat_bottom_bg"),0,0,LOADTEXT_TYPE_PLIST,self.bg2)
    ...
    --提示文字
    self.notice_label = createLabel(26,cc.c4b(0x7a,0x60,0x41,0xff),nil,buttomBg:getContentSize().width/2,buttomBg:getContentSize().height/2,TI18N("该频道下无法发言"),buttomBg)
...
function ChatWindow:createTabBtnList(  )
    ...
    widget:setContentSize(cc.size(670,90))
    widget:setAnchorPoint(cc.p(0,1))
    widget:setPosition(cc.p(15, self.bg1:getContentSize().height-20))
    ...
    self.tab_scrollview = CommonScrollViewLayout.new(widget, cc.p(0, 10) , ScrollViewDir.horizontal, ScrollViewStartPos.top, scroll_view_size, setting)
