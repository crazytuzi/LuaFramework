...
function ChatMsg:showHead(data)
   ...
   local roleVo = RoleController:getInstance():getRoleVo()
    ...
    if tolua.isnull(self.head_icon)  then
        ...
        --名字
        self.player_name = createLabel(20,Config.ColorData.data_color4[271],nil,109,94,"",self.head_root)
        ...
        --聊天内容 54f1ff
        self.content = createRichLabel(22, Config.ColorData.data_color4[175], cc.p(0, 0.5), cc.p(20, 64), 5, -1, self.view_width)
    ...
    self.chat_bg:setCapInsets(cc.rect(20,30,1,1))
...
function ChatMsg:setChannel(channel)
    ...
    if self.channel ~= channel then
        ...
        -- self.channel_label:setString(ChatController:getInstance():getString("name_"..ChatConst.ChannelRes[channel]))
        -- self.channel_label:setPosition(cc.p(self.channel_bg:getContentSize().width/2,self.channel_bg:getContentSize().height/2+2))
    end
end
