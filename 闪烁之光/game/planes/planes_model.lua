...
function PlanesModel:config()
    ...
    self.planes_hero_data = {} -- 位面所有英雄数据（包括雇佣的英雄）
    ...
    if dun_cfg then
        ...
        if dun_cfg.id_limit and dun_cfg.id_limit[1] then
            ...
            if not is_pass then
                ...
                if dun_info then
                    return TI18N(string.format("通关[%s]开启", dun_info.name or ""))
                end
            end
        end
        -- 等级
        ...
        if role_vo.lev < dun_cfg.lev_limit then
            return TI18N(string.format("等级达%d级", dun_cfg.lev_limit))
        end
        -- 战力
        if role_vo.power < dun_cfg.power_limit then
            return TI18N(string.format("战力达%d", dun_cfg.power_limit))
        end
    end
end
