...
Act_TotalCharge = class("Act_TotalCharge",Act_Template)
Act_TotalCharge.__index = Act_Template
Act_TotalCharge.szImage_ChargeStatus = "Image_ChargeStatus"
Act_TotalCharge.szImage_RewardStatus = "Image_RewardStatus"

--初始化
function Act_TotalCharge:init(panel, tbItemList)
	if not panel then
		return 
	end
    ...
    --设置累计充值元宝数
    self:setTotalCharge(g_Hero:getTotalChargeYuanBao())
end

function Act_TotalCharge:updateStatus()
...
function Act_TotalCharge:setTotalCharge(yuanbao)
    local strTip = string.format(_T("您已累计充值%d元宝"), yuanbao)
    self.mTotalChargeTtf:setText(strTip)
end

function Act_TotalCharge:onClickGainReward(pSender, eventType)
...
function Act_TotalCharge:setButtonState(buttonItem, state)
