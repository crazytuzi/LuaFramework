...
Act_TotalCostJR = class("Act_TotalCostJR",Act_Template)
Act_TotalCostJR.__index = Act_Template

--初始化
function Act_TotalCostJR:init(panel, tbItemList)
	if not panel then
		return 
	end
    ...
    --设置累计消耗元宝数
    self:setTotalCost(g_Hero:getTotalCostYuanBaoJR())
end

function Act_TotalCostJR:updateStatus()
...
function Act_TotalCostJR:setTotalCost(yuanbao)
