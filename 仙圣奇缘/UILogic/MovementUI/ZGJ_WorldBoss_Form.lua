...
Game_WorldBoss2 = class("Game_WorldBoss2")
Game_WorldBoss2.__index = Game_WorldBoss2

function Game_WorldBoss2:setCDBar(nTime)
	self.Image_DeadCD = self.rootWidget:getChildByName("Image_DeadCD")
	g_CDBar:register(self.Image_DeadCD, VipType.VipBuyOpType_SceneBossDeadCD, nTime, function ()
			g_RoleSystem:clearCD()
		end)
end

function Game_WorldBoss2:setBlock(bFlag)
	self.Image_Block:setVisible(bFlag)
	g_RoleSystem:setBlocked(bFlag)
end

function Game_WorldBoss2:setBossInfo(tbMonster)
	...
	local Button_FengYinGuide = tolua.cast(self.rootWidget:getChildByName("Button_FengYinGuide"), "Button")
	g_RegisterGuideTipButton(Button_FengYinGuide, nil, nil, CSV_ActivityWorldBoss.HunPoRewardID)
end

function Game_WorldBoss2:setRankInfo(tb)
...
function Game_WorldBoss2:onClickRank(widget, eventType)
	if ccs.TouchEventType.ended == eventType then
		g_WBSystem:showRankList()
	end
end

function Game_WorldBoss2:onClickFloor(widget, eventType)
	if ccs.TouchEventType.ended == eventType then
		local pos = widget:getTouchEndPos()
		pos.x = pos.x - self.Image_Floor:getPositionX()
		g_RoleSystem:myMove(pos)
		
		if g_PlayerGuide:checkCurrentGuideSequenceNode("ActionEventStart", "Game_WorldBoss2") then
			g_PlayerGuide:showCurrentGuideSequenceNode()
		end
	end
end

function Game_WorldBoss2:onCheckBox_AutoFight(widget, eventType)
...
function Game_WorldBoss2:onCheckBox_HidePlayer(widget, eventType)
	if ccs.CheckBoxEventType.selected == eventType then
...
function Game_WorldBoss2:onCheckBox_AutoReborn(widget, eventType)
	if ccs.CheckBoxEventType.selected == eventType then
...
function Game_WorldBoss2:onClickCheckBox_Image(widget, eventType)
	if ccs.TouchEventType.ended == eventType then
...
function Game_WorldBoss2:refreshGuWu()
	local nYueli = g_WBSystem:getGuWu(macro_pb.SCENE_BOSS_TYPE, COST.YueLi)
	if 0 == nYueli then
		self.Label_AttackBuff_YueLi:setText("")
	else
		self.Label_AttackBuff_YueLi:setText(_T("攻击 +").." "..(10*nYueli).."%")
	end
	local nYuanBao = g_WBSystem:getGuWu(macro_pb.SCENE_BOSS_TYPE, COST.YuanBao)
	if 0 == nYuanBao then
...
function Game_WorldBoss2:initGuWu()
	...
	local function onClickYueLi(widget, nTag)
		local cost = g_WBSystem:getGuWuCost(macro_pb.SCENE_BOSS_TYPE, COST.YueLi)
		if cost > g_Hero:getKnowledge() then
			g_ShowSysTips({text = _T("阅历不足")})
			return
		end
		g_WBSystem:requestGuWu(macro_pb.SCENE_BOSS_TYPE, false)
	end
	...
	local function onClickYuanBao(widget, nTag)
		local cost = g_WBSystem:getGuWuCost(macro_pb.SCENE_BOSS_TYPE, COST.YuanBao)
		if not g_CheckYuanBaoConfirm(cost, _T("元宝鼓舞需要消耗")..cost.._T("元宝, 您的元宝不足是否前往充值")) then
			return
		end
		g_WBSystem:requestGuWu(macro_pb.SCENE_BOSS_TYPE, true)
	end
...
function Game_WorldBoss2:initWnd()
	-- 请求离开战斗以免卡住
	...
	local csv = g_DataMgr:getCsvConfig_FirstKeyData("ActivityBase", macro_pb.ActivityType_SceneBoss)
	self.nOpenTime = csv.StarTimeH*60*60 + csv.StarTimeM*60
	self.nEndTime = csv.EndTimeH*60*60 + csv.EndTimeM*60
	self:setRefreshTime()
	self.nTimerID_Game_WorldBoss2_1 = g_Timer:pushLoopTimer(1, handler(self, self.setRefreshTime))
	...
	Image_Texture1:loadTexture(getBackgroundJpgImg("WorldBoss"))
	local Image_Texture2 = tolua.cast(Image_Background:getChildByName("Image_Texture2"), "ImageView")
	Image_Texture2:loadTexture(getBackgroundJpgImg("WorldBoss"))
...
function Game_WorldBoss2:checkData()
	if not g_WBSystem:isInit(macro_pb.SCENE_BOSS_TYPE) then
		-- 进入之前先向服务端请求离开场景以免卡住
		g_RoleSystem:requestExit()
		g_RoleSystem:requestEnter(macro_pb.SceneType_WorldBoss)
		g_WBSystem:requestEnterWB(macro_pb.SCENE_BOSS_TYPE)
		return false
	end
	return true
end

function Game_WorldBoss2:openWnd()
	
end

function Game_WorldBoss2:closeWnd()
	g_RoleSystem:requestExit()
	...
	g_Timer:destroyTimerByID(self.nTimerID_Game_WorldBoss2_1)
	self.nTimerID_Game_WorldBoss2_1 = nil
...
function Game_WorldBoss2:setRefreshTime()
--	if not g_WndMgr:getWnd("Game_WorldBoss2") then return true end

    local tbServerTime = os.date("*t", g_GetServerTime())
    local serverTimeSeconds = tbServerTime.hour*3600 + tbServerTime.min*60 + tbServerTime.sec
    if serverTimeSeconds < self.nOpenTime then  
        --boss还没刷新
        self.Image_RefreshCountPNL:setVisible(true)
		self.Image_BossInfoPNL:setVisible(false)
		self.Label_RefreshRemainTime:setText(TimeTableToStr(SecondsToTable(self.nOpenTime - serverTimeSeconds),":"))
    elseif serverTimeSeconds > self.nOpenTime and serverTimeSeconds < self.nEndTime then
        --boss已经刷新
        self.Image_RefreshCountPNL:setVisible(false)
		self.Image_BossInfoPNL:setVisible(true)
    elseif serverTimeSeconds > self.nEndTime then
        --boss战斗时间已过
        self.Image_RefreshCountPNL:setVisible(true)
		self.Image_BossInfoPNL:setVisible(false)
		self.Label_RefreshRemainTime:setText(TimeTableToStr(SecondsToTable(self.nOpenTime + 24*3600 - serverTimeSeconds),":"))
    end
end

function Game_WorldBoss2:clearCD()
	if g_WBSystem:getAutoReborn() and self.Image_DeadCD:isVisible() then
		self.bOpen = true
		local nType = VipType.VipBuyOpType_SceneBossDeadCD
...
function Game_WorldBoss2:ModifyWnd_viet_VIET()
