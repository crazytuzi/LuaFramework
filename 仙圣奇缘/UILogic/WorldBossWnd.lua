...
-- 应  用:  本例子是用类对象的方式实现
---------------------------------------------------------------------------------------
Game_WorldBoss1 = class("Game_WorldBoss1")
Game_WorldBoss1.__index = Game_WorldBoss1


function Game_WorldBoss1:checkTime(tbAct)
...
function Game_WorldBoss1:initWnd()
    ...
    local function onClickBuChallenge(pSender, nTag)
		...
		if self.bChallengeColdDown then
			...
			g_ClientMsgTips:showConfirm(str, function() 
				local function worlBossRemoveCd()
					...
					--删除定时器
					if self.nTimerID_Game_WorldBoss1_1 then
						g_Timer:destroyTimerByID(self.nTimerID_Game_WorldBoss1_1)
						self.nTimerID_Game_WorldBoss1_1 = nil
					end
				...
				g_VIPBase:responseFunc(worlBossRemoveCd)
		...
		local function setFlag()
		   ...
		   self.nTimerID_Game_WorldBoss1_2 = g_Timer:pushTimer(2, resetFlag)
		end
		if self:checkTime(tbAct) then
			g_MsgNetWorkWarning:showWarningText()
			g_WBSystem:requestAttack(macro_pb.WORLD_BOSS_TYPE)
    ...
    self.nMaxCount = CSV_ActivityBase.MaxTimes + g_VIPBase:getAddTableByNum(VipType.VipBuyOpType_WorldBossTimes)
	...
	Image_Background:loadTexture(getSceneImg("ZhanXianTai"))
end

function Game_WorldBoss1:getChallengeCount()
    return g_Hero:getDailyNoticeByType(macro_pb.Activity_AMBoss)
end

function Game_WorldBoss1:refreshChallengeCount()
...
function Game_WorldBoss1:refreshBossAttendTime(boss_left_hp)
		...
		if eventType == ccs.TouchEventType.ended then 	
			...
			g_ClientMsgTips:showConfirm(str, function() 
				local function sellBossNumFunc(times)
					...
					gTalkingData:onPurchase(TDPurchase_Type.TDP_WORLD_BOSS_NUM,1,gold)	
...
function Game_WorldBoss1:refreshWorldBossWnd(tbMsg)
	...
	local Button_ShenXianShiLianGuide = tolua.cast(self.rootWidget:getChildByName("Button_ShenXianShiLianGuide"), "Button")
	g_RegisterGuideTipButton(Button_ShenXianShiLianGuide, nil, nil, CSV_ActivityWorldBoss.HunPoRewardID)
end

function Game_WorldBoss1:setPanelCard()
    ...
    if self.nWaitTime then
        ...
        local function showWaitTime(ft, bOver)
			if not g_WndMgr:getWnd("Game_WorldBoss1") then return true end
        ...
        self.nTimerID_Game_WorldBoss1_1 = g_Timer:pushLimtCountTimer(self.nWaitTime, 1, showWaitTime)
        setCoolTime(Label_CoolTime, self.nWaitTime)
    else
        Panel_Card:setVisible(true)
    end
...
function Game_WorldBoss1:registerClickShowRank()
...
function Game_WorldBoss1:requestBossInfo(tag)
	self.bossTag = tag or self.bossTag
	g_MsgMgr:requestBossInfo()
end

function Game_WorldBoss1:closeWnd()
    self.tbData = nil
    self.bossTag = nil
    self.nWaitTime = nil
...
function Game_WorldBoss1:checkData()
   	if not g_WBSystem:isInit(macro_pb.WORLD_BOSS_TYPE) then
		g_WBSystem:requestEnterWB(macro_pb.WORLD_BOSS_TYPE)
		return false
	end
	return true
end
...
--显示主界面的伙伴详细介绍界面
function Game_WorldBoss1:openWnd(tbData)
    self.bSendMsgFlag = nil
	if(tbData)then
...
function Game_WorldBoss1:updateChallengeColdDown()
	 self.nLastChallenge =  g_GetServerTime() 
end

function Game_WorldBoss1:refreshGuWu()
	local nYueli = g_WBSystem:getGuWu(macro_pb.WORLD_BOSS_TYPE, COST.YueLi)
	if 0 == nYueli then
		self.Label_AttackBuff_YueLi:setText("")
	else
		self.Label_AttackBuff_YueLi:setText(_T("攻击 +").." "..(10*nYueli).."%")
	end
	local nYuanBao = g_WBSystem:getGuWu(macro_pb.WORLD_BOSS_TYPE, COST.YuanBao)
	if 0 == nYuanBao then
...
--鼓舞 add by zgj 
function Game_WorldBoss1:initGuWu()
	...
	local function onClickYueLi(pSender, nTag)
		local cost = g_WBSystem:getGuWuCost(macro_pb.WORLD_BOSS_TYPE, COST.YueLi)
		if cost > g_Hero:getKnowledge() then
			g_ShowSysTips({text = _T("阅历不足")})
			return 
		end
		g_WBSystem:requestGuWu(macro_pb.WORLD_BOSS_TYPE, false)
	end
	...
	local function onClickYuanBao(pSender, nTag)
		local cost = g_WBSystem:getGuWuCost(macro_pb.WORLD_BOSS_TYPE, COST.YuanBao)
		if not g_CheckYuanBaoConfirm(cost, _T("元宝鼓舞需要消耗")..cost.._T("元宝, 您的元宝不足是否前往充值")) then
			return false
		end
		g_WBSystem:requestGuWu(macro_pb.WORLD_BOSS_TYPE, true)
	end
