ZoneFriendList = ZoneFriendList or BaseClass()

function ZoneFriendList:__init(panel, mainWin)
    ...
    self.dataList = self.Mgr.friendzone_List
    ...
    self:OnTabChange(1)
    self.transform:Find("nonImage").gameObject:SetActive(self.pagenum == 0)
end

function ZoneFriendList:__delete()
...
function ZoneFriendList:Show()
    self.gameObject:SetActive(true)
end

function ZoneFriendList:Hide()
    self.gameObject:SetActive(false)
end

function ZoneFriendList:OnTabChange(index)
    self:InitPage(index)
...
function ZoneFriendList:InitPage(index)
...
function ZoneFriendList:SetItem(item, data)
    item:Find("Head"):GetComponent(Image).sprite = self.mainWin:GetHead(data.classes, data.sex)
    ...
    if data.photo ~= 0 then
        ...
        if BaseUtils.is_null(photo) then
            zoneManager:RequirePhotoQueue(data.id, data.platform, data.zone_id, function(photo) if not BaseUtils.isnull(item) then self:toPhoto(photo, item:Find("Head"):GetComponent(Image), data.id, data.platform, data.zone_id, data.photo) end end)
        else
...
function ZoneFriendList:toPhoto(photo, img, id, platform, zone_id, photoid)
