...
-- hosr
-- ----------------------------------
GuideGuardWakeup = GuideGuardWakeup or BaseClass(BasePanel)

function GuideGuardWakeup:__init(panel)
    self.panel = panel
...
function GuideGuardWakeup:__delete()
    self:GuideEnd()
...
function GuideGuardWakeup:InitPanel()
...
function GuideGuardWakeup:First()
    self.gameObject:SetActive(false)

    self.lastGameObject = self.panel.tab_btn2.gameObject
    ...
    self.effect.transform.localScale = Vector3.one
    self.effect.transform.localPosition = Vector3(30, 0, -400)
    Utils.ChangeLayersRecursively(self.effect.transform, "UI")
    self.lastGameObject:GetComponent(Button).onClick:AddListener(self.next)
    self.effect:SetActive(true)
    TipsManager.Instance:ShowGuide({gameObject = self.lastGameObject, data = TI18N("点击进入魂石功能"), forward = TipsEumn.Forward.Left})
end

function GuideGuardWakeup:Senond()
    print(self.panel)
    self.panel:GuideLeft3()
end

function GuideGuardWakeup:Next()
...
function GuideGuardWakeup:GuideEnd()
