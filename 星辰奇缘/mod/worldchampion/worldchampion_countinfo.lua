...
WorldChampionCountInfo = WorldChampionCountInfo or BaseClass(BasePanel)

function WorldChampionCountInfo:__init(model)
    self.model = model
    self.Mgr = self.model.mgr
    self.name = "WorldChampionCountInfo"
    ...
    self.complain_flag = false          --//是否下方显示举报按钮(默认不显示)

    self.complainstatuslistener = function(unique_id) self:ComplainStatus(unique_id) end
end

function WorldChampionCountInfo:__delete()
    WorldChampionManager.Instance.complainSuccess:RemoveListener(self.complainstatuslistener)

    BaseUtils.ReleaseImage(self.complaonBtn.transform:GetComponent(Image))
...
function WorldChampionCountInfo:InitPanel()
    ...
    self.complaonBtn = self.transform:Find("Main/Panel4/ComplainBtn"):GetComponent(Button)
    self.complaonBtn.gameObject:SetActive(true)
    self.complaonBtn.onClick:AddListener(function() self:ShowComplainBtnStatus() end)
    ...
    for i=1,10 do
        ...
        self.ItemList[i].ComplainObj = trans:Find("ComplainButton")
        self.ItemList[i].ComplainButton = trans:Find("ComplainButton/Image"):GetComponent(Button)
        self.ItemList[i].ComplainTxt = trans:Find("ComplainButton/I18NText")
    ...
    for i = 1 , 7 do
        self.honorextpoint_list[i] = self.IconDescCon:Find(string.format( "Bg/Item%d",i))
    ...
    local setting = {title = TI18N("分享武道会成绩"), type = 1}
    ...
    self.transform:Find("Main/playbackButton"):GetComponent(Button).onClick:AddListener(function()
        ...
        self.Mgr:Require16426(self.data.r_id, self.data.r_platform, self.data.r_zone_id)
    end)
    if self.data.type ~= nil and self.data.type == 1 then
    ...
    WorldChampionManager.Instance.complainSuccess:RemoveListener(self.complainstatuslistener)
    WorldChampionManager.Instance.complainSuccess:AddListener(self.complainstatuslistener)
end


function WorldChampionCountInfo:LoadItem()
    ...
    for i,v in ipairs(self.data.mates) do
        ...
        if v.rid == roleData.id and roleData.platform == v.platform and  roleData.zone_id == v.zone_id then
            ...
            item.ComplainObj.gameObject:SetActive(false)
        else
            item.GoodButton.gameObject:SetActive(not self.complain_flag)
            item.FriendButton.gameObject:SetActive(not self.complain_flag)
            item.ComplainObj.gameObject:SetActive(self.complain_flag)
            ...
            item.ComplainButton.onClick:RemoveAllListeners()
            item.ComplainButton.onClick:AddListener(function()
                self:ClickComplainBtn(i, v, true)
            end)
        end
    ...
    for i,v in ipairs(self.data.rival) do
        ...
        item.ComplainButton.onClick:RemoveAllListeners()
        item.ComplainButton.onClick:AddListener(function()
                self:ClickComplainBtn(i+5, v, false)
            end)
...
function WorldChampionCountInfo:PraseData()
    self.roleInfoList = {}
    for i,v in ipairs(self.data.mates) do
...
function WorldChampionCountInfo:GoodSuccess(rid, platform, zone_id)
    for i,v in ipairs(self.data.mates) do
...
function WorldChampionCountInfo:ClickItem(index , data)
    -- 改为守护不响应此操作
...
function WorldChampionCountInfo:SelectLeft()
    local roleData = RoleManager.Instance.RoleData
...
function WorldChampionCountInfo:SelectRight()
    local roleData = RoleManager.Instance.RoleData
    local i = self.currRoleindex + 1
    if i >= 11 then
        i = 1
...
function WorldChampionCountInfo:FormatNum(num)
    if num < 100000 then
        return tostring(num)
    else
...
function WorldChampionCountInfo:WatchIconByIndex(index)
...
function WorldChampionCountInfo:GiveEffect(go)
...
function WorldChampionCountInfo:ShowComplainBtnStatus()
    self.complain_flag = not self.complain_flag
    if self.complain_flag then 
        self.complaonBtn.transform:GetComponent(Image).sprite = self.assetWrapper:GetSprite(AssetConfig.no1inworld_textures,"I18NRedComplainBtn")
    else
        self.complaonBtn.transform:GetComponent(Image).sprite = self.assetWrapper:GetSprite(AssetConfig.no1inworld_textures,"I18NYellowComplainBtn")
    end
    self:LoadItem()
end

function WorldChampionCountInfo:ClickComplainBtn(index, data , side)
    if data == nil or data.zone_id == nil or data.zone_id == 0 then
        return
    end
    ...
    if side then   --//己方
        ...
        dat.content = TI18N(string.format("请确认是否举报<color='#5FC1E6'>%s</color><color='#E3FF34'>[消极比赛]</color>，若经<color='#F00000'>系统核实</color>将予以<color='#F00000'>禁赛</color>处罚",data.name))
        dat.sureLabel = TI18N("确认")
        dat.sureCallback = function() self.Mgr:Require16425(data.rid, data.platform, data.zone_id, self.data.r_id) end
        NoticeManager.Instance:ConfirmTips(dat)
    else           --//敌方
        local report_data = {name = data.name, roleid = data.rid, platform = data.platform, zone_id = data.zone_id}
        ReportManager.Instance.model:OpenWindow({report_data})
    end
end

function WorldChampionCountInfo:ComplainStatus(unique_id)
    for i,v in ipairs(self.data.mates) do
        local uniqueID = string.format( "%s,%s,%s",v.platform,v.zone_id,v.rid)
        if unique_id == uniqueID then 
            local item = self.ItemList[i]
            item.ComplainButton.gameObject:SetActive(false)
            item.ComplainTxt.gameObject:SetActive(true)
        end
    end
