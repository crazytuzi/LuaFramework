...
-- UI - 峡谷之巅地图
-- ----------------------------------------------------------
CanYonMapWindow = CanYonMapWindow or BaseClass(BaseWindow)

local GameObject = UnityEngine.GameObject
local Vector2 = UnityEngine.Vector2

function CanYonMapWindow:__init(model)
    self.model = model
    self.name = "CanYonMapWindow"
    ...
    self.resList = {
        {file = AssetConfig.canyon_map_window, type = AssetType.Main}
...
function CanYonMapWindow:__delete()
...
function CanYonMapWindow:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(AssetConfig.canyon_map_window))
    self.gameObject.name = "CanYonMapWindow"
...
function CanYonMapWindow:OnClickClose()
    WindowManager.Instance:CloseWindow(self)
end

function CanYonMapWindow:OnShow()
    if BaseUtils.is_null(self.gameObject) then
        return
    end
    if CanYonManager.Instance.cannonCd ~= 0 and CanYonManager.Instance.cannonCd <= Time.time then
...
function CanYonMapWindow:OnHide()
    if self.ticker ~= nil then
...
function CanYonMapWindow:opencurmap()
    self.is_worldmap = false
...
function CanYonMapWindow:openworldmap()
    self.is_worldmap = true
    self.curMapPanel:SetActive(false)
end

function CanYonMapWindow:shownpcpanel()
...
function CanYonMapWindow:FixedUpdate()
...
function CanYonMapWindow:updatedata()
    self.transform:FindChild("Main").gameObject:SetActive(true)
    self:updatecurmap()
end


function CanYonMapWindow:onmapbuttonclick(mapId)
...
function CanYonMapWindow:updatecurmap()
    local transform = self.transform
...
function CanYonMapWindow:curMapClick()
...
function CanYonMapWindow:drawloadpoint()
    self:cleanloadpoint()
...
function CanYonMapWindow:addLoadPointList(startpoint, endpoint)
    local distance = 0.4
    local list = {}
...
function CanYonMapWindow:cleanloadpoint()
...
function CanYonMapWindow:backtothecity()
    self:OnClickClose()
    SceneManager.Instance.sceneElementsModel:Self_Change_Top_Effect(2)
    SceneManager.Instance.sceneElementsModel:Self_Change_Top_Effect(4)
    SceneManager.Instance.sceneElementsModel:Self_Change_Top_Effect(6)
...
function CanYonMapWindow:backtohome()
    self:OnClickClose()
    SceneManager.Instance.sceneElementsModel:Self_Change_Top_Effect(2)
    SceneManager.Instance.sceneElementsModel:Self_Change_Top_Effect(4)
    SceneManager.Instance.sceneElementsModel:Self_Change_Top_Effect(6)
...
function CanYonMapWindow:InitColorSet()
    for i=1, 3 do
...
function CanYonMapWindow:UpdateMapInfo()
    if CanYonManager.Instance.towerData ~= nil then
        for k,v in pairs(CanYonManager.Instance.towerData) do
            if self.crystalList[v.unit_id] ~= nil then
                local rate = math.ceil(v.duration/DataCanyonSummit.data_tower_info[v.unit_id].duration*100)
