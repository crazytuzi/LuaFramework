-- 公会英雄战，战斗状态、观战面板
-- @author zgs
GuildLeagueTeamLookWindow = GuildLeagueTeamLookWindow or BaseClass(BaseWindow)

function GuildLeagueTeamLookWindow:__init(model)
    self.model = model
    self.Mgr = GuildLeagueManager.Instance
    self.name = "GuildLeagueTeamLookWindow"
    ...
    -- end)
    self.guildleagueLeaderUpdateFun = function ()
        self:updateWindow()
    end
    self.Mgr.LeagueTeamChange:AddListener(self.guildleagueLeaderUpdateFun)

    self.lastLookTeamIndex = 1

end

function GuildLeagueTeamLookWindow:OnInitCompleted()
    GuildLeagueManager.Instance:Require17615()
end

function GuildLeagueTeamLookWindow:__delete()
    self.itemDic = nil
    self.Mgr.LeagueTeamChange:RemoveListener(self.guildleagueLeaderUpdateFun)
...
function GuildLeagueTeamLookWindow:InitPanel()
...
--已结束
function GuildLeagueTeamLookWindow:onClickChangeBtn(index)
...
--未开始
function GuildLeagueTeamLookWindow:onClickSetBtn(index)
    self.lastLookTeamIndex = index
    local strDesc = TI18N("太阳代表队")
    if self.lastLookTeamIndex == 1 then
        strDesc = TI18N("月亮代表队")
...
--查看
function GuildLeagueTeamLookWindow:onClickLookBtn(index)
    -- 发协议，查看安排的队伍信息
    self.lastLookTeamIndex = index
    local strDesc = TI18N("太阳代表队")
    if self.lastLookTeamIndex == 1 then
...
--显示队伍信息
function GuildLeagueTeamLookWindow:updateTeamInfo(data)
...
function GuildLeagueTeamLookWindow:GetPosDataByPos(pos)
...
function GuildLeagueTeamLookWindow:updateWindow()
...
function GuildLeagueTeamLookWindow:OnClickClose()
    self.Mgr:CloseTeamLookWindow()
    -- GuildFightEliteManager.Instance.model:ShowEliteLookWindow(false)
end

function GuildLeagueTeamLookWindow:GetFightDataInfo(data)
