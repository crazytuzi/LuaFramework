...
GuildLeagueLeaderSetPanel = GuildLeagueLeaderSetPanel or BaseClass(BasePanel)

function GuildLeagueLeaderSetPanel:__init(model)
    self.model = model
    self.Mgr = self.model.Mgr
    self.name = "GuildLeagueLeaderSetPanel"
    ...
    self.sort_lastlogin_ud = function (a,b)
        if a.Status > b.Status then
            return true
        elseif a.Status == b.Status and a.Lev > b.Lev then
            return true
        elseif a.Status == b.Status and a.Lev == b.Lev and a.Rid < b.Rid then
            return true
        else
            return false
        end
...
function GuildLeagueLeaderSetPanel:OnInitCompleted()
    self.pos = self.openArgs[1]
    -- GuildfightManager.Instance:send15507()
    self:UpdatePanel()
end

function GuildLeagueLeaderSetPanel:__delete()
    self.Mgr.LeagueTeamChange:RemoveListener(self.updateFunc)
...
function GuildLeagueLeaderSetPanel:InitPanel()
    ...
    for i=1,15 do
        ...
        local item = GuildFightEliteMemberItem.New(obj,self, 1)
    ...
    self:DoClickPanel()
    self.Mgr.LeagueTeamChange:AddListener(self.updateFunc)
end

function GuildLeagueLeaderSetPanel:onClickSureBtn()
    local tableTemp = {}
    ...
    if #tableTemp > 0 then
        BaseUtils.dump(tableTemp[1],"GuildLeagueLeaderSetPanel:onClickSureBtn() pos = "..self.pos)
        GuildLeagueManager.Instance:Require17616(tableTemp[1].rid,tableTemp[1].r_platform,tableTemp[1].r_zone_id,self.pos)
    else
...
----------------------------------------------------列表排序逻辑
--点击成员列表标题，进行列表排序
function GuildLeagueLeaderSetPanel:on_mem_title_up_callback(index)
    --公会成员列表排序逻辑
...
function GuildLeagueLeaderSetPanel:OnClickClose()
    self:Hiden()
end

function GuildLeagueLeaderSetPanel:DoClickPanel()
...
function GuildLeagueLeaderSetPanel:UpdatePanel()
...
function GuildLeagueLeaderSetPanel:UpdateList(dataList)
    -- print(#dataList)
