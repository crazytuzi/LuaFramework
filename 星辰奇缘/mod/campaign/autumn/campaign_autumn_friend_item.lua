CampaignAutumnFriendItem = CampaignAutumnFriendItem or BaseClass()

function CampaignAutumnFriendItem:__init(Parent,gameObject,asset)
    self.Parent = Parent
...
function CampaignAutumnFriendItem:InitPanel()
    self.transform = self.gameObject.transform
    self.Msg = self.transform:Find("Msg"):GetComponent(Text)
    self.MsgExt = MsgItemExt.New(self.Msg,241,20,22)
    self.headBg = self.transform:Find("Headbg")
end

function CampaignAutumnFriendItem:SetData(data)
    local myHour = ""
    local mymini = ""
    self.data = data
    ...
    self.headData = {id = data.tar_role_id,platform = data.platform,zone_id = data.zone_id,classes = data.classes,sex = data.sex},{isSmall = true}
    self:SetHeadImg()
    self:Layout()
end

function CampaignAutumnFriendItem:__delete()
...
function CampaignAutumnFriendItem:Layout()
...
function CampaignAutumnFriendItem:SetHeadImg()
