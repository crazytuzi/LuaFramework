...
-- hosr
-- ------------------------------
QuestAnswerPanel = QuestAnswerPanel or BaseClass(BasePanel)

function QuestAnswerPanel:__init(model)
    self.model = model

    self.path = "prefabs/ui/exam/couplequestionwindow.unity3d"
...
function QuestAnswerPanel:__delete()
...
function QuestAnswerPanel:Close()
    QuestMarryManager.Instance:ClosePanel()
end

function QuestAnswerPanel:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(self.path))
    self.gameObject.name = "QuestAnswerPanel"
...
function QuestAnswerPanel:OnShow()
    self:UpdateQuestion()
end

function QuestAnswerPanel:OnHide()
end

function QuestAnswerPanel:ClickBtn(index)
    if not self.isTimeout then
        ...
        if answer ~= nil then 
            local id = answer.id
            QuestMarryManager.Instance:Send15401(self.openArgs.sid, answer.id)
        end
        self:TimeOut()
    else
        print("已提交过超时作答")
...
function QuestAnswerPanel:TimeOut()
    if not self.isSelect and self.openArgs.status == 0 then
        -- print("==================== 发送答题超时 ================")
        self:Over()
        self.isTimeout = true
        QuestMarryManager.Instance:Send15401(self.openArgs.sid, 0)
    end
end

function QuestAnswerPanel:Over()
    if self.loopId ~= nil then
        LuaTimer.Delete(self.loopId)
        self.loopId = nil
    end
...
function QuestAnswerPanel:Reset()
    if self.loopId ~= nil then
        LuaTimer.Delete(self.loopId)
        self.loopId = nil
    end
...
function QuestAnswerPanel:UpdateQuestion()
    print("更新题目")
    ...
    self.questionData = DataQuestion.data_couple_question[self.openArgs.qid]
    local questionStr = ""
...
function QuestAnswerPanel:UpdateStatus(status, option)
    -- if status == 0 then
    --     -- 超时或未答
    --     self.isTimeout = true
...
function QuestAnswerPanel:RandomAnswer(tempTab)
...
function QuestAnswerPanel:ShowAnswer()
...
function QuestAnswerPanel:SetTimeout()
...
function QuestAnswerPanel:Loop()
...
-- 展示双方结果
function QuestAnswerPanel:ShowOption(dat)
    self:Over()
...
function QuestAnswerPanel:ShowWhoAnswered(index, isSelf)
...
function QuestAnswerPanel:AllGrey()
    for i,btn in ipairs(self.buttons) do
...
-- 开始进入下一题倒计时
function QuestAnswerPanel:BeginNextTimeout()
    if self.nextId ~= 0 then
        LuaTimer.Delete(self.nextId)
        self.nextId = 0
    end
...
function QuestAnswerPanel:NextLoop()
...
function QuestAnswerPanel:Next()
    ...
    if self.openArgs.sid == 3 then
        ...
        QuestMarryManager.Instance:OpenAward(QuestionEumn.Type.Couple)
    else
