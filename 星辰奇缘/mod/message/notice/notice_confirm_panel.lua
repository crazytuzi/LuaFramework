...
-- hosr
-- ------------------------
NoticeConfirmPanel = NoticeConfirmPanel or BaseClass(BasePanel)

function NoticeConfirmPanel:__init(model)
    self.model = model
...
function NoticeConfirmPanel:__delete()
end

function NoticeConfirmPanel:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(self.path))
    self.transform = self.gameObject.transform
    self.gameObject.name = "NoticeConfirmPanel"
...
function NoticeConfirmPanel:Reset()
...
function NoticeConfirmPanel:SetData(confirmData)
    self:Reset()
    ...
    self.closeCall = self.confirmData.closeCallback
...
function NoticeConfirmPanel:Layout(type)
...
function NoticeConfirmPanel:OnTickForContent()
...
function NoticeConfirmPanel:OnTick()
...
function NoticeConfirmPanel:UpdateLabel()
...
function NoticeConfirmPanel:TimeOut()
    LuaTimer.Delete(self.timeId)
    self.timeId = 0
    self.sureCount = -1
    self.cancelCount = -1
...
function NoticeConfirmPanel:ClickSure()
...
function NoticeConfirmPanel:ClickCancel()
...
function NoticeConfirmPanel:ClickToggle(bool)
    if self.toggleCall ~= nil then
        self.toggleCall(bool)
    end
end

function NoticeConfirmPanel:Clear()
    self:Hiden()
    self:Reset()
...
function NoticeConfirmPanel:SetPanelButtonEnabled(bool)
