...
-- hosr
-- ------------------------
NoticeUpdatePanel = NoticeUpdatePanel or BaseClass(BasePanel)

function NoticeUpdatePanel:__init(model)
    self.model = model
...
function NoticeUpdatePanel:__delete()
end

function NoticeUpdatePanel:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(self.path))
    self.transform = self.gameObject.transform
    self.gameObject.name = "NoticeUpdatePanel"
...
function NoticeUpdatePanel:Reset()
...
function NoticeUpdatePanel:SetData(confirmData)
    BaseUtils.dump(confirmData)
    self:Reset()
    ...
    if self.confirmData.blueSure then
        ...
        self.sureLabel.text = string.format("<color='%s'>%s</color>", ColorHelper.ButtonLabelColor.Green, self.sureLabel.text)
    end

    self.cancelTxt:SetData(self.confirmData.cancelLabel)
    if self.confirmData.greenCancel then
        self.cancelLabel.text = string.format("<color='%s'>%s</color>", ColorHelper.ButtonLabelColor.Green, self.cancelLabel.text)
    else
    ...
    if self.confirmData.blueSure then
        ...
        self.sureImg.sprite = PreloadManager.Instance.assetWrapper:GetSprite(AssetConfig.base_textures, "DefaultButton2")
    end

    if self.confirmData.greenCancel then
        self.cancelImg.sprite = PreloadManager.Instance.assetWrapper:GetSprite(AssetConfig.base_textures, "DefaultButton2")
    else
    ...
    self.transform:Find("Panel"):GetComponent(Button).enabled = false
    self.transform:SetAsLastSibling()
end

-- 根据类型来处理窗口样式，按钮个数，窗口大小
function NoticeUpdatePanel:Layout(type)
...
function NoticeUpdatePanel:OnTickForContent()
...
function NoticeUpdatePanel:OnTick()
...
function NoticeUpdatePanel:UpdateLabel()
...
function NoticeUpdatePanel:TimeOut()
    LuaTimer.Delete(self.timeId)
    self.timeId = 0
    self.sureCount = -1
    self.cancelCount = -1
...
function NoticeUpdatePanel:ClickSure()
...
function NoticeUpdatePanel:ClickCancel()
...
function NoticeUpdatePanel:ClickToggle(bool)
    if self.toggleCall ~= nil then
        self.toggleCall(bool)
    end
end

function NoticeUpdatePanel:Clear()
    self:Hiden()
    self:Reset()
