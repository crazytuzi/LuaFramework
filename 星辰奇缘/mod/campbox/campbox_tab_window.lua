-- @author zyh
-- @date 2017年6月19日
CampBoxTabWindow = CampBoxTabWindow or BaseClass(BaseWindow)

function CampBoxTabWindow:__init(model)

    self.model = model
    self.mgr = CampBoxManager.Instance
    self.windowId = WindowConfig.WinID.campbox_tab_window
...
function CampBoxTabWindow:OnOpen()
...
function CampBoxTabWindow:RemoveListeners()
    self.mgr.onUpdateTabRedPoint:RemoveListener(self.redPointListener)
end


function CampBoxTabWindow:SwitchTabs(index)
...
function CampBoxTabWindow:SwitchTabss(index)
    ...
    if index ~= 4 then
        ...
        if index ==1 then
          if self.panelList[index] == nil then
                local panel = MarchEventPanel.New(self.model,self)
                self.panelList[index] = panel
            end
            self.panelList[index]:Show()
            self.titleText.text = TI18N("冰激凌")
        elseif index == 2 then
            if self.panelList[index] == nil then
...
function CampBoxTabWindow:__delete()
    self:RemoveListeners()
...
function CampBoxTabWindow:OnHide()
    self:RemoveListeners()
    self.model:HideAllPanel()
end

function CampBoxTabWindow:CheckRedPoint()
...
function CampBoxTabWindow:EnableTab(main, bool)
...
function CampBoxTabWindow:SetTab()
