BeginAutumnMainWindow = BeginAutumnMainWindow or BaseClass(BaseWindow)


function BeginAutumnMainWindow:__init(model,manager)
    self.mgr = manager.Instance
    self.model = model
    self.name = "BeginAutumnMainWindow"
    self.cacheMode = CacheMode.Visible
    self.windowId = WindowConfig.WinID.beginautumn_main_window
    ...
    self.setting = {
     ...
     self.secondId = 0
     self.campaignId = CampaignEumn.Type.BeginAutumn
     self.imgLoader = {}


end

function BeginAutumnMainWindow:__delete()
...
function BeginAutumnMainWindow:InitPanel()
    ...
    self.titleImg.sprite = self.assetWrapper:GetSprite(AssetConfig.beginautum, "TtileImg")
...
function BeginAutumnMainWindow:OnOpen()
...
function BeginAutumnMainWindow:OnHide()
    EventMgr.Instance:RemoveListener(event_name.campaign_change,self.redListener)
...
function BeginAutumnMainWindow:InitTabList()
       ...
       for index,v in pairs(temData) do
        if index ~= "count" then
            ...
            if v.sub[1].id == CampaignEumn.BeginAutumn.RechargeGift then
                self.tabList[v.sub[1].id].icon:GetComponent(Image).sprite = self.assetWrapper:GetSprite(AssetConfig.campaign_icon, "Dollar")
            elseif v.sub[1].id == CampaignEumn.BeginAutumn.TimeShop then
                self.tabList[v.sub[1].id].icon:GetComponent(Image).sprite = self.assetWrapper:GetSprite(AssetConfig.campaign_icon, "ChildFlowerGift")
            end
          end
       end
...
function BeginAutumnMainWindow:ChangeTab(campaignId)
...
--    end
    ...
    if campaignId == CampaignEumn.BeginAutumn.RechargeGift and self.panelIdList[campaignId] == nil then
    ...
    elseif campaignId == CampaignEumn.BeginAutumn.TimeShop and self.panelIdList[campaignId] == nil then
        WindowManager.Instance:OpenWindowById(WindowConfig.WinID.discountshopwindow)
    end
...
function BeginAutumnMainWindow:CheckRedPoint()
