GeneralTips = GeneralTips or BaseClass(BaseTips)

function GeneralTips:__init(model)
    self.model = model

    self.resList = {
        {file = AssetConfig.tips_general, type = AssetType.Main}
    }
...
function GeneralTips:RemoveTime()
    self.mgr.updateCall = nil
end

function GeneralTips:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(AssetConfig.tips_general))
    self.gameObject.name = "GeneralTips"
...
function GeneralTips:UpdateInfo(info)
    -- self:Default()
    for k,v in pairs(self.msgExtTab) do
        if v ~= nil then
            v:DeleteMe()
...
function GeneralTips:SetItem(textobj, data, pos, index)
    local w = 0
    local h = 0
...
function GeneralTips:UnRealUpdate()
