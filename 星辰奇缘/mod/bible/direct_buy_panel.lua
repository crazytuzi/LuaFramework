...
DirectBuyPanel = DirectBuyPanel or BaseClass(BasePanel)

function DirectBuyPanel:__init(model, parent)
    self.resList = {
        {file = AssetConfig.directbuypanel, type = AssetType.Main},
    ...
    self.OnOpenEvent:AddListener(
        function()
            self:OnOpen()
        end
    )
    self.OnHideEvent:AddListener(
        function()
            self:OnHide()
        end
    )

    self.reloadListener = function(data)
        self:ReloadData(data)
    end
end

function DirectBuyPanel:__delete()
...
function DirectBuyPanel:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(AssetConfig.directbuypanel))
    self.gameObject.name = "DirectBuyPanel"
    ...
    self.chestBtn.onClick:AddListener(
        function()
            self:ShowGift(self.gift_id)
        end
    )
    ...
    self.prePageBtn.onClick:AddListener(
        function()
            self:TurnPage(-1)
        end
    )
    self.nextPageBtn.onClick:AddListener(
        function()
            self:TurnPage(1)
        end
    )
    self.tipsData = {
        "1、萌萌喵喵培养进度说明：\n萌猫培养满升级即可获得萌猫的馈赠礼\n每级培养满500点经验升级\n2、萌猫培养值获得说明：\n<color='#ffff00'>温柔抚摸</color>+<color='#ffff00'>10点</color>经验值\n<color='#ffff00'>小鱼喂养</color>+<color='#ffff00'>10点</color>经验值\n<color='#ffff00'>薄荷草喂养</color>+<color='#ffff00'>60点</color>经验值\n<color='#ffff00'>金铃铛玩具</color>+<color='#ffff00'>120点</color>经验值\n3、此活动将不触发其他充值活动\n4、购买限购次数每日0点刷新，培养进度将不刷新"
    }

    local tipsBtn = self.transform:Find("TopCon/TipsBtn"):GetComponent(Button)
    tipsBtn.onClick:AddListener(
        function()
            TipsManager.Instance:ShowText({gameObject = tipsBtn.gameObject, itemData = self.tipsData})
        end
    )
end

function DirectBuyPanel:OnInitCompleted()
    self.OnOpenEvent:Fire()
end

function DirectBuyPanel:OnOpen()
...
function DirectBuyPanel:OnHide()
    self:RemoveListeners()
...
function DirectBuyPanel:RemoveListeners()
    self.mgr.directBuyUpdateEvent:RemoveListener(self.reloadListener)
end

--更新界面显示
function DirectBuyPanel:ReloadData(data)
    BaseUtils.dump(data, "data111111111")

    self.now_bring_lev = 1 --当前等级

    self.rewardData = BaseUtils.copytab(data.bring_up_gift)
    self.gift_id = self.rewardData[self.now_bring_lev].gift_id

    self.intimateTxtImg.sprite = self.assetWrapper:GetSprite(AssetConfig.open_server_textures2, string.format("IntimateTxtImg%s", self.now_bring_lev))
    ...
    local lev_old = self.model.now_bring_lev or self.now_bring_lev
    local val_old = 0

    local flag = val > data.now_bring_val   --判定是否升级的标志
    ...
    self.model.now_bring_lev = self.now_bring_lev
    ...
    for i, info in pairs(data.camp_info) do
        ...
        local list = {}
        for i = 1, count do
            list[i] = info.reward[count - i + 1]
        end

        for index = 1, 4 do
            local slotItem = tab.itemSlot[index]
            if index <= count then
                local slotData = list[index]
            ...
            tab.btn.onClick:AddListener(
                function()
                    if info.time == 0 then
                        ...
                        if not self.model.limitClick[i] then
                            self.model:StartLimitClickDirectBuy(i)
                            ...
                            NoticeManager.Instance:FloatTipsByString(TI18N("操作太快，请稍后再试"))
                        end
                    end
                end
            )
...
--翻页逻辑
function DirectBuyPanel:TurnPage(direction)
...
--打开礼包
function DirectBuyPanel:ShowGift(gift_id)
...
--处理特效
function DirectBuyPanel:DealExtraEffect()
    self.timerId_effect =
        LuaTimer.Add(
        0,
        200,
