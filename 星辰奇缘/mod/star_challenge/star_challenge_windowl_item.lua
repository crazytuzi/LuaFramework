...
StarChallengeWindowItem = StarChallengeWindowItem or BaseClass()

function StarChallengeWindowItem:__init(model, gameObject, parent)
    self.model = model
    self.parent = parent
...
function StarChallengeWindowItem:__delete()
    self.assetWrapper = nil
    self.iconImage.sprite = nil
    self.iconBgImage.sprite = nil
end

function StarChallengeWindowItem:update_my_self(data, index)
    self.index = index
    self.data = data
    ...
    self.iconImage.sprite = self.parent.assetWrapper:GetSprite(AssetConfig.starchallenge_textures, cfgData.head_id)
...
function StarChallengeWindowItem:SetActive(bool)
    self.gameObject:SetActive(bool)
end

function StarChallengeWindowItem:SetScale(value)
...
function StarChallengeWindowItem:OnClick()
...
function StarChallengeWindowItem:SetEffect(show)
    if show then
        if self.effect == nil then
            local fun = function(effectView)
                ...
                effectObject.transform.localPosition = Vector3(-6, -40, -400)
            ...
            self.effect = BaseEffectView.New({effectId = 20407, time = nil, callback = fun})
        else
