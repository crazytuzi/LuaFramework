...
StarChallengeHelpWindow = StarChallengeHelpWindow or BaseClass(BaseWindow)

function StarChallengeHelpWindow:__init(model)
    self.model = model
    self.name = "StarChallengeHelpWindow"
    self.mgr = StarChallengeManager.Instance
    self.windowid = WindowConfig.WinID.StarChallengeHelpWindow
    ...
    self.itemList = {}
    self.key = 1
    self.descFormat = TI18N("龙王预选赛帮助他人通关可获奖励，每日限一次(<color='#00ff00'>%s</color>/1)")
...
function StarChallengeHelpWindow:__delete()
...
function StarChallengeHelpWindow:InitPanel()
...
function StarChallengeHelpWindow:OnInitCompleted()
    self.OnOpenEvent:Fire()
end

function StarChallengeHelpWindow:OnOpen()
...
function StarChallengeHelpWindow:OnHide()
    self:RemoveListeners()
end

function StarChallengeHelpWindow:RemoveListeners()
    self.mgr.OnUpdateHelpGet:RemoveListener(self.reloadListener)
end

function StarChallengeHelpWindow:ReloadTimes()
...
function StarChallengeHelpWindow:ReloadItems()
    self.layout:ReSet()
