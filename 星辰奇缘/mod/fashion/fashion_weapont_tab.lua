FashionWeapontTab = FashionWeapontTab or BaseClass(BasePanel)

function FashionWeapontTab:__init(parent)
    self.parent = parent
    self.resList = {
        {file = AssetConfig.fashionweapontab, type = AssetType.Main}
...
function FashionWeapontTab:__delete()
...
function FashionWeapontTab:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(AssetConfig.fashionweapontab))
    self.gameObject.name = "FashionWeapontTab"
    ...
    self:update_item_list(self.parent.model:get_weapon_data_list())
end

--重置掉所有选中的item
function FashionWeapontTab:reset_weapon_selected_items()
...
--更新右边数据列表
function FashionWeapontTab:update_item_list(data_list)
    if self.has_init == false then
        return
    end
    ...
    for i=1,#self.current_data_list do
        ...
        if item == nil then
            item = FashionWeaponItem.New(self, self.origin_item, i)
        else
...
--选中某个武器
function FashionWeapontTab:update_left(item, _force)
    ...
    if item.selected then
        ...
        self.parent:UpdateLeftTopFaceScore(4, item.data.desc, item.data)
    else
        self.lastSelectedData = nil
        self.parent:UpdateLeftTopFaceScore(4, "", nil)
    end
...
--购买、保存、充能按钮的点击监听
function FashionWeapontTab:on_click_buy()
    --得判断下是购买还是保存
    local _weapon_ornament = 0
    if self.buy_btn_state == 2 then
        --穿上
        if self.parent.model.current_weapon_data.special_mark == 1 then
            FashionManager.Instance:request13205(1)
        elseif self.parent.model.current_weapon_data.special_mark == 2 then
            for key, value in pairs(DataBacksmith.data_equip_dianhua) do
                if self.parent.model.current_weapon_data.base_id == value.fashion_id then
                    EquipStrengthManager.Instance:request10619(1, value.looks)
                    break
                end
            end
        else
            if self.parent.model:check_is_weapon_can_wear(self.parent.model.current_weapon_data) then
                _weapon_ornament = self.parent.model.current_weapon_data.base_id
                FashionManager.Instance:request13205(_weapon_ornament)
            end
        end
    end
end

function FashionWeapontTab:on_click_chongneng()
    self.parent.model.belt_type = 2
    -- self.parent.model:InitFashionWeaponConfirmUI()
end

--更新穿戴按钮的样式
function FashionWeapontTab:update_put_on_btn()
    if self.has_init == false then
        return
    end
    ...
    if self.parent.model:check_is_weapon_data(self.parent.model.current_weapon_data) == false then
        ...
        local single = false
        --设置按钮显示状态
        if self.parent.model:check_is_weapon_data(self.parent.model.current_weapon_data) then
            --选择一件
            active = self.parent.model.current_weapon_data.active
            single = true
            self:set_buy_btn_state(active, single, self.parent.model.current_weapon_data)
        end
    end
end

--设置购买保存按钮状态
function FashionWeapontTab:set_buy_btn_state(active, single, data)
    self.ConClothBottom.gameObject:SetActive(false)
    self.ConBottom.gameObject:SetActive(true)
    if single then
        --只选了一件
        if active == 1 then
            ...
            if data ~= nil and data.is_wear == 1 then
                ...
                self.BtnBuy.transform:FindChild("Text"):GetComponent(Text).text = TI18N("使用中")
                self.BtnBuy:GetComponent(Image).sprite = PreloadManager.Instance:GetSprite(AssetConfig.base_textures, "DefaultButton4")
            else
                self.buy_btn_state = 2
                self.BtnBuy.transform:FindChild("Text"):GetComponent(Text).text = TI18N("保 存")
                self.BtnBuy:GetComponent(Image).sprite = PreloadManager.Instance:GetSprite(AssetConfig.base_textures, "DefaultButton3")
            end
        else
            ...
            if self.lastSelectedData == nil then
                if self.parent.model.current_weapon_data ~= nil and self.parent.model.current_weapon_data.active == 0 then
                    unActiveData = self.parent.model.current_weapon_data
                end
            else
    ...
    elseif active == 1 then
        ...
        self.BtnBuy.transform:GetComponent(RectTransform).anchoredPosition = Vector2(0, 0)
        self.BtnBuy.gameObject:SetActive(true)
        -- if (self.parent.model.current_weapon_data.is_wear == 1) then
        --     self.buy_btn_state = 1
...
--更新底部消耗
function FashionWeapontTab:UpdateClothCost(costDic)
    ...
    if baseId ~= 0 then
        ...
        self.BttomTxtLock.gameObject:SetActive(false)
        local hasNum = BackpackManager.Instance:GetItemCount(baseId)
    ...
    elseif self.lastSelectedData.special_mark == 1 then
        self.ClothCostCon.gameObject:SetActive(false)
        self.ClothSaveBtn.gameObject:SetActive(false)
        self.BttomTxtLock.gameObject:SetActive(true)
        self.BttomTxtLock.text = TI18N("默认武器外观")
    elseif self.lastSelectedData.special_mark == 2 then
        self.ClothCostCon.gameObject:SetActive(false)
        self.ClothSaveBtn.gameObject:SetActive(false)
        self.BttomTxtLock.gameObject:SetActive(true)
        local open_craft = 0
        for index, value in ipairs(DataBacksmith.data_equip_dianhua) do
            if value.fashion_id == self.lastSelectedData.base_id then
                open_craft = value.craft
                break
            end
        end
        if open_craft ~= 0 then
            ...
            self.BttomTxtLock.text = string.format(TI18N("武器精炼%s<color='#ffff00'>4星</color>可出现"),  name_str)
        end
    elseif self.lastSelectedData.special_mark == 3 then
        self.ClothCostCon.gameObject:SetActive(false)
        self.ClothSaveBtn.gameObject:SetActive(false)
        self.BttomTxtLock.gameObject:SetActive(true)
        self.BttomTxtLock.text = TI18N("勇夺职业首席\n获得首席专属武器使用权")
    elseif self.lastSelectedData.special_mark == 4 then
        self.ClothCostCon.gameObject:SetActive(false)
        self.ClothSaveBtn.gameObject:SetActive(false)
        self.BttomTxtLock.gameObject:SetActive(true)
        self.BttomTxtLock.text = TI18N("勇夺竞技场前5名\n获得竞技王者武器使用权")
    end
end

--slot逻辑
--创建slot
function FashionWeapontTab:CreateSlot(slot_con)
...
--对slot设置数据
function FashionWeapontTab:SetSlotData(slot, data)
    if data == nil then
        slot:SetAll(nil, nil)
        return
    end
