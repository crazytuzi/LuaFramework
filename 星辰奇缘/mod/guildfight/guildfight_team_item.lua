...
-- @author zgs
-- @time 2017.02.15
GuildfightTeamItem = GuildfightTeamItem or BaseClass()

function GuildfightTeamItem:__init(gameObject, parent)
...
function GuildfightTeamItem:onClick_InviteBtn()
...
--设置
function GuildfightTeamItem:SetActive(boo)
    self.gameObject:SetActive(boo)
end

function GuildfightTeamItem:Release()
    self.ImgHead.sprite = nil
    EventMgr.Instance:RemoveListener(event_name.guild_member_update, self.update_info_func)
end

function GuildfightTeamItem:InitPanel(_data)
    self:update_my_self(_data)
end

--设置索引
function GuildfightTeamItem:set_my_index(_index)
...
--更新内容
function GuildfightTeamItem:update_my_self(_data, _index)
    self.ImgSelected.gameObject:SetActive(false)
    self.data = _data
    self.TxtDeleted:SetActive(false)
...
function GuildfightTeamItem:on_select_mem_item(state)
    if state == nil then
...
--重置选中状态
function GuildfightTeamItem:reset_selected()
    self.selected_state = false
end