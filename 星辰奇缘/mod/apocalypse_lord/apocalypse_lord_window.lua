...
ApocalypseLordWindow = ApocalypseLordWindow or BaseClass(BaseWindow)

local GameObject = UnityEngine.GameObject
local Vector3 = UnityEngine.Vector3

function ApocalypseLordWindow:__init(model)
    self.model = model

    self.windowId = WindowConfig.WinID.ApocalypseLordwindow
...
function ApocalypseLordWindow:__delete()
    self:OnHide()
...
function ApocalypseLordWindow:InitPanel()
    ...
    for i=1,5 do
        self.itemList[i] = ApocalypseLordWindowItem.New(self.model, GameObject.Instantiate(self.cloner), self)
...
function ApocalypseLordWindow:OnClickClose()
    WindowManager.Instance:CloseWindow(self)
end

function ApocalypseLordWindow:OnShow()
    self:Update()
...
function ApocalypseLordWindow:UpdateInfo(y)
...
function ApocalypseLordWindow:UpdateBossInfo()
	self.titleText.text = TI18N("天启挑战")
...
function ApocalypseLordWindow:UpdateCommonMonsterInfo()
    self.titleText.text = TI18N("资格挑战")
    if self.currUnitConfigData ~= nil then
        self.descPanelTitle.text = DataUnit.data_unit[self.currUnitConfigData.index].name
    end
...
function ApocalypseLordWindow:UpdateTeamMask()
	...
	for i=1, #datalist do
        ...
        btn.onClick:RemoveAllListeners()
        btn.onClick:AddListener(function()
                self.model:OpenApocalypseLordTeamPanel({ self.index, data })
            end)
...
function ApocalypseLordWindow:UpdateRewardMask()
	local datalist = DataOracleTreasure.data_wave
	for i=1, #datalist do
	    local data = datalist[i]
...
-- 更新位置，使轨迹像圆
function ApocalypseLordWindow:UpdatePos()
    local y = nil
    local res = nil
    ...
    -- 然后就有下面的算法
    local containerY = self.container.anchoredPosition.y
...
-- 转到
function ApocalypseLordWindow:TweenTo(index)
	if index + 2 == 1 and self.model.status == 2 then
		NoticeManager.Instance:FloatTipsByString("获得资格的玩家在周五12点才能挑战天启{face_1,1}")
		return
	end
...
function ApocalypseLordWindow:StopAt(index)
...
function ApocalypseLordWindow:OnUp()
...
-- 设置选中框的透明度
function ApocalypseLordWindow:SetAlpha()
    local y = self.container.anchoredPosition.y
...
function ApocalypseLordWindow:ShowDescPanelTips()
	local tipsString = {
            TI18N("1.通过任一<color='#ffff00'>门徒</color>考验后可获得天启<color='#ffff00'>挑战资格</color>")
            , TI18N("2.门徒被成功挑战或刷天后，能力都将<color='#ffff00'>得到提升</color>")
            , TI18N("3.每位门徒对应的<color='#ffff00'>天启挑战资格</color>有限，只有<color='#ffff00'>最近通过挑战</color>的8支队伍才可获得")
        }
...
function ApocalypseLordWindow:ChangeTab(index)
	self.tabIndex = index
	if index == 1 then
		self.teamMask:SetActive(true)
		self.rewardMask:SetActive(false)
...
function ApocalypseLordWindow:OnOkButtonClick()
    if RoleManager.Instance.RoleData.event ~= RoleEumn.Event.ApocalypseLord then
        self.model:EnterScene()
    end

	if self.model.status == 2 then
       	if self.currUnitConfigData.index ~= 32031 then
            ...
            for k,v in pairs(units) do
       		...
       		NoticeManager.Instance:FloatTipsByString(TI18N("获得资格后可在周五15点后挑战天启"))
       	end
    elseif self.model.status == 3 then
        if self.currUnitConfigData.index == 32031 then
       		local units = SceneManager.Instance.sceneElementsModel:GetSceneData_Npc()
...
function ApocalypseLordWindow:ShowButtonEffect(show)
    if show then
        if self.effect == nil then
            local fun = function(effectView)
                local effectObject = effectView.gameObject
                if not BaseUtils.isnull(self.okButton) then
...
function ApocalypseLordWindow:UpdateRankPanel()
    self.rankDescText.text = string.format(TI18N("排行前%s的玩家\n可获得天启传承！"), 10)

    local itemData = ItemData.New()
    local itemBase = BackpackManager.Instance:GetItemBase(24055)
    itemData:SetBase(itemBase)
    self.rankItemSolt:SetAll(itemData)

    self.rankModelTitleText.text = TI18N("天启·黎明")

    local data = {type = PreViewType.Pet, skinId = 41135, modelId = 41035, animationId = 4103501, scale = 1, effects = {{effect_id = 102130}}}
...
function ApocalypseLordWindow:UpdateRank()
...
function ApocalypseLordWindow:onToggleChange(on)
    self.rankFriendOnlyMark = on

    self:UpdateRank()
end

function ApocalypseLordWindow:ChangeMainTab(index)
...
function ApocalypseLordWindow:OnRewardButtonClick()
    self.tipsPanel:SetActive(true)
    self.tipsPanelText.text = TI18N("成功挑战极品奖励:")

    local reward_data = DataOracleTreasure.data_reward[self.currUnitConfigData.index].base_reward
...
function ApocalypseLordWindow:ShowRewardTips()
    self.tipsPanel:SetActive(true)
    self.tipsPanelText.text = TI18N("首杀极品奖励:")

    local reward_data = DataOracleTreasure.data_reward[self.currUnitConfigData.index].frist_reward
