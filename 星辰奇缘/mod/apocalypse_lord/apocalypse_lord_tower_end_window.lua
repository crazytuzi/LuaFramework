ApocalypseLordTowerEndWindow = ApocalypseLordTowerEndWindow or BaseClass(BaseWindow)

local GameObject = UnityEngine.GameObject

function ApocalypseLordTowerEndWindow:__init(model)
    self.model = model

    self.name = "ApocalypseLordTowerEndWindow"
    self.Type = {
        normal = 1,
        rare = 2,
    }
...
function ApocalypseLordTowerEndWindow:__delete()
    for k,v in pairs(self.iconloader) do
        v:DeleteMe()
    end
    self.iconloader = {}
    if self.hasget == false then
        ApocalypseLordManager.Instance:Send20804(1)
    end
    self:ClearDepAsset()
end

function ApocalypseLordTowerEndWindow:InitPanel()
...
function ApocalypseLordTowerEndWindow:OnClose()
...
function ApocalypseLordTowerEndWindow:AddTips(go, base_id)
...
function ApocalypseLordTowerEndWindow:InitBox(_type)
...
function ApocalypseLordTowerEndWindow:CloseBox(boxTs, index)
    local callback = function () ApocalypseLordManager.Instance:Send20804(index)   end
...
function ApocalypseLordTowerEndWindow:OpenBox(index, data)
    self.hasget = true
...
function ApocalypseLordTowerEndWindow:DuangEffect(target, callback)
