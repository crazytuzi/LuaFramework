-- 龙王
-- ljh 20170725
ApocalypseLordSettlementWindow = ApocalypseLordSettlementWindow or BaseClass(BasePanel)

function ApocalypseLordSettlementWindow:__init(model)
    self.model = model
    self.windowId = WindowConfig.WinID.ApocalypseLordsettlementwindow

    self.resList = {
        {file = AssetConfig.ApocalypseLordsettlementwindow, type = AssetType.Main}
...
function ApocalypseLordSettlementWindow:__delete()
...
function ApocalypseLordSettlementWindow:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(AssetConfig.ApocalypseLordsettlementwindow))
    self.gameObject.name = "ApocalypseLordSettlementWindow"
...
function ApocalypseLordSettlementWindow:OnClickClose()
    -- WindowManager.Instance:CloseWindow(self)
    self.model:CloseApocalypseLordSettlementWindow()
end

function ApocalypseLordSettlementWindow:OnInitCompleted()
    self.OnOpenEvent:Fire()
end

function ApocalypseLordSettlementWindow:OnOpen()
    self:Update()

    ApocalypseLordManager.Instance.OnUpdateList:RemoveListener(self.updateListener)
    ApocalypseLordManager.Instance.OnUpdateList:AddListener(self.updateListener)
end

function ApocalypseLordSettlementWindow:OnHide()
    ApocalypseLordManager.Instance.OnUpdateList:RemoveListener(self.updateListener)
end

function ApocalypseLordSettlementWindow:Update()
	self:UpdateModel()
    self:UpdateItem()
...
function ApocalypseLordSettlementWindow:UpdateModel()
...
function ApocalypseLordSettlementWindow:PreviewLoaded(composite)
...
function ApocalypseLordSettlementWindow:UpdateItem()
	...
	if self.model.max_wave == 0 then
		...
		local reward_data = DataOracleTreasure.data_wave[self.model.max_wave].show_reward
		for i=1, #self.itemSlotList do
	...
	-- 处理下阶段奖励
	if #DataOracleTreasure.data_wave == self.model.max_wave then
        ...
        self.allCleanText.text = TI18N("您已经通过天启试炼所有阶段，敬请期待下次考验")
	else
		self.text3.text = string.format(TI18N("下阶段奖励：\n（达成人数<color='#13fc60'>%s%%</color>）"), self.model:GetBossWaveNum(self.model.max_wave+1))

		local reward_data = DataOracleTreasure.data_wave[self.model.max_wave+1].show_reward
		for i=1, #self.itemSlotList2 do
	        if reward_data[i] ~= nil then
	            local itemData = ItemData.New()
	            local itemBase = BackpackManager.Instance:GetItemBase(reward_data[i][1])
...
function ApocalypseLordSettlementWindow:OpenRank()
	self:OnClickClose()
	WindowManager.Instance:OpenWindowById(WindowConfig.WinID.ApocalypseLordwindow, {2, true})
end

function ApocalypseLordSettlementWindow:showBgAni()
	if self.rotateId ~= nil then
        LuaTimer.Delete(self.rotateId)
    end
...
function ApocalypseLordSettlementWindow:Rotate()
