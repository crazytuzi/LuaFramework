WorldRedBagUnOpenWindow  =  WorldRedBagUnOpenWindow or BaseClass(BasePanel)

function WorldRedBagUnOpenWindow:__init(model)
    self.name  =  "WorldRedBagUnOpenWindow"
    self.model  =  model
    -- 缓存
    self.resList  =  {
        {file  =  AssetConfig.world_red_bag_unopen_win, type  =  AssetType.Main}
        , {file = AssetConfig.guild_dep_res, type = AssetType.Dep}
    }

    self.windowId = WindowConfig.WinID.world_red_bag_unopen_win
...
function WorldRedBagUnOpenWindow:__delete()
    self.bg2.sprite = nil
    self.img.sprite = nil
...
function WorldRedBagUnOpenWindow:InitPanel()
    if self.gameObject ~=  nil then
        --加载回调两次，这里暂时处理
        return
    end

    self.gameObject = GameObject.Instantiate(self:GetPrefab(AssetConfig.world_red_bag_unopen_win))
    self.gameObject:SetActive(false)
    self.gameObject.name = "WorldRedBagUnOpenWindow"
...
function WorldRedBagUnOpenWindow:update_view()
...
function WorldRedBagUnOpenWindow:on_click_btn(g)
    ...
    RedBagManager.Instance:Send18503(self.model.current_red_bag.rid, self.model.current_red_bag.platform, self.model.current_red_bag.zone_id)
end