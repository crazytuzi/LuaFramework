...
-- hosr
-- ----------------------------
GodsWarSummaryPanel = GodsWarSummaryPanel or BaseClass(BasePanel)

function GodsWarSummaryPanel:__init(model)
    self.model = model
    self.Mgr = GodsWarManager.Instance
    self.name = "GodsWarSummaryPanel"
    -- self.GoodEffectPath = string.format(AssetConfig.effect, "20121")
    self.resList = {
        {file = AssetConfig.worldchampionfightinfo2, type = AssetType.Main},
...
function GodsWarSummaryPanel:__delete()
...
function GodsWarSummaryPanel:InitPanel()
    self.gameObject = GameObject.Instantiate(self:GetPrefab(AssetConfig.worldchampionfightinfo2))
    ...
    self.transform:Find("Main/Title/I18NText"):GetComponent(Text).text = TI18N("诸神之战统计")

    self.transform:Find("Main/I18NDesc").gameObject:SetActive(false)

    self.closeBtn = self.transform:Find("Main/CloseButton"):GetComponent(Button)
    self.closeBtn.onClick:AddListener(function() self.model:CloseSummary() end)

    self.continueBtn = self.transform:Find("Main/Button"):GetComponent(Button)
    self.transform:Find("Main/Button/I18NText"):GetComponent(Text).text = TI18N("确定")
    self.continueBtn.onClick:AddListener(function() self.model:CloseSummary() end)
    ...
    for i=1,10 do
        ...
        self.ItemList[i].gameObject = trans.gameObject
        ...
        -- end
        self.ItemList[i].RankImage.gameObject:SetActive(false)
        self.ItemList[i].GoodButton.gameObject:SetActive(false)
        self.ItemList[i].FriendButton.gameObject:SetActive(false)
    end
    ...
    self.Role_RepBtn.onClick:AddListener(function()
        -- self.Mgr:Require16425(self.currRoledata.rid, self.currRoledata.platform, self.currRoledata.zone_id)
    end)
    ...
    local setting = {title = TI18N("分享诸神之战成绩"), type = 1}
...
function GodsWarSummaryPanel:LoadItem()
    ...
    self.result0.gameObject:SetActive(self.data.result == 0 or self.data.is_win == 0)
    ...
    for i,v in ipairs(self.data.mates) do
        ...
        item.head.sprite = PreloadManager.Instance:GetSprite(AssetConfig.heads, v.classes.."_"..v.sex)
    ...
    for i,v in ipairs(self.data.rival) do
        ...
        if self.data.result == 0 or self.data.is_win == 0 then
            if v.mvp_score > self.mvper.num then
                self.mvper.ii = i+5
                self.mvper.num = v.mvp_score
            end
...
function GodsWarSummaryPanel:PraseData()
    self.roleInfoList = {}
    for i,v in ipairs(self.data.mates) do
...
function GodsWarSummaryPanel:GoodSuccess(rid, platform, zone_id)
    for i,v in ipairs(self.data.mates) do
...
function GodsWarSummaryPanel:ClickItem(index , data)
    ...
    player.classes = data.classes
    player.sex = data.sex
    player.name = data.name
    player.lev = data.lev
    TipsManager.Instance:ShowPlayer(player)

    -- self.RoleDescPanel.gameObject:SetActive(true)
    -- self.Role_Head.sprite = PreloadManager.Instance:GetSprite(AssetConfig.heads, data.classes.."_"..data.sex)
...
function GodsWarSummaryPanel:SelectLeft()
    local roleData = RoleManager.Instance.RoleData
...
function GodsWarSummaryPanel:SelectRight()
    local roleData = RoleManager.Instance.RoleData
    local i = self.currRoleindex + 1
    if i >= 11 then
        i = 1
...
function GodsWarSummaryPanel:FormatNum(num)
    if num < 100000 then
        return tostring(num)
    else
...
function GodsWarSummaryPanel:WatchIconByIndex(index)
    if index == nil or index == 0 then
        ...
        self.IconDescConBg.sizeDelta = Vector2(426.1, 385.5)
    else
        local num = 0
        local showindex = {}
        if self.godlike[index] == true then
        ...
        if num > 0 then
            ...
            self.IconDescConBg.sizeDelta = Vector2(426.1, 85.5+(num+1)*50)
...
function GodsWarSummaryPanel:GiveEffect(go)
