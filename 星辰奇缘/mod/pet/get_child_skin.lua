...
-- 获得皮肤
-- -------------------
GetChildSkin = GetChildSkin or BaseClass(BaseDramaPanel)

function GetChildSkin:__init()
    ...
    self.click = function()  self:UsedSkin()   self:Destroy() end
...
function GetChildSkin:__delete()
...
function GetChildSkin:InitPanel()
...
function GetChildSkin:OnInitCompleted()
    ...
    self.platform = self.openArgs.platform
    self.zone_id  = self.openArgs.zone_id
    self.child_id = self.openArgs.child_id

    self:SetData()
end

function GetChildSkin:SetData()
    DramaManager.Instance.model:ShowJump(false)
    self:Next()
end

function GetChildSkin:Next()
    if self.timeId ~= 0 then
        LuaTimer.Delete(self.timeId)
    end
...
function GetChildSkin:ShowTitle()
...
function GetChildSkin:ShowHaloLight()
    self.halo:SetActive(true)
    self.light:SetActive(true)
    self.rotateId = LuaTimer.Add(0, 10, function() self:Rotate() end)
end

function GetChildSkin:ShowPreview()
    local pet = DataChild.data_child[self.petId]
    ...
    for key, value in pairs(DataChild.data_child_skin) do
        if self.use_skin == value.skin_id then
            modelData.modelId = value.model_id
            modelData.skinId = value.texture
            modelData.animationId = value.animation_id
...
function GetChildSkin:PutNewImg()
    self.newImg:SetActive(true)
    self.timeId = LuaTimer.Add(500, function() self:Next() end)
end

function GetChildSkin:ShowButton()
...
function GetChildSkin:PlayAction()
...
function GetChildSkin:ActionDelay()
...
function GetChildSkin:ActionEnd()
...
function GetChildSkin:LoadPreview(modelData)
    self.rawImg:SetActive(false)
...
function GetChildSkin:SetRawImage(composite)
...
--根据模型包围盒计算中心点
function GetChildSkin:SetPosition()
...
function GetChildSkin:OnJump()
end

function GetChildSkin:BeginCountDown()
    self.effect:SetActive(true)
    self.countDownId = LuaTimer.Add(3000, function() self:Destroy() end)
end

function GetChildSkin:UsedSkin()
    ChildrenManager.Instance:Send18643(self.child_id, self.platform, self.zone_id,self.use_skin)
end
