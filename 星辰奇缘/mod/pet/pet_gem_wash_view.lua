...
-- UI - 宠物符石技能洗炼
-- ----------------------------------------------------------
PetGemWashView = PetGemWashView or BaseClass(BaseWindow)

local GameObject = UnityEngine.GameObject
local Vector3 = UnityEngine.Vector3

function PetGemWashView:__init(model)
    self.model = model
    self.name = "PetGemWashView"
    self.windowId = WindowConfig.WinID.petgemwash
    self.winLinkType = WinLinkType.Link

    self.resList = {
        {file = AssetConfig.pet_genwash_window, type = AssetType.Main}
...
function PetGemWashView:__delete()
...
function PetGemWashView:InitPanel()
	self.gameObject = GameObject.Instantiate(self:GetPrefab(AssetConfig.pet_genwash_window))
    self.gameObject.name = "PetGemWashView"
    ...
    self.buttonscript.key = "PetGemWash"
...
function PetGemWashView:OnClickClose()
    WindowManager.Instance:CloseWindow(self)
end

function PetGemWashView:OnShow()
...
function PetGemWashView:OnHide()
    self.tempSkillList = {}
...
function PetGemWashView:RemoveListeners()
    PetManager.Instance.On10526ButtonFreezon:Remove(self._updateFrezzonButton)
end

function PetGemWashView:update()
	self:updateItem()
end

function PetGemWashView:updateItem()
    local skillgem_list = BackpackManager.Instance:GetItemByBaseid(20615)
    local temp = BackpackManager.Instance:GetItemByBaseid(20616)
    for key, value in pairs(temp) do
        table.insert(skillgem_list, value)
    end
    temp = BackpackManager.Instance:GetItemByBaseid(20617)
    for key, value in pairs(temp) do
        table.insert(skillgem_list, value)
    end
    temp = BackpackManager.Instance:GetItemByBaseid(20638)
    for key, value in pairs(temp) do
        table.insert(skillgem_list, value)
    end
...
function PetGemWashView:setitemattr(item, attr)
    local attr_str = ""
    local skill_str = ""
...
function PetGemWashView:item_click(item, itemdata)
...
function PetGemWashView:updateInfo()
...
function PetGemWashView:lackCallback(baseidToBuyInfo)
...
function PetGemWashView:SetTempAttr(data)
...
function PetGemWashView:OnSaveButton()
...
function PetGemWashView:EnableFreezonBtn()
