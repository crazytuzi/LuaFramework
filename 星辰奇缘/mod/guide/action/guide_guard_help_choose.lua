...
-- hosr
-- ---------------------------------
GuideGuardHelpChoose = GuideGuardHelpChoose or BaseClass(BasePanel)

function GuideGuardHelpChoose:__init(panel)
    self.panel = panel
...
function GuideGuardHelpChoose:__delete()
    self:GuideEnd()
...
function GuideGuardHelpChoose:InitPanel()
...
function GuideGuardHelpChoose:Next()
...
function GuideGuardHelpChoose:ChooseStoneTab()
    self.effect:SetActive(false)
    self.gameObject:SetActive(false)

    self.lastGameObject = self.panel.guardTab[1].gameObject
    ...
    self.effect.transform.localScale = Vector3.one
    self.effect.transform.localPosition = Vector3(0, -35, -400)
    Utils.ChangeLayersRecursively(self.effect.transform, "UI")
    self.lastGameObject:GetComponent(Button).onClick:AddListener(self.next)
    self.effect:SetActive(true)
    TipsManager.Instance:ShowGuide({gameObject = self.lastGameObject, data = TI18N("选择助阵守护"), forward = TipsEumn.Forward.Left})
end

function GuideGuardHelpChoose:GuideEnd()
...
function GuideGuardHelpChoose:Notice()
    local data = NoticeConfirmData.New()
    data.type = ConfirmData.Style.Sure
    data.content = TI18N("守护助阵成功，所有<color='#00ff00'>上阵守护</color>将获得<color='#00ff00'>生命加成</color>\n<color='#ffff00'>加成数值将根据守护品质变化</color>")
