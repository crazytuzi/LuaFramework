...
-- hosr
-- ----------------------------------
GuideGuardUpgrade = GuideGuardUpgrade or BaseClass(BasePanel)

function GuideGuardUpgrade:__init(panel)
    self.panel = panel
...
function GuideGuardUpgrade:__delete()
    self:GuideEnd()
...
function GuideGuardUpgrade:InitPanel()
...
function GuideGuardUpgrade:First()
    self.gameObject:SetActive(false)
    if self.panel.subFirst ~= nil and self.panel.subFirst.equipList[1] ~= nil and self.panel.subFirst.equipList[1].equip_btn ~= nil then
        self.lastGameObject = self.panel.subFirst.equipList[1].equip_btn.gameObject
    end
    ...
    TipsManager.Instance:ShowGuide({gameObject = self.lastGameObject, data = TI18N("选择需要升级的守护装备"), forward = TipsEumn.Forward.Right})
end

function GuideGuardUpgrade:Next()
...
function GuideGuardUpgrade:GuideEnd()
