-- -------------------------------------------
-- 引导突破后属性兑换 第三步，属性兑换界面
-- hosr
-- -------------------------------------------
GuideBreakPointThird = GuideBreakPointThird or BaseClass(BasePanel)

function GuideBreakPointThird:__init(panel)
    self.panel = panel
...
function GuideBreakPointThird:__delete()
    self:GuideEnd()
...
function GuideBreakPointThird:InitPanel()
...
function GuideBreakPointThird:First()
    self.gameObject:SetActive(false)

    self.lastGameObject = self.panel.exchangeButton
    ...
    TipsManager.Instance:ShowGuide({gameObject = self.lastGameObject, data = TI18N("消耗经验可兑换额外的人物属性点哦"), forward = TipsEumn.Forward.Left})
end

function GuideBreakPointThird:Next()
...
function GuideBreakPointThird:GuideEnd()
	RoleManager.Instance.isGuideBreakThird = true
