...
-- hosr
-- ----------------------------------
GuideGuardHelp = GuideGuardHelp or BaseClass(BasePanel)

function GuideGuardHelp:__init(panel)
    self.panel = panel
...
function GuideGuardHelp:__delete()
    self:GuideEnd()
...
function GuideGuardHelp:InitPanel()
...
function GuideGuardHelp:Next()
	print("........................   " .. tostring(self.step))
    if not BaseUtils.is_null(self.lastGameObject) then
        self.lastGameObject:GetComponent(CustomEnterExsitButton).onClick:RemoveListener(self.next)
    end
...
function GuideGuardHelp:ChooseStoneTab()
    self.effect:SetActive(false)
    self.gameObject:SetActive(false)

    self.lastGameObject = self.panel.starList[1].gameObject
    ...
    Utils.ChangeLayersRecursively(self.effect.transform, "UI")
    self.lastGameObject:GetComponent(CustomEnterExsitButton).onClick:AddListener(self.next)
    self.effect:SetActive(true)
    TipsManager.Instance:ShowGuide({gameObject = self.lastGameObject, data = TI18N("点击选择助阵位"), forward = TipsEumn.Forward.Right})
end

function GuideGuardHelp:GuideEnd()
