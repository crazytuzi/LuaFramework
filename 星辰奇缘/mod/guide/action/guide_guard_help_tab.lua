...
-- hosr
-- ------------------------------------
GuideGuardHelpTab = GuideGuardHelpTab or BaseClass(BasePanel)

function GuideGuardHelpTab:__init(panel)
    self.panel = panel
...
function GuideGuardHelpTab:__delete()
    self:GuideEnd()
...
function GuideGuardHelpTab:InitPanel()
...
function GuideGuardHelpTab:Next()
...
function GuideGuardHelpTab:ChooseStoneTab()
    self.effect:SetActive(false)
    self.gameObject:SetActive(false)
    ...
    TipsManager.Instance:ShowGuide({gameObject = self.lastGameObject, data = TI18N("点击进入<color='#00ff00'>守护助阵</color>"), forward = TipsEumn.Forward.Left})
end

function GuideGuardHelpTab:GuideEnd()
