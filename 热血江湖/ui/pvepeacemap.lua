...
wnd_pvePeaceMap = i3k_class("wnd_pvePeaceMap", ui.wnd_base)

local g_mapSize = nil
function wnd_pvePeaceMap:ctor()
...
function wnd_pvePeaceMap:configure()
	local widgets = self._layout.vars
	widgets.closeBtn:onClick(self, self.onCloseUI)
end

function wnd_pvePeaceMap:onShow()

end

function wnd_pvePeaceMap:onHide()
	releaseSchedule()
end

function wnd_pvePeaceMap:refresh(mapId, cfg)
	local imgPath = g_i3k_db.i3k_db_get_icon_path(cfg.titleImagId)
...
function wnd_pvePeaceMap:rightMonsterList(mapId, size)
...
function wnd_pvePeaceMap:walkToPos(sender, needValue)
...
function wnd_pvePeaceMap:searchPath(sender, needValue)
	local size = needValue.size
	local world = i3k_game_get_world()
	local nowMapId = world._cfg.id
	local targetMapId = needValue.mapId
...
function wnd_pvePeaceMap:toTargetPos(mapId, pos, size, needValue, taskType)
	local hero = i3k_game_get_player_hero()
...
function wnd_pvePeaceMap:openOrPickup(sender, needValue)
...
function wnd_pvePeaceMap:close()
	releaseSchedule()
	g_i3k_ui_mgr:CloseUI(eUIID_SceneMap)
end

function wnd_pvePeaceMap:onUpdate(dTime)
