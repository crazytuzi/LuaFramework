...
-------------------------------------------------------
wnd_friendsFlyingEquipTips = i3k_class("wnd_friendsFlyingEquipTips",ui.wnd_base)
local bless_rankImg = {5139, 5140, 5141, 5142, 10856, 10857, 10858, 10859, 10860, 10861}
local LAYER_ZBTIPST = "ui/widgets/zbtipsfst"
local LAYER_ZBTIPST2 = "ui/widgets/zbtipsfst2"
local LAYER_ZBTIPST3 = "ui/widgets/zbtipsfst3"
local LAYER_ZBTIPST6 = "ui/widgets/zbtipsfst6"
...
function wnd_friendsFlyingEquipTips:ctor()

end

function wnd_friendsFlyingEquipTips:configure()
	local widgets = self._layout.vars
	widgets.globel_bt:onClick(self, self.onClose)
	widgets.mark_icon:hide()
end

function wnd_friendsFlyingEquipTips:onShow()

end

function wnd_friendsFlyingEquipTips:refresh(data)
	self:setTips(data)
end

function wnd_friendsFlyingEquipTips:equipSelfDate(data)
	local widgets = self._layout.vars
	...
	if wEquips and wEquips[data.id] and wEquips[data.id].equip then
		...
		widgets.naijiu1:setVisible(self.naijiu ~= -1)
		if self.naijiu and self.naijiu ~= -1 then
			widgets.naijiu1:setText("耐久度："..math.modf(self.naijiu/1000))
		end
		...
		if self.sLv ~= 0 then
			widgets.starlvlBg1:setImage(self.icon1.path)
		else
			widgets.starlvlBg1:hide()
		end
...
function wnd_friendsFlyingEquipTips:equipOtherDate(data)
	local widgets = self._layout.vars
	...
	if self.sLv ~= 0 then
		widgets.starlvlBg2:setImage(self.icon1.path)
	else
		widgets.starlvlBg2:hide()
	end
	widgets.naijiu2:setVisible(self.naijiu ~= -1)
	if self.naijiu and self.naijiu ~= -1 then
		widgets.naijiu2:setText("耐久度："..math.modf(self.naijiu/1000))
	end
	if self.M_require == 1 then
...
function wnd_friendsFlyingEquipTips:setTips(data)
	self:equipSelfDate(data);
	self:equipOtherDate(data);
end

function wnd_friendsFlyingEquipTips:showBaseProperty(list, legends)
	--基础属性
...
function wnd_friendsFlyingEquipTips:plusProperty(list, legends, equipID)
	--附加属性
	local index = 0
...
function wnd_friendsFlyingEquipTips:refineProerty(list, refine)
	--精炼属性
	if next(refine) then
...
function wnd_friendsFlyingEquipTips:showTemperProperty(list, smeltingProps, hammerSkill, equipID)
	--锤炼属性
...
function wnd_friendsFlyingEquipTips:showJewel(list, ratio)
	--宝石
	local inlayLvl = i3k_db_common.functionOpen.inlayLvl
	local lvl = g_i3k_game_context:GetLevel()
	if lvl >= inlayLvl and g_i3k_game_context:hasFacility(g_FACILITY_GEM_NESTING) then
...
function wnd_friendsFlyingEquipTips:setScrollLegends(list, legends, partID)
	for i,e in ipairs(legends) do
		if e ~= 0 then
			...
			widget.desc:setTextColor("ff80ffff")
			list:addItem(layer)
		end
	end
end

function wnd_friendsFlyingEquipTips:Getdata(wEquips,itemid,equip_t)
	--装备基础信息
...
function wnd_friendsFlyingEquipTips:onClose(sender)
	g_i3k_ui_mgr:CloseUI(eUIID_FriendsFlyingEquipTips)
end

function wnd_create(layout, ...)
	local wnd = wnd_friendsFlyingEquipTips.new();
		wnd:create(layout, ...);
	return wnd;
end 
