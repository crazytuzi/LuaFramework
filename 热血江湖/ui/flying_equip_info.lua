...
-------------------------------------------------------
wnd_flying_equip_info = i3k_class("wnd_flying_equip_info", ui.wnd_base)
...
function wnd_flying_equip_info:ctor()
	self._equip_id = nil
...
function wnd_flying_equip_info:configure()
...
function wnd_flying_equip_info:unInstall(sender)
...
function wnd_flying_equip_info:saleButton(sender)
...
function wnd_flying_equip_info:closeButton(sender)
	g_i3k_ui_mgr:CloseUI(eUIID_FlyingEquipInfo)
...
function wnd_flying_equip_info:onRepairEquip(sender, legends)
...
function wnd_flying_equip_info:refresh(equip, out, transformId)
...
function wnd_flying_equip_info:updateTwoEquipInfo(equip1, equip2)
	self.layer2:show()
	self.label2:hide()
	self.sale:hide()
	self.xiexia_bt:hide()
...
function wnd_flying_equip_info:updateBagEquipInfo(equip, isWarehouse, hideBtn)--右边装备信息
	if equip then
		self._equip_id = equip.equip_id
		self._equip_refine = equip.refine
		self._equip_guid = equip.equip_guid
...
function wnd_flying_equip_info:updateWearEquipInfo(equip, isWarehouse)--左边装备信息
	self._equip_id = equip.equip_id
	self._equip_guid = equip.equip_guid
	self._equip_refine = equip.refine
	...
	if not isWarehouse and not g_i3k_game_context:hasFacility(g_FACILITY_EQUIP_UPGRADE) then
		self.sale:hide()
	else
		self.sale:show()
	end
...
function wnd_flying_equip_info:onStrengBtn(sender, isWarehouse)
	if isWarehouse then
...
function wnd_flying_equip_info:SetTips(list, equip, isCompare)
	...
	local openGem = g_i3k_game_context:hasFacility(g_FACILITY_GEM_NESTING)
	if lvl >= inlayLvl and openGem then
...
function wnd_flying_equip_info:SetBagTips(list, equip, needIndex)
	local itemid = equip.equip_id
...
function wnd_flying_equip_info:setScrollLegends(list, legends, partID)
...
function wnd_flying_equip_info:setScrollData(list, base_attribute, expect_attribute, naijiu, attribute, refine, legends, equipID, smeltingProps, hammerSkill)
	--基础属性
	local des = require(LAYER_ZBTIPST3)()
...
function wnd_flying_equip_info:tryToTransform(hufu, groupId)
...
function wnd_flying_equip_info:gotoTransform(sender, info)
...
function wnd_create(layout)
	local wnd = wnd_flying_equip_info.new()
		wnd:create(layout)
	return wnd
end
