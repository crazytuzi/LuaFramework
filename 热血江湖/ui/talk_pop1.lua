...
-------------------------------------------------------
wnd_talk_pop1 = i3k_class("wnd_talk_pop1", ui.wnd_base)

function wnd_talk_pop1:ctor()
	self._timeTick = 0
	self.nextEntity = nil
	self.nextText = nil
end

function wnd_talk_pop1:configure()
...
function wnd_talk_pop1:updateTimer(dTime)
	...
	if self.entity and self.entity._rescfg then
		...
		if self._timeTick > i3k_db_common.mercenarypop.popalivetime/1000 then
			...
			self._timer = nil
			g_i3k_ui_mgr:CloseUI(eUIID_TalkPop1)
		end
	end
end

function wnd_talk_pop1:onShow()
	local function update(dTime)
		g_i3k_ui_mgr:InvokeUIFunction(eUIID_TalkPop1, "updateTimer", dTime)
	end
		
	self._timer = cc.Director:getInstance():getScheduler():scheduleScriptFunc(update, 0.01, false)
end

function wnd_talk_pop1:refresh()
	
end

function wnd_talk_pop1:treasureDialogue(isTrue, entity, text, nextEntity, nextText, callback)
	self.entity = entity
	self._isTrue = isTrue
...
function wnd_talk_pop1:onUpdate(dTime)
	
end


function wnd_create(layout, ...)
	local wnd = wnd_talk_pop1.new()
	wnd:create(layout, ...)
	return wnd;
end