...
wnd_homelandMap = i3k_class("wnd_homelandMap", ui.wnd_MapBase)

local g_mapSize = nil
function wnd_homelandMap:ctor()
...
function wnd_homelandMap:configure()
	local widgets = self._layout.vars
	widgets.closeBtn:onClick(self, self.onCloseUI)
end

function wnd_homelandMap:onShow()
	i3k_sbean.query_homeland_members()
end

function wnd_homelandMap:onHide()
	releaseSchedule()
end

function wnd_homelandMap:refresh(mapId, cfg)
...
function wnd_homelandMap:rightMonsterList(mapId, size)
...
function wnd_homelandMap:walkToPos(sender, needValue)
...
function wnd_homelandMap:transToPos(sender, needValue)
...
function wnd_homelandMap:searchPath(sender, needValue)
	local size = needValue.size
	local world = i3k_game_get_world()
	local nowMapId = world._cfg.id
	local targetMapId = needValue.mapId
...
function wnd_homelandMap:toTargetPos(mapId, pos, size, needValue, taskType)
	local hero = i3k_game_get_player_hero()
...
-- function wnd_homelandMap:openOrPickup(sender, needValue)
-- 	if needValue.isNpc then
...
function wnd_homelandMap:close()
	releaseSchedule()
	g_i3k_ui_mgr:CloseUI(eUIID_SceneMap)
end

function wnd_homelandMap:onUpdate(dTime)
	...
	self._updateTeam = self._updateTeam + dTime
	if self._updateTeam >= 1 then
		self._updateTeam = 0
		i3k_sbean.query_homeland_members()
	end
...
function wnd_homelandMap:updateTeammatePos(data)
	updateTeammatePos(data)
end

function wnd_create(layout, ...)
	local wnd = wnd_homelandMap.new();
	wnd:create(layout, ...);
	return wnd;
end
