...
local welfareTreasure = 34 --热江福利宝
local vitalityFull 		= 35 --热江福利宝
...
--月份
local monthNumber = {10920,10924,10925,10926,10927,10928,10929,10930,10931,10921,10922,10923,}
--Vip
...
local lv3BtnColor = "ff977c69" -- 151, 124, 105
local lv3BtnChooseColor = "ff914a15" -- 145, 74, 21


-- 非活动type, 特殊活动type
local noType = -1 -- 无对应活动
local signInType = -2 -- 签到（特殊）
local vitType = -3 -- 每日体力（特殊）
local lineType = -4 -- 在线奖励（特殊）
local dailyActType = -5 -- 每日活动
local presentType = -6 -- 江湖好礼
local welfareType = -7 -- 超值福利
local levelGiftType = -8 -- 等级礼包
local phoneRewardType = -9 -- 手机绑定
local hotBloodFundType = -10 -- 热血基金

local unKonwType = -11 -- 不清楚类型
...
function wnd_fu_li:ctor()
	...
	activitySyncTbl =
	{
		...
		[welfareTreasure] 	= 	{sync = i3k_sbean.sync_activities_welfaretreasure,	index = 0,	sort = 11},
		[vitalityFull] 		= 	{sync = i3k_sbean.newdailyonlinegift_sync,			index = 0,	sort = 12},

	}

	-- 活动内容排布顺序(有得活动类型文档中没有，但是程序中有)
	actStructTbl = 
	{
		{type = signInType}, -- 签到
		{type = vitType}, -- 每日体力
		{type = first_pay_gift}, -- 首次充值送礼
		{type = direct_purchase},
		{type = dailyActType, childType = {logingift_gift, vitalityFull, lineType, schedule_gift}},-- 每日活动
		{type = presentType, childType = {uograde_gift, gift_package, advertisement, phoneRewardType, extra_drop, exchange_gift--[[掉落兑换]] --[[手机绑定]]}}, -- 江湖好礼
		{type = hotBloodFundType, childType = {welfareTreasure, growth_fund, cycle_fund}}, -- 热血基金 
		{type = welfareType, childType = {giveMeRedPacket, buy_get, double_drop, inheritDivinework, useItems_reward--[[节日祝福]], backRoleDoubleDrop, battle_enjoy}}, -- 超级福利
		{type = levelGiftType, childType = {up_level--[[强化礼包]]}},
		{type = pay_gift},
		{type = consume_gift},
		...
		{type = lucky_gift},
		{type = share_gift},
		{type = more_role_discount},
		{type = oppoActivity},
		-- {type = welfareTreasure},
		{type = investment_fund},
		{type = charge_every_day},
		-- {type = unKonwType,    childType = {pay_gift, consume_gift, charge_every_day, charge_continuity, battle_enjoy, mobile_gift, one_arm_bandit, pay_rank, consume_rank, lucky_gift, share_gift, more_role_discount, oppoActivity, welfareTreasure, }},
	}

	-- 一级列表对应图标ID
	level1ActiveIconId = {
		[first_pay_gift] = {10965, 10966},
		[signInType] = {10935, 10936},
		[lineType] = {10933, 10934},
		[direct_purchase] = {10967, 10968},
		[vitType] = {10937, 10938},
		[dailyActType] = {10939, 10940},
		[presentType] = {10941, 10942},
		[welfareType] = {10947, 10948},
		[levelGiftType] = {10945, 10946},
		[hotBloodFundType] = {10943, 10944},
	}
...
function wnd_fu_li:refresh(info,checkinGift,dailyOnlineGift,monthlyCardReward,dailyVitReward, actName, bindPhoneReward, jumpId)
	self:updateActivitiesList(info,checkinGift,dailyOnlineGift,monthlyCardReward,dailyVitReward, actName, bindPhoneReward, jumpId)
end
...
function wnd_fu_li:updateActivitiesList(acts,checkinGift,dailyOnlineGift,monthGift,vitGift, actName, bindPhoneReward, jumpId)

	-- 特殊活动表 item
	self.specialActive = {
		[signInType]		= {title = "签到",		clickFun = self.onSingInbtn, 		 notice = checkinGift, 	      syncFuc = i3k_sbean.checkin_sync },
		[lineType]			= {title = "在线奖励",	clickFun = self.onLineBtn, 			notice = dailyOnlineGift, 	syncFuc = i3k_sbean.sync_activities_onlinegift },
		[vitType]			= {title = "每日体力",	clickFun = self.onVit, 				notice = vitGift, 			syncFuc = function() self:updateVitInfo()  end },
		[dailyActType]		= {title = "每日活动",	clickFun = nil, 		 notice = 0, 	      syncFuc = self.level1BtnClick },
		[presentType]		= {title = "江湖好礼",	clickFun = nil, 		 notice = 0, 	      syncFuc = self.level1BtnClick },
		[welfareType]		= {title = "超值福利",	clickFun = nil, 		 notice = 0, 	      syncFuc = self.level1BtnClick },
		[levelGiftType]		= {title = "等级礼包",	clickFun = nil, 		 notice = 0, 	      syncFuc = self.level1BtnClick },
		[hotBloodFundType]	= {title = "热血基金",	clickFun = nil, 		 notice = 0, 	      syncFuc = self.level1BtnClick },
		-- [unKonwType]		= {title = "不清楚类型",clickFun = nil, 		 notice = 0, 	      syncFuc = self.level1BtnClick },
	}
	-- 添加手机绑定
	if bindPhoneReward == 1 then
		self.specialActive[phoneRewardType] = {
			title = "手机绑定",
			clickFun = self.onPhoneRewardbtn,
			notice = false,
			syncFuc = i3k_sbean.phone_reward_sync,
		}
	end
	...
	for i = 1 , #self.actStructHasTbl  do
		...
		-- self._redPoinShow[infoType] = info.notice == 1
		if infoType == vitType then
			local need_lvl = self:getCurrentVitInfo()
			if need_lvl > g_i3k_game_context:GetLevel() then
				-- item.vars.red_point:hide()
				self.specialActive[vitType].notice = 0
			end
		end
		if ((actName == nil or actName == "") and (info.notice == 1 and hadReq == false and not isFirstPayUI)) or (jumpId and jumpId == i) then
			self._current_level1_bt_num = i
			self._before_level1_bt_num = i
			item.vars.select_icon:show()
			item.vars.bt:stateToPressed()

			if infoType == signInType then
				self:setSignInBg(true)
			else
				self:setSignInBg(false)
			end
		...
		if hadReq == false then
			if isFirstPayUI and info.atype == first_pay_gift or (info.notice == 1 and hadReq == false) then
				
				local hasTitle = false
				local chilPos = nil
				if actName == nil or actName == "" then
					hasTitle = true
				end
				local childTbl = tblV.childType
				if childTbl then
					for childTblIndex = 1, #childTbl do
						local childInfoType = childTbl[childTblIndex].type
						local childInfoId = childTbl[childTblIndex].id
						local childInfo = childInfoType > 0 and self.activeListTable[childInfoType][childInfoId] or self.specialActive[childInfoType]
						if childInfo.title == actName then
							hasTitle = true
							chilPos = childTblIndex
						end
					end
				else
					if actName == info.title then
						hasTitle = true
					end
				end

				if hasTitle then  --如果找不到对应活动则默认显示签到
					self._current_level1_bt_num = i
					self._before_level1_bt_num = i
	...
	if hadReq == false then
		...
		self._current_level1_bt_num = 1
		self._before_level1_bt_num = 1
		item.vars.select_icon:show()
		item.vars.bt:stateToPressed()

		self:setSignInBg(true)

		self:setLv1BtnIcon(self.actStructHasTbl[1].type, item, true)

		self.specialActive[signInType].syncFuc()
	end
	if tmp_index > 0 then
...
function wnd_fu_li:showFuliRedPointByAtype(atype, visible)
	...
	if atype > 0 then
		local tbl = self.activeListTable[atype]
		...
		for k,v in pairs(tbl) do
			v.notice = visible and 1 or 0

			local pos = self:getItemPosByTypeId(v.atype, v.id)
			if pos ~= nil then
				local pos1 = pos.pos1
				local pos2 = pos.pos2
				if pos2 then
					self._redPoints.level2[pos2]:setVisible(visible)
					...
					self._redPoints.level1[pos1]:setVisible(visible)
				end
			end
		end
	else
		self.specialActive[atype].notice = visible and 1 or 0
		local pos = self:getItemPosByTypeId(atype)
		if pos ~= nil then
			local pos1 = pos.pos1
			local pos2 = pos.pos2
			if pos2 then
...
function wnd_fu_li:changeContentSize(control)
	...
	local size = nil -- self._layout.vars.RightView:getContentSize()
...
function wnd_fu_li:updateRightView(control)
	...
	local rightView = self._layout.vars.RightView
	local rightView2 = self._layout.vars.RightView2

	local AddChild = rightView:getAddChild()
	for i,v in ipairs (AddChild) do
		rightView:removeChild(v)
	end
	local AddChild2 = rightView2:getAddChild()
	for i,v in ipairs (AddChild2) do
		rightView2:removeChild(v)
	end

	local childTypeTbl = self.actStructHasTbl[self._current_level1_bt_num].childType
	self:showLevel2List()
	if childTypeTbl then
		rightView2:addChild(control)
	else
		rightView:addChild(control)
	end
end
...
------------------------------二级界面common------begin-----------------------------
-- 设置签到背景图
function wnd_fu_li:setSignInBg(isShow)
	self._layout.vars.signinBg:setVisible(isShow)
end

-- 二级界面显示
-- 显示个数和名字
function wnd_fu_li:showLevel2List()
	local activitiesList2 = self._layout.vars.ActivitiesList2
	local childTypeTbl = self.actStructHasTbl[self._current_level1_bt_num].childType
	if childTypeTbl then
...
-- 二级界面显示选中框
function wnd_fu_li:showLevel2ListChoose(num)
	for i = 1, #self._level2List do
		local item = self._level2List[i]
		if i == num then
...
-- 二级界面按钮事件
-- 输入按钮编号
function wnd_fu_li:level2BtnClick(btnNum)
	g_i3k_game_context:SetDynamicActivityRedPointInfo(0)
...
-- 二级界面按钮注册事件
function wnd_fu_li:level2BtnClickFunc(sender,btnNum)
	self:level2BtnClick(btnNum)
end

-- 拥有二级界面的一级界面事件
function wnd_fu_li:level1BtnClick(num)
	self._layout.vars.ActivitiesList2:jumpToChildWithIndex(0)
	if num == nil then
...
-- 一级界面注册事件
function wnd_fu_li:level1BtnClickFunc(sender, num, level2Num)
...
-- 刷新带有子分页的红点信息
-- 一级按钮位置
function wnd_fu_li:refreshLevel1RedP(num)
...
-- 根据type id 返回位置
function wnd_fu_li:getItemPosByTypeId(type, id)
	if type < 0 then
...
-- 设置一级列表左侧图标
function wnd_fu_li:setLv1BtnIcon(type, item, visible)
...
function wnd_fu_li:updateSingInInfo(finishedDays,checkinId,monthCfg,canCheckIn, additional)
	...
	local qianDaoUI = require("ui/widgets/yyhdt7")()
	...
	local reason = qianDaoUI.vars.reason
	...
	if extraSign then
		...
		qianDaoUI.vars.item_suo:setVisible(g_i3k_db.i3k_db_get_reward_lock_visible(extraSign.itemId))	--jiasuo
	...
	self:refMonthNum(month,monthCfg, reason)
...
function wnd_fu_li:refMonthNum(month,monthCfg, reason)--刷新几月份签到
	...
	local monthNum = monthNumber[cfgMonth]
	local reasonNum = math.floor( (monthNum + 9) %12 / 3) + 1 -- monthToReason[monthNum]
	local reasonId = reasonImageId[reasonNum]
	reason:setImage(g_i3k_db.i3k_db_get_icon_path(reasonId))
end
...
function wnd_fu_li:hideQianDaoRedPoint()
	self.specialActive[signInType].notice = 0
	self:updateQianDaoRedPoint()
end

function wnd_fu_li:updateQianDaoRedPoint()
	local state = self.specialActive[signInType].notice == 1 or self.isMonthShow or self.isXiaoyaoShow
	
	local pos = self:getItemPosByTypeId(signInType)
	if pos ~= nil then
		local pos1 = pos.pos1
		local pos2 = pos.pos2
		if pos2 then
	...
	if self.specialActive[signInType].notice and not state then
...
function wnd_fu_li:updateOnLineInfo(info,index)
	local onLineUI = require("ui/widgets/yyhdt13")()
...
function wnd_fu_li:appendOnlineGiftLevelItem(onLineUI,gifts, keepTime, dayOnlineTime,reward,id)
	local PayGiftLevelWidgets = require("ui/widgets/yyhdt13t")()
...
function wnd_fu_li:updateOnLineRedPoint(state)
	if self.specialActive[lineType].notice and not state then
		g_i3k_game_context:RemoveFuliRedPointCount(1)
		self.specialActive[lineType].notice = 0
	end

	local pos = self:getItemPosByTypeId(lineType)
	if pos ~= nil then
		local pos1 = pos.pos1
		local pos2 = pos.pos2
		if pos2 then
...
function wnd_fu_li:updateVitInfo(is_award)
	if is_award then
		self.specialActive[vitType].notice = is_award
	end
	local vitUI = require("ui/widgets/yyhdt10")()
...
function wnd_fu_li:updateVitRedPoint(state)
	if self.specialActive[vitType].notice and not state then
		g_i3k_game_context:RemoveFuliRedPointCount(1)
		self.specialActive[vitType].notice = false
	end

	local pos = self:getItemPosByTypeId(vitType)
	if pos ~= nil then
		local pos1 = pos.pos1
		local pos2 = pos.pos2
		if pos2 then
...
function wnd_fu_li:updateVitIems(vitLayer)
	...
	if not isInVitTime then
		...
		if self.specialActive[vitType].notice == 1 then
	...
	for i = 1, 3 do
		...
		local tmp_suo = string.format("itemsuo%s",i)
		local itemBg = vitLayer.vars[tmp_bg]
		local itemSuo = vitLayer.vars[tmp_suo]
		if item_cfg and item_cfg[1] and item_cfg[1] ~= 0 and item_cfg[2] and item_cfg[2] ~= 0 then
			...
			itemSuo:setVisible(g_i3k_db.i3k_db_get_reward_lock_visible(item_cfg[1]))	--jiasuo
	...
	if need_lvl > g_i3k_game_context:GetLevel() then
		...
		self:updateVitRedPoint(self.specialActive[vitType].notice == 1)
	end
end
...
function wnd_fu_li:updateAdvertisement(iconID, content, htmlType)

	if not i3k_db_activity_imgs[iconID] then
		return
	end
	local ui = require("ui/widgets/yyhdt6")()
	self:updateRightView(ui)
	self:changeContentSize(ui)
	local imgID = i3k_db_activity_imgs[iconID]
	ui.vars.img:setImage(g_i3k_db.i3k_db_get_icon_path(imgID))
	if content == "" or htmlType == g_ADVERTISEMENT_NONE then
		ui.vars.buyBtn:hide()
	else
		ui.vars.buyBtn:show()
		ui.vars.buyBtn:onClick(self, self.onJingDongShopBtn, {content = content, htmlType = htmlType})
	end
end

function wnd_fu_li:onJingDongShopBtn(sender, info)
	if info.htmlType == g_ADVERTISEMENT_URL then
		local url = info.content
		i3k_open_url(url)
	elseif info.htmlType == g_ADVERTISEMENT_SDK then
		i3k_open_sdk_activity(info.content)
	end
end
...
function wnd_fu_li:updateMobileGiftInfo(item,info,actType,actId,index)
	...
	item.vars.fas:setText(string.format("已累计充值%d%s",#payDay,"天"))
	if need_pay > 0 then
		item.vars.GoalContent2:setText(string.format("每天充值%d%s",need_pay,"元宝即可领取奖励"))
	end
end
				...
				for j = 1 , #_gifts do
					...
					for k = 1 , 4 do
						...
						if ___info then
							...
							temp.vars["item_suo"..k]:setVisible(g_i3k_db.i3k_db_get_reward_lock_visible(___info.id))	--jiasuo
...
function wnd_fu_li:updateConsumeGiftLevelItem(item, effectiveTime, id, consume, gifts, needConsume,reward,actType,control)
	...
	local content = string.format("消费%s元宝可获得",needConsume)
...
function wnd_fu_li:updateGradeGiftInfo(actType, actId,effectiveTime, cfg, log,Index)
	local needValue = {actType = actType , actId = actId }
	local gradeGiftUI = require("ui/widgets/yyhdt16")()
...
function wnd_fu_li:appendGradeGiftLevelItem(control, effectiveTime, id, gift, needLevel,rewards,giftEx,actType,cfg,i)
	local GradeGiftwidgets = require("ui/widgets/yyhdt16t")()
...
function wnd_fu_li:updateGradeGiftLevelItem(item,effectiveTime, id, gift, needLevel,rewards ,giftEx,control,actType,time,limitedTime,auto_index)
	...
	local content = i3k_get_string(19349, needLevel)-- string.format("达到%s级可以领取",needLevel)
	item.vars.GoalContent:setText(content)
...
function wnd_fu_li:updateInvestmentfundGiftInfo(actType, actId,effectiveTime, cfg, log,Index)
	local needValue = {actType = actType , actId = actId }
	local investmentfundUI = require("ui/widgets/yyhdt11")()
	...
	self:excessTime(investmentfundUI,cfg.time)
...
function wnd_fu_li:appendInvestmentfundLevelItem(control, effectiveTime, id,  levelReq, rewards, day, count,buyTime, buyEndTime,actType,cfg)
	local Investmentfundwidgets = require("ui/widgets/yyhdt11t")()
...
function wnd_fu_li:updateGrowthfundGiftInfo(actType, actId,effectiveTime, cfg, log,Index)
	local needValue = {actType = actType , actId = actId }
	local growthfundUI = require("ui/widgets/yyhdt18")()
	...
	self:excessTime(growthfundUI,cfg.time)
...
-- 跳转到充值
function wnd_fu_li:goPay(sender)
...
function wnd_fu_li:updateGrowthfundBuyInfo(control, effectiveTime, log,cfg,actType)
	...
	control.vars.goBuyBtn:onClick(self, self.goPay) -- 跳转到充值
end
...
function wnd_fu_li:appendGrowthfundLevelItem(control, effectiveTime, id, levelReq, rewards, count, buyCount,actType,cfg)
	local Growthfundwidgets = require("ui/widgets/yyhdt18t")()
...
function wnd_fu_li:updateGrowthfundLevelItem(item, effectiveTime, id,  levelReq, rewards, count, buyCount,actType,control,cfg)
	...
	local content = i3k_get_string(19349, levelReq) -- string.format("等级达到%d级返利", levelReq)
	-- content = g_i3k_make_color_string(content,g_i3k_get_blue_color() )
...
-------龙魂币基金
function wnd_fu_li:updateWelfareTreasureInfo(actType, actId, effectiveTime, cfg, log, Index)
	local needValue = {actType = actType , actId = actId,}
	local growthfundUI = require("ui/widgets/yyhdt11")()
	self:updateRightView(growthfundUI)
	self:changeContentSize(growthfundUI)
	self:updateCommonGiftMainInfo(growthfundUI, cfg.time, cfg.title, cfg.content)
	self:updateWelfareTreasureBuyInfo(growthfundUI, effectiveTime, log, cfg, actType)
	self:updateWelfareTreasureLevelsInfo(growthfundUI, effectiveTime, cfg, log, actType, Index)
	self:setLeftRedPoint(actType, actId)
end
function wnd_fu_li:updateWelfareTreasureBuyInfo(control, effectiveTime, log, cfg, actType)
	if log.isbuy > 0 then
		control.vars.buyBtnText:setText("已购买")
		control.vars.buyBtn:disableWithChildren()
		control.vars.cztz:disableWithChildren()
	else
		local growthfundBuy = {
			actType = actType, 
			index = log.id, 
			...
			item = control, 
			head = cfg.title, 
			buyNeedId = cfg.costItemID, 
			buyNeedPrice = cfg.costItemCnt, 
			needLevel = cfg.levelNeed,
			needVipLevel = cfg.vipLevelNeed, 
		}
		control.vars.buyBtnText:setText("购买")
		control.vars.buyBtn:onClick(self, self.onBuyWelfareTreasure, growthfundBuy)
		local fun = (function(ok)
			if ok then
				if cfg.costItemID == g_BASE_ITEM_DIAMOND then
					g_i3k_logic:OpenChannelPayUI()
				elseif cfg.costItemID == g_BASE_ITEM_DRAGON_COIN then
					g_i3k_logic:OpenChannelPayUI(nil, g_CHANNEL_LONGHUNBI_TYPE)
				end
			end
		end)
		control.vars.cztz:onClick(self, fun)
	end
end
function wnd_fu_li:updateWelfareTreasureLevelsInfo(control, effectiveTime, cfg, log, actType, Index)
	local GrowthfundGiftList = control.vars.fundGiftList
	GrowthfundGiftList:removeAllChildren()
	for i, v in ipairs(cfg.returns) do
		self:appendWelfareTreasureLevelItem(control, effectiveTime, log.id, v.items, v.levelReq, log.rewards[v.levelReq], actType, cfg, i, log.isbuy)
	end
	local Index = activitySyncTbl[actType].index
	GrowthfundGiftList:jumpToListPercent(Index)
end
function wnd_fu_li:appendWelfareTreasureLevelItem(control, effectiveTime, id, rewards, needLevel, isGet, actType, cfg, i, isbuy)
	local Growthfundwidgets = require("ui/widgets/yyhdt11t")()
	self:updateWelfareTreasureLevelItem(Growthfundwidgets, effectiveTime, id, rewards, needLevel, isGet, control, actType, cfg.time, i, isbuy)
	control.vars.fundGiftList:addItem(Growthfundwidgets)
end
function wnd_fu_li:updateWelfareTreasureLevelItem(item, effectiveTime, id, gifts, needLevel, isGet, control, actType, time, auto_index, isbuy)
	local content = i3k_get_string(19349, needLevel) -- string.format("达到%s级可以领取", needLevel)
	item.vars.GoalContent:setText(content)
	item.vars.GoalContent:setTextColor(lv3BtnColor)
	local nowLevel = g_i3k_game_context:GetLevel()
	local goal = string.format("(%d/%d)", nowLevel, needLevel)
	item.vars.Count:setText(goal)
	item.vars.root_bg:setImage(g_i3k_db.i3k_db_get_icon_path(2423))
	item.vars.GetBtnText:setText(i3k_get_string(18321))
	for k, v in ipairs(gifts) do
		item.vars["item_bg"..k]:show()
		item.vars["item_bg"..k]:setImage(g_i3k_db.i3k_db_get_common_item_rank_frame_icon_path(v.id))
		item.vars["item_icon"..k]:setImage(g_i3k_db.i3k_db_get_common_item_icon_path(v.id,i3k_game_context:IsFemaleRole()))
		if v.count > 0 then
			item.vars["item_count"..k]:setText("x"..v.count)
		else
			item.vars["count_bg"..k]:hide()
			item.vars["item_count"..k]:hide()
		end
		item.vars["item_icon"..k]:onClick(self, self.onTips, v.id)
		item.vars["item_suo"..k]:setVisible(g_i3k_db.i3k_db_get_reward_lock_visible(v.id))	--jiasuo
	end
	item.vars.GetBtn:disableWithChildren()
	item.vars.GetBtnText:setText(i3k_get_string(18321))
	...
	if isbuy > 0 then
		if isGet then
			item.vars.GetBtn:disableWithChildren()
			item.vars.GetBtnText:setText(i3k_get_string(18320))
		else
			local TakeGradeGift = {
				Time = effectiveTime, 
				index = id , 
			...
			item.vars.GetBtn:onClick(self, self.onTakeGradeGiftReward, TakeGradeGift)
			item.vars.GetBtn:enableWithChildren()
			item.vars.GoalContent:setTextColor(lv3BtnChooseColor)
...
function wnd_fu_li:onBuyWelfareTreasure(sender, needValue)
	self._percent = needValue.actType
	self._actID = needValue.index
	local roleLvl = g_i3k_game_context:GetLevel()
	local roleVipLvl = g_i3k_game_context:GetPracticalVipLevel()
	if roleLvl < needValue.needLevel then
		local msg = i3k_get_string(19082, needValue.needLevel)
		g_i3k_ui_mgr:ShowMessageBox1(msg)
	elseif roleVipLvl < needValue.needVipLevel then
		local msg = i3k_get_string(19083, needValue.needVipLevel)
		g_i3k_ui_mgr:ShowMessageBox1(msg)
	else
		local name = g_i3k_db.i3k_db_get_common_item_name(needValue.buyNeedId)
		local function callback(isOk)
			if isOk then
				local hadCount = g_i3k_game_context:GetBaseItemCanUseCount(needValue.buyNeedId)
				if hadCount >= needValue.buyNeedPrice then
					i3k_sbean.activities_welfaretreasure_buy(needValue.Time, needValue.index, needValue.actType, needValue.buyNeedId, needValue.buyNeedPrice)
				else
					g_i3k_ui_mgr:PopupTipMessage(i3k_get_string(19081, name))
				end
			end
		end
		local desc = i3k_get_string(19080, needValue.buyNeedPrice, name)
...
function wnd_fu_li:updateDoubleDropInfo(actType, actId,time, content, title,Index)
	local needValue = {actType = actType , actId = actId }
	local doubleDropUI = require("ui/widgets/yyhdt22")()
...
function wnd_fu_li:updateExtraDropInfo(actType, actId,info,index)
	local needValue = {actType = actType , actId = actId }
	local extraDropUI = require("ui/widgets/yyhdt3")()
...
function wnd_fu_li:appendExtraDropLevelItem(control, id )
	local ExtraDropwidgets= require("ui/widgets/yyhdt3t")()
...
function wnd_fu_li:updateExchangeGiftInfo(actType, actId,effectiveTime, cfg, log,Index)
	local needValue = {actType = actType , actId = actId }
	local exchangeGiftUI = require("ui/widgets/yyhdt15")()
...
function wnd_fu_li:appendExchangeGiftLevelItem(control, effectiveTime,items, gift,seq, maxExchange, exchangeCount, id ,actType)
	local exchangeGiftwidgets= require("ui/widgets/yyhdt15t")()
...
function wnd_fu_li:updateLoginGiftInfo(actType, actId,effectiveTime, cfg, log,Index)
	local needValue = {actType = actType , actId = actId }
	local loginGiftUI = require("ui/widgets/yyhdt12")()
	...
	self:excessTime(loginGiftUI,cfg.time)
...
function wnd_fu_li:appendloginGiftLevelItem(control, effectiveTime, dayReq,gift,id,loginDays,rewards,lastLoginTime,actType,log,time,auto_index)
	local loginGiftwidgets = require("ui/widgets/yyhdt12t")()
...
function wnd_fu_li:updateGiftPackageInfo(actType, actId,effectiveTime, Index)
	local needValue = {actType = actType , actId = actId }
	local giftPackageUI = require("ui/widgets/yyhdt4")()
	...
	if content then
		if content == "" then
			...
			if self:checkKey(content) then
				...
				g_i3k_ui_mgr:PopupTipMessage("请输入正确的数字和字母的组合")
			end
		end
	end
end
...
function wnd_fu_li:onTakeGradeGiftReward(sender, needValue)
	...
	if isEnough then
		if needValue.actType == first_pay_gift then
			i3k_sbean.activities_firstpaygift_take(needValue.Time,needValue.index,gift,index,needValue.actType)
		elseif needValue.actType == uograde_gift then
			local percent = needValue.control.vars.GradeGiftList:getListPercent()
			activitySyncTbl[needValue.actType].index = needValue.control.vars.GradeGiftList:getListPercent()
			i3k_sbean.activities_gradegift_take(needValue.Time,needValue.index,needValue.level,needValue.actType,gift,percent,index)
		elseif needValue.actType == welfareTreasure then
			i3k_sbean.activities_welfaretreasure_take(needValue.Time, needValue.index, needValue.level, needValue.actType, needValue.gifts)
		end
	else
		g_i3k_ui_mgr:PopupTipMessage("背包空间不足")
	end
...
function wnd_fu_li:updateUpgradepurchase(actType, actId,info)
	local MRCZSL = require("ui/widgets/yyhdt26")()
	g_i3k_game_context:SetDynamicActivityRedPointInfo(0)
...
function wnd_fu_li:updateUpgradepurchaseInfo(item,info1,actType,actId)
	...
	for i,v in ipairs(gifts) do
		local temp = require("ui/widgets/yyhdt26t")()
		item.vars.GradeGiftList:addItem(temp)
	...
	item.vars.suo:setVisible(false)		--jiasuo
	...
	local callback = function(is_ok)
		if is_ok then
			if g_i3k_game_context:GetDiamond(true) < minfo.price then
				local tips = string.format("%s", "元宝不足，请充值后购买")
...
function wnd_fu_li:updateCycleFundGiftInfo(actType, actId, effectiveTime, cfg, log)
	local cycleFundUI = require("ui/widgets/yyhdt19")()
...
function wnd_fu_li:appendCycleFundLevelItem(control, effectiveTime, cfg, log, actType, seq, fundReturn)
	local cycleFundWidgets = require("ui/widgets/yyhdt19t")()
...
function wnd_fu_li:updateCycleFundLevelItem(item, control, effectiveTime, cfg, log, actType, seq, fundReturn)
	item.vars.GoalContent:setText(i3k_get_string(19350, seq))
	item.vars.GoalContent:setTextColor(lv3BtnColor)
...
function wnd_fu_li:updateUseItemsRewardInfo(actType, actId, effectiveTime, cfg, log)
	local useItemsRewardUI = require("ui/widgets/yyhdt24")()
...
function wnd_fu_li:updateUseItemsInfo(control, actType, actId, effectiveTime, cfg, log)
	...
	control.vars.item_suo:setVisible(g_i3k_db.i3k_db_get_reward_lock_visible(cfg.levels[1].uid))	--jiasuo
...
function wnd_fu_li:updateUseItemsScrollInfo(control, actType, actId, effectiveTime, cfg, log)
	...
	for i, v in ipairs(cfg.levels) do
		local rewardInfoUI = require("ui/widgets/yyhdt24t")()
		if #v.gifts == 1 then
...
function wnd_fu_li:updataBuyItemGetItem(extraGift)
	local control = require("ui/widgets/yyhdt21")()
	...
	self:excessTime(control,extraGift.time)
end
...
function wnd_fu_li:showBuyGetInfo(control, gifts)
	...
	for k, v in ipairs(giftList) do
		local widget = require("ui/widgets/yyhdt21t")()
...
function wnd_fu_li:updateRedPack(actType, actId, effectiveTime, cfg, log)
	local redPackUI = require("ui/widgets/yyhdt20")()
...
function wnd_fu_li:updateRedPackInfo(control, effectiveTime, cfg, log, actType, actId)
	...
	-- local allBars = redPackList:addChildWithCount("ui/widgets/yyhdt20t", 1, redPackCnt)
	local allBars = {}
	for addIndex = 1, redPackCnt do
		local item = require("ui/widgets/yyhdt20t")()
		redPackList:addItem(item)
		table.insert( allBars, item )
	end
...
function wnd_fu_li:updateMobileBindInfo(lastTime, phoneNumber)
	local bindUI = require("ui/widgets/yyhdt5")()
...
function wnd_fu_li:updateBindRewardScroll(bindUI)
	...
	for _, v in ipairs(i3k_db_common.mobileBindReward) do
		if v.id ~=0 and v.count > 0 then
			local widget = require("ui/widgets/yyhdt5t")()
...
function wnd_fu_li:onPhoneRewardbtn(sender,item)
...
function wnd_fu_li:updateScheduleGift(actType, actId, info)
	local ui = require("ui/widgets/yyhdt14")()
	self:updateRightView(ui)
	self:changeContentSize(ui)
	--self:updateCommonGiftMainInfo2(ui, info.cfg.time, info.cfg.content)  --时间，活动内容
	self:excessTime(ui, info.cfg.time)
...
function wnd_fu_li:appendScheduleGiftItem(control, actId, actType, info, needSchdule, gifts, auto_index)
	local scheduleGiftwidgets= require("ui/widgets/yyhdt14t")()
...
function wnd_fu_li:updateInheritDivinework(actId, actType, info)
	local CHOUBIN = require("ui/widgets/yyhdt23")()
...
function wnd_fu_li:updateBackRoleDoubleDrop(actId, actType, cfg)
	local widget = require("ui/widgets/yyhdt25")()
...
--------------------	元气礼包	--------------------
function wnd_fu_li:updateYuanQiLiBaoInfo(info)
	g_i3k_game_context:SetDynamicActivityRedPointInfo(0)
	local vitalityGiftUI = require("ui/widgets/yyhdt2")()
	self:updateRightView(vitalityGiftUI)
	self:changeContentSize(vitalityGiftUI)

	-- self:updateCommonGiftMainInfo2(vitalityGiftUI, info.time, info.title, info.content)
	self:updateVitalityFull(vitalityGiftUI, info)
	
	--更新左侧以及主界面红点
	self:setLeftRedPoint(vitalityFull, info.id)
end
function wnd_fu_li:updateVitalityFull(vitalityGiftUI, info)
	local value = info.curr
	local maxValue = info.max
	local percent = value / maxValue * 100
	local showRewardsIndex 
	
	vitalityGiftUI.vars.activeValue:setText(value)
	-- vitalityGiftUI.vars.activePercent:setPercent(percent)
	-- vitalityGiftUI.vars.activePercentText:setText(value.."/"..maxValue)
	vitalityGiftUI.levelCount = #info.levels

	local timeShowText = g_i3k_get_ActDateRange(info.time.startTime, info.time.endTime)
	vitalityGiftUI.vars.time:setText(timeShowText)

	local rewardList = vitalityGiftUI.vars.rewardList
	rewardList:removeAllChildren()
	for k,v in ipairs(info.levels) do
		local isOpen = info.got[v.need]
		local isCanOpen = (not isOpen) and value >= v.need
        -- vitalityGiftUI.vars['value'..k]:setText(v.need)
        -- vitalityGiftUI.vars['box'..k]:setVisible(not isOpen)
        -- vitalityGiftUI.vars['box_used'..k]:setVisible(isOpen)
		-- vitalityGiftUI.vars['box_btn'..k]:onClick(self, self.RefreshVitalityGift, {UI = vitalityGiftUI, index = k, need = v.need, rewards = v.gifts, tag = v.need, isOpen= isOpen, isCanOpen = (not isOpen) and value >= v.need})
        if not isOpen and value >= v.need then 
			g_i3k_game_context:SetDynamicActivityRedPointInfo(1)
			if not showRewardsIndex then
				showRewardsIndex = {UI = vitalityGiftUI, index = k, need = v.need, rewards = v.gifts, tag = v.need, isOpen= isOpen, isCanOpen = true}
			end
			-- vitalityGiftUI.anis['c_bx'..(k + 1)]:play()
        else
            -- vitalityGiftUI.anis['c_bx'..(k + 1)]:stop()
		end

		local item = require("ui/widgets/yyhdt2t")()
		rewardList:addItem(item)
		item.vars.tblType1:setVisible(true)
		item.vars.tblType2:setVisible(false)
		item.vars.tblType3:setVisible(false)
		local textShow = i3k_get_string(19346, v.need)
		item.vars.tbl1Text:setText(textShow)

		if isOpen then
			item.vars.tbl1Btn:setVisible(false)
			item.vars.tbl1Has:setVisible(true)
			item.vars.tabl1Whole:setVisible(false)
		elseif isCanOpen then
			item.vars.tbl1Btn:setVisible(true)
			item.vars.tbl1Has:setVisible(false)
			item.vars.tbl1Btn:enable()
			item.vars.tabl1Whole:setVisible(true)
		else
			item.vars.tbl1Btn:setVisible(true)
			item.vars.tbl1Has:setVisible(false)
			item.vars.tbl1Btn:disable()
			item.vars.tabl1Whole:setVisible(false)
		end

		item.vars.tbl1Btn:onClick(self,self.OnGetGiftRewards, { rewards = v.gifts, need = v.need})

		local rewards = v.gifts
		local rewardShowLen = 4
		for i = 1, rewardShowLen do
			local info = rewards[i]
			if i <= #rewards then
				item.vars["tbl1Item" .. i .. "Bg"]:setVisible(true)
				item.vars["tbl1Item" .. i .. "Bg"]:setImage(g_i3k_db.i3k_db_get_common_item_rank_frame_icon_path(info.id))
				item.vars["tbl1Item" .. i .. "Icon"]:setImage(g_i3k_db.i3k_db_get_common_item_icon_path(info.id))
				item.vars["tbl1Item" .. i .. "Suo"]:setVisible(info.id > 0)
				item.vars["tbl1Item" .. i .. "Count"]:setText("x".. info.count)
				item.vars["tbl1Item" .. i .. "Click"]:onClick(self, self.onTips, info.id)
			else
				item.vars["tbl1Item" .. i .. "Bg"]:setVisible(false)
			end
		end
...
function wnd_fu_li:OnGetGiftRewards(sender, data)
	i3k_sbean.newdailyonlinegift_take(data.need, data.rewards)
end
--------------------	元气礼包 END--------------------
