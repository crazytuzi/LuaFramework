...
-------------------------------------------------------
wnd_transfrom2 = i3k_class("wnd_transfrom2", ui.wnd_base)


--???üí¨ó?±3?°?òid

local common_use_iconid = 151
function wnd_transfrom2:ctor()
	
end

function wnd_transfrom2:configure()
...
function wnd_transfrom2:getMem(widgets)--获取UI控件
	self.otherArry = {}
	for i=1,3 do
...
function wnd_transfrom2:checkTransform(item,num1,num2,istrue)--文本颜色显示设置
	local is_ok=true
	if istrue then
...
function wnd_transfrom2:showTop()--初始化上方职位图标及介绍
	for i=1,2 do
...
function wnd_transfrom2:showTemp()--初始化属性
	for i=1,3 do
...
function wnd_transfrom2:showItem()--转职条件
	local is_ok1=true
	for i=1,2 do
...
function wnd_transfrom2:showSkill()--技能
	for i=1,2 do
...
function wnd_transfrom2:refData()--刷新基础数据
...
function wnd_transfrom2:refresh()
	self:refData()
	self:showTemp()
	self:showSkill()
	self:showTop()
end
function wnd_transfrom2:onSendTransfrom(sender,num)
	...
	g_i3k_ui_mgr:CloseUI(eUIID_Transfrom2)
end
function wnd_transfrom2:onSkillTips(sender)
	local tag = sender:getTag()
	g_i3k_ui_mgr:OpenUI(eUIID_TransfromSkillTips)
	g_i3k_ui_mgr:RefreshUI(eUIID_TransfromSkillTips,tag)
end
function wnd_transfrom2:onItemTips(sender)
	local tag = sender:getTag()
	g_i3k_ui_mgr:ShowCommonItemInfo(tag)
end
--[[function wnd_transfrom2:onClose(sender)
	g_i3k_ui_mgr:CloseUI(eUIID_Transfrom2)
end--]]
function wnd_create(layout, ...)
	local wnd = wnd_transfrom2.new();
	wnd:create(layout, ...);
	return wnd;
end
