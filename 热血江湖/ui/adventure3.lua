...
-------------------------------------------------------
wnd_adventure3 = i3k_class("wnd_adventure3",ui.wnd_base)

function wnd_adventure3:ctor()
	self._showTick = 0
	self.endTime = 0
end

function wnd_adventure3:configure()
	local widgets = self._layout.vars
	widgets.close:onClick(self, self.closeUI)
	self.time = widgets.time
end

function wnd_adventure3:refresh(trigID, force)
	local widgets = self._layout.vars
...
function wnd_adventure3:chooseTask(sender, taskId)
	i3k_sbean.adtask_selectReq(taskId)
	self:onCloseUI()
end

function wnd_adventure3:chooseHeadTask(sender, taskId)
	if taskId == 0 then
		g_i3k_ui_mgr:ShowMessageBox2(i3k_get_string(17137), function(ok)
			if ok then
				i3k_sbean.adtask_acceptReq(taskId)
				g_i3k_ui_mgr:InvokeUIFunction(eUIID_Adventure3, "closeUI")
			end
		end)
	else
		i3k_sbean.adtask_acceptReq(taskId)
...
function wnd_adventure3:setYesOrNo(yesTxt, noTxt)
	local widgets = self._layout.vars
	widgets.yes_desc:setText(yesTxt)
	widgets.no_desc:setText(noTxt)
end

function wnd_adventure3:closeUI(sender)
	g_i3k_ui_mgr:CloseUI(eUIID_Adventure3)
end

function wnd_adventure3:onUpdate(dTime)
