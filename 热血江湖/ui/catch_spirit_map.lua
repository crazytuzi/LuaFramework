...
wnd_catch_spirit_map = i3k_class("wnd_catch_spirit_map", ui.wnd_MapBase)

local g_mapSize = nil
function wnd_catch_spirit_map:ctor()
...
function wnd_catch_spirit_map:configure()
	local widgets = self._layout.vars
	widgets.closeBtn:onClick(self, self.onCloseUI)
end

function wnd_catch_spirit_map:onShow()
	
end

function wnd_catch_spirit_map:onHide()
	releaseSchedule()
end

function wnd_catch_spirit_map:refresh(mapId, cfg)
...
function wnd_catch_spirit_map:onRefreshList(mapId,size)
	self._layout.vars.list:removeAllChildren()
	self:rightNpcList(mapId, size)
	self:rightMonsterList(mapId, size)
end

-- function wnd_catch_spirit_map:rightNpcList(mapId, size)
...
-- 				node.vars.name:setText(npc.remarkName .. npc.typeDesc)
...
-- function wnd_catch_spirit_map:rightMonsterList(mapId, size)
...
function wnd_catch_spirit_map:walkToPos(sender, needValue)
...
function wnd_catch_spirit_map:transToPos(sender, needValue)
	...
	elseif not g_i3k_game_context:CheckCanTrans(needId, 1) then
...
function wnd_catch_spirit_map:searchPath(sender, needValue)
	local size = needValue.size
	local world = i3k_game_get_world()
	local nowMapId = world._cfg.id
	local targetMapId = needValue.mapId
...
-- 创建寻路位置目标
function wnd_catch_spirit_map:createTargetPos(toPos, size, mapId)
	local mapPos = i3k_engine_world_pos_to_minmap_pos(toPos, size.width, size.height, mapId, nil, true)
	createTargetPos(mapPos, mapId)
end

-- InvokeUIFunction
function wnd_catch_spirit_map:createTargetPosWithoutSize(toPos)
...
function wnd_catch_spirit_map:toTargetPos(mapId, pos, size, needValue, taskType)
	local hero = i3k_game_get_player_hero()
...
-- function wnd_catch_spirit_map:openOrPickup(sender, needValue)
-- 	if needValue.isNpc then
...
function wnd_catch_spirit_map:close()
	releaseSchedule()
	g_i3k_ui_mgr:CloseUI(eUIID_CatchSpiritMap)
end

function wnd_catch_spirit_map:onUpdate(dTime)
...
function wnd_catch_spirit_map:updateTeammatePos(data, iconId)
	updateTeammatePos(data, iconId)
end

function wnd_catch_spirit_map:updateSpiritBossPos(data, iconId)
	updateSpiritBossPos(data, iconId)
end

function wnd_create(layout, ...)
	local wnd = wnd_catch_spirit_map.new();
	wnd:create(layout, ...);
	return wnd;
end
