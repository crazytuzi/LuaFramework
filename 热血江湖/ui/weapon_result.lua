...
wnd_weapon_result = i3k_class("wnd_weapon_result",ui.wnd_base)
...
function wnd_weapon_result:ctor()
	self._openTime = 1
	self._costMoneyCount = 0
end

function wnd_weapon_result:configure()
...
function wnd_weapon_result:onClose(sender)
	i3k_sbean.mapcopy_leave()
end
	
function wnd_weapon_result:onShow()
	
end
	
function wnd_weapon_result:refresh(rewards, mapId, settlement, process)
...
function wnd_weapon_result:onHide()
end

function wnd_weapon_result:openCard(sender, index)
...
function wnd_weapon_result:reload(rewards, mapId, settlement, process)
	if settlement then
...
function wnd_weapon_result:onItemTips(sender, args)
	g_i3k_ui_mgr:ShowCommonItemInfo(args)
end

function wnd_weapon_result:openReward(reward)
	for i,v in pairs(self._cardBtn) do
		if i==self._tag then
			local anisLabel = "f"..i
...
function wnd_weapon_result:openRandom(reward)
...
function wnd_weapon_result:cantOpen(sender)
	local str
	if self._openTime>2 then
