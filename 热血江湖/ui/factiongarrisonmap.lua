...
wnd_factionGarrisonMap = i3k_class("wnd_factionGarrisonMap", ui.wnd_MapBase)

local g_mapSize = nil
function wnd_factionGarrisonMap:ctor()
...
function wnd_factionGarrisonMap:configure()
	local widgets = self._layout.vars
	widgets.closeBtn:onClick(self, self.onCloseUI)
end

function wnd_factionGarrisonMap:onShow()

end

function wnd_factionGarrisonMap:onHide()
	releaseSchedule()
end

function wnd_factionGarrisonMap:refresh(mapId, cfg)
...
function wnd_factionGarrisonMap:onRefreshList(mapId,size)
	self._layout.vars.list:removeAllChildren()
	self:rightNpcList(mapId, size)
	self:rightMonsterList(mapId, size)
end

-- function wnd_factionGarrisonMap:rightNpcList(mapId, size)
...
-- function wnd_factionGarrisonMap:rightMonsterList(mapId, size)
...
function wnd_factionGarrisonMap:walkToPos(sender, needValue)
...
function wnd_factionGarrisonMap:searchPath(sender, needValue)
	local size = needValue.size
	local world = i3k_game_get_world()
	local nowMapId = world._cfg.id
	local targetMapId = needValue.mapId
...
-- 创建寻路位置目标
function wnd_factionGarrisonMap:createTargetPos(toPos, size, mapId)
	local mapPos = i3k_engine_world_pos_to_minmap_pos(toPos, size.width, size.height, mapId, nil, true)
	createTargetPos(mapPos, mapId)
end

-- InvokeUIFunction
function wnd_factionGarrisonMap:createTargetPosWithoutSize(toPos)
...
function wnd_factionGarrisonMap:toTargetPos(mapId, pos, size, needValue, taskType)
	local hero = i3k_game_get_player_hero()
...
-- function wnd_factionGarrisonMap:openOrPickup(sender, needValue)
...
function wnd_factionGarrisonMap:close()
	releaseSchedule()
	g_i3k_ui_mgr:CloseUI(eUIID_SceneMap)
end

function wnd_factionGarrisonMap:onUpdate(dTime)
...
function wnd_create(layout, ...)
	local wnd = wnd_factionGarrisonMap.new();
	wnd:create(layout, ...);
	return wnd;
end
