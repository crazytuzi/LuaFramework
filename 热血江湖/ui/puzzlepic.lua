...
-------------------------------------------------------
wnd_puzzlePic = i3k_class("wnd_puzzlePic",ui.wnd_base)

function wnd_puzzlePic:ctor()
	self.callBack = nil
	self.imgs = {}
	self.puzzleId = 0
...
function wnd_puzzlePic:configure()
...
function wnd_puzzlePic:refresh(id, func)
	self.callBack = func
	self.puzzleId = id
...
function wnd_puzzlePic:movePic(sender)
		...
		if (dec ~= 1 and dec ~= 3) or (i%3 == 0 and j%3 == 1) or (i%3 == 1 and j%3 == 0) then
			...
			if trueNumber == 9 then
				g_i3k_coroutine_mgr:StartCoroutine(function ()
					...
					g_i3k_ui_mgr:InvokeUIFunction(eUIID_puzzlePic,"quit")
				end)
			end
		end
		return
...
function wnd_puzzlePic:quit()
	g_i3k_game_context:setPusslePicIsFinish(true)
	if self.callBack then
		self.callBack()
	end
	local callback = function()
		g_i3k_ui_mgr:CloseUI(eUIID_puzzlePic)
	end
...
function wnd_puzzlePic:startGame(sender)
	self.startState = 1
	sender:hide()
