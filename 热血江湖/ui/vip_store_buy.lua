...
vip_store_buy = i3k_class("vip_store_buy",ui.wnd_base)
...
function vip_store_buy:refresh(sendcommond, item, limitTimes)
	SALE_COUNT_TEXT = 1
...
function vip_store_buy:configure(...)
...
function vip_store_buy:judgeMarketType()
...
function vip_store_buy:showInfo()
...
function vip_store_buy:onItem(sender, id)
	g_i3k_ui_mgr:OpenUI(eUIID_ItemInfo)
	g_i3k_ui_mgr:RefreshUI(eUIID_ItemInfo, id, true)
end

function vip_store_buy:onClose(sender,eventType)
	if eventType == ccui.TouchEventType.ended then
		g_i3k_ui_mgr:CloseUI(eUIID_VIP_STROE_BUY)
	end
end

function vip_store_buy:jianButton(sender,eventType)
	if eventType == ccui.TouchEventType.ended then
...
function vip_store_buy:jiaButton(sender,eventType)
	if eventType == ccui.TouchEventType.ended then
...
--限购里
function vip_store_buy:maxButton(sender,eventType)
	if eventType == ccui.TouchEventType.ended then
		if self.limitTimes and SALE_COUNT_TEXT < self.limitTimes then
...
function vip_store_buy:jia10Button(sender,eventType)
	if eventType == ccui.TouchEventType.ended then
		local count = 10
...
function vip_store_buy:cancelButton(sender,eventType)
	if eventType == ccui.TouchEventType.ended then
...
function vip_store_buy:okButton(sender,eventType)
	if eventType == ccui.TouchEventType.ended then
		if SALE_COUNT_TEXT > 0 then
			if self:canAfford(SALE_COUNT_TEXT * self.item.finalprice) then
				if self:canBuyVIP(self.item) then
					if self.limitTimes == 0 then
						...
						if g_i3k_game_context:IsBagEnough(test) then
							...
							if self.market == 2 and SALE_COUNT_TEXT >= self._item_count_filter then
				...
				local fun = (function(ok)
					if ok then
						if free == e_Type_vip_store_longhun then
				...
				if free == e_Type_vip_store_hongli then
...
function vip_store_buy:canAfford(price)
	local have = g_i3k_game_context:GetBaseItemCanUseCount(marketType[self.market].currency)
	if have >= price then
		return true
	end
...
function vip_store_buy:canBuyVIP(Item)
	if Item.vipReq then
