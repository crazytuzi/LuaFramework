...
wnd_biography_career_map = i3k_class("wnd_biography_career_map", ui.wnd_MapBase)

local g_mapSize = nil
function wnd_biography_career_map:ctor()
...
function wnd_biography_career_map:configure()
	local widgets = self._layout.vars
	widgets.closeBtn:onClick(self, self.onCloseUI)
end

function wnd_biography_career_map:onShow()
	
end

function wnd_biography_career_map:onHide()
	releaseSchedule()
end

function wnd_biography_career_map:refresh(mapId, cfg)
	...
	self._isShowMonsterTransBtn = true
...
function wnd_biography_career_map:onRefreshList(mapId,size)
	self._layout.vars.list:removeAllChildren()
	self:rightNpcList(mapId, size)
	self:rightMonsterList(mapId, size)
end

-- function wnd_biography_career_map:rightNpcList(mapId, size)
...
-- function wnd_biography_career_map:rightMonsterList(mapId, size)
...
-- 					node.vars.selectBtn:onClick(self, self.walkToPos, needValue)
					...
					-- node.vars.transBtn:onClick(self, self.transToPos, needValue2)
					-- node.vars.transBtn:setVisible(true)
-- 					self._layout.vars.list:addItem(node)
...
function wnd_biography_career_map:walkToPos(sender, needValue)
...
function wnd_biography_career_map:transToPos(sender, needValue)
...
function wnd_biography_career_map:searchPath(sender, needValue)
	local size = needValue.size
	local world = i3k_game_get_world()
	local nowMapId = world._cfg.id
	local targetMapId = needValue.mapId
...
-- 创建寻路位置目标
function wnd_biography_career_map:createTargetPos(toPos, size, mapId)
	local mapPos = i3k_engine_world_pos_to_minmap_pos(toPos, size.width, size.height, mapId, nil, true)
	createTargetPos(mapPos, mapId)
end

-- InvokeUIFunction
function wnd_biography_career_map:createTargetPosWithoutSize(toPos)
...
function wnd_biography_career_map:toTargetPos(mapId, pos, size, needValue, taskType)
	local hero = i3k_game_get_player_hero()
...
-- function wnd_biography_career_map:openOrPickup(sender, needValue)
-- 	if needValue.isNpc then
...
function wnd_biography_career_map:close()
	releaseSchedule()
	g_i3k_ui_mgr:CloseUI(eUIID_BiographyCareerMap)
end

function wnd_biography_career_map:onUpdate(dTime)
...
function wnd_create(layout, ...)
	local wnd = wnd_biography_career_map.new();
	wnd:create(layout, ...);
	return wnd;
end
