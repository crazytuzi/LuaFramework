...
wnd_activity_pets = i3k_class("wnd_activity_pets", ui.wnd_base)

function wnd_activity_pets:ctor()
	self._isChange = false
end

function wnd_activity_pets:configure()
...
function wnd_activity_pets:refresh()
	self:setData()
end

function wnd_activity_pets:setData()
	self.headBg:setImage(g_i3k_get_head_bg_path(g_i3k_game_context:GetTransformBWtype(), g_i3k_game_context:GetRoleHeadFrameId()))
	...
	local hpNow, maxHP = g_i3k_game_context:GetRoleHp()
	...
	local children = scroll:addChildWithCount("ui/widgets/hdscczt", 5, count)
...
function wnd_activity_pets:onPetPlay(sender, totalCount)
	local id = sender:getTag()-1000
	local mercenary = i3k_db_mercenaries[id]
	if mercenary then
		g_i3k_game_context:AddActivityPets(mercenary, totalCount)
	end
	self._isChange = true
	self:setData()
end

function wnd_activity_pets:saveData(sender)
	if self._isChange then
		...
		i3k_sbean.goto_pet_activitymapset(petsTable)
	else
...
function wnd_create(layout)
	local wnd = wnd_activity_pets.new()
	wnd:create(layout)
	return wnd
end
