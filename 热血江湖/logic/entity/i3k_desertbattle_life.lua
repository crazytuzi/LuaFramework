...
------------------------------------------------------
i3k_desertBattle_life = i3k_class("i3k_desertBattle_life", BASE);

function i3k_desertBattle_life:ctor(guid)
	self._entityType	= eET_Player
	self._birthPos		= Engine.SVector3(0, 0, 0);
	self._timetick		= 0;
	self._deadTimeLine	= -1;
	self._inDesertBattle = true;
	self._bindSkills = { };
	local guids = string.split(self._guid, "@");
	self._realGUID		= guids[2];
end

function i3k_desertBattle_life:Create(id)
	local cfgID = math.abs(id)
	local cfg = i3k_db_desert_generals[cfgID];
...
function i3k_desertBattle_life:CreateTitle(reset)
	...
	if title.node:Create("hero_title_node_" .. self._guid) then
		...
		title.name	= title.node:AddTextLable(-0.5, 1, -0.5, 0.5, tonumber("0xffffffff", 16), i3k_get_string(17600, self._name, g_i3k_game_context:getDesertBattleMapScore())); 
	else
		title.node = nil;
	end
...
function i3k_desertBattle_life:InitSkills(resetBinds)
	local cooltimes = { };
...
function i3k_desertBattle_life:OnInitBaseProperty(props)
	local properties = BASE.OnInitBaseProperty(self, props);
	local temProperties = {}
	local wearEquips = g_i3k_game_context:GetDesertBattleEquipData()
	

	if wearEquips then
		for k, v in pairs(wearEquips) do
			if k ~= 0 and v ~= 0 then
				local cfg = i3k_db_desert_battle_equips[v]
				if cfg then
					for _, n in pairs(cfg.baseProp) do
						temProperties[n.propID] = (temProperties[n.propID] or 0) + n.propValue
					end
					
					for _, n in pairs(cfg.additionalProp) do
						temProperties[n.propID] = (temProperties[n.propID] or 0) + n.propValue
					end
				end
			end
	...
	for k, v in pairs(temProperties) do
		local prop = properties[k];
		prop:Set(prop._valueB.Base + v, ePropType_Base)
	end
	
	return properties;	
end

function i3k_desertBattle_life:CanRelease()
	return false;
end

function i3k_desertBattle_life:InitPlayerAttackList()
	if self:IsPlayer() then
		self._attackLst = { }
		local normalskill = {}
		local heroUseSkills = g_i3k_game_context:getDesertBattleSkills()
...
function i3k_desertBattle_life:ClearAutofightTriggerSkill() --继承于hero
	
end

function i3k_desertBattle_life:AddAutofightTriggerSkill() 
	
end

function i3k_desertBattle_life:UpdateFightSp(curFightSP, byBuff)

end

function i3k_desertBattle_life:GetGUID()
	return self._realGUID;
end

function i3k_desertBattle_life:SetTitleVisiable(vis)
	if self._title and self._title.node then
		self._title.node:SetVisible(vis);
	end
end

function i3k_desertBattle_life:ClearFightTime() -- 继承重写 为了屏蔽title
	self._fighttime = 0
