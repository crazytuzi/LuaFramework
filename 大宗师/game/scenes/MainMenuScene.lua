...
function MainMenuScene:ctor(showNote)
    ...
    CCNotificationCenter:sharedNotificationCenter():registerScriptObserver(self, updateInfo, "APP_ENTER_FOREGROUND_EVENT_IN_GAME")
...
--            dump(data)
    ...
    self.blackLayer = display.newColorLayer(ccc4(0,0,0,0))
    ...
    local function moveact(node,ac)
        ...
        local moveSequence = transition.sequence({
            CCMoveBy:create(time, ccp(3*ac, 3*ac)),
            CCMoveBy:create(time, ccp(-3*ac, -5*ac)),
            CCMoveBy:create(time, ccp(-5*ac, 2*ac)),
            CCMoveBy:create(time, ccp(5*ac, 0*ac)),
        })
...
function MainMenuScene:refreshPlayerBoard( ... )
    ...
    local function refreshBar( ... )
        ...
        barWidth*percent, bar:getTextureRect().size.height))
