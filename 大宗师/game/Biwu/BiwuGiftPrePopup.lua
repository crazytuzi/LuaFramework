...
function BiwuGiftRrePopup:setUpView()
    ...
    --title标签
    self._titleDisLabel = ui.newBMFontLabel({text = "奖励预览", size = self._titleDisFontSize, 
        align = ui.TEXT_ALIGN_CENTE,font = "fonts/font_title.fnt"})
...
function BiwuGiftRrePopup:setUpTableView()
	local titleDis = ui.newTTFLabel({text = "每周六晚23点发奖励，不要错过哟~", color = ccc3(92,38,1),size = 22, font = FONTS_NAME.font_fzcy, align = ui.TEXT_ALIGN_LEFT})
        :pos(self._innerContainerSize.width/2, self._innerContainerSize.height - 30)
    ...
    self._tableContainer = display.newScale9Sprite(commonRes.tableViewBngRes, 0, 0, 
                        ...
                        self._innerContainerSize.height+ 20))
...
function BiwuGiftRrePopup:tableCellAtIndex(table, idx) 
    ...
    local itemView = GiftGetItemView.new(cc.size(self._tableContainerSize.width,220), 
