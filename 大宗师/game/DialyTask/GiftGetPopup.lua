...
function GiftGetPopup:setUpView()
    ...
    --title标签
    self._titleDisLabel = ui.newBMFontLabel({text = "奖励领取", size = self._titleDisFontSize, 
...
function GiftGetPopup:setUpTableView()
    self._tableContainer = display.newScale9Sprite(commonRes.tableViewBngRes, 0, 0, 
                        ...
                        self._innerContainerSize.height - 10))
...
function GiftGetPopup:createTitleTask()
    ...
    local countDis = ui.newTTFLabel({text = jifen , font = FONTS_NAME.font_fzcy , size = 22,
    ...
    local beginLabel = ui.newTTFLabel({text = "0", size = 20, align = ui.TEXT_ALIGN_LEFT})
        :pos(margin, progressBngSize.height / 2)
        :addTo(progress)
    beginLabel:setAnchorPoint(cc.p(0,0.5))
    local endLabel   =  ui.newTTFLabel({text = TaskModel:getInstance():getMaxJIfen(), size = 20, align = ui.TEXT_ALIGN_LEFT})
