...
function CCBuilderReaderLoad(strFilePath,proxy,owner, parentNode, parentSize)
    if nil == proxy then
        return
    end
    ...
    if(nodesWithAnimationManagers ~= nil) then
        count = nodesWithAnimationManagers:count()
    else
        print("==>>nodesWithAnimationManagers is nil" )
    end
    -- printf("================= " .. tostring(count))
    for i = 1 , count do
