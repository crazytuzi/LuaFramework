local horsePreview = class("horsePreview", import(".panelBase"))
local tip = import(".wingInfo")
horsePreview.ctor = function (self, horseid)
...
horsePreview.onEnter = function (self)
...
horsePreview.loadMainPage = function (self)
...
horsePreview.fillListView = function (self)
	local horseCfg = def.horse:getBaseCfgByID(self.horseid)
	...
	for i, v in ipairs(def.wing.getAllBaseCfg()) do
		if i%10 == 0 then
			...
			display.newSprite(res.gettex2(string.format("pic/panels/horseUpgrade/head_%d_c.png", self.horseid))):add2(item):pos(37, 37):enableClick(function ()
				local ss = {}
				local level = i
				local lvStr = def.wing.level2str(level) .. "¡¤×øÆï"
			...
			an.newLabel(horseCfg.Name, 20, 1, {
