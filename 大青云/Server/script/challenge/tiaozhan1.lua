...
------------------------------------------
CurrentSceneScript.TrapID = 9001	--陨石
CurrentSceneScript.TrapPos = {		--陨石坑
	{ x=-342, 	y=42, 	dir=0},
	{ x=-364, 	y=-38, 	dir=0},
	{ x=-315, 	y=-141, dir=0},
	{ x=-289, 	y=-49, 	dir=0},
...
CurrentSceneScript.TrapNum = 5 		--陨石数量
CurrentSceneScript.TrapTick = 5 	--陨石间隔
CurrentSceneScript.TrapCounter = 0
------------------------------------------
...
function CurrentSceneScript:Startup()
	...
	CurrentSceneScript.TrapCounter = 1
end
...
function CurrentSceneScript:SendMonsterIdNum(human)
	...
	if human ~= nil then
		human:GetModChallengeDupl():SendMonster(data)
	else
		for k,v in pairs(self.Humans) do
			v:GetModChallengeDupl():SendMonster(data)
		end
	end
end
...
function CurrentSceneScript:OnTimerExpired(curr)
	...
	local layerConf = TiaozhanfubenConfig[tostring(self.CurrLayer)]
	...
	self:OnTrapUpdatePer1Sec()
end

function CurrentSceneScript:OnTrapUpdatePer1Sec()
	if self.TrapCounter % self.TrapTick == 0 then
		local pos = random_table(self.TrapPos, self.TrapNum)
		for i,v in ipairs(pos) do
			if v ~= nil then
				self.Scene:GetModSpawn():SpawnTrap(self.TrapID, v.x, v.y, v.dir)
			end
		end
	else
...
function CurrentSceneScript:ChallengeStart(human, layer, spwan)
	...
	if spwan == 0 then
		human:GetModChallengeDupl():OnDuplSucc(9999)
	else
...
function CurrentSceneScript:QiBingSucc()
	...
	for k,v in pairs(self.Humans) do
		v:GetModChallengeDupl():OnDuplSucc(remain)
	end
end
...
function CurrentSceneScript:QiBingFail()
	self.Status = 0
	for k,v in pairs(self.Humans) do
		v:GetModChallengeDupl():OnDuplFail()
	end
