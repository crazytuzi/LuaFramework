...
function CurrentSceneScript:OnHumanEnter(human)
    ...
    if self.random_event == 1 then
       _state = 2
       self:RandomEventInfo(self.random_event,_state,0,0)
    end
end
...
function CurrentSceneScript:OnBossKilled(boss,killer,id)
    --击杀boss事件
    if id == 10200001 or id == 10200002 or id == 10200003 then
        --boss隐藏BOSS星级界面
    ...
    if id == 10200004 then
        ...
        if self.monster_hold_num >= self.monster_hold_first_max then
            if self.monster_hold_flag == 0 then
               ...
               self.SModScript:SpawnMonsterBit(10200013,false)
            end
        end
...
function CurrentSceneScript:OnHumanStoryStep(id)
    ...
    if id == 3101001 then
        self.SModScript:SpawnMonsterBit(10200013,true) -- 设置场景石头怪
        self.SModDungeon:DungeonBlock("block001",7001,false) --阻挡,音效,false关闭阻挡
    end
    if id == 3101003 then
        self.SModDungeon:DungeonBlock("block003",7003,false)

        local data = {}
        local monster_id = "10200006" --怪物id
    ...
    if id == 3101007 then
        ...
        self.SModScript:SpawnMonsterBit(10200013,true)
    end
    if id == 1101008 then
        local data = {}
        local monster_id = "10200005"
    ...
    if id == 3101010 then
        --self.SModScript:DungeonBlock("block006",0,false)
    end
    if id == 2101010 then
        local data = {}
        local monster_dir = 3.09

        data[1] = 10200002 --半血刷小怪
        data[2] = 1
        data[3] = 131
        data[4] = -66
		self.SModScript:SpawnMonster(data,monster_dir)
    ...
    if id == 10200007 then -- 分裂小怪id
        self.SModScript:SpawnMonsterBit(10200002,false)
        self.split_flag = 2
    end
    if id == 3101012 then
...
function CurrentSceneScript:GetDropBloodPer(id)
    ...
    if self.split_flag == 1 and id == 10200002 then
        result = 1
    end
    return result
end
...
function CurrentSceneScript:OnDungeonMonster(id,hp_per,pos_x,pos_y)
	...
	if id == 10200002 then
        ...
        if hp_per>0 and hp_per <= 50  then  --BOSS 50%血量分裂出一批20只小怪
            ...
            self.SModDungeon:PlayStory(4,"dun101002") --剧情类型,id
            self.SModScript:SpawnMonsterRandom(data,params)
        end
    end
end
...
function CurrentSceneScript:RandomEventResult()
    ...
    if etype == 1 then
        --争分夺秒,经验(params:经验值),分钟
        local minutes = time/60
        if self.boss_killed and minutes <= 3 then
           fight_success = true
        end
    elseif etype == 2 then
        --秒杀的快感,道具(params:道具id,数量),秒
        if self.boss_killed and time <= 10 then
           fight_success = true
        end
...
function CurrentSceneScript:RandomEventCD(id)
    ...
    self.DelayTimer = self.SModScript:CreateTimer(sec, "RandomEventClose")
end
