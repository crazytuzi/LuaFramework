...
_G.UIToolsLingQiDraw = BaseUI:new("UIToolsLingQiDraw");

UIToolsLingQiDraw.defaultCfg = {
...
UIToolsLingQiDraw.currId = 0;
UIToolsLingQiDraw.list = {};
							
function UIToolsLingQiDraw:Create()
	self:AddSWF( "toolLingQiModelDisplay.swf", true, "center" );
end

function UIToolsLingQiDraw:OnLoaded(objSwf,name)
...
function UIToolsLingQiDraw:OnDelete()
	if self.objUIDraw then
		self.objUIDraw:SetUILoader(nil);
	end
end

function UIToolsLingQiDraw:GetWidth()
	return 640;
end

function UIToolsLingQiDraw:GetHeight()
	return 640;
end

function UIToolsLingQiDraw:OnShow(name)
	--清除无效数据
	for id, cfg in pairs(UIDrawLingQiCfg) do
		if not t_lingqi[id] then
			UIDrawLingQiCfg[id] = nil;
		end
	end
	--
	...
	self.list = {};
	for i,cfg in pairs(t_lingqi) do
		local listVO = {};
		listVO.name = cfg.name;
		listVO.flag = UIDrawLingQiCfg[cfg.id] and "√" or "";
		listVO.id = cfg.id;
	...
	objSwf.nameLoader.source = ResUtil:GetLingQiNameImg(1)
end

function UIToolsLingQiDraw:OnHide()
	if self.objUIDraw then
...
function UIToolsLingQiDraw:OnBtnCloseClick()
	self:Hide()
end

--缩小
function UIToolsLingQiDraw:OnBtnZoomOut()
	if self.objUIDraw then
...
--放大
function UIToolsLingQiDraw:OnBtnZoomIn()
	if self.objUIDraw then
		local newEye = self.objUIDraw.objCamera.eye:add(0,1,0);
		self.objUIDraw.objCamera.eye = newEye;
		self:OnCfgChange();
...
--左移
function UIToolsLingQiDraw:OnBtnLeft()
	if self.objUIDraw then
...
--右移
function UIToolsLingQiDraw:OnBtnRight()
	if self.objUIDraw then
		local newLook = self.objUIDraw.objCamera.look:add(1,0,0);
		self.objUIDraw.objCamera.look = newLook;
		self:OnCfgChange();
...
--上移
function UIToolsLingQiDraw:OnBtnUp()
	if self.objUIDraw then
...
--下移
function UIToolsLingQiDraw:OnBtnDown()
	if self.objUIDraw then
		local newLook = self.objUIDraw.objCamera.look:add(0,0,1);
		self.objUIDraw.objCamera.look = newLook;
		self:OnCfgChange();
...
--仰视
function UIToolsLingQiDraw:OnBtnLookUp()
	if self.objUIDraw then
...
--俯视
function UIToolsLingQiDraw:OnBtnLookDown()
	if self.objUIDraw then
		local newEye = self.objUIDraw.objCamera.eye:add(0,0,-1);
		self.objUIDraw.objCamera.eye = newEye;
		self:OnCfgChange();
...
local rotation = 0;
function UIToolsLingQiDraw:OnBtnTurnLeft()
	rotation = rotation + 0.05;
	self.objUIDraw.objEntity.objMesh.transform:setRotation( 0, 1, 0, rotation );
	self:OnCfgChange();
end

function UIToolsLingQiDraw:OnBtnTurnRight()
	rotation = rotation - 0.05;
	self.objUIDraw.objEntity.objMesh.transform:setRotation( 0, 1, 0, rotation );
	self:OnCfgChange();
end

function UIToolsLingQiDraw:Draw(id)
	local objSwf = self.objSwf;
	if not objSwf then return; end
	self.currId = id;
	local cfg = _G.t_lingqi[id]
	...
	local drawCfg = UIDrawLingQiCfg[id];
	if not drawCfg then
		drawCfg = self:GetDefaultCfg();
		UIDrawLingQiCfg[id] = drawCfg;
		self:SetListHasCfg(id);
	end
	if not self.objUIDraw then
		self.objUIDraw = UIDraw:new( "toolsLingQi",avatar, objSwf.loader,
...
--设置某项有了数据
function UIToolsLingQiDraw:SetListHasCfg(id)
	local objSwf = self.objSwf;
	if not objSwf then return; end
...
--配置变动
function UIToolsLingQiDraw:OnCfgChange()
	if not self.objUIDraw then return; end
	local id = self.currId;
	if not UIDrawLingQiCfg[id] then
		UIDrawLingQiCfg[id] = self:GetDefaultCfg();
	end
	local cfg = UIDrawLingQiCfg[id];
...
function UIToolsLingQiDraw:OnListItemClick(e)
	local id = e.item.id;
	self:Draw(id);
end

function UIToolsLingQiDraw:OnBtnUseParam()
	if not self.objUIDraw then return; end
	local objSwf = self.objSwf;
	if not objSwf then return; end
...
function UIToolsLingQiDraw:OnBtnSave()
	local file = _File:new();
	file:create(ClientConfigPath .. 'config/gui/UIDrawLingQiConfig.lua');
	file:write("_G.UIDrawLingQiCfg = {\n");
	for id,cfg in pairs(UIDrawLingQiCfg) do
		file:write("\t["..id.."] = \n\t{\n");
...
function UIToolsLingQiDraw:GetDefaultCfg()
	local cfg = {};
