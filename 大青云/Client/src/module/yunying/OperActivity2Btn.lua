--[[
每日首冲
2015年8月22日, PM 05:38:45
wangyanwei
]]

_G.OperActivity2Btn = BaseYunYingBtn:new();

YunYingBtnManager:RegisterBtnClass(YunYingConsts.BT_operActivity2,OperActivity2Btn);

function OperActivity2Btn:GetStageBtnName()
	return "operActivity2";
end

function OperActivity2Btn:IsShow()
	local btnStateVO = OperactivitiesModel:GetOperBtnState(OperactivitiesConsts.iconShouchongDay)
	local btnState = btnStateVO.reward
	if btnState == 2 then
		return false
	end
	
	if btnState < 0 then
		return false
	end
	
	-- local actVO = OperactivitiesModel:GetFirstChargeActVO(OperactivitiesConsts.iconShouchongDay)
	-- if not OperactivitiesModel:GetNeedActivityIsShow(actVO.group) then
		-- return false
	-- end
	
	if t_consts[126] then
		local constCfgNeedLevel = t_consts[126].val2
		if constCfgNeedLevel then
			local curRoleLvl = MainPlayerModel.humanDetailInfo.eaLevel -- 当前人物等级
			if curRoleLvl < constCfgNeedLevel then return false end
		end
	end
	
	return true
end

function OperActivity2Btn:OnBtnClick()
	OperActUIManager:ShowHideOperActUI(OperactivitiesConsts.iconShouchongDay)
end

function OperActivity2Btn:OnBtnInit()
	if self.button.initialized then
		if self.button.effect.initialized then
			self.button.effect:playEffect(0);
			self:OnGetIsShow();
		else
			self.button.effect.init = function()
				self.button.effect:playEffect(0);
				self:OnGetIsShow();
			end
		end
	end
	self:UnRegisterNotification()
	self:RegisterNotification()
end

function OperActivity2Btn:OnGetIsShow()

end

--处理消息
function OperActivity2Btn:HandleNotification(name, body)
	if not self:IsShow() then return end
	if name == NotifyConsts.UpdateOperActBtnIconState then
		self:OnGetIsShow();
	end
end

--消息处理
function OperActivity2Btn:RegisterNotification()
	local setNotificatioin = self:ListNotificationInterests();
	if not setNotificatioin then return; end
	if not self.notifierCallBack then
		self.notifierCallBack = function(name,body)
			self:HandleNotification(name, body);
		end
	end
	for i,name in pairs(setNotificatioin) do
		Notifier:registerNotification(name, self.notifierCallBack)
	end
end

--取消消息注册
function OperActivity2Btn:UnRegisterNotification()
	local setNotificatioin = self:ListNotificationInterests();
	if not setNotificatioin then return; end
	if not self.notifierCallBack then return end
	for i,name in pairs(setNotificatioin) do
		Notifier:unregisterNotification(name, self.notifierCallBack)
	end
end

--监听消息
function OperActivity2Btn:ListNotificationInterests()
	return {
		NotifyConsts.UpdateOperActBtnIconState,
	} 
end