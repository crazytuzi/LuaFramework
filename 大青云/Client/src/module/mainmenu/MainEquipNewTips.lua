...
_G.UIMainEquipNewTips = BaseUI:new("UIMainEquipNewTips");

UIMainEquipNewTips.itemId = nil;--物品cid

UIMainEquipNewTips.autoCloseTimer = nil;

function UIMainEquipNewTips:Create()
	if Version:IsLianYun() then
		self:AddSWF("equipNewTipsLianYun.swf", true, "bottomFloat");
	else
		self:AddSWF("equipNewTips.swf", true, "bottomFloat");
	end
end

function UIMainEquipNewTips:OnLoaded(objSwf)
...
function UIMainEquipNewTips:NeverDeleteWhenHide()
	return true;
end

function UIMainEquipNewTips:Open(itemId)
	if RemindFuncManager:IsOnShowList(RemindFuncConsts.RFC_SmithingResp) then
		return;
	end
...
function UIMainEquipNewTips:OnShow()
...
function UIMainEquipNewTips:OnHide()
	if self.autoCloseTimer then
		TimerManager:UnRegisterTimer(self.autoCloseTimer);
		self.autoCloseTimer = nil;
	end
	self:PlayEffects(false);
	EquipNewTipsManager:OnShowOneOver();
	if self.objSwf.btnOk then self.objSwf.btnOk:clearEffect(); end
end

function UIMainEquipNewTips:IsTween()
	return true;
end

UIMainEquipNewTips.TweenScale = 50;
--打开效果
function UIMainEquipNewTips:TweenShowEff(callback)
...
--关闭效果
function UIMainEquipNewTips:TweenHideEff(callback)
	local objSwf = self.objSwf;
...
function UIMainEquipNewTips:DoTweenShow()
	self:TweenShowEff(function()
		self:DoShow();
	end);
end

function UIMainEquipNewTips:DoTweenHide()
	self:TweenHideEff(function()
		self:DoHide();
	end);
end

function UIMainEquipNewTips:PlayEffects(play)
	local objSwf = self.objSwf;
...
function UIMainEquipNewTips:ShowInfo()
	local objSwf = self.objSwf;
	if not objSwf then return; end
	...
	objSwf.tfFight.htmlText = UIMainEquipNewTips:OnGetAddFight(item);
...
--比穿着装备高出的战斗力
function UIMainEquipNewTips:OnGetAddFight(item)
	local fightNum = BagUtil:CheckBetterFightNum(item:GetBagType(),item:GetPos());
	if not fightNum then return item:GetFight(); end
	return fightNum;
end

function UIMainEquipNewTips:OnItemRollOver()
	local objSwf = self.objSwf;
	if not objSwf then return; end
	local bagVO = BagModel:GetBag(BagConsts.BagType_Bag);
...
function UIMainEquipNewTips:OnItemRollOut()
	TipsManager:Hide();
end

function UIMainEquipNewTips:OnBtnOkClick(isAuto)
	local bagVO = BagModel:GetBag(BagConsts.BagType_Bag);
	if bagVO then
		local item = bagVO:GetItemById(self.itemId);
		if item then
...
function UIMainEquipNewTips:OnBtnCloseClick()
	self:Hide();
end