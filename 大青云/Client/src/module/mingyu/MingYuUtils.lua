...
-- 获取mwLvl级玉佩, prfcncyLvl级熟练度的熟练度上限值
function MingYuUtils:GetProficiencyCeiling( mwLvl, prfcncyLvl )
	local cfg = t_mingyu[mwLvl];
	local ceilingTab = cfg and split( cfg.proficiency, "#" );
	local tableRank = math.min( prfcncyLvl + 1, MingYuConsts.MaxLvlProficiency );
...
function MingYuUtils:GetFight( level, lvlPrfcncy )
	local attrList = {};
	...
	for attrType, attrValue in pairs(attrMap) do
		...
		local zzRate = ZiZhiUtil:GetZZTotalAddPercent(2);
...
--		vo.val = vo.val * (1+addP+vipUPRate);
	...
	local attrSXDMap = AttrParseUtil:ParseAttrToMap(t_consts[329].param);--属性丹
	for _, type in pairs( MingYuConsts.Attrs ) do
		attrMap[type] = attrMap[type] + (attrSXDMap[type] or 0) * MingYuModel:GetPillNum();
	end
	return PublicUtil:GetFigthValue( attrList );
end

-- 获取玉佩属性增量(键: "att", "def", "cri", "hp", "crivalue" )
function MingYuUtils:GetAttrIncrementMap( level )
	if level == MingYuConsts:GetMaxLevel() then return; end
	local cfg = t_mingyu[level];
	if not cfg then return; end
	local nextCfg = t_mingyu[level + 1];
	...
	local attr; -- 玉佩等阶增加的属性
	local currentLvlMaxAttr; -- 当前玉佩等阶的最高增加属性
	local nextLvlAttr -- 下一级玉佩增加的属性
	for _, type in pairs( MingYuConsts.Attrs ) do
		attrPerPrfcncyLvl = attrPerPrfcncyLvlMap[type] or 0;
		attr = cfg[type] or 0;
--		currentLvlMaxAttr = attrPerPrfcncyLvl * MingYuConsts.MaxLvlProficiency + attr
...
--获取列表VO
function MingYuUtils:GetSkillListVO(skillId, lvl)
	local vo = {};
	vo.skillId = skillId;
...
--获取技能是否可升级
function MingYuUtils:GetSkillCanLvlUp(skillId)
...
-- 获取玉佩的属性表 map[attrType] = attrValue
-- 包含属性丹 addby lizhuangzhuang 2015年11月7日23:11:54
-- @param level :玉佩等阶
-- @param lvlPrfcncy:熟练度等阶
function MingYuUtils:GetMagicWeaponAttrMap(level, lvlPrfcncy)
	local cfg = t_mingyu[level];
	if not cfg then
		-- Error( string.format( "cannot find config of shenbing in t_mingyu.lua.  level:%s", level ) );
...
function MingYuUtils:GetConsumeItem(level)
	local cfg = t_mingyu[level]
	if not cfg then return end
...
function MingYuUtils:GetConsumeMoney(level)
	local cfg = t_mingyu[level]
	if not cfg then return end
...
-----------------------------bing hun-----------------------------
function MingYuUtils:GetHunAttrMap()
	local map = {};
	for _, attrName in pairs(MingYuConsts.HunAttrs) do
		map[attrName] = 0;
	end
...
--返回是否开启，几阶开启
function MingYuUtils:GetHunStateByPos(pos)
	local level = MingYuModel:GetLevel();
	for i=1,99 do
		local cfg = t_mingyu[i];
		if cfg then
...
--得到套装list
function MingYuUtils:GetGroupHunList()
	local grouplist = {};
