...
_G.UILovelyPetRenewView = BaseUI:new("UILovelyPetRenewView")

UILovelyPetRenewView.timerKey = nil;

UILovelyPetRenewView.lovelypetid = 0;

UILovelyPetRenewView.lovelypettime = 0;

function UILovelyPetRenewView:Create()
	self:AddSWF("lovelypetRenewPanel.swf", true, "top")
end

function UILovelyPetRenewView:OnLoaded(objSwf,name)
	--关闭
...
function UILovelyPetRenewView:OnShow(name)
	--初始化数据
	self:InitData();
	--计时器
	self:StartTimer();
...
function UILovelyPetRenewView:OnHide()
	self:DelTimerKey();
...
function UILovelyPetRenewView:GetWidth()
	return 277;
end

--点击关闭按钮
function UILovelyPetRenewView:OnBtnCloseClick()
	self:Hide();
end

--点击续费按钮
function UILovelyPetRenewView:OnBtnRenewClick()
...
--点击取消按钮
function UILovelyPetRenewView:OnBtnCancelClick()
	self:Hide();
end

function UILovelyPetRenewView:OnBtnToolRollOver()
	local lovelypetcfg = t_lovelypet[self.lovelypetid];
	if not lovelypetcfg then
		return;
	end
...
function UILovelyPetRenewView:InitData()
...
function UILovelyPetRenewView:Open()
	if self:IsShow() then
		self:InitData();
		self:ShowLovelyPetRenewInfo();
	else
...
--显示信息
function UILovelyPetRenewView:ShowLovelyPetRenewInfo()
	self:ShowRemainTime();
	self:ShowRenewInfo();
...
--显示续费道具
function UILovelyPetRenewView:ShowRenewToolInfo()
	local objSwf = self.objSwf;
	if not objSwf then return; end
...
--显示续费时间
function UILovelyPetRenewView:ShowRenewInfo()
	local objSwf = self.objSwf;
	if not objSwf then return; end
...
--显示剩余时间
function UILovelyPetRenewView:ShowRemainTime()
	local objSwf = self.objSwf;
	if not objSwf then return; end
...
function UILovelyPetRenewView:StartTimer()
...
function UILovelyPetRenewView:DelTimerKey()
	if self.timerKey then
		TimerManager:UnRegisterTimer( self.timerKey );
		self.timerKey = nil;
	end
...
--计时器
function UILovelyPetRenewView:OnTimer()
	--非战斗状态不倒计时
	local state = LovelyPetUtil:GetLovelyPetState(UILovelyPetRenewView.lovelypetid);
	if state ~= LovelyPetConsts.type_fight then
		return;
	end
	
	UILovelyPetRenewView.lovelypettime = UILovelyPetRenewView.lovelypettime - 1;
	
	if UILovelyPetRenewView.lovelypettime < 0 then
		UILovelyPetRenewView.lovelypettime = 0;
	end
	
	UILovelyPetRenewView:ShowRemainTime();
end;

function UILovelyPetRenewView:GetTime(time)
...
function UILovelyPetRenewView:Update( interval )
...
--处理消息
function UILovelyPetRenewView:HandleNotification(name,body)
	if not self.bShowState then return; end
	local objSwf = self.objSwf;
	if not objSwf then return; end
...
--监听消息
function UILovelyPetRenewView:ListNotificationInterests()
