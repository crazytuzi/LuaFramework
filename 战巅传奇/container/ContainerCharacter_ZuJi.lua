...
local function showPanelPage(index)
	...
	if not var[pageName] and ContainerCharacter_ZuJi[initFunc] then
		ContainerCharacter_ZuJi[initFunc]()
	end
	if var[pageName] then
		if ContainerCharacter_ZuJi[openFunc] then
			ContainerCharacter_ZuJi[openFunc]()
		end
		var[pageName]:show()
	end
end

function ContainerCharacter_ZuJi.pushTabButtons(sender)
...
---------------------------------------以上为内部函数---------------------------------------
function ContainerCharacter_ZuJi.initView(extend)
	var = {
		boxTab,
		xmlPanel,
		xmlPageGem,
	...
	--加载称号素材
	asyncload_frames("ui/sprite/ContainerZuJi",".png")

	var.xmlPanel = GUIAnalysis.load("ui/layout/ContainerCharacter_ZuJi.uif")
	...
	if var.xmlPanel then
		...
		var.boxTab:addTabEventListener(ContainerCharacter_ZuJi.pushTabButtons)		
		return var.xmlPanel
	end
end

function ContainerCharacter_ZuJi.onPanelOpen(extend)
	var.panelExtend = extend
	var.boxTab:setItemMargin(3)
	var.boxTab:hideTab(2)
	var.boxTab:setSelectedTab(7)
	ContainerCharacter_ZuJi.initPageTitle()
end

function ContainerCharacter_ZuJi.onPanelClose()
...
--------------------------------------魂环--------------------------------------
function ContainerCharacter_ZuJi.initPageTitle()

	local function updateInnerLooks()
		ContainerCharacter_ZuJi.tryOnTitle()
	end
	...
	tabhtitle:addTabEventListener(ContainerCharacter_ZuJi.ckickTitleTab)
	
	var.xmlPanel:getWidgetByName("title_wear"):addClickEventListener(ContainerCharacter_ZuJi.ckickDressTitleButton)
	...
	btn_info:setTouchEnabled(true):addTouchEventListener(function(sender,eventType)
		...
		GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "alltitlelist"}))
	GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "mycurrenttitle"}))
	GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "mytitlelist"}))
	updateInnerLooks()
	cc.EventProxy.new(GameSocket, var.xmlPanel)
		:addEventListener(GameMessageCode.EVENT_ITEM_CHANGE, ContainerCharacter_ZuJi.freshTitlePage)
		:addEventListener(GameMessageCode.EVENT_AVATAR_CHANGE, updateInnerLooks)

		:addEventListener(GameMessageCode.EVENT_PUSH_PANEL_DATA,ContainerCharacter_ZuJi.setTitledata)

end
function ContainerCharacter_ZuJi.setTitledata(event)
	...
	elseif event.type == "ContainerZuJi" then
		local data = GameUtilSenior.decode(event.data)
		
		if data.cmd == "alltitlelist" then
			var.title_preview = data.Data;
			--GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "alltitlelist_2"}))
		end
		if data.cmd == "mytitlelist" then
			var.my_title_preview = data.Data;
			ContainerCharacter_ZuJi.freshTitleAttr()
			ContainerCharacter_ZuJi.ckickTitleTab(var.xmlPanel:getWidgetByName("tabhtitle"):getItemByIndex(1))
		end
		...
		if data.cmd == "titleAttr" then
			...
			ContainerCharacter_ZuJi.showTitleAttr(data.Data)
			--ContainerCharacter_ZuJi.freshTitleAttr()
...
function ContainerCharacter_ZuJi.freshTitlePage(event)
	local tabhtitle = var.xmlPanel:getWidgetByName("tabhtitle")
	--if pageKeys[var.boxTab:getCurIndex()] == "title" then
		-- if tabhtitle:getCurIndex() == table.indexof(titlePos,event.pos) then
			ContainerCharacter_ZuJi.ckickTitleTab(tabhtitle:getItemByIndex(tabhtitle:getCurIndex()))
		-- end
		if table.indexof(titlePos,event.pos)  then
			ContainerCharacter_ZuJi.freshTitleAttr(event)
		end
	--end
end
--显示已装备称号属性
function ContainerCharacter_ZuJi.freshTitleAttr(event)
	...
	if var.mycurrenttitle ~= nil and var.mycurrenttitle ~="" and var.my_title_preview~=nil and var.my_title_preview~="" then
		for k,val in ipairs(var.my_title_preview) do
			if val.id==var.mycurrenttitle then
				...
				--ContainerCharacter_ZuJi.showTitleAttr(val.detail)
				GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "titleAttr",typeID=val.type_id}))
...
function ContainerCharacter_ZuJi.showTitleAttr(val)
	if val==nil then
		return
	end
...
function ContainerCharacter_ZuJi.tryOnTitle(titleId)
		...
		if titleId then
			for k,v in ipairs(var.title_preview) do
				...
				if tonumber(v.type_id)==tonumber(titleId) then
					...
					GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "titleAttr",typeID=v.type_id}))
					--ContainerCharacter_ZuJi.showTitleAttr(v)
				end
			end
...
--function ContainerCharacter_ZuJi.ckickDressTitleButton( sender )
function ContainerCharacter_ZuJi.ckickDressTitleButton(  )
	--GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "settitle",titleid = sender.titleid}))
	if var.curTitleListIndex~=nil and tonumber(var.curTitleListIndex)>0 then
		GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "settitle",titleid = var.my_title_preview[var.curTitleListIndex].id}))
	else
		GameSocket:PushLuaTable("gui.ContainerZuJi.handlePanelData",GameUtilSenior.encode({actionid = "settitle",titleid = 0}))
		ContainerCharacter_ZuJi.ckickTitleTab(var.xmlPanel:getWidgetByName("tabhtitle"):getItemByIndex(1))
	end
end

--刷新称号列表
function ContainerCharacter_ZuJi.ckickTitleTab(tab)
	...
	local function pushSelectItem(item)
		if item and item.tagTitleOnlyId ~=nil and item.tagTitleOnlyId ~="" and item.tagTitleOnlyId~=0 then
			ContainerCharacter_ZuJi.TryTitleShow(item.tagTitleId,item.tag);
		elseif item and item.tagTitleId ~=nil and item.tagTitleId ~="" then
			--ContainerCharacter_ZuJi.tryOnTitle(item.tagTitleId)
		...
		list_btn:reloadData(#var.my_title_preview,function( subItem )
			...
			local function  showMapDetail( sender )
				for i,v in ipairs(items) do
					v:getWidgetByName("title_btn_font"):loadTexture(string.format("ContainerZuJi_left_btn_%d_1.png",(tonumber(v:getWidgetByName("title_btn_font").titleTypeid))),ccui.TextureResType.plistType)
					--v:getWidgetByName("title_btn_font"):loadTexturePressed(string.format("ContainerZuJi_left_btn_%d_3.png",(tonumber(v:getWidgetByName("title_btn_font").titleTypeid))),ccui.TextureResType.plistType)
					v:getWidgetByName("title_btn"):loadTexture("ContainerZuJi_2.png",ccui.TextureResType.plistType)
					v:getWidgetByName("title_btn_animal"):setVisible(false)
				end
				subItem:getWidgetByName("title_btn"):loadTexture("ContainerZuJi_3.png",ccui.TextureResType.plistType)
				subItem:getWidgetByName("title_btn_font"):loadTexture(string.format("ContainerZuJi_left_btn_%d_2.png",(tonumber(var.my_title_preview[subItem.tag].type_id))),ccui.TextureResType.plistType)
				--subItem:getWidgetByName("title_btn_font"):loadTexturePressed(string.format("ContainerZuJi_left_btn_%d_2.png",(tonumber(var.my_title_preview[subItem.tag].type_id))),ccui.TextureResType.plistType)
			...
			subItem:getWidgetByName("title_btn_font"):loadTexture(string.format("ContainerZuJi_left_btn_%d_1.png",(tonumber(var.my_title_preview[subItem.tag].type_id))),ccui.TextureResType.plistType)
			subItem:getWidgetByName("title_btn"):loadTexture("ContainerZuJi_2.png",ccui.TextureResType.plistType)
			subItem:getWidgetByName("title_btn_animal"):setVisible(false)
			--subItem:getWidgetByName("title_btn_font"):loadTexturePressed(string.format("ContainerZuJi_left_btn_%d_3.png",(tonumber(var.my_title_preview[subItem.tag].type_id))),ccui.TextureResType.plistType)
			...
			local function startShowTitleBg()
				...
				title_animal:loadTexture(string.format("ContainerZuJi_%d.png",startNum), ccui.TextureResType.plistType)
...
function ContainerCharacter_ZuJi.TryTitleShow(titleid,listindex)
	ContainerCharacter_ZuJi.tryOnTitle(titleid);
	--[[
...
function ContainerCharacter_ZuJi.openPageTitle()
	local tabhtitle = var.xmlPanel:getWidgetByName("tabhtitle")
	...
	--
	ContainerCharacter_ZuJi:updateGameMoney(var.xmlPanel)
	--
...
return ContainerCharacter_ZuJi