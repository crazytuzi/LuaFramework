local V4_ContainerJieFengXiTong = {}
local var = {}
...
function V4_ContainerJieFengXiTong.initView(extend)
	var = {
		xmlPanel,
		
	}
	
	var.xmlPanel = GUIAnalysis.load("ui/layout/V4_ContainerJieFengXiTong.uif")
	if var.xmlPanel then
		cc.EventProxy.new(GameSocket,var.xmlPanel)
			:addEventListener(GameMessageCode.EVENT_PUSH_PANEL_DATA, V4_ContainerJieFengXiTong.handlePanelData)
		
		V4_ContainerJieFengXiTong.showTitleAnimation()
		V4_ContainerJieFengXiTong.showRightAnimation()
					
		return var.xmlPanel
	end
end

function V4_ContainerJieFengXiTong.showUI()
	
	var.xmlPanel:getWidgetByName("dalu1"):addClickEventListener(function ( sender )
		V4_ContainerJieFengXiTong.showMapList(1)
	end)
	var.xmlPanel:getWidgetByName("dalu2"):addClickEventListener(function ( sender )
		V4_ContainerJieFengXiTong.showMapList(2)
	end)
	var.xmlPanel:getWidgetByName("dalu3"):addClickEventListener(function ( sender )
		V4_ContainerJieFengXiTong.showMapList(3)
	end)
	var.xmlPanel:getWidgetByName("dalu4"):addClickEventListener(function ( sender )
		V4_ContainerJieFengXiTong.showMapList(4)
	end)
	
	
	var.xmlPanel:getWidgetByName("btn_back"):addClickEventListener(function ( sender )
		V4_ContainerJieFengXiTong.showMapHome()
	end)
...
function V4_ContainerJieFengXiTong.showTitleAnimation()
...
function V4_ContainerJieFengXiTong.showRightAnimation()
...
function V4_ContainerJieFengXiTong.showMapHome()
	local bg = var.xmlPanel:getWidgetByName("bg")
	bg:loadTexture("panel_jfxt_1.png",ccui.TextureResType.plistType)
...
function V4_ContainerJieFengXiTong.showMapList(mapindex)
	local level = GameCharacter._mainAvatar:NetAttr(GameConst.net_level)
	local zslevel = GameCharacter._mainAvatar:NetAttr(GameConst.net_zslevel)
	...
	local bg = var.xmlPanel:getWidgetByName("bg")
	bg:loadTexture("panel_jfxt_9.png",ccui.TextureResType.plistType)
	...
	list_btn:reloadData(#currentDaLuInfo.data,function( subItem )
		...
		local function  showMapDetail( sender )
			if sender.mapInfoIndex==7 then
				local mParam = {
					name = GameMessageCode.EVENT_SHOW_TIPS, str = "confirm", lblConfirm = "分解将获得阶位*2数量的时装碎片，背包中不大于"..currentDaLuInfo.data[sender.mapInfoIndex].maxLevel.."阶时装全部拆分，请先将无需拆分的时装放入仓库",
					btnConfirm = "继续", btnCancel = "取消",
					confirmCallBack = function ()
						GameSocket:PushLuaTable("npc.v4_JieFengXiTong.handlePanelData",GameUtilSenior.encode({actionid = "exchange",dalu=currentDaLuInfoIndex,mapindex=sender.mapInfoIndex}))
					end
				}
				GameSocket:dispatchEvent(mParam)
			else
				local mParam = {
					name = GameMessageCode.EVENT_SHOW_TIPS, str = "confirm", lblConfirm = "解封需消耗："..currentDaLuInfo.data[sender.mapInfoIndex].fromItem..",元宝x"..currentDaLuInfo.data[sender.mapInfoIndex].needVcoin..",玄辰币x"..currentDaLuInfo.data[sender.mapInfoIndex].needBindGameMoney..",RMBx"..currentDaLuInfo.data[sender.mapInfoIndex].needBindVcoin.."，失败后材料不退。",
		...
		subItem:getWidgetByName("select_map"):loadTextureNormal("panel_jfxt_"..(tonumber(data.res)+0)..".png",ccui.TextureResType.plistType)
...
function V4_ContainerJieFengXiTong.handlePanelData(event)
	if event.type == "v4_PanelJieFengXiTong" then
		local data = GameUtilSenior.decode(event.data)
		if data.cmd =="getMessage" then
			mapList = data.mapList
			V4_ContainerJieFengXiTong.showUI()
		end
	end
end


function V4_ContainerJieFengXiTong.onPanelOpen(extend)
	GameSocket:PushLuaTable("npc.v4_JieFengXiTong.handlePanelData",GameUtilSenior.encode({actionid = "getMessage"}))
end

function V4_ContainerJieFengXiTong.onPanelClose()

end

return V4_ContainerJieFengXiTong