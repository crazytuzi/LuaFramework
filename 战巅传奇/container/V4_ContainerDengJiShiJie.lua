local V4_ContainerDengJiShiJie = {}
local var = {}
...
function V4_ContainerDengJiShiJie.initView(extend)
	var = {
		xmlPanel,
		
	}
	
	var.xmlPanel = GUIAnalysis.load("ui/layout/V4_ContainerDengJiShiJie.uif")
	if var.xmlPanel then
		cc.EventProxy.new(GameSocket,var.xmlPanel)
			:addEventListener(GameMessageCode.EVENT_PUSH_PANEL_DATA, V4_ContainerDengJiShiJie.handlePanelData)
		
		V4_ContainerDengJiShiJie.showTitleAnimation()
		V4_ContainerDengJiShiJie.showRightAnimation()
					
		return var.xmlPanel
	end
end

function V4_ContainerDengJiShiJie.showUI()
	
	var.xmlPanel:getWidgetByName("dalu1"):addClickEventListener(function ( sender )
		V4_ContainerDengJiShiJie.showMapList(1)
	end)
	var.xmlPanel:getWidgetByName("dalu2"):addClickEventListener(function ( sender )
		V4_ContainerDengJiShiJie.showMapList(2)
	end)
	var.xmlPanel:getWidgetByName("dalu3"):addClickEventListener(function ( sender )
		V4_ContainerDengJiShiJie.showMapList(3)
	end)
	var.xmlPanel:getWidgetByName("dalu4"):addClickEventListener(function ( sender )
		V4_ContainerDengJiShiJie.showMapList(4)
	end)
	...
	var.xmlPanel:getWidgetByName("map_enter_1"):addClickEventListener(function ( sender )
		...
		GameSocket:PushLuaTable("npc.v4_dengjishijie.handlePanelData",GameUtilSenior.encode({actionid = "enterMap",dalu=currentDaLuInfoIndex,mapindex=sender.mapInfoIndex,mapno=1}))
	...
	var.xmlPanel:getWidgetByName("map_enter_2"):addClickEventListener(function ( sender )
		...
		GameSocket:PushLuaTable("npc.v4_dengjishijie.handlePanelData",GameUtilSenior.encode({actionid = "enterMap",dalu=currentDaLuInfoIndex,mapindex=sender.mapInfoIndex,mapno=2}))
	end)
	
	
	var.xmlPanel:getWidgetByName("btn_back"):addClickEventListener(function ( sender )
		V4_ContainerDengJiShiJie.showMapHome()
	end)
...
function V4_ContainerDengJiShiJie.showTitleAnimation()
...
function V4_ContainerDengJiShiJie.showRightAnimation()
...
function V4_ContainerDengJiShiJie.showMapHome()
...
function V4_ContainerDengJiShiJie.showMapList(mapindex)
	local level = GameCharacter._mainAvatar:NetAttr(GameConst.net_level)
	local zslevel = GameCharacter._mainAvatar:NetAttr(GameConst.net_zslevel)
	...
	list_btn:reloadData(#currentDaLuInfo.data,function( subItem )
		...
		local function  showMapDetail( sender )
			...
			var.xmlPanel:getWidgetByName("map_enter_1"):loadTextureNormal("panel_djsj_"..(tonumber(currentDaLuInfo.data[sender.mapInfoIndex].bigres)+0)..".png",ccui.TextureResType.plistType)
			var.xmlPanel:getWidgetByName("map_enter_1"):loadTexturePressed("panel_djsj_"..(tonumber(currentDaLuInfo.data[sender.mapInfoIndex].bigres)+1)..".png",ccui.TextureResType.plistType)
			var.xmlPanel:getWidgetByName("map_enter_2").mapInfoIndex = sender.mapInfoIndex
			var.xmlPanel:getWidgetByName("map_enter_2"):loadTextureNormal("panel_djsj_"..(tonumber(currentDaLuInfo.data[sender.mapInfoIndex].bigres)+2)..".png",ccui.TextureResType.plistType)
			var.xmlPanel:getWidgetByName("map_enter_2"):loadTexturePressed("panel_djsj_"..(tonumber(currentDaLuInfo.data[sender.mapInfoIndex].bigres)+3)..".png",ccui.TextureResType.plistType)
		end
		
		local data = currentDaLuInfo.data[subItem.tag]
		subItem:getWidgetByName("select_map"):loadTextureNormal("panel_djsj_"..(tonumber(data.res)+0)..".png",ccui.TextureResType.plistType)
		subItem:getWidgetByName("select_map"):loadTexturePressed("panel_djsj_"..(tonumber(data.res)+1)..".png",ccui.TextureResType.plistType)
		subItem:getWidgetByName("select_map").mapInfoIndex = subItem.tag
		--subItem:getWidgetByName("select_map"):addClickEventListener(V4_ContainerDengJiShiJie.showMapDetail)
...
function V4_ContainerDengJiShiJie.handlePanelData(event)
	if event.type == "v4_PanelDengJiShiJie" then
		local data = GameUtilSenior.decode(event.data)
		if data.cmd =="getMessage" then
			mapList = data.mapList
			V4_ContainerDengJiShiJie.showUI()
		end
	end
end


function V4_ContainerDengJiShiJie.onPanelOpen(extend)
	GameSocket:PushLuaTable("npc.v4_dengjishijie.handlePanelData",GameUtilSenior.encode({actionid = "getMessage"}))
end

function V4_ContainerDengJiShiJie.onPanelClose()

end

return V4_ContainerDengJiShiJie