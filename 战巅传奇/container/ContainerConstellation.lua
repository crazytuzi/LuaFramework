...
function PanelGameConstellation.initView(extend)
	var = {
		xmlPanel,
		showOtherCharacter,
	...
	local function updateEquips()
		if var.showOtherCharacter==nil or var.showOtherCharacter=="" or var.showOtherCharacter==false then
			...
			GameUtilSenior.print_table( equipInfo.mTypeID )
				...
				--var.xmlPanel:getWidgetByName("equip_gray"..i):setVisible(false)
			else
				param = {
					parent = equip_block,
					-- pos	= equip_info[i].pos,--左边的装备tips显示在右边，反之亦然
				...
				--var.xmlPanel:getWidgetByName("equip_gray"..i):setVisible(true):loadTexture(equipFlagRes[equip_info[i].pos], ccui.TextureResType.plistType)
			end
			GUIItem.getItem(param)
		end
		end
	...
	var.xmlPanel = GUIAnalysis.load("ui/layout/ContainerConstellation.uif")
	if var.xmlPanel then
		GameUtilSenior.asyncload(var.xmlPanel, "bg", "ui/image/srxz_bg.png")
		updateEquips()
		return var.xmlPanel
	end
end

function PanelGameConstellation.onPanelOpen(extend)
	
end

function PanelGameConstellation.onPanelClose()

end

return PanelGameConstellation