local ContainerRechargePointGift={}
local var = {}

function ContainerRechargePointGift.initView()
	var = {
		xmlPanel = nil,	
		serverData = nil,
		vcionNum=nil,
	}
	var.xmlPanel = GUIAnalysis.load("ui/layout/ContainerRechargePointGift.uif")
	if var.xmlPanel then
		--GameUtilSenior.asyncload(var.xmlPanel, "panelBg", "ui/image/recharge_daily.png")
		cc.EventProxy.new(GameSocket,var.xmlPanel)
			:addEventListener(GameMessageCode.EVENT_PUSH_PANEL_DATA, ContainerRechargePointGift.handlePanelData)

		var.vcionNum = ccui.TextAtlas:create("0123456789","image/typeface/num_4.png", 16, 20,"0")
			:addTo(var.xmlPanel)
			:align(display.CENTER, 440,332)
			:setString(0)
		...
		local function prsBtnClick(sender)
			...
			elseif sender.state==1 then
				GameSocket:PushLuaTable("gui.ContainerRechargePointGift.onPanelData", GameUtilSenior.encode({actionid = "receive"}))
			end
		end
...
function ContainerRechargePointGift.onPanelOpen()
	GameSocket:PushLuaTable("gui.ContainerRechargePointGift.onPanelData", GameUtilSenior.encode({actionid = "reqUpdateData"}))
end

function ContainerRechargePointGift.onPanelClose()
	
end

function ContainerRechargePointGift.handlePanelData(event)
	if event.type ~= "ContainerRechargePointGift" then return end
	local data = GameUtilSenior.decode(event.data)
	-- print(event.data)
	if data.cmd =="updateRechargeAwards" then
		ContainerRechargePointGift.updatePanel(data)
	end
end

function ContainerRechargePointGift.updatePanel(data)
...
return ContainerRechargePointGift