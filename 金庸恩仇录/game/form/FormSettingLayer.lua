local FormSettingLayer = class("FormSettingLayer", function (param)
	...
	return require("utility.MyLayer").new({})
end)
...
function FormSettingLayer:ctor(param)
	...
	rootnode.titleLabel:setString(common:getLanguageString("@SettingsFormation"))
	...
	local function touchBegan(param)
		if not param.cardnode:isOpen() or param.cardnode:empty() then
		else
			_x, _y = param.image:getPosition()
			reorderCard(param.cardnode:getParent())
		end
	end
	local touchMove = function (param)
