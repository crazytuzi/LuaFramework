...
function Item:create(param)
	...
	--×°±¸
	self._rootnode.equipBtn:addHandleOfControlEvent(function(sender, eventName)
...
function Item:refresh(param)
	...
	for i = 1, 5 do
		if i <= _itemData.baseData.star[1] then
	...
	self.pzLabel:setString(common:getLanguageString("@Aptitudes", _itemData.baseData.arr_zizhi[_itemData.data.cls + 1]))
...
local HeroChooseScene = class("HeroChooseScene", BaseScene)

function HeroChooseScene:ctor(param)
	game.runningScene = self
	HeroChooseScene.super.ctor(self, {
	...
	local formList = game.player:getFormList()
	local helpList = HelpLineModel:getHelpList()
	local heroResInform = {}
	for key, hero in pairs(formList) do
		if _fromLayer == HEROINFOLAYER_FROM.FROM_NORMAL then
			if key ~= _index then
				heroResInform[hero.resId] = true
			end
		else
			heroResInform[hero.resId] = true
		end
	end
	for key, hero in pairs(helpList) do
		if _fromLayer == HEROINFOLAYER_FROM.FROM_ZHENWEI then
			if key ~= tostring(_index) then
				heroResInform[hero.resId] = true
			end
		else
			heroResInform[hero.resId] = true
	...
	for i = 1, #heroList do
		dump(heroList[i])
		if heroList[i].pos == 0 and not heroResInform[heroList[i].resId] and heroList[i].supportPos == 0 then
			_data[#_data + 1] = {
	...
	local function changeHelp(cellIdx)
		HelpLineModel:changeHelp({
		pos = getPos(),
		id = _data[cellIdx + 1].data._id,
		callback = function(data)		
			if _callback then
	...
	local changeFunc = onEquip
	if _fromLayer and _fromLayer == HEROINFOLAYER_FROM.FROM_ZHENWEI then
		changeFunc = changeHelp
	end
...
function HeroChooseScene:onEnter()
	game.runningScene = self
	HeroChooseScene.super.onEnter(self)
end

function HeroChooseScene:onExit()
	HeroChooseScene.super.onExit(self)
