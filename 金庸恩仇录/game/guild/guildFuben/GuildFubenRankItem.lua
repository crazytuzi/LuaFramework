...
function GuildFubenRankItem:createTTF(text, color, nodeName, size)
	...
	ResMgr.replaceKeyLableEx(lbl, self._rootnode, nodeName, 0, 0)
	return lbl
end
...
function GuildFubenRankItem:updateItem(itemData)
	...
	if itemData.rank > 3 then
		...
		local rankNumLbl = self:createTTF(tostring(itemData.rank), cc.c3b(251, 235, 197), "rank_num_lbl", fontSize)
