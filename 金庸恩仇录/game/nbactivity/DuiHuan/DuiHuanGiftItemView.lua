...
function DuiHuanGiftItemView:setUpView()
	self._containner = display.newScale9Sprite("#reward_item_bg.png", 0, 0, cc.size(self._frameSize.width - self._leftToRightOffset * 2, self._frameSize.height - self._topToDownOffset * 2)):pos(self._frameSize.width / 2, self._frameSize.height / 2 + 10)
	...
	nameLabel:setPosition(cc.p(titleBng:getContentSize().width * 0.5 - nameLabel:getContentSize().width / 2, titleBng:getContentSize().height * 0.5))
...
function DuiHuanGiftItemView:createItem(index, itemsViewBngs, containnerSize, data)
	...
	local marginLeft = 10
	local offset = 130
	if tonumber(data[1]) == ITEM_TYPE.zhenqi then
	...
	local nameColor = ResMgr.getItemNameColorByType(tonumber(data[2]), ResMgr.getResType(tonumber(data[1])))
	local nameLabel = ui.newTTFLabelWithShadow({
	...
	if tonumber(data[1]) == 6 then
		self._icon:setPositionY(self._icon:getPositionY() - 2)
		self._icon:setScale(0.95)
