...
local HeroInfoScene = class("HeroInfoScene", BaseScene)
...
function HeroInfoScene:ctor(param, infoType)
	...
	if infoType == 3 then
		self._rootnode["changeBtn"]:setVisible(false)
		self._rootnode["qiangHuBtn"]:setVisible(false)
		self._rootnode["jinJieBtn"]:setTitleForState("返回", CCControlStateNormal)
	end
...
function HeroInfoScene:onEnter()
	game.runningScene = self
	HeroInfoScene.super.onEnter(self)
	--[[
	if self._bExit then
		self._bExit = false
...
function HeroInfoScene:onExit()
	HeroInfoScene.super.onExit(self)
	self._bExit = true
