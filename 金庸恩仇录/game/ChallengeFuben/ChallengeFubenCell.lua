...
function ChallengeFubenCell:create(param)
	...
	self._bIsAllowPlay = param.bIsAllowPlay
	local proxy = CCBProxy:create()
	self._rootnode = {}
...
function ChallengeFubenCell:refreshItem(itemData)
	...
	local iconBg
	if self._bIsAllowPlay == true and itemData.isOpen == true then
		...
		iconBg = display.newGraySprite(titlIconName, {
		0.4,
		0.4,
		0.4,
		0.1
	...
	if itemData.isOpen == true then
		...
		self._rootnode.level_lbl:setString(common:getLanguageString("@Open", tostring(itemData.needLv)))
	end
end
