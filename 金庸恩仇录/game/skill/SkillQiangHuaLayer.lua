...
function SkillQiangHuaLayer:ctor(param)
	...
	for k, v in ipairs(game.player:getSkills()) do
		if self._info._id ~= v._id and v.cid == 0 and v.propsN == 0 and (data_item_item[v.resId].pos == 96 + _pos or _pos == data_item_item[v.resId].pos and data_item_item[v.resId].lysis and data_item_item[v.resId].lysis == 1) then
	...
	local function onChooseLayer(tag, sender)
		...
		local needExp = self:getNeedMaxExp()
	...
	self._rootnode.qianghuaBtn:addHandleOfControlEvent(function(eventName, sender)
		...
		if get_table_len(self._selected) > 0 then
			...
			show_tip_label(common:getLanguageString("@xiaohaowx"))
		end
	end,
	CCControlEventTouchUpInside)
...
function SkillQiangHuaLayer:getNeedMaxExp()
	local maxLevel = data_shangxiansheding_shangxiansheding[4].level
	local maxExp = data_kongfu_kongfu[maxLevel + 1].sumexp[self._info.star]
	local curAllExp = data_kongfu_kongfu[self._info.lv + 1].sumexp[self._info.star] + self._info.exp
	return maxExp - curAllExp
end

function SkillQiangHuaLayer:getSkillExpValue(skilldata)
	local curAllExp = data_kongfu_kongfu[skilldata.data.level + 1].sumexp[skilldata.baseData.quality] + skilldata.data.curExp + skilldata.baseData.exp
	return curAllExp
end
...
function SkillQiangHuaLayer:autoSelected()
	...
	local needExp = self:getNeedMaxExp()
	if needExp then
		for key, value in pairs(self._selected) do
			local exp = self:getSkillExpValue(self._listData[key])
			needExp = needExp - exp
		end
	end
	local bAuto = false
	for k, v in ipairs(self._listData) do
		...
		if self._selected[k] ~= true then
			if v.baseData.autoadd == 1 then
				...
				if needExp then
					local exp = self:getSkillExpValue(v)
					needExp = needExp - exp
				end
			else
				bAuto = true
	...
	if count() == 0 then
		if bAuto then
			show_tip_label(common:getLanguageString("@meiyouwx"))
		else
			show_tip_label(common:getLanguageString("@wuxueslbz"))
		end
	end
	self:refreshIcon()
end
...
function SkillQiangHuaLayer:initBaseInfo()
	...
	ResMgr.replaceKeyLableEx(heroName, self._rootnode, "itemNameLabel", 0, 0)
...
function SkillQiangHuaLayer:refreshSkillInfo()
	...
	bar:setTextureRect(cc.rect(bar:getTextureRect().x, bar:getTextureRect().y, BAR_SIZE.width * percent, BAR_SIZE.height))
	...
	for i = 1, 4 do
		if baseState[i] ~= 0 then
			...
			self._rootnode["stateName" .. index]:setString(baseNameStrs[i])
	...
	if unlockStates and unlockStates.arr_nature1 then
		for k, v in ipairs(unlockStates.arr_level) do
			...
			if v <= data_shangxiansheding_shangxiansheding[8].level then
				if v > self._info.lv then
					str = str .. "(" .. v .. common:getLanguageString("@jiesuo")
...
function SkillQiangHuaLayer:refreshIcon()
	...
	local maxLevel = data_shangxiansheding_shangxiansheding[4].level
	while tmpLv < maxLevel and tmpExp >= data_kongfu_kongfu[tmpLv + 1].exp[data_item_item[self._info.resId].quality] do
