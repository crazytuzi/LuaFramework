...
function YabiaoItemView:setUpView(param)
	...
	if self._mid then
		...
		local x = randomPosX(self._time, self._roleId)
		self._posX = posX[x]
	end
	self.carSprite = nil
	if self._mid then
	...
	local timeTitle = ui.newTTFLabel({
	text = common:getLanguageString("@daodasj"),
	font = FONTS_NAME.font_fzcy,
	align = ui.TEXT_ALIGN_LEFT,
	size = 22,
	...
	levelLabel:setPosition(cc.p(upShadowBng:getContentSize().width / 2, upShadowBng:getContentSize().height * 1 / 3 - 5))
	timeTitle:setPosition(cc.p(20, downShadowBng:getContentSize().height / 2))
	...
	size = self.carSprite:getContentSize(),
	swallow = true,
	parent = self.carSprite,
	touchHandler = function (event)
		if event.name == "began" then
			GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_guanbi))
			local function erroFunc()
				show_tip_label(data_error_error[3400007].prompt)
				self:completeTask()
			end
...
function YabiaoItemView:getYposBytime(time)
	return mapHeight * (1 - time / (self._totalTime * 60)) + 20
end
