...
function HuoDongCell:refresh(aid)
	...
	self.openCnt = HuoDongFuBenModel.getFubenData(actId).openCnt
	...
	if self.isAllowPlay == true then
		self._numLbl = nil
		self:setLeftCnt(HuoDongFuBenModel.getFubenData(actId).surplusCnt)
		if huodongData.isbuy == 1 then
			local plusBtn = ui.newImageMenuItem({
			image = "#plus_btn.png",
			imageSelected = "#plus_btn.png",
			})
			plusBtn:registerScriptTapHandler( function()
				GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
				if HuoDongFuBenModel.getFubenData(actId).surplusCnt > 0 then
					ResMgr.showMsg(6)
				else
					local buyMsgBox = require("game.Challenge.HuoDongBuyMsgBox").new({
					aid = actId,
					removeListener = self.refreshFunc
					})
					display.getRunningScene():addChild(buyMsgBox, 1000)
				end
			end)
			local x, y = leftCntLbl:getPosition()
			plusBtn:setScale(0.8)
			plusBtn:align(display.CENTER)
			local menu = ui.newMenu({plusBtn})
			menu:align(display.CENTER, x, y)
			leftCntLbl:getParent():addChild(menu)
		end
		...
		if itemID ~= 0 then
			...
			local useStr = ResMgr.createShadowMsgTTF({
			text = common:getLanguageString("@Use"),
			color = cc.c3b(0, 255, 0)
			})
			rowTable[#rowTable + 1] = useStr
			local itemData = data_item_item[itemID]
			local itemColor = NAME_COLOR[itemData.quality]
			local itemStr = ResMgr.createShadowMsgTTF({
			text = itemData.name,
			color = itemColor
			})
			...
			local chaStr = ResMgr.createShadowMsgTTF({
			text = common:getLanguageString("@ChallengeHave", itemNum),
			color = cc.c3b(0, 255, 0)
			})
			local x, y = leftCntLbl:getPosition()
			rowTable[#rowTable + 1] = chaStr
			local jiefuDescNode = ResMgr.getArrangedNode(rowTable)
			jiefuDescNode:setPosition(x - jiefuDescNode.rowWidth, y - 50)
			leftCntLbl:getParent():addChild(jiefuDescNode)
		end
	end
	local desc
	if huodongData.index ~= nil then
		...
		desc:align(display.CENTER, item:getContentSize().width / 2, item:getContentSize().height / 2)
		item:addChild(desc)
	end
end

function HuoDongCell:setLeftCnt(cnt)
	if self.isAllowPlay == true then
		local text = common:getLanguageString("@ToDayNumber", tostring(cnt))
		if self._numLbl == nil then
			self._numLbl = ui.newTTFLabelWithOutline({
			text = text,
			size = 22,
			color = cc.c3b(0, 255, 0),
			outlineColor = FONT_COLOR.BLACK,
			...
			ResMgr.replaceKeyLable(self._numLbl, self._rootnode.leftCnt_Lbl, -30, 0)
			self._numLbl:align(display.RIGHT_CENTER)
		else
			self._numLbl:setString(text)
		end
	end
end
