...
function GiftGetOkPopup:setUpView()
	...
	self:addChild(mask)
	self._mianPopup = display.newScale9Sprite("#win_base_bg2.png", 0, 0, cc.size(display.width - self._mainFrameWidthOffset, (display.width - self._mainFrameWidthOffset) / 2 + 90)):pos(display.cx, display.cy):addTo(self)
	self._mainPopupSize = self._mianPopup:getContentSize()
	self._innerContainerHeight = self._mainPopupSize.height - 80
	self._innerContainer = display.newScale9Sprite("#win_base_inner_bg_light.png", 0, 0, cc.size(self._mainPopupSize.width - self._innerContainerBorderOffset * 2, self._innerContainerHeight)):pos(self._mainPopupSize.width / 2, self._innerContainerBorderOffset):addTo(self._mianPopup)
	self._innerContainerSize = self._innerContainer:getContentSize()
	self._innerContainer:setAnchorPoint(cc.p(0.5, 0))
	cc.ui.UIPushButton.new(btnCloseRes):onButtonClicked(function()
	...
	text = common:getLanguageString("@ReachGet", self._data.jifen),
	size = 22,
	color = cc.c3b(225, 225, 225),
	shadowColor = FONT_COLOR.BLACK,
	font = FONTS_NAME.font_fzcy,
	...
	local itemViewBng = display.newScale9Sprite("#heroinfo_title_bg.png", 0, 0, cc.size(self._innerContainerSize.width - marginLeft - marginRight, itemsViewHeight)):pos(self._innerContainerSize.width / 2, disLabel:getPositionY() - disLabel:getContentSize().height - marginTop):addTo(self._innerContainer)
