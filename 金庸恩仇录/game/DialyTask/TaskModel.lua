TaskModel = {_data = nil, _instance = nil}
...
function TaskModel:_getTaskList(type)
	local result = {}
	for k, v in pairs(self._data.missions) do
...
function TaskModel:getRewordedList()
	return self._data.dailyMission.reawrds
end
...
function TaskModel:removeTaskById(id)
	for k, v in pairs(self._data.missions) do
		if v.missionDefineId == id then
			table.remove(self._data.missions, k)
		end
	end
end
...
function TaskModel:getTaskProgressById(id)
	for k, v in pairs(self._data.missions) do
		if v.missionDefineId == id then
			return v.missionDetail
		end
	end
end
...
function TaskModel:insertReword(id)
	table.insert(self._data.dailyMission.reawrds, id)
end
...
function TaskModel:getJifen()
	return self._data.dailyMission.jifen
end
...
function TaskModel:getTaskList(type)
	...
	for k, v in pairs(self:_getTaskList(type)) do
		...
		if dataBase then
			dataTemp.name = dataBase.name or common:getLanguageString("@NoName")
			dataTemp.dis = dataBase.description or common:getLanguageString("@NoDescription")
			dataTemp.jifen = dataBase.jifen or common:getLanguageString("@NoScore")
			dataTemp.missionDetail = v.missionDetail or common:getLanguageString("@NoProgress")
			dataTemp.id = v.missionDefineId
			local Temp = string.split(dataBase.prams, ",")
			local ret = Temp[#Temp]
			local bigan = #Temp == 1 and 2 or 1
			dataTemp.totalStep = string.sub(ret, bigan, string.len(ret) - 1) or common:getLanguageString("@NoTotalProgress")
			dataTemp.order = dataBase.renwupaixu
			dataTemp.missionCategory = v.missionCategory or common:getLanguageString("@NoClassify")
			dataTemp.status = v.status or common:getLanguageString("@NoStatus")
			dataTemp.quality = dataBase.quality or 0
			dataTemp.icon = dataBase.icon or "jinridenglu"
			dataTemp.goto = dataBase.navigation or dump(common:getLanguageString("@NoGPS"))
...
function TaskModel:getGoldBoxStateList()
	local ret = {}
	for k, v in pairs(data_jifenjiangli_jifenjiangli) do
		if v.jifen <= self:getJifen() then
			if self:getJifenState(v.id) then
				ret[k] = 2
			else
				ret[k] = 1
			end
...
function TaskModel:getActivityList()
	local tempData = {}
	self._data.extendMissionDefines = self._data.extendMissionDefines or {}
	for k, v in pairs(self._data.extendMissionDefines) do
		local data = {}
		data.title = v.name
		...
		data.renWuPaiXu = v.renWuPaiXu
		data.property = v.property
		data.totalStep = v.prams[#v.prams]
		data.curenStep = self:getTaskProgressById(v.id)
		local rewords = {}
		for key, var in pairs(v.rewardIds) do
			...
			rewordsItem.name = ResMgr.getItemNameByType(var, rewordsItem.iconType)
		...
		if data.property == 1008 then
			data.title = data.title .. "，"
			data.totalStep = v.prams[2]
		end
	end
	table.sort(tempData, function(a, b)
		return a.renWuPaiXu < b.renWuPaiXu
	end)
	return tempData
end
...
function TaskModel:checkRewordable(id)
	...
	if data_jifenjiangli_jifenjiangli[id] then
		...
		if minJinfen <= self:getJifen() and not self:getJifenState(id) then
			ret = true
		end
	end
	return ret
