...
function BattleScene:jumpFunc()
	...
	local function _callback(errorCode, mapData)
		if errorCode == "" then
			...
			if battleData.isOpenNewBigmap and battleData.cur_bigMapId == clickedBigMapId then
...
function BattleScene:result(data)
	...
	self:checkIsLevelup({beforeLevel = beforeLevel,curlevel = curlevel,
	curExp = curExp
	})
