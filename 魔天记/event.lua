...
function _event:Dump()	
	for _, v in ilist(self.list) do print("update Dump :", v.func, (v.obj and v.obj.name or "")) end	
end
...
--local LastUpdateTime = -1--UIUtil.GetMilliseconds();

--逻辑update
function Update(deltaTime, unscaledDeltaTime) 
	--if (deltaTime <= 0.03) then deltaTime = 0.03333 end
	Time:SetDeltaTime(deltaTime, unscaledDeltaTime)	
--	local curTime = UIUtil.GetMilliseconds()
--	if (LastUpdateTime == -1) then
--		LastUpdateTime = UIUtil.GetMilliseconds() - deltaTime;
--	end
	Timer.deltaTime = (Timer.deltaTime + unscaledDeltaTime) / 2
