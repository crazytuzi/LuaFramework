...
ProductSellPanel = Panel:New();

function ProductSellPanel:_Init()
    self:_InitReference();
    self:_InitListener();
end

function ProductSellPanel:_InitReference()
...
function ProductSellPanel:_InitListener()
...
function ProductSellPanel:_OnClickBtn_close()
    ModuleManager.SendNotification(ProductTipNotes.CLOSE_PRODUCTSELLPANELL);
end

function ProductSellPanel:_OnClickBtn_use()
       self.currUseNum = self._txtUsenum.text + 0;
    ProductTipProxy.SToSell(self.productInfo, self.currUseNum)
end

function ProductSellPanel:_OnBtn_min()
    self.currUseNum = 1;
    self._txtUsenum.text = "" .. self.currUseNum;
end

function ProductSellPanel:_OnBtn_max()
    local max_num = self.productInfo:GetAm();
    self.currUseNum = max_num;
    self._txtUsenum.text = "" .. self.currUseNum;
end

function ProductSellPanel:_OnClickBtn_add()
    --    self.currUseNum = self._txtUsenum.text + 0;
...
function ProductSellPanel:_OnClickBtn_sub()
    --    self.currUseNum = self._txtUsenum.text + 0;
    self.currUseNum = self.currUseNum - 1;
...
function ProductSellPanel:_Dispose()
    self:_DisposeListener();
    self:_DisposeReference();
end

function ProductSellPanel:_DisposeListener()
...
function ProductSellPanel:_DisposeReference()
    self._btn_close = nil;
    self._btn_use = nil;
    self._btn_add = nil;
    self._btn_sub = nil;
...
function ProductSellPanel:SetData(productInfo)
