...
local ArtifactPanel = class("ArtifactPanel",UIComponent);
function ArtifactPanel:New()
	self = { };
	setmetatable(self, { __index =ArtifactPanel });
	return self
end
local Mlev

function ArtifactPanel:_Init()
...
function ArtifactPanel:_InitReference()
...
function ArtifactPanel:_OnClickSkill(go)
...
function ArtifactPanel:UpdatePanel(spId)
	self:UseProduct(spId)
end

function ArtifactPanel:SetEnable(enable, panel)
...
function ArtifactPanel:_InitListener()
	...
	MessageManager.AddListener(MainUINotes, MainUINotes.ARTIFACT_CHANGE, ArtifactPanel.Update, self)
end

function ArtifactPanel:UseProduct(sid)
    local fid = nil
...
function ArtifactPanel:_OnClickBtnIcon1()
	self:SetAtts(1)
end

function ArtifactPanel:_OnClickBtnIcon2()
	self:SetAtts(2)
end

function ArtifactPanel:_OnClickBtnIcon3()
	self:SetAtts(3)
end

function ArtifactPanel:_OnClickBtnIcon4()
	self:SetAtts(4)
end
function ArtifactPanel:_SetAnimatorPlay(spr, f, trs)
...
function ArtifactPanel:SetAtts(id, auto)
...
function ArtifactPanel:SetProduct(c)
...
function ArtifactPanel:GetAttText(at, c2)
    local attr = FormationManager.GetAttAdd(at, c2)
    return ArtifactPanel:GetAttStr(attr, '\n')
end
function ArtifactPanel:GetAttAddText(at)
    local attr = FormationManager.GetAttGap(at)
	local s = ArtifactPanel:GetAttStr(attr, ' ')
    return LanguageMgr.Get("FormationUpPanel/upAdd",{s = s})
end
function ArtifactPanel:GetAttStr(attr, gap)
	local s = ''
...
function ArtifactPanel:Update(d)
...
function ArtifactPanel:OnClick(pf)
    --Warning('OnClick---' .. tostring(pf))
    self:_SendUse(pf, 1)
end
function ArtifactPanel:OnClickNot(pc)
	...
	ModuleManager.SendNotification(ProductGetNotes.SHOW_EQUIP_GET_PANEL
        , {id = pf:GetSpId(), msg= MainUINotes.CLOSE_MYROLEPANEL})
    SequenceManager.TriggerEvent(SequenceEventType.Guide.ZHENTU_TISHENG)
end
function ArtifactPanel:OnPress(f, pf, pc)
...
function ArtifactPanel:_AddNum(pc, pf)
...
function ArtifactPanel:_SendUse(pf, n)
    --Warning(tostring(self.addNum))
    if not self.addNum then return end
	MainUIProxy.SendFormationUpdate(self.cid, pf:GetSpId(), n and n or self.addNum)
    self.addNum = nil
    if self._timer then self._timer:Stop() self._timer = nil end
    SequenceManager.TriggerEvent(SequenceEventType.Guide.ZHENTU_TISHENG)
end

function ArtifactPanel:_Dispose()
...
function ArtifactPanel:_DisposeListener()
    ...
    MessageManager.RemoveListener(MainUINotes, MainUINotes.ARTIFACT_CHANGE, ArtifactPanel.Update)
end

function ArtifactPanel:_DisposeReference()
	self._btnIcon1 = nil;
	self._btnIcon2 = nil;
	self._btnIcon3 = nil;
	self._btnIcon4 = nil;
