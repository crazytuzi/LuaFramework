...
local FieldMapPanel = class("FieldMapPanel", Panel);
...
function FieldMapPanel:New()
    self = { };
    setmetatable(self, { __index = FieldMapPanel });
    return self
end 

function FieldMapPanel:_Init()
    self:_InitReference();
    self:_InitListener();
end

function FieldMapPanel:_MoveEnd()
    Util.SetLocalPos(self._trsTarget, 65535, 65535, 0)
end

function FieldMapPanel:_InitReference()
...
function FieldMapPanel:_InitListener()
...
function FieldMapPanel:_OnClickBtnCb()
...
function FieldMapPanel:_OnClickBtn_close()
    ModuleManager.SendNotification(MapNotes.CLOSE_FIELD_MAP_PANEL)
end

function FieldMapPanel:_OnClickBtnWorldMap()
    ModuleManager.SendNotification(MapNotes.OPEN_MAPWORLDPANEL)
end


function FieldMapPanel:_OnClickMap()
...
function FieldMapPanel:_Dispose()
    MessageManager.RemoveListener(PlayerManager, PlayerManager.SELFMOVEEND, FieldMapPanel._MoveEnd)
...
function FieldMapPanel:TransferLocalToWorld(posRate)
...
function FieldMapPanel:TransferWorldToLocal(pos)
...
function FieldMapPanel:_DisposeListener()
...
function FieldMapPanel:_DisposeReference()
    self._btn_close = nil;
...
function FieldMapPanel:SetData(info)
    self:UpdateMapPanel(info)
    MessageManager.AddListener(PlayerManager, PlayerManager.SELFMOVEEND, FieldMapPanel._MoveEnd, self)
    self._timer = Timer.New( function(val) self:_OnUpdata(val) end, 0.1, -1, false);
    self._timer:Start();
end
function FieldMapPanel:UpdateMapPanel(info)
    ...
    local monsterInfoes = MapProxy.GetFieldMonsters(self._map.id, ConfigManager.CONFIGNAME_WILDBOSS)
    local mapItemInfoes = { }
    local selectmonster
...
function FieldMapPanel:_OnUpdata()
...
function FieldMapPanel:SetOther(go, info)
    go:SetActive(true)
...
function FieldMapPanel:SelectItem(d)
