...
GuildWarMapPanel = class("GuildWarMapPanel", Panel);
...
function GuildWarMapPanel:_Init()
    ...
    MessageManager.AddListener(PlayerManager, PlayerManager.SELFMOVEEND, GuildWarMapPanel._MoveEnd, self)
...
function GuildWarMapPanel:_InitReference()
    ...
    self._phalanx:Init(self._phalanxInfo, nil)
    self._npcs = { }
    ...
    --    self._trsTarget.localPosition = Vector3(65535, 65535, 0)
    self._trsMain = UIUtil.GetChildByName(self._trsContent, "Transform", "trsMain");
    self._txtMain0 = UIUtil.GetChildByName(self._trsMain, "UILabel", "txtMain0");
    self._txtMain1 = UIUtil.GetChildByName(self._trsMain, "UILabel", "txtMain1");
    self._txtMain2 = UIUtil.GetChildByName(self._trsMain, "UILabel", "txtMain2");
    self._txtPoint = UIUtil.GetChildByName(self._trsContent, "UILabel", "trsPoint/txtPoint");
    self._txtMon1 = UIUtil.GetChildByName(self._trsContent, "UILabel", "trsMon1/txtMon1");
    self._txtMon2 = UIUtil.GetChildByName(self._trsContent, "UILabel", "trsMon2/txtMon2");
end

function GuildWarMapPanel:_InitListener()
    ...
    MessageManager.AddListener(GuildWarNotes, GuildWarNotes.ENV_WARINFO_CENTER_CHG, GuildWarMapPanel.UpdateMainLabel, self);    
end

function GuildWarMapPanel:_InitPoints()
    ...
    if (points) then
        for i, v in pairs(points) do
            ...
            if (info.type == 4) then
                ...
                label.color = GuildWarMapPanel.GetPointColor(pItem.isValid);
                go:SetActive(true);
...
--[[
function GuildWarMapPanel:_InitAllPlayers()
...
function GuildWarMapPanel:_RefreshList(blInit)
    local pls = { };
...
function GuildWarMapPanel:_OnClickBtn_close()
    ModuleManager.SendNotification(MapNotes.CLOSE_GUILDWARMAPPANEL)
end

function GuildWarMapPanel:_OnClickMap()
...
function GuildWarMapPanel:_Dispose()
    MessageManager.RemoveListener(PlayerManager, PlayerManager.SELFMOVEEND, GuildWarMapPanel._MoveEnd)
...
--[[
function GuildWarMapPanel:_AddRole(role)
...
function GuildWarMapPanel:_RemoveRole(role)
    local id = role.id;
...
function GuildWarMapPanel:_AddRoleHandler(role)
    if (role) then
        if (role.info.camp == self._camp) then
            self:_AddRole(role);
            self:_RefreshList();
...
function GuildWarMapPanel:_RemoveRoleHandler(role)
    if (role) then
...
function GuildWarMapPanel:TransferLocalToWorld(posRate)
...
function GuildWarMapPanel:TransferWorldToLocal(pos)
...
function GuildWarMapPanel:_DisposeListener()
    ...
    MessageManager.RemoveListener(GuildWarNotes, GuildWarNotes.ENV_WARINFO_CENTER_CHG, GuildWarMapPanel.UpdateMainLabel);
end

function GuildWarMapPanel:_DisposeReference()
    self._btn_close = nil;
...
function GuildWarMapPanel:UpdateMapPanel()
    ...
    self._txtMain1.text = LanguageMgr.Get("GuildWar/Map/Main", GuildDataManager.data);
    self._txtMain2.text = LanguageMgr.Get("GuildWar/Map/Main", {name = GuildDataManager.war.etgn});
    self._myCamp = GuildDataManager.war.camp;
...
function GuildWarMapPanel:UpdateMainLabel()
    local data = GuildDataManager.war;
    self._txtMain0.gameObject:SetActive(data.m == 3);
    self._txtMain1.gameObject:SetActive(data.camp == data.m);
    self._txtMain2.gameObject:SetActive(data.m ~= 3 and data.camp ~= data.m);
end

function GuildWarMapPanel:_OnUpdata()
    --[[
        ...
        for i, v in pairs(points) do
            ...
            if (pItem) then
                ...
                if (pItem.isValid ~= v:IsValid()) then
                    ...
                    pItem.label.color = GuildWarMapPanel.GetPointColor(pItem.isValid);
                end
    ...
    for i, v in ipairs(monsters) do
        if v.info.camp == 1 then
            monNum1 = monNum1 + 1;
        elseif v.info.camp == 2 then
            monNum2 = monNum2 + 1;
        end
    end
    
    self._txtMon1.text = self._myCamp == 1 and monNum1 or monNum2;
    self._txtMon2.text = self._myCamp == 1 and monNum2 or monNum1;

end

function GuildWarMapPanel:SetOther(go, info, isNpc)
    if (isNpc) then
...
function GuildWarMapPanel.GetPointColor(isAct)
    return isAct and ColorDataManager.Get_green() or ColorDataManager.Get_red();
end
