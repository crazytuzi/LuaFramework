VIPPOWER =
{
	...
	BUY_DABAO_COUNT					= 25,			-- 购买夺宝BOSS入场次数
	...
	BABYBOSS_ENTER_TIMES			= 36,			-- 宝宝boss进入次数
	TIANJIANG_ENTER_TIMES			= 37,			-- 天将boss进入次数
	SUOYAOTOWER_ENTER_TIMES			= 39,			-- 锁妖塔boss进入次数
}
...
function VipData:SetOpenParam(param)
	self.open_param = param
end

function VipData:GetOpenParam()
...
function VipData:GetEnterTimes(auth_type, vip_level)
	local main_vo = GameVoManager.Instance:GetMainRoleVo()
	if not auth_type  then return end
	local vip_level = vip_level or main_vo.vip_level
	for k, v in pairs(self:GetVipLevelCfg()) do
...
function VipData:SetMonthCardInfo(protocol)
	self.active_timestamp = protocol.active_timestamp
	self.last_days = protocol.last_days
	self.next_reward_day_idx = protocol.next_reward_day_idx
	self.buy_times = protocol.buy_times
end

--获取当前是否激活至尊会员
function VipData:IsActiveMonthCard()
	return self.active_timestamp > 0
end