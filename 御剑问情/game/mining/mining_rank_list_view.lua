...
function MiningRankListView:__init()
	self.ui_config = {"uis/views/mining_prefab","MiningRankView"}
	self.cell_list = {}
end
...
function MiningRankListView:LoadCallBack()
	...
	-- 生成滚动条
	self.scroller_data = RankData.Instance:GetRankList(RANK_KIND.PERSON, PERSON_RANK_TYPE.PERSON_RANK_TYPE_FIGHTING_CHALLENGE)
...
function MiningRankListView:OnFlush()
	...
	local color = "#0000f1"
	self.text_my_rank:SetValue(ToColorStr(my_rank, color))
	...
	self.text_my_score:SetValue(ToColorStr(my_score, color))
end
