...
function MiningSeaView:InitView()
	...
	self.text_tip:SetValue(Language.Mining.MiningSeaTip2)
...
function MiningSeaView:UpdataTimeText()
	...
	if self.text_go_num ~= nil then
		local num = MiningData.Instance:GetMiningSeaRobTimes()
		self.text_go_num:SetValue(num)
		if num == 0 then
			num = string.format(Language.Mining.NumColor, num)
		end
		self.text_num:SetValue(num)
	end
end
	...
	if self.item_data ~= nil then
		...
		if MiningData.Instance:GetIsMyMining(self.item_data.owner_uid) == true then
			...
			if info_data ~= nil then
				item_name = string.format(Language.Mining.Myself, info_data.name)
			end
			name = item_name
