...
function MiningData:__init()
	...
	self.capability_list={}									-- 当前决斗列表玩家的战力表
	...
	-- RemindManager.Instance:Register(RemindName.MiningMineRob, BindTool.Bind(self.GetMiningMineRobRemind, self))
	-- RemindManager.Instance:Register(RemindName.MiningSea, BindTool.Bind(self.GetMiningSeaRemind, self))
	-- RemindManager.Instance:Register(RemindName.MiningSeaRob, BindTool.Bind(self.GetMiningSeaRobRemind, self))
...
function MiningData:__delete()
	...
	-- RemindManager.Instance:UnRegister(RemindName.MiningMineRob)
	-- RemindManager.Instance:UnRegister(RemindName.MiningSeaRob)
...
function MiningData:GetMiningMineRemind()
	if self.red_point_list["MiningMine"] == 1 or self.red_point_list["MiningMineRecord"] == 1 or RemindManager.Instance:GetRemind(RemindName.MiningMineRob) > 0 then
		return 1
	end
...
--检查红点
function MiningData:GetMiningMineRobRemind()
	return self.red_point_list["MiningMineRob"] or 0
end

--检查红点
function MiningData:GetMiningSeaRemind()
	if self.red_point_list["MiningSea"] == 1 or self.red_point_list["MiningSeaRecord"] == 1 or RemindManager.Instance:GetRemind(RemindName.MiningSeaRob) > 0  then
		return 1
	end
...
--检查红点
function MiningData:GetMiningSeaRobRemind()
	return self.red_point_list["MiningSeaRob"] or 0
end
	...
	if self.red_point_list[type] ~= nil then
		if self.red_point_list[type] ~= nil then
			...
			if type == "MiningMine" or type ==  "MiningMineRob" then
				...
				RemindManager.Instance:Fire(RemindName.MiningMineRob)
			elseif type == "MiningSea" or type == "MiningSeaRob" then
				RemindManager.Instance:Fire(RemindName.MiningSea)
				RemindManager.Instance:Fire(RemindName.MiningSeaRob)
			end
		end
	end
end
...
function MiningData:GetMiningMineRobTimes()
	...
	if dm_rob_times <= self.today_rob_mine_times then
		return 0
	end
...
function MiningData:SetMiningMineList(protocol)
	...
	self.mine_list = self:SortDataList(protocol.mine_list)
end

function MiningData:SortDataList(protocol_list)
	local role_id = GameVoManager.Instance:GetMainRoleVo().role_id
	local role_cap = GameVoManager.Instance:GetMainRoleVo().capability
	local sort_list = {}
	local has_mine = false
	for k,v in pairs(protocol_list) do
		table.insert(sort_list, {index = k, data = v})
		if v.owner_uid == role_id then
			has_mine = true
		end
	end

	if not has_mine then
		table.insert(sort_list, {index = #sort_list + 1, data = {owner_uid = role_id, capability = role_cap}})
	end
	table.sort(sort_list, function (a, b)
		return a.data.capability < b.data.capability
	end)

	local mine_list = {}
	local add_list = {}
	local add_index_list = {}
	for k,v in pairs(sort_list) do
		if v.data.owner_uid == role_id then
			for i = 1, 5 do
				local min = sort_list[k - i]
				if min then
					min.random = math.random(1, 100)
					add_index_list[min.index] = true
					table.insert(add_list, min)
				end
				local max = sort_list[k + i]
				if max then
					max.random = math.random(1, 100)
					add_index_list[max.index] = true
					table.insert(add_list, max)
				end
			end
			break
		end
	end
	for i,v in ipairs(protocol_list) do
		if add_index_list[i] == nil then
			if v.owner_uid == role_id then
				table.insert(mine_list, 1, v)
			else
				table.insert(mine_list, v)
			end
		end
	end

	table.sort(add_list, function (a, b)
		return a.random < b.random
	end)
	for k,v in pairs(add_list) do
		if has_mine and mine_list[1] then
			table.insert(mine_list, 2, v.data)
		else
			table.insert(mine_list, 1, v.data)
		end
	end
	return mine_list
end
...
function MiningData:SetMiningSeaList(protocol)
	...
	self.sea_list = self:SortDataList(protocol.mine_list)
end
...
function MiningData:SetChallengeRoleInfo(opponent_list)
	...
	-- 防止机器人性别错误
	for k,v in pairs(self.opponent_list) do
		v.sex = PlayerData.Instance:GetSexByProf(v.prof)
	end
end
...
function MiningData:GetChallengeFightResuleReward()
	...
	if self.other_cfg.cf_win_item and self.other_cfg.cf_win_item[0].item_id > 0 then
		table.insert(reward_list, self.other_cfg.cf_win_item[0])
	end
...
function MiningData:SetCapabilityList(role_name, capability)
	self.capability_list[role_name] = capability
end

function MiningData:GetOtherRoleCapability(role_name)
	local capability = 0
	if self.capability_list[role_name] then
		capability = self.capability_list[role_name]
	end

	return capability
end

function MiningData:ClearCapabilityList()
