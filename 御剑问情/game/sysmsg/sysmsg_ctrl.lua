...
function SysMsgCtrl:OnNoticeNumAck(protocol)
	...
	local gather_type = GuildData.Instance:GetMoneyTreeGatherState() or 0
	if scene_type == SceneType.GuildStation and GuildData.Instance:GetMoneyTreeState() and protocol.result == 11825 and gather_type == 0 then
		GuildCtrl.Instance:MoveToTreeState(true)
	end
...
--系统中上部飘字
-- speed:飘字滚动的速度系数
function SysMsgCtrl:ErrorRemind(str, speed)
	TipsCtrl.Instance:ShowSystemMsg(str, speed)
end
