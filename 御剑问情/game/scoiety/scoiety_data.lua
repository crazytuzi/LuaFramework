...
ScoietyData.InviteOpenType = {
	...
	TeamTowerDefendInvite = "towerdefend", --塔防
	TeamYaoshouInvite = "TeamYaoshouInvite"	-- 妖兽
...
function ScoietyData:__init()
	...
	self.today_give_flower_times = 0
	...
	RemindManager.Instance:Register(RemindName.TeamFbFull, BindTool.Bind(self.CanRemindTeamFull, self))
end
...
function ScoietyData:__delete()
	...
	RemindManager.Instance:UnRegister(RemindName.TeamFbFull)
	self.mail_list = nil
...
function ScoietyData:SetFriendInfo(info)
	...
	self.today_give_flower_times = info.today_give_flower_times or 0
	table.sort(self.friend_list, ScoietyData.SortFriendList)
end

function ScoietyData:GetFlowerTimes()
	return (self.today_give_flower_times < 5)
end
...
function ScoietyData:SetEnemyList(info)
	self.enemy_list = info.enemy_list
	table.sort(self.enemy_list, ScoietyData.SortEnemyList)
end

function ScoietyData:ChangeEnemyList(info)
	if not next(info.enemy_info) or not self.enemy_list then return end
...
function ScoietyData:GetFriendRemind()
	local flower_state = self:GetFlowerTimes()
	return (self:IsSetGiftRedPoint() and flower_state) and 1 or 0
end
...
function ScoietyData:CanRemindTeamFull()
	local is_leader = self:IsLeaderById(GameVoManager.Instance:GetMainRoleVo().role_id)
	if not is_leader then
		return 0
	end

	local team_type = self.team_list.team_type or 0
	local team_member_list = self.team_list.team_member_list or {}
	if team_type == 0 or #team_member_list < 3 then
		return 0
	end
	if team_type == FuBenTeamType.TEAM_TYPE_TEAM_TOWERDEFEND then
		local num = TeamFbData.Instance:GetTeamFbCanEnterCount(DAY_COUNT.DAYCOUNT_ID_TEAM_TOWERDEFEND_JOIN_TIMES)
		return num
	elseif team_type == FuBenTeamType.TEAM_TYPE_TEAM_DAILY_FB then
		...
		local num = pay_times + other_cfg.day_times - enter_times
		return num
	elseif team_type == FuBenTeamType.TEAM_TYPE_EQUIP_TEAM_FB then
		local num = TeamFbData.Instance:GetTeamFbCanEnterCount(DAY_COUNT.DAYCOUNT_ID_EQUIP_TEAM_FB_JOIN_TIMES)
		return num
	end
	return 0
end