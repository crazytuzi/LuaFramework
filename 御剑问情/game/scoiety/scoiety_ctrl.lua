...
function ScoietyCtrl:OnTeamInfo(protocol)
	...
	if not team_state then
		...
		elseif req_team_type == ScoietyData.InviteOpenType.EquipTeamFbNew
			or req_team_type == ScoietyData.InviteOpenType.TeamTowerDefendInvite
			or req_team_type == ScoietyData.InviteOpenType.TeamYaoshouInvite then
			if not ViewManager.Instance:IsOpen(ViewName.FuBen) then
				TeamFbData.Instance:SetDefaultChoose(req_team_type)
	...
	if is_send_zhuagui_invite then
		...
		--判断能否发送世界邀请
		if self.can_send_world_chat and (Status.NowTime - self.send_team_invite_interval > self.last_send_team_time) then
...
function ScoietyCtrl:OnRoleTeamInfo(protocol)
	...
	if protocol.have_team == 0 then
		...
		--清除聊天消息
		ChatData.Instance:ClearChannelItemHeight(CHANNEL_TYPE.TEAM)
		ChatData.Instance:RemoveMsgToChannel(CHANNEL_TYPE.TEAM)
		ChatData.Instance:ClearTeamUnreadMsg()
...
function ScoietyCtrl:OnChangeBlacklist(protocol)
	...
	--刷新聊天数据
	ChatData.Instance:DelChannelListByBlack()
	ChatCtrl.Instance.view:Flush()

	--清除私聊列表中相关的聊天对象
	ChatData.Instance:RemovePrivateObjById(protocol.user_id)
	ChatCtrl.Instance:DelPriviteObjOnLocal(protocol.user_id)
...
function ScoietyCtrl:OnBlacklistsACK(protocol)
	...
	for _, v in ipairs(blacklist) do
		AvatarManager.Instance:SetAvatarKey(v.user_id, v.avatar_key_big, v.avatar_key_small)
		ChatCtrl.Instance:DelPriviteObjOnLocal(v.user_id)
	end
...
function ScoietyCtrl:ShowOperateList(open_type, name, click_obj, colse_call_back)
...
function ScoietyCtrl:ConnectToClearTeamInfo()
	MainUICtrl.Instance.view:Flush(MainUIViewChat.IconList.JOIN_REQ, {false})

	--如果队伍里面只有一个人的情况就清空队伍信息
	local member_count = self.scoiety_data:GetTeamNum()
