...
function RoleModelCell:SetRoleIcon()
	...
	CommonDataManager.NewSetAvatar(role_id, self.show_image, self.image_res, self.raw_image_obj, self.data.sex, self.data.prof, true)
end
...
function RoleModelCell:SetRoleInfo()
	...
	local prof_str = ToColorStr(Language.Common.ProfName[self.data.prof] or "", TEXT_COLOR.BLACK_1)
