...
function HunYinAllView:__init()
	self.ui_config = {"uis/views/hunqiview_prefab", "AllHunYin"}
end
...
function HunYinAllView:LoadCallBack()
	...
	self.effect_green = self:FindVariable("effect_green")
	self.effect_blue = self:FindVariable("effect_blue")
	self.effect_purple = self:FindVariable("effect_purple")
	...
	self:ListenEvent("BgClosen", BindTool.Bind(self.ClickClosen, self))
end
...
function HunYinAllView:ReleaseCallBack()
	...
	self.purple = nil
	self.orange = nil
	self.effect_green = nil
	self.effect_blue = nil
	self.effect_purple = nil
...
function HunYinAllView:OpenCallBack()
	...
	-- self.effect_green:SetAsset(ResPath.GetEffect(self.green_cell_data.effect))
	-- self.effect_blue:SetAsset(ResPath.GetEffect(self.blue_cell_data.effect))
	-- self.effect_purple:SetAsset(ResPath.GetEffect(self.purple_cell_data.effect))
...
function TopCell:OnFlush()
	...
	if nil ~= data then
		...
		local attr_data = self.hunyin_info[item_id] and self.hunyin_info[item_id][1]
