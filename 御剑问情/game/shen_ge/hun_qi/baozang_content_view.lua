...
function BaoZangContentView:ClickOpenBox(open_times)
	...
	if box_id <= 0 then
		...
		if baoxiang_num <= 0 then
			...
			HunQiCtrl.Instance.curent_open_box_type = 1 --设置开启的类型，1次还是10次
		...
		if open_times == 1 then 	
			HunQiCtrl.Instance.curent_open_box_type = 1
		else 	
			HunQiCtrl.Instance.curent_open_box_type = 10				
		end
...
function BaoZangContentView:FlushBaoZangCountDown()
	...
	if today_open_free_times < max_open_free_times then
		...
		-- local total_time = TimeCtrl.Instance:GetServerTime()
		-- local last_times = total_time - HunQiData.Instance:GetLastOpenFreeBoxTimeStamp()
		-- local x = 1
		...
		local server_time = TimeCtrl.Instance:GetServerTime()
		local now_year = os.date("%Y", server_time)
		local now_month = os.date("%m", server_time)
		local now_day = os.date("%d", server_time)
		local today_max_time_stamp = os.time({year=now_year,month=now_month,day=now_day,hour=24,min=0,sec=0})

		local left_time = today_max_time_stamp - server_time
		if left_time > 0 then
			local time_str = TimeUtil.FormatSecond(left_time)
			time_str = string.format(Language.HunQi.FreeText, time_str)
			self.free_time_des:SetValue(time_str)
			self.free_count_down = CountDown.Instance:AddCountDown(left_time, 1, timer_func)
		end
	end
end
...
function StandRoleItemCell:__init()
	...
	self.showname =self:FindVariable("ShowName")
...
function StandRoleItemCell:FlushRole(role_info)
	...
	if nil == self.model then
		self.model = RoleModel.New("hunqi_baozang_panel")
