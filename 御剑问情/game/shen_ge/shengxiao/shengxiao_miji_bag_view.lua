...
function MijiBagView:__init()
    self.ui_config = {"uis/views/shengxiaoview_prefab", "MijiBagView"}
...
function MijiBagView:GetCellNumber()  						--行数
	return math.ceil(#self.list_data/COLUMN)				--总/列数
end

function MijiBagView:CellRefresh(cell, data_index)  		--cell为绑定的prefab  data_index为当前刷新的cell下标（第几行）
	local group_cell = self.cell_list[cell]
	if not group_cell then
		group_cell = MijiBagGroupCell.New(cell.gameObject)  --把cell脚本绑定给cell的prefab
