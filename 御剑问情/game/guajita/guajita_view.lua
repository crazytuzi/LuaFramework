GuaJiTaView = GuaJiTaView or BaseClass(BaseView)

local TOP_CELL_SIZE = 343		-- 塔顶塔格子的大小
local BOTTOM_CELL_SIZE = 209	-- 塔底塔格子的大小
local NORMAL_CELL_SIZE = 192	-- 正常塔格子的大小

function GuaJiTaView:__init()
	self.ui_config = {"uis/views/guajitaview_prefab", "GuajiTaView"}
...
function GuaJiTaView:__delete()

end

function GuaJiTaView:LoadCallBack()
	...
	self.extra_item = ItemCell.New()
	self.extra_item:SetInstanceParent(self:FindObj("ExtraItem"))

	self.auto_btn = self:FindObj("AutoBtn")
	self.can_auto = self:FindVariable("CanAuto")
end

function GuaJiTaView:GetNumberOfCells()
	return GuaJiTaData.Instance:GetRuneMaxLayer() + 2
end

function GuaJiTaView:RefreshTowerCell(cell, data_index)
...
function GuaJiTaView:CellSizeDel(data_index)
...
function GuaJiTaView:JumpToIndex()
...
function GuaJiTaView:ScrollerScrolledDelegate(go, param1, param2, param3)
...
function GuaJiTaView:CellViewVisibilityChanged(obj, param1, param2)
end


function GuaJiTaView:ReleaseCallBack()
	...
	self.item_cells = {}
	if nil ~= self.extra_item then
		self.extra_item:DeleteMe()
		self.extra_item = nil
	end
...
function GuaJiTaView:OpenCallBack()
	self:Flush()
end

function GuaJiTaView:CloseCallBack()
	self.cur_layer = -1
	self.is_first_set_offtime = true
...
-- 领取离线时间
function GuaJiTaView:OnClickGetOffline()
	local rune_info = GuaJiTaData.Instance:GetRuneTowerInfo()
...
function GuaJiTaView:OnClickClose()
...
-- 点击 加号
function GuaJiTaView:OnClickAdd()
...
-- 设置离线时间
function GuaJiTaView:SetOffLineTime(offline_time)
...
function GuaJiTaView:RemoveTimerQuest()
	if self.animtion_timer_quest then
		GlobalTimerQuest:CancelQuest(self.animtion_timer_quest)
		self.animtion_timer_quest = nil
	end
...
function GuaJiTaView:OnFlush(param_list)
	...
	if special_level_cfg then
		...
		self.extra_item:SetData({item_id = special_level_cfg.sp_show})
	end
