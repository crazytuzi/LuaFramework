...
-- 给服务器发数据，请求挖矿，is_auto如果为false则表示是手动按挖矿
function KuaFuMiningGatherView:Result(angle, is_auto)
	...
	if is_auto and not KuaFuMiningData.Instance:GetMiningIsAuto() then 
		...
		KuaFuMiningCtrl.Instance:SetGuideState(false) 							--玩家没有点击挖矿且不在托管状态，则取消自动导航
		return
	end

	local cur_result = CROSS_MINING_AREA_TYPE.CROSS_MINING_AREA_TYPE_BLACK
	if is_auto and KuaFuMiningData.Instance:GetMiningIsAuto() then 				--如果处于托管状态
	...
	self:SendCSCross(MINING_REQ_TYPE.CROSS_MINING_REQ_TYPE_MINING, cur_result) 	--请求挖矿
end
...
function KuaFuMiningGatherView:AutoMining()
	...
	if KuaFuMiningData.Instance:GetIsMaxMiningTimes() then --如果采集已经达到上限
		return
	end
	local new_pos_list = {}
	...
	local new_pos_list = KuaFuMiningData.Instance:GetMinDistancePosList(pos_list)
	if not new_pos_list or not next(new_pos_list) then
		return
	end 
	MoveCache.param1 = new_pos_list[1].gather_id
	if KuaFuMiningData.Instance:GetMiningIsAuto() or KuaFuMiningCtrl.Instance:GetGuideState() then
