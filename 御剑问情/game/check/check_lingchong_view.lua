CheckLingChongView = CheckLingChongView or BaseClass(BaseRender)

function CheckLingChongView:__init(instance)
...
function CheckLingChongView:__delete()
	self.data = nil
...
function CheckLingChongView:OnFlush()
 	if self.data then
 		...
 		local temp_grade_info = LingChongData.Instance:GetLingChongGradeCfgInfoByGrade(self.data.grade)
...
function CheckLingChongView:SetAttr()
	local check_attr = CheckData.Instance:UpdateAttrView()
	if check_attr and check_attr.lingchong_attr then
		self.data = check_attr.lingchong_attr
		self:Flush()
	end
end

function CheckLingChongView:SetModle(grade)
	--对应等级数据
