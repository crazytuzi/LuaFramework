...
function CheckSpiritView:__init(instance)
	...
	for i=1,4 do
		...
		self.item_list[i].item_cell:ShowHighLight(false)
	...
	self.model = RoleModel.New("player_check_sprit_panel")
...
function CheckSpiritView:OnFlush()
	if self.spirit_attr then
		...
		local shengming = 0
		local kangbao = 0
		if self.show_spirit_id ~= 0 and spirit_cfg then
			local item_cfg = {}
		...
		self.zhan_li:SetValue(RankData.Instance:GetJingLingPower(self.show_spirit_id, self.show_spirit_level))
...
function CheckSpiritView:SetShowId(item_id)
	...
	if self.show_spirit_id ~= 0 then
		...
		local item_cfg = ItemData.Instance:GetItemConfig(self.show_spirit_id)
		self.name_text:SetValue(item_cfg.name)
...
function CheckSpiritView:SetModle()
	...
	if spirit_cfg ~= nil then
		...
		-- self.model:SetModelTransformParameter(DISPLAY_MODEL_TYPE[DISPLAY_TYPE.SPIRIT], asset, DISPLAY_PANEL.RANK)
