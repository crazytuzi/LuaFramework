CheckXianBao = CheckXianBao or BaseClass(BaseRender)

function CheckXianBao:__init(instance)
...
function CheckXianBao:__delete()
	self.data = nil
...
function CheckXianBao:OnFlush()
 	if self.data then
 		...
 		local temp_grade_info = XianBaoData.Instance:GetXianBaoGradeCfgInfoByGrade(self.data.grade)
...
function CheckXianBao:SetAttr()
	local check_attr = CheckData.Instance:UpdateAttrView()
	if check_attr and check_attr.xianbao_attr then
		self.data = check_attr.xianbao_attr
		self:Flush()
	end
end

function CheckXianBao:SetModle(grade)
	--对应等级数据
