...
function GoldHuntData:GetRewardCfgList()
	local reward_cfg_list = ServerActivityData.Instance:GetCurrentRandActivityConfig().mine_reward
...
function GoldHuntData:GetHuntOtherCfg()
	if self.hunt_other_cfg == nil then
		self.hunt_other_cfg = ServerActivityData.Instance:GetCurrentRandActivityConfig().other
	end
...
function GoldHuntData:GetHuntInfoCfg()
	local hunt_info_cfg = ServerActivityData.Instance:GetCurrentRandActivityConfig().mine_info
...
function GoldHuntData:GetMineralCloor(index)
	local info_cfg = GoldHuntData.Instance:GetHuntInfoCfg()
	for i,v in ipairs(info_cfg) do
		if v.seq == index then
			return v.color
...
function GoldHuntData:CanExchangeByIndex(index)
	local info = self:GetHuntInfo().gather_count_list
	if not info then
		return false
	end
	local exchang_cfg = self:GetHuntInfoCfg()
	if info[index - 1] >= exchang_cfg[index].exchange_need_num then
		return true
	end
	return false
end
