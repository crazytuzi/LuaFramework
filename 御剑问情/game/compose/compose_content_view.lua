...
function ComposeContentView:__init(instance)
	...
	self.item_show_list = {}
	self.item_compose_list = {}
	for i = 1, 3 do
		self.item_show_list[i] = self:FindVariable("item_show_" .. i)
        self.item_compose_list[i] = self:FindObj("item_frame_" .. i)
	end
	self.item_show_1 =  self:FindVariable("item_show_1")
	self.item_show_2 =  self:FindVariable("item_show_2")
	self.the_item_list = {}
	self.btn_num = 8
	for i=1,4 do
	...
	self.show_item_text =self:FindVariable("show_item_text")
	...
	if compose_cfg then
		...
		for i=1,3 do
			if stuff_list[i] ~= 0 then
				flag = flag + 1
				self.item_show_list[i]:SetValue(true)
				...
				self.item_show_list[i]:SetValue(false)
				local data = {}
				data.item_id = 0
        ...
        if flag == 2 then
            self.item_compose_list[1].transform.localPosition = Vector3(-45,-46,0)
            self.item_compose_list[2].transform.localPosition = Vector3(105,-46,0)
        elseif flag == 3 then
            self.item_compose_list[1].transform.localPosition = Vector3(30,-100,0)
            self.item_compose_list[2].transform.localPosition = Vector3(-98,-16,0)
            self.item_compose_list[3].transform.localPosition = Vector3(158,-16,0)
        else
            self.item_compose_list[1].transform.localPosition = Vector3(30,-60,0)
        end
		self.the_item_list[4].item_cell:SetData({item_id = self.current_item_id})
		self.show_item_text:SetValue(ItemData.Instance:GetItemConfig(self.current_item_id).name)
		-- self.show_item_text:SetValue()
	end
end
