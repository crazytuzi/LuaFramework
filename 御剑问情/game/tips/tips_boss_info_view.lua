...
function TipsBossInfoView:OnClickMoveTo()
	if self.boss_info.boss_type > 0 then
		...
		local ok_fun = function()
			...
			BossCtrl.SendEnterBossFamily(self.boss_info.boss_type, self.boss_info.scene_id, is_buy)
		end
...
function TipsBossInfoView:OnFlush()
	...
	-- 	level_text = ToColorStr(boss_info.boss_capability, TEXT_COLOR.GREEN)
	-- else
	-- 	level_text = ToColorStr(boss_info.boss_capability, TEXT_COLOR.RED)
	-- end
	...
	self.fight_power:SetValue(boss_level)
