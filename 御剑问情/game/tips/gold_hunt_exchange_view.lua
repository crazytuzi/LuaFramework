...
function GoldHuntExchangeView:__init()
	self.ui_config = {"uis/views/tips/goldhuntexchangetips_prefab", "GoldHuntExchangeTips"}
...
function GoldHuntExchangeCell:OnFlush()
	...
	self.show_red_point:SetValue(gold_hunt_data:CanExchangeByIndex(self.index))
	...
	local color = my_count >= info_cfg.exchange_need_num and TEXT_COLOR.BLUE_4 or TEXT_COLOR.RED
	self.remain_count_text:SetValue(ToColorStr(my_count, color) .." / "..info_cfg.exchange_need_num)
	local asset, name = ResPath.GetGoldHuntModelHeadImg("head_" .. self.index)
