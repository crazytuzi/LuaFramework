...
function TipsFirstChargeView:__init()
	self.ui_config = {"uis/views/tips/firstchargetip_prefab", "FirstChargeTips"}
...
function TipsFirstChargeView:OnBgClick()
	self:Close()
	-- ViewManager.Instance:Open(ViewName.FirstChargeView)
end
...
function TipsFirstChargeView:OnFlush(param_list)
	if self.model and not self.res_id then
		...
		if game_vo.prof == 1 then
			self.model:SetPanelName("trailer_tips_frame_1")
		else
			self.model:SetPanelName("trailer_tips_frame")
		end
