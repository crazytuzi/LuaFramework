...
function TipsCommonBuyView:CloseCallBack()
	if self.item_id and self.item_id > 0 and self.use_bind then
		TipsCommonBuyView.AUTO_LIST[self.item_id] = self.use_bind.toggle.isOn
	end
--	TipsOtherHelpData.Instance:SetIsAutoBuy(false)
...
function TipsCommonBuyView:OnClickBuyButton()
	...
	if self.ok_func ~= nil then
		...
		if self.use_bind.toggle.isOn then
	...
	TipsOtherHelpData.Instance:SetIsAutoBuy(self.use_bind.toggle.isOn)
	self:Close()
end

function TipsCommonBuyView:SetCallBack(callback, item_id, no_func, need_num)
...
function TipsCommonBuyView:OnFlush(param_t)
	...
	if money < shop_price or shop_item_cfg.bind_gold == 0 then
		...
		bundle, asset = ResPath.GetDiamonIcon(2)
		shop_price = shop_item_cfg.gold
	end
	self.diamon_image:SetAsset(bundle, asset)
...
function TipsCommonBuyView:FlushList(data_list)
	if self.discount_list == nil then
		...
		self.discount_list.scroller:RefreshActiveCellViews()
	end
end
...
function TipsCommonBuyDiscountItem:__init()
	...
	self.limit_buy_count = self:FindVariable("LimitBuyCount")
...
function TipsCommonBuyDiscountItem:OnFlush()
	...
	self.limit_buy_count:SetValue(self.data.limit_buy_count)
