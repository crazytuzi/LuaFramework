...
function TipsExpressView:__init()
	self.ui_config = {"uis/views/tips/chattips_prefab", "EmojiView"}
...
function TipsExpressView:ChangeView()
	...
	elseif self.show_state == DYNAMICL then
		...
		-- local bigface_count = 0
		-- local group_count = 0
		-- local big_face_cfg = CoolChatData.Instance:GetBigFaceConfig()
		-- if big_face_cfg then
		-- 	local group_cfg = big_face_cfg.group
		-- 	group_count = #group_cfg or 0
		-- 	bigface_count = group_count * COLUMN
		-- end
		-- self.cell_data = {}
		-- for i = 1, group_count do
		-- 	local active_list = CoolChatData.Instance:GetBigFaceByGroupId(i) or {}
		-- 	local j = 1
		-- 	for k,v in pairs(active_list) do
		-- 		self.cell_data[(i - 1) * COLUMN + j] = k
		-- 		j = j + 1
		-- 	end
		-- 	for j = j, COLUMN do
		-- 		self.cell_data[(i - 1) * COLUMN + j] = 0
		-- 	end
		-- end
		cur_row = BIGFACE_ROW
		cur_col = BIGFACE_COL
...
function ExpressShowGroup:__init(obj, show_state)
	...
	elseif show_state == DYNAMICL then
		for i = 1, BIGFACE_ROW do
			...
			-- BigFaceIconCell.New(self:FindObj("Icon1")),
			-- BigFaceIconCell.New(self:FindObj("Icon2")),
			-- BigFaceIconCell.New(self:FindObj("Icon3")),
			-- BigFaceIconCell.New(self:FindObj("Icon4")),
			-- BigFaceIconCell.New(self:FindObj("Icon5")),
			-- BigFaceIconCell.New(self:FindObj("Icon6")),
...
-- function BigFaceIconCell:Destroy()
-- 	if self.obj then
...
function BigFaceIconCell:ReloadIcon()
	local t = self.data - 1
	PrefabPool.Instance:Load(AssetID(ResPath.GetResBigFace(t,"Image")), function(prefab)
		if prefab then
			if nil == self.frame or nil == self.frame.transform then
				PrefabPool.Instance:Free(prefab)
				return
			end
			...
			if variable_table then
				self.gray = variable_table:FindVariable("Gray")
				if self.gray then
					self.gray:SetValue(self.is_gray)
				end
			end
...
function BigFaceIconCell:ClickIcon()
	...
	-- ChatCtrl.Instance:SetFace(face_id)
end
...
function NormalFaceIconCell:ReloadIcon()
	local t = self.data - 1
	-- local file_name =
	PrefabPool.Instance:Load(AssetID(ResPath.GetResNormalFace(t,"Image")), function(prefab)
		if prefab then
			local obj = GameObject.Instantiate(prefab)
			PrefabPool.Instance:Free(prefab)
			local transform = obj.transform
