...
function GuildTotemView:Flush()
	...
	if totem_config then
		...
		self.level:SetValue(totem_level + 1)
		...
		self.current_exp:SetValue((totem_config.percent_exp * 100) .. "%")
	...
	if totem_config then
		...
		self.next_exp:SetValue((totem_config.percent_exp * 100) .. "%")
...
function GuildTotemView:InitQizhiModel()
	if not self.qizhi_model then
		self.qizhi_model = RoleModel.New("guild_qizhi_panel")
