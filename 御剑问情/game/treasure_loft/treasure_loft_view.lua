...
function TreasureLoftView:__init()
	self.ui_config = {"uis/views/treasureloft_prefab","TreasureLoftView"}
	self.item_list = {}
...
function TreasureLoftView:LoadCallBack()
	...
	self:ListenEvent("open_log", BindTool.Bind(self.OpenLog, self))
...
function TreasureLoftView:HandleClose()
	self:Close()
end
...
function TreasureLoftView:OpenLog()
...
function TreasureLoftView:SendReq(num)
	...
	if num == 0 then
		...
		if role_gold > self.zhenbaoge_reflush_gold then
	...
	elseif num == 1 then
		...
		if role_gold > self.zhenbaoge_reflush_gold*self.zhenbaoge_auto_flush_times then
			self.refresh_tags = true
		end
	elseif num == 2 then
		...
		local role_gold = GameVoManager.Instance:GetMainRoleVo().gold
		if role_gold > self.all_buy_gold then
			self:BuyAllItemAnimation()
		end
	end
end
...
function TreasureLoftView:SendRollCard(index)
	...
	local yes_func = function()
		KaifuActivityCtrl.Instance:SendGetKaifuActivityInfo(ACTIVITY_TYPE.RAND_ACTIVITY_TREASURE_LOFT, RA_ZHENBAOGE_OPERA_TYPE.RA_ZHENBAOGE_OPERA_TYPE_BUY, index -1)
		if role_gold > self.item_buffer[index].price then
			self:BuyItemAnimation(index)
		end
	end
	TipsCtrl.Instance:ShowCommonAutoView("buy_one", tips, yes_func, nil, nil, nil, nil, nil, true, true)
	TipsCtrl.Instance:SetIndex(index ,role_gold > self.item_buffer[index].price)
end
...
function TreasureLoftView:SendBuyAllItemsReq()
	...
	if self.all_buy_gold > 0 then
		...
		local role_gold = GameVoManager.Instance:GetMainRoleVo().gold
		TipsCtrl.Instance:SetIndex(10 ,role_gold > self.all_buy_gold)
	else
		self:SendReq(2)
	end
...
function TreasureLoftView:BuyItemAnimation(index)
	...
	local func2 = function()
		self["is_show" .. index]:SetValue(true)
...
function TreasureLoftView:BuyAllItemAnimation()
	for i=1,9 do
		if NEQ_CARD_STATUS.DEFAULT ~= self.card_status[i] then
