...
function MapFindView:__init()
	...
	self.ui_config = {"uis/views/mapfind_prefab", "MapFind"}
...
function MapFindView:LoadCallBack()
	...
	self:ListenEvent("ClickRecord", BindTool.Bind(self.ClickRecord, self))
...
function MapFindView:ClickRecord()
...
function MapFindView:ConstructData()
	...
	if MapFindData.Instance.is_find then
...
--	MapFindData.Instance.is_find = false
end

function MapFindView:SendFlushInfo()
	MapFindCtrl.Instance:SendInfo(RA_MAP_HUNT_OPERA_TYPE.RA_MAP_HUNT_OPERA_TYPE_FLUSH)
end
...
function MapFindView:SetDataView()
	...
	for i, v in ipairs(self.camp) do
		v:SetAsset(ResPath.GetMapFindImg(self.city_route["city_" .. i]))
...
function MapFindView:SetSlider()
	...
	self.flush_times_value:SetValue(flush_times)
end
...
function MapRewardItem:SetData(day_range)
	...
	for i = 1, 3 do
		self.images[i]:SetAsset(ResPath.GetMapFindImg(data["city_" .. i]))
...
function MapFlushItem:ShowView(show_eff)
	...
	self.effect.gameObject:SetActive(show_eff)
...
function FindItem:SetData(data)
	...
	if route_info then
		...
		self.map:SetAsset(ResPath.GetMapFindImg(route_info.city_list[data]))
		...
		for i=1, #reward_cfg do
			...
			if cur_select[i] then
				...
				if MapFindData.Instance:GetNameById(route_info.city_list[data]) == temp.name then
		...
		-- local num = MapFindData.Instance:GetSelect()
		-- local tem = MapFindData.Instance:GetMapRewardData(num)
		-- if MapFindData.Instance:GetNameById(route_info.city_list[data]) == tem.name  then
		-- 	MapFindData.Instance.is_find = true
		-- 	MapFindCtrl.Instance:EndRush()
		-- end
