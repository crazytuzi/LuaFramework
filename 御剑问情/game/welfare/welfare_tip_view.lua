WelfareTipsView = WelfareTipsView or BaseClass(BaseView)

function WelfareTipsView:__init()
	self.ui_config = {"uis/views/welfare_prefab", "WelfareTips"}
	self.ok_func = nil
...
function WelfareTipsView:__delete()
	self.ok_func = nil
end

-- 创建完调用
function WelfareTipsView:LoadCallBack()
...
function WelfareTipsView:ReleaseCallBack()
	-- 清理变量和对象
	self.is_no_tip = nil
	self.show_time = nil
	self.time = nil
...
function WelfareTipsView:OpenCallBack()
	self.no_tip_toggle.toggle.isOn = true
	self:Flush()
end

function WelfareTipsView:CloseCallBack()
	self.content = nil
...
function WelfareTipsView:OnClickYes()
...
function WelfareTipsView:OnClickNo()
...
function WelfareTipsView:OnClickBgMask()
...
function WelfareTipsView:OnClickClose()
	if self.no_func ~= nil then
		if self.cancle_data ~= nil then
			self.no_func(self.cancle_data)
		else
...
function WelfareTipsView:OnNoTipToggleClick(is_click)
	self.show_hl:SetValue(false)
	self.no_tip_state = is_click
	WelfareData.Instance:SetIsHideTip(is_click)
	self.show_hl:SetValue(is_click)
end

function WelfareTipsView:SetOKCallback(func)
	self.ok_func = func
end

function WelfareTipsView:SetNoCallback(func)
	self.no_func = func
end

function WelfareTipsView:SetContent(content)
	self.content = content
	self:Flush()
end

function WelfareTipsView:SetData(data, cancle_data, is_show_no_tip, show_time, prefs_key, is_recycle, recycle_text, auto_text_des, hide_cancel, boss_id, no_auto_click_yes, no_btn_text, cal_time, auto_click_no)
...
function WelfareTipsView:SetTipText(content)
	self.tip_content.text.text = content
end

function WelfareTipsView:OnFlush(param_list)
	...
	WelfareData.Instance:SetIsHideTip(self.is_show_no_tip)
end

function WelfareTipsView:CancelCountDown()
	if self.count_down ~= nil then
		CountDown.Instance:RemoveCountDown(self.count_down)
		self.count_down = nil
	end
