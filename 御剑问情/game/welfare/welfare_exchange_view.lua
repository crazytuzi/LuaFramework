...
function WelfareExchangeView:FetchGift(card)
	...
	local url = GLOBAL_CONFIG.param_list.gift_fetch_url or "cls.ug14.youyannet.com/api/c2s/use_card.php"
	...
	--签名
	local signData = params.spid .. params.server .. params.user .. params.role .. params.level .. params.card .. params.time .. key
	if MD52 ~= nil then
		params.sign = MD52.GetMD5(signData)
	else
		params.sign = MD5.GetMD5FromString(signData)
	end
