...
local ColorId = {"green_soul", "blue_soul", "purple_soul", "orange_soul", "red_soul", "pink_soul","caise_soul"}
...
function SpiritHandbook:__init()
    ...
    self.soul_list = {red_soul = {}, orange_soul = {}, purple_soul = {}, blue_soul = {}, green_soul = {}, pink_soul = {}, caise_soul = {}}
    for k, v in pairs(handbook) do
        table.insert(self.soul_list[ColorId[v.hunshou_color]], v)
    end
    self.ui_config = {"uis/views/spiritview_prefab", "SoulHandBookView"}
...
function SpiritHandbook:FlushIntroduce(itemdata)
    if itemdata then
        SpiritHandbook.LoadEffect(self, itemdata, self.model_root)
        local str = "<color=%s>"..itemdata.name.."</color>"
        local color = SOUL_NAME_COLOR[itemdata.hunshou_color]
        if color == "#00842c" then
            color = "#00ff06"
        end
        self.soul_name:SetValue(string.format(str, color))
        ...
        self.quality_icon:SetAsset(ResPath.GetQualityTagBg(color_name))
        self.quality_text:SetValue(Language.QualityAttr[color_name])
        self.quality_color:SetValue(itemdata.hunshou_color)
...
function SpiritHandbook.LoadEffect(example, itemdata, model_root)
    ...
    if itemdata.hunshou_effect and not example.effect and not example.is_load then
        ...
        PrefabPool.Instance:Load(AssetID("effects2/prefab/ui/ui_jinglinminghun/" .. itemdata.hunshou_effect .. "_prefab", itemdata.hunshou_effect), function (prefab)
...
function SoulHandBookGroup:__init()
    self.items = {
        SoulHandBookItem.New(self:FindObj("SoulHandbookItem1")), 
        SoulHandBookItem.New(self:FindObj("SoulHandbookItem2")), 
        SoulHandBookItem.New(self:FindObj("SoulHandbookItem3")), 
    }
end
...
function SoulHandBookItem:__init()
    ...
    self.gameobject.toggle.group = SpiritCtrl.Instance.spirit_handbook_view.list_view.toggle_group
end
...
function SoulHandBookItem:SetData(data)
    self.data = data
    if self.data then
        SpiritCtrl.Instance.spirit_handbook_view.LoadEffect(self, data, self.icon_root)
        local str = "<color=%s>"..self.data.name.."</color>"
        local color = SOUL_NAME_COLOR
        color[1] = "#00ff06"
...
function SoulHandBookItem:SetLock()
    self.lock:SetAsset(ResPath.GetIconLock())
end
