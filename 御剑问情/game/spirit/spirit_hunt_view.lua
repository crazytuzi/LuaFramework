...
function SpiritHuntView:__init(instance)
	...
	self.show_warehouse_redpoint = self:FindVariable("ShowWarehouseRedPoint")
	self.page_count = 1
    --illustrated
    self.illustrated_display=self:FindObj("illustruate_display")
    self.illustrated_model=RoleModel.New("illustruate_panel")
    ...
    self.choose_text_show=self:FindVariable("ChooseTextShow")
	...
	for i = 1, 4 do
		...
		local dis_modle = RoleModel.New("spirit_hunt_panel")
		dis_modle:SetDisplay(display.ui3d_display)
		table.insert(self.dis_modle_list, dis_modle)
	end
...
function SpiritHuntView:__delete()
	...
	for _, v in ipairs(self.dis_modle_list) do
		v:DeleteMe()
	end
	self.dis_modle_list = {}
	...
	if self.illustrated_model ~= nil then
		self.illustrated_model:DeleteMe()
		self.illustrated_model = nil
	end

	for _, v in pairs(self.display_items) do
		v:DeleteMe()
	end
	self.display_items = {}

	for _, v in pairs(self.time_quest) do
...
function SpiritHuntView:RemindChangeCallBack(remind_name, num)
	if nil ~= self.show_warehouse_redpoint then
		self.self.show_warehouse_redpoint:SetValue(num > 0)
	end
end
function SpiritHuntView:FulushRedPoint()
	local item_list = SpiritData.Instance:GetHuntSpiritWarehouseList()
	self.show_warehouse_redpoint:SetValue(#item_list > 0)
end

function SpiritHuntView:OnClickWareHouse()
	SpiritCtrl.Instance.spirit_warehouse_view:Open()
end

function SpiritHuntView:OnClickExchange()
...
function SpiritHuntView:OnClickHelp()
	...
	local id = cfg[#cfg].icon_id
	self:IllustrateDefulatShow(id)
end

function SpiritHuntView:OnClickDanMu()
	RollingBarrageData.Instance:RecordBarrageState(CHEST_SHOP_TYPE.CHEST_SHOP_TYPE_JINGLING, self.dan_mu_toggle.isOn)
...
function SpiritHuntView:GetNumOfCell()
	...
	list_page_scroll:SetPageCount(MAX_GRID_NUM / (ROW * COLUMN))
...
function SpiritHuntView:HandleItemOnClick(data, group, i, index)
 ...
 --    end
    ...
    self.choose_text_show:SetValue(item_cfg.color)
...
function SpiritHuntView:IllustrateDefulatShow(item_id)
	...
	local max_id = defulat_data[#defulat_data].icon_id
	local min_id = defulat_data[1].icon_id
	local defulat_item_id=item_id or math.random(min_id, max_id)
    ...
    self.choose_text_show:SetValue(defulat_item_cfg.color)
...
function SpiritHuntView:SetModleRestAni(index)
	local timer = 8
	if not self.time_quest[index] then
		self.time_quest[index] = GlobalTimerQuest:AddRunQuest(function()
			...
			if timer <= 0 then
				local model = self.dis_modle_list[index]
				if model then
...
function SpiritHuntView:Flush()
	...
	if display_list and self.is_first_open then
		for k, v in pairs(display_list) do
			...
			local model = self.dis_modle_list[k]
			if model then
				model:SetMainAsset(bundle_main, asset_main, BindTool.Bind(self.SetModleRestAni, self, k))
				model:SetModelTransformParameter(DISPLAY_MODEL_TYPE[DISPLAY_TYPE.SPIRIT], asset_main, DISPLAY_PANEL.OPEN_TRAILER)
			end
	...
	self.dan_mu_toggle.isOn = RollingBarrageData.Instance:GetRecordBarrageState(CHEST_SHOP_TYPE.CHEST_SHOP_TYPE_JINGLING)
end
