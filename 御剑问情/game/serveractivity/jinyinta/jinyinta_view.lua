...
function JinYinTaView:__init()
	self.ui_config = {"uis/views/serveractivity/jinyinta_prefab", "JinYinTaView"}
...
function JinYinTaView:ReleaseCallBack()
	...
	self.ShenYuTime = nil
	self.ShenYuTime1 = nil
	self.ShenYuTime2 = nil
	...
	self.close_animator_toggle = nil
	self.TenChouGold = nil
	self.OneChouGold = nil
	self.show_key_ten_text = nil
...
function JinYinTaView:LoadCallBack()
	...
	self.ShenYuTime1 = self:FindVariable("ShenYuTime1")
	self.ShenYuTime2 = self:FindVariable("ShenYuTime2")
	...
	self.show_ten_coin_text = self:FindVariable("show_ten_key_coin")
	self.key_ten_text = self:FindVariable("key_ten_text")
	self.is_show_free = self:FindVariable("IsShowFree")
	...
	self.close_animator_toggle = self:FindObj("SetAniToggle").toggle
...
function JinYinTaView:OnFlush()
	local can_show_treasure = JinYinTaData.Instance:GetCanShowTreasure()
	if not can_show_treasure then
		return
	end
	...
	if CHEST_SHOP_MODE.CHEST_RANK_JINYIN_TA_MODE_1 == self.draw_type then
		...
		JinYinTaData.Instance:ResetCanShowTreasure()
		self.is_flush = true
		self:FlushCurrLevel()
		if self.show_reward_panel then
			GlobalTimerQuest:CancelQuest(self.show_reward_panel)
...
function JinYinTaView:TurnCell(currLevel)
	...
	self.time_quest = GlobalTimerQuest:AddRunQuest(function()
        ...
        --跳过抽奖动画
        if self.close_animator_toggle.isOn then
...
function JinYinTaView:FlushUpdataActEndTime()
	...
	local time_tab = TimeUtil.Format2TableDHMS(time_str)
 	self.ShenYuTime:SetValue(time_tab.day)
 	self.ShenYuTime1:SetValue(time_tab.hour)
 	self.ShenYuTime2:SetValue(time_tab.min)
 	if time_str <= 0  then
 		-- 移除计时器
		if self.act_next_timer then
			GlobalTimerQuest:CancelQuest(self.act_next_timer)
...
--10抽钥匙图标显示刷新
function JinYinTaView:FlushText()

	local randact_cfg = ServerActivityData.Instance:GetCurrentRandActivityConfig()
	local item_id = randact_cfg.other[1].jinyinta_consume_item
	local item_num = ItemData.Instance:GetItemNumInBagById(item_id)
	self.show_ten_coin_text:SetValue(item_num <= 0)
	local item_cfg = ItemData.Instance:GetItemConfig(item_id)
	self.show_key_ten_text:SetValue(item_num > 0)

	self.key_ten_text:SetValue(item_cfg.name..Language.Common.X..item_num)
end
...
function RewardItem:OnFlush()
	...
	if lottery_bool then
		...
		if can_lin then
			self.reward_item_cells:ShowGetEffect(true)
