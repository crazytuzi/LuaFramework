...
function HuanzhuangShopCtrl:OnSCRAMagicShopAllInfo(protocol)
	...
	RemindManager.Instance:Fire(RemindName.TitleShopPoint)
	if self.huan_zhuang_shop_view then
	...
	ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MAGIC_SHOP, self.huan_zhuang_shop_data:TitleShopPoint() > 0)
	-- for k,v in ipairs(cfg) do
	-- 	if 0 == magic_shop_buy_flag[33 - k] and HuanzhuangShopData.Instance:GetLoginFlag() then
	-- 		-- MainUIView.Instance:SetHuanZhuangShopEffect(true)
	-- 		break
	-- 	end
	-- end
end
...
function HuanzhuangShopCtrl:ActivityCallBack(activity_type, status)
	if activity_type == ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MAGIC_SHOP then
		-- MainUIView.Instance:ChangeHuanZhuangShopBtn(status == ACTIVITY_STATUS.OPEN and GameVoManager.Instance:GetMainRoleVo().level >= 130)
