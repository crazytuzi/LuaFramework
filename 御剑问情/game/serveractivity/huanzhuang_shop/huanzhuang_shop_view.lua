...
function HuanzhuangShopView:__init()
	self.ui_config = {"uis/views/randomact/huanzhuangshop_prefab", "HuangZhuangShopView"}
...
function HuanzhuangShopView:RemoveTimerQuest()
...
function HuanzhuangShopView.ChangeModel(model, item_id, item_id2, cell)
	...
	elseif display_role == DISPLAY_TYPE.FASHION then
		...
		if fashion_part_type == SHIZHUANG_TYPE.WUQI then
			...
			panel_name = display_prof_List[game_vo.prof] or display_List[res_id] or ""
		else
...
function HuanZhuangShopCell:FlushModel()
	...
	if 1 == self.show_type then
		...
		if #tbl == 1 then
			HuanzhuangShopView.ChangeModel(self.model, tonumber(tbl[1]), nil, self)
		elseif tbl[3] then
...
function HuanZhuangShopCell:UpAni(state)
