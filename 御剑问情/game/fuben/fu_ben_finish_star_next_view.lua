...
function FuBenFinishStarNextView:__delete()
	self:ClearCutDown()
end
...
function FuBenFinishStarNextView:LoadCallBack()
	...
	self.show_fight = self:FindVariable("ShowFightNumber")
	self.show_enter_next = self:FindVariable("ShowEnterNextActive")
	self.cancle_text = self:FindVariable("cancle_text")

	self.show_enter_next:SetValue(true)
	self.show_fight:SetValue(false)
	self.star_list = {}
	for i = 1, 3 do
...
function FuBenFinishStarNextView:OpenCallBack()
	self.cancle_text:SetValue(Language.Common.Quit)
...
function FuBenFinishStarNextView:ReleaseCallBack()
	...
	self.fight_num = nil
	self.show_fight = nil
	self.show_enter_next = nil
	self.cancle_text = nil
end
...
function FuBenFinishStarNextView:OnFlush(param_t)
	...
	for k, v in pairs(param_t) do
		if k == "finish" then
			...
			if v.pass_time then
				local str_pass = string.format(Language.Mount.ShowWhiteStr, TimeUtil.FormatSecond(v.pass_time, 4))
			...
			if v.cancle_text then
				self.cancle_text:SetValue(v.cancle_text)
			end

			self:FlushFight(v.fight)
			self:ShowButton(v.show_next, v.show_star)
		end
	end
end

function FuBenFinishStarNextView:ShowButton(is_show, show_star)
	if nil == is_show then
		return
	end

	self.show_enter_next:SetValue(is_show)
	if false == is_show then
		if self.cal_time_quest then
			GlobalTimerQuest:CancelQuest(self.cal_time_quest)
			self.cal_time_quest = nil
		end
...
function FuBenFinishStarNextView:FlushFight(data)
	if nil == data then
		self.show_fight:SetValue(false)
		return
	end

	self.show_fight:SetValue(true)
	self.fight_num:SetValue(data)
end
...
function FuBenFinishStarNextView:PlayTime(group_cell, count, elapse_time, total_time)
	...
	GameObjectPool.Instance:SpawnAsset("effects2/prefab/ui_x/ui_choujiang_prefab", "UI_choujiang", BindTool.Bind(self.LoadEffect, self, item_num, group_cell))
...
function FuBenFinishStarNextView:LoadEffect(item_num, group_cell, obj)
	...
	self.timer_quest = GlobalTimerQuest:AddDelayTimer(Free, 1)
end

function FuBenFinishStarNextView:RemoveDelayTime()
...
function FuBenFinishStarNextView:ClearCutDown()
	if self.play_count_down then
		CountDown.Instance:RemoveCountDown(self.play_count_down)
		self.play_count_down = nil
	end
