...
function FuBenFinishStarView:LoadCallBack()
	...
	self.death_count = self:FindVariable("DeathCount")
	self.is_tower = self:FindVariable("is_tower")
	self.is_normal = self:FindVariable("is_normal")
	self.star_list = {}
	for i = 1, 3 do
...
function FuBenFinishStarView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		...
		elseif k == "tower_finish" then
			self.is_tower:SetValue(true)
			self.is_normal:SetValue(false)
			if v.data ~= nil then
				self.death_count:SetValue(v.data)
			end
			if v.star then
				self.star_num = v.star
				self:StartPlayEffect()
...
function FuBenFinishStarView:RemoveDelayTime()
...
function FuBenFinishStarView:AddTimerQuest()
	if self.leave_timer == nil then
		self.leave_timer = GlobalTimerQuest:AddDelayTimer(BindTool.Bind(self.CloseView, self), 8)
	end
end

function FuBenFinishStarView:CloseView()
	self:Close()
end