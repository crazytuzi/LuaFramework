...
function ZhiBaoActiveDegreeView:CloseCallBack()
...
function ZhiBaoActiveDegreeView:CompleteCallBack()
	...
	self.show_effect:SetValue(next_value ~= 0)
	...
	self.active_degree_value:SetValue(next_total_degree)
...
function ZhiBaoActiveDegreeView:Flush()
	...
	self.active_degree_value:SetValue(active_degree_data.total_degree)
	local value = active_degree_data.total_degree / self.active_degree_limit
	self.show_effect:SetValue(value ~= 0)
	...
	if self.scroller and self.scroller.scroller.isActiveAndEnabled then
		...
		if is_change and start_fly_obj then
			...
			TipsCtrl.Instance:ShowFlyEffectManager(ViewName.BaoJu, "effects2/prefab/ui/ui_huobanshengji_lizi_prefab", "UI_huobanshengji_lizi", start_fly_obj, self.handle_slide_obj, nil, 1)
		end
	end
end
...
function ActiveDegreeScrollCell:OnFlush()
	...
	local item_data = {}
	local exp = ZhiBaoData.Instance:GetExpRatio() * self.data.add_exp
	...
	if degree >= self.data.max_times then
  	...
  	local str = string.format(Language.Rune.TreasureOneCost, color, degree,self.data.max_times)
	--self.times:SetValue("<color="..color..">"..degree..'</color> / '..self.data.max_times)
	self.times:SetValue(str)
...
function ActiveDegreeScrollCell:HandleTime()--设定时间格式
...
function ActiveDegreeScrollCell.OnGoClick(data)
	...
	if data.goto_panel ~= "" then
		...
		elseif data.goto_panel == "HuSong" then
			...
			return
		elseif data.goto_panel == "MojingTask" then
			local task_id = TaskData.Instance:GetRandomTaskIdByType(TASK_TYPE.HUAN)
			if task_id == nil or task_id == 0 then
				TipsCtrl.Instance:ShowSystemMsg(Language.BaoJu.NotMoJingTask)
				return
			end
			TaskCtrl.Instance:DoTask(task_id)
			ViewManager.Instance:Close(ViewName.BaoJu)
