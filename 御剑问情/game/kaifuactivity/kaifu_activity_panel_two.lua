...
function PanelListCellTwo:OnFlush()
	...
	local color = count > 0 and "0000f1" or "fe3030"
	...
	for i = 1, 4 do
		if self.data["stuff_id" .. i] and self.data["stuff_id" .. i].item_id > 0 and self.item_list[index] then
			...
			local color = num < self.data["stuff_id" .. i].num and "fe3030" or "0000f1"

			text_str = "<color=#" .. color .. ">" .. num .. "</color>"  .. "<color=#001828> / " .. self.data["stuff_id" .. i].num .. "</color>"
--			text_str = num .. "</color>"  .. "<color=#0065fc>/" .. self.data["stuff_id" .. i].num .. "</color>"
