...
function FightText:ShowText(bundle, asset, text, attach_point)
	...
	GameObjectPool.Instance:SpawnAsset(bundle, asset, function(obj)
		...
		if nil == text_component then
			text_component = text_obj:GetComponentInChildren(typeof(UnityEngine.UI.Text))
		end
		...
		obj.transform.position = Vector3(obj.transform.position.x + offset, obj.transform.position.y + offset, obj.transform.position.z)
...
function FightText:ShowHurt(text, pos, attach_point, text_type)
	...
	elseif text_type == FIGHT_TEXT_TYPE.BAOJU then
		text = Language.FightText.fabao .. text
		if pos.is_left then
	...
	-- 	if pos.is_left then
	-- 		self:ShowText("uis/views/floatingtext", "HurtLeftShenSheng" .. add_str, Language.Common.ShenSheng .. text, attach_point)
	-- 	else
	-- 		self:ShowText("uis/views/floatingtext", "HurtRightShenSheng" .. add_str, Language.Common.ShenSheng .. text, attach_point)
	-- 	end
	end
end
...
function FightText:ShowCritical(text, pos, attach_point, text_type)
	...
	elseif text_type == FIGHT_TEXT_TYPE.BAOJU then
		if pos.is_left then
			self:ShowText("uis/views/floatingtext_prefab", "HurtLeftBaoJu" .. add_str, Language.Common.PassvieSkillAttr.bao_ji .. text, attach_point)
		else
			self:ShowText("uis/views/floatingtext_prefab", "HurtRightBaoJu" .. add_str, Language.Common.PassvieSkillAttr.bao_ji .. text, attach_point)
		end
	elseif text_type == FIGHT_TEXT_TYPE.NVSHEN then
		if pos.is_left then
			self:ShowText("uis/views/floatingtext_prefab", "HurtLeftNvShen" .. add_str, Language.Common.PassvieSkillAttr.bao_ji .. text, attach_point)
		else
			self:ShowText("uis/views/floatingtext_prefab", "HurtRightNvShen" .. add_str, Language.Common.PassvieSkillAttr.bao_ji .. text, attach_point)
		end
	elseif text_type == FIGHT_TEXT_TYPE.BAOJI then
		if pos.is_left then
			self:ShowText("uis/views/floatingtext_prefab", "CriticalLeft" .. add_str, Language.Common.PassvieSkillAttr.bao_ji .. text, attach_point)
		else
			self:ShowText("uis/views/floatingtext_prefab", "CriticalRight" .. add_str, Language.Common.PassvieSkillAttr.bao_ji .. text, attach_point)
		end
	end
end

function FightText:ShowHuiXinYiJi(text, pos, attach_point, text_type)
	if pos.is_left then
		self:ShowText("uis/views/floatingtext_prefab", "HuiXinLeft", text, attach_point)
	else
		self:ShowText("uis/views/floatingtext_prefab", "HuiXinRight", text, attach_point)
	end
end

function FightText:ShowLingChongGongji(text, pos, attach_point, text_type)
	if pos.is_left then
		self:ShowText("uis/views/floatingtext_prefab", "LingChongLeft", text, attach_point)
	else
		self:ShowText("uis/views/floatingtext_prefab", "LingChongRight", text, attach_point)
	end
end

function FightText:ShowLianhunText(attach_point)
	self:ShowText("uis/views/floatingtext_prefab", "LianhunRight", "", attach_point)
end

function FightText:ShowYaZhi(text, pos, attach_point, text_type)
	local add_str = pos.is_top and "1" or ""
	text_type = text_type or FIGHT_TEXT_TYPE.NORMAL
	if text_type == FIGHT_TEXT_TYPE.NORMAL then
		if pos.is_left then
			self:ShowText("uis/views/floatingtext_prefab", "CriticalLeft" .. add_str, Language.Common.PassvieSkillAttr.ya_zhi .. text, attach_point)
		else
			self:ShowText("uis/views/floatingtext_prefab", "CriticalRight" .. add_str, Language.Common.PassvieSkillAttr.ya_zhi .. text, attach_point)
		end
	end
end
...
function FightText:ShowBeHurt(text, pos, attach_point)
	...
	-- else
	-- 	self:ShowText("uis/views/floatingtext", "BeHurtRight" .. add_str, text, attach_point)
	-- end
end
...
function FightText:ShowBeCritical(text, pos, attach_point)
	local add_str = pos.is_top and "1" or ""
	-- if pos.is_left then
		self:ShowText("uis/views/floatingtext_prefab", "BeHurtLeft", Language.Common.PassvieSkillAttr.bao_ji .. text, attach_point)
	-- else
	-- 	self:ShowText("uis/views/floatingtext", "BeHurtRight" .. add_str, text, attach_point)
	-- end
end
...
function FightText:ShowBeYaZhi(text, pos, attach_point, text_type)
	local add_str = pos.is_top and "1" or ""
	text_type = text_type or FIGHT_TEXT_TYPE.NORMAL
	if text_type == FIGHT_TEXT_TYPE.NORMAL then
		if pos.is_left then
			self:ShowText("uis/views/floatingtext_prefab", "BeHurtLeft" .. add_str, Language.Common.PassvieSkillAttr.ya_zhi .. text, attach_point)
		else
			self:ShowText("uis/views/floatingtext_prefab", "BeHurtRight" .. add_str, Language.Common.PassvieSkillAttr.ya_zhi .. text, attach_point)
		end
	end
end
...
function FightText:ShowDodge(pos, attach_point, is_main_role)
	...
	if not is_main_role then
		if math.random() <= 0.5 then
			self:ShowText("uis/views/floatingtext_prefab", "DodgeSelfLeft", Language.Common.PassvieSkillAttr.wei_ming_zhong, attach_point)
		else
			self:ShowText("uis/views/floatingtext_prefab", "DodgeSelfRight", Language.Common.PassvieSkillAttr.wei_ming_zhong, attach_point)
		end
	else
		self:ShowText("uis/views/floatingtext_prefab", "DodgeTarget", Language.Common.PassvieSkillAttr.shan_bi, attach_point)
	end
	-- else
	-- 	self:ShowText("uis/views/floatingtext", "BeHurtRight" .. add_str, Language.Common.PassvieSkillAttr.shan_bi, attach_point)
	-- end
end
