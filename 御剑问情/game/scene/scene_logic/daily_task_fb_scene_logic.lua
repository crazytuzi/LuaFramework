	...
	if self.talk_time <= now_time then
		...
		if #monster_key_t > 0 then
			...
			if self.monster then
				if not has_other_monster and self.talk_dec2 and self.talk_dec2 ~= "" then
					self.monster:SetBubble(self.talk_dec2)
				else
		...
		if self.branch_fb_type == DailyTaskFbData.FB_TYPE.SHENZHU then
			...
			if boss then
				if monster1 then
					...
					local line_renderers = self.line_t[monster1:GetMonsterId()].obj:GetLineRenderers()
					if line_renderers then
						if line_renderers.Length > 0 then
							for i = 0, line_renderers.Length - 1 do
								local line_renderer = line_renderers[i]
								if not IsNil(line_renderer) then
									...
									line_renderer:SetPosition(0, Vector3(pos.x, pos.y + 7.5, pos.z))
								end
							end
						end
					end
				...
				if monster2 then
					...
					local line_renderers = self.line_t[monster2:GetMonsterId()].obj:GetLineRenderers()
					if line_renderers then
						if line_renderers.Length > 0 then
							for i = 0, line_renderers.Length - 1 do
								local line_renderer = line_renderers[i]
...
function DailyTaskFbSceneLogic:GetIsShowSpecialImage(obj)
	...
	if cfg and obj:IsMonster() then
		for i= 1, 3 do
			if cfg["monster_" .. i] == obj:GetMonsterId() then
				return true, "uis/images_atlas", "df_score_" .. i
			end
		end
	end
	return false
...
function DailyTaskFbSceneLogic:CreateSDX(monster)
	...
	effect_vo.res = monster:GetMonsterId() == self.huixie_monster and "ta_lian_hong" or "ta_lian_lan"
