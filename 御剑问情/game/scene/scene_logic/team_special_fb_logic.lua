...
function TeamSpecialFbLogic:Enter(old_scene_type, new_scene_type)
	...
	if not self.is_insert_nostop_guaji_scene_id then
		self.is_insert_nostop_guaji_scene_id = true

		scene_id_t = GuajiCtrl.Instance:GetNoStopGuaJiSceneIdTable()
		if nil ~= scene_id_t then
			for i = self.first_nostop_scene_id, self.last_nostop_scene_id do
				scene_id_t[i] = 1
			end
		end
	end
