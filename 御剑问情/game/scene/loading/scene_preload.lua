...
function ScenePreload:__init(show_loading)
	...
	self.is_last_unity_scene = false
...
function ScenePreload:StartLoad(scene_id, load_list, download_scene_id, progress_fun, main_complete_fun, complete_fun)
	...
	if self.download_scene_id > 0 then
		ReportManager:Step(Report.STEP_UPDATE_SCENE_BEGIN, nil, nil, nil, nil, self.download_scene_id)
	end
...
function ScenePreload:OnLoadComplete()
	if self.download_scene_id > 0 then
		ReportManager:Step(Report.STEP_UPDATE_SCENE_COMPLETE, nil, nil, nil, nil, self.download_scene_id)
	end
	...
	-- if not IsLowMemSystem and not self.is_last_unity_scene then -- 低内存不再做静态合批，占用太大内存。场景与上个相同也不合批
	if not self.is_last_unity_scene then -- 低内存不再做静态合批，占用太大内存。场景与上个相同也不合批
	    ...
	    for i = 0, static_batching_roots.Length - 1 do
	        ...
	        UnityEngine.StaticBatchingUtility.Combine(root)
	    end
	end
...
function ScenePreload.GetLoadSkillList(list)
	...
	if prof == GameEnum.ROLE_PROF_1 then
		table.insert(bundle_name_list, "effects2/prefab/role/1101001_prefab")
	end

	if prof == GameEnum.ROLE_PROF_2 then
		table.insert(bundle_name_list, "effects2/prefab/role/1001_prefab")
	end

	if prof == GameEnum.ROLE_PROF_3 then
		table.insert(bundle_name_list, "effects2/prefab/role/1102001_prefab")
	end

	if prof == GameEnum.ROLE_PROF_4 then
		table.insert(bundle_name_list, "effects2/prefab/role/1002_prefab")
	end
...
function ScenePreload.GetLoadCgList(list, scene_id)
	...
	if scene_id == 101 then
		if 1 == role_level then
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun_prefab", asset_name = "W2_YW_01_TaoYuanCun_Cg1", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun_prefab", asset_name = "W2_YW_01_TaoYuanCun_Cg4", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun_prefab", asset_name = "W2_YW_01_TaoYuanCun_Cg5", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun_prefab", asset_name = "W2_YW_01_TaoYuanCun_Cg6", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun/nanshan/cg1_prefab", asset_name = "TYC_CG1_nanshan", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun/nanshan/cg2_prefab", asset_name = "TYC_CG2_nanshan", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun/nanzhan/cg1_prefab", asset_name = "TYC_CG1_nanzhan", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun/nanzhan/cg2_prefab", asset_name = "TYC_CG2_nanzhan", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun/nvliandao/cg1_prefab", asset_name = "TYC_CG1_liandao", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun/nvliandao/cg2_prefab", asset_name = "TYC_CG2_liandao", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun/nvqin/cg1_prefab", asset_name = "TYC_CG1_nvqin", bytes_total = 20})
			table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_01_taoyuancun/nvqin/cg2_prefab", asset_name = "TYC_CG2_nvqin", bytes_total = 20})
		end
	end

	if scene_id == 102 then
		if TaskData.Instance:GetTaskIsAccepted(370) then
		 	table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_02_yueyajian_prefab", asset_name = "W2_YW_02_YueYaJian_Cg1", bytes_total = 20})
		 	table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_02_yueyajian_prefab", asset_name = "W2_YW_02_YueYaJian_Cg3", bytes_total = 20})
		 	table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_02_yueyajian/nanshan_prefab", asset_name = "YYJ_CG1_nanshan", bytes_total = 20})
		 	table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_02_yueyajian/nanzhan_prefab", asset_name = "YYJ_CG1_nanzhan", bytes_total = 20})
		 	table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_02_yueyajian/nvliandao_prefab", asset_name = "YYJ_CG1_liandao", bytes_total = 20})
		 	table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_02_yueyajian/nvqin_prefab", asset_name = "YYJ_CG1_nvqin", bytes_total = 20})
       end
	end

	if scene_id == 103 then
		if TaskData.Instance:GetTaskIsAccepted(620) then
		 	table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_03(z)_huaxuguo_prefab", asset_name = "W2_YW_03(Z)_HuaXuGuo_Cg1", bytes_total = 20})
		 	table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_yw_03(z)_huaxuguo_prefab", asset_name = "W2_YW_03(Z)_HuaXuGuo_Cg2", bytes_total = 20})
       end
	end

	if scene_id == 4001 then
		table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_fb_zhufa_prefab", asset_name = "W2_FB_ZhuFa_Cg1", bytes_total = 20})
	end

	if scene_id == 4002 then
		table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_fb_huajuan_prefab", asset_name = "W2_FB_HuaJuan_Cg1", bytes_total = 20})
	end

	if scene_id == 4003 then
		table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_fb_qin_prefab", asset_name = "W2_FB_Qin_Cg1", bytes_total = 20})
	end

	if scene_id == 4004 then
		table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_fb_huajuan_prefab", asset_name = "W2_FB_HuaJuan_Cg2", bytes_total = 20})
	end

	if scene_id == 4005 then
		table.insert(list, {load_type = LOAD_TYPE.LOAD_CG, bundle_name = "cg/w2_fb_zhufa_prefab", asset_name = "W2_FB_ZhuFa_Cg2", bytes_total = 20})
	end
end