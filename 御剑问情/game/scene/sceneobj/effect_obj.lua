...
function EffectObj:GetLineRenderers()
	if self.line_renderers == nil and self.effect_root then
		self.line_renderers = self.effect_root:GetComponentsInChildren(typeof(UnityEngine.LineRenderer))
	end
	return self.line_renderers
end
...
function EffectObj:CreateFazhen()
	...
	if AOE_RANGE_TYPE.SELF_CENTERED_CIRCLE == skill_cfg.RangeType then
		...
		size = Vector2(skill_cfg.Range * 2, skill_cfg.Range * 2)
	...
	-- print('==========CreateFazhen333', size, angle_y, self.vo.disappear_time - self.vo.birth_time, TimeCtrl.Instance:GetServerTime() - self.vo.birth_time, self.vo.param1, self.vo.param2)
	local bunble, asset = ResPath.GetMiscEffect(effect)
...
function EffectObj:CreateEffect()
	...
	local bunble, asset = ResPath.GetMiscEffect(effect_name)
