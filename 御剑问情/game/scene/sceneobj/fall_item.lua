...
function FallItem:InitShow()
	...
	elseif self.vo.is_buff_falling == 1 then
		...
		local effect_index = self.cfg and self.cfg.color or 1
		effect_index = effect_index > 5 and 5 or effect_index 
		effect_name = "baoxiang_0".. effect_index
	end
...
function FallItem:PlayPick()
	...
	UtilU3d.PrefabLoad("effects2/prefab/drop_prefab", "drop_weapon", function(obj)
		if nil == obj then
			print_error("No Effect:  ", "effects2/prefab/drop_prefab", "drop_weapon")
			return
		end
...
function FallItem:CheckShowEff()
	...
	elseif self.vo.buff_appearan == BUFF_FALLING_APPEARAN_TYPE.NSTF_BUFF_2 then
		local main_role = Scene.Instance:GetMainRole()
		if main_role then
			main_role:AddEffect("BUFF_nvshenzhiqiang", 3, true)
		end
