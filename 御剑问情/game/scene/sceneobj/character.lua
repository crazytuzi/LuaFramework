...
function Character:InitInfo()
	...
	if self:IsFollowObj() then
		is_main_role = self:OwnerIsMainRole()
	end
	main_part:SetMainRole(is_main_role)
	main_part:EnableHalt(is_main_role)
	main_part:EnableCameraShake(is_main_role)
	main_part:EnableCameraFOV(is_main_role)
	main_part:EnableSceneFade(is_main_role)
...
function Character:OnAnimatorBegin(anim_name)
	if self:IsAtk() then
		self:SetIsAtkPlaying(true)
	end
end

function Character:OnAnimatorHit(anim_name)
	...
	if self:IsAtk() then
		...
		if actor_ctrl ~= nil and
			...
			actor_ctrl:PlayProjectile(anim_name, root, hurt_point, function()
...
function Character:OnAnimatorEnd(anim_name)
	if self:IsAtk() then
...
function Character:DoMove(pos_x, pos_y)
	...
	-- if pos_x == self.logic_pos.x and pos_y == self.logic_pos.y then
...
function Character:EnterStateAttack(anim_name)
	...
	if nil == self.animator_handle_t[anim_name.."/begin"] then
		self.animator_handle_t[anim_name.."/begin"] = animator:ListenEvent(anim_name.."/begin", BindTool.Bind(self.OnAnimatorBegin, self, anim_name))
	end

	if nil == self.animator_handle_t[anim_name.."/hit"] then
		self.animator_handle_t[anim_name.."/hit"] = animator:ListenEvent(anim_name.."/hit", BindTool.Bind(self.OnAnimatorHit, self, anim_name))
	end

	if nil == self.animator_handle_t[anim_name.."/end"] then
		self.animator_handle_t[anim_name.."/end"] = animator:ListenEvent(anim_name.."/end", BindTool.Bind(self.OnAnimatorEnd, self, anim_name))
	end
end
...
function Character:EnterStateDead()
	...
	local main_part = self.draw_obj:GetPart(SceneObjPart.Main)
	main_part:SetInteger(ANIMATOR_PARAM.STATUS, ActionStatus.Die)
	if self.is_init_dead then
		...
		if self:IsMonster() and self.dietype == 1 then
		 	if math.random() > 0.2 then
		 		...
		 		local move_len = math.random(4, 7)
...
function Character:DoBeHitAction(deliverer, real_blood, blood, fighttype, text_type)
	...
	if self:IsMonster() and not self.is_main_target and not self:IsBoss() then
		local main_part = self.draw_obj:GetPart(SceneObjPart.Main)
		main_part:SetFloat("speed", 0)
		main_part:SetTrigger("hurt")
		GlobalTimerQuest:AddDelayTimer(function()
			if nil ~= main_part then
				main_part:SetFloat("speed", 1)
			end
		end, 0.1)
	end
end
...
function Character:PlayFloatingText(data)
	...
	if data.is_main_role then
		...
		elseif FIGHT_TYPE.YAZHI == data.fighttype then
			FightText.Instance:ShowYaZhi(
				data.blood, data.pos, bottom_point, data.text_type)
	...
	elseif self:IsMainRole() then
		...
		elseif FIGHT_TYPE.YAZHI == data.fighttype then
			FightText.Instance:ShowBeYaZhi(
				data.blood, data.pos, bottom_point, data.text_type)
		elseif FIGHT_TYPE.BAOJI == data.fighttype then
...
function Character:AddBuff(buff_type)
	if nil == self.buff_effect_list[buff_type] then
		if BUFF_CONFIG[buff_type] then
			self.buff_type_list[64 - buff_type] = 1
			local draw_obj = self.draw_obj
			if nil == draw_obj or nil == draw_obj:GetPart(SceneObjPart.Main) then
				return
			end
			-- 女神buff则显示在女神身上
			...
			local attach_obj = nil

			if draw_obj then
				if self:IsMonster() and DRAW_PART_LAYER_BUFF_LIST[buff_type] == 1 then
					--一些特效直接放在DrawPart层
					attach_obj = draw_obj:GetRoot().transform
				else
					attach_obj = draw_obj:GetPart(SceneObjPart.Main):GetAttachPoint(BUFF_CONFIG[buff_type].attach_index)
				end
			end
			if attach_obj then
				self.buff_effect_list[buff_type] = AsyncLoader.New(attach_obj)
				if BUFF_CONFIG[buff_type].is_noraml_effect == 1 then
					--一些buff放在mix文件目录下
					if MIX_BUFF_LIST[buff_type] == 1 then
						self.buff_effect_list[buff_type]:Load(ResPath.GetMiscEffect(BUFF_CONFIG[buff_type].effect_id))
					else
						local cur_scene = Scene.Instance:GetSceneId()
						if BUFF_TYPE.XUANYUN == buff_type and cur_scene == FBScene.KUAFUWENQUAN then
							self.buff_effect_list[buff_type]:Load(ResPath.GetBuffEffect(BUFF_CONFIG[buff_type].effect_id))
						else
							self.buff_effect_list[buff_type]:Load(ResPath.GetBuffEffect(BUFF_CONFIG[buff_type].effect_id))
						end
					end
				else
					self.buff_effect_list[buff_type]:Load(ResPath.GetEffectBoss(BUFF_CONFIG[buff_type].effect_id))
				end
			end
		else
	...
	if Scene.Instance:GetSceneType() == SceneType.HotSpring then
		if buff_type == BUFF_TYPE.XUANYUN then
			...
			if nil == Scene.Instance:GetBoatByRole(self:GetObjId()) then
				local part = self.draw_obj:GetPart(SceneObjPart.Main)
				part:SetBool("hurt", true)
			end
		end
	end

	if self:IsRole() and buff_type == BUFF_TYPE.EBT_MOHUA then
		local transform = self.draw_obj:GetRoot().transform
		transform.localScale = Vector3(1.3, 1.3, 1.3)
	end
	if self:IsRole() and buff_type == BUFF_TYPE.EBT_GATHER_NOT_STOP then
		local scene_type = Scene.Instance:GetSceneType()
		if scene_type == SceneType.CrossShuijing then
			self:ChangeTianShenGraveWuDiGather(1)
		end
	end
end

function Character:RemoveBuff(buff_type)
	self.buff_type_list[64 - buff_type] = 0
	...
	if self:IsRole() and buff_type == BUFF_TYPE.EBT_GATHER_NOT_STOP then
		local scene_type = Scene.Instance:GetSceneType()
		if scene_type == SceneType.CrossShuijing then
			self:ChangeTianShenGraveWuDiGather(0)
		end
	end
end

function Character:AddEffect(res, attach_index, is_buff)
	...
	if attach_obj then
		...
		if is_buff then
			self.other_effect_list[res].eff:Load(ResPath.GetBuffEffect(res))
		else
			self.other_effect_list[res].eff:Load(ResPath.GetEffect(res))
		end
	end
end
