...
function BaoBaoAptitudeView:__init(instance, mother_view)
	...
	self:ListenEvent("ClickReturn", BindTool.Bind(self.ReturnClick, self))
...
function BaoBaoAptitudeView:FlushView()
	local baby_info = BaobaoData.Instance:GetSelectedBabyInfo()
	if nil == baby_info then return end
	local max_lv = BaobaoData.Instance:GetMaxBabyUpleveCfgLength()
	local next_level = baby_info.level >= max_lv and max_lv or baby_info.level + 1
	...
	for i = 1, 4 do
		if stuff_data[i - 1] then
			...
			if baby_index_info.level >= max_lv then
