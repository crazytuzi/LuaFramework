...
function MarketSellView:Flush()
	local unbind_list = ItemData.Instance:GetBagNoBindItemList()
	local list = {}
	for k,v in pairs(unbind_list) do
		local item_cfg, big_type = ItemData.Instance:GetItemConfig(v.item_id)
		if item_cfg and big_type then
			if GameEnum.ITEM_BIGTYPE_EQUIPMENT == big_type then
				-- 红色一星以下装备不能上架
				if item_cfg.color > GameEnum.ITEM_COLOR_ORANGE and v.param and v.param.xianpin_type_list and #v.param.xianpin_type_list > 0 then
					table.insert(list, v)
				end
			else
				table.insert(list, v)
	...
	MarketData.Instance.item_list = list
...
function MarketSellView:ChangeSelectCell(index)
	...
	if item_cfg.color == 1 then
		self.item_name:SetValue(ToColorStr(item_cfg.name, SOUL_NAME_COLOR[7]))
	else
		self.item_name:SetValue(ToColorStr(item_cfg.name, SOUL_NAME_COLOR[item_cfg.color]))
	end
	...
	self.select_cell:SetItemNumVisible(false)
...
function MarketSellView:CountInputEnd(str)
	...
	self.input_price.text = self.price >= 1 and self.price or 1
	self:OnCountChanged()
end

-- 数量改变时
function MarketSellView:OnCountChanged()
	self.price = math.floor(self.total_price / self.count) >= 1 and math.floor(self.total_price / self.count) or 1
...
function MarketSellViewScrollCell:Flush()
	for i = 1, 5 do
		...
		if(index > #MarketData.Instance.item_list) then    -- 如果超出可出售物品的数量，则此格子不显示
			self.item_list[i].cell:SetData(nil)
		else                                          	 -- 否则，显示该物品的信息
...
function MarketSellViewScrollCell:ListenAllEvent()
	for i = 1, 5 do
	self.item_list[i].cell:ListenClick(
		function()
