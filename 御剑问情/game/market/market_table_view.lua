...
function MarketTableViewScrollCell:Flush()
	...
	if item_cfg.color == 1 then
		self.name:SetValue(ToColorStr(item_cfg.name, SOUL_NAME_COLOR[7]))
	else
		self.name:SetValue(ToColorStr(item_cfg.name, SOUL_NAME_COLOR[item_cfg.color]))
	end
...
function MarketTableViewScrollCell:ListenAllEvent()
	self:ListenEvent("OnClickRemove", function() MarketCtrl.Instance:SendRemovePublicSaleItem(self.data.sale_index) end)
end