...
function InvestData:OnSCTouZiJiHuaInfo(protocol)
	...
	KaifuActivityCtrl.Instance:FlushLevelInvest()
	RemindManager.Instance:Fire(RemindName.KaiFu)
end

function InvestData:GetRewardStateInfo(plan)
	local list_num = #(self:GetPlanAuto(plan)) or 0
	local flag = self.invest_info["plan_" .. plan .. "_reward_flag"] or 0

	for i = 0, list_num - 1 do
		if bit:_and(1, bit:_rshift(flag, i)) <= 0 then
			return false
		end
	end
...
function InvestData:GetAllGold(plan_type, data_index)
	local gold = 0
	local data_info = self:GetPlanAuto(plan_type)
	if nil == data_info then
		return gold
	end
	for i = 0, data_index do
		local num = 0
		for k, v in pairs(data_info) do
			if i == v.seq then
				num = v.reward_gold_bind
			end
		end
		gold = gold + num
	end
