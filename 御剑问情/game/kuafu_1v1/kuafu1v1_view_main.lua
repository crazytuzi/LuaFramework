...
function KuaFu1v1ViewMain:OnClickEnter()
	...
	if scene_type ~= SceneType.Common or GuajiCtrl.Instance:IsSpecialCommonScene() or main_role:IsFightState() or main_role.vo.husong_taskid > 0 or
...
function KuaFu1v1ViewMain:Flush()
	...
	if info then
		...
		if current_config then
			...
			self.my_rank_name:SetValue(current_config.rank_name)
			if next_config then
		...
		elseif next_config then
			...
			self.next_rank:SetValue(next_config.rank_name)
			self.my_rank_name:SetValue(Language.Common.WuDuanWei)
