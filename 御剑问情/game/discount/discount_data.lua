...
function DisCountData:GetHaveNewDiscount()
	return self.have_new_discount, self.new_phase_index	
end
...
function DisCountData:SetNewPhaseList(list)
	...
	if #self.old_phase_list < #self.new_phase_list then
		self.new_phase_index = #self.new_phase_list
...
function DisCountData:CheckCanActive()
	...
	for k, v in ipairs(self.new_phase_list) do
		...
		if level >= v.active_level and v.close_timestamp > server_time then
...
function DisCountData:GetDiscountInfoByType(phase)
...
function DisCountData:GetPhaseIndex()
	if nil ~= self.phase_index and nil ~= self:GetDiscountInfoByType(self.phase_index) then
		local v, k = self:GetDiscountInfoByType(self.phase_index)
		return k
	end
