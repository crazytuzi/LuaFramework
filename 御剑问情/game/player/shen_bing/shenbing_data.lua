...
function ShenBingData:__init()
	...
	self.shenbing_level_cfg = ListToMap(self.shenbing_cfg.level_attr, "level")
...
function ShenBingData:SetAutoToggle(is_on)
	self.auto_toggle = is_on
end

function ShenBingData:GetAutoToggle()
	return self.auto_toggle
end

--获取对应等级的发包数量
function ShenBingData:GetPackNumByLevel(level)
	if self.shenbing_level_cfg[level] then
		return self.shenbing_level_cfg[level].pack_num
	end
	return 1
end