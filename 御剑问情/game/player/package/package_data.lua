...
function PackageData:__init()
	...
	self.knapsack_grid_extend_cfg = ConfigManager.Instance:GetAutoConfig("otherconfig_auto").knapsack_grid_extend_cfg
...
function PackageData:SetRecycleItemDataList(is_add, data_list, color)
	if is_add then
		...
		for i = #self.recycle_data_list ,1 ,-1 do
			...
			if color == 2 and item_cfg then
				if item_cfg.color <= 2 then
					table.remove( self.recycle_data_list, i )
				end
			else
				if item_cfg and item_cfg.color == color then
					table.remove( self.recycle_data_list, i )
				end
			end
		end
...
--判断是否在回收列表中
function PackageData:IsInRecycleList(index)
	for k, v in ipairs(self.recycle_data_list) do
...
function PackageData:IsShowBagRedPoint()
	return EquipmentShenData.Instance:GetShenEquipRemind() > 0 or MojieData.Instance:GetMojieRemind() > 0
end
...
function PackageData:GetCellOpenNeedCount(opened_num)
	for k,v in pairs(self.knapsack_grid_extend_cfg) do
		if v.grid_index == opened_num then
			return v.need_item_count
		end
	end
	return 0
end
...
function PackageData:GetCanOpenHowManySlot(storage_type, num)
	...
	if storage_type == GameEnum.STORAGER_TYPE_STORAGER then
		...
		for i = now_index + 1, max_gird_num do
			open_one_need = self:GetCellOpenNeedCount(ItemData.Instance:GridExtendTimesByItem() + i - now_index)
			old_need_num = need_number
			need_number = need_number + open_one_need
			if old_need_num == 0 then
				old_need_num = need_number
			end
			can_open_num = can_open_num + 1
			if need_number > num then
...
function PackageData:GetOpenCellNeedItemNum(item_id, need_index)
	...
	if item_id == ItemDataKnapsackId.Id then
		for i = 1, need_index - ItemData.Instance:GetMaxKnapsackValidNum() + 1 do
			num =  num + self:GetCellOpenNeedCount(i + ItemData.Instance:GridExtendTimesByItem())
		end
	else
...
function PackageData:AutoRecyclEquip()
	...
	local data_list = ItemData.Instance:GetBagItemDataList()
	local cant_recy_list = RedEquipData.Instance:GetOrangeEquipSlotInfo()
	for k, v in pairs(data_list) do
		...
		if k < COMMON_CONSTS.MAX_BAG_COUNT and big_type == GameEnum.ITEM_BIGTYPE_EQUIPMENT
			...
			and (item_cfg.color <= auto_blue or item_cfg.color == auto_purple or item_cfg.color == auto_orange or item_cfg.color == auto_red) then
				...
				local can_recy = true
				if item_cfg.color == auto_orange then
					if cant_recy_list and next(cant_recy_list) then
						can_recy = false
						for k2, v2 in pairs(cant_recy_list) do
							for k3, v3 in pairs(v2) do
								if v3.item_id ==  v.item_id then
									can_recy = true
								end
							end
						end

						if Language.YiJieChengZhuang[v.item_id] then
							can_recy = true
						end
					end
				end
