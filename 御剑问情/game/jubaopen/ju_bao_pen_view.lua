...
function JuBaoPenView:OpenCallBack()
	RemindManager.Instance:SetTodayDoFlag(RemindName.JuBaoPen)
	self:FlushPrice()
	self:FlushCharge()
...
function JuBaoPenView:FlushCharge()
	...
	self.total_charge:SetValue(CommonDataManager.ConverMoney(price))
	if price == 0 then
...
function JuBaoPenRecordInfo:OnFlush()
	if self.data then
		...
		self.rate:SetValue(self.data.reward_gold)
