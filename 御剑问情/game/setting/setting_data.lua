...
SpritePartApperanceSettingType = {
	[SceneObjPart.Halo] = SETTING_APPERANCE_TYPE.SHIELD_OTHER_LINGZHU,
}
...
function SettingData:__init()
	...
	--记录两小时不再提示Toggle值
	self.bosstip_key_list = {}
	-- 记录不再提示Toggle值
	self.commontip_key_list = {}
	self.marry_guaji_index = 0
	self.card_guaji_index = 0
	self.first_msg_call_back_list = {}
end
...
function SettingData:OnSettingInfo(set_data)
	-- 每次重新上线要重置送花特效
	-- 这里有bug，修改送花特效会把自动使用小飞鞋也修改了
	if self.is_first then
		-- self:SetSettingData(SETTING_TYPE.FLOWER_EFFECT, false, true)
	end
	...
	self.card_guaji_index = set_data_list[HOT_KEY.CARD_PIECE].item_id
end
...
function SettingData:GetSettingData(setting_type)
	...
	-- 策划要求强制自动拾取
	if setting_type == SETTING_TYPE.AUTO_PICK_PROPERTY then
		return true
	end
...
function SettingData:SetCardPieceIndex(card_guaji_index)
	self.card_guaji_index = card_guaji_index
	SettingCtrl.Instance:SendChangeHotkeyReq(HOT_KEY.CARD_PIECE, card_guaji_index)
end

function SettingData:GetCardPieceIndex()
	return self.card_guaji_index
end
...
function SettingData:SetFocustherBossKey(key, time)
	self.bosstip_key_list[key] = time
end

function SettingData:GetFocusBossKey(key)
	if nil == key or "" == key or nil == self.bosstip_key_list[key]then
		return false
	end
	local server_time = TimeCtrl.Instance:GetServerTime()
	local t_time = TimeUtil.Timediff(server_time, self.bosstip_key_list[key])
	local hour = tonumber(t_time.hour)
	if hour < 2 then
		return true
	end
	return false
...
function SettingData:SetFirstMsgCallBack(call_back)
	if not self.is_first then
		call_back()
		return
	end
	self.first_msg_call_back_list[call_back] = true
end

function SettingData:RemoveFirstMsgCallBack(call_back)
	self.first_msg_call_back_list[call_back] = nil
end

function SettingData:FireFirstMsgCallBack()
	if self.is_first then
		for k,v in pairs(self.first_msg_call_back_list) do
			k()
		end
		self.first_msg_call_back_list = {}
		GlobalEventSystem:Fire(SettingEventType.SHIELD_APPERANCE)
		GlobalEventSystem:Fire(SettingEventType.MAIN_CAMERA_SETTING_CHANGE)
	end
end

function SettingData:GetApperanceSetting(key)
	local apperance = self:GetSettingDataListByKey(HOT_KEY.APPERANCE).item_id or 0
	local apperance_list = bit:d2b(apperance)
	return apperance_list[33 - key] == 1
end

function SettingData:SetApperanceSetting(key, state)
	local old_apperance = self:GetSettingDataListByKey(HOT_KEY.APPERANCE).item_id or 0
	local apperance_list = bit:d2b(old_apperance)
	apperance_list[33 - key] = state and 1 or 0
	local new_apperance = bit:b2d(apperance_list)
	self:SetSettingDataListByKey(HOT_KEY.APPERANCE, new_apperance)
	SettingCtrl.Instance:SendChangeHotkeyReq(HOT_KEY.APPERANCE, new_apperance)
	GlobalEventSystem:Fire(SettingEventType.SHIELD_APPERANCE)
end