QilinBiHuanHuaView = QilinBiHuanHuaView or BaseClass(BaseView)

function QilinBiHuanHuaView:__init()
	self.ui_config = {"uis/views/appearance_prefab", "QilinBiHuanHuaView"}
	self.click_func = BindTool.Bind(self.ClickItem, self)
	self.item_change_func = BindTool.Bind(self.ItemChangeCallBack, self)
end

function QilinBiHuanHuaView:__delete()
end

function QilinBiHuanHuaView:ReleaseCallBack()
	if self.model then
		self.model:DeleteMe()
		self.model = nil
	end
...
function QilinBiHuanHuaView:LoadCallBack()
	...
	self.model = RoleModel.New("qilinbi_huan_hua_panel")
...
function QilinBiHuanHuaView:CloseWindow()
	self:Close()
end

function QilinBiHuanHuaView:ClickUpGrade()
	...
	local is_active = QilinBiData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local huanhua_cfg_info = QilinBiData.Instance:GetHuanHuaCfgInfo(image_id)
	...
	if not is_active then
		...
		QilinBiCtrl.Instance:SendQilinBiSpecialImgUpgrade(data.image_id)
	end
end

function QilinBiHuanHuaView:ItemNotEnough(huanhua_cfg_info)
...
function QilinBiHuanHuaView:ClickUsed()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	QilinBiCtrl.Instance:SendUseQilinBiImage(image_id)
end

function QilinBiHuanHuaView:GetNumberOfCells()
	return #self.list_data
end

function QilinBiHuanHuaView:RefreshCell(cell, data_index)
	data_index = data_index + 1

	local huanhua_cell = self.cell_list[cell]
	if nil == huanhua_cell then
		huanhua_cell = QilinBiHuanHuaCell.New(cell.gameObject)
...
function QilinBiHuanHuaView:ClickItem(cell)
	if cell == nil then
		return
	end
...
function QilinBiHuanHuaView:FlushView()
	self:FlushContent()
	self:FlushItem()
	self:FlushModel()
end

function QilinBiHuanHuaView:FlushContent()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	local now_level = QilinBiData.Instance:GetHuanHuaGrade(image_id)
	local max_level = QilinBiData.Instance:GetSpecialImgMaxLevel()
	local power = CommonDataManager.GetCapabilityCalculation(huanhua_cfg_info)
	local next_hunahua_cfg_info = nil
	local add_power = 0
	local is_active = QilinBiData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local is_used = QilinBiData.Instance:GetHuanHuaIdIsUsed(image_id, true)
	local item_cfg = ItemData.Instance:GetItemConfig(data.item_id)
	local color = item_cfg and item_cfg.color or 1
	
	if now_level < max_level then	--获取下一级的幻化信息
		next_hunahua_cfg_info = QilinBiData.Instance:GetHuanHuaCfgInfo(image_id, now_level + 1)
	end
...
function QilinBiHuanHuaView:FlushItem()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	local image_id = data.image_id
	local level = QilinBiData.Instance:GetHuanHuaGrade(image_id)
...
function QilinBiHuanHuaView:FlushModel()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	local bundle, asset = ResPath.GetQilinBiModel(data["res_id" .. main_vo.sex .. "_h"], main_vo.sex)
	self.model:ResetRotation()
	self.model:SetMainAsset(bundle, asset)
end

function QilinBiHuanHuaView:FlushList()
	self.list_data = QilinBiData.Instance:GetHuanHuaCfgList() or {}
	self.list_view.scroller:ReloadData(0)
end

function QilinBiHuanHuaView:FlushListRemind()
	for _, v in pairs(self.cell_list) do
		v:FlushRemind()
	end
end

function QilinBiHuanHuaView:ItemChangeCallBack()
	self:Flush("flush_item")
end

function QilinBiHuanHuaView:OpenCallBack()
...
function QilinBiHuanHuaView:CloseCallBack()
	ItemData.Instance:UnNotifyDataChangeCallBack(self.item_change_func)
end

function QilinBiHuanHuaView:OnFlush(param_t)
...
---------------------------------------------------------------------------------------------------
QilinBiHuanHuaCell = QilinBiHuanHuaCell or BaseClass(BaseCell)
function QilinBiHuanHuaCell:__init()
	self.name = self:FindVariable("name")
...
function QilinBiHuanHuaCell:__delete()
	if self.item_cell then
		self.item_cell:DeleteMe()
		self.item_cell = nil
	end
...
function QilinBiHuanHuaCell:SetToggleGroup(group)
	self.root_node.toggle.group = group
end

function QilinBiHuanHuaCell:SetToggleIsOn(is_on)
	self.root_node.toggle.isOn = is_on
end

function QilinBiHuanHuaCell:FlushRemind()
	...
	--已满级没红点
	if QilinBiData.Instance:GetHuanHuaGrade(image_id) >= QilinBiData.Instance:GetSpecialImgMaxLevel() then
...
function QilinBiHuanHuaCell:OnFlush()
	if self.data == nil then
		return
	end
	...
	local is_active = QilinBiData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local is_use = QilinBiData.Instance:GetHuanHuaIdIsUsed(image_id, true)
