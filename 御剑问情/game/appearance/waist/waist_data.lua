...
function WaistData:__init()
	...
	local waist_cfg = ConfigManager.Instance:GetAutoConfig("yaoshi_auto")

	self.other_cfg = waist_cfg.other[1]
	self.waist_grade_cfg = ListToMap(waist_cfg.grade, "grade")
	self.waist_image_list_cfg = ListToMap(waist_cfg.image_list, "image_id")
	self.waist_special_img_cfg = ListToMap(waist_cfg.special_img, "image_id")
	self.waist_special_img_item_cfg = ListToMap(waist_cfg.special_img, "item_id")
	self.waist_special_image_upgrade_cfg = ListToMap(waist_cfg.special_image_upgrade, "special_img_id", "grade")

	self.special_grade_max_level = self:CalcSpecialImgMaxLevel()

	RemindManager.Instance:Register(RemindName.Waist_UpGrade, BindTool.Bind(self.CalcUpgradeRemind, self))
	RemindManager.Instance:Register(RemindName.Waist_ZiZhi, BindTool.Bind(self.CalcZiZhiRemind, self))
	RemindManager.Instance:Register(RemindName.Waist_HuanHua, BindTool.Bind(self.CalcHuanHuaRemind, self))
end

function WaistData:__delete()
	RemindManager.Instance:UnRegister(RemindName.Waist_UpGrade)
	RemindManager.Instance:UnRegister(RemindName.Waist_ZiZhi)
	RemindManager.Instance:UnRegister(RemindName.Waist_HuanHua)
...
function WaistData:GetSpecialImagesCfg()
	return self.waist_special_img_cfg or {}
end

function WaistData:GetImageMaxShowGrade()
	local max_info = self.waist_image_list_cfg and self.waist_image_list_cfg[#self.waist_image_list_cfg]
...
--计算腰饰材料是否足够
function WaistData:CalcUpgradeRemind()
	if nil == self.yaoshi_info then
		return 0
	end

	--没有下一阶表示已满阶
	local next_grade_info = self:GetWaistGradeCfgInfoByGrade(self.yaoshi_info.grade + 1)
	if nil == next_grade_info then
		return 0
	end

	local grade_info = self:GetWaistGradeCfgInfoByGrade(self.yaoshi_info.grade)
	if nil == grade_info then
		return 0
	end
...
--计算资质丹红点
function WaistData:CalcZiZhiRemind()
	if nil == self.yaoshi_info then
		return 0
	end

	--判断资质升级是否达到上限
	if self.yaoshi_info.shuxingdan_count < self:GetMaxShuXingDanCount() then
		--是否拥有资质丹
	 	local zizhi_info = AppearanceData.Instance:GetZiZhiCfgInfoByType(ZIZHI_TYPE.WAIST)
