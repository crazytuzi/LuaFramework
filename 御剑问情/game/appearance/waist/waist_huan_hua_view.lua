...
function WaistHuanHuaView:__init()
	self.ui_config = {"uis/views/appearance_prefab", "WaistHuanHuaView"}
...
function WaistHuanHuaView:LoadCallBack()
	...
	self.model = RoleModel.New("waist_huan_hua_panel")
...
function WaistHuanHuaView:CloseWindow()
	self:Close()
end
...
function WaistHuanHuaView:ClickUpGrade()
	...
	local is_active = WaistData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local huanhua_cfg_info = WaistData.Instance:GetHuanHuaCfgInfo(image_id)
	...
	if not is_active then
		...
		WaistCtrl.Instance:SendYaoShiSpecialImgUpgrade(data.image_id)
	end
end

function WaistHuanHuaView:ItemNotEnough(huanhua_cfg_info)
...
function WaistHuanHuaView:ClickUsed()
	...
	WaistCtrl.Instance:SendUseYaoShiImage(image_id)
end
...
function WaistHuanHuaView:FlushContent()
	...
	local now_level = WaistData.Instance:GetHuanHuaGrade(image_id)
	...
	local add_power = 0
	local is_active = WaistData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local is_used = WaistData.Instance:GetHuanHuaIdIsUsed(image_id, true)
	local item_cfg = ItemData.Instance:GetItemConfig(data.item_id)
	local color = item_cfg and item_cfg.color or 1
	
	if now_level < max_level then	--获取下一级的幻化信息
		next_hunahua_cfg_info = WaistData.Instance:GetHuanHuaCfgInfo(image_id, now_level + 1)
	end
...
function WaistHuanHuaView:FlushItem()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	local image_id = data.image_id
	local level = WaistData.Instance:GetHuanHuaGrade(image_id)
...
function WaistHuanHuaView:FlushList()
	self.list_data = WaistData.Instance:GetHuanHuaCfgList() or {}
	self.list_view.scroller:ReloadData(0)
end

function WaistHuanHuaView:FlushListRemind()
	for _, v in pairs(self.cell_list) do
		v:FlushRemind()
	end
end

function WaistHuanHuaView:ItemChangeCallBack()
...
function WaistHuanHuaCell:FlushRemind()
	...
	--已满级没红点
	if WaistData.Instance:GetHuanHuaGrade(image_id) >= WaistData.Instance:GetSpecialImgMaxLevel() then
...
function WaistHuanHuaCell:OnFlush()
	...
	local is_active = WaistData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local is_use = WaistData.Instance:GetHuanHuaIdIsUsed(image_id, true)
