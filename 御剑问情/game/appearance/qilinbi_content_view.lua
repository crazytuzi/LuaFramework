QilinBiContentView = QilinBiContentView or BaseClass(BaseRender)

function QilinBiContentView:__init()
	...
	--红点
	self.red_point_list = {
		[RemindName.QilinBi_ZiZhi] = self:FindVariable("zizhi_remind"),
		[RemindName.QilinBi_HuanHua] = self:FindVariable("huanhua_remind"),
	}
...
function QilinBiContentView:__delete()
	if self.model then
		self.model:DeleteMe()
		self.model = nil
	end
...
function QilinBiContentView:RemindChangeCallBack(key, value)
	if self.red_point_list[key] then
		self.red_point_list[key]:SetValue(value > 0)
	end
end

function QilinBiContentView:ClickUse()
	local grade_info = QilinBiData.Instance:GetQilinBiGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then
		return
	end

	QilinBiCtrl.Instance:SendUseQilinBiImage(grade_info.image_id, 0)
end

function QilinBiContentView:ClickLeft()
	self.select_grade = self.select_grade - 1
	self:FlushView()
end

function QilinBiContentView:ClickRight()
	self.select_grade = self.select_grade + 1
	self:FlushView()
end

function QilinBiContentView:ClickUpgrade()
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if nil == qilinbi_info then
		return
	end

	local grade_info = QilinBiData.Instance:GetQilinBiGradeCfgInfoByGrade(qilinbi_info.grade)
	if nil == grade_info then
		return
	end

	--获取下一级，不存在则满级
	local next_grade_info = QilinBiData.Instance:GetQilinBiGradeCfgInfoByGrade(qilinbi_info.grade + 1)
	...
	if self.is_upgrade_state then
		QilinBiCtrl.Instance:SendUpgradeQilinBi(grade_info.pack_num, is_auto_buy)
	end
end

function QilinBiContentView:ClickHelp()
end

function QilinBiContentView:ClickZiZhi()
	ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "qilinbi_zizhi")
end

function QilinBiContentView:ClickHuanHua()
	ViewManager.Instance:Open(ViewName.QilinBiHuanHua)
end

-- 显示全属性加成面板
function QilinBiContentView:OnClickAllAttrBtn()
	self.all_attr_panel:SetActive(not self.all_attr_panel.gameObject.activeSelf)
end

function QilinBiContentView:FlushAllAttrPanel()
	local all_attr_percent = QilinBiData.Instance:GetAllAttrPercent()
	local active_need_grade = QilinBiData.Instance:GetActiveNeedGrade()
	local cur_grade = QilinBiData.Instance:GetGrade()
...
function QilinBiContentView:InitView()
	self.is_init = true
	...
	if nil ~= qilinbi_info then
		--默认选择已使用的形象阶数
		if qilinbi_info.grade == 1 then
			--零阶和一阶使用相同形象
			self.select_grade = qilinbi_info.grade
		else
			if qilinbi_info.used_imageid > GameEnum.MOUNT_SPECIAL_IMA_ID then			-- 使用了幻化形象
				local max_show_grade = QilinBiData.Instance:GetImageMaxShowGrade()
				self.select_grade = qilinbi_info.grade >= max_show_grade and qilinbi_info.grade or qilinbi_info.grade + 1
			else
				local image_info = QilinBiData.Instance:GetQilinBiImageCfgInfoByImageId(qilinbi_info.used_imageid)
...
function QilinBiContentView:FlushView()
	self:FlushLeft()
	self:FlushRight()
end

function QilinBiContentView:FlushModel()
	--对应等级数据
	local grade_info = QilinBiData.Instance:GetQilinBiGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then
		return
...
function QilinBiContentView:FlushLeft()
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if nil == qilinbi_info then
		return
	end
	...
	if self.old_grade < 0 then
		self.old_grade = qilinbi_info.grade
	end

	if self.old_grade ~= qilinbi_info.grade then
		--进阶成功
	...
	local temp_grade_info = QilinBiData.Instance:GetQilinBiGradeCfgInfoByGrade(self.select_grade)
	...
	--刷新使用状态
	if qilinbi_info.grade >= self.select_grade then
		self.can_use:SetValue(true)
		self.is_used:SetValue(temp_grade_info.image_id == qilinbi_info.used_imageid)
	else
		self.can_use:SetValue(false)
	end

	--刷新左右箭头
	local last_temp_grade_info = QilinBiData.Instance:GetQilinBiGradeCfgInfoByGrade(self.select_grade - 1)
	local is_show_left = true
	if nil == last_temp_grade_info or (qilinbi_info.grade > 1 and temp_grade_info.show_grade == 1) then
		--没有上一阶属性或者处于第一阶(服务器第二阶)不显示左箭头
		is_show_left = false
	end
	self.show_left:SetValue(is_show_left)

	local next_temp_grade_info = QilinBiData.Instance:GetQilinBiGradeCfgInfoByGrade(self.select_grade + 1)
	local is_show_right = true
	if nil == next_temp_grade_info or self.select_grade > qilinbi_info.grade then
		is_show_right = false
	end
	self.show_right:SetValue(is_show_right)
end

function QilinBiContentView:FlushRight()
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if nil == qilinbi_info then
		return
	end
	...
	local all_attr_percent_cap = QilinBiData.Instance:CalculateAllAttrCap(capability)
	...
	if nil ~= next_grade_info then
		...
		pro_value = qilinbi_info.grade_bless_val / grade_info.bless_val_limit
		pro_str = string.format("%s / %s", qilinbi_info.grade_bless_val, grade_info.bless_val_limit)
	end
...
function QilinBiContentView:UpGradeResult(result)
	...
	if self.toggle_check.isOn then
		QilinBiCtrl.Instance:SendUpgradeQilinBi(grade_info.pack_num, 1)
	else
		QilinBiCtrl.Instance:SendUpgradeQilinBi(grade_info.pack_num, 0)
	end
end

function QilinBiContentView:FlushItem()
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if nil == qilinbi_info then
		return
	end
...
function QilinBiContentView:PlayEffect()
...
function QilinBiContentView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "qilinbi" then
			self:FlushView()
		elseif k == "qilinbi_upgrade" then
			self:UpGradeResult(v[1])
		elseif k == "qilinbi_item_change" then
			self:FlushItem()
		end
	end
end