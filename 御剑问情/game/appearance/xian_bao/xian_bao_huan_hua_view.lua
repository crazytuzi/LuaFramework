...
function XianBaoHuanHuaView:__init()
	self.ui_config = {"uis/views/appearance_prefab", "XianBaoHuanHuaView"}
...
function XianBaoHuanHuaView:CloseWindow()
	self:Close()
end
...
function XianBaoHuanHuaView:ClickUpGrade()
	...
	XianBaoCtrl.Instance:SendXianBaoSpecialImgUpgrade(data.image_id)
end
...
function XianBaoHuanHuaView:ClickUsed()
	...
	--特殊形象加1000
	XianBaoCtrl.Instance:SendUseXianBaoImage(data.image_id + 1000)
end
...
function XianBaoHuanHuaView:FlushContent()
	...
	local huanhua_cfg_info = XianBaoData.Instance:GetHuanHuaCfgInfo(image_id)
	if huanhua_cfg_info == nil then
		return
	end

	local now_level = XianBaoData.Instance:GetHuanHuaGrade(image_id)
	...
	if now_level >= max_level then
		...
		--获取下一级的幻化信息
		next_hunahua_cfg_info = XianBaoData.Instance:GetHuanHuaCfgInfo(image_id, now_level + 1)
	end
	...
	local is_active = XianBaoData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	self.is_active:SetValue(is_active)

	local is_used = XianBaoData.Instance:GetHuanHuaIdIsUsed(image_id, true)
	self.is_used:SetValue(is_used)
end

function XianBaoHuanHuaView:FlushItem()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	--已满级不处理
	if XianBaoData.Instance:GetHuanHuaGrade(image_id) >= XianBaoData.Instance:GetSpecialImgMaxLevel() then
...
function XianBaoHuanHuaView:FlushModel()
	...
	self.model:SetXianBaoResid(data.res_id)
end

function XianBaoHuanHuaView:FlushList()
	self.list_data = XianBaoData.Instance:GetHuanHuaCfgList() or {}
	self.list_view.scroller:ReloadData(0)
end

function XianBaoHuanHuaView:FlushListRemind()
	for _, v in pairs(self.cell_list) do
		v:FlushRemind()
	end
end

function XianBaoHuanHuaView:ItemChangeCallBack()
...
function XianBaoHuanHuaCell:FlushRemind()
...
function XianBaoHuanHuaCell:OnFlush()
	if self.data == nil then
		return
	end
	...
	local is_use = XianBaoData.Instance:GetHuanHuaIdIsUsed(image_id, true)
