...
function XianBaoData:__init()
	...
	local xianbao_cfg = ConfigManager.Instance:GetAutoConfig("xianbao_auto")

	self.other_cfg = xianbao_cfg.other[1]
	self.xianbao_grade_cfg = ListToMap(xianbao_cfg.grade, "grade")
	self.xianbao_image_list_cfg = ListToMap(xianbao_cfg.image_list, "image_id")
	self.xianbao_special_img_cfg = ListToMap(xianbao_cfg.special_img, "image_id")
	self.xianbao_special_image_upgrade_cfg = ListToMap(xianbao_cfg.special_image_upgrade, "special_img_id", "grade")

	self.special_grade_max_level = self:CalcSpecialImgMaxLevel()

	RemindManager.Instance:Register(RemindName.XianBao_UpGrade, BindTool.Bind(self.CalcUpgradeRemind, self))
	RemindManager.Instance:Register(RemindName.XianBao_ZiZhi, BindTool.Bind(self.CalcZiZhiRemind, self))
	RemindManager.Instance:Register(RemindName.XianBao_HuanHua, BindTool.Bind(self.CalcHuanHuaRemind, self))
end

function XianBaoData:__delete()
	RemindManager.Instance:UnRegister(RemindName.XianBao_UpGrade)
	RemindManager.Instance:UnRegister(RemindName.XianBao_ZiZhi)
	RemindManager.Instance:UnRegister(RemindName.XianBao_HuanHua)
...
function XianBaoData:SetXianBaoInfo(info)
	...
	self.special_img_grade_list = self.xianbao_info.special_img_grade_list
...
function XianBaoData:GetHuanHuaIdIsUsed(image_id, is_special)
...
--获取对应幻化形象的红点
function XianBaoData:GetHuanHuaRemindByImageId(image_id)
	--没达到对应开服天数, 或者等级没达到要求没有红点
	local open_server_day = TimeCtrl.Instance:GetCurOpenServerDay()
	local main_vo = GameVoManager.Instance:GetMainRoleVo()
	local image_info = self.xianbao_special_img_cfg[image_id]
	...
	local grade_list = self.xianbao_special_image_upgrade_cfg[image_id]
...
--计算升级材料是否足够
function XianBaoData:CalcUpgradeRemind()
	if nil == self.xianbao_info then
		return 0
	end

	--没有下一阶表示已满阶
	local next_grade_info = self:GetXianBaoGradeCfgInfoByGrade(self.xianbao_info.grade + 1)
	if nil == next_grade_info then
		return 0
	end

	local grade_info = self:GetXianBaoGradeCfgInfoByGrade(self.xianbao_info.grade)
	if nil == grade_info then
		return 0
	end
...
function XianBaoData:CalcHuanHuaRemind()
	...
	for k, v in ipairs(self.special_img_grade_list) do
		...
		img_info = self.xianbao_special_img_cfg[k]
...
--计算资质丹红点
function XianBaoData:CalcZiZhiRemind()
	if nil == self.xianbao_info then
		return 0
	end

	--判断资质升级是否达到上限
	if self.xianbao_info.shuxingdan_count < self:GetMaxShuXingDanCount() then
		--是否拥有资质丹
	 	local zizhi_info = AppearanceData.Instance:GetZiZhiCfgInfoByType(ZIZHI_TYPE.XIANBAO)
