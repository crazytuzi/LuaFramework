...
function MaskHuanHuaView:__init()
	self.ui_config = {"uis/views/appearance_prefab", "MaskHuanHuaView"}
...
function MaskHuanHuaView:LoadCallBack()
	...
	self.model = RoleModel.New("mask_huan_hua_panel")
...
function MaskHuanHuaView:CloseWindow()
	self:Close()
end
...
function MaskHuanHuaView:ClickUpGrade()
	...
	local is_active = MaskData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local huanhua_cfg_info = MaskData.Instance:GetHuanHuaCfgInfo(image_id)
	...
	if not is_active then
		...
		MaskCtrl.Instance:SendMaskSpecialImgUpgrade(data.image_id)
	end
end

function MaskHuanHuaView:ItemNotEnough(huanhua_cfg_info)
...
function MaskHuanHuaView:ClickUsed()
	...
	MaskCtrl.Instance:SendUseMaskImage(image_id)
end
...
function MaskHuanHuaView:FlushContent()
	...
	local now_level = MaskData.Instance:GetHuanHuaGrade(image_id)
	...
	local add_power = 0
	local is_active = MaskData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local is_used = MaskData.Instance:GetHuanHuaIdIsUsed(image_id, true)
	local item_cfg = ItemData.Instance:GetItemConfig(data.item_id)
	local color = item_cfg and item_cfg.color or 1
	
	if now_level < max_level then	--获取下一级的幻化信息
		next_hunahua_cfg_info = MaskData.Instance:GetHuanHuaCfgInfo(image_id, now_level + 1)
	end
...
function MaskHuanHuaView:FlushItem()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	local image_id = data.image_id
	local level = MaskData.Instance:GetHuanHuaGrade(image_id)
...
function MaskHuanHuaView:FlushList()
	self.list_data = MaskData.Instance:GetHuanHuaCfgList() or {}
	self.list_view.scroller:ReloadData(0)
end

function MaskHuanHuaView:FlushListRemind()
	for _, v in pairs(self.cell_list) do
		v:FlushRemind()
	end	
end

function MaskHuanHuaView:ItemChangeCallBack()
...
function MaskHuanHuaCell:FlushRemind()
	...
	--已满级没红点
	if MaskData.Instance:GetHuanHuaGrade(image_id) >= MaskData.Instance:GetSpecialImgMaxLevel() then
...
function MaskHuanHuaCell:OnFlush()
	...
	local is_active = MaskData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	local is_use = MaskData.Instance:GetHuanHuaIdIsUsed(image_id, true)
