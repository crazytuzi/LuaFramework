MaskContentView = MaskContentView or BaseClass(BaseRender)

function MaskContentView:__init()
	...
	--红点
	self.red_point_list = {
		[RemindName.Mask_ZiZhi] = self:FindVariable("zizhi_remind"),
		[RemindName.Mask_HuanHua] = self:FindVariable("huanhua_remind"),
	}
...
function MaskContentView:__delete()
	if self.model then
		self.model:DeleteMe()
		self.model = nil
	end
...
function MaskContentView:RemindChangeCallBack(key, value)
	if self.red_point_list[key] then
		self.red_point_list[key]:SetValue(value > 0)
	end
end

function MaskContentView:ClickUse()
	local grade_info = MaskData.Instance:GetMaskGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then
		return
	end

	MaskCtrl.Instance:SendUseMaskImage(grade_info.image_id, 0)
end

function MaskContentView:ClickLeft()
	self.select_grade = self.select_grade - 1
	self:FlushView()
end

function MaskContentView:ClickRight()
	self.select_grade = self.select_grade + 1
	self:FlushView()
end

function MaskContentView:ClickUpgrade()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if nil == mask_info then
		return
	end

	local grade_info = MaskData.Instance:GetMaskGradeCfgInfoByGrade(mask_info.grade)
	if nil == grade_info then
		return
	end

	--获取下一级，不存在则满级
	local next_grade_info = MaskData.Instance:GetMaskGradeCfgInfoByGrade(mask_info.grade + 1)
	...
	if self.is_upgrade_state then
		MaskCtrl.Instance:SendUpgradeMask(grade_info.pack_num, is_auto_buy)
	end
end

function MaskContentView:ClickHelp()
end

function MaskContentView:ClickZiZhi()
	ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "mask_zizhi")
end

function MaskContentView:ClickHuanHua()
	ViewManager.Instance:Open(ViewName.MaskHuanHua)
end

-- 显示全属性加成面板
function MaskContentView:OnClickAllAttrBtn()
	self.all_attr_panel:SetActive(not self.all_attr_panel.gameObject.activeSelf)
end

function MaskContentView:FlushAllAttrPanel()
	local all_attr_percent = MaskData.Instance:GetAllAttrPercent()
	local active_need_grade = MaskData.Instance:GetActiveNeedGrade()
	local cur_grade = MaskData.Instance:GetGrade()
...
function MaskContentView:InitView()
	self.is_init = true
	...
	if nil ~= mask_info then
		--默认选择已使用的形象阶数
		if mask_info.grade == 1 then
			--零阶和一阶使用相同形象
			self.select_grade = mask_info.grade
		else
			if mask_info.used_imageid > GameEnum.MOUNT_SPECIAL_IMA_ID then			-- 使用了幻化形象
				local max_show_grade = MaskData.Instance:GetImageMaxShowGrade()
				self.select_grade = mask_info.grade >= max_show_grade and mask_info.grade or mask_info.grade + 1
			else
				local image_info = MaskData.Instance:GetMaskImageCfgInfoByImageId(mask_info.used_imageid)
...
function MaskContentView:FlushView()
	self:FlushLeft()
	self:FlushRight()
end

function MaskContentView:FlushModel()
	--对应等级数据
	local grade_info = MaskData.Instance:GetMaskGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then
		return
...
function MaskContentView:FlushLeft()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if nil == mask_info then
		return
	end
	...
	if self.old_grade < 0 then
		self.old_grade = mask_info.grade
	end

	if self.old_grade ~= mask_info.grade then
		--进阶成功
	...
	local temp_grade_info = MaskData.Instance:GetMaskGradeCfgInfoByGrade(self.select_grade)
	...
	--刷新使用状态
	if mask_info.grade >= self.select_grade then
		self.can_use:SetValue(true)
		self.is_used:SetValue(temp_grade_info.image_id == mask_info.used_imageid)
	else
		self.can_use:SetValue(false)
	end

	--刷新左右箭头
	local last_temp_grade_info = MaskData.Instance:GetMaskGradeCfgInfoByGrade(self.select_grade - 1)
	local is_show_left = true
	if nil == last_temp_grade_info or (mask_info.grade > 1 and temp_grade_info.show_grade == 1) then
		--没有上一阶属性或者处于第一阶(服务器第二阶)不显示左箭头
		is_show_left = false
	end
	self.show_left:SetValue(is_show_left)

	local next_temp_grade_info = MaskData.Instance:GetMaskGradeCfgInfoByGrade(self.select_grade + 1)
	local is_show_right = true
	if nil == next_temp_grade_info or self.select_grade > mask_info.grade then
		is_show_right = false
	end
	self.show_right:SetValue(is_show_right)
end

function MaskContentView:FlushRight()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if nil == mask_info then
		return
	end
	...
	local all_attr_percent_cap = MaskData.Instance:CalculateAllAttrCap(capability) 
	...
	if nil ~= next_grade_info then
		...
		pro_value = mask_info.grade_bless_val / grade_info.bless_val_limit
		pro_str = string.format("%s / %s", mask_info.grade_bless_val, grade_info.bless_val_limit)
	end
...
function MaskContentView:UpGradeResult(result)
	...
	if self.toggle_check.isOn then
		MaskCtrl.Instance:SendUpgradeMask(grade_info.pack_num, 1)
	else
		MaskCtrl.Instance:SendUpgradeMask(grade_info.pack_num, 0)
	end
end

function MaskContentView:FlushItem()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if nil == mask_info then
		return
	end
...
function MaskContentView:PlayEffect()
...
function MaskContentView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "mask" then
			self:FlushView()
		elseif k == "mask_upgrade" then
			self:UpGradeResult(v[1])
		elseif k == "mask_item_change" then
			self:FlushItem()
		end
	end
end