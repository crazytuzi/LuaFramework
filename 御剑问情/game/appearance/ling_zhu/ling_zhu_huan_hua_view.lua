LingZhuHuanHuaView = LingZhuHuanHuaView or BaseClass(BaseView)

function LingZhuHuanHuaView:__init()
	self.ui_config = {"uis/views/appearance_prefab", "LingZhuHuanHuaView"}
	self.click_func = BindTool.Bind(self.ClickItem, self)
	self.item_change_func = BindTool.Bind(self.ItemChangeCallBack, self)
end

function LingZhuHuanHuaView:__delete()
end

function LingZhuHuanHuaView:ReleaseCallBack()
	if self.model then
		self.model:DeleteMe()
		self.model = nil
	end
...
function LingZhuHuanHuaView:LoadCallBack()
	self.select_index = 1
...
function LingZhuHuanHuaView:CloseWindow()
	self:Close()
end

function LingZhuHuanHuaView:ClickUpGrade()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end

	LingZhuCtrl.Instance:SendLingZhuSpecialImgUpgrade(data.image_id)
end

function LingZhuHuanHuaView:ClickUsed()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	--特殊形象加1000
	LingZhuCtrl.Instance:SendUseLingZhuImage(data.image_id + 1000)
end

function LingZhuHuanHuaView:GetNumberOfCells()
	return #self.list_data
end

function LingZhuHuanHuaView:RefreshCell(cell, data_index)
	data_index = data_index + 1

	local huanhua_cell = self.cell_list[cell]
	if nil == huanhua_cell then
		huanhua_cell = LingZhuHuanHuaCell.New(cell.gameObject)
...
function LingZhuHuanHuaView:ClickItem(cell)
	if cell == nil then
		return
	end
...
function LingZhuHuanHuaView:FlushView()
	self:FlushContent()
	self:FlushItem()
	self:FlushModel()
end

function LingZhuHuanHuaView:FlushContent()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	local huanhua_cfg_info = LingZhuData.Instance:GetHuanHuaCfgInfo(image_id)
	if huanhua_cfg_info == nil then
		return
	end

	local now_level = LingZhuData.Instance:GetHuanHuaGrade(image_id)
	local max_level = LingZhuData.Instance:GetSpecialImgMaxLevel()
	...
	if now_level >= max_level then
		...
		--获取下一级的幻化信息
		next_hunahua_cfg_info = LingZhuData.Instance:GetHuanHuaCfgInfo(image_id, now_level + 1)
	end
	...
	local is_active = LingZhuData.Instance:GetHuanHuaIsActiveByImageId(image_id)
	self.is_active:SetValue(is_active)

	local is_used = LingZhuData.Instance:GetHuanHuaIdIsUsed(image_id, true)
	self.is_used:SetValue(is_used)
end

function LingZhuHuanHuaView:FlushItem()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	--已满级不处理
	if LingZhuData.Instance:GetHuanHuaGrade(image_id) >= LingZhuData.Instance:GetSpecialImgMaxLevel() then
...
function LingZhuHuanHuaView:FlushModel()
	local data = self.list_data[self.select_index]
	if data == nil then
		return
	end
	...
	self.model:SetLingZhuResid(data.res_id)
end

function LingZhuHuanHuaView:FlushList()
	self.list_data = LingZhuData.Instance:GetHuanHuaCfgList() or {}
	self.list_view.scroller:ReloadData(0)
end

function LingZhuHuanHuaView:FlushListRemind()
	for _, v in pairs(self.cell_list) do
		v:FlushRemind()
	end
end

function LingZhuHuanHuaView:ItemChangeCallBack()
	self:Flush("flush_item")
end

function LingZhuHuanHuaView:OpenCallBack()
...
function LingZhuHuanHuaView:CloseCallBack()
	ItemData.Instance:UnNotifyDataChangeCallBack(self.item_change_func)
end

function LingZhuHuanHuaView:OnFlush(param_t)
...
LingZhuHuanHuaCell = LingZhuHuanHuaCell or BaseClass(BaseCell)
function LingZhuHuanHuaCell:__init()
	self.name = self:FindVariable("name")
...
function LingZhuHuanHuaCell:__delete()
	if self.item_cell then
		self.item_cell:DeleteMe()
		self.item_cell = nil
	end
...
function LingZhuHuanHuaCell:SetToggleGroup(group)
	self.root_node.toggle.group = group
end

function LingZhuHuanHuaCell:SetToggleIsOn(is_on)
	self.root_node.toggle.isOn = is_on
end

function LingZhuHuanHuaCell:FlushRemind()
...
function LingZhuHuanHuaCell:OnFlush()
	if self.data == nil then
		return
	end
	...
	local is_use = LingZhuData.Instance:GetHuanHuaIdIsUsed(image_id, true)
