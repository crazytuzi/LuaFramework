TouShiContentView = TouShiContentView or BaseClass(BaseRender)

function TouShiContentView:__init()
	...
	--红点
	self.red_point_list = {
		[RemindName.TouShi_ZiZhi] = self:FindVariable("zizhi_remind"),
		[RemindName.TouShi_HuanHua] = self:FindVariable("huanhua_remind"),
	}
...
function TouShiContentView:__delete()
	if self.model then
		self.model:DeleteMe()
		self.model = nil
	end
...
function TouShiContentView:RemindChangeCallBack(key, value)
	if self.red_point_list[key] then
		self.red_point_list[key]:SetValue(value > 0)
	end
end

function TouShiContentView:ClickUse()
	local grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then
		return
	end

	TouShiCtrl.Instance:SendUseTouShiImage(grade_info.image_id, 0)
end

function TouShiContentView:ClickLeft()
	self.select_grade = self.select_grade - 1
	self:FlushView()
end

function TouShiContentView:ClickRight()
	self.select_grade = self.select_grade + 1
	self:FlushView()
end

function TouShiContentView:ClickUpgrade()
	local toushi_info = TouShiData.Instance:GetTouShiInfo()
	if nil == toushi_info then
		return
	end

	local grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(toushi_info.grade)
	if nil == grade_info then
		return
	end

	--获取下一级，不存在则满级
	local next_grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(toushi_info.grade + 1)
	...
	if self.is_upgrade_state then
		TouShiCtrl.Instance:SendUpgradeTouShi(grade_info.pack_num, is_auto_buy)
	end
end

function TouShiContentView:ClickHelp()
end

function TouShiContentView:ClickZiZhi()
	ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "toushi_zizhi")
end

function TouShiContentView:ClickHuanHua()
	ViewManager.Instance:Open(ViewName.TouShiHuanHua)
end

-- 显示全属性加成面板
function TouShiContentView:OnClickAllAttrBtn()
	self.all_attr_panel:SetActive(not self.all_attr_panel.gameObject.activeSelf)
end

function TouShiContentView:FlushAllAttrPanel()
	local all_attr_percent = TouShiData.Instance:GetAllAttrPercent()
	local active_need_grade = TouShiData.Instance:GetActiveNeedGrade()
	local cur_grade = TouShiData.Instance:GetGrade()
...
function TouShiContentView:InitView()
	self.is_init = true
	...
	if nil ~= toushi_info then
		--默认选择已使用的形象阶数
		if toushi_info.grade == 1 then
			--零阶和一阶使用相同形象
			self.select_grade = toushi_info.grade
		else
			if toushi_info.used_imageid > GameEnum.MOUNT_SPECIAL_IMA_ID then			-- 使用了幻化形象
				local max_show_grade = TouShiData.Instance:GetImageMaxShowGrade()
				self.select_grade = toushi_info.grade >= max_show_grade and toushi_info.grade or toushi_info.grade + 1
			else
				local image_info = TouShiData.Instance:GetTouShiImageCfgInfoByImageId(toushi_info.used_imageid)
...
function TouShiContentView:FlushView()
	self:FlushLeft()
	self:FlushRight()
end

function TouShiContentView:FlushModel()
	--对应等级数据
	local grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then
		return
...
function TouShiContentView:FlushLeft()
	local toushi_info = TouShiData.Instance:GetTouShiInfo()
	if nil == toushi_info then
		return
	end
	...
	if self.old_grade < 0 then
		self.old_grade = toushi_info.grade
	end

	if self.old_grade ~= toushi_info.grade then
		--进阶成功
	...
	local temp_grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(self.select_grade)
	...
	--刷新使用状态
	if toushi_info.grade >= self.select_grade then
		self.can_use:SetValue(true)
		self.is_used:SetValue(temp_grade_info.image_id == toushi_info.used_imageid)
	else
		self.can_use:SetValue(false)
	end

	--刷新左右箭头
	local last_temp_grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(self.select_grade - 1)
	local is_show_left = true
	if nil == last_temp_grade_info or (toushi_info.grade > 1 and temp_grade_info.show_grade == 1) then
		--没有上一阶属性或者处于第一阶(服务器第二阶)不显示左箭头
		is_show_left = false
	end
	self.show_left:SetValue(is_show_left)

	local next_temp_grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(self.select_grade + 1)
	local is_show_right = true
	if nil == next_temp_grade_info or self.select_grade > toushi_info.grade then
		is_show_right = false
	end
	self.show_right:SetValue(is_show_right)
end

function TouShiContentView:FlushRight()
	local toushi_info = TouShiData.Instance:GetTouShiInfo()
	if nil == toushi_info then
		return
	end
	...
	local all_attr_percent_cap = TouShiData.Instance:CalculateAllAttrCap(capability)
	...
	if nil ~= next_grade_info then
		...
		pro_value = toushi_info.grade_bless_val / grade_info.bless_val_limit
		pro_str = string.format("%s / %s", toushi_info.grade_bless_val, grade_info.bless_val_limit)
	end
...
function TouShiContentView:UpGradeResult(result)
	...
	if self.toggle_check.isOn then
		TouShiCtrl.Instance:SendUpgradeTouShi(grade_info.pack_num, 1)
	else
		TouShiCtrl.Instance:SendUpgradeTouShi(grade_info.pack_num, 0)
	end
end

function TouShiContentView:FlushItem()
	local toushi_info = TouShiData.Instance:GetTouShiInfo()
	if nil == toushi_info then
		return
	end
...
function TouShiContentView:PlayEffect()
...
function TouShiContentView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "toushi" then
			self:FlushView()
		elseif k == "toushi_upgrade" then
			self:UpGradeResult(v[1])
		elseif k == "toushi_item_change" then
			self:FlushItem()
		end
	end
end