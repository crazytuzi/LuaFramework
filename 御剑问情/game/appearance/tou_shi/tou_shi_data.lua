...
function TouShiData:__init()
	...
	local toushi_cfg = ConfigManager.Instance:GetAutoConfig("toushi_auto")

	self.other_cfg = toushi_cfg.other[1]
	self.toushi_grade_cfg = ListToMap(toushi_cfg.grade, "grade")
	self.toushi_image_list_cfg = ListToMap(toushi_cfg.image_list, "image_id")
	self.toushi_special_img_cfg = ListToMap(toushi_cfg.special_img, "image_id")
	self.toushi_special_img_item_cfg = ListToMap(toushi_cfg.special_img, "item_id")
	self.toushi_special_image_upgrade_cfg = ListToMap(toushi_cfg.special_image_upgrade, "special_img_id", "grade")

	self.special_grade_max_level = self:CalcSpecialImgMaxLevel()

	RemindManager.Instance:Register(RemindName.TouShi_UpGrade, BindTool.Bind(self.CalcUpgradeRemind, self))
	RemindManager.Instance:Register(RemindName.TouShi_ZiZhi, BindTool.Bind(self.CalcZiZhiRemind, self))
	RemindManager.Instance:Register(RemindName.TouShi_HuanHua, BindTool.Bind(self.CalcHuanHuaRemind, self))
end

function TouShiData:__delete()
	RemindManager.Instance:UnRegister(RemindName.TouShi_UpGrade)
	RemindManager.Instance:UnRegister(RemindName.TouShi_ZiZhi)
	RemindManager.Instance:UnRegister(RemindName.TouShi_HuanHua)
...
function TouShiData:GetSpecialImagesCfg()
	return self.toushi_special_img_cfg or {}
end

function TouShiData:GetImageMaxShowGrade()
	local max_info = self.toushi_image_list_cfg and self.toushi_image_list_cfg[#self.toushi_image_list_cfg]
...
--计算升级材料是否足够
function TouShiData:CalcUpgradeRemind()
	if nil == self.toushi_info then
		return 0
	end

	--没有下一阶表示已满阶
	local next_grade_info = self:GetTouShiGradeCfgInfoByGrade(self.toushi_info.grade + 1)
	if nil == next_grade_info then
		return 0
	end

	local grade_info = self:GetTouShiGradeCfgInfoByGrade(self.toushi_info.grade)
	if nil == grade_info then
		return 0
	end
...
--计算资质丹红点
function TouShiData:CalcZiZhiRemind()
	if nil == self.toushi_info then
		return 0
	end

	--判断资质升级是否达到上限
	if self.toushi_info.shuxingdan_count < self:GetMaxShuXingDanCount() then
		--是否拥有资质丹
	 	local zizhi_info = AppearanceData.Instance:GetZiZhiCfgInfoByType(ZIZHI_TYPE.TOUSHI)
