...
function LingQiData:__init()
	...
	local lingqi_cfg = ConfigManager.Instance:GetAutoConfig("lingqi_auto")

	self.other_cfg = lingqi_cfg.other[1]
	self.lingqi_grade_cfg = ListToMap(lingqi_cfg.grade, "grade")
	self.lingqi_image_list_cfg = ListToMap(lingqi_cfg.image_list, "image_id")
	self.lingqi_special_img_cfg = ListToMap(lingqi_cfg.special_img, "image_id")
	self.lingqi_special_image_upgrade_cfg = ListToMap(lingqi_cfg.special_image_upgrade, "special_img_id", "grade")

	self.special_grade_max_level = self:CalcSpecialImgMaxLevel()

	RemindManager.Instance:Register(RemindName.LingQi_UpGrade, BindTool.Bind(self.CalcUpgradeRemind, self))
	RemindManager.Instance:Register(RemindName.LingQi_ZiZhi, BindTool.Bind(self.CalcZiZhiRemind, self))
	RemindManager.Instance:Register(RemindName.LingQi_HuanHua, BindTool.Bind(self.CalcHuanHuaRemind, self))
end

function LingQiData:__delete()
	RemindManager.Instance:UnRegister(RemindName.LingQi_UpGrade)
	RemindManager.Instance:UnRegister(RemindName.LingQi_ZiZhi)
	RemindManager.Instance:UnRegister(RemindName.LingQi_HuanHua)
...
function LingQiData:SetLingQiInfo(info)
	...
	self.special_img_grade_list = self.lingqi_info.special_img_grade_list
...
function LingQiData:GetHuanHuaIdIsUsed(image_id, is_special)
...
--获取对应幻化形象的红点
function LingQiData:GetHuanHuaRemindByImageId(image_id)
	--没达到对应开服天数, 或者等级没达到要求没有红点
	local open_server_day = TimeCtrl.Instance:GetCurOpenServerDay()
	local main_vo = GameVoManager.Instance:GetMainRoleVo()
	local image_info = self.lingqi_special_img_cfg[image_id]
	...
	local grade_list = self.lingqi_special_image_upgrade_cfg[image_id]
...
--计算升级材料是否足够
function LingQiData:CalcUpgradeRemind()
	if nil == self.lingqi_info then
		return 0
	end
	...
	local grade_info = self:GetLingQiGradeCfgInfoByGrade(self.lingqi_info.grade)
	if nil == grade_info then
		return 0
	end
...
function LingQiData:CalcHuanHuaRemind()
	...
	for k, v in ipairs(self.special_img_grade_list) do
		...
		img_info = self.lingqi_special_img_cfg[k]
...
--计算资质丹红点
function LingQiData:CalcZiZhiRemind()
	if nil == self.lingqi_info then
		return 0
	end

	--判断资质升级是否达到上限
	if self.lingqi_info.shuxingdan_count < self:GetMaxShuXingDanCount() then
		--是否拥有资质丹
	 	local zizhi_info = AppearanceData.Instance:GetZiZhiCfgInfoByType(ZIZHI_TYPE.LINGQI)
