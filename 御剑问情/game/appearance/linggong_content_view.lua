...
function LingGongContentView:__init()
	...
	--红点
	self.red_point_list = {
		[RemindName.LingGong_ZiZhi] = self:FindVariable("zizhi_remind"),
		[RemindName.LingGong_HuanHua] = self:FindVariable("huanhua_remind"),
	}
...
function LingGongContentView:RemindChangeCallBack(key, value)
...
function LingGongContentView:ClickUse()
	...
	LingGongCtrl.Instance:SendUseLingGongImage(grade_info.image_id, 0)
end
...
function LingGongContentView:ClickUpgrade()
	...
	if self.is_upgrade_state then
		LingGongCtrl.Instance:SendUpgradeLingGong(grade_info.pack_num, is_auto_buy)
	end
end

function LingGongContentView:ClickHelp()
end

function LingGongContentView:ClickZiZhi()
	ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "linggong_zizhi")
end
...
-- 显示全属性加成面板
function LingGongContentView:OnClickAllAttrBtn()
	self.all_attr_panel:SetActive(not self.all_attr_panel.gameObject.activeSelf)
end

function LingGongContentView:FlushAllAttrPanel()
	local all_attr_percent = LingGongData.Instance:GetAllAttrPercent()
	local active_need_grade = LingGongData.Instance:GetActiveNeedGrade()
	local cur_grade = LingGongData.Instance:GetGrade()
...
function LingGongContentView:InitView()
	self.is_init = true
	...
	if nil ~= linggong_info then
		...
		if linggong_info.grade == 1 then
			...
			local image_info = LingGongData.Instance:GetLingGongImageCfgInfoByImageId(linggong_info.used_imageid)
...
function LingGongContentView:FlushLeft()
	...
	if linggong_info.grade >= self.select_grade then
		...
		self.is_used:SetValue(temp_grade_info.image_id == linggong_info.used_imageid)
	else
...
function LingGongContentView:FlushRight()
	...
	local all_attr_percent_cap = LingGongData.Instance:CalculateAllAttrCap(capability)
	...
	if nil ~= next_grade_info then
		...
		pro_value = linggong_info.grade_bless_val / grade_info.bless_val_limit
		pro_str = string.format("%s / %s", linggong_info.grade_bless_val, grade_info.bless_val_limit)
	end
...
function LingGongContentView:UpGradeResult(result)
	...
	if self.toggle_check.isOn then
		LingGongCtrl.Instance:SendUpgradeLingGong(grade_info.pack_num, 1)
	else
		LingGongCtrl.Instance:SendUpgradeLingGong(grade_info.pack_num, 0)
	end
end
