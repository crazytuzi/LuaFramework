LingZhuContentView = LingZhuContentView or BaseClass(BaseRender)

function LingZhuContentView:__init()
	...
	--红点
	self.red_point_list = {
		[RemindName.LingZhu_ZiZhi] = self:FindVariable("zizhi_remind"),
		[RemindName.LingZhu_HuanHua] = self:FindVariable("huanhua_remind"),
	}
...
function LingZhuContentView:__delete()
	if self.model then
		self.model:DeleteMe()
		self.model = nil
	end
...
function LingZhuContentView:RemindChangeCallBack(key, value)
	if self.red_point_list[key] then
		self.red_point_list[key]:SetValue(value > 0)
	end
end

function LingZhuContentView:ClickUse()
	local grade_info = LingZhuData.Instance:GetLingZhuGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then
		return
	end

	LingZhuCtrl.Instance:SendUseLingZhuImage(grade_info.image_id, 0)
end

function LingZhuContentView:ClickLeft()
	self.select_grade = self.select_grade - 1
	self:FlushView()
end

function LingZhuContentView:ClickRight()
	self.select_grade = self.select_grade + 1
	self:FlushView()
end

function LingZhuContentView:ClickUpgrade()
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	if nil == lingzhu_info then
		return
	end
	...
	--获取下一级，不存在则满级
	local next_grade_info = LingZhuData.Instance:GetLingZhuGradeCfgInfoByGrade(lingzhu_info.grade + 1)
	...
	if self.is_upgrade_state then
		LingZhuCtrl.Instance:SendUpgradeLingZhu(grade_info.pack_num, is_auto_buy)
	end
end

function LingZhuContentView:ClickHelp()
end

function LingZhuContentView:ClickZiZhi()
	ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "lingzhu_zizhi")
end

function LingZhuContentView:ClickHuanHua()
	ViewManager.Instance:Open(ViewName.LingZhuHuanHua)
end

function LingZhuContentView:InitView()
	self.is_init = true
	...
	if nil ~= lingzhu_info then
		--默认选择已使用的形象阶数
		if lingzhu_info.grade == 1 then
			--零阶和一阶使用相同形象
			self.select_grade = lingzhu_info.grade
		else
			local image_info = LingZhuData.Instance:GetLingZhuImageCfgInfoByImageId(lingzhu_info.used_imageid)
...
-- 显示全属性加成面板
function LingZhuContentView:OnClickAllAttrBtn()
	self.all_attr_panel:SetActive(not self.all_attr_panel.gameObject.activeSelf)
end

function LingZhuContentView:FlushAllAttrPanel()
	local all_attr_percent = LingZhuData.Instance:GetAllAttrPercent()
	local active_need_grade = LingZhuData.Instance:GetActiveNeedGrade()
	local cur_grade = LingZhuData.Instance:GetGrade()
...
function LingZhuContentView:FlushView()
	self:FlushLeft()
	self:FlushRight()
end

function LingZhuContentView:FlushModel()
	--对应等级数据
	local grade_info = LingZhuData.Instance:GetLingZhuGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then
		return
...
function LingZhuContentView:FlushLeft()
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	if nil == lingzhu_info then
		return
	end
	...
	if self.old_grade < 0 then
		self.old_grade = lingzhu_info.grade
	end

	if self.old_grade ~= lingzhu_info.grade then
		--进阶成功
	...
	local temp_grade_info = LingZhuData.Instance:GetLingZhuGradeCfgInfoByGrade(self.select_grade)
	...
	--刷新使用状态
	if lingzhu_info.grade >= self.select_grade then
		self.can_use:SetValue(true)
		self.is_used:SetValue(temp_grade_info.image_id == lingzhu_info.used_imageid)
	else
		self.can_use:SetValue(false)
	end

	--刷新左右箭头
	local last_temp_grade_info = LingZhuData.Instance:GetLingZhuGradeCfgInfoByGrade(self.select_grade - 1)
	local is_show_left = true
	if nil == last_temp_grade_info or (lingzhu_info.grade > 1 and temp_grade_info.show_grade == 1) then
		--没有上一阶属性或者处于第一阶(服务器第二阶)不显示左箭头
		is_show_left = false
	end
	self.show_left:SetValue(is_show_left)

	local next_temp_grade_info = LingZhuData.Instance:GetLingZhuGradeCfgInfoByGrade(self.select_grade + 1)
	local is_show_right = true
	if nil == next_temp_grade_info or self.select_grade > lingzhu_info.grade then
		is_show_right = false
	end
	self.show_right:SetValue(is_show_right)
end

function LingZhuContentView:FlushRight()
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	if nil == lingzhu_info then
		return
	end
	...
	local all_attr_percent_cap = LingZhuData.Instance:CalculateAllAttrCap(capability) 
	...
	if nil ~= next_grade_info then
		...
		pro_value = lingzhu_info.grade_bless_val / grade_info.bless_val_limit
		pro_str = string.format("%s / %s", lingzhu_info.grade_bless_val, grade_info.bless_val_limit)
	end
...
function LingZhuContentView:UpGradeResult(result)
	...
	if self.toggle_check.isOn then
		LingZhuCtrl.Instance:SendUpgradeLingZhu(grade_info.pack_num, 1)
	else
		LingZhuCtrl.Instance:SendUpgradeLingZhu(grade_info.pack_num, 0)
	end
end

function LingZhuContentView:FlushItem()
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	if nil == lingzhu_info then
		return
	end
...
function LingZhuContentView:PlayEffect()
...
function LingZhuContentView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "lingzhu" then
			self:FlushView()
		elseif k == "lingzhu_upgrade" then
			self:UpGradeResult(v[1])
		elseif k == "lingzhu_item_change" then
			self:FlushItem()
		end
	end
end