ArenaExchangeView = ArenaExchangeView or BaseClass(BaseRender)
ArenaExchangeView.CURRENT_PRICE_TYPE = 9
function ArenaExchangeView:__init(instance)
	self.contain_cell_list = {}
	self.is_default_select = true
	self.current_item_id = -1
	self.current_price_type = ArenaExchangeView.CURRENT_PRICE_TYPE
...
function ArenaExchangeView:__delete()
...
function ArenaExchangeView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num, useless_param, isLast)
	self:FlushAllFrame()
end

function ArenaExchangeView:OpenCallBack()
	self:OnFlushListView()
end

function ArenaExchangeView:InitListView()
...
function ArenaExchangeView:GetNumberOfCells()
...
function ArenaExchangeView:PlayerDataChangeCallback(attr_name, value, old_value)
	if self.current_price_type == EXCHANGE_PRICE_TYPE.RONGYAO and attr_name == "cross_honor" then
		self:FlushCoin()
	end
end

function ArenaExchangeView:GetCellList()
	return self.contain_cell_list
end

function ArenaExchangeView:RefreshCell(cell, cell_index)
	local contain_cell = self.contain_cell_list[cell]
	if contain_cell == nil then
		contain_cell = AreanExchangeContain.New(cell.gameObject, self)
...
function ArenaExchangeView:SetCurrentItemId(item_id)
	self.current_item_id = item_id
end

function ArenaExchangeView:GetCurrentItemId()
	return self.current_item_id
end

function ArenaExchangeView:SetBuyNum(buy_num)
	self.buy_num = buy_num
end

function ArenaExchangeView:OnFlushAllCell()
	for k,v in pairs(self.contain_cell_list) do
		v:OnFlushAllCell()
	end
end

function ArenaExchangeView:SetCurrentPriceType(price_type)
	self.current_price_type = price_type
end

function ArenaExchangeView:GetCurrentPriceType()
	return self.current_price_type
end

function ArenaExchangeView:OnFlushListView()
...
function ArenaExchangeView:FlushCoin()
...
function ArenaExchangeView:FlushAllFrame()
	for k,v in pairs(self.contain_cell_list) do
		v:FlushAllFrame()
	end
end

function ArenaExchangeView:SetIsOpen(is_open)
	self.is_open = is_open
end

function ArenaExchangeView:GetIsOpen()
	return self.is_open
end

------------------------------------------------------------------------
AreanExchangeContain = AreanExchangeContain  or BaseClass(BaseCell)

function AreanExchangeContain:__init()
	self.exchange_contain_list = {}
	for i = 1, 8 do
		self.exchange_contain_list[i] = {}
		self.exchange_contain_list[i] = ArenaExchangeItem.New(self:FindObj("item_" .. i))
	end
end

function AreanExchangeContain:__delete()
	for i=1,8 do
...
function AreanExchangeContain:GetFirstCell()
	return self.exchange_contain_list[1]
end

function AreanExchangeContain:InitItems(item_id_list)
	for i=1,8 do
...
function AreanExchangeContain:FlushItems(item_id_list,toggle_group)
	for i=1,8 do
...
function AreanExchangeContain:SetToggleGroup(toggle_group)
	for i=1,8 do
		self.exchange_contain_list[i]:SetToggleGroup(toggle_group)
	end
end

function AreanExchangeContain:FlushAllFrame()
	for i=1,8 do
		self.exchange_contain_list[i]:OnFlush()
	end
end
----------------------------------------------------------------------------
ArenaExchangeItem = ArenaExchangeItem or BaseClass(BaseCell)

function ArenaExchangeItem:__init()
...
function ArenaExchangeItem:__delete()
	if self.item_cell then
		self.item_cell:DeleteMe()
	end
end

function ArenaExchangeItem:SetItemId(item_id_list)
...
function ArenaExchangeItem:OnFlush()
	self.price_multile = 0
	self.cur_multile_price = 0
	self.multiple_time = 0
	self.is_max_multiple = false
	...
	local item_info = ExchangeData.Instance:GetExchangeCfg(self.item_id, ArenaExchangeView.CURRENT_PRICE_TYPE)
	if not item_info then return end
	local main_role_vo = GameVoManager.Instance:GetMainRoleVo()
	local text = ""
	local price_type = ArenaExchangeView.CURRENT_PRICE_TYPE
	...
	local res = ExchangeData.Instance:GetExchangeRes(ArenaExchangeView.CURRENT_PRICE_TYPE)
...
function ArenaExchangeItem:CloseCallBack()
...
function ArenaExchangeItem:InitData()
...
function ArenaExchangeItem:SetTime(time_tab)
...
function ArenaExchangeItem:SelectToggle()
	self.root_node.toggle.isOn = true
end

function ArenaExchangeItem:SetToggleGroup(toggle_group)
	self.root_node.toggle.group = toggle_group
	self.root_node.toggle.isOn = false
end

function ArenaExchangeItem:OnToggleClick(is_click)
	local item_cfg = ItemData.Instance:GetItemConfig(self.item_id)
	local item_info = ExchangeData.Instance:GetExchangeCfg(self.item_id, ArenaExchangeView.CURRENT_PRICE_TYPE)
	 if is_click then
		...
		if self.price_multile > 0 or self.is_max_multiple then
			...
			ExchangeCtrl.Instance:ShowExchangeView(self.item_id, ArenaExchangeView.CURRENT_PRICE_TYPE,
			EXCHANGE_CONVER_TYPE.DAO_JU, close_func, self.cur_multile_price, self.multiple_time, self.is_max_multiple, click_func)
		else
			TipsCtrl.Instance:ShowExchangeView(self.item_id, ArenaExchangeView.CURRENT_PRICE_TYPE, EXCHANGE_CONVER_TYPE.DAO_JU, close_func)
		end
	 end
end
