...
function RobertMgr:__init()
	...
	self.mount_speed = 1700								-- 机器人坐骑速度
	self.role_speed = 1500								-- 人的速度
...
function RobertMgr:__delete()
	...
	if self.loding_comple then
		GlobalEventSystem:UnBind(self.loding_comple)
		self.loding_comple = nil
	end
...
function RobertMgr:RecordRolePos()
	...
	if Scene.Instance:GetMainRole():IsJump() or Scene.Instance:GetMainRole().vo.move_mode == MOVE_MODE.MOVE_MODE_JUMP2 or
		Scene.Instance:GetMainRole().vo.move_mode == MOVE_MODE.MOVE_MODE_JUMP
		then
...
function RobertMgr:BornRobert(born_x, born_y, target_pos_list)
	...
	role_vo.appearance.wing_jinhua_grade = 0
	role_vo.appearance.xianjian_id = -1
	role_vo.appearance.wing_special_imageid = -1
	role_vo.appearance.footprint_effect_id = 0
	role_vo.appearance.fashion_wuqi = 0
	role_vo.appearance.fashion_body = 0
	role_vo.appearance.fashion_foot = 0
	role_vo.appearance.fashion_guanghuan = 0
	role_vo.footprint_effect_id = -1
	role_vo.use_fabao_id = -1

	role_vo.appearance.wing_select_id = 0				-- 羽翼选择形象ID
	role_vo.appearance.wing_special_id = 0 				-- 羽翼特殊形象
	role_vo.appearance.fabao_select_id = 0				-- 圣器形象
	role_vo.appearance.fabao_special_id = 0				-- 圣器特殊形象
	role_vo.appearance.footprint_select_id = 0			-- 足迹形象
	role_vo.appearance.footprint_special_id = 0			-- 足迹特殊形象
	role_vo.appearance.shizhuang_select_id = 0			-- 时装身体
	role_vo.appearance.shizhuang_special_id = 0			-- 时装特殊形象
	role_vo.appearance.shenbing_select_id = 0			-- 神兵选择ID
	role_vo.appearance.shenbing_special_id = 0			-- 神兵特殊形象
	role_vo.appearance.fazhen_select_id = 0				-- 法阵形象
	role_vo.appearance.fazhen_special_id = 0				-- 法阵特殊形象
	...
	self:TryToCreateSpriteShadow(robert)
...
--创建图片阴影
function RobertMgr:TryToCreateSpriteShadow(parent_obj)
...
function RobertMgr:CalcFollowRobertBornPos()
	...
	while loop < max_loop do
		...
		if t.scene_id == Scene.Instance:GetSceneId() and distance >= 40 * 40 then
...
function RobertMgr:CheckBornTaskRobert()
	if #self.robert_t_list >= self.max_robert_num or
		#self.role_history_pos_list < 2 or
