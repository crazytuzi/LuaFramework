...
function ChuiQiQiuRank:FlushTitle()
	...
	-- 刷新倒计时
	local activity_end_time = FestivalActivityData.Instance:GetActivityActTimeLeftById(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_PRINT_TREE)
    if self.count_down then
        CountDown.Instance:RemoveCountDown(self.count_down)
        self.count_down = nil
    end
...
function ChuiQiQiuRank:OnClickTitle()
	...
	if nil ~= reward_list then
		TipsCtrl.Instance:OpenItem(reward_list.planting_rank_reward[0])
	end
end
...
function ChuiQiQiuRankCell:SetIndex(cell_index)
