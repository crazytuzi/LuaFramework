FestivalChongZhiRankView = FestivalChongZhiRankView or BaseClass(BaseRender)

function FestivalChongZhiRankView:__init()
...
function FestivalChongZhiRankView:__delete()
...
function FestivalChongZhiRankView:OpenCallBack()
	RankCtrl.Instance:SendGetPersonRankListReq(PERSON_RANK_TYPE.PERSON_RANK_TYPE_RA_CHONGZHI_RANK_2)
	self:Flush()
end

function FestivalChongZhiRankView:GoShopping()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function FestivalChongZhiRankView:GetNumberOfCells()
	local num = #(FestivalActivityData.Instance:GetChongZhiRewardCfg()) or 0
	return num
end

function FestivalChongZhiRankView:RefreshCell(cell, cell_index)
	local rank_cell = self.cell_list[cell]
	if nil == rank_cell then
		rank_cell = FestivalChongZhiRankCell.New(cell.gameObject)
		self.cell_list[cell] = rank_cell
	end

	local index = cell_index + 1
	local item_id_group = FestivalActivityData.Instance:GetChongZhiRewardCfg()
	local data = item_id_group[index]
	rank_cell:SetIndex(index)
	rank_cell:SetData(data)
end

function FestivalChongZhiRankView:OnFlush()
	self:FlushTime()
	self.list_view.scroller:ReloadData(0)
	local cfg = FestivalActivityData.Instance:GetChongZhiRewardCfg() or {}
	local model_id = 0
	if cfg[1] then
	...
	local cost_money = FestivalActivityData.Instance:GetChongZhiRankInfo() or 0
	local rank = FestivalActivityData.Instance:GetChongZhiRank() or 0
...
function FestivalChongZhiRankView:FlushTime()
	local info = FestivalActivityData.Instance:GetActivityOpenListByActId(FESTIVAL_ACTIVITY_ID.RAND_ACTIVITY_TYPE_CHONGZHI_RANK)
...
function FestivalChongZhiRankView:PlayerDataChangeCallback(attr_name, value, old_value)
...
--------Cell-------
FestivalChongZhiRankCell = FestivalChongZhiRankCell or BaseClass(BaseCell)
function FestivalChongZhiRankCell:__init()
...
function FestivalChongZhiRankCell:__delete()
	for i = 1, 3 do
...
function FestivalChongZhiRankCell:OnFlush()
	if nil == self.data or nil == next(self.data) then
		return
	end

	local cost_money = FestivalActivityData.Instance:GetChongZhiRankInfo() or 0
	-- local money = cost_money .. "/" .. (self.data.limit_chongzhi or 0)
	local money = self.data.limit_chongzhi or 0
