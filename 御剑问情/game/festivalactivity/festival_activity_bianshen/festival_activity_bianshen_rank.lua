BianShenRank =  BianShenRank or BaseClass(BaseRender)

function BianShenRank:__init()
	self.rank_list = {}
...
function BianShenRank:__delete()
...
function BianShenRank:OpenCallBack()
	self:Flush()
end

function BianShenRank:CloseCallBack()
end

function BianShenRank:OnFlush()
	self.reward_list = FestivalActivityBianShenData.Instance:GetSpecialAppearanceRankCfg()
 	self.rank_list = FestivalActivityBianShenData.Instance:GetSpecialAppearanceRankList()
	self:FlushTitle()
	self:FlushBottom()
	self.rank_list_view.scroller:ReloadData(0)
end

function BianShenRank:GetNumberOfRankCells()
	return #self.rank_list
end

function BianShenRank:RefreshRankCell(cell, cell_index)
	cell_index = cell_index + 1
	local rank_cell = self.rank_item_list[cell]
	if rank_cell == nil then
		rank_cell = BianShenRankCell.New(cell.gameObject, self)
...
function BianShenRank:FlushTitle()
	...
	-- 刷新倒计时
	local activity_end_time = FestivalActivityData.Instance:GetActivityActTimeLeftById(ACTIVITY_TYPE.RAND_ACTIVITY_BIANSHENBANG)
    if self.count_down then
        CountDown.Instance:RemoveCountDown(self.count_down)
        self.count_down = nil
    end
...
function BianShenRank:FlushBottom()
	local cfg = ServerActivityData.Instance:GetCurrentRandActivityConfigOtherCfg()
	if nil == cfg then
		return
	end
	local part_reward_data = cfg.special_appearance_rank_join_reward
	self.part_reward:SetData(part_reward_data)
	local my_rank = FestivalActivityBianShenData.Instance:GetMySpecialAppearanceRank()
	if my_rank == -1 then
		self.my_rank:SetValue(Language.Rank.NoInRank)
	else
		self.my_rank:SetValue(my_rank)
	end
	self.my_time:SetValue(FestivalActivityBianShenData.Instance:GetSpecialAppearanceRoleChangeTimes())
end

function BianShenRank:OnClickTitle()
	if nil == next(self.reward_list) and nil == self.reward_list[1] then
		return
	end
...
function BianShenRank:OnClickHelp()
	TipsCtrl.Instance:ShowHelpTipView(280)
end

--------------------------BianShenRankCell---------------------------------
BianShenRankCell = BianShenRankCell or BaseClass(BaseCell)

function BianShenRankCell:__init()
...
function BianShenRankCell:__delete()
	for k,v in pairs(self.reward_item_list) do
		v:DeleteMe()
	end
end

function BianShenRankCell:SetIndex(cell_index)
	self.index = cell_index
end

function BianShenRankCell:SetData(data, reward_list)
	self.data = data
