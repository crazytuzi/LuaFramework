...
function JuHuaSuanView:__init()
	self.ui_config = {"uis/views/randomact/juhuasuan_prefab", "JuHuaSuanView"}
...
function JuHuaSuanView:FlushNextTime()
	...
	if time > 3600 * 24 then
		self.act_time:SetValue("<color='#001828'>" .. TimeUtil.FormatSecond(time, 6) .. "</color>")
	elseif time > 3600 then
		self.act_time:SetValue("<color='#001828'>" .. TimeUtil.FormatSecond(time, 1) .. "</color>")
	else
		self.act_time:SetValue("<color='#001828'>" .. TimeUtil.FormatSecond(time, 2) .. "</color>")
	end
end
...
function JuHuaSuanCell:__init()
	...
	self.has_buy = self:FindVariable("HasBuy")
	self.reward_list = {}
	for i = 1, 3 do
...
function JuHuaSuanCell:OnFlush()
	...
	self.has_buy:SetValue(not has_buy)
