...
function ChatPrivateView:FlushPrivateView()
	-- local curr_id = ChatData.Instance:GetCurrentId()
	-- local msg_list = {}
	-- if curr_id then
...
function LeftRoleCell:LoadCallBack(role_id, raw_image_obj, path)
	if self:IsNil() then
		return
	end
...
function LeftRoleCell:OnFlush()
	...
	if avatar_key == 0 then
		...
		if avatar_key ~= self.avatar_key then
			...
			AvatarManager.Instance:GetAvatar(self.data.role_id, false, BindTool.Bind(self.LoadCallBack, self, self.data.role_id, self.raw_image_obj))
		end
	end
...
function LeftRoleCell:ClickItem()
	...
	-- end
	-- self.red_point:SetValue(false)
	-- ChatData.Instance:SetCurrentId(self.data.role_id)
...
function LeftRoleCell:ClickDelete()
	...
	if cur_role_id == self.data.role_id then
		...
		ChatData.Instance:SetCurrentId(0)
	end
