...
function TempMountView:__init()
	self.ui_config = {"uis/views/tempmountview_prefab","TempMountView"}
end
...
function TempMountView:LoadCallBack()
	...
	self:ListenEvent("OnClickTakeOff",
		BindTool.Bind(self.OnClickTakeOff, self))
	self:ListenEvent("OnClickSelect",
		BindTool.Bind(self.OnClickSelect, self))
	self.name_list = {}
	for i =1, 2 do
		self:ListenEvent("OnClickUse" .. i,
			BindTool.Bind(self.OnClickUse, self, i))
