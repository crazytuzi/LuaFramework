...
function MainUIViewJoystick:CheckShowFightMount()
	local value = self.show_fight_mount:GetBoolean()
	if value then
		return
	end
	local main_role_vo = GameVoManager.Instance:GetMainRoleVo()
	local used_imageid = FightMountData.Instance:GetUsedImageId()
	local is_open = OpenFunData.Instance:CheckIsHide("fight_mount")
	self.show_fight_mount_flag = is_open or (nil ~= used_imageid and used_imageid > 0) or (nil ~= main_role_vo and nil ~= main_role_vo.fight_mount_appeid and main_role_vo.fight_mount_appeid > 0)
	self.show_fight_mount:SetValue(is_open or (nil ~= used_imageid and used_imageid > 0) or (nil ~= main_role_vo and nil ~= main_role_vo.fight_mount_appeid and main_role_vo.fight_mount_appeid > 0))
end

function MainUIViewJoystick:FlushFightMountState()
	local main_role_vo = GameVoManager.Instance:GetMainRoleVo()
	local fight_mount_appeid = main_role_vo.fight_mount_appeid

	self.fight_mount_state:SetValue(fight_mount_appeid <= 0)
end

function MainUIViewJoystick:ClickFightMount()
	local main_role_vo = GameVoManager.Instance:GetMainRoleVo()
...
function MainUIViewJoystick:OnFingerSwipeStart(gesture)
	if gesture.fingerIndex ~= self.joystick_finger_index and not self.is_drag then
		...
		self.can_drag_time = Status.NowTime + 0.01
	end
end
...
function MainUIViewJoystick:OnFingerSwipe(gesture)
	...
	if Status.NowTime >= self.can_drag_time and CAMERA_TYPE == CameraType.Free and not IsNil(MainCameraFollow) then
		...
		MainCameraFollow:Swipe(x, gesture.swipeVector.y / 2.5)
		self:CancelQuest()
