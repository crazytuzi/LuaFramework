...
local NORMAL_CELL_SIZE = 100	-- 正常塔格子的大小
...
function RuneTowerView:__init()
	...
	self.unlock_text = self:FindVariable("UnLockText")
	...
	self.unlock_reward_state = self:FindVariable("UnLockRewardCount")
	self.unlock_item_list = {}
	for i = 1, 3 do
		self.unlock_item_list[i] = ItemCell.New()
		self.unlock_item_list[i]:SetInstanceParent(self:FindObj("UnLockItem"..i))
	end

	self.auto_btn = self:FindObj("AutoBtn")
	self.auto_btn_text = self:FindObj("AutoBtnText")
...
function RuneTowerView:JumpToIndex()
	if self.list_view and self.list_view.scroller.isActiveAndEnabled then
		...
		jump_index = GuaJiTaData.Instance:GetRuneMaxLayer() - jump_index - 2
		local scrollerOffset = 0
		local cellOffset = -1.4
...
function RuneTowerView:ChangeUnlockContent(pass_layer)
	...
	if nil ~= next_fun_open_info then
		self.extra_layer:SetValue(next_fun_open_info.fb_layer)

		local sp_show = next_fun_open_info.sp_show
		if RuneSlot == next_fun_open_info.sp_type then
			unlock_reward_state = 0
			-- unlock_text = string.format(Language.Rune.UnLockText2, RuneData.Instance:GetUnLockSlotCount(), GameEnum.RUNE_SYSTEM_SLOT_MAX_NUM)
	...
	self.unlock_text:SetValue(unlock_text)
	self.unlock_reward_state:SetValue(unlock_reward_state)
end
...
function RuneTowerView:FlushView()
	...
	self.auto_btn_text.grayscale.GrayScale = rune_info.pass_layer == rune_info.fb_today_layer and 255 or 0
end
...
function RuneTowerListView:SetData(data, data_index)
	...
	if data.fb_layer then
		...
		local str_fight_power = temp_data.capability
		if capability < temp_data.capability then
			str_fight_power = string.format(Language.Rune.ShowRedStr, temp_data.capability)
		end
