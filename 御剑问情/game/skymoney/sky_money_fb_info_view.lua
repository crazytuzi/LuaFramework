...
function SkyMoneyFBInfoView:FightSmallQianDuoDuo()
	...
	if x and y then
		...
		SysMsgCtrl.Instance:ErrorRemind(Language.SkyMoney.WaitForFlush)
	end
end
...
function SkyMoneyFBInfoView:OnClickBig()
	...
	if x and y then
		...
		SysMsgCtrl.Instance:ErrorRemind(Language.SkyMoney.WaitForFlushBoss)
	end
end
...
function SkyMoneyFBInfoView:SetInfo()
	...
	if task_reward_cfg.complete_task_num and task_reward_cfg.complete_task_num > sky_money_info.has_finish_task_num then
		self.cur_achieve_task_num:SetValue(string.format("<color=#00FF90>%s</color>", sky_money_info.has_finish_task_num))
	else
	...
	for k, v in pairs(self.task_item_list) do
		...
		if cur_task_cfg.reward and cur_task_cfg.reward[k - 1] and k ~= #self.task_item_list then
	...
	self.cur_get_gold:SetValue(sky_money_info.get_total_shengwang)
