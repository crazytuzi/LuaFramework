...
function AdvanceHuashenProtectView:ClearTimerQuest()
	...
	if is_show then
		if self.model == nil then
			...
			self.delay_timer_quest = GlobalTimerQuest:AddDelayTimer(function()
				-- print_error("self.root_node.transform", self.root_node.transform:Find("HuaShen/HuaShenModle/Display/Display/New Game Object/BallModles(Clone)"))
			...
			-- if part then
				...
				-- print_error("ball_jin", self.model.draw_obj)
			-- end
		end
	end
end
