...
MountImageFulingType = {
	Type = 0
}

local DISPLAYNAME = {
	[7005001] = "mount_panel_special_1",
}
...
function AdvanceMountView:__init(instance)
	...
	local is_get_reward = CompetitionActivityData.Instance:IsGetReward(TabIndex.mount_jinjie)
	local vis = CompetitionActivityData.Instance:GetBiPinRewardTips(TabIndex.mount_jinjie)
	...
	self.model_view = RoleModel.New("mount_panel")
...
function AdvanceMountView:OnStartAdvance()
	...
	if nil == mount_info.grade or mount_info.grade == 0 then
		return
	end
	--判断坐骑比拼是否开启
	...
	if mount_grade_cfg == nil or mount_info.grade >= MountData.Instance:GetMaxGrade() then
		return
	end
	...
	MountCtrl.Instance:SendUpGradeReq(is_auto_buy, self.is_auto, math.floor(num / pack_num))
	self.jinjie_next_time = Status.NowTime + (next_time or 0.1)
end

function AdvanceMountView:CallOnStartAdvance()
	local mount_info = MountData.Instance:GetMountInfo()
	if nil == mount_info.grade or mount_info.grade == 0 then
		return
	end
	...
	local stuff_item_id = mount_grade_cfg.upgrade_stuff_id
	local pack_num = mount_grade_cfg.upgrade_stuff_count
	local num = ItemData.Instance:GetItemNumInBagById(stuff_item_id) + ItemData.Instance:GetItemNumInBagById(mount_grade_cfg.upgrade_stuff2_id)
	if num < pack_num and not is_auto_buy_toggle then
		self.is_auto = false
		self.is_can_auto = true
		self:SetAutoButtonGray()
...
function AdvanceMountView:OnClickSendMsg()
	local mount_info = MountData.Instance:GetMountInfo()
	if not mount_info or not next(mount_info) then return end
	...
	if mount_grade > 1000 then
		local image_list = MountData.Instance:GetSpecialImageCfg(mount_grade - 1000)
		...
		local image_list = MountData.Instance:GetMountImageCfg()[mount_info.used_imageid]
		if nil ~= image_list and nil ~= image_list.image_name then
			name = image_list.image_name
			local temp_grade = MountData.Instance:GetMountGradeByUseImageId(mount_info.used_imageid)
...
-- 形象赋灵
function AdvanceMountView:OnClickFuLing()
	ViewManager.Instance:Open(ViewName.ImageFuLing, TabIndex.img_fuling_content, "fuling_type_tab", {IMG_FULING_JINGJIE_TYPE.IMG_FULING_JINGJIE_TYPE_MOUNT})
end

function AdvanceMountView:OnClickBiPingReward()
...
-- 显示全属性加成面板
function AdvanceMountView:OnClickAllAttrBtn()
	if self.delay_timer then
		GlobalTimerQuest:CancelQuest(self.delay_timer)
		self.delay_timer = nil
	end
...
function AdvanceMountView:SetAllAttrPanel()
	local all_attr_percent = MountData.Instance:GetAllAttrPercent()
	local active_need_grade = MountData.Instance:GetActiveNeedGrade()
	local cur_grade = MountData.Instance:GetGrade()
	local jinjie_attr_percent, jinjie_name = JinJieRewardData.Instance:GetSystemShowPercentAndName(JINJIE_TYPE.JINJIE_TYPE_MOUNT)
...
function AdvanceMountView:FlushAllAttrPanel()
	local active_need_grade = MountData.Instance:GetActiveNeedGrade()
	local cur_grade = MountData.Instance:GetGrade()
...
function AdvanceMountView:FlushSkillIcon()
	...
	for k, v in pairs(self.mount_skill_list) do
		if v.icon.grayscale then
			v.icon.grayscale.GrayScale = mount_skill_list[k - 1] > 0 and 0 or 255
		end
	end
	for k,v in pairs(self.skill_mask_list) do
		v.skill_mask:SetValue(mount_skill_list[k-1]>0)
	end
end
...
function AdvanceMountView:SetMountAtrr()
	...
	if mount_info.mount_level == 0 or mount_info.grade == 0 then
		...
		self:SetAutoButtonGray()
		self.remainder_num:SetValue(ItemData.Instance:GetItemNumInBagById(mount_grade_cfg.upgrade_stuff_id) + ItemData.Instance:GetItemNumInBagById(mount_grade_cfg.upgrade_stuff2_id))
	...
	local attr = CommonDataManager.GetAttributteByClass(mount_grade_cfg)--MountData.Instance:GetMountAttrSum()
	self.old_attrs = attr
	local capability = CommonDataManager.GetCapabilityCalculation(attr)
	local all_attr_percent_cap = MountData.Instance:CalculateAllAttrCap(capability)
	...
	local bag_num = ItemData.Instance:GetItemNumInBagById(stuff_item_id) + ItemData.Instance:GetItemNumInBagById(mount_grade_cfg.upgrade_stuff2_id)
	...
	self.show_huanhua_redpoint:SetValue(MountData.Instance:IsCanHuanhuaUpgrade() ~= false)
	...
	self.show_equip_remind:SetValue(MountData.Instance:CalAllEquipRemind() > 0)
	self.show_fuling_remind:SetValue(AdvanceData.Instance:CalFulingRemind(MountImageFulingType.Type))
end
...
function AdvanceMountView:SetUseImageButtonState(cur_select_grade)
	...
	if multi_mount_flag == 0 then
		...
		self.show_use_button:SetValue(true)
...
function AdvanceMountView:ItemDataChangeCallback(item_id, index, reason, put_reason, old_num, new_num)
	...
	self.have_pro_num:SetValue(bag_num)
end

-- 切换坐骑阶数、名字、模型
function AdvanceMountView:SwitchGradeAndName(cur_select_grade)
	if cur_select_grade == nil then return end
	local mount_grade_cfg = MountData.Instance:GetMountGradeCfg(cur_select_grade)
	local image_cfg = MountData.Instance:GetMountImageCfg()
	...
	if image_cfg[mount_grade_cfg.image_id] and self.res_id ~= image_cfg[mount_grade_cfg.image_id].res_id then
		...
		local call_back = function(model, obj)
			local cfg = model:GetModelDisplayParameterCfg(DISPLAY_MODEL_TYPE[DISPLAY_TYPE.MOUNT], image_cfg[mount_grade_cfg.image_id].res_id)
			if obj then
				if cfg then
		...
		-- 	self.mount_model:SetMainAsset(ResPath.GetMountModel(image_cfg[mount_grade_cfg.image_id].res_id))
		-- end
		 self:OnFlush()
	end
end
...
function AdvanceMountView:OpenCallBack()
	...
	local info = MountData.Instance:GetMountInfo() or {}
...
function AdvanceMountView:ShowTextTip()
	local info = MountData.Instance:GetMountInfo() or {}
...
function AdvanceMountView:OnFlush(param_list)
	...
	local vis = CompetitionActivityData.Instance:GetBiPinRewardTips(TabIndex.mount_jinjie)
	self.bipin_reward:SetValue(vis and not is_get_reward)
	local bipin_redpoint = CompetitionActivityData.Instance:GetIsShowRedptByTabIndex(TabIndex.mount_jinjie)
	...
	if param_list == "mount" or type(param_list) == "table" then
...
--获取激活坐骑符数量
function AdvanceMountView:GetMountProNum()
	local info = MountData.Instance:GetMountInfo()
	if not info or not next(info) then return end
	local mount_grade_cfg = MountData.Instance:GetMountGradeCfg(info.grade)
	if not mount_grade_cfg then return end
	local pack_num = mount_grade_cfg.upgrade_stuff_count
	...
	if count < pack_num  then
		count = string.format(Language.Common.ShowRedNum, count)
	else
...
function AdvanceMountView:SetSpecialModle(modle_id)
	local display_name = "mount_panel"
