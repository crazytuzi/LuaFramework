...
function QingYuanContent:__init(instance)
	self.cur_type = -1
	self.cur_index = 0
	self.cur_page = 1
...
function QingYuanContent:CloseCallBack()
	self.top_rank_avatar_key = {}
end

function QingYuanContent:OnFlush(param_list)
	for k,v in pairs(param_list) do
		if k == "change_tab" then
			...
			if self.cur_type < 0 then
				...
				elseif TabIndex.rank_qingyuan == self.parent_tab_index then
					self.cur_type = PERSON_RANK_TYPE.PERSON_RANK_TYPE_DAY_QingYuan
...
function QingYuanContent:FlushCommon()
	self.rank_name:SetValue(Language.Rank.RankDes[self.cur_type])
...
function QingYuanContent:FlushTopRank()
	local rank_data_list = RankData.Instance:GetCoupleRankList(self.cur_type)
	if rank_data_list == nil then
		return
	end
	for i = 1, 6 do
		local index = math.floor((i + 1) / 2)
		if rank_data_list[index] == nil then
			self:FlushTopRankAvatar(0, self.rank_image[i], self.rank_def_image[i], self.top_rank_img_res[i], 0, 0)
			self.top_rank_name[i]:SetValue("")
		else
			if i % 2 == 0 then
				self.top_rank_name[i]:SetValue(rank_data_list[index] and rank_data_list[index].user_name_2 or "")
				self:FlushTopRankAvatar(rank_data_list[index].user_id_2, self.rank_image[i], self.rank_def_image[i], self.top_rank_img_res[i], 0, rank_data_list[index].prof_2)
			else
				self.top_rank_name[i]:SetValue(rank_data_list[index] and rank_data_list[index].user_name_1 or "")
				self:FlushTopRankAvatar(rank_data_list[index].user_id_1, self.rank_image[i], self.rank_def_image[i], self.top_rank_img_res[i], 1, rank_data_list[index].prof_1)
			end
		end


		-- if rank_data_list[index] then
		-- 	self:FlushTopRankAvatar(rank_data_list[index], self.rank_image[i], self.rank_def_image[i], self.top_rank_img_res[i])
		-- end
	end
	local _, img = RankData.Instance:GetRankTitleDes(self.cur_type)
	self.col_name_img:SetAsset("uis/views/rank/images_atlas",img)

	for i= 1, 3 do
		local rank_value = RankData.Instance:GetCoupleRankValue(self.cur_type, i)
...
function QingYuanContent:FlushTopRankAvatar(user_id, raw_img_obj, def_img_obj, def_img_assets, sex, prof)
	if nil == user_id then return end

	CommonDataManager.SetAvatar(user_id, raw_img_obj, def_img_obj, def_img_assets, sex, prof, true)
end

function QingYuanContent:FlushNormalRank()
	GlobalTimerQuest:AddDelayTimer(function()
		self.rank_list_view.scroller:ReloadData(0)
	end, 0)
end

function QingYuanContent:FlushMyRankInfo()
	...
	for i, v in ipairs(rank_data_list) do
		if v.user_id_1 == GameVoManager.Instance:GetMainRoleVo().role_id or v.user_id_2 == GameVoManager.Instance:GetMainRoleVo().role_id then
			my_rank_info = v
			my_rank = i
			break
		end
...
function QingYuanContent:GetCurRankTypeList()
	...
	elseif TabIndex.rank_qingyuan == self.parent_tab_index then
		rank_type_list = RankData.Instance:GetQingYuanRankTypeList()
	end
	return rank_type_list
end

function QingYuanContent:GetNumberOfCellsToggle()
	return #self:GetCurRankTypeList()
end

function QingYuanContent:RefreshCellToggle(cell, cell_index)
...
function QingYuanContent:SetCurIndex(index)
	self.cur_index = index
end

function QingYuanContent:GetCurIndex()
	return self.cur_index
end

function QingYuanContent:GetFirstPageRankList()
	local rank_data_list = RankData.Instance:GetCoupleRankList(self.cur_type)
	if rank_data_list == nil then
		return {}
	end
...
function QingYuanContent:RefreshCellFirstPage(cell, cell_index)
	...
	if the_cell == nil then
		the_cell = RankCellTwo.New(cell.gameObject, self)
...
function QingYuanContent:GetPageRankList(page)
	if page <= 1 then
		return {}
	end
...
function QingYuanContent:GetNumberOfCells()
	local rank_data_list = self:GetPageRankList(self.cur_page)
	return #rank_data_list
end

function QingYuanContent:RefreshCell(cell, cell_index)
...
function QingYuanContent:SetCurType(rank_type)
...
function QingYuanContent:GetCurType()
	return self.cur_type
end

function QingYuanContent:GerCurType()
...
function QingYuanContent:OnCheckRole(index)
	-- local flag_index = (index + 1) / 2
	-- local rank_data_list = RankData.Instance:GetCoupleRankList(self.cur_type)
	-- if rank_data_list == nil then
	-- 	return
	-- end
	-- local role_info = rank_data_list[flag_index]
	-- local sex_id = index % 2 == 1 and 1 or 2
	-- local my_id = GameVoManager.Instance:GetMainRoleVo().role_id
	-- if my_id == role_info["user_id_" .. sex_id] then
	-- 	TipsCtrl.Instance:ShowSystemMsg(Language.Common.CanNoyCheckSelf)
	-- 	return
	-- end
	-- if nil ~= role_info then
	-- 	CheckData.Instance:SetCurrentUserId(role_info["user_id_" .. sex_id])
	-- 	CheckCtrl.Instance:SendQueryRoleInfoReq(role_info["user_id_" .. sex_id])
	-- 	ViewManager.Instance:Open(ViewName.CheckEquip)
	-- end
end

function QingYuanContent:OnSendFlower(index)
	-- local rank_data_list = RankData.Instance:GetCoupleRankList(self.cur_type) or {}
...
function QingYuanContent:OpenNumKeyPad()
...
function QingYuanContent:SetCurRoleInfo(cur_rank_info)
	if self.cur_rank_info == nil then
		self.cur_rank_info = RankData.Instance:GetCoupleRankList(self.cur_type)[1]
	else
...
function QingYuanContent:GetCurRoleInfo()
	return self.cur_rank_info
end

--打开查看面板
function QingYuanContent:OnOpenCheckClick()
	ViewManager.Instance:Open(ViewName.CheckEquip)
	self:CancelTheQuest()
end

--查看角色有变化时
function QingYuanContent:RoleInfoChange(role_id)
...
--没人进排行榜
function QingYuanContent:CheckIsNoRank()
	if #RankData.Instance:GetCoupleRankList(self.cur_type) == 0 then
...
function QingYuanContent:SetModle()
	self:CancelTheQuest()
...
function QingYuanContent:SetMountModle()
	local role_info = CheckData.Instance:GetRoleInfo()
	local image_id = 0
	local res_id = nil
	if self.cur_type == PERSON_RANK_TYPE.PERSON_RANK_TYPE_MOUNT then
...
function QingYuanContent:SetGoddessModel(ignore_wing, ignore_weapon, display_type)
...
function QingYuanContent:SetSpiritModle()
...
function QingYuanContent:CancelTheQuest()
...
function QingYuanContent:SetAnim()
...
function QingYuanContent:PlayAnim(is_change_tab)
...
function QingYuanContent:CalToShowAnim(is_change_tab, is_shenyi)
	if self.time_quest then
		GlobalTimerQuest:CancelQuest(self.time_quest)
		self.time_quest = nil
	end
...
function QingYuanContent:Reload()
	if self.rank_list_view.scroller then
		self.rank_list_view.scroller:ReloadData(0)
	end
end

function QingYuanContent:SetZhanliText(show_zhanli_value)
	if show_zhanli_value ~= nil then
		self.all_power_text:SetValue(show_zhanli_value)
	end
end

function QingYuanContent:SetShowCheck(is_show)
