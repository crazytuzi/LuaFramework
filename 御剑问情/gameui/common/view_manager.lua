...
function ViewManager:OpenByCfg(cfg, data)
	...
	if t[3] ~= nil then
		...
		if view_name == ViewName.VipView then
			...
			VipData.Instance:SetOpenParam(tonumber(t[3]))
		end

		if view_name == ViewName.Player then
			local index = TabIndex[tab_index]
			if tonumber(tab_index)then
				index = tonumber(tab_index)
			end
			self:Open(view_name, index, param_t.to_ui_name, data)
			return
		end

		if view_name == ViewName.Boss then
			local index = TabIndex[tab_index]
			if tonumber(tab_index)then
				index = tonumber(tab_index)
			end
			BossData.Instance:SetBossLayer(tonumber(param_t.to_ui_name))
			self:Open(view_name, index, "index", {layer = param_t.to_ui_name, seq = param_t.to_ui_param})
			return
		end
	...
	self:Open(view_name, index, "all", data)
end
