...
function QuickLogin:InitModule()
	...
	table.insert(module_list, TipsEventNoticeManager.New())	-- 事件公告
...
function QuickLogin:ReqConnectLoginServer()
	...
	if nil == cfg then
		print(string.format("[QuickLogin] server %d is not exist", server_id))
		return
	end
...
function QuickLogin.GetRoleName(sex)
...
function QuickLogin.ReqCreateRole()
	...
	protocol.prof = prof
	protocol.sex = QuickLogin.GetSexByProf(prof)
	protocol.role_name = QuickLogin.GetRoleName(protocol.sex)
...
function QuickLogin:ReqConnectGameServer()
	...
	GlobalEventSystem:Bind(LoginEventType.GAME_SERVER_DISCONNECTED, function(reason)
...
function QuickLogin.GetSexByProf(prof)
