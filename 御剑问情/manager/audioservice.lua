...
function AudioService:__init()
	...
	self.last_asset_name = nil
	self.last_bundle_name = nil
end
...
function AudioService:OnLoadComplete(mixer)
	...
	self.audio_mixer:SetFloat("VoiceVolume", self.voice_volume)
end
...
function AudioService:SetVoiceVolume(volume)
	self.voice_volume = volume
	if self.audio_mixer ~= nil then
		self.audio_mixer:SetFloat("VoiceVolume", 80 * self.voice_volume - 80)
	end
end
...
function AudioService:PlayBgm(bundle, asset)
	if self.last_bundle_name == bundle and self.last_asset_name == asset then
		return
	end
	ScriptablePool.Instance:Load(AssetID(bundle, asset), function(audio_item)
		...
		self.last_bundle_name = bundle
		self.last_asset_name = asset
	end)
end
