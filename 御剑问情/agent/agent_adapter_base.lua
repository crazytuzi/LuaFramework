...
function AgentAdapterBase:Pay(product_id, amount, callback)
	...
	if GLOBAL_CONFIG.param_list.switch_list.gamewp then
		...
		local url = string.format(
			"http://cls.ug14.youyannet.com/%s/wp_type.php?channelId=%s&agentId=%s&pkg=%s&data=%s&device=%s",
			channelID,
			channelID,
			agentID,
...
function AgentAdapterBase:OnLogin(data)
	...
	local url = string.format("http://cls.ug14.youyannet.com/%s/login_verify.php?channelId=%s&agentId=%s&data=%s&device=%s",
		channel_id,
		channel_id,
		agent_id,
		mime.b64(data),
