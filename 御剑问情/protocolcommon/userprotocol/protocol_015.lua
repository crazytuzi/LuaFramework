...
function SCKnapsackInfoAck:Decode()
	...
	self.grid_extend_times_by_item = MsgAdapter.ReadInt()
...
SCKnapsackGridExtendAuto = SCKnapsackGridExtendAuto or BaseClass(BaseProtocolStruct)
function SCKnapsackGridExtendAuto:__init()
	self.msg_type = 1513
end

function SCKnapsackGridExtendAuto:Decode()
	self.online_time = MsgAdapter.ReadUInt()				--在线时间
	self.auto_extend_times = MsgAdapter.ReadInt()			--自动扩展格子数目
end

--请求获取在线时间信息
CSKnapsackGridExtendAuto = CSKnapsackGridExtendAuto or BaseClass(BaseProtocolStruct)
function CSKnapsackGridExtendAuto:__init()
	self.msg_type = 1514
end

function CSKnapsackGridExtendAuto:Encode()
