...
function SCBianShenView:Decode()
	...
	self.obj_id_des = MsgAdapter.ReadInt()
	self.model_size = MsgAdapter.ReadInt()
end
...
-- 人物是否隐身
SCInvisibleView = SCInvisibleView or BaseClass(BaseProtocolStruct)
function SCInvisibleView:__init()
	self.msg_type = 1323
	self.obj_id = 0
	self.is_invisible = 0
end

function SCInvisibleView:Decode()
	self.obj_id = MsgAdapter.ReadUShort()
	self.is_invisible = MsgAdapter.ReadShort()
end
