...
function SCFriendInfoAck:Decode()
	self.today_give_flower_times = MsgAdapter.ReadUShort()
	MsgAdapter.ReadShort()
	self.is_auto_dec_friend = MsgAdapter.ReadInt()
...
function SCAllCharmChange:Decode()
	...
	self.day_charm = MsgAdapter.ReadInt()
end
...
CSOfflineFriendAutoDecFlag = CSOfflineFriendAutoDecFlag or BaseClass(BaseProtocolStruct)
function CSOfflineFriendAutoDecFlag:__init()
	self.msg_type = 2114
end

function CSOfflineFriendAutoDecFlag:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteInt(self.is_auto_dec)
end
------黑名单-------------
...
function SCChangeBlacklist:Decode()
	...
	self.gamename = MsgAdapter.ReadStrN(32)
	self.changestate = MsgAdapter.ReadChar() --0 update 1 del
