...
function SCMultiMountAllInfo:__init()
	...
	self.cur_use_mount_id = 0
	self.cur_use_special_image_id = 0
	self.mount_list = {}
	self.special_img_active_flag = 0
...
function SCMultiMountAllInfo:Decode()
	...
	self.cur_use_special_image_id = MsgAdapter.ReadShort()					-- 当前使用的特殊形象id， 0不使用任何特殊形象

	self.mount_list = {}
	for i = 0, GameEnum.MULTIMOUNT_MAX_ID do
		local mount_item = {}
		mount_item.is_mount_active = MsgAdapter.ReadChar()
		mount_item.is_img_active = MsgAdapter.ReadChar()
		mount_item.reserve_sh = MsgAdapter.ReadShort()
		mount_item.grade_bless = MsgAdapter.ReadShort()
		mount_item.grade = MsgAdapter.ReadShort()
		self.mount_list[i] = mount_item
	end
	self.special_img_active_flag = {}
	local temp_flag = bit:d2b(MsgAdapter.ReadShort())						-- 特殊坐骑激活标志，0未激活，1激活
	for i=1,10 do
		self.special_img_active_flag[i] = temp_flag[32-i]
	end

	self.equip_level_list = {}
	for i = 0, GameEnum.MULTIMOUNT_EQUIP_TYPE_NUM do
...
function SCMultiMountNotifyArea:Decode()
	...
	self.parnter_objid = self.parnter_objid ~= 65535 and self.parnter_objid or COMMON_CONSTS.INVALID_OBJID
...
-- SCPersonalizeWindowBubbleSuitSingleInfo = SCPersonalizeWindowBubbleSuitSingleInfo or BaseClass(BaseProtocolStruct)
-- function SCPersonalizeWindowBubbleSuitSingleInfo:__init()
-- 	self.msg_type = 6127
-- end

-- function SCPersonalizeWindowBubbleSuitSingleInfo:Decode()
-- 	self.seq = MsgAdapter.ReadInt()
-- 	self.suit_active_flag = MsgAdapter.ReadUChar()
-- 	MsgAdapter.ReadChar()
-- 	MsgAdapter.ReadShort()
-- end

-- SCPersonalizeWindowBubbleRimInfo = SCPersonalizeWindowBubbleRimInfo or BaseClass(BaseProtocolStruct)
-- function SCPersonalizeWindowBubbleRimInfo:__init()
-- 	self.msg_type = 6128
-- end

-- function SCPersonalizeWindowBubbleRimInfo:Decode()
-- 	self.rim_suit_active_count = MsgAdapter.ReadInt()
-- 	self.rim_active_flag = MsgAdapter.ReadInt()
-- 	self.cur_use_seq = MsgAdapter.ReadShort()
-- 	MsgAdapter.ReadShort()
-- end
...
function SCMolongInfo:Decode()
	...
	self.info.rank_grade = MsgAdapter.ReadInt()
	self.info.rank_cumulate_gold = MsgAdapter.ReadInt()

end

CSMolongRankInfoReq = CSMolongRankInfoReq or BaseClass(BaseProtocolStruct)
function CSMolongRankInfoReq:__init()
	self.msg_type = 6143
end

function CSMolongRankInfoReq:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
end
...
function SCAvatarWindowInfo:Decode()
	...
	MsgAdapter.ReadShort()
	self.avatar_level = {}
	for i = 1,COMMON_CONSTS.AVATAR_WINDOW_MAX_TYPE do
