...
--妖兽祭坛副本信息
SCYsjtTeamFbSceneLogicInfo = SCYsjtTeamFbSceneLogicInfo or BaseClass(BaseProtocolStruct)

function SCYsjtTeamFbSceneLogicInfo:__init()
	self.msg_type = 5434
	...
	self.is_finish = 0
	self.is_pass = 0
	self.is_active_leave_fb = 0

	self.pass_wave = 0
	self.kill_boss_num = 0
	self.pass_time_s = 0
	self.mode = 0
	self.boss_attr_type = 0
	self.role_attrs = {}
end

function SCYsjtTeamFbSceneLogicInfo:Decode()
	...
	self.pass_wave = MsgAdapter.ReadInt()
	self.kill_boss_num = MsgAdapter.ReadInt()
	self.pass_time_s = MsgAdapter.ReadInt()
	self.mode = MsgAdapter.ReadInt()
	self.boss_attr_type = MsgAdapter.ReadInt()

	self.role_attrs = {}

	for i=1, 4  do
		self.role_attrs[i] = {}
		self.role_attrs[i].uid = MsgAdapter.ReadInt()
		self.role_attrs[i].attr = MsgAdapter.ReadInt()
	end
end
