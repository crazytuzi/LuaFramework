SevenRechargeContent = BaseClass(LuaUI)

SevenRechargeContent.CELL_NUM = 3
function SevenRechargeContent:__init(...)
	self.URL = "ui://wvu017cppzhgu"
	self:__property(...)
	self:Config()
	self:InitEvent()
end
function SevenRechargeContent:SetProperty(...)
	
end
function SevenRechargeContent:Config()
	self.model = RechargeModel:GetInstance()
	if not self.model:IsSevenReadToday() then
		self.model:SetSevenRead()
		GlobalDispatcher:DispatchEvent(EventName.SevenRechargeRedChange)
	end
end
function SevenRechargeContent:RegistUI(ui)
	self.ui = ui or self.ui or UIPackage.CreateObject("Recharge", "SevenRechargeContent")
	...
	self.textDesc = self.ui:GetChild("textDesc")
	self.cells = {}
	for i = 1, SevenRechargeContent.CELL_NUM do
		self.cells[i] = self.ui:GetChild("cell" .. i)
	end
end

function SevenRechargeContent.Create(ui, ...)
	return SevenRechargeContent.New(ui, "#", {...})
end

function SevenRechargeContent:__delete()
	self:RemoveEvent()
	self.model = nil
end

function SevenRechargeContent:RefreshUI()
	self.model = RechargeModel:GetInstance()
...
function SevenRechargeContent:RefreshTimeArea(cfg, state)
...
function SevenRechargeContent:RefreshRechargeNum()
	self.rechardNumDaily.text = StringFormat("{0}元", self.model:GetSevenRechargeNum())
end

function SevenRechargeContent:RefreshCells(cfg)
	for i = 1, SevenRechargeContent.CELL_NUM do
		self:RefreshOneCell(self.cells[i], i, cfg)
	end
end

function SevenRechargeContent:RefreshOneCell(cell, idx, cfg)
...
function SevenRechargeContent:RefreshComReward(com, idx, rewardList)
...
function SevenRechargeContent:RefreshOneCellCom(item, idx, vo)
	...
	if (not imgReward) or (not txtRewardName) or (not txtRewardNum) then
		return
	end
	imgReward.url = GoodsVo.GetIconUrl(vo.goodsType, vo.bid)
...
function SevenRechargeContent:InitEvent()
...
function SevenRechargeContent:RemoveEvent()
	if self.model then
		self.model:RemoveEventListener(self._hGetSevenData)
	end
end

function SevenRechargeContent:OnRechardDailyClick()
