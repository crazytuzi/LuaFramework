DzgKPAndMgr = class("DzgKPAndMgr", ChannelClassBase)
DzgKPAndMgr.cls_and = "com/dzgkp/channel/DzgKPInter"
DzgKPAndMgr.appID = "ex_xycq_bXwEMhv"
...
function DzgKPAndMgr:ctor()
  print(" 正在初始化      DzgKPAndMgr       ")
  self.m_LoginInfo = {}
  self.m_Listener = nil
  ...
  self.m_UserInfo = nil
  callStaticMethodJava(DzgKPAndMgr.cls_and, "setMessageListener", {
    handler(self, self.MessageCallBack)
  })
end
function DzgKPAndMgr:_callback(code, param)
...
function DzgKPAndMgr:MessageCallBack(data, isSucceed)
  if data == nil then
...
-------------------------------------
 DzgKPAndMgr:MessageCallBack:%s,%s
...
function DzgKPAndMgr:CallAfterPayDelay(cbkey)
  print("================>>>>>    DzgKPAndMgr:CallAfterPayDelay  ")
  g_DataMgr.isPaying = false
...
function DzgKPAndMgr:Init(gameParam, listener)
  print("   ********* getDeviceName = ", "--|" .. SyNative.getDeviceName() .. "|--")
  print("DzgKPAndMgr:Init:", listener)
  self.m_Listener = listener
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "InitSDK", {
    DzgKPAndMgr.appID
...
function DzgKPAndMgr:isLogined()
  local result, token, userType, userName = self:getCacheAuthInfo()
  print("DzgKPAndMgr:isLogined   ========>>> ", result, token, userType, userName)
...
function DzgKPAndMgr:momoPushInit()
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "momoPushInit", {}, "()Ljava/lang/String;")
  if ok ~= true and ret == "1" then
    return true
  end
  return false
end
function DzgKPAndMgr:getCacheAuthInfo()
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "getAuthInfo", {}, "()Ljava/lang/String;")
  if ok and ret ~= nil and ret.token ~= nil then
    return true, ret.token, ret.userType, ret.userName
  end
  return false
end
function DzgKPAndMgr:Login()
  self.m_IsLoginSucceedCallback = false
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "Login", {}, "()Ljava/lang/String;")
  if ok ~= true then
    self:_callback(ChannelCallbackStatus.kLoginFail)
  end
end
function DzgKPAndMgr:LoginFinished_(isCloseLoginView)
...
function DzgKPAndMgr:getAccount()
  return self.m_LoginInfo.userId
end
function DzgKPAndMgr:getUserInfo()
  if self.m_LoginInfo then
    return self.m_UserInfo
  end
end
function DzgKPAndMgr:setGameServer(serverParam)
  if serverParam == nil then
    serverParam = {}
  end
  local mserverId = serverParam.serverId or "0"
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "setGameServer", {mserverId}, "(Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function DzgKPAndMgr:Logout()
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "Logout", {}, "()Ljava/lang/String;")
  if ok ~= true then
    self:_callback(ChannelCallbackStatus.kLogoutFail)
  end
end
function DzgKPAndMgr:sendLoginProtocol(gameType, deveceType)
  print("---->> DzgKPAndMgr:sendLoginProtocol")
...
function DzgKPAndMgr:showToolBar(place)
  do return end
  if place == nil or type(place) ~= "number" or place >= 4 or place < 0 then
    place = 0
  end
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "setShowMomoLogo", {1, place}, "(II)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function DzgKPAndMgr:hideToolBar()
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "setShowMomoLogo", {0, 1}, "(II)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function DzgKPAndMgr:enterPersonCenter()
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "showPersonalCenter", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function DzgKPAndMgr:startPay(payParam)
  ...
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "startPay", sendParam, "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  self:_callback(ChannelPayResult.kPayFailed)
...
function DzgKPAndMgr:getDid()
  return 2
end
function DzgKPAndMgr:Clean()
  g_DataMgr.isPaying = false
  self:__printInterNotImplement("Clean")
end
function DzgKPAndMgr:requestExitGame(listener)
  self.m_ExitGameListener = listener
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "requestExitGame", {}, "()V")
  if ok == true then
    return true
  end
  return false
end
function DzgKPAndMgr:callExitGame_(result)
...
function DzgKPAndMgr:showFAQView()
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "questionCommint", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function DzgKPAndMgr:enterForumOrTieba()
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "launchToTieba", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function DzgKPAndMgr:getFriendList(listener)
  self.m_ReqFriendListListener = listener
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "getFriendList", {0}, "(I)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
...
function DzgKPAndMgr:CallbackReqFriendListListener(...)
...
function DzgKPAndMgr:addFriend(userId, listener, extParam)
  listener = self.m_ReqAddFriendListener
  local reson = extParam.reason or "邀请你一起玩转《缘定星辰》！"
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "addFriend", {userId, reson}, "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
...
function DzgKPAndMgr:CallbackReqAddFriendListener(...)
  listener = self.m_ReqAddFriendListener
  self.m_ReqAddFriendListener = nil
  if listener then
    listener(...)
...
function DzgKPAndMgr:shareToUser(userId, listener, contend, extParam)
  self.m_ReqShareToUserListener = listener
  local shareType = extParam.shareType
  if shareType == nil then
    shareType = 1
  end
  local ok, ret = callStaticMethodJava(DzgKPAndMgr.cls_and, "shareToUser", {
    userId,
    contend,
    shareType
...
function DzgKPAndMgr:CallbackReqShareToUserListener(...)
