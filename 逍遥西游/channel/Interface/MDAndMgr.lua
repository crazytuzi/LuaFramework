MDAndMgr = class("MDAndMgr", ChannelClassBase)
MDAndMgr.cls_and = "com/md/channel/MDInter"
MDAndMgr.appID = "ex_xycq_bXwEMhv"
...
function MDAndMgr:ctor()
  print(" 正在初始化      MDAndMgr       ")
  self.m_LoginInfo = {}
  self.m_Listener = nil
  ...
  self.m_UserInfo = nil
  callStaticMethodJava(MDAndMgr.cls_and, "setMessageListener", {
    handler(self, self.MessageCallBack)
  })
end
function MDAndMgr:_callback(code, param)
...
function MDAndMgr:MessageCallBack(data, isSucceed)
  if data == nil then
...
function MDAndMgr:CallAfterPayDelay(cbkey)
...
function MDAndMgr:Init(gameParam, listener)
  print("   ********* getDeviceName = ", "--|" .. SyNative.getDeviceName() .. "|--")
  print("MDAndMgr:Init:", listener)
  self.m_Listener = listener
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "InitSDK", {
    MDAndMgr.appID
...
function MDAndMgr:isLogined()
  local result, token, userType, userName = self:getCacheAuthInfo()
  print("MDAndMgr:isLogined   ========>>> ", result, token, userType, userName)
...
function MDAndMgr:momoPushInit()
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "momoPushInit", {}, "()Ljava/lang/String;")
  if ok ~= true and ret == "1" then
    return true
  end
  return false
end
function MDAndMgr:getCacheAuthInfo()
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "getAuthInfo", {}, "()Ljava/lang/String;")
  if ok and ret ~= nil and ret.token ~= nil then
    return true, ret.token, ret.userType, ret.userName
  end
  return false
end
function MDAndMgr:Login()
  self.m_IsLoginSucceedCallback = false
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "Login", {}, "()Ljava/lang/String;")
  if ok ~= true then
    self:_callback(ChannelCallbackStatus.kLoginFail)
  end
end
function MDAndMgr:LoginFinished_(isCloseLoginView)
...
function MDAndMgr:getAccount()
  return self.m_LoginInfo.userId
end
function MDAndMgr:getUserInfo()
  if self.m_LoginInfo then
    return self.m_UserInfo
  end
end
function MDAndMgr:setGameServer(serverParam)
  if serverParam == nil then
    serverParam = {}
  end
  local mserverId = serverParam.serverId or "0"
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "setGameServer", {mserverId}, "(Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MDAndMgr:Logout()
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "Logout", {}, "()Ljava/lang/String;")
  if ok ~= true then
    self:_callback(ChannelCallbackStatus.kLogoutFail)
  end
end
function MDAndMgr:sendLoginProtocol(gameType, deveceType)
  print("---->> MDAndMgr:sendLoginProtocol")
  ...
  NetSend({
    s_gf = gameType,
    s_userid = self.m_LoginInfo.userName,
    s_vtoken = vToken,
...
function MDAndMgr:showToolBar(place)
  do return end
  if place == nil or type(place) ~= "number" or place >= 4 or place < 0 then
    place = 0
  end
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "setShowMomoLogo", {1, place}, "(II)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MDAndMgr:hideToolBar()
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "setShowMomoLogo", {0, 1}, "(II)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MDAndMgr:enterPersonCenter()
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "showPersonalCenter", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MDAndMgr:startPay(payParam)
  ...
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "startPay", sendParam, "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  self:_callback(ChannelPayResult.kPayFailed)
...
function MDAndMgr:getDid()
  return 2
end
function MDAndMgr:Clean()
  g_DataMgr.isPaying = false
  self:__printInterNotImplement("Clean")
end
function MDAndMgr:requestExitGame(listener)
  self.m_ExitGameListener = listener
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "requestExitGame", {}, "()V")
  if ok == true then
    return true
  end
  return false
end
function MDAndMgr:callExitGame_(result)
...
function MDAndMgr:showFAQView()
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "questionCommint", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MDAndMgr:enterForumOrTieba()
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "launchToTieba", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MDAndMgr:getFriendList(listener)
  self.m_ReqFriendListListener = listener
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "getFriendList", {0}, "(I)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
...
function MDAndMgr:CallbackReqFriendListListener(...)
...
function MDAndMgr:addFriend(userId, listener, extParam)
  listener = self.m_ReqAddFriendListener
  local reson = extParam.reason or "邀请你一起玩转《缘定星辰》！"
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "addFriend", {userId, reson}, "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
...
function MDAndMgr:CallbackReqAddFriendListener(...)
  listener = self.m_ReqAddFriendListener
  self.m_ReqAddFriendListener = nil
  if listener then
    listener(...)
...
function MDAndMgr:shareToUser(userId, listener, contend, extParam)
  self.m_ReqShareToUserListener = listener
  local shareType = extParam.shareType
  if shareType == nil then
    shareType = 1
  end
  local ok, ret = callStaticMethodJava(MDAndMgr.cls_and, "shareToUser", {
    userId,
    contend,
    shareType
...
function MDAndMgr:CallbackReqShareToUserListener(...)
