MiyuAndMgr = class("MiyuAndMgr", ChannelClassBase)
MiyuAndMgr.cls_and = "com/miyuv/channel/DashiInter"
MiyuAndMgr.appID = "ex_xycq_bXwEMhv"
...
function MiyuAndMgr:ctor()
  print(" 正在初始化      MiyuAndMgr       ")
  self.m_LoginInfo = {}
  self.m_Listener = nil
  ...
  self.m_UserInfo = nil
  callStaticMethodJava(MiyuAndMgr.cls_and, "setMessageListener", {
    handler(self, self.MessageCallBack)
  })
end
function MiyuAndMgr:_callback(code, param)
...
function MiyuAndMgr:MessageCallBack(data, isSucceed)
  if data == nil then
...
-------------------------------------
 MiyuAndMgr:MessageCallBack:%s,%s
...
function MiyuAndMgr:CallAfterPayDelay(cbkey)
  print("================>>>>>    MiyuAndMgr:CallAfterPayDelay  ")
  g_DataMgr.isPaying = false
...
function MiyuAndMgr:Init(gameParam, listener)
  print("   ********* getDeviceName = ", "--|" .. SyNative.getDeviceName() .. "|--")
  print("MiyuAndMgr:Init:", listener)
  self.m_Listener = listener
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "InitSDK", {
    MiyuAndMgr.appID
...
function MiyuAndMgr:isLogined()
  local result, token, userType, userName = self:getCacheAuthInfo()
  print("MiyuAndMgr:isLogined   ========>>> ", result, token, userType, userName)
...
function MiyuAndMgr:momoPushInit()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "momoPushInit", {}, "()Ljava/lang/String;")
  if ok ~= true and ret == "1" then
    return true
  end
  return false
end
function MiyuAndMgr:getCacheAuthInfo()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "getAuthInfo", {}, "()Ljava/lang/String;")
  if ok and ret ~= nil and ret.token ~= nil then
    return true, ret.token, ret.userType, ret.userName
  end
  return false
end
function MiyuAndMgr:Login()
  self.m_IsLoginSucceedCallback = false
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "Login", {}, "()Ljava/lang/String;")
  if ok ~= true then
    self:_callback(ChannelCallbackStatus.kLoginFail)
  end
end
function MiyuAndMgr:LoginFinished_(isCloseLoginView)
...
function MiyuAndMgr:getAccount()
  return self.m_LoginInfo.userid
end
function MiyuAndMgr:getUserInfo()
  if self.m_LoginInfo then
    return self.m_UserInfo
  end
end
function MiyuAndMgr:setGameServer(serverParam)
  if serverParam == nil then
    serverParam = {}
  end
  local mserverId = serverParam.serverId or "0"
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "setGameServer", {mserverId}, "(Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MiyuAndMgr:Logout()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "Logout", {}, "()Ljava/lang/String;")
  if ok ~= true then
    self:_callback(ChannelCallbackStatus.kLogoutFail)
  end
end
function MiyuAndMgr:sendLoginProtocol(gameType, deveceType)
  print("---->> MiyuAndMgr:sendLoginProtocol")
...
function MiyuAndMgr:showToolBar(place)
  do return end
  if place == nil or type(place) ~= "number" or place >= 4 or place < 0 then
    place = 0
  end
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "setShowMomoLogo", {1, place}, "(II)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MiyuAndMgr:hideToolBar()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "setShowMomoLogo", {0, 1}, "(II)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MiyuAndMgr:enterPersonCenter()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "showPersonalCenter", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MiyuAndMgr:startPay(payParam)
  ...
  local sendParam = {
    ...
    payParam.customInfo,
    payParam.dataId,
    tostring(payParam.vipLv)
  }
  dump(sendParam, "startpay sendParam")
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "startPay", sendParam, "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  self:_callback(ChannelPayResult.kPayFailed)
...
function MiyuAndMgr:getDid()
  return 2
end
function MiyuAndMgr:Clean()
  g_DataMgr.isPaying = false
  self:__printInterNotImplement("Clean")
end
function MiyuAndMgr:requestExitGame(listener)
  self.m_ExitGameListener = listener
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "requestExitGame", {}, "()V")
  if ok == true then
    return true
  end
  return false
end
function MiyuAndMgr:callExitGame_(result)
...
function MiyuAndMgr:showFAQView()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "questionCommint", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MiyuAndMgr:enterForumOrTieba()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "launchToTieba", {}, "()Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
  return false
end
function MiyuAndMgr:getFriendList(listener)
  self.m_ReqFriendListListener = listener
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "getFriendList", {0}, "(I)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
...
function MiyuAndMgr:CallbackReqFriendListListener(...)
...
function MiyuAndMgr:addFriend(userId, listener, extParam)
  listener = self.m_ReqAddFriendListener
  local reson = extParam.reason or "邀请你一起玩转《缘定星辰》！"
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "addFriend", {userId, reson}, "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;")
  if ok == true and ret == "1" then
    return true
  end
...
function MiyuAndMgr:CallbackReqAddFriendListener(...)
  listener = self.m_ReqAddFriendListener
  self.m_ReqAddFriendListener = nil
  if listener then
    listener(...)
...
function MiyuAndMgr:shareToUser(userId, listener, contend, extParam)
  self.m_ReqShareToUserListener = listener
  local shareType = extParam.shareType
  if shareType == nil then
    shareType = 1
  end
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "shareToUser", {
    userId,
    contend,
    shareType
...
function MiyuAndMgr:CallbackReqShareToUserListener(...)
...
function MiyuAndMgr:createRole(roleParam)
  ...
  local mserverId = getsid()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "createRole", {
    mserverId,
...
function MiyuAndMgr:enterGame(roleParam)
  if roleParam == nil then
    roleParam = {}
  end
  if roleParam.serverId == nil then
  ...
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "enterGame", {
    roleParam.serverId,
    roleParam.serverName,
    roleParam.roleId,
    roleParam.roleName,
...
function MiyuAndMgr:RoleLevelUp(roleParam)
  if roleParam == nil then
    roleParam = {}
  end
  if roleParam.serverId == nil then
  ...
  local getsid = string.gmatch(roleParam.serverId, "%d+")
  local mserverId = getsid()
  local ok, ret = callStaticMethodJava(MiyuAndMgr.cls_and, "levelChange", {
    roleParam.roleId,
    roleParam.roleName,
    roleParam.roleLv,
    mserverId,
