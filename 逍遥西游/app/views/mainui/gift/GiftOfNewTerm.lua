local CNewTermRewardItem = class("CNewTermRewardItem", function()
  local widget = Widget:create()
  widget:setAnchorPoint(ccp(0, 0))
  widget:ignoreContentAdaptWithSize(false)
  return widget
end)
function CNewTermRewardItem:ctor(day, clickFunc)
  self.m_Day = day
...
function CNewTermRewardItem:getRewardId()
  return self.m_RewardId
end
function CNewTermRewardItem:getDaynum()
  return self.m_Day
end
function CNewTermRewardItem:Init()
  local delY = 10
  ...
  local data = data_GiftOfNewTermCheckIn[self.m_RewardId]
  if data == nil then
    return false
  end
  local function func()
...
function CNewTermRewardItem:reflush()
  if gift.newTermCheckIn:getIsHasAccept(self.m_Day) then
  ...
  if gift.newTermCheckIn:getIsCanAccept(self.m_Day) then
...
function CNewTermRewardItem:onCleanup()
  self.m_canCheckinAni = nil
  if self.m_ClickFunc then
    self.m_ClickFunc = nil
  end
end
local CNewTermRewardConffirm = class("CNewTermRewardConffirm", CcsSubView)
function CNewTermRewardConffirm:ctor(rewardId, canAccept, mclickOutSideToClose)
  print("==>>CNewTermRewardConffirm:", rewardId)
  CNewTermRewardConffirm.super.ctor(self, "views/newterm_rewards.json", {
    isAutoCenter = true,
    opacityBg = 100,
  ...
  local data = data_GiftOfNewTermCheckIn[rewardId]
  if data == nil then
    return
  end
  local reward = data.reward
...
function CNewTermRewardConffirm:OnBtn_OK(btnObj, touchType)
  print("CNewTermRewardConffirm:OnBtn_OK")
  if self.m_CanAccept == true then
    netsend.netgift.reqGetCheckInForNewTerm()
  end
  self:CloseSelf()
end
CNewTermCheckInView = class("CNewTermCheckInView", CcsSubView)
function CNewTermCheckInView:ctor(closeFunc)
  CNewTermCheckInView.super.ctor(self, "views/gift_newtermcheckin.json", {isAutoCenter = true, opacityBg = 100})
  local btnBatchListener = {
    btn_close = {
  ...
  local dayNumList = {}
  for day, _ in pairs(data_GiftOfNewTermCheckIn) do
    dayNumList[#dayNumList + 1] = day
  end
  table.sort(dayNumList)
  for _, day in ipairs(dayNumList) do
    local item = CNewTermRewardItem.new(day, clickFunc)
    if item:Init() then
  ...
  self.m_DelayTimerHandler = scheduler.performWithDelayGlobal(function()
    ...
    for k, v in pairs(allItems) do
      if gift.newTermCheckIn:getIsCanAccept(v:getDaynum()) then
...
function CNewTermCheckInView:ClickItem(item, isauto)
  print("===> CNewTermCheckInView:ClickItem:", item, item:getRewardId())
  ...
  if gift.newTermCheckIn:getIsCanAccept(item:getDaynum()) then
    getCurSceneView():addSubView({
      subView = CNewTermRewardConffirm.new(item:getRewardId(), true, clickOutSideFlag),
      zOrder = MainUISceneZOrder.menuView
    })
  else
    getCurSceneView():addSubView({
      subView = CNewTermRewardConffirm.new(item:getRewardId(), false),
      zOrder = MainUISceneZOrder.menuView
    })
  end
end
function CNewTermCheckInView:SetDayText(item, isauto)
  local day = gift.newTermCheckIn:GetNewTermCheckInDay()
  self:getNode("txt_times"):setText(string.format("连续签到:%d次", day))
end
function CNewTermCheckInView:SetTipsText()
...
function CNewTermCheckInView:OnBtn_Close(btnObj, touchType)
  self:CloseSelf()
end
function CNewTermCheckInView:OnMessage(msgSID, ...)
  if msgSID == MsgID_Gift_NewTermCheckInUpdate then
...
function CNewTermCheckInView:Clear()
  self.m_RewardItems = {}
