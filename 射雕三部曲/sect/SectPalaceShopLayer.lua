--[[
    文件名：SectPalaceShopLayer
    描述：江湖杀商店
    创建人：yanghongsheng
    创建时间：2018.9.27
-- ]]
local SectPalaceShopLayer = class("SectPalaceShopLayer", function()
...
-- 构造函数
function SectPalaceShopLayer:ctor(params)
    ...
    -- 保存数据
    self.mShopInfo = SectPalaceShopModel.items
...
-- 初始化UI元素
function SectPalaceShopLayer:initUI()
    -- 背景框
    ...
    -- 当前代币
    local coinInfo = Utility.analysisStrResList(SectPalaceShopModel.items[1].needCoin)[1]
    ...
    local coinImg = Utility.getDaibiImage(coinInfo.resourceTypeSub, coinInfo.modelId)
...
-- 刷新listview
function SectPalaceShopLayer:refreshListView()
...
--[[
    ...
    cellIndex           -- cell索引号
--]]
function SectPalaceShopLayer:addElementsToCell(cell, cellIndex)
    -- 获取cell数据
    ...
    for i = 1, #groupInfo do
        ...
        -- 购买物品
        local goods = Utility.analysisStrResList(groupInfo[i].exchangeResource)[1]
        -- 名字
        ...
        local coinInfo = Utility.analysisStrResList(groupInfo[i].needCoin)[1]
...
-- 整理物品信息，2个为一组
function SectPalaceShopLayer:dealGoodsInfo()
    self.mGoodsList = {}
    local tempList = {}
...
--[[
    ...
    num                      -- 选择数量
--]]
function SectPalaceShopLayer:requestBuyShopGoods(goodsIndex, num)
    HttpClient:request({
