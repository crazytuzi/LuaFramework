--[[
    文件名：KillerValleyShopLayer
    描述：丹药商店
    创建人：yanghongsheng
    创建时间：2017.12.21
-- ]]
local KillerValleyShopLayer = class("KillerValleyShopLayer", function()
    return display.newLayer(cc.c4b(0, 0, 0, 128))
end)

-- 构造函数
function KillerValleyShopLayer:ctor(params)
    -- 屏蔽下层点击事件
    ...
    -- 保存数据
    self.mShopInfo = KillervalleyShopModel.items
...
-- 初始化UI元素
function KillerValleyShopLayer:initUI()
    -- 背景框
    ...
    -- 当前代币
    local coinImg = Utility.getDaibiImage(ResourcetypeSub.eLoveFlower)
    local coinNum = Utility.getOwnedGoodsCount(ResourcetypeSub.eLoveFlower)
    local coinName = Utility.getGoodsName(ResourcetypeSub.eLoveFlower)
    local curCoinLabel = ui.newLabel({
            text = TR("当前%s: #d17b00{%s}%d", coinName, coinImg, coinNum),
    ...
    self.curCoinLabel.refesh = function ()
        local coinNum = Utility.getOwnedGoodsCount(ResourcetypeSub.eLoveFlower)
        self.curCoinLabel:setString(TR("当前%s: #d17b00{%s}%d", coinName, coinImg, coinNum))
    end
...
-- 刷新listview
function KillerValleyShopLayer:refreshListView()
...
--[[
    ...
    cellIndex           -- cell索引号
--]]
function KillerValleyShopLayer:addElementsToCell(cell, cellIndex)
    -- 获取cell数据
...
-- 整理物品信息，2个为一组
function KillerValleyShopLayer:dealGoodsInfo()
    self.mGoodsList = {}
    local tempList = {}
...
--[[
    ...
    num                      -- 选择数量
--]]
function KillerValleyShopLayer:requestBuyShopGoods(goodsIndex, num)
    HttpClient:request({
