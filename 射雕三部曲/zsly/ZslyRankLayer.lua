--[[
    文件名：ZslyRankLayer.lua
    描述：比武招亲排行
    创建人：yanghongsheng
    创建时间：2017.4.22
--]]

local ZslyRankLayer = class("ZslyRankLayer", function()
...
function ZslyRankLayer:ctor(params)
	-- 星星最大数量
...
function ZslyRankLayer:showTabLayer()
    -- 创建分页
    local buttonInfos = {
        {
            text = TR("排行榜"),
...
function ZslyRankLayer:refreshListView()
...
function ZslyRankLayer:refreshListItem(index)
	...
	headSprite:setHero({HeroModelId = info.HeadImageId, pvpInterLv = info.DesignationId}, showAttrs)
	...
	local historyId = info.MaxNodeId or 0
	if historyId > 2000 then
		local historyIndex = table.indexof(self.mZslyOrderIdList, historyId)
		historyId = self.mZslyOrderIdList[historyIndex-1]
	end
	-- 历史最高
	local maxLabel = ui.newLabel({
		text = TR("历史最高: %s%s", "#d17b00", ZslyNodeModel.items[historyId] and ZslyNodeModel.items[historyId].name or TR("0层")),
		color = Enums.Color.eBlack,
		anchorPoint = cc.p(0, 1),
		size = 20,
		x = 220,
...
function ZslyRankLayer:dealZslyIdOrder()
	self.mZslyOrderIdList = {}
...
function ZslyRankLayer:refreshUI()
	-- 刷新列表
	self:refreshListView()

	-- 历史最高
	local historyId = self.mRankData.MaxNodeId or 0
	if historyId > 2000 then
		local historyIndex = table.indexof(self.mZslyOrderIdList, historyId)
		historyId = self.mZslyOrderIdList[historyIndex-1]
	end

	local text = TR("我的排名: %s%s    %s历史最高：%s%s",
					"#d17b00",
					self.mRankData.RankingId or TR("暂无"),
					Enums.Color.eBlackH,
					"#20781b",
					ZslyNodeModel.items[historyId] and ZslyNodeModel.items[historyId].name or TR("0层"))
	...
	if self.historyMaxLabel then
		...
		self.historyMaxLabel:setAnchorPoint(cc.p(1, 0))
		self.historyMaxLabel:setPosition(cc.p(620, 130))
		self.mParentLayer:addChild(self.historyMaxLabel)
	end
end

-----------------网络相关---------------------
function ZslyRankLayer:requestRankInfo()
	HttpClient:request({
 		moduleName = "ZslyInfo",
        ...
        callback = function(response)
        	...
        	self.mRankData = response.Value.PlayerRanking
        	self.mRankInfo = response.Value.RankingList
