--[[
    文件名：OverallRankFapLayer
    描述：战斗力排行页面
    创建人：yuanhuangjing
    修改人：chenqiang
    创建时间：2016.4.28
--]]
local OverallRankFapLayer = class("OverallRankFapLayer", function()
	return display.newLayer()
end)

function OverallRankFapLayer:ctor()
...
--进入响应
function OverallRankFapLayer:onEnterTransitionFinish()
	--添加触摸层
	self:createTouchEventLayer()
end

--添加触摸事件处理
function OverallRankFapLayer:createTouchEventLayer()
...
--初始化UI
function OverallRankFapLayer:initUI()
	--创建场景的背景
...
--排名前三UI
function OverallRankFapLayer:setTop3UI()
...
--点赞序列动画
function OverallRankFapLayer:dianZanDongHua(zanNum)
	--设置序列动作
	local actionArray = {}
...
function OverallRankFapLayer:createSelfRankInfo()
	--创建自己排位栏
	...
	--我的战斗力
	self.mSelfFapLabel = ui.newLabel({
		text = "--",
		color = Enums.Color.eNormalYellow,
		anchorPoint = cc.p(0, 0.5),
		x = 275,
	...
	self.mSelfRankNode:addChild(self.mSelfFapLabel)
	--我的服务器
...
--刷新页面
function OverallRankFapLayer:refreshLayer()
	-- 点赞按钮状态
    ...
    for _, info in ipairs(self.mRankInfo) do
    	if info.PlayerId == playerId then
    		myRankInfo = nil   --myRankInfo = info 暂时屏蔽自己的排名显示
    	end
    end
    if myRankInfo == nil then
    	...
    	-- 我的战力
    	self.mSelfFapLabel:setString(Utility.numberFapWithUnit(myRankInfo.FAP))
    	-- 我的服务器
...
--展示前三
function OverallRankFapLayer:showTop3()
...
--刷新前三人的信息
function OverallRankFapLayer:refreshTopInfo(index)
...
--展示人物信息
function OverallRankFapLayer:showHeroDetails(parrent, index)
	local info = {}
	for k,v in ipairs(self.mRankInfo) do
		if v.Rank == index then
			info = v
...
--创建榜单信息
function OverallRankFapLayer:createRankTableView(type)
    --榜背景大小
	...
	if #self.mRankInfo == 0 then
		...
		local kongSprite = ui.createEmptyHint(TR("暂无战力榜数据,敬请期待"))
...
--==================网络请求相关==================
function OverallRankFapLayer:requestGetData()
	HttpClient:request({
...
function OverallRankFapLayer:requestDianZan()
	HttpClient:request({
        moduleName = "LeaderBoard",
        methodName = "Praise",
        svrMethodData = {1, self.mCurrentShow},
        callback = function(data)
            ...
            -- 显示领取到的奖励信息
            if data.Value and data.Value.BaseGetGameResourceList then
...
function OverallRankFapLayer:requestShowPlayerTeam(index)
	HttpClient:request({
        moduleName = "LeaderBoard",
