--[[
    文件名：JigsawPuzzleRankLayer.lua
    描述：新拼图活动排行界面
    创建人：lengjiazhi
    创建时间：2017.12.29
--]]

local JigsawPuzzleRankLayer = class("JigsawPuzzleRankLayer", function(params)
...
function JigsawPuzzleRankLayer:ctor(params)
    self.mRankInfo = {}
    self.mRewardInfo = {}
    self.mMyScore = nil
    self.mRank = nil
...
-- 添加UI相关
function JigsawPuzzleRankLayer:initUI()
    -- 页面元素父节点
    ...
    lessSCoreLabel:setPosition(265, 1010)
...
-- 创建分页控件，个人排名、公会排名、规则
function JigsawPuzzleRankLayer:addTabView()
    local buttonInfos = {}
    -- 道具按钮配置
    local btnInfo1 = {
        text = TR("个人排名"),
...
--排行页面
function JigsawPuzzleRankLayer:rankPage()
...
--奖励页面
function JigsawPuzzleRankLayer:rewardPage()
    if self.mRankPageNode then
...
--创建排行显示
function JigsawPuzzleRankLayer:createRankPage()
    ...
    local myRankBgSprite = ui.newScale9Sprite("c_25.png", cc.size(640, 50))
    ...
    myRankLabel:setPosition(20, 140)
    self.mRankPageNode:addChild(myRankLabel)

    local myScoreLabel = ui.newLabel({
        text = TR("步数：%s", self.mMyScore),
        size = 22,
        outlineColor = Enums.Color.eBlack,
        })
    myScoreLabel:setAnchorPoint(0, 0.5)
    myScoreLabel:setPosition(300, 140)
    self.mRankPageNode:addChild(myScoreLabel)

    local myFinishLabel = ui.newLabel({
        text = TR("张数：%s", self.mMyNum),
        size = 22,
        outlineColor = Enums.Color.eBlack,
        })
    myFinishLabel:setAnchorPoint(0, 0.5)
    myFinishLabel:setPosition(200, 140)
    self.mRankPageNode:addChild(myFinishLabel)

    local myTimeLabel = ui.newLabel({
        text = TR("时间：%s", MqTime.formatAsDay(self.mTotalSeconds)),
        size = 22,
        outlineColor = Enums.Color.eBlack,
        })
    myTimeLabel:setAnchorPoint(0, 0.5)
    myTimeLabel:setPosition(430, 140)
    self.mRankPageNode:addChild(myTimeLabel)
    ...
    local function createRankCell(i)
        ...
        local scoreLabel = ui.newLabel({
            text = TR("步数：#d17b00%s", info.Score),
            color = cc.c3b(0x46, 0x22, 0x0d),
            size = 20,
            })
        ...
        local totalTimeLabel = ui.newLabel({
            text = TR("时间：#d17b00%s", MqTime.formatAsDay(info.TotalSeconds)),
            color = cc.c3b(0x46, 0x22, 0x0d),
            size = 20,
            })
        totalTimeLabel:setAnchorPoint(0, 0.5)
        totalTimeLabel:setPosition(320, 35)
        layout:addChild(totalTimeLabel)

        local finishLabel = ui.newLabel({
            text = TR("完成拼图%s张", info.Num),
            color = cc.c3b(0x46, 0x22, 0x0d),
            size = 20,
            })
        finishLabel:setAnchorPoint(0, 0.5)
        finishLabel:setPosition(450, 95)
        layout:addChild(finishLabel)
...
--创建奖励显示
function JigsawPuzzleRankLayer:createRewardPage()
    ...
    local function createRewardCell(i)
        ...
        --整理数据
        local tempRewardList = Utility.analysisStrResList(info.Reward)
...
-----------------------------网络相关-------------------------------
-- 请求服务器，获取排行榜信息
function JigsawPuzzleRankLayer:requestRankList()
    HttpClient:request({
        moduleName = "TimedPuzzlematch", 
        methodName = "GetTotalRank",
        svrMethodData = {},
        callbackNode = self,
        callback = function (data)
            ...
            dump(data, "rank")
            self.mRankInfo = data.Value.Rank
            self.mRank = data.Value.MyRank 
            self.mMyScore = data.Value.Score
            self.mMyNum = data.Value.Num
...
--获取奖励信息
function JigsawPuzzleRankLayer:requestRewardList()
    HttpClient:request({
        moduleName = "TimedPuzzlematch", 
        methodName = "GetTotalRankReward",
        svrMethodData = {},
        callbackNode = self,
        callback = function (data)
            if data.Status ~= 0 then
            ...
            -- dump(data, "reward")
            self.mRewardInfo = data.Value.RewardConfigList
            self:createRewardPage()
        end
    })
end

return JigsawPuzzleRankLayer
