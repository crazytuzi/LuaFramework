--[[
    文件名：CommonActivityRankLayer.lua
    描述：通用活动排行界面
    创建人：lengjiazhi
    创建时间：2018.3.27
--]]

local CommonActivityRankLayer = class("CommonActivityRankLayer", function(params)
...
--[[
    moduleName :接口模块名称
    methodNameRank :排行信息接口名
    methodNameReward :奖励信息接口名
    scoreName  :积分对应的名称
--]]
function CommonActivityRankLayer:ctor(params)
    self.mRankInfo = {}
    self.mRewardInfo = {}
    self.mMyScore = nil
    self.mRank = nil
    self.mModuleName = params.moduleName
    self.mMethodNameRank = params.methodNameRank
    self.mMethodNameReward = params.methodNameReward
    self.mScoreName = params.scoreName
...
-- 添加UI相关
function CommonActivityRankLayer:initUI()
    -- 页面元素父节点
...
-- 创建分页控件，个人排名、公会排名、规则
function CommonActivityRankLayer:addTabView()
    local buttonInfos = {}
    -- 道具按钮配置
    local btnInfo1 = {
        text = TR("个人排名"),
...
--排行页面
function CommonActivityRankLayer:rankPage()
...
--奖励页面
function CommonActivityRankLayer:rewardPage()
    if self.mRankPageNode then
...
--创建排行显示
function CommonActivityRankLayer:createRankPage()
    ...
    local myScoreLabel = ui.newLabel({
        text = TR("个人%s：%s", self.mScoreName, self.mMyScore or TR("暂无")),
        size = 22,
        outlineColor = Enums.Color.eBlack,
        })
    ...
    local function createRankCell(i)
        ...
        local scoreLabel = ui.newLabel({
            text = string.format("%s：#d17b00%s", self.mScoreName, info.Score),
            color = cc.c3b(0x46, 0x22, 0x0d),
            size = 20,
            })
...
--创建奖励显示
function CommonActivityRankLayer:createRewardPage()
...
-----------------------------网络相关-------------------------------
-- 请求服务器，获取排行榜信息
function CommonActivityRankLayer:requestRankList()
    HttpClient:request({
        moduleName = self.mModuleName, 
        methodName = self.mMethodNameRank,
        svrMethodData = {},
        callbackNode = self,
        callback = function (data)
            ...
            self.mLessScore = data.Value.LessScore 

            self:createRankPage()

            self.mLessScoreLabel:setString(TR("进入前500名需要高于：%s%s", self.mLessScore, self.mScoreName))
...
--获取奖励信息
function CommonActivityRankLayer:requestRewardList()
    HttpClient:request({
        moduleName = self.mModuleName, 
        methodName = self.mMethodNameReward,
        svrMethodData = {},
        callbackNode = self,
        callback = function (data)
            if data.Status ~= 0 then
...
return CommonActivityRankLayer
