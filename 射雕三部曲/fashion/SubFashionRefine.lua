--[[
	文件名：SubFashionRefine.lua
	描述：时装分解页面
	创建人: 杨宏生
	创建时间: 2018.05.04
--]]

local SubFashionRefine = class("SubFashionRefine", function()
...
-- 构造函数
function SubFashionRefine:ctor(params)
	-- 读取参数
...
-- 初始化界面
function SubFashionRefine:initUI()
	-- 黑背景
	local bgSize = cc.size(self.viewSize.width-30, self.viewSize.height-30)
	local blackBg = ui.newScale9Sprite("c_17.png", bgSize)
	blackBg:setPosition(self.viewSize.width*0.5, self.viewSize.height-10)
	...
	-- 空提示
	self.mEmptyHint = ui.createEmptyHint(TR("没有可分解的绝学"))
	...
	self.mRefineBtn:setPosition(self.viewSize.width*0.5, 60)
	self:addChild(self.mRefineBtn)
end

-- 创建列表项
function SubFashionRefine:createCell(fashionInfo)
	...
	-- 名字
	local fashionModel = FashionModel.items[fashionInfo.ModelId]
...
-- 刷新界面
function SubFashionRefine:refreshUI()
	-- 清空列表
	self.mRefineListView:removeAllChildren()
	
	-- 分解时装列表
	local refineFashionList = FashionObj:getRefineFashionList()
...
----------------------网络相关---------------------
-- 分解时装
function SubFashionRefine:requestRefine()
	local fashionIdList = {}
