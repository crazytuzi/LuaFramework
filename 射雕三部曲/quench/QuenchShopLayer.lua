--[[
    ...
    创建时间：2017.12.21
-- ]]
local QuenchShopLayer = class("QuenchShopLayer", function()
    return display.newLayer(cc.c4b(0, 0, 0, 128))
end)

-- 构造函数
function QuenchShopLayer:ctor(params)
    -- 屏蔽下层点击事件
    ...
    -- 保存数据
    self.mShopInfo = MedicineShopModel.items
...
-- 初始化UI元素
function QuenchShopLayer:initUI()
    -- 背景框
    ...
    -- 当前代币
    local coinImg = Utility.getDaibiImage(ResourcetypeSub.eMedicineCoin)
    local coinNum = Utility.getOwnedGoodsCount(ResourcetypeSub.eMedicineCoin)
    local curCoinLabel = ui.newLabel({
            text = TR("当前药元: #d17b00{%s}%d", coinImg, coinNum),
    ...
    self.curCoinLabel.refesh = function ()
        local coinNum = Utility.getOwnedGoodsCount(ResourcetypeSub.eMedicineCoin)
        self.curCoinLabel:setString(TR("当前药元: #d17b00{%s}%d", coinImg, coinNum))
    end
...
-- 刷新listview
function QuenchShopLayer:refreshListView()
...
--[[
    ...
    cellIndex           -- cell索引号
--]]
function QuenchShopLayer:addElementsToCell(cell, cellIndex)
    -- 获取cell数据
    ...
    for i = 1, #groupInfo do
        ...
        -- 购买物品
        local goods = Utility.analysisStrResList(groupInfo[i].sellGoods)[1]
        -- 名字
        ...
        -- 所需资源
        local useGoods = Utility.analysisStrResList(groupInfo[i].price)[1]
        ...
        local buyBtn = ui.newButton({
            ...
            clickAction = function()
                if ownNum >= useGoods.num then
                    ...
                    ui.showFlashView({
                        text = TR("%s不足,可出售丹药获得药元", Utility.getGoodsName(useGoods.resourceTypeSub, useGoods.modelId))
                    })
                end
            end
        })
...
-- 整理物品信息，2个为一组
function QuenchShopLayer:dealGoodsInfo()
    self.mGoodsList = {}
    local tempList = {}
...
--[[
    ...
    num                      -- 选择数量
--]]
function QuenchShopLayer:requestBuyShopGoods(goodsIndex, num)
    HttpClient:request({
