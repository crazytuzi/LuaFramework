...
local function skill_func(params)
    ...
    for i, af in ipairs(params.to) do
        table.insert(actionArray, cc.CallFunc:create(function()
            ...
            atkPos.y = atkPos.y + 50

            local atkEffect = bd.interface.newEffect({
                parent = nodeParent,
                effectName = "effect_gongsunlve_pugong",
                animation = "pugong",
...
local skill = {
    res = {"effect_gongsunlve_pugong" },
    audio = {"hero_gongsunlve_pugong.mp3"},
    audiofunc = function()
        bd.adapter.audio.playSound("hero_gongsunlve_pugong.mp3")
    end,
