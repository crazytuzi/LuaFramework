...
local function skill_func(params)
    ...
    if params.dmgTimes == 2 then
        ...
        if params.from <= 6 then
            atkPos.x = atkPos.x - 270 * bd.ui_config.MinScale
        else
            atkPos.x = atkPos.x + 360 * bd.ui_config.MinScale
        end
        -- atkPos.y = atkPos.y - 60 * bd.ui_config.MinScale
       
        local atkEffect = bd.interface.newEffect({
                parent = nodeParent,
                effectName = "hero_rulaishenzhang_nuji",
                animation = "nuji_di",
...
local skill = {
    zhua = true,
    res = { },
    audio = {"hero_rulaishenzhang_nuji.mp3"},
    audiofunc = function()
        bd.adapter.audio.playSound("hero_rulaishenzhang_nuji.mp3")
    end,
