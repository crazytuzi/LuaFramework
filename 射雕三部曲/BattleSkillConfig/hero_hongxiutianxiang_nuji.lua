...
local function skill_func(params)
    ...
    if params.dmgTimes == 2 then
        ...
        atkPos.y = atkPos.y - 80 * bd.ui_config.MinScale
       
        local atkEffect = bd.interface.newEffect({
                parent = nodeParent,
                effectName = "hero_hongxiutianxiang_nuji",
...
local skill = {
    zhua = true,
    res = { },
    audio = {"hero_hongxiutianxiang_nuji.mp3"},
    audiofunc = function()
        bd.adapter.audio.playSound("hero_hongxiutianxiang_nuji.mp3")
    end,
