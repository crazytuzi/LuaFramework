...
local function skill_func(params)
    ...
    if params.dmgTimes == 1 then
        ...
        local atkEffect = bd.interface.newEffect({
                parent = nodeParent,
                effectName = "effect_yuelongmen_nuji",
                animation = "nuji_di",
                scale = bd.ui_config.MinScale*0.3,
                position3D = atkPos,
                endRelease = true,
                
            })
        atkEffect:setLocalZOrder(-atkPos.y - bd.ui_config.diffz-300)
...
local skill = {
    zhua = true,
    res = { },
    audio = {"hero_yuelongmen_nuji.mp3"},
    audiofunc = function()
        bd.adapter.audio.playSound("hero_yuelongmen_nuji.mp3")
    end,
