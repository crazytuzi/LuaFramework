...
local function skill_func(params)
    ...
    for i, af in ipairs(params.to) do
        table.insert(actionArray, cc.CallFunc:create(function()
            ...
            atkPos.y = atkPos.y + bd.ui_config.MinScale*100

            if params.from > 6 then
                atkPos.x = atkPos.x + bd.ui_config.MinScale*150
            else
                atkPos.x = atkPos.x - bd.ui_config.MinScale*150
            end
            ...
            local atkEffect = bd.interface.newEffect({
                parent = nodeParent,
                effectName = "effect_wg_tanzhitong",
            ...
            bd.adapter.audio.playSound("skill_tanzhitong.mp3")
            if params.from > 6 then
...
local skillConfig = {
    res = {"effect_wg_tanzhitong", },
    audio = {"skill_tanzhitong.mp3"},
