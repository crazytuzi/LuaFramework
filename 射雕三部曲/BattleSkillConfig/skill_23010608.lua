...
local function skill_func(params)
    ...
    for i, af in ipairs(params.to) do
        table.insert(actionArray, cc.CallFunc:create(function()
            ...
            atkPos.y = atkPos.y + bd.ui_config.MinScale*150

            local atkEffect = bd.interface.newEffect({
                parent = nodeParent,
                effectName = "effect_wg_dugujian",
                scale = bd.ui_config.MinScale*0.7,
                position3D = atkPos,
            ...
            bd.adapter.audio.playSound("skill_dugujian.mp3")
        end))
...
local skillConfig = {
    res = {"effect_wg_dugujian", },
    audio = {"skill_dugujian.mp3"},
