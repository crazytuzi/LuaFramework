local HHQDRebateView = classGc(view, function(self,_msg)
	self.msg = _msg
end)
...
function HHQDRebateView.create(self,_tag,_data)
	...
	local rechargeStr= _G.Util : createLabel("累计签到：", FONTSIZE)
    ...
    local tipsStr= _G.Util : createLabel("每天需充值", FONTSIZE)
    tipsStr : setColor(_G.ColorUtil : getRGBA(_G.Const.CONST_COLOR_ORED))
    tipsStr : setPosition(rightSize.width/2+50,16)
    ...
    local labWidth = rechargeStr:getContentSize().width
	self.rechargeLab = _G.Util : createLabel(cmp.."天", FONTSIZE)
	self.rechargeLab : setColor(_G.ColorUtil : getRGBA(_G.Const.CONST_COLOR_GRASSGREEN))
	self.rechargeLab : setPosition(7+labWidth,16)
	self.rechargeLab : setAnchorPoint(cc.p(0.0,0.5))
	self.m_container : addChild(self.rechargeLab)

    local cmp = _data.cmp or 0
    local labWidth = tipsStr:getContentSize().width
    self.tipsNumLab = _G.Util : createLabel(cmp.."钻石", FONTSIZE)
    self.tipsNumLab : setColor(_G.ColorUtil : getRGBA(_G.Const.CONST_COLOR_ORED))
    self.tipsNumLab : setPosition(rightSize.width/2+labWidth+5,16)
    self.tipsNumLab : setAnchorPoint(cc.p(0.0,0.5))
    self.m_container : addChild(self.tipsNumLab)
	...
	self : ScrollView(_tag,_data)
	return self.m_container
end

function HHQDRebateView.ScrollView(self,_tag,_data)
...
function HHQDRebateView.Widgetreturn(self,_tag,msg)
    ...
    for k,v in pairs(self.msg.msg2) do
        -- print("Widgetreturn--->>1",v.id_sub,msg.id_sub)
        if v.id_sub == msg.id_sub then
            -- print("Widgetreturn--->>2",v.id_sub,v.value)
            local tab = {}
            for i=1,#v.msg do
    ...
    self.tipsNumLab:setString(RMB.."钻石")

	local addupLab = _G.Util : createLabel("第"..self.num.."天", FONTSIZE)
	addupLab : setColor(_G.ColorUtil : getRGBA(_G.Const.CONST_COLOR_LBLUE))
	addupLab : setPosition(10, self.oneHeight-23)
...
function HHQDRebateView.onBtnCallBack(self, sender, eventType, _tag)
...
function HHQDRebateView.tagfullData(self, id_sub,state)
...
function HHQDRebateView.getTimeStr( self, _time)
...
function HHQDRebateView.__removeScheduler(self)
