local XFPHRebateView = classGc(view, function(self,_msg)
	self.msg = _msg
end)
...
function XFPHRebateView.create(self,tag,_data)
    ...
    if self.myNo == 0 then self.myNo = "无消费记录"
...
function XFPHRebateView.ScrollView(self,tag,_data)
    ...
    -- local function sort(m1,m2)
    --     if m1.cost > m2.cost then
    --         print("paixu",m1.cost,m2.cost)
    --         return true
    --     end
    --     return false
    -- end
...
function XFPHRebateView.Widgetreturn(self,name,sub_id)
...
function XFPHRebateView.getTimeStr( self, _time)
...
function XFPHRebateView.__removeScheduler(self)
