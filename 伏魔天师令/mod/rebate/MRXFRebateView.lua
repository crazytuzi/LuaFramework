local MRXFRebateView = classGc(view, function(self,_msg)
	self.msg = _msg
end)
...
function MRXFRebateView.create(self,tag,_data)
    ...
    local consumeStr= _G.Util : createLabel("今日消费：", FONTSIZE)
...
function MRXFRebateView.ScrollView(self,tag,_data)
...
function MRXFRebateView.Widgetreturn(self,_tag,msg)
    ...
    local addupLab = _G.Util : createLabel("今日累计消费达到：", FONTSIZE)
...
function MRXFRebateView.onBtnCallBack(self, sender, eventType, _tag )
...
function MRXFRebateView.tagfullData(self, id_sub,state)
...
function MRXFRebateView.getTimeStr( self, _time)
...
function MRXFRebateView.__removeScheduler(self)
