local TTHLRebateView = classGc(view, function(self,_msg)
	self.msg=_msg
end)
...
function TTHLRebateView.create(self,tag,_data)
    ...
    local rechargeStr = _G.Util : createLabel("当前充值：", FONTSIZE)
    ...
    local RebateStr= _G.Util : createLabel("每天每个奖励都可以领取", FONTSIZE)
    RebateStr : setColor(_G.ColorUtil : getRGBA(_G.Const.CONST_COLOR_ORED))
    RebateStr : setPosition(rightSize.width/2+106, 16)
    ...
    self.rechargeLab = _G.Util : createLabel(cmp.."钻石", FONTSIZE)
...
function TTHLRebateView.ScrollView(self,tag,_data)
...
function TTHLRebateView.Widgetreturn(self,_tag,msg)
    ...
    local addupLab = _G.Util : createLabel("累计充值：", FONTSIZE)
...
function TTHLRebateView.onBtnCallBack(self, sender, eventType, _tag )
...
function TTHLRebateView.tagfullData(self, id_sub,state)
...
function TTHLRebateView.getTimeStr( self, _time)
...
function TTHLRebateView.__removeScheduler(self)
