local QFXFRebateView = classGc(view, function(self,_msg)
	self.msg = _msg
end)
...
function QFXFRebateView.create(self,tag,_data)
    ...
    local TipsStr = _G.Util : createLabel("每天0点清零消费数额", FONTSIZE)
    TipsStr : setColor(_G.ColorUtil : getRGBA(_G.Const.CONST_COLOR_ORED))
    TipsStr : setPosition(rightSize.width/2+93, 16)
    self.m_container : addChild(TipsStr)

    local consumeStr = _G.Util : createLabel("今日全服消费：", FONTSIZE)
    -- consumeStr : setColor(_G.ColorUtil : getRGBA(_G.Const.CONST_COLOR_YELLOW))
    consumeStr : setPosition(50, 16)
    ...
    self.consumeLab  : setPosition(120, 16)
...
function QFXFRebateView.ScrollView(self,tag,_data)
...
function QFXFRebateView.Widgetreturn(self,_tag,msg)
    ...
    local addupLab = _G.Util : createLabel("今日全服累计消费：", FONTSIZE)
...
function QFXFRebateView.onBtnCallBack(self, sender, eventType, _tag )
...
function QFXFRebateView.tagfullData(self,id_sub, state)
...
function QFXFRebateView.getTimeStr( self, _time)
...
function QFXFRebateView.__removeScheduler(self)
