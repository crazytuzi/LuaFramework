local MRCZRebateView = classGc(view, function(self,_msg)
	self.msg = _msg
end)
...
function MRCZRebateView.create(self,_tag,_data)
	...
	local rechargeStr= _G.Util : createLabel("今日充值：", FONTSIZE)
...
function MRCZRebateView.ScrollView(self,_tag,_data)
...
function MRCZRebateView.Widgetreturn(self,_tag,msg)
    ...
    local addupLab = _G.Util : createLabel("今日累计充值达到：", FONTSIZE)
...
function MRCZRebateView.onBtnCallBack(self, sender, eventType, _tag )
...
function MRCZRebateView.tagfullData(self, id_sub,state)
...
function MRCZRebateView.getTimeStr( self, _time)
...
function MRCZRebateView.__removeScheduler(self)
