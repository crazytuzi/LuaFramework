local JJPHRebateView = classGc(view, function(self,_msg)
	self.msg = _msg
end)
...
function JJPHRebateView.create(self,tag,_data)
    ...
    TipsStr : setPosition(rightSize.width/2+150, 16)
    self.m_container : addChild(TipsStr)

    local myNo = _data.selfrank or "100+"
    if myNo == 0 then myNo = "无竞技记录"
    elseif myNo > 100 then myNo = "100+" end
    local cmp = _data.cmp or 0
    local MyLab = _G.Util : createLabel(string.format("%s",myNo), FONTSIZE)
...
function JJPHRebateView.ScrollView(self,tag,_data)
...
function JJPHRebateView.Widgetreturn(self,name,sub_id)
    ...
    local addupLab = _G.Util : createLabel("竞技排行第"..Num.."名：", FONTSIZE)
    ...
    for j=1, 4 do
        ...
        roleBg[j] : setPosition(60+(j-1)*(iconSize.width+30), self.oneHeight/2-15)
...
function JJPHRebateView.getTimeStr( self, _time)
...
function JJPHRebateView.__removeScheduler(self)
