local CopyMopMediator = classGc(mediator, function(self, _view)
	self.name = "CopyMopMediator"
	self.view = _view
	self:regSelf()
end)

CopyMopMediator.protocolsList={
...
CopyMopMediator.commandsList={
	CPropertyCommand.TYPE,
}

function CopyMopMediator.processCommand(self, _command)
...
-- 7850	 挂机返回
function CopyMopMediator.ACK_COPY_UP_RESULT(self, _ackMsg)
...
-- 7860	 挂机完成
function CopyMopMediator.ACK_COPY_UP_OVER(self, _ackMsg)
	print("ACK_COPY_UP_OVER--->".._ackMsg.type)
	self.view :ACK_COPY_UP_OVER(_ackMsg)
end

 -- 7865	 登陆提醒挂机
function CopyMopMediator.ACK_COPY_LOGIN_NOTICE(self,_ackMsg)
...
-- 7877	 领取挂机奖励返回
function CopyMopMediator.ACK_COPY_UP_REWARD_REPLY(self,_ackMsg)
	self.view:getRewardCallBack()
end

function CopyMopMediator.ACK_COPY_COPY_DATA(self,_ackMsg)
