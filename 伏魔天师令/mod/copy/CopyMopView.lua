...
--副本扫荡
---------------------------
local CopyMopView = classGc(view, function(self, _data)
	...
	self.m_isOffLine=_data._isOffLine
...
function CopyMopView.create( self )
...
function CopyMopView.__addMediator( self )
	self.m_mediator=require("mod.copy.CopyMopMediator")(self)
end

function CopyMopView.__initParams( self )
	...
	CCLOG("[CopyMopView.__initParams]--->>>vip=%d,  %d,  %d",myVip,self.m_vipUseTimes,self.m_vipMaxToSpeed)
end

function CopyMopView.__initView( self )
    ...
    -- local maxBtn=gc.CButton:create("general_max.png")
    -- local maxBtnSize=maxBtn:getContentSize()
    -- maxBtn:addTouchEventListener(f)
    -- maxBtn:setTag(P_TAG_MAX)
    -- maxBtn:setPosition(nPosX+240,nPosY)
    -- maxBtn:ignoreContentAdaptWithSize(false)
    -- maxBtn:setContentSize(cc.size(maxBtnSize.width+20,maxBtnSize.height+10))
    -- mainBg:addChild(maxBtn)
...
function CopyMopView.setCurState( self, _type )
	self.m_myType = _type
...
function CopyMopView.__updateLabel(self)
...
function CopyMopView.updateSurplusTimes(self,_times)
	self.m_surplusTimes=_times
	self.m_surplusTimesLabel:setString(tostring(_times))
end
function CopyMopView.__updateUseTimesLabel( self )
...
function CopyMopView.__registerSchedule( self )
	CCLOG("CopyMopView.__registerSchedule------>>>> 11")
	...
	CCLOG("CopyMopView.__registerSchedule------>>>> 22")
	self.m_mySchedule=_G.Scheduler:schedule(local_updateFun,1)
end

function CopyMopView.unregisterSchedule( self )
	CCLOG("CopyMopView.unregisterSchedule------>>>>")
	if self.m_mySchedule~=nil then
...
function CopyMopView.updateTimesByOffLine(self)
...
function CopyMopView.addOneReward( self, _ackMsg )
...
function CopyMopView.__createOneReward( self, _reward )
...
function CopyMopView.createUPNum( self, _num )
	print( "CopyMopView.createUPNum" )
    local node = cc.Node : create()
...
function CopyMopView.createScrollView( self )
	self :__removeScrollView()
...
function CopyMopView.__removeScrollView( self )
	if self.m_rewardScrollView~=nil then
...
function CopyMopView.getTimeStr( self, _time )
...
function CopyMopView.closeWindow( self )
...
--飘字
function CopyMopView.showErrorCode( self, _msg, _colorType )
    local command=CErrorBoxCommand(_msg,_colorType)
    controller:sendCommand(command)
end
-- [7840]开始挂机 -- 副本 
function CopyMopView.REQ_COPY_UP_START( self, _isUseAll, _num, _type )
	local msg=REQ_COPY_UP_START()
	msg:setArgs(self.m_copyId,_isUseAll,_num,_type)
	_G.Network:send(msg)
end
-- [7845]加速挂机 -- 副本 
function CopyMopView.REQ_COPY_UP_SPEED( self )
	local msg=REQ_COPY_UP_SPEED()
	_G.Network:send(msg)
end
-- [7870]停止挂机 -- 副本 
function CopyMopView.REQ_COPY_UP_STOP( self )
	local msg=REQ_COPY_UP_STOP()
	_G.Network:send(msg)
end

function CopyMopView.startHuangup( self, _ackMsg )
...
function CopyMopView.ACK_COPY_UP_OVER( self, _ackMsg )
...
function CopyMopView.getRewardCallBack(self)
	self:setCurState(P_TYPE_STOP)
end

function CopyMopView.huangupFinish(self)
