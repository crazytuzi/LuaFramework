 ...
 * 区域战,每20秒执行一次
 *
 * /opt/tankserver/embedded/bin/php serverWarTeam.php --host=127.0.0.1 --port=15001 --zid=1 --group=a
 *
 */
...
if (!isset($argv['host']) || !isset($argv['port']) || !isset($argv['zid'])){
    $params = print_r($argv,true);
    exit($params);
}

$cmd = array(
    "cmd"=>"areateamwarserver.battle",
    "params"=>array(
    	"runId"=>$argv['runId'],
    	"areaGroup"=>$argv['areaGroup'],
    	"areaServerId"=>$argv['areaServerId'],
    ),
    "ts"=>time(),
    "zoneid"=>(int)$argv['zid'],
);
