...
-- 玩家家里没兵也一样扫荡
-- 
function api_funmerge_echallengeraid(request)
    local response = {
        ret=-1,
        msg='error',
        data = {echallengeraid={}},
    ...
    local tmpstpass = request.params.stpass
    local tmpetpass = request.params.etpass
    if tmpstpass<=0 or tmpetpass<=0 then
        response.ret = -102
        return response
    end
    local stpass = 0
    local etpass = 0
    if tmpstpass>tmpetpass then
        etpass = tmpstpass
        stpass = tmpetpass
    else
        etpass = tmpetpass
    ...
    local lastchallenges = {}
    for i=stpass,etpass do
        local k = 's'..i
        if table.contains(challenges,k) then
            table.insert(lastchallenges,k)
        end
    end
    if #lastchallenges <= 0 then
        response.ret = -6008
        return response
    end
     
    for _,v in ipairs(lastchallenges) do
