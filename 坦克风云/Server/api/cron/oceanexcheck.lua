-- 
-- 远洋征战定时 报名/竞选元帅,队长 结算校验
-- yunhe
-- 
function api_cron_oceanexcheck(request)
    local response = {
        ret=-1,
        msg='error',
        data = {},
    ...
    if not mOceanMatch then
        writeLog("远洋征战定时校验脚本未获取到mOceanMatch当前bid="..bid, "ocean")
    end
    ...
    if mOceanMatch.isstage1(ts,tonumber(oceaninfo.st),cfg) then
        local players = {}
        local effn = 0
        local res = db:getRow("select count(*) as total from oceanexpedition where job>0 and bid= "..bid.." and apply_at>="..oceaninfo.st.." and apply_at<="..oceaninfo.et)
        if type(res)=='table' then
            effn = tonumber(res.total) or 0
        end 

        if effn>0 then
            writeLog("报名/元帅竞选脚本执行过了", "ocean")
            return false
        end
        ...
        writeLog("报名/元帅竞选脚本又执行一遍 结果:"..json.encode(players), "ocean")
    end
    ...
    if mOceanMatch.isstage2(ts,tonumber(oceaninfo.st),cfg) then
        local players = {}
        local effn = 0
        local res = db:getRow("select count(*) as total from oceanexpedition where job=2 and bid= "..bid.." and apply_at>="..oceaninfo.st.." and apply_at<="..oceaninfo.et)
        if type(res)=='table' then
            effn = tonumber(res.total) or 0
        end 

        if effn>0 then
            writeLog("确认队长脚本执行过了", "ocean")
            return false
        end
        ...
        writeLog("确认队长定时又执行了一遍 结果:" ..json.encode(players), "ocean")
    end
    ...
    if mOceanMatch.isstage3(ts,tonumber(oceaninfo.st),cfg) then    -- 检测队id
        ...
        mOceanMatch.save()
        writeLog("队伍成员分配又执行一遍 结果:"..json.encode(players), "ocean")
    end
