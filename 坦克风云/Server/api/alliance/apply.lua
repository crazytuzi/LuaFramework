-- 军团报名 
function api_alliance_apply(request)
    local response = {
        ret=-1,
        msg='error',
        data = {},
    ...
    if mems then
        local cmd = 'alliance.memapplybattle'
        local data = {
                alliance = {
                    alliance={
    ...
    if(send ~=nil and send==1) then 
        local cronParams = {cmd ="alliance.sendbattlemsg",params={positionId=areaid,warId=warId,opents=opts}}

        if not(setGameCron(cronParams,(date+ents+5)-ts)) then
            setGameCron(cronParams,(date+ents+5)-ts) 
        end 
        local cronParams = {cmd = "alliancewar.getwarpoint",params={positionId=areaid}}
        if not (setGameCron(cronParams,opts.st-ts)) then
            setGameCron(cronParams,opts.st-ts)
        end  
    end
    response.ret = 0
