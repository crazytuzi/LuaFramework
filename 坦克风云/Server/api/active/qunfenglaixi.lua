...
-- @params aid 舰船id
function api_active_qunfenglaixi(request)
    local response = {
        ret     = -1,
        msg     = 'error',
        data    = {},
    ...
    local function lottery(needCheck)
        ...
        if moduleIsEnabled('harmonyversion') ==1 then
            local hReward,hClientReward = harVerGifts('active','qunfenglaixi', num)
        ...
        table.insert(rlog, 1, {clientF, ts, hasbig}) 
