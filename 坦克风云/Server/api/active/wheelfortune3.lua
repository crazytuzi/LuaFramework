...
-- 增加了十连抽功能
function api_active_wheelfortune3(request)
    -- 活动名称，莫斯科赌局
    local aname = 'wheelFortune3'
    ...
    -- 奖励值转积分
    local function getPointByReward(awardKey,cfg)
        local point = 0

        if type(awardKey) == 'table' then
            for _,rk in pairs(awardKey) do                
                point = point + rand(cfg[rk][1],cfg[rk][2])
            end
        end
    ...
    local function lottery(cfg,lnum)
        ...
        elseif lnum >1 then
            ...
            for i=1,lnum do
                local result,rkey = getRewardByPool(cfg.pool)                
                local point = getPointByReward(rkey,cfg.res4point)
    ...
    local activeCfg = getActiveCfg(uid, aname).serverreward -- getConfig("active.".. aname..".serverreward")
    ...
    if action == 1 then          
        ...
        if (mUseractive.info[aname].d.fn or 0) < userfreeN then
            ...
            gemCost = activeCfg.lotteryConsume_1x
            if gemCost < 10 or not mUserinfo.useGem(gemCost) then
                response.ret = -109
                return response
            end
        ...
        local rk = nil
        reward,rk = lottery(activeCfg)
        local point = getPointByReward(rk,activeCfg.res4point)
        ...
        -- 如果花费大于0,记录actionLog
        if gemCost > 0 then
            regActionLogs(uid,1,{action=31,item="",value=gemCost,params={buyNum=1,reward=reward}})
        end
    ...
    elseif action == 5 then       
        -- 抽奖次数  
        local lnum = 10
        local gemCost = activeCfg.lotteryConsume_10x
        ...
        regActionLogs(uid,1,{action=31,item="",value=gemCost,params={buyNum=lnum,reward=reward}})
