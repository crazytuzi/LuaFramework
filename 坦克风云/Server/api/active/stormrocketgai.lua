...
function api_active_stormrocketgai(request)
    -- 活动名称，莫斯科赌局
    local aname = 'stormrocketGai'
    ...
    if action == 1 then
        ...
        if not isfree then
            local aGemCost = activeCfg.reward.gemCost - tonumber(mUserinfo.vip)
            if aGemCost < 28 then
                aGemCost = 28
            end
            if num == 1 then
