...
function api_active_luckcard2018(request)
    local response = {
        ret=-1,
        msg='error',
        data = {},
    ...
    local action = tonumber(request.params.action) or 1  -- 1抽奖 2 抽奖记录
    local free   = request.params.free or nil
    local aname  = 'luckcard2018'
    local num    = request.params.num or 1--- 1单抽 10抽
    ...
    if action==1 then
        ...
        if uobjs.save() then  
            ...
            table.insert(data,{ts,report,num,harCReward})   
            if next(data) then
                if #data >10 then
                    for i=1,#data-10 do
                        table.remove(data,1)
