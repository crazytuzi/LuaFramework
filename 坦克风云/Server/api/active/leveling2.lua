...
-- Time: 下午3:03
--
function api_active_leveling2(request)

    -- 活动名称，莫斯科赌局
    local aname = 'leveling2'
    ...
    if action == 'l'..con[1] then
        ...
        if not takeReward(uid, cfg.serverreward["l"..con[1]]) then
            return response
        end
        defaultData[1] = 1
        --70
    elseif action == 'l'..con[2] then
        ...
        if not takeReward(uid, cfg.serverreward["l"..con[2]]) then
            return response
        end
        defaultData[2] = 1
    end
