-- --------------------------------------------------------
-- 主机:                           192.168.8.204
-- 服务器版本:                        5.5.20-log - Source distribution
-- 服务器操作系统:                      Linux
-- HeidiSQL 版本:                  8.1.0.4650
-- --------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `battle` (
  ...
  `zrank` int(10) unsigned NOT NULL COMMENT '服名次',
  ...
  `binfo` varchar(20000) NOT NULL COMMENT '战斗信息',
  `status` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '状态1,2,3',
  `servers` varchar(50) NOT NULL,
  `st` int(10) unsigned NOT NULL COMMENT '起始时间',
  `et` int(10) unsigned NOT NULL COMMENT '结束时间',
  `pos` varchar(32) DEFAULT NULL COMMENT '上一场匹配的位置',
  `log` varchar(500) DEFAULT NULL COMMENT '记录参赛轮次,比分',
  `battle_at` int(10) unsigned NOT NULL DEFAULT '0',
  `updated_at` int(10) unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uid` (`uid`,`zid`,`bid`),
  KEY `bid` (`bid`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='战斗表';



-- 导出  表 tank_kuafu.battlelog 结构
CREATE TABLE IF NOT EXISTS `battlelog` (
  ...
  KEY `bkey` (`bkey`),
  KEY `bid` (`bid`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='战斗记录';
