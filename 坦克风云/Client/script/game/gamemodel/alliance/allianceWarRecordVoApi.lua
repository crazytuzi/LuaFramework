...
function allianceWarRecordVoApi:clearRecord()
...
function allianceWarRecordVoApi:clear()
	self.redPoint=0
	self.bluePoint=0
	self.redDestroy=0
	self.blueDestroy=0
...
function allianceWarRecordVoApi:getHasNew()
	return self.hasNew
end
function allianceWarRecordVoApi:setHasNew(hasNew)
	self.hasNew=hasNew
end
function allianceWarRecordVoApi:getRFlag()
	return self.rFlag
end
function allianceWarRecordVoApi:setRFlag(rFlag)
	self.rFlag=rFlag
end
function allianceWarRecordVoApi:getDFlag()
	-- return self.dFlag[index]
	return self.dFlag
end
function allianceWarRecordVoApi:setDFlag(dFlag)
	-- if index then
...
function allianceWarRecordVoApi:formatResultData(data)
	--红方积分
	...
	if data and SizeOfTable(data)>0 then
	    ...
	    local pointTab=allianceWarVoApi:getMaxPointTb(tempTab)
...
function allianceWarRecordVoApi:formatStatsData(data)
	if data then
		...
		for k,v in pairs(tankCfg) do
			if v then
				...
				if pHasKey==false and (mykillNum>0 or mydieNum>0) then
					local destroyTab={tonumber(v.sid),mykillNum,mydieNum,tonumber(v.sortId)}
...
function allianceWarRecordVoApi:formatRecordData(data)
 --    [battleId] => 10
 --    [warId] => 516212
 --    [attacker] => 1000324
 --    [defender] => 1000354
	...
	if data then
		...
		for k,v in pairs(data) do
			...
			if attId==1 and defId==playerVoApi:getUid() then
				...
				isBattle=false
				isAttacker=false
				local vo = allianceWarRecordVo:new()
...
-- function allianceWarRecordVoApi:formatAllianceRecordData(data)
-- 	if data then
...
-- 				isWin=true
-- 			end
-- 			local vo1 = allianceWarRecordVo:new()
-- 			local selfAlliance1=allianceVoApi:getSelfAlliance()
-- 			local isAttacker1
-- 			if selfAlliance1 and selfAlliance1.aid==attAName then
-- 				isAttacker1=true
-- 			else
-- 				isAttacker1=false
-- 			end
...
function allianceWarRecordVoApi:hasMore()
	local pMaxNum=allianceWarRecordVoApi:getPersonMaxNum()
	local pNum=0
...
function allianceWarRecordVoApi:getPersonMaxNum()
	return self.personMaxNum
end
function allianceWarRecordVoApi:setPersonMaxNum(num)
	self.personMaxNum=num
end
-- function allianceWarRecordVoApi:getAllianceMaxNum()
-- 	return self.allianceMaxNum
-- end
-- function allianceWarRecordVoApi:setAllianceMaxNum(num)
-- 	self.allianceMaxNum=num
-- end

function allianceWarRecordVoApi:getMinAndMaxTime(type)
	...
	-- elseif type==2 then
	-- 	local allianceRecordTab=allianceWarRecordVoApi:getAllianceRecordTab()
...
function allianceWarRecordVoApi:getPersonRecordTab()
	if self.personRecordTab==nil then
		self.personRecordTab={}
	end
	return self.personRecordTab
end
-- function allianceWarRecordVoApi:getAllianceRecordTab()
-- 	if self.allianceRecordTab==nil then
-- 		self.allianceRecordTab={}
-- 	end
-- 	return self.allianceRecordTab
-- end
function allianceWarRecordVoApi:getPersonDestroyTab()
	if self.personDestroyTab==nil then
		self.personDestroyTab={}
	end
	return self.personDestroyTab
end
function allianceWarRecordVoApi:getAllianceDestroyTab()
	if self.allianceDestroyTab==nil then
		self.allianceDestroyTab={}
	end
...
function allianceWarRecordVoApi:getPersonNum()
	local destroyNum=0
	local lostNum=0
...
function allianceWarRecordVoApi:getAreaNameByIndex(idx)
	return getlocal("hold_name"..idx)
end

function allianceWarRecordVoApi:isRed()
...
function allianceWarRecordVoApi:isVictory()
