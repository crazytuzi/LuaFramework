acDayRechargeForEquipVoApi = {}

function acDayRechargeForEquipVoApi:getAcVo()
	return activityVoApi:getActivityVo("dayRechargeForEquip")
end

function acDayRechargeForEquipVoApi:getAcCfg()
	local acVo = self:getAcVo()
	if acVo and acVo.datas then
		return acVo.datas
	end
	return nil
	--return activityCfg["dayRechargeForEquip"]
end
function acDayRechargeForEquipVoApi:getTodayMoney()
	local acVo = self:getAcVo()
...
function acDayRechargeForEquipVoApi:addTodayMoney(money)
	local acVo = self:getAcVo()
	if acVo ~= nil then
...
function acDayRechargeForEquipVoApi:getMaxNeedMoney()
...
function acDayRechargeForEquipVoApi:canReward()
	local acCfg = self:getAcCfg()
...
function acDayRechargeForEquipVoApi:getNeedMoneyById(id)
	local acCfg = self:getAcCfg()
...
function acDayRechargeForEquipVoApi:checkIfHadRewardById(id)
	local acVo = self:getAcVo()
...
function acDayRechargeForEquipVoApi:checkIfCanRewardById(id)
...
-- 得到当前可以领取的奖励
function acDayRechargeForEquipVoApi:getCurrentCanGetReward()
	local acCfg = self:getAcCfg()
	if acCfg ~= nil then
		local rewardLen = SizeOfTable(acCfg.reward)
		if rewardLen ~= nil and rewardLen > 0 then
...
function acDayRechargeForEquipVoApi:getRewardById(id)
	local acCfg = self:getAcCfg()
	if acCfg ~= nil and acCfg.reward ~= nil then
...
function acDayRechargeForEquipVoApi:afterGetReward(id)
	local acVo = self:getAcVo()
	if acVo ~= nil and G_isToday(acVo.t) == true then
		if acVo.c == nil then
			acVo.c = 1
...
-- 从前一天过度到后一天时重新获取数据
function acDayRechargeForEquipVoApi:refresh()
	local acVo = self:getAcVo()
	if acVo ~= nil then
