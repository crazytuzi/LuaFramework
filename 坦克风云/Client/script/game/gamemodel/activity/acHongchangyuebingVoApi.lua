...
function acHongchangyuebingVoApi:getAcVo()
	return activityVoApi:getActivityVo("hongchangyuebing")
end

function acHongchangyuebingVoApi:getComposeTankID()
	return self.rewardTank
end

--获取单次抽奖的花费
function acHongchangyuebingVoApi:getSingleCost()
	return self.gemCost
end

--获取十连抽的花费
function acHongchangyuebingVoApi:getTuhaoCost()
	return self.tuhaoCost
end

--获取暴击倍数
function acHongchangyuebingVoApi:getCriticalPercent()
...
function acHongchangyuebingVoApi:getNumTb()
...
function acHongchangyuebingVoApi:play(type,callback)
	...
	local function onRequestEnd(fn,data)
		...
		if ret==true then
			...
			local result=sData.data.hongchangyuebing
	...
	socketHelper:activeHongchangyuebingRocket(1,param,onRequestEnd)
end

--获取当前可以合成多少个坦克
function acHongchangyuebingVoApi:getComposeNum()
	local vo=self:getAcVo()
	if(vo==nil)then
		return 0
	end
...
--凑齐碎片了，合成坦克
function acHongchangyuebingVoApi:compose(callback)
	...
	socketHelper:activeHongchangyuebingRocket(2,{},onRequestEnd)
end

--购买碎片
--param part: 要购买的碎片部位
function acHongchangyuebingVoApi:buyFragment(part,callback)
	local function onRequestEnd(fn,data)
		local ret,sData=base:checkServerData(data)
		if ret==true then
	...
	socketHelper:activeHongchangyuebingRocket(3,param,onRequestEnd)
end

--是否有免费次数
function acHongchangyuebingVoApi:hasFreeTime()
	local vo=self:getAcVo()
	if(vo)then
...
function acHongchangyuebingVoApi:canReward()
	local vo=self:getAcVo()
	if(vo)then
		local zeroTime=G_getWeeTs(base.serverTime)
		if(vo.lastTime<zeroTime)then
...
function acHongchangyuebingVoApi:returnTankData()
	local vo = self:getAcVo()
...
function acHongchangyuebingVoApi:getVersion()
	local vo = self:getAcVo()
