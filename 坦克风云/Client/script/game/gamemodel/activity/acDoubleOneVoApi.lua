acDoubleOneVoApi={
	discountTb = {2,3,4}--默认新增折扣 ，如果新配置修改折扣，需要添加新字段，修改这里
}

function acDoubleOneVoApi:getAcVo( )
	return activityVoApi:getActivityVo("new112018")
end

function acDoubleOneVoApi:canReward()

	return false
end

function acDoubleOneVoApi:getVersion( )
...
function acDoubleOneVoApi:getStEdTime( )--拿到抢购时间段
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:setLastTime(t)
	local vo = self:getAcVo()
	if vo and t then
		vo.lastTime =t
	end
end
function acDoubleOneVoApi:updateLastTime()
	local vo = self:getAcVo()
	if vo then
		vo.lastTime = base.serverTime
	end
end
function acDoubleOneVoApi:isToday()
	local isToday=false
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getBeginGameTime( ) --拿到活动开启时间 不是抢购
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getCountdown( ) --拿到倒计时
...
function acDoubleOneVoApi:getScratchBeginOutTime(isNext)
...
function acDoubleOneVoApi:getScratchEndOutTime(isNext)
	local PanicTimeTb = self:getStEdTime()
...
function acDoubleOneVoApi:isInTime( )--判断 当前时间是否在抢购时间内 并且拿到当前时间
	local vo  = self:getAcVo()
...
function acDoubleOneVoApi:getCurShowShopIndex(nextSellShow)--拿到当前需要展示的商店idx,抢购商店的开关Tab
...
function acDoubleOneVoApi:returWhiPanicShop(nextSellShow)---返回当前可以抢购的商店Tab,在商店列表里的具体第几个
...
function acDoubleOneVoApi:getShowDiaAllNums(num,nextSellShow)--拿到可显示的板子数量，num 为一个板子可显示的数量,nextSellShow 第一个签使用的
...
function acDoubleOneVoApi:getPanicShopTbData(idx,num,nextSellShow)--idx 在板子上需要显示的id,num 为行数,nextSellShow--下一个需要展示
...
function acDoubleOneVoApi:getFuncTb( )---抢购 拿到抢购商店的对应功能信息信息
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getSwitchTab()--抢购 确定所有商店的开关,拿到开张的总商店个数
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getwhiSelfShop(whiNum)--拿到当前需要初始化的商店
...
function acDoubleOneVoApi:getShowSelfAllNums(num,idx)--拿到可显示的板子数量，num 页面可放置几列,idx 第几个商店
...
function acDoubleOneVoApi:getSelfShopTbData(idx,num,whiIdx)--idx 在板子上需要显示的id,num 为行数,
...
function acDoubleOneVoApi:getbuyShop( )---拿到打折商店的信息
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getPanicedShopNums( )--拿到本轮的道具抢购的次数
	local  vo  = self:getAcVo()
	if vo and vo.shopPanicedNums then
		return vo.shopPanicedNums
	end
end
function acDoubleOneVoApi:setPanicedShopNums(shopPanicedNums)--更新本轮的道具抢购的次数
	local  vo  = self:getAcVo()
...
function acDoubleOneVoApi:getbuyShopNums( )--拿到所有商店所有物品购买的次数
	local  vo  = self:getAcVo()
	if vo and vo.buyShopNumsTb then
		return vo.buyShopNumsTb
	end
end
function acDoubleOneVoApi:setbuyShopNums(buyShopNumsTb)--更新所有商店所有物品购买的次数
	local  vo  = self:getAcVo()
...
function acDoubleOneVoApi:getPanicShopTb( ) ---拿到抢购商店的信息
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:setgDoubleOnerTb( )
	local pShopItem = self:getPanicShopTb( )
	for k,v in pairs(pShopItem) do
		...
		for i=1,vNum do
			...
			gDoubleOnerTb[k][i] = useTb[useNum]
...
function acDoubleOneVoApi:setPanicedTab(panicedTab )--设置本轮已抢购的tab
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getPanicedTab( )--拿到本轮已抢购的tab
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getScratchTimeTb( )
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:setScratchTimeTb(scratchTimeTb)
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getScratchTb( )--拿到刮刮奖 奖池的金币数量
	local vo =self:getAcVo()
	if vo and vo.scratchTb then
		return vo.scratchTb
	end
end
function acDoubleOneVoApi:setScratchTb(scratchTb)--更新刮刮奖 奖池的金币数量
	local vo =self:getAcVo()
	if vo  then
...
function acDoubleOneVoApi:getLastScratchGold( )--拿到当前一次刮奖的金币数量
	local vo =self:getAcVo()
...
function acDoubleOneVoApi:setLastScratchGold(gold)
	local vo =self:getAcVo()
	if vo and gold then
...
function acDoubleOneVoApi:getNearScratchGold()
	local vo =self:getAcVo()
	if vo and vo.nearScratchGold then
		return vo.nearScratchGold
	end
end
function acDoubleOneVoApi:setNearScratchGold(nearScratchGoldTb )--最近一次挂奖钱数
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:isLastTim( )
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getChatSillValue( )
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:setChatSillValue(chatSillValue)
	local vo = self:getAcVo()
...
function acDoubleOneVoApi:getRefShopTbSocket(callBack)
	local function getRefShopCall(fn,data)
        local ret,sData = base:checkServerData(data)
        if ret == true then
            if sData and sData.data and sData.data.refshop then
     ...
     socketHelper:doubleOnePanicBuying( getRefShopCall,"refshop")
end

function acDoubleOneVoApi:setRefShopTb(newRefShopTb)
	local vo = self:getAcVo()
	if newRefShopTb then
		vo.refShopTb = newRefShopTb
	end
end
function acDoubleOneVoApi:getRefShopTb( )
	local vo = self:getAcVo()
	if vo.refShopTb then
		return vo.refShopTb
	end
end

function acDoubleOneVoApi:getOpenShopNum()
	local vo = self:getAcVo()
	local zeroTime=G_getWeeTs(base.serverTime)
	local hour = base.serverTime - zeroTime
	if vo and vo.refTime then
		if hour >= vo.refTime[2] * 3600 - vo.closeTime * 60  then
			return 2
		end
	end
	return 1
...
function acDoubleOneVoApi:getFirstShowShop( )
	local shopIdx = self:getOpenShopNum()
	return self:getRefShopTb()[shopIdx][1]
end

function acDoubleOneVoApi:getCurShopNewDiscount(curShopIdx)
	local curOpenIdx = self:getOpenShopNum()
	local refShopTb = self:getRefShopTb()
	for k,v in pairs(refShopTb[curOpenIdx]) do
		if v == curShopIdx then
			return true,self.discountTb[k]
		end
	end
	return false
end

function acDoubleOneVoApi:getNewSelfShopTipPic(idx)
	local refShopTb = self:getRefShopTb()
	local curOpenShopIdx = self:getOpenShopNum()
	-- local nextOpenShopIdx = 3 - curOpenShopIdx
	local curOpenShop = refShopTb[1]
	local nextOpenShop = refShopTb[2]
	local pic1,pic2 = nil
	local picTb = {"starCornerMark_1.png","starCornerMark_2.png","starCornerMark_3.png"}
	for k,v in pairs(curOpenShop) do
		if v == idx then
			pic1 = picTb[k]
			do break end
		end
	end
	for k,v in pairs(nextOpenShop) do
		if v == idx then
			pic2 = picTb[k]
			do break end
		end
	end
	return pic1,pic2,curOpenShopIdx
end
--activity_double11_countdownStr

function acDoubleOneVoApi:refeshBuyTime( )

	local vo = self:getAcVo()
	local zeroTime=G_getWeeTs(base.serverTime)
	local curTime = base.serverTime - zeroTime
	-- print("curTime=====>>>>",curTime)

	if not vo.refTime then
		-- print " ~~~~~~~~~~~~ e r r o r ==> vo.refTime is nil ~~~~~~~~~~~~"
		return false,nil
	end

	local tOpen1  = vo.refTime[2] * 3600 - vo.closeTime * 60 --下午关闭前

	if curTime >= tOpen1 then
		local tClose2 = vo.refTime[2] * 3600 + vo.openTime * 60 -- 下午开启前
		local tOpen2  = vo.timeShowTb[2] * 3600 -- 晚上关闭前
		if curTime < tClose2 then
			return false, GetTimeStr(tClose2 - curTime,true),curTime > vo.refTime[2] * 3600 and true or false
		elseif curTime < tOpen2 then
			if curTime >= tOpen2 - 300 then
				return false, GetTimeStr(tOpen2 - curTime,true),nil,true
			else
				return true, GetTimeStr(tOpen2 - curTime - 300,true)
			end
		else
			-- print("~~~~~~~~~~~~~~~e r r o r 1 in refeshBuyTime [curTime,tClose1,tOpen1,tClose2,tOpen2 is ",curTime,tClose1,tOpen1,tClose2,tOpen2)
		end
	else
		local tClose1 = vo.refTime[1] * 3600 + vo.openTime * 60 --上午开启前
		if curTime < tClose1  then
			return false, GetTimeStr(tClose1 - curTime,true)
		elseif curTime < tOpen1 then
			return true, GetTimeStr(tOpen1 - curTime,true)
		else
			-- print("~~~~~~~~~~~~~~~e r r o r 2 in refeshBuyTime [curTime,tClose1,tOpen1,tClose2,tOpen2 is ",curTime,tClose1,tOpen1,tClose2,tOpen2)
		end
	end

	-- print("~~~~~~~~~~~~~~~e r r o r 3 in refeshBuyTime [curTime,tOpen1 is ",curTime,tOpen1)
	return false,nil
end

function acDoubleOneVoApi:getBuyedTb( )
	local vo = self:getAcVo()
	if vo and vo.buyedTb then
		return vo.buyedTb
	end
	return nil
end
function acDoubleOneVoApi:setBuyedTb(newBuyedTb	)
	local vo = self:getAcVo()
	if newBuyedTb then
		vo.buyedTb = newBuyedTb
	else
		vo.buyedTb = {}
	end
end

function acDoubleOneVoApi:isCanBuy(idx1,idx2)
	local vo = self:getAcVo()
	-- print("idx1====idx2====>>>>",idx1,idx2)
	local thisShop = vo.buyedTb and vo.buyedTb[idx1] or nil
	if thisShop then
		local res = thisShop[idx2] or nil
		if res and res >= vo.spLimitNum then
			return false
		end
	else
		return true
