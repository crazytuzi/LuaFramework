alienTechFactoryDialogTab2={}

function alienTechFactoryDialogTab2:new()
    local nc={}
    ...
    self.isShowSpeical =nil

    return nc
end

function alienTechFactoryDialogTab2:init(layerNum,parent)
...
--设置对话框里的tableView
function alienTechFactoryDialogTab2:initTableView()
 ...
 self.tv=LuaCCTableView:createWithEventHandler(hd,CCSizeMake(self.bgLayer:getContentSize().width-10,self.bgLayer:getContentSize().height-85-120),nil)
    ...
    local canTransformTb=alienTechVoApi:getCanTransformTb(2,nil,self.isShowSpeical)
...
function alienTechFactoryDialogTab2:eventHandler(handler,fn,idx,cel)
    if fn=="numberOfCellsInTableView" then
        local canTransformTb=alienTechVoApi:getCanTransformTb(2)
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        -- 无线火力活动是否开启
        -- print("self.isShowSpeical-------techId",self.isShowSpeical,techId)
        if self.isShowSpeical ==true and alienTechVoApi:isInCanTransformTankIdTb(tid)  == true then
            ...
            if tLevel and tLevel>0 then
                ...
                str = str .." (".. getlocal("activity_luckyPoker_unlock")..")"
            end
        end
        ...
        local  sale = 1
        if self.isShowSpeical ==true and alienTechVoApi:isInCanTransformTankIdTb(tankId)  == true then
            local tLevel=alienTechVoApi:getTechLevel(techId)
            if tLevel and tLevel>0 then
                sale=acLuckyPokerVoApi:getSaleValue( )
            end
        end
        local haveReR4=playerVoApi:getR4() or 0
        local haveReR3=playerVoApi:getR3() or 0
        local haveReR2=playerVoApi:getR2() or 0
        local haveReR1=playerVoApi:getR1() or 0

        local costReR1=tankCfg[m_tankIndex].upgradeMetalConsume*sale or 0
        local costReR2=tankCfg[m_tankIndex].upgradeOilConsume*sale or 0
        local costReR3=tankCfg[m_tankIndex].upgradeSiliconConsume*sale or 0
        local costReR4=tankCfg[m_tankIndex].upgradeUraniumConsume*sale or 0
        local minNum1,minNum2,minNum3,minNum4
        if haveReR4>0 and costReR4>0 then
            minNum4=math.floor(haveReR4/costReR4)
        else
            minNum4=0
        end
        if haveReR3>0 and costReR3>0 then
            minNum3=math.floor(haveReR3/costReR3)
        else
            minNum3=0
        end
        if haveReR2>0 and costReR2>0 then
            minNum2=math.floor(haveReR2/costReR2)
        else
            minNum2=0
        end
        if haveReR1>0 and costReR1>0 then
            minNum1=math.floor(haveReR1/costReR1)
        else
            minNum1=0
        end
        local minNum=math.min(minNum1,minNum2,minNum3,minNum4)
        if minNum>0 then
...
function alienTechFactoryDialogTab2:cellClick(idx)
...
function alienTechFactoryDialogTab2:initExpand(idx,cell)
    if cell then
        local canTransformTb=alienTechVoApi:getCanTransformTb(2)
        local tankId=canTransformTb[idx+1][1]
        local techId=canTransformTb[idx+1][2]
        ...
        -- local tb={
        --     {titleStr="metal",spName="resourse_normal_metal.png",needStr=FormatNumber(costReR1),haveStr=FormatNumber(playerVoApi:getR1()),num1=playerVoApi:getR1(),num2=tonumber(costReR1)},
        --     {titleStr="oil",spName="resourse_normal_oil.png",needStr=FormatNumber(costReR2),haveStr=FormatNumber(playerVoApi:getR2()),num1=playerVoApi:getR2(),num2=tonumber(costReR2)},
        --     {titleStr="silicon",spName="resourse_normal_silicon.png",needStr=FormatNumber(costReR3),haveStr=FormatNumber(playerVoApi:getR3()),num1=playerVoApi:getR3(),num2=tonumber(costReR3)},
        ...
        -- }
        local tb={}
        local resNum1,resNum2,resNum3,resNum4=tonumber(costReR1),tonumber(costReR2),tonumber(costReR3),tonumber(costReR4)
        if resNum1 and resNum1>0 then
            local resItem={titleStr="metal",spName="resourse_normal_metal.png",needStr=FormatNumber(costReR1),haveStr=FormatNumber(playerVoApi:getR1()),num1=playerVoApi:getR1(),num2=resNum1}
            table.insert(tb,resItem)
        end
        if resNum2 and resNum2>0 then
            local resItem={titleStr="oil",spName="resourse_normal_oil.png",needStr=FormatNumber(costReR2),haveStr=FormatNumber(playerVoApi:getR2()),num1=playerVoApi:getR2(),num2=resNum2}
            table.insert(tb,resItem)
        end
        if resNum3 and resNum3>0 then
            local resItem={titleStr="silicon",spName="resourse_normal_silicon.png",needStr=FormatNumber(costReR3),haveStr=FormatNumber(playerVoApi:getR3()),num1=playerVoApi:getR3(),num2=resNum3}
            table.insert(tb,resItem)
        end
        if resNum4 and resNum4>0 then
            local resItem={titleStr="uranium",spName="resourse_normal_uranium.png",needStr=FormatNumber(costReR4),haveStr=FormatNumber(playerVoApi:getR4()),num1=playerVoApi:getR4(),num2=resNum4}
            table.insert(tb,resItem)
        end
        ...
        local function touch1()
            ...
            if tid and nums and techId then
                ...
                local result,flag=alienTechVoApi:getIsCanTransform(tid,nums,true,sale)
                if result==true then
        ...
        if upgradePropConsume~="" then
            ...
            if playerVoApi:getR1()>=tonumber(costReR1) and playerVoApi:getR2()>=tonumber(costReR2) and playerVoApi:getR3()>=tonumber(costReR3) and playerVoApi:getR4()>=tonumber(costReR4) and
                isTankEnough==true and isPropEnough==true then
            
                local tnum1=playerVoApi:getR1()/tonumber(costReR1)
                local num1 = math.floor(tnum1)
                local tnum2=playerVoApi:getR2()/tonumber(costReR2)
                local num2 = math.floor(tnum2)
                local tnum3=playerVoApi:getR3()/tonumber(costReR3)
                ...
                for k,v in pairs(upgradePropConsume) do
                    ...
                    local num=v[2]
                    local hasPropNum = bagVoApi:getItemNumId(tonumber(RemoveFirstChar(pid)))
                    table.insert(numTab,math.floor(hasPropNum/num))
                end
            ...
            if playerVoApi:getR1()>=tonumber(costReR1) and playerVoApi:getR2()>=tonumber(costReR2) and playerVoApi:getR3()>=tonumber(costReR3) and playerVoApi:getR4()>=tonumber(costReR4) and isTankEnough==true then
...
function alienTechFactoryDialogTab2:refresh(idx,cell)
    if self.tv then
...
function alienTechFactoryDialogTab2:tick()

end

function alienTechFactoryDialogTab2:dispose()
    self.tv=nil
    self.expandIdx={}
    self.noProduceLb=nil
    self.isShowSpeical =nil
