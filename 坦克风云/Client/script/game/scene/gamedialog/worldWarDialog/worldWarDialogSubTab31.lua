worldWarDialogSubTab31={}

function worldWarDialogSubTab31:new()
    local nc={}
...
function worldWarDialogSubTab31:init(layerNum,parent)
...
function worldWarDialogSubTab31:initDesc()
	local str=getlocal("world_war_shop_desc1")
	...
	self.descLb=GetTTFLabelWrap(getlocal("world_war_shop_open_1"),30,CCSizeMake(self.maskSp:getContentSize().width-60,0),kCCTextAlignmentCenter,kCCVerticalTextAlignmentCenter)
...
function worldWarDialogSubTab31:initTableView()
...
function worldWarDialogSubTab31:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
		local commonList=worldWarVoApi:getCommonList()
	...
	elseif fn=="tableCellAtIndex" then
		...
		local showList=worldWarVoApi:getCommonList()
		local shopVo=showList[idx+1]
		local id=shopVo.id
		local num=shopVo.num or 0

		local commonItems=worldWarVoApi:getShopCommonItems()
		...
		local function onClick(tag,object)
			if self and self.tv and self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
				...
				-- 	self:buyItem(shopVo)
				-- -- end
				local saveLocalKey = "keyWorldWar1"
...
function worldWarDialogSubTab31:buyItem(shopVo)
	local id=shopVo.id
	...
	if (num<maxNum) and (worldWarVoApi:getPoint()>=price) then
		...
		worldWarVoApi:buyItem(1,id,callback)
	end
end

function worldWarDialogSubTab31:doUserHandler()

end

function worldWarDialogSubTab31:tick()
...
function worldWarDialogSubTab31:refresh()

end

function worldWarDialogSubTab31:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
    self.cellHeght=nil
