--世界争霸献花面板
worldWarFlowerDialog=smallDialog:new()

function worldWarFlowerDialog:new(type,battleData)
	local nc={}
	...
	self.betData=worldWarVoApi:getBetList(self.type)[self.battleData.roundID]
	...
	local cfgIndex=worldWarCfg["betStyle4Round"..self.type][self.battleData.roundID]
	self.costCfg=worldWarCfg["betGem_"..cfgIndex]
	self.flowerCfg=worldWarCfg["winner_"..cfgIndex]
	return nc
end

function worldWarFlowerDialog:init(layerNum)
	self.isTouch=nil
...
function worldWarFlowerDialog:initContent()
	...
	self.countDown=worldWarVoApi:getBattleTimeList(self.type)[self.battleData.roundID] - worldWarCfg.betTime - base.serverTime
	...
	local function onClickHead(object,fn,tag)
		if(tag and tag~=0)then
			local index=tag - 518
			worldWarVoApi:showPlayerDetailDialog(self.battleData["player"..index],self.layerNum+1)
		end
	end
	for i=1,2 do
...
function worldWarFlowerDialog:getNextFlowerCost()
	if(self.betData and self.betData.times>0)then
...
function worldWarFlowerDialog:getNextCoinCost()
	if(self.betData)then
...
function worldWarFlowerDialog:bet()
	--已经投注而且投的不是这一场
	...
	if(times<#self.costCfg)then
		...
		local function onConfirm()
			...
			worldWarVoApi:bet(self.type,self.battleData.roundID,self.battleData.battleID,self.battleData["id"..self.betIndex],callback)
		end
...
function worldWarFlowerDialog:tick()
