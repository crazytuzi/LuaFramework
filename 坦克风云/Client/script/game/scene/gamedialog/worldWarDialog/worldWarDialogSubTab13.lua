worldWarDialogSubTab13={}
function worldWarDialogSubTab13:new()
	local nc={}
...
function worldWarDialogSubTab13:init(layerNum)
	...
	worldWarVoApi:getScheduleInfo(2,onGetSchedule)
	return self.bgLayer
end

function worldWarDialogSubTab13:initLayers()
	...
	if(self.warStatus==30 + worldWarVoApi.tmatchDays)then
		...
		if(signStatus==nil or signStatus==1)then
			self:switchLayer(1)
		else
			local selfID=playerVoApi:getUid().."-"..base.curZoneID
			local battleList=worldWarVoApi:getBattleList(2)
...
function worldWarDialogSubTab13:switchLayer(page)
	self.curPage=page
...
function worldWarDialogSubTab13:tick()
...
function worldWarDialogSubTab13:dispose()
	if(self)then
		if(self.bgLayer)then
