acJunshijiangtanTab2 = {}

function acJunshijiangtanTab2:new()
    local nc={}
...
function acJunshijiangtanTab2:init(layerNum)
...
--设置对话框里的tableView
function acJunshijiangtanTab2:initTableView()
	...
	local pointLabel=GetTTFLabel(getlocal("active_junshijiangtan_score"),22)
    ...
    local function rewardHandler()
        local rank=acJunshijiangtanVoApi:getRank()
        local function equipsearchRewardCallback(fn,data)
            local ret,sData=base:checkServerData(data)
            if ret==true then
                local cfg=acJunshijiangtanVoApi:getrankReward()
                ...
                acJunshijiangtanVoApi:setReceive(1)
                self:refresh()
            end
        end
        if rank and rank>0 then
            socketHelper:activityJunshijiangtanGetReward(equipsearchRewardCallback)
        end
    end
    ...
    if acJunshijiangtanVoApi:rankCanReward()>0 then
        self.rewardBtn:setEnabled(true)
    end
    local isReceive=acJunshijiangtanVoApi:isReceive()
    if isReceive==1 then
...
function acJunshijiangtanTab2:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
		local num=1
    local rankList=acJunshijiangtanVoApi:getRanklist()
...
function acJunshijiangtanTab2:doUserHandler()
    local capInSet = CCRect(20, 20, 10, 10);
    local function bgClick(hd,fn,idx)
    end
    ...
    self.descLb=GetTTFLabel(getlocal("activity_junshijiangtan_has_num",{acJunshijiangtanVoApi:getScore()}),25)
    ...
    self.descLb1=GetTTFLabelWrap(getlocal("activity_junshijiangtan_rank_point",{acJunshijiangtanVoApi:getLimitScore()}),25,CCSizeMake(titleBg:getContentSize().width-100,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
    ...
    local function onClickDesc()
        ...
        local strTab={" ",getlocal("activity_equipSearch_rank_tip_4",{rewardStrTab[1],rewardStrTab[2],rewardStrTab[3],rewardStrTab[4],rewardStrTab[5]}),getlocal("activity_equipSearch_rank_tip_3"),getlocal("activity_junshijiangtan_tab2_tip2"),getlocal("activity_junshijiangtan_tab2_tip1")," "}
        local colorTab={}
    ...
    local btnPosWidth = titleBg:getContentSize().width-descBtnItem:getContentSize().width-30
    if G_getCurChoseLanguage() =="ru" then
        btnPosWidth =btnPosWidth+20
    end
    descBtn:setPosition(ccp(btnPosWidth,titleBg:getContentSize().height/2))
    descBtn:setTouchPriority(-(self.layerNum-1)*20-4)
    titleBg:addChild(descBtn,2)

end

function acJunshijiangtanTab2:refresh()
   self.descLb:setString(getlocal("activity_junshijiangtan_has_num",{acJunshijiangtanVoApi:getScore()}))
   self.tv:reloadData()
   local rank=acJunshijiangtanVoApi:getRank(rank)
   if rank==nil then
    self.rewardBtn:setEnabled(false)
   else
    self.rewardBtn:setEnabled(true)
...
function acJunshijiangtanTab2:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
