...
function acMayDayTab2:init(layerNum,parent)
  self.layerNum=layerNum
  self.parent=parent
  self.bgLayer=CCLayer:create()
  self.version = acMayDayVoApi:getVersion()
...
function acMayDayTab2:initWithData(data)
  ...
  --           },
    local shopList = acMayDayVoApi:getShopCfg()
  self.data={}
  ...
  if shopList then
    for k,v in pairs(shopList) do
      ...
      cellData.curbuyNum=acMayDayVoApi:getHasBuyNumByID(cellData.id)
...
function acMayDayTab2:initheader()
  ...
  local function showTankInfo( ... )
    ...
    local icon,tab2Str
    icon = GetBgIcon("acMayDayBgGold.png",showTankInfo,nil,50,100)  
    tab2Str = getlocal("acMayDay_shopDesc")
    --icon:setScale(0.8)
    ...
    self.numLb = GetTTFLabelWrap(acMayDayVoApi:getDaiBi(),25,CCSizeMake(120,0),kCCTextAlignmentCenter,kCCVerticalTextAlignmentCenter)
...
function acMayDayTab2:updateTokenNum()
  self.numLb:setString(acMayDayVoApi:getDaiBi())
end
function acMayDayTab2:getIcon(pCfg,id)
  local function showInfoHandler(hd,fn,idx)
    ...
    local hadNum = tonumber(acMayDayVoApi:getTokenNumByID(id))
...
function acMayDayTab2:initTableView()
...
function acMayDayTab2:eventHandler(handler,fn,idx,cel)
  ...
  elseif fn=="tableCellAtIndex" then
    ...
    local goldSp = CCSprite:createWithSpriteFrameName("acMayDayBgGold.png")
    goldSp:setScale(0.3)
    goldSp:setAnchorPoint(ccp(0,0.5))
    goldSp:setPosition(ccp(priceLb:getContentSize().width+2,priceLb:getContentSize().height/2))
    priceLb:addChild(goldSp)

    local canbuy = true

    local posX = backSprie:getContentSize().width-120
    local poxY=self.cellHeght/2+20
    ...
    --     local pCfg = acMayDayVoApi:getTokenCfgForShowByPid(kid)
    --     local icon 
    ...
    --     backSprie:addChild(icon)
        ...
        local hadNum = acMayDayVoApi:getDaiBi()
          ...
          local function onRequestEnd(fn,data)
            local ret,sData=base:checkServerData(data)
              if(ret==true)then
                ...
                --   acMayDayVoApi:updateSelfTokens(k,-v)
                --             self:updateTokenNum(k)
                -- end

                acMayDayVoApi:setDaiBi(acMayDayVoApi:getDaiBi()-cellData.price)
                self:updateTokenNum()
                acMayDayVoApi:updateHasBuyNumByID(cellData.id,1)
                cellData.curbuyNum=acMayDayVoApi:getHasBuyNumByID(cellData.id)
                local recordPoint = self.tv:getRecordPoint()
                self.tv:reloadData()
                self.tv:recoverToRecordPoint(recordPoint)

                if acMayDayVoApi:getIsChatByID(cellData.id)==true then
                  local propName=award.name
                  local message={key="activity_sendChatMessage",param={playerVoApi:getPlayerName(),getlocal("activity_mayDay_title"),propName}}
                  chatVoApi:sendSystemMessage(message)
                end
              end
            end

          socketHelper:mayDayBuy(cellData.id,onRequestEnd)
        end
          end
...
function acMayDayTab2:tick()
  
end

function acMayDayTab2:buyItem(index)
  
end


function acMayDayTab2:dispose()
  self.data=nil
end