acChongzhisongliDialog = commonDialog:new()

function acChongzhisongliDialog:new()
	local nc={}
...
function acChongzhisongliDialog:resetTab()
...
function acChongzhisongliDialog:initTableView()
	self.cost = acChongzhisongliVoApi:getCostLevel()
	...
	local function rechargeCallback(tag,object)
        ...
        if self.rechargeFlag==2 then
	        local selectReward = acChongzhisongliVoApi:getR1()
	        ...
	        local ChooseFlagList= acChongzhisongliVoApi:getChooseFlagList()
	        ...
	        local function linagqujiangli(fn,data)
	        	...
	        	if ret == true then
	        		if sData and sData.data and sData.data.aname then
	        			-- print("++++++++++++111")
	        			acChongzhisongliVoApi:updataData(sData.data.aname)
	        		end
	        ...
	        local function callback()
	        	socketHelper:acChongzhisongli(self.dangci,dijige,linagqujiangli)
	        end
	        if dijige==nil then
	        	callback()
	        else
	        	local sd=acXiaofeisongliSmallDialog2:new(self.layerNum + 1)
				local dialog= sd:init(callback,self.dangci,dijige,1)
	        end
	    else
    ...
    local rewardBtn=GetButtonItem("creatRoleBtn.png","creatRoleBtn_Down.png","creatRoleBtn_Down.png",rechargeCallback,nil,getlocal("daily_scene_get"),25,11)
    rewardBtn:setAnchorPoint(ccp(0.5,0))
    self.btnLb = tolua.cast(rewardBtn:getChildByTag(11),"CCLabelTTF")
    ...
    if self.dangci==nil then
    	self.btnLb:setString(getlocal("recharge"))
    	self.rechargeFlag=1
	else
		self.rechargeFlag=2
    end
...
function acChongzhisongliDialog:doUserHandler()
	...
	local acVo = acChongzhisongliVoApi:getAcVo()
	...
	local desStr = getlocal("activity_chongzhisongli_des_" .. acChongzhisongliVoApi:getVersion())
	...
	self.totalMoney = acChongzhisongliVoApi:getAlreadyCost()
	self.moneyX = 20 + rechargeLabel:getContentSize().width
	self.totalMoneyLabel = GetTTFLabel(tostring(self.totalMoney), 30)
...
function acChongzhisongliDialog:eventHandler( handler,fn,idx,cel )
	if fn=="numberOfCellsInTableView" then
		return 1
	elseif fn=="tableCellSizeForIndex" then
		print("self.height*self.numberCell+20======>>>>>",self.height*self.numberCell+20)
	...
	elseif fn=="tableCellAtIndex" then
		...
		local selectReward = acChongzhisongliVoApi:getR1()
		local gudinReward = acChongzhisongliVoApi:getR2()
		local ChooseFlagList = acChongzhisongliVoApi:getChooseFlagList()
		local isreward = acChongzhisongliVoApi:getIsreward()
		local alReadyCost = acChongzhisongliVoApi:getAlreadyCost()
		local flagList = acChongzhisongliVoApi:getFlaglist()
		local hadAwardList = acChongzhisongliVoApi:gethadAwardList()

		for i=1,self.numberCell do
			local selectNum = SizeOfTable(selectReward[i] or {})
			local spWidth=210
			local Flag = acChongzhisongliVoApi:getStateByid(i)
			...
			if selectNum==0 then
				...
				local function touchSelect()
					if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
					   ...
					   local dialog= sd:init(addSelectSp,1)
					end
				end
		...
		local per = acChongzhisongliVoApi:getPercentage()
...
function acChongzhisongliDialog:refreshBtn()
	if self.dangci==nil then
    	self.btnLb:setString(getlocal("recharge"))
    	self.rechargeFlag=1
    else
    	self.btnLb:setString(getlocal("daily_scene_get"))
    	self.rechargeFlag=2
    end
end


function acChongzhisongliDialog:openInfo()
	if G_checkClickEnable()==false then
		do
			return
		end
	...
	PlayEffect(audioCfg.mouseClick)
	local version=1
	if acChongzhisongliVoApi:getVersion()==1 then
		version=1
	end
	local td=smallDialog:new()
	local tabStr = {getlocal("activity_xiaofeisongli_tip4_" .. version),getlocal("activity_xiaofeisongli_tip3_" .. version), getlocal("activity_xiaofeisongli_tip2_" .. version),getlocal("activity_chongzhisongli_tip1_" .. version),"\n"}
	local colorTb = {nil,G_ColorWhite,G_ColorWhite,G_ColorWhite,G_ColorWhite,G_ColorWhite,nil}

	local flag = false
	local selectRew = acChongzhisongliVoApi:getR1() -- 可选奖励
...
function acChongzhisongliDialog:refresh()
	self.totalMoney = acChongzhisongliVoApi:getAlreadyCost()
	self.totalMoneyLabel:setString(self.totalMoney)
	self.dangci=nil
	local recordPoint=self.tv:getRecordPoint()
	self.tv:reloadData()
	self.tv:recoverToRecordPoint(recordPoint)
...
function acChongzhisongliDialog:tick()
    local vo=acChongzhisongliVoApi:getAcVo()
    ...
    if self.totalMoney and self.totalMoney ~= acChongzhisongliVoApi:getAlreadyCost() then
    	self:refresh()
    end
    if self.timeLb then
	    local acVo = acChongzhisongliVoApi:getAcVo()
...
function acChongzhisongliDialog:dispose()
	self.dangci=nil
	self.rechargeFlag = nil
	self.totalMoney = nil
	self.btnLb = nil
	self.timeLb = nil
end