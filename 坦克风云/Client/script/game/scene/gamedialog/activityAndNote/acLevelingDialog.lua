acLevelingDialog=commonDialog:new()

function acLevelingDialog:new()
    local nc={}
...
function acLevelingDialog:initTableView()
...
function acLevelingDialog:doUserHandler()
  ...
  local acVo = acLevelingVoApi:getAcVo()
  ...
  local function btnCallback(tag,object)
    ...
    if acLevelingVoApi:getCurrentLev() >= maxLv then -- 指挥中心已达到满级
...
function acLevelingDialog:eventHandler(handler,fn,idx,cel)
...
if  G_getCurChoseLanguage()=="cn" or G_getCurChoseLanguage()=="tw" then 
  ...
  elseif fn=="tableCellAtIndex" then
    ...
    -- "leveling":{"st":"1407218280","desVate":0.5,"type":1,"sortId":197,"reward":{"l70":{"u":[{"index":1,"gems":1000}]},"l60":{"u":[{"index":1,"r1":100000000},{"index":2,"r2":100000000},{"r3":100000000,"index":3},{"index":4,"r4":100000000},{"gold":100000000,"index":5}]}},"et":"1421992680","condition":[60,70]}
    ...
    if idx == 0 then
      ...
      desc = getlocal("activity_leveling_des1")
    elseif idx == 1 then
      pic = "item_baoxiang_05.png"
      title = getlocal("activity_leveling_t2")
      local needLv = acLevelingVoApi:getNeedLevel(idx)
      local k,rewardCfg,reward1,rewardNum = acLevelingVoApi:getRewardCfgByLev(needLv)
    ...
    if idx ~= 0 then
      ...
      local currentLev = acLevelingVoApi:getCurrentLev()
      local needLv = acLevelingVoApi:getNeedLevel(idx)
      ...
      -- 领奖按钮
      local canReward = acLevelingVoApi:checkIfCanReward(idx)
      local hadReward = acLevelingVoApi:checkIfHadReward(idx)
    ...
    if idx == 0 then
      ...
      descLabel2:setPosition(ccp(lastW + 170,py))
...
function acLevelingDialog:getReward(id)
  if self and self.tv and self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
    local function getRawardCallback(fn,data)
      if base:checkServerData(data)==true then
        local needLv = acLevelingVoApi:getNeedLevel(id)
        local k,rewardCfg,reward1,rewardNum = acLevelingVoApi:getRewardCfgByLev(needLv)
        ...
        acLevelingVoApi:afterGetReward(id)
      end
    end
    local needLv = acLevelingVoApi:getNeedLevel(id)
    local cfg = acLevelingVoApi:getRewardCfgByLev(needLv)
    socketHelper:getLevelingReward(cfg, getRawardCallback)
  end
end


function acLevelingDialog:tick()
end

function acLevelingDialog:update()
  local acVo = acLevelingVoApi:getAcVo()
  if acVo ~= nil then
...
function acLevelingDialog:dispose()
  self=nil
end
