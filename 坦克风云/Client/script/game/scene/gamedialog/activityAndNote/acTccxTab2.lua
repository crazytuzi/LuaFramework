...
function acTccxTab2:init(layerNum,parent)
...
function acTccxTab2:initUp()
    ...
    local scoreLb=GetTTFLabel(getlocal("activity_tccx_skilled_score2",{acTccxVoApi:getPoint()}),25)
...
function acTccxTab2:initTableView()
    local startH=self.bgLayer:getContentSize().height-165
    ...
    goldLineSprite1:setPosition(ccp(bottomBg:getContentSize().width/2,bottomBg:getContentSize().height-3))
    bottomBg:addChild(goldLineSprite1)

    self.shop=acTccxVoApi:getShop()
    self.shopNum=SizeOfTable(self.shop)
    self.trueShop=acTccxVoApi:getSortShop()

    local function callBack(...)
         return self:eventHandler(...)
    end
    local hd= LuaEventHandler:createHandler(callBack)
    local tvHeight=bottomBg:getContentSize().height-goldLineSprite1:getContentSize().height-15
...
function acTccxTab2:eventHandler(handler,fn,idx,cel)
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        local reward=self.shop[id].reward
        ...
        local myPoint=acTccxVoApi:getPoint()

        local nameLb = GetTTFLabel(rewardItem.name,25)
        ...
        local function touchBuy()
            if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
                ...
                local function refreshFunc()
                    ...
                    if self.shop[id].needTell then
                        local desStr
                        desStr="activity_tccx_chatMessage1"
                        local paramTab={}
                        paramTab.functionStr="tccx"
                        paramTab.addStr="i_also_want"
                        local message={key=desStr,param={playerVoApi:getPlayerName(),getlocal("activity_tccx_title"),rewardItem.name}}
                ...
                local cmd="active.tuichenchuxin.buy"
                local tid=id
                acTccxVoApi:socketTccx2017(refreshFunc,cmd,tid)
            end
        end
...
function acTccxTab2:refresh()
    ...
    if self.scoreLb then
        self.scoreLb:setString(getlocal("activity_tccx_skilled_score2",{acTccxVoApi:getPoint()}))
    end
end

function acTccxTab2:dispose()
