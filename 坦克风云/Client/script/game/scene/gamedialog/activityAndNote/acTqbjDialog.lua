acTqbjDialog=commonDialog:new()

function acTqbjDialog:new(layerNum)
	local nc={}
	...
	nc.upHeight =220
	nc.cellNum        = acTqbjVoApi:getCellNum( )
	nc.curPlayerLevel = playerVoApi:getPlayerLevel()  --获取玩家等级
	nc.isTodayFlag    = acTqbjVoApi:isToday()
	return nc
end

function acTqbjDialog:dispose()
	self.url      = nil
	self.upPosY   = nil
	self.upHeight = nil
	self.cellNum        = nil
...
function acTqbjDialog:doUserHandler()
	...
	local acLabel     = GetTTFLabel(acTqbjVoApi:getTimer(),22,"Helvetica-bold")
    ...
    local function touchTip() acTqbjVoApi:showInfoTipTb(self.layerNum + 1) end 
...
function acTqbjDialog:initTableView()
	self.cellHeight = 180
...
function acTqbjDialog:eventHandler(handler,fn,idx,cel)
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        local curTopAwardNum,curRecharge,curAwardTb,curAwardNum = acTqbjVoApi:getCurCellAwardAndRechargeNum(useIdx,self.curPlayerLevel)
        local isCanRec,recNum = acTqbjVoApi:getCurCellInfo(useIdx )
        self:initCurCell(idx,useIdx,cell,curTopAwardNum,curRecharge,curAwardTb,curAwardNum,isCanRec,recNum)
     	return cell
    end
end

function acTqbjDialog:initCurCell(oldIdx,curIdx,parent,canRecTopNum,needRecharge,awardTb,awardTbNum,isCanRec,recNum)
	...
	local function curCellAwardHandeler( )
        ...
        local rewardTb = acTqbjVoApi:getCurCellAllLevelAward(curIdx)
        local levelTb = acTqbjVoApi:getLevelGroup()
        local titleStr = getlocal("activity_tqbj_levelStr",{needRecharge})
        local needTb = {"tqbj",titleStr,levelTb,rewardTb,SizeOfTable(rewardTb),G_ColorYellowPro2}
        local levelAwardDialog = acThrivingSmallDialog:new(self.layerNum+1,needTb)
        levelAwardDialog:init()
	end 
	...
	local mirrorPic = CCSprite:createWithSpriteFrameName("datebaseShow2.png")
	mirrorPic:setPosition(leftPos.x + 30,leftPos.y - 25)
	parent:addChild(mirrorPic,3)
	...
	if isCanRec then
		local function recHandle()
			...
			acTqbjVoApi:recSocket(curIdx,socketEndCall)
		end
...
function acTqbjDialog:initSunShine( parent ,titleHeight , leftPos)
...
function acTqbjDialog:tick( )
	if self.timeLb then
    	self.timeLb:setString(acTqbjVoApi:getTimer())
    end

    local isEnd=acTqbjVoApi:isEnd()
    if isEnd==false then
        local todayFlag=acTqbjVoApi:isToday()
        if self.isTodayFlag==true and todayFlag==false and self.tv then
            self.isTodayFlag=false
            acTqbjVoApi:removeCurData()
            self.tv:reloadData()
        end
    end
end
