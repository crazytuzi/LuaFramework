acYuebingshenchaTab2 = {}

function acYuebingshenchaTab2:new()
	local nc={}
    ...
    if G_isIphone5() then
       self.expandHeight=G_VisibleSize.height-186
    else
...
function acYuebingshenchaTab2:init(layerNum)
...
function acYuebingshenchaTab2:initLayer()
	self.aidTb,self.tankidTb=acYuebingshenchaVoApi:getTankID()
	self:initTableView()
end

function acYuebingshenchaTab2:initTableView()
...
function acYuebingshenchaTab2:eventHandler(handler,fn,idx,cel)
...
function acYuebingshenchaTab2:loadCCTableViewCell(cell,idx)
...
function acYuebingshenchaTab2:exbgCellForId(container,idx)
  local addH=11;
  local reR1,reR2,reR3,reR4,reUpgradedMoney = acYuebingshenchaVoApi:getUpgradedTankResources(self.aidTb[idx+1],self.tankidTb[idx+1])
  ...
  local UpgradePropConsume = acYuebingshenchaVoApi:getUpgradePropConsume(self.aidTb[idx+1],self.tankidTb[idx+1])
  ...
  local needTankIDTb,needTankNumTb = acYuebingshenchaVoApi:getRefitNeedTankIDAndNum(self.aidTb[idx+1],self.tankidTb[idx+1])
  local haveTankNum
  local haveTankNum1
  ...
  minusSp:setTouchPriority(-(self.layerNum-1)*20-4);
    ...
    local function touch1()
        ...
        local str = getlocal("activity_yuebingshencha_title")
        if acYuebingshenchaVoApi:getVersion()==2 then
          str = getlocal("activity_yuebingshencha_title_2")
        end
		...
		socketHelper:acYuebingshenchaGaizao(nums,self.aidTb[idx+1],serverUpgrade)
...
function acYuebingshenchaTab2:cellClick(idx,parent)
    if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
        PlayEffect(audioCfg.mouseClick)
        if self.expandIdx["k"..(idx-1000)]==nil then
            ...
            local exbg =  parent:getChildByTag(2)
            if exbg then
...
function acYuebingshenchaTab2:moreOrless(idx)
  -- 四种资源
  local reR1,reR2,reR3,reR4,reUpgradedMoney = acYuebingshenchaVoApi:getUpgradedTankResources(self.aidTb[idx+1],self.tankidTb[idx+1])
  ...
  local UpgradePropConsume = acYuebingshenchaVoApi:getUpgradePropConsume(self.aidTb[idx+1],self.tankidTb[idx+1])
   ...
   local haveStr2 = bagVoApi:getItemNumId(tonumber(RemoveFirstChar(pid2)))
...
function acYuebingshenchaTab2:refresh()
...
function acYuebingshenchaTab2:dispose()
	self.bgLayer=nil
    self.layerNum=nil
