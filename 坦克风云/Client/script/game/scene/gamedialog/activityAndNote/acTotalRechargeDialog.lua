acTotalRechargeDialog=commonDialog:new()

--顶端是先进配件箱

function acTotalRechargeDialog:new()
    local nc={}
...
function acTotalRechargeDialog:initTableView()
  ...
  local acCfg = acTotalRechargeVoApi:getAcCfg()
...
function acTotalRechargeDialog:eventHandler(handler,fn,idx,cel)
  ...
  elseif fn=="tableCellAtIndex" then
    ...
    local totalMoney = acTotalRechargeVoApi:getTotalMoney()
    ...
    if acCfg ~= nil and acCfg.reward ~= nil then
      ...
      if rewardLen ~= nil and rewardLen > 0 then
          for i=1,rewardLen do
            ...
            local award=FormatItem(acTotalRechargeVoApi:getRewardById(rewardLen - i + 1),true)
            ...
            local canReward = acTotalRechargeVoApi:checkIfCanRewardById(rewardLen - i + 1)
            if canReward == true then
              local hadReward = acTotalRechargeVoApi:checkIfHadRewardById(rewardLen - i + 1)
      ...
      for j=1,rewardLen do
        local money = acTotalRechargeVoApi:getNeedMoneyById(j) -- 当前需要的金币
        if addContinue == true then
          if tonumber(totalMoney) >= tonumber(money) then
            ...
            if j == 1 then
              ...
              lastMoney = acTotalRechargeVoApi:getNeedMoneyById(j - 1)
            end
...
function acTotalRechargeDialog:getReward()
  if acTotalRechargeVoApi:canReward() == true then
    local function getRawardCallback(fn,data)
      if base:checkServerData(data)==true then
          if self==nil or self.tv==nil then
              do return end
          end
          local currentCanGetReward,index = acTotalRechargeVoApi:getCurrentCanGetReward()
          ...
          acTotalRechargeVoApi:afterGetReward(index)
    ...
    socketHelper:getTotalRechargeReward(getRawardCallback)
  end
end

function acTotalRechargeDialog:getAwardStr(reward)
...
function acTotalRechargeDialog:initNeedMoney(id)
  local needMoney = acTotalRechargeVoApi:getNeedMoneyById(id)
  local needMoneyLabel=GetTTFLabel(tostring(needMoney),28)
  needMoneyLabel:setColor(G_ColorGreen)
  return needMoneyLabel
end

function acTotalRechargeDialog:doUserHandler()
  ...
  local acVo = acTotalRechargeVoApi:getAcVo()
  ...
  self.totalMoneyLabel = GetTTFLabel(tostring(acTotalRechargeVoApi:getTotalMoney()), 30)
  ...
  local function rewardHandler(tag,object)
    ...
    self.rewardBtn=GetButtonItem("creatRoleBtn.png","creatRoleBtn_Down.png","creatRoleBtn_Down.png",rewardHandler,0,getlocal("daily_scene_get"),28)
    self.rewardBtn:setAnchorPoint(ccp(0.5, 0))
    local menuAward=CCMenu:createWithItem(self.rewardBtn)
    menuAward:setPosition(ccp(G_VisibleSizeWidth/2,adaH))
...
-- 更新今日充值金额
function acTotalRechargeDialog:updatetotalMoneyLabel()
  if self == nil then
    do 
     return
    end
  ...
  if self.totalMoneyLabel ~= nil then
    self.totalMoneyLabel:setString(tostring(acTotalRechargeVoApi:getTotalMoney()))
...
function acTotalRechargeDialog:openInfo()
...
function acTotalRechargeDialog:tick()
  if self.timeLb then
    local acVo = acTotalRechargeVoApi:getAcVo()
    G_updateActiveTime(acVo,self.timeLb)
  end
...
function acTotalRechargeDialog:update()
  local acVo = acTotalRechargeVoApi:getAcVo()
  if acVo ~= nil then
...
function acTotalRechargeDialog:dispose()
  self.totalMoneyLabel = nil
  self.goldIcon = nil
  self.moneyX = nil
  self.rewardBtn = nil
