acNewYearsEveSubTab22={

}

function acNewYearsEveSubTab22:new()
    local nc={
    }
...
function acNewYearsEveSubTab22:init(layerNum,parent)
...
function acNewYearsEveSubTab22:initTableView()
...
function acNewYearsEveSubTab22:eventHandler(handler,fn,idx,cel)
...
function acNewYearsEveSubTab22:tick()
...
--用户处理特殊需求,没有可以不写此方法
function acNewYearsEveSubTab22:initTitleAndBtn()
        ...
        local function callback()
            ...
            if self.parent then
                if self.parent.refreshIconTipVisible then
                    self.parent:refreshIconTipVisible(2)
...
function acNewYearsEveSubTab22:refresh()
    if self.rewardBtn then
        local canReward,status=acNewYearsEveVoApi:canRankReward(self.curTab)
        if canReward==true then
...
function acNewYearsEveSubTab22:updateUI()
    if self then
        self:refresh()
        if self.tv then
            self.tv:reloadData()
...
function acNewYearsEveSubTab22:dispose()
    self.noRankLb=nil
    self.curTab=2
