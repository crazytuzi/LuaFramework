...
function acHxghShop:init(layerNum,parent)
...
function acHxghShop:initUp()
    ...
    local scoreLb=GetTTFLabel(getlocal("activity_tccx_skilled_score2",{acHxghVoApi:getPoint()}),25)
    ...
    local function infoHandler(tag,object)
        ...
        for i=1,2 do
            local str=""
            if i==1 then
                str=getlocal("activity_hxgh_exrule1",{getlocal("activity_hxgh_plan")})
            elseif i==2 then
                str=getlocal("activity_phlt_exrule2")
            end
...
function acHxghShop:initTableView()
    local startH=self.bgLayer:getContentSize().height-165
    ...
    self.shop=acHxghVoApi:getShop()
    self.shopNum=SizeOfTable(self.shop)
    self.trueShop=acHxghVoApi:getSortShop()
    self.salelist={}
...
function acHxghShop:eventHandler(handler,fn,idx,cel)
    local strSize3 = 22
    if G_getCurChoseLanguage()=="cn" or G_getCurChoseLanguage()=="ja" or G_getCurChoseLanguage()=="ko" or G_getCurChoseLanguage()=="tw" then
            strSize3=25
    end
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        local blog=acHxghVoApi:getBuyBlog()
        local buyNum=blog[id] or 0
        local limit=self.shop[id].limit
        local myPoint=acHxghVoApi:getPoint()
        local nameLb=GetTTFLabel(rewardItem.name,strSize3)
        ...
        local function showNewPropInfo()
            G_showNewPropInfo(self.layerNum+1,true,true,nil,rewardItem,nil,nil,nil,nil,true)
            return false
        end
        ...
        local flickIdx=self.shop[id].flick
        if flickIdx and (flickIdx=="p" or flickIdx=="b" or flickIdx=="y") then
            G_addRectFlicker2(icon,1.2,1.2,2,flickIdx)
        end
            ...
            if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
                ...
                local function refreshFunc()
                    ...
                    if self.shop[id].notice and tonumber(self.shop[id].notice)==1 then
                        ...
                        local paramTab={}
                        paramTab.functionStr="hxgh"
                        paramTab.addStr="i_also_want"
                        local message={key=desStr,param={playerVoApi:getPlayerName(),getlocal("activity_hxgh_title"),rewardItem.name}}
                        chatVoApi:sendSystemMessage(message,paramTab)
                    end
                    self:refresh()
                end
                acHxghVoApi:acHxghRequest({action=2,tid=id},refreshFunc)
            end
        end
...
function acHxghShop:updateUI()
    self:refresh()
end

function acHxghShop:refresh()
    if self.tv then
        self.trueShop=acHxghVoApi:getSortShop()
    ...
    if self.scoreLb then
        self.scoreLb:setString(getlocal("activity_tccx_skilled_score2",{acHxghVoApi:getPoint()}))
    end
end

function acHxghShop:dispose()
