...
function acKfczTabOne:new(layerNum)
	local nc = {}
...
function acKfczTabOne:init()
	self.bgLayer = CCLayer:create()
    self:initUI()
    return self.bgLayer
end

function acKfczTabOne:initUI()
    ...
    local timeStr1 = acKfczVoApi:getTimeStr()
    local timeStr2 = acKfczVoApi:getRewardTimeStr()
    ...
    local upTitleBg = CCSprite:createWithSpriteFrameName("acznjl_rvbg.png")
    ...
    local upTitleLb = GetTTFLabel(getlocal("activity_kfcz_tab1_title1"), 24, true)
    ...
    local downTitleBg = CCSprite:createWithSpriteFrameName("acznjl_rvbg.png")
    ...
    local downTitleLb = GetTTFLabel(getlocal("activity_kfcz_tab1_title2"), 24, true)
    ...
    local downFrameBg = LuaCCScale9Sprite:createWithSpriteFrameName("acKfcz_frame2.png", CCRect(8, 17, 4, 210), function()end)
    ...
    local luckyRewardTb = acKfczVoApi:getLuckyReward()
    if luckyRewardTb then
        ...
        local descLb = GetTTFLabelWrap(getlocal("activity_kfcz_tab1_desc4"), 20, CCSize(downFrameNode:getContentSize().width - downFrameBgOffset - 20, 0), kCCTextAlignmentCenter, kCCVerticalTextAlignmentTop, "Helvetica-bold")
    ...
    local pageList = {}
    local listNum = 0
    local rankRewardTb = acKfczVoApi:getRankReward()
    if rankRewardTb then
    	rankRewardTb = G_clone(rankRewardTb)
    end
    local keepReward = acKfczVoApi:getKeepReward()
    if keepReward then
    	if rankRewardTb == nil then
    		rankRewardTb = {}
    	end
    	table.insert(rankRewardTb, 1, { -2, keepReward })
    end
    local listReward = acKfczVoApi:getListReward()
    if listReward then
        if rankRewardTb == nil then
            rankRewardTb = {}
        end
        table.insert(rankRewardTb, 1, { -1, listReward })
    end
    if rankRewardTb then
        listNum = SizeOfTable(rankRewardTb)
        for i, v in pairs(rankRewardTb) do
            local pageBg = LuaCCScale9Sprite:createWithSpriteFrameName("acKfcz_frame1.png", CCRect(16, 16, 12, 12), function()end)
            ...
            local labelStr = getlocal("rankOne", {rankStr})
            local descLbStr = getlocal("activity_kfcz_tab1_desc3", {rankStr})
            if rankStr == -1 then
                labelStr = getlocal("activity_kfcz_listReward")
                descLbStr = getlocal("activity_kfcz_tab1_desc1", {acKfczVoApi:getRankRecharge()})
            elseif rankStr == -2 then
                labelStr = getlocal("activity_kfcz_keepReward")
                descLbStr = getlocal("activity_kfcz_tab1_desc2")
            end
            ...
            local descLb = GetTTFLabelWrap(descLbStr, 20, CCSize(centerFrameSize.width - centerFrameBgOffset - 30, 0), kCCTextAlignmentCenter, kCCVerticalTextAlignmentTop, "Helvetica-bold")
            descLb:setAnchorPoint(ccp(0.5, 1))
            descLb:setPosition((centerFrameSize.width - centerFrameBgOffset) / 2, descLbPosY)
            descLb:setColor(G_ColorYellowPro)
            pageBg:addChild(descLb)
            
            local lbBgPosY = descLbPosY - descLb:getContentSize().height - 20
            ...
            local label = GetTTFLabel(labelStr, 25, true)
            ...
            if rewardTb then
                ...
                local iconSize = 90
                local iconSpaceX, iconSpaceY = 95, 20
                if G_getIphoneType() == G_iphone4 then
                elseif G_getIphoneType() == G_iphone5 then
                    iconSpaceY = 100
                elseif G_getIphoneType() == G_iphoneX then
                ...
                if rewardTbSize == 6 then
                    iconSpaceY = iconSpaceY / 2
                end
...
function acKfczTabOne:updateAcTime()
    if self.timeLb and tolua.cast(self.timeLb, "CCLabelTTF") then
        self.timeLb:setString(acKfczVoApi:getTimeStr())
    end
    if self.rewardTimeLb and tolua.cast(self.rewardTimeLb, "CCLabelTTF") then
        self.rewardTimeLb:setString(acKfczVoApi:getRewardTimeStr())
    end
end

function acKfczTabOne:tick()
    self:updateAcTime()
end

function acKfczTabOne:dispose()
	self = nil
end