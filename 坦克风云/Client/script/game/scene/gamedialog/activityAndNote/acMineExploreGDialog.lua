require "luascript/script/game/scene/gamedialog/activityAndNote/acMineExploreGLottery"
require "luascript/script/game/scene/gamedialog/activityAndNote/acMineExploreGShop"
acMineExploreGDialog=commonDialog:new()

function acMineExploreGDialog:new()
    local nc={
        layerTab1=nil,
        layerTab2=nil,
...
function acMineExploreGDialog:resetTab()
...
function acMineExploreGDialog:resetForbidLayer()
...
function acMineExploreGDialog:initTableView()
    ...
    local function initInfo()
        ...
        local descStr=getlocal("activity_mineExploreG_desc"..version)
        if descStr==nil then
            descStr=getlocal("activity_mineExploreG_desc1")
        end        
        local desTv,desLabel=G_LabelTableView(CCSizeMake(descW,70),descStr,25,kCCTextAlignmentLeft)
...
function acMineExploreGDialog:showInfor()
    if self.selectedTabIndex==0 then
        local tabStr={}
        local tabColor={}
        local tabAlignment={}
        tabStr={"\n",getlocal("activity_mineExploreG_rule2"),"\n",getlocal("activity_mineExploreG_rule1"),"\n",getlocal("activityDescription"),"\n"}
...
function acMineExploreGDialog:tabClick(idx,isEffect)
...
function acMineExploreGDialog:hideTabAll()
    for i=1,3 do
        if self then
...
function acMineExploreGDialog:getDataByIdx(tabType)
    self:hideTabAll()
    if tabType==nil then
        tabType=1
    end
...
function acMineExploreGDialog:switchTab(tabType)
    if tabType==nil then
        tabType=1
    end
    if self.infoPage then
   	...
   	if self["exploreTab"..tabType]==nil then
   		local tab
   		if(tabType==1)then
	   		tab=acMineExploreGLottery:new()
	   	else
	   		tab=acMineExploreGShop:new()
	   	end
...
function acMineExploreGDialog:tick()
    if acMineExploreGVoApi:isEnd()==true then
        self:close()
        do return end
    end
...
function acMineExploreGDialog:refreshIconTipVisible()
end

function acMineExploreGDialog:dispose()
