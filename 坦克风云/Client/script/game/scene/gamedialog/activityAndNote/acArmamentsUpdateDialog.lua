acArmamentsUpdateDialog=commonDialog:new()

function acArmamentsUpdateDialog:new()
    local nc={}
...
function acArmamentsUpdateDialog:initTableView()
    ...
    characterSp:setPosition(ccp(20,self.bgLayer:getContentSize().height-120))
    ...
    descBg:setContentSize(CCSizeMake(420,220))
    descBg:setAnchorPoint(ccp(0,1))
    descBg:setPosition(ccp(220-40,self.bgLayer:getContentSize().height-140))
    self.bgLayer:addChild(descBg,1)
    local function descCallBack(...)
       return self:eventHandler(...)
    end
    local deschd= LuaEventHandler:createHandler(descCallBack)
    self.tv=LuaCCTableView:createWithEventHandler(deschd,CCSizeMake(350,200),nil)
    self.tv:setTableViewTouchPriority(-(self.layerNum-1)*20-4)
    self.tv:setPosition(ccp(80,10))
    descBg:addChild(self.tv,1)
    ...
    local icon = CCSprite:createWithSpriteFrameName("item_prop_393.png")
    ...
    local refitIcon = CCSprite:createWithSpriteFrameName("resourse_normal_gold.png")
    refitIcon:setAnchorPoint(ccp(0,1))
    refitIcon:setPosition(ccp(10,headerSprie:getContentSize().height-20))
    refitSprie:addChild(refitIcon)

    local function refitDescBgtouch()
    end
    local refitDesccapInSet = CCRect(20, 20, 10, 10)
    self.refitDescBg =LuaCCScale9Sprite:createWithSpriteFrameName("RechargeBg.png",refitDesccapInSet,refitDescBgtouch)
    self.refitDescBg:setContentSize(CCSizeMake(440,self.normalHeight))
    self.refitDescBg:setAnchorPoint(ccp(0,1))
    self.refitDescBg:setPosition(ccp(20+refitIcon:getContentSize().width,refitSprie:getContentSize().height-20))
    refitSprie:addChild(self.refitDescBg,1)
    ...
    self.refitDescBg:addChild(self.tv2,1)
    ...
    local function gotoRefitHandler(tag,object)
        ...
        activityAndNoteDialog:gotoByTag(6,self.layerNum)
    end
    ...
    refitMenu:setPosition(ccp(headerSprie:getContentSize().width/2+50,10))
    refitMenu:setTouchPriority(-(self.layerNum-1)*20-4) 
    refitSprie:addChild(refitMenu,1) --]]
    self:initmaterialsSprite()
    self:initRefitSprie()

end
function acArmamentsUpdateDialog:initmaterialsSprite( ... )
    
end
function acArmamentsUpdateDialog:initRefitSprie( ... )
    
end
function acArmamentsUpdateDialog:eventHandler(handler,fn,idx,cel)
    if fn=="numberOfCellsInTableView" then
        return 1
    elseif fn=="tableCellSizeForIndex" then
        local tmpSize
        local descLabel=GetTTFLabelWrap(getlocal("activity_armamentsUpdate_activityDesc"),25,CCSizeMake(300,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
...
--[[function acArmamentsUpdateDialog:eventHandler1(handler,fn,idx,cel)
    if fn=="numberOfCellsInTableView" then
        return 1
    elseif fn=="tableCellSizeForIndex" then
        local tmpSize
        local descLabel=GetTTFLabelWrap(getlocal("activity_armamentsUpdate_materialsDesc",{11}),25,CCSizeMake(350,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
...
function acArmamentsUpdateDialog:eventHandler2(handler,fn,idx,cel)
    if fn=="numberOfCellsInTableView" then
        return 1
    elseif fn=="tableCellSizeForIndex" then
        local tmpSize
        local descLabel=GetTTFLabelWrap(getlocal("activity_armamentsUpdate_tankDesc1"),25,CCSizeMake(350,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
...
--]]
function acArmamentsUpdateDialog:tick()
        
end

function acArmamentsUpdateDialog:dispose()
