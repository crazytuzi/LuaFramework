acDanrichongzhiDialog = commonDialog:new()

function acDanrichongzhiDialog:new()
	local nc={}
...
function acDanrichongzhiDialog:resetTab()
...
function acDanrichongzhiDialog:initTableView()
	self.cost = acDanrichongzhiVoApi:getCostLevel()
	...
	local function rechargeCallback(tag,object)
        ...
        if self.rechargeFlag==2 then
	        local selectReward = acDanrichongzhiVoApi:getR1()
	        ...
	        local ChooseFlagList= acDanrichongzhiVoApi:getChooseFlagList()
	        ...
	        local function linagqujiangli(fn,data)
	        	...
	        	if ret == true then
	        		if sData and sData.data and sData.data.aname then
	        			-- print("++++++++++++111")
	        			acDanrichongzhiVoApi:updataData(sData.data.aname)
	        		end
	        ...
	        local function callback()
	        	socketHelper:acDanrichongzhi(self.dangci,dijige,linagqujiangli)
	        end
	        if dijige==nil then
	        	callback()
	        else
	        	local sd=acXiaofeisongliSmallDialog2:new(self.layerNum + 1)
				local dialog= sd:init(callback,self.dangci,dijige,3)
	        end
	    else
...
function acDanrichongzhiDialog:doUserHandler()
	...
	local acVo = acDanrichongzhiVoApi:getAcVo()
	...
	local desStr = getlocal("activity_danrichongzhi_des_" .. acDanrichongzhiVoApi:getVersion())
	...
	self.totalMoney = acDanrichongzhiVoApi:getAlreadyCost()
...
function acDanrichongzhiDialog:eventHandler( handler,fn,idx,cel )
	...
	elseif fn=="tableCellAtIndex" then
		...
		local selectReward = acDanrichongzhiVoApi:getR1()
		local gudinReward = acDanrichongzhiVoApi:getR2()
		local ChooseFlagList = acDanrichongzhiVoApi:getChooseFlagList()
		local isreward = acDanrichongzhiVoApi:getIsreward()
		local alReadyCost = acDanrichongzhiVoApi:getAlreadyCost()
		local flagList = acDanrichongzhiVoApi:getFlaglist()
		local hadAwardList = acDanrichongzhiVoApi:gethadAwardList()

		for i=1,self.numberCell do
			local selectNum = SizeOfTable(selectReward[i] or {})
			local spWidth=210
			local Flag = acDanrichongzhiVoApi:getStateByid(i)
			...
			if selectNum==0 then
				...
				local function touchSelect()
					if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
					   ...
					   local dialog= sd:init(addSelectSp,3)
					end
				end
		...
		local per = acDanrichongzhiVoApi:getPercentage()
...
function acDanrichongzhiDialog:refreshBtn()
	if self.dangci==nil then
    	self.btnLb:setString(getlocal("recharge"))
    	self.rechargeFlag=1
    else
...
function acDanrichongzhiDialog:openInfo()
	if G_checkClickEnable()==false then
		do
			return
		end
	...
	PlayEffect(audioCfg.mouseClick)
	local version=1
	if acDanrichongzhiVoApi:getVersion()==1 then
		version=1
	end
	local td=smallDialog:new()
	local tabStr = {getlocal("activity_xiaofeisongli_tip4_" .. version),getlocal("activity_danrixiaofei_tip3_" .. version), getlocal("activity_danrixiaofei_tip2_" .. version),getlocal("activity_danrichongzhi_tip1_" .. version),"\n"}
	local colorTb = {nil,G_ColorWhite,G_ColorWhite,G_ColorWhite,G_ColorWhite,G_ColorWhite,nil}

	local flag = false
	local selectRew = acDanrichongzhiVoApi:getR1() -- 可选奖励
...
function acDanrichongzhiDialog:refresh()
	self.totalMoney = acDanrichongzhiVoApi:getAlreadyCost()
...
function acDanrichongzhiDialog:tick()
    local vo=acDanrichongzhiVoApi:getAcVo()
    if activityVoApi:isStart(vo) == false then -- 活动突然结束了并且当前板子还打开着，就要关闭板子
     ...
     if acDanrichongzhiVoApi:isToday()==false then
    	acDanrichongzhiVoApi:refresh()
    	self:refresh()
    end

     if self.totalMoney and self.totalMoney ~= acDanrichongzhiVoApi:getAlreadyCost() then
    	self:refresh()
    end
...
function acDanrichongzhiDialog:dispose()
	self.dangci=nil
