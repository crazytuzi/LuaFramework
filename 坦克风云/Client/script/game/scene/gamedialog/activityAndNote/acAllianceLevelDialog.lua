acAllianceLevelDialog=commonDialog:new()

function acAllianceLevelDialog:new(layerNum)
    local nc={}
...
function acAllianceLevelDialog:resetTab()
    local index=0
...
--设置对话框里的tableView
function acAllianceLevelDialog:initTableView()
...
function acAllianceLevelDialog:eventHandler(handler,fn,idx,cel)
...
--点击tab页签 idx:索引
function acAllianceLevelDialog:tabClick(idx)
    ...
    if idx==1 then
        ...
        if self.rankLayer==nil then
            self.rankTab=acAllianceLevelTab2:new()
    ...
    elseif idx==0 then
        ...
        if self.desLayer==nil then
            self.desTab=acAllianceLevelTab1:new()
...
function acAllianceLevelDialog:tick()
  if self and self.desTab and self.desTab.tick then
      self.desTab:tick()
  end
  if acAllianceLevelVoApi:getEndTime() < base.serverTime then
    do 
      return
    end
  end
  local vo = acAllianceLevelVoApi:getAcVo()
  if acAllianceLevelVoApi.lastSt + 300 < base.serverTime and self.getTimes <= 2 then
      local function getList(fn,data)
        ...
        if ret==true then
           ...
           if sData ~= nil then
              acAllianceLevelVoApi:updateRankList(sData, true)
              acAllianceLevelVoApi:setLastSt()
              self.getTimes = 0
              self:update()
           end
      ...
      if selfAlliance ~= nil then
        print("*******acAllianceLevelDialog:tick2**********")
...
function acAllianceLevelDialog:update()
...
function acAllianceLevelDialog:dispose()
    if self.desTab~=nil then
        self.desTab:dispose()
    end
