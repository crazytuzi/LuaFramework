...
function acYijizaitanTab2:init(layerNum)
...
function acYijizaitanTab2:initLayer()
  self.aidTb,self.tankidTb=acYijizaitanVoApi:getTankID()
  self.cellNum=SizeOfTable(acYijizaitanVoApi:getRefitTankNeedCfg())
  self:initTableView()
end

function acYijizaitanTab2:initTableView()
...
function acYijizaitanTab2:eventHandler(handler,fn,idx,cel)
...
function acYijizaitanTab2:loadCCTableViewCell(cell,idx)
...
function acYijizaitanTab2:exbgCellForId(container,idx)
  local addH=11;
  local reR1,reR2,reR3,reR4,reUpgradedMoney = acYijizaitanVoApi:getUpgradedTankResources(self.aidTb[idx+1],self.tankidTb[idx+1])
  ...
  local UpgradePropConsume = acYijizaitanVoApi:getUpgradePropConsume(self.aidTb[idx+1],self.tankidTb[idx+1])
  ...
  local needTankIDTb,needTankNumTb = acYijizaitanVoApi:getRefitNeedTankIDAndNum(self.aidTb[idx+1],self.tankidTb[idx+1])
  local haveTankNum
  local haveTankNum1
  ...
  minusSp:setTouchPriority(-(self.layerNum-1)*20-4);
    ...
    local function serverUpgrade(fn,data)
      ...
      if ret==true then
        ...
        local str = getlocal("activity_yijizaitan_title")
    ...
    socketHelper:activityYijizaitanRefitTank(nums,self.aidTb[idx+1],serverUpgrade)
...
function acYijizaitanTab2:cellClick(idx)
    if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
        PlayEffect(audioCfg.mouseClick)
...
function acYijizaitanTab2:moreOrless(idx)
  -- 四种资源
  local reR1,reR2,reR3,reR4,reUpgradedMoney = acYijizaitanVoApi:getUpgradedTankResources(self.aidTb[idx+1],self.tankidTb[idx+1])
...
function acYijizaitanTab2:refresh()
...
function acYijizaitanTab2:dispose()
  self.bgLayer=nil
    self.layerNum=nil
