acRoulette2DialogTab2={}

function acRoulette2DialogTab2:new()
    local nc={}
...
function acRoulette2DialogTab2:init(layerNum,selectedTabIndex,acRouletteDialog)
...
--设置对话框里的tableView
function acRoulette2DialogTab2:initTableView()
    ...
    local function rewardHandler()
        local rank=acRoulette2VoApi:rankCanReward()
        local function wheelfortuneCallback(fn,data)
            ...
            if ret==true then
                ...
                acRoulette2VoApi:setListRewardNum()
                self:refresh()
            elseif sData.ret==-1975 then
                ...
                socketHelper:activeWheelfortune2(2,getListCallback)
            end
        ...
        -- end
        if rank>0 then
            socketHelper:activeWheelfortune2(4,wheelfortuneCallback,rank)
        end
    end
    ...
    if acRoulette2VoApi:rankCanReward()>0 then
...
function acRoulette2DialogTab2:eventHandler(handler,fn,idx,cel)
	...
	elseif fn=="tableCellAtIndex" then
        ...
        if idx==0 then
        	rank=acRoulette2VoApi:getSelfRank()
...
function acRoulette2DialogTab2:doUserHandler()
    local capInSet = CCRect(20, 20, 10, 10);
    local function bgClick(hd,fn,idx)
    end
    ...
    local cfg1=acRoulette2VoApi:getRouletteCfg()
...
function acRoulette2DialogTab2:tick()
end

function acRoulette2DialogTab2:subtick()
end

function acRoulette2DialogTab2:refresh()
    if self and self.bgLayer then
        local vo=acRoulette2VoApi:getAcVo()
        if self.descLb then
...
function acRoulette2DialogTab2:dispose()
