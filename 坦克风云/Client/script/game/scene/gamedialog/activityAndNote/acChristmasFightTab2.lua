acChristmasFightTab2={}

function acChristmasFightTab2:new()
    local nc={}
...
function acChristmasFightTab2:init(layerNum,selectedTabIndex,parentDialog)
...
--设置对话框里的tableView
function acChristmasFightTab2:initTableView()
    ...
    local pointLabel=GetTTFLabel(getlocal("alliance_contribution"),22)
...
function acChristmasFightTab2:eventHandler(handler,fn,idx,cel)
...
function acChristmasFightTab2:doUserHandler()
    local acVo=acChristmasFightVoApi:getAcVo()
    local capInSet = CCRect(20, 20, 10, 10);
    local function bgClick(hd,fn,idx)
    end
    ...
    self.descLb=GetTTFLabelWrap(getlocal("activity_christmasfight_crank_desc",{acVo.cRankLimit}),25,CCSizeMake(titleBg:getContentSize().width-100,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
    ...
    -- self.descLb1=GetTTFLabelWrap(getlocal("allianceShop_myDonate")..acVo.cPoint,25,CCSizeMake(titleBg:getContentSize().width-100,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
    ...
    self.descLb1=GetTTFLabel(getlocal("allianceShop_myDonate")..acVo.cPoint,25)
    ...
    local function onClickDesc()
        ...
        local tabStr={" "}
        for i=1,3 do
            table.insert(tabStr,getlocal("activity_christmasfight_crank_tip"..i))
        end
        local tabColor={}
        local acVo=acChristmasFightVoApi:getAcVo()
        local rankRewardCfg=acVo.cRankRewardCfg
        local rewardStrTab={}
...
function acChristmasFightTab2:refresh()
    if self then
...
function acChristmasFightTab2:tick()
    if self and self.descLb1 then
        local acVo=acChristmasFightVoApi:getAcVo()
        if acVo then
            self.descLb1:setString(getlocal("allianceShop_myDonate")..acVo.cPoint)
        end
    end
end

function acChristmasFightTab2:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
