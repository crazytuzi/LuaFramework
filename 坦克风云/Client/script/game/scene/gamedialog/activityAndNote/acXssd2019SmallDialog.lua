--世界地图点击叛军弹出的叛军面板
acXssd2019SmallDialog=smallDialog:new()

--param data: 数据vo,worldBaseVo
function acXssd2019SmallDialog:new(data,sender,corpLimit,acType)
	local nc={}
...
function acXssd2019SmallDialog:init(layerNum)
	local subP = 10
	...
	if self.acType == "xssd2019" then
		upSenderLbStr = getlocal("activity_xssd2019_RedBagCorpSenderLb",{self.sender})
		titleLBStr =getlocal("activity_xssd2019_RedBagCorp")
	end
...
function acXssd2019SmallDialog:eventHandler( handler,fn,idx,cel)
	local strSize3 = 20
	local varWidht = 10
	if G_getCurChoseLanguage()=="cn" or G_getCurChoseLanguage()=="tw" or G_getCurChoseLanguage()=="ja" or G_getCurChoseLanguage()=="ko" then
		strSize3 =24
...
function acXssd2019SmallDialog:dispose()
	self.tv =nil 
