...
function acMineExploreGShop:new(closeCallback)
    local nc={}
    nc.cellHeight=180
    nc.cellWidth=0
    nc.cellNum=0
...
function acMineExploreGShop:init(layerNum,parent)
    self.bgLayer=CCLayer:create()
    self.layerNum=layerNum
    self.parent=parent
    self.cellWidth=self.bgLayer:getContentSize().width-40
    self.saleList={}
    self.shopCfg=acMineExploreGVoApi:getShopCfg()
    self.shopIndexTb=acMineExploreGVoApi:getShopIndexTb() or {}
...
function acMineExploreGShop:initTableView()
    ...
    local version=acMineExploreGVoApi:getVersion()
    local descStr=""
    if tonumber(version)>1 then
        descStr=getlocal("activity_mineExploreG_shopDes"..version)
    else
        descStr=getlocal("activity_mineExploreG_shopDes")
    end
    local desTv,desLabel=G_LabelTableView(CCSizeMake(backSprie:getContentSize().width-220,110),descStr,25,kCCTextAlignmentLeft)
...
function acMineExploreGShop:showInfor()
    local tabStr={}
    local tabColor={}
    local tabAlignment={}
    tabStr={"\n",getlocal("activity_mineExploreG_shopRule3"),"\n",getlocal("activity_mineExploreG_shopRule2"),"\n",getlocal("activity_mineExploreG_shopRule1"),"\n",getlocal("activityDescription"),"\n"}
...
function acMineExploreGShop:eventHandler(handler,fn,idx,cel)
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        local ownNum=acMineExploreGVoApi:getMyPoint()
        ...
        local cur=acMineExploreGVoApi:getBuyData(saleId) or 0
            ...
            if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
                ...
                local function callback()
                    ...
                    if saleCfg.notice and saleCfg.notice==1 then
                        acMineExploreGVoApi:sendRewardNotice(2,saleItem)
                    end
                end
                acMineExploreGVoApi:mineExploreGRequest("active.mineexploreg.shop",{saleId},callback)
            end
        end
...
function acMineExploreGShop:refresh()
    local num=acMineExploreGVoApi:getMyPoint()
    if self.numLb then
        self.numLb:setString(getlocal("propInfoNum",{num}))
    end
    self.shopIndexTb=acMineExploreGVoApi:getShopIndexTb()
...
function acMineExploreGShop:updateUI()
    self:refresh()
end

function acMineExploreGShop:tick()
end

function acMineExploreGShop:dispose()
    self.cellHeight=180
    self.cellWidth=0
    self.cellNum=0
    self.shopCfg=nil
