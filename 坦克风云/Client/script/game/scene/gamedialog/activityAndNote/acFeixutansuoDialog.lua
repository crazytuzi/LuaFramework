acFeixutansuoDialog=commonDialog:new()

function acFeixutansuoDialog:new()
	local nc={}
...
function acFeixutansuoDialog:resetTab()
	local index=0
...
function acFeixutansuoDialog:initTableView()
    ...
    if acFeixutansuoVoApi:getRewardList()==nil then
    	socketHelper:activityFeixutansuoRewardList(callback)
    end
	self:tabClick(0,false)
end
function acFeixutansuoDialog:tabClick(idx,isEffect)
	if(isEffect)then
	...
	if(idx==0)then
		if(self.acTab1==nil)then
			if base.mustmodel==1 and acFeixutansuoVoApi:getMustMode() then
                require "luascript/script/game/scene/gamedialog/activityAndNote/acFeixutansuoNewTab1"
                self.acTab1=acFeixutansuoNewTab1:new()
            else
                self.acTab1=acFeixutansuoTab1:new()
            end
	...
	elseif(idx==1)then
		if(self.acTab2==nil)then
			self.acTab2=acFeixutansuoTab2:new()
...
function acFeixutansuoDialog:tick()
	local vo=acFeixutansuoVoApi:getAcVo()
...
function acFeixutansuoDialog:update()
	if self and self.bgLayer and self.acTab1 and self.layerTab1 then 
		self.acTab1:updateShowTv()
	end
end
function acFeixutansuoDialog:dispose()
	if self.layerTab1 then
		self.acTab1:dispose()
	end
	if self.layerTab2 then
