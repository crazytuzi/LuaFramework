acRouletteDialogTab1={}

function acRouletteDialogTab1:new()
    local nc={}
...
function acRouletteDialogTab1:init(layerNum,selectedTabIndex,acRouletteDialog)
...
--初始化上半部的今日抽奖信息
function acRouletteDialogTab1:initDesc()
...
function acRouletteDialogTab1:eventHandler(handler,fn,idx,cel)
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        self.descLb1=GetTTFLabel(getlocal("activity_wheelFortune_reward_count",{acRouletteVoApi:getUsedNum()}),25)
...
function acRouletteDialogTab1:getCellHeight()
    local vo=acRouletteVoApi:getAcVo()
    -- self.descLb=GetTTFLabel(getlocal("activity_wheelFortune_cost"),25)
    local numLb=GetTTFLabel(vo.consume or 0,30)
    local descLb1=GetTTFLabel(getlocal("activity_wheelFortune_reward_count",{acRouletteVoApi:getUsedNum()}),25)
...
--初始化赌博机
function acRouletteDialogTab1:initRoulette()
    local rouletteCfg=acRouletteVoApi:getRouletteCfg()
    ...
    -- btnBg:setContentSize(CCSizeMake(iconWidth*2,iconHeight*2))
    btnBg:setContentSize(CCSizeMake(iconWidth*2,230))
    btnBg:setAnchorPoint(ccp(0.5,0.5))
    -- btnBg:setPosition(ccp(G_VisibleSize.width/2,(G_VisibleSize.height-85-80-100)/2))
    btnBg:setPosition(ccp(G_VisibleSize.width/2,290))
    btnBg:setTouchPriority(0)
    self.bgLayer:addChild(btnBg)
    self.playBtnBg=btnBg

    local leftChips=GetTTFLabelWrap(getlocal("activity_wheelFortune_rest_count"),28,CCSizeMake(btnBg:getContentSize().width-20,0),kCCTextAlignmentCenter,kCCVerticalTextAlignmentTop)
    leftChips:setAnchorPoint(ccp(0.5,0.5));
    leftChips:setPosition(ccp(btnBg:getContentSize().width/2,btnBg:getContentSize().height-leftChips:getContentSize().height/2-15));
    btnBg:addChild(leftChips,2);

    local leftNum=acRouletteVoApi:getLeftNum()
    self.chipNumLb=GetTTFLabel(leftNum,30)
    self.chipNumLb:setAnchorPoint(ccp(0.5,0));
    self.chipNumLb:setPosition(ccp(btnBg:getContentSize().width/2,btnBg:getContentSize().height-leftChips:getContentSize().height-self.chipNumLb:getContentSize().height-20));
    ...
    local function onPlay()
        ...
        if (acRouletteVoApi:checkCanPlay())==true then
            ...
            local function wheelfortuneCallback(fn,data)
                ...
                if ret==true then
                    if self and self.bgLayer then
                        if sData and sData.data and sData.data.wheelFortune and sData.data.wheelFortune.active then
                            local vo=acRouletteVoApi:getAcVo()
                            local oldPoint=vo.point

                            local updateData=sData.data.wheelFortune.active
                            acRouletteVoApi:updateData(updateData)
                            if self and self.acRouletteDialog then
                                self.acRouletteDialog:refresh()
                            end

                            local newVo=acRouletteVoApi:getAcVo()
                        ...
                        if sData and sData.data and sData.data.reward then
                            self.reward=FormatItem(sData.data.reward) or {}
            ...
            socketHelper:activeWheelfortune(1,wheelfortuneCallback)
        end
    end
    local textSize = 28
    ...
    playBtnMenu:setPosition(ccp(btnBg:getContentSize().width/2,25))
...
function acRouletteDialogTab1:initRewardIcon(iconBg,i,item)
...
function acRouletteDialogTab1:play()
    self.tickIndex=0
    self.tickInterval=3
    self.tickConst=3
    ...
    for k,v in pairs(self.rewardList) do
        if self.rewardList and v and v.key==self.reward[1].key then
            self.endIdx=k
        end
    end
...
function acRouletteDialogTab1:fastTick()
...
function  acRouletteDialogTab1:playEndEffect()
...
function acRouletteDialogTab1:tick()
end

function acRouletteDialogTab1:subtick()
end

function acRouletteDialogTab1:refresh()
    if self and self.bgLayer then
        local leftNum=acRouletteVoApi:getLeftNum()
        ...
        if self.playBtn and (acRouletteVoApi:checkCanPlay())==true then
            self.playBtn:setEnabled(true)
        else
            self.playBtn:setEnabled(false)
        end
        ...
        if self.titleBg then
            ...
            if self.descLb1 then
                local usedNum=acRouletteVoApi:getUsedNum()
                if usedNum then
...
function acRouletteDialogTab1:dispose()
    --base:removeFromNeedRefresh(self)
    if self.touchEnabledSp then
