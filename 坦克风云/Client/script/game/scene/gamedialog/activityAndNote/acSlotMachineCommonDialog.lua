acSlotMachineCommonDialog=commonDialog:new()

function acSlotMachineCommonDialog:new()
    local nc={}
...
function acSlotMachineCommonDialog:initTableView()
  ...
  girlDescBg:setPosition(200,G_VisibleSizeHeight - 330)
...
function acSlotMachineCommonDialog:desEventHandler(handler,fn,idx,cel)
    ...
    elseif fn=="tableCellSizeForIndex" then
        ...
        local descStr 
        local version = acSlotMachineCommonVoApi:getCfgVersion()
        print("version",version)
        if version then
          if version==8 then
            descStr=getlocal("activity_slotMachine_desc")
          else
            descStr=getlocal("activity_slotMachine2_desc")
          end
        end

        local descLb=GetTTFLabelWrap(descStr,25,CCSizeMake(350,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
...
function acSlotMachineCommonDialog:eventHandler(handler,fn,idx,cel)
  if fn=="numberOfCellsInTableView" then
    return 1
  elseif fn=="tableCellSizeForIndex" then
  ...
  elseif fn=="tableCellAtIndex" then
    ...
    for i=1,3 do
        local startId = acSlotMachineCommonVoApi:getLastResultByLine(i)
        for i2=1,2 do
          if startId == 1 then
            startId = 5
          else
        ...
        for id=1,self.turnNum do
          ...
          if startId == self.turnNum then
            pic = acSlotMachineCommonVoApi:getPicById(1)
          else
            pic = acSlotMachineCommonVoApi:getPicById(startId)
          end
...
function acSlotMachineCommonDialog:iconFlicker(icon)
...
-- 删除所有的边框效果
function acSlotMachineCommonDialog:resetMetalSps()
...
function acSlotMachineCommonDialog:initSp()
  
end

function acSlotMachineCommonDialog:moveSp(tb)
...
function acSlotMachineCommonDialog:fuwei(key,tb)
...
function acSlotMachineCommonDialog:result(tb)
  self.spTb1Speed=0
  self.spTb2Speed=0
  self.spTb3Speed=0
  self.isStop1=true
...
function acSlotMachineCommonDialog:fastTick()
  if self.state == 2 then
...
function acSlotMachineCommonDialog:startPalyAnimation()
...
function acSlotMachineCommonDialog:stopPlayAnimation()
  print("正常~")
  self.state = 0
  self.lastSt = 0 -- 上一次设置动画时间点（用于计算tick中设置动画播放时间间隔）
  ...
  acSlotMachineCommonVoApi:afterGameOver()
  self.touchDialogBg:setIsSallow(false) -- 点击事件透下去
end

-- 把手播放动画
function acSlotMachineCommonDialog:barPaly()
  if self ~= nil then
...
function acSlotMachineCommonDialog:stopBarPlay()
  self.leftIcon:setVisible(false)
  self.rightIcon:setVisible(false)
end

-- 抽取奖励
function acSlotMachineCommonDialog:getReward()
  self.state = 1
  local free = nil
  local num = nil
  local cost = nil
  if acSlotMachineCommonVoApi:checkIfFreeGame() == true then
    free = 0
    num = 1
    cost = 0
  else
    ...
    if self.selectMul == true then
      num = acSlotMachineCommonVoApi:getCfgMul()
      cost = acSlotMachineCommonVoApi:getMulCost()
    else
      num = 1
      cost = acSlotMachineCommonVoApi:getCfgCost()
    end
  end
      ...
      local function getRawardCallback(fn,data)
        ...
        if ret==true then
          ...
          --base:formatPlayerData(data)这个方法同步数据时有不同数据用了同一个标识的情况
            if sData.data ~= nil and sData.data.slotMachineCommon ~= nil and sData.data.slotMachineCommon.show ~= nil and sData.data.slotMachineCommon.free ~= nil and sData.data.slotMachineCommon.num ~= nil then
              self.playIds = sData.data.slotMachineCommon.show
              acSlotMachineCommonVoApi:updateLastResult(self.playIds)
              self:addRewardAndCostMoney(sData.data.slotMachineCommon.free,sData.data.slotMachineCommon.num)
              self:startPalyAnimation()
            end
        end
      end
      socketHelper:getSlotMachineCommonReward(free,num,getRawardCallback)
    end
  end
...
function acSlotMachineCommonDialog:addRewardAndCostMoney(free,num)
  ...
  if free == 0 then -- 免费的
    ...
    if  num > 1 then
      aNum = acSlotMachineCommonVoApi:getCfgMul()
      aCost = acSlotMachineCommonVoApi:getMulCost()
    else
      aNum = 1
      aCost = acSlotMachineCommonVoApi:getCfgCost()
    end
  end
  self.lastMul = aNum
  ...
  for k,v in pairs(getTable) do
    if v ~= nil and v > 0 then
      local cfg = acSlotMachineCommonVoApi:getCfgConversionTableByIdAndNum(tonumber(k),tonumber(v))
...
function acSlotMachineCommonDialog:resetData(data)
  local getTable = {}
...
-- 处理后台得到返回抽取结构后前台的处理
function acSlotMachineCommonDialog:aftetGetReward(getTable)
  ...
  for k,v in pairs(getTable) do
    if v ~= nil and v > 0 then
      ...
      if cfg ~= nil then
        -- 根据最终获得奖励的特效处理
        if tonumber(v)==2 or tonumber(v)==3 then
          -- 抽中一样的图标，图标有金框特效。
          ...
          if tonumber(v)==3 then
            local tank, tankNum = acSlotMachineCommonVoApi:getTankCfgAndNumByCfg(cfg.reward.o[1])
            self:playParticles()
...
function acSlotMachineCommonDialog:playParticles()
    --粒子效果
  self.particleS = {}
  local pX = nil
  local PY = nil
...
function acSlotMachineCommonDialog:removeParticles()
...
function acSlotMachineCommonDialog:doUserHandler()
  ...
  self.mulDesc=GetTTFLabelWrap(getlocal("activity_slotMachine_selectLb",{acSlotMachineCommonVoApi:getCfgMul()}),25,CCSizeMake(G_VisibleSizeWidth - mulX-10,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
  ...
  self.rewardBtn=GetButtonItem("BtnRecharge.png","BtnRecharge.png","BtnRecharge.png",touch,2,getlocal("activity_slotMachine_free"),28,11)
  self.rewardBtn:setAnchorPoint(ccp(0.5, 0))
  local menuAward=CCMenu:createWithItem(self.rewardBtn)
  menuAward:setPosition(ccp(costX,30))
...
-- 选择或取消收益倍数后相关ui刷新
function acSlotMachineCommonDialog:updateBySelectMul()
...
-- 刷新领奖按钮
function acSlotMachineCommonDialog:updateRewardBtn()
...
function acSlotMachineCommonDialog:openInfo()
  local td=smallDialog:new()
  local tabStr = {"\n",getlocal("activity_slotMachine_rule4",{acSlotMachineCommonVoApi:getCfgMul(),acSlotMachineCommonVoApi:getCfgMulCost(),acSlotMachineCommonVoApi:getCfgMul()}),"\n",getlocal("activity_slotMachine_rule3"),"\n",getlocal("activity_slotMachine_rule2"),"\n", getlocal("activity_slotMachine_rule1",{acSlotMachineCommonVoApi:getCfgFree()}),"\n"}
...
function acSlotMachineCommonDialog:update()
  local acVo = acSlotMachineCommonVoApi:getAcVo()
  if acVo ~= nil then
...
function acSlotMachineCommonDialog:dispose()
  self.costLabel = nil
  self.rewardBtn = nil
