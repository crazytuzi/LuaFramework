acChallengeranknewDialog=commonDialog:new()

function acChallengeranknewDialog:new(layerNum)
    local nc={}
...
function acChallengeranknewDialog:resetTab()
    local index=0
    local spaceH=80--190
    for k,v in pairs(self.allTabs) do
         local  tabBtnItem=v
         if index==0 then
          tabBtnItem:setPosition(tabBtnItem:getContentSize().width/2+20,self.bgSize.height-tabBtnItem:getContentSize().height/2-spaceH)
         elseif index==1 then
          tabBtnItem:setPosition(tabBtnItem:getContentSize().width/2+24+tabBtnItem:getContentSize().width,self.bgSize.height-tabBtnItem:getContentSize().height/2-spaceH)
         end
...
--设置对话框里的tableView
function acChallengeranknewDialog:initTableView()
    local function callBack(...)
       return self:eventHandler(...)
    end
    local hd= LuaEventHandler:createHandler(callBack)
    self.panelLineBg:setAnchorPoint(ccp(0.5, 0))
    self.panelLineBg:setContentSize(CCSizeMake(G_VisibleSizeWidth-20, G_VisibleSizeHeight - 275 + 110))
...
function acChallengeranknewDialog:eventHandler(handler,fn,idx,cel)
...
--点击tab页签 idx:索引
function acChallengeranknewDialog:tabClick(idx)
    ...
    if idx==1 then
        ...
        if self.rankLayer==nil then
            self.rankTab=acChallengeranknewTab2:new()
    ...
    elseif idx==0 then
        ...
        if self.desLayer==nil then
            self.desTab=acChallengeranknewTab1:new()
...
function acChallengeranknewDialog:doUserHandler()
...
function acChallengeranknewDialog:tick()
  local acVo = acChallengeranknewVoApi:getAcVo()
  if activityVoApi:isStart(acVo)==true then
      -- if acChallengeranknewVoApi:getEndTime() < base.serverTime then
      --   do 
      --     return
      --   end
      -- end
      local oldStarNum = acChallengeranknewVoApi:getSelfStarNum()
      local newStarNum = checkPointVoApi:getStarNum()
      if (acChallengeranknewVoApi.lastSt + 300 < base.serverTime or oldStarNum ~= newStarNum) and self.getTimes <= 2 then
          local function getList(fn,data)
            ...
            if ret==true then
               ...
               if sData ~= nil then
                  acChallengeranknewVoApi:updateRankList(sData)
                  acChallengeranknewVoApi:setLastSt()
                  self.getTimes = 0
                  self:update()
               end
...
function acChallengeranknewDialog:update()
  local acVo = acChallengeranknewVoApi:getAcVo()
  if acVo ~= nil then
...
function acChallengeranknewDialog:dispose()
    if self.desTab~=nil then
        self.desTab:dispose()
    end
