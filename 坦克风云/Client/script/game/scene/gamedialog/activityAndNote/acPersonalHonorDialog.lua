acPersonalHonorDialog=commonDialog:new()

function acPersonalHonorDialog:new(layerNum)
    local nc={}
...
function acPersonalHonorDialog:resetTab()
    local index=0
...
--设置对话框里的tableView
function acPersonalHonorDialog:initTableView()
...
function acPersonalHonorDialog:eventHandler(handler,fn,idx,cel)
...
--点击tab页签 idx:索引
function acPersonalHonorDialog:tabClick(idx)
    ...
    if idx==1 then
        ...
        if self.rankLayer==nil then
            self.rankTab=acPersonalHonorTab2:new()
    ...
    elseif idx==0 then
        ...
        if self.desLayer==nil then
            self.desTab=acPersonalHonorTab1:new()
...
function acPersonalHonorDialog:doUserHandler()
...
function acPersonalHonorDialog:tick()
  if acPersonalHonorVoApi:getEndTime() < base.serverTime then
    do 
      return
    end
  end
  local old = acPersonalHonorVoApi:getSelfCredit()
  local new = playerVoApi:getCredit()
  if (acPersonalHonorVoApi.lastSt + 300 < base.serverTime or old ~= new) and self.getTimes <= 2 then
      local function getList(fn,data)
        ...
        if ret==true then
           ...
           if sData ~= nil then
              acPersonalHonorVoApi:updateRankList(sData)
              acPersonalHonorVoApi:setLastSt()
              self.getTimes = 0
              self:update()
           end
...
function acPersonalHonorDialog:update()
  local acVo = acPersonalHonorVoApi:getAcVo()
  if acVo ~= nil then
...
function acPersonalHonorDialog:dispose()
    if self.desTab~=nil then
        self.desTab:dispose()
    end
