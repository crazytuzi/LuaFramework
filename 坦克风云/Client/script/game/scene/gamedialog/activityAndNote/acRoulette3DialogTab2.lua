acRoulette3DialogTab2={}

function acRoulette3DialogTab2:new()
    local nc={}
...
function acRoulette3DialogTab2:init(layerNum,selectedTabIndex,acRoulette3Dialog)
...
--设置对话框里的tableView
function acRoulette3DialogTab2:initTableView()
	...
	local pointLabel=GetTTFLabel(getlocal("activity_wheelFortune_subTitle_3"),22)
    ...
    local function rewardHandler()
        local rank=acRoulette3VoApi:rankCanReward()
        local function wheelfortuneCallback(fn,data)
            ...
            if ret==true then
                ...
                acRoulette3VoApi:setListRewardNum()
                self:refresh()
            elseif sData.ret==-1975 then
                ...
                socketHelper:activeWheelfortune3(2,getListCallback)
            end
        ...
        -- end
        if rank>0 then
            socketHelper:activeWheelfortune3(4,wheelfortuneCallback,rank)
        end
    end
    ...
    if acRoulette3VoApi:rankCanReward()>0 then
...
function acRoulette3DialogTab2:eventHandler(handler,fn,idx,cel)
	...
	elseif fn=="tableCellAtIndex" then
        ...
        if idx==0 then
        	rank=acRoulette3VoApi:getSelfRank()
...
function acRoulette3DialogTab2:doUserHandler()
    local capInSet = CCRect(20, 20, 10, 10);
    local function bgClick(hd,fn,idx)
    end
    ...
    local cfg1=acRoulette3VoApi:getRouletteCfg()
...
function acRoulette3DialogTab2:tick()
end

function acRoulette3DialogTab2:subtick()
end

function acRoulette3DialogTab2:refresh()
    if self and self.bgLayer then
        local vo=acRoulette3VoApi:getAcVo()
        if self.descLb then
...
function acRoulette3DialogTab2:dispose()
