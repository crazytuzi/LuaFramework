...
function acEquipSearchTab2:new()
    local nc={}
...
function acEquipSearchTab2:init(layerNum,selectedTabIndex,acEquipSearchDialog)
...
--设置对话框里的tableView
function acEquipSearchTab2:initTableView()
    ...
    local function rewardHandler()
        local rank=acEquipSearchVoApi:rankCanReward()
        local function equipsearchRewardCallback(fn,data)
            ...
            if ret==true then
                ...
                acEquipSearchVoApi:setListRewardNum()
                self:refresh()
            elseif sData.ret==-1975 or sData.ret==-1976 then
                ...
                socketHelper:activeEquipsearch(2,getListCallback)
            end
        end
        if rank>0 then
            socketHelper:activeEquipsearch(3,equipsearchRewardCallback,nil,rank)
        end
    end
    ...
    if acEquipSearchVoApi:rankCanReward()>0 then
...
function acEquipSearchTab2:eventHandler(handler,fn,idx,cel)
	...
	elseif fn=="tableCellAtIndex" then
        ...
        if idx==0 then
        	rank=acEquipSearchVoApi:getSelfRank()
...
function acEquipSearchTab2:doUserHandler()
    local capInSet = CCRect(20, 20, 10, 10);
    local function bgClick(hd,fn,idx)
    end
    ...
    local cfg=acEquipSearchVoApi:getEquipSearchCfg()
    self.descLb1=GetTTFLabelWrap(getlocal("activity_wheelFortune_rank_point",{cfg.rankPoint}),25,CCSizeMake(titleBg:getContentSize().width-100,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
    ...
    local function onClickDesc()
        ...
        local cfg=activityCfg.equipSearch
        local rewardStrTab={}
...
function acEquipSearchTab2:refresh()
    if self and self.bgLayer then
        local vo=acEquipSearchVoApi:getAcVo()
        if self.descLb then
...
function acEquipSearchTab2:dispose()
