...
function acJsysDialogTabTwo:new()
    local nc={}
...
function acJsysDialogTabTwo:init(layerNum)
    self.layerNum=layerNum
    self.rankList = acJsysVoApi:getPlayerList( )
    self.rankAwardList = acJsysVoApi:getRankAwardList()
...
function acJsysDialogTabTwo:refresh( )
    self.rankList = acJsysVoApi:getPlayerList( )
    tolua.cast(self.bgLayer:getChildByTag(111),"CCLabelTTF"):setString(getlocal("dailyAnswer_tab1_recentLabelNum",{acJsysVoApi:getScore()}))
    self.tv:reloadData()
end

function acJsysDialogTabTwo:initTableView(  )
	self.tvHeight=self.bgLayer:getContentSize().height-380
...
function acJsysDialogTabTwo:eventHandler(handler,fn,idx,cel)
    if fn=="numberOfCellsInTableView" then
        local num=1
	    local playerList=acJsysVoApi:getPlayerList()
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        local rankList=acJsysVoApi:getPlayerList()
        ...
        if idx==0 then
            rank=acJsysVoApi:getSelfPos()
            name=playerVoApi:getPlayerName()
            point=acJsysVoApi:getScore()
...
function acJsysDialogTabTwo:initLayer( )
    ...
    local score = acJsysVoApi:getScore()
    ...
    local floor = acJsysVoApi:getScoreFloor( )
    ...
    local function touch(tag,object)
        ...
        local tabStr=acJsysVoApi:getRankInfoStr()
    ...
    local function onClickDesc()
        local isReaward = acJsysVoApi:isReaward( )
        if acJsysVoApi:getedBigAward() ~=nil then
        ...
        if isReaward ==true and acJsysVoApi:acIsStop() ==true then
            self:getBigReward()
        end
    end
...
function acJsysDialogTabTwo:getBigReward( )
    local function getRanklist(fn,data)
        ...
        local bigAwardIdx = acJsysVoApi:getSelfPos()
	    local function getBigAwardCall(fn,data)
	        local ret,sData = base:checkServerData(data)
	        if ret==true then
	            acJsysVoApi:getAndShowBigAward(bigAwardIdx)
	            self.bigAwardClick:setEnabled(false)
	        end
	    end
	    socketHelper:acJsysRequest({action=4,rank=bigAwardIdx},getBigAwardCall)

      end
    socketHelper:acJsysRequest({action=2},getRanklist)
end

function acJsysDialogTabTwo:tick( )
    if acJsysVoApi:getedBigAward() then
        self.bigAwardClick:setEnabled(false)
    elseif acJsysVoApi:acIsStop() ==true and acJsysVoApi:isReaward() ==true and acJsysVoApi:getedBigAward( ) == nil then
        self.bigAwardClick:setEnabled(true)
    end
end

function acJsysDialogTabTwo:dispose( )
