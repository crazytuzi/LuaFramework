acRoulette4DialogTab1={}

function acRoulette4DialogTab1:new()
    local nc={}
...
function acRoulette4DialogTab1:init(layerNum,selectedTabIndex,acRoulette4Dialog)
...
--初始化上半部的今日抽奖信息
function acRoulette4DialogTab1:initDesc()
...
function acRoulette4DialogTab1:eventHandler(handler,fn,idx,cel)
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        self.descLb1=GetTTFLabel(getlocal("activity_wheelFortune_reward_count",{acRoulette4VoApi:getUsedNum()}),25)
        ...
        local timeTab=acRoulette4VoApi:getTimeTab()
        table.insert(timeTab,rouletteCfg.lotteryConsume)
        local descStr=getlocal("activity_wheelFortune4_desc_1",timeTab)
...
function acRoulette4DialogTab1:getCellHeight()
    -- local vo=acRoulette4VoApi:getAcVo()
    -- self.descLb=GetTTFLabel(getlocal("activity_wheelFortune_cost"),25)
    -- local numLb=GetTTFLabel(vo.consume or 0,30)
    local descLb1=GetTTFLabel(getlocal("activity_wheelFortune_reward_count",{acRoulette4VoApi:getUsedNum()}),25)
    -- local descStr=getlocal("activity_wheelFortune_tip_1")
    local rouletteCfg=acRoulette4VoApi:getRouletteCfg()
    local timeTab=acRoulette4VoApi:getTimeTab()
    table.insert(timeTab,rouletteCfg.lotteryConsume)
    local descStr=getlocal("activity_wheelFortune4_desc_1",rouletteCfg.lotteryConsume)
...
--初始化赌博机
function acRoulette4DialogTab1:initRoulette()
    local rouletteCfg=acRoulette4VoApi:getRouletteCfg()
    ...
    local leftNum=acRoulette4VoApi:getLeftNum()
    ...
    local function onPlay()
        ...
        if (acRoulette4VoApi:checkCanPlay())==true then
            -- self.touchEnabledSp:setVisible(true)
            -- self.touchEnabledSp:setPosition(ccp(0,0))
            -- if self.acRoulette4Dialog then
            --     self.acRoulette4Dialog.canClickTab=false
            -- end
            local function wheelfortuneCallback(fn,data)
                ...
                if ret==true then
                    local leftNum=acRoulette4VoApi:getLeftNum()
                    local coinNum=acRoulette4VoApi:getCoinNum()
                    if leftNum==0 and coinNum>0 then
                        acRoulette4VoApi:useCoin(1)
                    end
                    ...
                    if self and self.bgLayer then
                        if sData and sData.data and sData.data.wheelFortune4 and sData.data.wheelFortune4.active then
                            ...
                            if self and self.acRoulette4Dialog then
                                self.acRoulette4Dialog:refresh()
                            end

                            -- local newVo=acRoulette4VoApi:getAcVo()
                        ...
                        if sData and sData.data and sData.data.wheelFortune4 and sData.data.wheelFortune4.report then


                            local report=sData.data.wheelFortune4.report or {}
                            self.reward=FormatItem(sData.data.wheelFortune4.report[1][1]) or {}
                    ...
                    if self.acRoulette4Dialog then
                        self.acRoulette4Dialog.canClickTab=true
                    end
                end
            end
            local leftNum=acRoulette4VoApi:getLeftNum()
            ...
            local function sureHandler()
                local leftNum1=acRoulette4VoApi:getLeftNum()
                local coinNum1=acRoulette4VoApi:getCoinNum()
                local useProp1=nil
                if leftNum1<=0 and coinNum1>0 then
                    useProp1=1
                end
                if leftNum1>0 or coinNum1>0 then
                    socketHelper:activeWheelfortune4(1,1,wheelfortuneCallback,useProp1)
                end
            end
            if leftNum<=0 then
                smallDialog:showSureAndCancle("PanelHeaderPopup.png",CCSizeMake(550,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),sureHandler,getlocal("dialog_title_prompt"),getlocal("activity_wheelFortune4_cost",{useProp}),nil,self.layerNum+1)
            else
                sureHandler()
            end
        end
    ...
    -- self.freeBtn:setEnabled(false)
    -- if (acRoulette4VoApi:checkCanPlay())==true then
    --     local leftNum1,freeNum=acRoulette4VoApi:getLeftNum()
    --     if freeNum>0 then
    ...
    local function onTenPlay()
        ...
        if (acRoulette4VoApi:checkCanTenPlay())==true then
            if (acRoulette4VoApi:checkCanPlay())==true then
                local leftNum1,freeNum=acRoulette4VoApi:getLeftNum()
            ...
            local function wheelfortuneCallback(fn,data)
                ...
                if ret==true then
                    ...
                    if leftNum<10 and leftNum+coinNum>=10 then
                        acRoulette4VoApi:useCoin(10-leftNum)
                    end
                    ...
                    if self and self.bgLayer then
                        ...
                        if sData and sData.data and sData.data.wheelFortune4 and sData.data.wheelFortune4.report then
                            local report=sData.data.wheelFortune4.report or {}
                            local cfg=acRoulette4VoApi:getRouletteCfg()
                            local content={}
                            for k,v in pairs(report) do
                                local awardTb=FormatItem(v[1],nil,true) or {}
                                for m,n in pairs(awardTb) do
                                    local award=n or {}
                                    local index=acRoulette4VoApi:getIndexByNameAndType(award.name,award.type,award.num)
                                    -- print("award.type",award.type)
                                    -- print("award.key",award.key)
                                    -- print("award.num",award.num)
                                    -- print("index",index)
                                    if index and index>0 then
                                        table.insert(content,{award=award,point=0,index=index})
                                    end
                                    ...
                                    if award.type=="p" and (award.key=="p293" or award.key=="p294" or award.key=="p295") then
                                        local kayStr="chatSystemMessage11"
                                        -- if award.key=="p293" then
                                        --     kayStr="chatSystemMessage11_1"
                                        -- elseif award.key=="p294" then
                                        --     kayStr="chatSystemMessage11_2"
                                        -- elseif award.key=="p295" then
                                        --     kayStr="chatSystemMessage11_3"
                                        -- end
                                        if kayStr and kayStr~="" then
            ...
            local function sureHandler()
                ...
                local coinNum1=acRoulette4VoApi:getCoinNum()
                local useProp1=nil
                if leftNum1<10 and leftNum1+coinNum1>=10 then
                    useProp1=10-leftNum1
                end
                if leftNum1+coinNum1>=10 then
                    socketHelper:activeWheelfortune4(1,10,wheelfortuneCallback,useProp1)
                end
            end
            if leftNum<10 then
                smallDialog:showSureAndCancle("PanelHeaderPopup.png",CCSizeMake(550,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),sureHandler,getlocal("dialog_title_prompt"),getlocal("activity_wheelFortune4_cost",{useProp}),nil,self.layerNum+1)
...
function acRoulette4DialogTab1:showFlicker(icon)
...
function acRoulette4DialogTab1:hideFlicker()
...
function acRoulette4DialogTab1:initRewardIcon(iconBg,i,item)
    ...
    local function showInfoHandler(hd,fn,idx)
        ...
        if item and item.key and item.name and item.num then
            ...
            propInfoDialog:create(sceneGame,item,self.layerNum+1,nil,isAddBg)
...
function acRoulette4DialogTab1:play()
    self.tickIndex=0
    self.tickInterval=3
    self.tickConst=3
    ...
    -- 测试
    -- if self.lastEndIdx==nil then
    --     self.lastEndIdx=1
    -- else
    --     self.lastEndIdx=self.lastEndIdx+1
    --     if self.lastEndIdx>12 then
    --         self.lastEndIdx=self.lastEndIdx-12
    --     end
    -- end
    -- self.endIdx=self.lastEndIdx 

    self.slowTime=4

    if self.endIdx>0 then
        -- self.slowStartIndex=self.endIdx-6--目标点减去减速需要几次，得出减速的起始点
        -- if (self.slowStartIndex<=0) then
        --     self.slowStartIndex=self.slowStartIndex+12
        -- end

        -- self.count=24*self.tickConst --转两圈之后开始减速
...
function acRoulette4DialogTab1:fastTick()
    if self.tickIndex ~=nil then
        ...
        if(self.tickInterval<=0)then
            ...
            if (self.tickIndex>=self.count) then 
                -- print("self.haloPos",self.haloPos)
                -- print("self.slowStartIndex",self.slowStartIndex)
                -- print("self.tickConst",self.tickConst)
                -- print("self.endIdx",self.endIdx)
                -- print("self.count",self.count)
                -- print("self.tickIndex",self.tickIndex)
...
function  acRoulette4DialogTab1:playEndEffect()
...
function acRoulette4DialogTab1:tick()
end

function acRoulette4DialogTab1:subtick()
end

function acRoulette4DialogTab1:refresh()
    if self and self.bgLayer then
        local leftNum=acRoulette4VoApi:getLeftNum()
        ...
        if self.playBtn and (acRoulette4VoApi:checkCanPlay())==true then
            self.playBtn:setEnabled(true)
        else
            self.playBtn:setEnabled(false)
        end

        if self.playTenBtn and (acRoulette4VoApi:checkCanTenPlay())==true then
            self.playTenBtn:setEnabled(true)
        else
            self.playTenBtn:setEnabled(false)
        end
        ...
        if self.titleBg then
            ...
            if self.descLb1 then
                local usedNum=acRoulette4VoApi:getUsedNum()
                if usedNum then
...
function acRoulette4DialogTab1:dispose()
    --base:removeFromNeedRefresh(self)
    if self.touchEnabledSp then
