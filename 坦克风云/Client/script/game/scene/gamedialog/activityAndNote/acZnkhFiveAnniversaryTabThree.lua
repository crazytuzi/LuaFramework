acZnkhFiveAnniversaryTabThree = {}

function acZnkhFiveAnniversaryTabThree:new(layerNum)
    local nc = {}
...
function acZnkhFiveAnniversaryTabThree:init()
    self.bgLayer = CCLayer:create()
    self:initUI()
    return self.bgLayer
end

function acZnkhFiveAnniversaryTabThree:initUI()
    ...
    infoMenu:setPosition(ccp(G_VisibleSizeWidth - 10 - infoBtn:getContentSize().width / 2, G_VisibleSizeHeight - 250))
    infoMenu:setTouchPriority( - (self.layerNum - 1) * 20 - 2)
    self.bgLayer:addChild(infoMenu)
    
    local descLb1 = GetTTFLabelWrap(getlocal("activity_znkh2018_tab2_desc3", {acZnkhFiveAnniversaryVoApi:getYN()}), 22, CCSizeMake(G_VisibleSizeWidth - infoBtn:getContentSize().width - 50, 0), kCCTextAlignmentLeft, kCCVerticalTextAlignmentCenter, "Helvetica-bold")
    descLb1:setAnchorPoint(ccp(0, 0.5))
    descLb1:setPosition(20, G_VisibleSizeHeight - 230)
    self.bgLayer:addChild(descLb1)
    local descLb2 = GetTTFLabelWrap(getlocal("activity_znkh2018_tab3_desc1"), 22, CCSizeMake(G_VisibleSizeWidth - infoBtn:getContentSize().width - 50, 0), kCCTextAlignmentLeft, kCCVerticalTextAlignmentCenter, "Helvetica-bold")
    ...
    local myLabel4 = GetTTFLabel(tostring(acZnkhFiveAnniversaryVoApi:getYN()), fontSize)
    ...
    self.rankData = acZnkhFiveAnniversaryVoApi:getLucky()
...
function acZnkhFiveAnniversaryTabThree:tvCallBack(handler, fn, index, cel)
...
function acZnkhFiveAnniversaryTabThree:dispose()
    self = nil
end