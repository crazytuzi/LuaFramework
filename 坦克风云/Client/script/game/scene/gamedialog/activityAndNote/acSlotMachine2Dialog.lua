acSlotMachine2Dialog=commonDialog:new()

function acSlotMachine2Dialog:new()
    local nc={}
...
function acSlotMachine2Dialog:initTableView()
...
function acSlotMachine2Dialog:desEventHandler(handler,fn,idx,cel)
    if fn=="numberOfCellsInTableView" then
        return 1
    elseif fn=="tableCellSizeForIndex" then
        local tmpSize
        local cellHeight = nil
        local descLb=GetTTFLabelWrap(getlocal("activity_slotMachine2_desc"),25,CCSizeMake(350,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
...
function acSlotMachine2Dialog:eventHandler(handler,fn,idx,cel)
  if fn=="numberOfCellsInTableView" then
    return 1
  elseif fn=="tableCellSizeForIndex" then
  ...
  elseif fn=="tableCellAtIndex" then
    ...
    for i=1,3 do
        local startId = acSlotMachine2VoApi:getLastResultByLine(i)
        for i2=1,2 do
          if startId == 1 then
            startId = 5
          else
        ...
        for id=1,self.turnNum do
          ...
          if startId == self.turnNum then
            pic = acSlotMachine2VoApi:getPicById(1)
          else
            pic = acSlotMachine2VoApi:getPicById(startId)
          end
...
function acSlotMachine2Dialog:iconFlicker(icon)
...
-- 删除所有的边框效果
function acSlotMachine2Dialog:resetMetalSps()
...
function acSlotMachine2Dialog:initSp()
  
end

function acSlotMachine2Dialog:moveSp(tb)
...
function acSlotMachine2Dialog:fuwei(key,tb)
...
function acSlotMachine2Dialog:result(tb)
  self.spTb1Speed=0
  self.spTb2Speed=0
  self.spTb3Speed=0
  self.isStop1=true
...
function acSlotMachine2Dialog:fastTick()
  if self.state == 2 then
...
function acSlotMachine2Dialog:startPalyAnimation()
...
function acSlotMachine2Dialog:stopPlayAnimation()
  print("正常~")
  self.state = 0
  self.lastSt = 0 -- 上一次设置动画时间点（用于计算tick中设置动画播放时间间隔）
  ...
  acSlotMachine2VoApi:afterGameOver()
  self.touchDialogBg:setIsSallow(false) -- 点击事件透下去
end

-- 把手播放动画
function acSlotMachine2Dialog:barPaly()
  if self ~= nil then
...
function acSlotMachine2Dialog:stopBarPlay()
  self.leftIcon:setVisible(false)
  self.rightIcon:setVisible(false)
end

-- 抽取奖励
function acSlotMachine2Dialog:getReward()
  self.state = 1
  local free = nil
  local num = nil
  local cost = nil
  if acSlotMachine2VoApi:checkIfFreeGame() == true then
    free = 0
    num = 1
    cost = 0
  else
    ...
    if self.selectMul == true then
      num = acSlotMachine2VoApi:getCfgMul()
      cost = acSlotMachine2VoApi:getMulCost()
    else
      num = 1
      cost = acSlotMachine2VoApi:getCfgCost()
    end
  end
      ...
      local function getRawardCallback(fn,data)
        ...
        if ret==true then
          ...
          --base:formatPlayerData(data)这个方法同步数据时有不同数据用了同一个标识的情况
            if sData.data ~= nil and sData.data.slotMachine2 ~= nil and sData.data.slotMachine2.show ~= nil and sData.data.slotMachine2.free ~= nil and sData.data.slotMachine2.num ~= nil then
              self.playIds = sData.data.slotMachine2.show
              acSlotMachine2VoApi:updateLastResult(self.playIds)
              self:addRewardAndCostMoney(sData.data.slotMachine2.free,sData.data.slotMachine2.num)
              self:startPalyAnimation()
            end
        end
      end
      socketHelper:getSlotMachine2Reward(free,num,getRawardCallback)
    end
  end
...
function acSlotMachine2Dialog:addRewardAndCostMoney(free,num)
  ...
  if free == 0 then -- 免费的
    ...
    if  num > 1 then
      aNum = acSlotMachine2VoApi:getCfgMul()
      aCost = acSlotMachine2VoApi:getMulCost()
    else
      aNum = 1
      aCost = acSlotMachine2VoApi:getCfgCost()
    end
  end
  self.lastMul = aNum
  ...
  for k,v in pairs(getTable) do
    if v ~= nil and v > 0 then
      local cfg = acSlotMachine2VoApi:getCfgConversionTableByIdAndNum(tonumber(k),tonumber(v))
...
function acSlotMachine2Dialog:resetData(data)
  local getTable = {}
...
-- 处理后台得到返回抽取结构后前台的处理
function acSlotMachine2Dialog:aftetGetReward(getTable)
    ...
    if v ~= nil and v > 0 then
      ...
      if cfg ~= nil then
        ...
        if tonumber(v) == 3 then
          local tank, tankNum = acSlotMachine2VoApi:getTankCfgAndNumByCfg(cfg.reward.o[1])
          self:playParticles()
...
function acSlotMachine2Dialog:playParticles()
    --粒子效果
  self.particleS = {}
  local pX = nil
  local PY = nil
...
function acSlotMachine2Dialog:removeParticles()
...
function acSlotMachine2Dialog:doUserHandler()
  local function touch(tag,object)
    ...
    elseif tag == 2 or tag == 4 then
      self:getReward()
    elseif tag == 3 then
      local exchangeDialog= acSlotMachineExchange2Dialog:new()
  ...
  self.mulDesc=GetTTFLabelWrap(getlocal("activity_slotMachine_selectLb",{acSlotMachine2VoApi:getCfgMul()}),25,CCSizeMake(G_VisibleSizeWidth - mulX-10,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
...
-- 选择或取消收益倍数后相关ui刷新
function acSlotMachine2Dialog:updateBySelectMul()
...
-- 刷新领奖按钮
function acSlotMachine2Dialog:updateRewardBtn()
...
function acSlotMachine2Dialog:openInfo()
  local td=smallDialog:new()
  local tabStr = {"\n",getlocal("activity_slotMachine_rule4",{acSlotMachine2VoApi:getCfgMul(),acSlotMachine2VoApi:getCfgMulCost(),acSlotMachine2VoApi:getCfgMul()}),"\n",getlocal("activity_slotMachine_rule3"),"\n",getlocal("activity_slotMachine_rule2"),"\n", getlocal("activity_slotMachine_rule1",{acSlotMachine2VoApi:getCfgFree()}),"\n"}
...
function acSlotMachine2Dialog:update()
  local acVo = acSlotMachine2VoApi:getAcVo()
  if acVo ~= nil then
...
function acSlotMachine2Dialog:dispose()
  self.costLabel = nil
  self.rewardBtn = nil
