acRefitPlanDialogTab2={}

function acRefitPlanDialogTab2:new()
	local nc={}
...
function acRefitPlanDialogTab2:init(layerNum)
...
function acRefitPlanDialogTab2:updateTv()
  if self and self.tv2 then
    self.tv2:reloadData()
  end
end
function acRefitPlanDialogTab2:initTurning( ... )

  self.aid,self.tankid=acRefitPlanVoApi:getTankID()
  ...
  desTv:setMaxDisToBottomOrTop(100)
   ...
   local function touch(tag,object)
    ...
    tabStr = {"\n",getlocal("activity_refitPlanT99_Tab2_desc2"),getlocal("activity_refitPlanT99_Tab2_desc1"),"\n"}
...
function acRefitPlanDialogTab2:eventHandler2(handler,fn,idx,cel)
...
function acRefitPlanDialogTab2:loadCCTableViewCell(cell,idx,refresh)
    local rect = CCRect(0, 0, 50, 50);
    local capInSet = CCRect(20, 20, 10, 10);
    local function touchHander()
...
function acRefitPlanDialogTab2:exbgCellForId(container)
  local addH=11;
  local reR1,reR2,reR3,reR4,reUpgradedMoney = acRefitPlanVoApi:getUpgradedTankResources()
  ...
  local UpgradePropConsume = acRefitPlanVoApi:getUpgradePropConsume()
  ...
  local needTankID,needTankNum = acRefitPlanVoApi:getRefitNeedTankIDAndNum()
  local haveTankNum
        ...
        if self and self.tv2 and self.tv2:getScrollEnable()==true and self.tv2:getIsScrolled()==false then
            local function serverUpgrade(fn,data)
              ...
              if ret==true then
                ...
                local message={key="chatSystemMessage17",param={playerVoApi:getPlayerName(),nameData}}
                chatVoApi:sendSystemMessage(message)
              end
          end
          socketHelper:activityRefitPlanTank(nums,self.aid,serverUpgrade)
        end
