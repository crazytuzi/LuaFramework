acRoulette2DialogTab1={}

function acRoulette2DialogTab1:new()
    local nc={}
...
function acRoulette2DialogTab1:init(layerNum,selectedTabIndex,acRouletteDialog)
...
--初始化上半部的今日抽奖信息
function acRoulette2DialogTab1:initDesc()
...
function acRoulette2DialogTab1:eventHandler(handler,fn,idx,cel)
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        self.descLb1=GetTTFLabel(getlocal("activity_wheelFortune_reward_count",{acRoulette2VoApi:getUsedNum()}),25)
        ...
        local rouletteCfg=acRoulette2VoApi:getRouletteCfg()
        ...
        local descStr1=getlocal("activity_wheelFortune_tip_2")
...
function acRoulette2DialogTab1:getCellHeight()
    local vo=acRoulette2VoApi:getAcVo()
    -- self.descLb=GetTTFLabel(getlocal("activity_wheelFortune_cost"),25)
    local numLb=GetTTFLabel(vo.consume or 0,30)
    local descLb1=GetTTFLabel(getlocal("activity_wheelFortune_reward_count",{acRoulette2VoApi:getUsedNum()}),25)
...
--初始化赌博机
function acRoulette2DialogTab1:initRoulette()
    local rouletteCfg=acRoulette2VoApi:getRouletteCfg()
    ...
    local leftNum=acRoulette2VoApi:getLeftNum()
    ...
    local function onPlay()
        ...
        if (acRoulette2VoApi:checkCanPlay())==true then
            -- self.touchEnabledSp:setVisible(true)
            -- self.touchEnabledSp:setPosition(ccp(0,0))
            -- if self.acRouletteDialog then
            --     self.acRouletteDialog.canClickTab=false
            -- end
            local function wheelfortuneCallback(fn,data)
                ...
                if ret==true then
                    if self and self.bgLayer then
                        if sData and sData.data and sData.data.wheelFortune2 and sData.data.wheelFortune2.active then
                            local vo=acRoulette2VoApi:getAcVo()
                            local oldPoint=vo.point

                            local updateData=sData.data.wheelFortune2.active
                            acRoulette2VoApi:updateData(updateData)
                            if self and self.acRouletteDialog then
                                self.acRouletteDialog:refresh()
                            end

                            local newVo=acRoulette2VoApi:getAcVo()
                        ...
                        if sData and sData.data and sData.data.wheelFortune2 and sData.data.wheelFortune2.reward then
                            self.reward=FormatItem(sData.data.wheelFortune2.reward) or {}
                    ...
                    if self.acRouletteDialog then
                        self.acRouletteDialog.canClickTab=true
                    end
                end
            end

            socketHelper:activeWheelfortune2(1,wheelfortuneCallback)
        end
    end
    local textSize = 25 
    ...
    -- self.freeBtn:setEnabled(false)
    -- if (acRoulette2VoApi:checkCanPlay())==true then
    --     local leftNum1,freeNum=acRoulette2VoApi:getLeftNum()
    --     if freeNum>0 then
    ...
    local function onTenPlay()
        ...
        if (acRoulette2VoApi:checkCanTenPlay())==true then
            if (acRoulette2VoApi:checkCanPlay())==true then
                local leftNum1,freeNum=acRoulette2VoApi:getLeftNum()
                if freeNum>0 then
            ...
            local function wheelfortuneCallback(fn,data)
                ...
                if ret==true then
                    if self and self.bgLayer then
                        ...
                        if sData and sData.data and sData.data.wheelFortune2 and sData.data.wheelFortune2.report then
                            local report=sData.data.wheelFortune2.report or {}
                            local cfg=acRoulette2VoApi:getRouletteCfg()
                            local content={}
                            for k,v in pairs(report) do
                                local awardTb=FormatItem(v[1],nil,true) or {}
                                for m,n in pairs(awardTb) do
                                    local award=n or {}
                                    local index=acRoulette2VoApi:getIndexByNameAndType(award.name,award.type,award.num)
            ...
            socketHelper:activeWheelfortune2(5,wheelfortuneCallback)
        end
    end
...
function acRoulette2DialogTab1:showFlicker(icon)
...
function acRoulette2DialogTab1:hideFlicker()
...
function acRoulette2DialogTab1:initRewardIcon(iconBg,i,item)
...
function acRoulette2DialogTab1:play()
    self.tickIndex=0
    self.tickInterval=3
    self.tickConst=3
...
function acRoulette2DialogTab1:fastTick()
...
function  acRoulette2DialogTab1:playEndEffect()
...
function acRoulette2DialogTab1:tick()
end

function acRoulette2DialogTab1:subtick()
end

function acRoulette2DialogTab1:refresh()
    if self and self.bgLayer then
        local leftNum=acRoulette2VoApi:getLeftNum()
        ...
        if self.playBtn and (acRoulette2VoApi:checkCanPlay())==true then
            self.playBtn:setEnabled(true)
        else
            self.playBtn:setEnabled(false)
        end

        if self.playTenBtn and (acRoulette2VoApi:checkCanTenPlay())==true then
            self.playTenBtn:setEnabled(true)
        else
            self.playTenBtn:setEnabled(false)
        end
        ...
        if self.titleBg then
            ...
            if self.descLb1 then
                local usedNum=acRoulette2VoApi:getUsedNum()
                if usedNum then
...
function acRoulette2DialogTab1:dispose()
    --base:removeFromNeedRefresh(self)
    if self.touchEnabledSp then
