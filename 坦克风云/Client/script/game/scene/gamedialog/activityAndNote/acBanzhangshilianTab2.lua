acBanzhangshilianTab2={}

function acBanzhangshilianTab2:new()
    local nc={}
...
function acBanzhangshilianTab2:init(layerNum,selectedTabIndex,parentDialog)
...
--设置对话框里的tableView
function acBanzhangshilianTab2:initTableView()
    ...
    local pointLabel=GetTTFLabel(getlocal("activity_banzhangshilian_star"),22)
    ...
    local function rewardHandler()
      ...
      local rank=acBanzhangshilianVoApi:getRank()
      local function equipsearchRewardCallback(fn,data)
          local ret,sData=base:checkServerData(data)
          if ret==true then
              local cfg=acBanzhangshilianVoApi:getrankReward()
              ...
              elseif rank>=6 and rank<=10 then
                  ...
                  reward=FormatItem(rewardCfg[6][2]) or {}
              end
              if reward then
              ...
              acBanzhangshilianVoApi:setReceive(1)
              self:refresh()
          end
      end
      if rank and rank>0 then
          socketHelper:acBanzhangshilianRankReward(rank,equipsearchRewardCallback)
      end
    end
    ...
    if acBanzhangshilianVoApi:rankCanReward()>0 then
        self.rewardBtn:setEnabled(true)
    end
    local isReceive=acBanzhangshilianVoApi:isReceive()
    if isReceive==1 then
...
function acBanzhangshilianTab2:eventHandler(handler,fn,idx,cel)
  if fn=="numberOfCellsInTableView" then
    local num=1
    local rankList=acBanzhangshilianVoApi:getRanklist()
...
function acBanzhangshilianTab2:doUserHandler()
    local capInSet = CCRect(20, 20, 10, 10);
    local function bgClick(hd,fn,idx)
    end
    ...
    self.descLb=GetTTFLabel(getlocal("activity_banzhangshilian_has_num",{acBanzhangshilianVoApi:getScore()}),25)
    ...
    self.descLb1=GetTTFLabelWrap(getlocal("activity_banzhangshilian_rank_point",{acBanzhangshilianVoApi:getLimitScore()}),25,CCSizeMake(titleBg:getContentSize().width-100,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
    ...
    local function onClickDesc()
        ...
        local num = SizeOfTable(cfg)
        local rankStr=getlocal("activity_equipSearch_rank_tip_4",{rewardStrTab[1],rewardStrTab[2],rewardStrTab[3],rewardStrTab[4],rewardStrTab[5]})
        rankStr = rankStr .. getlocal("activity_banzhangshilian_rank_reward",{rewardStrTab[6]})
        if num == 5 then
          rankStr=getlocal("activity_equipSearch_rank_tip_4",{rewardStrTab[1],rewardStrTab[2],rewardStrTab[3],rewardStrTab[4],rewardStrTab[5]})
        end

        local strTab={" ",rankStr,getlocal("activity_equipSearch_rank_tip_3"),getlocal("activity_banzhangshilian_tab2_tip2"),getlocal("activity_banzhangshilian_tab2_tip1")," "}
        local colorTab={}
...
function acBanzhangshilianTab2:refresh()
    self.descLb:setString(getlocal("activity_banzhangshilian_has_num",{acBanzhangshilianVoApi:getScore()}))
    self.tv:reloadData()
    local rank=acBanzhangshilianVoApi:getRank(rank)
    if rank==nil then
    self.rewardBtn:setEnabled(false)
    else
    self.rewardBtn:setEnabled(true)
...
function acBanzhangshilianTab2:tick()
end

function acBanzhangshilianTab2:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
