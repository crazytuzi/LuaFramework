acXiaofeisongliDialog = commonDialog:new()

function acXiaofeisongliDialog:new()
	local nc={}
...
function acXiaofeisongliDialog:resetTab()
...
function acXiaofeisongliDialog:initTableView()
	self.cost = acXiaofeisongliVoApi:getCostLevel()
	...
	local function rechargeCallback(tag,object)
        ...
        -- print("+++++++",self.dangci)
        local selectReward = acXiaofeisongliVoApi:getR1()
        ...
        local ChooseFlagList= acXiaofeisongliVoApi:getChooseFlagList()
        ...
        local function linagqujiangli(fn,data)
        	...
        	if ret == true then
        		if sData and sData.data and sData.data.aname then
        			-- print("++++++++++++111")
        			acXiaofeisongliVoApi:updataData(sData.data.aname)
        		end
        ...
        local function callback()
        	socketHelper:acXiaofeisongli(self.dangci,dijige,linagqujiangli)
        end
        if dijige==nil then
        	callback()
        else
        	local sd=acXiaofeisongliSmallDialog2:new(self.layerNum + 1)
			local dialog= sd:init(callback,self.dangci,dijige)
        end
...
function acXiaofeisongliDialog:doUserHandler()
	...
	local acVo = acXiaofeisongliVoApi:getAcVo()
	...
	local desStr = getlocal("activity_xiaofeisongli_des_" .. acXiaofeisongliVoApi:getVersion())
	...
	local rechargeLabel = GetTTFLabel(getlocal("activity_xiaofeisongli_totalMoney_" .. acXiaofeisongliVoApi:getVersion()),28)
	...
	self.totalMoneyLabel = GetTTFLabel(tostring(acXiaofeisongliVoApi:getAlreadyCost()), 30)
...
function acXiaofeisongliDialog:eventHandler( handler,fn,idx,cel )
	...
	elseif fn=="tableCellAtIndex" then
		...
		local selectReward = acXiaofeisongliVoApi:getR1()
		local gudinReward = acXiaofeisongliVoApi:getR2()
		local ChooseFlagList = acXiaofeisongliVoApi:getChooseFlagList()
		local isreward = acXiaofeisongliVoApi:getIsreward()
		local alReadyCost = acXiaofeisongliVoApi:getAlreadyCost()
		local flagList = acXiaofeisongliVoApi:getFlaglist()
		local hadAwardList = acXiaofeisongliVoApi:gethadAwardList()

		for i=1,self.numberCell do
			local selectNum = SizeOfTable(selectReward[i] or {})
			local spWidth=210
			local Flag = acXiaofeisongliVoApi:getStateByid(i)
			...
			if selectNum==0 then
				...
				local function touchSelect()
					if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
					   ...
					   local dialog= sd:init(addSelectSp)
					end
				end
		...
		local per = acXiaofeisongliVoApi:getPercentage()
...
function acXiaofeisongliDialog:refreshBtn()
	if self.dangci==nil then
...
function acXiaofeisongliDialog:openInfo()
	if G_checkClickEnable()==false then
		do
			return
		end
	...
	PlayEffect(audioCfg.mouseClick)
	local version=1
	if acXiaofeisongliVoApi:getVersion()==1 then
		version=1
	end
	local td=smallDialog:new()
	local tabStr = {getlocal("activity_xiaofeisongli_tip4_" .. version),getlocal("activity_xiaofeisongli_tip3_" .. version), getlocal("activity_xiaofeisongli_tip2_" .. version),getlocal("activity_xiaofeisongli_tip1_" .. version),"\n"}
	local colorTb = {nil,G_ColorWhite,G_ColorWhite,G_ColorWhite,G_ColorWhite,G_ColorWhite,nil}

	local flag = false
	local selectRew = acXiaofeisongliVoApi:getR1() -- 可选奖励
...
function acXiaofeisongliDialog:tick()
    local vo=acXiaofeisongliVoApi:getAcVo()
...
function acXiaofeisongliDialog:dispose()
	self.dangci=nil
end