acPersonalCheckPointTab1={
   rewardBtnState = nil,
}

function acPersonalCheckPointTab1:new()
    local nc={}
...
function acPersonalCheckPointTab1:init(layerNum)
    ...
    for i=1,4 do
      local tip
      if i == 1 then
        tip = getlocal("activity_personalCheckPoint_acContent")
      else
        tip = getlocal("activity_personalCheckPoint_rule"..(i - 1))
      end
...
function acPersonalCheckPointTab1:getDes(content, size)
...
-- 更新领奖按钮显示
function acPersonalCheckPointTab1:updateRewardBtn()

  local state = 1
  if acPersonalCheckPointVoApi:hadReward() == true then
      state = 3
  elseif acPersonalCheckPointVoApi:canReward() == true then
      state = 2
  else
      state = 1
  end
...
function acPersonalCheckPointTab1:initTableView()
...
function acPersonalCheckPointTab1:eventHandler(handler,fn,idx,cel)
  ...
  elseif fn=="tableCellSizeForIndex" then
    ...
    elseif idx == 1 then
      ...
      tmpSize = CCSizeMake(G_VisibleSizeWidth - 50,150 * SizeOfTable(acPersonalCheckPointVoApi:getAcCfg()) + 40)
    end
    return  tmpSize
  elseif fn=="tableCellAtIndex" then
    ...
    elseif idx == 2 then
      local acCfg = acPersonalCheckPointVoApi:getAcCfg()
...
function acPersonalCheckPointTab1:addRewardIcons(award,bg,initX,initY)
...
function acPersonalCheckPointTab1:getReward()
  local function getRewardSuccess(fn,data)
    self:getRewardSuccess(fn,data)
  end
  PlayEffect(audioCfg.mouseClick)
  if acPersonalCheckPointVoApi:canReward() == true then
      socketHelper:getPersonalCheckPointReward(acPersonalCheckPointVoApi:getSelfRank(),getRewardSuccess)
  end
end

function acPersonalCheckPointTab1:getRewardSuccess(fn,data)
  local ret,sData=base:checkServerData(data)
  if ret==true then        
    local reward = acPersonalCheckPointVoApi:getMyReward()
    ...
    acPersonalCheckPointVoApi:afterGetReward()
    self:updateRewardBtn() -- 更新领奖按钮
  elseif sData.ret==-1975 then
    acPersonalCheckPointVoApi:update()
  end
end

function acPersonalCheckPointTab1:dispose()
