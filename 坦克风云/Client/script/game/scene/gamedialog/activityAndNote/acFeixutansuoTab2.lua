...
function acFeixutansuoTab2:new()
	local nc={}
...
function acFeixutansuoTab2:init(layerNum)
...
function acFeixutansuoTab2:updateTv()
  if self and self.tv2 then
    self.tv2:reloadData()
  end
end
function acFeixutansuoTab2:initTurning( ... )

  self.aid,self.tankid=acFeixutansuoVoApi:getTankID()
  ...
  desTv:setMaxDisToBottomOrTop(100)
   ...
   local function touch(tag,object)
    ...
    if base.mustmodel==1 and acFeixutansuoVoApi:getMustMode() then
      tip1=getlocal("activity_feixutansuoNew_tab2_tip1")
    else
      tip1=getlocal("activity_feixutansuo_tab2_tip1")
    end
    tabStr = {"\n",getlocal("activity_feixutansuo_tab2_tip2"),tip1,"\n"}
...
function acFeixutansuoTab2:eventHandler2(handler,fn,idx,cel)
...
function acFeixutansuoTab2:loadCCTableViewCell(cell,idx,refresh)
    local rect = CCRect(0, 0, 50, 50);
    local capInSet = CCRect(20, 20, 10, 10);
    local function touchHander()
    ...
    local attTypeLb=GetTTFLabelWrap(getlocal(typeStr),24,CCSizeMake(200,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
    ...
    if abilityCfg[tankCfg[self.tankid].abilityID]~=nil then
        ...
        local attackTypeLb1=GetTTFLabelWrap(getlocal(nameN),24,CCSizeMake(200,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)--第二列第二行增加宽度限制
...
function acFeixutansuoTab2:exbgCellForId(container)
  local addH=11;
  local reR1,reR2,reR3,reR4,reUpgradedMoney = acFeixutansuoVoApi:getUpgradedTankResources()
  ...
  local UpgradePropConsume = acFeixutansuoVoApi:getUpgradePropConsume()
  ...
  local needTankID,needTankNum = acFeixutansuoVoApi:getRefitNeedTankIDAndNum()
  local haveTankNum
        ...
        if self and self.tv2 and self.tv2:getScrollEnable()==true and self.tv2:getIsScrolled()==false then
            local function serverUpgrade(fn,data)
              ...
              if ret==true then
                ...
                local message={key="activity_feixutansuo_chatSystemMessage",param={playerVoApi:getPlayerName(),nameData}}
                chatVoApi:sendSystemMessage(message,paramTab)
              end
          end
          socketHelper:activityFeixutansuoRefitTank(nums,self.aid,serverUpgrade)
        end
...
function acFeixutansuoTab2:dispose()
