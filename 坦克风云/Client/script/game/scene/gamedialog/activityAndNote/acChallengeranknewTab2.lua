acChallengeranknewTab2={

}

function acChallengeranknewTab2:new()
    local nc={}
...
function acChallengeranknewTab2:init(layerNum)
    self.bgLayer=CCLayer:create()
    self.layerNum=layerNum
    local function click(hd,fn,idx)
    end
    local tvBg =LuaCCScale9Sprite:createWithSpriteFrameName("letterBgWrite.png",CCRect(20, 20, 10, 10),click)
    tvBg:setContentSize(CCSizeMake(G_VisibleSizeWidth-50,G_VisibleSizeHeight - 340+100))
...
function acChallengeranknewTab2:initTableView()
  local function callBack(...)
     return self:eventHandler(...)
  end
  local hd= LuaEventHandler:createHandler(callBack)
  self.tv=LuaCCTableView:createWithEventHandler(hd,CCSizeMake(G_VisibleSizeWidth-50,G_VisibleSizeHeight-360+100),nil)
...
function acChallengeranknewTab2:eventHandler(handler,fn,idx,cel)
  if fn=="numberOfCellsInTableView" then
    return SizeOfTable(acChallengeranknewVoApi.rankList) + 1
  ...
  elseif fn=="tableCellAtIndex" then
    ...
    if acVo then
      ...
      local singleVo
      if idx == 0 then
        singleVo = acChallengeranknewVoApi.selfList
      else
        singleVo = acChallengeranknewVoApi.rankList[idx]
      end
      ...
      if singleVo ~= nil then
        ...
        elseif tonumber(rankStr) > tonumber(acVo.maxRank) or tonumber(rankStr) == 0 then
...
function acChallengeranknewTab2:tick()

end


--用户处理特殊需求,没有可以不写此方法
function acChallengeranknewTab2:initTitles()
   ...
   local height=G_VisibleSizeHeight-290+100
    local lbSize=22
    local widthSpace=80
...
function acChallengeranknewTab2:dispose()
