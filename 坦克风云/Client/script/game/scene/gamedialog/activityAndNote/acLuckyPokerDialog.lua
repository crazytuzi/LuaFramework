acLuckyPokerDialog=commonDialog:new()


function acLuckyPokerDialog:resetTab()
...
function acLuckyPokerDialog:doUserHandler()--VipLineYellow
	...
	local needTbViewWidth = middleBg:getContentSize().width
	...
	-- self:initTableView_2()
	 ...
	 --    local yellBoder =CCParticleSystemQuad:create("public/JARrect.plist")
		...
		-- yellBoder:setPosition(ccp(bgLayer:getContentSize().width*0.5,bgLayer:getContentSize().height*0.5))
	  ...
	  local function btnClick( tag,object)
        ...
        if free == true then --免费的直接抽
            ...
            G_dailyConfirm("luckpoker.lottery", getlocal("second_tip_des", {needSubCost}), realLotteryHandler, self.layerNum + 1)
        end
    end 
--------
...
--------
    ...
    elseif tankID ==10084 then
	  ...
	  tankSp:setPosition(ccp(G_VisibleSizeWidth*0.35+25,G_VisibleSizeHeight*unFloorHeightScale-subPosH))
	  ...
	  if tankBarrelSP and tankID ~=10145 then
    ...
    titleBg1:setPosition(ccp(G_VisibleSizeWidth*0.5,G_VisibleSizeHeight*titleNeedScale))
...
function acLuckyPokerDialog:getAllAwardToShowWithDialog(isClick)
...
function acLuckyPokerDialog:openRecordDia()
...
function acLuckyPokerDialog:showBattle()
...
function acLuckyPokerDialog:refreshVisibleWithRecord()
...
function acLuckyPokerDialog:refreshVisible2()
...
function acLuckyPokerDialog:tick()
	local acVo = acLuckyPokerVoApi:getAcVo()
	if acVo ~= nil then
...
function acLuckyPokerDialog:updateAcTime()
    local acVo=acLuckyPokerVoApi:getAcVo()
...
function acLuckyPokerDialog:iniAwardPosBg(bgPic,scalXX,scalYY,bgLayer)--BgEmptyTank.png 
	...
	for i=1,8 do
		...
		-- print("(posH*0.5+20-aHeight*120)/posH------>",(posH*0.5+20-aHeight*120)/posH)
		if i ==1 then
		...
		-- local yellBoder = CCSprite:createWithSpriteFrameName("VipLineYellow.png")
		-- yellBoder:setAnchorPoint(ccp(0.5,0.5))
		local parPos1 = bgPic:convertToNodeSpace(ccp(iconBg:getPositionX(),iconBg:getPositionY()))
		-- local yellBoder = G_addRectFlicker(bgLayer,1.1,1.1,nil,11)
		-- yellBoder:setPosition(ccp(parPos1.x+iconBg:getContentSize().width*0.29,parPos1.y+25-aHeight*42))---确定黄匡最终落点坐标使用
		-- self.yellBoderPosStart = ccp(bgLayer:getContentSize().width*0.5,bgLayer:getContentSize().height*0.5)
		-- yellBoder:setPosition(self.yellBoderPosStart)
		-- yellBoder:setScale(self.yellBoderStartScale)
		-- bgLayer:addChild(yellBoder,11)
		table.insert(self.yellBoderPosEndTb,ccp(parPos1.x+iconBg:getContentSize().width*0.29,0))
		-- table.insert(self.yellBoderTb,yellBoder)
		-- yellBoder:setVisible(false)

		-- print("i-------->self.yellBoderPosEndTb[i].y",i,self.yellBoderPosEndTb[i].y)
    ...
    --grayBoder:setContentSize(CCSizeMake(100+8,100+8))
...
function acLuckyPokerDialog:needMoneyDia(cost,playerGems,wholeTouchBgSp)
...
function acLuckyPokerDialog:initTableView_1(needPosH,needTbViewHeight,needTbViewWidth)
	-- local middleBg = LuaCCScale9Sprite:createWithSpriteFrameName("panelItemBg.png",CCRect(20, 20, 10, 10),function () do return end end)
	-- middleBg:setContentSize(CCSizeMake(needTbViewWidth,75))
	-- middleBg:setAnchorPoint(ccp(0,0))
	-- middleBg:setPosition(ccp(11,needPosH-needTbViewHeight*0.5+10))
	-- self.bgLayer:addChild(middleBg)
	local needNum = self.needBgPosScal_1+0.035
	if G_isIphone5() ==true then
		needNum = self.needBgPosScal_1+0.03
	...
	self.tv:setPosition(ccp(11,self.middleBg:getPositionY()-self.middleBg:getContentSize().height*self.needScalYY*(1-needNum)))--0.535  (1-0.61)
...
function acLuckyPokerDialog:eventHandler(handler,fn,idx,cel)
	local strSize2 = 21
   ...
   elseif fn=="tableCellSizeForIndex" then
       ...
       return  CCSizeMake(G_VisibleSizeWidth-24,75)-- -100
   elseif fn=="tableCellAtIndex" then
       local cell=CCTableViewCell:new()
       	if self.cellBgSp1 ==nil then
			...
			self.cellBgSp1:setContentSize(CCSizeMake(G_VisibleSizeWidth-24,75))
			...
			for k,v in pairs(formatTb) do
				if k <5 then
					...
					for m,n in pairs(v) do
				        ...
				        icon:setPosition(ccp((G_VisibleSizeWidth-24)*posWidthScalTb[awidth]+(4-awidth),self.constPosH*m))
...
function acLuckyPokerDialog:initTableView_2(needPosH,needTbViewHeight,needTbViewWidth)
	-- local middleBg = LuaCCScale9Sprite:createWithSpriteFrameName("panelItemBg.png",CCRect(20, 20, 10, 10),function () do return end end)
	-- middleBg:setContentSize(CCSizeMake(needTbViewWidth,75))
	-- middleBg:setAnchorPoint(ccp(0,0))
	-- middleBg:setPosition(ccp(11,needPosH-needTbViewHeight*0.5-133))
	-- self.bgLayer:addChild(middleBg)
	...
	self.tv2:setPosition(ccp(11,self.middleBg:getPositionY()-self.middleBg:getContentSize().height*self.needScalYY*(1-needNum)))--(1-0.158)
	self.tv2:setAnchorPoint(ccp(0,0))
	self.bgLayer:setTouchPriority(-(self.layerNum-1) * 20 - 1)
	self.tv2:setTableViewTouchPriority(-(self.layerNum-1)*20-3)
	self.tv2:setMaxDisToBottomOrTop(0)
end
function acLuckyPokerDialog:eventHandle2(handler,fn,idx,cel)
	local strSize2 = 21
	if G_getCurChoseLanguage()=="cn" or G_getCurChoseLanguage()=="tw" or G_getCurChoseLanguage()=="ja" or G_getCurChoseLanguage()=="ko" then
		strSize2 =23
	end
   ...
   elseif fn=="tableCellAtIndex" then
       local cell=CCTableViewCell:new()
       	if self.cellBgSp2 ==nil then
			...
			self.cellBgSp2:setContentSize(CCSizeMake(G_VisibleSizeWidth-24,75))
...
function acLuckyPokerDialog:cleanData( )
	for i=1,#self.nextShow do
		self.nextShow[i] = 0
	end
	for i=1,#self.grayBoderTb do
	-- 	self.yellBoderTb[i]:setScale(self.yellBoderStartScale)
	-- 	self.yellBoderTb[i]:setVisible(false)
	-- 	self.yellBoderTb[i]:setPosition(self.yellBoderPosStart)
...
function acLuckyPokerDialog:startPalyAnimation()
	-- print("in startPalyAnimation~~~~~~~")
	self:playJARrectParticles(1)
	self.actionWithAwardTb = acLuckyPokerVoApi:getAwardWithAction()
	-- self.boderRevolutionX =4--公转（向结束坐标点便宜速率）
	-- self.boderRevolutionY =2.5
	-- self.boderRotation =0--自转 起始
	-- self.boderScaleSpeed =0.4--缩小 速率
	-- self.boderRoSpeed =6 --自转 速率
...
function acLuckyPokerDialog:finishPalyAnimation(isClick)
...
function acLuckyPokerDialog:clickFinishAnimation( )
	print("here????")
...
function acLuckyPokerDialog:moveSp( )
	local isStop = false
	local isShow = nil
	local isTank = false
	for k,v in pairs(self.poolSpTb1) do
		...
		if self.nextShow[k] ==1 then
			...
			for m,n in pairs(v) do
				...
				-- if actionAwardTb.name == awardTigTb[m].name then
				-- 	print("actionAwardTb.name == awardTigTb[m].name-----id-----num------->",actionAwardTb.name, awardTigTb[m].name,actionAwardTb.id , awardTigTb[m].id,actionAwardTb.num, awardTigTb[m].num )
				-- end
			...
			-- 	if self.boderIsShow == false then
			-- 		self.boderIsShow =true
					-- self.yellBoderTb[k]:setVisible(true)
				-- end
				-- local curPosX = self.yellBoderTb[k]:getPositionX()
				-- local curPosY = self.yellBoderTb[k]:getPositionY()
				-- if k <3 or (k>4 and k<7) then
				-- 	-- if k ==5 then
				-- 	-- 		print("self.boderPosMovXTb------>",self.boderPosMovXTb[5],self.boderPosMovYTb[5])
				-- 	-- 	end
				-- 	self.boderRotation =self.boderRotation - self.boderRoSpeed
				-- 	self.yellBoderTb[k]:setRotation(self.boderRotation)
				-- 	if curPosX <= self.yellBoderPosEndTb[k].x+5 and curPosY >= self.yellBoderPosEndTb[k].y+5 then
				-- 		self.yellBoderTb[k]:setPosition(self.yellBoderPosEndTb[k])
				-- 	else
				-- 		if curPosX > self.yellBoderPosEndTb[k].x then
				-- 			curPosX = curPosX -	self.boderRevolutionX
				-- 		end
				-- 		if curPosY < self.yellBoderPosEndTb[k].y then
				-- 			curPosY = curPosY+self.boderRevolutionY
				-- 		end
				-- 	end
				-- else
				-- 	-- if k ==4 then
				-- 	-- 		print("self.boderPosMovXTb------>",self.boderPosMovXTb[4],self.boderPosMovYTb[4])
				-- 	-- 	end
				-- 	self.boderRotation =self.boderRotation + self.boderRoSpeed
				-- 	self.yellBoderTb[k]:setRotation(self.boderRotation)
				-- 	if curPosX >= self.yellBoderPosEndTb[k].x+5 and curPosY <= self.yellBoderPosEndTb[k].y+5 then
				...
				-- 	end
				-- end
				-- self.yellBoderTb[k]:setPosition(ccp(curPosX,curPosY))

				-- if self.yellBoderStartScale > self.yellBoderEndScale+0.2 then
				-- 	self.yellBoderStartScale = self.yellBoderStartScale - self.boderScaleSpeed
				-- 	self.yellBoderTb[k]:setScale(self.yellBoderStartScale)
				-- end
			-- end
	...
	if isStop ==true then
		...
		-- self.yellBoderTb[isShow]:setScale(self.yellBoderEndScale)
		-- self.yellBoderTb[isShow]:setPosition(self.yellBoderPosEndTb[isShow])
		
		-- if isShow <3 or (isShow>4 and isShow<7) then
		-- 	self.yellBoderTb[isShow]:setRotation(0)
		-- else
		-- 	self.yellBoderTb[isShow]:setRotation(-360)
		-- end
		...
		if isShow+1 <= #self.actionWithAwardTb then
			...
			-- self.boderRevolutionX =self.boderPosMovXTb[isShow+1]--公转（向结束坐标点便宜速率）
			-- self.boderRevolutionY =self.boderPosMovYTb[isShow+1]
			-- self.boderRotation =0--自转 起始
			-- self.boderScaleSpeed =0.08--缩小 速率
			-- self.boderRoSpeed =6 --自转 速率
			-- self.yellBoderStartScale =5
...
function acLuckyPokerDialog:fastTick( )
...
function acLuckyPokerDialog:playJARrectParticles(idx)
	local yellBoder =CCParticleSystemQuad:create("public/JARrect.plist")
	yellBoder:setAnchorPoint(ccp(0.5,0.5))
	yellBoder.positionType = kCCPositionTypeFree
	yellBoder:setScale(self.yellBoderStartScale)
...
function acLuckyPokerDialog:playParticles(isTrue,needPos,parentBg)
    --粒子效果
	self.particleS = {}
...
function acLuckyPokerDialog:removeParticles()
...
function acLuckyPokerDialog:removeJARrectParticles( )
...
function acLuckyPokerDialog:new()
	local nc={}
	...
	self.boderRoSpeed =6
	-- self.boderPosMovXTb ={ 4,1.3,1.4,4.5,4.3,2,2,5.2 }
	-- self.boderPosMovYTb ={ 3.5,3.3,3.3,3.2,1,1.5,1.5,1.5} --1:2.5
	-- if G_isIphone5() ==true then
	-- 	self.boderPosMovXTb ={ 4,1.3,1.4,5.5,4.3,2,2,5.2 }
	-- 	self.boderPosMovYTb ={ 3.5,5.3,5.3,5.5,1,1.5,1.5,1.5}--1:2.5
	-- end
	self.middleBg =nil
	self.boderIsShow =false
	self.isFinished =true
...
function acLuckyPokerDialog:dispose( )
	...
	-- self.boderPosMovXTb =nil
	-- self.boderPosMovYTb =nil
