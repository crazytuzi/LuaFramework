...
function acRoulette5Tab2:init(layerNum,selectedTabIndex,acRoulette5Dialog)
...
--设置对话框里的tableView
function acRoulette5Tab2:initTableView()
    ...
    --     local rank=acRoulette5VoApi:rankCanReward()
    ...
    --             end
    --             acRoulette5VoApi:setListRewardNum()
    --             self:refresh()
    ...
    --                             acRoulette5VoApi:clearRankList()

    --                             rankList=sData.data.wheelFortune4
    --                             acRoulette5VoApi:updateData(rankList)
    --                             self:refresh()

    --                             acRoulette5VoApi:setLastListTime(base.serverTime)
    --                             -- acRoulette5VoApi:setFlag(2,1)
    --                         end
    --                     end
    --                 end
    --             end
    ...
    -- if acRoulette5VoApi:rankCanReward()>0 then
...
function acRoulette5Tab2:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
		local num=0
        -- local num=1
        local rankList=acRoulette5VoApi:getlist()
	...
	elseif fn=="tableCellAtIndex" then
        ...
        local index=idx+1
   --      if idx==0 then
   --      	rank=acRoulette5VoApi:getSelfRank()
   ...
   --          rank=idx
        ...
        --   2        用户获取的奖励类型
            ...
            rData=rankList[index]            
            id=rData[1]
            awardId=tonumber(rData[4])
            name=rData[2] 
            level=rData[3]
            local cfg = acRoulette5VoApi:getRouletteCfg()
            award=cfg.rewardtype[awardId]
		-- end
  ...
  --       local rankSp
	 ...
	 --      	rankSp:setPosition(ccp(lbWidth,lbHeight))
        ...
        local awardName=GetTTFLabel(award,lbSize)
        awardName:setPosition(ccp(lbWidth+120*3+40,lbHeight))
        cell:addChild(awardName,2)
...
function acRoulette5Tab2:doUserHandler()
    local capInSet = CCRect(20, 20, 10, 10);
    local function bgClick(hd,fn,idx)
    end
...
function acRoulette5Tab2:getCellHeight()
    local descLb=GetTTFLabelWrap(getlocal("activity_wheelFortune5_rank_desc"),22,CCSizeMake(G_VisibleSize.width-80,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
...
function acRoulette5Tab2:eventHandler1(handler,fn,idx,cel)
    if fn=="numberOfCellsInTableView" then
        return 1
    elseif fn=="tableCellSizeForIndex" then
        local tmpSize
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        self.descLb=GetTTFLabelWrap(getlocal("activity_wheelFortune5_rank_desc"),22,CCSizeMake(G_VisibleSize.width-80,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
        self.descLb:setAnchorPoint(ccp(0,0.5))
...
function acRoulette5Tab2:tick()
    
end

function acRoulette5Tab2:subtick()
end

function acRoulette5Tab2:refresh()
    if self and self.bgLayer then
        local vo=acRoulette5VoApi:getAcVo()
        -- if self.descLb then
...
function acRoulette5Tab2:dispose()
