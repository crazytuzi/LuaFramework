...
function acMingjiangTab2:init(layerNum)
...
--设置对话框里的tableView
function acMingjiangTab2:initTableView()
	...
	local pointLabel=GetTTFLabel(getlocal("active_mingjiang_score"),22)
    ...
    local function rewardHandler()
        local rank=acMingjiangVoApi:getRank()
        local function equipsearchRewardCallback(fn,data)
            local ret,sData=base:checkServerData(data)
            if ret==true then
                local cfg=acMingjiangVoApi:getrankReward()
                ...
                acMingjiangVoApi:setReceive(1)
                self:refresh()
            end
        end
        if rank and rank>0 then
            socketHelper:activityMingjiangGetReward(equipsearchRewardCallback)
        end
    end
    ...
    if acMingjiangVoApi:rankCanReward()>0 then
        self.rewardBtn:setEnabled(true)
    end
    local isReceive=acMingjiangVoApi:isReceive()
    if isReceive==1 then
...
function acMingjiangTab2:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
        local num=1
    local rankList=acMingjiangVoApi:getRanklist()
...
function acMingjiangTab2:initLayer()
    local capInSet = CCRect(20, 20, 10, 10);
    local function bgClick(hd,fn,idx)
    end
    ...
    self.descLb=GetTTFLabel(getlocal("activity_mingjiang_has_num",{score}),25)
    ...
    self.descLb1=GetTTFLabelWrap(getlocal("activity_mingjiang_rank_point",{acMingjiangVoApi:getscoreLimit()}),25,CCSizeMake(titleBg:getContentSize().width-130,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
    ...
    local function onClickDesc()
        ...
        local strTab={" ",getlocal("activity_equipSearch_rank_tip_4",{rewardStrTab[1],rewardStrTab[2],rewardStrTab[3],rewardStrTab[4],rewardStrTab[5]}),getlocal("activity_mingjiang_tab2_tip2"),getlocal("activity_mingjiang_tab2_tip1")," "}
        local colorTab={}
...
function acMingjiangTab2:refresh()
    local score = acMingjiangVoApi:getScore() 
    if score==nil then
        score=0
    end
   self.descLb:setString(getlocal("activity_mingjiang_has_num",{score}))
   self.tv:reloadData()
   local rank=acMingjiangVoApi:getRank(rank)
   if rank==nil then
    self.rewardBtn:setEnabled(false)
   else
    self.rewardBtn:setEnabled(true)
...
function acMingjiangTab2:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
