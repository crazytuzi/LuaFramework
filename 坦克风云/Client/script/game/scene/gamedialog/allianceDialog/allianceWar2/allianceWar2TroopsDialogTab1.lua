allianceWar2TroopsDialogTab1 = {}

function allianceWar2TroopsDialogTab1:new()
    local nc = {}
...
function allianceWar2TroopsDialogTab1:init(layerNum, parent)
...
function allianceWar2TroopsDialogTab1:initTabLayer()
...
function allianceWar2TroopsDialogTab1:initDesc()
...
function allianceWar2TroopsDialogTab1:initRepair()
    local rect = CCRect(0, 0, 50, 50)
    local capInSet = CCRect(20, 20, 10, 10)
    local function cellClick(hd, fn, idx)
    end
    ...
    local function repairHandler()
        ...
        --         self:tick()
        --     end
        --     allianceWar2VoApi:showRepairDialog(reviveTime,self.layerNum+1,reviveCallback)
        -- end
    end
    local scale = 0.8
...
function allianceWar2TroopsDialogTab1:tick()
    -- if self then
...
function allianceWar2TroopsDialogTab1:updateData()
    ...
    if status == 30 and tanksTb and SizeOfTable(tanksTb) > 0 then
    ...
    if isUseCurTroops == true then
        ...
        if tskin then
            tankSkinVoApi:setTankSkinListByBattleType(self.type, G_clone(tskin))
        end
        local hero = G_clone(heroVoApi:getAllianceWar2HeroList())
        heroVoApi:setAllianceWar2CurHeroList(hero)
        local aitroops = G_clone(AITroopsFleetVoApi:getAllianceWar2AITroopsList())
        ...
        airShipVoApi:setBattleEquip(self.type, airShipVoApi:getBattleEquip(32))
    end
    -- print("self.type>>>>>>",self.type)
    -- G_dayin(tankVoApi:getTanksTbByType(self.type))
end

function allianceWar2TroopsDialogTab1:refresh()
...
function allianceWar2TroopsDialogTab1:dispose()
    self.bgLayer = nil
    self.currentShow = 1
    self.currentFundsLb = nil
    self.timeLb = nil
