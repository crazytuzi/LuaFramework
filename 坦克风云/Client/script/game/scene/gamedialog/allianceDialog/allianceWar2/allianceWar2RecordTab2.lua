allianceWar2RecordTab2={

}

function allianceWar2RecordTab2:new()
    local nc={}
...
function allianceWar2RecordTab2:init(layerNum,parentDialog)
...
function allianceWar2RecordTab2:initTabLayer()
    -- self:resetTab()
        ...
        if ret==true then
            if sData.data then
                if sData.data.unum then
                    allianceWar2RecordVoApi:setAllianceMaxNum(tonumber(sData.data.unum))
                end
                if sData.data.ulog then
                    allianceWar2RecordVoApi:formatAllianceRecordData(sData.data.ulog)
                end
                if self then
    ...
    local maxNum=allianceWar2RecordVoApi:getAllianceMaxNum()
    local personRecordTab=allianceWar2RecordVoApi:getAllianceRecordTab()
    local rFlag=allianceWar2RecordVoApi:getAFlag()
    -- print("rFlag",rFlag)
    if rFlag==-1 or (maxNum and personRecordTab and maxNum>SizeOfTable(personRecordTab)) then
        ...
        if warid and warid>0 then
            socketHelper:alliancewarnewGetbattlelog(warid,4,aid,uid,minTs,maxTs,getbattlelogCallback)
        else
            self:doUserHandler()
        end
    else
...
function allianceWar2RecordTab2:initTableView()
    if self.tv then
        self.cellHeightTab={}
        self.tv:reloadData()
    else
    ...
    self:doUserHandler()
    allianceWar2RecordVoApi:setAllianceHasNew(false)
end

function allianceWar2RecordTab2:getCellHeight(index)
    local personRecordTab=allianceWar2RecordVoApi:getAllianceRecordTab()
...
function allianceWar2RecordTab2:eventHandler(handler,fn,idx,cel)
    if fn=="numberOfCellsInTableView" then   
        local personRecordTab=allianceWar2RecordVoApi:getAllianceRecordTab()

        local num=0
        if personRecordTab then
            local hasMore=allianceWar2RecordVoApi:allianceHasMore()
            if hasMore then
...
function allianceWar2RecordTab2:refreshTableView()
...
function allianceWar2RecordTab2:tick()
    if self then
        local rFlag=allianceWar2RecordVoApi:getAFlag()
        if rFlag==0 then
            self:refreshTableView()
...
--用户处理特殊需求,没有可以不写此方法
function allianceWar2RecordTab2:doUserHandler()
...
--点击了cell或cell上某个按钮
function allianceWar2RecordTab2:cellClick(idx)

end

function allianceWar2RecordTab2:dispose()
