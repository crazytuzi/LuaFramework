allianceWarBidDialog=smallDialog:new()

function allianceWarBidDialog:new(parent,cityID)
	local nc={}
...
function allianceWarBidDialog:init(layerNum)
	...
	local cityCfg=allianceWarCfg.city[self.cityID]
...
function allianceWarBidDialog:bid()
    if CCUserDefault:sharedUserDefault():getIntegerForKey("test_turntest")==1 then
        allianceWarCfg.minRegistrationFee=1
    end
	local bidFunds=tonumber(self.lastNumValue)
	if(bidFunds==nil or bidFunds<allianceWarCfg.minRegistrationFee)then
		smallDialog:showTipsDialog("PanelPopup.png",CCSizeMake(500,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),getlocal("allianceWar_illegalFunds",{allianceWarCfg.minRegistrationFee}),30)
	else
		...
		allianceWarVoApi:bid(bidFunds,self.cityID,onBidCallback)
	end
end

function allianceWarBidDialog:createWithBuffId(buffId,layerNum)
	local sd=allianceWarBidDialog:new()
	sd:initWithBuffId(buffId,layerNum)
	return sd

end
function allianceWarBidDialog:initWithBuffId(buffId,layerNum)
	self.isTouch=false
	self.isUseAmi=false
	...
	local bid="b"..buffId
	local iconSp=CCSprite:createWithSpriteFrameName(allianceWarCfg.buffSkill[bid].icon)
    ...
    local nameLb=GetTTFLabelWrap(getlocal("buffName",{getlocal(allianceWarCfg.buffSkill[bid].name)}),26,CCSize(250,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
	...
	local buffLv=tonumber(allianceWarVoApi:getBattlefieldUser()[bid])
	...
	local costLb=GetTTFLabel(allianceWarCfg.buffSkill[bid].cost,26)
	...
	if tonumber(allianceWarVoApi:getBattlefieldUser()[bid])==allianceWarCfg.buffSkill[bid]["maxLv"] then
		...
		local per=tonumber(allianceWarCfg.buffSkill[bid]["probability"][buffLv+1])*100
    ...
    local contentLb=GetTTFLabelWrap(getlocal(allianceWarCfg.buffSkill[bid]["des"]),25,CCSize(420,0),kCCTextAlignmentCenter,kCCVerticalTextAlignmentCenter)
   contentLb:setAnchorPoint(ccp(0.5,0.5))
	...
	local oldLv=tonumber(allianceWarVoApi:getBattlefieldUser()[bid])
	...
	local function activation()
		if tonumber(allianceWarVoApi:getBattlefieldUser()[bid])==allianceWarCfg.buffSkill[bid]["maxLv"] then
			local str=getlocal("technology_max_level",{getlocal(allianceWarCfg.buffSkill[bid].name)})
			smallDialog:showTipsDialog("PanelPopup.png",CCSizeMake(500,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),str,30)
			do
				return
			end
		end
		if playerVoApi:getGems()<allianceWarCfg.buffSkill[bid].cost then
		...
		local function callback(fn,data)
			if base:checkServerData(data)==true then
				...
				local gems=playerVoApi:getGems()-allianceWarCfg.buffSkill[bid].cost
					playerVoApi:setGems(gems)
				smallDialog:showTipsDialog("PanelPopup.png",CCSizeMake(500,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),str,30)

			end
		end
		socketHelper:alliancewarBuybuff(bid,callback)
	end
