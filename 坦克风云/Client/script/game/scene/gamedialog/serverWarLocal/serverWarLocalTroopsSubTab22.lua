serverWarLocalTroopsSubTab22 = {}

function serverWarLocalTroopsSubTab22:new()
    local nc = {}
...
function serverWarLocalTroopsSubTab22:init(layerNum, parent)
...
function serverWarLocalTroopsSubTab22:initTabLayer()
...
function serverWarLocalTroopsSubTab22:initSaveBtn()
    local function save()
...
function serverWarLocalTroopsSubTab22:saveHandler(callback)
    ...
    local function setinfoHandler(fn, data)
        ...
        if ret == true then
            ...
            local btype = self.type + self.selectedTabIndex
            local troopsIndex = self.selectedTabIndex + 1
            self["serverWarLocalTanks" .. (self.selectedTabIndex + 1)] = G_clone(tankVoApi:getTanksTbByType(btype))
            ...
            heroVoApi:setServerWarLocalHeroList(self.selectedTabIndex + 1, heroList)
...
function serverWarLocalTroopsSubTab22:isChangeFleet()
    local btype = self.type + self.selectedTabIndex
    local troopsIndex = self.selectedTabIndex + 1
    local fleetInfo = tankVoApi:getTanksTbByType(btype)
...
function serverWarLocalTroopsSubTab22:initFormationBtn()
...
function serverWarLocalTroopsSubTab22:initDesc()
...
-- function serverWarLocalTroopsSubTab22:judgeFight()
--     local bestTab2={}
--     local allfight1=0
--     local allfight2=0
...
function serverWarLocalTroopsSubTab22:doUserHandler()
...
function serverWarLocalTroopsSubTab22:tick()
    -- if self then
    --     local setFleetStatus=serverWarLocalVoApi:getSetFleetStatus()
...
function serverWarLocalTroopsSubTab22:setTanksRestore()
...
function serverWarLocalTroopsSubTab22:updateData()
    heroVoApi:clearTroops()
    AITroopsFleetVoApi:clearAITroops()
    self:setTanksRestore()
end
function serverWarLocalTroopsSubTab22:refresh()
    self:updateData()
    local tType = self.type + self.selectedTabIndex
...
function serverWarLocalTroopsSubTab22:dispose()
    self:setTanksRestore()
