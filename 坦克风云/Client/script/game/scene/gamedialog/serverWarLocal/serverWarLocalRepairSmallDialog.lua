serverWarLocalRepairSmallDialog=smallDialog:new()

function serverWarLocalRepairSmallDialog:new()
	local nc={}
...
function serverWarLocalRepairSmallDialog:init(troopID,time,layerNum,callback)
    ...
    local costGems=cdTime+serverWarLocalCfg.reviveCost
    self.descLb=GetTTFLabelWrap(getlocal("serverWarLocal_repairDesc",{costGems}),25,CCSizeMake(self.dialogWidth-80,0),kCCTextAlignmentCenter,kCCVerticalTextAlignmentCenter)
    ...
    local function sureHandler()
        ...
        if time then
            local costGems=(time-base.serverTime)+serverWarLocalCfg.reviveCost
            ...
            serverWarLocalFightVoApi:revive(troopID,reviveCallback)
        end
    end
...
function serverWarLocalRepairSmallDialog:tick()
	if self and self.time then
		...
		if cdTime<0 then
			...
			if self.descLb then
				local costGems=cdTime+serverWarLocalCfg.reviveCost
			    self.descLb:setString(getlocal("serverWarLocal_repairDesc",{costGems}))
			end
		end
	end
end
