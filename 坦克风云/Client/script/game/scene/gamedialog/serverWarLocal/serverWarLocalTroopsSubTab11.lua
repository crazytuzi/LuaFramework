serverWarLocalTroopsSubTab11 = {}

function serverWarLocalTroopsSubTab11:new()
    local nc = {}
...
function serverWarLocalTroopsSubTab11:init(layerNum, parent)
...
function serverWarLocalTroopsSubTab11:initTabLayer()
...
function serverWarLocalTroopsSubTab11:initDesc()
...
function serverWarLocalTroopsSubTab11:initRepair()
    local rect = CCRect(0, 0, 50, 50)
    local capInSet = CCRect(20, 20, 10, 10)
    local function cellClick(hd, fn, idx)
    end
...
function serverWarLocalTroopsSubTab11:tick()
    if self then
...
function serverWarLocalTroopsSubTab11:updateData()
    ...
    if (checkStatus == 21 or checkStatus == 23) and tanksTb and SizeOfTable(tanksTb) > 0 then
    ...
    if (checkStatus == 21 or checkStatus == 23) and heroTb and SizeOfTable(heroTb) > 0 then
    ...
    if (checkStatus == 21 or checkStatus == 23) and aitroops and SizeOfTable(aitroops) > 0 then
    ...
    if checkStatus == 21 or checkStatus == 23 then
        isUseCurEmblem = true
    end
    local troopID = btype - 26
    if isUseCurEmblem == false or (isUseCurEmblem == true and serverWarLocalFightVoApi:checkTroopInBase(troopID) == true) then
...
function serverWarLocalTroopsSubTab11:refresh()
...
function serverWarLocalTroopsSubTab11:dispose()
    self.selectedTabIndex = 0
    self.type = 27
    self.bgLayer = nil
    self.currentShow = {1, 1, 1}
