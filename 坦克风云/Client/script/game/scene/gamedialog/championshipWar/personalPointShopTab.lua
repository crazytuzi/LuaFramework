personalPointShopTab = {}

function personalPointShopTab:new()
    local nc = {}
...
function personalPointShopTab:init(layerNum, parent)
    ...
    local pointSp = CCSprite:createWithSpriteFrameName("csi_point.png")
    ...
    local pointShop = warCfg.pointShop
    self.cellNum = SizeOfTable(pointShop)
    self.shopList = {}
    for k = 1, self.cellNum do
        local id = "p"..k
        local shopItem = pointShop[id]
        local reward = FormatItem(shopItem.r)[1]
        table.insert(self.shopList, {id = id, bn = shopItem.bn, p = shopItem.p, r = reward})
    end
    self.buyData = championshipWarVoApi:getBuyData().b1 or {}
    self:updateData()
...
function personalPointShopTab:updateData()
    self.buyData = championshipWarVoApi:getBuyData().b1 or {}
...
function personalPointShopTab:eventHandler(handler, fn, idx, cel)
    ...
    elseif fn == "tableCellAtIndex" then
        ...
        local function buyHandler()
            if self.tv and self.tv:getScrollEnable() == true and self.tv:getIsScrolled() == false then
                ...
                local function realBuy()
                    ...
                    championshipWarVoApi:championshipWarShopBuy(1, id, nil, buyCallBack)
                end
                ...
                if G_isPopBoard(key) then
                    G_showSecondConfirm(self.layerNum + 1, true, true, getlocal("dialog_title_prompt"), getlocal("ladder_shopBuy", {cost..getlocal("serverwar_point"), reward.name}), true, realBuy, secondTipFunc)
                else
                    realBuy()
                end
            end
        ...
        local point = championshipWarVoApi:getMyPoint()
        if (point < cost) or (limit > 0 and buyNum >= limit) then --功勋值不够或者购买已达上限或者没有达到段位需求不可购买
            buyItem:setEnabled(false)
        end
        
        local costSp = CCSprite:createWithSpriteFrameName("csi_point.png")
...
function personalPointShopTab:refresh()
    self:updateData()
    local point = championshipWarVoApi:getMyPoint()
...
function personalPointShopTab:tick()
    
end

function personalPointShopTab:dispose()
