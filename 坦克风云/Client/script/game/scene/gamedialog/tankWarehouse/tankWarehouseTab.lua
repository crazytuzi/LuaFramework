...
function tankWarehouseTab:new()
    local nc = {}
    setmetatable(nc, self)
    self.__index = self
    return nc
end
function tankWarehouseTab:show(layerNum,useZorder)
	self.layerNum = layerNum or 3
	self.useZorder = useZorder or 99
	...
	local function refreshinitTanks(event,data)
		if self.clayer then
			self:initTanks()
			self.clayer:setPositionY(G_VisibleSize.height - self.sceneHeight*self.sceneScale)	
		end
	end
	refreshinitTanks()
...
function tankWarehouseTab:touchEvent(fn,x,y,touch)
	if(self.clayer==nil or tolua.cast(self.clayer,"CCLayer")==nil) or self.bgLayer:getPositionY() ~= 0 then
		do return end
	end
	if fn=="began" then
...
function tankWarehouseTab:checkBound(pos)
	local tmpPos
	if pos==nil then
...
function tankWarehouseTab:initTanks()
	...
	if self.sceneSp and self.sceneSp:getParent() then--tolua.cast(self.sceneSp,"CCSprite"):getParent() then
	...
	self.sceneSp:setPosition(ccp(0,-60))
		...
		if v[1]~=0 then
			...
			if tolua.cast(self.clayer:getChildByTag(k),"CCNode")==nil then
				local function showSkinDetail()
					tankSkinVoApi:showTankSkinDialog(k, self.layerNum + 1)
				end
				...
				local function pbUIhandler()
					...
					if flag==true then
						...
						tankInfoDialog:create(sceneGame,k,self.layerNum + 1)
					end
				end
...
function tankWarehouseTab:initOldTanks()
...
function tankWarehouseTab:checkNewTank(tankID)
	if(self.initFlag and self.oldTanks[tankID]==nil)then
		self.newTanks[tankID]=1
	end
end

function tankWarehouseTab:dispose()
