...
function alienMinesRankTab2:new()
    local nc={}
...
function alienMinesRankTab2:init(layerNum)
 	self.bgLayer=CCLayer:create()
	self.layerNum=layerNum
	self.data = alienMinesVoApi:getAlianList()
	-- 初始化标签
	self:initTitleLb()
...
function alienMinesRankTab2:initTitleLb()
	...
	local nameLabel=GetTTFLabel(getlocal("alliance_list_scene_name"),25)
	...
	local limitLb = GetTTFLabelWrap(getlocal("alienMines_res_limit",{alienMineCfg.allianceRanking.needpoint}),25,CCSizeMake(self.bgLayer:getContentSize().width-40,0),kCCTextAlignmentCenter,kCCVerticalTextAlignmentCenter)
	...
	limitLb:setVisible(false)
	self.limitLb=limitLb
end

function alienMinesRankTab2:initTableView()
...
function alienMinesRankTab2:eventHandler(handler,fn,idx,cel)
	...
	elseif fn=="tableCellAtIndex" then
		...
		rankData= alienMinesVoApi:getAlianList()
		if rankData~=nil then
			rankStr=rankData[idx+1][4]
			nameStr=rankData[idx+1][2]
			levelStr=rankData[idx+1][3]
			valueStr=rankData[idx+1][5]
		end
...
function alienMinesRankTab2:tick()
	if alienMinesVoApi:checkIsActive4()==true and alienMinesVoApi:getRefreshRankTime2()~=-1 then
...
function alienMinesRankTab2:dispose()
	self.bgLayer=nil
	self.layerNum=nil
	self.tv=nil
	self.limitLb=0
