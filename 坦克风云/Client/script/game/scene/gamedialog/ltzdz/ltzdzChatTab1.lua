ltzdzChatTab1 ={}
function ltzdzChatTab1:new(layerNum)
    local nc={}
...
function ltzdzChatTab1:init()
    ...
    eventDispatcher:addEventListener("ltzdz.newChat1",refreshChat)

	return self.bgLayer
end

function ltzdzChatTab1:initLayer()
    self:initTableView()
end
function ltzdzChatTab1:initTableView()

    self.chatList=G_clone(ltzdzChatVoApi:getChat1())
...
function ltzdzChatTab1:eventHandlerNew(handler,fn,idx,cel)
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        local senderStr=chatVo.nickname
        ...
        if tonumber(senderId)==tonumber(ally) then
            senderStr=getlocal("ltzdz_ally_friend") .. senderStr
            color=G_ColorBlue
        end

        -- print("chatVo.zid ",chatVo.zid )
        local zid=chatVo.zid or base.curZoneID
        local serverName=GetServerNameByID(zid)

        senderStr=senderStr .. "-" .. serverName


        senderLabel=GetTTFLabel(senderStr or "",lbSize)
...
function ltzdzChatTab1:resetTvPos()
...
function ltzdzChatTab1:refreshChat(data)
    if self.tv then
        self.chatList=G_clone(ltzdzChatVoApi:getChat1())
        local chatNum1=ltzdzChatVoApi:getMaxMore(1)
        if self.chatNum+1>chatNum1 then
...
function ltzdzChatTab1:dispose( )
    if self.refreshChatListener then
        eventDispatcher:removeEventListener("ltzdz.newChat1",self.refreshChatListener)
