newAllianceMemberRankDialog={

}

function newAllianceMemberRankDialog:new(searchName)
    local nc={}
...
function newAllianceMemberRankDialog:init(parentDialog,layerNum,isGuide)
  --[[
...
function newAllianceMemberRankDialog:initTableView()
...
function newAllianceMemberRankDialog:eventHandler(handler,fn,idx,cel)
    ...
    grayBgSp:setTouchPriority(-(self.layerNum-1)*20-2)
    ...
    numLabel:setPosition(widthSpace+253,height)
    ...
    local viewBtn = GetButtonItem("newGreenBtn.png","newGreenBtn_down.png","newGreenBtn_down.png",viewHandler,idx+100,getlocal("alliance_list_check_info"),28/0.7,idx+1)
      viewBtn:setAnchorPoint(ccp(0.5,0.5))
    viewBtn:setScale(0.5)
    local viewMenu=CCMenu:createWithItem(viewBtn)
      viewMenu:setPosition(ccp(self.bgLayer:getContentSize().width-viewBtn:getContentSize().width/2-10,height))
    ...
    local applyMenu=CCMenu:createWithItem(applyBtn)
      applyMenu:setPosition(ccp(self.bgLayer:getContentSize().width-applyBtn:getContentSize().width/2-10,height))
...
function newAllianceMemberRankDialog:tick()
...
--用户处理特殊需求,没有可以不写此方法
function newAllianceMemberRankDialog:doUserHandler(searchName)
  ...
  if self.rankLabel==nil then
    self.rankLabel=GetTTFLabel(getlocal("alliance_scene_rank_title"),lbSize,true)
  ...
  if self.viewLabel==nil then
    self.viewLabel=GetTTFLabel(getlocal("alliance_scene_check"),lbSize,true)
    self.viewLabel:setPosition(widthSpace+477,height)
  ...
  if self.searchBtn==nil then
    local function searchAlliance(tag,object)
      ...
      self.searchBtn = GetButtonItem("creatRoleBtn.png","creatRoleBtn_Down.png","creatRoleBtn_Down.png",searchAlliance,nil,getlocal("alliance_list_scene_search"),28)
...
--点击了cell或cell上某个按钮
function newAllianceMemberRankDialog:cellClick(idx)
...
--点击了cell或cell上某个按钮
function newAllianceMemberRankDialog:refresh(isTabClick)
...
function newAllianceMemberRankDialog:dispose()
