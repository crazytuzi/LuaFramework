dimensionalWarInforTab1={}

function dimensionalWarInforTab1:new()
    local nc={}
...
function dimensionalWarInforTab1:init(layerNum,parent)
    require "luascript/script/game/scene/gamedialog/dimensionalWar/dimensionalWarInforSubTab11"
    require "luascript/script/game/scene/gamedialog/dimensionalWar/dimensionalWarInforSubTab12"
...
function dimensionalWarInforTab1:initSubTab()
    local tabStr={getlocal("dimensionalWar_rank_title1"),getlocal("dimensionalWar_rank_title2")}
...
function dimensionalWarInforTab1:switchSubTab(type,isEffect)
    ...
    local function rankListCallback( ... )
        self:realSwitchSubTab(type)
    end
    dimensionalWarVoApi:formatRankList(type-1,rankListCallback)
end
function dimensionalWarInforTab1:realSwitchSubTab(type)
    
    if(type==1)then
        if(self.tab1==nil)then
            self.tab1=dimensionalWarInforSubTab11:new()
            self.layerTab1=self.tab1:init(self.layerNum,self)
            self.bgLayer:addChild(self.layerTab1,1)
        end
    elseif(type==2)then
        if(self.tab2==nil)then
            self.tab2=dimensionalWarInforSubTab12:new()
...
function dimensionalWarInforTab1:refresh()

end

function dimensionalWarInforTab1:tick()
    for i=1,2 do
...
function dimensionalWarInforTab1:dispose()
    if(self.tab1)then
        self.tab1:dispose()
    end
    if(self.tab2)then
