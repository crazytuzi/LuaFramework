platWarDialogSubTab32={}

function platWarDialogSubTab32:new()
    local nc={}
...
function platWarDialogSubTab32:init(layerNum,parent)
...
function platWarDialogSubTab32:initDesc()
	local str=getlocal("plat_war_shop_desc2")
	...
	local status=platWarVoApi:getShopShowStatus(2)
	if status<=0 then
		self.maskSp:setPosition(ccp(25,25))
		self.descLb:setString(getlocal("plat_war_shop_open_1"))
	else
		if status==2 then
			self.maskSp:setPosition(ccp(10000,0))
		else
			self.maskSp:setPosition(ccp(25,25))
			self.descLb:setString(getlocal("plat_war_shop_open_2"))
		end
	end
end

function platWarDialogSubTab32:initTableView()
...
function platWarDialogSubTab32:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
		local rareList=platWarVoApi:getRareList()
	...
	elseif fn=="tableCellAtIndex" then
		...
		local showList=platWarVoApi:getRareList()
		local shopVo=showList[idx+1]
		local id=shopVo.id
		local num=shopVo.num or 0

		local rareItems=platWarVoApi:getShopRareItems()
...
function platWarDialogSubTab32:buyItem(shopVo)
	local id=shopVo.id
	...
	if (num<maxNum) and (platWarVoApi:getPoint()>=price) then
		...
		platWarVoApi:buyItem(2,id,callback)
	end
end

function platWarDialogSubTab32:doUserHandler()

end

function platWarDialogSubTab32:tick()
...
function platWarDialogSubTab32:refresh()

end

function platWarDialogSubTab32:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
    self.cellHeght=nil
