platWarReportDialogTab2={}
function platWarReportDialogTab2:new()
    local nc={}
...
function platWarReportDialogTab2:init(layerNum,parent)
...
function platWarReportDialogTab2:initLayer()
...
function platWarReportDialogTab2:eventHandler(handler,fn,idx,cel)
...
function platWarReportDialogTab2:getDescStr(vo)
    local descStr=""
    local isVictory=0
    local targetStr=""
    if vo then
...
--点击了cell或cell上某个按钮
function platWarReportDialogTab2:cellClick(idx)
    if self and self.tv and self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
        if G_checkClickEnable()==false then
            do
                return
...
function platWarReportDialogTab2:doUserHandler()
...
function platWarReportDialogTab2:tick()
    if self and self.mailClick and self.mailClick>0 then
        self.mailClick=0
    end
end

function platWarReportDialogTab2:dispose()
    self.noRecordLb=nil
    self.mailClick=nil
    self.canClick=nil
