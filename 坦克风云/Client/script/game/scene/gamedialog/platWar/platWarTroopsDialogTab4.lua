platWarTroopsDialogTab4 = {}

function platWarTroopsDialogTab4:new()
    local nc = {}
    ...
    self.platWarTanks3 = {{}, {}, {}, {}, {}, {}}
    self.platWarHero3 = {0, 0, 0, 0, 0, 0}
    self.platWarAITroops3 = {0, 0, 0, 0, 0, 0}
    self.platWarEmblem3 = nil
    self.platWarPlane3 = nil
...
function platWarTroopsDialogTab4:init(layerNum, parent)
...
--设置对话框里的tableView
function platWarTroopsDialogTab4:initTableView()
...
function platWarTroopsDialogTab4:initTabLayer()
    self:initTabLayer1()
    self:initSaveBtn()
    self:initFormationBtn()
end

function platWarTroopsDialogTab4:initTabLayer1()
    ...
    self.platWarTanks3 = G_clone(tankVoApi:getTanksTbByType(btype))
    self.maxPowerBtn3 = G_addSelectTankLayer(btype, self.bgLayer3, self.layerNum, callback)
    self.platWarHero3 = G_clone(heroVoApi:getPlatWarHeroList(self.selectedTabIndex + 1))
    self.platWarAITroops3 = G_clone(AITroopsFleetVoApi:getPlatWarAITroopsList(self.selectedTabIndex + 1))
    self.platWarEmblem3 = emblemVoApi:getBattleEquip(btype)
    self.platWarPlane3 = planeVoApi:getBattleEquip(btype)
    self.warAirship = airShipVoApi:getBattleEquip(btype)
    tankVoApi:setPlatWarTempTanks(G_clone(tankVoApi:getTanksTbByType(btype)))
end

function platWarTroopsDialogTab4:initSaveBtn()
    local function save()
        self:saveHandler()
    end
...
function platWarTroopsDialogTab4:saveHandler(callFunc)
    ...
    if isCanSetFleet == true then
        ...
        local costTanks, isSame = tankVoApi:setFleetCostTanks(self.platWarTanks3, fleetInfo)
...
function platWarTroopsDialogTab4:isChangeFleet()
    local bType = self.selectedTabIndex + 21
    local fleetInfo = tankVoApi:getTanksTbByType(bType)
    local costTanks, isSame = tankVoApi:setFleetCostTanks(self.platWarTanks3, fleetInfo)
    
    local hero1 = heroVoApi:getBindFleetHeroList(self.platWarHero3, self.platWarTanks3, bType, false)
    -- local heroList=heroVoApi:getPlatWarHeroList(self.selectedTabIndex+1)
    local heroList = heroVoApi:getTroopsHeroList()
    ...
    local aitroops1 = AITroopsFleetVoApi:getBindFleetAITroopsList(self.platWarAITroops3, self.platWarTanks3, bType, false)
...
function platWarTroopsDialogTab4:initFormationBtn()
...
function platWarTroopsDialogTab4:doUserHandler()
...
function platWarTroopsDialogTab4:tick()
    if self then
...
function platWarTroopsDialogTab4:setTanksRestore()
    local tType = self.selectedTabIndex + 21
    for i = 1, 6 do
        local id = i
        if self.platWarTanks3 and self.platWarTanks3[id] and self.platWarTanks3[id][1] then
            local tid = self.platWarTanks3[id][1]
            local num = self.platWarTanks3[id][2] or 0
            tankVoApi:setTanksByType(tType, id, tid, num)
        else
            tankVoApi:deleteTanksTbByType(tType, id)
        end
        tankVoApi:setPlatWarTempTanks(self.platWarTanks3)
        
        if self.platWarHero3 then
            local hid = self.platWarHero3[id]
            if hid then
        ...
        if self.platWarAITroops3 then
            local atid = self.platWarAITroops3[id]
            if atid then
    ...
    emblemVoApi:setTmpEquip(self.platWarEmblem3, tType)
    planeVoApi:setTmpEquip(self.platWarPlane3, tType)
    airShipVoApi:setTempLineupId(self.warAirship, tType)
end

function platWarTroopsDialogTab4:updateData()
    heroVoApi:clearTroops()
    AITroopsFleetVoApi:clearAITroops()
    self:setTanksRestore()
end
function platWarTroopsDialogTab4:refresh()
    heroVoApi:clearTroops()
    AITroopsFleetVoApi:clearAITroops()
    self:setTanksRestore()
    G_updateSelectTankLayer(self.selectedTabIndex + 21, self.bgLayer3, self.layerNum, self.currentShow)
    self:tick()
end

function platWarTroopsDialogTab4:dispose()
    self:setTanksRestore()
