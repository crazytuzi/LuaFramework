...
hellChallengeDialog = commonDialog:new()

function hellChallengeDialog:new( ... )
	local nc = {
		curIndex = 1,
		tabItemList = {},
		maxTankPage = 6,
...
function hellChallengeDialog:doUserHandler( ... )
	...
	local webImage=LuaCCWebImage:createWithURL(G_downloadUrl("limittask/hellChallenge.jpg"),onLoadIcon)
	...
	self.taskTb,self.pointTb,self.showTb = limitChallengeVoApi:getHellTask()
	self:initSizeTb()
...
function hellChallengeDialog:initDown( ... )
    ...
    for i=1,3 do
		...
		local tabItem = G_createBotton(self.bgLayer,ccp(20+72.5+150*(i-1),G_VisibleSizeHeight-85-282-37),{getlocal("limitChanllengeBtText"..i),strSize},"hellChallengeUnChose.png","hellChallengeChose.png","hellChallengeChose.png",touchInfo,1,-(self.layerNum-1)*20-4,nil,nil,nil,true)
	...
	local downSpire = LuaCCScale9Sprite:createWithSpriteFrameName("hellKuang.png",CCRect(15,15,1,1),function() end)
...
function hellChallengeDialog:eventHandler(handler,fn,idx,cel)
...
function hellChallengeDialog:initTableView( ... )
	-- body
end

function hellChallengeDialog:initGift( ... )
	...
	local point = limitChallengeVoApi:getHpoint()
	local minusPoint = 0

	local giftDetail = limitChallengeVoApi:getRewardHellDetail()
...
-- 获取下阶段的积分差值
function hellChallengeDialog:getMinusPoint(scoreList,point)
	for k,v in pairs(scoreList) do
		if point < v then
			return (v-point)
		end
...
function hellChallengeDialog:getGiftimg(i)
	local str 
	if i == 1 then
		str = "packs4.png"
	elseif i == 2 then
...
function hellChallengeDialog:touchInfo(idx)
...
function hellChallengeDialog:initCell(cell,idx)
	...
	if self.curIndex == 1 then
		...
		local cellBg = LuaCCScale9Sprite:createWithSpriteFrameName("hellChallengeItem.png",CCRect(5,5,1,1),onclick)
	    ...
	    cellBg:addChild(titleLabel)
	   	...
	   	local arrowSp = CCSprite:createWithSpriteFrameName("hellChallengeArrow1.png")
	   			...
	   			if count == (idx+1) then
	   				...
	   				for kk,vv in pairs(v) do
	   					...
	   					for i=1,num do
							...
							local expandBg = LuaCCScale9Sprite:createWithSpriteFrameName("rankKuang.png",CCRect(15,15,2,2),function() end)
		   					...
		   					if temp == sizeV and i == num then
			   					...
			   					local lineSp = LuaCCScale9Sprite:createWithSpriteFrameName("hellChallengeLine3.png",CCRect(2,1,1,1),function ()end)
							    lineSp:setContentSize(CCSizeMake(G_VisibleSizeWidth-90,3))
	    ...
	    for i=1,2 do
			local bottomLine = LuaCCScale9Sprite:createWithSpriteFrameName("hellChallengeLine2.png",CCRect(18,6,1,1),function () end)
			bottomLine:setContentSize(CCSizeMake((G_VisibleSizeWidth-65)/2,12))
	 ...
	 elseif self.curIndex == 2 then
	 	...
	 	local grayBgSp = LuaCCScale9Sprite:createWithSpriteFrameName("hellChallengebottom2.png",CCRect(4,4,1,1),function ()end)
...
function hellChallengeDialog:getRank(index)
...
function hellChallengeDialog:getCellNum( ... )
	if self.curIndex == 1 then
...
function hellChallengeDialog:getTaskNum( ... )
	local count = 0
...
function hellChallengeDialog:getCellSize(idx)
	if self.curIndex == 1 then
...
function hellChallengeDialog:initSizeTb( ... )
	self.sizeTb = {}
	for k,v in pairs(self.taskTb) do
...
function hellChallengeDialog:setExpandTb(idx)
	if self.sizeTb[idx+1].height > 50 then
...
function hellChallengeDialog:getChildNum(key)
...
function hellChallengeDialog:getKVFromTb(index)
	local count = 0
	for k,v in pairs(self.taskTb) do
		count = count + 1
		if index == count then
...
function hellChallengeDialog:getCellTitle(key)
...
function hellChallengeDialog:taskHandler( ... )
...
function hellChallengeDialog:rankHandler( ... )
	...
	if self.rankTb and (#self.rankTb) > 0 then
		...
		self.rankTb = limitChallengeVoApi:getHellRankData()
	end
	...
	local wholeBgSp=LuaCCScale9Sprite:createWithSpriteFrameName("hellChallengebottom1.png",CCRect(4, 4, 1, 1),function () end)
...
function hellChallengeDialog:tankHandler( ... )
    ...
    local lineSp = LuaCCScale9Sprite:createWithSpriteFrameName("hellChallengeLine3.png",CCRect(2,1,1,1),function ()end)
    lineSp:setContentSize(CCSizeMake(G_VisibleSizeWidth-30-20,3))
    lineSp:setAnchorPoint(ccp(0.5,0))
	...
	local arrowCfg={
		  {startPos=ccp(30,removeNode:getContentSize().height/2-40),targetPos=ccp(25,removeNode:getContentSize().height/2-40),callback=leftPageHandler,angle=180},
		  {startPos=ccp(removeNode:getContentSize().width-30,removeNode:getContentSize().height/2-40),targetPos=ccp(removeNode:getContentSize().width-25,removeNode:getContentSize().height/2-40),callback=rightPageHandler,angle=0}
	}
	for i=1,2 do
        local cfg=arrowCfg[i]
        local arrowBtn=GetButtonItem("hellChallengeArrow2.png","hellChallengeArrow2.png","hellChallengeArrow2.png",function () end,11,nil,nil)
...
function hellChallengeDialog:runTankMoveAction(offset)
...
function hellChallengeDialog:resetDisplayPage( ... )
...
function hellChallengeDialog:refreshTips( ... )
...
function hellChallengeDialog:leftPageHandler( ... )
	self:runTankMoveAction(-1)
end

function hellChallengeDialog:rightPageHandler( ... )
	self:runTankMoveAction(1)
end

-- 刷新数据
function hellChallengeDialog:refreshTV( ... )
...
function hellChallengeDialog:tick( ... )
	...
	-- 	if base.serverTime - self.oldTime >= 10 and self.requestFlag == 0 then
	-- 		self.requestFlag = 1
	-- 		self:refreshTV()			
	-- 	end
	-- end
...
function hellChallengeDialog:dispose( ... )
