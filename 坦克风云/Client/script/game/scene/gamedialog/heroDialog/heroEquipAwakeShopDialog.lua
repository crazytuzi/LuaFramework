heroEquipAwakeShopDialog = commonDialog:new()

function heroEquipAwakeShopDialog:new(closeCallback)
    local nc={}
...
function heroEquipAwakeShopDialog:resetTab()
...
function heroEquipAwakeShopDialog:initTableView()
	self.pShopItems=heroEquipAwakeShopCfg.pShopItems
...
function heroEquipAwakeShopDialog:eventHandler(handler,fn,idx,cel)
	...
	elseif fn=="tableCellAtIndex" then
        ...
        -- propSp=CCSprite:createWithSpriteFrameName(item[1].pic)
		...
		pointSp:setScale(0.7)
		pointSp:setAnchorPoint(ccp(0,0.5))
		pointSp:setPosition(ccp(-35,bgSp:getContentSize().height/2))
        ...
        local function exchange()
            if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
				...
				local function buycallback()
	                local function callback(fn,data)
	                    ...
	                    if ret==true then
	                        ...
	                        -- bagVoApi:addBag(item[1].id,item[1].num)
	                        -- bagVoApi:useItemNumId(self.id,self.num)
				...
				smallDialog:showSureAndCancle("PanelHeaderPopup.png",CCSizeMake(550,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),buycallback,getlocal("dialog_title_prompt"),getlocal("equip_shopBuy",{num  .. self.name,namestr}),nil,self.layerNum+1)
			end
        end
...
function heroEquipAwakeShopDialog:doUserHandler()
...
function heroEquipAwakeShopDialog:refresh()
	self.numLb:setString(bagVoApi:getItemNumId(self.id))
end

function heroEquipAwakeShopDialog:tick()
end

function heroEquipAwakeShopDialog:dispose()
