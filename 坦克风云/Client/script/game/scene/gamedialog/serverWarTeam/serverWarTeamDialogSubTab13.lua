serverWarTeamDialogSubTab13={}

function serverWarTeamDialogSubTab13:new()
	local nc={}
...
function serverWarTeamDialogSubTab13:init(layerNum)
...
function serverWarTeamDialogSubTab13:initTableView()
	...
	if rankList and SizeOfTable(rankList)>0 then
		...
		nameLabel:setPosition(widthSpace+150-30+40,height)
		self.bgLayer:addChild(nameLabel,1)
		
		local serverLabel=GetTTFLabel(getlocal("serverwar_server_name"),25)
		serverLabel:setPosition(widthSpace+150*2+20+70,height)
		self.bgLayer:addChild(serverLabel,1)
		
		-- local valueLabel=GetTTFLabel(getlocal("showAttackRank"),25)
...
function serverWarTeamDialogSubTab13:eventHandler(handler,fn,idx,cel)
   ...
   elseif fn=="tableCellAtIndex" then
		...
		if rankStr>=3 and rankStr<=4 then
			rankStr=3
		elseif rankStr>=5 and rankStr<=8 then
			rankStr=4
		end
