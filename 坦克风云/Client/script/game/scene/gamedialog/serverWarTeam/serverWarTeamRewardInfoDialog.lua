serverWarTeamRewardInfoDialog=commonDialog:new()

function serverWarTeamRewardInfoDialog:new()
	local nc={}
	setmetatable(nc,self)
	self.__index=self

	self.cellHeght1=320-40
	self.cellHeght2=200

	return nc
end

function serverWarTeamRewardInfoDialog:getDataByType()
	
end

function serverWarTeamRewardInfoDialog:initTableView()
	...
	local myRank=serverWarTeamVoApi:getMyRank()
	...
	if myRank and myRank>0 and rewardPoint and rewardPoint>0 then
		...
		-- local hasReward=serverWarTeamVoApi:getIsRewardRank()
		-- if hasReward==true then
			...
			local function rewardHandler(tag,object)
		        ...
		        local function rewardCallback()
			        serverWarTeamVoApi:setIsRewardRank(true)
		       	...
		       	serverWarTeamVoApi:rewardRank(rewardCallback)
			end
			local scale=0.8
			...
			local isReward=serverWarTeamVoApi:getIsRewardRank()
...
function serverWarTeamRewardInfoDialog:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
		local rankRewardCfg=serverWarTeamVoApi:getRankRewardCfg()
		local num=SizeOfTable(rankRewardCfg)
		return num
	elseif fn=="tableCellSizeForIndex" then
		local tmpSize
		local isHasServerReward=serverWarTeamVoApi:isHasServerReward(idx+1)
	...
	elseif fn=="tableCellAtIndex" then
		...
		local rankList=serverWarTeamVoApi:getRankList()
		...
		if rankVo then
			...
			allianceName=rankVo.name or ""
		end
		local rankStr=""
		local allianceStr=getlocal("serverwar_server_player",{serverName,allianceName})

		if idx<=(SizeOfTable(severRewardCfg)-1) then
			if idx==0 then
			...
			-- 		else
			-- 			rankStr=getlocal("serverwarteam_rank_reward",{minRank.."-"..maxRank})
			-- 		end
			-- 	else
			-- 		rankStr=getlocal("serverwar_rank_reward",{minRank})
			-- 	end
			-- end
			rankStr=getlocal("serverwarteam_rank_reward",{idx+1})
		end
		...
		if isHasServerReward==true then
			...
			descLb2:setPosition(ccp(20,backSprie:getContentSize().height-85+hSpace))
			backSprie:addChild(descLb2,1)

			local lineSp=CCSprite:createWithSpriteFrameName("LineCross.png")
	        lineSp:setAnchorPoint(ccp(0.5,0.5))
	        lineSp:setScale(0.95)
	        lineSp:setPosition(ccp(cellWidth/2,backSprie:getContentSize().height-115+hSpace))
...
function serverWarTeamRewardInfoDialog:dispose()
