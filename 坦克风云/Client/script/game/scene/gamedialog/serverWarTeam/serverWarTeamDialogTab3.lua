require "luascript/script/game/scene/gamedialog/serverWarTeam/serverWarTeamDialogSubTab31"
require "luascript/script/game/scene/gamedialog/serverWarTeam/serverWarTeamDialogSubTab32"
require "luascript/script/game/scene/gamedialog/serverWarTeam/serverWarTeamDialogSubTab33"
serverWarTeamDialogTab3={}

function serverWarTeamDialogTab3:new()
    local nc={}
...
function serverWarTeamDialogTab3:initTab(tabTb)
...
function serverWarTeamDialogTab3:resetTab()
	self.allTabs={}
	for i=1,3 do
...
function serverWarTeamDialogTab3:init(layerNum,serverWarDialog)
...
function serverWarTeamDialogTab3:initDesc()
	...
	self.myPointLb=GetTTFLabel(serverWarTeamVoApi:getPoint(),28)
...
function serverWarTeamDialogTab3:tabClick(idx)
...
function serverWarTeamDialogTab3:getDataByType(type)
	if(type==nil)then
		type=1
	end
	if(type==1)then
		if(self.subTab1==nil)then
			self.subTab1=serverWarTeamDialogSubTab31:new()
	...
	elseif(type==2)then
		if(self.subTab2==nil)then
			self.subTab2=serverWarTeamDialogSubTab32:new()
		...
		if(self.subTab3==nil)then
			local function getWarInfoHandler()
				local function formatPointDetailCallback()
					self.subTab3=serverWarTeamDialogSubTab33:new()
				...
				serverWarTeamVoApi:formatPointDetail(formatPointDetailCallback)
			end
...
function serverWarTeamDialogTab3:switchTab(type)
	if type==nil then
		type=1
	end
	for i=1,3 do
...
function serverWarTeamDialogTab3:doUserHandler()
	if self and self.myPointLb then
		self.myPointLb:setString(serverWarTeamVoApi:getPoint())
	end
end

function serverWarTeamDialogTab3:tick()
	for i=1,3 do
...
function serverWarTeamDialogTab3:refresh()

end

function serverWarTeamDialogTab3:dispose()
	for i=1,3 do
