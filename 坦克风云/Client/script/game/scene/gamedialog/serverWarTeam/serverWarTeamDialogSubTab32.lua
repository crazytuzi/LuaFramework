serverWarTeamDialogSubTab32={}

function serverWarTeamDialogSubTab32:new()
    local nc={}
...
function serverWarTeamDialogSubTab32:init(layerNum,parent)
...
function serverWarTeamDialogSubTab32:initDesc()
	...
	self.descLb=GetTTFLabelWrap(getlocal("serverwarteam_shop_can_buy_treasure"),30,CCSizeMake(self.maskSp:getContentSize().width-60,0),kCCTextAlignmentCenter,kCCVerticalTextAlignmentCenter)
	...
	local buyStatus=serverWarTeamVoApi:getBuyStatus()
	if buyStatus==1 then
...
function serverWarTeamDialogSubTab32:initTableView()
...
function serverWarTeamDialogSubTab32:eventHandler(handler,fn,idx,cel)
	local strSize2 = 20
	if G_getCurChoseLanguage() =="cn" or G_getCurChoseLanguage() =="ko" or G_getCurChoseLanguage() =="ja" or G_getCurChoseLanguage() =="tw" then
        strSize2 =25
    end
	if fn=="numberOfCellsInTableView" then
		local rareList=serverWarTeamVoApi:getRareList()
	...
	elseif fn=="tableCellAtIndex" then
		...
		local showList=serverWarTeamVoApi:getRareList()
		local shopVo=showList[idx+1]
		local id=shopVo.id
		local num=shopVo.num or 0

		local rareItems=serverWarTeamVoApi:getShopRareItems()
		...
		local nameLb=GetTTFLabel(table.concat(nameStrTb, ", "),strSize2)
		...
		local function onClick(tag,object)
			if self and self.tv and self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
				...
				-- 	self:buyItem(shopVo)
				-- -- end
				local saveLocalKey = "keyServerWarTeam2"
...
function serverWarTeamDialogSubTab32:buyItem(shopVo)
	local id=shopVo.id
	...
	if (num<maxNum) and (serverWarTeamVoApi:getPoint()>=price) then
		...
		serverWarTeamVoApi:buyItem(2,id,callback)
	end
end

function serverWarTeamDialogSubTab32:doUserHandler()

end

function serverWarTeamDialogSubTab32:tick()
...
function serverWarTeamDialogSubTab32:refresh()

end

function serverWarTeamDialogSubTab32:dispose()
	self.priceTb=nil
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
