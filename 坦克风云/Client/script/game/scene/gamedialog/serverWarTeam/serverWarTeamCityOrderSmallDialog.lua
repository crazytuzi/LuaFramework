--在跨服军团战向军团成员发送命令的面板
serverWarTeamCityOrderSmallDialog=smallDialog:new()

--param cityID: 城市ID, a1到a11
function serverWarTeamCityOrderSmallDialog:new(cityID)
	local nc={}
...
function serverWarTeamCityOrderSmallDialog:init(layerNum)
	self.isTouch=nil
...
function serverWarTeamCityOrderSmallDialog:initBackground()
...
function serverWarTeamCityOrderSmallDialog:initContent()
	...
	local cityName=GetTTFLabel(getlocal(serverWarTeamFightVoApi:getCity(self.cityID).cfg.name),28)
	...
	if(side==0)then
		...
		local attackers
		if(side==1)then
			attackers=serverWarTeamFightVoApi:getLivePlayersInCity(self.cityID,2)
		else
			attackers=serverWarTeamFightVoApi:getLivePlayersInCity(self.cityID,1)
		end
		if(#attackers>0)then
	...
	local function onConfirm()
		...
		serverWarTeamFightVoApi:sendOrder(self.data.id,self.curOrder,callback)
	end
...
function serverWarTeamCityOrderSmallDialog:dealEvent(event,data)
	if(data.type=="over")then
		self:close()
	end
end

function serverWarTeamCityOrderSmallDialog:dispose()
