serverWarTeamDialogSubTab33={}

function serverWarTeamDialogSubTab33:new()
    local nc={}
...
function serverWarTeamDialogSubTab33:init(layerNum,parent)
...
function serverWarTeamDialogSubTab33:initDesc()
...
function serverWarTeamDialogSubTab33:initTableView()
...
function serverWarTeamDialogSubTab33:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
        local pointDetail=serverWarTeamVoApi:getPointDetail()
        local num=SizeOfTable(pointDetail)
        -- local isHasMore=serverWarTeamVoApi:isHasMore()
        -- if isHasMore then
        --     num=num+1
        -- end
        return num
    ...
    elseif fn=="tableCellAtIndex" then
        ...
        local function cellClick(hd,fn,idx)
            if self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
                ...
                --             local addHeight=serverWarTeamVoApi:formatData(sData.data.alliance.events)
                --             local newHasMore=serverWarTeamVoApi:isHasMore()
                --             if serverWarTeamVoApi:getPage()>=1 then
                ...
                --     end
                -- end
                -- local page=serverWarTeamVoApi:getPage()
        ...
        local timeStr=serverWarTeamVoApi:getTimeStr(time)
        local timeLabel=GetTTFLabel(timeStr,20)
        timeLabel:setAnchorPoint(ccp(0.5,0.5))
        timeLabel:setPosition(ccp(79,height/2))
        cell:addChild(timeLabel,1)

        local textLabel=GetTTFLabelWrap(message,20,CCSizeMake(width,0),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter)
...
function serverWarTeamDialogSubTab33:doUserHandler()
...
function serverWarTeamDialogSubTab33:tick()
	local flag=serverWarTeamVoApi:getPointDetailFlag()
    local detailExpireTime=serverWarTeamVoApi:getDetailExpireTime()
    if (self.callbackNum<3 and ((detailExpireTime and detailExpireTime>0 and base.serverTime>=detailExpireTime) or flag==-1)) then
        ...
        serverWarTeamVoApi:setPointDetailFlag(-1)
        serverWarTeamVoApi:formatPointDetail(callback)
...
function serverWarTeamDialogSubTab33:refresh()

end

function serverWarTeamDialogSubTab33:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
    self.cellHeght=nil
