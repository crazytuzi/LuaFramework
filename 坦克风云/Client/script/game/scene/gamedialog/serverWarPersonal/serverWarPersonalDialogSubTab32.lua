serverWarPersonalDialogSubTab32={}

function serverWarPersonalDialogSubTab32:new()
    local nc={}
...
function serverWarPersonalDialogSubTab32:init(layerNum,parent)
...
function serverWarPersonalDialogSubTab32:initDesc()
	local str=getlocal("serverwar_shop_desc2")
	...
	local function showInfo()
		...
		local tabStr={"\n",getlocal("serverwar_shop_tip_4"),"\n",getlocal("serverwar_shop_tip_3",{serverWarPersonalCfg.shoppingtime}),"\n",getlocal("serverwar_shop_tip_2"),"\n",getlocal("serverwar_shop_tip_1"),"\n"};
		local tabColor={nil,G_ColorYellow,nil,G_ColorYellow,nil,G_ColorYellow,nil,G_ColorYellow,nil}
	...
	-- 	self.maskSp:setPosition(ccp(25,25))
	-- 	self.descLb:setString(getlocal("serverwar_cannot_reward"))
	-- else
		local buyStatus=serverWarPersonalVoApi:getBuyStatus()
		if buyStatus==2 then
			self.maskSp:setPosition(ccp(10000,0))
		else
			self.maskSp:setPosition(ccp(25,25))
			self.descLb:setString(getlocal("serverwar_cannot_reward_rare"))
		end
	-- end
end

function serverWarPersonalDialogSubTab32:initTableView()
...
function serverWarPersonalDialogSubTab32:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
		local rareList=serverWarPersonalVoApi:getRareList()
	...
	elseif fn=="tableCellAtIndex" then
		...
		local showList=serverWarPersonalVoApi:getRareList()
		local shopVo=showList[idx+1]
		local id=shopVo.id
		local num=shopVo.num or 0

		local rareItems=serverWarPersonalVoApi:getShopRareItems()
		...
		if(serverWarPersonalVoApi:getPoint()<price)then
		...
		local function onClick(tag,object)
			if self and self.tv and self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
				...
				-- 	self:buyItem(shopVo)
				-- -- end
				local saveLocalKey = "keyServerWarPersonal2"
...
function serverWarPersonalDialogSubTab32:buyItem(shopVo)
	local id=shopVo.id
	...
	if (num<maxNum) and (serverWarPersonalVoApi:getPoint()>=price) then
		local function callback()
		...
		serverWarPersonalVoApi:buyItem(2,id,callback)
	end
end

function serverWarPersonalDialogSubTab32:doUserHandler()

end

function serverWarPersonalDialogSubTab32:tick()
...
function serverWarPersonalDialogSubTab32:refresh()

end

function serverWarPersonalDialogSubTab32:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
    self.cellHeght=nil
