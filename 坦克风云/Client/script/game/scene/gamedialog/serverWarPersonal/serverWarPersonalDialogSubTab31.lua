serverWarPersonalDialogSubTab31={}

function serverWarPersonalDialogSubTab31:new()
    local nc={}
...
function serverWarPersonalDialogSubTab31:init(layerNum,parent)
...
function serverWarPersonalDialogSubTab31:initDesc()
...
function serverWarPersonalDialogSubTab31:initTableView()
...
function serverWarPersonalDialogSubTab31:eventHandler(handler,fn,idx,cel)
	if fn=="numberOfCellsInTableView" then
		local commonList=serverWarPersonalVoApi:getCommonList()
	...
	elseif fn=="tableCellAtIndex" then
		...
		local showList=serverWarPersonalVoApi:getCommonList()
		local shopVo=showList[idx+1]
		local id=shopVo.id
		local num=shopVo.num or 0

		local commonItems=serverWarPersonalVoApi:getShopCommonItems()
		...
		local function onClick(tag,object)
			if self and self.tv and self.tv:getScrollEnable()==true and self.tv:getIsScrolled()==false then
				...
				local saveLocalKey = "keyServerWarPersonal1"
...
function serverWarPersonalDialogSubTab31:buyItem(shopVo)
	local id=shopVo.id
	...
	if (num<maxNum) and (serverWarPersonalVoApi:getPoint()>=price) then
		...
		serverWarPersonalVoApi:buyItem(1,id,callback)
	end
end

function serverWarPersonalDialogSubTab31:doUserHandler()

end

function serverWarPersonalDialogSubTab31:tick()
...
function serverWarPersonalDialogSubTab31:refresh()

end

function serverWarPersonalDialogSubTab31:dispose()
    self.tv=nil
    self.bgLayer=nil
    self.layerNum=nil
    self.cellHeght=nil
