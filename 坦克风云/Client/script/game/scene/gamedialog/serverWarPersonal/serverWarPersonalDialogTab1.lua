require "luascript/script/game/scene/gamedialog/serverWarPersonal/serverWarPersonalDialogSubTab11"
require "luascript/script/game/scene/gamedialog/serverWarPersonal/serverWarPersonalDialogSubTab12"
require "luascript/script/game/scene/gamedialog/serverWarPersonal/serverWarPersonalDialogSubTab13"
require "luascript/script/game/scene/gamedialog/serverWarPersonal/serverWarPersonalRewardInfoDialog"
serverWarPersonalDialogTab1={}

function serverWarPersonalDialogTab1:new()
	local nc={}
...
function serverWarPersonalDialogTab1:init(layerNum,parent)
...
function serverWarPersonalDialogTab1:initSubTab()
...
function serverWarPersonalDialogTab1:setTipsVisibleByIdx(isVisible,idx)
    if self==nil then
        do
            return 
        end
...
function serverWarPersonalDialogTab1:subTabClick(type)
	...
	if(self["subTab"..type]==nil)then
		if(type==1)then
			self["subTab"..type]=serverWarPersonalDialogSubTab11:new()
			self["layerTab"..type]=self["subTab"..type]:init(self.layerNum,self.parent)
			self.bgLayer:addChild(self["layerTab"..type],1)
		elseif(type==2)then
			self["subTab"..type]=serverWarPersonalDialogSubTab12:new()
			self["layerTab"..type]=self["subTab"..type]:init(self.layerNum)
			self.bgLayer:addChild(self["layerTab"..type],1)
		elseif(type==3)then
			local function callback()
				self["subTab"..type]=serverWarPersonalDialogSubTab13:new()
				self["layerTab"..type]=self["subTab"..type]:init(self.layerNum)
				self.bgLayer:addChild(self["layerTab"..type],1)
			end
			serverWarPersonalVoApi:formatRankList(callback)
		end
	end
	for i=1,3 do
		if(type==i)then
	...
	if(type==2)then
		serverWarPersonalVoApi.todayScheduleTabHasClick=playerVoApi:getUid().."-"..base.curZoneID
		self:setTipsVisibleByIdx(false,2)
	elseif(type==3)then
		serverWarPersonalVoApi:setRankHasOpen()
	end
end

function serverWarPersonalDialogTab1:initBg()
...
function serverWarPersonalDialogTab1:checkInitPlayerTip()
...
function serverWarPersonalDialogTab1:tick()
	local rankHasOpen=serverWarPersonalVoApi:getRankHasOpen()
	...
	if(self.playerCurRound)then
		local roundStatus=serverWarPersonalVoApi:getRoundStatus(self.playerCurRound)
		if(roundStatus>=20 and roundStatus<30)then
			if(serverWarPersonalVoApi.todayScheduleTabHasClick~=playerVoApi:getUid().."-"..base.curZoneID)then
...
function serverWarPersonalDialogTab1:dispose()
	for i=1,3 do
