...
function chatDialogTab1:init(layerNum, selectedTabIndex, chatDialog, chatType)
    self.bgLayer = CCLayer:create()
    self.layerNum = layerNum
    self.selectedTabIndex = selectedTabIndex
    self.chatDialog = chatDialog
    if chatType and chatType > 3 then
...
--设置对话框里的tableView
function chatDialogTab1:initTableView()
...
function chatDialogTab1:eventHandler(handler, fn, idx, cel)
    if fn == "numberOfCellsInTableView" then
        local msgNum = chatVoApi:getChatNum(self.chatType)
        if msgNum <= 0 then
            do return end
        end
        return msgNum
    elseif fn == "tableCellSizeForIndex" then
        local chatVo = chatVoApi:getChatVo(idx + 1, self.chatType)
        if chatVo == nil then
            do return end
        end
        ...
        if(base.ifChatTransOpen == 1 and chatVo.params.language ~= G_getCurChoseLanguage()) and chatVo.type <= 3 and chatVo.contentType ~= 3 then
    ...
    elseif fn == "tableCellAtIndex" then
        ...
        local wSpace = 5
        local hSpace = 5
        if(base.ifChatTransOpen == 1 and chatVo.params.language ~= G_getCurChoseLanguage()) and type <= 3 and chatVo.contentType ~= 3 then
            hSpace = 60
        end
        ...
        if type <= 3 and chatVo.contentType ~= 3 then
            ...
            local noticeLabel = GetTTFLabel(getlocal("chat_system_notice"), 24)
            ...
            if params.paramTab and ((params.paramTab.redid and params.paramTab.redid>0) or params.paramTab.functionStr) then
                ...
                if params.paramTab.functionStr then
                    if params.paramTab.functionStr =="double11NewWithRedBag" and acDouble11NewVoApi then
                        if redbagTg then
                            ...
                            if params.paramTab.redBagTb.redtype ==1 then
                                acDouble11NewVoApi:showActionTip( cell,redbagTg,ccPos)
                            elseif params.paramTab.redBagTb.redtype ==2 then
               ...
               local function cellClick2(hd,fn,idx)
                    if G_checkClickEnable()==false or GM_UidCfg[playerVoApi:getUid()] then
                        do
                            return
                        end
                    end
                            ...
                            elseif params.paramTab.functionStr then
                                ...
                                if params.paramTab.functionStr ~="double11NewWithRedBag" and params.paramTab.functionStr ~="acXssd2019WithRedBag" then
                                    ...
                                    if params.paramTab.functionStr =="acXssd2019WithRedBag" then
                                        ...
                                        if params.paramTab.redBagTb.redtype ==1 then
                                            acDouble11NewVoApi:setNewGetRecord(params.paramTab.redBagTb)
                                        elseif params.paramTab.redBagTb.redtype ==2 then
                                    ...
                                    if redbagTg then
                                        ...
                                        -- chatVoApi:setChatVoKillRedBagTag(index,idx,redbagTag )

                                        if params.paramTab.functionStr =="acXssd2019WithRedBag" then
                                            acXssd2019VoApi:setRecBagTbTagNil(redbagTg,nil)
                                            chatVoApi:setChatVoKillRedBagTag(nil,1,redbagTg,3)
                                            self.chatDialog:refreshEveryTabRedBagIcon(redbagTg,2)
                                        elseif params.paramTab.redBagTb.redtype ==2 then
                                ...
                                elseif params.paramTab.functionStr == "map" and params.paramTab.airShipBoss then --飞艇欧米伽小队跳转
                                    jump_judgment(params.paramTab.functionStr, params.paramTab.airShipBoss)
                                else
        ...
        if chatVo.contentType and chatVo.contentType == 2 then --战报
            ...
            if chatVo.contentType and chatVo.contentType == 1 and content == getlocal("activity_koulinghongbao_desc") and acKoulinghongbaoVoApi then
        ...
        --注意下
        -- if isGM then
        -- if chatVo.subType == 2 then
        -- messageLabel:setColor(GM_Color2)
        -- else
        -- messageLabel:setColor(GM_Color)
        -- end
        -- end
        ...
        -- local acVo=acKoulinghongbaoVoApi:getAcVo()
        -- if acVo and activityVoApi:isStart(acVo)==true then
        -- messageLabel:setColor(G_ColorRed3)
        -- isChangeColor=true
        -- end
        -- end
        -- if isChangeColor==false then
...
--这里是tableView的详细逻辑 handler:方法索引  fn:方法名 idx:cell索引 cel:cell
function chatDialogTab1:eventHandlerNew(handler, fn, idx, cel)
    if fn == "numberOfCellsInTableView" then
    ...
    elseif fn == "tableCellAtIndex" then
        ...
        -- if type<=3 and chatVo.contentType~=3 and isGM == false then
        -- else
        -- headSp:setPosition(typeSp:getContentSize().width/2,typeSp:getContentSize().height/2)
        -- end
        -- if isGM then
        ...
        elseif params and params.xlpd_invite then --协力攀登活动邀请
        ...
        if type <= 3 and chatVo.contentType ~= 3 then
            ...
            if chatVo.sender and chatVo.senderName then--普通聊天和战报
                ...
                if allianceRoleLb then
                    ...
                    titleLb:setString("") --如果有军团职位就不显示称号
                else
                ...
                if G_chatVip == true then
                    ...
                    if vip and vip ~= 0 then
                        if platCfg.platCfgBMImage[G_curPlatName()] ~= nil then
                            ...
                            -- else
                            vipIcon:setAnchorPoint(ccp(0, 0.5))
                            vipIcon:setPosition(ccp(_posX, _posY))
...
--点击了cell或cell上某个按钮 type 1:用户信息,2:战报,3:军团战战报，5:远征战报，13:叛军坐标，可以跳转地图 16:分享数据
function chatDialogTab1:cellClick(idx, type)
    PlayEffect(audioCfg.mouseClick)
    if GM_UidCfg[playerVoApi:getUid()] and type > 3 then
        do return end
    end
    local chatVo = chatVoApi:getChatVoByIndex(idx, self.chatType)
    if chatVo and chatVo.type <= 3 then
                        ...
                        if(retData["ret"] == 0 or retData["ret"] == "0")then
                            if retData.data and retData.data.report and retData.data.report.content then
                                ...
                                if type ~= 16 then --战报的处理 type==16是分享的处理
                                    chatVoApi:setChatReport(idx, self.chatType, nil, data)
                                end
        ...
        if type == 1 then
            ...
            local function addBlackList()
                ...
                --             end
                -- smallDialog:showSureAndCancle("PanelHeaderPopup.png",CCSizeMake(550,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),confirmHandler,getlocal("dialog_title_prompt"),mailStr,nil,self.layerNum+2)
            end
            local function addMailList()
                ...
                -- -- 发送邀请成功
                --                smallDialog:showTipsDialog("PanelPopup.png",CCSizeMake(500,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),getlocal("addMailListSuccess",{name}),28)
                -- friendInfoVoApi:addFriendInvitesTb(chatVo.sender)
                
                -- if btnItem then
                -- btnItem:setEnabled(false)
                --           local btnLabel = tolua.cast(btnItem:getChildByTag(1016),"CCLabelTTF")
                ...
                --        end
                -- end
                -- socketHelper:sendfriendApply(chatVo.sender,callback)
                -- end
                -- smallDialog:showSureAndCancle("PanelHeaderPopup.png",CCSizeMake(550,400),CCRect(0, 0, 400, 350),CCRect(168, 86, 10, 10),confirmHandler,getlocal("dialog_title_prompt"),getlocal("mailListDesc",{name}),nil,self.layerNum+2)
            end
            local params = chatVo.params
        ...
        elseif type == 4 then
            local params = chatVo.params
            local redid = params.paramTab.redid
            print("self.redid: ", redid)
            if redid > 0 then
                local vrd = acGrabRedGrabDialog:new()
                local vd = vrd:init(self.layerNum + 1, redid)
            end
        elseif type == 6 then
            local params = chatVo.params
            local report
        ...
        elseif type == 15 then
            ...
            if activity and activity.key and activity.inviteCode then
                jump_judgment(activity.key, {inviteCode = activity.inviteCode})
            end
...
function chatDialogTab1:tick()
    --self:reloadChatDialog1WhenChatLogin()
    local selectedLanguage = chatVoApi:getSelectedLanguage()
    if chatVoApi:getHasNewData(self.chatType, selectedLanguage) == true then
        if(self.tv == nil or tolua.cast(self.tv, "LuaCCTableView") == nil)then
            do return end
        end
        local recordPoint = self.tv:getRecordPoint()
        --self:resetData()
        local tabIndex = self.chatType
        local isMaxMore = chatVoApi:getMaxMore(tabIndex, selectedLanguage)
        if isMaxMore == true then
            self.tv:removeCellAtIndex(0)
            chatVoApi:setMaxMore(tabIndex, false, selectedLanguage)
        end
        local msgNum = chatVoApi:getChatNum(tabIndex, selectedLanguage)
        self.tv:insertCellAtIndex(msgNum - 1)
        --mainUI:setLastChat()
        
        if eventDispatcher then
            eventDispatcher:dispatchEvent("dimensionalWarMap.chat", {})
        end
        chatVoApi:setNoNewData(tabIndex, selectedLanguage)
...
function chatDialogTab1:resetTvPos()
    local recordPoint = self.tv:getRecordPoint()
...
function chatDialogTab1:refresh()
    if self and self.tv then
        self.tv:reloadData()
        local selectedLanguage = chatVoApi:getSelectedLanguage()
        chatVoApi:setNoNewData(self.chatType, selectedLanguage)
        self:resetTvPos()
        self.isFirst = true
    end
end
...
--用户处理特殊需求,没有可以不写此方法
function chatDialogTab1:doUserHandler()
    
end

function chatDialogTab1:reloadChatDialog1WhenChatLogin()
    if self and self.tv and chatVoApi:getIsReload() then
        self.tv:reloadData()
        chatVoApi:setIsReload(false)
    end
end

function chatDialogTab1:dispose()
