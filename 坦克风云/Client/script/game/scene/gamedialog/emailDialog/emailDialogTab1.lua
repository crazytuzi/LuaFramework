...
function emailDialogTab1:init(layerNum,selectedTabIndex,emailDialog)
...
--设置对话框里的tableView
function emailDialogTab1:initTableView()
...
--[[
function emailDialogTab1:getDataByType(type)
	if type==nil then
		type=1
	end	
...
function emailDialogTab1:eventHandler(handler,fn,idx,cel)
	...
	elseif fn=="tableCellAtIndex" then
		...
		if hasMore and idx==num then
			...
			local moreLabel=GetTTFLabel(getlocal("showMoreTen"),24,true)
		...
		local emailIcon
		if emailVo.gift and emailVo.gift>=1 then
			emailIcon=CCSprite:createWithSpriteFrameName("mainBtnGift.png")
		elseif emailVo.isRead==1 then
		...
		if GM_UidCfg[emailVo.sender] then
			emailIcon=CCSprite:createWithSpriteFrameName(GM_Icon)
		...
		if emailVo.isAllianceEmail and emailVo.isAllianceEmail==1 then
			...
			fromToLabel:setPosition(_posX-GM_SubWidth,15)
		end
		...
		if titleStr and titleStr~="" then
			...
			titleLabel:setPosition(lbx-GM_SubWidth,80)
...
--点击了cell或cell上某个按钮
function emailDialogTab1:cellClick(idx)
...
function emailDialogTab1:showDetailDialog(emailVo)
...
function emailDialogTab1:refresh()
	if self~=nil then
...
function emailDialogTab1:dispose()
	self.mailClick=nil
	self.canClick=nil
