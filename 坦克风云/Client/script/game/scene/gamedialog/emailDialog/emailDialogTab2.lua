...
function emailDialogTab2:init(layerNum,selectedTabIndex,emailDialog)
...
--设置对话框里的tableView
function emailDialogTab2:initTableView()
...
--[[
function emailDialogTab2:getDataByType(type)
	if type==nil then
		type=1
	end	
...
function emailDialogTab2:eventHandler(handler,fn,idx,cel)
...
--点击了cell或cell上某个按钮
function emailDialogTab2:cellClick(idx)
...
function emailDialogTab2:showDetailDialog(emailVo)
	...
	if self.selectedTabIndex==1 then
		...
		if report~=nil then
			...
			elseif report.type==5 then
				titleStr=getlocal("search_report_title_3304")
			elseif report.type==6 then
				titleStr=getlocal("search_report_title_3305")
			elseif report.type==7 or report.type==8 then
				titleStr=getlocal("fight_content_return_title")
			elseif report.type==9 then
				titleStr=getlocal("hitfly_email_title1")
			elseif report.type==10 then
...
function emailDialogTab2:refresh()
	if self~=nil then
...
function emailDialogTab2:dispose()
	self.mailClick=nil
	self.canClick=nil
