...
local function UpdateTimeStamp()
    AddUpdateEvent("Event.XCCJ.updateTimeStamp", function(deltatime)
        local cd = self.infoList.freeCountUpdateTimeStamp - ServerTime.GetServerUnixTime()
        if not self.menu.Visible then
            RemoveUpdateEvent("Event.XCCJ.updateTimeStamp", true)
            return
        end
        if cd <= 0 then
            RemoveUpdateEvent("Event.XCCJ.updateTimeStamp", true)
            EventManager.Fire("Event.XCCJ.RequestInfo", {})
        else
...
local function RequestInfo()
    if self.menu then
        ActivityAPI.DailyDrawInfoRequest(1,function(data)
            if self.menu then
                self.infoList = data
...
function  _M.OnEnter()
    ...
    RequestInfo()
    EventManager.Subscribe("Event.XCCJ.RequestInfo",RequestInfo)
end

function _M.OnExit()
    RemoveUpdateEvent("Event.XCCJ.updateTimeStamp", true)
    EventManager.Unsubscribe("Event.XCCJ.RequestInfo",RequestInfo)
end
...
local function InitControls(view, names, tbl)
    ...
    self.btn_free.TouchClick = function (sender)
        ActivityAPI.DailyDrawRequest(0,0,1,function(data)
    ...
    self.btn_one.TouchClick = function (sender)
        if self.infoList.exploredTicketCountLeft > 0 then
            ActivityAPI.DailyDrawRequest(0,1,1,function(data)
                DealDrawReasult(data)
            end)
        else
            ...
            nil,nil,nil,nil,
            function()
                ActivityAPI.DailyDrawRequest(0,2,1,function(data)
                    DealDrawReasult(data)
                end)
            end,
            nil
    ...
    self.btn_ten.TouchClick = function (sender)
        if self.infoList.exploredTicketCountLeft >= 10 then
            ActivityAPI.DailyDrawRequest(1,1,1,function(data)
                DealDrawReasult(data)
            end)
        else
            ...
            nil,nil,nil,nil,
            function()
                ActivityAPI.DailyDrawRequest(1,2,1,function(data)
                    DealDrawReasult(data)
                end)
            end,
            nil
