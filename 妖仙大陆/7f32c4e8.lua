...
local function UpdateState(cell, state, idx)
    ...
    opBtn.TouchClick = OnOpCZBtnClick
...
local function UpdateCell(gx, gy, node)
    ...
    node:FindChildByEditName("tb_condition", true).UnityRichText = info.currNum/100 .. "/" .. info.needNum/100 .. Util.GetText(TextConfig.Type.SIGN, "yuan")
end
...
function OnOpCZBtnClick(sender)
    ...
    if info.state == ActivityAPI.StateCanNotGet then
        if self.activityData.ActivityKey == "leichong" then
            ...
            ActivityAPI.requestAward(self.activityData.ActivityID, info.awardId, emptyFunc)
        end
...
local function RequestInfo()
    if self.menu then
        local func = ActivityAPI.requestCumulativeRecharge(function ( data )
            if self.menu then
