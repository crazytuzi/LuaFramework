...
local function setSubViews(self)
...
function _M:setValues(rootValue,subValues)
    ...
    if rootView then
        ...
        if isAuto then
            ...
            for i = 1,(rootIndex - 1), 1 do
                ...
                rh = rh + rootView.root.Height
                if rootView.subCanvas.Visible then
                    rh = rh + rootView.subCanvas.Height
                end
            end
            if subIndex then
                local ctrl = self.subViews[rootIndex][subIndex]
                rh = rh + (subIndex - 1)*ctrl.Height
            end
            if rh + 150 > self.resSize2D.y then
                if self.scrollPan then
                    local function testYield()
                        local a = self.scrollPan.UnityObject:GetComponent("DisplayNodeBehaviour");
                        a:StartCoroutine(GameGlobal.Instance.WaitForSeconds(0.02, function()
                            self.scrollPan.Scrollable:LookAt(Vector2.New(0,rh + 150))
                        end ))
                    end
                    testYield()
                end
...
local function InitComponent(self,spacing,size2D)
    self.view = HZCanvas.New()
    self.view.UnityObject.name = "TreeView"
    self.view.Size2D = size2D
    local layout = self.view.UnityObject:AddComponent(typeof(UnityEngine.UI.VerticalLayoutGroup))
...
function _M.Create(spacing,indent,size2D,selectMode,funcCancel)
    local ret = {}
    setmetatable(ret,_M)
    if(spacing == nil) then
        spacing = 1
    ...
    InitComponent(ret,spacing,size2D)
    return ret    
end
