...
local function OnEnter()
  ...
  GdDepotRq.getDepotRecordRequest(1,function ()
      self.GuildRecord = GdDepotRq.GetDepotDynamic()
...
local function initUI()
  ...
  self.sp_see.Scrollable.event_Scrolled = function ()
    ...
    if not self.isRushRq and self.sp_see.Scrollable.Container.Y < size then
      ...
      self.isRushRq = true
      GdDepotRq.getDepotRecordRequest(pagenum,function (indexPage)
...
local function Init(params)
  self.menu = LuaMenuU.Create("xmds_ui/guild/guild_cangkudongtai.gui.xml", GlobalHooks.UITAG.GameUIGuildWareHouseDynamic)
