...
local function OnCardSubmit( self )
  ...
  if BcardLVUp then
    CardRq.CardPreLevelUpRequest(self.item.TemplateId,function ()
      Close(self)
      print (",,,,----"..CardRq.GetPreLvUPCard().s2c_card.id)
    ...
    local function num_input_cb(input_obj,result)
      ...
      DetailModel.UseItemRequest(item.Index, result, function (items)
        ...
        if result == pre_num then
          ...
          self.detailMenu:SetCurNum(pre_num - result)
        end
      end)
    end
...
local button_text = {
  ...
  ['Event.CardSubmit']      = {txt = Util.GetText(TextConfig.Type.ITEM,'btnRegister'),func = OnCardSubmit},
