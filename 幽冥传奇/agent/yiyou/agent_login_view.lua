...
function AgentLoginView:LoadCallBack()
	...
	local img_btn_bg
	local showBool = ClientHideBtnCfg[AgentAdapter:GetSpid()]
	if showBool then 
		img_btn_bg = XImage:create(ResPath.GetLogin("login_btn_bg_yiyou"), true)
	else
		img_btn_bg = XImage:create(ResPath.GetLogin("login_btn_bg"), true)
	end
	...
	local showBool = ClientHideBtnCfg[AgentAdapter:GetSpid()]
	if showBool then 
		img_btn_text = XUI.CreateImageView(btn_size.width / 2, btn_size.height / 2, ResPath.GetLogin("font_login_yiyou"), true)
	else
		img_btn_text = XUI.CreateImageView(btn_size.width / 2, btn_size.height / 2, ResPath.GetLogin("font_login"), true)
	end
