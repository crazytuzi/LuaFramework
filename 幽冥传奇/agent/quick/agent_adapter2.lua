...
AgentAdapter = {
    ...
    login_verify_url = "http://l.cq2.jianguogame.com:88/agent/jzby/login_verify.php",
    pay_verify_url = "http://l.cq2.jianguogame.com:88/agent/jzby/pay_verify.php",
	uid = "",
	token = "",
	channel_id = "";--quick渠道id
}
...
function AgentAdapter:GetPlatName()
	return "quick_"..self.channel_id.."_".. self.uid
end
...
function AgentAdapter:LoginCallback(result)--登陆后回调
	...
	local channelId = json_obj.channelId
	local verify_callback = function(url, arg, data, size)
		...
		self.channel_id = ret_t.user.channel_id
	...
	local real_url = string.format("%s?spid=%s&userId=%s&token=%s&device=%s&channelId=%s", self.login_verify_url, self:GetSpid(), userId, token, tostring(PlatformAdapter.GetPhoneUniqueId()),channelId)
...
function AgentAdapter:Pay(role_id, role_name, amount, server_id, callback)--购买
	...
	local pay_verify_callback = function(url, arg, data, size)
		...
		agent_helper:Pay(cjson.encode(param_obj), nil)
		return true
	end
	...
	param_obj.role_createtime = os.time()
	...
	Log("Pay_param_obj==123" .. cjson.encode(param_obj))
	--game_orderid=abcd123456789&game_price=60.00&subject=60.00_60
	local real_url = string.format("%s?uid=%s&amount=%s&count=%s&orderid=%s&server_id=%s&good_name=%s&plat=%s&token=%s&goods_desc=%s&role_createtime=%s", self.pay_verify_url, self.uid,amount, param_obj.count,orderid, server_id, param_obj.goods_name,self:GetPhonePlat(),self.token,param_obj.goods_desc,param_obj.role_createtime)
