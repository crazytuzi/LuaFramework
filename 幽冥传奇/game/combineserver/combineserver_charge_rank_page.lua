CombineServerChargeRankPage = CombineServerChargeRankPage or BaseClass()

function CombineServerChargeRankPage:__init()
	
end

function CombineServerChargeRankPage:__delete()
	self:RemoveEvent()
...
--初始化页面接口
function CombineServerChargeRankPage:InitPage(view)
	--绑定要操作的元素
...
--初始化事件
function CombineServerChargeRankPage:InitEvent()
	self.view.node_t_list.btn_charge_1.node:addClickEventListener(BindTool.Bind(self.GetChargeData, self))
	
end

function CombineServerChargeRankPage:RemoveEvent()

end

function CombineServerChargeRankPage:CreateRankInfoList()
	if not self.sports_rank_list then
		local ph = self.view.ph_list.ph_item_list
		self.sports_rank_list = ListView.New()
		self.sports_rank_list:Create(ph.x, ph.y, ph.w, ph.h, direction, CombineServerChargeRankItem, nil, false, self.view.ph_list.ph_list_item_1)
...
function CombineServerChargeRankPage:UpdateData(index)
	local data = nil
	local rank_data = CombineServerData.Instance:GetChanrgeConsumeRankData(8)
		data = CombineServerData.Instance:GetRechangeRankCfg()

		self.view.node_t_list.txt_my_charge.node:setString(string.format(Language.CombineServerActivity.ChargeYuanbao, rank_data.my_money))
...
function CombineServerChargeRankPage:GetChargeData()
...
--
CombineServerChargeRankItem = CombineServerChargeRankItem or BaseClass(BaseRender)
function CombineServerChargeRankItem:__init()
	self.cells_list = {}
	self.cells_view_container = nil
end

function CombineServerChargeRankItem:__delete()
...
function CombineServerChargeRankItem:CreateChild()
	BaseRender.CreateChild(self)
	self:CreateCellsContainer()
end

function CombineServerChargeRankItem:OnFlush()
...
function CombineServerChargeRankItem:CreateCellsContainer()
...
function CombineServerChargeRankItem:SetAwardCells(awards_data)
...
-- 创建选中特效
function CombineServerChargeRankItem:CreateSelectEffect()
