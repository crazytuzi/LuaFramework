...
CombindeServerArenaSucessPage = CombindeServerArenaSucessPage or BaseClass(XuiBaseView)

function CombindeServerArenaSucessPage:__init()
	self.is_modal = true
...
function CombindeServerArenaSucessPage:__delete()
	
end

function CombindeServerArenaSucessPage:ReleaseCallBack()
...
function CombindeServerArenaSucessPage:LoadCallBack(index, loaded_times)
...
function CombindeServerArenaSucessPage:OnFlush(paramt, index)
	for k,v in pairs(paramt) do
		if k == "Arena" then
			local data = v.key 
			local reward_data = CombineServerArenaCfg.arena.winAwards
			local cur_data = {}
			...
			-- 	self.node_t_list.img_bg_1.node:loadTexture(ResPath.GetStrenfthFb("bg_14"))
			-- end
		end
	end
end

function CombindeServerArenaSucessPage:CreateCell()
	self.reward_cell = {}
	for i = 1, 3 do
		local ph = self.ph_list["ph_rew_"..i]
		...
		self.node_t_list.layout_combine_arena_title.node:addChild(cell:GetView(), 103)
		table.insert(self.reward_cell, cell)
	end
end

function CombindeServerArenaSucessPage:OpenCallBack()
end

function CombindeServerArenaSucessPage:CloseCallBack()
end

function CombindeServerArenaSucessPage:OnExitFuben()
...
function CombindeServerArenaSucessPage:ChangeScene()
	if self:IsOpen() then
		self:Close()
	end
end