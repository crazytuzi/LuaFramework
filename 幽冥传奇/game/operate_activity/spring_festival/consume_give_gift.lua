-- 充值送礼界面
SpringConsumeGiveGiftPage = SpringConsumeGiveGiftPage or BaseClass()

function SpringConsumeGiveGiftPage:__init()
	self.view = nil

end

function SpringConsumeGiveGiftPage:__delete()
	self:RemoveEvent()
...
function SpringConsumeGiveGiftPage:InitPage(view)
	self.view = view
...
function SpringConsumeGiveGiftPage:InitEvent()
...
function SpringConsumeGiveGiftPage:RemoveEvent()
...
function SpringConsumeGiveGiftPage:CreateAwarInfoList()
	...
	self.view.node_t_list.layout_spring_consume_give_gift.node:addChild(self.normal_awar_list:GetView(), 99)
	...
	self.view.node_t_list.layout_spring_consume_give_gift.node:addChild(self.fetch_state_list:GetView(), 99)
	...
	self.view.node_t_list.layout_spring_consume_give_gift.node:addChild(self.special_awar:GetView(), 99)

end

function SpringConsumeGiveGiftPage:CreateNumBar()
	...
	self.view.node_t_list.layout_spring_consume_give_gift.node:addChild(self.num_bar:GetView(), 300, 300)
	local num = 0
	...
	self.view.node_t_list.layout_spring_consume_give_gift.node:addChild(self.num_bar_2:GetView(), 300, 300)
	local data_list = OperateActivityData.Instance:GetConsumeGiveAllFetchStateList()
	local num = #data_list
	self.num_bar_2:SetNumber(num)
end

function SpringConsumeGiveGiftPage:OnFetchStateListCallback(item, index)
	if nil == item or nil == item:GetData() then return end
	self:FlushShowAwards(index)
end

function SpringConsumeGiveGiftPage:OnConsumeGiveGiftEvent()
	local data_list = OperateActivityData.Instance:GetConsumeGiveAllFetchStateList()
...
function SpringConsumeGiveGiftPage:FlushShowAwards(index)
...
function SpringConsumeGiveGiftPage:FlushTime()
	local opened_day = OperateActivityData.Instance:GetConsumeGiveOpenInfo().opened_day
	local time_str = ""
...
function SpringConsumeGiveGiftPage:OnClickChongzhiHandler()
	if self.view then
		self.view:Close()
...
function SpringConsumeGiveGiftPage:OnClickFetchAwardHandler()
...
function SpringConsumeGiveGiftPage:UpdateData()
