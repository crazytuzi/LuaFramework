-- 充值送礼界面
ChargeGiveGiftPage = ChargeGiveGiftPage or BaseClass()

function ChargeGiveGiftPage:__init()
	self.view = nil

end

function ChargeGiveGiftPage:__delete()
	self:RemoveEvent()
...
function ChargeGiveGiftPage:InitPage(view)
	self.view = view
...
function ChargeGiveGiftPage:InitEvent()
...
function ChargeGiveGiftPage:RemoveEvent()
...
function ChargeGiveGiftPage:CreateAwarInfoList()
	...
	self.view.node_t_list.layout_charge_give_gift.node:addChild(self.normal_awar_list:GetView(), 99)
	...
	self.view.node_t_list.layout_charge_give_gift.node:addChild(self.fetch_state_list:GetView(), 99)
	...
	self.view.node_t_list.layout_charge_give_gift.node:addChild(self.special_awar:GetView(), 99)

end

function ChargeGiveGiftPage:CreateNumBar()
	...
	self.view.node_t_list.layout_charge_give_gift.node:addChild(self.num_bar:GetView(), 300, 300)
	local num = 0
	...
	self.view.node_t_list.layout_charge_give_gift.node:addChild(self.num_bar_2:GetView(), 300, 300)
	local data_list = OperateActivityData.Instance:GetChargeGiveAllFetchStateList()
	local num = #data_list
	self.num_bar_2:SetNumber(num)
end

function ChargeGiveGiftPage:OnFetchStateListCallback(item, index)
	if nil == item or nil == item:GetData() then return end
	self:FlushShowAwards(index)
end

function ChargeGiveGiftPage:OnChargeGiveGiftEvent()
	local data_list = OperateActivityData.Instance:GetChargeGiveAllFetchStateList()
...
function ChargeGiveGiftPage:FlushShowAwards(index)
...
function ChargeGiveGiftPage:FlushTime()
	local opened_day = OperateActivityData.Instance:GetChargeGiveOpenInfo().opened_day
	local time_str = ""
...
function ChargeGiveGiftPage:OnClickChongzhiHandler()
	if self.view then
		self.view:Close()
...
function ChargeGiveGiftPage:OnClickFetchAwardHandler()
...
function ChargeGiveGiftPage:UpdateData()
