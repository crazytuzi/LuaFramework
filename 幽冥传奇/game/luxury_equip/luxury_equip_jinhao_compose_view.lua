local LuxuryJihaoEquipComposeView = BaseClass(SubView)

function LuxuryJihaoEquipComposeView:__init()
	...
	self.config_tab = {
		...
		{"luxury_equip_ui_cfg", 7, {0}},
	}
end

LuxuryJihaoEquipComposeView_JinHao_EquipPos =   {
...
function LuxuryJihaoEquipComposeView:ReleaseCallBack()
...
function LuxuryJihaoEquipComposeView:LoadCallBack(index, loaded_times)
	--if loaded_times <= 1 then
		self.select_pos =  EquipData.EquipSlot.itAnCrownPos
		self.equip_cell = {}
		self:CreateItemCell()
		self:CreateCell()
		self:CreateNumBar()
...
function LuxuryJihaoEquipComposeView:OpenSuitView()
	 ViewManager.Instance:OpenViewByDef(ViewDef.LuxuryEquipTip)
end

function LuxuryJihaoEquipComposeView:RoleDataChangeCallback(vo)
	if vo.key == OBJ_ATTR.ACTOR_COIN then
		self:FlushRedPoint()
	end
end

function LuxuryJihaoEquipComposeView:ComposeEquip()
...
function LuxuryJihaoEquipComposeView:OpenShowDesc()
	DescTip.Instance:SetContent(Language.DescTip.JiHaoComposeContent, Language.DescTip.WanHaoComposeTitle)
end

function LuxuryJihaoEquipComposeView:CreateItemCell()
	self.equip_cell = {}
	for k, v in pairs(LuxuryJihaoEquipComposeView_JinHao_EquipPos) do
...
function LuxuryJihaoEquipComposeView:FlushWanHaoShow()
	for k, v in pairs(LuxuryJihaoEquipComposeView_JinHao_EquipPos ) do
...
function LuxuryJihaoEquipComposeView:CreateNumBar()
...
function LuxuryJihaoEquipComposeView:OnClickEquipCell(cell)
...
function LuxuryJihaoEquipComposeView:CreateCellRender(k)
	local cell = LuxyComposeEquipRender.New()
	local render_ph = self.ph_list["ph_jihao_item_render"..k] 
	cell:SetUiConfig(render_ph, true)
	cell:GetView():setPosition(render_ph.x, render_ph.y)
	self.node_t_list["layout_jinhao"].node:addChild(cell:GetView(), 101)
	if cur_data then
...
function LuxuryJihaoEquipComposeView:ItemDataListChangeCallback()
	self:FlushWanHaoShow()
	self:FlushRedPoint()
end

function LuxuryJihaoEquipComposeView:OnChangeOneEquip()
	self:FlushWanHaoShow()
	self:FlushRightShow()
	self:FlushRedPoint()
end

function LuxuryJihaoEquipComposeView:OpenCallBack()
	-- body
end

function LuxuryJihaoEquipComposeView:ShowIndexCallBack(index)
	self:Flush(index)
end

function LuxuryJihaoEquipComposeView:CloseCallBack()
	-- body
end

function LuxuryJihaoEquipComposeView:OnFlush( ... )
	self:FlushWanHaoShow()
...
function LuxuryJihaoEquipComposeView:CreateCell()
...
function LuxuryJihaoEquipComposeView:FlushRedPoint()
	for k, v in pairs(LuxuryJihaoEquipComposeView_JinHao_EquipPos) do
		local cell = self.equip_cell[v.equip_slot]
		if cell then
...
function LuxuryJihaoEquipComposeView:FlushRightShow()
...
function LuxuryJihaoEquipComposeView:FlushComsumeMoney(consume)
	local text = ""
...
return LuxuryJihaoEquipComposeView