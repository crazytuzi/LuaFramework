...
function BossTeamPage:CreateTeam()
	if self.select_data ~= nil then
		BossSportCtrl.Instance:CreateFubenTeam(self.select_data.fuben_id)
	end
end
function BossTeamPage:JoinTeam()
	if self.select_data ~= nil then
		BossSportCtrl.Instance:ReqJoinFubenTeam(2, self.select_data.fuben_id)
	end
end

function BossTeamPage:OpenFuBen()
	if self.leader_id == RoleData.Instance:GetAttr(OBJ_ATTR.ENTITY_ID) then
		BossSportCtrl.Instance:LeaderReqOpenFuben()
	else
...
function BossTeamPage:LevelTeam()
	BossSportCtrl.Instance:ReqExitFubenTeam(self.my_team_id)
	BossSportCtrl.Instance:ReqFubenData(self.my_fuben_id)
end
...
function BossTeamPage:UpdateData(data)
	local fuben_id, team_id, cur_data = BossSportData.Instance:GetMyData()
	...
	if self.cur_fuben_index == nil then
		if self.fuben_team_list ~= nil then
			if team_id == 0 then
				...
				self.fuben_team_list:SelectIndex(index)
			end
		end
	end
	local list = BossSportData.Instance:GetCurAllFubenTeamList()
	self.team_list:SetDataList(list)
	
	self.leader_id, self.my_team = BossSportData.Instance:GetMyTeamListData()
...
function BossTeamPage:SelectTeamBossListCallback(item, index)
	...
	if self.my_team_id == 0 then
		...
		self:FlushReward()
		BossSportCtrl.Instance:ReqFubenData(self.select_data.fuben_id)
	else
		local data = item:GetData()
		local index = item:GetIndex()
		if self.my_fuben_id ~= (data and data.fuben_id) then
			self.fuben_team_list:CancelSelect()
			SysMsgCtrl.Instance:FloatingTopRightText(Language.Boss.ErrorDesc)
		end
	end
end
...
function TeamBossRender:CreateChild()
	...
	self.node_tree.img_bg.node:loadTexture(ResPath.GetBigPainting("fuben_team"..self.index, true))
end
...
function TeamListRender:EnterTeam()
	BossSportCtrl.Instance:ReqJoinFubenTeam(1, self.data.team_id)
end
...
function MyTeamListRender:OnFlush()
	...
	local leader_id = BossSportData.Instance:GetMyTeamListData()
...
function MyTeamListRender:ShotOffTeam()
	local leader_id = BossSportData.Instance:GetMyTeamListData()
	if leader_id == RoleData.Instance:GetAttr(OBJ_ATTR.ENTITY_ID) then
		BossSportCtrl.Instance:ReqLeaderExitMemberSigleFubenTeam(self.data.member_id)
	else
		SysMsgCtrl.Instance:FloatingTopRightText(Language.Boss.FabBind)
	end
end
