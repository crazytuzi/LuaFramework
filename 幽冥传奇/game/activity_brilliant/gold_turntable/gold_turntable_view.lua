...
function GoldTurntableView:OnFlush(param_list, index)
	...
	for k,v in pairs(param_list) do
		if k == "result" and v.act_id == act_cfg.act_id and v.is_draw == 0  and v.result ~= 0 then
			...
			self.node_t_list.layout_point.node:stopAllActions()
			local rotate = self.node_t_list.layout_point.node:getRotation() % 360
			...
			self.node_t_list.layout_point.node:runAction(sequence)
		else
...
function GoldTurntableView:CreateGoldNum()
	...
	self.turntable_cap:SetPosition(cap_x + 20, cap_y + 10)
...
function GoldTurntableView:CreateTurntableReward()
	self.table_reward_t = {}
	local r = 165
	local x, y = self.node_t_list.layout_point.node:getPosition()
	for i = 1, TURNTABLE_COUNT do
		local cell = ActBaseCell.New()
		cell:SetPosition(x + r * math.cos(math.rad(70 - 360 / TURNTABLE_COUNT * (i - 1))),y + 10 + r * math.sin(math.rad(72 - 360 / TURNTABLE_COUNT * (i - 1))))
		cell:SetCellBg()
		cell:SetIndex(i)
		cell:SetAnchorPoint(0.5, 0.5)
		self.node_t_list.layout_gold_turntable.node:addChild(cell:GetView(), 15)
...
function GoldTurntableView:OnClickGoldTurntableHandler()
	...
	--跳过动画
	if self.node_t_list.layout_33_auto_hook.img_hook.node:isVisible() and not self.is_ten_draw then 
...
function GoldTurntableView:InitWorldRecord()
	...
	-- self.world_record_list:Create(ph.x + 170, ph.y + 150, ph.w - 25, ph.h, TurnItemRender)
...
function GoldTurntableView:FlushWorldRecord()
	local item_name_list,item_index_list = ActivityBrilliantData.Instance:GetTurntableList()
...
function TurntableItemRender:OnFlush()
	...
	if self.data then
		if self.data.award.type == tagAwardType.qatEquipment then
			self.qianggou_cell:SetData({["item_id"] = self.data.award.id, ["num"] = self.data.award.count, is_bind = self.data.award.bind, effectId = self.data.award.effectId})
		else
			local virtual_item_id = ItemData.GetVirtualItemId(self.data.award.type)
			if virtual_item_id then
				self.qianggou_cell:SetData({["item_id"] = virtual_item_id, ["num"] = self.data.award.count, is_bind = self.data.award.bind or 0})
			end
		end
	else
	...
	self.can_get_reward = self.data.count <= ActivityBrilliantData.Instance.gold_draw_num
	...
	local num = self.data.draw_num >= self.data.count and self.data.count or self.data.draw_num
	local color = self.data.draw_num >= self.data.count and "1eff00" or "DC143C"
	local text = string.format(Language.ActivityBrilliant.Text9, color, num, self.data.count)
