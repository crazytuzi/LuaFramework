...
function RareBossView:CreateMonsterAnimation()
	if nil == self.monster_display then
		...
		self.monster_display:SetAnimPosition(310,265)
...
function RareBossView:CreateBossList()
	...
	self.boss_list = RareBossView.PerBossListView.New()
	self.boss_list:Create(ph.x + 6, ph.y, ph.w, ph.h, nil, RareBossView.BossItemRender, nil, nil, self.ph_list.ph_boss_item)
...
function RareBossView:SelectBossListCallback(item)
...
RareBossView.BossItemRender = BaseClass(BaseRender)
local BossItemRender = RareBossView.BossItemRender
...
function BossItemRender:OnFlush()
	...
	if self.data.fubenName ~= nil and self.node_tree.rich_boss_name.node and nil ~= self.data.index then
		...
		self.node_tree.img_unopen.node:setVisible(not is_enough)
		local str = string.format(Language.Boss.PerBossName, color, self.data.fubenName, tip)
