...
-- 		Behavior.FindKillMonster(GuajiCache.monster_id)
-- 		return
-- 	end
...
-- 		end
-- 		GlobalEventSystem:Fire(ObjectEventType.BE_SELECT, target_obj, "select")
-- 	elseif target_obj and target_obj:GetType() == SceneObjType.Monster and target_obj:IsRealDead() then
-- 		local monster_id = target_obj.vo and target_obj.vo.monster_id or 0
-- 		if StdMonster and StdMonster[monster_id] and StdMonster[monster_id].bDeathDig then
-- 			MoveCache.end_type = MoveEndType.ExcavateBoss
-- 			self:MoveToObj(target_obj)
-- 			-- target_obj = self:SelectAtkTarget(false)
-- 			DiamondPetCtrl.Instance:SetObjId(target_obj:GetObjId())
-- 			return
-- 		else
-- 			target_obj = self:SelectAtkTarget(false)
-- 		end
