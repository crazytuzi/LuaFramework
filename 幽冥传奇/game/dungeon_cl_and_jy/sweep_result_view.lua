SweepResultView = SweepResultView or BaseClass(BaseView)

function SweepResultView:__init()
	self:SetModal(true)
	self.texture_path_list = {
		'res/xui/fuben_cl.png',
	}
	self.config_tab = {
        {"practice_result_ui_cfg", 3, {0}, nil, 999},
	}
...
function SweepResultView:__delete()

end

function SweepResultView:ReleaseCallBack()
...
function SweepResultView:LoadCallBack(index, loaded_times)
	XUI.AddClickEventListener(self.node_t_list.btn_get_one.node, BindTool.Bind(self.OnOne, self), true)
...
function SweepResultView:ShowIndexCallBack(index)
	self:Flush()
end

function SweepResultView:OnOne( ... )
	DungeonCtrl.EnterFubenReq(4, self.fb_id, 0)
	self:Close()
end

function SweepResultView:OnDoble()
...
function SweepResultView:SetData(awards, scene_id)
	self.awards = awards

	for k, v in pairs(FubenZongGuanCfg.fubens) do
		if v.senceid == scene_id then
...
function SweepResultView:OnFlush()
	...
	if not self.grid_view then
		...
		self.node_t_list.layout_sweep_result.node:addChild(self.grid_view, 300)
	end
...
function SweepResultView:PraseAward(awards)
	local list = {}
...
function SweepResultView:CloseCallBack(is_all)
