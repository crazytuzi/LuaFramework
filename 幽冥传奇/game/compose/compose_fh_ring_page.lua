--复活页面
ComposeFhPage = ComposeFhPage or BaseClass()


function ComposeFhPage:__init()
	self.view = nil
	self.page = nil
	self.upLevelBtn = nil
    self.lookBtn = nil
...
function ComposeFhPage:__delete()
...
--初始化页面接口
function ComposeFhPage:InitPage(view)
	--绑定要操作的元素
	self.view = view
	self.page = view.node_t_list.page7
	self.innerContainer = self.page.componment1.innerContainer.node
	self.innerContainerY = self.innerContainer:getPositionY()
	self.actionDir = 1
	self.equipType = ComposeType.FuHuoRing

	self.equipType = ComposeData.Instance:GetComposeTypeByItemType(ItemData.ItemType.itSpecialRing, TabIndex.compose_new_fh)
...
function ComposeFhPage:CreateViewElement()
	if nil == self.buy_scroll_list then
...
--初始化事件
function ComposeFhPage:InitEvent()
...
--移除事件
function ComposeFhPage:RemoveEvent()
...
function ComposeFhPage:Update(now_time, elapse_time)
...
--更新视图界面
function ComposeFhPage:UpdateData(data)
	local equip = EquipData.Instance:GetEquipByType(ItemData.ItemType.itSpecialRing, 1)
	if equip then
...
--点击升级
function ComposeFhPage:OnClickUpLevel()
	if IS_ON_CROSSSERVER then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.OnCrossServerTip)
		return 
	end	
...
--物品改变
function ComposeFhPage:ItemDataListChangeCallback()
	self:UpdateData()
end

function ComposeFhPage:RoleDataChangeCallback(key, value)
	if key == OBJ_ATTR.ACTOR_RING_CRYSTAL then
		self:UpdateData()
	end	
end	

function ComposeFhPage:UpdateAttr(equip)
...
function ComposeFhPage:UpdateShop()
	if self.buy_scroll_list then
		self.buy_scroll_list:SetDataList(ShopData.Instance:GetShopQuickBuyItem(QuicklyBuyType.Type_9))
	end
end	

function ComposeFhPage:Clear()
	for i = 1, 5 do
...
function ComposeFhPage:UpdateConsume(level)
	local step,star = ComposeData.Instance:GetStepStar(level)
