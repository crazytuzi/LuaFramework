--护体页面
ComposeHtPage = ComposeHtPage or BaseClass()


function ComposeHtPage:__init()
	self.view = nil
	self.page = nil
	self.upLevelBtn = nil
    self.lookBtn = nil
...
function ComposeHtPage:__delete()
...
--初始化页面接口
function ComposeHtPage:InitPage(view)
	--绑定要操作的元素
	self.view = view
	self.page = view.node_t_list.page6
	self.innerContainer = self.page.componment1.innerContainer.node
	self.innerContainerY = self.innerContainer:getPositionY()
	self.actionDir = 1
	self.equipType = ComposeType.HuTiRing

	self.equipType = ComposeData.Instance:GetComposeTypeByItemType(ItemData.ItemType.itSpecialRing, TabIndex.compose_new_ht)
...
function ComposeHtPage:CreateViewElement()
	if nil == self.buy_scroll_list then
...
--初始化事件
function ComposeHtPage:InitEvent()
...
--移除事件
function ComposeHtPage:RemoveEvent()
...
function ComposeHtPage:Update(now_time, elapse_time)
...
--更新视图界面
function ComposeHtPage:UpdateData(data)
...
--点击升级
function ComposeHtPage:OnClickUpLevel()
	if IS_ON_CROSSSERVER then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.OnCrossServerTip)
		return 
	end	
...
--物品改变
function ComposeHtPage:ItemDataListChangeCallback()
	self:UpdateData()
end

function ComposeHtPage:RoleDataChangeCallback(key, value)
	if key == OBJ_ATTR.ACTOR_RING_CRYSTAL then
		self:UpdateData()
	end	
end	

function ComposeHtPage:UpdateAttr(equip)
...
function ComposeHtPage:UpdateShop()
	if self.buy_scroll_list then
		self.buy_scroll_list:SetDataList(ShopData.Instance:GetShopQuickBuyItem(QuicklyBuyType.Type_9))
	end
end	

function ComposeHtPage:Clear()
	for i = 1, 5 do
...
function ComposeHtPage:UpdateConsume(level)
	local step,star = ComposeData.Instance:GetStepStar(level)
