--宝石页面
ComposeBsPage = ComposeBsPage or BaseClass()

function ComposeBsPage:__init()
	self.view = nil
	self.page = nil
	self.upLevelBtn = nil
	self.lookBtn = nil
...
function ComposeBsPage:__delete()
	ClientCommonButtonDic[CommonButtonType.COMPOSE_BS_ACTIVATE_BTN] = nil
	self:RemoveEvent()
...
--初始化页面接口
function ComposeBsPage:InitPage(view)
	--绑定要操作的元素
	self.view = view
	self.page = view.node_t_list.page3
	...
	self.equipType = ComposeType.Baoshi
	...
	ClientCommonButtonDic[CommonButtonType.COMPOSE_BS_ACTIVATE_BTN] = self.page.componment5.active_btn.node
	Runner.Instance:AddRunObj(self)
	
end	

function ComposeBsPage:OpenUnionView( )
	self.is_first_login = false
	ViewManager.Instance:Open(ViewName.UnionProperty)
	ViewManager.Instance:FlushView(ViewName.UnionProperty, 0, "Compose")
end

-- function ComposeBsPage:BoolCanShowActive()
...
function ComposeBsPage:CreateViewElement()
	if nil == self.buy_scroll_list then
		...
		-- self.buy_scroll_list:SetDataList(ShopData.Instance:GetShopQuickBuyItem(QuicklyBuyType.Type_5))
...
--初始化事件
function ComposeBsPage:InitEvent()
...
--移除事件
function ComposeBsPage:RemoveEvent()
...
function ComposeBsPage:Update(now_time, elapse_time)
...
--更新视图界面
function ComposeBsPage:UpdateData(data)
...
--激活
function ComposeBsPage:OnActive()
...
--升级点击
function ComposeBsPage:OnClickUpLevel()
	if IS_ON_CROSSSERVER then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.OnCrossServerTip)
		return 
	end	
...
--物品改变
function ComposeBsPage:ItemDataListChangeCallback()
	self:UpdateData()
end

function ComposeBsPage:RoleDataChangeCallback(key, value)
	if key == OBJ_ATTR.ACTOR_GEM_CRYSTAL then
		self:UpdateData()
	end	
end	

function ComposeBsPage:UpdateAttr(equip)
...
function ComposeBsPage:UpdateShop()
	if self.buy_scroll_list then
		self.buy_scroll_list:SetDataList(ShopData.Instance:GetShopQuickBuyItem(QuicklyBuyType.Type_5))
	end
end	

function ComposeBsPage:Clear()
	for i = 1, 5 do
...
function ComposeBsPage:UpdateConsume(level)
	local step,star = ComposeData.Instance:GetStepStar(level)
	...
	if consume then
		...
		local roleValue = RoleData.Instance:GetAttr(OBJ_ATTR.ACTOR_GEM_CRYSTAL)
