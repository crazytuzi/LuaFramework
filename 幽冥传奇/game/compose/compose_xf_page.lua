--血符页面
ComposeXfPage = ComposeXfPage or BaseClass()


function ComposeXfPage:__init()
	self.view = nil
	self.page = nil
	self.upLevelBtn = nil
	self.lookBtn = nil
...
function ComposeXfPage:__delete()
...
function ComposeXfPage:CreateViewElement()
	if nil == self.buy_scroll_list then
		...
		-- self.buy_scroll_list:SetDataList(ShopData.Instance:GetShopQuickBuyItem(QuicklyBuyType.Type_3))
		self.buy_scroll_list:JumpToTop()
	end
end	

--初始化事件
function ComposeXfPage:InitEvent()
...
--移除事件
function ComposeXfPage:RemoveEvent()
...
function ComposeXfPage:Update(now_time, elapse_time)
...
--更新视图界面
function ComposeXfPage:UpdateData(data)
	local equip = EquipData.Instance:GetEquipByType(ItemData.ItemType.itStoveBloodRune)--检测是否有血符装备
	if equip then
...
--激活
function ComposeXfPage:OnActive()
...
--升级点击
function ComposeXfPage:OnClickUpLevel()
	if IS_ON_CROSSSERVER then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.OnCrossServerTip)
		return 
	end	
...
--物品改变
function ComposeXfPage:ItemDataListChangeCallback()
	self:UpdateData()
	-- self:BoolCanShowActive()
end

function ComposeXfPage:RoleDataChangeCallback(key, value)
	if key == OBJ_ATTR.ACTOR_MAGIC_SOUL then
		self:UpdateData()
	end	
end	

function ComposeXfPage:UpdateAttr(equip)
...
function ComposeXfPage:UpdateShop()
	if self.buy_scroll_list then
		self.buy_scroll_list:SetDataList(ShopData.Instance:GetShopQuickBuyItem(QuicklyBuyType.Type_3))
	end
end	

function ComposeXfPage:Clear()
	for i = 1, 5 do
...
function ComposeXfPage:UpdateConsume(level)
	local step,star = ComposeData.Instance:GetStepStar(level)
	...
	if consume then
		...
		local roleValue = RoleData.Instance:GetAttr(OBJ_ATTR.ACTOR_MAGIC_SOUL)
