--魂珠页面
ComposeHzPage = ComposeHzPage or BaseClass()

function ComposeHzPage:__init()
	self.view = nil
	self.page = nil
	self.upLevelBtn = nil
	self.lookBtn = nil
...
function ComposeHzPage:__delete()
	ClientCommonButtonDic[CommonButtonType.COMPOSE_HZ_ACTIVATE_BTN] = nil
	self:RemoveEvent()
...
function ComposeHzPage:InitPage(view)
	...
	ClientCommonButtonDic[CommonButtonType.COMPOSE_HZ_ACTIVATE_BTN] = self.page.componment5.active_btn.node
	Runner.Instance:AddRunObj(self)
	
end	

function ComposeHzPage:CreateViewElement()
	if nil == self.buy_scroll_list then
		...
		-- self.buy_scroll_list:SetDataList(ShopData.Instance:GetShopQuickBuyItem(QuicklyBuyType.Type_6))
...
--初始化事件
function ComposeHzPage:InitEvent()
...
--移除事件
function ComposeHzPage:RemoveEvent()
...
function ComposeHzPage:Update(now_time, elapse_time)
...
--更新视图界面
function ComposeHzPage:UpdateData(data)
	local equip = EquipData.Instance:GetEquipByType(ItemData.ItemType.itStoveSealBead)--检测是否有血符装备
	if equip then
...
--激活
function ComposeHzPage:OnActive()
...
--升级点击
function ComposeHzPage:OnClickUpLevel()
	if IS_ON_CROSSSERVER then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.OnCrossServerTip)
		return 
	end	
...
--物品改变
function ComposeHzPage:ItemDataListChangeCallback()
	self:UpdateData()
end

function ComposeHzPage:RoleDataChangeCallback(key, value)
	if key == OBJ_ATTR.ACTOR_PEARL_CHIP then
		self:UpdateData()
	end	
end	

function ComposeHzPage:UpdateAttr(equip)
	...
	local prof = RoleData.Instance:GetAttr(OBJ_ATTR.ACTOR_PROF)
	local title_attrs_1 = RoleData.FormatRoleAttrStr(attrs_t, is_range)
	for i = 1, 5 do
...
function ComposeHzPage:UpdateShop()
	if self.buy_scroll_list then
		self.buy_scroll_list:SetDataList(ShopData.Instance:GetShopQuickBuyItem(QuicklyBuyType.Type_6))
	end
end	

function ComposeHzPage:Clear()
	for i = 1, 5 do
...
function ComposeHzPage:UpdateConsume(level)
	local step,star = ComposeData.Instance:GetStepStar(level)
	...
	if consume then
		...
		local roleValue = RoleData.Instance:GetAttr(OBJ_ATTR.ACTOR_PEARL_CHIP)
