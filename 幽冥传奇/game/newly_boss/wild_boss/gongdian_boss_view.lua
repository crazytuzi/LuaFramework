...
local GongDianBossView = BaseClass(SubView)

function GongDianBossView:__init()
...
function GongDianBossView:__delete()
end

function GongDianBossView:LoadCallBack(index, loaded_times)
	self.layer_list = nil
...
function GongDianBossView:ShowIndexCallBack()
	self:Flush()
end

function GongDianBossView:ReleaseCallBack()
...
-- 左边按钮点击
function GongDianBossView:OnBtnLeft()
...
-- 右边按钮点击
function GongDianBossView:OnBtnRight()
	local index = self.layer_list:GetCurPageIndex() or 0
...
function GongDianBossView:UpdateBtnState()
...
function GongDianBossView:CreateLayerList()
	local ph = self.ph_list.ph_layer_list
	local cell_num = #NewlyBossData.Instance:GetPalaceData()+1
	if nil == self.layer_list  then
		self.layer_list = BaseGrid.New() 
		self.layer_list:SetPageChangeCallBack(BindTool.Bind(self.OnPageChangeCallBack, self))
		local grid_node = self.layer_list:CreateCells({w = ph.w, h = ph.h, itemRender = GongDianBossView.LayerRender, ui_config = self.ph_list.ph_layer_item, cell_count = cell_num, col = 3, row = 1})
...
function GongDianBossView:OnPageChangeCallBack( ... )
	self:UpdateBtnState()
end

function GongDianBossView:OnMayaSelectCallBack(item)
	self.select_index = item:GetIndex() + 1
	self:SetLayerComsume(self.select_index)
end


function GongDianBossView:OnFlush(param_t)
	local boss_list = NewlyBossData.Instance:GetPalaceData()
...
function GongDianBossView:SetLayerComsume(index)
	...
	if item then
		...
		self.node_t_list.lbl_consume.node:setVisible(false)
		self.node_t_list.img_item.node:setVisible(false)
		self.node_t_list.lbl_need_num.node:setVisible(false)
	end
end

function GongDianBossView:RoleAttrChange(vo)
...
function GongDianBossView:OnClickTipHandler()
	DescTip.Instance:SetContent(Language.Boss.RareBossTips, Language.Boss.RareBossTipsName)
end

function GongDianBossView:OnClickChallengeHandler()
...
function GongDianBossView:SceneChange()
...
GongDianBossView.LayerRender = BaseClass(BaseRender)
local LayerRender = GongDianBossView.LayerRender
...
function LayerRender:CreateChild()
	...
	if self.boss_list == nil then
		...
		self.boss_list:Create(ph.x, ph.y, ph.w, ph.h, nil, GongDianBossView.MayaNumRender, nil, nil, self.ph_list.ph_bossnum_item)
...
function LayerRender:OnFlush()
	...
	local layer_data = NewlyBossData.Instance:GetSceneData(self.data.scene_id, 16)
	local lv_data = NewlyBossData.Instance:GetBossLv(layer_data, 16)
	
	self.boss_list:SetDataList(lv_data)
	
	self.node_tree.img_layer_bg.node:loadTexture(ResPath.GetBigPainting("mysd_bg_" .. 5, false))
	self.node_tree.img_layer_num.node:loadTexture(ResPath.GetBoss("gong_name_" .. self.data.layer))
end
...
GongDianBossView.MayaNumRender = BaseClass(BaseRender)
local MayaNumRender = GongDianBossView.MayaNumRender
...
function MayaNumRender:OnFlush()
	...
	local num = NewlyBossData.Instance:GetBossNum(self.data.id_cfg, 16)
