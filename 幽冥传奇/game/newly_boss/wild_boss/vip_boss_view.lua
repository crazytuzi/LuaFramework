...
local VipBossView = BaseClass(SubView)
...
function VipBossView:LoadCallBack(index, loaded_times)
	...
	self.node_t_list.rich_lv_need.node:setPositionX(560)
end

function VipBossView:ShowIndexCallBack()
	self.select_index = 1
...
function VipBossView:CreateMonsterAnimation()
...
function VipBossView:CreateBossList()
	...
	self.boss_list:Create(ph.x, ph.y, ph.w, ph.h, nil, VipBossView.VipBossRender, nil, nil, self.ph_list.ph_boss_item)
	...
	self.node_t_list.layout_other_boss.node:addChild(self.boss_list:GetView(), 20)
end

function VipBossView:CreateAwardCells()
...
function VipBossView:OnFlush(param_t)
	local boss_list
...
function VipBossView:OnGetUiNode(node_name)
	-- 选择boss
	...
	return VipBossView.super.OnGetUiNode(self, node_name)
end

function VipBossView:RoleAttrChange(vo)
...
function VipBossView:OnBossStateChange()
	self:Flush()
end

function VipBossView:SelectBossListCallback(item, index)
...
function VipBossView:FlushPersonalBossInfo(data)
	...
	if item then
		...
		self.node_t_list.lbl_need_num.node:setString(n .. "/" .. (StdMonster[data.boss_id].nKillDevilTokenLimit or 0))
...
function VipBossView:FlushAwardList(data_list)
	for k, v in pairs(self.award_cell_list) do
...
function VipBossView:SceneChange()
...
VipBossView.VipBossRender = BaseClass(BaseRender)
local VipBossRender = VipBossView.VipBossRender
function VipBossRender:__init()
end

function VipBossRender:__delete()
end

function VipBossRender:CreateChild()
	VipBossRender.super.CreateChild(self)
	XUI.RichTextSetCenter(self.node_tree.rich_boss_name.node)
	XUI.RichTextSetCenter(self.node_tree.rich_boss_lv.node)
end

function VipBossRender:OnFlush()
	...
	if self.data.boss_name ~= nil and self.node_tree.rich_boss_name.node and nil ~= self.data.rindex then
		...
		-- 	str_lv = string.format(Language.Boss.FieldBossLv, lv_color, self.data.bosslv)
			...
			txt_state = tip and tip .. "开启" or ""--string.format(Language.Boss.CircleBossLv[1], lv_color, tip)
...
-- function VipBossRender:CreateSelectEffect()
...
-- function VipBossRender:OnSelectChange(is_select)
...
VipBossView.PerBossListView = BaseClass(ListView)
local PerBossListView = VipBossView.PerBossListView
