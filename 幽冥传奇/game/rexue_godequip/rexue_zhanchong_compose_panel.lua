local  RexueZhanChongComposePanel = BaseClass(SubView)

function RexueZhanChongComposePanel:__init( ... )
...
function RexueZhanChongComposePanel:__delete( ... )
	-- body
end

function RexueZhanChongComposePanel:LoadCallBack( ... )
...
function RexueZhanChongComposePanel:ItemDataListChangeCallback( ... )
	self:FlushRightShow()
	self:FlushPoint()
end

function RexueZhanChongComposePanel:ComposePanel( ... )
	local index = 16 
...
function RexueZhanChongComposePanel:CreateZhangCHongAccordition( ... )
...
function RexueZhanChongComposePanel:CreateCell( ... )
...
function RexueZhanChongComposePanel:CreateNumber( ... )
...
function RexueZhanChongComposePanel:SelectTreeNodeCallBack(item)
	-- body
...
function RexueZhanChongComposePanel:SelectChildNodeCallback(item)
	if item == nil or item:GetData() == nil then
		return
	end
	local data = item:GetData()
...
function RexueZhanChongComposePanel:FlushRightShow()
	if  self.select_data then
		...
		local num = BagData.Instance:GetItemNumInBagById(consume[1].id, nil)
		...
		local equip_data =ZhanjiangCtrl.Instance:GetEquipData(type)
...
function RexueZhanChongComposePanel:OpenCallBack( ... )
	-- body
end

function RexueZhanChongComposePanel:CloseCallBack( ... )
	-- body
end

function RexueZhanChongComposePanel:ShowIndexCallBack(index)
	self:Flush(index)
end

function RexueZhanChongComposePanel:OnFlush( ... )
	local data  = ReXueGodEquipData.Instance:GetComspoeData(3)
	if self.tabbar_list then
...
function RexueZhanChongComposePanel:FlushPoint( ... )
	local data = ReXueGodEquipData.Instance:GetComspoeData(3)
	if data == nil then
		return
	end
...
return RexueZhanChongComposePanel