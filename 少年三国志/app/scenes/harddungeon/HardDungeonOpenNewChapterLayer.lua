local HardDungeonOpenNewChapterLayer = class("HardDungeonOpenNewChapterLayer", UFCCSModelLayer)

function HardDungeonOpenNewChapterLayer.create(...)
    return HardDungeonOpenNewChapterLayer.new("ui_layout/dungeon_DungeonOpenNewChpater.json",Colors.modelColor, ...)
end

function HardDungeonOpenNewChapterLayer:ctor(json, color, ...)
    self.super.ctor(self, json, color, ...)
    self:adapterWithScreen()
    local data = hard_dungeon_chapter_info.get(G_Me.hardDungeonData:getOpenNewChapterId())
    self:getLabelByName("Label_ChapterName"):setText(data.name)

    data = hard_dungeon_chapter_info.get(G_Me.hardDungeonData:getCurrChapterId())
...
function HardDungeonOpenNewChapterLayer:onLayerEnter(...)
...
function HardDungeonOpenNewChapterLayer:showEffect()
        self:_playEffect()
end

-- 播放盖章
function HardDungeonOpenNewChapterLayer:_playEffect()
...
-- 显示新章节动画
function HardDungeonOpenNewChapterLayer:playAnimation()
        -- 旧章节消失
        local oldChpater = self:getImageViewByName("Image_ChapterName")
...
-- 新章节动画
function HardDungeonOpenNewChapterLayer:showNewChpaterAnimation()
...
function HardDungeonOpenNewChapterLayer:sendShake()
    G_SoundManager:playSound(require("app.const.SoundConst").GameSound.STAR_SOUND)
    uf_eventManager:dispatchEvent(G_EVENTMSGID.EVENT_HARD_DUNGEON_SHOWSHAKE, nil, false,nil)
end
function HardDungeonOpenNewChapterLayer:_showTalk()
...
function HardDungeonOpenNewChapterLayer:onTouchEnd(xPos,yPos)
    local _parent = self:getParent()
    if G_Me.hardDungeonData:getCurrChapterId() == 3 or G_Me.hardDungeonData:getCurrChapterId() == 12 then
        local HeroQualityResult = require("app.scenes.common.HeroQualityResult")
        HeroQualityResult.showHeroQualityResult(
            function ( ... )
               _parent:addChild(require("app.scenes.harddungeon.HardDungeonNpcLayer").create(),5)
            end
        )
    else
        self:getParent():addChild(require("app.scenes.harddungeon.HardDungeonNpcLayer").create(),5)
    end
    self:close()
end

function HardDungeonOpenNewChapterLayer:onLayerExit( ... )
end

return HardDungeonOpenNewChapterLayer
