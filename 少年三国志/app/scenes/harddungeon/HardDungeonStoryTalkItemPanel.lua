local HardDungeonStoryTalkItemPanel = class("HardDungeonStoryTalkItemPanel",function()
    return Layout:create()
end
    )

function HardDungeonStoryTalkItemPanel:ctor(...)
    self._height = 0
    self.strList = {}
    self.maxHeight = 550
...
function HardDungeonStoryTalkItemPanel:createItem(name,text,pos,dialogue_type)
    self.isCanClick = false
    self._skip = false
...
-- 设置是否跳过当次对话
function HardDungeonStoryTalkItemPanel:setIsSkip(_bSkip)
    self._skip = _bSkip
end

function HardDungeonStoryTalkItemPanel:isSkip()
    return self._skip
end

-- 设置块透明度
function HardDungeonStoryTalkItemPanel:setItemOpacity(isGray)
...
function HardDungeonStoryTalkItemPanel:_movePanel(item)
    self:setPositionY(self._height)
end

function HardDungeonStoryTalkItemPanel:_showText(item,bg,dialogue_type)
...
function HardDungeonStoryTalkItemPanel:splitStr(str)
    for uchar in string.gfind(str, "[%z\1-\127\194-\244][\128-\191]*") do 
        self.strList[#self.strList+1] = uchar 
    end
end

function HardDungeonStoryTalkItemPanel:isClick()
    return self.isCanClick
end

function HardDungeonStoryTalkItemPanel:setClick(bClick)
    self.isCanClick = bClick
end

function HardDungeonStoryTalkItemPanel:touchMovePanel(dir)
...
return HardDungeonStoryTalkItemPanel
