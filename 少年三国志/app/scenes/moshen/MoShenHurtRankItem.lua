local MoShenHurtRankItem = class("MoShenHurtRankItem",function()
	--记得修改json
    return CCSItemCellBase:create("ui_layout/moshen_MoShenHurtRankItem.json")
end)
require("app.cfg.knight_info")

function MoShenHurtRankItem:ctor(...)

    self._labelName = self:getLabelByName("Label_name")   --名字
    self._labelLevel = self:getLabelByName("Label_level")  --等级
    self._shanghaiLabel = self:getLabelByName("Label_shanghai")    --伤害值
    ...
    -- self._shanghaiLabel:createStroke(Colors.strokeBrown,1)
    -- self._zhanliLabel:createStroke(Colors.strokeBrown,1)
    -- self:getLabelByName("Label_shanghaiTag"):createStroke(Colors.strokeBrown,1)
    -- self:getLabelByName("Label_zhanliTag"):createStroke(Colors.strokeBrown,1)

end

function MoShenHurtRankItem:checkZhenrong(func)
    self._checkPubFuc = func
end

function MoShenHurtRankItem:update(rebel)
    if rebel == nil then
        return
    end
    ...
    self._shanghaiLabel:setText(rebel.value)
