local WheelTopTxtCell = class ("WheelTopTxtCell", function (  )
	return CCSItemCellBase:create("ui_layout/wheel_PaiHangTxtCel.json")
end)

require("app.cfg.wheel_prize_info")

function WheelTopTxtCell:ctor(list, index,_type)
    self._type = _type

end

function WheelTopTxtCell:updateData( list, index,data)
    local info = data
    if info == nil then
        return
    end
    ...
    self:getLabelByName("Label_txt"):setText(G_lang:get("LANG_WHEEL_RANKSTR",{rank=str}))
    for i = 1 , 3 do 
