local CrossPVPKickAnimation = class("CrossPVPKickAnimation",function()  return display.newNode() end )
...
function CrossPVPKickAnimation.create(attack, defense, from, callback)
	local animation = CrossPVPKickAnimation.new()
	animation:init(attack, defense, from, callback)

	return animation
end

function CrossPVPKickAnimation:init(attack, defense, from, callback)
	self._from = from 
	self._attack = attack
