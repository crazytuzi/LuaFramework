local CrossPVPFlowerRankItem = class("CrossPVPFlowerRankItem", function()
	return CCSItemCellBase:create("ui_layout/crosspvp_FlowerRankItem.json")
end)

require("app.cfg.crosspvp_value_info")

function CrossPVPFlowerRankItem:ctor()
	self:enableLabelStroke("Label_UserName", Colors.strokeBrown, 1)
end

function CrossPVPFlowerRankItem:update(rank, data)
	...
	self:showTextWithLabel("Label_RankNum", tostring(data.sp2))
	self:showTextWithLabel("Label_FlowerNum", tostring(data.sp3))
	self:showTextWithLabel("Label_EggNum", tostring(data.sp4))
...
function CrossPVPFlowerRankItem:_updateBoardColor(isMe)
	if isMe then
