-- PetBagBookCellThree.lua

require("app.cfg.pet_info")
local MergeEquipment = require("app.data.MergeEquipment")
local EffectNode = require "app.common.effects.EffectNode"

local PetBagBookCellThree = class("PetBagBookCellThree",function()
    return CCSItemCellBase:create("ui_layout/petbag_BookCell3.json")
end)

PetBagBookCellThree.MAX_CELL_NUM = 3 -- 战宠最大数量
...
function PetBagBookCellThree:ctor()

        self:setTouchEnabled(true)
        for i = 1, PetBagBookCellThree.MAX_CELL_NUM do
...
function PetBagBookCellThree:click(index)
...
function PetBagBookCellThree:updateData(data)
...
function PetBagBookCellThree:initPet(index,baseid)
...
function PetBagBookCellThree:getWidth()
      local width = self:getContentSize().width
      return width
end

function PetBagBookCellThree:updateAttrs(data )
    ...
    if data then
        ...
        for i = 1,4 do 
            ...
            if data["attribute_value_"..i] == 0 then
                ...
                if G_Me.bagData.petData:hasPetBookById(data.pet_1) and
                    G_Me.bagData.petData:hasPetBookById(data.pet_2) and
                    G_Me.bagData.petData:hasPetBookById(data.pet_3) then
