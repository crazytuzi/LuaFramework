--LegionDamageRankLegionItem.lua


local LegionDamageRankLegionItem = class("LegionDamageRankLegionItem", function ( ... )
	return CCSItemCellBase:create("ui_layout/legion_DungeonMemberRankItem.json")
end)

function LegionDamageRankLegionItem:ctor( ... )
	self:enableLabelStroke("Label_member_name", Colors.strokeBrown, 1 )
end

function LegionDamageRankLegionItem:updateItem( memberIndex )
	if type(memberIndex) ~= "number" then 
		memberIndex = 1 
	end
	
	local rankInfo = G_Me.legionData:getLegionRankByIndex(memberIndex)
...
return LegionDamageRankLegionItem
