...
local DungeonStoryTalkLayer = class("DungeonStoryTalkLayer", UFCCSModelLayer)
...
function DungeonStoryTalkLayer:ctor(json,obj,data,...)
...
--    self._storyId = 2021
    ...
    self._moveY = 0
    self._step = 1
    self.talkitemPanel = require("app.scenes.dungeon.DungeonStoryTalkItemPanel").new()
...
function DungeonStoryTalkLayer:onLayerEnter( ... )
    self:showBgAction(true)
    __Log("DungeonStoryTalkLayer:onLayerEnter isRunnint:%d", self:isRunning() and 1 or 0)
end

-- 背景动画
function DungeonStoryTalkLayer:showBgAction(isUp)
...
function DungeonStoryTalkLayer.create(data)
    ...
    --                 __Log("_onEquipment")
    --         end }))
    --return DungeonStoryTalkLayer.new("ui_layout/dungeon_DungeonStoryTalkLayer.json",Colors.modelColor,data)
end



-- @desc 显示对话
function DungeonStoryTalkLayer:_showTalk()
    local data = story_dialogue.get(self._storyId,self._step)
    if data then
...
-- 显示道具特效
function DungeonStoryTalkLayer:_showProp(data)
...
-- 道具跳动动画
function DungeonStoryTalkLayer:_jumpProp()
...
function DungeonStoryTalkLayer:onLayerLoad( ... )
end

function DungeonStoryTalkLayer:onTouchEnd(x,y)
...
function DungeonStoryTalkLayer:onTouchBegin(x,y)
    self._moveY = y
end

function DungeonStoryTalkLayer:onTouchMove(x,y)
    local pt = ccp(x,y)
    if  G_WP8.CCRectContainPt(self._clippPanel:getCascadeBoundingBox(), pt) then
...
function DungeonStoryTalkLayer:onLayerExit()
    self._callback  = nil
end

function DungeonStoryTalkLayer:showFace(resId)
    for i=1,3 do
        if resId[i] ~= 0 then
...
function DungeonStoryTalkLayer:showFaceAction(face,_status)
...
-- 人物创建
function DungeonStoryTalkLayer:showRole(monsterId,toward,talkRoleId)
    local name = "" 
    local pos = 0
    for i=1,3 do
...
-- 角色移动
function DungeonStoryTalkLayer:move(role,_posType,_status,color,isJump)
