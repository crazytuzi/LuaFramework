...
local DungeonBoxLayer = class("DungeonTopLayer", UFCCSModelLayer)
    
function DungeonBoxLayer.create(name,value,id,startPos, ...)
    return DungeonBoxLayer.new("ui_layout/dungeon_DungeonBoxLayer.json", Colors.modelColor, name, value, id, startPos, ...)
end

function DungeonBoxLayer:ctor(json,color,box_Type,value,id,startPos,...)
    --self:init()
...
function DungeonBoxLayer:onLayerEnter()
...
function DungeonBoxLayer:onBackKeyEvent( ... )
    self:_closeWindow()
    return true
end

-- 缓动动画
function DungeonBoxLayer:showAnimation(dir)
    self._dir = dir
    local startScale = 1
    local endScale = 1
...
function DungeonBoxLayer:_appendRichText(txt,color)
    local str = "<text value='" .. txt .. "'color='" .. color .. "'/>" 
    return str
end

function DungeonBoxLayer:_createRichText()
...
function DungeonBoxLayer:_initGateBouns(id)
...
function DungeonBoxLayer:setStarBoxStatus(gateStar,isGet,conditionTxt,_totalStar)
    local _btn = self:getButtonByName("getbounsbtn")
    local _descLabel = self:getLabelByName("Label_Desc")
...
function DungeonBoxLayer:_init(box_Type,value,id)
    self.bounsId = id
    self._name = name
    ...
    if box_Type ~= BOXTYPE.STORYGATEBOX then
        local chapterId = G_Me.dungeonData:getCurrChapterId()
        local data = dungeon_chapter_info.get(chapterId)
        local _totalStar = G_Me.dungeonData:getChapterStar(chapterId)


        local _isOpenCopperbox,_isOpenSilverbox,_isOpenGoldbox = 
            G_Me.dungeonData:getBoxStuatus(chapterId)
    ...
    for i=1,5 do
        ...
        local data = G_Goods.convert(_dropInfo["type_" .. tostring(i)],_dropInfo["value_" .. tostring(i)])
        if bouns then
            if data then
                bouns:setTag(i)
                self:registerWidgetTouchEvent("bouns" .. tostring(i),handler(self,self.onClick))
                self:_setBouns(bouns,{info = data,bounsnum = _dropInfo["max_num_" .. tostring(i)]})
...
function DungeonBoxLayer:_initStoryGateBouns()
...
--@desc 设置奖励信息
function DungeonBoxLayer:_setBouns(widget,data)
...
function DungeonBoxLayer:_closeWindow()
    --self:close()
    self:showAnimation("small")
end

function DungeonBoxLayer:_getBouns()
    -- 检查包裹已满
    ...
    if self._boxType == BOXTYPE.DUNGEONGATEBOX then
        G_HandlersManager.dungeonHandler:sendExecuteStage(self.bounsId, true)
    elseif self._boxType == BOXTYPE.STORYGATEBOX then
        ...
        G_HandlersManager.dungeonHandler:sendFinishChapterBoxRwd(G_Me.dungeonData:getCurrChapterId(),self._boxType)
    end
    self:_closeWindow()
end

-- @desc 点击物品信息
function DungeonBoxLayer:onClick(widget,_type)
...
function DungeonBoxLayer:_setParentLayer( parent )
    self._parent = parent
