local DungeonSubtitleLayer = class("DungeonSubtitleLayer", UFCCSModelLayer)

function DungeonSubtitleLayer.create(bgTexture, ...)
    return DungeonSubtitleLayer.new("ui_layout/dungeon_DungeonSubtitleLayer.json", nil, bgTexture, ...)
end

function DungeonSubtitleLayer:ctor(json,color,bgTexture, ...)
    self.super.ctor(self, json, color, ...)
    self:adapterWithScreen()
    self.strList = {}
    self._timer  = nil
    self._isFirstEnter = G_Me.dungeonData:isFirstEnter(G_Me.dungeonData:getCurrChapterId())
...
function DungeonSubtitleLayer:onLayerExit()
    if self._timer then
...
function DungeonSubtitleLayer:startShowDesc()
    -- 打完最后一个怪才显示对话
    if  self._isFirstEnter == true then 
...
-- 显示通关动画
function DungeonSubtitleLayer:showPassAction()
...
function DungeonSubtitleLayer:splitStr(str)
    for uchar in string.gfind(str, "[%z\1-\127\194-\244][\128-\191]*") do 
        self.strList[#self.strList+1] = uchar 
    end
end

function DungeonSubtitleLayer:onTouchEnd()
...
function DungeonSubtitleLayer:_doExitLayer( ... )
    if self.__EFFECT_FINISH_CALLBACK__ then 
        self.__EFFECT_FINISH_CALLBACK__()
        self.__EFFECT_FINISH_CALLBACK__ = nil 
    end

    uf_eventManager:dispatchEvent(G_EVENTMSGID.EVENT_DUNGEON_CLOSESUBTITLElAYER, nil, false,nil)
    self:unregisterTouchEvent()
    self:close()
end

return DungeonSubtitleLayer
