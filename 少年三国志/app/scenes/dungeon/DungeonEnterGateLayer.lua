...
local DungeonEnterGateLayer = class("DungeonEnterGateLayer", UFCCSModelLayer)
...
function DungeonEnterGateLayer:onBackKeyEvent( ... )
    self:_closeWindow()
    return true
end

function DungeonEnterGateLayer.create(stageId,lastStarNum,callback, ...)
    return DungeonEnterGateLayer.new("ui_layout/dungeon_DungeonEnterGateLayer.json",Colors.modelColor,stageId,lastStarNum,callback, ...)
end

function DungeonEnterGateLayer:ctor(json,color,stageId,lastStarNum,callback,...)
    ...
    --关卡执行次数
    local _data = G_Me.dungeonData:getStageById(self._stageId)
    ...
    if roleData then
        -- 经验获得
        self:showTextWithLabel("Label_ExperienceValue", roleData.pve_exp)
        -- 获得银两
        self:showTextWithLabel("Label_SilverValue", roleData.pve_money) 

        self:getLabelByName("Label_rookieInfo"):setText(
            G_Me.rookieBuffData:checkInBuff() and G_lang:get("LANG_ROOKIE_BUFF_PERIOD") or "")
        --新手光环经验
        self:getLabelByName("Label_rookieBuffValue"):setText(G_Me.userData:getExpAdd(roleData.pve_exp))
    end

    -- 精英图片不显示
    self:getImageViewByName("Image_hard"):setVisible(false)
    ...
    self:showTextWithLabel("Label_config", G_lang:get("LANG_DUNGEON_DOUBLE"))
...
function DungeonEnterGateLayer:showExeCount()
    local _stageData = dungeon_stage_info.get(self._stageId)
    local _dungeonInfo = G_GlobalFunc.getDungeonData(_stageData.value)
    local _data = G_Me.dungeonData:getStageById(self._stageId)
    -- 1.先根据当前体力，算出可扫荡次数
...
function DungeonEnterGateLayer:_restDungeon()
    --self:close()
      local _seckillBtn = self:getButtonByName("Button_Seckill")
      if _seckillBtn then
        _seckillBtn:setTouchEnabled(false)
      end
    G_HandlersManager.dungeonHandler:sendResetDungeonExecution(self._stageId)
end


function DungeonEnterGateLayer:_closeWindow()
    self:animationToClose() 
end

function DungeonEnterGateLayer:onLayerExit()
...
function DungeonEnterGateLayer:onLayerEnter()
    self:closeAtReturn(true)
    uf_eventManager:addEventListener(G_EVENTMSGID.EVENT_DUNGEON_DUNGEONRESTSUCC, self.restDungeonSucc, self)
    uf_eventManager:addEventListener(G_EVENTMSGID.EVENT_RECV_ROLE_INFO, self.showExeCount, self)
    require("app.common.effects.EffectSingleMoving").run(self:getWidgetByName("ImageView_762"), "smoving_bounce", function ( event )
        if event and event == "finish" then
...
function DungeonEnterGateLayer:restDungeonSucc()
     self:showExeCount()
     self:_initSeckillStatus()
    local _seckillBtn = self:getButtonByName("Button_Seckill")
...
-- 秒杀状态
function DungeonEnterGateLayer:_initSeckillStatus()
    local _stageData = G_Me.dungeonData:getStageById(self._stageId)
    if _stageData then
        local _seckillBtn = self:getButtonByName("Button_Seckill")
...
-- 物品掉落
function DungeonEnterGateLayer:_initDrop(_stageData)
    if _stageData.type == 1 then
        ...
        function initGoods(_index,data)
            ...
            elseif not G_Me.activityData.holiday:isActivate() and G_Goods.checkHolidayGood(data) == 1 then
                _bg:setVisible(false)
                borderBg:setVisible(false)
        ...
        G_Me.dungeonData:clearFragmentList()
        for i=1, 5 do
            local nType = _dungeonInfo["item"..i.."_type"]
            if type(nType) == "number" and nType == 6 then
                tAward = {type = _dungeonInfo["item"..i.."_type"], value = _dungeonInfo["item"..i.."_value"], size = _dungeonInfo["item"..i.."_size"]}
                G_Me.dungeonData:addFragment(tAward)
            end
        end
    else
        -- 掉落宝箱
...
function DungeonEnterGateLayer:_showRestDungeonBox(cost,count)
    if count > 0 then
...
function DungeonEnterGateLayer:_onChallenge(widget)
...
--            self:getImageViewByName("ImageView_4774"):showAsGray(true)
                G_Me.dungeonData:setCurrStageId(self._stageId)
                G_Me.dungeonData:setCurrStageLastStar(self._lastStarNum)
...
-- 体力不足
function DungeonEnterGateLayer:_showVit()
    G_GlobalFunc.showPurchasePowerDialog(1)
end


 -- 检查是否有足够体力
function DungeonEnterGateLayer:_isEnoughVit(times)
    if times > 10 then times = 10 end
    local  _stage_data = dungeon_stage_info.get(self._stageId)
    
    if _stage_data == nil then 
        return false 
    end
    
    local _info = G_GlobalFunc.getDungeonData(_stage_data.value)
    if _info then
...
-- 布阵
function DungeonEnterGateLayer:_onBuZhen(widget)
    G_SoundManager:playSound(require("app.const.SoundConst").GameSound.BUTTON_SHORT)
    require("app.scenes.hero.HerobuZhengLayer").showBuZhengLayer()
end

-- 扫荡
function DungeonEnterGateLayer:_onSeckill(widget)
    if not self._clickValid then 
        return
    end
            ...
            if _stageData._isFinished == true then -- 是否通关
                if _stageData._executeCount >= 1 then -- 是否超过挑战次数
                    if self:_isEnoughVit( self.fastTimes) then
                        --uf_notifyLayer:getModelNode():addChild(require("app.scenes.dungeon.DungeonFastBounsLayer").create())
                            local CheckFunc = require("app.scenes.common.CheckFunc")
                            local nChapterId = G_Me.dungeonData:getCurrChapterId()
                            -- 如果传nil则设备上会有问题
                            local scenePack = G_GlobalFunc.sceneToPack("app.scenes.dungeon.DungeonMainScene", {_, _, self._stageId, nChapterId})
                            if CheckFunc.checkKnightFull(scenePack) == false then
                                G_Me.dungeonData:setCurrStageId(self._stageId)
                                G_Me.dungeonData:setCurrStageLastStar(self._lastStarNum)
                                uf_sceneManager:getCurScene():addChild(require("app.scenes.dungeon.DungeonFastBounsLayer").create())
                                self:_closeWindow()
                            end
                    else
                        self:_showVit()
...
-- @desc 点击物品信息
function DungeonEnterGateLayer:onClickGoods(widget,_type)
    if  _type == TOUCH_EVENT_ENDED then
        ...
        local nChapterId = G_Me.dungeonData:getCurrChapterId()
        local nStageId = self._stageId
        local scenePack = G_GlobalFunc.sceneToPack("app.scenes.dungeon.DungeonMainScene", {0, 0, nStageId, nChapterId, nil})

        require("app.scenes.common.dropinfo.DropInfo"
         ).show(_dungeonInfo["item" .. tostring(widget:getTag()) .. "_type"],_dungeonInfo["item" .. tostring(widget:getTag()) .. "_value"], scenePack)
    end
...
-- 播放NPC声音
function DungeonEnterGateLayer:_playStageSound( ... )
  local tStageTmpl = dungeon_stage_info.get(self._stageId)
  if tStageTmpl then
