local DungeonTopLayer = class("DungeonTopLayer",  UFCCSModelLayer)
...
function DungeonTopLayer.create(...)
    return DungeonTopLayer.new("ui_layout/dungeon_DungeonTopLayer.json",Colors.modelColor, ...)
end

function DungeonTopLayer:ctor(json, color, ...)
...
function DungeonTopLayer:onBackKeyEvent( ... )
    self:_onClick()
    return true
end
...
function DungeonTopLayer:onLayerEnter()
    uf_eventManager:addEventListener(G_EVENTMSGID.EVENT_DUNGEON_GETDUNGEONRANK, self._getStarBous, self)
    uf_eventManager:addEventListener(G_EVENTMSGID.EVENT_DUNGEON_FINISHSTARBOUNS, self._getBounsSucc, self)
    G_HandlersManager.dungeonHandler:sendGetDungeonRank()
...
function DungeonTopLayer:onLayerExit()
    uf_eventManager:removeListenerWithTarget(self)
end


-- @desc领取奖励完成
function DungeonTopLayer:_getBounsSucc(data)
...
-- @desc 得到排名
function DungeonTopLayer:_getStarBous(data)
    local _rankList = G_Me.dungeonData:getDungeonRankList()
    ...
    label_StarNum:setText(tostring(G_Me.dungeonData:getAllStar()))
    label_StarNum:setVisible(true)
    label_StarNum:createStroke(Colors.strokeBrown,1)
    
    if G_Me.dungeonData:getMyRank() >0 then
        self:getLabelBMFontByName("LabelBMFont_WeiShangBang"):setText(tostring(G_Me.dungeonData:getMyRank()))
    end
    
    self:getLabelBMFontByName("LabelBMFont_WeiShangBang"):setVisible(G_Me.dungeonData:getMyRank() >0)
    self:getLabelByName("Label_WeiShangBang"):setVisible(G_Me.dungeonData:getMyRank() <= 0)
...
function DungeonTopLayer:_changeTab(groupId, oldName, newName, widget)
    if groupId == 1 then
 ...
 -- @desc 提示有奖励
function DungeonTopLayer:_showTips()
...
function DungeonTopLayer:_creatListView(panelName,json,updateFunc,listnum)
        ...
        local cell = CCSItemCellBase:create(json)
            cell:regisgerWidgetTouchEvent("ImageView_Bg",function ( widget, _touchtype)
                if _touchtype == TOUCH_EVENT_ENDED then -- 点击事件
                    G_HandlersManager.dungeonHandler:sendFinishChapterAchvRwd(widget:getTag())
...
function DungeonTopLayer._updateTopRank(list, index, cell)
    local _rankList = G_Me.dungeonData:getDungeonRankList()
    if _rankList then
        local num = index + 1
...
function DungeonTopLayer:_onClick(widget)
    uf_eventManager:dispatchEvent(G_EVENTMSGID.EVENT_DUNGEON_UPDATETIPS, nil, false,nil)
    self:animationToClose()
end

return DungeonTopLayer
