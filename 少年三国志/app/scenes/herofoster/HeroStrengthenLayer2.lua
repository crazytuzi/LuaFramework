--HeroStrengthenLayer2.lua


local HeroStrengthenLayer2 = class ("HeroStrengthenLayer2", UFCCSNormalLayer)
...
-- 最大材料数
local MATERIAL_MAX = 10

function HeroStrengthenLayer2:ctor( ... )
...
function HeroStrengthenLayer2:onLayerLoad( jsonFile, knightId, ... )
	...
	for i=1, MATERIAL_MAX do 
		self:registerBtnClickEvent("Button_add_"..i, function ( widget )
			self:_onChangeHero( i, widget, knightId )
		end)
	end
	...
	for i=1, MATERIAL_MAX do
		self:registerBtnClickEvent("Button_remove_"..i, function ( widget )
			self:_onRemoveKnight(i)
		end)
	end
	...
	-- self:enableLabelStroke("Label_jingjie", Colors.strokeBrown, 1 )
	 ...
	 for i=1, MATERIAL_MAX do
		self:enableLabelStroke("Label_name_"..i, Colors.strokeBrown, 1 )
	 end
 ...
 --    createStroke("Label_level_title")
	...
	for i=1, MATERIAL_MAX do
		table.insert(self._knightBtns, #self._knightBtns + 1, self:getWidgetByName("Panel_main_icon_"..i))
	end
...
function HeroStrengthenLayer2:onLayerUnload(  )
	uf_eventManager:removeListenerWithTarget(self)
end

function HeroStrengthenLayer2:onLayerEnter( ... )
	--[[
	...
	for i=1, MATERIAL_MAX do
		self:_blurWidget("Button_add_"..i, true, 2.5)
	end
	...
	GlobalFunc.flyFromWidget({self:getWidgetByName("ImageView_knight_1"), 
		...
		self:getWidgetByName("ImageView_knight_6"),
		self:getWidgetByName("ImageView_knight_7"),
		self:getWidgetByName("ImageView_knight_8"),
		self:getWidgetByName("ImageView_knight_9"),
		self:getWidgetByName("ImageView_knight_10"),
	}, 
...
function HeroStrengthenLayer2:adapterLayer( ... )
	--self:adapterWidgetHeight("Panel_mainBody", "", "Panel_baseInfo", 0, 0)
	
end

function HeroStrengthenLayer2:_initMainHero( knightId )
...
--[[
function HeroStrengthenLayer2:_initKnightLine( ... )
...
-- |升到最大级别所需要经验|，|目前经验进度|
function HeroStrengthenLayer2:_calcUpgradeDestLevel( expOffset, maxLevel )
...
function HeroStrengthenLayer2:_checkKnightLevel(  )
	local mainKnightId = G_Me.formationData:getMainKnightId()
	local knightInfo = G_Me.bagData.knightsData:getKnightByKnightId(self._mainKnightId)		
	local mainKnightInfo = G_Me.bagData.knightsData:getKnightByKnightId(mainKnightId)
...
function HeroStrengthenLayer2:_onChangeHero( index, widget, except )
...
function HeroStrengthenLayer2:_onRemoveKnight( index )
	index = index or 0
...
function HeroStrengthenLayer2:_addKnightToBorder( panel, knightId, index, delay, time, func )
	-- 在底盘上加载武将并转动底盘效果
...
function HeroStrengthenLayer2:_onSelectedHeros( selecteKnights, acquireExp )
...
function HeroStrengthenLayer2:_updateExpChange( ... )
	local label = self:getLabelByName("Label_yinbi_value")
	if label then
...
function HeroStrengthenLayer2:_blurWidget( labelName, blur, offset )
...
--[[
function HeroStrengthenLayer2:_brighterKnightLine( index )
...
function HeroStrengthenLayer2:_generateMaterialKnights( force )
...
function HeroStrengthenLayer2:_onAutoAddHero(  )
    ...
    -- 最多选择5个材料武将
    while #autoSelectKnight > MATERIAL_MAX do
    ...
    if #retKnight < 1 then
    	require("app.scenes.common.acquireInfo.AcquireInfoLayer").show(G_Goods.TYPE_KNIGHT, 2003,
                                GlobalFunc.sceneToPack("app.scenes.herofoster.HeroDevelopScene", 
...
function HeroStrengthenLayer2:_onStrengthHero(  )
...
function HeroStrengthenLayer2:_hideAllAddBtns( ... )
	--[[
	...
	for i=1, MATERIAL_MAX do
		self:showWidgetByName("Button_add_"..i, false)
	end
end

function HeroStrengthenLayer2:_onReceiveStrengthRet( ret )
...
function HeroStrengthenLayer2:_playStrengthenAnimation( hasLevelup, func )
...
function HeroStrengthenLayer2:_flyAttributeChange( ... )
	local knightInfo = G_Me.bagData.knightsData:getKnightByKnightId(self._mainKnightId)
...
-- 这是旧的属性变化提示，现在不用了
function HeroStrengthenLayer2:_playAttributeChange( ... )
	local knightInfo = G_Me.bagData.knightsData:getKnightByKnightId(self._mainKnightId)
	if not knightInfo then
		return 
	end
