...
local TreasureFragementPage = class("TreasureFragementPage", KnightPageBase)

function TreasureFragementPage.create(...)
	return KnightPageBase._create_(TreasureFragementPage.new(...), "ui_layout/BaseInfo_EquipFragement.json", ...)
end

function TreasureFragementPage.delayCreate(...)
	local page = KnightPageBase._create_(TreasureFragementPage.new(...), nil, ...)
	page:delayLoad("ui_layout/BaseInfo_EquipFragement.json")

	return page
end

function TreasureFragementPage:ctor( baseId, fragmentId, scenePack, ... )
...
function TreasureFragementPage:afterLayerLoad( ... )
	...
	if icon ~= nil then
		local heroPath = G_Path.getTreasureFragmentIcon(fragmentInfo.res_id)
	...
	if label then 
		if equipInfo then 
			label:setText(G_lang.getEquipNameByType(equipInfo.type+4)..G_lang:get("LANG_TREASURE"))
		end
	end

	local fragment = G_Me.bagData.treasureFragmentList:getItemByKey(fragmentInfo.id)
	if fragment then 
		self:showTextWithLabel("Label_count_title_value", fragment.num)
	else
		self:showTextWithLabel("Label_count_title_value", 0)
	end

	local label = self:getLabelByName("Label_desc")
	if label and equipInfo then 
...
function TreasureFragementPage:_initAcquireWays( fragmentInfo )
    ...
    local wayTypeInfo = way_type_info.get(G_Goods.TYPE_TREASURE_FRAGMENT, fragmentInfo.id)
    if wayTypeInfo then
