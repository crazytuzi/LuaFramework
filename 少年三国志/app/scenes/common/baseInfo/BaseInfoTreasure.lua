--BaseInfoTreasure.lua

local KnightInfoPage = require("app.scenes.common.baseInfo.treasure.TreasureInfoPage")
local KnightFragementPage = require("app.scenes.common.baseInfo.treasure.TreasureFragementPage")

local EffectSingleMoving = require "app.common.effects.EffectSingleMoving"
local BaseInfoTreasure = class("BaseInfoTreasure", UFCCSModelLayer)

BaseInfoTreasure.CONST_COLOR = {
	HIGHLIGHT_COLOR = ccc3(0xf2, 0x79, 0x0d),
	NORMAL_COLOR = ccc3(0xe1, 0xb2, 0x7c),
	GRAY_COLOR = ccc3(0xae, 0xae, 0xae),
}

BaseInfoTreasure.PAGE_TYPE = {
	PAGE_FRAGMENT = 1,
	PAGE_BASEINFO = 2,
}

function BaseInfoTreasure.showWidthBaseId( baseId, scenePack )
	local baseInfo = BaseInfoTreasure.new("ui_layout/BaseInfo_EqupiMain.json",
	 Colors.modelColor, baseId, 0, BaseInfoTreasure.PAGE_TYPE.PAGE_BASEINFO, scenePack)	
	uf_sceneManager:getCurScene():addChild(baseInfo)
end

function BaseInfoTreasure.showWidthFragmentId( fragmentId, scenePack )
	local baseInfo = BaseInfoTreasure.new("ui_layout/BaseInfo_EqupiMain.json", 
		Colors.modelColor, 0, fragmentId, BaseInfoTreasure.PAGE_TYPE.PAGE_FRAGMENT, scenePack)		
	uf_sceneManager:getCurScene():addChild(baseInfo)
end

function BaseInfoTreasure:ctor( ... )
	self._mainBaseId = 0
	self._fragmentId = 0
...
function BaseInfoTreasure:onLayerLoad( _, _, baseId, fragmentId, pageId, scenePack )
	self._mainBaseId = baseId
	self._fragmentId = fragmentId
	self._defaultPageId = pageId or BaseInfoTreasure.PAGE_TYPE
	...
	self:showTextWithLabel("Label_page_2", G_lang:get("LANG_TREASURE"))
	...
	self:registerWidgetClickEvent("Label_page_2", function ( ... )
		self:_onSwitchViewPage(BaseInfoTreasure.PAGE_TYPE.PAGE_BASEINFO)
	end)
	...
	self:registerWidgetClickEvent("Label_page_1", function ( ... )
		self:_onSwitchViewPage(BaseInfoTreasure.PAGE_TYPE.PAGE_FRAGMENT)
	end)
...
function BaseInfoTreasure:_initEquipAndFragment( ... )
	if self._mainBaseId <= 0 and self._fragmentId <= 0 then 
		return 
	end
	if self._mainBaseId <= 0 then 
		
		local fragmentInfo = treasure_fragment_info.get(self._fragmentId)
		if not fragmentInfo then 
			assert("wrong fragment id:"..self._fragmentId)
			return 
		end

		self._mainBaseId = fragmentInfo.treasure_id
	else
		
		local knightInfo = treasure_info.get(self._mainBaseId)
...
function BaseInfoTreasure:_calcPageCount( ... )
	...
	if type(self._fragmentId) == "number" and self._fragmentId > 0 then 
		...
		self._typeToPagePair[BaseInfoTreasure.PAGE_TYPE.PAGE_FRAGMENT] = typeToPageIndex
		self._pageToTypePair[pageToTypeIndex] = BaseInfoTreasure.PAGE_TYPE.PAGE_FRAGMENT
		typeToPageIndex = typeToPageIndex + 1
	end

	if type(self._mainBaseId) == "number" and self._mainBaseId > 0 then 
		pageToTypeIndex = pageToTypeIndex + 1
		self._typeToPagePair[BaseInfoTreasure.PAGE_TYPE.PAGE_BASEINFO] = typeToPageIndex
		self._pageToTypePair[pageToTypeIndex] = BaseInfoTreasure.PAGE_TYPE.PAGE_BASEINFO
		typeToPageIndex = typeToPageIndex + 1
	end
...
function BaseInfoTreasure:_initInvalidPageFlag( ... )
	for loopi = 1, 2, 1 do 
		if not self._typeToPagePair[loopi] then 
			...
			if label then 
				label:setColor(BaseInfoTreasure.CONST_COLOR.GRAY_COLOR)
			end
		end
	end
end

function BaseInfoTreasure:onLayerEnter( ... )
	self:_loadPage()
...
function BaseInfoTreasure:_loadPage( ... )
...
function BaseInfoTreasure:_doCreatePageItem( index )
	index = index or 0
	local pageItem = nil
	local pageModel = nil
	...
	if pageTypeIndex == BaseInfoTreasure.PAGE_TYPE.PAGE_FRAGMENT then 
...
function BaseInfoTreasure:_onPageChanged( index )
	...
	if label then 
		label:setColor(BaseInfoTreasure.CONST_COLOR.NORMAL_COLOR)
	end

	label = self:getLabelByName("Label_page_"..curCtrlIndex)
	if label then 
		label:setColor(BaseInfoTreasure.CONST_COLOR.HIGHLIGHT_COLOR)
	end
end

return BaseInfoTreasure
