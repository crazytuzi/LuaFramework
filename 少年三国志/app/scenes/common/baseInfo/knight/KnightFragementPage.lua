...
local KnightFragementPage = class("KnightInfoPage", KnightPageBase)

function KnightFragementPage.create(...)
	return KnightPageBase._create_(KnightFragementPage.new(...), "ui_layout/BaseInfo_KnightFragement.json", ...)
end

function KnightFragementPage.delayCreate(...)
	local page = KnightPageBase._create_(KnightFragementPage.new(...), nil, ...)
	page:delayLoad("ui_layout/BaseInfo_KnightFragement.json")

	return page
end

function KnightFragementPage:ctor( baseId, fragmentId, scenePack, ... )
...
function KnightFragementPage:afterLayerLoad( ... )
	...
	if icon ~= nil then
		local heroPath = G_Path.getKnightIcon(fragmentInfo.res_id)
...
function KnightFragementPage:_initAcquireWays( fragmentInfo )
    ...
    self._listview:setCreateCellHandler(function ( list, index)
        return require("app.scenes.common.baseInfo.knight.KnightFragmentItem").new(list, index, "ui_layout/BaseInfo_KnightFragementItem.json")
    end)
