local DressBookCellThree = class("DressBookCellThree",function()
    return CCSItemCellBase:create("ui_layout/dress_BookCell3.json")
end)
...
function DressBookCellThree:ctor()
        ...
        self:registerWidgetClickEvent("Image_heroClick3", function()
                self:click(3)
        end) 
        ...
        if require("app.scenes.mainscene.SettingLayer").showEffectEnable() then
            ...
            self._effect3 = EffectNode.new("effect_szkpgy", 
                function(event, frameIndex)
                    if event == "finish" then
            ...
            self._effect3:setPosition(ccp(0,0))
            self._effect3:play()
            self:getImageViewByName("Image_heroClick3"):addNode(self._effect3) 
        end
end

function DressBookCellThree:click(index)
...
function DressBookCellThree:updateData(data)
        self._data = data
        ...
        if data then
            ...
            self:initKnight(3,data.dress_3)
...
function DressBookCellThree:initKnight(index,baseid)
      if baseid ~= 0 then
...
function DressBookCellThree:getWidth()
      local width = self:getContentSize().width
      return width
end

function DressBookCellThree:updateAttrs(data )
...
function DressBookCellThree:onLayerUnload()
    
    uf_eventManager:removeListenerWithTarget(self)

end

return DressBookCellThree
