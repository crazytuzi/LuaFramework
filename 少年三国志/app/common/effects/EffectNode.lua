...
local EffectNode = class("EffectNode", function() return display.newNode() end)
...
function EffectNode:ctor(effectJsonName, eventHandler, colorOffset, resourceIniter, effectNodeCreator, isGray)
        ...
        _defaultResourceIniter = {
            jsonGetter = function(effectJsonName) 
                if  self._jsonList[effectJsonName] == nil then
                    ...
                    self._jsonList[effectJsonName] = decodeJsonFile("effect/"..effectJsonName.."/"..effectJsonName..".json")
                else
                    -- print("cache json")
                end
            ...
            framesGetter = function(png) 
                ...
                if self._frames[png] == nil then
                    self._frames[png] = display.newSpriteFrame(png)
...
function EffectNode:gotoAndStop(n)
    self:_reset()
    for n=1, n do 
        self:_step()
    end
    ...
    if lastFrameStart or subInfo[fx] then
        ...
        if lastFrameStart == frameIndex then
            ...
            if not playingSub then
                ...
                if isTypeOfEffectNode(playingSub) and subInfo['first_frame'] then
...
function EffectNode:_createDrawNode(mask_info)
    ...
    if mask_info.mask_type == "circle" then
        ...
        if device.platform == "wp8" or device.platform == "winrt" then
            G_WP8.drawPolygon(effectNode, pointarr1, pointsCount, ccc4f(1.0, 1.0, 1, 1), 1, ccc4f(0.1, 1, 0.1, 1))
        else
...
function EffectNode:_createSprite(png, sprite, key, colorOffset)   
    ...
    if sprite == nil then
        ...
        if type(self._isGray) == "boolean" then
            sprite:showAsGray(self._isGray)
        end
