-- Filename: AlertGetLayer.lua
-- Author: DJN
-- Date: 2014-08-22
-- Purpose: 主角学习了某种技能后的弹窗

module("AlertGetLayer", package.seeall)
...
function createBgUI()
    ...
    --恭喜主公获得*******那句话
    local strA = CCLabelTTF:create(GetLocalizeStringBy("key_1682"),g_sFontPangWa,21)
    ...
    --快去装备上试试威力 那句话
    local strE = CCLabelTTF:create(GetLocalizeStringBy("djn_34") ,g_sFontPangWa,21)
    strE:setColor(ccc3(0xff,0xff,0xff))
    strE:setAnchorPoint(ccp(0.5,1))
    strE:setPosition(ccp(brownSprite:getContentSize().width*0.5,187))
    brownSprite:addChild(strE)
