-- FileName: SevenLotteryShopCell.lua 
-- Author: fuqiongqiong
-- Date: 2016-8-4
-- Purpose: 七星台商店cell

module("SevenLotteryShopCell",package.seeall)
...
local function buyAction( tag)
	...
	-- end
	-- 弹出选择数量界面
	require "script/ui/sevenlottery/shop/SevenLotteryShopBuyLayer"
	SevenLotteryShopBuyLayer.showPurchaseLayer(tag)
end

function createCell( goods_data,p_index ,p_touchProp)
	local tCell = CCTableViewCell:create()
	--背景
	local cellBackground = CCScale9Sprite:create("images/reward/cell_back.png")
	cellBackground:setContentSize(CCSizeMake(454, 180))
	...
	buyMenuBar:setTouchPriority(p_touchProp)
	...
	if(tonumber(itemType) == 1)then
		...
		iconSprite = ItemSprite.getItemSpriteById(item_id,nil, showDownMenu,nil,-730)
	...
	-- local rewardInDb = ItemUtil.getItemsDataByStr(goods_data.items)
 ...
 --    nameLabel:setPosition(ccp(textBg:getContentSize().width*0.4,textBg:getContentSize().height*0.8))
 ...
 --    textBg:addChild(nameLabel)
	...
	buyMenuBar:addChild(buyBtn, 1, tonumber(p_index))
	-- 物品名称
	...
	if(goods_data.level_num == nil)then --如果没有level_num这个字段
		maxLimitNum =  tonumber(goods_data.baseNum) - SevenLotteryShopData.getBuyNumBy(goods_data.id)
		
	else
        local _number,_level = SevenLotteryShopData.getLevelnumber(goods_data)
  		maxLimitNum = _number - SevenLotteryShopData.getBuyNumBy(goods_data.id)
	end
	if(maxLimitNum <= 0)then
	...
	-- 价格
	local priceLabel = CCRenderLabel:create(GetLocalizeStringBy("fqq_130"), g_sFontName, 18, 1, ccc3(0x00, 0x00, 0x00), type_shadow)
    priceLabel:setColor(ccc3(0xff, 0xf6, 0x00))
    priceLabel:setPosition(110,55)
    textBg:addChild(priceLabel)
    local goldSp_1 = CCSprite:create("images/common/star_point_small.png")
	...
	local curPrice = tonumber(goods_data.costPoint)
