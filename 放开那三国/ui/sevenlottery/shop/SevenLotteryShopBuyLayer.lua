-- FileName: SevenLotteryShopBuyLayer.lua 
-- Author: fuqiongqiong
-- Date: 2016-8-4
-- Purpose: 七星台商店批量购买

module("SevenLotteryShopBuyLayer",package.seeall)
...
function buyAction( tag, itemBtn )
   ...
   --      else
        	...
        	-- -- 判断积分是否够
			if(_totalPrice <= SevenLotteryShopData.getScoreOfSevenLottery())then  --当表中的价格小于等于玩家的钱
				closeAction()
				local callBack = function ( ... )
				    	 local callbackFunc = function ( ... )
				            --扣积分
				            SevenLotteryShopData.setScoreOfSevenLottery(_totalPrice)
				            --修改缓存
				            SevenLotteryShopData.addBuyNumberBy(_index,tonumber(_curNumber))
				            -- 弹提示
				        local item_type, item_id, item_num = SevenLotteryShopData.getItemData(_goodsData.items)
			        ...
			        SevenLotteryShopController.buy(_goodsData.id,tonumber(_curNumber),callbackFunc)
		        	end
			        ...
			        local richInfo = {
			            elements = {
			                ...
			                {
			                    ["type"] = "CCSprite",
			                    image = "images/common/star_point_small.png"
			                 }
			            }
			        }
			        local newRichInfo = GetNewRichInfo(GetLocalizeStringBy("fqq_132"), richInfo)  
				...
				AnimationTip.showTip(GetLocalizeStringBy("fqq_133"))  --积分不足
			end
        -- end
...
function changeNumberAction( tag, itemBtn )
	...
	if(_goodsData.level_num == nil)then
		_maxLimitNum = tonumber(_goodsData.baseNum) - SevenLotteryShopData.getBuyNumBy(_goodsData.id)
	else 
	   	require "script/ui/arena/ArenaData"
		local number,level = SevenLotteryShopData.getLevelnumber(_goodsData)
		_maxLimitNum = tonumber(number - SevenLotteryShopData.getBuyNumBy(_goodsData.id))
	...
	local jifen = SevenLotteryShopData.getScoreOfSevenLottery()
	_totalPrice = tonumber(_goodsData.costPoint) * tonumber(_curNumber)
	if(_totalPrice > jifen)then
		local num = jifen%tonumber(_goodsData.costPoint)
		local jifenNum = jifen - num
		_curNumber = tonumber(jifenNum/tonumber(_goodsData.costPoint))
		if(_curNumber == 0)then
			_curNumber = 1
		end
	end
...
---- 加减道具的按钮
	...
	-- 总价
	local totalTipLabel = CCRenderLabel:create(GetLocalizeStringBy("fqq_131"), g_sFontName, 36, 1, ccc3(0x49, 0x00, 0x00), type_stroke)
    totalTipLabel:setColor(ccc3(0xfe, 0xdb, 0x1c))
    totalTipLabel:setPosition(ccp(110, 72) )
    _innerBgSp:addChild(totalTipLabel)
    local goldSp_2 = CCSprite:create("images/common/star_point_small.png")
	goldSp_2:setAnchorPoint(ccp(0,0))
	goldSp_2:setPosition(ccp(265, 38))
	_innerBgSp:addChild(goldSp_2)
	_totalPrice = tonumber(_goodsData.costPoint)
...
function showPurchaseLayer( tag)
	...
	-- 商品数据
	_goodsData = SevenLotteryShopData.getItemById(tag)
	_index = tag
	print("------_index------",_index)
	print_t(_goodsData)
	-- 兑换商品中物品的数据
