-- Filename: GuildSetIconLayer.lua
-- Author: bzx
-- Date: 2015-1-14
-- Purpose: 设置军团军旗

module("GuildSetIconLayer", package.seeall)
...
function create( p_touchPriority, p_zOrder )
	...
	local dialogInfo = {
		title = GetLocalizeStringBy("key_10176"),
		callbackClose = nil,
	...
	local tip = CCLabelTTF:create(GetLocalizeStringBy("key_10177"), g_sFontPangWa, 25)
...
function callbackSelected(p_tag, p_guidIconItem)
	...
	if guildLevel < legionIconDb.needlegionlv then
		SingleTip.showTip(string.format(GetLocalizeStringBy("key_10178"), legionIconDb.needlegionlv))
		return
	end
...
function handleModifyIcon( ... )
	...
	GuildMainLayer.refreshGuildIcon()
end
