-- Filename:RobBattleResultView.lua
-- Author: 	lichenyang
-- Date: 	2013-11-7
-- Purpose: 抢夺结算面包 夺宝


module("RobBattleResultView", package.seeall)
...
local itemName               = nil   --抢夺得到的物品名字
local _robTable              = nil   --
local trayButton             = nil

local m_appraisal            = nil
local m_enemyUid             = nil

local cardGuideAreaNode      = nil   -- 新手引导区域
...
-- flopData:掉落翻牌数据
-- CallFun: 自定义确定按钮后回调
function createBattleResultLayer( rob_table,item_name,appraisal, enemyUid, enemyName, enemyUtid, enemyFightData, silverData, expData, flopData, CallFun, fightStr )
    -- 初始化
    init()
    _robTable      = rob_table
    itemName       = item_name
    ...
    local myUtid = UserModel.getUserUtid()
    print("myUtid",myUtid)
    local myName = UserModel.getUserName()
    --  -- 我方姓名的颜色
    local name_color,stroke_color = getHeroNameColor( myUtid )
    local myName_font = CCRenderLabel:create( myName, g_sFontPangWa, 25, 1, stroke_color, type_stroke)
    ...
    -- 敌方名字 战斗力
    --  -- 敌方姓名的颜色
    local enemyUtid = enemyUtid or 2
    local name_color,stroke_color = getHeroNameColor( enemyUtid )
    local enemyName_font = CCRenderLabel:create( enemyName, g_sFontPangWa, 25, 1, stroke_color, type_stroke)
    ...
    if(isWin) then
        ...
        if(itemName ~= nil) then
            local huode     = CCSprite:create("images/treasure/robed.png")
            local huodeItem = CCRenderLabel:create( itemName , g_sFontPangWa, 30, 2, ccc3( 0x00, 0x00, 0x00), type_stroke)
            huodeItem:setColor(ccc3(0xff, 0xe4, 0x00))
            local huodeNode = BaseUI.createHorizontalNode({huode, huodeItem})
            huodeNode:setAnchorPoint(ccp(0.5,0.5))
            huodeNode:setPosition(ccp(middle_bg:getContentSize().width*0.5,middle_bg:getContentSize().height-35))
            middle_bg:addChild(huodeNode)
        else
            local failed = CCSprite:create("images/treasure/failed.png")
            failed:setAnchorPoint(ccp(0.5, 0.5))
            failed:setPosition(ccp(middle_bg:getContentSize().width*0.25, middle_bg:getContentSize().height-35))
            middle_bg:addChild(failed)

            --再强一次会导致新手引导逻辑错误，暂时去掉
            local trayAgainMenu = CCMenu:create()
            trayAgainMenu:setPosition(ccp(0, 0))
            trayAgainMenu:setAnchorPoint(ccp(0, 0))
            middle_bg:addChild(trayAgainMenu)

            trayButton =  LuaCC.create9ScaleMenuItem("images/common/btn/btn_red_n.png","images/common/btn/btn_red_h.png",CCSizeMake(203, 73),GetLocalizeStringBy("key_2006"),ccc3(0xff, 0xcc, 0x00),35,g_sFontPangWa,1, ccc3(0x00, 0x00, 0x00))
            trayButton:setAnchorPoint(ccp(0, 0.5))
            trayButton:setPosition(ccp(failed:getPositionX() + failed:getContentSize().width/2 + 4, middle_bg:getContentSize().height-35))
            trayButton:registerScriptTapHandler(function ( ... )
                --再次挑战
                okItemFun()

                --判断是否有功能节点开启
                require "script/ui/switch/SwitchOpen"
                if(SwitchOpen.isSwithPanelShow == true) then
                    return
                end

                require "script/ui/treasure/TreasureService"
                TreasureService.seizeRicher(_robTable.uid, _robTable.fragmentId, function ( isSuccess )
                    if(isSuccess) then
            ...
            trayAgainMenu:addChild(trayButton,15)
            trayButton:setEnabled(false)
            trayAgainMenu:setTouchPriority(-600)
        end
       ...
       -- 左中右真是掉落实物品
        ...
        cardGuideAreaNode = CCNode:create()
        cardGuideAreaNode:setContentSize(CCSizeMake(right_item_bg:getContentSize().width * 3, right_item_bg:getContentSize().height))
        cardGuideAreaNode:setPosition(ccp(midItem:getPositionX() - right_item_bg:getContentSize().width * 1.5,midItem:getPositionY() - right_item_bg:getContentSize().height * 0.5))
        middle_bg:addChild(cardGuideAreaNode)

        -- 对方阵容位置
        if(userFormationItem) then
            userFormationItem:setPosition(ccp(bg_sprite:getContentSize().width*0.3,164))
        end
        
        -- _sendMsgItem:setPosition(ccp( bg_sprite:getContentSize().width*0.7,164))
        print("m_enemyUid is  ",m_enemyUid)
        -- added by zhz
        _sendMsgItem:setPosition(ccp( bg_sprite:getContentSize().width*0.5,164))
        if(enemyUid ~= nil) then
           _sendMsgItem:setPosition(ccp( bg_sprite:getContentSize().width*0.7,164))
...
function cardMenuActionFun( tag, item_obj )
    ...
    -- 启用按钮
    print("启用查看阵容按钮 userFormationItem =",userFormationItem, "enemyUid=", m_enemyUid)
    if(userFormationItem ~= nil) then
        userFormationItem:setEnabled(true)
        userFormationItem_font:setColor(ccc3(0xfe, 0xdb, 0x1c))
        print(GetLocalizeStringBy("key_2014"))
    end
    if(trayButton ~= nil) then
...
function addNewGuideStepFive( ... )
    local guideFunc = function ( ... )
        require "script/guide/RobTreasureGuide"
        if(NewGuide.guideClass ==  ksGuideRobTreasure and RobTreasureGuide.stepNum == 4) then
            RobTreasureGuide.changLayer()
            local robTreasure = getCardArea()
            local touchRect   = getSpriteScreenRect(robTreasure)
            RobTreasureGuide.show(5, touchRect)
        end
    end
...
function addNewGuideStepSix( ... )
    local guideFunc = function ( ... )
        require "script/guide/RobTreasureGuide"
        if(NewGuide.guideClass ==  ksGuideRobTreasure and RobTreasureGuide.stepNum == 5) then
            RobTreasureGuide.changLayer()
            local robTreasure = getOkButton()
            local touchRect   = getSpriteScreenRect(robTreasure)
            RobTreasureGuide.show(6, touchRect)
        end
    end
