...
RewardRecordCell = class("RewardRecordCell", function ( ... )
	return CCTableViewCell:create()
end)

RewardRecordCell.__index = RewardRecordCell

RewardRecordCell.time 						= nil			--发奖时间
RewardRecordCell.title 						= nil			--奖励类型标题
RewardRecordCell.content 					= nil			--奖励内容描述
RewardRecordCell.rid							= nil			--物品id
RewardRecordCell.receiveRewardCallback		= nil			--领取物品回调
RewardRecordCell.haveTime					= nil			--剩余过期时间
--创建tableCell
function RewardRecordCell:create( rewardInfo, cellIndex,t_reviceCallback )
	local tableCell = RewardRecordCell.new()


	-- print("rewardInfo",rewardInfo)
	print_table("RewardRecordCell:create", rewardInfo)
	...
	cellBackground:setContentSize(CCSizeMake(568, 237))
	...
	--发奖时间
	local rewardTime = GetLocalizeStringBy("llp_224") .. tostring(rewardInfo.time)
	-- print(rewardTime)
	...
	--内容描述
	tableCell.content = CCLabelTTF:create(rewardTime, g_sFontName, 20)
	tableCell.content:setAnchorPoint(ccp(0, 1))
	tableCell.content:setPosition(ccp(26, 175))
	...
	itemback:setContentSize(CCSizeMake(520, 135))
	...
	local function rewardItemTableCallback( fn, table, a1, a2 )
		...
		elseif fn == "cellAtIndex" then
			-- if not a2 then
				...
				local numLabel = CCRenderLabel:create(tostring(rewardInfo.items[a1+1].num), g_sFontName, 23, 1, ccc3(0,0,0))
	...
	local tableViewSize = CCSizeMake(508,128)
...
--更新tableCell
function RewardRecordCell:updateCell( rewardInfo )
