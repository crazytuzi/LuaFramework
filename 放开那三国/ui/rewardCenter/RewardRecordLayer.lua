-- Filename: RewardRecordLayer.lua
-- Author: llp
-- Date: 2015-06-30
-- Purpose: 领奖记录
...
module("RewardRecordLayer", package.seeall)
...
function create( )
	...
	local titleLabel = CCRenderLabel:create(GetLocalizeStringBy("llp_222"), g_sFontPangWa, 35, 1, ccc3(0,0,0))
	...
	-- --奖励一周内不领取会消失
	-- local rewardAlert = CCRenderLabel:create(GetLocalizeStringBy("llp_223"), g_sFontName, 21, 1, ccc3(0,0,0))
	-- rewardAlert:setPosition(ccp(background:getContentSize().width*0.5, background:getContentSize().height - 81))
	-- rewardAlert:setColor(ccc3(0xFF, 0xEF, 0x08))
	...
	--全部领取按钮
	local allReceive = CCRenderLabel:create(GetLocalizeStringBy("llp_225"), g_sFontPangWa, 25, 1, ccc3(0,0,0))--LuaCC.create9ScaleMenuItem("images/common/btn/btn1_d.png","images/common/btn/btn1_n.png",CCSizeMake(210,73),GetLocalizeStringBy("key_1635"),ccc3(255,222,0))
	...
	background:addChild(allReceive)
	allReceive:setColor(ccc3(0x26,0xFF,0x0F))
	-- allReceive:registerScriptTapHandler(allReceiveCallback)

	--拉数据
	RewardCenterService.getRewardRecordList(0, 0, function ( ... )
...
function createTableView( layer )
	...
	rewardList = {}
	rewardTable= {}
	local tempList = RewardCenterData.getRewardList(true)
print("--------")
print_t(tempList)
	local rowNum = 20
	for i=1,#tempList do
	...
	for i=1,#rewardList do
		...
		local  function rewardTableCallback(fn, t_table, a1, a2)
			require "script/ui/rewardCenter/RewardRecordCell"
			local r
			if fn == "cellSize" then
				r = CCSizeMake(568, 237)
			elseif fn == "cellAtIndex" then
				a2 = RewardRecordCell:create(rewardList[i][a1 + 1], a1, receiveRewardCallback)
				r = a2
		...
		rewardTable[i]:setTouchPriority(-6660)
		rewardTable[i]:setVerticalFillOrder(kCCTableViewFillTopDown)

		print("crate reward table ", i)
	end
	--当前奖励数
	-- rewardCountNum = CCLabelTTF:create(GetLocalizeStringBy("key_2672") .. RewardCenterData.getRewardRecordCount(), g_sFontName, 21)
...
function receiveRewardCallback( rid, t_cellIndex )
	...
	local function requestCallback( ... )
		for i,v in ipairs(rewardList[pageIndex]) do
			if(tonumber(rid) == tonumber(v.rid)) then
				table.remove(rewardList[pageIndex], i)
			end
		end
		if(#rewardList[pageIndex] == 0) then
		...
		rewardTable[pageIndex]:removeCellAtIndex(t_cellIndex)
		rewardTable[pageIndex]:reloadData()
		-- rewardCountNum:setString(GetLocalizeStringBy("key_2672") .. RewardCenterData.getRewardRecordCount())
	end
