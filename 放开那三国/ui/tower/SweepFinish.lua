...
module("SweepFinish", package.seeall)
...
function getTowerInfoCallback( cbFlag, dictData, bRet )
    ...
    if(TowerCache.getTowerInfo().va_tower_info.sweep_info.end_level~=nil)then
        UserModel.addGoldNumber(-(tonumber(TowerCache.getTowerInfo().va_tower_info.sweep_info.end_level)-tonumber(TowerCache.getTowerInfo().cur_level)+1)*cp.wipeGold)
...
function okCb()
    ...
    if((tonumber(TowerCache.getTowerInfo().va_tower_info.sweep_info.end_level)-tonumber(TowerCache.getTowerInfo().cur_level)+1)*cp.wipeGold>UserModel.getGoldNumber())then
        -- 金币不足
    ...
    local inNum = TowerCache.getTowerInfo().va_tower_info.sweep_info.end_level
    inputNum = tonumber(inNum)
    -- 发送请求
    local args = Network.argsHandler(inputNum)
...
function showLayer()
    ...
    local costNum = CCLabelTTF:create(TowerCache.getTowerInfo().va_tower_info.sweep_info.end_level,g_sFontName,24)
    costNum:setColor(ccc3(0x78,0x25,0x00))
    local goldShowSprite = CCSprite:create("images/common/gold.png")

    cp = DB_Tower.getDataById(1)
    goldCostLabel = CCLabelTTF:create((tonumber(TowerCache.getTowerInfo().va_tower_info.sweep_info.end_level)-tonumber(TowerCache.getTowerInfo().cur_level)+1)*cp.wipeGold,g_sFontName,24)
    goldCostLabel:setColor(ccc3(0x78,0x25,0x00))

    require "script/utils/BaseUI"
    local aleteNode = BaseUI.createHorizontalNode({content,goldShowSprite,goldCostLabel,content2,costNum,content3})
