-- FileName: TowerRankLayer.lua 
-- Author: Li Cong 
-- Date: 14-1-7 
-- Purpose: function description of module 


module("TowerRankLayer", package.seeall)
...
function initRankingsLayer( ... )
    ...
    if(Platform.getLayout ~= nil and Platform.getLayout() == "enLayout" ) then
        myScore_font:setPosition(ccp(350,_layerBg:getContentSize().height-78))
    else
        myScore_font:setPosition(ccp(425,_layerBg:getContentSize().height-78))
    end
    _layerBg:addChild(myScore_font)
    local myScoreData = _mySelfData.max_level or " " 
...
function createRankingList( ... )
    ...
    -- cellBg的size
    require "script/ui/tower/TowerRankCell"
    ...
    local handler = LuaEventHandler:create(function(fn, table, a1, a2)
        ...
        elseif (fn == "cellAtIndex") then
            r = TowerRankCell.createCell( _rankingListData[a1+1] )
...
-- 获取排行榜信息回调
function getTowerRankCallFun( cbFlag, dictData, bRet )
...
function createRankingsLayer()
	...
	RequestCenter.tower_getTowerRank(getTowerRankCallFun,args)
	return _bgLayer
end
