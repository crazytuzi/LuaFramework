...
require "script/ui/rechargeActive/MysteryMerchantCell"
...
function loadBG()
    ...
    _layer:addChild(_border_bottom)
    ...
    _border_bottom:setPosition(0, border_bottom_y)
    
    -- 神秘商人图像
    local mystery_merchant_role = CCSprite:create("images/recharge/mystery_merchant/ren.png")
    _layer:addChild(mystery_merchant_role)
    mystery_merchant_role:setAnchorPoint(ccp(0, 1))
    mystery_merchant_role:setPosition(0, border_top_y + 20 * MainScene.elementScale)
...
function loadJadeCount()
	...
	_layer:addChild(_spirit_jade_count_tip)
end
...
function loadHeroList()
    _hero_node = CCScale9Sprite:create("images/star/intimate/bottom9s.png",
                                       ...
                                       CCRectMake(0, 0, 0, 0))
	_layer:addChild(_hero_node, 100)    
...
function loadItemList()
    _item_node = CCScale9Sprite:create("images/star/intimate/bottom9s.png",
                                       CCRectMake(0, 0, 75, 75),
                                       CCRectMake(0, 0, 0, 0))
	_layer:addChild(_item_node, 100)
    ...
    updateRefreshTime()
    _update_timer = CCDirector:sharedDirector():getScheduler():scheduleScriptFunc(updateRefreshTime, 1, false)
end
...
function loadRefresh()
    ...
    _layer:addChild(_next_refresh_time_tip)
    if ActiveCache.MysteryMerchant:isPerpetual() then
        _next_refresh_time_tip:setPosition(g_winSize.width - 10 * MainScene.elementScale, 200 * g_fScaleX)
    else
        _next_refresh_time_tip:setPosition(g_winSize.width - 10 * MainScene.elementScale, 245 * g_fScaleX)
    end
    ...
    if not ActiveCache.MysteryMerchant:isPerpetual() then
        ...
        _layer:addChild(_disappear_time_tip)
        _disappear_time_tip:setScale(MainScene.elementScale)
        _disappear_time_tip:setAnchorPoint(ccp(1, 0))
        _disappear_time_tip:setPosition(g_winSize.width - 50 * MainScene.elementScale, g_winSize.height - 250 * g_fScaleX)
   end
   refreshRefreshBtn()
end
...
function loadSummoned()
    ...
    if ActiveCache.MysteryMerchant:isExist() == true then
        _summoned_btn:setPosition(215 * MainScene.elementScale, 210 * g_fScaleX)
    else
        _summoned_btn:setPosition(g_winSize.width - 215 * MainScene.elementScale, 210 * g_fScaleX)
    end
    local tip_bg = CCScale9Sprite:create("images/common/bgng_lefttimes.png")
    _layer:addChild(tip_bg)
    tip_bg:setAnchorPoint(ccp(0.5, 1))
    tip_bg:setPosition(ccp(g_winSize.width * 0.5, 175 * g_fScaleX))
...
function handleSummoned(cbFlag, dictData, bRet)
    ...
    ActiveCache.MysteryMerchant:requestData(refreshLayer)
end

function refreshLayer()
    _layer:unregisterScriptHandler()
    ...
    _layer = createLayer()
    require "script/ui/rechargeActive/RechargeActiveMain"
    RechargeActiveMain.changeButtomLayer(_layer)
    _layer:registerScriptHandler(onNodeEvent)
end
...
function refreshRefreshBtn()
    ...
    if ActiveCache.MysteryMerchant:isPerpetual() then
        _refresh_btn:setPosition(g_winSize.width - 111 * MainScene.elementScale, 165 * g_fScaleX)
    else
        _refresh_btn:setPosition(g_winSize.width - 111 * MainScene.elementScale, 210 * g_fScaleX)
    end
