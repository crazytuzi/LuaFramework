-- FileName: ChooseSoulLayer.lua 
-- Author: licong 
-- Date: 15/9/24 
-- Purpose: 选择战魂


module("ChooseSoulLayer", package.seeall)
...
function initSelectData( )
	-- 得到材料列表
	local materialListData = SoulRebornData.getCanRebornSoul()
	-- 得到已选择的列表
	_selectList = SoulRebornData.getSelectList()
...
function closeButtonCallback( tag, sender )
	...
	-- 返回还原原来的
	SoulRebornData.setSelectList(_lastSelectList)
...
function checkedSelectCell( item_id )
	local isIn = SoulRebornData.getIsInSelectListByItemId(item_id)
	if(isIn == false)then
		-- 清除选择的材料
		SoulRebornData.cleanSelectList()
		--  添加材料
		SoulRebornData.addItemToSelectList(item_id)
	else
		--  添加材料
		SoulRebornData.addItemToSelectList(item_id)
	end
...
function createTopUI( )
	...
	-- 选择材料标题
	local titleSp = CCSprite:create("images/formation/changeequip/fightSoul_title.png")
...
function createSelectTableView( ... )
	...
	local h = LuaEventHandler:create(function(fn, table, a1, a2) 	--创建
		...
		elseif fn == "cellAtIndex" then
            a2 = FightSoulCell.createCell( _materialListData[a1 + 1], false, true, _selectList, false, _layer_priority-2)
...
--[[
	...
	@return :
--]]
function showSelectLayer( p_CallBack, p_layer_priority, p_zOrderNum )
