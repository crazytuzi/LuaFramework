-- Filename: ScoreShopIntroduce.lua
-- Author: DJN
-- Date: 2015-03-11
-- Purpose: 积分商店介绍

module("ScoreShopIntroduce", package.seeall)
...
function createBgUI()
    ...
    local bgSize = CCSizeMake(620,420)
    ...
    --正文
    local desString = ActivityConfigUtil.getDataByKey("scoreShop").data[1]["discribe"]
    desString = string.gsub(desString, "\\n", "\n")
    local descLabel = CCLabelTTF:create(desString,g_sFontName,24,CCSizeMake(bgSize.width*0.9,bgSize.height*0.6),kCCTextAlignmentLeft)
    descLabel:setColor(ccc3( 0x78, 0x25, 0x00));
    descLabel:setAnchorPoint(ccp(0.5,0.5))
    descLabel:setPosition(bgSize.width*0.5,bgSize.height*0.45)
