-- Filename：	BenefitActiveLayer.lua
-- Author：		Zhang zihang
-- Date：		2014-2-19
-- Purpose：		福利活动界面

module("BenefitActiveLayer", package.seeall)
...
local function getWealAllData()
	local wealActive= ActivityConfigUtil.getDataByKey("weal")
    return wealActive
end
...
local function openedActiveMes()
	...
	activeMes.openTime = openingData.openTime
	activeMes.endTime = openingData.endId
...
local function isActiveOpen()
	return ActivityConfigUtil.isActivityOpen("weal")
end
...
function getFormatYMDHM(typeNum)
	local oriTime
	if typeNum == 1 then
		oriTime = activeMes.openTime
	else
		oriTime = activeMes.endTime
	end
	local year = math.floor(oriTime/10000000000)
	oriTime = oriTime - year*10000000000
	local month = math.floor(oriTime/100000000)
	oriTime = oriTime - month*100000000
	local day = math.floor(oriTime/1000000)
	oriTime = oriTime - day*1000000
	local hour = math.floor(oriTime/10000)
	if hour < 10 then
		hour = "0" .. hour
	end
	oriTime = oriTime - hour*10000
	local min = math.floor(oriTime/100)
	if min < 10 then
		min = "0" .. min
	end
