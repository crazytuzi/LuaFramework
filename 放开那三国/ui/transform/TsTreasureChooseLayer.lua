-- FileName: TsTreasureChooseLayer.lua 
-- Author: licong 
-- Date: 16/3/4 
-- Purpose: 宝物选择界面


module("TsTreasureChooseLayer", package.seeall)
...
function initSelectData( )
	-- 得到材料列表
	local materialListData = TsTreasureData.getChooseItemData()
	-- 得到已选择的列表
	_selectList = TsTreasureData.getSelectList()
...
function checkedSelectCell( item_id )
	...
	--  添加新的
	TsTreasureData.addToSelectList(item_id)
end
---------------------------------------------------------------- 创建ui --------------------------------------------------------------------
...
function createTopUI( )
	...
	-- 选择材料标题
	local titleSp = CCSprite:create("images/treasure/treas_select.png")
...
function createSelectTableView( ... )
	...
	local _scrollview_height = _bgLayer:getContentSize().height - nHeightOfTitle
	...
	local h = LuaEventHandler:create(function(fn, table, a1, a2) 	--创建
		...
		elseif fn == "cellAtIndex" then
            a2 = TreasBagCell.createTreasCell( _materialListData[a1 + 1], false, nil, true,nil,_selectList)
		...
		elseif fn == "cellTouched" then
			...
			menuBtn_M:selected()
			-- 添加数据
			checkedSelectCell(tonumber(m_data.item_id))
			-- 删除自己
			if(_bgLayer ~= nil)then
				_bgLayer:removeFromParentAndCleanup(true)
				_bgLayer = nil
