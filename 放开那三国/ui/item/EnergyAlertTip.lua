...
-- Filename：	EnergyAlertTip.lua
-- Author：		licong
-- Date：		2013-10-13
-- Purpose：		体力提示框

module("EnergyAlertTip", package.seeall)
...
local energy_template_id 	= 10032
local energy_goods_id 		= 1
...
function buyCallback( cbFlag, dictData, bRet )
	if(dictData.err == "ok") then
		require "db/DB_Goods"
		local goodsData = DB_Goods.getDataById(energy_goods_id)
...
function useItemCallback( cbFlag, dictData, bRet )
	if (dictData.err == "ok") then
		AnimationTip.showTip(GetLocalizeStringBy("key_2817"))
		UserModel.changeEnergyValue(25)
...
function menuAction( tag, itemBtn )
	...
	elseif (tag == 10002) then
		local energyInfo = ItemUtil.getCacheItemInfoBy(energy_template_id)
		if((not table.isEmpty(energyInfo)) and tonumber(energyInfo.item_num) > 0) then
			local args = Network.argsHandler(energyInfo.gid, energyInfo.item_id, 1,1)
			RequestCenter.bag_useItem(useItemCallback, args)
		else
			AnimationTip.showTip(GetLocalizeStringBy("key_2866"))
		end
	end
...
function showTip( p_callBack)
	...
	-- 描述
	local tipText = GetLocalizeStringBy("key_2476")
	...
	-- ICON
	local iconSprite = ItemSprite.getItemSpriteByItemId(energy_template_id)
	...
	-- 体力+25
	local energyLabel = CCRenderLabel:create(GetLocalizeStringBy("key_1880"), g_sFontName, 20, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    ...
    -- 拥有个数
	_curNumber = 0
	local cacheInfo = ItemUtil.getCacheItemInfoBy(energy_template_id)
