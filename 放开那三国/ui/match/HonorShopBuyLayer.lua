-- FileName: HonorShopBuyLayer.lua 
-- Author: licong 
-- Date: 14-6-19 
-- Purpose: function description of module 


module("HonorShopBuyLayer", package.seeall)
...
function buyAction( tag, itemBtn )
	...
	if(tag == 10001) then
		-- 判断荣誉值是否够
		if(_totalPrice <= MatchData.getHonorNum()) then  --当表中的价格小于等于玩家的钱
			-- 下一步创建与数据有关UI
    		local function createNext( ... )
    			-- 减去声望
    			MatchData.subHonorNum(_totalPrice)
  				-- 刷新声望
  				HonorShop.refreshHonorNum()
  				-- 得到物品的个数
				local itemType, item_id, item_num = MatchData.getItemData( _goodsData.items )
				...
				--更新购买次数
				MatchData.addBuyNumberBy( _goodsData.id, _curNumber )
				-- 关闭当前界面
				closeAction()
				-- 刷新列表
				HonorShop.reloadDataFunc()  --刷新tableview
    		end
			MatchService.buy(_goodsData.id,_curNumber,createNext)
		else
			AnimationTip.showTip(GetLocalizeStringBy("lic_1085"))  --荣誉不足
		end
...
function changeNumberAction( tag, itemBtn )
	...
	_totalPrice = tonumber(_goodsData.costHonor) * tonumber(_curNumber)
	if(_totalPrice > numData)then
		local num = numData%tonumber(_goodsData.costHonor)
		local jifenNum = numData - num
		_curNumber = tonumber(jifenNum/tonumber(_goodsData.costHonor))
		if(_curNumber == 0)then
			_curNumber = 1
		end
	end
...
---- 准备数据
	...
	-- 限购次数
	   if(_goodsData.level_num == nil)then
		_maxLimitNum = tonumber(_goodsData.baseNum) - MatchData.getBuyNumBy(_goodsData.id)
	   else 
	   	require "script/ui/match/MatchData"
	   	local _number,_level = MatchData.getLevelnumber(_goodsData)
	   	_maxLimitNum = tonumber(_number - MatchData.getBuyNumBy(_goodsData.id))
	   end
	-- 一共拥有
...
---- 加减道具的按钮
	...
	-- 总价
	local totalTipLabel = CCRenderLabel:create(GetLocalizeStringBy("lic_1082"), g_sFontName, 36, 1, ccc3(0x49, 0x00, 0x00), type_stroke)
    totalTipLabel:setColor(ccc3(0xfe, 0xdb, 0x1c))
    totalTipLabel:setPosition(ccp(110, 72) )
    innerBgSp:addChild(totalTipLabel)
    local goldSp_2 = CCSprite:create("images/common/s_honor.png")
	goldSp_2:setAnchorPoint(ccp(0,0))
	goldSp_2:setPosition(ccp(280, 35))
	innerBgSp:addChild(goldSp_2)
	_totalPrice = tonumber(_goodsData.costHonor)
...
function showPurchaseLayer( goods_id )
	...
	-- 商品数据
	_goodsData = DB_Contest_shop.getDataById(goods_id)
	-- 兑换商品中物品的数据
	local item_type, item_id, item_num = MatchData.getItemData( _goodsData.items )
