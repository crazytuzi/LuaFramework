-- Filename：	TeamInviteCell.lua
-- Author：		zhz
-- Date：		2013-3-17
-- Purpose：		邀请其他玩家的cell

module ("TeamInviteCell", package.seeall)
...
function inviteMenuAction( tag,item)
	...
	local function callBackFunc( pRet )
		local uname = TeamGroupData.removeInviteByUid(uid)
		TeamInviteLayer.rfcTableView()

		--[[
			玩家1在进入邀请队员界面时，玩家2未勾选不接受其它军团成员副本组队邀请，
		    进入邀请队员界面后，玩家2才勾选，此时玩家1邀请玩家2则会返回“fail”，表
		    示发送邀请失败,发送成功返回“ok”
	    --]]
	    require "script/ui/tip/SingleTip"
		if pRet == "fail" then
			SingleTip.showSingleTip(GetLocalizeStringBy("zz_136",uname))
		else
			--pRet == "ok"
	...
	TeamGruopService.inviteGuildMem(callBackFunc,uid )
end
