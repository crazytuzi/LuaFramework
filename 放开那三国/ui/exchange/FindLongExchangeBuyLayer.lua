...
function buySuccessful()
	...
	print("FindLongExchangeCell.getCanExchangeNumSp()",FindLongExchangeCell.getCanExchangeNumSp())
	...
	if remainExchangeNum < 1 then
		...
		FindLongExchangeLayer.freshFindLongExchangeCellData()
	...
	local renderLabel = tolua.cast(FindLongExchangeLayer.getExchangeInfoLayer():getChildByTag(90),"CCRenderLabel")
	renderLabel:setString(tostring(curFindDragonNum))
...
-- showPurchaseLayer
function showPurchaseLayer(cellData)
	init()
