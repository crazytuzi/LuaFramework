-- Filename：	DesLayer.lua
-- Author：		LiuLiPeng
-- Date：		2014-4-18
-- Purpose：		城池大地图说明界面

module ("DesLayer", package.seeall)
...
function create()
    ...
    local dialog_info = {}
    dialog_info.title = GetLocalizeStringBy("llp_81")
    dialog_info.callbackClose = closeCallback
    dialog_info.size = CCSizeMake(630, 630)
...
-- 显示文字
function loadTips()
    local texts = string.split(DB_Help_tips.getDataById(3).tips, "|")
