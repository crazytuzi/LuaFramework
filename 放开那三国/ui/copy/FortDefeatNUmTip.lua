...
-- Filename：	FortDefeatNUmTip.lua
-- Author：		Cheng Liang
-- Date：		2013-11-11
-- Purpose：		据点攻打次数不够,重置

module("FortDefeatNUmTip", package.seeall)
...
function resetAtkNumCallback( cbFlag, dictData, bRet )
	if(dictData.err == "ok")then
		DataCache.addDefeatNumByCopyAndFort( _copyId, _baseid, _maxAtkTimes )
		if(_isGold==true)then
			UserModel.addGoldNumber(-_costGold)
			DataCache.addRestDefeatNumTimes( _copyId, _baseid, 1 )
		end
		-- 回调
...
function menuAction( tag, itemBtn )
	...
	if(tag == 10001) then
		...
		local number = ItemUtil.getCacheItemNumBy( 60014 )
		if(number>0)then
			-- 如果有强攻令
			_isGold = false
			local args = Network.argsHandler(_baseid, 2)
			...
			if(UserModel.getGoldNumber() < _costGold)then
				...
				_isGold = true
				local args = Network.argsHandler(_baseid, 1)
				RequestCenter.resetAtkNum(resetAtkNumCallback, args)
			end
		end
...
--
function showAlert( copyid, baseid, maxAtkTimes, confirmCBFunc)
	_cormfirmCBFunc = confirmCBFunc

	confirmTitle = GetLocalizeStringBy("key_1985")
	cancelTitle = GetLocalizeStringBy("key_1202")
	_baseid = baseid
	_maxAtkTimes = maxAtkTimes
	_copyId = copyid
	_costGold = DataCache.getResetDefeatNumGoldBy(_copyId, _baseid)
	if(alertLayer) then
		alertLayer:removeFromParentAndCleanup(true)
		alertLayer = nil
	end
    ...
    goldSprite:setPosition(ccp(115, 225))
    alertBg:addChild(goldSprite)

    -- 重置令
    local resetSprite = CCSprite:create("images/base/props/qianggongqi.png")
    resetSprite:setAnchorPoint(ccp(0.5,0.5))
    resetSprite:setPosition(ccp(300, 225))
    alertBg:addChild(resetSprite)

	-- 描述
	local number = ItemUtil.getCacheItemNumBy( 60014 )
	local descLabel = CCLabelTTF:create(GetLocalizeStringBy("key_2173") ..  _costGold .. GetLocalizeStringBy("key_1828") .. number .. GetLocalizeStringBy("key_1926"), g_sFontName, 25, CCSizeMake(460, 120), kCCTextAlignmentCenter, kCCVerticalTextAlignmentTop)
