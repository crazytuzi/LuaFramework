...
function createMenuItemByData( mineral_data )
	...
	if( tonumber(mineral_data.uid) >0 ) then
        ...
        if mineral_data.guild_name ~= nil then
            ...
            if maxCountGuildName == mineral_data.guild_name and maxCount >= 3 then
                local arrow = XMLSprite:create("images/base/effect/muniuliumajiantou/muniuliumajiantou")
                itemImage:addChild(arrow)
                arrow:setAnchorPoint(ccp(0, 0.5))
                arrow:setScale(0.7)
                arrow:setPosition(ccp(guild_name:getPositionX() + guild_name:getContentSize().width * 0.5 + 20, guild_name:getPositionY() - 6))
            end
        end
	else
...
function createElfMenuItemByData( p_mineralData )
    local itemImage = CCMenuItemImage:create("images/active/mineral/elves_n.png", "images/active/mineral/elves_h.png")
    local itemImageSize = itemImage:getContentSize()
    if tonumber(p_mineralData.uid) > 0 then
        -- 等级
        local level_and_name = {}
        level_and_name[1] = CCSprite:create("images/common/lv.png")
        level_and_name[2] = CCRenderLabel:create(tostring(p_mineralData.level) .. "  ", g_sFontName, 22, 1,ccc3(0x00, 0x00, 0x00), type_stroke)
        level_and_name[2]:setColor(ccc3(0xfe, 0xdb, 0x1c))
        level_and_name[3] = CCRenderLabel:create(p_mineralData.uname, g_sFontName, 22, 1,ccc3(0x00, 0x00, 0x00), type_stroke)
        local level_and_name_node = BaseUI.createHorizontalNode(level_and_name)
        itemImage:addChild(level_and_name_node, 10)
        level_and_name_node:setAnchorPoint(ccp(0.5, 0.5))
        level_and_name_node:setPosition(ccp(itemImageSize.width * 0.5, itemImageSize.height * 0.2 - 20))
        if p_mineralData.guild_name ~= nil then
            local guild_name = CCRenderLabel:create(string.format("[%s]", p_mineralData.guild_name), g_sFontName, 22, 1,ccc3(0x00, 0x00, 0x00), type_shadow)
            itemImage:addChild(guild_name, 10)
            guild_name:setColor(ccc3(0xff, 0xf6, 0x00))
            guild_name:setAnchorPoint(ccp(0.5, 0.5))
            guild_name:setPosition(ccp(itemImageSize.width * 0.5, itemImageSize.height * 0.2 - 40))
        end

        if tonumber(p_mineralData.uid) == UserModel.getUserUid() then
            addEffect(itemImage)
        end
    else
        local textLabel = CCRenderLabel:create(GetLocalizeStringBy("key_10363"), g_sFontName, 21, 1,ccc3(0x00, 0x00, 0x00), type_stroke)
        textLabel:setAnchorPoint(ccp(0.5, 0.5))
        textLabel:setPosition(ccp(itemImageSize.width * 0.5, itemImageSize.height * 0.2))
        itemImage:addChild(textLabel, 10)
    end
    local effect = XMLSprite:create("images/base/effect/baozangTX/baozangTX")
    itemImage:addChild(effect)
    effect:setAnchorPoint(ccp(0.5, 0.5))
    effect:setPosition(ccpsprite(0.5, 0.5, itemImage))
    return itemImage
end
