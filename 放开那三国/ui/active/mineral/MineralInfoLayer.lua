...
local function createUI()
    ...
    bgSprite:setPreferredSize(CCSizeMake(515, 760))
...
---- 中间的UI
	...
	local maxCountGuildName, maxCount = MineralLayer.getGuildAdditionInfo()
	local guildAddition = 0
	if _curMineralInfo.guild_name == maxCountGuildName and maxCount >= 3 then
		local normalConfigDb = DB_Normal_config.getDataById(1)
		local extraRes = parseField(normalConfigDb.extraRes)
		guildAddition = extraRes[maxCount - 2][2]
	end
	local text = nil
	if guildAddition > 0 then
		text = string.format(GetLocalizeStringBy("key_10349"), attr_arr[1], guildAddition)
	else
		text = tostring(attr_arr[1])
	end
	local productNumLabel = CCLabelTTF:create(text, g_sFontName, 23)
	...
	rewardSprite:setPreferredSize(CCSizeMake(460, 120))
	...
	rewardTitleSp:setPosition(ccp(0,120 - info_bar_height_1 * 0.5))
	...
	timeLabel:setPosition(ccp(90, 60))
	...
	local allMoneys = math.ceil(attr_arr[1] * (time_hour * 60 * 60 + total_guards_time * DB_Normal_config.getDataById(1).oneHelpArmyEnhance / 100) * 0.00005 * lv * (1 + addition))
	...
	-- rewardNumLabel:setPosition(ccp(cell_x + timeLabel:getContentSize().width, 25))
	-- rewardSprite:addChild(rewardNumLabel)
    info_bar_y = info_bar_y - rewardSprite:getContentSize().height - info_bar_pitch
    local silverRichInfo = {
    	labelDefaultSize = 23,
    	labelDefaultColor = ccc3(0x00, 0x6d, 0x2f),
    	elements = {
    		{
    			type = "CCNode",
    			create = function ( ... )
    				return CCSprite:create("images/common/coin_silver.png")
    			end
    		},
    		{text = allMoneys}
    	}
	}
	local silverLabel = GetLocalizeLabelSpriteBy_2(GetLocalizeStringBy("keybu0_100"), silverRichInfo)
	rewardSprite:addChild(silverLabel)
	silverLabel:setAnchorPoint(ccp(0, 0.5))
	silverLabel:setPosition(ccp(210, 60))

	local timeLabel = CCLabelTTF:create( GetLocalizeStringBy("key_3409") .. time_hour .. GetLocalizeStringBy("key_1769") , g_sFontName, 23)
	timeLabel:setColor(ccc3(0x78, 0x25, 0x00))
	timeLabel:setAnchorPoint(ccp(0, 0.5))
	timeLabel:setPosition(ccp(90, 25))
	rewardSprite:addChild(timeLabel)
	local ironNumbers = parseField(mineralDesc.ironNumber)
    local ironNumber = math.floor(ironNumbers[tonumber(_curMineralInfo.pit_id)] * time_hour * 60 * 60 * 0.00001)
    local ironRichInfo = {
    	labelDefaultSize = 23,
    	labelDefaultColor = ccc3(0x00, 0x6d, 0x2f),
    	elements = {
    		{
    			type = "CCNode",
    			create = function ( ... )
    				return CCSprite:create("images/base/props/jingtie28.png")
    			end
    		},
    		{text = ironNumber}
    	}
	}
    local ironLabel = GetLocalizeLabelSpriteBy_2(GetLocalizeStringBy("keybu0_100"), ironRichInfo)
	rewardSprite:addChild(ironLabel)
	ironLabel:setAnchorPoint(ccp(0, 0.5))
	ironLabel:setPosition(ccp(210, 25))
...
--]]
    ...
    if true then
        ...
        if  _curMineralInfo.domain_type == "3" then
            ...
            for i = 1, guard_limit do
                ...
                _helper_name_labels[i]:setPosition(ccp(170 * ((i- 1) % 2) + 40, 85 - math.floor((i - 1) / 2) * 30))
            end
            ...
            grab_item:setPosition(ccp(410, 73))
	...
	descSprite:setPosition(ccp(bgSprite:getContentSize().width * 0.5, 125))
...
---- 按钮
	...
	_forceOccupyBtn:setPosition(ccp(bgSprite:getContentSize().width*0.5 - 100, buttonY))
    ...
    if UserModel.getUserUid() ~= tonumber(_curMineralInfo.uid) then
        if tonumber(_curMineralInfo.uid) ~= 0 then
            if MineralUtil.isMyGuardMineral(_curMineralInfo) then
                ...
                giveUpBtn:setPosition(ccp(bgSprite:getContentSize().width*0.5, buttonY))
                occupyMenuBar:addChild(giveUpBtn)
                giveUpBtn:registerScriptTapHandler(callbackGiveUpGuard)
                _forceOccupyBtn:setVisible(false)
            else
                if _curMineralInfo. domain_type ~= "3" then
                    _occupyBtn:setPosition(ccp(bgSprite:getContentSize().width*0.5 - 100, buttonY))
                    ...
                    guard_item:setPosition(ccp(bgSprite:getContentSize().width*0.5 + 100, buttonY))
                    guard_item:registerScriptTapHandler(callbackGuardItem)
                else
                    _occupyBtn:setPosition(ccp(bgSprite:getContentSize().width * 0.5, buttonY))
                    _forceOccupyBtn:setPosition(ccp(bgSprite:getContentSize().width * 0.5, buttonY))
                end
            end
        else
                _occupyBtn:setPosition(ccp(bgSprite:getContentSize().width * 0.5, buttonY))
        ...
        if current_add_times < #add_time_array then
            ...
            add_time_btn:setPosition(ccp(168, buttonY))
            ...
            add_time_tip:setPosition(ccp(add_time_btn:getContentSize().width * 0.5, -10))
            add_time_tip:setColor(ccc3(0x00, 0xff, 0x18))
            
            give_up_btn_size = CCSizeMake(210, 73)
            give_up_btn_position = ccp(390, buttonY)
        end
        if give_up_btn_size == nil then
            give_up_btn_size = CCSizeMake(230, 73)
            give_up_btn_position = ccp(bgSprite:getContentSize().width * 0.5, buttonY)
        end
...
function callbackGrabGuard()
    ...
    require "script/ui/active/mineral/MineralGrabGuard"
