-- Filename：	MineralDescLayer.lua
-- Author：		bzx
-- Date：		2014-11-11
-- Purpose：		资源矿说明

module ("MineralDescLayer", package.seeall)
...
function create()
    ...
    local dialog_info = {}
    dialog_info.title = GetLocalizeStringBy("zzh_1281")
    dialog_info.callbackClose = closeCallback
    dialog_info.size = CCSizeMake(630, 580)
...
-- 显示文字
function loadTips()
    local texts = string.split(DB_Help_tips.getDataById(4).tips, "|")
    ...
    for i = 1, #texts do
        ...
        text_number_label:setPosition(5, text_label:getContentSize().height)
