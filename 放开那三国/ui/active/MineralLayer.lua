...
function showMineralRobInfoNode()
    ...
    local resDb = DB_Res.getDataById(tonumber(pushMineralRobInfo.domain_id))
    ...
    local tipNodes = {}
    tipNodes[1] = CCLabelTTF:create(pushMineralRobInfo.now_capture, g_sFontName, 21)
    tipNodes[1]:setColor(ccc3(0x00,0xe4,0xff))
    tipNodes[2] = CCLabelTTF:create(GetLocalizeStringBy("key_8313"), g_sFontName, 21)
    tipNodes[3] = CCLabelTTF:create(pushMineralRobInfo.pre_capture, g_sFontName, 21)
    ...
    tipNodes[7] = CCLabelTTF:create(resDb["res_name" .. pushMineralRobInfo.pit_id], g_sFontName, 21)
...
local function backAction(tag, itembtn)
    ...
    RequestCenter.mineral_leave(handleLeave, nil)
end
...
function callbackMyMineral(tag, menu_item)
    ...
    if mineral_info == nil then
        ...
        sendDomainRequestBy(tonumber(mineral_info.domain_id))
    end
end
...
function descriptionCallback( ... )
    require "script/ui/active/MineralDescLayer"
...
-- 请求某页前页的矿
function sendDomainRequestBy( domain_id )

	local args = Network.argsHandler(domain_id)
	RequestCenter.mineral_getPitsByDomain(pitsByDomainCallback, args)
end
...
function createLayer( domin_id)
	...
	createTopUI()
	RequestCenter.mineral_getSelfPitsInfo(selfPitsInfoCallback, nil)
	return _bgLayer
end
