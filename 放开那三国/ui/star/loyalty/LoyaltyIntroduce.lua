-- Filename: LoyaltyIntroduce.lua
-- Author: DJN
-- Date: 2015-7-11
-- Purpose: 聚义厅玩法介绍

module("LoyaltyIntroduce", package.seeall)
...
function createBgUI()
    ...
    local bgSize = CCSizeMake(620,670)
    ...
    --标题
    local titleLabel = CCLabelTTF:create(GetLocalizeStringBy("djn_194"), g_sFontPangWa, 30)
    ...
    local descLabel = CCLabelTTF:create(string.gsub(DB_Hall_friendship.getDataById(1).des1, "\\n", "\n"),g_sFontName,24,CCSizeMake(bgSize.width*0.9,bgSize.height*0.8),kCCTextAlignmentLeft)
    descLabel:setColor(ccc3( 0x78, 0x25, 0x00));
    descLabel:setAnchorPoint(ccp(0.5,0.5))
    descLabel:setPosition(bgSize.width*0.5,bgSize.height*0.5)
