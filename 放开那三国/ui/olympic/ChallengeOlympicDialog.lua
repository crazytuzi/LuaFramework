-- FileName: ChallengeOlympicDialog.lua 
-- Author: licong 
-- Date: 14-7-19 
-- Purpose: 挑战该位置上的参赛玩家 花费挑战确认 


module("ChallengeOlympicDialog", package.seeall)
...
-- 第一行
    -- 该位置已有玩家报名,花费
    local font1 = CCLabelTTF:create(GetLocalizeStringBy("lic_1168"), g_sFontName, 25)
    ...
    -- 后
    local font2 = CCLabelTTF:create(GetLocalizeStringBy("lic_1169"), g_sFontName, 25)
    ...
    font1:setPosition(ccp(posX, alertBgSize.height*0.65))
...
-- 第二行    
    -- 可挑战此玩家,挑战成功后替换该位置
    local font3 = CCLabelTTF:create(GetLocalizeStringBy("lic_1170"), g_sFontName, 25)
    font3:setColor(ccc3(0x78, 0x25, 0x00))
    font3:setAnchorPoint(ccp(0, 0.5))
    alertBg:addChild(font3)
    font3:setPosition(ccp(font1:getPositionX(),alertBgSize.height*0.55))

-- 第三行
    -- 的玩家,是否进行挑战?
    local font4 = CCLabelTTF:create(GetLocalizeStringBy("lic_1171"), g_sFontName, 25)
    font4:setColor(ccc3(0x78, 0x25, 0x00))
    font4:setAnchorPoint(ccp(0, 0.5))
    alertBg:addChild(font4)
    font4:setPosition(ccp(font1:getPositionX(),alertBgSize.height*0.45))
