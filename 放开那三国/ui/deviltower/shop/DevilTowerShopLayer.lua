-- FileName: DevilTowerShopLayer.lua 
-- Author: fuqiongqiong
-- Date: 2016-7-29
-- Purpose:试练塔商店layer

module("DevilTowerShopLayer",package.seeall)
require "script/ui/deviltower/shop/DevilTowerShopData"
require "script/ui/deviltower/shop/DevilTowerShopCell"
require "script/ui/deviltower/shop/DevilTowerShopController"
...
function createCenterLayer(p_centerLayerSizse, p_touchPriority, p_zOrder, p_isShow)
    ...
    DevilTowerShopController.getShopInfo(function ( ... )
        loadBg()
        print("zenmobuchuxiana")
        loadTableView()
     end)
    return _centerLayer
end
...
function loadBg( ... )
    if not _isShow then
        ...
        local underLayer = CCScale9Sprite:create("images/tower/button.png")
    ...
    --商店的人物图片
    local girlSprite = CCSprite:create("images/tower/boysprite.png")
    ...
    --商店名称
    local titleSprite = CCSprite:create("images/tower/shoptitle.png")
    ...
    --商店积分
    wmTitle = CCRenderLabel:create(GetLocalizeStringBy("fqq_128"), g_sFontPangWa, 24, 1, ccc3( 0x00, 0x00, 0x00), type_shadow)
    ...
    wmTitle:setPosition(ccp(g_winSize.width*0.6,titleSprite:getPositionY() - titleSprite:getContentSize().height*g_fElementScaleRatio))

    --商店图标
    local wmIcon = CCSprite:create("images/tower/jifenbi.png")
    -- wmIcon:setScale(0.4)
    wmTitle:addChild(wmIcon)
    wmIcon:setAnchorPoint(ccp(0,0))
    wmIcon:setPosition(ccp(wmTitle:getContentSize().width,0))

    _wmLabel = CCRenderLabel:create(DevilTowerShopData.getDevilTowerScore(), g_sFontPangWa, 24, 1, ccc3(0x00, 0x00, 0x00), type_shadow)
...
function loadTableView( ... )
    ...
    _centerLayer:addChild(viewBgSprite)
     ...
     _shopInfo = DevilTowerShopData.getItemList()
     print("商店的信息输出:")
     print_t(_shopInfo)
    local h = LuaEventHandler:create(function ( fn,p_table,a1,a2)
        ...
        elseif fn == "cellAtIndex" then
            a2 = DevilTowerShopCell.createCell(_shopInfo[a1 + 1], a1 + 1,_priority - 30)
            r = a2
...
function updateWmLable( ... )
    _wmLabel:setString(tostring(DevilTowerShopData.getDevilTowerScore()))
end