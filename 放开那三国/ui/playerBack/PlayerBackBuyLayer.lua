-- FileName: PlayerBackBuyLayer.lua 
-- Author: fuqiongqiong
-- Date: 2016-8-19
-- Purpose: 老玩家回归活动购买界面

module("PlayerBackBuyLayer",package.seeall)
			...
			if(_totalPrice <= UserModel.getGoldNumber())then  --当表中的价格小于等于玩家的钱
				...
				local callBack = function ( ... )
				    	 local callbackFunc = function ( ... )
				            ...
				            --修改缓存
				            PlayerBackData.setInfoOfGoods(_index,tonumber(_curNumber))
				            -- 弹提示
				        local item_type, item_id, item_num = PlayerBackData.getItemData(_goodsData.discountitem)
				        local daData = _goodsData.discountitem
			        ...
			        PlayerBackController.buy(_goodsData.id,tonumber(_curNumber),callbackFunc)
		        	end
				...
				AnimationTip.showTip(GetLocalizeStringBy("fqq_141"))  --积分不足
			end
        -- end
...
function changeNumberAction( tag, itemBtn )
	...
	if(_curNumber<=0)then
		...
		_maxLimitNum = tonumber(_goodsData.bugtimes) - PlayerBackData.getInfoOfGoods(_goodsData.id)
		print_t(_goodsData)
	...
	-- 总价 
	_totalPrice = tonumber(PlayerBackData.getMoneyCost(_goodsData.cost)[2]) * tonumber(_curNumber)
...
---- 加减道具的按钮
	...
	goldSp_2:setPosition(ccp(285, 35))
	_innerBgSp:addChild(goldSp_2)
	_totalPrice = tonumber(PlayerBackData.getMoneyCost(_goodsData.cost)[2])
...
function showPurchaseLayer( tag)
	...
	-- 商品数据
	_goodsData = PlayerBackData.getRewardInfo(tag)
	_index = tag
	-- 兑换商品中物品的数据
	local item_type, item_id, item_num = PlayerBackData.getItemData(_goodsData.discountitem)
	-- -- 物品类型 1是物品 2是英雄
