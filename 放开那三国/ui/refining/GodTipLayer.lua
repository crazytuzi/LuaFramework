-- Filename: GodTipLayer.lua
-- Author: zhang zihang
-- Date: 2015-4-28
-- Purpose: 该文件用于: 神兵炼化提示

module ("GodTipLayer", package.seeall)
...
function showLayer( itemInfo, callBackFn,touchProperty, zOrder )
    ...
    if table.count(itemInfo) > 3 then
        ...
        for i = 1,3 do
            ...
            local itemQuality = GodWeaponItemUtil.getGodWeaponQualityAndEvolveNum(nil,nil,itemInfo[i])
    ...
    content2:setColor(ccc3(0x78,0x25,0x00))
    local content3 = CCRenderLabel:create(GetLocalizeStringBy("zzh_1329"), g_sFontName ,24,1,ccc3(0x0,0x0,0x0), type_stroke)
