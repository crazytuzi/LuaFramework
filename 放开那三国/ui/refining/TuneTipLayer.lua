-- Filename: TuneTipLayer.lua
-- Author: DJN
-- Date: 2015-4-28
-- Purpose: 该文件用于: 炼化提示

module ("TuneTipLayer", package.seeall)
...
function showLayer( itemInfo, callBackFn,touchProperty, zOrder )
    ...
    local aleteNode = BaseUI.createHorizontalNode({content2, content3})
    aleteNode:setAnchorPoint(ccp(0.5, 0.5))

    local contentNew1 = CCLabelTTF:create(GetLocalizeStringBy("zzh_1330"), g_sFontName ,24)
    ...
    local contentNew = BaseUI.createHorizontalNode({contentNew1})
