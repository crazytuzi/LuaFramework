...
function setShopInfo( data )
	...
	setStonTime(_shopInfo.stone_refresh_num)
end
---得到商店信息数据
...
--获取神兵令
function getGodCoinNum( ... )
	-- print("numGod",tonumber(getShopInfo().coin))
	return tonumber(getShopInfo().coin)
end
--减去消耗的神兵令
function setGodCoinNum( pNum)
	_shopInfo.coin = getGodCoinNum() + pNum
...
function isTokenEnough(p_num )
	local token = GodShopData.getGodCoinNum()
...
function updateCost( p_id )
	...
	if(cost[1] == 1)then
		local allData = getGodCoinNum()
		-- allData = allData - cost[2]
		setGodCoinNum(-cost[2])
		-- local data = getShopInfo()
		-- GodWeaponCopyData.setCopyInfo(data)
...
--获取刷新石刷新次数
function setStonTime( pData )
	_stonRef = tonumber(pData)
end

function getStonTime( ... )
	return _stonRef
end
...
--获取刷新令
function getRfreshIdAndNum( ... )
	local info = DB_Overcomeshop.getDataById(1).godarm_item
