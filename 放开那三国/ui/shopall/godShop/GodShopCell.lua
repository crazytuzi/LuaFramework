...
--local _index = nil
function createCell(p_cellValue,p_index,p_tableView,pSize)
    -- print("cell",p_index)
    -- print_t(p_cellValue)
   ...
   -- _cost = GodShopData.getCostById(p_cellValue.id)
    ...
    if(p_cellValue.isSpecial)then
        --是特殊商品，背景图片与其他不同
        cellBgSprite = CCScale9Sprite:create("images/common/bg/bg_9s_9.png")
    ...
    if(_costType[p_index] == 1)then
        ...
        -- midSp:setScale(g_fElementScaleRatio)
    ...
    local icon,itemName,itemColor = ItemUtil.createGoodsIcon(rewardInDb,GodShopLayer.getTouchPriority()-50,
    ...
    ------名称
    local nameLabel = CCRenderLabel:create(itemName, g_sFontPangWa,23,1,ccc3( 0x00, 0x00, 0x00),type_stroke)
    ...
    -- cellMenu:setScale(g_fElementScaleRatio)
    ...
    exchangeMenuItem:setPosition(ccp(innerBgSprite:getPositionX()+innerBgSprite:getContentSize().width-5,innerBgSprite:getPositionY() + innerBgSprite:getContentSize().height/2))
    ...
    -- buyLabel:setScale(g_fElementScaleRatio)
    buyLabel:setScale(0.8)
    ...
    print_t(p_cellValue)
    print("_remainCount[p_cellValue.id]")
    print_t(_remainCount)
    
    if(_remainCount[p_index] <= 0)then
        ...
        if(_costType[p_index] == 1 or _costType[p_index] == 3)then
            ...
            hasReceiveItem:setPosition(ccp(innerBgSprite:getPositionX()+innerBgSprite:getContentSize().width-5,innerBgSprite:getPositionY() + innerBgSprite:getContentSize().height/2))
    ...
    -- numLabel:setScale(g_fElementScaleRatio)
    numLabel:setAnchorPoint(ccp(0.5,0))
    numLabel:setPosition(ccp(exchangeMenuItem:getPositionX()+exchangeMenuItem:getContentSize().width*0.5,24))
