...
local function guildShopBuyCb( cbFlag, dictData, bRet )
    ...
    -- 新增额外消耗扣除
    if (_good_data.extraCost) then
        local costInfo = _good_data.extraCost
        costInfo.num = -_good_data.extraCost.num
    ...
    if( _index ==2) then
        print("道具=====")
        GuildShopLayer.refreshMiddleUI_2()
    end
...
function buyAction( tag, item )
    ...
    if (_good_data.extraCost) then
        local costInfo = _good_data.extraCost
        if (ItemUtil.checkItemCountByType(costInfo.type,costInfo.num,true) == false) then
            return
        end
    end
    ...
    if(canBuy== true) then
        -- 购买商品
        local num = _good_data.personalLimit- buyNum.num
        if(_index==1 or num==1)then
            ...
            require "script/ui/guild/GuildShopBuyLayer"
            if(_index==1)then
                GuildShopBuyLayer.showPurchaseLayer(goodId,1)
            else
                GuildShopBuyLayer.showPurchaseLayer(goodId,3)
            end
        end
    else
...
function createCell( goods_data , index, callbackFn,pSize)
    ...
    print("goods_data")
    print_t(goods_data)
    local iconItem= ActiveUtil.getItemIcon(goods_data.type,goods_data.tid,-775)
    ...
    -- buyBtn:setScale(g_fElementScaleRatio)
    buyBtn:setAnchorPoint(ccp(0, 0.5))
    -- buyBtn:setTouchPriority(-1000)
    buyBtn:setPosition(ccp(textBg:getPositionX()+textBg:getContentSize().width,textBg:getPositionY()+textBg:getContentSize().height/2))
    ...
    if(tonumber(goods_data.tid) >= 1800000 and tonumber(goods_data.tid)<= 1900000 ) then
    ...
    local donateNumLabel = CCRenderLabel:create("" .. goods_data.costContribution ,g_sFontName, 23,1,ccc3(0x00,0x00,0x00), type_stroke)
    ...
    -- 新增额外消耗
    if (goods_data.extraCost) then
        -- 调整位置
        donateLabel:setPosition(donateLabel:getPositionX(),40)
        donateNumLabel:setPosition(donateNumLabel:getPositionX(),40)

        local extraCostTab = goods_data.extraCost
        -- print("extraCost")
        -- print_t(extraCostTab)

        -- 额外消耗
        local costName = string.gsub(extraCostTab.name,"^%s*(.-)%s*$","%1") --去掉多余空格
        local posX = 114
        -- icon
        local costIcon = ItemUtil.getSmallSprite(extraCostTab)
        if (costIcon) then
            costIcon:setPosition(126,8)
            costIcon:setAnchorPoint(ccp(0,0))
            textBg:addChild(costIcon)
            costName = ""
            posX = 114+47
        end

        local costLabel = CCRenderLabel:create(costName .. "：", g_sFontName,23,1, ccc3(0x00,0x00,0x00), type_stroke)
        costLabel:setColor(ccc3(0xff,0xf6,0x00))
        costLabel:setPosition(posX,12)
        costLabel:setAnchorPoint(ccp(0,0))
        textBg:addChild(costLabel)

        local costNumLabel = CCRenderLabel:create("" .. extraCostTab.num ,g_sFontName, 23,1,ccc3(0x00,0x00,0x00), type_stroke)
        costNumLabel:setColor(ccc3(0xff,0xff,0xff))
        -- 兼容东南亚英文版
        if(Platform.getLayout ~= nil and Platform.getLayout() == "enLayout" ) then
            costNumLabel:setPosition(200,12)
        else
            costNumLabel:setPosition(176,12)
        end
    ...
    -- leftBuyNumNode:setScale(g_fElementScaleRatio)
    ...
    -- levelNode:setScale(g_fElementScaleRatio)
    ...
    if(leftNumber<=0)then
        ...
        if(_index==2) then
            ...
            hasReceiveItem:setPosition(ccp(cellBackground:getContentSize().width-15,cellBackground:getContentSize().height/2))
    ...
    -- personalLevelNode:setScale(g_fElementScaleRatio)
