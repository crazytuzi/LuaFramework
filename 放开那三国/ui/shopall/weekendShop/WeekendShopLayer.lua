...
local kMidRectSize = nil
local kTableBgSize = nil
...
function init( ... )
	...
	kTableBgSize = CCSizeMake(472,kMidRectSize.height-212)
	_mainLayer = nil
	_btnTable = {}
	_topLabelTable = {}
	_timeLabelTable = {}
...
function createUINode( ... )
	...
	--周末商人
	local titleLabel = CCSprite:create("images/shop/shopall/zhoumoshangren.png")
...
function createCell( pCellIndex )
	...
	costLabel.parent:setPosition(ccp(113,19))
	...
	cellBtn:setPosition(ccp(325,58))
	...
	if cellData.remain <= 0 then
		...
		if cellData.config.costType == WeekendShopData.kSoulJewelTag then
		    ...
		    hasReceiveItem:setPosition(ccp(325,58))
...
function createCenterLayer(p_centerSize)
	...
	local getShopNumCb = function ( pNum )
		...
		if WeekendShopData.doShopOpen() then
			...
			layer = ShopClosedLayer.createCenterLayer(_centerSize)
		end
		layer:setPosition(ccp(0, -MenuLayer.getHeight()))
