...
--]]
function init( ... )
	kMidRectSize = CCSizeMake(640, kAdaptSize.height-MenuLayer.getLayerContentSize().height
	                                 -ShoponeLayer.getTopGoldContentSize().height-ShoponeLayer.getTopBgHeight()/g_fScaleX)
	kTableBgSize = CCSizeMake(640,kMidRectSize.height-315)
	_mainLayer = nil
	_timeDownLabel = nil
	_tableView = nil
end
...
function  updateTime( ... )
	...
	if leftTime ==0 and leftTime >= -1 then
		ShoponeLayer.changeButtomLayer(WeekendShopLayer.createCenterLayer())
		return
...
function createCenterLayer(p_centerSize)
	...
	_centerLayer:addChild(_mainLayer)
