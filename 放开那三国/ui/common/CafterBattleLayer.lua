-- FileName: CafterBattleLayer.lua 
-- Author: Li Cong 
-- Date: 13-10-8 
-- Purpose: 比武结算

require "script/utils/BaseUI"
module("CafterBattleLayer", package.seeall)
local mainLayer = nil
local menu = nil
...
-- honorData:获得荣誉数 没有可为nil
-- expData:获得将魂数 没有可为nil
-- flopData:掉落翻牌数据
-- CallFun: 自定义确定按钮后回调
-- isShowMenu: 是否显示主菜单   add by yangrui 15-10-12
function createAfterBattleLayer( appraisal, enemyUid, enemyName, enemyUtid, enemyFightData, honorData, expData, flopData, CallFun ,fightStr, isShowMenu)
    -- 初始化
    init()
    ...
    ---[[ mddify by yangrui 15-10-12
    if isShowMenu == false then
        okItem:registerScriptTapHandler(okItemFunNoMenu)
    else
    ...
    if(isWin) then
        ...
        -- 银币
        local honorSp = CCSprite:create("images/arena/honor.png")
        honorSp:setAnchorPoint(ccp(0,1))
        honorSp:setPosition(ccp(10,middle_bg:getContentSize().height-80))
        middle_bg:addChild(honorSp)
        local honorData = honorData or 0
        local yibi_data = CCRenderLabel:create("+".. honorData , g_sFontPangWa, 20, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
       ...
       -- 左中右真是掉落实物品
        ...
        local honorSp = CCSprite:create("images/arena/honor.png")
        honorSp:setAnchorPoint(ccp(0,1))
        honorSp:setPosition(ccp(10,middle_bg:getContentSize().height-65))
        middle_bg:addChild(honorSp)
        local honorData = honorData or 0
        local yibi_data = CCRenderLabel:create("+".. honorData , g_sFontPangWa, 20, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
        yibi_data:setColor(ccc3(0x70,0xff,0x18))
...
-- 确定回调   不显示菜单
function okItemFunNoMenu( tag, item_obj )
    -- 音效
    require "script/audio/AudioUtil"
    AudioUtil.playEffect("audio/effect/zhujiemian.mp3")
    print(GetLocalizeStringBy("key_2060") .. tag )
