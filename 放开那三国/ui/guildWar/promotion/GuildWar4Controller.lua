-- FileName: GuildWar4Controller.lua 
-- Author: bzx
-- Date: 15-1-19
-- Purpose:  跨服军团战4强到冠军的控制层

module("GuildWar4Controller", package.seeall)
...
function fightInfoCallback( ... )
    ...
    MyGuildWarInfoDialog.show(GuildWar4Layer.getTouchPriority() - 550, GuildWar4Layer.getZOrder() + 10)
end
...
function cheerCallback(tag, btn)
    -- require "script/utils/ModuleUtil"
    ModuleUtil.cleanupModuleByName("GuildWarSupportDialog", "script/ui/guildWar/support/GuildWarSupportDialog")
    
    require "script/ui/guildWar/support/GuildWarSupportDialog"
    GuildWarSupportDialog.show()
...
function reviewWarCallback( ... )
    GuildWar16Layer.show(nil, nil, "GuildWar4Layer")
end