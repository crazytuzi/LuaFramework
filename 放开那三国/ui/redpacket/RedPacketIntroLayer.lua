-- Filename: RedPacketIntroLayer.lua
-- Author: llp
-- Date: 2015-12-25
-- Purpose: 红包说明

module("RedPacketIntroLayer", package.seeall)
...
function createBgUI()
    ...
    local bgSize = CCSizeMake(620,600)
    ...
    --正文
    local descLabel = CCLabelTTF:create(string.gsub(ActivityConfig.ConfigCache.envelope.data[1]["ActivityDes"], "\\n", "\n"),g_sFontName,24,CCSizeMake(bgSize.width*0.9,bgSize.height*0.8),kCCTextAlignmentLeft)
...
function showLayer(p_touchPriority,p_ZOrder)
    ...
    curScene:addChild(_bgLayer,_ZOrder,343) 
    --创建背景UI
    createBgUI()
