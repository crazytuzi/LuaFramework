-- Filename: PlayerCardLayerLee.lua
-- Author: k
-- Date: 2013-05-27
-- Purpose: 战斗场景
...
--require "amf3"
-- 主城场景模块声明
module("PlayerCardLayerLee", package.seeall)
...
local m_layerSize   = CCSizeMake(640,600)            -- 层尺寸
...
local function initPlayerCards()
    ...
    for i=0,5 do
        ...
        if(m_formation["" .. i]~=nil and (m_formation["" .. i])~=0) then
            ...
            --print("------------------ BattleCardUtilLee.getBattlePlayerCard:",m_formation["" .. i])
            local card = BattleLayerLee.createBattleCard(m_formation["" .. i])
            card:setTag(1000+i)
    ...
    for i=0,5 do
        ...
        if(cardBg:getTag()-1000==m_onMovingCard:getTag()) then
            ...
            if(location.x>=0 and location.y>=0 and location.x<=cardBg:getContentSize().width and location.y<=cardBg:getContentSize().height) then
                ...
                BattleLayerLee.reviveCardByHid(hid1)
            end
        else
            local location = cardBg:convertToNodeSpace(ccp(x, y))
            if(location.x>=0 and location.y>=0 and location.x<=cardBg:getContentSize().width and location.y<=cardBg:getContentSize().height) then
