 ...
 if (role.isactor and (not G_RIDING_INFO.id[1] )) or (is_up and (not ride_id))  then
  ...
  if self == role then
    ...
    if G_MAINSCENE and G_MAINSCENE.map_layer and G_MAINSCENE.map_layer:hasPath() then
      G_MY_STEP_SOUND =  AudioEnginer.randStepMus(is_up)
    end
  end
  if is_up then 
    role.up_ride = true
  ...
  if self == role then
    ...
    if eClothing then
      ...
      --if isHighride then w_resId = 6010501 end
      if w_resId > 0 then
        local r_path = "role/" .. (w_resId)
...
function RoleSprite:setSkills(params)
    ...
    for k,v in pairs(params) do
      ...
      elseif jnfenlie == 7 then
        local learnLv = getConfigItemByKey("SkillCfg","skillID",v[1],"learnLv")
        table.insert(wingTemp,{v,learnLv})
      elseif jnfenlie == 9 then

      end
    end
    table.sort(wingTemp, function(a,b) return a[2] < b[2] end) --技能id跟学习等级不成正比
    for k,v in pairs(wingTemp) do
      table.insert(self.wingskills,v[1])
    end
end
...
function RoleSprite:setFactionName_ex(role, factionName)
  ...
  local addIcon = function(parent, tab)
    ...
    for i,v in ipairs(tab) do
      ...
      if parent then
        ...
        local iconNode = Mprop.new({protoId = v.matId, effect = true})
...
function RoleSprite:updateHodeInfo(isHide, index)
  local node = G_MAINSCENE:getChildByName("shaWarHoldBtnNode")
  if not isHide then
      ...
      local holdNode = cc.Node:create()
      holdNode:setName("shaWarHoldBtnNode")
      G_MAINSCENE:addChild(holdNode, 100)

      local exitHold = function()
      		g_msgHandlerInst:sendNetDataByTableExEx(SHAWAR_CS_DEALHOLD, "DealHoldProtocol", {holeIndex = self.shaWarHoldIndex, dealType = 3})
      end
      local item = createMenuItem(holdNode, "res/empire/shaWar/cheli.png", cc.p(g_scrSize.width - 60, 520), exitHold)
      --createLabel(item, game.getStrByKey("shaWar_exitHold"), getCenterPos(item), nil, 18, true):setColor(MColor.lable_yellow)
  else
      if node and self.shaWarHoldIndex == index then
          removeFromParent(node)
...
function RoleSprite:showShaName(role, factionId)
  if role then
    ...
    if facNameLabel and strName ~= "" then
      ...
      if tempDef ~= 0 and factionId == tempDef then
