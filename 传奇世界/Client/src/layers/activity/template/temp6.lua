...
function weekList:ctor(chooseTemp)
    ...
    local callback = function(idx)
        ...
        DATA_Activity:readData(self.createLayout,self.title_select_idx+7)
    end
    local dayTab = {}
    local day = 7
    for i = 1 , day do
        i = i + 7
        if i < 11 then
            table.insert(dayTab,game.getStrByKey("the")..game.getStrByKey("num_"..i)..game.getStrByKey("day"))
        else
            table.insert(dayTab,game.getStrByKey("the")..game.getStrByKey("num_10")..game.getStrByKey("num_"..(i-10))..game.getStrByKey("day"))
        end
    end
