local FBTowerResult = class("FBTowerResult", function() return cc.Layer:create() end)
local comPath = "res/fb/tower/"
local commConst = require("src/config/CommDef");

function FBTowerResult:ctor(endData)
...
function FBTowerResult:addTitle()
...
function FBTowerResult:addWinFlg()
	if not self.isWin then
		return 0
	end
...
function FBTowerResult:addAward()
	...
	local flg = false
	local num = 0
	local specPropCfg = {[444444] = true, [222222] = true, [888888] = true, [999999] = true, [999998] = true}
	for m,n in pairs(self.netData.awardData) do
		if flg then
			str = str .. "+"
		end
		str = str .. MPropOp.name(tonumber(n[1])) .. "x" .. n[2]
		if not specPropCfg[tonumber(n[1])] then
			num = num + 1
		end
		flg = true
	end
	...
	if bagPack and num > 0 then
		--背包已满
		if true or bagPack:numOfGirdRemain() < num then
			str = str .. "(" .. game.getStrByKey("bag_space_lack_tips") .. ")"
		end
	end		
...
function FBTowerResult:addStar()
...
function FBTowerResult:addFlop()
...
function FBTowerResult:addFailTipsBtn()
	if not self.isWin then
		...
		for i=1,3 do
			local spr = createMenuItem(self.overView, "res/fb/tower/btn"..i..".png", cc.p(480 - (2-i)* 170, 330), function()
				g_EventHandler["FbTowerFailCallBack"] = function() func(i) end 
...
function FBTowerResult:winOutBtn()
	if not self.isWin then return 0 end
	...
	local func = function()
		if isGotoNext then
			if needLev > ILevel then
				if not isTips then
					isTips = true
					TIPS(getConfigItemByKeys("clientmsg", {"sth", "mid"}, {13000, -11}))
					performWithDelay(self.overView, function() self:closeResultLayer() end, 1)
				end
				return
			end
			userInfo.lastFb = newFbID
			g_msgHandlerInst:sendNetDataByTableExEx(COPY_CS_ENTERCOPY, "EnterCopyProtocol",{copyId = newFbID, friendId = 0, isInCopy = 1})
		else
			self:closeResultLayer()
		end
	end
	if isGotoNext then
		local item = createMenuItem(self, "res/component/button/50.png", cc.p(g_scrSize.width - 67, g_scrSize.height - 170 ), func)
		nextCopyLab = createLabel(item, game.getStrByKey("carry_tip_next_floor1") .. "[" .. autoGetTimeLeft.. "]", getCenterPos(item), nil, 22, true):setColor(MColor.lable_yellow)
	end

	if not isGotoNext then
		MessageBox(game.getStrByKey("fb_TowerLastLayer"), game.getStrByKey("exit"), function() self:closeResultLayer() end)
	end
	...
	if isGotoNext then
		timer = startTimerActionEx(self.overView, 1, true, timeLabUpdate)
	end

	return 0
end

function FBTowerResult:failOutBtn()
	if self.isWin then return 0 end
...
function FBTowerResult:flopCallBack()
	self.isOpenFlop = true
