...
local tAction = {
	...
	dress = function(params, self)
		...
		if girdId then
			MPackManager:dress(girdId, dressId)
		end
	...
	use = function(params)
		...
		if girdId then
            ...
            elseif protoId == 6200024 then  --神仙醉
                g_msgHandlerInst:sendNetDataByTableExEx(3801, "DrinkWineProtocol", {slotIndex=girdId})
...
[LUA-print] - }
			...
			if (quality > 2 and MpropOp.category(protoId)~=21 )or bHasRandom then
				-- 标题
				local n_attr_title = Mnode.createLabel(
				{
					src = "洗炼属性" .. "：",
