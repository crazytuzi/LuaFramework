local wingSkill = class("wingSkill",function() return cc.Layer:create() end)

function wingSkill:ctor(parent,skill_id,level,open,skillExp)
	self.parent = parent
	...
	self:registerScriptHandler(function(event)
		if event == "enter" then
			g_EventHandler["wingskillexpupdate"] = function(theSkillID,skillLev,skillExp,haveUpdate,skillExpAdd)
		...
		elseif event == "exit" then
			g_EventHandler["wingskillexpupdate"] = nil
	...
	if canUpdate == 0 and open ~= 2 and skill_info.maxlv > self.level and sldTemp ~= -1 then
		...
		G_TUTO_NODE:setTouchNode(self.updateButton, TOUCH_SKILL_UPDATE)
	...
	if open ~= 2 then
		...
		if canUpdate == 0 then
			...
			createLabel(self.textBg,game.getStrByKey("skillUpdateDesc3"),cc.p(370,150),cc.p(0.5,0.5),19,nil,nil,nil,MColor.red)
		end
	else
		...
		local lv = getConfigItemByKey("SkillCfg","skillID",self.skill_id,"learnLv")
		createLabel(self.textBg,string.format(game.getStrByKey("skill_wingNeed"),string.format(game.getStrByKey("num_"..tostring(lv)))),cc.p(370,200),nil,20,true,nil,nil,MColor.red)
	end
...
function wingSkill:youUpdate(skill_id,way)
...
function wingSkill:showProp(skillId,level,skillExp,skillExpSum,skill_info,skillLevelInfo,refreshWay)
	...
	-- if refreshWay == 2 then
		...
		if skillExpSum ~= -1 and skill_info.maxlv > level then
			if skillLevelInfo.needbook_Num and skillLevelInfo.needbook_ID and skillExp == skillExpSum then
				...
				self.tipText = require("src/RichText").new(self.lab, cc.p(470,190), cc.size(500, 50), cc.p(0.5, 0.5), 22, 20, MColor.white)
				...
				self.lab = cc.Node:create()
				self.textBg:addChild(self.lab)
				self.tipText = require("src/RichText").new(self.lab, cc.p(460,190), cc.size(500, 50), cc.p(0.5, 0.5), 22, 20, MColor.white)
				self.tipText:addText(game.getStrByKey("skill_tipText"),MColor.deep_brown,true)
				...
				-- else
				-- 	self.payText = require("src/RichText").new(self.title1, cc.p(130,-15), cc.size(465, 50),cc.p(0.5,0.5), 22, 19, MColor.black)
...
function wingSkill:proficiency(skillID,skillPro,level,refreshWay)
...
function wingSkill:SkillUpdateExpFun(theSkillID,skillLev,skillExp,refreshWay)  --获取技能每10点熟练度消息
...
function wingSkill:SkillUpdateTip(skillId ,skillLev )
...
function wingSkill:updateWingInfo(skillId, skillLevel)
	...
	if G_WING_INFO and G_WING_INFO.skillTab then
		for i,v in ipairs(G_WING_INFO.skillTab) do
			if skillTab[v.skillId] and skillTab[v.skillId] == skillId then
				v.skillLevel = skillLevel
			end
		end
	end
	--dump(skillTab)
...
function wingSkill:networkHander(buff,msgid)
	local switch = {
		[SKILL_SC_UPGRADESKILL] = function()
			...
			self.level = skills[2]
			if getConfigItemByKey("SkillCfg","skillID",skills[1],"jnfenlie") == 7 then
				self:SkillUpdateExpFun(skills[1],skills[2],skills[4],2)
				self:SkillUpdateTip(skills[1],skills[2])
				if G_ROLE_MAIN and G_ROLE_MAIN.wingskills then --由客户端修改本地强化度的值
   ...
   -- 			for i = 1,#G_SKILL_REDCHECK+1 do
			-- 	if G_SKILL_REDCHECK[i] == skills[1] then
			-- 		table.remove(G_SKILL_REDCHECK,i)
			-- 	end
			-- end
			-- local isRed = false
			-- checkWingSkillRed()
   ...
   --  		end
    		...
    		self:updateWingInfo(t.skillId, t.level)
		end,
	}
